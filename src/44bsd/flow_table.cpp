...
void CSttFlowTableStats::Clear() { memset(&m_sts, 0, sizeof(m_sts)); }
...
bool CFlowTable::Create(uint32_t size, bool client_side) {
    m_verbose = false;
...
void CFlowTable::Delete() { m_ft.Delete(); }
...
void CFlowTable::reset_stats() { m_sts.Clear(); }

void CFlowTable::redirect_to_rx_core(CTcpPerThreadCtx *ctx, struct rte_mbuf *mbuf) {
...
void CFlowTable::parse_packet(struct rte_mbuf *mbuf, CSimplePacketParser &parser, CFlowKeyTuple &tuple,
                              CFlowKeyFullTuple &ftuple, bool rx_l4_check, tcp_rx_pkt_action_t &action,
                              tvpid_t port_id) {
    action = tDROP;
    ...
    if ((parser.m_protocol != IPHeader::Protocol::TCP) && (parser.m_protocol != IPHeader::Protocol::UDP)) {
...
void CFlowTable::terminate_flow(CTcpPerThreadCtx *ctx, CFlowBase *flow, bool remove_from_ft) {
...
void CFlowTable::terminate_all_flows() { m_ft.detach_all(this, on_flow_free_cb); }

HOT_FUNC void CFlowTable::handle_close(CTcpPerThreadCtx *ctx, CFlowBase *flow, bool remove_from_ft) {
...
void HOT_FUNC CFlowTable::process_tcp_packet(CTcpPerThreadCtx *ctx, CTcpFlow *flow, struct rte_mbuf *mbuf,
                                             TCPHeader *lpTcp, CFlowKeyFullTuple &ftuple) {

    tcp_flow_input(flow->m_pctx, &flow->m_tcp, mbuf, lpTcp, ftuple.m_l7_offset, ftuple.m_l7_total_len);
...
void CFlowTable::generate_rst_pkt(CPerProfileCtx *pctx, uint32_t src, uint32_t dst, uint16_t src_port,
                                  uint16_t dst_port, uint16_t vlan, bool is_ipv6, TCPHeader *lpTcp, uint8_t *pkt,
                                  IPv6Header *ipv6, CFlowKeyFullTuple &ftuple) {
    ...
    */
    ...
    CTcpFlow *flow = alloc_flow(pctx, src, dst, src_port, dst_port, vlan, is_ipv6);
    if (flow == 0) {
        return;
    }
    ...
    if (lpTcp->getAckFlag()) {
        ...
        tcp_respond(flow->m_pctx, &flow->m_tcp, 0, lpTcp->getAckNumber(), TH_RST);
        ...
        tcp_respond(flow->m_pctx, &flow->m_tcp, lpTcp->getSeqNumber() + tlen, seq, TH_RST | TH_ACK);
    }

    free_flow(flow);
}

CUdpFlow *CFlowTable::alloc_flow_udp(CPerProfileCtx *pctx, uint32_t src, uint32_t dst, uint16_t src_port,
                                     uint16_t dst_port, uint16_t vlan, bool is_ipv6, bool client, uint16_t tg_id,
...
CTcpFlow *CFlowTable::alloc_flow(CPerProfileCtx *pctx, uint32_t src, uint32_t dst, uint16_t src_port, uint16_t dst_port,
                                 uint16_t vlan, bool is_ipv6, uint16_t tg_id, uint16_t template_id) {
...
bool CFlowTable::insert_new_flow(CFlowBase *flow, CFlowKeyTuple &tuple) {
...
void CFlowTable::rx_non_process_packet(tcp_rx_pkt_action_t action, CTcpPerThreadCtx *ctx, struct rte_mbuf *mbuf) {
    if (action == tDROP) {
...
void CFlowTable::process_udp_packet(CTcpPerThreadCtx *ctx, CUdpFlow *flow, struct rte_mbuf *mbuf, UDPHeader *lpUDP,
                                    CFlowKeyFullTuple &ftuple) {

    flow->on_rx_packet(mbuf, lpUDP, ftuple.m_l7_offset, ftuple.m_l7_total_len);
...
bool CFlowTable::rx_handle_packet_udp_no_flow(CTcpPerThreadCtx *ctx, struct rte_mbuf *mbuf, flow_hash_ent_t *lpflow,
                                              CSimplePacketParser &parser, CFlowKeyTuple &tuple,
                                              CFlowKeyFullTuple &ftuple, uint32_t hash) {
    CUdpFlow *flow;
    ...
    flow = ctx->m_ft.alloc_flow_udp(pctx, dest_ip, tuple.get_ip(), dst_port, tuple.get_port(), vlan, is_ipv6, false,
                                    tg_id, c_template_idx);
...
bool CFlowTable::rx_handle_packet_tcp_no_flow(CTcpPerThreadCtx *ctx, struct rte_mbuf *mbuf, flow_hash_ent_t *lpflow,
                                              CSimplePacketParser &parser, CFlowKeyTuple &tuple,
                                              CFlowKeyFullTuple &ftuple, uint32_t hash) {
    CTcpFlow *lptflow;
    ...
    if (m_client_side) {
        if (ctx->tcp_blackhole == 0) {
            ...
            generate_rst_pkt(FALLBACK_PROFILE_CTX(ctx), dest_ip, source_ip, dst_port, lpTcp->getSourcePort(), vlan,
                             is_ipv6, lpTcp, pkt, parser.m_ipv6, ftuple);
        }
    ...
    if ((lpTcp->getFlags() & TCPHeader::Flag::SYN) == 0) {
        ...
        if (ctx->tcp_blackhole != 2) {
            generate_rst_pkt(FALLBACK_PROFILE_CTX(ctx), dest_ip, tuple.get_ip(), dst_port, tuple.get_port(), vlan,
                             is_ipv6, lpTcp, pkt, parser.m_ipv6, ftuple);
        }
        rte_pktmbuf_free(mbuf);
    ...
    if (!server_info) {
        if (ctx->tcp_blackhole == 0) {
            generate_rst_pkt(pctx, dest_ip, tuple.get_ip(), dst_port, tuple.get_port(), vlan, is_ipv6, lpTcp, pkt,
                             parser.m_ipv6, ftuple);
        }
    ...
    lptflow = ctx->m_ft.alloc_flow(pctx, dest_ip, tuple.get_ip(), dst_port, tuple.get_port(), vlan, is_ipv6, tg_id,
                                   c_template_idx);
...
HOT_FUNC bool CFlowTable::rx_handle_packet(CTcpPerThreadCtx *ctx, struct rte_mbuf *mbuf, bool is_idle,
                                           tvpid_t port_id) {
    ...
    parse_packet(mbuf, parser, tuple, ftuple, ctx->get_rx_checksum_check(), action, port_id);
