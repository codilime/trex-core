...
CAstfFifRampup::CAstfFifRampup(CPerProfileCtx *pctx, uint16_t rampup_sec, double cps) {
    /* total ticks */
    m_pctx = pctx;
    m_ctx = pctx->m_ctx;
...
void CAstfFifRampup::on_timer_update(CAstfTimerFunctorObj *tmr) {
    ...
    m_ctx->m_timer_w.timer_start(tmr, tw_time_msec_to_ticks(CAstfFifRampup::TICK_MSEC));
    m_cur_tick++;
}
