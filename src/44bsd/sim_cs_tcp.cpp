...
bool CTcpSimShaper::Create(uint32_t queue_size, double rate_bps, CClientServerTcp *p, int dir) {
    m_state_on = false;
...
void CTcpSimShaper::next_packet() { trigger_next(); }

void CTcpCtxPcapWrt::write_pcap_mbuf(rte_mbuf_t *m, double time) {
...
int CTcpCtxDebug::on_flow_close(CTcpPerThreadCtx *ctx, CFlowBase *flow) {
    /* bothing to do */

    return (0);
}

int CTcpCtxDebug::on_redirect_rx(CTcpPerThreadCtx *ctx, rte_mbuf_t *m) {
    assert(0);
    return (0);
}

int CTcpCtxDebug::on_tx(CTcpPerThreadCtx *ctx, struct tcpcb *tp, rte_mbuf_t *m) {
    int dir = 1;
    assert(tp->m_flow);
...
void CClientServerTcp::set_rtt(uint32_t rtt_usec) { m_rtt_sec = (double)rtt_usec / (1000000.0); }
...
void CClientServerTcp::set_debug_mode(bool enable) { m_debug = enable; }

static bool compare_two_pcap_files(std::string c1, std::string c2, double dtime) {
    CErfCmp cmp;
    cmp.dump = 1;
    cmp.d_sec = dtime;
...
bool CClientServerTcp::Create(std::string out_dir, std::string pcap_file) {
...
void CClientServerTcp::on_tx(int dir, rte_mbuf_t *m) { on_tx_shaper(dir, m); /* start shaping */ }

void CClientServerTcp::on_tx_transmit(int dir, rte_mbuf_t *m) {
...
bool CTcpSimEventTxPkt::on_event(CSimEventDriven *sched, bool &reschedule) {
...
bool CTcpSimEventRx::on_event(CSimEventDriven *sched, bool &reschedule) {
    reschedule = false;
    m_p->on_rx(m_dir, m_pkt);
    return (false);
}

bool CTcpSimEventTimers::on_event(CSimEventDriven *sched, bool &reschedule) {
    reschedule = true;
    m_time += m_d_time;
...
void CClientServerTcp::on_rx(int dir, rte_mbuf_t *m) {
    ...
    if (dir == 1) {
        ctx = &m_s_ctx, m_s_pcap.write_pcap_mbuf(m, t);
    } else {
        ctx = &m_c_ctx;
...
static char http_req[] = {
    };

static char http_res[] = {
    0x48, 0x54, 0x54, 0x50, 0x2f, 0x31, 0x2e, 0x31, 0x20, 0x32, 0x30, 0x30, 0x20, 0x4f, 0x4b, 0x0d, 0x0a, 0x53, 0x65,
    0x72, 0x76, 0x65, 0x72, 0x3a, 0x20, 0x4d, 0x69, 0x63, 0x72, 0x6f, 0x73, 0x6f, 0x66, 0x74, 0x2d, 0x49, 0x49, 0x53,
    0x2f, 0x36, 0x2e, 0x30, 0x0d, 0x0a, 0x43, 0x6f, 0x6e, 0x74, 0x65, 0x6e, 0x74, 0x2d, 0x54, 0x79, 0x70, 0x65, 0x3a,
    0x20, 0x74, 0x65, 0x78, 0x74, 0x2f, 0x68, 0x74, 0x6d, 0x6c, 0x0d, 0x0a, 0x43, 0x6f, 0x6e, 0x74, 0x65, 0x6e, 0x74,
    0x2d, 0x4c, 0x65, 0x6e, 0x67, 0x74, 0x68, 0x3a, 0x20, 0x33, 0x32, 0x30, 0x30, 0x30, 0x0d, 0x0a, 0x0d, 0x0a, 0x3c,
    0x68, 0x74, 0x6d, 0x6c, 0x3e, 0x3c, 0x70, 0x72, 0x65, 0x3e, 0x2a, 0x2a, 0x2a, 0x2a, 0x2a, 0x2a, 0x2a};

static char http_res_post[] = {0x3c, 0x2f, 0x70, 0x72, 0x65, 0x3e, 0x3c, 0x2f, 0x68, 0x74, 0x6d, 0x6c, 0x3e};
...
int CClientServerTcp::build_http(CMbufBuffer *buf_req, CMbufBuffer *buf_res, CEmulAppProgram *prog_c,
...
int CClientServerTcp::build_http_rst(CMbufBuffer *buf_req, CMbufBuffer *buf_res, CEmulAppProgram *prog_c,
                                     CEmulAppProgram *prog_s, uint32_t http_r_size) {
    CEmulAppCmd cmd;
...
int CClientServerTcp::build_http_fin_ack(CMbufBuffer *buf_req, CMbufBuffer *buf_res, CEmulAppProgram *prog_c,
                                         CEmulAppProgram *prog_s, uint32_t http_r_size) {
    CEmulAppCmd cmd;
...
int CClientServerTcp::build_http_connect(CMbufBuffer *buf_req, CMbufBuffer *buf_res, CEmulAppProgram *prog_c,
                                         CEmulAppProgram *prog_s, uint32_t http_r_size) {
    CEmulAppCmd cmd;
...
int CClientServerTcp::build_http_connect_rst(CMbufBuffer *buf_req, CMbufBuffer *buf_res, CEmulAppProgram *prog_c,
                                             CEmulAppProgram *prog_s, uint32_t http_r_size) {
    CEmulAppCmd cmd;
...
int CClientServerTcp::build_http_connect_rst2(CMbufBuffer *buf_req, CMbufBuffer *buf_res, CEmulAppProgram *prog_c,
                                              CEmulAppProgram *prog_s, uint32_t http_r_size) {
    CEmulAppCmd cmd;
...
#define RX_BYTES 32768
    ...
    printf(" [%d %d] [%d %d] [%d %d] \n", (int)m_c_ctx.get_tcpstat()->m_sts.tcps_sndbyte,
           (int)m_c_ctx.get_tcpstat()->m_sts.tcps_rcvbyte, (int)m_s_ctx.get_tcpstat()->m_sts.tcps_rcvbyte,
           (int)m_s_ctx.get_tcpstat()->m_sts.tcps_sndbyte, (int)TX_BYTES, (int)RX_BYTES);

    if (m_check_counters) {
        if (m_s_ctx.get_tcpstat()->m_sts.tcps_sndbyte > 0 && m_s_ctx.get_tcpstat()->m_sts.tcps_rcvbyte > 0) {
            if ((m_c_ctx.get_tcpstat()->m_sts.tcps_drops == 0) && (m_s_ctx.get_tcpstat()->m_sts.tcps_drops == 0)) {
...
int CClientServerTcp::fill_from_file() {
    ...
    c_flow = m_c_ctx.m_ft.alloc_flow(DEFAULT_PROFILE_CTX(&m_c_ctx), 0x10000001, 0x30000001, src_port, dst_port, m_vlan,
                                     false, tg_id);
