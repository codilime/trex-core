...
uint32_t sbspace(struct sockbuf *sb) { return (sb->sb_hiwat - sb->sb_cc); }

void sbflush(struct sockbuf *sb) { sb->sb_cc = 0; }
...
void soisconnecting(struct tcp_socket *so) { so->m_app->on_bh_event(te_SOISCONNECTING); }

void soisconnected(struct tcp_socket *so) { so->m_app->on_bh_event(te_SOISCONNECTED); }

void socantrcvmore(struct tcp_socket *so) { so->m_app->on_bh_event(te_SOCANTRCVMORE); }
...
void sowwakeup(struct tcp_socket *so) { so->m_app->on_bh_event(te_SOWWAKEUP); }

void sorwakeup(struct tcp_socket *so) { so->m_app->on_bh_event(te_SORWAKEUP); }

void soisdisconnecting(struct tcp_socket *so) { so->m_app->on_bh_event(te_SOISDISCONNECTING); }

void soisdisconnected(struct tcp_socket *so) { so->m_app->on_bh_event(te_SOISDISCONNECTED); }
...
void CMbufObject::Dump(FILE *fd) {
    fprintf(fd, " %p (len:%lu) %s \n", m_mbuf->buf_addr, (ulong)m_mbuf->pkt_len, get_mbuf_type_name(m_type).c_str());
}
...
void CMbufBuffer::Dump(FILE *fd) {
    fprintf(fd, " total_size %lu, blk: %lu, pkts: %lu \n", (ulong)m_t_bytes, (ulong)m_blk_size, (ulong)m_num_pkt);
...
/* on tick */
void CEmulApp::on_tick() { next(); }

void CEmulApp::run_cmd_delay_rand(htw_ticks_t min_ticks, htw_ticks_t max_ticks) {
...
CEmulAppCmd *CEmulApp::process_cmd_one(CEmulAppCmd *cmd) {
    ...
    case tcDELAY_RAND:
        run_cmd_delay_rand((htw_ticks_t)cmd->u.m_delay_rnd.m_min_ticks, (htw_ticks_t)cmd->u.m_delay_rnd.m_max_ticks);
        break;
    case tcSET_VAR: {
...
int CEmulApp::on_bh_rx_pkts(uint32_t rx_bytes, struct rte_mbuf *m) {
    set_interrupt(true);
    if (m) {
        rte_pktmbuf_free(m);
    }
...
/* rx bytes */
int CEmulApp::on_bh_rx_bytes(uint32_t rx_bytes, struct rte_mbuf *m) {
    set_interrupt(true);
...
void CEmulAppProgram::add_cmd(CEmulAppCmd &cmd) { m_cmds.push_back(cmd); }

bool CEmulAppProgram::is_common_commands(tcp_app_cmd_t cmd_id) {
    if ((cmd_id == tcDELAY) || (cmd_id == tcDELAY_RAND) || (cmd_id == tcSET_VAR) || (cmd_id == tcJMPNZ)) {
        return (true);
    }
    return (false);
}
...
bool CEmulAppProgram::is_udp_commands(tcp_app_cmd_t cmd_id) {
    if ((cmd_id == tcTX_PKT) || (cmd_id == tcRX_PKT) || (cmd_id == tcKEEPALIVE) || (cmd_id == tcCLOSE_PKT)) {
        return (true);
    }
    return (false);
}
...
bool CEmulAppProgram::sanity_check(std::string &err) {
    ...
    for (i = 0; i < m_cmds.size(); i++) {
        ...
        if (m_stream) {
            ...
            if ((!is_common_commands(lp->m_cmd)) && (!is_udp_commands(lp->m_cmd))) {
...
int utl_mbuf_buffer_create_and_copy(uint8_t socket, CMbufBuffer *buf, uint32_t blk_size, uint8_t *d, uint32_t size,
                                    bool mbuf_const) {
...
*/
bool CEmulTxQueue::on_bh_tx_acked(uint32_t tx_bytes, uint32_t &add_to_tcp_queue, bool is_zero) {
    uint32_t z = m_q.size();
    assert(z > 0);
    ...
    if (is_zero) {
        ...
        if ((residue == 0) || ((m_v_cc > 0) && (m_v_cc < m_wnd_div_2))) {
            /*
            There is no need to go next (add another buffer) if  v_cc==0 (byte in outer queue), in any case pipeline
            won't happen and BDP would be larger. For  example, in case that window> buffer_size. Adding next buffer
            cost CPU% and better to do it only when there is a need for that.
            */
            return true;
        }
        return (false);
