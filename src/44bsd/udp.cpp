...
CUdpStats::CUdpStats(uint16_t num_of_tg_ids) { Init(num_of_tg_ids); }

CUdpStats::~CUdpStats() { delete[] m_sts_tg_id; }
...
void CUdpStats::ClearPerTGID(uint16_t tg_id) { memset(m_sts_tg_id + tg_id, 0, sizeof(udp_stat_int_t)); }
...
void CUdpFlow::Create(CPerProfileCtx *pctx, bool client, uint16_t tg_id) {
...
HOT_FUNC void CUdpFlow::update_checksum_and_lenght(CFlowTemplate *ftp, rte_mbuf_t *m, uint16_t udp_pyld_bytes,
                                                   char *p) {
    ...
    if (ftp->m_offload_flags & OFFLOAD_TX_CHKSUM) {
        if (!ftp->m_is_ipv6) {
            ...
            udp->setChecksumRaw(
                pkt_AddInetChecksumRaw(ftp->m_l4_pseudo_checksum, PKT_NTOHS(udp_pyld_bytes + UDP_HEADER_LEN)));
        } else {
            m->l2_len = ftp->m_offset_ip;
            m->l3_len = ftp->m_offset_l4 - ftp->m_offset_ip;
 ...
 */
static inline void udp_pktmbuf_fix_mbuf(struct rte_mbuf *m, uint16_t adj_len, uint16_t l7_len) {
...
void CUdpFlow::on_rx_packet(struct rte_mbuf *m, UDPHeader *udp, int offset_l7, int total_l7_len) {
