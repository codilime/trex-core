...
void CTcpDataAssocTranslation::insert_hash(const CTcpDataAssocParams &params, CEmulAppProgram *prog,
                                           CTcpTuneables *tune, uint32_t temp_idx) {
...
bool CAstfDB::compile_profile_dp(uint8_t socket_id) { return (true); }
...
void CAstfDB::set_profile_one_msg(Json::Value msg) { m_val = msg; }
...
void CAstfDB::dump() { std::cout << m_val << std::endl; }
...
void CAstfDB::fill_tx_mode(uint32_t program_index, uint32_t cmd_index, CEmulAppCmd &res) {
    Json::Value cmd;
...
void CAstfDB::fill_delay_rnd(uint32_t program_index, uint32_t cmd_index, CEmulAppCmd &res) {
    Json::Value cmd;
...
void CAstfDB::fill_set_var(uint32_t program_index, uint32_t cmd_index, CEmulAppCmd &res) {
    Json::Value cmd;
...
void CAstfDB::fill_jmpnz(uint32_t program_index, uint32_t cmd_index, CEmulAppCmd &res) {
    Json::Value cmd;
...
void CAstfDB::fill_tx_pkt(uint32_t program_index, uint32_t cmd_index, uint8_t socket_id, CEmulAppCmd &res) {
    Json::Value cmd;
...
void CAstfDB::fill_rx_pkt(uint32_t program_index, uint32_t cmd_index, CEmulAppCmd &res) {
    Json::Value cmd;
...
void CAstfDB::fill_keepalive_pkt(uint32_t program_index, uint32_t cmd_index, CEmulAppCmd &res) {
    Json::Value cmd;
...
CJsonData_err CAstfDB::verify_data(uint16_t max_threads) {
    ...
    for (int i = 0; i < ip_gen_list.size(); i++) {
        ...
        if (num_ips < max_threads) {
            err_str = "Pool:(" + ip_gen_list[i]["ip_start"].asString() + "-" + ip_gen_list[i]["ip_end"].asString() +
                      ") has only " + std::to_string(num_ips) + " address" + ((num_ips == 1) ? "" : "es") +
                      ". Number of IPs in each pool must exceed the number of" + " Data path threads, which is " +
...
bool CAstfDB::read_tunable_uint8(CTcpTuneables *tune, const Json::Value &parent, const std::string &param,
                                 uint32_t enum_val, uint8_t &val) {
...
bool CAstfDB::read_tunable_uint16(CTcpTuneables *tune, const Json::Value &parent, const std::string &param,
                                  uint32_t enum_val, uint16_t &val) {
...
bool CAstfDB::read_tunable_uint32(CTcpTuneables *tune, const Json::Value &parent, const std::string &param,
                                  uint32_t enum_val, uint32_t &val) {
...
bool CAstfDB::read_tunable_uint64(CTcpTuneables *tune, const Json::Value &parent, const std::string &param,
                                  uint32_t enum_val, uint64_t &val) {
...
bool CAstfDB::read_tunable_double(CTcpTuneables *tune, const Json::Value &parent, const std::string &param,
                                  uint32_t enum_val, double &val) {
...
bool CAstfDB::read_tunable_bool(CTcpTuneables *tune, const Json::Value &parent, const std::string &param,
                                uint32_t enum_val, double &val) {
...
void CAstfDB::tunable_min_max_u32(std::string param, uint32_t val, uint32_t min, uint32_t max) {
    Json::Value result;

    if (val < min) {
        generate_parse_err(result, "field '" + param + "' value " + std::to_string(val) + " is smaller than " +
                                       std::to_string(min));
    }
    if (val > max) {
        generate_parse_err(result, "field '" + param + "' value " + std::to_string(val) + " is greater than " +
                                       std::to_string(max));
    }
}

void CAstfDB::tunable_min_max_u64(std::string param, uint64_t val, uint64_t min, uint64_t max) {
    Json::Value result;
    if (val < min) {
        generate_parse_err(result, "field '" + param + "' value " + std::to_string(val) + " is smaller than " +
                                       std::to_string(min));
...
void CAstfDB::tunable_min_max_d(std::string param, double val, double min, double max) {
    Json::Value result;
    if (val < min) {
        generate_parse_err(result, "field '" + param + "' value " + std::to_string(val) + " is smaller than " +
                                       std::to_string(min));
...
bool CAstfDB::read_tunables_ipv6_field(CTcpTuneables *tune, Json::Value json, void *field, uint32_t enum_val) {
    ...
    */
    ...
    try {
        ...
        if (tune_json["tcp"] != Json::nullValue) {
            ...
            if (read_tunable_uint8(tune, json, "rexmtthresh", CTcpTuneables::tcp_rexmtthresh,
            ...
            if (read_tunable_uint16(tune, json, "delay_ack_msec", CTcpTuneables::tcp_delay_ack,
                                    tune->m_tcp_delay_ack_msec)) {
        ...
        if (tune_json["ipv6"] != Json::nullValue) {
            ...
            read_tunables_ipv6_field(tune, src_l, (void *)tune->m_ipv6_src, CTcpTuneables::ipv6_src_addr);

            Json::Value dst_l = json["dst_msb"];

            read_tunables_ipv6_field(tune, dst_l, (void *)tune->m_ipv6_dst, CTcpTuneables::ipv6_dst_addr);
        }
...
bool CAstfDB::get_latency_info(uint32_t &src_ipv4, uint32_t &dst_ipv4, uint32_t &dual_port_mask) {
...
void CAstfDB::get_thread_ip_range(uint16_t thread_id, uint16_t max_threads, uint16_t dual_port_id, std::string ip_start,
                                  std::string ip_end, std::string ip_offset, bool per_core_dist, CIpPortion &portion) {
    ...
    if (poolinfo.m_per_core_distro) {
        ...
        split_ips_v2(max_threads, rss_thread_id, rss_thread_max, CGlobalInfo::m_options.get_expected_dual_ports(),
                     dual_port_id, poolinfo, portion);
    } else {
        split_ips(thread_id, max_threads, dual_port_id, poolinfo, portion);
    }
}

CAstfTemplatesRW *CAstfDB::get_db_template_rw(uint8_t socket_id, CTupleGeneratorSmart *g_gen, uint16_t thread_id,
                                              uint16_t max_threads, uint16_t dual_port_id) {
    ...
    fill_rss_vals(thread_id, rss_thread_id, rss_thread_max);
    if (rss_thread_max > 1) {
        g_gen->set_astf_rss_mode(rss_thread_id, rss_thread_max,
                                 CGlobalInfo::m_options.m_reta_mask); /* configure the generator */
    }
    ...
    for (uint32_t index = 0; index < m_val["templates"].size(); index++) {
        ...
        template_ro.m_dual_mask =
            poolinfo.m_dual_interface_mask; // Should be the same for all poolinfo, so just take from last one
        ...
        if (c_temp["limit"] != Json::nullValue) {
            uint32_t cnt = utl_split_int(c_temp["limit"].asUInt(), thread_id, max_threads);
...
bool CAstfDB::build_assoc_translation(uint8_t socket_id) {
    ...
    for (uint16_t index = 0; index < num_of_tg_ids - 1; index++) {
        ...
        if (tg_name.length() > MAX_TG_NAME_LENGTH) {
            throw TrexException("TG name too long. Please limit TG names to " + std::to_string(MAX_TG_NAME_LENGTH) +
                                " characters");
        }
        if (tg_name.size() == 0) {
