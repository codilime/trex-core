...
class CTRexDummyCommand : public TrexRpcCommand {
  ...
  public:
    CTRexDummyCommand() : TrexRpcCommand("dummy", (TrexRpcComponent *)NULL, false, false) {}

    virtual ~CTRexDummyCommand() {}

  protected:
    virtual trex_rpc_cmd_rc_e _run(const Json::Value &params, Json::Value &result) { return (TREX_RPC_CMD_OK); }
};
...
class CTcpTuneables {
  ...
  public:
    ...
    bool is_valid_field(uint32_t val) { return (((m_bitfield & val) == val) ? true : false); }
...
class CTcpDataAssocTransHelp {
    ...
    CTcpDataAssocTransHelp(const CTcpDataAssocParams &params, CEmulAppProgram *prog, CTcpTuneables *tune,
                           uint32_t temp_idx) {
        m_params = params;
...
class CAstfDbRO {
  ...
  public:
    ...
    CEmulAppProgram *get_client_prog(uint32_t temp_id) const { return m_templates[temp_id].m_client_prog; }

    CTcpServreInfo *get_server_info_by_port(uint16_t port, bool stream = true);
    double get_total_cps() { return (m_cps_sum); }
    double get_total_cps_per_thread(uint16_t max_threads) { return (m_cps_sum / (double)max_threads); }
    double get_delta_tick_sec_thread(uint16_t max_threads) { return (1.0 / get_total_cps_per_thread(max_threads)); }

    uint16_t get_template_tg_id(uint16_t template_id) { return m_templates[template_id].m_tg_id; }

    uint16_t get_num_of_tg_ids() { return m_num_of_tg_ids; }
...
class CAstfDB : public CTRexDummyCommand {
  ...
  public:
    ...
    static bool has_instance(profile_id_t profile_id = 0) { return m_instances.find(profile_id) != m_instances.end(); }
    ...
    */
    ...
    void set_client_cfg_db(ClientCfgDB *client_config_info) { m_client_config_info = client_config_info; }
    ...
    CAstfTemplatesRW *get_db_template_rw(uint8_t socket_id, CTupleGeneratorSmart *g_gen, uint16_t thread_id,
    ...
    bool get_latency_info(uint32_t &src_ipv4, uint32_t &dst_ipv4, uint32_t &dual_port_mask);

    void get_thread_ip_range(uint16_t thread_id, uint16_t max_threads, uint16_t dual_port_id, std::string ip_start,
  ...
  private:
    ...
    void fill_tx_pkt(uint32_t program_index, uint32_t cmd_index, uint8_t socket_id, CEmulAppCmd &res);
  ...
  private:
    bool read_tunables_ipv6_field(CTcpTuneables *tune, Json::Value json, void *field, uint32_t enum_val);

    bool read_tunable_uint8(CTcpTuneables *tune, const Json::Value &parent, const std::string &param, uint32_t enum_val,
                            uint8_t &val);

    bool read_tunable_uint16(CTcpTuneables *tune, const Json::Value &parent, const std::string &param,
                             uint32_t enum_val, uint16_t &val);

    bool read_tunable_uint32(CTcpTuneables *tune, const Json::Value &parent, const std::string &param,
                             uint32_t enum_val, uint32_t &val);

    bool read_tunable_uint64(CTcpTuneables *tune, const Json::Value &parent, const std::string &param,
                             uint32_t enum_val, uint64_t &val);

    bool read_tunable_double(CTcpTuneables *tune, const Json::Value &parent, const std::string &param,
                             uint32_t enum_val, double &val);

    bool read_tunable_bool(CTcpTuneables *tune, const Json::Value &parent, const std::string &param, uint32_t enum_val,
                           double &val);

    void tunable_min_max_u32(std::string param, uint32_t val, uint32_t min, uint32_t max);

    void tunable_min_max_u64(std::string param, uint64_t val, uint64_t min, uint64_t max);

    void tunable_min_max_d(std::string param, double val, double min, double max);
  ...
  public:
    bool is_smart_gen(uint16_t thread_id) { return (m_smart_gen.find(thread_id) != m_smart_gen.end()); }
