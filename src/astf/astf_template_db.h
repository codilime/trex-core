...
class CAstfPerTemplateRW {
  ...
  public:
    bool Create(CTupleGeneratorSmart *global_gen, astf_t_id_t tid, astf_thread_id_t thread_id, CAstfPerTemplateRO *info,
  ...
  public:
    ...
    bool is_udp() { return m_is_udp; }

    uint16_t get_dest_port() { return (m_dest_port); }
    void set_limit(uint32_t limit) { m_limit = limit + 1; /* need to add 1*/ }
...
class CAstfTemplatesRW {
  ...
  public:
    bool Create(astf_thread_id_t thread_id, astf_thread_id_t max_threads);
    void Delete();

    astf_thread_id_t get_thread_id() { return (m_thread_id); }

    astf_thread_id_t get_max_threads() { return (m_max_threads); }
