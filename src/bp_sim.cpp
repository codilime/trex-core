...
uint32_t getDualPortId(uint32_t thread_id) { return (thread_id % (CGlobalInfo::m_options.get_expected_dual_ports())); }
...
void CFlowGenStats::dump(FILE *fd) {
    std::string s_bytes = double_to_human_str((double)(m_total_bytes), "bytes", KBYE_1024);

    std::string s_pkt = double_to_human_str((double)(m_total_pkt), "pkt", KBYE_1000);

    std::string s_flows = double_to_human_str((double)(m_total_open_flows), "flows", KBYE_1000);
...
void CPacketDescriptor::Dump(FILE *fd) {
    ...
    fprintf(fd,
            " flow_ID      : %d , max_pkts : %u, max_aging: %d sec , pkt_id : %u, init: %d   ( dir:%d  dir_max :%d  ) "
            "bid:%d \n",
            getFlowId(), GetMaxPktsPerFlow(), GetMaxFlowTimeout(), getFlowPktNum(), IsInitSide(),
            GetDirInfo()->GetPktNum(), GetDirInfo()->GetMaxPkts(), IsBiDirectionalFlow() ? 1 : 0
...
uint64_t CCPacketParserCounters::getTotalErrors() {
    uint64_t res = m_non_ip + m_arp + m_mpls + m_non_valid_ipv4_ver + m_ip_checksum_error + m_ip_length_error +
                   m_ip_not_first_fragment_error + m_ip_ttl_is_zero_error + m_ip_multicast_error +

                   m_non_tcp_udp_ah + m_non_tcp_udp_esp + m_non_tcp_udp_icmp + m_non_tcp_udp_gre + m_non_tcp_udp_ip +
...
bool CPacketParser::ProcessPacket(CPacketIndication *pkt_indication, CCapPktRaw *raw_packet) {
...
uint8_t CPacketIndication::ProcessIpPacketProtocol(CCPacketParserCounters *m_cnt, uint8_t protocol, int *offset) {
...
void CPacketIndication::ProcessIpPacket(CPacketParser *parser, int offset) {
...
void CPacketIndication::ProcessIpv6Packet(CPacketParser *parser, int offset) {
...
bool CPacketIndication::ConvertPacketToIpv6InPlace(CCapPktRaw *pkt, int offset) {
...
void CPacketIndication::ProcessPacket(CPacketParser *parser, CCapPktRaw *pkt) {
...
/* process packet */
void CPacketIndication::_ProcessPacket(CPacketParser *parser, CCapPktRaw *pkt) {
...
void CFlowTableManagerBase::Dump(FILE *fd) { m_stats.Dump(fd); }
...
void CFlowTableMap::Delete() { remove_all(); }
...
uint64_t CFlowTableMap::count() { return (m_map.size()); }
...
void CFlowPktInfo::Dump(FILE *fd) { m_pkt_indication.Dump(fd, 0); }

void CCapFileFlowInfo::generate_flow(CTupleTemplateGeneratorSmart *tuple_gen, CNodeGenerator *gen, dsec_t time,
                                     uint64_t flow_id, CFlowYamlInfo *template_info, CGenNode *node) {
    dsec_t c_time = time;
    ...
    node->m_flow_id = (flow_id & (0x000fffffffffffffULL)) | (((uint64_t)(tuple_gen->GetThreadId() & 0xff)) << 56);
    ...
    if (unlikely(CGlobalInfo::is_learn_mode())) {
        ...
        if (lp->m_pkt_indication.m_desc.IsBiDirectionalFlow()) {
            ...
            if (CGlobalInfo::is_learn_mode(CParserOption::LEARN_MODE_IP_OPTION) ||
                ...
                is_ok = lpThread->associate(((uint32_t)flow_id) & NAT_FLOW_ID_MASK_TCP_ACK,
                                            node); /* associate flow_id=>node */
                assert(is_ok);
            } else {
                is_ok = lpThread->associate(((uint32_t)flow_id) & NAT_FLOW_ID_MASK_IP_ID,
                                            node); /* associate flow_id=>node */
                if (!is_ok) {
                    fprintf(
                        stderr,
                        "With given learn mode, max supported rate of UDP flows is limited. Please reduce the rate\n");
                    exit(1);
                }
            }
...
enum CCapFileFlowInfo::load_cap_file_err CCapFileFlowInfo::is_valid_template_load_time() {
    ...
    if (CGlobalInfo::is_learn_mode()) {
        ...
        if (CGlobalInfo::is_learn_mode(CParserOption::LEARN_MODE_TCP) &&
            (pkt_0_indication.getIpProto() == IPPROTO_TCP)) {
            is_tcp = true;
            if (CGlobalInfo::is_learn_mode(CParserOption::LEARN_MODE_TCP_ACK)) {
                if (Size() < 3) {
                    fprintf(stderr, "Error: In the chosen learn mode, need at least the 3 TCP handshake packets.\n");
            ...
            if (CGlobalInfo::is_learn_mode(CParserOption::LEARN_MODE_TCP_ACK) && is_tcp) {
                // To support TCP seq randomization from server to client, we need second packet in flow to be the
                // server SYN+ACK
                bool error = false;
                ...
                if ((!pkt_2_indication.m_desc.IsInitSide())) {
                    fprintf(stderr, "Error: Wrong third packet. In the chosen learn mode, need at least the 3 TCP "
                                    "handshake packets.\n");
                    fprintf(stderr, "       Please give different CAP file, or try different --learn-mode\n");
                    return kTCPLearnModeBadFlow;
                }
                if ((pkt_0_indication.m_cap_ipg < (double)LEARN_MODE_MIN_IPG / 1000) ||
                    ...
                    fprintf(stderr, "Error: Bad cap file timings. In the chosen learn mode");
                    ...
                    fprintf(stderr, "       Please give different CAP file, try different --learn-mode, or edit ipg "
                                    "parameters in template file\n");
...
void CCapFileFlowInfo::update_info(CFlowYamlInfo *flow_info) {
    ...
    for (i = 0; i < Size(); i++) {
        ...
        pkt_dir_t dir =
            desc->IsInitSide() ? CLIENT_SIDE : SERVER_SIDE; // with respect to the first sub-flow in the template
    ...
    for (i = 0; i < Size(); i++) {
        ...
        if ((lpFlow->m_per_dir[0].m_pkt_id > 0) && (lpFlow->m_per_dir[1].m_pkt_id > 0)) {
            /* we have both dir */
...
enum CCapFileFlowInfo::load_cap_file_err CCapFileFlowInfo::load_cap_file(std::string cap_file, uint16_t _id,
                                                                         uint8_t plugin_id) {
    CFlowYamlInfo info;
...
enum CCapFileFlowInfo::load_cap_file_err CCapFileFlowInfo::load_cap_file(std::string cap_file, uint16_t _id,
    ...
    while (true) {
        ...
        if (parser.ProcessPacket(&pkt_indication, &raw_packet)) {
            ...
            if (pkt_indication.m_desc.IsValidPkt()) {
                ...
                if (is_fif) {
                    ...
                    if (is_custom_dirs) {
                        ...
                        is_init_side = ((first_flow_fif_is_swap ? true : false) == pkt_indication.m_desc.IsSwapTuple())
                                           ? true
                                           : false;
                    } else {
                        if (lpflow == first_flow) {
                            // add to
                            is_init_side = ((lpflow->is_fif_swap ? true : false) == pkt_indication.m_desc.IsSwapTuple())
                                               ? true
                                               : false;
                        } else {
                ...
                fprintf(stderr,
                        "ERROR packet %d is not supported, should be Ethernet/IP(0x0800)/(TCP|UDP) format try to "
                        "convert it using Wireshark !\n",
                        cnt);
                return kPktNotSupp;
            }
            ...
            fprintf(stderr,
                    "ERROR packet %d is not supported, should be Ethernet/IP(0x0800)/(TCP|UDP) format try to convert "
                    "it using Wireshark !\n",
                    cnt);
    ...
    for (i = 1; i < Size(); i++) {
        ...
        lp_prev->m_pkt_indication.m_cap_ipg = lp->m_pkt_indication.m_cap_ipg - lp_prev->m_pkt_indication.m_cap_ipg;
        if (lp->m_pkt_indication.m_desc.IsInitSide() != lp_prev->m_pkt_indication.m_desc.IsInitSide()) {
    ...
    if (m_total_errors > 0) {
        ...
        fprintf(stderr, " ERORR in one of the cap file, you should have one flow per cap file, set valid plugin or "
                        "multi_flow_enabled flag\n");
        return kCapFileErr;
    }
    return kOK;
}
...
void CCapFileFlowInfo::update_ipg_by_factor(double factor, CFlowYamlInfo *flow_info) {
    int i;
...
void CCapFileFlowInfo::update_min_ipg(dsec_t min_ipg, dsec_t override_ipg) {
...
void CCapFileFlowInfo::Delete() { RemoveAll(); }
...
void operator>>(const YAML::Node &node, CFlowYamlInfo &fi) {
    ...
    if (utl_yaml_read_ip_addr(node, "server_addr", fi.m_server_addr)) {
        try {
...
void operator>>(const YAML::Node &node, CFlowsYamlInfo &flows_info) {
    ...
    for (unsigned i = 0; i < cap_info.size(); i++) {
        CFlowYamlInfo fi;
        cap_info[i] >> fi;
        fi.m_client_pool_idx = flows_info.m_tuple_gen.get_client_pool_id(fi.m_client_pool_name);
        fi.m_server_pool_idx = flows_info.m_tuple_gen.get_server_pool_id(fi.m_server_pool_name);
...
void CFlowStats::DumpHeader(FILE *fd) {
    fprintf(fd, " %2s %-40s %-8s %-9s %-6s %-8s %-8s %-7s %-6s %-7s %-6s %-4s %-4s\n", "id", "name", "tps", "cps",
            "f-pkts", "f-bytes", "duration", "Mb/sec", "MB/sec", "c-flows", "PPS", "errors", "flows");
}
void CFlowStats::Dump(FILE *fd) {
    fprintf(fd, " %02d %-40s %8.2f %9.2f %6.0f %8.0f %8.2f %7.2f %6.2f %7.0f %6.0f %4llu %4llu\n", m_id, m_name.c_str(),
            m_cps, get_normal_cps(), m_pkt, m_bytes, duration_sec, m_mb_sec, m_mB_sec, m_c_flows, m_pps,
            (unsigned long long)m_errors, (unsigned long long)m_flows);
}

bool CFlowGeneratorRecPerThread::Create(CTupleGeneratorSmart *global_gen, CFlowYamlInfo *info,
                                        CFlowsYamlInfo *yaml_flow_info, CCapFileFlowInfo *flow_info, uint16_t _id,
                                        uint32_t thread_id) {

    BP_ASSERT(info);
    m_thread_id = thread_id;

    tuple_gen.Create(global_gen, info->m_client_pool_idx, info->m_server_pool_idx);
    CTupleGenYamlInfo *lpt;
    lpt = &yaml_flow_info->m_tuple_gen;

    tuple_gen.SetSingleServer(info->m_one_app_server, info->m_server_addr, getDualPortId(thread_id),
...
void CFlowGeneratorRecPerThread::Delete() { tuple_gen.Delete(); }
...
void CFlowGeneratorRec::updateIpg(double factor) { m_flow_info.update_ipg_by_factor(factor, m_info); }
...
void CFlowGeneratorRec::fixup_ipg_if_needed(void) {
    ...
    if ((m_flows_info->m_cap_mode) && (m_flows_info->m_cap_ipg_min_set) && (m_flows_info->m_cap_overide_ipg_set)) {
        m_flow_info.update_min_ipg(m_flows_info->m_cap_ipg_min, m_flows_info->m_cap_overide_ipg);
    }
}

bool CFlowGeneratorRec::Create(CFlowYamlInfo *info, CFlowsYamlInfo *flows_info, uint16_t _id) {
    BP_ASSERT(info);
    m_id = _id;
    m_info = info;
...
void CFlowGeneratorRec::Delete() { m_flow_info.Delete(); }
...
void CGenNode::Dump(FILE *fd) {
    fprintf(fd, "%.6f,%llx,%p,%llu,%d,%d,%d,%d,%d,%d,%x,%x,%d\n", m_time, (unsigned long long)m_flow_id, m_pkt_info,
            (unsigned long long)m_pkt_info->m_pkt_indication.m_packet->pkt_cnt,
            m_pkt_info->m_pkt_indication.m_packet->pkt_len, m_pkt_info->m_pkt_indication.m_desc.getId(),
            (m_pkt_info->m_pkt_indication.m_desc.IsInitSide() ? 1 : 0), m_pkt_info->m_pkt_indication.m_desc.IsLastPkt(),
            m_type, m_thread_id, m_src_ip, m_dest_ip, m_src_port);
}

void CNodeGenerator::set_vif(CVirtualIF *v_if) { m_v_if = v_if; }
...
void CNodeGenerator::Delete() { m_realtime_his.Delete(); }

void CNodeGenerator::add_node(CGenNode *mynode) { m_p_queue.push(mynode); }
...
int CNodeGenerator::open_file(std::string file_name, CPreviewMode *preview_mode) {
    BP_ASSERT(m_v_if);
...
bool CFlowGenListPerThread::Create(uint32_t thread_id, CFlowGenList *flow_list, uint32_t max_threads) {
    ...
    m_node_pool =
        utl_rte_mempool_create_non_pkt(name, flow_nodes, sizeof(CGenNode), 128, socket_id, false, use_hugepages);
    ...
    if (!get_is_tcp_mode()) {
        ...
        for (int i = 0; i < tuple_gen->m_client_pool.size(); i++) {
            split_ips(m_thread_id, m_max_threads, getDualPortId(), tuple_gen->m_client_pool[i], portion);

            m_smart_gen.add_client_pool(tuple_gen->m_client_pool[i].m_dist, portion.m_ip_start, portion.m_ip_end,
                                        active_flows_per_core, m_flow_list->m_client_config_info,
        ...
        for (int i = 0; i < tuple_gen->m_server_pool.size(); i++) {
            split_ips(m_thread_id, m_max_threads, getDualPortId(), tuple_gen->m_server_pool[i], portion);
            m_smart_gen.add_server_pool(tuple_gen->m_server_pool[i].m_dist, portion.m_ip_start, portion.m_ip_end,
                                        active_flows_per_core, tuple_gen->m_server_pool[i].m_is_bundling);
        }
...
FORCE_NO_INLINE void CFlowGenListPerThread::handler_defer_job(CGenNode *p) {
    ...
    for (i = 0; i < defer->m_cnt; i++) {
        m_smart_gen.FreePort(defer->m_pool_idx[i], defer->m_clients[i], defer->m_ports[i]);
    }
}

void CFlowGenListPerThread::defer_client_port_free(bool is_tcp, uint32_t c_idx, uint16_t port, pool_index_t c_pool_idx,
...
void CFlowGenListPerThread::defer_client_port_free(CGenNode *p) {
    defer_client_port_free(p->m_pkt_info->m_pkt_indication.m_desc.IsTcp(), p->m_src_idx, p->m_src_port,
                           p->m_template_info->m_client_pool_idx, p->m_tuple_gen);
}
...
void CFlowGenListPerThread::init_from_global() {
    ...
    for (i = 0; i < (int)m_flow_list->m_cap_gen.size(); i++) {
        ...
        yaml_info->m_restart_time =
            (yaml_info->m_limit_was_set) ? (yaml_info->m_limit / (yaml_info->m_k_cps * 1000.0)) : 0;

        lp_thread->Create(&m_smart_gen, yaml_info, lp->m_flows_info, &lp->m_flow_info, lp->m_id, m_thread_id);

        m_cap_gen.push_back(lp_thread);
    }
}

static void free_map_flow_id_to_node(CGenNode *p) { CGlobalInfo::free_node(p); }
...
void CNodeGenerator::add_exit_node(CFlowGenListPerThread *thread, dsec_t max_time) {
...
inline bool CNodeGenerator::handle_stl_node(CGenNode *node, CFlowGenListPerThread *thread) {
...
#define unsafe_container_of(var, ptr, type, member) ((type *)((uint8_t *)(ptr)-offsetof(type, member)))
...
template <bool TEARDOWN> inline void CFlowGenListPerThread::on_flow_tick(CGenNode *node) {
    ...
    if (likely(!node->is_repeat_flow())) {
        ...
        if (node->is_last_in_flow()) {
            ...
            /* need to update both accurate time and bucket time in this case update_next_pkt_in_flow_both() for
             * reschedule to be accurate */
            m_tw.timer_start(&node->m_tmr, node->update_next_pkt_in_flow_both());
        }
    }
}

static void tw_free_node(void *userdata, CHTimerObj *tmr) {
...
static void tw_on_tick_per_thread_cb_always(void *userdata, CHTimerObj *tmr) {
    CFlowGenListPerThread *thread = (CFlowGenListPerThread *)userdata;
    UNSAFE_CONTAINER_OF_PUSH;
    CGenNode *node = unsafe_container_of(node, tmr, CGenNode, m_tmr);
    UNSAFE_CONTAINER_OF_POP;
...
void tw_on_tick_per_thread_cb(void *userdata, CHTimerObj *tmr) {
    CFlowGenListPerThread *thread = (CFlowGenListPerThread *)userdata;
...
inline bool CNodeGenerator::do_work_stl(CGenNode *node, CFlowGenListPerThread *thread, bool on_terminate) {
...
inline bool CNodeGenerator::do_work_both(CGenNode *node, CFlowGenListPerThread *thread, dsec_t d_time) {
...
inline bool CNodeGenerator::do_work(CGenNode *node, CFlowGenListPerThread *thread, dsec_t d_time) {
...
inline void CNodeGenerator::do_sleep(dsec_t &cur_time, CFlowGenListPerThread *thread, dsec_t n_time) {
...
inline int CNodeGenerator::teardown(CFlowGenListPerThread *thread, bool on_terminate, double &old_offset,
                                    double offset) {
...
HOT_FUNC inline int CNodeGenerator::flush_file_realtime(dsec_t max_time, dsec_t d_time, CFlowGenListPerThread *thread,
                                                        double &old_offset) {
    CGenNode *node;
    dsec_t offset = 0.0;
    dsec_t cur_time;
...
FORCE_NO_INLINE void CNodeGenerator::handle_slow_operations(sch_state_t &state, CGenNode *&node, dsec_t &cur_time,
                                                            dsec_t &n_time, dsec_t &offset,
 ...
 *
 */
void CNodeGenerator::handle_time_strech(CGenNode *&node, dsec_t &cur_time, dsec_t &n_time, dsec_t &offset,
                                        CFlowGenListPerThread *thread) {
...
int CNodeGenerator::flush_file_sim(dsec_t max_time, dsec_t d_time, bool on_terminate, CFlowGenListPerThread *thread,
                                   double &old_offset) {
    CGenNode *node;
...
int CNodeGenerator::flush_file(dsec_t max_time, dsec_t d_time, bool on_terminate, CFlowGenListPerThread *thread,
                               double &old_offset) {
#ifdef TREX_SIM
...
void CNodeGenerator::handle_batch_tw_level1(CGenNode *node, CFlowGenListPerThread *thread, bool &exit_scheduler,
                                            bool on_terminate) {
    ...
    if (on_terminate && (thread->m_tw.is_any_events_left() == false)) {
        thread->free_node(node);
    } else {
        m_p_queue.push(node);
    }
...
void CNodeGenerator::handle_maintenance(CFlowGenListPerThread *thread) {
...
bool CNodeGenerator::handle_slow_messages(uint8_t type, CGenNode *node, CFlowGenListPerThread *thread,
                                          bool on_terminate) {
...
void CFlowGenListPerThread::DumpStats(FILE *fd) { m_stats.dump(fd); }
...
uint32_t CFlowGenListPerThread::getDualPortId() { return (::getDualPortId(m_thread_id)); }
...
double CFlowGenListPerThread::get_longest_flow(pool_index_t pool_idx, bool is_client) {
    ...
    for (i = 0; i < (int)m_cap_gen.size(); i++) {
        CFlowGeneratorRecPerThread *lp = m_cap_gen[i];
        if (is_client && lp->m_info->m_client_pool_idx != pool_idx)
            continue;
        if (!is_client && lp->m_info->m_server_pool_idx != pool_idx)
            continue;
        double tmp_len;
...
double CFlowGenListPerThread::get_delta_flow_is_sec() { return (1.0 / (1000.0 * get_total_kcps())); }
...
int CFlowGenListPerThread::generate_flows_roundrobin(bool *done) {
    ...
    *done = true;
    for (i = 0; i < (int)m_cap_gen.size(); i++) {
        ...
        if (!(cur->m_info->m_limit_was_set) || (cur->m_info->m_flowcnt < cur->m_info->m_limit)) {
            *done = false;
...
void CFlowGenListPerThread::handle_nat_msg(CGenNodeNatInfo *msg) {
    ...
    for (i = 0; i < msg->m_cnt; i++) {
        ...
        if (first) {
#ifdef NAT_TRACE_
            printf(" %.03f  RX :set node %p:%x  %x:%x TCP diff %x\n", now_sec(), node, nat_msg->m_fid,
                   nat_msg->m_external_ip, nat_msg->m_external_port, node->get_nat_tcp_seq_diff_client());
#endif
            ...
            if (CGlobalInfo::is_learn_verify_mode()) {
                if (!node->is_external_is_eq_to_internal_ip() || node->get_nat_tcp_seq_diff_client() != 0) {
...
void CFlowGenListPerThread::handle_stl_rx(CGenNode *node, bool on_terminate) {
...
void CFlowGenListPerThread::no_memory_error() {
    ...
    printf("\n");
    printf("\n");
    printf("ERROR, not enough flow objects, try to enlarge the number of objects in trex_cfg file or reduce the "
           "bandwidth \n");
...
void CFlowGenList::dump_client_config(FILE *fd) { m_client_config_info.dump(fd); }
...
int CFlowGenList::load_from_yaml(std::string file_name, uint32_t num_threads) {
    uint8_t idx;
...
double CFlowGenList::get_delta_flow_is_sec() { return (1.0 / (1000.0 * get_total_kcps())); }
...
static uint32_t get_rand_32(uint32_t MinimumRange, uint32_t MaximumRange);

static uint32_t get_rand_32(uint32_t MinimumRange, uint32_t MaximumRange) __attribute__((unused));
...
pkt_dir_t CErfIFStl::port_id_to_dir(uint8_t port_id) { return ((pkt_dir_t)(port_id & 1)); }
...
void CPluginCallbackSimple::on_node_first(uint8_t plugin_id, CGenNode *node, CFlowYamlInfo *template_info,
                                          CTupleTemplateGeneratorSmart *tuple_gen, CFlowGenListPerThread *flow_gen) {
...
rte_mbuf_t *CPluginCallbackSimple::http_plugin(uint8_t plugin_id, CGenNode *node, CFlowPktInfo *pkt_info) {
...
rte_mbuf_t *CPluginCallbackSimple::dyn_pyload_plugin(uint8_t plugin_id, CGenNode *node, CFlowPktInfo *pkt_info) {
...
rte_mbuf_t *CPluginCallbackSimple::sip_voice_plugin(uint8_t plugin_id, CGenNode *node, CFlowPktInfo *pkt_info) {
    ...
    case 0:
        ...
        case 0: {
            ...
            if (CGlobalInfo::is_ipv6_enable()) {
                via_replace_cmd.m_add_pkt_len = (((INET6_ADDRSTRLEN + 2) - 9) * 3) + ((INET_PORTSTRLEN * 2) - 9);
                ...
                via_replace_cmd.m_add_pkt_len = ((INET_ADDRSTRLEN - 9) * 3) + ((INET_PORTSTRLEN * 2) - 9);
            }
...
rte_mbuf_t *CPluginCallbackSimple::rtsp_plugin(uint8_t plugin_id, CGenNode *node, CFlowPktInfo *pkt_info) {
    ...
    case 0:
        ...
        case 5: {
            ...
            if (CGlobalInfo::is_ipv6_enable()) {
                // For IPv6, accomodate use of brackets (+2 bytes)
                replace_port_cmd.m_add_pkt_len = ((INET6_ADDRSTRLEN + 2) - 9) + ((INET_PORTSTRLEN * 2) - 8);
                ...
                replace_port_cmd.m_add_pkt_len = (INET_ADDRSTRLEN - 9) + ((INET_PORTSTRLEN * 2) - 8);
            }
...
int CMiniVM::mini_vm_replace_ip_via_ip_ip_ports(CMiniVMReplaceIP_PORT_IP_IP_Port *cmd) {
    ...
    p += cp_pkt_to_from(p, original_l7_ptr, 0, cmd->m_ip_via_start);
    ...
    /* up to the IP */
    p += cp_pkt_to_from(p, original_l7_ptr, cmd->m_ip_via_stop, cmd->m_ip0_start);
    /*IP */
    if (cmd->m_flags & CMiniVMCmdBase::MIN_VM_V6) {
        p[-2] = '6';
    ...
    /* up to IP 2 */
    p += cp_pkt_to_from(p, original_l7_ptr, cmd->m_ip0_stop, cmd->m_ip1_start);
    /* IP2 */
    if (cmd->m_flags & CMiniVMCmdBase::MIN_VM_V6) {
        p[-2] = '6';
        p += ipv6_to_str(&cmd->m_ip, p);
    ...
    /* up to port */
    p += cp_pkt_to_from(p, original_l7_ptr, cmd->m_ip1_stop, cmd->m_port_start);
    /* port */
    p += sprintf(p, "%u", cmd->m_port);

    /* up to end */
    p += cp_pkt_to_from(p, original_l7_ptr, cmd->m_port_stop, len);
...
int CMiniVM::mini_vm_replace_ip_ip_ports(CMiniVMReplaceIP_IP_Port *cmd) {
    ...
    /* up to the IP */
    p += cp_pkt_to_from(p, original_l7_ptr, 0, cmd->m_ip0_start);
    /*IP */
    if (cmd->m_flags & CMiniVMCmdBase::MIN_VM_V6) {
        p += ipv6_to_str(&cmd->m_ip, p);
    } else {
...
int CMiniVM::mini_vm_replace_ports(CMiniVMReplaceIPWithPort *cmd) {
    ...
    p += sprintf(p, "%u-%u;server_port=%u-%u", cmd->m_client_port, cmd->m_client_port + 1, cmd->m_server_port,
                 cmd->m_server_port + 1);
