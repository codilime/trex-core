...
typedef enum {
    VM_REPLACE_IP_OFFSET = 0x12,        /* fix ip at offset  */
    ...
    VM_REPLACE_IP_IP_PORT,              /* SMID command to replace IPV4 , IPV4, PORT in 3 diffrent location , see
                                           CMiniVMReplaceIP_IP_Port*/
    VM_REPLACE_IPVIA_IP_IP_PORT, /* SMID command to replace ip,port IPV4 , IPV4, PORT in 3 diffrent location , see
                                    CMiniVMReplaceIP_PORT_IP_IP_Port*/
    VM_DYN_PYLOAD,
...
/* callback */
void on_node_first(uint8_t plugin_id, CGenNode *node, CFlowYamlInfo *template_info,
                   CTupleTemplateGeneratorSmart *tuple_gen, CFlowGenListPerThread *flow_gen);
...
class CVirtualIF {
  public:
    CVirtualIF() { m_preview_mode = NULL; }
    virtual ~CVirtualIF() {}
    ...
    virtual uint16_t rx_burst(pkt_dir_t dir, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) { assert(0); }

    /* send one packet */
    virtual int send_node(CGenNode *node) = 0;

    /* by default does the same */
    virtual int send_node_service_mode(CGenNode *node) { return send_node(node); }
    ...
    virtual pkt_dir_t port_id_to_dir(uint8_t port_id) { return (CS_INVALID); }

    virtual void set_review_mode(CPreviewMode *preview_mode) { m_preview_mode = preview_mode; }

    virtual CVirtualIFPerSideStats *get_stats() { return m_stats; }
    virtual bool redirect_to_rx_core(pkt_dir_t dir, rte_mbuf_t *m) { return (false); }
...
struct CGenNodeBase {
  public:
    ...
    enum {
        ...
        NODE_FLAGS_INIT_START_FROM_SERVER_SIDE_SERVER_ADDR =
            0x100,                   /* init packet start from server side with server addr */
  ...
  public:
    bool operator<(const CGenNodeBase *rsh) const { return (m_time < rsh->m_time); }
    bool operator==(const CGenNodeBase *rsh) const { return (m_time == rsh->m_time); }
    bool operator>(const CGenNodeBase *rsh) const { return (m_time > rsh->m_time); }

  public:
    void set_socket_id(socket_id_t socket) { m_socket_id = socket; }

    socket_id_t get_socket_id() { return (m_socket_id); }
    ...
    inline bool get_is_slow_path() const { return ((m_flags & NODE_FLAGS_SLOW_PATH) ? true : false); }
...
struct CGenNode : public CGenNodeBase {
  ...
  public:
    ...
    inline uint8_t get_plugin_id(void) { return (m_template_info->m_plugin_id); }

    inline bool is_responder_pkt();
    inline bool is_initiator_pkt();

    inline bool is_eligible_from_server_side() { return (((m_src_ip & 1) == 1) ? true : false); }
    ...
    inline pkt_dir_t get_mbuf_cache_dir() { return ((pkt_dir_t)(m_flags & 1)); }
  ...
  public:
    inline void set_rx_check() { m_flags |= NODE_FLAGS_SAMPLE_RX_CHECK; }

    inline bool is_rx_check_enabled() { return ((m_flags & NODE_FLAGS_SAMPLE_RX_CHECK) ? true : false); }
  ...
  public:
    ...
    inline bool is_nat_first_state() { return (btGetMaskBit16(m_flags, 4, 3) == 1 ? true : false); }

    inline void set_nat_wait_state() { btSetMaskBit16(m_flags, 4, 3, 2); }

    inline bool is_nat_wait_state() { return (btGetMaskBit16(m_flags, 4, 3) == 2 ? true : false); }

    // We saw first TCP SYN. Waiting for SYN+ACK
    inline void set_nat_wait_ack_state() { btSetMaskBit16(m_flags, 4, 3, 3); }

    inline bool is_nat_wait_ack_state() { return (btGetMaskBit16(m_flags, 4, 3) == 3) ? true : false; }

    inline void set_nat_learn_state() { m_type = FLOW_PKT; /* normal operation .. repeat might work too */ }

  public:
    inline uint32_t get_short_fid(void) { return (((uint32_t)m_flow_id) & NAT_FLOW_ID_MASK_TCP_ACK); }

    inline uint8_t get_thread_id(void) { return (m_thread_id); }

    inline void set_nat_tcp_seq_diff_client(uint32_t diff) { m_nat_tcp_seq_diff_client = diff; }

    inline uint32_t get_nat_tcp_seq_diff_client() { return m_nat_tcp_seq_diff_client; }

    inline void set_nat_tcp_seq_diff_server(uint32_t diff) { m_nat_tcp_seq_diff_server = diff; }

    inline uint32_t get_nat_tcp_seq_diff_server() { return m_nat_tcp_seq_diff_server; }

    inline void set_nat_ipv4_addr(uint32_t ip) { m_nat_external_ipv4 = ip; }

    inline void set_nat_ipv4_port(uint16_t port) { m_nat_external_port = port; }

    inline uint32_t get_nat_ipv4_addr() { return (m_nat_external_ipv4); }

    inline uint16_t get_nat_ipv4_port() { return (m_nat_external_port); }

    bool is_external_is_eq_to_internal_ip() {
        /* this API is used to check TRex itself */
        if ((get_nat_ipv4_addr() == m_src_ip) && (get_nat_ipv4_port() == m_src_port)) {
            return (true);
        } else {
            return (false);
        }
...
struct CGenNodeDeferPort {
  ...
  public:
    ...
    bool add_client(pool_index_t pool_idx, uint32_t client, uint16_t port) {
...
struct CGenNodeCompare {
    bool operator()(const CGenNode *lhs, const CGenNode *rhs) { return lhs->m_time > rhs->m_time; }
};
...
class CNodeGenerator {
  public:
    ...
    typedef enum { scINIT = 0x17, scWORK, scWAIT, scSTRECH, scTERMINATE } sch_state_t;
...
#define WAIT_WINDOW_SIZE (-1.0 / 1000000)
    ...
    CFlowGenListPerThread *Parent() { return (m_parent); }

  public:
    void add_node(CGenNode *mynode);
    void remove_all(CFlowGenListPerThread *thread);

    int open_file(std::string file_name, CPreviewMode *preview);
    int close_file(CFlowGenListPerThread *thread);
    int flush_file(dsec_t max_time, dsec_t d_time, bool on_terminate, CFlowGenListPerThread *thread,
                   double &old_offset);
     ...
     */
    void set_packet_limit(uint64_t limit) { m_limit = limit; }
...
#define UPDATE_STATS(a)
    ...
    inline int flush_one_node_to_file(CGenNode *node) { return (m_v_if->send_node(node)); }
    int update_stl_stats(CGenNodeStateless *node_sl);
    bool has_limit_reached();

    FORCE_NO_INLINE bool handle_slow_messages(uint8_t type, CGenNode *node, CFlowGenListPerThread *thread,
                                              bool on_terminate);

  private:
    void add_exit_node(CFlowGenListPerThread *thread, dsec_t max_time);

    inline bool handle_stl_node(CGenNode *node, CFlowGenListPerThread *thread);

    FORCE_INLINE bool do_work_stl(CGenNode *node, CFlowGenListPerThread *thread, bool on_terminate);

    template <bool ON_TERMINATE>
    FORCE_INLINE bool do_work_both(CGenNode *node, CFlowGenListPerThread *thread, dsec_t d_time);

    template <int SCH_MODE, bool ON_TERMINATE>
    FORCE_INLINE bool do_work(CGenNode *node, CFlowGenListPerThread *thread, dsec_t d_time);

    FORCE_INLINE void do_sleep(dsec_t &cur_time, CFlowGenListPerThread *thread, dsec_t ntime);

    FORCE_INLINE int teardown(CFlowGenListPerThread *thread, bool on_terminate, double &old_offset, double offset);

    template <int SCH_MODE, bool ON_TERIMATE>
    int flush_file_realtime(dsec_t max_time, dsec_t d_time, CFlowGenListPerThread *thread, double &old_offset);

    int flush_file_sim(dsec_t max_time, dsec_t d_time, bool always, CFlowGenListPerThread *thread, double &old_offset);

    FORCE_NO_INLINE void handle_slow_operations(sch_state_t &state, CGenNode *&node, dsec_t &cur_time, dsec_t &n_time,
                                                dsec_t &offset, CFlowGenListPerThread *thread);

    void handle_time_strech(CGenNode *&node, dsec_t &cur_time, dsec_t &n_time, dsec_t &offset,
...
class CPacketDescriptorPerDir {
  ...
  public:
    ...
    uint16_t GetMaxPkts(void) { return (m_max_dir_flow_pkts); }
    ...
    uint16_t GetPktNum(void) { return (m_dir_pkt_num); }
...
class CPacketDescriptor {
  ...
  public:
    ...
    inline uint8_t getPluginId() { return (m_plugin_id); }
    inline void SetPluginId(uint8_t plugin_id) { m_plugin_id = plugin_id; }

    inline bool IsLearn() { return (btGetMaskBit32(m_flags, LEARN_MODE_ENABLE, LEARN_MODE_ENABLE) ? true : false); }
    inline void SetLearn(bool enable) { btSetMaskBit32(m_flags, LEARN_MODE_ENABLE, LEARN_MODE_ENABLE, enable ? 1 : 0); }

    inline bool IsPluginEnable() { return (btGetMaskBit32(m_flags, PLUGIN_ENABLE_S, PLUGIN_ENABLE_S) ? true : false); }
    ...
    inline void SetFlowPktNum(uint32_t pkt_id) { m_flow_pkt_num = pkt_id; }
    /**
     ...
     * @return
     */
    inline uint32_t getFlowPktNum() { return (m_flow_pkt_num); }

    inline void SetFlowId(uint16_t flow_id) { btSetMaskBit32(m_flags, 12, 8, flow_id); }

    inline uint16_t getFlowId() { return ((uint16_t)btGetMaskBit32(m_flags, 12, 8)); }

    inline void SetPcapTiming(bool is_pcap) {
        btSetMaskBit32(m_flags, IS_PCAP_TIMING, IS_PCAP_TIMING, is_pcap ? 1 : 0);
    }
    inline bool IsPcapTiming() { return (btGetMaskBit32(m_flags, IS_PCAP_TIMING, IS_PCAP_TIMING) ? true : false); }
    ...
    inline bool IsRtt() { return (btGetMaskBit32(m_flags, IS_RTT, IS_RTT) ? true : false); }
    inline void SetRtt(bool is_rtt) { btSetMaskBit32(m_flags, IS_RTT, IS_RTT, is_rtt ? 1 : 0); }

    /* this is in respect to the first flow  */
    inline bool IsInitSide() { return (btGetMaskBit32(m_flags, IS_INIT_SIDE, IS_INIT_SIDE) ? true : false); }
    ...
    /* per flow */
    inline bool IsSwapTuple() { return (btGetMaskBit32(m_flags, IS_SWAP_S, IS_SWAP_E) ? true : false); }
    inline void SetSwapTuple(bool is_swap) { btSetMaskBit32(m_flags, IS_SWAP_S, IS_SWAP_E, is_swap ? 1 : 0); }

    inline bool IsValidPkt() { return (btGetMaskBit32(m_flags, IS_VALID_S, IS_VALID_E) ? true : false); }

    inline void SetIsValidPkt(bool is_valid) { btSetMaskBit32(m_flags, IS_VALID_S, IS_VALID_E, is_valid ? 1 : 0); }

    inline void SetIsTcp(bool is_valid) { btSetMaskBit32(m_flags, PROTO_S, PROTO_E, is_valid ? 1 : 0); }

    inline bool IsTcp() { return ((btGetMaskBit32(m_flags, PROTO_S, PROTO_E) == 1) ? true : false); }

    inline void SetIsUdp(bool is_valid) { btSetMaskBit32(m_flags, PROTO_S, PROTO_E, is_valid ? 2 : 0); }

    inline bool IsUdp() { return ((btGetMaskBit32(m_flags, PROTO_S, PROTO_E) == 2) ? true : false); }

    inline void SetIsIcmp(bool is_valid) { btSetMaskBit32(m_flags, PROTO_S, PROTO_E, is_valid ? 3 : 0); }

    inline bool IsIcmp() { return ((btGetMaskBit32(m_flags, PROTO_S, PROTO_E) == 3) ? true : false); }

    inline void SetId(uint16_t _id) { btSetMaskBit32(m_flags, FLOW_ID_E, FLOW_ID_S, _id); }

    inline uint16_t getId() { return ((uint16_t)btGetMaskBit32(m_flags, FLOW_ID_E, FLOW_ID_S)); }

    inline void SetIsLastPkt(bool is_last) { btSetMaskBit32(m_flags, IS_LAST_PKT_S, IS_LAST_PKT_E, is_last ? 1 : 0); }

    /* last packet of couple of flows */
    inline bool IsLastPkt() { return (btGetMaskBit32(m_flags, IS_LAST_PKT_S, IS_LAST_PKT_E) ? true : false); }

    inline bool isKeepDstIP() { return btGetMaskBit32(m_flags2, KEEP_DST_IP, KEEP_DST_IP); }

    inline void setKeepDstIP(bool is_valid) { btSetMaskBit32(m_flags2, KEEP_DST_IP, KEEP_DST_IP, is_valid ? 1 : 0); }

    inline bool isKeepSrcIP() { return btGetMaskBit32(m_flags2, KEEP_SRC_IP, KEEP_SRC_IP); }

    inline void setKeepSrcIP(bool is_valid) { btSetMaskBit32(m_flags2, KEEP_SRC_IP, KEEP_SRC_IP, is_valid ? 1 : 0); }

    inline bool isSwapDir() { return btGetMaskBit32(m_flags2, SWAP_FLOW_DIR, SWAP_FLOW_DIR); }

    inline void setSwapDir(bool is_valid) { btSetMaskBit32(m_flags2, SWAP_FLOW_DIR, SWAP_FLOW_DIR, is_valid ? 1 : 0); }
    ...
    inline uint16_t GetMaxPktsPerFlow() { return (m_max_flow_pkts); }
    // there could be couple of flows per template in case of plugin
    ...
    inline uint16_t GetMaxFlowTimeout(void) { return (m_max_flow_aging); }

    /* return per dir info , the dir is with respect to the first flow client/server side , this is tricky */
    CPacketDescriptorPerDir *GetDirInfo(void) { return (&m_per_dir[IsInitSide() ? CLIENT_SIDE : SERVER_SIDE]); }

    bool IsOneDirectionalFlow(void) {
        if ((m_per_dir[CLIENT_SIDE].GetMaxPkts() == GetMaxPktsPerFlow()) ||
            (m_per_dir[SERVER_SIDE].GetMaxPkts() == GetMaxPktsPerFlow())) {
            return (true);
        } else {
            return (false);
        }
...
class CPacketIndication {
  ...
  public:
    ...
    void Clean();
    bool ConvertPacketToIpv6InPlace(CCapPktRaw *pkt, int offset);
  ...
  public:
    bool is_ipv6() { return (m_is_ipv6); }
    char *getBasePtr() { return ((char *)m_packet->raw); }
    ...
    uint8_t getFastEtherOffset(void) { return (m_ether_offset); }
    uint8_t getFastIpOffsetFast(void) { return (m_ip_offset); }
    uint8_t getFastTcpOffset(void) { return (m_udp_tcp_offset); }
    uint8_t getFastPayloadOffset(void) { return (m_payload_offset); }

    /* first r/w mbuf size could be 64 /128 */
    uint8_t get_rw_mbuf_size() { return (m_rw_mbuf_size); }

    /* zero mean there is no mbuf */
    uint16_t get_cons_mbuf_size() { return (m_ro_mbuf_size); }

    /* this is the size of the packet to append, could be smaller than rw_mbuf */
    uint8_t get_rw_pkt_size() { return (m_packet->pkt_len > m_rw_mbuf_size) ? m_rw_mbuf_size : m_packet->pkt_len; }

  private:
    void SetKey(void);
    uint8_t ProcessIpPacketProtocol(CCPacketParserCounters *m_cnt, uint8_t protocol, int *offset);
...
class CPacketParser {
  ...
  public:
    bool Create();
    void Delete();
    bool ProcessPacket(CPacketIndication *pkt_indication, CCapPktRaw *raw_packet);
...
class CFlowPktInfo {
  public:
    ...
    void do_generate_new_mbuf_rxcheck(rte_mbuf_t *m, CGenNode *node, bool single_port);
    ...
    inline rte_mbuf_t *do_generate_new_mbuf_ex_vm(CGenNode *node, CFlowInfo *flow_info, int16_t *s_size);
  ...
  public:
    /* push the number of bytes into the packets and make more room
      ...
      this function should not be called in runtime, only when template is loaded due to it heavey cost of operation (
      malloc/free memory )
    */
  ...
  private:
    inline void append_big_mbuf(rte_mbuf_t *m, CGenNode *node);

    inline void update_pkt_info(char *p, CGenNode *node);

    inline void update_pkt_info2(char *p, CFlowInfo *flow_info, int update_len, CGenNode *node);

    inline void update_tcp_cs(TCPHeader *tcp, IPHeader *ipv4);

    inline void update_udp_cs(UDPHeader *udp, IPHeader *ipv4);
    ...
    rte_mbuf_t *get_big_mbuf(socket_id_t socket_id) { return (m_big_mbuf[socket_id]); }
...
inline void CFlowPktInfo::update_pkt_info2(char *p, CFlowInfo *flow_info, int update_len, CGenNode *node) {
    IPHeader *ipv4 = (IPHeader *)(p + m_pkt_indication.getFastIpOffsetFast());

    EthernetHeader *et = (EthernetHeader *)(p + m_pkt_indication.getFastEtherOffset());
...
inline void CFlowPktInfo::update_tcp_cs(TCPHeader *tcp, IPHeader *ipv4) {

    if (CGlobalInfo::m_options.preview.getChecksumOffloadEnable()) {
        /* set pseudo-header checksum */
        if (m_pkt_indication.is_ipv6()) {
            tcp->setChecksumRaw(
                rte_ipv6_phdr_cksum((struct ipv6_hdr *)ipv4->getPointer(), PKT_TX_IPV6 | PKT_TX_TCP_CKSUM));
        } else {
...
inline void CFlowPktInfo::update_udp_cs(UDPHeader *udp, IPHeader *ipv4) {

    if (CGlobalInfo::m_options.preview.getChecksumOffloadEnable()) {
        /* set pseudo-header checksum */
        if (m_pkt_indication.is_ipv6()) {
            udp->setChecksumRaw(
                rte_ipv6_phdr_cksum((struct ipv6_hdr *)ipv4->getPointer(), PKT_TX_IPV6 | PKT_TX_UDP_CKSUM));
        } else {
...
inline void CFlowPktInfo::update_pkt_info(char *p, CGenNode *node) {

    uint8_t ip_offset = m_pkt_indication.getFastIpOffsetFast();
    IPHeader *ipv4 = (IPHeader *)(p + ip_offset);
        ...
        if (unlikely(CGlobalInfo::is_learn_mode())) {
            if (m_pkt_indication.m_desc.IsLearn()) {
                ...
                if (CGlobalInfo::is_learn_mode(CParserOption::LEARN_MODE_IP_OPTION)) {
                    ...
                    if (ipv4->getProtocol() == IPPROTO_TCP) {
                        ...
                        if (tcp->getSynFlag()) {
                            tcp->setAckNumber(
                                CNatRxManager::calc_tcp_ack_val(node->get_short_fid(), node->get_thread_id()));
                        }
#ifdef NAT_TRACE_
                        printf(" %.3f : flow_id: %x thread_id %x TCP ack %x seq %x\n", now_sec(), node->get_short_fid(),
                               node->get_thread_id(), tcp->getAckNumber(), tcp->getSeqNumber());
#endif
                    } else {
            ...
            if (m_pkt_indication.m_desc.IsInitSide()) {
                ...
                if (node->m_flags != CGenNode::NODE_FLAGS_LATENCY) {
                    printf(" %.3f : DP : i %x:%x -> %x  flow_id: %lx\n", now_sec(), node->m_src_ip, node->m_src_port,
                           node->m_dest_ip, node->m_flow_id);
                }
#endif
...
#ifdef NAT_TRACE_
                if (node->m_flags != CGenNode::NODE_FLAGS_LATENCY) {
                    printf(" %.3f : r %x   -> %x:%x  flow_id: %lx \n", now_sec(), node->m_dest_ip, node->m_src_ip,
                           node->m_src_port, node->m_flow_id);
                }
#endif
...
inline rte_mbuf_t *CFlowPktInfo::do_generate_new_mbuf_ex(CGenNode *node, CFlowInfo *flow_info) {
    rte_mbuf_t *m;
...
inline rte_mbuf_t *CFlowPktInfo::do_generate_new_mbuf_ex_big(CGenNode *node, CFlowInfo *flow_info) {
    rte_mbuf_t *m;
...
inline rte_mbuf_t *CFlowPktInfo::do_generate_new_mbuf_ex_vm(CGenNode *node, CFlowInfo *flow_info, int16_t *s_size) {
    rte_mbuf_t *m;
...
inline void CFlowPktInfo::append_big_mbuf(rte_mbuf_t *m, CGenNode *node) {
...
class CCapFileFlowInfo {
  public:
    ...
    void Delete();
    void clear(void);
    uint64_t Size(void) { return (m_flow_pkts.size()); }
    ...
    void generate_flow(CTupleTemplateGeneratorSmart *tuple_gen, CNodeGenerator *gen, dsec_t time, uint64_t flow_id,
                       CFlowYamlInfo *template_info, CGenNode *node);

    inline uint64_t get_total_bytes() { return (m_total_bytes); }
    inline uint64_t get_total_flows() { return (m_total_flows); }

    inline uint64_t get_total_errors() { return (m_total_errors); }
...
struct CFlowsYamlInfo {
  ...
  public:
    bool is_any_template() { return (m_vec.size() > 1 ? true : false); }
    void set_astf_mode();
    void Dump(FILE *fd);
    int load_from_yaml_file(std::string file_name);
    bool verify_correctness(uint32_t num_threads);
    bool is_any_plugin_configured() { return (m_is_plugin_configured); }
};
...
class CFlowStats {
  public:
    ...
    double get_normal_cps() { return (m_cps * (double)m_flows); }
...
class CFlowGeneratorRecPerThread {
  ...
  public:
    bool Create(CTupleGeneratorSmart *global_gen, CFlowYamlInfo *info, CFlowsYamlInfo *yaml_flow_info,
                CCapFileFlowInfo *flow_info, uint16_t _id, uint32_t thread_id);
    void Delete();

  public:
    void Dump(FILE *fd);
    inline void generate_flow(CNodeGenerator *gen, dsec_t time, uint64_t flow_id, CGenNode *node);
...
class CFlowGeneratorRec {
  ...
  public:
    bool Create(CFlowYamlInfo *info, CFlowsYamlInfo *flow_info, uint16_t _id);
    void Delete();
...
class CFlowGenListPerThread {
  ...
  public:
    ...
    bool Create(uint32_t thread_id, CFlowGenList *flow_list, uint32_t max_threads);
    void Delete();

    uint32_t get_max_active_flows_per_core_tcp();

    void set_terminate_mode(bool is_terminate) { m_terminated_by_master = is_terminate; }
    bool is_terminated_by_master() { return (m_terminated_by_master); }

    void set_vif(CVirtualIF *v_if) { m_node_gen.set_vif(v_if); }

    void set_sync_barrier(CSyncBarrier *sync_b) { m_sync_b = sync_b; }
    CSyncBarrier *get_sync_b() { return (m_sync_b); }

    void flush_tx_queue() { m_node_gen.m_v_if->flush_tx_queue(); }

    void tickle() { m_monitor.tickle(); }

    template <bool TEARDOWN> inline void on_flow_tick(CGenNode *node);
  ...
  public:
    ...
    void Update(void) { m_cpu_cp_u.Update(); }
    double getCpuUtil(void) { return (m_cpu_cp_u.GetVal()); }

    bool check_msgs();

    TrexDpCore *get_dp_core() { return m_dp_core; }
  ...
  private:
    ...
    void defer_client_port_free(bool is_tcp, uint32_t c_ip, uint16_t port, pool_index_t pool_idx,
...
inline void CFlowGeneratorRecPerThread::generate_flow(CNodeGenerator *gen, dsec_t time, uint64_t flow_id,
                                                      CGenNode *node) {

    m_flow_info->generate_flow(&tuple_gen, gen, time, flow_id, m_info, node);
}

inline bool CGenNode::is_responder_pkt() { return (m_pkt_info->m_pkt_indication.m_desc.IsInitSide() ? false : true); }

inline bool CGenNode::is_initiator_pkt() { return (m_pkt_info->m_pkt_indication.m_desc.IsInitSide() ? true : false); }

inline uint16_t CGenNode::get_template_id() { return (m_pkt_info->m_pkt_indication.m_desc.getId()); }

inline bool CGenNode::is_last_in_flow() { return (m_pkt_info->m_pkt_indication.m_desc.IsLastPkt()); }

inline bool CGenNode::is_repeat_flow() { return (m_template_info->m_limit_was_set); }
...
inline void CGenNode::reset_pkt_in_flow(void) { m_pkt_info = m_flow_info->GetPacket(0); }
...
class CPluginCallback {
  public:
    ...
    virtual void on_node_first(uint8_t plugin_id, CGenNode *node, CFlowYamlInfo *template_info,
                               CTupleTemplateGeneratorSmart *tuple_gen, CFlowGenListPerThread *flow_gen) = 0;
...
inline pkt_dir_t CGenNode::cur_interface_dir() {
    ...
    bool init_from_server =
        (get_is_initiator_start_from_server() || get_is_initiator_start_from_server_with_server_addr());
    bool is_init = lp->m_pkt_indication.m_desc.IsInitSide() ^ init_from_server;
