...
void CFlowPktInfo::do_generate_new_mbuf_rxcheck(rte_mbuf_t *m, CGenNode *node, bool single_port) {
    ...
    if (unlikely(m_pkt_indication.is_ipv6())) {
        ...
        ipv6->setPayloadLen(ipv6->getPayloadLen() + sizeof(CRx_check_header));
    ...
    rxhdr->m_flow_id =
        node->m_flow_id | (((uint64_t)(desc->getFlowId() & 0xf))
                           << 52); // include thread_id, node->flow_id, sub_flow in case of multi-flow template
    rxhdr->m_flags = 0;
    ...
    if (likely(!m_pkt_indication.is_ipv6())) {
        if (CGlobalInfo::m_options.preview.getChecksumOffloadEnable()) {
            ...
            ipv4->updateCheckSum2((uint8_t *)ipv4, IPHeader::DefaultSize, (uint8_t *)rxhdr,
                                  old_ipv4_header_len + opt_len - IPHeader::DefaultSize);
        }
    }
