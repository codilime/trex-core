...
class IPHeader {
  ...
  public:
    IPHeader() {
        myVer_HeaderLength =
            0; // must initialize it cause when doing setLenght or setVersion we mask it (found in purify)
        setDestIp(0xDEADBEEF);
        setSourceIp(0xBEEFDEAD);
        setProtocol(0xDD);
    };

    IPHeader(uint32_t argSource, uint32_t argDestinaction, uint8_t argProtocol) {
  ...
  public:
    ...
    inline uint16_t getFirstWord() { return PKT_NTOHS(*((uint16_t *)&myVer_HeaderLength)); }

    inline void setFirstWord(uint16_t word) { *((uint16_t *)&myVer_HeaderLength) = PKT_NTOHS(word); }
    ...
    inline void setFragment(uint16_t argOffset, bool argMoreFrag, bool argDontFrag);
  ...
  public:
    ...
    inline void ClearCheckSum() { myChecksum = 0; }
    inline void SetCheckSumRaw(uint16_t Checksum) { myChecksum = Checksum; }
