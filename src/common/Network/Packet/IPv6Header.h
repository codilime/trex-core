...
static uint16_t default_ipv6[8] = {0xDEAD, 0xBEEF, 0xDEAD, 0xBEEF, 0xDEAD, 0xBEEF, 0xDEAD, 0xBEEF};
...
//--------------------------------
inline uint8_t IPv6Header::getVersion() { return getMaskBit32(PKT_NTOHL(myVer_TrafficClass_FlowLabel), 0, 3); }
...
//--------------------------------
inline uint16_t IPv6Header::getTrafficClass() { return getMaskBit32(PKT_NTOHL(myVer_TrafficClass_FlowLabel), 4, 11); }

inline void IPv6Header::setTrafficClass(uint16_t argTrafficClass) {
...
//--------------------------------
inline uint32_t IPv6Header::getFlowLabel() { return getMaskBit32(PKT_NTOHL(myVer_TrafficClass_FlowLabel), 12, 31); }
...
inline uint32_t IPv6Header::getSourceIpv6LSB() { return (PKT_NTOHL(*((uint32_t *)&mySource[6]))); }

inline uint32_t IPv6Header::getDestIpv6LSB() { return (PKT_NTOHL(*((uint32_t *)&myDestination[6]))); }

//--------------------------------
inline uint16_t IPv6Header::getPayloadLen() { return PKT_NTOHS(myPayloadLen); }

inline void IPv6Header::setPayloadLen(uint16_t argPayloadLen) { myPayloadLen = PKT_HTONS(argPayloadLen); }

//--------------------------------
inline uint8_t IPv6Header::getNextHdr() { return myNextHdr; }

inline void IPv6Header::setNextHdr(uint8_t argNextHdr) { myNextHdr = argNextHdr; }

//--------------------------------
inline uint8_t IPv6Header::getHopLimit() { return myHopLimit; }

inline void IPv6Header::setHopLimit(uint8_t argHopLimit) { myHopLimit = argHopLimit; }
