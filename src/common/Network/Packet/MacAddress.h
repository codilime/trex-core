...
class MacAddress {
  public:
    MacAddress() { set(0xff, 0xff, 0xff, 0xff, 0xff, 0xff); };

    MacAddress(uint8_t a0, uint8_t a1, uint8_t a2, uint8_t a3, uint8_t a4, uint8_t a5) { set(a0, a1, a2, a3, a4, a5); };

    MacAddress(const uint8_t macAddr[ETHER_ADDR_LEN]) {
        set(macAddr[0], macAddr[1], macAddr[2], macAddr[3], macAddr[4], macAddr[5]);
    };

    void set(uint8_t a0, uint8_t a1, uint8_t a2, uint8_t a3, uint8_t a4, uint8_t a5) {
        data[0] = a0;
        data[1] = a1;
        data[2] = a2;
        data[3] = a3;
    ...
    void set(uint8_t *argPtr) { memcpy(data, argPtr, sizeof(data)); }
    ...
    void setIdentifierAsBogusAddr(uint32_t identifier) { *(uint32_t *)data = identifier; }

    uint32_t getIdentifierFromBogusAddr() { return *(uint32_t *)data; }
    ...
    const uint8_t *GetConstBuffer() const { return data; }
