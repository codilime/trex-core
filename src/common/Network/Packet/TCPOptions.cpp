...
TCPOptions::TCPOptions(uint8_t *argOptionsP, uint16_t argOptionsSize)
    : myOptionsP(argOptionsP), myOptionsSize(argOptionsSize), myCurrentOptionP((Option *)argOptionsP) {
    ;
}
...
void TCPOptions::dump(FILE *argOutputFile) { fprintf(argOutputFile, "\nNot supported yet!\n"); }
...
uint8_t TCPOptions::getCurrentOptionLength() {
    if (myCurrentOptionP->theKind == Kind::NO_OP || myCurrentOptionP->theKind == Kind::EOL) {
        return 1;
    } else {
