...
void utl_DumpChar(FILE *fd, void *src, unsigned int eln, unsigned int width) {
    int size = eln * width;
...
void utl_DumpBufferLine(FILE *fd, void *src, int offset, unsigned int eln, unsigned int width, unsigned int mask) {
    unsigned char *p = (unsigned char *)src;
    uint32 addr;
...
bool utl_str_to_macaddr(const std::string &s, uint8_t *mac) {
    int last = -1;
    int rc = sscanf(s.c_str(), "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx%n", mac + 0, mac + 1, mac + 2, mac + 3, mac + 4, mac + 5,
                    &last);
...
std::string utl_uint32_to_ipv4(uint32_t ipv4_addr) {
    std::stringstream ss;
    ss << ((ipv4_addr >> 24) & 0xff) << "." << ((ipv4_addr >> 16) & 0xff) << "." << ((ipv4_addr >> 8) & 0xff) << "."
       << (ipv4_addr & 0xff);
    return ss.str();
}
