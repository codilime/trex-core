...
typedef enum capture_type { LIBPCAP, ERF, LAST_TYPE } capture_type_e;

#define MIN_PKT_SIZE (14)
...
#define _NSEC_TO_SEC 1000000000.0
    ...
    double getNsec() const { return ((double)m_time_sec + (double)m_time_nsec / (_NSEC_TO_SEC)); }

    double diff(const CPktNsecTimeStamp &obj) { return (std::abs(getNsec() - obj.getNsec())); }
...
class CCapPktRaw {
  ...
  public:
    ...
    void setInterface(uint8_t _if) { btSetMaskBit16(flags, 10, 8, _if); }

    uint8_t getInterface() { return ((uint8_t)btGetMaskBit16(flags, 10, 8)); }

    void setDoNotFree(bool do_not_free) { btSetMaskBit16(flags, 0, 0, do_not_free ? 1 : 0); }

    bool getDoNotFree() { return ((btGetMaskBit16(flags, 0, 0) ? true : false)); }

    bool Compare(CCapPktRaw *obj, int dump, double dsec);

  public:
    inline uint16_t getTotalLen(void) { return (pkt_len); }
