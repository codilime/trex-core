...
void *Page_Create(size_t size) {
     ...
     */
    allocation = (caddr_t)mmap(startAddr, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
void *Page_Create(size_t size) {
     ...
     * generated by other software, etc.
     */
    allocation = (caddr_t)mmap(startAddr, size, PROT_READ | PROT_WRITE, MAP_PRIVATE, devZeroFd, 0);
...
static void mprotectFailed(void) { EF_Exit("mprotect() failed: %s", stringErrorReport()); }
...
void Page_Delete(void *address, size_t size) { Page_DenyAccess(address, size); }

#if defined(_SC_PAGESIZE)
size_t Page_Size(void) { return (size_t)sysconf(_SC_PAGESIZE); }
#elif defined(_SC_PAGE_SIZE)
size_t Page_Size(void) { return (size_t)sysconf(_SC_PAGE_SIZE); }
#else
/* extern int	getpagesize(); */
size_t Page_Size(void) { return getpagesize(); }
#endif
