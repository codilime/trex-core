...
static void prob_conv_fix_size(std::vector<double> prob, std::vector<u32> &result) {
    double sum = 0.0;
    for (auto &n : prob)
        sum += n;
    result.clear();
...
static void computeBiDist(std::vector<u32> &p, u32 n, Distribution &dist, u32 aIdx, u32 bIdx);

// Since we are using fixed point math, we first implement
// a fixed 0.32 point inversion:  1/(n-1)
#define DIST_INV(n) (0xFFFFFFFF / ((n)-1))

KxuNuRand::KxuNuRand(const std::vector<double> prob, KxuRandUniform *rand) {
...
void KxuNuRand::init(const std::vector<u32> &dist, KxuRandUniform *rand) {
    mRand = rand;
...
KxuNuRand::KxuNuRand(const std::vector<u32> &dist, KxuRandUniform *rand) { init(dist, rand); }
