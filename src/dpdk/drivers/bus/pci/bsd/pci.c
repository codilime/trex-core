...
int pci_uio_alloc_resource(struct rte_pci_device *dev, struct mapped_pci_resource **uio_res) {
    ...
    snprintf(devname, sizeof(devname), "/dev/uio@pci:%u:%u:%u", dev->addr.bus, dev->addr.devid, dev->addr.function);
...
int pci_uio_map_resource_by_index(struct rte_pci_device *dev, int res_idx, struct mapped_pci_resource *uio_res,
    ...
    mapaddr = pci_map_resource(NULL, fd, (off_t)offset, (size_t)dev->mem_resource[res_idx].len, 0);
    close(fd);
...
static int pci_scan_one(int dev_pci_fd, struct pci_conf *conf) {
    ...
    /* get class id */
    dev->id.class_id = (conf->pc_class << 16) | (conf->pc_subclass << 8) | (conf->pc_progif);
...
int rte_pci_read_config(const struct rte_pci_device *dev, void *buf, size_t len, off_t offset) {
    int fd = -1;
    int size;
...
int rte_pci_write_config(const struct rte_pci_device *dev, const void *buf, size_t len, off_t offset) {
    int fd = -1;
...
static void pci_uio_ioport_read(struct rte_pci_ioport *p, void *data, size_t len, off_t offset) {
#if defined(RTE_ARCH_X86)
    uint8_t *d;
    int size;
...
static void pci_uio_ioport_write(struct rte_pci_ioport *p, const void *data, size_t len, off_t offset) {
#if defined(RTE_ARCH_X86)
    const uint8_t *s;
    int size;
    unsigned short reg = p->base + offset;
