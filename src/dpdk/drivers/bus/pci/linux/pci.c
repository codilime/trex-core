...
static int pci_get_kernel_driver_by_path(const char *filename, char *dri_name, size_t len) {
    int count;
    char path[PATH_MAX];
    char *name;
 ...
 * string is modified)
 */
int pci_parse_one_sysfs_resource(char *line, size_t len, uint64_t *phys_addr, uint64_t *end_addr, uint64_t *flags) {
...
static int pci_parse_sysfs_resource(const char *filename, struct rte_pci_device *dev) {
    FILE *f;
    char buf[BUFSIZ];
    int i;
    ...
    for (i = 0; i < PCI_MAX_RESOURCE; i++) {
        ...
        if (pci_parse_one_sysfs_resource(buf, sizeof(buf), &phys_addr, &end_addr, &flags) < 0)
            goto error;
...
int pci_update_device(const struct rte_pci_addr *addr) {
    ...
    snprintf(filename, sizeof(filename), "%s/" PCI_PRI_FMT, rte_pci_get_sysfs_path(), addr->domain, addr->bus,
 ...
 */
static int parse_pci_addr_format(const char *buf, int bufsize, struct rte_pci_addr *addr) {
    /* first split on ':' */
    union splitaddr {
        struct {
            char *domain;
 ...
 */
static inline int pci_one_device_is_bound(void) {
    struct rte_pci_device *dev = NULL;
    int ret = 0;

    FOREACH_DEVICE_ON_PCIBUS(dev) {
        if (dev->kdrv == RTE_KDRV_UNKNOWN || dev->kdrv == RTE_KDRV_NONE) {
            continue;
        } else {
            ret = 1;
            break;
 ...
 */
static inline int pci_one_device_bound_uio(void) {
    struct rte_pci_device *dev = NULL;
    ...
    FOREACH_DEVICE_ON_PCIBUS(dev) {
        ...
        if (dev->kdrv == RTE_KDRV_IGB_UIO || dev->kdrv == RTE_KDRV_UIO_GENERIC) {
            return 1;
        }
    }
    return 0;
 ...
 */
static inline int pci_one_device_has_iova_va(void) {
    struct rte_pci_device *dev = NULL;
    ...
    FOREACH_DRIVER_ON_PCIBUS(drv) {
        if (drv && drv->drv_flags & RTE_PCI_DRV_IOVA_AS_VA) {
            FOREACH_DEVICE_ON_PCIBUS(dev) {
                if (dev->kdrv == RTE_KDRV_VFIO && rte_pci_match(drv, dev))
                    return 1;
            }
        }
    }
...
static bool pci_one_device_iommu_support_va(struct rte_pci_device *dev) {
...
#define X86_VA_WIDTH 47 /* From Documentation/x86/x86_64/mm.txt */
    ...
    snprintf(filename, sizeof(filename), "%s/" PCI_PRI_FMT "/iommu/intel-iommu/cap", rte_pci_get_sysfs_path(),
...
static bool pci_one_device_iommu_support_va(__rte_unused struct rte_pci_device *dev) { return false; }
#else
static bool pci_one_device_iommu_support_va(__rte_unused struct rte_pci_device *dev) { return true; }
#endif

/*
 * All devices IOMMUs support VA as IOVA
 */
static bool pci_devices_iommu_support_va(void) {
    struct rte_pci_device *dev = NULL;
    struct rte_pci_driver *drv = NULL;
...
#ifdef VFIO_PRESENT
    is_vfio_noiommu_enabled = rte_vfio_noiommu_is_enabled() == true ? true : false;
#endif

    if (has_iova_va && !is_bound_uio && !is_vfio_noiommu_enabled && !iommu_no_va)
        return RTE_IOVA_VA;
...
#if defined(RTE_ARCH_X86)
static int pci_ioport_map(struct rte_pci_device *dev, int bar __rte_unused, struct rte_pci_ioport *p) {
    uint16_t start, end;
    FILE *fp;
    char *line = NULL;
    char pci_id[16];
    ...
    snprintf(pci_id, sizeof(pci_id), PCI_PRI_FMT, dev->addr.domain, dev->addr.bus, dev->addr.devid, dev->addr.function);
