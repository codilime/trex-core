...
int pci_vfio_read_config(const struct rte_intr_handle *intr_handle, void *buf, size_t len, off_t offs) {
    return pread64(intr_handle->vfio_dev_fd, buf, len, VFIO_GET_REGION_ADDR(VFIO_PCI_CONFIG_REGION_INDEX) + offs);
}

int pci_vfio_write_config(const struct rte_intr_handle *intr_handle, const void *buf, size_t len, off_t offs) {
    return pwrite64(intr_handle->vfio_dev_fd, buf, len, VFIO_GET_REGION_ADDR(VFIO_PCI_CONFIG_REGION_INDEX) + offs);
}
...
static int pci_vfio_get_msix_bar(int fd, struct pci_msix_table *msix_table) {
    ...
    ret = pread64(fd, &reg, sizeof(reg), VFIO_GET_REGION_ADDR(VFIO_PCI_CONFIG_REGION_INDEX) + PCI_CAPABILITY_LIST);
    if (ret != sizeof(reg)) {
        RTE_LOG(ERR, EAL,
    ...
    while (cap_offset) {
        ...
        ret = pread64(fd, &reg, sizeof(reg), VFIO_GET_REGION_ADDR(VFIO_PCI_CONFIG_REGION_INDEX) + cap_offset);
        if (ret != sizeof(reg)) {
            RTE_LOG(ERR, EAL,
        ...
        /* else, read table offset */
            ...
            ret = pread64(fd, &reg, sizeof(reg), VFIO_GET_REGION_ADDR(VFIO_PCI_CONFIG_REGION_INDEX) + cap_offset + 4);
            if (ret != sizeof(reg)) {
                RTE_LOG(ERR, EAL,
            ...
            ret =
                pread64(fd, &flags, sizeof(flags), VFIO_GET_REGION_ADDR(VFIO_PCI_CONFIG_REGION_INDEX) + cap_offset + 2);
            if (ret != sizeof(flags)) {
                RTE_LOG(ERR, EAL,
...
static int pci_vfio_set_bus_master(int dev_fd, bool op) {
    ...
    ret = pread64(dev_fd, &reg, sizeof(reg), VFIO_GET_REGION_ADDR(VFIO_PCI_CONFIG_REGION_INDEX) + PCI_COMMAND);
    if (ret != sizeof(reg)) {
    ...
    ret = pwrite64(dev_fd, &reg, sizeof(reg), VFIO_GET_REGION_ADDR(VFIO_PCI_CONFIG_REGION_INDEX) + PCI_COMMAND);
...
static int pci_vfio_setup_interrupts(struct rte_pci_device *dev, int vfio_dev_fd) {
    int i, ret, intr_idx;
...
static int pci_vfio_enable_notifier(struct rte_pci_device *dev, int vfio_dev_fd) {
    int ret;
    int fd = -1;
    ...
    if (ret) {
        ...
        ret = rte_intr_callback_unregister(&dev->vfio_req_intr_handle, pci_vfio_req_handler, (void *)&dev->device);
        if (ret < 0)
...
static int pci_vfio_is_ioport_bar(int vfio_dev_fd, int bar_index) {
    ...
    ret = pread64(vfio_dev_fd, &ioport_bar, sizeof(ioport_bar),
                  VFIO_GET_REGION_ADDR(VFIO_PCI_CONFIG_REGION_INDEX) + PCI_BASE_ADDRESS_0 + bar_index * 4);
...
static int pci_rte_vfio_setup_device(struct rte_pci_device *dev, int vfio_dev_fd) {
...
static int pci_vfio_mmap_bar(int vfio_dev_fd, struct mapped_pci_resource *vfio_res, int bar_index,
    ...
    bar_addr = mmap(bar->addr, bar->size, 0, MAP_PRIVATE | MAP_ANONYMOUS | additional_flags, -1, 0);
    if (bar_addr != MAP_FAILED) {
        void *map_addr = NULL;
        if (memreg[0].size) {
            /* actual map of first part */
            map_addr = pci_map_resource(bar_addr, vfio_dev_fd, memreg[0].offset, memreg[0].size, MAP_FIXED);
        }

        /* if there's a second part, try to map it */
        if (map_addr != MAP_FAILED && memreg[1].offset && memreg[1].size) {
            void *second_addr = RTE_PTR_ADD(bar_addr, memreg[1].offset - (uintptr_t)bar->offset);
            map_addr = pci_map_resource(second_addr, vfio_dev_fd, memreg[1].offset, memreg[1].size, MAP_FIXED);
        }
 ...
 */
static int pci_vfio_get_region_info(int vfio_dev_fd, struct vfio_region_info **info, int region) {
...
static struct vfio_info_cap_header *pci_vfio_info_cap(struct vfio_region_info *info, int cap) {
...
static int pci_vfio_map_resource_primary(struct rte_pci_device *dev) {
    ...
    ret = rte_vfio_setup_device(rte_pci_get_sysfs_path(), pci_addr, &vfio_dev_fd, &device_info);
    if (ret)
        return ret;
    ...
    if (vfio_res == NULL) {
        RTE_LOG(ERR, EAL, "%s(): cannot store vfio mmap details\n", __func__);
        goto err_vfio_dev_fd;
    }
    memcpy(&vfio_res->pci_addr, &dev->addr, sizeof(vfio_res->pci_addr));

    /* get number of registers (up to BAR5) */
    vfio_res->nb_maps = RTE_MIN((int)device_info.num_regions, VFIO_PCI_BAR5_REGION_INDEX + 1);
    ...
    if (vfio_res->msix_table.bar_index != -1) {
        int ret = pci_vfio_msix_is_mappable(vfio_dev_fd, vfio_res->msix_table.bar_index);
        if (ret < 0) {
...
static struct mapped_pci_resource *find_and_unmap_vfio_resource(struct mapped_pci_res_list *vfio_res_list,
...
static int pci_vfio_unmap_resource_primary(struct rte_pci_device *dev) {
    ...
    ret = rte_vfio_release_device(rte_pci_get_sysfs_path(), pci_addr, dev->intr_handle.vfio_dev_fd);
    if (ret < 0) {
        RTE_LOG(ERR, EAL, "%s(): cannot release device\n", __func__);
        return ret;
    }
...
int pci_vfio_ioport_map(struct rte_pci_device *dev, int bar, struct rte_pci_ioport *p) {
...
void pci_vfio_ioport_read(struct rte_pci_ioport *p, void *data, size_t len, off_t offset) {
    const struct rte_intr_handle *intr_handle = &p->dev->intr_handle;

    if (pread64(intr_handle->vfio_dev_fd, data, len, p->base + offset) <= 0)
        RTE_LOG(ERR, EAL, "Can't read from PCI bar (%" PRIu64 ") : offset (%x)\n", VFIO_GET_REGION_IDX(p->base),
                (int)offset);
}

void pci_vfio_ioport_write(struct rte_pci_ioport *p, const void *data, size_t len, off_t offset) {
    const struct rte_intr_handle *intr_handle = &p->dev->intr_handle;

    if (pwrite64(intr_handle->vfio_dev_fd, data, len, p->base + offset) <= 0)
