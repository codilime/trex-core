 ...
 */
int rte_pci_match(const struct rte_pci_driver *pci_drv, const struct rte_pci_device *pci_dev) {
    const struct rte_pci_id *id_table;

    for (id_table = pci_drv->id_table; id_table->vendor_id != 0; id_table++) {
        /* check if device's identifiers match the driver's ones */
        if (id_table->vendor_id != pci_dev->id.vendor_id && id_table->vendor_id != PCI_ANY_ID)
            continue;
        if (id_table->device_id != pci_dev->id.device_id && id_table->device_id != PCI_ANY_ID)
            continue;
        ...
        if (id_table->subsystem_device_id != pci_dev->id.subsystem_device_id &&
            ...
            continue;
        if (id_table->class_id != pci_dev->id.class_id && id_table->class_id != RTE_CLASS_ANY_ID)
            continue;
...
static int pci_dump_one_device(FILE *f, struct rte_pci_device *dev) {
    ...
    fprintf(f, PCI_PRI_FMT, dev->addr.domain, dev->addr.bus, dev->addr.devid, dev->addr.function);
    fprintf(f, " - vendor:%x device:%x\n", dev->id.vendor_id, dev->id.device_id);

    for (i = 0; i != sizeof(dev->mem_resource) / sizeof(dev->mem_resource[0]); i++) {
...
void rte_pci_add_device(struct rte_pci_device *pci_dev) { TAILQ_INSERT_TAIL(&rte_pci_bus.device_list, pci_dev, next); }

/* Insert a device into a predefined position in PCI bus */
void rte_pci_insert_device(struct rte_pci_device *exist_pci_dev, struct rte_pci_device *new_pci_dev) {
...
static struct rte_device *pci_find_device(const struct rte_device *start, rte_dev_cmp_t cmp, const void *data) {
 ...
 */
static struct rte_pci_device *pci_find_device_by_addr(const void *failure_addr) {
    ...
    FOREACH_DEVICE_ON_PCIBUS(pdev) {
        for (i = 0; i != RTE_DIM(pdev->mem_resource); i++) {
            ...
            if (check_point >= start && check_point < end) {
                RTE_LOG(DEBUG, EAL, "Failure address %16.16" PRIx64 " belongs to device %s!\n", check_point,
                        pdev->device.name);
                return pdev;
            }
        }
...
static int pci_sigbus_handler(const void *failure_addr) {
    ...
    if (!pdev) {
        ...
        if (ret) {
            RTE_LOG(ERR, EAL, "Failed to handle hot-unplug for device %s", pdev->name);
            ret = -1;
        }
    }
    return ret;
...
static int pci_dma_map(struct rte_device *dev, void *addr, uint64_t iova, size_t len) {
    ...
    if (pdev->kdrv == RTE_KDRV_VFIO)
        return rte_vfio_container_dma_map(RTE_VFIO_DEFAULT_CONTAINER_FD, (uintptr_t)addr, iova, len);
    rte_errno = ENOTSUP;
    return -1;
}

static int pci_dma_unmap(struct rte_device *dev, void *addr, uint64_t iova, size_t len) {
    struct rte_pci_device *pdev = RTE_DEV_TO_PCI(dev);
     ...
     */
    if (pdev->kdrv == RTE_KDRV_VFIO)
        return rte_vfio_container_dma_unmap(RTE_VFIO_DEFAULT_CONTAINER_FD, (uintptr_t)addr, iova, len);
    rte_errno = ENOTSUP;
    return -1;
}
