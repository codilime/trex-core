...
static int pci_addr_kv_cmp(const char *key __rte_unused, const char *value, void *_addr2) {
...
static int pci_dev_match(const struct rte_device *dev, const void *_kvlist) {
    ...
    if (rte_kvargs_process(kvlist, pci_params_keys[RTE_PCI_PARAM_ADDR], &pci_addr_kv_cmp,
                           (void *)(intptr_t)&pdev->addr))
        return 1;
    return 0;
}

void *rte_pci_dev_iterate(const void *start, const char *str, const struct rte_dev_iterator *it __rte_unused) {
