...
static struct vdev_device_list vdev_device_list = TAILQ_HEAD_INITIALIZER(vdev_device_list);
/* The lock needs to be recursive because a vdev can manage another vdev. */
static rte_spinlock_recursive_t vdev_device_list_lock = RTE_SPINLOCK_RECURSIVE_INITIALIZER;

static struct vdev_driver_list vdev_driver_list = TAILQ_HEAD_INITIALIZER(vdev_driver_list);
...
static struct vdev_custom_scans vdev_custom_scans = TAILQ_HEAD_INITIALIZER(vdev_custom_scans);
static rte_spinlock_t vdev_custom_scan_lock = RTE_SPINLOCK_INITIALIZER;

/* register a driver */
void rte_vdev_register(struct rte_vdev_driver *driver) { TAILQ_INSERT_TAIL(&vdev_driver_list, driver, next); }

/* unregister a driver */
void rte_vdev_unregister(struct rte_vdev_driver *driver) { TAILQ_REMOVE(&vdev_driver_list, driver, next); }
...
int rte_vdev_remove_custom_scan(rte_vdev_scan_callback callback, void *user_arg) {
    ...
    TAILQ_FOREACH_SAFE(custom_scan, &vdev_custom_scans, next, tmp_scan) {
        if (custom_scan->callback != callback ||
            (custom_scan->user_arg != (void *)-1 && custom_scan->user_arg != user_arg))
            continue;
...
static int vdev_parse(const char *name, void *addr) {
    ...
    TAILQ_FOREACH(driver, &vdev_driver_list, next) {
        if (strncmp(driver->driver.name, name, strlen(driver->driver.name)) == 0)
            break;
        if (driver->driver.alias && strncmp(driver->driver.alias, name, strlen(driver->driver.alias)) == 0)
            break;
    }
...
static int insert_vdev(const char *name, const char *args, struct rte_vdev_device **p_dev, bool init) {
    struct rte_vdev_device *dev;
    struct rte_devargs *devargs;
    int ret;
...
static int vdev_remove_driver(struct rte_vdev_device *dev) {
    ...
    driver = container_of(dev->device.driver, const struct rte_vdev_driver, driver);
    ...
    case VDEV_SCAN_REQ:
        ...
        TAILQ_FOREACH(dev, &vdev_device_list, next) {
            ...
            if (rte_mp_sendmsg(&mp_resp) < 0)
                VDEV_LOG(ERR, "send vdev, %s, failed, %s", devname, strerror(rte_errno));
            num++;
        }
        rte_spinlock_recursive_unlock(&vdev_device_list_lock);
...
static int vdev_scan(void) {
    ...
    if (rte_mp_action_register(VDEV_MP_KEY, vdev_action) < 0 && rte_errno != EEXIST) {
...
static int vdev_probe(void) {
    ...
    TAILQ_FOREACH(dev, &vdev_device_list, next) {
        ...
        if (r != 0) {
            if (r == -EEXIST)
                continue;
            VDEV_LOG(ERR, "failed to initialize %s device", rte_vdev_device_name(dev));
            ret = -1;
        }
    }
...
struct rte_device *rte_vdev_find_device(const struct rte_device *start, rte_dev_cmp_t cmp, const void *data) {
...
static int vdev_plug(struct rte_device *dev) { return vdev_probe_all_drivers(RTE_DEV_TO_VDEV(dev)); }

static int vdev_unplug(struct rte_device *dev) { return rte_vdev_uninit(dev->name); }
