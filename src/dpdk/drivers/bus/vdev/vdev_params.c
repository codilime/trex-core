...
static int vdev_dev_match(const struct rte_device *dev, const void *_kvlist) {
    ...
    if (name == NULL)
        return -1;
    ret = rte_kvargs_process(kvlist, vdev_params_keys[RTE_VDEV_PARAM_NAME], rte_kvargs_strcmp, name);
    free(name);
    if (ret != 0)
        return -1;
...
void *rte_vdev_dev_iterate(const void *start, const char *str, const struct rte_dev_iterator *it __rte_unused) {
