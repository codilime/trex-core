...
#define PMD_LOG(level, fmt, args...) rte_log(RTE_LOG_##level, af_packet_logtype, "%s(): " fmt "\n", __func__, ##args)

static uint16_t eth_af_packet_rx(void *queue, struct rte_mbuf **bufs, uint16_t nb_pkts) {
    unsigned i;
 ...
 */
static uint16_t eth_af_packet_tx(void *queue, struct rte_mbuf **bufs, uint16_t nb_pkts) {
    struct tpacket2_hdr *ppd;
    struct rte_mbuf *mbuf;
    uint8_t *pbuf;
    unsigned int framecount, framenum;
    ...
    for (i = 0; i < nb_pkts; i++) {
        ...
        /* point at the next incoming frame */
        if ((ppd->tp_status != TP_STATUS_AVAILABLE) && (poll(&pfd, 1, -1) < 0))
            break;
...
static int eth_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *igb_stats) {
    ...
    imax = (internal->nb_queues < RTE_ETHDEV_QUEUE_STAT_CNTRS ? internal->nb_queues : RTE_ETHDEV_QUEUE_STAT_CNTRS);
    for (i = 0; i < imax; i++) {
...
static int eth_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id, uint16_t nb_rx_desc __rte_unused,
    ...
    if (data_size > buf_size) {
        PMD_LOG(ERR, "%s: %d bytes will not fit in mbuf (%d bytes)", dev->device->name, data_size, buf_size);
        return -ENOMEM;
    }
...
static void eth_dev_change_flags(char *if_name, uint32_t flags, uint32_t mask) {
    struct ifreq ifr;
    int s;
...
static void eth_dev_promiscuous_enable(struct rte_eth_dev *dev) {
    struct pmd_internals *internals = dev->data->dev_private;

    eth_dev_change_flags(internals->if_name, IFF_PROMISC, ~0);
}

static void eth_dev_promiscuous_disable(struct rte_eth_dev *dev) {
    struct pmd_internals *internals = dev->data->dev_private;
 ...
 */
static int open_packet_iface(const char *key __rte_unused, const char *value __rte_unused, void *extra_args) {
...
static int rte_pmd_init_internals(struct rte_vdev_device *dev, const int sockfd, const unsigned nb_queues,
                                  unsigned int blocksize, unsigned int blockcnt, unsigned int framesize,
                                  unsigned int framecnt, unsigned int qdisc_bypass, struct pmd_internals **internals,
    ...
    PMD_LOG(INFO, "%s: creating AF_PACKET-backed ethdev on numa socket %u", name, numa_node);
    ...
    *internals = rte_zmalloc_socket(name, sizeof(**internals), 0, numa_node);
    ...
    if (ioctl(sockfd, SIOCGIFINDEX, &ifr) == -1) {
        PMD_LOG(ERR, "%s: ioctl failed (SIOCGIFINDEX)", name);
        return -1;
    }
    ...
    if (ioctl(sockfd, SIOCGIFHWADDR, &ifr) == -1) {
        PMD_LOG(ERR, "%s: ioctl failed (SIOCGIFHWADDR)", name);
        return -1;
    }
    ...
    for (q = 0; q < nb_queues; q++) {
        ...
        if (qsockfd == -1) {
            PMD_LOG(ERR, "%s: could not open AF_PACKET socket", name);
            return -1;
        }
        ...
        if (rc == -1) {
            PMD_LOG(ERR, "%s: could not set PACKET_VERSION on AF_PACKET socket for %s", name, pair->value);
            goto error;
        }

        discard = 1;
        rc = setsockopt(qsockfd, SOL_PACKET, PACKET_LOSS, &discard, sizeof(discard));
        if (rc == -1) {
            PMD_LOG(ERR, "%s: could not set PACKET_LOSS on AF_PACKET socket for %s", name, pair->value);
            goto error;
        }

#if defined(PACKET_QDISC_BYPASS)
        rc = setsockopt(qsockfd, SOL_PACKET, PACKET_QDISC_BYPASS, &qdisc_bypass, sizeof(qdisc_bypass));
        if (rc == -1) {
            PMD_LOG(ERR, "%s: could not set PACKET_QDISC_BYPASS on AF_PACKET socket for %s", name, pair->value);
            goto error;
        }
#else
        ...
        if (rc == -1) {
            PMD_LOG(ERR, "%s: could not set PACKET_RX_RING on AF_PACKET socket for %s", name, pair->value);
            goto error;
        }
        ...
        rx_queue->map = mmap(NULL, 2 * req->tp_block_size * req->tp_block_nr, PROT_READ | PROT_WRITE,
                             MAP_SHARED | MAP_LOCKED, qsockfd, 0);
        if (rx_queue->map == MAP_FAILED) {
            PMD_LOG(ERR, "%s: call to mmap failed on AF_PACKET socket for %s", name, pair->value);
            goto error;
        }
        ...
        if (rc == -1) {
            PMD_LOG(ERR, "%s: could not bind AF_PACKET socket to %s", name, pair->value);
            goto error;
        }

#if defined(PACKET_FANOUT)
        rc = setsockopt(qsockfd, SOL_PACKET, PACKET_FANOUT, &fanout_arg, sizeof(fanout_arg));
        if (rc == -1) {
            PMD_LOG(ERR,
...
error:
    if (qsockfd != -1)
        close(qsockfd);
    for (q = 0; q < nb_queues; q++) {
        munmap((*internals)->rx_queue[q].map, 2 * req->tp_block_size * req->tp_block_nr);

        rte_free((*internals)->rx_queue[q].rd);
        rte_free((*internals)->tx_queue[q].rd);
        if (((*internals)->rx_queue[q].sockfd != 0) && ((*internals)->rx_queue[q].sockfd != qsockfd))
...
static int rte_eth_from_packet(struct rte_vdev_device *dev, int const *sockfd, struct rte_kvargs *kvlist) {
    const char *name = rte_vdev_device_name(dev);
    ...
    for (k_idx = 0; k_idx < kvlist->count; k_idx++) {
        ...
        if (strstr(pair->key, ETH_AF_PACKET_NUM_Q_ARG) != NULL) {
            ...
            if (qpairs < 1 || qpairs > RTE_PMD_AF_PACKET_MAX_RINGS) {
    ...
    if (rte_pmd_init_internals(dev, *sockfd, qpairs, blocksize, blockcount, framesize, framecount, qdisc_bypass,
...
static int rte_pmd_af_packet_probe(struct rte_vdev_device *dev) {
    ...
    if (rte_kvargs_count(kvlist, ETH_AF_PACKET_IFACE_ARG) == 1) {
        ...
        ret = rte_kvargs_process(kvlist, ETH_AF_PACKET_IFACE_ARG, &open_packet_iface, &sockfd);
        if (ret < 0)
            goto exit;
    }
