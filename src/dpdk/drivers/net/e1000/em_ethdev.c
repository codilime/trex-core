...
static void eth_em_infos_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info);
static int eth_em_flow_ctrl_get(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf);
static int eth_em_flow_ctrl_set(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf);
...
static int eth_em_interrupt_action(struct rte_eth_dev *dev, struct rte_intr_handle *handle);
...
static int eth_em_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id, int on);
...
static int eth_em_rar_set(struct rte_eth_dev *dev, struct ether_addr *mac_addr, uint32_t index, uint32_t pool);
static void eth_em_rar_clear(struct rte_eth_dev *dev, uint32_t index);
static int eth_em_default_mac_addr_set(struct rte_eth_dev *dev, struct ether_addr *addr);

static int eth_em_set_mc_addr_list(struct rte_eth_dev *dev, struct ether_addr *mc_addr_set, uint32_t nb_mc_addr);
...
static int eth_em_dev_init(struct rte_eth_dev *eth_dev) {
    ...
    if (e1000_setup_init_funcs(hw, TRUE) != E1000_SUCCESS || em_hw_init(hw) != 0) {
        PMD_INIT_LOG(ERR,
    ...
    ether_addr_copy((struct ether_addr *)hw->mac.addr, eth_dev->data->mac_addrs);
...
static int eth_em_dev_uninit(struct rte_eth_dev *eth_dev) {
    ...
    rte_intr_callback_unregister(intr_handle, eth_em_interrupt_handler, eth_dev);

    return 0;
}

static int eth_em_pci_probe(struct rte_pci_driver *pci_drv __rte_unused, struct rte_pci_device *pci_dev) {
    return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct e1000_adapter), eth_em_dev_init);
}
...
static int eth_em_start(struct rte_eth_dev *dev) {
    ...
    if (rte_intr_allow_others(intr_handle)) {
        ...
        rte_intr_callback_unregister(intr_handle, eth_em_interrupt_handler, (void *)dev);
        if (dev->data->dev_conf.intr_conf.lsc != 0)
...
static void eth_em_stop(struct rte_eth_dev *dev) {
    ...
    if (!rte_intr_allow_others(intr_handle))
        ...
        rte_intr_callback_register(intr_handle, eth_em_interrupt_handler, (void *)dev);
...
static int em_hardware_init(struct e1000_hw *hw) {
    ...
    } else if (hw->mac.type == e1000_pch_lpt || hw->mac.type == e1000_pch_spt || hw->mac.type == e1000_pch_cnp) {
...
static int eth_em_rx_queue_intr_enable(struct rte_eth_dev *dev, __rte_unused uint16_t queue_id) {
    struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
    struct rte_intr_handle *intr_handle = &pci_dev->intr_handle;
...
static int eth_em_rx_queue_intr_disable(struct rte_eth_dev *dev, __rte_unused uint16_t queue_id) {
    struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static void eth_em_infos_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info) {
    ...
    dev_info->rx_offload_capa = em_get_rx_port_offloads_capa(dev) | dev_info->rx_queue_offload_capa;
    dev_info->tx_queue_offload_capa = em_get_tx_queue_offloads_capa(dev);
    dev_info->tx_offload_capa = em_get_tx_port_offloads_capa(dev) | dev_info->tx_queue_offload_capa;
 ...
 *  - On failure, a negative value.
 */
static int eth_em_interrupt_action(struct rte_eth_dev *dev, struct rte_intr_handle *intr_handle) {
    struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
    struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct e1000_interrupt *intr = E1000_DEV_PRIVATE_TO_INTR(dev->data->dev_private);
    struct rte_eth_link link;
    ...
    if (link.link_status) {
        PMD_INIT_LOG(INFO, " Port %d: Link Up - speed %u Mbps - %s", dev->data->port_id, link.link_speed,
                     link.link_duplex == ETH_LINK_FULL_DUPLEX ? "full-duplex" : "half-duplex");
    } else {
        PMD_INIT_LOG(INFO, " Port %d: Link Down", dev->data->port_id);
    }
    PMD_INIT_LOG(DEBUG, "PCI Address: %04d:%02d:%02d:%d", pci_dev->addr.domain, pci_dev->addr.bus, pci_dev->addr.devid,
...
static int eth_em_rar_set(struct rte_eth_dev *dev, struct ether_addr *mac_addr, uint32_t index,
...
/* see e1000_logs.c */
RTE_INIT(igb_init_log) { e1000_igb_init_log(); }
