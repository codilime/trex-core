...
#define E1000_TX_OFFLOAD_MASK (PKT_TX_IPV6 | PKT_TX_IPV4 | PKT_TX_IP_CKSUM | PKT_TX_L4_MASK | PKT_TX_VLAN_PKT)

#define E1000_TX_OFFLOAD_NOTSUP_MASK (PKT_TX_OFFLOAD_MASK ^ E1000_TX_OFFLOAD_MASK)
 ...
 */
static inline void em_set_xmit_ctx(struct em_tx_queue *txq, volatile struct e1000_context_desc *ctx_txd, uint64_t flags,
    ...
    ctx.lower_setup.ip_fields.ipcso = (uint8_t)(l2len + offsetof(struct ipv4_hdr, hdr_checksum));
    ...
    case PKT_TX_TCP_CKSUM:
        ctx.upper_setup.tcp_fields.tucso = (uint8_t)(ipcse + offsetof(struct tcp_hdr, cksum));
 ...
 */
static inline uint32_t what_ctx_update(struct em_tx_queue *txq, uint64_t flags, union em_vlan_macip hdrlen) {
    /* If match with the current context */
    if (likely(txq->ctx_cache.flags == flags &&
               ((txq->ctx_cache.hdrlen.data ^ hdrlen.data) & txq->ctx_cache.cmp_mask) == 0))
        return EM_CTX_0;
...
uint16_t eth_em_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    ...
    for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
        ...
        PMD_TX_LOG(DEBUG,
                   ...
                   (unsigned)txq->port_id, (unsigned)txq->queue_id, (unsigned)tx_pkt->pkt_len, (unsigned)tx_id,
                   (unsigned)tx_last);
 ...
 *
 **********************************************************************/
uint16_t eth_em_prep_pkts(__rte_unused void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    int i, ret;
    struct rte_mbuf *m;
...
uint16_t eth_em_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    ...
    while (nb_rx < nb_pkts) {
        ...
        PMD_RX_LOG(DEBUG,
                   ...
                   (unsigned)rxq->port_id, (unsigned)rxq->queue_id, (unsigned)rx_id, (unsigned)status,
...
uint16_t eth_em_recv_scattered_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    ...
    next_desc:
        ...
        first_seg->ol_flags = first_seg->ol_flags | rx_desc_error_to_pkt_flags(rxd.errors);
...
void eth_em_tx_queue_release(void *txq) { em_tx_queue_release(txq); }
...
uint64_t em_get_tx_port_offloads_capa(struct rte_eth_dev *dev) {
    ...
    RTE_SET_USED(dev);
    tx_offload_capa = DEV_TX_OFFLOAD_MULTI_SEGS | DEV_TX_OFFLOAD_VLAN_INSERT | DEV_TX_OFFLOAD_IPV4_CKSUM |
...
int eth_em_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx, uint16_t nb_desc, unsigned int socket_id,
     ...
     * of E1000_ALIGN.
     */
    if (nb_desc % EM_TXD_ALIGN != 0 || (nb_desc > E1000_MAX_RING_DESC) || (nb_desc < E1000_MIN_RING_DESC)) {
        return -(EINVAL);
    }
    ...
    tz = rte_eth_dma_zone_reserve(dev, "tx_ring", queue_idx, tsize, RTE_CACHE_LINE_SIZE, socket_id);
    if (tz == NULL)
        return -ENOMEM;
    ...
    if ((txq->sw_ring = rte_zmalloc("txq->sw_ring", sizeof(txq->sw_ring[0]) * nb_desc, RTE_CACHE_LINE_SIZE)) == NULL) {
...
void eth_em_rx_queue_release(void *rxq) { em_rx_queue_release(rxq); }
...
uint64_t em_get_rx_port_offloads_capa(struct rte_eth_dev *dev) {
    ...
    rx_offload_capa = DEV_RX_OFFLOAD_VLAN_STRIP | DEV_RX_OFFLOAD_VLAN_FILTER | DEV_RX_OFFLOAD_IPV4_CKSUM |
                      DEV_RX_OFFLOAD_UDP_CKSUM | DEV_RX_OFFLOAD_TCP_CKSUM | DEV_RX_OFFLOAD_KEEP_CRC |
...
int eth_em_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx, uint16_t nb_desc, unsigned int socket_id,
     ...
     * of E1000_ALIGN.
     */
    if (nb_desc % EM_RXD_ALIGN != 0 || (nb_desc > E1000_MAX_RING_DESC) || (nb_desc < E1000_MIN_RING_DESC)) {
        return -EINVAL;
    }
    ...
    rz = rte_eth_dma_zone_reserve(dev, "rx_ring", queue_idx, rsize, RTE_CACHE_LINE_SIZE, socket_id);
    if (rz == NULL)
        return -ENOMEM;

    /* Allocate the RX queue data structure. */
    if ((rxq = rte_zmalloc("ethdev RX queue", sizeof(*rxq), RTE_CACHE_LINE_SIZE)) == NULL)
        return -ENOMEM;

    /* Allocate software ring. */
    if ((rxq->sw_ring = rte_zmalloc("rxq->sw_ring", sizeof(rxq->sw_ring[0]) * nb_desc, RTE_CACHE_LINE_SIZE)) == NULL) {
 ...
 */
static uint32_t em_rctl_bsize(__rte_unused enum e1000_mac_type hwtyp, uint32_t *bufsz) {
    /*
...
int eth_em_rx_init(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i < dev->data->nb_rx_queues; i++) {
        ...
        E1000_WRITE_REG(hw, E1000_RDLEN(i), rxq->nb_rx_desc * sizeof(*rxq->rx_ring));
         ...
         * one buffer.
         */
        if (rxmode->offloads & DEV_RX_OFFLOAD_JUMBO_FRAME || rctl_bsize < ETHER_MAX_LEN) {
    ...
    if ((hw->mac.type == e1000_ich9lan || hw->mac.type == e1000_pch2lan || hw->mac.type == e1000_ich10lan) &&
...
void eth_em_tx_init(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i < dev->data->nb_tx_queues; i++) {
        ...
        E1000_WRITE_REG(hw, E1000_TDLEN(i), txq->nb_tx_desc * sizeof(*txq->tx_ring));
