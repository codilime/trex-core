...
static inline int igb_vf_perm_addr_gen(struct rte_eth_dev *dev, uint16_t vf_num) {
    unsigned char vf_mac_addr[ETHER_ADDR_LEN];
    struct e1000_vf_info *vfinfo = *E1000_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private);
    uint16_t vfn;
...
void igb_pf_host_init(struct rte_eth_dev *eth_dev) {
    struct e1000_vf_info **vfinfo = E1000_DEV_PRIVATE_TO_P_VFDATA(eth_dev->data->dev_private);
...
int igb_pf_host_configure(struct rte_eth_dev *eth_dev) {
    ...
    vtctl |= RTE_ETH_DEV_SRIOV(eth_dev).def_vmdq_idx << E1000_VT_CTL_DEFAULT_POOL_SHIFT;
    ...
    rah = E1000_READ_REG(hw, E1000_RAH(0));
    rah |= (0x1 << (RTE_ETH_DEV_SRIOV(eth_dev).def_vmdq_idx + E1000_RAH_POOLSEL_SHIFT));
    E1000_WRITE_REG(hw, E1000_RAH(0), rah);
...
static void set_rx_mode(struct rte_eth_dev *dev) {
    ...
    if ((hw->mac.type == e1000_82576) || (hw->mac.type == e1000_i350)) {
        vmolr |= E1000_READ_REG(hw, E1000_VMOLR(vfn)) & ~(E1000_VMOLR_MPME | E1000_VMOLR_ROMPE | E1000_VMOLR_ROPE);
...
static inline void igb_vf_reset_event(struct rte_eth_dev *dev, uint16_t vf) {
    ...
    vmolr |= (E1000_VMOLR_ROPE | E1000_VMOLR_ROMPE | E1000_VMOLR_BAM | E1000_VMOLR_AUPE);
...
static int igb_vf_reset(struct rte_eth_dev *dev, uint16_t vf, uint32_t *msgbuf) {
    struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct e1000_vf_info *vfinfo = *(E1000_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
...
static int igb_vf_set_mac_addr(struct rte_eth_dev *dev, uint32_t vf, uint32_t *msgbuf) {
    struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct e1000_vf_info *vfinfo = *(E1000_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
    int rar_entry = hw->mac.rar_entry_count - (vf + 1);
    uint8_t *new_mac = (uint8_t *)(&msgbuf[1]);
    ...
    if (is_unicast_ether_addr((struct ether_addr *)new_mac)) {
        if (!is_zero_ether_addr((struct ether_addr *)new_mac))
            rte_memcpy(vfinfo[vf].vf_mac_addresses, new_mac, sizeof(vfinfo[vf].vf_mac_addresses));
...
static int igb_vf_set_multicast(struct rte_eth_dev *dev, __rte_unused uint32_t vf, uint32_t *msgbuf) {
    int i;
    uint32_t vector_bit;
    uint32_t vector_reg;
    uint32_t mta_reg;
...
static int igb_vf_set_vlan(struct rte_eth_dev *dev, uint32_t vf, uint32_t *msgbuf) {
    int add, vid;
    struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct e1000_vf_info *vfinfo = *(E1000_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
...
static int igb_vf_set_rlpml(struct rte_eth_dev *dev, uint32_t vf, uint32_t *msgbuf) {
    struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
