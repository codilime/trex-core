...
#define IGB_TX_OFFLOAD_MASK                                                                                            \
    (PKT_TX_OUTER_IPV6 | PKT_TX_OUTER_IPV4 | PKT_TX_IPV6 | PKT_TX_IPV4 | PKT_TX_VLAN_PKT | PKT_TX_IP_CKSUM |           \
     PKT_TX_L4_MASK | PKT_TX_TCP_SEG | IGB_TX_IEEE1588_TMST)

#define IGB_TX_OFFLOAD_NOTSUP_MASK (PKT_TX_OFFLOAD_MASK ^ IGB_TX_OFFLOAD_MASK)
...
/** Mac + IP + TCP + Mss mask. */
#define TX_TSO_CMP_MASK (TX_MACIP_LEN_CMP_MASK | TX_TCP_LEN_CMP_MASK | TX_TSO_MSS_CMP_MASK)
 ...
 */
static inline uint64_t check_tso_para(uint64_t ol_req, union igb_tx_offload ol_para) {
...
static inline void igbe_set_xmit_ctx(struct igb_tx_queue *txq, volatile struct e1000_adv_tx_context_desc *ctx_txd,
    ...
    if (ol_flags & PKT_TX_TCP_SEG) {
        ...
        if (ol_flags & PKT_TX_IP_CKSUM)
            type_tucmd_mlhl =
                E1000_ADVTXD_TUCMD_IPV4 | E1000_ADVTXD_TUCMD_L4T_TCP | E1000_ADVTXD_DTYP_CTXT | E1000_ADVTXD_DCMD_DEXT;
        else
            type_tucmd_mlhl =
                E1000_ADVTXD_TUCMD_IPV6 | E1000_ADVTXD_TUCMD_L4T_TCP | E1000_ADVTXD_DTYP_CTXT | E1000_ADVTXD_DCMD_DEXT;
        ...
        case PKT_TX_UDP_CKSUM:
            type_tucmd_mlhl |= E1000_ADVTXD_TUCMD_L4T_UDP | E1000_ADVTXD_DTYP_CTXT | E1000_ADVTXD_DCMD_DEXT;
            mss_l4len_idx |= sizeof(struct udp_hdr) << E1000_ADVTXD_L4LEN_SHIFT;
            break;
        case PKT_TX_TCP_CKSUM:
            type_tucmd_mlhl |= E1000_ADVTXD_TUCMD_L4T_TCP | E1000_ADVTXD_DTYP_CTXT | E1000_ADVTXD_DCMD_DEXT;
            mss_l4len_idx |= sizeof(struct tcp_hdr) << E1000_ADVTXD_L4LEN_SHIFT;
            break;
        case PKT_TX_SCTP_CKSUM:
            type_tucmd_mlhl |= E1000_ADVTXD_TUCMD_L4T_SCTP | E1000_ADVTXD_DTYP_CTXT | E1000_ADVTXD_DCMD_DEXT;
            mss_l4len_idx |= sizeof(struct sctp_hdr) << E1000_ADVTXD_L4LEN_SHIFT;
            break;
        default:
            type_tucmd_mlhl |= E1000_ADVTXD_TUCMD_L4T_RSV | E1000_ADVTXD_DTYP_CTXT | E1000_ADVTXD_DCMD_DEXT;
            break;
        }
    }

    txq->ctx_cache[ctx_curr].flags = ol_flags;
    txq->ctx_cache[ctx_curr].tx_offload.data = tx_offload_mask.data & tx_offload.data;
 ...
 */
static inline uint32_t what_advctx_update(struct igb_tx_queue *txq, uint64_t flags, union igb_tx_offload tx_offload) {
...
uint16_t eth_igb_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    ...
    for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
         ...
         *   - E1000_TXD_CMD_RS
         */
        cmd_type_len = txq->txd_type | E1000_ADVTXD_DCMD_IFCS | E1000_ADVTXD_DCMD_DEXT;
         ...
         */
        txd->read.cmd_type_len |= rte_cpu_to_le_32(E1000_TXD_CMD_EOP | E1000_TXD_CMD_RS);
    }
end_of_tx:
    rte_wmb();
 ...
 *
 **********************************************************************/
uint16_t eth_igb_prep_pkts(__rte_unused void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    int i, ret;
    struct rte_mbuf *m;
    ...
    for (i = 0; i < nb_pkts; i++) {
        ...
        if (m->ol_flags & PKT_TX_TCP_SEG)
            if ((m->tso_segsz > IGB_TSO_MAX_MSS) || (m->l2_len + m->l3_len + m->l4_len > IGB_TSO_MAX_HDRLEN)) {
                rte_errno = -EINVAL;
                return i;
            }
...
static inline uint32_t igb_rxd_pkt_info_to_pkt_type(uint16_t pkt_info) {
    static const uint32_t ptype_table[IGB_PACKET_TYPE_MAX] __rte_cache_aligned = {
        ...
        [IGB_PACKET_TYPE_IPV4_IPV6_EXT] =
            RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP | RTE_PTYPE_INNER_L3_IPV6_EXT,
        [IGB_PACKET_TYPE_IPV4_TCP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_TCP,
        [IGB_PACKET_TYPE_IPV6_TCP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_TCP,
        [IGB_PACKET_TYPE_IPV4_IPV6_TCP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP |
                                          RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_TCP,
        [IGB_PACKET_TYPE_IPV6_EXT_TCP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT | RTE_PTYPE_L4_TCP,
        [IGB_PACKET_TYPE_IPV4_IPV6_EXT_TCP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP |
                                              RTE_PTYPE_INNER_L3_IPV6_EXT | RTE_PTYPE_INNER_L4_TCP,
        [IGB_PACKET_TYPE_IPV4_UDP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_UDP,
        [IGB_PACKET_TYPE_IPV6_UDP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_UDP,
        [IGB_PACKET_TYPE_IPV4_IPV6_UDP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP |
                                          RTE_PTYPE_INNER_L3_IPV6 | RTE_PTYPE_INNER_L4_UDP,
        [IGB_PACKET_TYPE_IPV6_EXT_UDP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT | RTE_PTYPE_L4_UDP,
        [IGB_PACKET_TYPE_IPV4_IPV6_EXT_UDP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP |
                                              RTE_PTYPE_INNER_L3_IPV6_EXT | RTE_PTYPE_INNER_L4_UDP,
        [IGB_PACKET_TYPE_IPV4_SCTP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_SCTP,
        [IGB_PACKET_TYPE_IPV4_EXT_SCTP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT | RTE_PTYPE_L4_SCTP,
    };
...
static inline uint64_t rx_desc_hlen_type_rss_to_pkt_flags(struct igb_rx_queue *rxq, uint32_t hl_tp_rs) {
...
static inline uint64_t rx_desc_status_to_pkt_flags(uint32_t rx_status) {
    ...
    pkt_flags = ((rx_status & E1000_RXD_STAT_VP) ? PKT_RX_VLAN | PKT_RX_VLAN_STRIPPED : 0);
...
static inline uint64_t rx_desc_error_to_pkt_flags(uint32_t rx_status) {
    ...
    return error_to_pkt_flags_map[(rx_status >> E1000_RXD_ERR_CKSUM_BIT) & E1000_RXD_ERR_CKSUM_MSK];
}
...
uint16_t eth_igb_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    ...
    while (nb_rx < nb_pkts) {
         ...
         */
        if ((staterr & rte_cpu_to_le_32(E1000_RXDEXT_STATERR_LB)) && (rxq->flags & IGB_RXQ_FLAG_LB_BSWAP_VLAN)) {
...
uint16_t eth_igb_recv_scattered_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    struct igb_rx_queue *rxq;
    volatile union e1000_adv_rx_desc *rx_ring;
    volatile union e1000_adv_rx_desc *rxdp;
    struct igb_rx_entry *sw_ring;
    ...
    next_desc:
        ...
        if (unlikely(rxq->crc_len > 0)) {
            ...
            if (data_len <= ETHER_CRC_LEN) {
                ...
                first_seg->nb_segs--;
                last_seg->data_len = (uint16_t)(last_seg->data_len - (ETHER_CRC_LEN - data_len));
                last_seg->next = NULL;
            } else
                rxm->data_len = (uint16_t)(data_len - ETHER_CRC_LEN);
        }
        ...
        first_seg->packet_type = igb_rxd_pkt_info_to_pkt_type(rxd.wb.lower.lo_dword.hs_rss.pkt_info);
...
void eth_igb_tx_queue_release(void *txq) { igb_tx_queue_release(txq); }
...
int eth_igb_tx_done_cleanup(void *txq, uint32_t free_cnt) { return igb_tx_done_cleanup(txq, free_cnt); }

static void igb_reset_tx_queue_stat(struct igb_tx_queue *txq) {
    txq->tx_head = 0;
    txq->tx_tail = 0;
    txq->ctx_curr = 0;
    memset((void *)&txq->ctx_cache, 0, IGB_CTX_NUM * sizeof(struct igb_advctx_info));
}

static void igb_reset_tx_queue(struct igb_tx_queue *txq, struct rte_eth_dev *dev) {
...
int eth_igb_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx, uint16_t nb_desc, unsigned int socket_id,
     ...
     * of E1000_ALIGN.
     */
    if (nb_desc % IGB_TXD_ALIGN != 0 || (nb_desc > E1000_MAX_RING_DESC) || (nb_desc < E1000_MIN_RING_DESC)) {
        return -EINVAL;
    }
    ...
    tz = rte_eth_dma_zone_reserve(dev, "tx_ring", queue_idx, size, E1000_ALIGN, socket_id);
    if (tz == NULL) {
...
void eth_igb_rx_queue_release(void *rxq) { igb_rx_queue_release(rxq); }
...
uint64_t igb_get_rx_port_offloads_capa(struct rte_eth_dev *dev) {
    ...
    RTE_SET_USED(dev);
    rx_offload_capa = DEV_RX_OFFLOAD_VLAN_STRIP | DEV_RX_OFFLOAD_VLAN_FILTER | DEV_RX_OFFLOAD_IPV4_CKSUM |
                      DEV_RX_OFFLOAD_UDP_CKSUM | DEV_RX_OFFLOAD_TCP_CKSUM | DEV_RX_OFFLOAD_JUMBO_FRAME |
                      DEV_RX_OFFLOAD_KEEP_CRC | DEV_RX_OFFLOAD_SCATTER;
...
int eth_igb_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx, uint16_t nb_desc, unsigned int socket_id,
     ...
     * of E1000_ALIGN.
     */
    if (nb_desc % IGB_RXD_ALIGN != 0 || (nb_desc > E1000_MAX_RING_DESC) || (nb_desc < E1000_MIN_RING_DESC)) {
        return -EINVAL;
    }
    ...
    PMD_INIT_LOG(DEBUG, "sw_ring=%p hw_ring=%p dma_addr=0x%" PRIx64, rxq->sw_ring, rxq->rx_ring,
...
uint32_t eth_igb_rx_queue_count(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
#define IGB_RXQ_SCAN_INTERVAL 4
    ...
    while ((desc < rxq->nb_rx_desc) && (rxdp->wb.upper.status_error & E1000_RXD_STAT_DD)) {
...
static void igb_hw_rss_hash_set(struct e1000_hw *hw, struct rte_eth_rss_conf *rss_conf) {
    uint8_t *hash_key;
    uint32_t rss_key;
    uint32_t mrqc;
    uint64_t rss_hf;
...
int eth_igb_rss_hash_update(struct rte_eth_dev *dev, struct rte_eth_rss_conf *rss_conf) {
    struct e1000_hw *hw;
    uint32_t mrqc;
    uint64_t rss_hf;
...
int eth_igb_rss_hash_conf_get(struct rte_eth_dev *dev, struct rte_eth_rss_conf *rss_conf) {
    struct e1000_hw *hw;
    uint8_t *hash_key;
    uint32_t rss_key;
    uint32_t mrqc;
...
static void igb_rss_configure(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i < 128; i++) {
        ...
        q_idx = (uint8_t)((dev->data->nb_rx_queues > 1) ? i % dev->data->nb_rx_queues : 0);
...
static int igb_vmdq_rx_hw_configure(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i < E1000_VMOLR_SIZE; i++) {
        ...
        vmolr &= ~(E1000_VMOLR_AUPE | E1000_VMOLR_ROMPE | E1000_VMOLR_ROPE | E1000_VMOLR_BAM | E1000_VMOLR_MPME);
    ...
    for (i = 0; i < cfg->nb_pool_maps; i++) {
        ...
        E1000_WRITE_REG(hw, E1000_VLVF(i),
                        (E1000_VLVF_VLANID_ENABLE | (cfg->pool_map[i].vlan_id & ETH_VLAN_ID_MAX) |
                         ((cfg->pool_map[i].pools << E1000_VLVF_POOLSEL_SHIFT) & E1000_VLVF_POOLSEL_MASK)));
    }
...
int eth_igb_rx_init(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i < dev->data->nb_rx_queues; i++) {
        ...
        if (buf_size >= 1024) {
             ...
             */
            srrctl |= ((buf_size >> E1000_SRRCTL_BSIZEPKT_SHIFT) & E1000_SRRCTL_BSIZEPKT_MASK);
            buf_size = (uint16_t)((srrctl & E1000_SRRCTL_BSIZEPKT_MASK) << E1000_SRRCTL_BSIZEPKT_SHIFT);

            /* It adds dual VLAN length for supporting dual VLAN */
            if ((dev->data->dev_conf.rxmode.max_rx_pkt_len + 2 * VLAN_TAG_SIZE) > buf_size) {
    ...
    if (rxmode->offloads & DEV_RX_OFFLOAD_IPV4_CKSUM)
        ...
        rxcsum &= ~E1000_RXCSUM_IPOFL;
    if (rxmode->offloads & (DEV_RX_OFFLOAD_TCP_CKSUM | DEV_RX_OFFLOAD_UDP_CKSUM))
    ...
    if (dev->data->dev_conf.rxmode.offloads & DEV_RX_OFFLOAD_KEEP_CRC) {
        ...
        if (hw->mac.type == e1000_i350 || hw->mac.type == e1000_i210 || hw->mac.type == e1000_i211 ||
...
int eth_igbvf_rx_init(struct rte_eth_dev *dev) {
    ...
    /* setup MTU */
    e1000_rlpml_set_vf(hw, (uint16_t)(dev->data->dev_conf.rxmode.max_rx_pkt_len + VLAN_TAG_SIZE));
    ...
    for (i = 0; i < dev->data->nb_rx_queues; i++) {
        ...
        bus_addr = rxq->rx_ring_phys_addr;
        E1000_WRITE_REG(hw, E1000_RDLEN(i), rxq->nb_rx_desc * sizeof(union e1000_adv_rx_desc));
...
void eth_igbvf_tx_init(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i < dev->data->nb_tx_queues; i++) {
        ...
        txq = dev->data->tx_queues[i];
        bus_addr = txq->tx_ring_phys_addr;
        E1000_WRITE_REG(hw, E1000_TDLEN(i), txq->nb_tx_desc * sizeof(union e1000_adv_tx_desc));
...
void igb_rxq_info_get(struct rte_eth_dev *dev, uint16_t queue_id, struct rte_eth_rxq_info *qinfo) {
    struct igb_rx_queue *rxq;
...
void igb_txq_info_get(struct rte_eth_dev *dev, uint16_t queue_id, struct rte_eth_txq_info *qinfo) {
    struct igb_tx_queue *txq;
...
int igb_rss_conf_init(struct rte_eth_dev *dev, struct igb_rte_flow_rss_conf *out,
                      const struct rte_flow_action_rss *in) {
    struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);

    if (in->key_len > RTE_DIM(out->key) ||
        ((hw->mac.type == e1000_82576) && (in->queue_num > IGB_MAX_RX_QUEUE_NUM_82576)) ||
...
int igb_action_rss_same(const struct rte_flow_action_rss *comp, const struct rte_flow_action_rss *with) {
...
int igb_config_rss_filter(struct rte_eth_dev *dev, struct igb_rte_flow_rss_conf *conf, bool add) {
    uint32_t shift;
    uint16_t i, j;
    struct rte_eth_rss_conf rss_conf = {
