...
struct vnic_dev {
    ...
    void *(*alloc_consistent)(void *priv, size_t size, dma_addr_t *dma_handle, u8 *name);
    void (*free_consistent)(void *priv, size_t size, void *vaddr, dma_addr_t dma_handle);
    struct vnic_counter_counts *flow_counters;
    dma_addr_t flow_counters_pa;
    u8 flow_counters_dma_active;
};

#define VNIC_MAX_RES_HDR_SIZE (sizeof(struct vnic_resource_header) + sizeof(struct vnic_resource) * RES_TYPE_MAX)
...
static int vnic_dev_discover_res(struct vnic_dev *vdev, struct vnic_dev_bar *bar, unsigned int num_bars) {
    ...
    if ((ioread32(&rh->magic) != VNIC_RES_MAGIC) || (ioread32(&rh->version) != VNIC_RES_VERSION)) {
        if ((ioread32(&mrh->magic) != MGMTVNIC_MAGIC) || (ioread32(&mrh->version) != MGMTVNIC_VERSION)) {
            pr_err("vNIC BAR0 res magic/version error "
                   "exp (%lx/%lx) or (%lx/%lx), curr (%x/%x)\n",
                   VNIC_RES_MAGIC, VNIC_RES_VERSION, MGMTVNIC_MAGIC, MGMTVNIC_VERSION, ioread32(&rh->magic),
                   ioread32(&rh->version));
            return -EINVAL;
        }
    }
...
unsigned int vnic_dev_get_res_count(struct vnic_dev *vdev, enum vnic_res_type type) { return vdev->res[type].count; }

void __iomem *vnic_dev_get_res(struct vnic_dev *vdev, enum vnic_res_type type, unsigned int index) {
...
void vnic_dev_clear_desc_ring(struct vnic_dev_ring *ring) { memset(ring->descs, 0, ring->size); }

int vnic_dev_alloc_desc_ring(struct vnic_dev *vdev, struct vnic_dev_ring *ring, unsigned int desc_count,
                             unsigned int desc_size, __attribute__((unused)) unsigned int socket_id, char *z_name) {
    void *alloc_addr;
    dma_addr_t alloc_pa = 0;

    vnic_dev_desc_ring_size(ring, desc_count, desc_size);
    alloc_addr = vdev->alloc_consistent(vdev->priv, ring->size_unaligned, &alloc_pa, (u8 *)z_name);
    if (!alloc_addr) {
    ...
    if (!alloc_pa) {
        ...
        vdev->free_consistent(vdev->priv, ring->size_unaligned, alloc_addr, alloc_pa);
        return -ENOMEM;
    }
    ring->base_addr_unaligned = alloc_pa;

    ring->base_addr = VNIC_ALIGN(ring->base_addr_unaligned, ring->base_align);
    ring->descs = (u8 *)ring->descs_unaligned + (ring->base_addr - ring->base_addr_unaligned);
...
void vnic_dev_free_desc_ring(__attribute__((unused)) struct vnic_dev *vdev, struct vnic_dev_ring *ring) {
    if (ring->descs) {
        vdev->free_consistent(vdev->priv, ring->size_unaligned, ring->descs_unaligned, ring->base_addr_unaligned);
        ring->descs = NULL;
    }
}
...
static int vnic_dev_cmd_proxy(struct vnic_dev *vdev, enum vnic_devcmd_cmd proxy_cmd, enum vnic_devcmd_cmd cmd,
...
static int vnic_dev_cmd_no_proxy(struct vnic_dev *vdev, enum vnic_devcmd_cmd cmd, u64 *args, int nargs, int wait) {
    int err;
...
int vnic_dev_cmd(struct vnic_dev *vdev, enum vnic_devcmd_cmd cmd, u64 *a0, u64 *a1, int wait) {
    ...
    switch (vdev->proxy) {
    case PROXY_BY_INDEX:
        err = vnic_dev_cmd_proxy(vdev, CMD_PROXY_BY_INDEX, cmd, args, ARRAY_SIZE(args), wait);
        break;
    case PROXY_BY_BDF:
        err = vnic_dev_cmd_proxy(vdev, CMD_PROXY_BY_BDF, cmd, args, ARRAY_SIZE(args), wait);
        break;
    case PROXY_NONE:
    default:
...
int vnic_dev_cmd_args(struct vnic_dev *vdev, enum vnic_devcmd_cmd cmd, u64 *args, int nargs, int wait) {
    switch (vdev->proxy) {
    case PROXY_BY_INDEX:
        return vnic_dev_cmd_proxy(vdev, CMD_PROXY_BY_INDEX, cmd, args, nargs, wait);
    case PROXY_BY_BDF:
        return vnic_dev_cmd_proxy(vdev, CMD_PROXY_BY_BDF, cmd, args, nargs, wait);
    case PROXY_NONE:
    default:
...
int vnic_dev_fw_info(struct vnic_dev *vdev, struct vnic_devcmd_fw_info **fw_info) {
    ...
    if (!vdev->fw_info) {
        ...
        vdev->fw_info =
            vdev->alloc_consistent(vdev->priv, sizeof(struct vnic_devcmd_fw_info), &vdev->fw_info_pa, (u8 *)name);
        if (!vdev->fw_info)
            return -ENOMEM;
        a0 = vdev->fw_info_pa;
...
static int vnic_dev_advanced_filters_cap(struct vnic_dev *vdev, u64 *args, int nargs) {
 ...
 */
int vnic_dev_capable_filter_mode(struct vnic_dev *vdev, u32 *mode, u8 *filter_actions) {
    u64 args[4];
    int err;
    u32 max_level = 0;
...
int vnic_dev_spec(struct vnic_dev *vdev, unsigned int offset, size_t size, void *value) {
    u64 a0, a1;
    int wait = 1000;
    int err;
 ...
 */
int vnic_dev_counter_dma_cfg(struct vnic_dev *vdev, u32 period, u32 num_counters) {
    u64 args[3];
    int wait = 1000;
    int err;
    ...
    if (num_counters > VNIC_MAX_FLOW_COUNTERS)
        return -ENOMEM;
    if (period > 0 && (period < VNIC_COUNTER_DMA_MIN_PERIOD || num_counters == 0))
        return -EINVAL;
    ...
    if (!err)
        vdev->flow_counters_dma_active = (num_counters != 0 && period != 0);
...
int vnic_dev_packet_filter(struct vnic_dev *vdev, int directed, int multicast, int broadcast, int promisc,
                           int allmulti) {
    u64 a0, a1 = 0;
    int wait = 1000;
    int err;

    a0 = (directed ? CMD_PFILTER_DIRECTED : 0) | (multicast ? CMD_PFILTER_MULTICAST : 0) |
         (broadcast ? CMD_PFILTER_BROADCAST : 0) | (promisc ? CMD_PFILTER_PROMISCUOUS : 0) |
...
void vnic_dev_set_reset_flag(struct vnic_dev *vdev, int state) { vdev->in_reset = state; }

static inline int vnic_dev_in_reset(struct vnic_dev *vdev) { return vdev->in_reset; }
...
int vnic_dev_notify_set(struct vnic_dev *vdev, u16 intr) {
    ...
    if (!vnic_dev_in_reset(vdev)) {
        ...
        notify_addr = vdev->alloc_consistent(vdev->priv, sizeof(struct vnic_devcmd_notify), &notify_pa, (u8 *)name);
        if (!notify_addr)
            return -ENOMEM;
    }
...
int vnic_dev_notify_unset(struct vnic_dev *vdev) {
    if (vdev->notify && !vnic_dev_in_reset(vdev)) {
        vdev->free_consistent(vdev->priv, sizeof(struct vnic_devcmd_notify), vdev->notify, vdev->notify_pa);
    }
...
u32 vnic_dev_get_intr_coal_timer_max(struct vnic_dev *vdev) { return vdev->intr_coal_timer_info.max_usec; }
...
int vnic_dev_alloc_stats_mem(struct vnic_dev *vdev) {
    ...
    vdev->stats = vdev->alloc_consistent(vdev->priv, sizeof(struct vnic_stats), &vdev->stats_pa, (u8 *)name);
...
int vnic_dev_alloc_counter_mem(struct vnic_dev *vdev) {
    ...
    vdev->flow_counters = vdev->alloc_consistent(
        vdev->priv, sizeof(struct vnic_counter_counts) * VNIC_MAX_FLOW_COUNTERS, &vdev->flow_counters_pa, (u8 *)name);
...
void vnic_dev_unregister(struct vnic_dev *vdev) {
    if (vdev) {
        if (vdev->notify)
            vdev->free_consistent(vdev->priv, sizeof(struct vnic_devcmd_notify), vdev->notify, vdev->notify_pa);
        if (vdev->stats)
            vdev->free_consistent(vdev->priv, sizeof(struct vnic_stats), vdev->stats, vdev->stats_pa);
        if (vdev->flow_counters) {
            ...
            vdev->free_consistent(vdev->priv, sizeof(struct vnic_counter_counts) * VNIC_MAX_FLOW_COUNTERS,
                                  vdev->flow_counters, vdev->flow_counters_pa);
        }
        if (vdev->fw_info)
            vdev->free_consistent(vdev->priv, sizeof(struct vnic_devcmd_fw_info), vdev->fw_info, vdev->fw_info_pa);
        rte_free(vdev);
    }
}

struct vnic_dev *vnic_dev_register(struct vnic_dev *vdev, void *priv, struct rte_pci_device *pdev,
                                   struct vnic_dev_bar *bar, unsigned int num_bars) {
    if (!vdev) {
        char name[NAME_MAX];
        snprintf((char *)name, sizeof(name), "%s-vnic", pdev->device.name);
        vdev = (struct vnic_dev *)rte_zmalloc_socket(name, sizeof(struct vnic_dev), RTE_CACHE_LINE_SIZE,
 ...
 * @action: action data
 */
int vnic_dev_classifier(struct vnic_dev *vdev, u8 cmd, u16 *entry, struct filter_v2 *data,
    ...
    if (cmd == CLSF_ADD) {
        dev_cmd = (data->type >= FILTER_DPDK_1) ? CMD_ADD_ADV_FILTER : CMD_ADD_FILTER;
        ...
        tlv_va = vdev->alloc_consistent(vdev->priv, tlv_size, &tlv_pa, (u8 *)z_name);
        if (!tlv_va)
            return -ENOMEM;
        tlv = tlv_va;
        a0 = tlv_pa;
        ...
        tlv = (struct filter_tlv *)((char *)tlv + sizeof(struct filter_tlv) + filter_size);
...
int vnic_dev_capable_vxlan(struct vnic_dev *vdev) {
    ...
    return ret == 0 &&
           (a1 & (FEATURE_VXLAN_IPV6 | FEATURE_VXLAN_MULTI_WQ)) == (FEATURE_VXLAN_IPV6 | FEATURE_VXLAN_MULTI_WQ);
}
...
bool vnic_dev_counter_query(struct vnic_dev *vdev, uint32_t idx, bool reset, uint64_t *packets, uint64_t *bytes) {
    u64 a0 = idx;
    u64 a1 = reset ? 1 : 0;
    int wait = 1000;
