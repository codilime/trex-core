...
int vnic_rq_alloc(struct vnic_dev *vdev, struct vnic_rq *rq, unsigned int index, unsigned int desc_count,
    ...
    rc = vnic_dev_alloc_desc_ring(vdev, &rq->ring, desc_count, desc_size, rq->socket_id, res_name);
    return rc;
}

void vnic_rq_init_start(struct vnic_rq *rq, unsigned int cq_index, unsigned int fetch_index, unsigned int posted_index,
...
void vnic_rq_clean(struct vnic_rq *rq, void (*buf_clean)(struct rte_mbuf **buf)) {
    struct rte_mbuf **buf;
    u32 fetch_index, i;
    unsigned int count = rq->ring.desc_count;
