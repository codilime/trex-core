...
static void copy_fltr_v1(struct filter_v2 *fltr, const struct rte_eth_fdir_input *input,
                         const struct rte_eth_fdir_masks *masks);
static void copy_fltr_v2(struct filter_v2 *fltr, const struct rte_eth_fdir_input *input,
                         const struct rte_eth_fdir_masks *masks);

void enic_fdir_stats_get(struct enic *enic, struct rte_eth_fdir_stats *stats) { *stats = enic->fdir.stats; }
...
void enic_fdir_info(struct enic *enic) {
    ...
    enic->fdir.types_mask = 1 << RTE_ETH_FLOW_NONFRAG_IPV4_UDP | 1 << RTE_ETH_FLOW_NONFRAG_IPV4_TCP;
    if (enic->adv_filters) {
        enic->fdir.types_mask |= 1 << RTE_ETH_FLOW_NONFRAG_IPV4_OTHER | 1 << RTE_ETH_FLOW_NONFRAG_IPV4_SCTP |
                                 1 << RTE_ETH_FLOW_NONFRAG_IPV6_UDP | 1 << RTE_ETH_FLOW_NONFRAG_IPV6_TCP |
                                 1 << RTE_ETH_FLOW_NONFRAG_IPV6_SCTP | 1 << RTE_ETH_FLOW_NONFRAG_IPV6_OTHER;
...
static void enic_set_layer(struct filter_generic_1 *gp, unsigned int flag, enum filter_generic_1_layer layer,
                           void *mask, void *val, unsigned int len) {
    gp->mask_flags |= flag;
...
static void copy_fltr_v1(struct filter_v2 *fltr, const struct rte_eth_fdir_input *input,
    ...
    fltr->u.ipv4.src_addr = rte_be_to_cpu_32(input->flow.ip4_flow.src_ip);
    fltr->u.ipv4.dst_addr = rte_be_to_cpu_32(input->flow.ip4_flow.dst_ip);
    fltr->u.ipv4.src_port = rte_be_to_cpu_16(input->flow.udp4_flow.src_port);
    fltr->u.ipv4.dst_port = rte_be_to_cpu_16(input->flow.udp4_flow.dst_port);
...
#ifdef TREX_PATCH
void copy_fltr_recv_all(struct filter_v2 *fltr, struct rte_eth_fdir_input *input, struct rte_eth_fdir_masks *masks) {
    ...
    gp->position = 0;
    enic_set_layer(gp, 0, FILTER_GENERIC_1_L2, &eth_mask, &eth_val, sizeof(struct ether_hdr));
...
static void copy_fltr_v2(struct filter_v2 *fltr, const struct rte_eth_fdir_input *input,
    ...
    if (input->flow_type == RTE_ETH_FLOW_NONFRAG_IPV4_UDP) {
        ...
        enic_set_layer(gp, FILTER_GENERIC_1_UDP, FILTER_GENERIC_1_L4, &udp_mask, &udp_val, sizeof(struct udp_hdr));
        ...
        enic_set_layer(gp, FILTER_GENERIC_1_TCP, FILTER_GENERIC_1_L4, &tcp_mask, &tcp_val, sizeof(struct tcp_hdr));
         ...
         */
        enic_set_layer(gp, 0, FILTER_GENERIC_1_L4, &sctp_mask, &sctp_val, sizeof(struct sctp_hdr));
    }

    if (input->flow_type == RTE_ETH_FLOW_NONFRAG_IPV4_UDP || input->flow_type == RTE_ETH_FLOW_NONFRAG_IPV4_TCP ||
        input->flow_type == RTE_ETH_FLOW_NONFRAG_IPV4_SCTP || input->flow_type == RTE_ETH_FLOW_NONFRAG_IPV4_OTHER) {
        ...
        enic_set_layer(gp, FILTER_GENERIC_1_IPV4, FILTER_GENERIC_1_L3, &ip4_mask, &ip4_val, sizeof(struct ipv4_hdr));
    }
    ...
    if (input->flow_type == RTE_ETH_FLOW_NONFRAG_IPV6_UDP || input->flow_type == RTE_ETH_FLOW_NONFRAG_IPV6_TCP ||
        input->flow_type == RTE_ETH_FLOW_NONFRAG_IPV6_SCTP || input->flow_type == RTE_ETH_FLOW_NONFRAG_IPV6_OTHER) {
        ...
        memcpy(ipv6_mask.src_addr, masks->ipv6_mask.src_ip, sizeof(ipv6_mask.src_addr));
        memcpy(ipv6_val.src_addr, input->flow.ipv6_flow.src_ip, sizeof(ipv6_val.src_addr));
        memcpy(ipv6_mask.dst_addr, masks->ipv6_mask.dst_ip, sizeof(ipv6_mask.dst_addr));
        memcpy(ipv6_val.dst_addr, input->flow.ipv6_flow.dst_ip, sizeof(ipv6_val.dst_addr));
        ...
        enic_set_layer(gp, FILTER_GENERIC_1_IPV6, FILTER_GENERIC_1_L3, &ipv6_mask, &ipv6_val, sizeof(struct ipv6_hdr));
    }
}
...
int enic_fdir_del_fltr(struct enic *enic, struct rte_eth_fdir_filter *params) {
    ...
    default:
        ...
        /* Delete the filter */
        vnic_dev_classifier(enic->vdev, CLSF_DEL, &key->fltr_id, NULL, NULL);
        rte_free(key);
...
int enic_fdir_add_fltr(struct enic *enic, struct rte_eth_fdir_filter *params) {
    ...
    flowtype_supported = enic->fdir.types_mask & (1 << params->input.flow_type);

    flex_bytes = ((params->input.flow_ext.flexbytes[1] << 8 & 0xFF00) | (params->input.flow_ext.flexbytes[0] & 0xFF));

    if (!enic->fdir.hash || (params->input.flow_ext.vlan_tci & 0xFFF) || !flowtype_supported || flex_bytes ||
    ...
    case -ENOENT:
        ...
        key = rte_zmalloc("enic_fdir_node", sizeof(struct enic_fdir_node), 0);
        if (!key) {
            enic->fdir.stats.f_add++;
            return -ENOMEM;
        }
    ...
    default:
        ...
        if (unlikely(key->rq_index == queue)) {
            ...
            dev_warning(enic, "FDIR rule is already present\n");
            return 0;
        }
    ...
    default:
#endif
        enic->fdir.copy_fltr_fn(&fltr, &params->input, &enic->rte_dev->data->dev_conf.fdir_conf.mask);
#ifdef TREX_PATCH
    }
#endif
    ...
    if (!vnic_dev_classifier(enic->vdev, CLSF_ADD, &queue, &fltr, &action)) {
        key->fltr_id = queue;
    } else {
    ...
    if (do_free)
        vnic_dev_classifier(enic->vdev, CLSF_DEL, &old_fltr_id, NULL, NULL);
    else {
        enic->fdir.stats.free--;
        enic->fdir.stats.add++;
    }
