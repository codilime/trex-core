...
static int enicpmd_fdir_ctrl_func(struct rte_eth_dev *eth_dev, enum rte_filter_op filter_op, void *arg) {
    ...
    case RTE_ETH_FILTER_UPDATE:
        ret = enic_fdir_add_fltr(enic, (struct rte_eth_fdir_filter *)arg);
        break;

    case RTE_ETH_FILTER_DELETE:
        ret = enic_fdir_del_fltr(enic, (struct rte_eth_fdir_filter *)arg);
        break;
...
static int enicpmd_dev_filter_ctrl(struct rte_eth_dev *dev, enum rte_filter_type filter_type,
    ...
    case RTE_ETH_FILTER_FDIR:
        ...
        break;
    default:
        dev_warning(enic, "Filter type (%d) not supported", filter_type);
        ret = -EINVAL;
        break;
    }
...
static int enicpmd_dev_tx_queue_start(struct rte_eth_dev *eth_dev, uint16_t queue_idx) {
    struct enic *enic = pmd_priv(eth_dev);
...
static int enicpmd_dev_tx_queue_stop(struct rte_eth_dev *eth_dev, uint16_t queue_idx) {
    int ret;
    struct enic *enic = pmd_priv(eth_dev);
...
static int enicpmd_dev_rx_queue_start(struct rte_eth_dev *eth_dev, uint16_t queue_idx) {
    struct enic *enic = pmd_priv(eth_dev);
...
static int enicpmd_dev_rx_queue_stop(struct rte_eth_dev *eth_dev, uint16_t queue_idx) {
    int ret;
    struct enic *enic = pmd_priv(eth_dev);
...
static uint32_t enicpmd_dev_rx_queue_count(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
...
static int enicpmd_dev_rx_queue_setup(struct rte_eth_dev *eth_dev, uint16_t queue_idx, uint16_t nb_desc,
    ...
    eth_dev->data->rx_queues[queue_idx] = (void *)&enic->rq[enic_rte_rq_idx_to_sop_idx(queue_idx)];

    ret = enic_alloc_rq(enic, queue_idx, socket_id, mp, nb_desc, rx_conf->rx_free_thresh);
    if (ret) {
...
static int enicpmd_vlan_offload_set(struct rte_eth_dev *eth_dev, int mask) {
    ...
    if ((mask & ETH_VLAN_FILTER_MASK) && (offloads & DEV_RX_OFFLOAD_VLAN_FILTER)) {
        dev_warning(enic, "Configuration of VLAN filter is not supported\n");
    }

    if ((mask & ETH_VLAN_EXTEND_MASK) && (offloads & DEV_RX_OFFLOAD_VLAN_EXTEND)) {
        dev_warning(enic, "Configuration of extended VLAN is not supported\n");
    }
...
static int enicpmd_dev_configure(struct rte_eth_dev *eth_dev) {
    ...
    enic->mc_count = 0;
    enic->hw_ip_checksum = !!(eth_dev->data->dev_conf.rxmode.offloads & DEV_RX_OFFLOAD_CHECKSUM);
...
static int enicpmd_dev_link_update(struct rte_eth_dev *eth_dev, __rte_unused int wait_to_complete) {
    struct enic *enic = pmd_priv(eth_dev);
...
static int enicpmd_dev_stats_get(struct rte_eth_dev *eth_dev, struct rte_eth_stats *stats) {
    struct enic *enic = pmd_priv(eth_dev);
...
static void enicpmd_dev_info_get(struct rte_eth_dev *eth_dev, struct rte_eth_dev_info *device_info) {
    struct enic *enic = pmd_priv(eth_dev);
    ...
    device_info->default_rxconf = (struct rte_eth_rxconf){.rx_free_thresh = ENIC_DEFAULT_RX_FREE_THRESH};
    ...
    device_info->default_rxportconf = (struct rte_eth_dev_portconf){
        ...
        .ring_size = RTE_MIN(device_info->rx_desc_lim.nb_max, ENIC_DEFAULT_RX_RING_SIZE),
    ...
    device_info->default_txportconf = (struct rte_eth_dev_portconf){
        ...
        .ring_size = RTE_MIN(device_info->tx_desc_lim.nb_max, ENIC_DEFAULT_TX_RING_SIZE),
...
static const uint32_t *enicpmd_dev_supported_ptypes_get(struct rte_eth_dev *dev) {
    ...
    if (dev->rx_pkt_burst != enic_dummy_recv_pkts && dev->rx_pkt_burst != NULL) {
        struct enic *enic = pmd_priv(dev);
...
static int enicpmd_add_mac_addr(struct rte_eth_dev *eth_dev, struct ether_addr *mac_addr, __rte_unused uint32_t index,
...
static int enicpmd_set_mac_addr(struct rte_eth_dev *eth_dev, struct ether_addr *addr) {
    struct enic *enic = pmd_priv(eth_dev);
    int ret;
...
static int enicpmd_set_mc_addr_list(struct rte_eth_dev *eth_dev, struct ether_addr *mc_addr_set, uint32_t nb_mc_addr) {
    struct enic *enic = pmd_priv(eth_dev);
    char mac_str[ETHER_ADDR_FMT_SIZE];
    ...
    for (i = 0; i < nb_mc_addr && mc_addr_set != NULL; i++) {
        addr = &mc_addr_set[i];
        if (!is_multicast_ether_addr(addr) || is_broadcast_ether_addr(addr)) {
            ether_format_addr(mac_str, ETHER_ADDR_FMT_SIZE, addr);
    ...
    memcpy(enic->mc_addrs, mc_addr_set, nb_mc_addr * sizeof(struct ether_addr));
...
static int enicpmd_dev_rss_reta_query(struct rte_eth_dev *dev, struct rte_eth_rss_reta_entry64 *reta_conf,
                                      uint16_t reta_size) {
    struct enic *enic = pmd_priv(dev);
    ...
    for (i = 0; i < reta_size; i++) {
        ...
        if (reta_conf[idx].mask & (1ULL << shift))
            reta_conf[idx].reta[shift] = enic_sop_rq_idx_to_rte_idx(enic->rss_cpu.cpu[i / 4].b[i % 4]);
    }

    return 0;
}

static int enicpmd_dev_rss_reta_update(struct rte_eth_dev *dev, struct rte_eth_rss_reta_entry64 *reta_conf,
                                       uint16_t reta_size) {
    struct enic *enic = pmd_priv(dev);
    ...
    for (i = 0; i < reta_size; i++) {
        ...
        shift = i % RTE_RETA_GROUP_SIZE;
        if (reta_conf[idx].mask & (1ULL << shift))
            rss_cpu.cpu[i / 4].b[i % 4] = enic_rte_rq_idx_to_sop_idx(reta_conf[idx].reta[shift]);
    }
    return enic_set_rss_reta(enic, &rss_cpu);
}

static int enicpmd_dev_rss_hash_update(struct rte_eth_dev *dev, struct rte_eth_rss_conf *rss_conf) {
    struct enic *enic = pmd_priv(dev);
...
static int enicpmd_dev_rss_hash_conf_get(struct rte_eth_dev *dev, struct rte_eth_rss_conf *rss_conf) {
    struct enic *enic = pmd_priv(dev);
...
static void enicpmd_dev_rxq_info_get(struct rte_eth_dev *dev, uint16_t rx_queue_id, struct rte_eth_rxq_info *qinfo) {
    struct enic *enic = pmd_priv(dev);
...
static void enicpmd_dev_txq_info_get(struct rte_eth_dev *dev, uint16_t tx_queue_id, struct rte_eth_txq_info *qinfo) {
    struct enic *enic = pmd_priv(dev);
...
static int enicpmd_dev_rx_queue_intr_enable(struct rte_eth_dev *eth_dev, uint16_t rx_queue_id) {
    struct enic *enic = pmd_priv(eth_dev);
...
static int enicpmd_dev_rx_queue_intr_disable(struct rte_eth_dev *eth_dev, uint16_t rx_queue_id) {
    struct enic *enic = pmd_priv(eth_dev);
...
static int udp_tunnel_common_check(struct enic *enic, struct rte_eth_udp_tunnel *tnl) {
...
static int update_vxlan_port(struct enic *enic, uint16_t port) {
    if (vnic_dev_overlay_offload_cfg(enic->vdev, OVERLAY_CFG_VXLAN_PORT_UPDATE, port)) {
...
static int enicpmd_dev_udp_tunnel_port_add(struct rte_eth_dev *eth_dev, struct rte_eth_udp_tunnel *tnl) {
    struct enic *enic = pmd_priv(eth_dev);
    int ret;
...
static int enicpmd_dev_udp_tunnel_port_del(struct rte_eth_dev *eth_dev, struct rte_eth_udp_tunnel *tnl) {
    struct enic *enic = pmd_priv(eth_dev);
    int ret;
...
static int enicpmd_dev_fw_version_get(struct rte_eth_dev *eth_dev, char *fw_version, size_t fw_size) {
...
static int enic_parse_zero_one(const char *key, const char *value, void *opaque) {
    struct enic *enic;
    bool b;
...
static int enic_parse_ig_vlan_rewrite(__rte_unused const char *key, const char *value, void *opaque) {
    struct enic *enic;
    ...
    if (strcmp(value, "trunk") == 0) {
        ...
        dev_err(enic,
                "Invalid value for " ENIC_DEVARG_IG_VLAN_REWRITE ": expected=trunk|untag|priority|pass given=%s\n",
                value);
        return -EINVAL;
    }
    return 0;
...
static int enic_check_devargs(struct rte_eth_dev *dev) {
    static const char *const valid_keys[] = {ENIC_DEVARG_DISABLE_OVERLAY, ENIC_DEVARG_ENABLE_AVX2_RX,
                                             ENIC_DEVARG_IG_VLAN_REWRITE, NULL};
    struct enic *enic = pmd_priv(dev);
    ...
    if (!kvlist)
        return -EINVAL;
    if (rte_kvargs_process(kvlist, ENIC_DEVARG_DISABLE_OVERLAY, enic_parse_zero_one, enic) < 0 ||
        rte_kvargs_process(kvlist, ENIC_DEVARG_ENABLE_AVX2_RX, enic_parse_zero_one, enic) < 0 ||
        rte_kvargs_process(kvlist, ENIC_DEVARG_IG_VLAN_REWRITE, enic_parse_ig_vlan_rewrite, enic) < 0) {
...
static int eth_enicpmd_dev_init(struct rte_eth_dev *eth_dev) {
    ...
    snprintf(enic->bdf_name, ENICPMD_BDF_LENGTH, "%04x:%02x:%02x.%x", addr->domain, addr->bus, addr->devid,
             addr->function);
...
static int eth_enic_pci_probe(struct rte_pci_driver *pci_drv __rte_unused, struct rte_pci_device *pci_dev) {
    return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct enic), eth_enicpmd_dev_init);
}

static int eth_enic_pci_remove(struct rte_pci_device *pci_dev) { return rte_eth_dev_pci_generic_remove(pci_dev, NULL); }
...
RTE_PMD_REGISTER_PARAM_STRING(net_enic, ENIC_DEVARG_DISABLE_OVERLAY "=0|1 " ENIC_DEVARG_ENABLE_AVX2_RX
