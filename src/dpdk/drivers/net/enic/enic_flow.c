...
#define FLOW_TRACE() rte_log(RTE_LOG_DEBUG, enicpmd_logtype_flow, "%s()\n", __func__)
#define FLOW_LOG(level, fmt, args...) rte_log(RTE_LOG_##level, enicpmd_logtype_flow, fmt "\n", ##args)
...
typedef int(copy_action_fn)(struct enic *enic, const struct rte_flow_action actions[],
...
static const enum rte_flow_action_type enic_supported_actions_v2_id[] = {
    RTE_FLOW_ACTION_TYPE_QUEUE, RTE_FLOW_ACTION_TYPE_MARK,     RTE_FLOW_ACTION_TYPE_FLAG,
    RTE_FLOW_ACTION_TYPE_RSS,   RTE_FLOW_ACTION_TYPE_PASSTHRU, RTE_FLOW_ACTION_TYPE_END,
};

static const enum rte_flow_action_type enic_supported_actions_v2_drop[] = {
    RTE_FLOW_ACTION_TYPE_QUEUE, RTE_FLOW_ACTION_TYPE_MARK,     RTE_FLOW_ACTION_TYPE_FLAG, RTE_FLOW_ACTION_TYPE_DROP,
    RTE_FLOW_ACTION_TYPE_RSS,   RTE_FLOW_ACTION_TYPE_PASSTHRU, RTE_FLOW_ACTION_TYPE_END,
};

static const enum rte_flow_action_type enic_supported_actions_v2_count[] = {
    RTE_FLOW_ACTION_TYPE_QUEUE, RTE_FLOW_ACTION_TYPE_MARK, RTE_FLOW_ACTION_TYPE_FLAG,     RTE_FLOW_ACTION_TYPE_DROP,
    RTE_FLOW_ACTION_TYPE_COUNT, RTE_FLOW_ACTION_TYPE_RSS,  RTE_FLOW_ACTION_TYPE_PASSTHRU, RTE_FLOW_ACTION_TYPE_END,
};
...
static int mask_exact_match(const u8 *supported, const u8 *supplied, unsigned int size) {
    unsigned int i;
    for (i = 0; i < size; i++) {
...
static int enic_copy_item_ipv4_v1(struct copy_item_args *arg) {
    ...
    if (!mask_exact_match((const u8 *)&supported_mask, (const u8 *)item->mask, sizeof(*mask))) {
 ...
 */
static int copy_inner_common(struct filter_generic_1 *gp, uint8_t *inner_ofst, const void *val, const void *mask,
                             uint8_t val_size, uint8_t proto_off, uint16_t proto_val, uint8_t proto_size) {
...
static int enic_copy_item_inner_eth_v2(struct copy_item_args *arg) {
    ...
    return copy_inner_common(&arg->filter->u.generic_1, off, arg->item->spec, mask, sizeof(struct ether_hdr),
...
static int enic_copy_item_inner_vlan_v2(struct copy_item_args *arg) {
    ...
    return copy_inner_common(&arg->filter->u.generic_1, off, arg->item->spec, mask, sizeof(struct vlan_hdr),
...
static int enic_copy_item_inner_ipv4_v2(struct copy_item_args *arg) {
    ...
    return copy_inner_common(&arg->filter->u.generic_1, off, arg->item->spec, mask, sizeof(struct ipv4_hdr),
...
static int enic_copy_item_inner_ipv6_v2(struct copy_item_args *arg) {
    ...
    return copy_inner_common(&arg->filter->u.generic_1, off, arg->item->spec, mask, sizeof(struct ipv6_hdr),
...
static int enic_copy_item_inner_udp_v2(struct copy_item_args *arg) {
    ...
    return copy_inner_common(&arg->filter->u.generic_1, off, arg->item->spec, mask, sizeof(struct udp_hdr),
...
static int enic_copy_item_inner_tcp_v2(struct copy_item_args *arg) {
    ...
    return copy_inner_common(&arg->filter->u.generic_1, off, arg->item->spec, mask, sizeof(struct tcp_hdr),
...
static int enic_copy_item_eth_v2(struct copy_item_args *arg) {
    ...
    memcpy(enic_spec.d_addr.addr_bytes, spec->dst.addr_bytes, ETHER_ADDR_LEN);
    memcpy(enic_spec.s_addr.addr_bytes, spec->src.addr_bytes, ETHER_ADDR_LEN);

    memcpy(enic_mask.d_addr.addr_bytes, mask->dst.addr_bytes, ETHER_ADDR_LEN);
    memcpy(enic_mask.s_addr.addr_bytes, mask->src.addr_bytes, ETHER_ADDR_LEN);
    enic_spec.ether_type = spec->type;
    enic_mask.ether_type = mask->type;

    /* outer header */
    memcpy(gp->layer[FILTER_GENERIC_1_L2].mask, &enic_mask, sizeof(struct ether_hdr));
    memcpy(gp->layer[FILTER_GENERIC_1_L2].val, &enic_spec, sizeof(struct ether_hdr));
    return 0;
}
...
static int enic_copy_item_ipv4_v2(struct copy_item_args *arg) {
    ...
    memcpy(gp->layer[FILTER_GENERIC_1_L3].mask, &mask->hdr, sizeof(struct ipv4_hdr));
    memcpy(gp->layer[FILTER_GENERIC_1_L3].val, &spec->hdr, sizeof(struct ipv4_hdr));
    return 0;
}
...
static int enic_copy_item_ipv6_v2(struct copy_item_args *arg) {
    ...
    memcpy(gp->layer[FILTER_GENERIC_1_L3].mask, &mask->hdr, sizeof(struct ipv6_hdr));
    memcpy(gp->layer[FILTER_GENERIC_1_L3].val, &spec->hdr, sizeof(struct ipv6_hdr));
    return 0;
}
...
static int enic_copy_item_udp_v2(struct copy_item_args *arg) {
    ...
    memcpy(gp->layer[FILTER_GENERIC_1_L4].mask, &mask->hdr, sizeof(struct udp_hdr));
    memcpy(gp->layer[FILTER_GENERIC_1_L4].val, &spec->hdr, sizeof(struct udp_hdr));
    return 0;
}
...
static int enic_copy_item_tcp_v2(struct copy_item_args *arg) {
    ...
    memcpy(gp->layer[FILTER_GENERIC_1_L4].mask, &mask->hdr, sizeof(struct tcp_hdr));
    memcpy(gp->layer[FILTER_GENERIC_1_L4].val, &spec->hdr, sizeof(struct tcp_hdr));
    return 0;
}
...
static int enic_copy_item_sctp_v2(struct copy_item_args *arg) {
    ...
    *ip_proto_mask = 0xff;
    ...
    memcpy(gp->layer[FILTER_GENERIC_1_L4].mask, &mask->hdr, sizeof(struct sctp_hdr));
    memcpy(gp->layer[FILTER_GENERIC_1_L4].val, &spec->hdr, sizeof(struct sctp_hdr));
    return 0;
}
...
static int enic_copy_item_vxlan_v2(struct copy_item_args *arg) {
    ...
    memcpy(gp->layer[FILTER_GENERIC_1_L5].mask, mask, sizeof(struct vxlan_hdr));
    memcpy(gp->layer[FILTER_GENERIC_1_L5].val, spec, sizeof(struct vxlan_hdr));
...
static int enic_copy_item_raw_v2(struct copy_item_args *arg) {
    ...
    if (spec->length == 0 || spec->length + sizeof(struct udp_hdr) > FILTER_GENERIC_1_KEY_LEN || !spec->pattern ||
        !mask->pattern)
        return EINVAL;
    /*
    ...
    if (mask->length != 0 && mask->length < spec->length)
        return EINVAL;
    memcpy(gp->layer[FILTER_GENERIC_1_L4].mask + sizeof(struct udp_hdr), mask->pattern, spec->length);
    memcpy(gp->layer[FILTER_GENERIC_1_L4].val + sizeof(struct udp_hdr), spec->pattern, spec->length);
 ...
 */
static int item_stacking_valid(enum rte_flow_item_type prev_item, const struct enic_items *item_info,
                               u8 is_first_item) {
 ...
 */
static void fixup_l5_layer(struct enic *enic, struct filter_generic_1 *gp, uint8_t inner_ofst) {
    ...
    memcpy(gp->layer[FILTER_GENERIC_1_L4].mask + sizeof(struct udp_hdr), gp->layer[FILTER_GENERIC_1_L5].mask, vxlan);
    memcpy(gp->layer[FILTER_GENERIC_1_L4].val + sizeof(struct udp_hdr), gp->layer[FILTER_GENERIC_1_L5].val, vxlan);
 ...
 * @param error[out]
 */
static int enic_copy_filter(const struct rte_flow_item pattern[], const struct enic_filter_cap *cap, struct enic *enic,
                            struct filter_v2 *enic_filter, struct rte_flow_error *error) {
    int ret;
    ...
    for (; item->type != RTE_FLOW_ITEM_TYPE_END; item++) {
        ...
        if (item->type > cap->max_item_type || item_info->copy_item == NULL ||
            ...
            rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, NULL, "Unsupported item.");
            return -rte_errno;
        }
        ...
        args.item = item;
        copy_fn = inner_ofst > 0 ? item_info->inner_copy_item : item_info->copy_item;
        ret = copy_fn(&args);
        if (ret)
...
item_not_supported:
    rte_flow_error_set(error, ret, RTE_FLOW_ERROR_TYPE_ITEM, NULL, "enic type error");
    return -rte_errno;

stacking_error:
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item, "stacking error");
    return -rte_errno;
}
 ...
 * @param error[out]
 */
static int enic_copy_action_v1(__rte_unused struct enic *enic, const struct rte_flow_action actions[],
                               struct filter_action_v2 *enic_action) {
    enum {
        FATE = 1,
    };
    ...
    for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
        ...
        case RTE_FLOW_ACTION_TYPE_QUEUE: {
            const struct rte_flow_action_queue *queue = (const struct rte_flow_action_queue *)actions->conf;
 ...
 * @param error[out]
 */
static int enic_copy_action_v2(struct enic *enic, const struct rte_flow_action actions[],
                               struct filter_action_v2 *enic_action) {
    enum {
        FATE = 1,
        MARK = 2,
    ...
    for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
        ...
        case RTE_FLOW_ACTION_TYPE_MARK: {
            const struct rte_flow_action_mark *mark = (const struct rte_flow_action_mark *)actions->conf;
        ...
        case RTE_FLOW_ACTION_TYPE_RSS: {
            const struct rte_flow_action_rss *rss = (const struct rte_flow_action_rss *)actions->conf;
            bool allow;
            uint16_t i;
            ...
            allow = rss->func == RTE_ETH_HASH_FUNCTION_DEFAULT && rss->level == 0 &&
                    (rss->types == 0 || rss->types == enic->rss_hf) && rss->queue_num == enic->rq_count &&
                    rss->key_len == 0;
...
static int enic_match_action(const struct rte_flow_action *action, const enum rte_flow_action_type *supported_actions) {
...
static void enic_dump_filter(const struct filter_v2 *filt) {
    ...
    case FILTER_DPDK_1:
        ...
        if (gp->mask_flags & FILTER_GENERIC_1_IPV4)
            sprintf(ip4, "%s ", (gp->val_flags & FILTER_GENERIC_1_IPV4) ? "ip4(y)" : "ip4(n)");
        else
            sprintf(ip4, "%s ", "ip4(x)");

        if (gp->mask_flags & FILTER_GENERIC_1_IPV6)
            sprintf(ip6, "%s ", (gp->val_flags & FILTER_GENERIC_1_IPV4) ? "ip6(y)" : "ip6(n)");
        else
            sprintf(ip6, "%s ", "ip6(x)");

        if (gp->mask_flags & FILTER_GENERIC_1_UDP)
            sprintf(udp, "%s ", (gp->val_flags & FILTER_GENERIC_1_UDP) ? "udp(y)" : "udp(n)");
        else
            sprintf(udp, "%s ", "udp(x)");

        if (gp->mask_flags & FILTER_GENERIC_1_TCP)
            sprintf(tcp, "%s ", (gp->val_flags & FILTER_GENERIC_1_TCP) ? "tcp(y)" : "tcp(n)");
        else
            sprintf(tcp, "%s ", "tcp(x)");

        if (gp->mask_flags & FILTER_GENERIC_1_TCP_OR_UDP)
            sprintf(tcpudp, "%s ", (gp->val_flags & FILTER_GENERIC_1_TCP_OR_UDP) ? "tcpudp(y)" : "tcpudp(n)");
        else
            sprintf(tcpudp, "%s ", "tcpudp(x)");

        if (gp->mask_flags & FILTER_GENERIC_1_IP4SUM_OK)
            sprintf(ip4csum, "%s ", (gp->val_flags & FILTER_GENERIC_1_IP4SUM_OK) ? "ip4csum(y)" : "ip4csum(n)");
        else
            sprintf(ip4csum, "%s ", "ip4csum(x)");

        if (gp->mask_flags & FILTER_GENERIC_1_L4SUM_OK)
            sprintf(l4csum, "%s ", (gp->val_flags & FILTER_GENERIC_1_L4SUM_OK) ? "l4csum(y)" : "l4csum(n)");
        else
            sprintf(l4csum, "%s ", "l4csum(x)");

        if (gp->mask_flags & FILTER_GENERIC_1_IPFRAG)
            sprintf(ipfrag, "%s ", (gp->val_flags & FILTER_GENERIC_1_IPFRAG) ? "ipfrag(y)" : "ipfrag(n)");
        else
            sprintf(ipfrag, "%s ", "ipfrag(x)");
        FLOW_LOG(INFO, "\tFlags: %s%s%s%s%s%s%s%s\n", ip4, ip6, udp, tcp, tcpudp, ip4csum, l4csum, ipfrag);
...
static void enic_dump_flow(const struct filter_action_v2 *ea, const struct filter_v2 *filt) {
 ...
 */
static int enic_flow_parse(struct rte_eth_dev *dev, const struct rte_flow_attr *attrs,
                           const struct rte_flow_item pattern[], const struct rte_flow_action actions[],
                           struct rte_flow_error *error, struct filter_v2 *enic_filter,
                           struct filter_action_v2 *enic_action) {
    unsigned int ret = 0;
    ...
    if (!pattern) {
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_NUM, NULL, "No pattern specified");
        return -rte_errno;
    }

    if (!actions) {
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_NUM, NULL, "No action specified");
        return -rte_errno;
    }
    ...
    if (attrs) {
        if (attrs->group) {
            ...
            rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_PRIORITY, NULL, "priorities are not supported");
            return -rte_errno;
        } else if (attrs->egress) {
            rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_EGRESS, NULL, "egress is not supported");
            return -rte_errno;
        } else if (attrs->transfer) {
            rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_TRANSFER, NULL, "transfer is not supported");
            return -rte_errno;
        } else if (!attrs->ingress) {
            rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_INGRESS, NULL, "only ingress is supported");
            return -rte_errno;
        }

    } else {
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR, NULL, "No attribute specified");
        return -rte_errno;
    }

    /* Verify Actions. */
    enic_action_cap = enic_get_action_cap(enic);
    for (action = &actions[0]; action->type != RTE_FLOW_ACTION_TYPE_END; action++) {
    ...
    if (action->type != RTE_FLOW_ACTION_TYPE_END) {
        rte_flow_error_set(error, EPERM, RTE_FLOW_ERROR_TYPE_ACTION, action, "Invalid action.");
        return -rte_errno;
    }
    ret = enic_action_cap->copy_fn(enic, actions, enic_action);
    if (ret) {
        rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_HANDLE, NULL, "Unsupported action.");
        return -rte_errno;
    }
    ...
    if (enic_filter_cap == NULL) {
        rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_HANDLE, NULL, "Flow API not available");
        return -rte_errno;
    }
 ...
 * @param error[out]
 */
static struct rte_flow *enic_flow_add_filter(struct enic *enic, struct filter_v2 *enic_filter,
                                             struct filter_action_v2 *enic_action, struct rte_flow_error *error) {
    ...
    if (!flow) {
        rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_HANDLE, NULL, "cannot allocate flow memory");
        return NULL;
    }
    ...
    if (enic_action->flags & FILTER_ACTION_COUNTER_FLAG) {
        if (!vnic_dev_counter_alloc(enic->vdev, (uint32_t *)&ctr_idx)) {
            rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_ACTION_CONF, NULL, "cannot allocate counter");
        ...
        if (ctr_idx > enic->max_flow_counter) {
            err = vnic_dev_counter_dma_cfg(enic->vdev, VNIC_FLOW_COUNTER_UPDATE_MSECS, ctr_idx + 1);
            if (err) {
                rte_flow_error_set(error, -err, RTE_FLOW_ERROR_TYPE_ACTION_CONF, NULL, "counter DMA config failed");
    ...
    err = vnic_dev_classifier(enic->vdev, CLSF_ADD, &entry, enic_filter, enic_action);
    if (err) {
        rte_flow_error_set(error, -err, RTE_FLOW_ERROR_TYPE_HANDLE, NULL, "vnic_dev_classifier error");
...
unwind_ctr_dma_cfg:
    if (last_max_flow_ctr != -1) {
        ...
        vnic_dev_counter_dma_cfg(enic->vdev, VNIC_FLOW_COUNTER_UPDATE_MSECS, last_max_flow_ctr + 1);
 ...
 * @param error[out]
 */
static int enic_flow_del_filter(struct enic *enic, struct rte_flow *flow, struct rte_flow_error *error) {
    u16 filter_id;
    int err;
    ...
    if (err) {
        rte_flow_error_set(error, -err, RTE_FLOW_ERROR_TYPE_HANDLE, NULL, "vnic_dev_classifier failed");
        return -err;
    }
 ...
 * @see rte_flow_ops
 */
static int enic_flow_validate(struct rte_eth_dev *dev, const struct rte_flow_attr *attrs,
                              const struct rte_flow_item pattern[], const struct rte_flow_action actions[],
    ...
    ret = enic_flow_parse(dev, attrs, pattern, actions, error, &enic_filter, &enic_action);
    if (!ret)
 ...
 * @see rte_flow_ops
 */
static struct rte_flow *enic_flow_create(struct rte_eth_dev *dev, const struct rte_flow_attr *attrs,
                                         const struct rte_flow_item pattern[], const struct rte_flow_action actions[],
                                         struct rte_flow_error *error) {
    int ret;
    struct filter_v2 enic_filter;
 ...
 * @see rte_flow_ops
 */
static int enic_flow_flush(struct rte_eth_dev *dev, struct rte_flow_error *error) {
    struct rte_flow *flow;
    struct enic *enic = pmd_priv(dev);
...
static int enic_flow_query_count(struct rte_eth_dev *dev, struct rte_flow *flow, void *data,
                                 struct rte_flow_error *error) {
    struct enic *enic = pmd_priv(dev);
    ...
    if (flow->counter_idx == -1) {
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, "flow does not have counter");
    }
    query = (struct rte_flow_query_count *)data;
    if (!vnic_dev_counter_query(enic->vdev, flow->counter_idx, !!query->reset, &packets, &bytes)) {
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, "cannot read counter");
    }
    query->hits_set = 1;
    query->bytes_set = 1;
    query->hits = packets;
...
static int enic_flow_query(struct rte_eth_dev *dev, struct rte_flow *flow, const struct rte_flow_action *actions,
