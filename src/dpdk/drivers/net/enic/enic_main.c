...
static int is_zero_addr(uint8_t *addr) { return !(addr[0] | addr[1] | addr[2] | addr[3] | addr[4] | addr[5]); }
...
static void enic_rxmbuf_queue_release(__rte_unused struct enic *enic, struct vnic_rq *rq) {
    uint16_t i;
...
static void enic_free_rq_buf(struct rte_mbuf **mbuf) {
    if (*mbuf == NULL)
        return;
...
void enic_init_vnic_resources(struct enic *enic) {
    ...
    for (index = 0; index < enic->rq_count; index++) {
        ...
        vnic_rq_init(&enic->rq[enic_rte_rq_idx_to_sop_idx(index)], cq_idx, error_interrupt_enable,
                     error_interrupt_offset);

        data_rq = &enic->rq[enic_rte_rq_idx_to_data_idx(index)];
        if (data_rq->in_use)
            vnic_rq_init(data_rq, cq_idx, error_interrupt_enable, error_interrupt_offset);

        vnic_cq_init(&enic->cq[cq_idx], 0 /* flow_control_enable */, 1 /* color_enable */, 0 /* cq_head */,
                     0 /* cq_tail */, 1 /* cq_tail_color */, rxq_interrupt_enable, 1 /* cq_entry_enable */,
                     0 /* cq_message_enable */, rxq_interrupt_offset, 0 /* cq_message_addr */);
        if (rxq_interrupt_enable)
            rxq_interrupt_offset++;
    }

    for (index = 0; index < enic->wq_count; index++) {
        vnic_wq_init(&enic->wq[index], enic_cq_wq(enic, index), error_interrupt_enable, error_interrupt_offset);
        /* Compute unsupported ol flags for enic_prep_pkts() */
        enic->wq[index].tx_offload_notsup_mask = PKT_TX_OFFLOAD_MASK ^ enic->tx_offload_mask;

        cq_idx = enic_cq_wq(enic, index);
        vnic_cq_init(&enic->cq[cq_idx], 0 /* flow_control_enable */, 1 /* color_enable */, 0 /* cq_head */,
                     0 /* cq_tail */, 1 /* cq_tail_color */, 0 /* interrupt_enable */, 0 /* cq_entry_enable */,
                     1 /* cq_message_enable */, 0 /* interrupt offset */, (u64)enic->wq[index].cqmsg_rz->iova);
    }

    for (index = 0; index < enic->intr_count; index++) {
        vnic_intr_init(&enic->intr[index], enic->config.intr_timer_usec, enic->config.intr_timer_type,
                       /*mask_on_assertion*/ 1);
    }
}

static int enic_alloc_rx_queue_mbufs(struct enic *enic, struct vnic_rq *rq) {
    struct rte_mbuf *mb;
    ...
    for (i = 0; i < rq->ring.desc_count; i++, rqd++) {
        ...
        rq_enet_desc_enc(rqd, dma_addr, (rq->is_sop ? RQ_ENET_TYPE_ONLY_SOP : RQ_ENET_TYPE_NOT_SOP), rq_buf_len);
        rq->mbuf_ring[i] = mb;
    }
    /*
...
static void enic_initial_post_rx(struct enic *enic, struct vnic_rq *rq) {
    ...
    dev_debug(enic, "port=%u, qidx=%u, Write %u posted idx, %u sw held\n", enic->port_id, rq->index, rq->posted_index,
              rq->rx_nb_hold);
...
static void *enic_alloc_consistent(void *priv, size_t size, dma_addr_t *dma_handle, u8 *name) {
    void *vaddr;
    ...
    *dma_handle = 0;
    ...
    rz = rte_memzone_reserve_aligned((const char *)name, size, SOCKET_ID_ANY, RTE_MEMZONE_IOVA_CONTIG, ENIC_ALIGN);
    if (!rz) {
...
static void enic_free_consistent(void *priv, __rte_unused size_t size, void *vaddr, dma_addr_t dma_handle) {
    struct enic_memzone_entry *mze;
    struct enic *enic = (struct enic *)priv;
    ...
    if (mze == NULL) {
        rte_spinlock_unlock(&enic->memzone_list_lock);
        dev_warning(enic, "Tried to free memory, but couldn't find it in the memzone list\n");
        return;
    }
...
static int enic_rxq_intr_init(struct enic *enic) {
    ...
    intr_handle->intr_vec = rte_zmalloc("enic_intr_vec", rxq_intr_count * sizeof(int), 0);
 ...
 */
__rte_weak bool enic_use_vector_rx_handler(__rte_unused struct enic *enic) { return false; }
...
int enic_enable(struct enic *enic) {
    ...
    for (index = 0; index < enic->rq_count; index++) {
        err = enic_alloc_rx_queue_mbufs(enic, &enic->rq[enic_rte_rq_idx_to_sop_idx(index)]);
        if (err) {
            dev_err(enic, "Failed to alloc sop RX queue mbufs\n");
            return err;
        }
        err = enic_alloc_rx_queue_mbufs(enic, &enic->rq[enic_rte_rq_idx_to_data_idx(index)]);
        if (err) {
            /* release the allocated mbufs for the sop rq*/
            enic_rxmbuf_queue_release(enic, &enic->rq[enic_rte_rq_idx_to_sop_idx(index)]);
     ...
     */
    simple_tx_offloads =
        enic->tx_offload_capa & (DEV_TX_OFFLOAD_OUTER_IPV4_CKSUM | DEV_TX_OFFLOAD_VLAN_INSERT |
    ...
    rte_intr_callback_register(&(enic->pdev->intr_handle), enic_intr_handler, (void *)enic->rte_dev);
...
int enic_alloc_intr_resources(struct enic *enic) {
    ...
    dev_info(enic,
             ...
             enic->wq_count, enic_vnic_rq_count(enic), enic->cq_count, enic->intr_count);
...
void enic_free_rq(void *rxq) {
    ...
    if (rq_sop->free_mbufs) {
        ...
        for (i = ENIC_RX_BURST_MAX - rq_sop->num_free_mbufs; i < ENIC_RX_BURST_MAX; i++)
...
int enic_alloc_rq(struct enic *enic, uint16_t queue_idx, unsigned int socket_id, struct rte_mempool *mp,
    ...
    if (enic->rte_dev->data->dev_conf.rxmode.offloads & DEV_RX_OFFLOAD_SCATTER) {
    ...
    if (mbufs_per_pkt > 1) {
         ...
         */
        if (max_rx_pkt_len < enic_mtu_to_max_rx_pktlen(enic->max_mtu)) {
    ...
    if (mbufs_per_pkt > 1) {
        ...
        max_sop = ((enic->config.rq_desc_count / (mbufs_per_pkt - 1)) & ENIC_ALIGN_DESCS_MASK);
    ...
    if (mbufs_per_pkt > 1) {
        dev_info(enic,
                 ...
                 max_rx_pkt_len, mbuf_size, min_sop + min_data, max_sop + max_data);
    }
    dev_info(enic, "Using %d rx descriptors (sop %d, data %d)\n", nb_sop_desc + nb_data_desc, nb_sop_desc,
             nb_data_desc);

    /* Allocate sop queue resources */
    rc = vnic_rq_alloc(enic->vdev, rq_sop, sop_queue_idx, nb_sop_desc, sizeof(struct rq_enet_desc));
    if (rc) {
    ...
    if (rq_data->in_use) {
        ...
        rc = vnic_rq_alloc(enic->vdev, rq_data, data_queue_idx, nb_data_desc, sizeof(struct rq_enet_desc));
        if (rc) {
    ...
    rc = vnic_cq_alloc(enic->vdev, &enic->cq[queue_idx], queue_idx, socket_id, nb_sop_desc + nb_data_desc,
    ...
    rq_sop->mbuf_ring = (struct rte_mbuf **)rte_zmalloc_socket("rq->mbuf_ring", sizeof(struct rte_mbuf *) * nb_sop_desc,
    ...
    if (rq_data->in_use) {
        rq_data->mbuf_ring = (struct rte_mbuf **)rte_zmalloc_socket(
            "rq->mbuf_ring", sizeof(struct rte_mbuf *) * nb_data_desc, RTE_CACHE_LINE_SIZE, rq_sop->socket_id);
        if (rq_data->mbuf_ring == NULL)
            goto err_free_sop_mbuf;
    }

    rq_sop->free_mbufs = (struct rte_mbuf **)rte_zmalloc_socket(
        "rq->free_mbufs", sizeof(struct rte_mbuf *) * ENIC_RX_BURST_MAX, RTE_CACHE_LINE_SIZE, rq_sop->socket_id);
...
int enic_alloc_wq(struct enic *enic, uint16_t queue_idx, unsigned int socket_id, uint16_t nb_desc) {
    ...
    err = vnic_wq_alloc(enic->vdev, &enic->wq[queue_idx], queue_idx, nb_desc, sizeof(struct wq_enet_desc));
    if (err) {
        dev_err(enic, "error in allocation of wq\n");
        return err;
    }

    err = vnic_cq_alloc(enic->vdev, &enic->cq[cq_index], cq_index, socket_id, nb_desc, sizeof(struct cq_enet_wq_desc));
    if (err) {
        vnic_wq_free(wq);
    ...
    /* setup up CQ message */
    snprintf((char *)name, sizeof(name), "vnic_cqmsg-%s-%d-%d", enic->bdf_name, queue_idx, instance++);

    wq->cqmsg_rz = rte_memzone_reserve_aligned((const char *)name, sizeof(uint32_t), SOCKET_ID_ANY,
...
int enic_disable(struct enic *enic) {
    ...
    rte_intr_callback_unregister(&enic->pdev->intr_handle, enic_intr_handler, (void *)enic->rte_dev);
...
static int enic_dev_wait(struct vnic_dev *vdev, int (*start)(struct vnic_dev *, int),
...
static int enic_set_rsskey(struct enic *enic, uint8_t *user_key) {
    ...
    rss_key_buf_va = enic_alloc_consistent(enic, sizeof(union vnic_rss_key), &rss_key_buf_pa, name);
    if (!rss_key_buf_va)
        return -ENOMEM;
    ...
    enic_free_consistent(enic, sizeof(union vnic_rss_key), rss_key_buf_va, rss_key_buf_pa);
...
int enic_set_rss_reta(struct enic *enic, union vnic_rss_cpu *rss_cpu) {
    ...
    rss_cpu_buf_va = enic_alloc_consistent(enic, sizeof(union vnic_rss_cpu), &rss_cpu_buf_pa, name);
    if (!rss_cpu_buf_va)
        return -ENOMEM;
    ...
    enic_free_consistent(enic, sizeof(union vnic_rss_cpu), rss_cpu_buf_va, rss_cpu_buf_pa);
...
static int enic_set_niccfg(struct enic *enic, u8 rss_default_cpu, u8 rss_hash_type, u8 rss_hash_bits, u8 rss_base_cpu,
                           u8 rss_enable) {
    const u8 tso_ipid_split_en = 0;
    int err;

    err = enic_set_nic_cfg(enic, rss_default_cpu, rss_hash_type, rss_hash_bits, rss_base_cpu, rss_enable,
                           tso_ipid_split_en, enic->ig_vlan_strip_en);
...
int enic_init_rss_nic_cfg(struct enic *enic) {
    static uint8_t default_rss_key[] = {
        85, 67, 83, 97, 119, 101, 115, 111, 109, 101, 80, 65, 76, 79, 117, 110, 105, 113, 117, 101,
        76, 73, 78, 85, 88,  114, 111, 99,  107, 115, 69, 78, 73, 67, 105, 115, 99,  111, 111, 108,
    };
    ...
    if (enic->rss_enable) {
        ...
        for (i = 0; i < ENIC_RSS_RETA_SIZE; i++) {
            rss_cpu.cpu[i / 4].b[i % 4] = enic_rte_rq_idx_to_sop_idx(i % enic->rq_count);
        }
...
int enic_setup_finish(struct enic *enic) {
    ...
    /* Default conf */
    vnic_dev_packet_filter(enic->vdev, 1 /* directed  */, 1 /* multicast */, 1 /* broadcast */, 0 /* promisc   */,
                           1 /* allmulti  */);
...
static int enic_rss_conf_valid(struct enic *enic, struct rte_eth_rss_conf *rss_conf) {
    /* RSS is disabled per VIC settings. Ignore rss_conf. */
    if (enic->flow_type_rss_offloads == 0)
        return 0;
    if (rss_conf->rss_key != NULL && rss_conf->rss_key_len != ENIC_RSS_HASH_KEY_SIZE) {
        dev_err(enic, "Given rss_key is %d bytes, it must be %d\n", rss_conf->rss_key_len, ENIC_RSS_HASH_KEY_SIZE);
        return -EINVAL;
    }
    if (rss_conf->rss_hf != 0 && (rss_conf->rss_hf & enic->flow_type_rss_offloads) == 0) {
...
int enic_set_rss_conf(struct enic *enic, struct rte_eth_rss_conf *rss_conf) {
    ...
    if (enic->rq_count > 1 && (eth_dev->data->dev_conf.rxmode.mq_mode & ETH_MQ_RX_RSS_FLAG) && rss_hf != 0) {
        rss_enable = 1;
        if (rss_hf & (ETH_RSS_IPV4 | ETH_RSS_FRAG_IPV4 | ETH_RSS_NONFRAG_IPV4_OTHER))
        ...
        if (rss_hf & (ETH_RSS_IPV6 | ETH_RSS_IPV6_EX | ETH_RSS_FRAG_IPV6 | ETH_RSS_NONFRAG_IPV6_OTHER))
    ...
    ret = enic_set_niccfg(enic, ENIC_RSS_DEFAULT_CPU, rss_hash_type, ENIC_RSS_HASH_BITS, ENIC_RSS_BASE_CPU, rss_enable);
    if (!ret) {
        enic->rss_hf = rss_hf;
...
int enic_set_vlan_strip(struct enic *enic) {
     ...
     */
    return enic_set_niccfg(enic, ENIC_RSS_DEFAULT_CPU, enic->rss_hash_type, ENIC_RSS_HASH_BITS, ENIC_RSS_BASE_CPU,
                           enic->rss_enable);
}

void enic_add_packet_filter(struct enic *enic) {
    /* Args -> directed, multicast, broadcast, promisc, allmulti */
    vnic_dev_packet_filter(enic->vdev, 1, 1, 1, enic->promisc, enic->allmulti);
}

int enic_get_link_status(struct enic *enic) { return vnic_dev_link_status(enic->vdev); }
...
static int enic_reinit_rq(struct enic *enic, unsigned int rq_idx) {
    ...
    vnic_cq_init(&enic->cq[cq_idx], 0 /* flow_control_enable */, 1 /* color_enable */, 0 /* cq_head */, 0 /* cq_tail */,
                 1 /* cq_tail_color */, 0 /* interrupt_enable */, 1 /* cq_entry_enable */, 0 /* cq_message_enable */,
                 0 /* interrupt offset */, 0 /* cq_message_addr */);

    vnic_rq_init_start(sop_rq, enic_cq_rq(enic, enic_rte_rq_idx_to_sop_idx(rq_idx)), 0, sop_rq->ring.desc_count - 1, 1,
                       0);
    if (data_rq->in_use) {
        vnic_rq_init_start(data_rq, enic_cq_rq(enic, enic_rte_rq_idx_to_data_idx(rq_idx)), 0,
...
int enic_set_mtu(struct enic *enic, uint16_t new_mtu) {
    ...
    eth_dev->data->dev_conf.rxmode.max_rx_pkt_len = enic_mtu_to_max_rx_pktlen(new_mtu);
    ...
    for (rq_idx = 0; rq_idx < enic->rq_count; rq_idx++) {
        ...
        enic_free_rq(rq);
        rc = enic_alloc_rq(enic, rq_idx, rq->socket_id, rq->mp, rq->tot_nb_desc, rq->rx_free_thresh);
        if (rc) {
...
static int enic_dev_init(struct enic *enic) {
    ...
    enic->cq = rte_zmalloc("enic_vnic_cq", sizeof(struct vnic_cq) * enic->conf_cq_count, 8);
    enic->intr = rte_zmalloc("enic_vnic_intr", sizeof(struct vnic_intr) * enic->conf_intr_count, 8);
    enic->rq = rte_zmalloc("enic_vnic_rq", sizeof(struct vnic_rq) * enic->conf_rq_count, 8);
    enic->wq = rte_zmalloc("enic_vnic_wq", sizeof(struct vnic_wq) * enic->conf_wq_count, 8);
    ...
    eth_dev->data->mac_addrs =
        rte_zmalloc("enic_mac_addr", sizeof(struct ether_addr) * ENIC_UNICAST_PERFECT_FILTERS, 0);
    if (!eth_dev->data->mac_addrs) {
        dev_err(enic, "mac addr storage alloc failed, aborting.\n");
        return -1;
    }
    ether_addr_copy((struct ether_addr *)enic->mac_addr, eth_dev->data->mac_addrs);
    ...
    if (enic->disable_overlay && enic->vxlan) {
         ...
         */
        if (vnic_dev_overlay_offload_ctrl(enic->vdev, OVERLAY_FEATURE_VXLAN, OVERLAY_OFFLOAD_DISABLE)) {
    ...
    if (!enic->disable_overlay && enic->vxlan &&
        ...
        vnic_dev_overlay_offload_ctrl(enic->vdev, OVERLAY_FEATURE_VXLAN, OVERLAY_OFFLOAD_ENABLE) == 0) {
        enic->tx_offload_capa |=
            DEV_TX_OFFLOAD_OUTER_IPV4_CKSUM | DEV_TX_OFFLOAD_GENEVE_TNL_TSO | DEV_TX_OFFLOAD_VXLAN_TNL_TSO;
        enic->tx_offload_mask |= PKT_TX_OUTER_IPV6 | PKT_TX_OUTER_IPV4 | PKT_TX_OUTER_IP_CKSUM | PKT_TX_TUNNEL_MASK;
    ...
    if (enic->vxlan) {
         ...
         */
        if (vnic_dev_overlay_offload_cfg(enic->vdev, OVERLAY_CFG_VXLAN_PORT_UPDATE, ENIC_DEFAULT_VXLAN_PORT)) {
...
int enic_probe(struct enic *enic) {
    ...
    err = vnic_dev_set_ig_vlan_rewrite_mode(enic->vdev, enic->ig_vlan_rewrite_mode);
    if (err) {
