 ...
 */
uint16_t enic_dummy_recv_pkts(__rte_unused void *rx_queue, __rte_unused struct rte_mbuf **rx_pkts,
...
uint16_t enic_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    ...
    if ((sop_rq->rx_nb_hold + data_rq->rx_nb_hold) > sop_rq->rx_free_thresh) {
        if (data_rq->in_use) {
            data_rq->posted_index = enic_ring_add(data_rq->ring.desc_count, data_rq->posted_index, data_rq->rx_nb_hold);
            data_rq->rx_nb_hold = 0;
        }
        sop_rq->posted_index = enic_ring_add(sop_rq->ring.desc_count, sop_rq->posted_index, sop_rq->rx_nb_hold);
        sop_rq->rx_nb_hold = 0;
...
uint16_t enic_noscatter_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    ...
    while (max_rx) {
        ...
        mb->port = port_id;
        tnl = overlay && (cqd->completed_index_flags & CQ_ENET_RQ_DESC_FLAGS_FCOE) != 0;
...
static inline void enic_free_wq_bufs(struct vnic_wq *wq, u16 completed_index) {
    ...
    nb_to_free = enic_ring_sub(desc_count, wq->tail_idx, completed_index) + 1;
...
uint16_t enic_prep_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
...
uint16_t enic_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    uint16_t index;
    ...
    for (index = 0; index < nb_pkts; index++) {
        ...
        if (tso) {
            ...
            if (unlikely(header_len == 0 || ((tx_pkt->tso_segsz + header_len) > ENIC_TX_MAX_PKT_SIZE))) {
                rte_pktmbuf_free(tx_pkt);
                rte_atomic64_inc(tx_oversized);
                continue;
            }
        ...
        wq_enet_desc_enc(&desc_tmp, bus_addr, data_len, mss, header_len, offload_mode, eop, cq, 0, vlan_tag_insert,
                         vlan_id, 0);
        ...
        *desc_p = desc_tmp;
        ...
        if (!eop) {
            for (tx_pkt = tx_pkt->next; tx_pkt; tx_pkt = tx_pkt->next) {
                ...
                desc_p = descs + head_idx;
                bus_addr = (dma_addr_t)(tx_pkt->buf_iova + tx_pkt->data_off);
                wq_enet_desc_enc((struct wq_enet_desc *)&desc_tmp, bus_addr, data_len, mss, 0, offload_mode, eop, cq, 0,
...
static void enqueue_simple_pkts(struct rte_mbuf **pkts, struct wq_enet_desc *desc, uint16_t n, struct enic *enic) {
    struct rte_mbuf *p;
    uint16_t mss;
    ...
    while (n) {
        ...
        desc->header_length_flags &= ((1 << WQ_ENET_FLAGS_EOP_SHIFT) | (1 << WQ_ENET_FLAGS_CQ_ENTRY_SHIFT));
...
uint16_t enic_simple_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
