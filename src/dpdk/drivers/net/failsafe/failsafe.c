...
static int fs_sub_device_alloc(struct rte_eth_dev *dev, const char *params) {
    ...
    PRIV(dev)->subs = rte_zmalloc(NULL, sizeof(struct sub_device) * nb_subs, RTE_CACHE_LINE_SIZE);
    ...
    FOREACH_SUBDEV(sdev, sdev_iterator, dev) { sdev->sdev_port_id = RTE_MAX_ETHPORTS; }
    return 0;
}

static void fs_sub_device_free(struct rte_eth_dev *dev) { rte_free(PRIV(dev)->subs); }
...
static int fs_eth_dev_create(struct rte_vdev_device *vdev) {
    ...
    snprintf(priv->my_owner.name, sizeof(priv->my_owner.name), FAILSAFE_OWNER_NAME);
    DEBUG("Failsafe port %u owner info: %s_%016" PRIX64, dev->data->port_id, priv->my_owner.name, priv->my_owner.id);
    ret = rte_eth_dev_callback_register(RTE_ETH_ALL, RTE_ETH_EVENT_NEW, failsafe_eth_new_event_callback, dev);
    if (ret) {
    ...
    if (failsafe_mac_from_arg) {
        ...
        if (sdev->state >= DEV_PROBED) {
            ether_addr_copy(&ETH(sdev)->data->mac_addrs[0], mac);
            break;
        }
        /*
    ...
    INFO("MAC address is %02x:%02x:%02x:%02x:%02x:%02x", mac->addr_bytes[0], mac->addr_bytes[1], mac->addr_bytes[2],
...
unregister_new_callback:
    rte_eth_dev_callback_unregister(RTE_ETH_ALL, RTE_ETH_EVENT_NEW, failsafe_eth_new_event_callback, dev);
free_args:
...
static int rte_pmd_failsafe_probe(struct rte_vdev_device *vdev) {
    ...
    if (rte_eal_process_type() == RTE_PROC_SECONDARY && strlen(rte_vdev_device_args(vdev)) == 0) {
