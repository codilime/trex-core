...
typedef int(parse_cb)(struct rte_eth_dev *dev, const char *params, uint8_t head);
...
static int fs_parse_device_param(struct rte_eth_dev *dev, const char *param, uint8_t head) {
    struct fs_priv *priv;
...
static int fs_parse_sub_devices(parse_cb *cb, struct rte_eth_dev *dev, const char *params) {
    size_t a, b;
    uint8_t head;
    int ret;
...
static int fs_get_u64_arg(const char *key __rte_unused, const char *value, void *out) {
    uint64_t *u64 = out;
    char *endptr = NULL;
...
static int fs_get_mac_addr_arg(const char *key __rte_unused, const char *value, void *out) {
    struct ether_addr *ea = out;
    int ret;

    if ((value == NULL) || (out == NULL))
        return -EINVAL;
    ret = sscanf(value, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &ea->addr_bytes[0], &ea->addr_bytes[1], &ea->addr_bytes[2],
...
int failsafe_args_parse(struct rte_eth_dev *dev, const char *params) {
    ...
    if (strnlen(mut_params, sizeof(mut_params)) > 0) {
        ...
        if (arg_count == 1) {
            ret = rte_kvargs_process(kvlist, PMD_FAILSAFE_HOTPLUG_POLL_KVARG, &fs_get_u64_arg, &failsafe_hotplug_poll);
            if (ret < 0)
                goto free_kvlist;
        }
        /* MAC addr */
        ...
        if (arg_count > 0) {
            ret = rte_kvargs_process(kvlist, PMD_FAILSAFE_MAC_KVARG, &fs_get_mac_addr_arg, &dev->data->mac_addrs[0]);
            if (ret < 0)
                goto free_kvlist;
...
static int fs_count_device(struct rte_eth_dev *dev, const char *param, uint8_t head __rte_unused) {
    size_t b = 0;

    while (param[b] != '(' && param[b] != '\0')
        b++;
    if (strncmp(param, "dev", b) != 0 && strncmp(param, "exec", b) != 0 && strncmp(param, "fd(", b) != 0) {
        ERROR("Unrecognized device type: %.*s", (int)b, param);
        return -EINVAL;
    }
