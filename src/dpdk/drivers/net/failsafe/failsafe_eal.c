...
static int fs_ethdev_portid_get(const char *name, uint16_t *port_id) {
    ...
    for (pid = 0; pid < RTE_MAX_ETHPORTS; pid++) {
        if (rte_eth_dev_is_valid_port(pid) && !strncmp(name, rte_eth_devices[pid].device->name, len)) {
            *port_id = pid;
            return 0;
        }
    }
        ...
        if (fs_ethdev_portid_get(da->name, &pid) != 0) {
            ...
            if (pid_owner.id != PRIV(dev)->my_owner.id) {
                INFO("sub_device %d owner(%s_%016" PRIX64 ") is not my,"
                     ...
                     i, pid_owner.name, pid_owner.id, PRIV(dev)->my_owner.name, PRIV(dev)->my_owner.id);
                continue;
            }
        } else {
            ...
            if (probed_da != NULL)
                snprintf(devstr, sizeof(devstr), "%s,%s", probed_da->name, probed_da->args);
            else
            ...
            if (ret < 0) {
                INFO("sub_device %d owner set failed (%s), "
                     ...
                     i, strerror(-ret));
                continue;
            } else if (strncmp(rte_eth_devices[pid].device->name, da->name, strlen(da->name)) != 0) {
                /*
...
static int fs_bus_uninit(struct rte_eth_dev *dev) {
    ...
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_PROBED) {
        ...
        if (sdev_ret) {
            ERROR("Failed to remove requested device %s (err: %d)", sdev->dev->name, sdev_ret);
            continue;
        }
