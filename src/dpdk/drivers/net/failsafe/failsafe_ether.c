...
static int eth_dev_flow_isolate_set(struct rte_eth_dev *dev, struct sub_device *sdev) {
...
static int fs_eth_dev_conf_apply(struct rte_eth_dev *dev, struct sub_device *sdev) {
    ...
    for (i = 0; i < dev->data->nb_rx_queues; i++) {
        ...
        ret =
            rte_eth_rx_queue_setup(PORT_ID(sdev), i, rxq->info.nb_desc, rxq->socket_id, &rxq->info.conf, rxq->info.mp);
        if (ret) {
    ...
    for (i = 0; i < dev->data->nb_tx_queues; i++) {
        ...
        ret = rte_eth_tx_queue_setup(PORT_ID(sdev), i, txq->info.nb_desc, txq->socket_id, &txq->info.conf);
        if (ret) {
    ...
    ret = rte_eth_dev_default_mac_addr_set(PORT_ID(sdev), &dev->data->mac_addrs[0]);
    if (ret) {
    ...
    if (PRIV(dev)->nb_mac_addr > 1)
        DEBUG("Configure additional MAC address%s", (PRIV(dev)->nb_mac_addr > 2 ? "es" : ""));
    for (i = 1; i < PRIV(dev)->nb_mac_addr; i++) {
        struct ether_addr *ea;

        ea = &dev->data->mac_addrs[i];
        ret = rte_eth_dev_mac_addr_add(PORT_ID(sdev), ea, PRIV(dev)->mac_addr_pool[i]);
        if (ret) {
    ...
    if (PRIV(dev)->nb_mcast_addr > 0) {
        ...
        ret = rte_eth_dev_set_mc_addr_list(PORT_ID(sdev), PRIV(dev)->mcast_addrs, PRIV(dev)->nb_mcast_addr);
        if (ret) {
    ...
    if (TAILQ_EMPTY(&PRIV(dev)->flow_list)) {
        ...
        TAILQ_FOREACH(flow, &PRIV(dev)->flow_list, next) {
            ...
            flow->flows[SUB_ID(sdev)] =
                rte_flow_create(PORT_ID(sdev), flow->rule.attr, flow->rule.pattern, flow->rule.actions, &ferror);
            ret = rte_errno;
            if (ret)
                break;
        }
...
static void fs_dev_stats_save(struct sub_device *sdev) {
    ...
    failsafe_stats_increment(&PRIV(fs_dev(sdev))->stats_accumulator, err ? &sdev->stats_snapshot.stats : &stats);
...
void failsafe_eth_dev_unregister_callbacks(struct sub_device *sdev) {
    ...
    if (sdev->rmv_callback) {
        ret = rte_eth_dev_callback_unregister(PORT_ID(sdev), RTE_ETH_EVENT_INTR_RMV, failsafe_eth_rmv_event_callback,
                                              sdev);
        if (ret)
    ...
    if (sdev->lsc_callback) {
        ret = rte_eth_dev_callback_unregister(PORT_ID(sdev), RTE_ETH_EVENT_INTR_LSC, failsafe_eth_lsc_event_callback,
                                              sdev);
        if (ret)
...
static int failsafe_eth_dev_rx_queues_sync(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i < dev->data->nb_rx_queues; i++) {
        ...
        if (rxq->info.conf.rx_deferred_start && dev->data->rx_queue_state[i] == RTE_ETH_QUEUE_STATE_STARTED) {
            /*
...
static int failsafe_eth_dev_tx_queues_sync(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i < dev->data->nb_tx_queues; i++) {
        ...
        if (txq->info.conf.tx_deferred_start && dev->data->tx_queue_state[i] == RTE_ETH_QUEUE_STATE_STARTED) {
            /*
...
void failsafe_stats_increment(struct rte_eth_stats *to, struct rte_eth_stats *from) {
    uint32_t i;
...
int failsafe_eth_rmv_event_callback(uint16_t port_id __rte_unused, enum rte_eth_event_type event __rte_unused,
...
int failsafe_eth_lsc_event_callback(uint16_t port_id __rte_unused, enum rte_eth_event_type event __rte_unused,
                                    void *cb_arg, void *out __rte_unused) {
    ...
    if (ret)
        return _rte_eth_dev_callback_process(dev, RTE_ETH_EVENT_INTR_LSC, NULL);
    else
        return 0;
}

/* Take sub-device ownership before it becomes exposed to the application. */
int failsafe_eth_new_event_callback(uint16_t port_id, enum rte_eth_event_type event __rte_unused, void *cb_arg,
