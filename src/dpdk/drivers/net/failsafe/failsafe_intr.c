...
static int fs_rx_event_proxy_service_install(struct fs_priv *priv) {
    ...
    snprintf(service.name, sizeof(service.name), "%s_Rx_service", priv->data->name);
...
static int failsafe_eth_rx_intr_ctl_subdevice(struct sub_device *sdev, int op) {
    ...
    fsdev = fs_dev(sdev);
    if (sdev == NULL || (ETH(sdev) == NULL) || fsdev == NULL || (PRIV(fsdev) == NULL)) {
...
int failsafe_rx_intr_install_subdevice(struct sub_device *sdev) {
    ...
    struct rte_eth_dev *fsdev;
    struct rxq **rxq;
    const struct rte_intr_conf *const intr_conf = &ETH(sdev)->data->dev_conf.intr_conf;
...
int failsafe_rx_intr_install(struct rte_eth_dev *dev) {
    ...
    const struct rte_intr_conf *const intr_conf = &priv->data->dev_conf.intr_conf;
