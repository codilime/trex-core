...
static struct rte_eth_dev_info default_infos = {
     ...
     */
    .rx_offload_capa = DEV_RX_OFFLOAD_VLAN_STRIP | DEV_RX_OFFLOAD_IPV4_CKSUM | DEV_RX_OFFLOAD_UDP_CKSUM |
                       DEV_RX_OFFLOAD_TCP_CKSUM | DEV_RX_OFFLOAD_TCP_LRO | DEV_RX_OFFLOAD_QINQ_STRIP |
                       DEV_RX_OFFLOAD_OUTER_IPV4_CKSUM | DEV_RX_OFFLOAD_MACSEC_STRIP | DEV_RX_OFFLOAD_HEADER_SPLIT |
                       DEV_RX_OFFLOAD_VLAN_FILTER | DEV_RX_OFFLOAD_VLAN_EXTEND | DEV_RX_OFFLOAD_JUMBO_FRAME |
                       DEV_RX_OFFLOAD_SCATTER | DEV_RX_OFFLOAD_TIMESTAMP | DEV_RX_OFFLOAD_SECURITY,
    .rx_queue_offload_capa = DEV_RX_OFFLOAD_VLAN_STRIP | DEV_RX_OFFLOAD_IPV4_CKSUM | DEV_RX_OFFLOAD_UDP_CKSUM |
                             DEV_RX_OFFLOAD_TCP_CKSUM | DEV_RX_OFFLOAD_TCP_LRO | DEV_RX_OFFLOAD_QINQ_STRIP |
                             DEV_RX_OFFLOAD_OUTER_IPV4_CKSUM | DEV_RX_OFFLOAD_MACSEC_STRIP |
                             DEV_RX_OFFLOAD_HEADER_SPLIT | DEV_RX_OFFLOAD_VLAN_FILTER | DEV_RX_OFFLOAD_VLAN_EXTEND |
                             DEV_RX_OFFLOAD_JUMBO_FRAME | DEV_RX_OFFLOAD_SCATTER | DEV_RX_OFFLOAD_TIMESTAMP |
                             DEV_RX_OFFLOAD_SECURITY,
    .tx_offload_capa = DEV_TX_OFFLOAD_MULTI_SEGS | DEV_TX_OFFLOAD_MBUF_FAST_FREE | DEV_TX_OFFLOAD_IPV4_CKSUM |
                       DEV_TX_OFFLOAD_UDP_CKSUM | DEV_TX_OFFLOAD_TCP_CKSUM | DEV_TX_OFFLOAD_TCP_TSO,
    .flow_type_rss_offloads = ETH_RSS_IP | ETH_RSS_UDP | ETH_RSS_TCP,
    .dev_capa = RTE_ETH_DEV_CAPA_RUNTIME_RX_QUEUE_SETUP | RTE_ETH_DEV_CAPA_RUNTIME_TX_QUEUE_SETUP,
};
...
static int fs_dev_configure(struct rte_eth_dev *dev) {
    ...
    FOREACH_SUBDEV(sdev, i, dev) {
        ...
        if (sdev->state != DEV_PROBED && !(PRIV(dev)->alarm_lock == 0 && sdev->state == DEV_ACTIVE))
            continue;
        ...
        lsc_interrupt = lsc_enabled && (ETH(sdev)->data->dev_flags & RTE_ETH_DEV_INTR_LSC);
        if (lsc_interrupt) {
        ...
        ret = rte_eth_dev_configure(PORT_ID(sdev), dev->data->nb_rx_queues, dev->data->nb_tx_queues,
                                    &dev->data->dev_conf);
        if (ret) {
            if (!fs_err(sdev, ret))
                continue;
        ...
        if (rmv_interrupt && sdev->rmv_callback == 0) {
            ret = rte_eth_dev_callback_register(PORT_ID(sdev), RTE_ETH_EVENT_INTR_RMV, failsafe_eth_rmv_event_callback,
                                                sdev);
            if (ret)
        ...
        if (lsc_interrupt && sdev->lsc_callback == 0) {
            ret = rte_eth_dev_callback_register(PORT_ID(sdev), RTE_ETH_EVENT_INTR_LSC, failsafe_eth_lsc_event_callback,
                                                dev);
            if (ret)
...
static void fs_dev_stop(struct rte_eth_dev *dev) {
    struct sub_device *sdev;
    uint8_t i;
...
static void fs_dev_close(struct rte_eth_dev *dev) {
    struct sub_device *sdev;
    uint8_t i;
...
static void fs_rx_queue_release(void *queue) {
    ...
    if (rxq->event_fd > 0)
        close(rxq->event_fd);
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
        if (ETH(sdev)->data->rx_queues != NULL && ETH(sdev)->data->rx_queues[rxq->qid] != NULL) {
...
static int fs_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id, uint16_t nb_rx_desc, unsigned int socket_id,
    ...
    rxq = rte_zmalloc(NULL, sizeof(*rxq) + sizeof(rte_atomic64_t) * PRIV(dev)->subs_tail, RTE_CACHE_LINE_SIZE);
    if (rxq == NULL) {
        fs_unlock(dev, 0);
        return -ENOMEM;
    }
    ...
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
        ret = rte_eth_rx_queue_setup(PORT_ID(sdev), rx_queue_id, nb_rx_desc, socket_id, rx_conf, mb_pool);
        if ((ret = fs_err(sdev, ret))) {
...
static void fs_tx_queue_release(void *queue) {
    struct rte_eth_dev *dev;
    struct sub_device *sdev;
    uint8_t i;
    struct txq *txq;
    ...
    fs_lock(dev, 0);
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
        if (ETH(sdev)->data->tx_queues != NULL && ETH(sdev)->data->tx_queues[txq->qid] != NULL) {
...
static int fs_tx_queue_setup(struct rte_eth_dev *dev, uint16_t tx_queue_id, uint16_t nb_tx_desc, unsigned int socket_id,
    ...
    txq = rte_zmalloc("ethdev TX queue", sizeof(*txq) + sizeof(rte_atomic64_t) * PRIV(dev)->subs_tail,
                      RTE_CACHE_LINE_SIZE);
    if (txq == NULL) {
        fs_unlock(dev, 0);
        return -ENOMEM;
    ...
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
        ret = rte_eth_tx_queue_setup(PORT_ID(sdev), tx_queue_id, nb_tx_desc, socket_id, tx_conf);
        if ((ret = fs_err(sdev, ret))) {
...
static void fs_promiscuous_enable(struct rte_eth_dev *dev) {
    struct sub_device *sdev;
    uint8_t i;
...
static void fs_promiscuous_disable(struct rte_eth_dev *dev) {
    struct sub_device *sdev;
    uint8_t i;
...
static void fs_allmulticast_enable(struct rte_eth_dev *dev) {
    struct sub_device *sdev;
    uint8_t i;
...
static void fs_allmulticast_disable(struct rte_eth_dev *dev) {
    struct sub_device *sdev;
    uint8_t i;
...
static int fs_link_update(struct rte_eth_dev *dev, int wait_to_complete) {
    ...
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
        ...
        if (ret && ret != -1 && sdev->remove == 0 && rte_eth_dev_is_removed(PORT_ID(sdev)) == 0) {
...
static int fs_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
    ...
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
        ...
        if (ret) {
            ...
            ERROR("Operation rte_eth_stats_get failed for sub_device %d with error %d", i, ret);
            *timestamp = 0;
            fs_unlock(dev, 0);
            return ret;
        }
...
static void fs_stats_reset(struct rte_eth_dev *dev) {
    struct sub_device *sdev;
    uint8_t i;
 ...
 *
 */
static void fs_dev_infos_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *infos) {
    struct sub_device *sdev;
    uint8_t i;
    ...
    if (sdev == NULL) {
        ...
        rte_memcpy(&PRIV(dev)->infos, &default_infos, sizeof(default_infos));
    } else {
        ...
        FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_PROBED) {
            ...
            rxq_offload_capa &= PRIV(dev)->infos.rx_queue_offload_capa;
            rss_hf_offload_capa &= PRIV(dev)->infos.flow_type_rss_offloads;
        ...
        PRIV(dev)->infos.tx_offload_capa &= default_infos.tx_offload_capa;
        PRIV(dev)->infos.tx_queue_offload_capa &= default_infos.tx_queue_offload_capa;
    }
...
static int fs_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
    ...
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
        ...
        if ((ret = fs_err(sdev, ret))) {
            ERROR("Operation rte_eth_dev_set_mtu failed for sub_device %d with error %d", i, ret);
            fs_unlock(dev, 0);
            return ret;
        }
    }
...
static int fs_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id, int on) {
    struct sub_device *sdev;
    uint8_t i;
    int ret;
...
static int fs_flow_ctrl_get(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf) {
    struct sub_device *sdev;
    int ret;
...
static int fs_flow_ctrl_set(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf) {
    struct sub_device *sdev;
    uint8_t i;
    int ret;
...
static int fs_mac_addr_add(struct rte_eth_dev *dev, struct ether_addr *mac_addr, uint32_t index, uint32_t vmdq) {
    struct sub_device *sdev;
    int ret;
    uint8_t i;
    ...
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
        ...
        if ((ret = fs_err(sdev, ret))) {
            ERROR("Operation rte_eth_dev_mac_addr_add failed for sub_device %" PRIu8 " with error %d", i, ret);
            fs_unlock(dev, 0);
            return ret;
        }
    }
...
static int fs_mac_addr_set(struct rte_eth_dev *dev, struct ether_addr *mac_addr) {
    struct sub_device *sdev;
    uint8_t i;
    int ret;
    ...
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
        ...
        ret = fs_err(sdev, ret);
        if (ret) {
            ERROR("Operation rte_eth_dev_mac_addr_set failed for sub_device %d with error %d", i, ret);
            fs_unlock(dev, 0);
            return ret;
        }
    }
...
static int fs_set_mc_addr_list(struct rte_eth_dev *dev, struct ether_addr *mc_addr_set, uint32_t nb_mc_addr) {
    struct sub_device *sdev;
    uint8_t i;
    int ret;
    void *mcast_addrs;
    ...
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
        ...
        if (ret != 0) {
            ERROR("Operation rte_eth_dev_set_mc_addr_list failed for sub_device %d with error %d", i, ret);
            goto rollback;
        }
    }

    mcast_addrs = rte_realloc(PRIV(dev)->mcast_addrs, nb_mc_addr * sizeof(PRIV(dev)->mcast_addrs[0]), 0);
    if (mcast_addrs == NULL && nb_mc_addr > 0) {
        ret = -ENOMEM;
        goto rollback;
    }
    rte_memcpy(mcast_addrs, mc_addr_set, nb_mc_addr * sizeof(PRIV(dev)->mcast_addrs[0]));
...
rollback:
    FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
        int rc = rte_eth_dev_set_mc_addr_list(PORT_ID(sdev), PRIV(dev)->mcast_addrs, PRIV(dev)->nb_mcast_addr);
        if (rc != 0) {
            ERROR("Multicast MAC address list rollback for sub_device %d failed with error %d", i, rc);
        }
    }
...
static int fs_rss_hash_update(struct rte_eth_dev *dev, struct rte_eth_rss_conf *rss_conf) {
    struct sub_device *sdev;
    uint8_t i;
    int ret;
...
static int fs_filter_ctrl(struct rte_eth_dev *dev __rte_unused, enum rte_filter_type type, enum rte_filter_op op,
                          void *arg) {
