...
static int fm10k_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id, int on);
static void fm10k_MAC_filter_set(struct rte_eth_dev *dev, const u8 *mac, bool add, uint32_t pool);
...
static void fm10k_dev_infos_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info);
...
#define FM10K_NB_HW_XSTATS (sizeof(fm10k_hw_stats_strings) / sizeof(fm10k_hw_stats_strings[0]))
...
#define FM10K_NB_RX_Q_XSTATS (sizeof(fm10k_hw_stats_rx_q_strings) / sizeof(fm10k_hw_stats_rx_q_strings[0]))
...
#define FM10K_NB_TX_Q_XSTATS (sizeof(fm10k_hw_stats_tx_q_strings) / sizeof(fm10k_hw_stats_tx_q_strings[0]))

#define FM10K_NB_XSTATS (FM10K_NB_HW_XSTATS + FM10K_MAX_QUEUES_PF * (FM10K_NB_RX_Q_XSTATS + FM10K_NB_TX_Q_XSTATS))
static int fm10k_dev_rxq_interrupt_setup(struct rte_eth_dev *dev);

static void fm10k_mbx_initlock(struct fm10k_hw *hw) { rte_spinlock_init(FM10K_DEV_PRIVATE_TO_MBXLOCK(hw->back)); }
...
static void fm10k_mbx_unlock(struct fm10k_hw *hw) { rte_spinlock_unlock(FM10K_DEV_PRIVATE_TO_MBXLOCK(hw->back)); }

/* Stubs needed for linkage when vPMD is disabled */
__rte_weak int fm10k_rx_vec_condition_check(__rte_unused struct rte_eth_dev *dev) { return -1; }

__rte_weak uint16_t fm10k_recv_pkts_vec(__rte_unused void *rx_queue, __rte_unused struct rte_mbuf **rx_pkts,
                                        __rte_unused uint16_t nb_pkts) {
    return 0;
}

__rte_weak uint16_t fm10k_recv_scattered_pkts_vec(__rte_unused void *rx_queue, __rte_unused struct rte_mbuf **rx_pkts,
                                                  __rte_unused uint16_t nb_pkts) {
    return 0;
}
...
__rte_weak void fm10k_rx_queue_release_mbufs_vec(__rte_unused struct fm10k_rx_queue *rxq) { return; }

__rte_weak void fm10k_txq_vec_setup(__rte_unused struct fm10k_tx_queue *txq) { return; }

__rte_weak int fm10k_tx_vec_condition_check(__rte_unused struct fm10k_tx_queue *txq) { return -1; }

__rte_weak uint16_t fm10k_xmit_fixed_burst_vec(__rte_unused void *tx_queue, __rte_unused struct rte_mbuf **tx_pkts,
                                               __rte_unused uint16_t nb_pkts) {
    return 0;
}
...
static int fm10k_check_mq_mode(struct rte_eth_dev *dev) {
    ...
    if (vmdq_conf->nb_queue_pools > sizeof(vmdq_conf->pool_map[0].pools) * CHAR_BIT ||
...
#define RSS_KEY_SIZE 40
    ...
    if (dev_conf->rxmode.mq_mode != ETH_MQ_RX_RSS || dev_conf->rx_adv_conf.rss_conf.rss_hf == 0) {
...
static void fm10k_dev_logic_port_update(struct rte_eth_dev *dev, uint16_t nb_lport_new) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    uint32_t i;
    ...
    for (i = 0; i < nb_lport_new; i++) {
         ...
         */
        fm10k_mbx_lock(hw);
        hw->mac.ops.update_xcast_mode(hw, hw->mac.dglort_map + i, FM10K_XCAST_MODE_NONE);
        fm10k_mbx_unlock(hw);
    }
}
...
static void fm10k_dev_mq_rx_configure(struct rte_eth_dev *dev) {
    ...
    memset(dev->data->mac_addrs, 0, ETHER_ADDR_LEN * FM10K_MAX_MACADDR_NUM);
    ether_addr_copy((const struct ether_addr *)hw->mac.addr, &dev->data->mac_addrs[0]);
...
static int fm10k_dev_tx_init(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i < dev->data->nb_tx_queues; ++i) {
        ...
        FM10K_WRITE_REG(hw, FM10K_TDBAH(i), base_addr >> (CHAR_BIT * sizeof(uint32_t)));
...
static int fm10k_dev_rx_init(struct rte_eth_dev *dev) {
    ...
    if (rte_intr_dp_is_en(intr_handle)) {
        for (; i < dev->data->nb_rx_queues; i++) {
            ...
            if (hw->mac.type == fm10k_mac_pf)
                FM10K_WRITE_REG(hw, FM10K_ITR(Q2V(pdev, i)), FM10K_ITR_AUTOMASK | FM10K_ITR_MASK_CLEAR);
            else
                FM10K_WRITE_REG(hw, FM10K_VFITR(Q2V(pdev, i)), FM10K_ITR_AUTOMASK | FM10K_ITR_MASK_CLEAR);
        }
    }
    ...
    for (i = 0; i < dev->data->nb_rx_queues; ++i) {
        ...
        FM10K_WRITE_REG(hw, FM10K_SRRCTL(i),
                        (buf_size >> FM10K_SRRCTL_BSIZEPKT_SHIFT) | FM10K_SRRCTL_LOOPBACK_SUPPRESS);

        /* It adds dual VLAN length for supporting dual VLAN */
        if ((dev->data->dev_conf.rxmode.max_rx_pkt_len + 2 * FM10K_VLAN_TAG_SIZE) > buf_size ||
...
static int fm10k_dev_rx_queue_start(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    int err;
    uint32_t reg;
    struct fm10k_rx_queue *rxq;
...
static int fm10k_dev_rx_queue_stop(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int fm10k_dev_tx_queue_start(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int fm10k_dev_tx_queue_stop(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static void fm10k_dev_promiscuous_enable(struct rte_eth_dev *dev) {
    ...
    fm10k_mbx_lock(hw);
    status = hw->mac.ops.update_xcast_mode(hw, hw->mac.dglort_map, FM10K_XCAST_MODE_PROMISC);
    fm10k_mbx_unlock(hw);
...
static void fm10k_dev_allmulticast_enable(struct rte_eth_dev *dev) {
    ...
    fm10k_mbx_lock(hw);
    status = hw->mac.ops.update_xcast_mode(hw, hw->mac.dglort_map, FM10K_XCAST_MODE_ALLMULTI);
    fm10k_mbx_unlock(hw);
...
static void fm10k_dev_allmulticast_disable(struct rte_eth_dev *dev) {
    ...
    status = hw->mac.ops.update_xcast_mode(hw, hw->mac.dglort_map, FM10K_XCAST_MODE_NONE);
    fm10k_mbx_unlock(hw);
...
static void fm10k_dev_dglort_map_configure(struct rte_eth_dev *dev) {
    ...
    dglortmask = (GLORT_PF_MASK << FM10K_DGLORTMAP_MASK_SHIFT) | hw->mac.dglort_map;
    ...
    dglortmask = (GLORT_FD_MASK << FM10K_DGLORTMAP_MASK_SHIFT) | (hw->mac.dglort_map + GLORT_FD_Q_BASE);
...
static void fm10k_dev_stop(struct rte_eth_dev *dev) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct rte_pci_device *pdev = RTE_ETH_DEV_TO_PCI(dev);
    struct rte_intr_handle *intr_handle = &pdev->intr_handle;
    int i;
...
static void fm10k_dev_close(struct rte_eth_dev *dev) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int fm10k_link_update(struct rte_eth_dev *dev, __rte_unused int wait_to_complete) {
    struct fm10k_dev_info *dev_info = FM10K_DEV_PRIVATE_TO_INFO(dev->data->dev_private);
    PMD_INIT_FUNC_TRACE();

    dev->data->dev_link.link_speed = ETH_SPEED_NUM_50G;
    dev->data->dev_link.link_duplex = ETH_LINK_FULL_DUPLEX;
    dev->data->dev_link.link_status = dev_info->sm_down ? ETH_LINK_DOWN : ETH_LINK_UP;
    dev->data->dev_link.link_autoneg = ETH_LINK_FIXED;

    return 0;
}

static int fm10k_xstats_get_names(__rte_unused struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats_names,
    ...
    if (xstats_names != NULL) {
        ...
        for (q = 0; q < FM10K_MAX_QUEUES_PF; q++) {
            for (i = 0; i < FM10K_NB_RX_Q_XSTATS; i++) {
                snprintf(xstats_names[count].name, sizeof(xstats_names[count].name), "rx_q%u_%s", q,
                         fm10k_hw_stats_rx_q_strings[i].name);
                count++;
            }
            for (i = 0; i < FM10K_NB_TX_Q_XSTATS; i++) {
                snprintf(xstats_names[count].name, sizeof(xstats_names[count].name), "tx_q%u_%s", q,
...
static int fm10k_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats, unsigned n) {
    struct fm10k_hw_stats *hw_stats = FM10K_DEV_PRIVATE_TO_STATS(dev->data->dev_private);
    ...
    /* Global stats */
    for (i = 0; i < FM10K_NB_HW_XSTATS; i++) {
        xstats[count].value = *(uint64_t *)(((char *)hw_stats) + fm10k_hw_stats_strings[count].offset);
    ...
    for (q = 0; q < FM10K_MAX_QUEUES_PF; q++) {
        for (i = 0; i < FM10K_NB_RX_Q_XSTATS; i++) {
            xstats[count].value = *(uint64_t *)(((char *)&hw_stats->q[q]) + fm10k_hw_stats_rx_q_strings[i].offset);
            xstats[count].id = count;
            count++;
        }
        for (i = 0; i < FM10K_NB_TX_Q_XSTATS; i++) {
            xstats[count].value = *(uint64_t *)(((char *)&hw_stats->q[q]) + fm10k_hw_stats_tx_q_strings[i].offset);
            xstats[count].id = count;
            count++;
        }
    }
...
static int fm10k_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
    ...
    for (i = 0; (i < RTE_ETHDEV_QUEUE_STAT_CNTRS) && (i < hw->mac.max_queues); ++i) {
...
static void fm10k_stats_reset(struct rte_eth_dev *dev) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct fm10k_hw_stats *hw_stats = FM10K_DEV_PRIVATE_TO_STATS(dev->data->dev_private);
...
static void fm10k_dev_infos_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info) {
    ...
    dev_info->rx_offload_capa = fm10k_get_rx_port_offloads_capa(dev) | dev_info->rx_queue_offload_capa;
    dev_info->tx_queue_offload_capa = fm10k_get_tx_queue_offloads_capa(dev);
    dev_info->tx_offload_capa = fm10k_get_tx_port_offloads_capa(dev) | dev_info->tx_queue_offload_capa;
    ...
    dev_info->speed_capa = ETH_LINK_SPEED_1G | ETH_LINK_SPEED_2_5G | ETH_LINK_SPEED_10G | ETH_LINK_SPEED_25G |
...
static const uint32_t *fm10k_dev_supported_ptypes_get(struct rte_eth_dev *dev) {
    if (dev->rx_pkt_burst == fm10k_recv_pkts || dev->rx_pkt_burst == fm10k_recv_scattered_pkts) {
        static uint32_t ptypes[] = {/* refers to rx_desc_to_ol_flags() */
                                    RTE_PTYPE_L2_ETHER,    RTE_PTYPE_L3_IPV4, RTE_PTYPE_L3_IPV4_EXT, RTE_PTYPE_L3_IPV6,
                                    RTE_PTYPE_L3_IPV6_EXT, RTE_PTYPE_L4_TCP,  RTE_PTYPE_L4_UDP,      RTE_PTYPE_UNKNOWN};

        return ptypes;
    } else if (dev->rx_pkt_burst == fm10k_recv_pkts_vec || dev->rx_pkt_burst == fm10k_recv_scattered_pkts_vec) {
        static uint32_t ptypes_vec[] = {/* refers to fm10k_desc_to_pktype_v() */
...
#else
static const uint32_t *fm10k_dev_supported_ptypes_get(struct rte_eth_dev *dev __rte_unused) { return NULL; }
#endif
...
static int fm10k_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id, int on) {
    ...
    for (mac_index = 0; (mac_index < FM10K_MAX_MACADDR_NUM) && (result == FM10K_SUCCESS); mac_index++) {
        ...
        fm10k_mbx_lock(hw);
        result = fm10k_update_uc_addr(hw, hw->mac.dglort_map, data->mac_addrs[mac_index].addr_bytes, vlan_id, on, 0);
        fm10k_mbx_unlock(hw);
        mac_num++;
    }
    if (result != FM10K_SUCCESS) {
...
static int fm10k_vlan_offload_set(struct rte_eth_dev *dev, int mask) {
...
static void fm10k_MAC_filter_set_main_vsi(struct rte_eth_dev *dev, const u8 *mac, bool add, uint32_t pool) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct fm10k_macvlan_filter_info *macvlan;
    uint32_t i, j, k;
    ...
    for (i = 0, j = 0; j < FM10K_VFTA_SIZE; j++) {
        ...
        for (k = 0; k < FM10K_UINT32_BIT_SIZE; k++) {
            ...
            fm10k_mbx_lock(hw);
            fm10k_update_uc_addr(hw, hw->mac.dglort_map, mac, j * FM10K_UINT32_BIT_SIZE + k, add, 0);
            fm10k_mbx_unlock(hw);
            i++;
        }
    }
...
static void fm10k_MAC_filter_set_vmdq(struct rte_eth_dev *dev, const u8 *mac, bool add, uint32_t pool) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct fm10k_macvlan_filter_info *macvlan;
    struct rte_eth_vmdq_rx_conf *vmdq_conf;
    uint32_t i;
    ...
    for (i = 0; i < vmdq_conf->nb_pool_maps; i++) {
        if (!(vmdq_conf->pool_map[i].pools & (1UL << pool)))
            continue;
        fm10k_mbx_lock(hw);
        fm10k_update_uc_addr(hw, hw->mac.dglort_map + pool, mac, vmdq_conf->pool_map[i].vlan_id, add, 0);
        fm10k_mbx_unlock(hw);
    }
}
...
static int fm10k_macaddr_add(struct rte_eth_dev *dev, struct ether_addr *mac_addr, uint32_t index, uint32_t pool) {
    struct fm10k_macvlan_filter_info *macvlan;
...
static void fm10k_macaddr_remove(struct rte_eth_dev *dev, uint32_t index) {
    ...
    macvlan = FM10K_DEV_PRIVATE_TO_MACVLAN(dev->data->dev_private);
    fm10k_MAC_filter_set(dev, data->mac_addrs[index].addr_bytes, FALSE, macvlan->mac_vmdq_id[index]);
    macvlan->mac_vmdq_id[index] = 0;
}

static inline int check_nb_desc(uint16_t min, uint16_t max, uint16_t mult, uint16_t request) {
...
static inline int check_thresh(uint16_t min, uint16_t max, uint16_t div, uint16_t request) {
...
static inline int handle_rxconf(struct fm10k_rx_queue *q, const struct rte_eth_rxconf *conf) {
    ...
    if (check_thresh(FM10K_RX_FREE_THRESH_MIN(q), FM10K_RX_FREE_THRESH_MAX(q), FM10K_RX_FREE_THRESH_DIV(q),
                     rx_free_thresh)) {
        PMD_INIT_LOG(ERR,
                     ...
                     "and a divisor of %u",
                     rx_free_thresh, FM10K_RX_FREE_THRESH_MAX(q), FM10K_RX_FREE_THRESH_MIN(q),
...
static uint64_t fm10k_get_rx_port_offloads_capa(struct rte_eth_dev *dev) {
    ...
    return (uint64_t)(DEV_RX_OFFLOAD_VLAN_STRIP | DEV_RX_OFFLOAD_VLAN_FILTER | DEV_RX_OFFLOAD_IPV4_CKSUM |
                      DEV_RX_OFFLOAD_UDP_CKSUM | DEV_RX_OFFLOAD_TCP_CKSUM | DEV_RX_OFFLOAD_JUMBO_FRAME |
                      DEV_RX_OFFLOAD_HEADER_SPLIT);
}

static int fm10k_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_id, uint16_t nb_desc, unsigned int socket_id,
    ...
    if (check_nb_desc(FM10K_MIN_RX_DESC, FM10K_MAX_RX_DESC, FM10K_MULT_RX_DESC, nb_desc)) {
        PMD_INIT_LOG(ERR,
                     "Number of Rx descriptors (%u) must be "
                     "less than or equal to %" PRIu32 ", "
                     "greater than or equal to %u, "
                     "and a multiple of %u",
                     nb_desc, (uint32_t)FM10K_MAX_RX_DESC, FM10K_MIN_RX_DESC, FM10K_MULT_RX_DESC);
        return -EINVAL;
    }
    ...
    q->queue_id = queue_id;
    q->tail_ptr = (volatile uint32_t *)&((uint32_t *)hw->hw_addr)[FM10K_RDT(queue_id)];
    q->offloads = offloads;
    if (handle_rxconf(q, conf))
        return -EINVAL;

    /* allocate memory for the software ring */
    q->sw_ring = rte_zmalloc_socket("fm10k sw ring", (nb_desc + q->nb_fake_desc) * sizeof(struct rte_mbuf *),
     ...
     */
    mz = rte_eth_dma_zone_reserve(dev, "rx_ring", queue_id, FM10K_MAX_RX_RING_SZ, FM10K_ALIGN_RX_DESC, socket_id);
    if (mz == NULL) {
...
static inline int handle_txconf(struct fm10k_tx_queue *q, const struct rte_eth_txconf *conf) {
    ...
    /* make sure the requested threshold satisfies the constraints */
    if (check_thresh(FM10K_TX_FREE_THRESH_MIN(q), FM10K_TX_FREE_THRESH_MAX(q), FM10K_TX_FREE_THRESH_DIV(q),
                     tx_free_thresh)) {
        PMD_INIT_LOG(ERR,
                     ...
                     "greater than or equal to %u, "
                     "and a divisor of %u",
                     tx_free_thresh, FM10K_TX_FREE_THRESH_MAX(q), FM10K_TX_FREE_THRESH_MIN(q),
    ...
    /* make sure the requested threshold satisfies the constraints */
    if (check_thresh(FM10K_TX_RS_THRESH_MIN(q), FM10K_TX_RS_THRESH_MAX(q), FM10K_TX_RS_THRESH_DIV(q), tx_rs_thresh)) {
        PMD_INIT_LOG(ERR,
                     "tx_rs_thresh (%u) must be "
                     "less than or equal to %u, "
                     "greater than or equal to %u, "
                     "and a divisor of %u",
                     tx_rs_thresh, FM10K_TX_RS_THRESH_MAX(q), FM10K_TX_RS_THRESH_MIN(q), FM10K_TX_RS_THRESH_DIV(q));
        return -EINVAL;
    }
...
static uint64_t fm10k_get_tx_port_offloads_capa(struct rte_eth_dev *dev) {
    ...
    return (uint64_t)(DEV_TX_OFFLOAD_VLAN_INSERT | DEV_TX_OFFLOAD_MULTI_SEGS | DEV_TX_OFFLOAD_IPV4_CKSUM |
                      DEV_TX_OFFLOAD_UDP_CKSUM | DEV_TX_OFFLOAD_TCP_CKSUM | DEV_TX_OFFLOAD_TCP_TSO);
}

static int fm10k_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_id, uint16_t nb_desc, unsigned int socket_id,
    ...
    /* make sure a valid number of descriptors have been requested */
    if (check_nb_desc(FM10K_MIN_TX_DESC, FM10K_MAX_TX_DESC, FM10K_MULT_TX_DESC, nb_desc)) {
        PMD_INIT_LOG(ERR,
                     "Number of Tx descriptors (%u) must be "
                     "less than or equal to %" PRIu32 ", "
                     "greater than or equal to %u, "
                     "and a multiple of %u",
                     nb_desc, (uint32_t)FM10K_MAX_TX_DESC, FM10K_MIN_TX_DESC, FM10K_MULT_TX_DESC);
        return -EINVAL;
    }
    ...
    q->offloads = offloads;
    q->ops = &def_txq_ops;
    q->tail_ptr = (volatile uint32_t *)&((uint32_t *)hw->hw_addr)[FM10K_TDT(queue_id)];
    if (handle_txconf(q, conf))
        return -EINVAL;

    /* allocate memory for the software ring */
    q->sw_ring =
        rte_zmalloc_socket("fm10k sw ring", nb_desc * sizeof(struct rte_mbuf *), RTE_CACHE_LINE_SIZE, socket_id);
    if (q->sw_ring == NULL) {
        PMD_INIT_LOG(ERR, "Cannot allocate software ring");
        rte_free(q);
        return -ENOMEM;
     ...
     * resizing in later calls to the queue setup function.
     */
    mz = rte_eth_dma_zone_reserve(dev, "tx_ring", queue_id, FM10K_MAX_TX_RING_SZ, FM10K_ALIGN_TX_DESC, socket_id);
    if (mz == NULL) {
        PMD_INIT_LOG(ERR, "Cannot allocate hardware ring");
        rte_free(q->sw_ring);
        rte_free(q);
     ...
     */
    q->rs_tracker.list = rte_zmalloc_socket("fm10k rs tracker", ((nb_desc + 1) / q->rs_thresh) * sizeof(uint16_t),
                                            RTE_CACHE_LINE_SIZE, socket_id);
...
static int fm10k_reta_update(struct rte_eth_dev *dev, struct rte_eth_rss_reta_entry64 *reta_conf, uint16_t reta_size) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int fm10k_reta_query(struct rte_eth_dev *dev, struct rte_eth_rss_reta_entry64 *reta_conf, uint16_t reta_size) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    uint16_t i, j, idx, shift;
    uint8_t mask;
    uint32_t reta;
...
static int fm10k_rss_hash_update(struct rte_eth_dev *dev, struct rte_eth_rss_conf *rss_conf) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    ...
    if (key && (rss_conf->rss_key_len < FM10K_RSSRK_SIZE * FM10K_RSSRK_ENTRIES_PER_REG))
        return -EINVAL;
...
static int fm10k_rss_hash_conf_get(struct rte_eth_dev *dev, struct rte_eth_rss_conf *rss_conf) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    uint32_t *key = (uint32_t *)rss_conf->rss_key;
    uint32_t mrqc;
    uint64_t hf;
...
static void fm10k_dev_enable_intr_pf(struct rte_eth_dev *dev) {
    ...
    FM10K_WRITE_REG(hw, FM10K_EIMR,
                    FM10K_EIMR_ENABLE(PCA_FAULT) | FM10K_EIMR_ENABLE(THI_FAULT) | FM10K_EIMR_ENABLE(FUM_FAULT) |
                        FM10K_EIMR_ENABLE(MAILBOX) | FM10K_EIMR_ENABLE(SWITCHREADY) |
                        FM10K_EIMR_ENABLE(SWITCHNOTREADY) | FM10K_EIMR_ENABLE(SRAMERROR) | FM10K_EIMR_ENABLE(VFLR));
...
static void fm10k_dev_disable_intr_pf(struct rte_eth_dev *dev) {
    ...
    /* Disable misc causes */
    FM10K_WRITE_REG(hw, FM10K_EIMR,
                    FM10K_EIMR_DISABLE(PCA_FAULT) | FM10K_EIMR_DISABLE(THI_FAULT) | FM10K_EIMR_DISABLE(FUM_FAULT) |
                        FM10K_EIMR_DISABLE(MAILBOX) | FM10K_EIMR_DISABLE(SWITCHREADY) |
                        FM10K_EIMR_DISABLE(SWITCHNOTREADY) | FM10K_EIMR_DISABLE(SRAMERROR) | FM10K_EIMR_DISABLE(VFLR));
...
static void fm10k_dev_enable_intr_vf(struct rte_eth_dev *dev) {
    ...
    /* Enable ITR 0 */
    FM10K_WRITE_REG(hw, FM10K_VFITR(0), FM10K_ITR_AUTOMASK | FM10K_ITR_MASK_CLEAR);
    FM10K_WRITE_FLUSH(hw);
}
...
static int fm10k_dev_rx_queue_intr_enable(struct rte_eth_dev *dev, uint16_t queue_id) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct rte_pci_device *pdev = RTE_ETH_DEV_TO_PCI(dev);

    /* Enable ITR */
    if (hw->mac.type == fm10k_mac_pf)
        FM10K_WRITE_REG(hw, FM10K_ITR(Q2V(pdev, queue_id)), FM10K_ITR_AUTOMASK | FM10K_ITR_MASK_CLEAR);
    else
        FM10K_WRITE_REG(hw, FM10K_VFITR(Q2V(pdev, queue_id)), FM10K_ITR_AUTOMASK | FM10K_ITR_MASK_CLEAR);
    rte_intr_enable(&pdev->intr_handle);
    return 0;
}

static int fm10k_dev_rx_queue_intr_disable(struct rte_eth_dev *dev, uint16_t queue_id) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct rte_pci_device *pdev = RTE_ETH_DEV_TO_PCI(dev);
...
static int fm10k_dev_rxq_interrupt_setup(struct rte_eth_dev *dev) {
     ...
     */
    if (!rte_intr_cap_multiple(intr_handle) || dev->data->dev_conf.intr_conf.rxq == 0)
        return result;
    ...
    if (rte_intr_dp_is_en(intr_handle) && !result) {
        ...
        if (intr_handle->intr_vec) {
            for (queue_id = 0, vec = FM10K_RX_VEC_START; queue_id < dev->data->nb_rx_queues; queue_id++) {
...
static int fm10k_dev_handle_fault(struct fm10k_hw *hw, uint32_t eicr) {
    ...
    if (eicr & FM10K_EICR_PCA_FAULT) {
        ...
        PMD_INIT_LOG(ERR, "%s: %s(%d) Addr:0x%" PRIx64 " Spec: 0x%x", estr, fault.func ? "VF" : "PF", fault.func,
...
static void fm10k_dev_interrupt_handler_pf(void *param) {
    ...
    if (cause & FM10K_EICR_SWITCHREADY) {
        ...
        if (dev_info->sm_down == 1) {
            ...
            status_mbx = hw->mac.ops.update_lport_state(hw, hw->mac.dglort_map, MAX_LPORT_NUM, 1);
            ...
            status_mbx = hw->mac.ops.update_xcast_mode(hw, hw->mac.dglort_map, FM10K_XCAST_MODE_NONE);
            if (status_mbx != FM10K_SUCCESS)
    ...
    cause &= FM10K_EICR_SWITCHNOTREADY | FM10K_EICR_MAILBOX | FM10K_EICR_SWITCHREADY;
    if (cause)
...
static int ftag_check_handler(__rte_unused const char *key, const char *value, __rte_unused void *opaque) {
    if (strcmp(value, "1"))
        return -1;
...
static uint16_t fm10k_xmit_pkts_vec(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    uint16_t nb_tx = 0;
...
static void __attribute__((cold)) fm10k_set_tx_function(struct rte_eth_dev *dev) {
    struct fm10k_tx_queue *txq;
    int i;
    int use_sse = 1;
...
static void __attribute__((cold)) fm10k_set_rx_function(struct rte_eth_dev *dev) {
    struct fm10k_dev_info *dev_info = FM10K_DEV_PRIVATE_TO_INFO(dev->data->dev_private);
     ...
     */
    if (!fm10k_rx_vec_condition_check(dev) && dev_info->rx_vec_allowed && !rx_ftag_en) {
    ...
    rx_using_sse = (dev->rx_pkt_burst == fm10k_recv_scattered_pkts_vec || dev->rx_pkt_burst == fm10k_recv_pkts_vec);
...
static void fm10k_params_init(struct rte_eth_dev *dev) {
    struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct fm10k_dev_info *info = FM10K_DEV_PRIVATE_TO_INFO(dev->data->dev_private);
...
static int eth_fm10k_dev_init(struct rte_eth_dev *dev) {
    ...
    dev->data->mac_addrs = rte_zmalloc("fm10k", ETHER_ADDR_LEN * FM10K_MAX_MACADDR_NUM, 0);
    ...
    if (diag != FM10K_SUCCESS || !is_valid_assigned_ether_addr(dev->data->mac_addrs)) {
    ...
    if (hw->mac.type == fm10k_mac_pf) {
        ...
        rte_intr_callback_register(intr_handle, fm10k_dev_interrupt_handler_pf, (void *)dev);

        /* enable MISC interrupt */
        fm10k_dev_enable_intr_pf(dev);
    } else { /* VF */
        rte_intr_callback_register(intr_handle, fm10k_dev_interrupt_handler_vf, (void *)dev);
...
static int eth_fm10k_dev_uninit(struct rte_eth_dev *dev) {
    ...
    if (hw->mac.type == fm10k_mac_pf) {
        ...
        rte_intr_callback_unregister(intr_handle, fm10k_dev_interrupt_handler_pf, (void *)dev);
    } else {
        /* disable interrupt */
        fm10k_dev_disable_intr_vf(dev);

        rte_intr_callback_unregister(intr_handle, fm10k_dev_interrupt_handler_vf, (void *)dev);
    }

    return 0;
}

static int eth_fm10k_pci_probe(struct rte_pci_driver *pci_drv __rte_unused, struct rte_pci_device *pci_dev) {
    return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct fm10k_adapter), eth_fm10k_dev_init);
}
