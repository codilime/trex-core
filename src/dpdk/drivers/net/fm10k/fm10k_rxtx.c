...
static inline void dump_rxd(union fm10k_rx_desc *rxd) {
    ...
    PMD_RX_LOG(DEBUG, "|   0x%08x   |   0x%08x   |", rxd->d.vlan_len, rxd->d.staterr);
    PMD_RX_LOG(DEBUG, "+----------------|----------------+");
...
#define FM10K_TX_OFFLOAD_MASK                                                                                          \
    (PKT_TX_VLAN_PKT | PKT_TX_IPV6 | PKT_TX_IPV4 | PKT_TX_IP_CKSUM | PKT_TX_L4_MASK | PKT_TX_TCP_SEG)

#define FM10K_TX_OFFLOAD_NOTSUP_MASK (PKT_TX_OFFLOAD_MASK ^ FM10K_TX_OFFLOAD_MASK)

/* @note: When this function is changed, make corresponding change to
 * fm10k_dev_supported_ptypes_get()
 */
static inline void rx_desc_to_ol_flags(struct rte_mbuf *m, const union fm10k_rx_desc *d) {
    static const uint32_t ptype_table[FM10K_RXD_PKTTYPE_MASK >> FM10K_RXD_PKTTYPE_SHIFT] __rte_cache_aligned = {
        ...
        [FM10K_PKTTYPE_IPV4 | FM10K_PKTTYPE_TCP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_TCP,
        [FM10K_PKTTYPE_IPV6 | FM10K_PKTTYPE_TCP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_TCP,
        [FM10K_PKTTYPE_IPV4 | FM10K_PKTTYPE_UDP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_UDP,
        [FM10K_PKTTYPE_IPV6 | FM10K_PKTTYPE_UDP] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_UDP,
    };

    m->packet_type = ptype_table[(d->w.pkt_info & FM10K_RXD_PKTTYPE_MASK) >> FM10K_RXD_PKTTYPE_SHIFT];

    if (d->w.pkt_info & FM10K_RXD_RSSTYPE_MASK)
        m->ol_flags |= PKT_RX_RSS_HASH;

    if (unlikely((d->d.staterr & (FM10K_RXD_STATUS_IPCS | FM10K_RXD_STATUS_IPE)) ==
                 (FM10K_RXD_STATUS_IPCS | FM10K_RXD_STATUS_IPE)))
        m->ol_flags |= PKT_RX_IP_CKSUM_BAD;
    else
        m->ol_flags |= PKT_RX_IP_CKSUM_GOOD;

    if (unlikely((d->d.staterr & (FM10K_RXD_STATUS_L4CS | FM10K_RXD_STATUS_L4E)) ==
...
uint16_t fm10k_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    ...
    if ((q->next_dd > q->next_trigger) || (alloc == 1)) {
        ret = rte_mempool_get_bulk(q->mp, (void **)&q->sw_ring[q->next_alloc], q->alloc_thresh);
...
uint16_t fm10k_recv_scattered_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    struct rte_mbuf *mbuf;
    union fm10k_rx_desc desc;
    struct fm10k_rx_queue *q = rx_queue;
    uint16_t count = 0;
    ...
    for (count = 0; count < nb_seg; count++) {
        ...
        if (!first_seg) {
            ...
            first_seg->pkt_len = (uint16_t)(first_seg->pkt_len + rte_pktmbuf_data_len(mbuf));
            first_seg->nb_segs++;
            last_seg->next = mbuf;
        }
...
static inline void tx_xmit_pkt(struct fm10k_tx_queue *q, struct rte_mbuf *mb) {
    ...
    if (mb->ol_flags & PKT_TX_TCP_SEG) {
        hdrlen = mb->outer_l2_len + mb->outer_l3_len + mb->l2_len + mb->l3_len + mb->l4_len;
        if (q->hw_ring[q->next_free].flags & FM10K_TXD_FLAG_FTAG)
            hdrlen += sizeof(struct fm10k_ftag);

        if (likely((hdrlen >= FM10K_TSO_MIN_HEADERLEN) && (hdrlen <= FM10K_TSO_MAX_HEADERLEN) &&
...
uint16_t fm10k_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
...
uint16_t fm10k_prep_pkts(__rte_unused void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    int i, ret;
    struct rte_mbuf *m;
    ...
    for (i = 0; i < nb_pkts; i++) {
        ...
        if ((m->ol_flags & PKT_TX_TCP_SEG) && (m->tso_segsz < FM10K_TSO_MINMSS)) {
            rte_errno = -EINVAL;
            return i;
        }
