...
#define I40E_PFINT_ICR0_ENA_MASK                                                                                       \
    (I40E_PFINT_ICR0_ENA_ECC_ERR_MASK | I40E_PFINT_ICR0_ENA_MAL_DETECT_MASK | I40E_PFINT_ICR0_ENA_GRST_MASK |          \
     I40E_PFINT_ICR0_ENA_PCI_EXCEPTION_MASK | I40E_PFINT_ICR0_ENA_STORM_DETECT_MASK |                                  \
     I40E_PFINT_ICR0_ENA_HMC_ERR_MASK | I40E_PFINT_ICR0_ENA_PE_CRITERR_MASK | I40E_PFINT_ICR0_ENA_VFLR_MASK |          \
     I40E_PFINT_ICR0_ENA_ADMINQ_MASK)

#define I40E_FLOW_TYPES                                                                                                \
    ((1UL << RTE_ETH_FLOW_FRAG_IPV4) | (1UL << RTE_ETH_FLOW_NONFRAG_IPV4_TCP) |                                        \
     (1UL << RTE_ETH_FLOW_NONFRAG_IPV4_UDP) | (1UL << RTE_ETH_FLOW_NONFRAG_IPV4_SCTP) |                                \
     (1UL << RTE_ETH_FLOW_NONFRAG_IPV4_OTHER) | (1UL << RTE_ETH_FLOW_FRAG_IPV6) |                                      \
     (1UL << RTE_ETH_FLOW_NONFRAG_IPV6_TCP) | (1UL << RTE_ETH_FLOW_NONFRAG_IPV6_UDP) |                                 \
     (1UL << RTE_ETH_FLOW_NONFRAG_IPV6_SCTP) | (1UL << RTE_ETH_FLOW_NONFRAG_IPV6_OTHER) |                              \
...
static int i40e_dev_queue_stats_mapping_set(struct rte_eth_dev *dev, uint16_t queue_id, uint8_t stat_idx,
                                            uint8_t is_rx);
static int i40e_fw_version_get(struct rte_eth_dev *dev, char *fw_version, size_t fw_size);
static void i40e_dev_info_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info);
...
static int i40e_macaddr_add(struct rte_eth_dev *dev, struct ether_addr *mac_addr, uint32_t index, uint32_t pool);
static void i40e_macaddr_remove(struct rte_eth_dev *dev, uint32_t index);
static int i40e_dev_rss_reta_update(struct rte_eth_dev *dev, struct rte_eth_rss_reta_entry64 *reta_conf,
                                    uint16_t reta_size);
static int i40e_dev_rss_reta_query(struct rte_eth_dev *dev, struct rte_eth_rss_reta_entry64 *reta_conf,
                                   uint16_t reta_size);
...
static void i40e_stat_update_48(struct i40e_hw *hw, uint32_t hireg, uint32_t loreg, bool offset_loaded,
...
static inline int i40e_find_all_mac_for_vlan(struct i40e_vsi *vsi, struct i40e_macvlan_filter *mv_f, int num,
                                             uint16_t vlan);
...
static int i40e_dev_filter_ctrl(struct rte_eth_dev *dev, enum rte_filter_type filter_type, enum rte_filter_op filter_op,
                                void *arg);
...
static enum i40e_status_code i40e_aq_del_mirror_rule(struct i40e_hw *hw, uint16_t seid, uint16_t rule_type,
                                                     uint16_t *entries, uint16_t count, uint16_t rule_id);
static int i40e_mirror_rule_set(struct rte_eth_dev *dev, struct rte_eth_mirror_conf *mirror_conf, uint8_t sw_id,
                                uint8_t on);
...
static int i40e_set_default_mac_addr(struct rte_eth_dev *dev, struct ether_addr *mac_addr);

static int i40e_dev_mtu_set(struct rte_eth_dev *dev, uint16_t mtu);

static int i40e_ethertype_filter_convert(const struct rte_eth_ethertype_filter *input,
                                         struct i40e_ethertype_filter *filter);
static int i40e_sw_ethertype_filter_insert(struct i40e_pf *pf, struct i40e_ethertype_filter *filter);

static int i40e_tunnel_filter_convert(struct i40e_aqc_cloud_filters_element_bb *cld_filter,
...
static const char *const valid_keys[] = {ETH_I40E_FLOATING_VEB_ARG,     ETH_I40E_FLOATING_VEB_LIST_ARG,
                                         ETH_I40E_SUPPORT_MULTI_DRIVER, ETH_I40E_QUEUE_NUM_PER_VF_ARG,
                                         ETH_I40E_USE_LATEST_VEC,       NULL};
...
#define I40E_NB_ETH_XSTATS (sizeof(rte_i40e_stats_strings) / sizeof(rte_i40e_stats_strings[0]))

static const struct rte_i40e_xstats_name_off rte_i40e_hw_port_strings[] = {
    {"tx_link_down_dropped", offsetof(struct i40e_hw_port_stats, tx_dropped_link_down)},
    {"rx_crc_errors", offsetof(struct i40e_hw_port_stats, crc_errors)},
    {"rx_illegal_byte_errors", offsetof(struct i40e_hw_port_stats, illegal_bytes)},
    ...
    {"mac_remote_errors", offsetof(struct i40e_hw_port_stats, mac_remote_faults)},
    ...
    {"rx_size_65_to_127_packets", offsetof(struct i40e_hw_port_stats, rx_size_127)},
    {"rx_size_128_to_255_packets", offsetof(struct i40e_hw_port_stats, rx_size_255)},
    {"rx_size_256_to_511_packets", offsetof(struct i40e_hw_port_stats, rx_size_511)},
    {"rx_size_512_to_1023_packets", offsetof(struct i40e_hw_port_stats, rx_size_1023)},
    {"rx_size_1024_to_1522_packets", offsetof(struct i40e_hw_port_stats, rx_size_1522)},
    {"rx_size_1523_to_max_packets", offsetof(struct i40e_hw_port_stats, rx_size_big)},
    ...
    {"rx_mac_short_dropped", offsetof(struct i40e_hw_port_stats, mac_short_packet_dropped)},
    ...
    {"tx_size_65_to_127_packets", offsetof(struct i40e_hw_port_stats, tx_size_127)},
    {"tx_size_128_to_255_packets", offsetof(struct i40e_hw_port_stats, tx_size_255)},
    {"tx_size_256_to_511_packets", offsetof(struct i40e_hw_port_stats, tx_size_511)},
    {"tx_size_512_to_1023_packets", offsetof(struct i40e_hw_port_stats, tx_size_1023)},
    {"tx_size_1024_to_1522_packets", offsetof(struct i40e_hw_port_stats, tx_size_1522)},
    {"tx_size_1523_to_max_packets", offsetof(struct i40e_hw_port_stats, tx_size_big)},
    {"rx_flow_director_atr_match_packets", offsetof(struct i40e_hw_port_stats, fd_atr_match)},
    {"rx_flow_director_sb_match_packets", offsetof(struct i40e_hw_port_stats, fd_sb_match)},
    {"tx_low_power_idle_status", offsetof(struct i40e_hw_port_stats, tx_lpi_status)},
    {"rx_low_power_idle_status", offsetof(struct i40e_hw_port_stats, rx_lpi_status)},
    {"tx_low_power_idle_count", offsetof(struct i40e_hw_port_stats, tx_lpi_count)},
    {"rx_low_power_idle_count", offsetof(struct i40e_hw_port_stats, rx_lpi_count)},
};

#define I40E_NB_HW_PORT_XSTATS (sizeof(rte_i40e_hw_port_strings) / sizeof(rte_i40e_hw_port_strings[0]))
...
#define I40E_NB_RXQ_PRIO_XSTATS (sizeof(rte_i40e_rxq_prio_strings) / sizeof(rte_i40e_rxq_prio_strings[0]))
...
static const struct rte_i40e_xstats_name_off rte_i40e_txq_prio_strings[] = {
    ...
    {"xon_to_xoff_packets", offsetof(struct i40e_hw_port_stats, priority_xon_2_xoff)},
};

#define I40E_NB_TXQ_PRIO_XSTATS (sizeof(rte_i40e_txq_prio_strings) / sizeof(rte_i40e_txq_prio_strings[0]))

static int eth_i40e_pci_probe(struct rte_pci_driver *pci_drv __rte_unused, struct rte_pci_device *pci_dev) {
    ...
    for (i = 0; i < eth_da.nb_representor_ports; i++) {
        struct i40e_vf_representor representor = {
            ...
            .switch_domain_id = I40E_DEV_PRIVATE_TO_PF(pf_ethdev->data->dev_private)->switch_domain_id,
            .adapter = I40E_DEV_PRIVATE_TO_ADAPTER(pf_ethdev->data->dev_private)};

        /* representor port net_bdf_port */
        snprintf(name, sizeof(name), "net_%s_representor_%d", pci_dev->device.name, eth_da.representor_ports[i]);

        retval = rte_eth_dev_create(&pci_dev->device, name, sizeof(struct i40e_vf_representor), NULL, NULL,
...
static struct rte_pci_driver rte_i40e_pmd = {
    ...
    .drv_flags = RTE_PCI_DRV_NEED_MAPPING | RTE_PCI_DRV_INTR_LSC | RTE_PCI_DRV_IOVA_AS_VA,
    .probe = eth_i40e_pci_probe,
    .remove = eth_i40e_pci_remove,
};

static inline void i40e_write_global_rx_ctl(struct i40e_hw *hw, uint32_t reg_addr, uint32_t reg_val) {
...
static inline void i40e_config_automask(struct i40e_pf *pf) {
    ...
    val |= I40E_GLINT_CTL_DIS_AUTOMASK_PF0_MASK | I40E_GLINT_CTL_DIS_AUTOMASK_VF0_MASK;
...
static void i40e_add_tx_flow_control_drop_filter(struct i40e_pf *pf) {
    struct i40e_hw *hw = I40E_PF_TO_HW(pf);
    uint16_t flags = I40E_AQC_ADD_CONTROL_PACKET_FLAGS_IGNORE_MAC | I40E_AQC_ADD_CONTROL_PACKET_FLAGS_DROP |
                     I40E_AQC_ADD_CONTROL_PACKET_FLAGS_TX;
    int ret;

    ret = i40e_aq_add_rem_control_packet_filter(hw, NULL, I40E_FLOW_CONTROL_ETHERTYPE, flags, pf->main_vsi_seid, 0,
                                                TRUE, NULL, NULL);
    if (ret)
        PMD_INIT_LOG(ERR, "Failed to add filter to drop flow control frames from VSIs.");
}

static int floating_veb_list_handler(__rte_unused const char *key, const char *floating_veb_value, void *opaque) {
    int idx = 0;
...
static void config_vf_floating_veb(struct rte_devargs *devargs, uint16_t floating_veb, bool *vf_floating_veb) {
     ...
     */
    if (rte_kvargs_process(kvlist, floating_veb_list, floating_veb_list_handler, vf_floating_veb) < 0) {
        rte_kvargs_free(kvlist);
        return;
    }
    rte_kvargs_free(kvlist);
...
static int i40e_check_floating_handler(__rte_unused const char *key, const char *value, __rte_unused void *opaque) {
    if (strcmp(value, "1"))
        return -1;
...
static int is_floating_veb_supported(struct rte_devargs *devargs) {
     ...
     */
    if (rte_kvargs_process(kvlist, floating_veb_key, i40e_check_floating_handler, NULL) < 0) {
        rte_kvargs_free(kvlist);
        return 0;
    }
    rte_kvargs_free(kvlist);
...
static void config_floating_veb(struct rte_eth_dev *dev) {
    ...
    if (hw->aq.fw_maj_ver >= FLOATING_VEB_SUPPORTED_FW_MAJ) {
        ...
        config_vf_floating_veb(pci_dev->device.devargs, pf->floating_veb, pf->floating_veb_list);
    } else {
...
static int i40e_init_ethtype_filter_list(struct rte_eth_dev *dev) {
    ...
    snprintf(ethertype_hash_name, RTE_HASH_NAMESIZE, "ethertype_%s", dev->device->name);
    ...
    ethertype_rule->hash_map = rte_zmalloc("i40e_ethertype_hash_map",
                                           sizeof(struct i40e_ethertype_filter *) * I40E_MAX_ETHERTYPE_FILTER_NUM, 0);
...
static int i40e_init_tunnel_filter_list(struct rte_eth_dev *dev) {
    ...
    tunnel_rule->hash_map =
        rte_zmalloc("i40e_tunnel_hash_map", sizeof(struct i40e_tunnel_filter *) * I40E_MAX_TUNNEL_FILTER_NUM, 0);
...
static int i40e_init_fdir_filter_list(struct rte_eth_dev *dev) {
    ...
    fdir_info->hash_map =
        rte_zmalloc("i40e_fdir_hash_map", sizeof(struct i40e_fdir_filter *) * I40E_MAX_FDIR_FILTER_NUM, 0);
...
static int i40e_parse_multi_drv_handler(__rte_unused const char *key, const char *value, void *opaque) {
    struct i40e_pf *pf;
    ...
    if (support_multi_driver == 1 || support_multi_driver == 0)
        ...
        PMD_DRV_LOG(WARNING, "%s must be 1 or 0,",
                    "enable global configuration by default." ETH_I40E_SUPPORT_MULTI_DRIVER);
    return 0;
}
...
static int i40e_support_multi_driver(struct rte_eth_dev *dev) {
    ...
    if (rte_kvargs_process(kvlist, ETH_I40E_SUPPORT_MULTI_DRIVER, i40e_parse_multi_drv_handler, pf) < 0) {
        rte_kvargs_free(kvlist);
        return -EINVAL;
    }
...
static int i40e_aq_debug_write_global_register(struct i40e_hw *hw, uint32_t reg_addr, uint64_t reg_val,
...
static int i40e_parse_latest_vec_handler(__rte_unused const char *key, const char *value, void *opaque) {
...
static int i40e_use_latest_vec(struct rte_eth_dev *dev) {
    ...
    if (rte_kvargs_process(kvlist, ETH_I40E_USE_LATEST_VEC, i40e_parse_latest_vec_handler, ad) < 0) {
        rte_kvargs_free(kvlist);
        return -EINVAL;
    }
...
static int eth_i40e_dev_init(struct rte_eth_dev *dev, void *init_params __rte_unused) {
    ...
    PMD_INIT_LOG(INFO, "FW %d.%d API %d.%d NVM %02d.%02d.%02d eetrack %04x", hw->aq.fw_maj_ver, hw->aq.fw_min_ver,
                 hw->aq.api_maj_ver, hw->aq.api_min_ver, ((hw->nvm.version >> 12) & 0xf),
                 ((hw->nvm.version >> 4) & 0xff), (hw->nvm.version & 0xf), hw->nvm.eetrack);
    ...
    if (!pf->support_multi_driver) {
        ret = i40e_aq_debug_write_global_register(hw, I40E_GLQF_L3_MAP(40), 0x00000028, NULL);
        if (ret)
            PMD_INIT_LOG(ERR, "Failed to write L3 MAP register %d", ret);
        PMD_INIT_LOG(DEBUG, "Global register 0x%08x is changed with 0x28", I40E_GLQF_L3_MAP(40));
    }
    ...
    ret = i40e_res_pool_init(&pf->msix_pool, 1, hw->func_caps.num_msix_vectors - 1);
    if (ret < 0) {
    ...
    /* Initialize lan hmc */
    ret = i40e_init_lan_hmc(hw, hw->func_caps.num_tx_qp, hw->func_caps.num_rx_qp, 0, 0);
    if (ret != I40E_SUCCESS) {
    ...
    ether_addr_copy((struct ether_addr *)hw->mac.addr, (struct ether_addr *)hw->mac.perm_addr);
    ...
    ether_addr_copy((struct ether_addr *)hw->mac.perm_addr, &dev->data->mac_addrs[0]);
    ...
    memset(&pf->rss_info, 0, sizeof(struct i40e_rte_flow_rss_conf));
...
static int eth_i40e_dev_uninit(struct rte_eth_dev *dev) {
    ...
    do {
        ret = rte_intr_callback_unregister(intr_handle, i40e_dev_interrupt_handler, dev);
        if (ret >= 0) {
            break;
        } else if (ret != -EAGAIN) {
...
void i40e_vsi_queues_unbind_intr(struct i40e_vsi *vsi) {
    ...
    if (vsi->type != I40E_VSI_SRIOV) {
        if (!rte_intr_allow_others(intr_handle)) {
            ...
            I40E_WRITE_REG(hw, I40E_PFINT_LNKLSTN(msix_vect - 1), I40E_PFINT_LNKLSTN_FIRSTQ_INDX_MASK);
            I40E_WRITE_REG(hw, I40E_PFINT_ITRN(I40E_ITR_INDEX_DEFAULT, msix_vect - 1), 0);
        }
    } else {
        uint32_t reg;
        reg = (hw->func_caps.num_msix_vectors_vf - 1) * vsi->user_param + (msix_vect - 1);

        I40E_WRITE_REG(hw, I40E_VPINT_LNKLSTN(reg), I40E_VPINT_LNKLSTN_FIRSTQ_INDX_MASK);
    }
    I40E_WRITE_FLUSH(hw);
}

static void __vsi_queues_bind_intr(struct i40e_vsi *vsi, uint16_t msix_vect, int base_queue, int nb_queue,
                                   uint16_t itr_idx) {
    int i;
    uint32_t val;
    struct i40e_hw *hw = I40E_VSI_TO_HW(vsi);
...
#endif
              ((base_queue + i + 1) << I40E_QINT_RQCTL_NEXTQ_INDX_SHIFT) | (0 << I40E_QINT_RQCTL_NEXTQ_TYPE_SHIFT) |
    ...
    if (vsi->type != I40E_VSI_SRIOV) {
        ...
        if (msix_vect == I40E_MISC_VEC_ID) {
            ...
            I40E_WRITE_REG(hw, I40E_PFINT_ITRN(I40E_ITR_INDEX_DEFAULT, msix_vect - 1), interval);
        }
    } else {
        uint32_t reg;
...
void i40e_vsi_queues_bind_intr(struct i40e_vsi *vsi, uint16_t itr_idx) {
    ...
    /* VF bind interrupt */
    if (vsi->type == I40E_VSI_SRIOV) {
        __vsi_queues_bind_intr(vsi, msix_vect, vsi->base_queue, vsi->nb_qps, itr_idx);
        return;
    }
    ...
    for (i = 0; i < vsi->nb_used_qps; i++) {
        if (nb_msix <= 1) {
            ...
            __vsi_queues_bind_intr(vsi, msix_vect, vsi->base_queue + i, vsi->nb_used_qps - i, itr_idx);
...
static void i40e_vsi_enable_queues_intr(struct i40e_vsi *vsi) {
    struct rte_eth_dev *dev = vsi->adapter->eth_dev;
    struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
    struct rte_intr_handle *intr_handle = &pci_dev->intr_handle;
    struct i40e_hw *hw = I40E_VSI_TO_HW(vsi);
    ...
    if (rte_intr_allow_others(intr_handle) && !pf->support_multi_driver)
        for (i = 0; i < vsi->nb_msix; i++) {
            ...
            I40E_WRITE_REG(hw, I40E_PFINT_DYN_CTLN(msix_intr - 1),
                           I40E_PFINT_DYN_CTLN_INTENA_MASK | I40E_PFINT_DYN_CTLN_CLEARPBA_MASK |
                               I40E_PFINT_DYN_CTLN_ITR_INDX_MASK);
        }
    else
        I40E_WRITE_REG(hw, I40E_PFINT_DYN_CTL0,
                       I40E_PFINT_DYN_CTL0_INTENA_MASK | I40E_PFINT_DYN_CTL0_CLEARPBA_MASK |
                           I40E_PFINT_DYN_CTL0_ITR_INDX_MASK);

    I40E_WRITE_FLUSH(hw);
}

static void i40e_vsi_disable_queues_intr(struct i40e_vsi *vsi) {
    struct rte_eth_dev *dev = vsi->adapter->eth_dev;
    struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
    struct rte_intr_handle *intr_handle = &pci_dev->intr_handle;
    struct i40e_hw *hw = I40E_VSI_TO_HW(vsi);
    ...
    if (rte_intr_allow_others(intr_handle) && !pf->support_multi_driver)
        for (i = 0; i < vsi->nb_msix; i++) {
            msix_intr = vsi->msix_intr + i;
            I40E_WRITE_REG(hw, I40E_PFINT_DYN_CTLN(msix_intr - 1), I40E_PFINT_DYN_CTLN_ITR_INDX_MASK);
        }
    else
...
static int i40e_phy_conf_link(struct i40e_hw *hw, uint8_t abilities, uint8_t force_speed, bool is_up) {
    ...
    const uint8_t mask =
        I40E_AQ_PHY_FLAG_PAUSE_TX | I40E_AQ_PHY_FLAG_PAUSE_RX | I40E_AQ_PHY_FLAG_PAUSE_RX | I40E_AQ_PHY_FLAG_LOW_POWER;
    int ret = -ENOTSUP;
     ...
     */
    if (is_up && phy_ab.phy_type != 0 && abilities & I40E_AQ_PHY_AN_ENABLED && phy_ab.link_speed != 0)
        return I40E_SUCCESS;
    ...
    phy_conf.phy_type_ext = is_up ? (I40E_AQ_PHY_TYPE_EXT_25G_KR | I40E_AQ_PHY_TYPE_EXT_25G_CR |
    ...
    PMD_DRV_LOG(DEBUG, "\tCurrent: abilities %x, link_speed %x", phy_ab.abilities, phy_ab.link_speed);
    PMD_DRV_LOG(DEBUG, "\tConfig:  abilities %x, link_speed %x", phy_conf.abilities, phy_conf.link_speed);
...
static int i40e_apply_link_speed(struct rte_eth_dev *dev) {
    ...
    if (conf->link_speeds == ETH_LINK_SPEED_AUTONEG) {
        conf->link_speeds = ETH_LINK_SPEED_40G | ETH_LINK_SPEED_25G | ETH_LINK_SPEED_20G | ETH_LINK_SPEED_10G |
                            ETH_LINK_SPEED_1G | ETH_LINK_SPEED_100M;
    }
    speed = i40e_parse_link_speeds(conf->link_speeds);
    abilities |= I40E_AQ_PHY_ENABLE_ATOMIC_LINK | I40E_AQ_PHY_AN_ENABLED | I40E_AQ_PHY_LINK_ENABLED;
...
static int i40e_dev_start(struct rte_eth_dev *dev) {
    ...
    if (dev->data->dev_conf.link_speeds & ETH_LINK_SPEED_FIXED) {
        PMD_INIT_LOG(ERR, "Invalid link_speeds for port %u, autonegotiation disabled", dev->data->port_id);
        return -EINVAL;
    }
    ...
    if (rte_intr_dp_is_en(intr_handle) && !intr_handle->intr_vec) {
        ...
        if (!intr_handle->intr_vec) {
            PMD_INIT_LOG(ERR, "Failed to allocate %d rx_queues intr_vec", dev->data->nb_rx_queues);
            return -ENOMEM;
        }
    }
    ...
    main_vsi->nb_used_qps = dev->data->nb_rx_queues - pf->nb_cfg_vmdq_vsi * RTE_LIBRTE_I40E_QUEUE_NUM_PER_VM;
    ...
    for (i = 0; i < pf->nb_cfg_vmdq_vsi; i++) {
        ...
        i40e_vsi_queues_bind_intr(pf->vmdq[i].vsi, I40E_ITR_INDEX_DEFAULT);
        i40e_vsi_enable_queues_intr(pf->vmdq[i].vsi);
    }

    /* enable FDIR MSIX interrupt */
    if (pf->fdir.fdir_vsi) {
        i40e_vsi_queues_bind_intr(pf->fdir.fdir_vsi, I40E_ITR_INDEX_NONE);
    ...
    if (!rte_intr_allow_others(intr_handle)) {
        rte_intr_callback_unregister(intr_handle, i40e_dev_interrupt_handler, (void *)dev);
        /* configure and enable device interrupt */
        ...
        ret = i40e_aq_set_phy_int_mask(
            hw, ~(I40E_AQ_EVENT_LINK_UPDOWN | I40E_AQ_EVENT_MODULE_QUAL_FAIL | I40E_AQ_EVENT_MEDIA_NA), NULL);
        if (ret != I40E_SUCCESS)
...
static void i40e_dev_stop(struct rte_eth_dev *dev) {
    ...
    if (!rte_intr_allow_others(intr_handle))
        ...
        rte_intr_callback_register(intr_handle, i40e_dev_interrupt_handler, (void *)dev);
...
static void i40e_dev_close(struct rte_eth_dev *dev) {
    ...
    while ((p_mirror = TAILQ_FIRST(&pf->mirror_list))) {
        ret = i40e_aq_del_mirror_rule(hw, pf->main_vsi->veb->seid, p_mirror->rule_type, p_mirror->entries,
...
static void i40e_dev_promiscuous_disable(struct rte_eth_dev *dev) {
    ...
    status = i40e_aq_set_vsi_unicast_promiscuous(hw, vsi->seid, false, NULL, true);
    if (status != I40E_SUCCESS)
...
static __rte_always_inline void update_link_reg(struct i40e_hw *hw, struct rte_eth_link *link) {
...
static __rte_always_inline void update_link_aq(struct i40e_hw *hw, struct rte_eth_link *link, bool enable_lse,
...
int i40e_dev_link_update(struct rte_eth_dev *dev, int wait_to_complete) {
    ...
    link.link_duplex = ETH_LINK_FULL_DUPLEX;
    link.link_autoneg = !(dev->data->dev_conf.link_speeds & ETH_LINK_SPEED_FIXED);
...
void i40e_update_vsi_stats(struct i40e_vsi *vsi) {
    ...
    i40e_stat_update_48(hw, I40E_GLV_GORCH(idx), I40E_GLV_GORCL(idx), vsi->offset_loaded, &oes->rx_bytes,
                        &nes->rx_bytes);
    i40e_stat_update_48(hw, I40E_GLV_UPRCH(idx), I40E_GLV_UPRCL(idx), vsi->offset_loaded, &oes->rx_unicast,
                        &nes->rx_unicast);
    i40e_stat_update_48(hw, I40E_GLV_MPRCH(idx), I40E_GLV_MPRCL(idx), vsi->offset_loaded, &oes->rx_multicast,
                        &nes->rx_multicast);
    i40e_stat_update_48(hw, I40E_GLV_BPRCH(idx), I40E_GLV_BPRCL(idx), vsi->offset_loaded, &oes->rx_broadcast,
                        &nes->rx_broadcast);
    /* exclude CRC bytes */
    nes->rx_bytes -= (nes->rx_unicast + nes->rx_multicast + nes->rx_broadcast) * ETHER_CRC_LEN;

    i40e_stat_update_32(hw, I40E_GLV_RDPC(idx), vsi->offset_loaded, &oes->rx_discards, &nes->rx_discards);
    /* GLV_REPC not supported */
    /* GLV_RMPC not supported */
    i40e_stat_update_32(hw, I40E_GLV_RUPP(idx), vsi->offset_loaded, &oes->rx_unknown_protocol,
                        &nes->rx_unknown_protocol);
    i40e_stat_update_48(hw, I40E_GLV_GOTCH(idx), I40E_GLV_GOTCL(idx), vsi->offset_loaded, &oes->tx_bytes,
                        &nes->tx_bytes);
    i40e_stat_update_48(hw, I40E_GLV_UPTCH(idx), I40E_GLV_UPTCL(idx), vsi->offset_loaded, &oes->tx_unicast,
                        &nes->tx_unicast);
    i40e_stat_update_48(hw, I40E_GLV_MPTCH(idx), I40E_GLV_MPTCL(idx), vsi->offset_loaded, &oes->tx_multicast,
                        &nes->tx_multicast);
    i40e_stat_update_48(hw, I40E_GLV_BPTCH(idx), I40E_GLV_BPTCL(idx), vsi->offset_loaded, &oes->tx_broadcast,
                        &nes->tx_broadcast);
    /* GLV_TDPC not supported */
    i40e_stat_update_32(hw, I40E_GLV_TEPC(idx), vsi->offset_loaded, &oes->tx_errors, &nes->tx_errors);
    vsi->offset_loaded = true;

    PMD_DRV_LOG(DEBUG, "***************** VSI[%u] stats start *******************", vsi->vsi_id);
    ...
    PMD_DRV_LOG(DEBUG, "rx_unknown_protocol: %" PRIu64 "", nes->rx_unknown_protocol);
    ...
    PMD_DRV_LOG(DEBUG, "***************** VSI[%u] stats end *******************", vsi->vsi_id);
}
...
static void i40e_read_stats_registers(struct i40e_pf *pf, struct i40e_hw *hw) {
    ...
    i40e_stat_update_48(hw, I40E_GLV_GORCH(hw->port), I40E_GLV_GORCL(hw->port), pf->offset_loaded,
                        &pf->internal_stats_offset.rx_bytes, &pf->internal_stats.rx_bytes);

    i40e_stat_update_48(hw, I40E_GLV_GOTCH(hw->port), I40E_GLV_GOTCL(hw->port), pf->offset_loaded,
                        &pf->internal_stats_offset.tx_bytes, &pf->internal_stats.tx_bytes);
    /* Get total internal rx packet count */
    i40e_stat_update_48(hw, I40E_GLV_UPRCH(hw->port), I40E_GLV_UPRCL(hw->port), pf->offset_loaded,
                        &pf->internal_stats_offset.rx_unicast, &pf->internal_stats.rx_unicast);
    i40e_stat_update_48(hw, I40E_GLV_MPRCH(hw->port), I40E_GLV_MPRCL(hw->port), pf->offset_loaded,
                        &pf->internal_stats_offset.rx_multicast, &pf->internal_stats.rx_multicast);
    i40e_stat_update_48(hw, I40E_GLV_BPRCH(hw->port), I40E_GLV_BPRCL(hw->port), pf->offset_loaded,
                        &pf->internal_stats_offset.rx_broadcast, &pf->internal_stats.rx_broadcast);
    /* Get total internal tx packet count */
    i40e_stat_update_48(hw, I40E_GLV_UPTCH(hw->port), I40E_GLV_UPTCL(hw->port), pf->offset_loaded,
                        &pf->internal_stats_offset.tx_unicast, &pf->internal_stats.tx_unicast);
    i40e_stat_update_48(hw, I40E_GLV_MPTCH(hw->port), I40E_GLV_MPTCL(hw->port), pf->offset_loaded,
                        &pf->internal_stats_offset.tx_multicast, &pf->internal_stats.tx_multicast);
    i40e_stat_update_48(hw, I40E_GLV_BPTCH(hw->port), I40E_GLV_BPTCL(hw->port), pf->offset_loaded,
                        &pf->internal_stats_offset.tx_broadcast, &pf->internal_stats.tx_broadcast);

    /* exclude CRC size */
    pf->internal_stats.rx_bytes -=
        (pf->internal_stats.rx_unicast + pf->internal_stats.rx_multicast + pf->internal_stats.rx_broadcast) *
        ETHER_CRC_LEN;

    /* Get statistics of struct i40e_eth_stats */
    i40e_stat_update_48(hw, I40E_GLPRT_GORCH(hw->port), I40E_GLPRT_GORCL(hw->port), pf->offset_loaded,
                        &os->eth.rx_bytes, &ns->eth.rx_bytes);
    i40e_stat_update_48(hw, I40E_GLPRT_UPRCH(hw->port), I40E_GLPRT_UPRCL(hw->port), pf->offset_loaded,
                        &os->eth.rx_unicast, &ns->eth.rx_unicast);
    i40e_stat_update_48(hw, I40E_GLPRT_MPRCH(hw->port), I40E_GLPRT_MPRCL(hw->port), pf->offset_loaded,
                        &os->eth.rx_multicast, &ns->eth.rx_multicast);
    i40e_stat_update_48(hw, I40E_GLPRT_BPRCH(hw->port), I40E_GLPRT_BPRCL(hw->port), pf->offset_loaded,
                        &os->eth.rx_broadcast, &ns->eth.rx_broadcast);
    /* Workaround: CRC size should not be included in byte statistics,
     * so subtract ETHER_CRC_LEN from the byte counter for each rx packet.
     */
    ns->eth.rx_bytes -= (ns->eth.rx_unicast + ns->eth.rx_multicast + ns->eth.rx_broadcast) * ETHER_CRC_LEN;
    ...
    i40e_stat_update_32(hw, I40E_GLPRT_RDPC(hw->port), pf->offset_loaded, &os->eth.rx_discards, &ns->eth.rx_discards);
    /* GLPRT_REPC not supported */
    /* GLPRT_RMPC not supported */
    i40e_stat_update_32(hw, I40E_GLPRT_RUPP(hw->port), pf->offset_loaded, &os->eth.rx_unknown_protocol,
                        &ns->eth.rx_unknown_protocol);
    i40e_stat_update_48(hw, I40E_GLPRT_GOTCH(hw->port), I40E_GLPRT_GOTCL(hw->port), pf->offset_loaded,
                        &os->eth.tx_bytes, &ns->eth.tx_bytes);
    i40e_stat_update_48(hw, I40E_GLPRT_UPTCH(hw->port), I40E_GLPRT_UPTCL(hw->port), pf->offset_loaded,
                        &os->eth.tx_unicast, &ns->eth.tx_unicast);
    i40e_stat_update_48(hw, I40E_GLPRT_MPTCH(hw->port), I40E_GLPRT_MPTCL(hw->port), pf->offset_loaded,
                        &os->eth.tx_multicast, &ns->eth.tx_multicast);
    i40e_stat_update_48(hw, I40E_GLPRT_BPTCH(hw->port), I40E_GLPRT_BPTCL(hw->port), pf->offset_loaded,
                        &os->eth.tx_broadcast, &ns->eth.tx_broadcast);
    ns->eth.tx_bytes -= (ns->eth.tx_unicast + ns->eth.tx_multicast + ns->eth.tx_broadcast) * ETHER_CRC_LEN;
    ...
    i40e_stat_update_32(hw, I40E_GLPRT_TDOLD(hw->port), pf->offset_loaded, &os->tx_dropped_link_down,
                        &ns->tx_dropped_link_down);
    i40e_stat_update_32(hw, I40E_GLPRT_CRCERRS(hw->port), pf->offset_loaded, &os->crc_errors, &ns->crc_errors);
    i40e_stat_update_32(hw, I40E_GLPRT_ILLERRC(hw->port), pf->offset_loaded, &os->illegal_bytes, &ns->illegal_bytes);
    /* GLPRT_ERRBC not supported */
    i40e_stat_update_32(hw, I40E_GLPRT_MLFC(hw->port), pf->offset_loaded, &os->mac_local_faults, &ns->mac_local_faults);
    i40e_stat_update_32(hw, I40E_GLPRT_MRFC(hw->port), pf->offset_loaded, &os->mac_remote_faults,
                        &ns->mac_remote_faults);
    i40e_stat_update_32(hw, I40E_GLPRT_RLEC(hw->port), pf->offset_loaded, &os->rx_length_errors, &ns->rx_length_errors);
    i40e_stat_update_32(hw, I40E_GLPRT_LXONRXC(hw->port), pf->offset_loaded, &os->link_xon_rx, &ns->link_xon_rx);
    i40e_stat_update_32(hw, I40E_GLPRT_LXOFFRXC(hw->port), pf->offset_loaded, &os->link_xoff_rx, &ns->link_xoff_rx);
    for (i = 0; i < 8; i++) {
        i40e_stat_update_32(hw, I40E_GLPRT_PXONRXC(hw->port, i), pf->offset_loaded, &os->priority_xon_rx[i],
                            &ns->priority_xon_rx[i]);
        i40e_stat_update_32(hw, I40E_GLPRT_PXOFFRXC(hw->port, i), pf->offset_loaded, &os->priority_xoff_rx[i],
                            &ns->priority_xoff_rx[i]);
    }
    i40e_stat_update_32(hw, I40E_GLPRT_LXONTXC(hw->port), pf->offset_loaded, &os->link_xon_tx, &ns->link_xon_tx);
    i40e_stat_update_32(hw, I40E_GLPRT_LXOFFTXC(hw->port), pf->offset_loaded, &os->link_xoff_tx, &ns->link_xoff_tx);
    for (i = 0; i < 8; i++) {
        i40e_stat_update_32(hw, I40E_GLPRT_PXONTXC(hw->port, i), pf->offset_loaded, &os->priority_xon_tx[i],
                            &ns->priority_xon_tx[i]);
        i40e_stat_update_32(hw, I40E_GLPRT_PXOFFTXC(hw->port, i), pf->offset_loaded, &os->priority_xoff_tx[i],
                            &ns->priority_xoff_tx[i]);
        i40e_stat_update_32(hw, I40E_GLPRT_RXON2OFFCNT(hw->port, i), pf->offset_loaded, &os->priority_xon_2_xoff[i],
                            &ns->priority_xon_2_xoff[i]);
    }
    i40e_stat_update_48(hw, I40E_GLPRT_PRC64H(hw->port), I40E_GLPRT_PRC64L(hw->port), pf->offset_loaded,
                        &os->rx_size_64, &ns->rx_size_64);
    i40e_stat_update_48(hw, I40E_GLPRT_PRC127H(hw->port), I40E_GLPRT_PRC127L(hw->port), pf->offset_loaded,
                        &os->rx_size_127, &ns->rx_size_127);
    i40e_stat_update_48(hw, I40E_GLPRT_PRC255H(hw->port), I40E_GLPRT_PRC255L(hw->port), pf->offset_loaded,
                        &os->rx_size_255, &ns->rx_size_255);
    i40e_stat_update_48(hw, I40E_GLPRT_PRC511H(hw->port), I40E_GLPRT_PRC511L(hw->port), pf->offset_loaded,
                        &os->rx_size_511, &ns->rx_size_511);
    i40e_stat_update_48(hw, I40E_GLPRT_PRC1023H(hw->port), I40E_GLPRT_PRC1023L(hw->port), pf->offset_loaded,
                        &os->rx_size_1023, &ns->rx_size_1023);
    i40e_stat_update_48(hw, I40E_GLPRT_PRC1522H(hw->port), I40E_GLPRT_PRC1522L(hw->port), pf->offset_loaded,
                        &os->rx_size_1522, &ns->rx_size_1522);
    i40e_stat_update_48(hw, I40E_GLPRT_PRC9522H(hw->port), I40E_GLPRT_PRC9522L(hw->port), pf->offset_loaded,
                        &os->rx_size_big, &ns->rx_size_big);
    i40e_stat_update_32(hw, I40E_GLPRT_RUC(hw->port), pf->offset_loaded, &os->rx_undersize, &ns->rx_undersize);
    i40e_stat_update_32(hw, I40E_GLPRT_RFC(hw->port), pf->offset_loaded, &os->rx_fragments, &ns->rx_fragments);
    i40e_stat_update_32(hw, I40E_GLPRT_ROC(hw->port), pf->offset_loaded, &os->rx_oversize, &ns->rx_oversize);
    i40e_stat_update_32(hw, I40E_GLPRT_RJC(hw->port), pf->offset_loaded, &os->rx_jabber, &ns->rx_jabber);
    i40e_stat_update_48(hw, I40E_GLPRT_PTC64H(hw->port), I40E_GLPRT_PTC64L(hw->port), pf->offset_loaded,
                        &os->tx_size_64, &ns->tx_size_64);
    i40e_stat_update_48(hw, I40E_GLPRT_PTC127H(hw->port), I40E_GLPRT_PTC127L(hw->port), pf->offset_loaded,
                        &os->tx_size_127, &ns->tx_size_127);
    i40e_stat_update_48(hw, I40E_GLPRT_PTC255H(hw->port), I40E_GLPRT_PTC255L(hw->port), pf->offset_loaded,
                        &os->tx_size_255, &ns->tx_size_255);
    i40e_stat_update_48(hw, I40E_GLPRT_PTC511H(hw->port), I40E_GLPRT_PTC511L(hw->port), pf->offset_loaded,
                        &os->tx_size_511, &ns->tx_size_511);
    i40e_stat_update_48(hw, I40E_GLPRT_PTC1023H(hw->port), I40E_GLPRT_PTC1023L(hw->port), pf->offset_loaded,
                        &os->tx_size_1023, &ns->tx_size_1023);
    i40e_stat_update_48(hw, I40E_GLPRT_PTC1522H(hw->port), I40E_GLPRT_PTC1522L(hw->port), pf->offset_loaded,
                        &os->tx_size_1522, &ns->tx_size_1522);
    i40e_stat_update_48(hw, I40E_GLPRT_PTC9522H(hw->port), I40E_GLPRT_PTC9522L(hw->port), pf->offset_loaded,
                        &os->tx_size_big, &ns->tx_size_big);
#ifndef TREX_PATCH
    i40e_stat_update_32(hw, I40E_GLQF_PCNT(pf->fdir.match_counter_index), pf->offset_loaded, &os->fd_sb_match,
                        &ns->fd_sb_match);
#endif
...
static int i40e_dev_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
    ...
    stats->ipackets = pf->main_vsi->eth_stats.rx_unicast + pf->main_vsi->eth_stats.rx_multicast +
                      pf->main_vsi->eth_stats.rx_broadcast - pf->main_vsi->eth_stats.rx_discards;
    ...
    stats->ierrors =
        ns->crc_errors + ns->rx_length_errors + ns->rx_undersize + ns->rx_oversize + ns->rx_fragments + ns->rx_jabber;
    ...
    if (pf->vfs) {
        for (i = 0; i < pf->vf_num; i++) {
            ...
            stats->ipackets += (vsi->eth_stats.rx_unicast + vsi->eth_stats.rx_multicast + vsi->eth_stats.rx_broadcast -
    ...
    PMD_DRV_LOG(DEBUG, "rx_unknown_protocol: %" PRIu64 "", ns->eth.rx_unknown_protocol);
    ...
    PMD_DRV_LOG(DEBUG, "tx_dropped_link_down:     %" PRIu64 "", ns->tx_dropped_link_down);
    ...
    PMD_DRV_LOG(DEBUG, "mac_remote_faults:        %" PRIu64 "", ns->mac_remote_faults);
    PMD_DRV_LOG(DEBUG, "rx_length_errors:         %" PRIu64 "", ns->rx_length_errors);
    PMD_DRV_LOG(DEBUG, "link_xon_rx:              %" PRIu64 "", ns->link_xon_rx);
    PMD_DRV_LOG(DEBUG, "link_xoff_rx:             %" PRIu64 "", ns->link_xoff_rx);
    for (i = 0; i < 8; i++) {
        PMD_DRV_LOG(DEBUG, "priority_xon_rx[%d]:      %" PRIu64 "", i, ns->priority_xon_rx[i]);
        PMD_DRV_LOG(DEBUG, "priority_xoff_rx[%d]:     %" PRIu64 "", i, ns->priority_xoff_rx[i]);
    }
    PMD_DRV_LOG(DEBUG, "link_xon_tx:              %" PRIu64 "", ns->link_xon_tx);
    PMD_DRV_LOG(DEBUG, "link_xoff_tx:             %" PRIu64 "", ns->link_xoff_tx);
    for (i = 0; i < 8; i++) {
        PMD_DRV_LOG(DEBUG, "priority_xon_tx[%d]:      %" PRIu64 "", i, ns->priority_xon_tx[i]);
        PMD_DRV_LOG(DEBUG, "priority_xoff_tx[%d]:     %" PRIu64 "", i, ns->priority_xoff_tx[i]);
        PMD_DRV_LOG(DEBUG, "priority_xon_2_xoff[%d]:  %" PRIu64 "", i, ns->priority_xon_2_xoff[i]);
    }
    ...
    PMD_DRV_LOG(DEBUG, "mac_short_packet_dropped: %" PRIu64 "", ns->mac_short_packet_dropped);
    PMD_DRV_LOG(DEBUG, "checksum_error:           %" PRIu64 "", ns->checksum_error);
...
static uint32_t i40e_xstats_calc_num(void) {
    return I40E_NB_ETH_XSTATS + I40E_NB_HW_PORT_XSTATS + (I40E_NB_RXQ_PRIO_XSTATS * 8) + (I40E_NB_TXQ_PRIO_XSTATS * 8);
}

static int i40e_dev_xstats_get_names(__rte_unused struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats_names,
    ...
    for (i = 0; i < I40E_NB_ETH_XSTATS; i++) {
        strlcpy(xstats_names[count].name, rte_i40e_stats_strings[i].name, sizeof(xstats_names[count].name));
        count++;
    }

    /* Get individiual stats from i40e_hw_port struct */
    for (i = 0; i < I40E_NB_HW_PORT_XSTATS; i++) {
        strlcpy(xstats_names[count].name, rte_i40e_hw_port_strings[i].name, sizeof(xstats_names[count].name));
        count++;
    }

    for (i = 0; i < I40E_NB_RXQ_PRIO_XSTATS; i++) {
        for (prio = 0; prio < 8; prio++) {
            snprintf(xstats_names[count].name, sizeof(xstats_names[count].name), "rx_priority%u_%s", prio,
    ...
    for (i = 0; i < I40E_NB_TXQ_PRIO_XSTATS; i++) {
        for (prio = 0; prio < 8; prio++) {
            snprintf(xstats_names[count].name, sizeof(xstats_names[count].name), "tx_priority%u_%s", prio,
...
static int i40e_dev_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats, unsigned n) {
    ...
    /* Get stats from i40e_eth_stats struct */
    for (i = 0; i < I40E_NB_ETH_XSTATS; i++) {
        xstats[count].value = *(uint64_t *)(((char *)&hw_stats->eth) + rte_i40e_stats_strings[i].offset);
    ...
    /* Get individiual stats from i40e_hw_port struct */
    for (i = 0; i < I40E_NB_HW_PORT_XSTATS; i++) {
        xstats[count].value = *(uint64_t *)(((char *)hw_stats) + rte_i40e_hw_port_strings[i].offset);
        xstats[count].id = count;
        count++;
    }
    ...
    for (i = 0; i < I40E_NB_RXQ_PRIO_XSTATS; i++) {
        for (prio = 0; prio < 8; prio++) {
            xstats[count].value =
                *(uint64_t *)(((char *)hw_stats) + rte_i40e_rxq_prio_strings[i].offset + (sizeof(uint64_t) * prio));
            xstats[count].id = count;
            count++;
        }
    }
    ...
    for (i = 0; i < I40E_NB_TXQ_PRIO_XSTATS; i++) {
        for (prio = 0; prio < 8; prio++) {
            xstats[count].value =
                *(uint64_t *)(((char *)hw_stats) + rte_i40e_txq_prio_strings[i].offset + (sizeof(uint64_t) * prio));
            xstats[count].id = count;
            count++;
        }
    }
...
static int i40e_dev_queue_stats_mapping_set(__rte_unused struct rte_eth_dev *dev, __rte_unused uint16_t queue_id,
...
static int i40e_fw_version_get(struct rte_eth_dev *dev, char *fw_version, size_t fw_size) {
    ...
    ret = snprintf(fw_version, fw_size, "%d.%d%d 0x%08x %d.%d.%d", ((hw->nvm.version >> 12) & 0xf),
                   ((hw->nvm.version >> 4) & 0xff), (hw->nvm.version & 0xf), hw->nvm.eetrack, ver, build, patch);
...
static void i40e_dev_info_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info) {
    ...
    dev_info->rx_offload_capa = DEV_RX_OFFLOAD_VLAN_STRIP | DEV_RX_OFFLOAD_QINQ_STRIP | DEV_RX_OFFLOAD_IPV4_CKSUM |
                                DEV_RX_OFFLOAD_UDP_CKSUM | DEV_RX_OFFLOAD_TCP_CKSUM | DEV_RX_OFFLOAD_OUTER_IPV4_CKSUM |
                                DEV_RX_OFFLOAD_KEEP_CRC | DEV_RX_OFFLOAD_SCATTER | DEV_RX_OFFLOAD_VLAN_EXTEND |
    ...
    dev_info->tx_offload_capa = DEV_TX_OFFLOAD_VLAN_INSERT | DEV_TX_OFFLOAD_QINQ_INSERT | DEV_TX_OFFLOAD_IPV4_CKSUM |
                                DEV_TX_OFFLOAD_UDP_CKSUM | DEV_TX_OFFLOAD_TCP_CKSUM | DEV_TX_OFFLOAD_SCTP_CKSUM |
                                DEV_TX_OFFLOAD_OUTER_IPV4_CKSUM | DEV_TX_OFFLOAD_TCP_TSO |
                                DEV_TX_OFFLOAD_VXLAN_TNL_TSO | DEV_TX_OFFLOAD_GRE_TNL_TSO |
                                DEV_TX_OFFLOAD_IPIP_TNL_TSO | DEV_TX_OFFLOAD_GENEVE_TNL_TSO |
                                DEV_TX_OFFLOAD_MULTI_SEGS | dev_info->tx_queue_offload_capa;
    dev_info->dev_capa = RTE_ETH_DEV_CAPA_RUNTIME_RX_QUEUE_SETUP | RTE_ETH_DEV_CAPA_RUNTIME_TX_QUEUE_SETUP;
...
static int i40e_vlan_tpid_set_by_registers(struct rte_eth_dev *dev, enum rte_vlan_type vlan_type, uint16_t tpid,
                                           int qinq) {
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    uint64_t reg_r = 0;
    uint64_t reg_w = 0;
    ...
    ret = i40e_aq_debug_read_register(hw, I40E_GL_SWT_L2TAGCTRL(reg_id), &reg_r, NULL);
    if (ret != I40E_SUCCESS) {
    ...
    ret = i40e_aq_debug_write_global_register(hw, I40E_GL_SWT_L2TAGCTRL(reg_id), reg_w, NULL);
    if (ret != I40E_SUCCESS) {
        PMD_DRV_LOG(ERR, "Fail to debug write to I40E_GL_SWT_L2TAGCTRL[%d]", reg_id);
        return -EIO;
    }
    PMD_DRV_LOG(DEBUG, "Global register 0x%08x is changed with value 0x%08x", I40E_GL_SWT_L2TAGCTRL(reg_id),
                (uint32_t)reg_w);
...
static int i40e_vlan_tpid_set(struct rte_eth_dev *dev, enum rte_vlan_type vlan_type, uint16_t tpid) {
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct i40e_pf *pf = I40E_DEV_PRIVATE_TO_PF(dev->data->dev_private);
    int qinq = dev->data->dev_conf.rxmode.offloads & DEV_RX_OFFLOAD_VLAN_EXTEND;
    int ret = 0;

    if ((vlan_type != ETH_VLAN_TYPE_INNER && vlan_type != ETH_VLAN_TYPE_OUTER) ||
    ...
    if (hw->flags & I40E_HW_FLAG_802_1AD_CAPABLE) {
        ...
        if (ret != I40E_SUCCESS) {
            PMD_DRV_LOG(ERR, "Set switch config failed aq_err: %d", hw->aq.asq_last_status);
            ret = -EIO;
        }
    } else
...
static void i40e_vlan_strip_queue_set(__rte_unused struct rte_eth_dev *dev, __rte_unused uint16_t queue,
                                      __rte_unused int on) {
    PMD_INIT_FUNC_TRACE();
}
...
static int i40e_flow_ctrl_get(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf) {
    ...
    pf->fc_conf.high_water[I40E_MAX_TRAFFIC_CLASS] = I40E_READ_REG(hw, I40E_GLRPB_GHW) >> I40E_KILOSHIFT;
    pf->fc_conf.low_water[I40E_MAX_TRAFFIC_CLASS] = I40E_READ_REG(hw, I40E_GLRPB_GLW) >> I40E_KILOSHIFT;
...
static int i40e_flow_ctrl_set(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf) {
    ...
    struct i40e_hw *hw;
    struct i40e_pf *pf;
    enum i40e_fc_mode rte_fcmode_2_i40e_fcmode[] = {[RTE_FC_NONE] = I40E_FC_NONE,
    ...
    if ((fc_conf->high_water > max_high_water) || (fc_conf->high_water < fc_conf->low_water)) {
    ...
    if (I40E_PHY_TYPE_SUPPORT_40G(hw->phy.phy_types)) {
        ...
        I40E_WRITE_REG(hw, I40E_PRTMAC_HSEC_CTL_TX_PAUSE_REFRESH_TIMER(8), pf->fc_conf.pause_time);
         ...
         * pause operation
         */
        I40E_WRITE_REG(hw, I40E_PRTMAC_HSEC_CTL_TX_PAUSE_QUANTA(8), pf->fc_conf.pause_time);
    ...
    if (!pf->support_multi_driver) {
        ...
        I40E_WRITE_GLB_REG(hw, I40E_GLRPB_PLW,
                           (pf->fc_conf.low_water[I40E_MAX_TRAFFIC_CLASS] << I40E_KILOSHIFT) /
                               I40E_PACKET_AVERAGE_SIZE);
        I40E_WRITE_GLB_REG(hw, I40E_GLRPB_GHW, pf->fc_conf.high_water[I40E_MAX_TRAFFIC_CLASS] << I40E_KILOSHIFT);
        I40E_WRITE_GLB_REG(hw, I40E_GLRPB_GLW, pf->fc_conf.low_water[I40E_MAX_TRAFFIC_CLASS] << I40E_KILOSHIFT);
    } else {
...
static int i40e_priority_flow_ctrl_set(__rte_unused struct rte_eth_dev *dev,
...
static int i40e_macaddr_add(struct rte_eth_dev *dev, struct ether_addr *mac_addr, __rte_unused uint32_t index,
                            uint32_t pool) {
    struct i40e_pf *pf = I40E_DEV_PRIVATE_TO_PF(dev->data->dev_private);
    ...
    if (pool != 0 && (!(pf->flags & I40E_FLAG_VMDQ) || !pf->nb_cfg_vmdq_vsi)) {
        PMD_DRV_LOG(ERR, "VMDQ not %s, can't set mac to pool %u", pf->flags & I40E_FLAG_VMDQ ? "configured" : "enabled",
                    pool);
        return -ENOTSUP;
    }

    if (pool > pf->nb_cfg_vmdq_vsi) {
        PMD_DRV_LOG(ERR, "Pool number %u invalid. Max pool is %u", pool, pf->nb_cfg_vmdq_vsi);
        return -EINVAL;
    }
...
static int i40e_vf_mac_filter_set(struct i40e_pf *pf, struct rte_eth_mac_filter *filter, bool add) {
    struct i40e_hw *hw;
    struct i40e_mac_filter_info mac_filter;
    ...
    if (add) {
        ...
        rte_memcpy(hw->mac.addr, new_mac->addr_bytes, ETHER_ADDR_LEN);
        rte_memcpy(&mac_filter.mac_addr, &filter->mac_addr, ETHER_ADDR_LEN);
        ...
        rte_memcpy(hw->mac.addr, hw->mac.perm_addr, ETHER_ADDR_LEN);
...
/* MAC filter handle */
static int i40e_mac_filter_handle(struct rte_eth_dev *dev, enum rte_filter_op filter_op, void *arg) {
    struct i40e_pf *pf = I40E_DEV_PRIVATE_TO_PF(dev->data->dev_private);
...
static int i40e_get_rss_lut(struct i40e_vsi *vsi, uint8_t *lut, uint16_t lut_size) {
    struct i40e_pf *pf = I40E_VSI_TO_PF(vsi);
    struct i40e_hw *hw = I40E_VSI_TO_HW(vsi);
    uint32_t reg;
    int ret;
...
int i40e_set_rss_lut(struct i40e_vsi *vsi, uint8_t *lut, uint16_t lut_size) {
    ...
    if (pf->flags & I40E_FLAG_RSS_AQ_CAPABLE) {
        ret = i40e_aq_set_rss_lut(hw, vsi->vsi_id, vsi->type != I40E_VSI_SRIOV, lut, lut_size);
        if (ret) {
...
static int i40e_dev_rss_reta_update(struct rte_eth_dev *dev, struct rte_eth_rss_reta_entry64 *reta_conf,
    ...
    if (reta_size != lut_size || reta_size > ETH_RSS_RETA_SIZE_512) {
        PMD_DRV_LOG(
            ERR, "The size of hash lookup table configured (%d) doesn't match the number hardware can supported (%d)",
            reta_size, lut_size);
        return -EINVAL;
    }
 ...
 **/
enum i40e_status_code i40e_allocate_dma_mem_d(__attribute__((unused)) struct i40e_hw *hw, struct i40e_dma_mem *mem,
    ...
    mz = rte_memzone_reserve_bounded(z_name, size, SOCKET_ID_ANY, RTE_MEMZONE_IOVA_CONTIG, alignment, RTE_PGSIZE_2M);
    if (!mz)
    ...
    PMD_DRV_LOG(DEBUG, "memzone %s allocated with physical address: %" PRIu64, mz->name, mem->pa);
 ...
 **/
enum i40e_status_code i40e_free_dma_mem_d(__attribute__((unused)) struct i40e_hw *hw, struct i40e_dma_mem *mem) {
    if (!mem)
        return I40E_ERR_PARAM;
 ...
 * @size: size of memory requested
 **/
enum i40e_status_code i40e_allocate_virt_mem_d(__attribute__((unused)) struct i40e_hw *hw, struct i40e_virt_mem *mem,
                                               u32 size) {
    if (!mem)
        return I40E_ERR_PARAM;
 ...
 **/
enum i40e_status_code i40e_free_virt_mem_d(__attribute__((unused)) struct i40e_hw *hw, struct i40e_virt_mem *mem) {
    if (!mem)
        return I40E_ERR_PARAM;
...
void i40e_init_spinlock_d(struct i40e_spinlock *sp) { rte_spinlock_init(&sp->spinlock); }

void i40e_acquire_spinlock_d(struct i40e_spinlock *sp) { rte_spinlock_lock(&sp->spinlock); }

void i40e_release_spinlock_d(struct i40e_spinlock *sp) { rte_spinlock_unlock(&sp->spinlock); }

void i40e_destroy_spinlock_d(__attribute__((unused)) struct i40e_spinlock *sp) { return; }
...
static int i40e_get_cap(struct i40e_hw *hw) {
    ...
    len = sizeof(struct i40e_aqc_list_capabilities_element_resp) * I40E_MAX_CAP_ELE_NUM;
    ...
    ret = i40e_aq_discover_capabilities(hw, buf, len, &size, i40e_aqc_opc_list_func_capabilities, NULL);
    if (ret != I40E_SUCCESS)
...
static int i40e_pf_parse_vf_queue_number_handler(const char *key, const char *value, void *opaque) {
    struct i40e_pf *pf;
    unsigned long num;
    char *end;
...
static int i40e_pf_config_vf_rxq_number(struct rte_eth_dev *dev) {
    ...
    rte_kvargs_process(kvlist, ETH_I40E_QUEUE_NUM_PER_VF_ARG, i40e_pf_parse_vf_queue_number_handler, pf);
...
static int i40e_pf_parameter_init(struct rte_eth_dev *dev) {
    ...
    if (hw->func_caps.sr_iov_1_1 && pci_dev->max_vfs) {
        ...
        PMD_DRV_LOG(DEBUG, "%u VF VSIs, %u queues per VF VSI, in total %u queues", pf->vf_num, pf->vf_nb_qps,
    ...
    if (hw->func_caps.vmdq) {
        if (qp_count < hw->func_caps.num_tx_qp && vsi_count < hw->func_caps.num_vsis) {
            pf->max_nb_vmdq_vsi = (hw->func_caps.num_tx_qp - qp_count) / pf->vmdq_nb_qp_max;

            /* Limit the maximum number of VMDq vsi to the maximum
             * ethdev can support
             */
            pf->max_nb_vmdq_vsi = RTE_MIN(pf->max_nb_vmdq_vsi, hw->func_caps.num_vsis - vsi_count);
            pf->max_nb_vmdq_vsi = RTE_MIN(pf->max_nb_vmdq_vsi, ETH_64_POOLS);
            if (pf->max_nb_vmdq_vsi) {
                pf->flags |= I40E_FLAG_VMDQ;
                pf->vmdq_nb_qps = pf->vmdq_nb_qp_max;
                PMD_DRV_LOG(DEBUG, "%u VMDQ VSIs, %u queues per VMDQ VSI, in total %u queues", pf->max_nb_vmdq_vsi,
                            pf->vmdq_nb_qps, pf->vmdq_nb_qps * pf->max_nb_vmdq_vsi);
            } else {
    ...
    if (qp_count > hw->func_caps.num_tx_qp) {
        PMD_DRV_LOG(ERR, "Failed to allocate %u queues, which exceeds the hardware maximum %u", qp_count,
                    hw->func_caps.num_tx_qp);
        return -EINVAL;
    }
    if (vsi_count > hw->func_caps.num_vsis) {
        PMD_DRV_LOG(ERR, "Failed to allocate %u VSIs, which exceeds the hardware maximum %u", vsi_count,
...
static int i40e_pf_get_switch_config(struct i40e_pf *pf) {
    ...
    switch_config = (struct i40e_aqc_get_switch_config_resp *)rte_zmalloc("i40e", I40E_AQ_LARGE_BUF, 0);
    if (!switch_config) {
    ...
    ret = i40e_aq_get_switch_config(hw, switch_config, I40E_AQ_LARGE_BUF, &start_seid, NULL);
    if (ret != I40E_SUCCESS) {
...
static void i40e_res_pool_destroy(struct i40e_res_pool_info *pool) {
    ...
    for (entry = LIST_FIRST(&pool->alloc_list); entry && (next_entry = LIST_NEXT(entry, next), 1); entry = next_entry) {
        LIST_REMOVE(entry, next);
        rte_free(entry);
    }

    for (entry = LIST_FIRST(&pool->free_list); entry && (next_entry = LIST_NEXT(entry, next), 1); entry = next_entry) {
        LIST_REMOVE(entry, next);
        rte_free(entry);
    }
 ...
 **/
static inline int bitmap_is_subset(uint8_t src1, uint8_t src2) { return !((src1 ^ src2) & src2); }

static enum i40e_status_code validate_tcmap_parameter(struct i40e_vsi *vsi, uint8_t enabled_tcmap) {
    struct i40e_hw *hw = I40E_VSI_TO_HW(vsi);
    ...
    if (!bitmap_is_subset(hw->func_caps.enabled_tcmap, enabled_tcmap)) {
        PMD_DRV_LOG(ERR, "Enabled TC map 0x%x not applicable to HW support 0x%x", hw->func_caps.enabled_tcmap,
                    enabled_tcmap);
        return I40E_NOT_SUPPORTED;
    }
    return I40E_SUCCESS;
...
int i40e_vsi_vlan_pvid_set(struct i40e_vsi *vsi, struct i40e_vsi_vlan_pvid_info *info) {
    ...
    vsi->info.port_vlan_flags &= ~(I40E_AQ_VSI_PVLAN_INSERT_PVID | I40E_AQ_VSI_PVLAN_MODE_MASK);
...
static int i40e_vsi_update_tc_bandwidth(struct i40e_vsi *vsi, uint8_t enabled_tcmap) {
    struct i40e_hw *hw = I40E_VSI_TO_HW(vsi);
    int i, ret;
    ...
    rte_memcpy(vsi->info.qs_handle, tc_bw_data.qs_handles, sizeof(vsi->info.qs_handle));
    return I40E_SUCCESS;
}

static enum i40e_status_code i40e_vsi_config_tc_queue_mapping(struct i40e_vsi *vsi,
    ...
    for (i = 0; i < I40E_MAX_TRAFFIC_CLASS; i++) {
        if (vsi->enabled_tc & (1 << i)) {
            info->tc_mapping[i] = rte_cpu_to_le_16((qp_idx << I40E_AQ_VSI_TC_QUE_OFFSET_SHIFT) |
...
static struct i40e_veb *i40e_veb_setup(struct i40e_pf *pf, struct i40e_vsi *vsi) {
    ...
    ret = i40e_aq_get_veb_parameters(hw, veb->seid, NULL, NULL, &veb->stats_idx, NULL, NULL, NULL);
    if (ret != I40E_SUCCESS) {
        PMD_DRV_LOG(ERR, "Get veb statistics index failed, aq_err: %d", hw->aq.asq_last_status);
        goto fail;
    }
...
int i40e_vsi_release(struct i40e_vsi *vsi) {
    ...
    if (vsi->type != I40E_VSI_MAIN && ((vsi->type != I40E_VSI_SRIOV) || !pf->floating_veb_list[user_param])) {
    ...
    if ((vsi->type == I40E_VSI_SRIOV) && pf->floating_veb_list[user_param]) {
        ...
        TAILQ_REMOVE(&vsi->parent_vsi->floating_veb->head, &vsi->sib_vsi_list, list);
...
static int i40e_update_default_filter_setting(struct i40e_vsi *vsi) {
    ...
    def_filter.flags = I40E_AQC_MACVLAN_DEL_PERFECT_MATCH | I40E_AQC_MACVLAN_DEL_IGNORE_VLAN;
    ...
    rte_memcpy(&filter.mac_addr, (struct ether_addr *)(hw->mac.perm_addr), ETH_ADDR_LEN);
    filter.filter_type = RTE_MACVLAN_PERFECT_MATCH;
    return i40e_vsi_add_mac(vsi, &filter);
}

#ifdef TREX_PATCH_LOW_LATENCY
static int i40e_vsi_update_tc_max_bw(struct i40e_vsi *vsi, u16 credit) {
    struct i40e_hw *hw = I40E_VSI_TO_HW(vsi);
    int ret;
...
static int i40e_vsi_update_tc_bandwidth_ex(struct i40e_vsi *vsi) {
    struct i40e_hw *hw = I40E_VSI_TO_HW(vsi);
    int i, ret;
    ...
    (void)rte_memcpy(vsi->info.qs_handle, res_buffer->qs_handles, sizeof(vsi->info.qs_handle));
...
static enum i40e_status_code i40e_vsi_get_bw_config(struct i40e_vsi *vsi) {
    ...
    if (ret != I40E_SUCCESS) {
        PMD_DRV_LOG(ERR, "VSI failed to get bandwidth configuration %u", hw->aq.asq_last_status);
        return ret;
    }
    ...
    for (i = 0; i < I40E_MAX_TRAFFIC_CLASS; i++) {
        ...
        vsi->bw_info.bw_ets_credits[i] = rte_le_to_cpu_16(ets_sla_config.credits[i]);
        /* 4 bits per TC, 4th bit is reserved */
        vsi->bw_info.bw_ets_max[i] = (uint8_t)((bw_max >> (i * I40E_4_BIT_WIDTH)) & RTE_LEN2MASK(3, uint8_t));
        PMD_DRV_LOG(DEBUG, "\tVSI TC%u:share credits %u", i, vsi->bw_info.bw_ets_share_credits[i]);
        PMD_DRV_LOG(DEBUG, "\tVSI TC%u:credits %u", i, vsi->bw_info.bw_ets_credits[i]);
        PMD_DRV_LOG(DEBUG, "\tVSI TC%u: max credits: %u", i, vsi->bw_info.bw_ets_max[i]);
    }
...
static inline void i40e_enable_pf_lb(struct i40e_pf *pf) {
    ...
    if (ret) {
        PMD_DRV_LOG(ERR, "cannot get pf vsi config, err %d, aq_err %d", ret, hw->aq.asq_last_status);
        return;
    }
...
/* Setup a VSI */
struct i40e_vsi *i40e_vsi_setup(struct i40e_pf *pf, enum i40e_vsi_type type, struct i40e_vsi *uplink_vsi,
                                uint16_t user_param) {
    struct i40e_hw *hw = I40E_PF_TO_HW(pf);
    struct i40e_vsi *vsi;
    struct i40e_mac_filter_info filter;
    int ret;
    struct i40e_vsi_context ctxt;
    struct ether_addr broadcast = {.addr_bytes = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff}};
    ...
    if (type == I40E_VSI_SRIOV && uplink_vsi == NULL && pf->main_vsi->floating_veb == NULL) {
    ...
    if (type == I40E_VSI_MAIN) {
        if (pf->support_multi_driver) {
            ...
            ret = i40e_res_pool_alloc(&pf->msix_pool, RTE_MIN(vsi->nb_qps, RTE_MAX_RXTX_INTR_VEC_ID));
            if (ret < 0) {
    ...
    if (type == I40E_VSI_MAIN) {
        ...
        rte_memcpy(&vsi->info, &ctxt.info, sizeof(struct i40e_aqc_vsi_properties_data));
        ...
        rte_memcpy(&ctxt.info, &vsi->info, sizeof(struct i40e_aqc_vsi_properties_data));
        ...
        rte_memcpy(&vsi->info.tc_mapping, &ctxt.info.tc_mapping, sizeof(vsi->info.tc_mapping));
        rte_memcpy(&vsi->info.queue_mapping, &ctxt.info.queue_mapping, sizeof(vsi->info.queue_mapping));
        ...
        ret = i40e_vsi_config_tc_queue_mapping(vsi, &ctxt.info, hw->func_caps.enabled_tcmap);
        if (ret != I40E_SUCCESS) {
            PMD_DRV_LOG(ERR, "Failed to configure TC queue mapping");
            goto fail_msix_alloc;
        }
    ...
    if (vsi->type != I40E_VSI_MAIN) {
        ...
        if (vsi->type == I40E_VSI_SRIOV && uplink_vsi == NULL) {
            TAILQ_INSERT_TAIL(&pf->main_vsi->floating_veb->head, &vsi->sib_vsi_list, list);
        } else {
...
int i40e_vsi_config_vlan_filter(struct i40e_vsi *vsi, bool on) {
    ...
    mac_filter = rte_zmalloc("mac_filter_info_data", num * sizeof(*mac_filter), 0);
    if (mac_filter == NULL) {
        PMD_DRV_LOG(ERR, "failed to allocate memory");
        return I40E_ERR_NO_MEMORY;
    }
    ...
    TAILQ_FOREACH_SAFE(f, &vsi->mac_list, next, temp) {
        ...
        if (ret) {
            PMD_DRV_LOG(ERR, "Update VSI failed to %s vlan filter", on ? "enable" : "disable");
            goto DONE;
        }
        i++;
    }
...
int i40e_vsi_config_vlan_stripping(struct i40e_vsi *vsi, bool on) {
    ...
    if (vsi->info.valid_sections & rte_cpu_to_le_16(I40E_AQ_VSI_PROP_VLAN_VALID)) {
        if (on) {
            if ((vsi->info.port_vlan_flags & I40E_AQ_VSI_PVLAN_EMOD_MASK) == 0)
                return 0; /* already on */
        } else {
            if ((vsi->info.port_vlan_flags & I40E_AQ_VSI_PVLAN_EMOD_MASK) == I40E_AQ_VSI_PVLAN_EMOD_MASK)
    ...
    ret = i40e_aq_update_vsi_params(hw, &ctxt, NULL);
    if (ret)
        PMD_DRV_LOG(INFO, "Update VSI failed to %s vlan stripping", on ? "enable" : "disable");
...
static int i40e_dev_init_vlan(struct rte_eth_dev *dev) {
    ...
    /* Apply pvid setting */
    ret = i40e_vlan_pvid_set(dev, data->dev_conf.txmode.pvid, data->dev_conf.txmode.hw_vlan_insert_pvid);
    if (ret)
...
static int i40e_pf_setup(struct i40e_pf *pf) {
    ...
    if (ret)
        PMD_INIT_LOG(WARNING, "failed to allocate switch domain for device %d", ret);
    ...
    if (hw->func_caps.rss_table_size == ETH_RSS_RETA_SIZE_128)
        ...
        PMD_DRV_LOG(ERR, "Hash lookup table size (%u) not supported", hw->func_caps.rss_table_size);
        return I40E_ERR_PARAM;
    }
    PMD_DRV_LOG(INFO, "Hardware capability of hash lookup table size: %u", hw->func_caps.rss_table_size);
...
int i40e_switch_tx_queue(struct i40e_hw *hw, uint16_t q_idx, bool on) {
    ...
    for (j = 0; j < I40E_CHK_Q_ENA_COUNT; j++) {
        ...
        if (!(((reg >> I40E_QTX_ENA_QENA_REQ_SHIFT) & 0x1) ^ ((reg >> I40E_QTX_ENA_QENA_STAT_SHIFT) & 0x1))) {
            break;
        }
    }
    if (on) {
    ...
    for (j = 0; j < I40E_CHK_Q_ENA_COUNT; j++) {
        ...
        reg = I40E_READ_REG(hw, I40E_QTX_ENA(q_idx));
        if (on) {
            if ((reg & I40E_QTX_ENA_QENA_REQ_MASK) && (reg & I40E_QTX_ENA_QENA_STAT_MASK))
                break;
        } else {
            if (!(reg & I40E_QTX_ENA_QENA_REQ_MASK) && !(reg & I40E_QTX_ENA_QENA_STAT_MASK))
                break;
    ...
    if (j >= I40E_CHK_Q_ENA_COUNT) {
        PMD_DRV_LOG(ERR, "Failed to %s tx queue[%u]", (on ? "enable" : "disable"), q_idx);
...
static int i40e_dev_switch_tx_queues(struct i40e_pf *pf, bool on) {
...
int i40e_switch_rx_queue(struct i40e_hw *hw, uint16_t q_idx, bool on) {
    ...
    for (j = 0; j < I40E_CHK_Q_ENA_COUNT; j++) {
        ...
        if (!((reg >> I40E_QRX_ENA_QENA_REQ_SHIFT) & 0x1) ^ ((reg >> I40E_QRX_ENA_QENA_STAT_SHIFT) & 0x1))
            break;
    }
    ...
    for (j = 0; j < I40E_CHK_Q_ENA_COUNT; j++) {
        ...
        reg = I40E_READ_REG(hw, I40E_QRX_ENA(q_idx));
        if (on) {
            if ((reg & I40E_QRX_ENA_QENA_REQ_MASK) && (reg & I40E_QRX_ENA_QENA_STAT_MASK))
                break;
        } else {
            if (!(reg & I40E_QRX_ENA_QENA_REQ_MASK) && !(reg & I40E_QRX_ENA_QENA_STAT_MASK))
                break;
    ...
    /* Check if it is timeout */
    if (j >= I40E_CHK_Q_ENA_COUNT) {
        PMD_DRV_LOG(ERR, "Failed to %s rx queue[%u]", (on ? "enable" : "disable"), q_idx);
        return I40E_ERR_TIMEOUT;
    }
...
static int i40e_dev_switch_rx_queues(struct i40e_pf *pf, bool on) {
    struct rte_eth_dev_data *dev_data = pf->dev_data;
...
static int i40e_dev_tx_init(struct i40e_pf *pf) {
    ...
    if (ret == I40E_SUCCESS)
        i40e_set_tx_function(container_of(pf, struct i40e_adapter, pf)->eth_dev);
...
static int i40e_dev_rx_init(struct i40e_pf *pf) {
    ...
    if (ret == I40E_SUCCESS)
        i40e_set_rx_function(container_of(pf, struct i40e_adapter, pf)->eth_dev);
...
static int i40e_vmdq_setup(struct rte_eth_dev *dev) {
    ...
    if (conf_vsis > pf->max_nb_vmdq_vsi) {
        PMD_INIT_LOG(ERR, "VMDQ config: %u, max support:%u", conf->rx_adv_conf.vmdq_rx_conf.nb_queue_pools,
                     pf->max_nb_vmdq_vsi);
        return -ENOTSUP;
    }
    ...
    for (i = 0; i < conf_vsis; i++) {
        vsi = i40e_vsi_setup(pf, I40E_VSI_VMDQ2, pf->main_vsi, vmdq_conf->enable_loop_back);
        if (vsi == NULL) {
    ...
    for (i = 0; i < vmdq_conf->nb_pool_maps; i++) {
        for (j = 0; j < loop && j < pf->nb_cfg_vmdq_vsi; j++) {
            if (vmdq_conf->pool_map[i].pools & (1UL << j)) {
                PMD_INIT_LOG(INFO, "Add vlan %u to vmdq pool %u", vmdq_conf->pool_map[i].vlan_id, j);

                err = i40e_vsi_add_vlan(pf->vmdq[j].vsi, vmdq_conf->pool_map[i].vlan_id);
                if (err) {
...
static void i40e_stat_update_32(struct i40e_hw *hw, uint32_t reg, bool offset_loaded, uint64_t *offset,
                                uint64_t *stat) {
    uint64_t new_data;
    ...
    if (new_data >= *offset)
        ...
        *stat = (uint64_t)((new_data + ((uint64_t)1 << I40E_32_BIT_WIDTH)) - *offset);
}
...
static void i40e_stat_update_48(struct i40e_hw *hw, uint32_t hireg, uint32_t loreg, bool offset_loaded,
    ...
    new_data |= ((uint64_t)(I40E_READ_REG(hw, hireg) & I40E_16_BIT_MASK)) << I40E_32_BIT_WIDTH;
    ...
    if (new_data >= *offset)
        ...
        *stat = (uint64_t)((new_data + ((uint64_t)1 << I40E_48_BIT_WIDTH)) - *offset);
...
static void i40e_pf_config_irq0(struct i40e_hw *hw, bool no_queue) {
    ...
    I40E_WRITE_REG(hw, I40E_PFINT_STAT_CTL0, I40E_PFINT_STAT_CTL0_OTHER_ITR_INDX_MASK);
...
static void i40e_dev_handle_aq_msg(struct rte_eth_dev *dev) {
    ...
    while (pending) {
        ...
        if (ret != I40E_SUCCESS) {
            PMD_DRV_LOG(INFO, "Failed to read msg from AdminQ, aq_err: %u", hw->aq.asq_last_status);
            break;
        }
        ...
        case i40e_aqc_opc_send_msg_to_pf:
            ...
            i40e_pf_host_handle_vf_msg(dev, rte_le_to_cpu_16(info.desc.retval), rte_le_to_cpu_32(info.desc.cookie_high),
                                       rte_le_to_cpu_32(info.desc.cookie_low), info.msg_buf, info.msg_len);
            break;
    ...
    do {
        ...
        for (i = 0; i < actual_num; i++) {
            ...
            case RTE_MAC_PERFECT_MATCH:
                flags = I40E_AQC_MACVLAN_ADD_PERFECT_MATCH | I40E_AQC_MACVLAN_ADD_IGNORE_VLAN;
                break;
            case RTE_MACVLAN_PERFECT_MATCH:
                flags = I40E_AQC_MACVLAN_ADD_PERFECT_MATCH;
                break;
            case RTE_MAC_HASH_MATCH:
                flags = I40E_AQC_MACVLAN_ADD_HASH_MATCH | I40E_AQC_MACVLAN_ADD_IGNORE_VLAN;
                break;
...
int i40e_remove_macvlan_filters(struct i40e_vsi *vsi, struct i40e_macvlan_filter *filter, int total) {
    ...
    do {
        ...
        for (i = 0; i < actual_num; i++) {
            ...
            switch (filter[num + i].filter_type) {
            case RTE_MAC_PERFECT_MATCH:
                flags = I40E_AQC_MACVLAN_DEL_PERFECT_MATCH | I40E_AQC_MACVLAN_DEL_IGNORE_VLAN;
                break;
            case RTE_MACVLAN_PERFECT_MATCH:
                flags = I40E_AQC_MACVLAN_DEL_PERFECT_MATCH;
                break;
            case RTE_MAC_HASH_MATCH:
                flags = I40E_AQC_MACVLAN_DEL_HASH_MATCH | I40E_AQC_MACVLAN_DEL_IGNORE_VLAN;
                break;
            case RTE_MACVLAN_HASH_MATCH:
        ...
        ret = i40e_aq_remove_macvlan(hw, vsi->seid, req_list, actual_num, NULL);
        if (ret != I40E_SUCCESS) {
            PMD_DRV_LOG(ERR, "Failed to remove macvlan filter");
            goto DONE;
        }
...
static struct i40e_mac_filter *i40e_find_mac_filter(struct i40e_vsi *vsi, struct ether_addr *macaddr) {
    struct i40e_mac_filter *f;
...
static void i40e_store_vlan_filter(struct i40e_vsi *vsi, uint16_t vlan_id, bool on) {
    uint32_t vid_idx, vid_bit;
 ...
 */
int i40e_find_all_vlan_for_mac(struct i40e_vsi *vsi, struct i40e_macvlan_filter *mv_f, int num,
...
static int i40e_vsi_remove_all_macvlan_filter(struct i40e_vsi *vsi) {
    ...
    if (vsi->vlan_num == 0) {
        ...
        TAILQ_FOREACH(f, &vsi->mac_list, next) {
            ret = i40e_find_all_vlan_for_mac(vsi, &mv_f[i], vsi->vlan_num, &f->mac_info.mac_addr);
            if (ret != I40E_SUCCESS)
                goto DONE;
...
int i40e_vsi_add_mac(struct i40e_vsi *vsi, struct i40e_mac_filter_info *mac_filter) {
    struct i40e_mac_filter *f;
    struct i40e_macvlan_filter *mv_f;
    int i, vlan_num = 0;
    int ret = I40E_SUCCESS;
    ...
    if (f != NULL)
        return I40E_SUCCESS;
    if ((mac_filter->filter_type == RTE_MACVLAN_PERFECT_MATCH) || (mac_filter->filter_type == RTE_MACVLAN_HASH_MATCH)) {
    ...
    } else if ((mac_filter->filter_type == RTE_MAC_PERFECT_MATCH) || (mac_filter->filter_type == RTE_MAC_HASH_MATCH))
        vlan_num = 1;
    ...
    if (mac_filter->filter_type == RTE_MACVLAN_PERFECT_MATCH || mac_filter->filter_type == RTE_MACVLAN_HASH_MATCH) {
        ret = i40e_find_all_vlan_for_mac(vsi, mv_f, vlan_num, &mac_filter->mac_addr);
        if (ret != I40E_SUCCESS)
            goto DONE;
    }
...
int i40e_vsi_delete_mac(struct i40e_vsi *vsi, struct ether_addr *addr) {
    ...
    if (filter_type == RTE_MACVLAN_PERFECT_MATCH || filter_type == RTE_MACVLAN_HASH_MATCH) {
        if (vlan_num == 0) {
            PMD_DRV_LOG(ERR, "VLAN number shouldn't be 0");
            return I40E_ERR_PARAM;
        }
    } else if (filter_type == RTE_MAC_PERFECT_MATCH || filter_type == RTE_MAC_HASH_MATCH)
        vlan_num = 1;
...
/* Disable RSS */
static void i40e_pf_disable_rss(struct i40e_pf *pf) {
    struct i40e_hw *hw = I40E_PF_TO_HW(pf);
...
int i40e_set_rss_key(struct i40e_vsi *vsi, uint8_t *key, uint8_t key_len) {
    struct i40e_pf *pf = I40E_VSI_TO_PF(vsi);
    struct i40e_hw *hw = I40E_VSI_TO_HW(vsi);
    uint16_t key_idx = (vsi->type == I40E_VSI_SRIOV) ? I40E_VFQF_HKEY_MAX_INDEX : I40E_PFQF_HKEY_MAX_INDEX;
    int ret = 0;
    ...
    if (pf->flags & I40E_FLAG_RSS_AQ_CAPABLE) {
        struct i40e_aqc_get_set_rss_key_data *key_dw = (struct i40e_aqc_get_set_rss_key_data *)key;
...
static int i40e_get_rss_key(struct i40e_vsi *vsi, uint8_t *key, uint8_t *key_len) {
    struct i40e_pf *pf = I40E_VSI_TO_PF(vsi);
    struct i40e_hw *hw = I40E_VSI_TO_HW(vsi);
    uint32_t reg;
    int ret;
    ...
    if (pf->flags & I40E_FLAG_RSS_AQ_CAPABLE) {
        ret = i40e_aq_get_rss_key(hw, vsi->vsi_id, (struct i40e_aqc_get_set_rss_key_data *)key);
        if (ret) {
...
static int i40e_hw_rss_hash_set(struct i40e_pf *pf, struct rte_eth_rss_conf *rss_conf) {
    struct i40e_hw *hw = I40E_PF_TO_HW(pf);
    uint64_t hena;
    int ret;
...
static int i40e_dev_rss_hash_conf_get(struct rte_eth_dev *dev, struct rte_eth_rss_conf *rss_conf) {
    ...
    ret = i40e_get_rss_key(pf->main_vsi, rss_conf->rss_key, &rss_conf->rss_key_len);
    if (ret)
        return ret;
...
static int i40e_tunnel_filter_convert(struct i40e_aqc_cloud_filters_element_bb *cld_filter,
    ...
    if ((rte_le_to_cpu_16(cld_filter->element.flags) & I40E_AQC_ADD_CLOUD_FLAGS_IPV6) == I40E_AQC_ADD_CLOUD_FLAGS_IPV6)
    ...
    rte_memcpy(tunnel_filter->input.general_fields, cld_filter->general_fields, sizeof(cld_filter->general_fields));

    return 0;
}

/* Check if there exists the tunnel filter */
struct i40e_tunnel_filter *i40e_sw_tunnel_filter_lookup(struct i40e_tunnel_rule *tunnel_rule,
...
int i40e_dev_tunnel_filter_set(struct i40e_pf *pf, struct rte_eth_tunnel_filter_conf *tunnel_filter, uint8_t add) {
    ...
    cld_filter = rte_zmalloc("tunnel_filter", sizeof(struct i40e_aqc_add_rm_cloud_filt_elem_ext), 0);
    ...
    ether_addr_copy(&tunnel_filter->outer_mac, (struct ether_addr *)&pfilter->element.outer_mac);
    ether_addr_copy(&tunnel_filter->inner_mac, (struct ether_addr *)&pfilter->element.inner_mac);
    ...
    if (tunnel_filter->ip_type == RTE_TUNNEL_IPTYPE_IPV4) {
        ...
        rte_memcpy(&pfilter->element.ipaddr.v4.data, &ipv4_addr_le, sizeof(pfilter->element.ipaddr.v4.data));
    } else {
        ip_type = I40E_AQC_ADD_CLOUD_FLAGS_IPV6;
        for (i = 0; i < 4; i++) {
            convert_ipv6[i] = rte_cpu_to_le_32(rte_be_to_cpu_32(tunnel_filter->ip_addr.ipv6_addr[i]));
        }
        rte_memcpy(&pfilter->element.ipaddr.v6.data, &convert_ipv6, sizeof(pfilter->element.ipaddr.v6.data));
    }
    ...
    pfilter->element.flags |=
        rte_cpu_to_le_16(I40E_AQC_ADD_CLOUD_FLAGS_TO_QUEUE | ip_type | (tun_type << I40E_AQC_ADD_CLOUD_TNL_TYPE_SHIFT));
...
static enum i40e_status_code i40e_replace_mpls_l1_filter(struct i40e_pf *pf) {
    ...
    filter_replace_buf.data[10] =
        I40E_TR_VXLAN_GRE_KEY_MASK | I40E_TR_GENEVE_KEY_MASK | I40E_TR_GENERIC_UDP_TUNNEL_MASK;
    filter_replace_buf.data[11] =
        (I40E_TR_GRE_KEY_MASK | I40E_TR_GRE_KEY_WITH_XSUM_MASK | I40E_TR_GRE_NO_KEY_MASK) >> 8;

    status = i40e_aq_replace_cloud_filters(hw, &filter_replace, &filter_replace_buf);
    if (!status && (filter_replace.old_filter_type != filter_replace.new_filter_type))
        PMD_DRV_LOG(WARNING,
                    "i40e device %s changed cloud l1 type."
                    " original: 0x%x, new: 0x%x",
                    dev->device->name, filter_replace.old_filter_type, filter_replace.new_filter_type);

    return status;
}

static enum i40e_status_code i40e_replace_mpls_cloud_filter(struct i40e_pf *pf) {
    struct i40e_aqc_replace_cloud_filters_cmd filter_replace;
    struct i40e_aqc_replace_cloud_filters_cmd_buf filter_replace_buf;
    struct i40e_hw *hw = I40E_PF_TO_HW(pf);
    struct rte_eth_dev *dev = ((struct i40e_adapter *)hw->back)->eth_dev;
...
static enum i40e_status_code i40e_replace_gtp_l1_filter(struct i40e_pf *pf) {
    ...
    memset(&filter_replace_buf, 0, sizeof(struct i40e_aqc_replace_cloud_filters_cmd_buf));
    /* create L1 filter */
    filter_replace.old_filter_type = I40E_AQC_REPLACE_CLOUD_CMD_INPUT_FV_TUNNLE_KEY;
    filter_replace.new_filter_type = I40E_AQC_ADD_L1_FILTER_0X13;
    filter_replace.tr_bit = I40E_AQC_NEW_TR_21 | I40E_AQC_REPLACE_CLOUD_CMD_INPUT_VALIDATED;
    /* Prepare the buffer, 2 entries */
    filter_replace_buf.data[0] = I40E_AQC_REPLACE_CLOUD_CMD_INPUT_FV_TEID_WORD0;
    filter_replace_buf.data[0] |= I40E_AQC_REPLACE_CLOUD_CMD_INPUT_VALIDATED;
    filter_replace_buf.data[2] = 0xFF;
...
int i40e_dev_consistent_tunnel_filter_set(struct i40e_pf *pf, struct i40e_tunnel_filter_conf *tunnel_filter,
                                          uint8_t add) {
    uint16_t ip_type;
    uint32_t ipv4_addr, ipv4_addr_le;
    uint8_t i, tun_type = 0;
    ...
    case I40E_TUNNEL_TYPE_MPLSoUDP:
        ...
        pfilter->general_fields[I40E_AQC_ADD_CLOUD_FV_FLU_0X11_WORD0] = teid_le >> 4;
        pfilter->general_fields[I40E_AQC_ADD_CLOUD_FV_FLU_0X11_WORD1] = (teid_le & 0xF) << 12;
    ...
    case I40E_TUNNEL_TYPE_GTPC:
        ...
        teid_le = rte_cpu_to_le_32(tunnel_filter->tenant_id);
        pfilter->general_fields[I40E_AQC_ADD_CLOUD_FV_FLU_0X12_WORD0] = (teid_le >> 16) & 0xFFFF;
        pfilter->general_fields[I40E_AQC_ADD_CLOUD_FV_FLU_0X12_WORD1] = teid_le & 0xFFFF;
    ...
    case I40E_TUNNEL_TYPE_GTPU:
        ...
        teid_le = rte_cpu_to_le_32(tunnel_filter->tenant_id);
        pfilter->general_fields[I40E_AQC_ADD_CLOUD_FV_FLU_0X13_WORD0] = (teid_le >> 16) & 0xFFFF;
        pfilter->general_fields[I40E_AQC_ADD_CLOUD_FV_FLU_0X13_WORD1] = teid_le & 0xFFFF;
...
static int i40e_add_vxlan_port(struct i40e_pf *pf, uint16_t port, int udp_type) {
    int idx, ret;
    uint8_t filter_idx;
    struct i40e_hw *hw = I40E_PF_TO_HW(pf);
    ...
    PMD_DRV_LOG(INFO, "Added port %d with AQ command with index %d", port, filter_idx);
...
static int i40e_dev_udp_tunnel_port_add(struct rte_eth_dev *dev, struct rte_eth_udp_tunnel *udp_tunnel) {
    ...
    case RTE_TUNNEL_TYPE_VXLAN:
        ret = i40e_add_vxlan_port(pf, udp_tunnel->udp_port, I40E_AQC_TUNNEL_TYPE_VXLAN);
        break;
    case RTE_TUNNEL_TYPE_VXLAN_GPE:
        ret = i40e_add_vxlan_port(pf, udp_tunnel->udp_port, I40E_AQC_TUNNEL_TYPE_VXLAN_GPE);
        break;
...
static int i40e_pf_calc_configured_queues_num(struct i40e_pf *pf) {
    struct rte_eth_dev_data *data = pf->dev_data;
    int i, num;
    struct i40e_rx_queue *rxq;
...
static int i40e_pf_config_rss(struct i40e_pf *pf) {
    ...
    PMD_INIT_LOG(INFO, "Max of contiguous %u PF queues are configured", num);
    ...
    if (pf->adapter->rss_reta_updated == 0) {
        for (i = 0, j = 0; i < hw->func_caps.rss_table_size; i++, j++) {
            if (j == num)
                j = 0;
            lut = (lut << 8) | (j & ((0x1 << hw->func_caps.rss_table_entry_width) - 1));
            if ((i & 3) == 3)
    ...
    if (rss_conf.rss_key == NULL || rss_conf.rss_key_len < (I40E_PFQF_HKEY_MAX_INDEX + 1) * sizeof(uint32_t)) {
        /* Random default keys */
        static uint32_t rss_key_default[] = {0x6b793944, 0x23504cb5, 0x5bea75b6, 0x309f4f12, 0x3dc0a2b8,
                                             0x024ddcdf, 0x339b8ca0, 0x4c4af64a, 0x34fac605, 0x55d85839,
                                             0x3a58997d, 0x2ec938e1, 0x66031581};

        rss_conf.rss_key = (uint8_t *)rss_key_default;
        rss_conf.rss_key_len = (I40E_PFQF_HKEY_MAX_INDEX + 1) * sizeof(uint32_t);
    }

    return i40e_hw_rss_hash_set(pf, &rss_conf);
}

static int i40e_tunnel_filter_param_check(struct i40e_pf *pf, struct rte_eth_tunnel_filter_conf *filter) {
    ...
    if ((filter->filter_type & ETH_TUNNEL_FILTER_OMAC) && (is_zero_ether_addr(&filter->outer_mac))) {
        PMD_DRV_LOG(ERR, "Cannot add NULL outer MAC address");
        return -EINVAL;
    }

    if ((filter->filter_type & ETH_TUNNEL_FILTER_IMAC) && (is_zero_ether_addr(&filter->inner_mac))) {
...
static int i40e_dev_set_gre_key_len(struct i40e_hw *hw, uint8_t len) {
    ...
    if (reg != val) {
        ret = i40e_aq_debug_write_global_register(hw, I40E_GL_PRS_FVBM(2), reg, NULL);
        if (ret != 0)
            return ret;
        PMD_DRV_LOG(DEBUG,
    ...
    PMD_DRV_LOG(DEBUG, "Read modified GL_PRS_FVBM with 0x%08x", I40E_READ_REG(hw, I40E_GL_PRS_FVBM(2)));

    return ret;
}

static int i40e_dev_global_config_set(struct i40e_hw *hw, struct rte_eth_global_cfg *cfg) {
    int ret = -EINVAL;
...
static int i40e_filter_ctrl_global_config(struct rte_eth_dev *dev, enum rte_filter_op filter_op, void *arg) {
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    int ret = I40E_ERR_PARAM;

    switch (filter_op) {
    case RTE_ETH_FILTER_SET:
        ret = i40e_dev_global_config_set(hw, (struct rte_eth_global_cfg *)arg);
        break;
    default:
        PMD_DRV_LOG(ERR, "unknown operation %u", filter_op);
        break;
...
static int i40e_tunnel_filter_handle(struct rte_eth_dev *dev, enum rte_filter_op filter_op, void *arg) {
...
static void i40e_get_symmetric_hash_enable_per_port(struct i40e_hw *hw, uint8_t *enable) {
...
static void i40e_set_symmetric_hash_enable_per_port(struct i40e_hw *hw, uint8_t enable) {
    uint32_t reg = i40e_read_rx_ctl(hw, I40E_PRTQF_CTL_0);
 ...
 */
static int i40e_get_hash_filter_global_config(struct i40e_hw *hw, struct rte_eth_hash_global_conf *g_cfg) {
    ...
    PMD_DRV_LOG(DEBUG, "Hash function is %s", (reg & I40E_GLQF_CTL_HTOEP_MASK) ? "Toeplitz" : "Simple XOR");
    ...
    for (i = RTE_ETH_FLOW_UNKNOWN + 1; i < UINT64_BIT; i++) {
        if (!adapter->pctypes_tbl[i])
            continue;
        for (j = I40E_FILTER_PCTYPE_INVALID + 1; j < I40E_FILTER_PCTYPE_MAX; j++) {
            if (adapter->pctypes_tbl[i] & (1ULL << j)) {
                reg = i40e_read_rx_ctl(hw, I40E_GLQF_HSYM(j));
                if (reg & I40E_GLQF_HSYM_SYMH_ENA_MASK) {
                    g_cfg->sym_hash_enable_mask[0] |= (1ULL << i);
                }
            }
        }
    }
...
static int i40e_hash_global_config_check(const struct i40e_adapter *adapter,
                                         const struct rte_eth_hash_global_conf *g_cfg) {
    uint32_t i;
    uint64_t mask0, i40e_mask = adapter->flow_types_mask;

    if (g_cfg->hash_func != RTE_ETH_HASH_FUNCTION_TOEPLITZ && g_cfg->hash_func != RTE_ETH_HASH_FUNCTION_SIMPLE_XOR &&
        g_cfg->hash_func != RTE_ETH_HASH_FUNCTION_DEFAULT) {
        PMD_DRV_LOG(ERR, "Unsupported hash function type %d", g_cfg->hash_func);
        return -EINVAL;
    }
 ...
 */
static int i40e_set_hash_filter_global_config(struct i40e_hw *hw, struct rte_eth_hash_global_conf *g_cfg) {
    struct i40e_adapter *adapter = (struct i40e_adapter *)hw->back;
    struct i40e_pf *pf = &((struct i40e_adapter *)hw->back)->pf;
    int ret;
    uint16_t i, j;
    ...
    for (i = RTE_ETH_FLOW_UNKNOWN + 1; mask0 && i < UINT64_BIT; i++) {
        if (mask0 & (1UL << i)) {
            reg = (g_cfg->sym_hash_enable_mask[0] & (1ULL << i)) ? I40E_GLQF_HSYM_SYMH_ENA_MASK : 0;

            for (j = I40E_FILTER_PCTYPE_INVALID + 1; j < I40E_FILTER_PCTYPE_MAX; j++) {
                if (adapter->pctypes_tbl[i] & (1ULL << j))
                    i40e_write_global_rx_ctl(hw, I40E_GLQF_HSYM(j), reg);
            }
        }
    }
 ...
 */
static uint64_t i40e_get_valid_input_set(enum i40e_filter_pctype pctype, enum rte_filter_type filter) {
    uint64_t valid;
    ...
    static const uint64_t valid_hash_inset_table[] = {
        [I40E_FILTER_PCTYPE_FRAG_IPV4] =
            ...
            I40E_INSET_IPV4_SRC | I40E_INSET_IPV4_DST | I40E_INSET_IPV4_TOS | I40E_INSET_IPV4_PROTO |
            I40E_INSET_IPV4_TTL | I40E_INSET_TUNNEL_DMAC | I40E_INSET_TUNNEL_ID | I40E_INSET_FLEX_PAYLOAD,
        [I40E_FILTER_PCTYPE_NONF_IPV4_UDP] = I40E_INSET_DMAC | I40E_INSET_SMAC | I40E_INSET_VLAN_OUTER |
        ...
        [I40E_FILTER_PCTYPE_NONF_UNICAST_IPV4_UDP] =
            I40E_INSET_DMAC | I40E_INSET_SMAC | I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_VLAN_TUNNEL |
            I40E_INSET_IPV4_TOS | I40E_INSET_IPV4_PROTO | I40E_INSET_IPV4_TTL | I40E_INSET_TUNNEL_DMAC |
            I40E_INSET_TUNNEL_ID | I40E_INSET_IPV4_SRC | I40E_INSET_IPV4_DST | I40E_INSET_SRC_PORT |
        ...
        [I40E_FILTER_PCTYPE_NONF_IPV4_TCP] =
            ...
            I40E_INSET_IPV4_TOS | I40E_INSET_IPV4_PROTO | I40E_INSET_IPV4_TTL | I40E_INSET_TUNNEL_DMAC |
            I40E_INSET_TUNNEL_ID | I40E_INSET_IPV4_SRC | I40E_INSET_IPV4_DST | I40E_INSET_SRC_PORT |
            I40E_INSET_DST_PORT | I40E_INSET_TCP_FLAGS | I40E_INSET_FLEX_PAYLOAD,
        ...
        [I40E_FILTER_PCTYPE_NONF_IPV4_SCTP] =
            ...
            I40E_INSET_IPV4_TOS | I40E_INSET_IPV4_PROTO | I40E_INSET_IPV4_TTL | I40E_INSET_TUNNEL_DMAC |
            I40E_INSET_TUNNEL_ID | I40E_INSET_IPV4_SRC | I40E_INSET_IPV4_DST | I40E_INSET_SRC_PORT |
            I40E_INSET_DST_PORT | I40E_INSET_SCTP_VT | I40E_INSET_FLEX_PAYLOAD,
        [I40E_FILTER_PCTYPE_NONF_IPV4_OTHER] =
            I40E_INSET_DMAC | I40E_INSET_SMAC | I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_VLAN_TUNNEL |
            I40E_INSET_IPV4_TOS | I40E_INSET_IPV4_PROTO | I40E_INSET_IPV4_TTL | I40E_INSET_TUNNEL_DMAC |
            I40E_INSET_TUNNEL_ID | I40E_INSET_IPV4_SRC | I40E_INSET_IPV4_DST | I40E_INSET_FLEX_PAYLOAD,
        [I40E_FILTER_PCTYPE_FRAG_IPV6] = I40E_INSET_DMAC | I40E_INSET_SMAC | I40E_INSET_VLAN_OUTER |
                                         I40E_INSET_VLAN_INNER | I40E_INSET_VLAN_TUNNEL | I40E_INSET_IPV6_TC |
                                         I40E_INSET_IPV6_FLOW | I40E_INSET_IPV6_NEXT_HDR | I40E_INSET_IPV6_HOP_LIMIT |
                                         I40E_INSET_TUNNEL_DMAC | I40E_INSET_TUNNEL_ID | I40E_INSET_IPV6_SRC |
                                         I40E_INSET_IPV6_DST | I40E_INSET_FLEX_PAYLOAD,
        [I40E_FILTER_PCTYPE_NONF_IPV6_UDP] = I40E_INSET_DMAC | I40E_INSET_SMAC | I40E_INSET_VLAN_OUTER |
                                             I40E_INSET_VLAN_INNER | I40E_INSET_VLAN_TUNNEL | I40E_INSET_IPV6_TC |
                                             I40E_INSET_IPV6_FLOW | I40E_INSET_IPV6_NEXT_HDR |
                                             I40E_INSET_IPV6_HOP_LIMIT | I40E_INSET_IPV6_SRC | I40E_INSET_IPV6_DST |
                                             I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT | I40E_INSET_FLEX_PAYLOAD,
        [I40E_FILTER_PCTYPE_NONF_UNICAST_IPV6_UDP] =
            I40E_INSET_DMAC | I40E_INSET_SMAC | I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_VLAN_TUNNEL |
            I40E_INSET_IPV6_TC | I40E_INSET_IPV6_FLOW | I40E_INSET_IPV6_NEXT_HDR | I40E_INSET_IPV6_HOP_LIMIT |
            I40E_INSET_IPV6_SRC | I40E_INSET_IPV6_DST | I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT |
        ...
        [I40E_FILTER_PCTYPE_NONF_IPV6_SCTP] =
            I40E_INSET_DMAC | I40E_INSET_SMAC | I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_VLAN_TUNNEL |
            I40E_INSET_IPV6_TC | I40E_INSET_IPV6_FLOW | I40E_INSET_IPV6_NEXT_HDR | I40E_INSET_IPV6_HOP_LIMIT |
            I40E_INSET_IPV6_SRC | I40E_INSET_IPV6_DST | I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT | I40E_INSET_SCTP_VT |
            I40E_INSET_FLEX_PAYLOAD,
        [I40E_FILTER_PCTYPE_NONF_IPV6_OTHER] =
            I40E_INSET_DMAC | I40E_INSET_SMAC | I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_VLAN_TUNNEL |
            I40E_INSET_IPV6_TC | I40E_INSET_IPV6_FLOW | I40E_INSET_IPV6_NEXT_HDR | I40E_INSET_IPV6_HOP_LIMIT |
            I40E_INSET_IPV6_SRC | I40E_INSET_IPV6_DST | I40E_INSET_TUNNEL_ID | I40E_INSET_FLEX_PAYLOAD,
        [I40E_FILTER_PCTYPE_L2_PAYLOAD] = I40E_INSET_DMAC | I40E_INSET_SMAC | I40E_INSET_VLAN_OUTER |
                                          I40E_INSET_VLAN_INNER | I40E_INSET_VLAN_TUNNEL | I40E_INSET_LAST_ETHER_TYPE |
                                          I40E_INSET_FLEX_PAYLOAD,
    };
    ...
    static const uint64_t valid_fdir_inset_table[] = {
        [I40E_FILTER_PCTYPE_FRAG_IPV4] = I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_IPV4_SRC |
                                         I40E_INSET_IPV4_DST | I40E_INSET_IPV4_TOS | I40E_INSET_IPV4_PROTO |
                                         I40E_INSET_IPV4_TTL,
        [I40E_FILTER_PCTYPE_NONF_IPV4_UDP] = I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_IPV4_SRC |
                                             I40E_INSET_IPV4_DST | I40E_INSET_IPV4_TOS | I40E_INSET_IPV4_TTL |
                                             I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT,
        [I40E_FILTER_PCTYPE_NONF_UNICAST_IPV4_UDP] = I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER |
                                                     I40E_INSET_IPV4_SRC | I40E_INSET_IPV4_DST | I40E_INSET_IPV4_TOS |
                                                     I40E_INSET_IPV4_TTL | I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT,
        [I40E_FILTER_PCTYPE_NONF_MULTICAST_IPV4_UDP] = I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER |
                                                       I40E_INSET_IPV4_SRC | I40E_INSET_IPV4_DST | I40E_INSET_IPV4_TOS |
                                                       I40E_INSET_IPV4_TTL | I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT,
        [I40E_FILTER_PCTYPE_NONF_IPV4_TCP] = I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_IPV4_SRC |
                                             I40E_INSET_IPV4_DST | I40E_INSET_IPV4_TOS | I40E_INSET_IPV4_TTL |
                                             I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT,
        [I40E_FILTER_PCTYPE_NONF_IPV4_TCP_SYN_NO_ACK] =
            I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_IPV4_SRC | I40E_INSET_IPV4_DST |
            I40E_INSET_IPV4_TOS | I40E_INSET_IPV4_TTL | I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT,
        [I40E_FILTER_PCTYPE_NONF_IPV4_SCTP] = I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_IPV4_SRC |
                                              I40E_INSET_IPV4_DST | I40E_INSET_IPV4_TOS | I40E_INSET_IPV4_TTL |
                                              I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT | I40E_INSET_SCTP_VT,
        [I40E_FILTER_PCTYPE_NONF_IPV4_OTHER] = I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_IPV4_SRC |
                                               I40E_INSET_IPV4_DST | I40E_INSET_IPV4_TOS | I40E_INSET_IPV4_PROTO |
                                               I40E_INSET_IPV4_TTL,
        [I40E_FILTER_PCTYPE_FRAG_IPV6] = I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_IPV6_SRC |
                                         I40E_INSET_IPV6_DST | I40E_INSET_IPV6_TC | I40E_INSET_IPV6_NEXT_HDR |
                                         I40E_INSET_IPV6_HOP_LIMIT,
        [I40E_FILTER_PCTYPE_NONF_IPV6_UDP] = I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_IPV6_SRC |
        ...
        [I40E_FILTER_PCTYPE_NONF_UNICAST_IPV6_UDP] =
            ...
            I40E_INSET_IPV6_TC | I40E_INSET_IPV6_HOP_LIMIT | I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT,
        [I40E_FILTER_PCTYPE_NONF_MULTICAST_IPV6_UDP] =
            I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_IPV6_SRC | I40E_INSET_IPV6_DST |
            I40E_INSET_IPV6_TC | I40E_INSET_IPV6_HOP_LIMIT | I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT,
        [I40E_FILTER_PCTYPE_NONF_IPV6_TCP] = I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_IPV6_SRC |
                                             I40E_INSET_IPV6_DST | I40E_INSET_IPV6_TC | I40E_INSET_IPV6_HOP_LIMIT |
                                             I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT,
        [I40E_FILTER_PCTYPE_NONF_IPV6_TCP_SYN_NO_ACK] =
            I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_IPV6_SRC | I40E_INSET_IPV6_DST |
            I40E_INSET_IPV6_TC | I40E_INSET_IPV6_HOP_LIMIT | I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT,
        [I40E_FILTER_PCTYPE_NONF_IPV6_SCTP] = I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_IPV6_SRC |
                                              I40E_INSET_IPV6_DST | I40E_INSET_IPV6_TC | I40E_INSET_IPV6_HOP_LIMIT |
                                              I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT | I40E_INSET_SCTP_VT,
        [I40E_FILTER_PCTYPE_NONF_IPV6_OTHER] = I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_IPV6_SRC |
                                               I40E_INSET_IPV6_DST | I40E_INSET_IPV6_TC | I40E_INSET_IPV6_NEXT_HDR |
                                               I40E_INSET_IPV6_HOP_LIMIT,
        [I40E_FILTER_PCTYPE_L2_PAYLOAD] = I40E_INSET_VLAN_OUTER | I40E_INSET_VLAN_INNER | I40E_INSET_LAST_ETHER_TYPE,
    };
...
uint64_t i40e_get_default_input_set(uint16_t pctype) {
    static const uint64_t default_inset_table[] = {
        [I40E_FILTER_PCTYPE_FRAG_IPV4] = I40E_INSET_IPV4_SRC | I40E_INSET_IPV4_DST,
        [I40E_FILTER_PCTYPE_NONF_IPV4_UDP] =
            I40E_INSET_IPV4_SRC | I40E_INSET_IPV4_DST | I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT,
        [I40E_FILTER_PCTYPE_NONF_UNICAST_IPV4_UDP] =
            I40E_INSET_IPV4_SRC | I40E_INSET_IPV4_DST | I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT,
        [I40E_FILTER_PCTYPE_NONF_MULTICAST_IPV4_UDP] =
            I40E_INSET_IPV4_SRC | I40E_INSET_IPV4_DST | I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT,
        ...
        [I40E_FILTER_PCTYPE_NONF_IPV4_TCP_SYN_NO_ACK] =
            I40E_INSET_IPV4_SRC | I40E_INSET_IPV4_DST | I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT,
        [I40E_FILTER_PCTYPE_NONF_IPV4_SCTP] =
            I40E_INSET_IPV4_SRC | I40E_INSET_IPV4_DST | I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT | I40E_INSET_SCTP_VT,
        [I40E_FILTER_PCTYPE_NONF_IPV4_OTHER] = I40E_INSET_IPV4_SRC | I40E_INSET_IPV4_DST,
        [I40E_FILTER_PCTYPE_FRAG_IPV6] = I40E_INSET_IPV6_SRC | I40E_INSET_IPV6_DST,
        ...
        [I40E_FILTER_PCTYPE_NONF_IPV6_SCTP] =
            I40E_INSET_IPV6_SRC | I40E_INSET_IPV6_DST | I40E_INSET_SRC_PORT | I40E_INSET_DST_PORT | I40E_INSET_SCTP_VT,
        [I40E_FILTER_PCTYPE_NONF_IPV6_OTHER] = I40E_INSET_IPV6_SRC | I40E_INSET_IPV6_DST,
 ...
 */
static int i40e_parse_input_set(uint64_t *inset, enum i40e_filter_pctype pctype, enum rte_eth_input_set_field *field,
                                uint16_t size) {
    uint16_t i, j;
    int ret = -EINVAL;
    ...
    static const struct {
        ...
        {RTE_ETH_INPUT_SET_L3_IP6_NEXT_HEADER, I40E_INSET_IPV6_NEXT_HDR},
        {RTE_ETH_INPUT_SET_L3_IP6_HOP_LIMITS, I40E_INSET_IPV6_HOP_LIMIT},
        ...
        {RTE_ETH_INPUT_SET_L4_SCTP_VERIFICATION_TAG, I40E_INSET_SCTP_VT},
        {RTE_ETH_INPUT_SET_TUNNEL_L2_INNER_DST_MAC, I40E_INSET_TUNNEL_DMAC},
        {RTE_ETH_INPUT_SET_TUNNEL_L2_INNER_VLAN, I40E_INSET_VLAN_TUNNEL},
        {RTE_ETH_INPUT_SET_TUNNEL_L4_UDP_KEY, I40E_INSET_TUNNEL_ID},
        {RTE_ETH_INPUT_SET_TUNNEL_GRE_KEY, I40E_INSET_TUNNEL_ID},
        {RTE_ETH_INPUT_SET_FLEX_PAYLOAD_1ST_WORD, I40E_INSET_FLEX_PAYLOAD_W1},
        {RTE_ETH_INPUT_SET_FLEX_PAYLOAD_2ND_WORD, I40E_INSET_FLEX_PAYLOAD_W2},
        {RTE_ETH_INPUT_SET_FLEX_PAYLOAD_3RD_WORD, I40E_INSET_FLEX_PAYLOAD_W3},
        {RTE_ETH_INPUT_SET_FLEX_PAYLOAD_4TH_WORD, I40E_INSET_FLEX_PAYLOAD_W4},
        {RTE_ETH_INPUT_SET_FLEX_PAYLOAD_5TH_WORD, I40E_INSET_FLEX_PAYLOAD_W5},
        {RTE_ETH_INPUT_SET_FLEX_PAYLOAD_6TH_WORD, I40E_INSET_FLEX_PAYLOAD_W6},
        {RTE_ETH_INPUT_SET_FLEX_PAYLOAD_7TH_WORD, I40E_INSET_FLEX_PAYLOAD_W7},
        {RTE_ETH_INPUT_SET_FLEX_PAYLOAD_8TH_WORD, I40E_INSET_FLEX_PAYLOAD_W8},
    };
...
int i40e_generate_inset_mask_reg(uint64_t inset, uint32_t *mask, uint8_t nb_elem) {
    ...
    for (i = 0, idx = 0; i < RTE_DIM(inset_mask_map); i++) {
         ...
         */
        if ((inset & inset_mask_map[i].inset) == inset_mask_map[i].inset && inset_mask_map[i].mask == 0)
            inset_need_mask &= ~inset_mask_map[i].inset;
        if (!inset_need_mask)
            return 0;
    }
    for (i = 0, idx = 0; i < RTE_DIM(inset_mask_map); i++) {
        if ((inset_need_mask & inset_mask_map[i].inset) == inset_mask_map[i].inset) {
            if (idx >= nb_elem) {
...
void i40e_check_write_reg(struct i40e_hw *hw, uint32_t addr, uint32_t val) {
    ...
    PMD_DRV_LOG(DEBUG, "[0x%08x] after: 0x%08x", addr, (uint32_t)i40e_read_rx_ctl(hw, addr));
}
...
void i40e_check_write_global_reg(struct i40e_hw *hw, uint32_t addr, uint32_t val) {
    ...
    if (reg != val) {
        ...
        PMD_DRV_LOG(WARNING,
                    "i40e device %s changed global register [0x%08x]."
                    " original: 0x%08x, new: 0x%08x",
                    dev->device->name, addr, reg, (uint32_t)i40e_read_rx_ctl(hw, addr));
    }
}
...
static void i40e_filter_input_set_init(struct i40e_pf *pf) {
    ...
    for (pctype = I40E_FILTER_PCTYPE_NONF_IPV4_UDP; pctype <= I40E_FILTER_PCTYPE_L2_PAYLOAD; pctype++) {
        ...
        num = i40e_generate_inset_mask_reg(input_set, mask_reg, I40E_INSET_MASK_NUM_REG);
        if (num < 0)
            return;
        ...
        i40e_check_write_reg(hw, I40E_PRTQF_FD_INSET(pctype, 0), (uint32_t)(inset_reg & UINT32_MAX));
        i40e_check_write_reg(hw, I40E_PRTQF_FD_INSET(pctype, 1),
                             (uint32_t)((inset_reg >> I40E_32_BIT_WIDTH) & UINT32_MAX));
        if (!pf->support_multi_driver) {
            i40e_check_write_global_reg(hw, I40E_GLQF_HASH_INSET(0, pctype), (uint32_t)(inset_reg & UINT32_MAX));
...
int i40e_hash_filter_inset_select(struct i40e_hw *hw, struct rte_eth_input_set_conf *conf) {
    ...
    if (conf->op != RTE_ETH_INPUT_SET_SELECT && conf->op != RTE_ETH_INPUT_SET_ADD) {
    ...
    if (hw->mac.type == I40E_MAC_X722) {
        ...
        pctype = (enum i40e_filter_pctype)i40e_read_rx_ctl(hw, I40E_GLQF_FD_PCTYPES((int)pctype));
    }

    ret = i40e_parse_input_set(&input_set, pctype, conf->field, conf->inset_size);
    if (ret) {
...
int i40e_fdir_filter_inset_select(struct i40e_pf *pf, struct rte_eth_input_set_conf *conf) {
    ...
    i40e_check_write_reg(hw, I40E_PRTQF_FD_INSET(pctype, 0), (uint32_t)(inset_reg & UINT32_MAX));
    i40e_check_write_reg(hw, I40E_PRTQF_FD_INSET(pctype, 1), (uint32_t)((inset_reg >> I40E_32_BIT_WIDTH) & UINT32_MAX));
...
static int i40e_hash_filter_get(struct i40e_hw *hw, struct rte_eth_hash_filter_info *info) {
    int ret = 0;
    ...
    case RTE_ETH_HASH_FILTER_GLOBAL_CONFIG:
        ret = i40e_get_hash_filter_global_config(hw, &(info->info.global_conf));
        break;
    default:
        PMD_DRV_LOG(ERR, "Hash filter info type (%d) not supported", info->info_type);
        ret = -EINVAL;
        break;
    }
...
static int i40e_hash_filter_set(struct i40e_hw *hw, struct rte_eth_hash_filter_info *info) {
    int ret = 0;
    ...
    case RTE_ETH_HASH_FILTER_SYM_HASH_ENA_PER_PORT:
        ...
        break;
    case RTE_ETH_HASH_FILTER_GLOBAL_CONFIG:
        ret = i40e_set_hash_filter_global_config(hw, &(info->info.global_conf));
        break;
    case RTE_ETH_HASH_FILTER_INPUT_SET_SELECT:
        ret = i40e_hash_filter_inset_select(hw, &(info->info.input_set_conf));
        break;
...
static int i40e_hash_filter_ctrl(struct rte_eth_dev *dev, enum rte_filter_op filter_op, void *arg) {
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    int ret = 0;
    ...
    case RTE_ETH_FILTER_GET:
        ret = i40e_hash_filter_get(hw, (struct rte_eth_hash_filter_info *)arg);
        break;
    case RTE_ETH_FILTER_SET:
        ret = i40e_hash_filter_set(hw, (struct rte_eth_hash_filter_info *)arg);
        break;
    default:
        PMD_DRV_LOG(WARNING, "Filter operation (%d) not supported", filter_op);
        ret = -ENOTSUP;
        break;
    }
...
struct i40e_ethertype_filter *i40e_sw_ethertype_filter_lookup(struct i40e_ethertype_rule *ethertype_rule,
...
int i40e_ethertype_filter_set(struct i40e_pf *pf, struct rte_eth_ethertype_filter *filter, bool add) {
    ...
    if (filter->ether_type == ETHER_TYPE_IPv4 || filter->ether_type == ETHER_TYPE_IPv6) {
        PMD_DRV_LOG(INFO, "unsupported ether_type(0x%04x) in control packet filter.", filter->ether_type);
        return -EINVAL;
    }
    ...
    ret = i40e_aq_add_rem_control_packet_filter(hw, filter->mac_addr.addr_bytes, filter->ether_type, flags,
                                                pf->main_vsi->seid, filter->queue, add, &stats, NULL);

    PMD_DRV_LOG(INFO,
                "add/rem control packet filter, return %d, mac_etype_used = %u, etype_used = %u, mac_etype_free = %u, "
                "etype_free = %u",
                ret, stats.mac_etype_used, stats.etype_used, stats.mac_etype_free, stats.etype_free);
    if (ret < 0)
        return -ENOSYS;

    /* Add or delete a filter in SW list */
    if (add) {
        ethertype_filter = rte_zmalloc("ethertype_filter", sizeof(*ethertype_filter), 0);
...
static int i40e_ethertype_filter_handle(struct rte_eth_dev *dev, enum rte_filter_op filter_op, void *arg) {
    ...
    if (arg == NULL) {
        PMD_DRV_LOG(ERR, "arg shouldn't be NULL for operation %u", filter_op);
        return -EINVAL;
    }

    switch (filter_op) {
    case RTE_ETH_FILTER_ADD:
        ret = i40e_ethertype_filter_set(pf, (struct rte_eth_ethertype_filter *)arg, TRUE);
        break;
    case RTE_ETH_FILTER_DELETE:
        ret = i40e_ethertype_filter_set(pf, (struct rte_eth_ethertype_filter *)arg, FALSE);
        break;
    default:
...
uint16_t i40e_pctype_to_flowtype(const struct i40e_adapter *adapter, enum i40e_filter_pctype pctype) {
    ...
    for (flowtype = RTE_ETH_FLOW_UNKNOWN + 1; flowtype < I40E_FLOW_TYPE_MAX; flowtype++) {
...
#define I40E_GL_SWR_PM_EF_DEVICE(dev) .device_id = (dev), .val = I40E_GL_SWR_PM_UP_THR_EF_VALUE

#define I40E_GL_SWR_PM_SF_DEVICE(dev) .device_id = (dev), .val = I40E_GL_SWR_PM_UP_THR_SF_VALUE

    static const struct {
        uint16_t device_id;
        uint32_t val;
    } swr_pm_table[] = {
        {I40E_GL_SWR_PM_EF_DEVICE(I40E_DEV_ID_SFP_XL710)},  {I40E_GL_SWR_PM_EF_DEVICE(I40E_DEV_ID_KX_C)},
        {I40E_GL_SWR_PM_EF_DEVICE(I40E_DEV_ID_10G_BASE_T)}, {I40E_GL_SWR_PM_EF_DEVICE(I40E_DEV_ID_10G_BASE_T4)},

        {I40E_GL_SWR_PM_SF_DEVICE(I40E_DEV_ID_KX_B)},       {I40E_GL_SWR_PM_SF_DEVICE(I40E_DEV_ID_QSFP_A)},
        {I40E_GL_SWR_PM_SF_DEVICE(I40E_DEV_ID_QSFP_B)},     {I40E_GL_SWR_PM_SF_DEVICE(I40E_DEV_ID_20G_KR2)},
        {I40E_GL_SWR_PM_SF_DEVICE(I40E_DEV_ID_20G_KR2_A)},  {I40E_GL_SWR_PM_SF_DEVICE(I40E_DEV_ID_25G_B)},
...
static void i40e_configure_registers(struct i40e_hw *hw) {
    ...
    for (i = 0; i < RTE_DIM(reg_table); i++) {
        ...
        PMD_DRV_LOG(DEBUG, "Read from 0x%" PRIx32 ": 0x%" PRIx64, reg_table[i].addr, reg);
        if (reg == reg_table[i].val)
            continue;

        ret = i40e_aq_debug_write_register(hw, reg_table[i].addr, reg_table[i].val, NULL);
        if (ret < 0) {
            PMD_DRV_LOG(ERR, "Failed to write 0x%" PRIx64 " to the address of 0x%" PRIx32, reg_table[i].val,
                        reg_table[i].addr);
            break;
        }
        PMD_DRV_LOG(DEBUG,
...
static int i40e_config_qinq(struct i40e_hw *hw, struct i40e_vsi *vsi) {
    ...
    if ((reg & 0xff) != I40E_VSI_L2TAGSTXVALID_QINQ) {
        ...
        ret = i40e_aq_debug_write_register(hw, I40E_VSI_L2TAGSTXVALID(vsi->vsi_id), reg, NULL);
        if (ret < 0) {
 ...
 *
 **/
static enum i40e_status_code i40e_aq_add_mirror_rule(struct i40e_hw *hw, uint16_t seid, uint16_t dst_id,
                                                     uint16_t rule_type, uint16_t *entries, uint16_t count,
                                                     uint16_t *rule_id) {
    ...
    i40e_fill_default_direct_cmd_desc(&desc, i40e_aqc_opc_add_mirror_rule);
    memset(&cmd, 0, sizeof(cmd));

    buff_len = sizeof(uint16_t) * count;
    desc.datalen = rte_cpu_to_le_16(buff_len);
    if (buff_len > 0)
        desc.flags |= rte_cpu_to_le_16((uint16_t)(I40E_AQ_FLAG_BUF | I40E_AQ_FLAG_RD));
    cmd.rule_type = rte_cpu_to_le_16(rule_type << I40E_AQC_MIRROR_RULE_TYPE_SHIFT);
    ...
    PMD_DRV_LOG(INFO,
                ...
                hw->aq.asq_last_status, resp->rule_id, resp->mirror_rules_used, resp->mirror_rules_free);
...
static enum i40e_status_code i40e_aq_del_mirror_rule(struct i40e_hw *hw, uint16_t seid, uint16_t rule_type,
    ...
    i40e_fill_default_direct_cmd_desc(&desc, i40e_aqc_opc_delete_mirror_rule);
    memset(&cmd, 0, sizeof(cmd));
...
static int i40e_mirror_rule_set(struct rte_eth_dev *dev, struct rte_eth_mirror_conf *mirror_conf, uint8_t sw_id,
    ...
    if (mirror_conf->dst_pool > pf->vf_num) {
        PMD_DRV_LOG(ERR, "invalid destination pool %u.", mirror_conf->dst_pool);
        return -EINVAL;
    }
    ...
    if (mirr_rule && sw_id == mirr_rule->index) {
        if (on) {
            ...
            ret = i40e_aq_del_mirror_rule(hw, seid, mirr_rule->rule_type, mirr_rule->entries, mirr_rule->num_entries,
                                          mirr_rule->id);
            if (ret < 0) {
    ...
    mirr_rule = rte_zmalloc("i40e_mirror_rule", sizeof(struct i40e_mirror_rule), 0);
    if (!mirr_rule) {
        PMD_DRV_LOG(ERR, "failed to allocate memory");
        return I40E_ERR_NO_MEMORY;
    }
    ...
    case ETH_MIRROR_VLAN:
        for (i = 0, j = 0; i < ETH_MIRROR_MAX_VLANS; i++) {
            if (mirror_conf->vlan.vlan_mask & (1ULL << i)) {
                mirr_rule->entries[j] = mirror_conf->vlan.vlan_id[i];
                j++;
            }
        }
        if (j == 0) {
    ...
    case ETH_MIRROR_VIRTUAL_POOL_DOWN:
        ...
        mirr_rule->rule_type = (mirror_conf->rule_type == ETH_MIRROR_VIRTUAL_POOL_UP)
    ...
    case ETH_MIRROR_DOWNLINK_PORT:
        ...
        break;
    default:
        PMD_DRV_LOG(ERR, "unsupported mirror type %d.", mirror_conf->rule_type);
        rte_free(mirr_rule);
        return -EINVAL;
    }
    ...
    ret = i40e_aq_add_mirror_rule(hw, seid, dst_seid, mirr_rule->rule_type, mirr_rule->entries, j, &rule_id);
    if (ret < 0) {
...
static uint64_t i40e_read_rx_tstamp_cyclecounter(struct rte_eth_dev *dev, uint8_t index) {
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    uint64_t rx_tstamp;
...
static int i40e_timesync_write_time(struct rte_eth_dev *dev, const struct timespec *ts) {
    uint64_t ns;
    struct i40e_adapter *adapter = (struct i40e_adapter *)dev->data->dev_private;
...
static int i40e_timesync_read_time(struct rte_eth_dev *dev, struct timespec *ts) {
 ...
 * Returns 0 on success, negative value on failure
 */
static int i40e_parse_dcb_configure(struct rte_eth_dev *dev, struct i40e_dcbx_config *dcb_cfg, uint8_t *tc_map) {
...
i40e_vsi_update_queue_mapping(struct i40e_vsi *vsi, struct i40e_aqc_vsi_properties_data *info, uint8_t enabled_tcmap) {
    enum i40e_status_code ret;
    int i, total_tc = 0;
    uint16_t qpnum_per_tc, bsf, qp_idx;
    ...
    if (vsi->type == I40E_VSI_MAIN)
        used_queues = dev_data->nb_rx_queues - pf->nb_cfg_vmdq_vsi * RTE_LIBRTE_I40E_QUEUE_NUM_PER_VM;
    else if (vsi->type == I40E_VSI_VMDQ2)
 ...
 * Returns 0 on success, negative value on failure
 */
static enum i40e_status_code i40e_config_switch_comp_tc(struct i40e_veb *veb, uint8_t tc_map) {
    ...
    if (ret) {
        PMD_INIT_LOG(ERR, "AQ command Config switch_comp BW allocation per TC failed = %d", hw->aq.asq_last_status);
        return ret;
    }
    ...
    bw_max = rte_le_to_cpu_16(bw_query.tc_bw_max[0]) | (rte_le_to_cpu_16(bw_query.tc_bw_max[1]) << I40E_16_BIT_WIDTH);
    for (i = 0; i < I40E_MAX_TRAFFIC_CLASS; i++) {
        ...
        /* 4 bits per TC, 4th bit is reserved */
        veb->bw_info.bw_ets_max[i] = (uint8_t)((bw_max >> (i * I40E_4_BIT_WIDTH)) & RTE_LEN2MASK(3, uint8_t));
        PMD_DRV_LOG(DEBUG, "\tVEB TC%u:share credits %u", i, veb->bw_info.bw_ets_share_credits[i]);
        PMD_DRV_LOG(DEBUG, "\tVEB TC%u:credits %u", i, veb->bw_info.bw_ets_credits[i]);
        PMD_DRV_LOG(DEBUG, "\tVEB TC%u: max credits: %u", i, veb->bw_info.bw_ets_max[i]);
    }
 ...
 * Returns 0 on success, negative value on failure
 */
static enum i40e_status_code i40e_vsi_config_tc(struct i40e_vsi *vsi, uint8_t tc_map) {
    ...
    if (ret) {
        PMD_INIT_LOG(ERR, "AQ command Config VSI BW allocation per TC failed = %d", hw->aq.asq_last_status);
        goto out;
    }
    for (i = 0; i < I40E_MAX_TRAFFIC_CLASS; i++)
    ...
    ret = i40e_aq_update_vsi_params(hw, &ctxt, NULL);
    if (ret) {
        PMD_INIT_LOG(ERR, "Failed to configure TC queue mapping = %d", hw->aq.asq_last_status);
        goto out;
    }
    ...
    if (ret) {
        PMD_INIT_LOG(ERR, "Failed updating vsi bw info, err %s aq_err %s", i40e_stat_str(hw, ret),
 ...
 * Returns 0 on success, negative value on failure
 */
static enum i40e_status_code i40e_dcb_hw_configure(struct i40e_pf *pf, struct i40e_dcbx_config *new_cfg,
                                                   uint8_t tc_map) {
    struct i40e_hw *hw = I40E_PF_TO_HW(pf);
    ...
    if (!(((hw->aq.fw_maj_ver == 4) && (hw->aq.fw_min_ver >= 4)) || (hw->aq.fw_maj_ver >= 5))) {
    ...
    *old_cfg = *new_cfg;
    ...
    if (ret) {
        PMD_INIT_LOG(ERR, "Set DCB Config failed, err %s aq_err %s", i40e_stat_str(hw, ret),
                     i40e_aq_str(hw, hw->aq.asq_last_status));
        return ret;
    }
    ...
    for (i = 0; i <= I40E_PRTDCB_RETSTCC_MAX_INDEX; i++) {
        ...
        val &= ~(I40E_PRTDCB_RETSTCC_BWSHARE_MASK | I40E_PRTDCB_RETSTCC_UPINTC_MODE_MASK |
                 I40E_PRTDCB_RETSTCC_ETSTC_SHIFT);
        val |= ((uint32_t)old_cfg->etscfg.tcbwtable[i] << I40E_PRTDCB_RETSTCC_BWSHARE_SHIFT) &
               I40E_PRTDCB_RETSTCC_BWSHARE_MASK;
        val |= ((uint32_t)1 << I40E_PRTDCB_RETSTCC_UPINTC_MODE_SHIFT) & I40E_PRTDCB_RETSTCC_UPINTC_MODE_MASK;
        val |= ((uint32_t)1 << I40E_PRTDCB_RETSTCC_ETSTC_SHIFT) & I40E_PRTDCB_RETSTCC_ETSTC_MASK;
    ...
    if (main_vsi->veb) {
        ...
        if (ret)
            PMD_INIT_LOG(WARNING, "Failed configuring TC for VEB seid=%d", main_vsi->veb->seid);
    }
    /* Update each VSI */
    ...
    if (main_vsi->veb) {
        TAILQ_FOREACH(vsi_list, &main_vsi->veb->head, list) {
            ...
            if (ret)
                PMD_INIT_LOG(WARNING, "Failed configuring TC for VSI seid=%d", vsi_list->vsi->seid);
            /* continue */
        }
    }
    return I40E_SUCCESS;
...
int i40e_dcb_init_configure(struct rte_eth_dev *dev, bool sw_dcb) {
    ...
    if (sw_dcb == TRUE) {
         ...
         */
        if ((ret == I40E_SUCCESS) || (ret != I40E_SUCCESS && hw->aq.asq_last_status == I40E_AQ_RC_EPERM)) {
            ...
            ret = i40e_set_dcb_config(hw);
            if (ret) {
                PMD_INIT_LOG(INFO, "default dcb config fails. err = %d, aq_err = %d.", ret, hw->aq.asq_last_status);
                return -ENOSYS;
            }
#ifdef TREX_PATCH_LOW_LATENCY
            if (i40e_vsi_update_tc_bandwidth_ex(pf->main_vsi) != I40E_SUCCESS) {
                PMD_DRV_LOG(ERR, "Failed to update TC bandwidth");
                return -ENOSYS;
            }
#endif
        } else {
            PMD_INIT_LOG(INFO, "DCB initialization in FW fails, err = %d, aq_err = %d.", ret, hw->aq.asq_last_status);
            return -ENOTSUP;
        }
    } else {
        ...
        if (!ret) {
            if (hw->dcbx_status == I40E_DCBX_STATUS_DISABLED) {
                PMD_INIT_LOG(INFO, "HW doesn't support DCBX offload.");
                return -ENOTSUP;
            }
        } else {
            PMD_INIT_LOG(INFO, "DCBX configuration failed, err = %d, aq_err = %d.", ret, hw->aq.asq_last_status);
            return -ENOTSUP;
        }
    }
    return 0;
    ...
    if (!pf->nb_cfg_vmdq_vsi) {
        for (i = 0; i < I40E_MAX_TRAFFIC_CLASS; i++) {
            ...
            dcb_info->tc_queue.tc_rxq[j][i].base =
                (tc_mapping & I40E_AQ_VSI_TC_QUE_OFFSET_MASK) >> I40E_AQ_VSI_TC_QUE_OFFSET_SHIFT;
            dcb_info->tc_queue.tc_txq[j][i].base = dcb_info->tc_queue.tc_rxq[j][i].base;
            bsf = (tc_mapping & I40E_AQ_VSI_TC_QUE_NUMBER_MASK) >> I40E_AQ_VSI_TC_QUE_NUMBER_SHIFT;
            dcb_info->tc_queue.tc_rxq[j][i].nb_queue = 1 << bsf;
            dcb_info->tc_queue.tc_txq[j][i].nb_queue = dcb_info->tc_queue.tc_rxq[j][i].nb_queue;
        }
        return 0;
    }
...
static int i40e_dev_rx_queue_intr_disable(struct rte_eth_dev *dev, uint16_t queue_id) {
    ...
    if (msix_intr == I40E_MISC_VEC_ID)
        ...
        I40E_WRITE_REG(hw, I40E_PFINT_DYN_CTLN(msix_intr - I40E_RX_VEC_START), I40E_PFINT_DYN_CTLN_ITR_INDX_MASK);
    I40E_WRITE_FLUSH(hw);
...
static inline int i40e_valid_regs(enum i40e_mac_type type, uint32_t reg_offset) {
    if ((type != I40E_MAC_X722) &&
        ((reg_offset >= 0x2b800 && reg_offset <= 0x2bb00) || (reg_offset >= 0x38700 && reg_offset <= 0x38a00) ||
         (reg_offset >= 0x3d800 && reg_offset <= 0x3db00) || (reg_offset >= 0x208e00 && reg_offset <= 0x209000) ||
         (reg_offset >= 0x20be00 && reg_offset <= 0x20c000) || (reg_offset >= 0x263c00 && reg_offset <= 0x264000) ||
...
static int i40e_get_module_info(struct rte_eth_dev *dev, struct rte_eth_dev_module_info *modinfo) {
    ...
    case I40E_MODULE_TYPE_SFP:
        status = i40e_aq_get_phy_register(hw, I40E_AQ_PHY_REG_ACCESS_EXTERNAL_MODULE, I40E_I2C_EEPROM_DEV_ADDR, 1,
    ...
    case I40E_MODULE_TYPE_QSFP_PLUS:
        ...
        status = i40e_aq_get_phy_register(hw, I40E_AQ_PHY_REG_ACCESS_EXTERNAL_MODULE, 0, 1, I40E_MODULE_REVISION_ADDR,
                                          &sff8636_rev, NULL);
        if (status)
            return -EIO;
...
static int i40e_get_module_eeprom(struct rte_eth_dev *dev, struct rte_dev_eeprom_info *info) {
    ...
    for (i = 0; i < info->length; i++) {
        ...
        status = i40e_aq_get_phy_register(hw, I40E_AQ_PHY_REG_ACCESS_EXTERNAL_MODULE, addr, offset, 1, &value, NULL);
        if (status)
            return -EIO;
...
static int i40e_set_default_mac_addr(struct rte_eth_dev *dev, struct ether_addr *mac_addr) {
    ...
    ret = i40e_aq_mac_address_write(hw, I40E_AQC_WRITE_TYPE_LAA_WOL, mac_addr->addr_bytes, NULL);
    if (ret != I40E_SUCCESS) {
...
static int i40e_dev_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
    ...
    if (dev_data->dev_started) {
        PMD_DRV_LOG(ERR, "port %d must be stopped before configuration", dev_data->port_id);
        return -EBUSY;
    }
...
static void i40e_ethertype_filter_restore(struct i40e_pf *pf) {
    struct i40e_hw *hw = I40E_PF_TO_HW(pf);
    struct i40e_ethertype_filter_list *ethertype_list = &pf->ethertype.ethertype_list;
    ...
    TAILQ_FOREACH(f, ethertype_list, rules) {
        ...
        memset(&stats, 0, sizeof(stats));
        i40e_aq_add_rem_control_packet_filter(hw, f->input.mac_addr.addr_bytes, f->input.ether_type, flags,
    ...
    PMD_DRV_LOG(INFO,
                ...
                stats.mac_etype_used, stats.etype_used, stats.mac_etype_free, stats.etype_free);
}
...
static void i40e_tunnel_filter_restore(struct i40e_pf *pf) {
    ...
    TAILQ_FOREACH(f, tunnel_list, rules) {
        ...
        ether_addr_copy((struct ether_addr *)&f->input.outer_mac, (struct ether_addr *)&cld_filter.element.outer_mac);
        ether_addr_copy((struct ether_addr *)&f->input.inner_mac, (struct ether_addr *)&cld_filter.element.inner_mac);
        ...
        rte_memcpy(cld_filter.general_fields, f->input.general_fields, sizeof(f->input.general_fields));

        if (((f->input.flags & I40E_AQC_ADD_CLOUD_FILTER_0X11) == I40E_AQC_ADD_CLOUD_FILTER_0X11) ||
            ((f->input.flags & I40E_AQC_ADD_CLOUD_FILTER_0X12) == I40E_AQC_ADD_CLOUD_FILTER_0X12) ||
            ((f->input.flags & I40E_AQC_ADD_CLOUD_FILTER_0X10) == I40E_AQC_ADD_CLOUD_FILTER_0X10))
            big_buffer = 1;
...
static inline void i40e_rss_filter_restore(struct i40e_pf *pf) {
    struct i40e_rte_flow_rss_conf *conf = &pf->rss_info;
...
static bool is_device_supported(struct rte_eth_dev *dev, struct rte_pci_driver *drv) {
...
bool is_i40e_supported(struct rte_eth_dev *dev) { return is_device_supported(dev, &rte_i40e_pmd); }
...
static int i40e_update_customized_pctype(struct rte_eth_dev *dev, uint8_t *pkg, uint32_t pkg_size, uint32_t proto_num,
                                         struct rte_pmd_i40e_proto_info *proto, enum rte_pmd_i40e_package_op op) {
    struct i40e_pf *pf = I40E_DEV_PRIVATE_TO_PF(dev->data->dev_private);
    uint32_t pctype_num;
    ...
    ret = rte_pmd_i40e_get_ddp_info(pkg, pkg_size, (uint8_t *)&pctype_num, sizeof(pctype_num),
    ...
    ret = rte_pmd_i40e_get_ddp_info(pkg, pkg_size, (uint8_t *)pctype, buff_size, RTE_PMD_I40E_PKG_INFO_PCTYPE_LIST);
    if (ret) {
...
static int i40e_update_customized_ptype(struct rte_eth_dev *dev, uint8_t *pkg, uint32_t pkg_size, uint32_t proto_num,
                                        struct rte_pmd_i40e_proto_info *proto, enum rte_pmd_i40e_package_op op) {
    ...
    ret = rte_pmd_i40e_get_ddp_info(pkg, pkg_size, (uint8_t *)&ptype_num, sizeof(ptype_num),
    ...
    ret = rte_pmd_i40e_get_ddp_info(pkg, pkg_size, (uint8_t *)ptype, buff_size, RTE_PMD_I40E_PKG_INFO_PTYPE_LIST);
    if (ret) {
    ...
    for (i = 0; i < ptype_num; i++) {
        ...
        for (j = 0; j < RTE_PMD_I40E_PROTO_NUM; j++) {
            ...
            for (n = 0; n < proto_num; n++) {
                ...
                if (!strncasecmp(name, "PPPOE", 5))
                ...
                else if ((!strncasecmp(name, "ICMP", 4) || !strncasecmp(name, "ICMPV6", 6)) && !in_tunnel)
                    ...
                    in_tunnel = true;
                } else if (!strncasecmp(name, "L2TPV2CTL", 9) || !strncasecmp(name, "L2TPV2", 6)) {
...
void i40e_update_customized_info(struct rte_eth_dev *dev, uint8_t *pkg, uint32_t pkg_size,
    ...
    ret = rte_pmd_i40e_get_ddp_info(pkg, pkg_size, (uint8_t *)&proto_num, sizeof(proto_num),
    ...
    ret = rte_pmd_i40e_get_ddp_info(pkg, pkg_size, (uint8_t *)proto, buff_size, RTE_PMD_I40E_PKG_INFO_PROTOCOL_LIST);
    if (ret) {
...
static int i40e_cloud_filter_qinq_create(struct i40e_pf *pf) {
    ...
    filter_replace_buf.data[4] |= I40E_AQC_REPLACE_CLOUD_CMD_INPUT_VALIDATED;
    ret = i40e_aq_replace_cloud_filters(hw, &filter_replace, &filter_replace_buf);
    if (!ret && (filter_replace.old_filter_type != filter_replace.new_filter_type))
        PMD_DRV_LOG(WARNING,
                    "i40e device %s changed cloud filter type."
                    " original: 0x%x, new: 0x%x",
                    dev->device->name, filter_replace.old_filter_type, filter_replace.new_filter_type);
...
int i40e_rss_conf_init(struct i40e_rte_flow_rss_conf *out, const struct rte_flow_action_rss *in) {
    ...
    out->conf = (struct rte_flow_action_rss){
        ...
        .queue = memcpy(out->queue, in->queue, sizeof(*in->queue) * in->queue_num),
    };
    if (in->key)
...
int i40e_action_rss_same(const struct rte_flow_action_rss *comp, const struct rte_flow_action_rss *with) {
    return (comp->func == with->func && comp->level == with->level && comp->types == with->types &&
            comp->key_len == with->key_len && comp->queue_num == with->queue_num &&
            !memcmp(comp->key, with->key, with->key_len) &&
            !memcmp(comp->queue, with->queue, sizeof(*with->queue) * with->queue_num));
}
...
int i40e_config_rss_filter(struct i40e_pf *pf, struct i40e_rte_flow_rss_conf *conf, bool add) {
    ...
    if (!add) {
        if (i40e_action_rss_same(&rss_info->conf, &conf->conf)) {
            i40e_pf_disable_rss(pf);
            memset(rss_info, 0, sizeof(struct i40e_rte_flow_rss_conf));
            return 0;
        }
        return -EINVAL;
    }
    ...
    PMD_DRV_LOG(INFO, "Max of contiguous %u PF queues are configured", num);
    ...
    for (i = 0, j = 0; i < hw->func_caps.rss_table_size; i++, j++) {
        if (j == num)
            j = 0;
        lut = (lut << 8) | (conf->conf.queue[j] & ((0x1 << hw->func_caps.rss_table_entry_width) - 1));
        if ((i & 3) == 3)
...
RTE_PMD_REGISTER_PARAM_STRING(net_i40e, ETH_I40E_FLOATING_VEB_ARG
                              "=1" ETH_I40E_FLOATING_VEB_LIST_ARG "=<string>" ETH_I40E_QUEUE_NUM_PER_VF_ARG
                              "=1|2|4|8|16" ETH_I40E_SUPPORT_MULTI_DRIVER "=1" ETH_I40E_USE_LATEST_VEC "=0|1");
