...
static void i40evf_dev_info_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info);
static int i40evf_dev_link_update(struct rte_eth_dev *dev, int wait_to_complete);
static int i40evf_dev_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats);
static int i40evf_dev_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats, unsigned n);
static int i40evf_dev_xstats_get_names(struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats_names,
                                       unsigned limit);
static void i40evf_dev_xstats_reset(struct rte_eth_dev *dev);
static int i40evf_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id, int on);
...
static int i40evf_dev_rx_queue_start(struct rte_eth_dev *dev, uint16_t rx_queue_id);
static int i40evf_dev_rx_queue_stop(struct rte_eth_dev *dev, uint16_t rx_queue_id);
static int i40evf_dev_tx_queue_start(struct rte_eth_dev *dev, uint16_t tx_queue_id);
static int i40evf_dev_tx_queue_stop(struct rte_eth_dev *dev, uint16_t tx_queue_id);
static int i40evf_add_mac_addr(struct rte_eth_dev *dev, struct ether_addr *addr, uint32_t index, uint32_t pool);
static void i40evf_del_mac_addr(struct rte_eth_dev *dev, uint32_t index);
static int i40evf_dev_rss_reta_update(struct rte_eth_dev *dev, struct rte_eth_rss_reta_entry64 *reta_conf,
                                      uint16_t reta_size);
static int i40evf_dev_rss_reta_query(struct rte_eth_dev *dev, struct rte_eth_rss_reta_entry64 *reta_conf,
                                     uint16_t reta_size);
static int i40evf_config_rss(struct i40e_vf *vf);
static int i40evf_dev_rss_hash_update(struct rte_eth_dev *dev, struct rte_eth_rss_conf *rss_conf);
static int i40evf_dev_rss_hash_conf_get(struct rte_eth_dev *dev, struct rte_eth_rss_conf *rss_conf);
static int i40evf_dev_mtu_set(struct rte_eth_dev *dev, uint16_t mtu);
static int i40evf_set_default_mac_addr(struct rte_eth_dev *dev, struct ether_addr *mac_addr);
static int i40evf_dev_rx_queue_intr_enable(struct rte_eth_dev *dev, uint16_t queue_id);
static int i40evf_dev_rx_queue_intr_disable(struct rte_eth_dev *dev, uint16_t queue_id);
static void i40evf_handle_pf_event(struct rte_eth_dev *dev, uint8_t *msg, uint16_t msglen);

static int i40evf_add_del_mc_addr_list(struct rte_eth_dev *dev, struct ether_addr *mc_addr_set, uint32_t nb_mc_addr,
                                       bool add);
static int i40evf_set_mc_addr_list(struct rte_eth_dev *dev, struct ether_addr *mc_addr_set, uint32_t nb_mc_addr);
...
#define I40EVF_NB_XSTATS (sizeof(rte_i40evf_stats_strings) / sizeof(rte_i40evf_stats_strings[0]))
 ...
 */
static enum i40evf_aq_result i40evf_read_pfmsg(struct rte_eth_dev *dev, struct i40evf_arq_msg_info *data) {
    ...
    if (opcode == VIRTCHNL_OP_EVENT) {
        ...
        case VIRTCHNL_EVENT_LINK_CHANGE:
            ...
            vf->link_speed = vpe->event_data.link_event.link_speed;
 ...
 */
static inline void _clear_cmd(struct i40e_vf *vf) {
    rte_wmb();
 ...
 */
static inline int _atomic_set_cmd(struct i40e_vf *vf, enum virtchnl_ops ops) {
...
static int i40evf_execute_vf_cmd(struct rte_eth_dev *dev, struct vf_cmd_info *args) {
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    ...
    err = i40e_aq_send_msg_to_pf(hw, args->ops, I40E_SUCCESS, args->in_args, args->in_args_size, NULL);
    if (err) {
 ...
 */
static int i40evf_check_api_version(struct rte_eth_dev *dev) {
    ...
    if ((vf->version_major == VIRTCHNL_VERSION_MAJOR) && (vf->version_minor <= VIRTCHNL_VERSION_MINOR))
        PMD_DRV_LOG(INFO, "Peer is Linux PF host");
    else {
        PMD_INIT_LOG(ERR, "PF/VF API version mismatch:(%u.%u)-(%u.%u)", vf->version_major, vf->version_minor,
...
static int i40evf_get_vf_resource(struct rte_eth_dev *dev) {
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    int err;
    struct vf_cmd_info args;
    ...
    args.out_size = I40E_AQ_BUF_SZ;
    if (PF_IS_V11(vf)) {
        caps = VIRTCHNL_VF_OFFLOAD_L2 | VIRTCHNL_VF_OFFLOAD_RSS_AQ | VIRTCHNL_VF_OFFLOAD_RSS_REG |
...
static int i40evf_config_promisc(struct rte_eth_dev *dev, bool enable_unicast, bool enable_multicast) {
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    int err;
    struct vf_cmd_info args;
...
static int i40evf_enable_vlan_strip(struct rte_eth_dev *dev) {
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    struct vf_cmd_info args;
    int ret;
...
static int i40evf_disable_vlan_strip(struct rte_eth_dev *dev) {
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    struct vf_cmd_info args;
    int ret;
...
static void i40evf_fill_virtchnl_vsi_txq_info(struct virtchnl_txq_info *txq_info, uint16_t vsi_id, uint16_t queue_id,
                                              uint16_t nb_txq, struct i40e_tx_queue *txq) {
...
static void i40evf_fill_virtchnl_vsi_rxq_info(struct virtchnl_rxq_info *rxq_info, uint16_t vsi_id, uint16_t queue_id,
                                              uint16_t nb_rxq, uint32_t max_pkt_size, struct i40e_rx_queue *rxq) {
    ...
    if (queue_id < nb_rxq) {
        ...
        rxq_info->databuffer_size = (rte_pktmbuf_data_room_size(rxq->mp) - RTE_PKTMBUF_HEADROOM);
    }
}

static int i40evf_configure_vsi_queues(struct rte_eth_dev *dev) {
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    struct i40e_rx_queue **rxq = (struct i40e_rx_queue **)dev->data->rx_queues;
    struct i40e_tx_queue **txq = (struct i40e_tx_queue **)dev->data->tx_queues;
    struct virtchnl_vsi_queue_config_info *vc_vqci;
    struct virtchnl_queue_pair_info *vc_qpi;
    struct vf_cmd_info args;
    uint16_t i, nb_qp = vf->num_queue_pairs;
    const uint32_t size = I40E_VIRTCHNL_CONFIG_VSI_QUEUES_SIZE(vc_vqci, nb_qp);
    uint8_t buff[size];
    int ret;
    ...
    for (i = 0, vc_qpi = vc_vqci->qpair; i < nb_qp; i++, vc_qpi++) {
        i40evf_fill_virtchnl_vsi_txq_info(&vc_qpi->txq, vc_vqci->vsi_id, i, dev->data->nb_tx_queues, txq[i]);
        i40evf_fill_virtchnl_vsi_rxq_info(&vc_qpi->rxq, vc_vqci->vsi_id, i, dev->data->nb_rx_queues, vf->max_pkt_len,
                                          rxq[i]);
    }
    memset(&args, 0, sizeof(args));
...
static int i40evf_config_irq_map(struct rte_eth_dev *dev) {
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    struct vf_cmd_info args;
    uint8_t cmd_buffer[sizeof(struct virtchnl_irq_map_info) + sizeof(struct virtchnl_vector_map)];
    ...
    if (dev->data->dev_conf.intr_conf.rxq != 0 && rte_intr_allow_others(intr_handle))
...
static int i40evf_switch_queue(struct rte_eth_dev *dev, bool isrx, uint16_t qid, bool on) {
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    ...
    err = i40evf_execute_vf_cmd(dev, &args);
    if (err)
        PMD_DRV_LOG(ERR, "fail to switch %s %u %s", isrx ? "RX" : "TX", qid, on ? "on" : "off");

    return err;
}

static int i40evf_start_queues(struct rte_eth_dev *dev) {
...
static int i40evf_stop_queues(struct rte_eth_dev *dev) {
    int i;
...
static int i40evf_add_mac_addr(struct rte_eth_dev *dev, struct ether_addr *addr, __rte_unused uint32_t index,
                               __rte_unused uint32_t pool) {
    struct virtchnl_ether_addr_list *list;
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    uint8_t cmd_buffer[sizeof(struct virtchnl_ether_addr_list) + sizeof(struct virtchnl_ether_addr)];
    int err;
    struct vf_cmd_info args;

    if (is_zero_ether_addr(addr)) {
        PMD_DRV_LOG(ERR, "Invalid mac:%x:%x:%x:%x:%x:%x", addr->addr_bytes[0], addr->addr_bytes[1], addr->addr_bytes[2],
...
static void i40evf_del_mac_addr_by_addr(struct rte_eth_dev *dev, struct ether_addr *addr) {
    struct virtchnl_ether_addr_list *list;
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    uint8_t cmd_buffer[sizeof(struct virtchnl_ether_addr_list) + sizeof(struct virtchnl_ether_addr)];
    int err;
...
static int i40evf_query_stats(struct rte_eth_dev *dev, struct i40e_eth_stats **pstats) {
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
...
static void i40evf_update_stats(struct i40e_vsi *vsi, struct i40e_eth_stats *nes) {
    struct i40e_eth_stats *oes = &vsi->eth_stats_offset;

    i40evf_stat_update_48(&oes->rx_bytes, &nes->rx_bytes);
    i40evf_stat_update_48(&oes->rx_unicast, &nes->rx_unicast);
    i40evf_stat_update_48(&oes->rx_multicast, &nes->rx_multicast);
    i40evf_stat_update_48(&oes->rx_broadcast, &nes->rx_broadcast);
    i40evf_stat_update_32(&oes->rx_discards, &nes->rx_discards);
    i40evf_stat_update_32(&oes->rx_unknown_protocol, &nes->rx_unknown_protocol);
    i40evf_stat_update_48(&oes->tx_bytes, &nes->tx_bytes);
    i40evf_stat_update_48(&oes->tx_unicast, &nes->tx_unicast);
    i40evf_stat_update_48(&oes->tx_multicast, &nes->tx_multicast);
    i40evf_stat_update_48(&oes->tx_broadcast, &nes->tx_broadcast);
...
static int i40evf_dev_xstats_get_names(__rte_unused struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats_names,
    ...
    if (xstats_names != NULL)
        for (i = 0; i < I40EVF_NB_XSTATS; i++) {
            snprintf(xstats_names[i].name, sizeof(xstats_names[i].name), "%s", rte_i40evf_stats_strings[i].name);
        }
...
static int i40evf_dev_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats, unsigned n) {
    ...
    for (i = 0; i < I40EVF_NB_XSTATS; i++) {
        xstats[i].id = i;
        xstats[i].value = *(uint64_t *)(((char *)pstats) + rte_i40evf_stats_strings[i].offset);
    }

    return I40EVF_NB_XSTATS;
}

static int i40evf_add_vlan(struct rte_eth_dev *dev, uint16_t vlanid) {
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    struct virtchnl_vlan_filter_list *vlan_list;
    uint8_t cmd_buffer[sizeof(struct virtchnl_vlan_filter_list) + sizeof(uint16_t)];
    int err;
    struct vf_cmd_info args;
...
static int i40evf_request_queues(struct rte_eth_dev *dev, uint16_t num) {
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
...
static int i40evf_del_vlan(struct rte_eth_dev *dev, uint16_t vlanid) {
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    struct virtchnl_vlan_filter_list *vlan_list;
    uint8_t cmd_buffer[sizeof(struct virtchnl_vlan_filter_list) + sizeof(uint16_t)];
    int err;
...
/* Disable IRQ0 */
static inline void i40evf_disable_irq0(struct i40e_hw *hw) {
    /* Disable all interrupt types */
    I40E_WRITE_REG(hw, I40E_VFINT_ICR0_ENA1, 0);
    I40E_WRITE_REG(hw, I40E_VFINT_DYN_CTL01, I40E_VFINT_DYN_CTL01_ITR_INDX_MASK);
    I40EVF_WRITE_FLUSH(hw);
}

/* Enable IRQ0 */
static inline void i40evf_enable_irq0(struct i40e_hw *hw) {
    ...
    I40E_WRITE_REG(hw, I40E_VFINT_DYN_CTL01,
                   I40E_VFINT_DYN_CTL01_INTENA_MASK | I40E_VFINT_DYN_CTL01_CLEARPBA_MASK |
...
static int i40evf_check_vf_reset_done(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i < MAX_RESET_WAIT_CNT; i++) {
        reset = I40E_READ_REG(hw, I40E_VFGEN_RSTAT) & I40E_VFGEN_RSTAT_VFR_STATE_MASK;
...
static int i40evf_reset_vf(struct rte_eth_dev *dev) {
    int ret;
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int i40evf_init_vf(struct rte_eth_dev *dev) {
    int i, err, bufsz;
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    uint16_t interval = i40e_calc_itr_interval(0, 0);
    ...
    I40E_WRITE_REG(hw, I40E_VFINT_DYN_CTL01,
                   (I40E_ITR_INDEX_DEFAULT << I40E_VFINT_DYN_CTL0_ITR_INDX_SHIFT) |
                       (interval << I40E_VFINT_DYN_CTL0_INTERVAL_SHIFT));
    I40EVF_WRITE_FLUSH(hw);
...
static int i40evf_uninit_vf(struct rte_eth_dev *dev) {
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static void i40evf_handle_pf_event(struct rte_eth_dev *dev, uint8_t *msg, __rte_unused uint16_t msglen) {
...
static void i40evf_handle_aq_msg(struct rte_eth_dev *dev) {
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    ...
    while (pending) {
        ...
        case i40e_aqc_opc_send_msg_to_vf:
            if (msg_opc == VIRTCHNL_OP_EVENT)
                /* process event*/
                i40evf_handle_pf_event(dev, info.msg_buf, info.msg_len);
            else {
            ...
            break;
        default:
            PMD_DRV_LOG(ERR, "Request %u is not supported yet", aq_opc);
            break;
        }
    }
}
 ...
 *  void
 */
static void i40evf_dev_alarm_handler(void *param) {
...
static int i40evf_dev_init(struct rte_eth_dev *eth_dev) {
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
    ...
    rte_eal_alarm_set(I40EVF_ALARM_INTERVAL, i40evf_dev_alarm_handler, eth_dev);

    /* configure and enable device interrupt */
    i40evf_enable_irq0(hw);

    /* copy mac addr */
    eth_dev->data->mac_addrs = rte_zmalloc("i40evf_mac", ETHER_ADDR_LEN * I40E_NUM_MACADDR_MAX, 0);
...
static int eth_i40evf_pci_probe(struct rte_pci_driver *pci_drv __rte_unused, struct rte_pci_device *pci_dev) {
    return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct i40e_adapter), i40evf_dev_init);
}
...
static int i40evf_dev_configure(struct rte_eth_dev *dev) {
    ...
    uint16_t num_queue_pairs = RTE_MAX(dev->data->nb_rx_queues, dev->data->nb_tx_queues);
    ...
    if (num_queue_pairs > vf->vsi_res->num_queue_pairs) {
        ...
        PMD_DRV_LOG(INFO, "change queue pairs from %u to %u", vf->vsi_res->num_queue_pairs, num_queue_pairs);
...
static int i40evf_rxq_init(struct rte_eth_dev *dev, struct i40e_rx_queue *rxq) {
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct rte_eth_dev_data *dev_data = dev->data;
    ...
    rxq->max_pkt_len = RTE_MIN(len, dev_data->dev_conf.rxmode.max_rx_pkt_len);
    ...
    if ((dev_data->dev_conf.rxmode.offloads & DEV_RX_OFFLOAD_SCATTER) || rxq->max_pkt_len > buf_size)
        dev_data->scattered_rx = 1;

    return 0;
}

static int i40evf_rx_init(struct rte_eth_dev *dev) {
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    uint16_t i;
...
static void i40evf_tx_init(struct rte_eth_dev *dev) {
    uint16_t i;
    struct i40e_tx_queue **txq = (struct i40e_tx_queue **)dev->data->tx_queues;
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static inline void i40evf_enable_queues_intr(struct rte_eth_dev *dev) {
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
    struct rte_intr_handle *intr_handle = &pci_dev->intr_handle;
...
static inline void i40evf_disable_queues_intr(struct rte_eth_dev *dev) {
    struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
    struct rte_intr_handle *intr_handle = &pci_dev->intr_handle;
    ...
    if (msix_intr == I40E_MISC_VEC_ID)
        I40E_WRITE_REG(hw, I40E_VFINT_DYN_CTL01,
                       I40E_VFINT_DYN_CTL01_INTENA_MASK | I40E_VFINT_DYN_CTL01_CLEARPBA_MASK |
                           ...
                           (interval << I40E_VFINT_DYN_CTL01_INTERVAL_SHIFT));
    else
        I40E_WRITE_REG(hw, I40E_VFINT_DYN_CTLN1(msix_intr - I40E_RX_VEC_START),
                       I40E_VFINT_DYN_CTLN1_INTENA_MASK | I40E_VFINT_DYN_CTLN1_CLEARPBA_MASK |
                           (0 << I40E_VFINT_DYN_CTLN1_ITR_INDX_SHIFT) |
                           (interval << I40E_VFINT_DYN_CTLN1_INTERVAL_SHIFT));
...
static int i40evf_dev_rx_queue_intr_disable(struct rte_eth_dev *dev, uint16_t queue_id) {
    ...
    if (msix_intr == I40E_MISC_VEC_ID)
        ...
        I40E_WRITE_REG(hw, I40E_VFINT_DYN_CTLN1(msix_intr - I40E_RX_VEC_START), 0);
...
static void i40evf_add_del_all_mac_addr(struct rte_eth_dev *dev, bool add) {
    struct virtchnl_ether_addr_list *list;
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    int err, i, j;
    int next_begin = 0;
    ...
    do {
        ...
        for (i = begin; i < next_begin; i++) {
            ...
            if (is_zero_ether_addr(addr))
                continue;
            rte_memcpy(list->list[j].addr, addr->addr_bytes, sizeof(addr->addr_bytes));
            PMD_DRV_LOG(DEBUG, "add/rm mac:%x:%x:%x:%x:%x:%x", addr->addr_bytes[0], addr->addr_bytes[1],
        ...
        err = i40evf_execute_vf_cmd(dev, &args);
        if (err) {
            PMD_DRV_LOG(ERR, "fail to execute command %s", add ? "OP_ADD_ETHER_ADDRESS" : "OP_DEL_ETHER_ADDRESS");
        } else {
            if (add)
                vf->vsi.mac_num++;
            else
...
static int i40evf_dev_start(struct rte_eth_dev *dev) {
    ...
    vf->num_queue_pairs = RTE_MAX(dev->data->nb_rx_queues, dev->data->nb_tx_queues);
    ...
    i40evf_add_del_mc_addr_list(dev, vf->mc_addrs, vf->mc_addrs_num, TRUE);
...
err_mac:
    ...
    i40evf_add_del_mc_addr_list(dev, vf->mc_addrs, vf->mc_addrs_num, FALSE);
err_queue:
    return -1;
}
...
static int i40evf_dev_link_update(struct rte_eth_dev *dev, __rte_unused int wait_to_complete) {
...
static void i40evf_dev_info_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info) {
    ...
    dev_info->tx_offload_capa = DEV_TX_OFFLOAD_VLAN_INSERT | DEV_TX_OFFLOAD_QINQ_INSERT | DEV_TX_OFFLOAD_IPV4_CKSUM |
                                ...
                                DEV_TX_OFFLOAD_IPIP_TNL_TSO | DEV_TX_OFFLOAD_GENEVE_TNL_TSO | DEV_TX_OFFLOAD_MULTI_SEGS;
...
static int i40evf_dev_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
    ...
    if (ret == 0) {
        ...
        stats->opackets = pstats->tx_broadcast + pstats->tx_multicast + pstats->tx_unicast;
...
static int i40evf_get_rss_lut(struct i40e_vsi *vsi, uint8_t *lut, uint16_t lut_size) {
    ...
    if (vf->flags & I40E_FLAG_RSS_AQ_CAPABLE) {
        ret = i40e_aq_get_rss_lut(hw, vsi->vsi_id, FALSE, lut, lut_size);
        if (ret) {
...
static int i40evf_set_rss_lut(struct i40e_vsi *vsi, uint8_t *lut, uint16_t lut_size) {
    struct i40e_vf *vf;
    struct i40e_hw *hw;
    int ret;
    ...
    if (vf->flags & I40E_FLAG_RSS_AQ_CAPABLE) {
        ret = i40e_aq_set_rss_lut(hw, vsi->vsi_id, FALSE, lut, lut_size);
        if (ret) {
...
static int i40evf_set_rss_key(struct i40e_vsi *vsi, uint8_t *key, uint8_t key_len) {
    struct i40e_vf *vf = I40E_VSI_TO_VF(vsi);
    struct i40e_hw *hw = I40E_VSI_TO_HW(vsi);
    int ret = 0;
...
static int i40evf_get_rss_key(struct i40e_vsi *vsi, uint8_t *key, uint8_t *key_len) {
    struct i40e_vf *vf = I40E_VSI_TO_VF(vsi);
    struct i40e_hw *hw = I40E_VSI_TO_HW(vsi);
    int ret;
...
static int i40evf_hw_rss_hash_set(struct i40e_vf *vf, struct rte_eth_rss_conf *rss_conf) {
    struct i40e_hw *hw = I40E_VF_TO_HW(vf);
    uint64_t hena;
    int ret;

    ret = i40evf_set_rss_key(&vf->vsi, rss_conf->rss_key, rss_conf->rss_key_len);
    if (ret)
        return ret;
...
static void i40evf_disable_rss(struct i40e_vf *vf) {
    struct i40e_hw *hw = I40E_VF_TO_HW(vf);
...
static int i40evf_config_rss(struct i40e_vf *vf) {
    ...
    if (rss_conf.rss_key == NULL || rss_conf.rss_key_len < (I40E_VFQF_HKEY_MAX_INDEX + 1) * sizeof(uint32_t)) {
        /* Calculate the default hash key */
        for (i = 0; i <= I40E_VFQF_HKEY_MAX_INDEX; i++)
            rss_key_default[i] = (uint32_t)rte_rand();
        rss_conf.rss_key = (uint8_t *)rss_key_default;
        rss_conf.rss_key_len = (I40E_VFQF_HKEY_MAX_INDEX + 1) * sizeof(uint32_t);
    }
...
static int i40evf_dev_rss_hash_conf_get(struct rte_eth_dev *dev, struct rte_eth_rss_conf *rss_conf) {
    ...
    i40evf_get_rss_key(&vf->vsi, rss_conf->rss_key, &rss_conf->rss_key_len);
...
static int i40evf_add_del_mc_addr_list(struct rte_eth_dev *dev, struct ether_addr *mc_addrs, uint32_t mc_addrs_num,
                                       bool add) {
    struct virtchnl_ether_addr_list *list;
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    ...
    for (i = 0; i < mc_addrs_num; i++) {
        if (!I40E_IS_MULTICAST(mc_addrs[i].addr_bytes)) {
            PMD_DRV_LOG(ERR, "Invalid mac:%x:%x:%x:%x:%x:%x", mc_addrs[i].addr_bytes[0], mc_addrs[i].addr_bytes[1],
                        mc_addrs[i].addr_bytes[2], mc_addrs[i].addr_bytes[3], mc_addrs[i].addr_bytes[4],
                        mc_addrs[i].addr_bytes[5]);
            return -EINVAL;
        }

        memcpy(list->list[i].addr, mc_addrs[i].addr_bytes, sizeof(list->list[i].addr));
    }

    args.ops = add ? VIRTCHNL_OP_ADD_ETH_ADDR : VIRTCHNL_OP_DEL_ETH_ADDR;
    args.in_args = cmd_buffer;
    args.in_args_size = sizeof(struct virtchnl_ether_addr_list) + i * sizeof(struct virtchnl_ether_addr);
    args.out_buffer = vf->aq_resp;
    args.out_size = I40E_AQ_BUF_SZ;
    err = i40evf_execute_vf_cmd(dev, &args);
    if (err) {
        PMD_DRV_LOG(ERR, "fail to execute command %s", add ? "OP_ADD_ETH_ADDR" : "OP_DEL_ETH_ADDR");
        return err;
    }
...
static int i40evf_set_mc_addr_list(struct rte_eth_dev *dev, struct ether_addr *mc_addrs, uint32_t mc_addrs_num) {
    struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
    int err;

    /* flush previous addresses */
    err = i40evf_add_del_mc_addr_list(dev, vf->mc_addrs, vf->mc_addrs_num, FALSE);
    if (err)
        return err;
    ...
    /* add new ones */
    err = i40evf_add_del_mc_addr_list(dev, mc_addrs, mc_addrs_num, TRUE);
    if (err)
        return err;
