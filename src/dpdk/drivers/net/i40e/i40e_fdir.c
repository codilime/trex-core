...
#define I40E_FDIR_FLOWS                                                                                                \
    ((1ULL << RTE_ETH_FLOW_FRAG_IPV4) | (1ULL << RTE_ETH_FLOW_NONFRAG_IPV4_UDP) |                                      \
     (1ULL << RTE_ETH_FLOW_NONFRAG_IPV4_TCP) | (1ULL << RTE_ETH_FLOW_NONFRAG_IPV4_SCTP) |                              \
     (1ULL << RTE_ETH_FLOW_NONFRAG_IPV4_OTHER) | (1ULL << RTE_ETH_FLOW_FRAG_IPV6) |                                    \
     (1ULL << RTE_ETH_FLOW_NONFRAG_IPV6_UDP) | (1ULL << RTE_ETH_FLOW_NONFRAG_IPV6_TCP) |                               \
     (1ULL << RTE_ETH_FLOW_NONFRAG_IPV6_SCTP) | (1ULL << RTE_ETH_FLOW_NONFRAG_IPV6_OTHER) |                            \
     (1ULL << RTE_ETH_FLOW_L2_PAYLOAD))

static int i40e_fdir_filter_programming(struct i40e_pf *pf, enum i40e_filter_pctype pctype,
                                        const struct rte_eth_fdir_filter *filter, bool add);
static int i40e_fdir_filter_convert(const struct i40e_fdir_filter_conf *input, struct i40e_fdir_filter *filter);
static struct i40e_fdir_filter *i40e_sw_fdir_filter_lookup(struct i40e_fdir_info *fdir_info,
                                                           const struct i40e_fdir_input *input);
static int i40e_sw_fdir_filter_insert(struct i40e_pf *pf, struct i40e_fdir_filter *filter);
static int i40e_flow_fdir_filter_programming(struct i40e_pf *pf, enum i40e_filter_pctype pctype,
                                             const struct i40e_fdir_filter_conf *filter, bool add);
...
int i40e_fdir_setup(struct i40e_pf *pf) {
    ...
    PMD_DRV_LOG(INFO,
                ...
                hw->func_caps.fd_filters_guaranteed, hw->func_caps.fd_filters_best_effort);
    ...
    snprintf(z_name, sizeof(z_name), "%s_%s_%d", eth_dev->device->driver->name, I40E_FDIR_MZ_NAME,
    ...
    PMD_DRV_LOG(INFO, "FDIR setup successfully, with programming queue %u.", vsi->base_queue);
    return I40E_SUCCESS;
...
static inline int i40e_fdir_empty(struct i40e_hw *hw) {
    ...
    guarant_cnt = (uint32_t)((I40E_READ_REG(hw, I40E_PFQF_FDSTAT) & I40E_PFQF_FDSTAT_GUARANT_CNT_MASK) >>
                             I40E_PFQF_FDSTAT_GUARANT_CNT_SHIFT);
    best_cnt = (uint32_t)((I40E_READ_REG(hw, I40E_PFQF_FDSTAT) & I40E_PFQF_FDSTAT_BEST_CNT_MASK) >>
 ...
 */
static inline uint16_t i40e_srcoff_to_flx_pit(const uint16_t *src_offset, struct i40e_fdir_flex_pit *flex_pit) {
...
static inline int i40e_check_fdir_flex_payload(const struct rte_eth_flex_payload_cfg *flex_cfg) {
    ...
    for (i = 0; i < num; i++) {
        if (flex_pit[i].size & 0x01 || flex_pit[i].dst_offset & 0x01 || flex_pit[i].src_offset & 0x01) {
 ...
 * arguments are valid
 */
static int i40e_check_fdir_flex_conf(const struct i40e_adapter *adapter, const struct rte_eth_fdir_flex_conf *conf) {
 ...
 */
static void i40e_set_flx_pld_cfg(struct i40e_pf *pf, const struct rte_eth_flex_payload_cfg *cfg) {
    struct i40e_hw *hw = I40E_PF_TO_HW(pf);
    ...
    memset(flex_pit, 0, sizeof(flex_pit));
    num = RTE_MIN(i40e_srcoff_to_flx_pit(cfg->src_offset, flex_pit), RTE_DIM(flex_pit));

    if (num) {
        flx_ort = (1 << I40E_GLQF_ORT_FLX_PAYLOAD_SHIFT) | (num << I40E_GLQF_ORT_FIELD_CNT_SHIFT) |
    ...
    for (i = 0; i < num; i++) {
        ...
        pf->fdir.flex_set[field_idx].src_offset = flex_pit[i].src_offset / sizeof(uint16_t);
        pf->fdir.flex_set[field_idx].size = flex_pit[i].size / sizeof(uint16_t);
        pf->fdir.flex_set[field_idx].dst_offset = flex_pit[i].dst_offset / sizeof(uint16_t);
    ...
    for (; i < I40E_MAX_FLXPLD_FIED; i++) {
        ...
        I40E_WRITE_REG(hw, I40E_PRTQF_FLX_PIT(layer_idx * I40E_MAX_FLXPLD_FIED + i), flx_pit);
        min_next_off++;
    }
}
 ...
 */
static void i40e_set_flex_mask_on_pctype(struct i40e_pf *pf, enum i40e_filter_pctype pctype,
        ...
        for (i = 0; i < conf->nb_flexmasks; i++) {
            if (hw->mac.type == I40E_MAC_X722) {
                ...
                pctype = (enum i40e_filter_pctype)i40e_read_rx_ctl(
                    hw, I40E_GLQF_FD_PCTYPES((int)i40e_flowtype_to_pctype(pf->adapter, conf->flex_mask[i].flow_type)));
            } else {
                pctype = i40e_flowtype_to_pctype(pf->adapter, conf->flex_mask[i].flow_type);
            }
...
static inline int i40e_fdir_fill_eth_ip_head(const struct rte_eth_fdir_input *fdir_input, unsigned char *raw_pkt,
                                             bool vlan) {
    ...
    static const uint8_t next_proto[] = {
        [RTE_ETH_FLOW_FRAG_IPV4] = IPPROTO_IP,           [RTE_ETH_FLOW_NONFRAG_IPV4_TCP] = IPPROTO_TCP,
        [RTE_ETH_FLOW_NONFRAG_IPV4_UDP] = IPPROTO_UDP,   [RTE_ETH_FLOW_NONFRAG_IPV4_SCTP] = IPPROTO_SCTP,
        [RTE_ETH_FLOW_NONFRAG_IPV4_OTHER] = IPPROTO_IP,  [RTE_ETH_FLOW_FRAG_IPV6] = IPPROTO_NONE,
        [RTE_ETH_FLOW_NONFRAG_IPV6_TCP] = IPPROTO_TCP,   [RTE_ETH_FLOW_NONFRAG_IPV6_UDP] = IPPROTO_UDP,
        [RTE_ETH_FLOW_NONFRAG_IPV6_SCTP] = IPPROTO_SCTP, [RTE_ETH_FLOW_NONFRAG_IPV6_OTHER] = IPPROTO_NONE,
    };

    raw_pkt += 2 * sizeof(struct ether_addr);
    if (vlan && fdir_input->flow_ext.vlan_tci) {
        rte_memcpy(raw_pkt, vlan_frame, sizeof(vlan_frame));
        rte_memcpy(raw_pkt + sizeof(uint16_t), &fdir_input->flow_ext.vlan_tci, sizeof(uint16_t));
    ...
    case RTE_ETH_FLOW_FRAG_IPV4:
        ...
        *ether_type = rte_cpu_to_be_16(ETHER_TYPE_IPv4);
        ...
        ip->next_proto_id =
            fdir_input->flow.ip4_flow.proto ? fdir_input->flow.ip4_flow.proto : next_proto[fdir_input->flow_type];
        ip->time_to_live = fdir_input->flow.ip4_flow.ttl ? fdir_input->flow.ip4_flow.ttl : I40E_FDIR_IP_DEFAULT_TTL;
    ...
    case RTE_ETH_FLOW_FRAG_IPV6:
        ...
        *ether_type = rte_cpu_to_be_16(ETHER_TYPE_IPv6);
        ip6->vtc_flow = rte_cpu_to_be_32(I40E_FDIR_IPv6_DEFAULT_VTC_FLOW | (fdir_input->flow.ipv6_flow.tc <<
#ifndef TREX_PATCH
        ...
        ip6->proto =
            fdir_input->flow.ipv6_flow.proto ? fdir_input->flow.ipv6_flow.proto : next_proto[fdir_input->flow_type];
        ip6->hop_limits = fdir_input->flow.ipv6_flow.hop_limits ? fdir_input->flow.ipv6_flow.hop_limits
 ...
 */
static int i40e_fdir_construct_pkt(struct i40e_pf *pf, const struct rte_eth_fdir_input *fdir_input,
    ...
    len = i40e_fdir_fill_eth_ip_head(fdir_input, raw_pkt, !!fdir_input->flow_ext.vlan_tci);
    if (len < 0)
        return -EINVAL;
    ...
    case RTE_ETH_FLOW_L2_PAYLOAD:
         ...
         */
        if (fdir_input->flow.l2_flow.ether_type == rte_cpu_to_be_16(ETHER_TYPE_ARP))
    ...
    for (i = 0; i < I40E_MAX_FLXPLD_FIED; i++) {
        ...
        rte_memcpy(ptr, &fdir_input->flow_ext.flexbytes[dst], size * sizeof(uint16_t));
    }

    return 0;
}

static struct i40e_customized_pctype *i40e_flow_fdir_find_customized_pctype(struct i40e_pf *pf, uint8_t pctype) {
...
static inline int i40e_flow_fdir_fill_eth_ip_head(struct i40e_pf *pf, const struct i40e_fdir_input *fdir_input,
    ...
    static const uint8_t next_proto[] = {
        [I40E_FILTER_PCTYPE_FRAG_IPV4] = IPPROTO_IP,        [I40E_FILTER_PCTYPE_NONF_IPV4_TCP] = IPPROTO_TCP,
        [I40E_FILTER_PCTYPE_NONF_IPV4_UDP] = IPPROTO_UDP,   [I40E_FILTER_PCTYPE_NONF_IPV4_SCTP] = IPPROTO_SCTP,
        [I40E_FILTER_PCTYPE_NONF_IPV4_OTHER] = IPPROTO_IP,  [I40E_FILTER_PCTYPE_FRAG_IPV6] = IPPROTO_NONE,
        [I40E_FILTER_PCTYPE_NONF_IPV6_TCP] = IPPROTO_TCP,   [I40E_FILTER_PCTYPE_NONF_IPV6_UDP] = IPPROTO_UDP,
        [I40E_FILTER_PCTYPE_NONF_IPV6_SCTP] = IPPROTO_SCTP, [I40E_FILTER_PCTYPE_NONF_IPV6_OTHER] = IPPROTO_NONE,
    };
    ...
    if (pctype == I40E_FILTER_PCTYPE_L2_PAYLOAD)
        *ether_type = fdir_input->flow.l2_flow.ether_type;
             ...
             pctype == I40E_FILTER_PCTYPE_NONF_IPV4_SCTP || pctype == I40E_FILTER_PCTYPE_NONF_IPV4_OTHER ||
             pctype == I40E_FILTER_PCTYPE_FRAG_IPV4 || is_customized_pctype) {
        ip = (struct ipv4_hdr *)raw_pkt;
        ...
        *ether_type = rte_cpu_to_be_16(ETHER_TYPE_IPv4);
        ...
        if (!is_customized_pctype)
            ip->next_proto_id =
                fdir_input->flow.ip4_flow.proto ? fdir_input->flow.ip4_flow.proto : next_proto[fdir_input->pctype];
        else if (cus_pctype->index == I40E_CUSTOMIZED_GTPC || cus_pctype->index == I40E_CUSTOMIZED_GTPU_IPV4 ||
                 cus_pctype->index == I40E_CUSTOMIZED_GTPU_IPV6 || cus_pctype->index == I40E_CUSTOMIZED_GTPU)
            ip->next_proto_id = IPPROTO_UDP;
        len += sizeof(struct ipv4_hdr);
    } else if (pctype == I40E_FILTER_PCTYPE_NONF_IPV6_TCP || pctype == I40E_FILTER_PCTYPE_NONF_IPV6_UDP ||
        ...
        *ether_type = rte_cpu_to_be_16(ETHER_TYPE_IPv6);
        ...
        ip6->payload_len = rte_cpu_to_be_16(I40E_FDIR_IPv6_PAYLOAD_LEN);
        ip6->proto =
            fdir_input->flow.ipv6_flow.proto ? fdir_input->flow.ipv6_flow.proto : next_proto[fdir_input->pctype];
        ip6->hop_limits = fdir_input->flow.ipv6_flow.hop_limits ? fdir_input->flow.ipv6_flow.hop_limits
                                                                : I40E_FDIR_IPv6_DEFAULT_HOP_LIMITS;
        /**
         * The source and destination fields in the transmitted packet
 ...
 * @raw_pkt: a packet to be constructed
 */
static int i40e_flow_fdir_construct_pkt(struct i40e_pf *pf, const struct i40e_fdir_input *fdir_input,
                                        unsigned char *raw_pkt) {
    ...
    if (fdir_input->flow_ext.pkt_template) {
        memcpy(raw_pkt, fdir_input->flow.raw_flow.packet, fdir_input->flow.raw_flow.length);
        return 0;
    }

    /* fill the ethernet and IP head */
    len = i40e_flow_fdir_fill_eth_ip_head(pf, fdir_input, raw_pkt, !!fdir_input->flow_ext.vlan_tci);
    if (len < 0)
        return -EINVAL;
    ...
    if (pctype == I40E_FILTER_PCTYPE_NONF_IPV4_UDP) {
        ...
        sctp->dst_port = fdir_input->flow.sctp4_flow.src_port;
        sctp->tag = fdir_input->flow.sctp4_flow.verify_tag;
    } else if (pctype == I40E_FILTER_PCTYPE_NONF_IPV4_OTHER || pctype == I40E_FILTER_PCTYPE_FRAG_IPV4) {
        payload = raw_pkt + len;
        set_idx = I40E_FLXPLD_L3_IDX;
        ...
        sctp->dst_port = fdir_input->flow.sctp6_flow.src_port;
        sctp->tag = fdir_input->flow.sctp6_flow.verify_tag;
    } else if (pctype == I40E_FILTER_PCTYPE_NONF_IPV6_OTHER || pctype == I40E_FILTER_PCTYPE_FRAG_IPV6) {
        payload = raw_pkt + len;
        set_idx = I40E_FLXPLD_L3_IDX;
        ...
        if (cus_pctype->index == I40E_CUSTOMIZED_GTPC || cus_pctype->index == I40E_CUSTOMIZED_GTPU_IPV4 ||
            ...
            gtp = (struct rte_flow_item_gtp *)((unsigned char *)udp + sizeof(struct udp_hdr));
            ...
            if (cus_pctype->index == I40E_CUSTOMIZED_GTPU_IPV4) {
                ...
                gtp_ipv4 = (struct ipv4_hdr *)((unsigned char *)gtp + sizeof(struct rte_flow_item_gtp));
                gtp_ipv4->version_ihl = I40E_FDIR_IP_DEFAULT_VERSION_IHL;
                gtp_ipv4->next_proto_id = IPPROTO_IP;
                gtp_ipv4->total_length = rte_cpu_to_be_16(I40E_FDIR_INNER_IP_DEFAULT_LEN);
                payload = (unsigned char *)gtp_ipv4 + sizeof(struct ipv4_hdr);
            } else if (cus_pctype->index == I40E_CUSTOMIZED_GTPU_IPV6) {
                gtp->msg_type = I40E_FDIR_GTP_MSG_TYPE_0XFF;
                gtp_ipv6 = (struct ipv6_hdr *)((unsigned char *)gtp + sizeof(struct rte_flow_item_gtp));
                gtp_ipv6->vtc_flow =
                    rte_cpu_to_be_32(I40E_FDIR_IPv6_DEFAULT_VTC_FLOW | (0 << I40E_FDIR_IPv6_TC_OFFSET));
                gtp_ipv6->proto = IPPROTO_NONE;
                gtp_ipv6->payload_len = rte_cpu_to_be_16(I40E_FDIR_INNER_IPV6_DEFAULT_LEN);
                gtp_ipv6->hop_limits = I40E_FDIR_IPv6_DEFAULT_HOP_LIMITS;
                payload = (unsigned char *)gtp_ipv6 + sizeof(struct ipv6_hdr);
            } else
    ...
    for (i = 0; i < I40E_MAX_FLXPLD_FIED; i++) {
        ...
        dst = pf->fdir.flex_set[pit_idx].dst_offset * sizeof(uint16_t);
        ptr = payload + pf->fdir.flex_set[pit_idx].src_offset * sizeof(uint16_t);
        (void)rte_memcpy(ptr, &fdir_input->flow_ext.flexbytes[dst], size * sizeof(uint16_t));
    }
 ...
 * tx queue
 */
static inline int i40e_check_fdir_programming_status(struct i40e_rx_queue *rxq) {
    ...
    if (rx_status & (1 << I40E_RX_DESC_STATUS_DD_SHIFT)) {
        ...
        id = (qword1 & I40E_RX_PROG_STATUS_DESC_QW1_PROGID_MASK) >> I40E_RX_PROG_STATUS_DESC_QW1_PROGID_SHIFT;

        if (len == I40E_RX_PROG_STATUS_DESC_LENGTH && id == I40E_RX_PROG_STATUS_DESC_FD_FILTER_STATUS) {
            error = (qword1 & I40E_RX_PROG_STATUS_DESC_QW1_ERROR_MASK) >> I40E_RX_PROG_STATUS_DESC_QW1_ERROR_SHIFT;
...
static int i40e_fdir_filter_convert(const struct i40e_fdir_filter_conf *input, struct i40e_fdir_filter *filter) {
    ...
    if (input->input.flow_ext.pkt_template) {
        ...
        filter->fdir.input.flow.raw_flow.length = rte_hash_crc(
            input->input.flow.raw_flow.packet, input->input.flow.raw_flow.length, input->input.flow.raw_flow.pctype);
    }
    return 0;
}
...
static struct i40e_fdir_filter *i40e_sw_fdir_filter_lookup(struct i40e_fdir_info *fdir_info,
    ...
    if (input->flow_ext.pkt_template)
        ret = rte_hash_lookup_with_hash(fdir_info->hash_table, (const void *)input, input->flow.raw_flow.length);
    else
...
int i40e_sw_fdir_filter_del(struct i40e_pf *pf, struct i40e_fdir_input *input) {
    ...
    if (input->flow_ext.pkt_template)
        ret = rte_hash_del_key_with_hash(fdir_info->hash_table, input, input->flow.raw_flow.length);
    else
...
int i40e_flow_add_del_fdir_filter(struct rte_eth_dev *dev, const struct i40e_fdir_filter_conf *filter, bool add) {
    ...
    if (filter->input.flow_ext.pkt_template) {
        if (filter->input.flow.raw_flow.length > I40E_FDIR_PKT_LEN || !filter->input.flow.raw_flow.packet) {
    ...
    */
    ...
    if (!add && !node) {
        PMD_DRV_LOG(ERR, "There's no corresponding flow firector filter!");
        return -EINVAL;
    }
...
static int i40e_fdir_filter_programming(struct i40e_pf *pf, enum i40e_filter_pctype pctype,
    ...
    fdirdp = (volatile struct i40e_filter_program_desc *)(&(txq->tx_ring[txq->tx_tail]));

    fdirdp->qindex_flex_ptype_vsi =
        rte_cpu_to_le_32((fdir_action->rx_queue << I40E_TXD_FLTR_QW0_QINDEX_SHIFT) & I40E_TXD_FLTR_QW0_QINDEX_MASK);

    fdirdp->qindex_flex_ptype_vsi |=
        rte_cpu_to_le_32((fdir_action->flex_off << I40E_TXD_FLTR_QW0_FLEXOFF_SHIFT) & I40E_TXD_FLTR_QW0_FLEXOFF_MASK);

    fdirdp->qindex_flex_ptype_vsi |=
        rte_cpu_to_le_32((pctype << I40E_TXD_FLTR_QW0_PCTYPE_SHIFT) & I40E_TXD_FLTR_QW0_PCTYPE_MASK);
    ...
    fdirdp->qindex_flex_ptype_vsi |=
        rte_cpu_to_le_32(((uint32_t)vsi_id << I40E_TXD_FLTR_QW0_DEST_VSI_SHIFT) & I40E_TXD_FLTR_QW0_DEST_VSI_MASK);

    fdirdp->dtype_cmd_cntindex = rte_cpu_to_le_32(I40E_TX_DESC_DTYPE_FILTER_PROG);

    if (add)
        fdirdp->dtype_cmd_cntindex |=
            rte_cpu_to_le_32(I40E_FILTER_PROGRAM_DESC_PCMD_ADD_UPDATE << I40E_TXD_FLTR_QW1_PCMD_SHIFT);
    else
        fdirdp->dtype_cmd_cntindex |=
            rte_cpu_to_le_32(I40E_FILTER_PROGRAM_DESC_PCMD_REMOVE << I40E_TXD_FLTR_QW1_PCMD_SHIFT);
    ...
    fdirdp->dtype_cmd_cntindex |= rte_cpu_to_le_32((fdir_action->report_status << I40E_TXD_FLTR_QW1_FD_STATUS_SHIFT) &
                                                   I40E_TXD_FLTR_QW1_FD_STATUS_MASK);

    fdirdp->dtype_cmd_cntindex |= rte_cpu_to_le_32(I40E_TXD_FLTR_QW1_CNT_ENA_MASK);
    fdirdp->dtype_cmd_cntindex |=
#ifdef TREX_PATCH
    ...
    for (i = 0; i < I40E_FDIR_MAX_WAIT_US; i++) {
        if ((txdp->cmd_type_offset_bsz & rte_cpu_to_le_64(I40E_TXD_QW1_DTYPE_MASK)) ==
    ...
    PMD_DRV_LOG(ERR, "Failed to program FDIR filter: programming status reported.");
    return -ETIMEDOUT;
}
...
static int i40e_flow_fdir_filter_programming(struct i40e_pf *pf, enum i40e_filter_pctype pctype,
    ...
    fdirdp->dtype_cmd_cntindex |= rte_cpu_to_le_32(
        ((uint32_t)pf->fdir.match_counter_index << I40E_TXD_FLTR_QW1_CNTINDEX_SHIFT) & I40E_TXD_FLTR_QW1_CNTINDEX_MASK);
    ...
    if (i >= I40E_FDIR_MAX_WAIT_US) {
        PMD_DRV_LOG(ERR, "Failed to program FDIR filter: time out to get DD on tx queue.");
        return -ETIMEDOUT;
    }
    /* totally delay 10 ms to check programming status*/
...
int i40e_fdir_flush(struct rte_eth_dev *dev) {
    ...
    guarant_cnt = (uint16_t)((I40E_READ_REG(hw, I40E_PFQF_FDSTAT) & I40E_PFQF_FDSTAT_GUARANT_CNT_MASK) >>
                             I40E_PFQF_FDSTAT_GUARANT_CNT_SHIFT);
    best_cnt = (uint16_t)((I40E_READ_REG(hw, I40E_PFQF_FDSTAT) & I40E_PFQF_FDSTAT_BEST_CNT_MASK) >>
                          I40E_PFQF_FDSTAT_BEST_CNT_SHIFT);
...
static inline void i40e_fdir_info_get_flex_set(struct i40e_pf *pf, struct rte_eth_flex_payload_cfg *flex_set,
                                               uint16_t *num) {
    ...
    for (layer_idx = I40E_FLXPLD_L2_IDX; layer_idx <= I40E_FLXPLD_L4_IDX; layer_idx++) {
...
static inline void i40e_fdir_info_get_flex_mask(struct i40e_pf *pf, struct rte_eth_fdir_flex_mask *flex_mask,
                                                uint16_t *num) {
    ...
    for (i = I40E_FILTER_PCTYPE_NONF_IPV4_UDP; i <= I40E_FILTER_PCTYPE_L2_PAYLOAD; i++) {
        mask = &pf->fdir.flex_mask[i];
        flow_type = i40e_pctype_to_flowtype(pf->adapter, (enum i40e_filter_pctype)i);
        if (flow_type == RTE_ETH_FLOW_UNKNOWN)
            continue;
 ...
 */
static void i40e_fdir_info_get(struct rte_eth_dev *dev, struct rte_eth_fdir_info *fdir) {
    struct i40e_pf *pf = I40E_DEV_PRIVATE_TO_PF(dev->data->dev_private);
    struct i40e_hw *hw = I40E_PF_TO_HW(pf);
 ...
 */
static void i40e_fdir_stats_get(struct rte_eth_dev *dev, struct rte_eth_fdir_stats *stat) {
    struct i40e_pf *pf = I40E_DEV_PRIVATE_TO_PF(dev->data->dev_private);
    struct i40e_hw *hw = I40E_PF_TO_HW(pf);
    uint32_t fdstat;

    fdstat = I40E_READ_REG(hw, I40E_PFQF_FDSTAT);
    stat->guarant_cnt = (uint32_t)((fdstat & I40E_PFQF_FDSTAT_GUARANT_CNT_MASK) >> I40E_PFQF_FDSTAT_GUARANT_CNT_SHIFT);
    stat->best_cnt = (uint32_t)((fdstat & I40E_PFQF_FDSTAT_BEST_CNT_MASK) >> I40E_PFQF_FDSTAT_BEST_CNT_SHIFT);
}

static int i40e_fdir_filter_set(struct rte_eth_dev *dev, struct rte_eth_fdir_filter_info *info) {
    struct i40e_pf *pf = I40E_DEV_PRIVATE_TO_PF(dev->data->dev_private);
    int ret = 0;
    ...
    case RTE_ETH_FDIR_FILTER_INPUT_SET_SELECT:
        ...
        break;
    default:
        PMD_DRV_LOG(ERR, "FD filter info type (%d) not supported", info->info_type);
        return -EINVAL;
    }
...
int i40e_fdir_ctrl_func(struct rte_eth_dev *dev, enum rte_filter_op filter_op, void *arg) {
    ...
    case RTE_ETH_FILTER_ADD:
        ret = i40e_add_del_fdir_filter(dev, (struct rte_eth_fdir_filter *)arg, TRUE);
        break;
    case RTE_ETH_FILTER_DELETE:
        ret = i40e_add_del_fdir_filter(dev, (struct rte_eth_fdir_filter *)arg, FALSE);
        break;
    ...
    case RTE_ETH_FILTER_SET:
        ret = i40e_fdir_filter_set(dev, (struct rte_eth_fdir_filter_info *)arg);
        break;
...
void i40e_fdir_filter_restore(struct i40e_pf *pf) {
    ...
    fdstat = I40E_READ_REG(hw, I40E_PFQF_FDSTAT);
    guarant_cnt = (uint32_t)((fdstat & I40E_PFQF_FDSTAT_GUARANT_CNT_MASK) >> I40E_PFQF_FDSTAT_GUARANT_CNT_SHIFT);
    best_cnt = (uint32_t)((fdstat & I40E_PFQF_FDSTAT_BEST_CNT_MASK) >> I40E_PFQF_FDSTAT_BEST_CNT_SHIFT);
