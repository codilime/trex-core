...
static int i40e_pf_host_switch_queues(struct i40e_pf_vf *vf, struct virtchnl_queue_select *qsel, bool on);
...
static int i40e_pf_vf_queues_mapping(struct i40e_pf_vf *vf) {
     ...
     */
    i40e_write_rx_ctl(hw, I40E_VSILAN_QBASE(vsi_id), I40E_VSILAN_QBASE_VSIQTABLE_ENA_MASK);
...
#define I40E_VF_PEND_MASK 0x20
    ...
    I40E_WRITE_REG(hw, I40E_PF_PCI_CIAA, I40E_VF_PCI_ADDR | (abs_vf_id << I40E_PF_PCI_CIAA_VF_NUM_SHIFT));
    for (i = 0; i < VFRESET_MAX_WAIT_CNT; i++) {
        rte_delay_us(1);
    ...
    if (pf->floating_veb && pf->floating_veb_list[vf_id]) {
        ...
        vf->vsi = i40e_vsi_setup(vf->pf, I40E_VSI_SRIOV, vf->pf->main_vsi, vf->vf_idx);
    }
...
int i40e_pf_host_send_msg_to_vf(struct i40e_pf_vf *vf, uint32_t opcode, uint32_t retval, uint8_t *msg,
                                uint16_t msglen) {
    struct i40e_hw *hw = I40E_PF_TO_HW(vf->pf);
...
static void i40e_pf_host_process_cmd_version(struct i40e_pf_vf *vf, uint8_t *msg, bool b_op) {
    ...
    if (b_op)
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_VERSION, I40E_SUCCESS, (uint8_t *)&info, sizeof(info));
    else
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_VERSION, I40E_NOT_SUPPORTED, (uint8_t *)&info, sizeof(info));
}
...
static int i40e_pf_host_process_cmd_get_vf_resource(struct i40e_pf_vf *vf, uint8_t *msg, bool b_op) {
    ...
    if (!b_op) {
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_GET_VF_RESOURCES, I40E_NOT_SUPPORTED, NULL, 0);
        return ret;
    }

    /* only have 1 VSI by default */
    len = sizeof(struct virtchnl_vf_resource) + I40E_DEFAULT_VF_VSI_NUM * sizeof(struct virtchnl_vsi_resource);
    ...
    if (vf->request_caps & VIRTCHNL_VF_OFFLOAD_RSS_PF) {
        ...
        I40E_WRITE_REG(hw, I40E_VFQF_HENA1(1, vf->vf_idx), (uint32_t)(default_hena >> 32));
        I40E_WRITE_FLUSH(hw);
    }
    ...
    ether_addr_copy(&vf->mac_addr, (struct ether_addr *)vf_res->vsi_res[0].default_mac_addr);

send_msg:
    i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_GET_VF_RESOURCES, ret, (uint8_t *)vf_res, len);
    rte_free(vf_res);

    return ret;
}

static int i40e_pf_host_hmc_config_rxq(struct i40e_hw *hw, struct i40e_pf_vf *vf, struct virtchnl_rxq_info *rxq,
                                       uint8_t crcstrip) {
...
static inline uint8_t i40e_vsi_get_tc_of_queue(struct i40e_vsi *vsi, uint16_t queue_id) {
    ...
    for (i = 0; i < I40E_MAX_TRAFFIC_CLASS; i++) {
        if (vsi->enabled_tc & (1 << i)) {
            qp_idx = rte_le_to_cpu_16((info->tc_mapping[i] & I40E_AQ_VSI_TC_QUE_OFFSET_MASK) >>
                                      I40E_AQ_VSI_TC_QUE_OFFSET_SHIFT);
            bsf = rte_le_to_cpu_16((info->tc_mapping[i] & I40E_AQ_VSI_TC_QUE_NUMBER_MASK) >>
...
static int i40e_pf_host_hmc_config_txq(struct i40e_hw *hw, struct i40e_pf_vf *vf, struct virtchnl_txq_info *txq) {
    int err = I40E_SUCCESS;
    ...
    qtx_ctl = (I40E_QTX_CTL_VF_QUEUE << I40E_QTX_CTL_PFVF_Q_SHIFT) |
              ...
              (((vf->vf_idx + hw->func_caps.vf_base_id) << I40E_QTX_CTL_VFVM_INDX_SHIFT) & I40E_QTX_CTL_VFVM_INDX_MASK);
...
static int i40e_pf_host_process_cmd_config_vsi_queues(struct i40e_pf_vf *vf, uint8_t *msg, uint16_t msglen, bool b_op) {
    struct i40e_hw *hw = I40E_PF_TO_HW(vf->pf);
    struct i40e_vsi *vsi = vf->vsi;
    struct virtchnl_vsi_queue_config_info *vc_vqci = (struct virtchnl_vsi_queue_config_info *)msg;
    struct virtchnl_queue_pair_info *vc_qpi;
    int i, ret = I40E_SUCCESS;

    if (!b_op) {
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_CONFIG_VSI_QUEUES, I40E_NOT_SUPPORTED, NULL, 0);
        return ret;
    }

    if (!msg || vc_vqci->num_queue_pairs > vsi->nb_qps || vc_vqci->num_queue_pairs > I40E_MAX_VSI_QP ||
        msglen < I40E_VIRTCHNL_CONFIG_VSI_QUEUES_SIZE(vc_vqci, vc_vqci->num_queue_pairs)) {
    ...
    for (i = 0; i < vc_vqci->num_queue_pairs; i++) {
        if (vc_qpi[i].rxq.queue_id > vsi->nb_qps - 1 || vc_qpi[i].txq.queue_id > vsi->nb_qps - 1) {
            ret = I40E_ERR_PARAM;
            goto send_msg;
        }
         ...
         */
        if (i40e_pf_host_hmc_config_rxq(hw, vf, &vc_qpi[i].rxq, I40E_CFG_CRCSTRIP_DEFAULT) != I40E_SUCCESS) {
...
static void i40e_pf_config_irq_link_list(struct i40e_pf_vf *vf, struct virtchnl_vector_map *vvm) {
#define BITS_PER_CHAR 8
    ...
    if (!vector_id)
        ...
        reg_idx = I40E_VPINT_LNKLSTN(((hw->func_caps.num_msix_vectors_vf - 1) * vf->vf_idx) + (vector_id - 1));
        ...
        if (tempmap & 0x1) {
            ...
            if (b_first_q) {
                ...
                reg = (vector_id) | (qtype << I40E_QINT_RQCTL_NEXTQ_TYPE_SHIFT) |
                      (qid << I40E_QINT_RQCTL_NEXTQ_INDX_SHIFT) | BIT(I40E_QINT_RQCTL_CAUSE_ENA_SHIFT) |
    ...
    reg = (vector_id) | (0 << I40E_QINT_RQCTL_NEXTQ_TYPE_SHIFT) | (0x7FF << I40E_QINT_RQCTL_NEXTQ_INDX_SHIFT) |
          BIT(I40E_QINT_RQCTL_CAUSE_ENA_SHIFT) | (itr_idx << I40E_QINT_RQCTL_ITR_INDX_SHIFT);
    I40E_WRITE_REG(hw, reg_idx, reg);
...
static int i40e_pf_host_process_cmd_config_irq_map(struct i40e_pf_vf *vf, uint8_t *msg, uint16_t msglen, bool b_op) {
    int ret = I40E_SUCCESS;
    ...
    if (!b_op) {
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_CONFIG_IRQ_MAP, I40E_NOT_SUPPORTED, NULL, 0);
        return ret;
    }
...
static int i40e_pf_host_process_cmd_enable_queues(struct i40e_pf_vf *vf, uint8_t *msg, uint16_t msglen) {
    int ret = I40E_SUCCESS;
...
static int i40e_pf_host_process_cmd_disable_queues(struct i40e_pf_vf *vf, uint8_t *msg, uint16_t msglen, bool b_op) {
    int ret = I40E_SUCCESS;
    struct virtchnl_queue_select *q_sel = (struct virtchnl_queue_select *)msg;

    if (!b_op) {
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_DISABLE_QUEUES, I40E_NOT_SUPPORTED, NULL, 0);
        return ret;
    }
...
static int i40e_pf_host_process_cmd_add_ether_address(struct i40e_pf_vf *vf, uint8_t *msg, uint16_t msglen, bool b_op) {
    int ret = I40E_SUCCESS;
    struct virtchnl_ether_addr_list *addr_list = (struct virtchnl_ether_addr_list *)msg;
    struct i40e_mac_filter_info filter;
    int i;
    struct ether_addr *mac;

    if (!b_op) {
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_ADD_ETH_ADDR, I40E_NOT_SUPPORTED, NULL, 0);
        return ret;
    }
    ...
    for (i = 0; i < addr_list->num_elements; i++) {
        ...
        if (is_zero_ether_addr(mac) || i40e_vsi_add_mac(vf->vsi, &filter)) {
...
static int i40e_pf_host_process_cmd_del_ether_address(struct i40e_pf_vf *vf, uint8_t *msg, uint16_t msglen, bool b_op) {
    int ret = I40E_SUCCESS;
    struct virtchnl_ether_addr_list *addr_list = (struct virtchnl_ether_addr_list *)msg;
    int i;
    struct ether_addr *mac;
    ...
    if (!b_op) {
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_DEL_ETH_ADDR, I40E_NOT_SUPPORTED, NULL, 0);
        return ret;
    }
    ...
    for (i = 0; i < addr_list->num_elements; i++) {
        mac = (struct ether_addr *)(addr_list->list[i].addr);
        if (is_zero_ether_addr(mac) || i40e_vsi_delete_mac(vf->vsi, mac)) {
            ret = I40E_ERR_INVALID_MAC_ADDR;
            goto send_msg;
        }
    }
...
static int i40e_pf_host_process_cmd_add_vlan(struct i40e_pf_vf *vf, uint8_t *msg, uint16_t msglen, bool b_op) {
    int ret = I40E_SUCCESS;
    struct virtchnl_vlan_filter_list *vlan_filter_list = (struct virtchnl_vlan_filter_list *)msg;
    int i;
    uint16_t *vid;

    if (!b_op) {
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_ADD_VLAN, I40E_NOT_SUPPORTED, NULL, 0);
        return ret;
    }
...
static int i40e_pf_host_process_cmd_del_vlan(struct i40e_pf_vf *vf, uint8_t *msg, uint16_t msglen, bool b_op) {
    int ret = I40E_SUCCESS;
    struct virtchnl_vlan_filter_list *vlan_filter_list = (struct virtchnl_vlan_filter_list *)msg;
    int i;
    uint16_t *vid;
    ...
    if (!b_op) {
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_DEL_VLAN, I40E_NOT_SUPPORTED, NULL, 0);
        return ret;
    }
...
static int i40e_pf_host_process_cmd_config_promisc_mode(struct i40e_pf_vf *vf, uint8_t *msg, uint16_t msglen,
    ...
    if (!b_op) {
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_CONFIG_PROMISCUOUS_MODE, I40E_NOT_SUPPORTED, NULL, 0);
        return ret;
    }
    ...
    if (promisc->flags & FLAG_VF_UNICAST_PROMISC)
        unicast = TRUE;
    ret = i40e_aq_set_vsi_unicast_promiscuous(hw, vf->vsi->seid, unicast, NULL, true);
    if (ret != I40E_SUCCESS)
        goto send_msg;

    if (promisc->flags & FLAG_VF_MULTICAST_PROMISC)
        multicast = TRUE;
    ret = i40e_aq_set_vsi_multicast_promiscuous(hw, vf->vsi->seid, multicast, NULL);

send_msg:
    i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_CONFIG_PROMISCUOUS_MODE, ret, NULL, 0);

    return ret;
}

static int i40e_pf_host_process_cmd_get_stats(struct i40e_pf_vf *vf, bool b_op) {
    i40e_update_vsi_stats(vf->vsi);

    if (b_op)
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_GET_STATS, I40E_SUCCESS, (uint8_t *)&vf->vsi->eth_stats,
                                    sizeof(vf->vsi->eth_stats));
    else
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_GET_STATS, I40E_NOT_SUPPORTED, (uint8_t *)&vf->vsi->eth_stats,
                                    sizeof(vf->vsi->eth_stats));

    return I40E_SUCCESS;
}

static int i40e_pf_host_process_cmd_enable_vlan_strip(struct i40e_pf_vf *vf, bool b_op) {
    int ret = I40E_SUCCESS;

    if (!b_op) {
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_ENABLE_VLAN_STRIPPING, I40E_NOT_SUPPORTED, NULL, 0);
        return ret;
    }
    ...
    i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_ENABLE_VLAN_STRIPPING, ret, NULL, 0);

    return ret;
}

static int i40e_pf_host_process_cmd_disable_vlan_strip(struct i40e_pf_vf *vf, bool b_op) {
    int ret = I40E_SUCCESS;

    if (!b_op) {
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_DISABLE_VLAN_STRIPPING, I40E_NOT_SUPPORTED, NULL, 0);
        return ret;
    }
    ...
    i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_DISABLE_VLAN_STRIPPING, ret, NULL, 0);

    return ret;
}

static int i40e_pf_host_process_cmd_set_rss_lut(struct i40e_pf_vf *vf, uint8_t *msg, uint16_t msglen, bool b_op) {
    struct virtchnl_rss_lut *rss_lut = (struct virtchnl_rss_lut *)msg;
    uint16_t valid_len;
    int ret = I40E_SUCCESS;

    if (!b_op) {
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_CONFIG_RSS_LUT, I40E_NOT_SUPPORTED, NULL, 0);
        return ret;
    }
...
static int i40e_pf_host_process_cmd_set_rss_key(struct i40e_pf_vf *vf, uint8_t *msg, uint16_t msglen, bool b_op) {
    struct virtchnl_rss_key *rss_key = (struct virtchnl_rss_key *)msg;
    uint16_t valid_len;
    int ret = I40E_SUCCESS;

    if (!b_op) {
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_DEL_VLAN, VIRTCHNL_OP_CONFIG_RSS_KEY, NULL, 0);
        return ret;
    }
...
void i40e_notify_vf_link_status(struct rte_eth_dev *dev, struct i40e_pf_vf *vf) {
    ...
    if (tval & I40E_VF_ATQLEN_ATQLEN_MASK || tval & I40E_VF_ATQLEN_ATQENABLE_MASK ||
        rval & I40E_VF_ARQLEN_ARQLEN_MASK || rval & I40E_VF_ARQLEN_ARQENABLE_MASK)
        i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_EVENT, I40E_SUCCESS, (uint8_t *)&event, sizeof(event));
}
...
static void i40e_vc_notify_vf_reset(struct i40e_pf_vf *vf) {
    ...
    i40e_aq_send_msg_to_vf(hw, abs_vf_id, VIRTCHNL_OP_EVENT, 0, (u8 *)&pfe, sizeof(struct virtchnl_pf_event), NULL);
}

static int i40e_pf_host_process_cmd_request_queues(struct i40e_pf_vf *vf, uint8_t *msg) {
    ...
    if (req_pairs == 0) {
        ...
        PMD_DRV_LOG(ERR,
                    ...
                    vf->vf_idx, vfres->num_queue_pairs, req_pairs, cur_pairs + pf->qp_pool.num_free);
    ...
    return i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_REQUEST_QUEUES, 0, (u8 *)vfres, sizeof(*vfres));
}

void i40e_pf_host_handle_vf_msg(struct rte_eth_dev *dev, uint16_t abs_vf_id, uint32_t opcode,
                                __rte_unused uint32_t retval, uint8_t *msg, uint16_t msglen) {
    ...
    if (ret) {
        PMD_DRV_LOG(ERR, "Invalid message from VF %u, opcode %u, len %u", vf_id, opcode, msglen);
    ...
    case VIRTCHNL_OP_ENABLE_QUEUES:
        ...
        if (b_op) {
            ...
            i40e_pf_host_send_msg_to_vf(vf, VIRTCHNL_OP_ENABLE_QUEUES, I40E_NOT_SUPPORTED, NULL, 0);
        }
        break;
