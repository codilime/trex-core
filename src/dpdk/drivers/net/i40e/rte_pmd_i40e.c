...
int rte_pmd_i40e_set_vf_mac_anti_spoof(uint16_t port, uint16_t vf_id, uint8_t on) {
    ...
    if (vsi->info.valid_sections & rte_cpu_to_le_16(I40E_AQ_VSI_PROP_SECURITY_VALID)) {
        if (on) {
            if ((vsi->info.sec_flags & I40E_AQ_VSI_SEC_FLAG_ENABLE_MAC_CHK) == I40E_AQ_VSI_SEC_FLAG_ENABLE_MAC_CHK)
                return 0; /* already on */
        } else {
...
static int i40e_vsi_rm_mac_filter(struct i40e_vsi *vsi) {
    ...
    TAILQ_FOREACH_SAFE(f, &vsi->mac_list, next, temp) {
        ...
        if (filter_type == RTE_MACVLAN_PERFECT_MATCH || filter_type == RTE_MACVLAN_HASH_MATCH) {
            ret = i40e_find_all_vlan_for_mac(vsi, mv_f, vlan_num, &f->mac_info.mac_addr);
            if (ret != I40E_SUCCESS) {
                rte_free(mv_f);
                return ret;
            }
...
static int i40e_vsi_restore_mac_filter(struct i40e_vsi *vsi) {
    ...
    TAILQ_FOREACH_SAFE(f, &vsi->mac_list, next, temp) {
        ...
        if (f->mac_info.filter_type == RTE_MACVLAN_PERFECT_MATCH || f->mac_info.filter_type == RTE_MACVLAN_HASH_MATCH) {
            ret = i40e_find_all_vlan_for_mac(vsi, mv_f, vlan_num, &f->mac_info.mac_addr);
            if (ret != I40E_SUCCESS) {
                rte_free(mv_f);
                return ret;
...
static int i40e_vsi_set_tx_loopback(struct i40e_vsi *vsi, uint8_t on) {
    ...
    /* Check if it has been already on or off */
    if (vsi->info.valid_sections & rte_cpu_to_le_16(I40E_AQ_VSI_PROP_SWITCH_VALID)) {
        if (on) {
            if ((vsi->info.switch_id & I40E_AQ_VSI_SW_ID_FLAG_ALLOW_LB) == I40E_AQ_VSI_SW_ID_FLAG_ALLOW_LB)
                return 0; /* already on */
        } else {
...
int rte_pmd_i40e_remove_vf_mac_addr(uint16_t port, uint16_t vf_id, struct ether_addr *mac_addr) {
    struct rte_eth_dev *dev;
    struct i40e_pf_vf *vf;
    struct i40e_vsi *vsi;
    struct i40e_pf *pf;
...
int rte_pmd_i40e_get_vf_stats(uint16_t port, uint16_t vf_id, struct rte_eth_stats *stats) {
    ...
    stats->opackets = vsi->eth_stats.tx_unicast + vsi->eth_stats.tx_multicast + vsi->eth_stats.tx_broadcast;
...
int rte_pmd_i40e_set_vf_max_bw(uint16_t port, uint16_t vf_id, uint32_t bw) {
    ...
    if (bw > I40E_QOS_BW_MAX) {
        PMD_DRV_LOG(ERR, "Bandwidth should not be larger than %dMbps.", I40E_QOS_BW_MAX);
        return -EINVAL;
    }

    if (bw % I40E_QOS_BW_GRANULARITY) {
        PMD_DRV_LOG(ERR, "Bandwidth should be the multiple of %dMbps.", I40E_QOS_BW_GRANULARITY);
        return -EINVAL;
    }
...
int rte_pmd_i40e_set_vf_tc_bw_alloc(uint16_t port, uint16_t vf_id, uint8_t tc_num, uint8_t *bw_weight) {
    ...
    if (ret) {
        PMD_DRV_LOG(ERR, "Failed to set VF %d TC bandwidth weight, err(%d).", vf_id, ret);
        return -EINVAL;
    }
...
int rte_pmd_i40e_set_vf_tc_max_bw(uint16_t port, uint16_t vf_id, uint8_t tc_no, uint32_t bw) {
    ...
    if (ret) {
        PMD_DRV_LOG(ERR, "Failed to set VF %d TC %d max bandwidth, err(%d).", vf_id, tc_no, ret);
        return -EINVAL;
    }
...
int rte_pmd_i40e_set_tc_strict_prio(uint16_t port, uint8_t tc_map) {
    ...
    if ((tc_map & veb->enabled_tc) != tc_map) {
        PMD_DRV_LOG(ERR, "TC bitmap isn't the subset of enabled TCs 0x%x.", veb->enabled_tc);
        return -EINVAL;
    }
    ...
    if (!veb->strict_prio_tc)
        ret = i40e_aq_config_switch_comp_ets(hw, veb->uplink_seid, &ets_data, i40e_aqc_opc_enable_switching_comp_ets,
                                             NULL);
    else if (tc_map)
        ret = i40e_aq_config_switch_comp_ets(hw, veb->uplink_seid, &ets_data, i40e_aqc_opc_modify_switching_comp_ets,
                                             NULL);
    else
        ret = i40e_aq_config_switch_comp_ets(hw, veb->uplink_seid, &ets_data, i40e_aqc_opc_disable_switching_comp_ets,
                                             NULL);
...
static void i40e_generate_profile_info_sec(char *name, struct i40e_ddp_version *version, uint32_t track_id,
                                           uint8_t *profile_info_sec, bool add) {
...
static enum i40e_status_code i40e_add_rm_profile_info(struct i40e_hw *hw, uint8_t *profile_info_sec) {
    ...
    sec = (struct i40e_profile_section_header *)profile_info_sec;
    track_id = ((struct i40e_profile_info *)(profile_info_sec + sec->section.offset))->track_id;
...
static int i40e_check_profile_info(uint16_t port, uint8_t *profile_info_sec) {
    ...
    pinfo = (struct rte_pmd_i40e_profile_info *)(profile_info_sec + sizeof(struct i40e_profile_section_header));
    if (pinfo->track_id == 0) {
        PMD_DRV_LOG(INFO, "Read-only profile.");
        return 0;
    }
    buff = rte_zmalloc("pinfo_list", (I40E_PROFILE_INFO_SIZE * I40E_MAX_PROFILE_NUM + 4), 0);
    if (!buff) {
        PMD_DRV_LOG(ERR, "failed to allocate memory");
        return -1;
    }

    ret = i40e_aq_get_ddp_list(hw, (void *)buff, (I40E_PROFILE_INFO_SIZE * I40E_MAX_PROFILE_NUM + 4), 0, NULL);
    if (ret) {
...
int rte_pmd_i40e_process_ddp_package(uint16_t port, uint8_t *buff, uint32_t size, enum rte_pmd_i40e_package_op op) {
    ...
    if (op != RTE_PMD_I40E_PKG_OP_WR_ADD && op != RTE_PMD_I40E_PKG_OP_WR_ONLY && op != RTE_PMD_I40E_PKG_OP_WR_DEL) {
    ...
    if (size < (sizeof(struct i40e_package_header) + sizeof(struct i40e_metadata_segment) + sizeof(uint32_t) * 2)) {
    ...
    metadata_seg_hdr = i40e_find_segment_in_package(SEGMENT_TYPE_METADATA, pkg_hdr);
    if (!metadata_seg_hdr) {
    ...
    profile_info_sec = rte_zmalloc("i40e_profile_info",
                                   sizeof(struct i40e_profile_section_header) + sizeof(struct i40e_profile_info), 0);
    if (!profile_info_sec) {
        PMD_DRV_LOG(ERR, "Failed to allocate memory");
        return -EINVAL;
    }
    ...
    i40e_generate_profile_info_sec(((struct i40e_profile_segment *)profile_seg_hdr)->name,
    ...
    if (op == RTE_PMD_I40E_PKG_OP_WR_DEL) {
        status = i40e_rollback_profile(hw, (struct i40e_profile_segment *)profile_seg_hdr, track_id);
        if (status) {
        ...
        status = i40e_write_profile(hw, (struct i40e_profile_segment *)profile_seg_hdr, track_id);
        if (status) {
            if (op == RTE_PMD_I40E_PKG_OP_WR_ADD)
...
static unsigned int i40e_get_tlv_section_size(struct i40e_profile_section_header *sec) {
...
int rte_pmd_i40e_get_ddp_info(uint8_t *pkg_buff, uint32_t pkg_size, uint8_t *info_buff, uint32_t info_size,
    ...
    if (!pkg_buff ||
        pkg_size < (sizeof(struct i40e_package_header) + sizeof(struct i40e_metadata_segment) + sizeof(uint32_t) * 2)) {
    ...
    if (type == RTE_PMD_I40E_PKG_INFO_GLOBAL_HEADER) {
        struct rte_pmd_i40e_profile_info *info = (struct rte_pmd_i40e_profile_info *)info_buff;
        ...
        info->track_id = ((struct i40e_metadata_segment *)metadata_seg_hdr)->track_id;

        memcpy(info->name, ((struct i40e_metadata_segment *)metadata_seg_hdr)->name, I40E_DDP_NAME_SIZE);
        memcpy(&info->version, &((struct i40e_metadata_segment *)metadata_seg_hdr)->version,
    ...
    if (type == RTE_PMD_I40E_PKG_INFO_HEADER) {
        ...
        memcpy(info->name, ((struct i40e_profile_segment *)i40e_seg_hdr)->name, I40E_DDP_NAME_SIZE);
        memcpy(&info->version, &((struct i40e_profile_segment *)i40e_seg_hdr)->version,
               sizeof(struct i40e_ddp_version));
        return I40E_SUCCESS;
    }
    ...
    if (type == RTE_PMD_I40E_PKG_INFO_DEVID_NUM) {
        ...
        *(uint32_t *)info_buff = ((struct i40e_profile_segment *)i40e_seg_hdr)->device_table_count;
        return I40E_SUCCESS;
    }
    ...
    if (type == RTE_PMD_I40E_PKG_INFO_DEVID_LIST) {
        ...
        memcpy(info_buff, ((struct i40e_profile_segment *)i40e_seg_hdr)->device_table,
    ...
    if (type == RTE_PMD_I40E_PKG_INFO_PROTOCOL_NUM) {
        ...
        proto = i40e_find_section_in_profile(SECTION_TYPE_PROTO, (struct i40e_profile_segment *)i40e_seg_hdr);
    ...
    if (type == RTE_PMD_I40E_PKG_INFO_PCTYPE_NUM) {
        ...
        pctype = i40e_find_section_in_profile(SECTION_TYPE_PCTYPE, (struct i40e_profile_segment *)i40e_seg_hdr);
    ...
    if (type == RTE_PMD_I40E_PKG_INFO_PCTYPE_LIST) {
        ...
        for (i = 0; i < nb_proto_info; i++)
            memset(&pinfo[i], RTE_PMD_I40E_PROTO_UNUSED, sizeof(struct rte_pmd_i40e_ptype_info));
        pctype = i40e_find_section_in_profile(SECTION_TYPE_PCTYPE, (struct i40e_profile_segment *)i40e_seg_hdr);
    ...
    if (type == RTE_PMD_I40E_PKG_INFO_PTYPE_NUM) {
        ...
        ptype = i40e_find_section_in_profile(SECTION_TYPE_PTYPE, (struct i40e_profile_segment *)i40e_seg_hdr);
...
static int check_invalid_pkt_type(uint32_t pkt_type) {
    ...
    if (l2 && l2 != RTE_PTYPE_L2_ETHER && l2 != RTE_PTYPE_L2_ETHER_TIMESYNC && l2 != RTE_PTYPE_L2_ETHER_ARP &&
        l2 != RTE_PTYPE_L2_ETHER_LLDP && l2 != RTE_PTYPE_L2_ETHER_NSH && l2 != RTE_PTYPE_L2_ETHER_VLAN &&
        l2 != RTE_PTYPE_L2_ETHER_QINQ && l2 != RTE_PTYPE_L2_ETHER_PPPOE)
        return -1;

    if (l3 && l3 != RTE_PTYPE_L3_IPV4 && l3 != RTE_PTYPE_L3_IPV4_EXT && l3 != RTE_PTYPE_L3_IPV6 &&
        l3 != RTE_PTYPE_L3_IPV4_EXT_UNKNOWN && l3 != RTE_PTYPE_L3_IPV6_EXT && l3 != RTE_PTYPE_L3_IPV6_EXT_UNKNOWN)
        return -1;

    if (l4 && l4 != RTE_PTYPE_L4_TCP && l4 != RTE_PTYPE_L4_UDP && l4 != RTE_PTYPE_L4_FRAG && l4 != RTE_PTYPE_L4_SCTP &&
        l4 != RTE_PTYPE_L4_ICMP && l4 != RTE_PTYPE_L4_NONFRAG)
        return -1;

    if (tnl && tnl != RTE_PTYPE_TUNNEL_IP && tnl != RTE_PTYPE_TUNNEL_GRENAT && tnl != RTE_PTYPE_TUNNEL_VXLAN &&
    ...
    if (il2 && il2 != RTE_PTYPE_INNER_L2_ETHER && il2 != RTE_PTYPE_INNER_L2_ETHER_VLAN &&
        il2 != RTE_PTYPE_INNER_L2_ETHER_QINQ)
        return -1;

    if (il3 && il3 != RTE_PTYPE_INNER_L3_IPV4 && il3 != RTE_PTYPE_INNER_L3_IPV4_EXT && il3 != RTE_PTYPE_INNER_L3_IPV6 &&
        il3 != RTE_PTYPE_INNER_L3_IPV4_EXT_UNKNOWN && il3 != RTE_PTYPE_INNER_L3_IPV6_EXT &&
        il3 != RTE_PTYPE_INNER_L3_IPV6_EXT_UNKNOWN)
        return -1;

    if (il4 && il4 != RTE_PTYPE_INNER_L4_TCP && il4 != RTE_PTYPE_INNER_L4_UDP && il4 != RTE_PTYPE_INNER_L4_FRAG &&
        il4 != RTE_PTYPE_INNER_L4_SCTP && il4 != RTE_PTYPE_INNER_L4_ICMP && il4 != RTE_PTYPE_INNER_L4_NONFRAG)
        return -1;

    return 0;
}

static int check_invalid_ptype_mapping(struct rte_pmd_i40e_ptype_mapping *mapping_table, uint16_t count) {
    int i;
...
int rte_pmd_i40e_ptype_mapping_update(uint16_t port, struct rte_pmd_i40e_ptype_mapping *mapping_items, uint16_t count,
                                      uint8_t exclusive) {
    struct rte_eth_dev *dev;
...
int rte_pmd_i40e_ptype_mapping_get(uint16_t port, struct rte_pmd_i40e_ptype_mapping *mapping_items, uint16_t size,
                                   uint16_t *count, uint8_t valid_only) {
    struct rte_eth_dev *dev;
    struct i40e_adapter *ad;
    int n = 0;
    uint16_t i;
...
int rte_pmd_i40e_flow_type_mapping_update(uint16_t port, struct rte_pmd_i40e_flow_type_mapping *mapping_items,
    ...
    for (i = 0; i < count; i++)
        if (mapping_items[i].flow_type >= I40E_FLOW_TYPE_MAX || mapping_items[i].flow_type == RTE_ETH_FLOW_UNKNOWN ||
...
static int i40e_vsi_update_queue_region_mapping(struct i40e_hw *hw, struct i40e_pf *pf) {
    uint16_t i;
    ...
    for (i = 0; i < region_info->queue_region_number; i++) {
        ...
        vsi_info->tc_mapping[tc_index] = rte_cpu_to_le_16((queue_offset << I40E_AQ_VSI_TC_QUE_OFFSET_SHIFT) |
    ...
    vsi_info->valid_sections |= rte_cpu_to_le_16(I40E_AQ_VSI_PROP_QUEUE_MAP_VALID);

    /* Update the VSI after updating the VSI queue-mapping information */
    ret = i40e_aq_update_vsi_params(hw, &ctxt, NULL);
    if (ret) {
        PMD_DRV_LOG(ERR, "Failed to configure queue region mapping = %d ", hw->aq.asq_last_status);
        return ret;
    }
...
static int i40e_queue_region_set_region(struct i40e_pf *pf, struct rte_pmd_i40e_queue_region_conf *conf_ptr) {
    uint16_t i;
    struct i40e_vsi *main_vsi = pf->main_vsi;
    struct i40e_queue_regions *info = &pf->queue_region;
    int32_t ret = -EINVAL;

    if (!((rte_is_power_of_2(conf_ptr->queue_num)) && conf_ptr->queue_num <= 64)) {
        PMD_DRV_LOG(ERR,
    ...
    if ((conf_ptr->queue_start_index + conf_ptr->queue_num) > main_vsi->nb_used_qps) {
...
static int i40e_queue_region_set_flowtype(struct i40e_pf *pf, struct rte_pmd_i40e_queue_region_conf *rss_region_conf) {
    int32_t ret = -EINVAL;
    struct i40e_queue_regions *info = &pf->queue_region;
    uint16_t i, j;
    ...
    info->region[region_index].hw_flowtype[flowtype_index] = rss_region_conf->hw_flowtype;
    info->region[region_index].flowtype_num++;

    return 0;
}

static void i40e_queue_region_pf_flowtype_conf(struct i40e_hw *hw, struct i40e_pf *pf) {
    uint8_t hw_flowtype;
    ...
    for (i = 0; i < info->queue_region_number; i++) {
        for (j = 0; j < info->region[i].flowtype_num; j++) {
            ...
            if ((hw_flowtype & 0x7) == 0) {
                pfqf_hregion |= info->region[i].region_id << I40E_PFQF_HREGION_REGION_0_SHIFT;
                pfqf_hregion |= 1 << I40E_PFQF_HREGION_OVERRIDE_ENA_0_SHIFT;
            } else if ((hw_flowtype & 0x7) == 1) {
                pfqf_hregion |= info->region[i].region_id << I40E_PFQF_HREGION_REGION_1_SHIFT;
                pfqf_hregion |= 1 << I40E_PFQF_HREGION_OVERRIDE_ENA_1_SHIFT;
            } else if ((hw_flowtype & 0x7) == 2) {
                pfqf_hregion |= info->region[i].region_id << I40E_PFQF_HREGION_REGION_2_SHIFT;
                pfqf_hregion |= 1 << I40E_PFQF_HREGION_OVERRIDE_ENA_2_SHIFT;
            } else if ((hw_flowtype & 0x7) == 3) {
                pfqf_hregion |= info->region[i].region_id << I40E_PFQF_HREGION_REGION_3_SHIFT;
                pfqf_hregion |= 1 << I40E_PFQF_HREGION_OVERRIDE_ENA_3_SHIFT;
            } else if ((hw_flowtype & 0x7) == 4) {
                pfqf_hregion |= info->region[i].region_id << I40E_PFQF_HREGION_REGION_4_SHIFT;
                pfqf_hregion |= 1 << I40E_PFQF_HREGION_OVERRIDE_ENA_4_SHIFT;
            } else if ((hw_flowtype & 0x7) == 5) {
                pfqf_hregion |= info->region[i].region_id << I40E_PFQF_HREGION_REGION_5_SHIFT;
                pfqf_hregion |= 1 << I40E_PFQF_HREGION_OVERRIDE_ENA_5_SHIFT;
            } else if ((hw_flowtype & 0x7) == 6) {
                pfqf_hregion |= info->region[i].region_id << I40E_PFQF_HREGION_REGION_6_SHIFT;
                pfqf_hregion |= 1 << I40E_PFQF_HREGION_OVERRIDE_ENA_6_SHIFT;
            } else {
                pfqf_hregion |= info->region[i].region_id << I40E_PFQF_HREGION_REGION_7_SHIFT;
...
static int i40e_queue_region_set_user_priority(struct i40e_pf *pf,
...
static int i40e_queue_region_dcb_configure(struct i40e_hw *hw, struct i40e_pf *pf) {
    ...
    *old_cfg = *dcb_cfg;
    ...
    if (ret) {
        PMD_DRV_LOG(ERR, "Set queue region DCB Config failed, err %s aq_err %s", i40e_stat_str(hw, ret),
...
static int i40e_queue_region_get_all_info(struct i40e_pf *pf, struct i40e_queue_regions *regions_ptr) {
    struct i40e_queue_regions *info = &pf->queue_region;
...
int rte_pmd_i40e_rss_queue_region_conf(uint16_t port_id, enum rte_pmd_i40e_queue_region_op op_type, void *arg) {
    ...
    case RTE_PMD_I40E_RSS_QUEUE_REGION_SET:
        ret = i40e_queue_region_set_region(pf, (struct rte_pmd_i40e_queue_region_conf *)arg);
        break;
    case RTE_PMD_I40E_RSS_QUEUE_REGION_FLOWTYPE_SET:
        ret = i40e_queue_region_set_flowtype(pf, (struct rte_pmd_i40e_queue_region_conf *)arg);
        break;
    case RTE_PMD_I40E_RSS_QUEUE_REGION_USER_PRIORITY_SET:
        ret = i40e_queue_region_set_user_priority(pf, (struct rte_pmd_i40e_queue_region_conf *)arg);
        break;
...
int rte_pmd_i40e_flow_add_del_packet_template(uint16_t port, const struct rte_pmd_i40e_pkt_template_conf *conf,
                                              uint8_t add) {
    struct rte_eth_dev *dev = &rte_eth_devices[port];
    ...
    filter_conf.action.report_status = (enum i40e_fdir_status)conf->action.report_status;
    filter_conf.action.flex_off = conf->action.flex_off;

    return i40e_flow_add_del_fdir_filter(dev, &filter_conf, add);
}

int rte_pmd_i40e_inset_get(uint16_t port, uint8_t pctype, struct rte_pmd_i40e_inset *inset,
...
int rte_pmd_i40e_inset_set(uint16_t port, uint8_t pctype, struct rte_pmd_i40e_inset *inset,
                           enum rte_pmd_i40e_inset_type inset_type) {
    struct rte_eth_dev *dev;
    struct i40e_hw *hw;
    struct i40e_pf *pf;
    ...
    case INSET_FDIR:
        ...
        break;
    case INSET_FDIR_FLX:
        i40e_check_write_reg(hw, I40E_PRTQF_FD_FLXINSET(pctype), (uint32_t)(inset_reg & UINT32_MAX));
        for (i = 0; i < 2; i++)
