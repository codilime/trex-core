...
void ixgbe_init_mac_link_ops_82599(struct ixgbe_hw *hw) {
     ...
     * and MNG not enabled
     */
    if ((mac->ops.get_media_type(hw) == ixgbe_media_type_fiber) && !ixgbe_mng_enabled(hw)) {
    ...
    if (hw->phy.multispeed_fiber) {
        ...
        if ((ixgbe_get_media_type(hw) == ixgbe_media_type_backplane) &&
            (hw->phy.smart_speed == ixgbe_smart_speed_auto || hw->phy.smart_speed == ixgbe_smart_speed_on) &&
...
s32 ixgbe_setup_sfp_modules_82599(struct ixgbe_hw *hw) {
    ...
    if (hw->phy.sfp_type != ixgbe_sfp_type_unknown) {
        ...
        ret_val = hw->mac.ops.prot_autoc_write(hw, hw->mac.orig_autoc | IXGBE_AUTOC_LMS_10G_SERIAL, false);
...
s32 ixgbe_init_ops_82599(struct ixgbe_hw *hw) {
    ...
    mac->ops.get_supported_physical_layer = ixgbe_get_supported_physical_layer_82599;
    ...
    mac->arc_subsystem_valid = !!(IXGBE_READ_REG(hw, IXGBE_FWSM_BY_MAC(hw)) & IXGBE_FWSM_MODE_MASK);
    ...
    mac->ops.init_thermal_sensor_thresh = ixgbe_init_thermal_sensor_thresh_generic;
...
s32 ixgbe_get_link_capabilities_82599(struct ixgbe_hw *hw, ixgbe_link_speed *speed, bool *autoneg) {
    ...
    if (hw->phy.sfp_type == ixgbe_sfp_type_1g_cu_core0 || hw->phy.sfp_type == ixgbe_sfp_type_1g_cu_core1 ||
...
void ixgbe_stop_mac_link_on_d3_82599(struct ixgbe_hw *hw) {
    ...
    if (!ixgbe_mng_present(hw) && !hw->wol_enabled && ee_ctrl_2 & IXGBE_EEPROM_CCD_BIT) {
...
s32 ixgbe_setup_mac_link_82599(struct ixgbe_hw *hw, ixgbe_link_speed speed, bool autoneg_wait_to_complete) {
    ...
    u32 current_autoc =
        IXGBE_READ_REG(hw, IXGBE_AUTOC); /* holds the value of AUTOC register at this current point in time */
    ...
    if (link_mode == IXGBE_AUTOC_LMS_KX4_KX_KR || link_mode == IXGBE_AUTOC_LMS_KX4_KX_KR_1G_AN ||
        ...
        if (speed & IXGBE_LINK_SPEED_10GB_FULL) {
            ...
            if ((orig_autoc & IXGBE_AUTOC_KR_SUPP) && (hw->phy.smart_speed_active == false))
    ...
    } else if ((pma_pmd_10g_serial == IXGBE_AUTOC2_10G_SFI) && (link_mode == IXGBE_AUTOC_LMS_10G_SERIAL)) {
...
mac_reset_top:
    ...
    if (ixgbe_validate_mac_addr(hw->mac.san_addr) == 0) {
        ...
        hw->mac.ops.set_rar(hw, hw->mac.san_mac_rar_index, hw->mac.san_addr, 0, IXGBE_RAH_AV);
...
s32 ixgbe_reinit_fdir_tables_82599(struct ixgbe_hw *hw) {
     ...
     */
    IXGBE_WRITE_REG(hw, IXGBE_FDIRCMD, (IXGBE_READ_REG(hw, IXGBE_FDIRCMD) | IXGBE_FDIRCMD_CLEARHT));
    IXGBE_WRITE_FLUSH(hw);
    IXGBE_WRITE_REG(hw, IXGBE_FDIRCMD, (IXGBE_READ_REG(hw, IXGBE_FDIRCMD) & ~IXGBE_FDIRCMD_CLEARHT));
    IXGBE_WRITE_FLUSH(hw);
    /*
...
s32 ixgbe_init_fdir_signature_82599(struct ixgbe_hw *hw, u32 fdirctrl) {
     ...
     */
    fdirctrl |= (0x6 << IXGBE_FDIRCTRL_FLEX_SHIFT) | (0xA << IXGBE_FDIRCTRL_MAX_LENGTH_SHIFT) |
...
s32 ixgbe_init_fdir_perfect_82599(struct ixgbe_hw *hw, u32 fdirctrl, bool cloud_mode) {
    ...
    fdirctrl |= IXGBE_FDIRCTRL_PERFECT_MATCH | IXGBE_FDIRCTRL_REPORT_STATUS |
                (IXGBE_FDIR_DROP_QUEUE << IXGBE_FDIRCTRL_DROP_Q_SHIFT) | (0x6 << IXGBE_FDIRCTRL_FLEX_SHIFT) |
                (0xA << IXGBE_FDIRCTRL_MAX_LENGTH_SHIFT) | (4 << IXGBE_FDIRCTRL_FULL_THRESH_SHIFT);
 ...
 **/
void ixgbe_fdir_add_signature_filter_82599(struct ixgbe_hw *hw, union ixgbe_atr_hash_dword input,
    ...
    fdircmd = IXGBE_FDIRCMD_CMD_ADD_FLOW | IXGBE_FDIRCMD_FILTER_UPDATE | IXGBE_FDIRCMD_LAST | IXGBE_FDIRCMD_QUEUE_EN;
...
#define IXGBE_STORE_AS_BE32(_value)                                                                                    \
    (((u32)(_value) >> 24) | (((u32)(_value)&0x00FF0000) >> 8) | (((u32)(_value)&0x0000FF00) << 8) |                   \
     ((u32)(_value) << 24))
 ...
 **/
s32 ixgbe_fdir_add_perfect_filter_82599(struct ixgbe_hw *hw, union ixgbe_atr_input *input,
                                        union ixgbe_atr_input *input_mask, u16 soft_id, u8 queue, bool cloud_mode) {
    ...
    case IXGBE_ATR_FLOW_TYPE_TUNNELED_UDPV4:
        input_mask->formatted.flow_type = IXGBE_ATR_L4TYPE_IPV6_MASK | IXGBE_ATR_L4TYPE_MASK;
        break;
    default:
        DEBUGOUT(" Error on flow type input\n");
        return err;
    ...
    return ixgbe_fdir_write_perfect_filter_82599(hw, input, soft_id, queue, cloud_mode);
}
...
u64 ixgbe_get_supported_physical_layer_82599(struct ixgbe_hw *hw) {
    ...
    case IXGBE_AUTOC_LMS_1G_LINK_NO_AN:
        if (pma_pmd_1g == IXGBE_AUTOC_1G_KX_BX) {
            physical_layer = IXGBE_PHYSICAL_LAYER_1000BASE_KX | IXGBE_PHYSICAL_LAYER_1000BASE_BX;
            goto out;
        } else
...
STATIC s32 ixgbe_verify_fw_version_82599(struct ixgbe_hw *hw) {
    ...
    if (hw->eeprom.ops.read(hw, IXGBE_FW_PTR, &fw_offset)) {
        ERROR_REPORT2(IXGBE_ERROR_INVALID_STATE, "eeprom read at offset %d failed", IXGBE_FW_PTR);
    ...
    if (hw->eeprom.ops.read(hw, (fw_offset + IXGBE_FW_PASSTHROUGH_PATCH_CONFIG_PTR), &fw_ptp_cfg_offset)) {
    ...
    if (hw->eeprom.ops.read(hw, (fw_ptp_cfg_offset + IXGBE_FW_PATCH_VERSION_4), &fw_version)) {
        ERROR_REPORT2(IXGBE_ERROR_INVALID_STATE, "eeprom read at offset %d failed",
...
bool ixgbe_verify_lesm_fw_enabled_82599(struct ixgbe_hw *hw) {
    ...
    status = hw->eeprom.ops.read(hw, (fw_offset + IXGBE_FW_LESM_PARAMETERS_PTR), &fw_lesm_param_offset);

    if ((status != IXGBE_SUCCESS) || (fw_lesm_param_offset == 0) || (fw_lesm_param_offset == 0xFFFF))
        goto out;

    /* get the LESM state word */
    status = hw->eeprom.ops.read(hw, (fw_lesm_param_offset + IXGBE_FW_LESM_STATE_1), &fw_lesm_state);

    if ((status == IXGBE_SUCCESS) && (fw_lesm_state & IXGBE_FW_LESM_STATE_ENABLED))
        lesm_enabled = true;
...
STATIC s32 ixgbe_read_eeprom_buffer_82599(struct ixgbe_hw *hw, u16 offset, u16 words, u16 *data) {
     ...
     */
    if ((eeprom->type == ixgbe_eeprom_spi) && (offset + (words - 1) <= IXGBE_EERD_MAX_ADDR))
        ret_val = ixgbe_read_eerd_buffer_generic(hw, offset, words, data);
    else
        ret_val = ixgbe_read_eeprom_buffer_bit_bang_generic(hw, offset, words, data);
