...
bool ixgbe_device_supports_autoneg_fc(struct ixgbe_hw *hw) {
    ...
    if (!supported)
        ERROR_REPORT2(IXGBE_ERROR_UNSUPPORTED, "Device %x does not support flow control autoneg", hw->device_id);
    return supported;
}
...
s32 ixgbe_setup_fc_generic(struct ixgbe_hw *hw) {
    ...
    if (hw->phy.media_type == ixgbe_media_type_backplane) {
        ...
        if (ret_val)
            goto out;
    } else if ((hw->phy.media_type == ixgbe_media_type_copper) && (ixgbe_device_supports_autoneg_fc(hw))) {
        hw->phy.ops.write_reg(hw, IXGBE_MDIO_AUTO_NEG_ADVT, IXGBE_MDIO_AUTO_NEG_DEV_TYPE, reg_cu);
    }
 ...
 *
 **/
s32 ixgbe_read_pba_raw(struct ixgbe_hw *hw, u16 *eeprom_buf, u32 eeprom_buf_size, u16 max_pba_block_size,
    ...
    if (eeprom_buf == NULL) {
        ret_val = hw->eeprom.ops.read_buffer(hw, IXGBE_PBANUM0_PTR, 2, &pba->word[0]);
        if (ret_val)
            return ret_val;
    } else {
    ...
    if (pba->word[0] == IXGBE_PBANUM_PTR_GUARD) {
        ...
        ret_val = ixgbe_get_pba_block_size(hw, eeprom_buf, eeprom_buf_size, &pba_block_size);
        if (ret_val)
            return ret_val;
        ...
        if (eeprom_buf == NULL) {
            ret_val = hw->eeprom.ops.read_buffer(hw, pba->word[1], pba_block_size, pba->pba_block);
            if (ret_val)
                return ret_val;
        } else {
            if (eeprom_buf_size > (u32)(pba->word[1] + pba_block_size)) {
                memcpy(pba->pba_block, &eeprom_buf[pba->word[1]], pba_block_size * sizeof(u16));
            } else {
                return IXGBE_ERR_PARAM;
            }
        }
...
s32 ixgbe_write_pba_raw(struct ixgbe_hw *hw, u16 *eeprom_buf, u32 eeprom_buf_size, struct ixgbe_pba *pba) {
    ...
    if (eeprom_buf == NULL) {
        ret_val = hw->eeprom.ops.write_buffer(hw, IXGBE_PBANUM0_PTR, 2, &pba->word[0]);
        if (ret_val)
            return ret_val;
    } else {
        if (eeprom_buf_size > IXGBE_PBANUM1_PTR) {
    ...
    if (pba->word[0] == IXGBE_PBANUM_PTR_GUARD) {
        ...
        if (eeprom_buf == NULL) {
            ret_val = hw->eeprom.ops.write_buffer(hw, pba->word[1], pba->pba_block[0], pba->pba_block);
            if (ret_val)
                return ret_val;
        } else {
            if (eeprom_buf_size > (u32)(pba->word[1] + pba->pba_block[0])) {
                memcpy(&eeprom_buf[pba->word[1]], pba->pba_block, pba->pba_block[0] * sizeof(u16));
            } else {
                return IXGBE_ERR_PARAM;
            }
        }
...
s32 ixgbe_get_pba_block_size(struct ixgbe_hw *hw, u16 *eeprom_buf, u32 eeprom_buf_size, u16 *pba_block_size) {
    ...
    if (eeprom_buf == NULL) {
        ret_val = hw->eeprom.ops.read_buffer(hw, IXGBE_PBANUM0_PTR, 2, &pba_word[0]);
        if (ret_val)
            return ret_val;
    } else {
        if (eeprom_buf_size > IXGBE_PBANUM1_PTR) {
...
void ixgbe_set_lan_id_multi_port_pcie(struct ixgbe_hw *hw) {
    ...
    if (hw->device_id == IXGBE_DEV_ID_X550EM_A_SFP) {
        ...
        bus->instance_id = (ee_ctrl_4 & IXGBE_EE_CTRL_4_INST_ID) >> IXGBE_EE_CTRL_4_INST_ID_SHIFT;
    }
}
...
s32 ixgbe_write_eeprom_buffer_bit_bang_generic(struct ixgbe_hw *hw, u16 offset, u16 words, u16 *data) {
    ...
    for (i = 0; i < words; i += IXGBE_EEPROM_RD_BUFFER_MAX_COUNT) {
        count = (words - i) / IXGBE_EEPROM_RD_BUFFER_MAX_COUNT > 0 ? IXGBE_EEPROM_RD_BUFFER_MAX_COUNT : (words - i);
        status = ixgbe_write_eeprom_buffer_bit_bang(hw, offset + i, count, &data[i]);
...
STATIC s32 ixgbe_write_eeprom_buffer_bit_bang(struct ixgbe_hw *hw, u16 offset, u16 words, u16 *data) {
    ...
    if (status == IXGBE_SUCCESS) {
        for (i = 0; i < words; i++) {
            ...
            ixgbe_shift_out_eeprom_bits(hw, IXGBE_EEPROM_WREN_OPCODE_SPI, IXGBE_EEPROM_OPCODE_BITS);
            ...
            ixgbe_shift_out_eeprom_bits(hw, (u16)((offset + i) * 2), hw->eeprom.address_bits);
...
STATIC s32 ixgbe_detect_eeprom_page_size_generic(struct ixgbe_hw *hw, u16 offset) {
    ...
    status = ixgbe_write_eeprom_buffer_bit_bang(hw, offset, IXGBE_EEPROM_PAGE_SIZE_MAX, data);
...
s32 ixgbe_write_eewr_buffer_generic(struct ixgbe_hw *hw, u16 offset, u16 words, u16 *data) {
    ...
    for (i = 0; i < words; i++) {
        eewr = ((offset + i) << IXGBE_EEPROM_RW_ADDR_SHIFT) | (data[i] << IXGBE_EEPROM_RW_REG_DATA) |
...
s32 ixgbe_poll_eerd_eewr_done(struct ixgbe_hw *hw, u32 ee_reg) {
    ...
    if (i == IXGBE_EERD_EEWR_ATTEMPTS)
        ERROR_REPORT1(IXGBE_ERROR_POLLING, "EEPROM read/write done polling timed out");
...
STATIC s32 ixgbe_get_eeprom_semaphore(struct ixgbe_hw *hw) {
    ...
    if (status == IXGBE_SUCCESS) {
        ...
        ERROR_REPORT1(IXGBE_ERROR_POLLING, "Software semaphore SMBI between device drivers "
                                           "not granted.\n");
    }
...
STATIC s32 ixgbe_ready_eeprom(struct ixgbe_hw *hw) {
    ...
    for (i = 0; i < IXGBE_EEPROM_MAX_RETRY_SPI; i += 5) {
        ixgbe_shift_out_eeprom_bits(hw, IXGBE_EEPROM_RDSR_OPCODE_SPI, IXGBE_EEPROM_OPCODE_BITS);
...
s32 ixgbe_validate_mac_addr(u8 *mac_addr) {
    ...
    } else if (mac_addr[0] == 0 && mac_addr[1] == 0 && mac_addr[2] == 0 && mac_addr[3] == 0 && mac_addr[4] == 0 &&
               mac_addr[5] == 0) {
        status = IXGBE_ERR_INVALID_MAC_ADDR;
    }
    return status;
...
s32 ixgbe_set_rar_generic(struct ixgbe_hw *hw, u32 index, u8 *addr, u32 vmdq, u32 enable_addr) {
     ...
     */
    rar_low = ((u32)addr[0] | ((u32)addr[1] << 8) | ((u32)addr[2] << 16) | ((u32)addr[3] << 24));
    /*
...
s32 ixgbe_init_rx_addrs_generic(struct ixgbe_hw *hw) {
    ...
    if (ixgbe_validate_mac_addr(hw->mac.addr) == IXGBE_ERR_INVALID_MAC_ADDR) {
        ...
        DEBUGOUT3(" Keeping Current RAR0 Addr =%.2X %.2X %.2X ", hw->mac.addr[0], hw->mac.addr[1], hw->mac.addr[2]);
        DEBUGOUT3("%.2X %.2X %.2X\n", hw->mac.addr[3], hw->mac.addr[4], hw->mac.addr[5]);
    } else {
        /* Setup the receive address. */
        DEBUGOUT("Overriding MAC Address in RAR[0]\n");
        DEBUGOUT3(" New MAC Addr =%.2X %.2X %.2X ", hw->mac.addr[0], hw->mac.addr[1], hw->mac.addr[2]);
        DEBUGOUT3("%.2X %.2X %.2X\n", hw->mac.addr[3], hw->mac.addr[4], hw->mac.addr[5]);
 ...
 **/
s32 ixgbe_update_mc_addr_list_generic(struct ixgbe_hw *hw, u8 *mc_addr_list, u32 mc_addr_count, ixgbe_mc_addr_itr next,
                                      bool clear) {
    u32 i;
    u32 vmdq;
...
s32 ixgbe_negotiate_fc(struct ixgbe_hw *hw, u32 adv_reg, u32 lp_reg, u32 adv_sym, u32 adv_asm, u32 lp_sym, u32 lp_asm) {
    ...
    } else if (!(adv_reg & adv_sym) && (adv_reg & adv_asm) && (lp_reg & lp_sym) && (lp_reg & lp_asm)) {
...
STATIC s32 ixgbe_fc_autoneg_fiber(struct ixgbe_hw *hw) {
    ...
    if ((!!(linkstat & IXGBE_PCS1GLSTA_AN_COMPLETE) == 0) || (!!(linkstat & IXGBE_PCS1GLSTA_AN_TIMED_OUT) == 1)) {
    ...
    ret_val = ixgbe_negotiate_fc(hw, pcs_anadv_reg, pcs_lpab_reg, IXGBE_PCS1GANA_SYM_PAUSE, IXGBE_PCS1GANA_ASM_PAUSE,
...
STATIC s32 ixgbe_fc_autoneg_backplane(struct ixgbe_hw *hw) {
    ...
    ret_val = ixgbe_negotiate_fc(hw, autoc_reg, anlp1_reg, IXGBE_AUTOC_SYM_PAUSE, IXGBE_AUTOC_ASM_PAUSE,
...
STATIC s32 ixgbe_fc_autoneg_copper(struct ixgbe_hw *hw) {
    ...
    hw->phy.ops.read_reg(hw, IXGBE_MDIO_AUTO_NEG_ADVT, IXGBE_MDIO_AUTO_NEG_DEV_TYPE, &technology_ability_reg);
    hw->phy.ops.read_reg(hw, IXGBE_MDIO_AUTO_NEG_LP, IXGBE_MDIO_AUTO_NEG_DEV_TYPE, &lp_technology_ability_reg);

    return ixgbe_negotiate_fc(hw, (u32)technology_ability_reg, (u32)lp_technology_ability_reg, IXGBE_TAF_SYM_PAUSE,
                              IXGBE_TAF_ASM_PAUSE, IXGBE_TAF_SYM_PAUSE, IXGBE_TAF_ASM_PAUSE);
}
...
s32 ixgbe_disable_pcie_master(struct ixgbe_hw *hw) {
    ...
    if (!(IXGBE_READ_REG(hw, IXGBE_STATUS) & IXGBE_STATUS_GIO) || IXGBE_REMOVED(hw->hw_addr))
        goto out;
...
STATIC s32 ixgbe_get_san_mac_addr_offset(struct ixgbe_hw *hw, u16 *san_mac_offset) {
    ...
    if (ret_val) {
        ERROR_REPORT2(IXGBE_ERROR_INVALID_STATE, "eeprom at offset %d failed", IXGBE_SAN_MAC_ADDR_PTR);
    }
...
s32 ixgbe_get_san_mac_addr_generic(struct ixgbe_hw *hw, u8 *san_mac_addr) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (ret_val) {
            ERROR_REPORT2(IXGBE_ERROR_INVALID_STATE, "eeprom read at offset %d failed", san_mac_offset);
            goto san_mac_addr_out;
        }
...
s32 ixgbe_insert_mac_addr_generic(struct ixgbe_hw *hw, u8 *addr, u32 vmdq) {
    ...
    for (rar = 0; rar < hw->mac.rar_highwater; rar++) {
        ...
        if (((IXGBE_RAH_AV & rar_high) == 0) && first_empty_rar == NO_EMPTY_RAR_FOUND) {
            first_empty_rar = rar;
...
s32 ixgbe_clear_vmdq_generic(struct ixgbe_hw *hw, u32 rar, u32 vmdq) {
    ...
    if (mpsar_lo == 0 && mpsar_hi == 0 && rar != 0 && rar != hw->mac.san_mac_rar_index)
...
s32 ixgbe_set_vfta_generic(struct ixgbe_hw *hw, u32 vlan, u32 vind, bool vlan_on, bool vlvf_bypass) {
     ...
     */
    ret_val = ixgbe_set_vlvf_generic(hw, vlan, vind, vlan_on, &vfta_delta, vfta, vlvf_bypass);
    if (ret_val != IXGBE_SUCCESS) {
        if (vlvf_bypass)
            goto vfta_update;
        return ret_val;
 ...
 **/
s32 ixgbe_set_vlvf_generic(struct ixgbe_hw *hw, u32 vlan, u32 vind, bool vlan_on, u32 *vfta_delta, u32 vfta,
                           bool vlvf_bypass) {
    u32 bits;
    s32 vlvf_index;
    ...
    if (!bits && !IXGBE_READ_REG(hw, IXGBE_VLVFB(vlvf_index * 2 + 1 - vind / 32))) {
    ...
    case IXGBE_LINKS_SPEED_10_X550EM_A:
        *speed = IXGBE_LINK_SPEED_UNKNOWN;
        ...
        if (hw->device_id == IXGBE_DEV_ID_X550EM_A_1G_T || hw->device_id == IXGBE_DEV_ID_X550EM_A_1G_T_L ||
            hw->device_id == IXGBE_DEV_ID_X550EM_A_SGMII || hw->device_id == IXGBE_DEV_ID_X550EM_A_SGMII_L)
...
s32 ixgbe_get_wwn_prefix_generic(struct ixgbe_hw *hw, u16 *wwnn_prefix, u16 *wwpn_prefix) {
    ...
    *wwpn_prefix = 0xFFFF;
    ...
    if (hw->eeprom.ops.read(hw, offset, wwnn_prefix)) {
        ERROR_REPORT2(IXGBE_ERROR_INVALID_STATE, "eeprom read at offset %d failed", offset);
    }
...
s32 ixgbe_hic_unlocked(struct ixgbe_hw *hw, u32 *buffer, u32 length, u32 timeout) {
    ...
    if ((timeout && i == timeout) || !(IXGBE_READ_REG(hw, IXGBE_HICR) & IXGBE_HICR_SV)) {
...
s32 ixgbe_host_interface_command(struct ixgbe_hw *hw, u32 *buffer, u32 length, u32 timeout, bool return_data) {
    ...
    if (resp->cmd == 0x30) {
        ...
        buf_len = (((u16)(resp->cmd_or_resp.ret_status) << 3) & 0xF00) | resp->buf_len;
        hdr_size += (2 << 2);
    } else {
 ...
 *  semaphore or IXGBE_ERR_HOST_INTERFACE_COMMAND when command fails.
 **/
s32 ixgbe_set_fw_drv_ver_generic(struct ixgbe_hw *hw, u8 maj, u8 min, u8 build, u8 sub, u16 len,
...
STATIC const u8 ixgbe_emc_temp_data[4] = {IXGBE_EMC_INTERNAL_DATA, IXGBE_EMC_DIODE1_DATA, IXGBE_EMC_DIODE2_DATA,
                                          IXGBE_EMC_DIODE3_DATA};
STATIC const u8 ixgbe_emc_therm_limit[4] = {IXGBE_EMC_INTERNAL_THERM_LIMIT, IXGBE_EMC_DIODE1_THERM_LIMIT,
...
s32 ixgbe_get_thermal_sensor_data_generic(struct ixgbe_hw *hw) {
    ...
    if ((hw->mac.type != ixgbe_mac_82599EB) || (IXGBE_READ_REG(hw, IXGBE_STATUS) & IXGBE_STATUS_LAN_ID_1)) {
    ...
    for (i = 0; i < num_sensors; i++) {
        ...
        sensor_index = ((ets_sensor & IXGBE_ETS_DATA_INDEX_MASK) >> IXGBE_ETS_DATA_INDEX_SHIFT);
        sensor_location = ((ets_sensor & IXGBE_ETS_DATA_LOC_MASK) >> IXGBE_ETS_DATA_LOC_SHIFT);

        if (sensor_location != 0) {
            status = hw->phy.ops.read_i2c_byte(hw, ixgbe_emc_temp_data[sensor_index], IXGBE_I2C_THERMAL_SENSOR_ADDR,
                                               &data->sensor[i].temp);
            if (status)
                goto out;
        }
...
s32 ixgbe_init_thermal_sensor_thresh_generic(struct ixgbe_hw *hw) {
    ...
    low_thresh_delta = ((ets_cfg & IXGBE_ETS_LTHRES_DELTA_MASK) >> IXGBE_ETS_LTHRES_DELTA_SHIFT);
    num_sensors = (ets_cfg & IXGBE_ETS_NUM_SENSORS_MASK);
    ...
    for (i = 0; i < num_sensors; i++) {
        ...
        hw->phy.ops.write_i2c_byte(hw, ixgbe_emc_therm_limit[sensor_index], IXGBE_I2C_THERMAL_SENSOR_ADDR, therm_limit);
...
void ixgbe_get_orom_version(struct ixgbe_hw *hw, struct ixgbe_nvm_version *nvm_ver) {
    ...
    if ((eeprom_cfg_blkl | eeprom_cfg_blkh) == 0x0 || eeprom_cfg_blkl == NVM_VER_INVALID ||
        eeprom_cfg_blkh == NVM_VER_INVALID)
        return;

    nvm_ver->or_valid = true;
    nvm_ver->or_major = eeprom_cfg_blkl >> NVM_OROM_SHIFT;
    nvm_ver->or_build = (eeprom_cfg_blkl << NVM_OROM_SHIFT) | (eeprom_cfg_blkh >> NVM_OROM_SHIFT);
...
void ixgbe_get_oem_prod_version(struct ixgbe_hw *hw, struct ixgbe_nvm_version *nvm_ver) {
    ...
    if (mod_len != NVM_OEM_PROD_VER_MOD_LEN || (cap & NVM_OEM_PROD_VER_CAP_MASK) != 0x0)
        return;
    ...
    if ((rel_num | prod_ver) == 0x0 || rel_num == NVM_VER_INVALID || prod_ver == NVM_VER_INVALID)
        return;
...
s32 ixgbe_setup_mac_link_multispeed_fiber(struct ixgbe_hw *hw, ixgbe_link_speed speed, bool autoneg_wait_to_complete) {
    ...
    if (speed & IXGBE_LINK_SPEED_10GB_FULL) {
        ...
        status = ixgbe_setup_mac_link(hw, IXGBE_LINK_SPEED_10GB_FULL, autoneg_wait_to_complete);
        if (status != IXGBE_SUCCESS)
            return status;
    ...
    if (speed & IXGBE_LINK_SPEED_1GB_FULL) {
        ...
        status = ixgbe_setup_mac_link(hw, IXGBE_LINK_SPEED_1GB_FULL, autoneg_wait_to_complete);
        if (status != IXGBE_SUCCESS)
            return status;
     ...
     */
    if (speedcnt > 1)
        status = ixgbe_setup_mac_link_multispeed_fiber(hw, highest_link_speed, autoneg_wait_to_complete);
...
void ixgbe_set_soft_rate_select_speed(struct ixgbe_hw *hw, ixgbe_link_speed speed) {
    ...
    /* Set RS0 */
    status = hw->phy.ops.read_i2c_byte(hw, IXGBE_SFF_SFF_8472_OSCB, IXGBE_I2C_EEPROM_DEV_ADDR2, &eeprom_data);
    if (status) {
    ...
    status = hw->phy.ops.write_i2c_byte(hw, IXGBE_SFF_SFF_8472_OSCB, IXGBE_I2C_EEPROM_DEV_ADDR2, eeprom_data);
    if (status) {
    ...
    /* Set RS1 */
    status = hw->phy.ops.read_i2c_byte(hw, IXGBE_SFF_SFF_8472_ESCB, IXGBE_I2C_EEPROM_DEV_ADDR2, &eeprom_data);
    if (status) {
    ...
    status = hw->phy.ops.write_i2c_byte(hw, IXGBE_SFF_SFF_8472_ESCB, IXGBE_I2C_EEPROM_DEV_ADDR2, eeprom_data);
    if (status) {
