...
s32 ixgbe_write_mbx(struct ixgbe_hw *hw, u32 *msg, u16 size, u16 mbx_id) {
    ...
    if (size > mbx->size) {
        ...
        ERROR_REPORT2(IXGBE_ERROR_ARGUMENT, "Invalid mailbox message size %d", size);
...
STATIC s32 ixgbe_poll_for_msg(struct ixgbe_hw *hw, u16 mbx_id) {
    ...
    if (countdown == 0)
        ERROR_REPORT2(IXGBE_ERROR_POLLING, "Polling for VF%d mailbox message timedout", mbx_id);
...
STATIC s32 ixgbe_poll_for_ack(struct ixgbe_hw *hw, u16 mbx_id) {
    ...
    if (countdown == 0)
        ERROR_REPORT2(IXGBE_ERROR_POLLING, "Polling for VF%d mailbox ack timedout", mbx_id);
...
STATIC s32 ixgbe_obtain_mbx_lock_pf(struct ixgbe_hw *hw, u16 vf_number) {
    ...
    if (p2v_mailbox & IXGBE_PFMAILBOX_PFU)
        ...
        ERROR_REPORT2(IXGBE_ERROR_POLLING, "Failed to obtain mailbox lock for VF%d", vf_number);
...
void ixgbe_init_mbx_params_pf(struct ixgbe_hw *hw) {
    ...
    if (hw->mac.type != ixgbe_mac_82599EB && hw->mac.type != ixgbe_mac_X550 && hw->mac.type != ixgbe_mac_X550EM_x &&
