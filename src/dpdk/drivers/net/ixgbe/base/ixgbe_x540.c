...
s32 ixgbe_init_ops_X540(struct ixgbe_hw *hw) {
    ...
    mac->ops.get_supported_physical_layer = ixgbe_get_supported_physical_layer_X540;
...
s32 ixgbe_calc_eeprom_checksum_X540(struct ixgbe_hw *hw) {
    ...
    for (i = ptr_start; i < IXGBE_FW_PTR; i++) {
        ...
        if (length == 0xFFFF || length == 0 || (pointer + length) >= hw->eeprom.word_size)
            continue;
...
s32 ixgbe_acquire_swfw_sync_X540(struct ixgbe_hw *hw, u32 mask) {
     ...
     */
    if (ixgbe_get_swfw_sync_semaphore(hw)) {
        DEBUGOUT("Failed to get NVM sempahore and register semaphore while forcefully ignoring FW sempahore bit(s) and "
                 "setting SW semaphore bit(s), returning IXGBE_ERR_SWFW_SYNC\n");
        return IXGBE_ERR_SWFW_SYNC;
    }
    swfw_sync = IXGBE_READ_REG(hw, IXGBE_SWFW_SYNC_BY_MAC(hw));
    ...
    if (swfw_sync & swmask) {
        u32 rmask =
            IXGBE_GSSR_EEP_SM | IXGBE_GSSR_PHY0_SM | IXGBE_GSSR_PHY1_SM | IXGBE_GSSR_MAC_CSR_SM | IXGBE_GSSR_SW_MNG_SM;
...
void ixgbe_init_swfw_sync_X540(struct ixgbe_hw *hw) {
    ...
    rmask = IXGBE_GSSR_EEP_SM | IXGBE_GSSR_PHY0_SM | IXGBE_GSSR_PHY1_SM | IXGBE_GSSR_MAC_CSR_SM | IXGBE_GSSR_SW_MNG_SM;
