 ...
 **/
static s32 ixgbe_setup_mac_link_multispeed_fixed_fiber(struct ixgbe_hw *hw, ixgbe_link_speed speed,
    ...
    if (speed & IXGBE_LINK_SPEED_10GB_FULL) {
        ...
        status = ixgbe_setup_mac_link_82599(hw, IXGBE_LINK_SPEED_10GB_FULL, autoneg_wait_to_complete);
        if (status != IXGBE_SUCCESS)
            return status;
    ...
    if (speed & IXGBE_LINK_SPEED_1GB_FULL) {
        ...
        status = ixgbe_setup_mac_link_82599(hw, IXGBE_LINK_SPEED_1GB_FULL, autoneg_wait_to_complete);
        if (status != IXGBE_SUCCESS)
            return status;
     ...
     */
    if (speedcnt > 1)
        status = ixgbe_setup_mac_link_multispeed_fixed_fiber(hw, highest_link_speed, autoneg_wait_to_complete);
