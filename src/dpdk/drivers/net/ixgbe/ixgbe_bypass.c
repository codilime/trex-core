...
s32 ixgbe_bypass_state_store(struct rte_eth_dev *dev, u32 *new_state) {
    ...
    /* Set the new state */
    ret_val = adapter->bps.ops.bypass_set(hw, BYPASS_PAGE_CTL0, BYPASS_MODE_OFF_M, *new_state);
    if (ret_val)
        goto exit;

    /* Set AUTO back on so FW can receive events */
    ret_val = adapter->bps.ops.bypass_set(hw, BYPASS_PAGE_CTL0, BYPASS_MODE_OFF_M, BYPASS_AUTO);
...
s32 ixgbe_bypass_ver_show(struct rte_eth_dev *dev, u32 *ver) {
    ...
    cmd |= (BYPASS_EEPROM_VER_ADD << BYPASS_CTL2_OFFSET_SHIFT) & BYPASS_CTL2_OFFSET_M;
