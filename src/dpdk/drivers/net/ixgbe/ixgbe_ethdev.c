...
static int ixgbe_dev_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats);
static int ixgbe_dev_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats, unsigned n);
static int ixgbevf_dev_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats, unsigned n);
static int ixgbe_dev_xstats_get_by_id(struct rte_eth_dev *dev, const uint64_t *ids, uint64_t *values, unsigned int n);
static void ixgbe_dev_stats_reset(struct rte_eth_dev *dev);
static void ixgbe_dev_xstats_reset(struct rte_eth_dev *dev);
static int ixgbe_dev_xstats_get_names(struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats_names,
                                      unsigned int size);
static int ixgbevf_dev_xstats_get_names(struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats_names,
                                        unsigned limit);
static int ixgbe_dev_xstats_get_names_by_id(struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats_names,
                                            const uint64_t *ids, unsigned int limit);
static int ixgbe_dev_queue_stats_mapping_set(struct rte_eth_dev *eth_dev, uint16_t queue_id, uint8_t stat_idx,
                                             uint8_t is_rx);
static int ixgbe_fw_version_get(struct rte_eth_dev *dev, char *fw_version, size_t fw_size);
...
static int ixgbe_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id, int on);
static int ixgbe_vlan_tpid_set(struct rte_eth_dev *dev, enum rte_vlan_type vlan_type, uint16_t tpid_id);
static void ixgbe_vlan_hw_strip_bitmap_set(struct rte_eth_dev *dev, uint16_t queue, bool on);
static void ixgbe_vlan_strip_queue_set(struct rte_eth_dev *dev, uint16_t queue, int on);
static void ixgbe_config_vlan_strip_on_all_queues(struct rte_eth_dev *dev, int mask);
...
static int ixgbe_flow_ctrl_get(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf);
static int ixgbe_flow_ctrl_set(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf);
static int ixgbe_priority_flow_ctrl_set(struct rte_eth_dev *dev, struct rte_eth_pfc_conf *pfc_conf);
static int ixgbe_dev_rss_reta_update(struct rte_eth_dev *dev, struct rte_eth_rss_reta_entry64 *reta_conf,
                                     uint16_t reta_size);
static int ixgbe_dev_rss_reta_query(struct rte_eth_dev *dev, struct rte_eth_rss_reta_entry64 *reta_conf,
                                    uint16_t reta_size);
...
static int ixgbe_add_rar(struct rte_eth_dev *dev, struct ether_addr *mac_addr, uint32_t index, uint32_t pool);
static void ixgbe_remove_rar(struct rte_eth_dev *dev, uint32_t index);
static int ixgbe_set_default_mac_addr(struct rte_eth_dev *dev, struct ether_addr *mac_addr);
...
static int ixgbevf_dev_link_update(struct rte_eth_dev *dev, int wait_to_complete);
...
static int ixgbevf_dev_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats);
static void ixgbevf_dev_stats_reset(struct rte_eth_dev *dev);
static int ixgbevf_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id, int on);
static void ixgbevf_vlan_strip_queue_set(struct rte_eth_dev *dev, uint16_t queue, int on);
...
static void ixgbevf_set_ivar_map(struct ixgbe_hw *hw, int8_t direction, uint8_t queue, uint8_t msix_vector);
...
static int ixgbe_uc_hash_table_set(struct rte_eth_dev *dev, struct ether_addr *mac_addr, uint8_t on);
static int ixgbe_uc_all_hash_table_set(struct rte_eth_dev *dev, uint8_t on);
static int ixgbe_mirror_rule_set(struct rte_eth_dev *dev, struct rte_eth_mirror_conf *mirror_conf, uint8_t rule_id,
                                 uint8_t on);
static int ixgbe_mirror_rule_reset(struct rte_eth_dev *dev, uint8_t rule_id);
...
static void ixgbe_set_ivar_map(struct ixgbe_hw *hw, int8_t direction, uint8_t queue, uint8_t msix_vector);
static void ixgbe_configure_msix(struct rte_eth_dev *dev);

static int ixgbevf_add_mac_addr(struct rte_eth_dev *dev, struct ether_addr *mac_addr, uint32_t index, uint32_t pool);
static void ixgbevf_remove_mac_addr(struct rte_eth_dev *dev, uint32_t index);
static int ixgbevf_set_default_mac_addr(struct rte_eth_dev *dev, struct ether_addr *mac_addr);
static int ixgbe_syn_filter_get(struct rte_eth_dev *dev, struct rte_eth_syn_filter *filter);
static int ixgbe_syn_filter_handle(struct rte_eth_dev *dev, enum rte_filter_op filter_op, void *arg);
static int ixgbe_add_5tuple_filter(struct rte_eth_dev *dev, struct ixgbe_5tuple_filter *filter);
static void ixgbe_remove_5tuple_filter(struct rte_eth_dev *dev, struct ixgbe_5tuple_filter *filter);
static int ixgbe_ntuple_filter_handle(struct rte_eth_dev *dev, enum rte_filter_op filter_op, void *arg);
static int ixgbe_get_ntuple_filter(struct rte_eth_dev *dev, struct rte_eth_ntuple_filter *filter);
static int ixgbe_ethertype_filter_handle(struct rte_eth_dev *dev, enum rte_filter_op filter_op, void *arg);
static int ixgbe_get_ethertype_filter(struct rte_eth_dev *dev, struct rte_eth_ethertype_filter *filter);
static int ixgbe_dev_filter_ctrl(struct rte_eth_dev *dev, enum rte_filter_type filter_type,
                                 enum rte_filter_op filter_op, void *arg);
static int ixgbevf_dev_set_mtu(struct rte_eth_dev *dev, uint16_t mtu);

static int ixgbe_dev_set_mc_addr_list(struct rte_eth_dev *dev, struct ether_addr *mc_addr_set, uint32_t nb_mc_addr);
static int ixgbe_dev_get_dcb_info(struct rte_eth_dev *dev, struct rte_eth_dcb_info *dcb_info);
...
static int ixgbe_get_module_info(struct rte_eth_dev *dev, struct rte_eth_dev_module_info *modinfo);
static int ixgbe_get_module_eeprom(struct rte_eth_dev *dev, struct rte_dev_eeprom_info *info);
...
static int ixgbe_timesync_read_rx_timestamp(struct rte_eth_dev *dev, struct timespec *timestamp, uint32_t flags);
static int ixgbe_timesync_read_tx_timestamp(struct rte_eth_dev *dev, struct timespec *timestamp);
static int ixgbe_timesync_adjust_time(struct rte_eth_dev *dev, int64_t delta);
static int ixgbe_timesync_read_time(struct rte_eth_dev *dev, struct timespec *timestamp);
static int ixgbe_timesync_write_time(struct rte_eth_dev *dev, const struct timespec *timestamp);
static void ixgbevf_dev_interrupt_handler(void *param);

static int ixgbe_dev_l2_tunnel_eth_type_conf(struct rte_eth_dev *dev, struct rte_eth_l2_tunnel_conf *l2_tunnel);
static int ixgbe_dev_l2_tunnel_offload_set(struct rte_eth_dev *dev, struct rte_eth_l2_tunnel_conf *l2_tunnel,
                                           uint32_t mask, uint8_t en);
static int ixgbe_dev_l2_tunnel_filter_handle(struct rte_eth_dev *dev, enum rte_filter_op filter_op, void *arg);

static int ixgbe_dev_udp_tunnel_port_add(struct rte_eth_dev *dev, struct rte_eth_udp_tunnel *udp_tunnel);
static int ixgbe_dev_udp_tunnel_port_del(struct rte_eth_dev *dev, struct rte_eth_udp_tunnel *udp_tunnel);
...
static const struct rte_ixgbe_xstats_name_off rte_ixgbe_stats_strings[] = {
    ...
    {"flow_director_added_filters", offsetof(struct ixgbe_hw_stats, fdirustat_add)},
    {"flow_director_removed_filters", offsetof(struct ixgbe_hw_stats, fdirustat_remove)},
    {"flow_director_filter_add_errors", offsetof(struct ixgbe_hw_stats, fdirfstat_fadd)},
    {"flow_director_filter_remove_errors", offsetof(struct ixgbe_hw_stats, fdirfstat_fremove)},
    {"flow_director_matched_filters", offsetof(struct ixgbe_hw_stats, fdirmatch)},
    ...
    {"rx_fcoe_no_direct_data_placement", offsetof(struct ixgbe_hw_stats, fcoe_noddp)},
    {"rx_fcoe_no_direct_data_placement_ext_buff", offsetof(struct ixgbe_hw_stats, fcoe_noddp_ext_buff)},
...
#define IXGBE_NB_HW_STATS (sizeof(rte_ixgbe_stats_strings) / sizeof(rte_ixgbe_stats_strings[0]))

/* MACsec statistics */
static const struct rte_ixgbe_xstats_name_off rte_ixgbe_macsec_strings[] = {
    {"out_pkts_untagged", offsetof(struct ixgbe_macsec_stats, out_pkts_untagged)},
    {"out_pkts_encrypted", offsetof(struct ixgbe_macsec_stats, out_pkts_encrypted)},
    {"out_pkts_protected", offsetof(struct ixgbe_macsec_stats, out_pkts_protected)},
    {"out_octets_encrypted", offsetof(struct ixgbe_macsec_stats, out_octets_encrypted)},
    {"out_octets_protected", offsetof(struct ixgbe_macsec_stats, out_octets_protected)},
    ...
    {"in_pkts_unknownsci", offsetof(struct ixgbe_macsec_stats, in_pkts_unknownsci)},
    {"in_octets_decrypted", offsetof(struct ixgbe_macsec_stats, in_octets_decrypted)},
    {"in_octets_validated", offsetof(struct ixgbe_macsec_stats, in_octets_validated)},
    {"in_pkts_unchecked", offsetof(struct ixgbe_macsec_stats, in_pkts_unchecked)},
    ...
    {"in_pkts_notusingsa", offsetof(struct ixgbe_macsec_stats, in_pkts_notusingsa)},
};

#define IXGBE_NB_MACSEC_STATS (sizeof(rte_ixgbe_macsec_strings) / sizeof(rte_ixgbe_macsec_strings[0]))
...
#define IXGBE_NB_RXQ_PRIO_STATS (sizeof(rte_ixgbe_rxq_strings) / sizeof(rte_ixgbe_rxq_strings[0]))
...
#define IXGBE_NB_TXQ_PRIO_STATS (sizeof(rte_ixgbe_txq_strings) / sizeof(rte_ixgbe_txq_strings[0]))
...
#define IXGBEVF_NB_XSTATS (sizeof(rte_ixgbevf_stats_strings) / sizeof(rte_ixgbevf_stats_strings[0]))
...
#define QMAP_FIELD_RESERVED_BITS_MASK 0x0f
    ...
    struct ixgbe_stat_mapping_registers *stat_mappings = IXGBE_DEV_PRIVATE_TO_STAT_MAPPINGS(eth_dev->data->dev_private);
    ...
    if ((hw->mac.type != ixgbe_mac_82599EB) && (hw->mac.type != ixgbe_mac_X540) && (hw->mac.type != ixgbe_mac_X550) &&
        (hw->mac.type != ixgbe_mac_X550EM_x) && (hw->mac.type != ixgbe_mac_X550EM_a))
        return -ENOSYS;

    PMD_INIT_LOG(DEBUG, "Setting port %d, %s queue_id %d to stat index %d", (int)(eth_dev->data->port_id),
                 is_rx ? "RX" : "TX", queue_id, stat_idx);
    ...
    PMD_INIT_LOG(DEBUG, "Set port %d, %s queue_id %d to stat index %d", (int)(eth_dev->data->port_id),
                 is_rx ? "RX" : "TX", queue_id, stat_idx);
    ...
    if (is_rx) {
        PMD_INIT_LOG(DEBUG, "Write 0x%x to RX IXGBE stat mapping reg:%d", stat_mappings->rqsmr[n], n);
        IXGBE_WRITE_REG(hw, IXGBE_RQSMR(n), stat_mappings->rqsmr[n]);
    } else {
        PMD_INIT_LOG(DEBUG, "Write 0x%x to TX IXGBE stat mapping reg:%d", stat_mappings->tqsm[n], n);
...
static void ixgbe_restore_statistics_mapping(struct rte_eth_dev *dev) {
    struct ixgbe_stat_mapping_registers *stat_mappings = IXGBE_DEV_PRIVATE_TO_STAT_MAPPINGS(dev->data->dev_private);
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    int i;
...
static void ixgbe_dcb_init(struct ixgbe_hw *hw, struct ixgbe_dcb_config *dcb_config) {
    ...
    for (i = 0; i < dcb_max_tc; i++) {
        ...
        tc->path[IXGBE_DCB_TX_CONFIG].bwg_percent = (uint8_t)(100 / dcb_max_tc + (i & 1));
        tc->path[IXGBE_DCB_RX_CONFIG].bwg_id = i;
        tc->path[IXGBE_DCB_RX_CONFIG].bwg_percent = (uint8_t)(100 / dcb_max_tc + (i & 1));
    ...
    if (hw->mac.type == ixgbe_mac_X540 || hw->mac.type == ixgbe_mac_X550 || hw->mac.type == ixgbe_mac_X550EM_x ||
 ...
 */
static int eth_ixgbe_dev_init(struct rte_eth_dev *eth_dev, void *init_params __rte_unused) {
    ...
    struct ixgbe_dcb_config *dcb_config = IXGBE_DEV_PRIVATE_TO_DCB_CFG(eth_dev->data->dev_private);
    struct ixgbe_filter_info *filter_info = IXGBE_DEV_PRIVATE_TO_FILTER_INFO(eth_dev->data->dev_private);
    struct ixgbe_bw_conf *bw_conf = IXGBE_DEV_PRIVATE_TO_BW_CONF(eth_dev->data->dev_private);
    uint32_t ctrl_ext;
    uint16_t csum;
    int diag, i;
    ...
    eth_dev->data->mac_addrs = rte_zmalloc("ixgbe", ETHER_ADDR_LEN * hw->mac.num_rar_entries, 0);
    ...
    ether_addr_copy((struct ether_addr *)hw->mac.perm_addr, &eth_dev->data->mac_addrs[0]);

    /* Allocate memory for storing hash filter MAC addresses */
    eth_dev->data->hash_mac_addrs = rte_zmalloc("ixgbe", ETHER_ADDR_LEN * IXGBE_VMDQ_NUM_UC_MAC, 0);
    ...
    if (ixgbe_is_sfp(hw) && hw->phy.sfp_type != ixgbe_sfp_type_not_present)
        PMD_INIT_LOG(DEBUG, "MAC: %d, PHY: %d, SFP+: %d", (int)hw->mac.type, (int)hw->phy.type, (int)hw->phy.sfp_type);
    else
        PMD_INIT_LOG(DEBUG, "MAC: %d, PHY: %d", (int)hw->mac.type, (int)hw->phy.type);

    PMD_INIT_LOG(DEBUG, "port %d vendorID=0x%x deviceID=0x%x", eth_dev->data->port_id, pci_dev->id.vendor_id,
...
static int eth_ixgbe_dev_uninit(struct rte_eth_dev *eth_dev) {
    ...
    do {
        ret = rte_intr_callback_unregister(intr_handle, ixgbe_dev_interrupt_handler, eth_dev);
        if (ret >= 0) {
            break;
        } else if (ret != -EAGAIN) {
...
static int ixgbe_ntuple_filter_uninit(struct rte_eth_dev *eth_dev) {
    ...
    memset(filter_info->fivetuple_mask, 0, sizeof(uint32_t) * IXGBE_5TUPLE_ARRAY_SIZE);

    return 0;
}

static int ixgbe_fdir_filter_uninit(struct rte_eth_dev *eth_dev) {
    struct ixgbe_hw_fdir_info *fdir_info = IXGBE_DEV_PRIVATE_TO_FDIR_INFO(eth_dev->data->dev_private);
...
static int ixgbe_l2_tn_filter_uninit(struct rte_eth_dev *eth_dev) {
    struct ixgbe_l2_tn_info *l2_tn_info = IXGBE_DEV_PRIVATE_TO_L2_TN_INFO(eth_dev->data->dev_private);
...
static int ixgbe_fdir_filter_init(struct rte_eth_dev *eth_dev) {
    ...
    fdir_info->hash_map = rte_zmalloc("ixgbe", sizeof(struct ixgbe_fdir_filter *) * IXGBE_MAX_FDIR_FILTER_NUM, 0);
    if (!fdir_info->hash_map) {
...
static int ixgbe_l2_tn_filter_init(struct rte_eth_dev *eth_dev) {
    ...
    snprintf(l2_tn_hash_name, RTE_HASH_NAMESIZE, "l2_tn_%s", eth_dev->device->name);
    l2_tn_info->hash_handle = rte_hash_create(&l2_tn_hash_params);
    if (!l2_tn_info->hash_handle) {
        PMD_INIT_LOG(ERR, "Failed to create L2 TN hash table!");
        return -EINVAL;
    }
    l2_tn_info->hash_map = rte_zmalloc("ixgbe", sizeof(struct ixgbe_l2_tn_filter *) * IXGBE_MAX_L2_TN_FILTER_NUM, 0);
    if (!l2_tn_info->hash_map) {
...
static void ixgbevf_negotiate_api(struct ixgbe_hw *hw) {
    ...
    for (i = 0; i != RTE_DIM(sup_ver) && ixgbevf_negotiate_api_version(hw, sup_ver[i]) != 0; i++)
        ;
}

static void generate_random_mac_addr(struct ether_addr *mac_addr) {
    uint64_t random;
...
static int eth_ixgbevf_dev_init(struct rte_eth_dev *eth_dev) {
    ...
    if (rte_eal_process_type() != RTE_PROC_PRIMARY) {
        ...
        if (eth_dev->data->tx_queues) {
            ...
            /* Use default TX function if we get here */
            PMD_INIT_LOG(NOTICE, "No TX queues configured yet. Using default TX function.");
        }
    ...
    /* Allocate memory for storing MAC addresses */
    eth_dev->data->mac_addrs = rte_zmalloc("ixgbevf", ETHER_ADDR_LEN * hw->mac.num_rar_entries, 0);
    if (eth_dev->data->mac_addrs == NULL) {
        PMD_INIT_LOG(ERR,
                     "Failed to allocate %u bytes needed to store "
                     "MAC addresses",
    ...
    if (is_zero_ether_addr(perm_addr)) {
        ...
        PMD_INIT_LOG(INFO,
                     ...
                     perm_addr->addr_bytes[0], perm_addr->addr_bytes[1], perm_addr->addr_bytes[2],
                     perm_addr->addr_bytes[3], perm_addr->addr_bytes[4], perm_addr->addr_bytes[5]);
    }
    ...
    PMD_INIT_LOG(DEBUG, "port %d vendorID=0x%x deviceID=0x%x mac.type=%s", eth_dev->data->port_id,
                 pci_dev->id.vendor_id, pci_dev->id.device_id, "ixgbe_mac_82599_vf");
...
static int eth_ixgbevf_dev_uninit(struct rte_eth_dev *eth_dev) {
    ...
    rte_intr_disable(intr_handle);
    rte_intr_callback_unregister(intr_handle, ixgbevf_dev_interrupt_handler, eth_dev);
...
static int eth_ixgbe_pci_probe(struct rte_pci_driver *pci_drv __rte_unused, struct rte_pci_device *pci_dev) {
    ...
    retval = rte_eth_dev_create(&pci_dev->device, pci_dev->device.name, sizeof(struct ixgbe_adapter),
    ...
    for (i = 0; i < eth_da.nb_representor_ports; i++) {
        ...
        retval = rte_eth_dev_create(&pci_dev->device, name, sizeof(struct ixgbe_vf_representor), NULL, NULL,
...
static int eth_ixgbevf_pci_probe(struct rte_pci_driver *pci_drv __rte_unused, struct rte_pci_device *pci_dev) {
    return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct ixgbe_adapter), eth_ixgbevf_dev_init);
}
...
static int ixgbe_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id, int on) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbe_vfta *shadow_vfta = IXGBE_DEV_PRIVATE_TO_VFTA(dev->data->dev_private);
    uint32_t vfta;
    uint32_t vid_idx;
    uint32_t vid_bit;
...
static int ixgbe_vlan_tpid_set(struct rte_eth_dev *dev, enum rte_vlan_type vlan_type, uint16_t tpid) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    int ret = 0;
    uint32_t reg;
    uint32_t qinq;
    ...
    case ETH_VLAN_TYPE_INNER:
        if (qinq) {
            ...
            reg = (reg & (~IXGBE_DMATXCTL_VT_MASK)) | ((uint32_t)tpid << IXGBE_DMATXCTL_VT_SHIFT);
    ...
    case ETH_VLAN_TYPE_OUTER:
        if (qinq) {
            ...
            IXGBE_WRITE_REG(hw, IXGBE_EXVET, (uint32_t)tpid << IXGBE_EXVET_VET_EXT_SHIFT);
        } else {
            reg = IXGBE_READ_REG(hw, IXGBE_VLNCTRL);
            reg = (reg & (~IXGBE_VLNCTRL_VET)) | (uint32_t)tpid;
            IXGBE_WRITE_REG(hw, IXGBE_VLNCTRL, reg);
...
static void ixgbe_vlan_hw_strip_bitmap_set(struct rte_eth_dev *dev, uint16_t queue, bool on) {
    struct ixgbe_hwstrip *hwstrip = IXGBE_DEV_PRIVATE_TO_HWSTRIP_BITMAP(dev->data->dev_private);
...
static void ixgbe_vlan_hw_extend_enable(struct rte_eth_dev *dev) {
    ...
    if (hw->mac.type == ixgbe_mac_X550 || hw->mac.type == ixgbe_mac_X550EM_x || hw->mac.type == ixgbe_mac_X550EM_a) {
...
static int ixgbe_check_vf_rss_rxq_num(struct rte_eth_dev *dev, uint16_t nb_rx_q) {
    ...
    RTE_ETH_DEV_SRIOV(dev).nb_q_per_pool = IXGBE_MAX_RX_QUEUE_NUM / RTE_ETH_DEV_SRIOV(dev).active;
...
static int ixgbe_check_mq_mode(struct rte_eth_dev *dev) {
    ...
    if (RTE_ETH_DEV_SRIOV(dev).active != 0) {
        ...
        if ((nb_rx_q > RTE_ETH_DEV_SRIOV(dev).nb_q_per_pool) || (nb_tx_q > RTE_ETH_DEV_SRIOV(dev).nb_q_per_pool)) {
            PMD_INIT_LOG(ERR,
                         "SRIOV is active,"
        ...
        if (dev_conf->rxmode.mq_mode == ETH_MQ_RX_DCB) {
            ...
            if (!(conf->nb_tcs == ETH_4_TCS || conf->nb_tcs == ETH_8_TCS)) {
                PMD_INIT_LOG(ERR,
         ...
         */
        if (dev_conf->txmode.mq_mode == ETH_MQ_TX_NONE && hw->mac.type != ixgbe_mac_82598EB) {
...
int ixgbe_set_vf_rate_limit(struct rte_eth_dev *dev, uint16_t vf, uint16_t tx_rate, uint64_t q_msk) {
    struct ixgbe_hw *hw;
    struct ixgbe_vf_info *vfinfo;
...
static int ixgbe_dev_start(struct rte_eth_dev *dev) {
    ...
    struct ixgbe_tm_conf *tm_conf = IXGBE_DEV_PRIVATE_TO_TM_CONF(dev->data->dev_private);
    ...
    if (dev->data->dev_conf.link_speeds & ETH_LINK_SPEED_FIXED) {
        PMD_INIT_LOG(ERR, "Invalid link_speeds for port %u, fix speed not supported", dev->data->port_id);
        return -EINVAL;
    }
    ...
    if ((rte_intr_cap_multiple(intr_handle) || !RTE_ETH_DEV_SRIOV(dev).active) &&
        ...
        if (intr_vector > IXGBE_MAX_INTR_QUEUE_NUM) {
            PMD_INIT_LOG(ERR, "At most %d intr queues supported", IXGBE_MAX_INTR_QUEUE_NUM);
            return -ENOTSUP;
        }
    ...
    case ixgbe_mac_X550EM_a:
        ...
        if (hw->device_id == IXGBE_DEV_ID_X550EM_A_1G_T || hw->device_id == IXGBE_DEV_ID_X550EM_A_1G_T_L)
            allowed_speeds = ETH_LINK_SPEED_10M | ETH_LINK_SPEED_100M | ETH_LINK_SPEED_1G;
        break;
    default:
        allowed_speeds = ETH_LINK_SPEED_100M | ETH_LINK_SPEED_1G | ETH_LINK_SPEED_10G;
    }
...
static void ixgbe_dev_stop(struct rte_eth_dev *dev) {
    ...
    if (!rte_intr_allow_others(intr_handle))
        ...
        rte_intr_callback_register(intr_handle, ixgbe_dev_interrupt_handler, (void *)dev);
...
static void ixgbe_read_stats_registers(struct ixgbe_hw *hw, struct ixgbe_hw_stats *hw_stats,
                                       struct ixgbe_macsec_stats *macsec_stats, uint64_t *total_missed_rx,
                                       uint64_t *total_qbrc, uint64_t *total_qprc, uint64_t *total_qprdc) {
    ...
    if (hw->mac.type != ixgbe_mac_82598EB) {
        ...
        hw_stats->fdirustat_remove += (IXGBE_READ_REG(hw, IXGBE_FDIRUSTAT) >> 16) & 0xFFFF;
        hw_stats->fdirfstat_fadd += IXGBE_READ_REG(hw, IXGBE_FDIRFSTAT) & 0xFFFF;
        hw_stats->fdirfstat_fremove += (IXGBE_READ_REG(hw, IXGBE_FDIRFSTAT) >> 16) & 0xFFFF;
    }
...
static int ixgbe_dev_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbe_hw_stats *hw_stats = IXGBE_DEV_PRIVATE_TO_STATS(dev->data->dev_private);
    struct ixgbe_macsec_stats *macsec_stats = IXGBE_DEV_PRIVATE_TO_MACSEC_STATS(dev->data->dev_private);
    ...
    ixgbe_read_stats_registers(hw, hw_stats, macsec_stats, &total_missed_rx, &total_qbrc, &total_qprc, &total_qprdc);
    ...
    stats->ierrors = hw_stats->crcerrs + hw_stats->mspdc + hw_stats->rlec + hw_stats->ruc + hw_stats->roc +
                     hw_stats->illerrc + hw_stats->errbc + hw_stats->rfc + hw_stats->fccrc + hw_stats->fclast;
...
static void ixgbe_dev_stats_reset(struct rte_eth_dev *dev) {
    struct ixgbe_hw_stats *stats = IXGBE_DEV_PRIVATE_TO_STATS(dev->data->dev_private);
...
static unsigned ixgbe_xstats_calc_num(void) {
    return IXGBE_NB_HW_STATS + IXGBE_NB_MACSEC_STATS + (IXGBE_NB_RXQ_PRIO_STATS * IXGBE_NB_RXQ_PRIO_VALUES) +
...
static int ixgbe_dev_xstats_get_names(__rte_unused struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats_names,
    ...
    if (xstats_names != NULL) {
        ...
        for (i = 0; i < IXGBE_NB_HW_STATS; i++) {
            strlcpy(xstats_names[count].name, rte_ixgbe_stats_strings[i].name, sizeof(xstats_names[count].name));
            count++;
        }

        /* MACsec Stats */
        for (i = 0; i < IXGBE_NB_MACSEC_STATS; i++) {
            strlcpy(xstats_names[count].name, rte_ixgbe_macsec_strings[i].name, sizeof(xstats_names[count].name));
            count++;
        }
        ...
        for (stat = 0; stat < IXGBE_NB_RXQ_PRIO_STATS; stat++) {
            for (i = 0; i < IXGBE_NB_RXQ_PRIO_VALUES; i++) {
                snprintf(xstats_names[count].name, sizeof(xstats_names[count].name), "rx_priority%u_%s", i,
        ...
        for (stat = 0; stat < IXGBE_NB_TXQ_PRIO_STATS; stat++) {
            for (i = 0; i < IXGBE_NB_TXQ_PRIO_VALUES; i++) {
                snprintf(xstats_names[count].name, sizeof(xstats_names[count].name), "tx_priority%u_%s", i,
...
static int ixgbe_dev_xstats_get_names_by_id(struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats_names,
    ...
    ixgbe_dev_xstats_get_names_by_id(dev, xstats_names_copy, NULL, size);
...
static int ixgbevf_dev_xstats_get_names(__rte_unused struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats_names,
                                        unsigned limit) {
    unsigned i;
    ...
    if (xstats_names != NULL)
        for (i = 0; i < IXGBEVF_NB_XSTATS; i++)
            strlcpy(xstats_names[i].name, rte_ixgbevf_stats_strings[i].name, sizeof(xstats_names[i].name));
...
static int ixgbe_dev_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats, unsigned n) {
    ...
    count = 0;
    for (i = 0; i < IXGBE_NB_HW_STATS; i++) {
        xstats[count].value = *(uint64_t *)(((char *)hw_stats) + rte_ixgbe_stats_strings[i].offset);
    ...
    /* MACsec Stats */
    for (i = 0; i < IXGBE_NB_MACSEC_STATS; i++) {
        xstats[count].value = *(uint64_t *)(((char *)macsec_stats) + rte_ixgbe_macsec_strings[i].offset);
        xstats[count].id = count;
        count++;
    }
    ...
    for (stat = 0; stat < IXGBE_NB_RXQ_PRIO_STATS; stat++) {
        for (i = 0; i < IXGBE_NB_RXQ_PRIO_VALUES; i++) {
            xstats[count].value =
                *(uint64_t *)(((char *)hw_stats) + rte_ixgbe_rxq_strings[stat].offset + (sizeof(uint64_t) * i));
            xstats[count].id = count;
            count++;
        }
    }
    ...
    for (stat = 0; stat < IXGBE_NB_TXQ_PRIO_STATS; stat++) {
        for (i = 0; i < IXGBE_NB_TXQ_PRIO_VALUES; i++) {
            xstats[count].value =
                *(uint64_t *)(((char *)hw_stats) + rte_ixgbe_txq_strings[stat].offset + (sizeof(uint64_t) * i));
            xstats[count].id = count;
            count++;
        }
    }
...
static int ixgbe_dev_xstats_get_by_id(struct rte_eth_dev *dev, const uint64_t *ids, uint64_t *values, unsigned int n) {
    if (!ids) {
        ...
        ixgbe_read_stats_registers(hw, hw_stats, macsec_stats, &total_missed_rx, &total_qbrc, &total_qprc,
                                   &total_qprdc);
        ...
        count = 0;
        for (i = 0; i < IXGBE_NB_HW_STATS; i++) {
            values[count] = *(uint64_t *)(((char *)hw_stats) + rte_ixgbe_stats_strings[i].offset);
            count++;
        }

        /* MACsec Stats */
        for (i = 0; i < IXGBE_NB_MACSEC_STATS; i++) {
            values[count] = *(uint64_t *)(((char *)macsec_stats) + rte_ixgbe_macsec_strings[i].offset);
            count++;
        }
...
static void ixgbevf_update_stats(struct rte_eth_dev *dev) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbevf_hw_stats *hw_stats = (struct ixgbevf_hw_stats *)IXGBE_DEV_PRIVATE_TO_STATS(dev->data->dev_private);
    ...
    UPDATE_VF_STAT_36BIT(IXGBE_VFGORC_LSB, IXGBE_VFGORC_MSB, hw_stats->last_vfgorc, hw_stats->vfgorc);
    ...
    UPDATE_VF_STAT_36BIT(IXGBE_VFGOTC_LSB, IXGBE_VFGOTC_MSB, hw_stats->last_vfgotc, hw_stats->vfgotc);
...
static int ixgbevf_dev_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats, unsigned n) {
    ...
    for (i = 0; i < IXGBEVF_NB_XSTATS; i++) {
        xstats[i].id = i;
        xstats[i].value = *(uint64_t *)(((char *)hw_stats) + rte_ixgbevf_stats_strings[i].offset);
    }
...
static void ixgbe_dev_info_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info) {
    ...
    dev_info->rx_offload_capa = (ixgbe_get_rx_port_offloads(dev) | dev_info->rx_queue_offload_capa);
...
static const uint32_t *ixgbe_dev_supported_ptypes_get(struct rte_eth_dev *dev) {
    static const uint32_t ptypes[] = {/* For non-vec functions,
    ...
    if (dev->rx_pkt_burst == ixgbe_recv_pkts || dev->rx_pkt_burst == ixgbe_recv_pkts_lro_single_alloc ||
        dev->rx_pkt_burst == ixgbe_recv_pkts_lro_bulk_alloc || dev->rx_pkt_burst == ixgbe_recv_pkts_bulk_alloc)
        return ptypes;

#if defined(RTE_ARCH_X86)
    if (dev->rx_pkt_burst == ixgbe_recv_pkts_vec || dev->rx_pkt_burst == ixgbe_recv_scattered_pkts_vec)
        return ptypes;
#endif
    return NULL;
}
...
static int ixgbevf_check_link(struct ixgbe_hw *hw, ixgbe_link_speed *speed, int *link_up, int wait_to_complete) {
...
int ixgbe_dev_link_update_share(struct rte_eth_dev *dev, int wait_to_complete, int vf) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct rte_eth_link link;
...
static int ixgbe_dev_interrupt_get_status(struct rte_eth_dev *dev) {
    ...
    if (hw->mac.type == ixgbe_mac_X550EM_x && hw->phy.type == ixgbe_phy_x550em_ext_t &&
...
static void ixgbe_dev_link_status_print(struct rte_eth_dev *dev) {
    ...
    if (link.link_status) {
        PMD_INIT_LOG(INFO, "Port %d: Link Up - speed %u Mbps - %s", (int)(dev->data->port_id),
                     (unsigned)link.link_speed,
                     link.link_duplex == ETH_LINK_FULL_DUPLEX ? "full-duplex" : "half-duplex");
    } else {
        PMD_INIT_LOG(INFO, " Port %d: Link Down", (int)(dev->data->port_id));
    }
    PMD_INIT_LOG(DEBUG, "PCI Address: " PCI_PRI_FMT, pci_dev->addr.domain, pci_dev->addr.bus, pci_dev->addr.devid,
...
static int ixgbe_dev_interrupt_action(struct rte_eth_dev *dev) {
    ...
    if (intr->flags & IXGBE_FLAG_NEED_LINK_UPDATE) {
        ...
        ixgbe_dev_link_status_print(dev);
        if (rte_eal_alarm_set(timeout * 1000, ixgbe_dev_interrupt_delayed_handler, (void *)dev) < 0)
...
static int ixgbe_flow_ctrl_set(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf) {
    ...
    uint32_t mflcn;
    enum ixgbe_fc_mode rte_fcmode_2_ixgbe_fcmode[] = {ixgbe_fc_none, ixgbe_fc_rx_pause, ixgbe_fc_tx_pause,
                                                      ixgbe_fc_full};
...
static int ixgbe_priority_flow_ctrl_set(struct rte_eth_dev *dev, struct rte_eth_pfc_conf *pfc_conf) {
    ...
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbe_dcb_config *dcb_config = IXGBE_DEV_PRIVATE_TO_DCB_CFG(dev->data->dev_private);
     ...
     */
    max_high_water = (rx_buf_size - ETHER_MAX_LEN) >> IXGBE_RXPBSIZE_SHIFT;
    if ((pfc_conf->fc.high_water > max_high_water) || (pfc_conf->fc.high_water <= pfc_conf->fc.low_water)) {
        PMD_INIT_LOG(ERR, "Invalid high/low water setup value in KB");
        PMD_INIT_LOG(ERR, "High_water must <= 0x%x", max_high_water);
        return -EINVAL;
    }
...
static int ixgbe_dev_rss_reta_query(struct rte_eth_dev *dev, struct rte_eth_rss_reta_entry64 *reta_conf,
    ...
    for (i = 0; i < reta_size; i += IXGBE_4_BIT_WIDTH) {
        ...
        for (j = 0; j < IXGBE_4_BIT_WIDTH; j++) {
            if (mask & (0x1 << j))
                reta_conf[idx].reta[shift + j] = ((reta >> (CHAR_BIT * j)) & IXGBE_8_BIT_MASK);
        }
    }
...
static int ixgbe_set_default_mac_addr(struct rte_eth_dev *dev, struct ether_addr *addr) {
    struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
...
bool is_ixgbe_supported(struct rte_eth_dev *dev) { return is_device_supported(dev, &rte_ixgbe_pmd); }
...
static int ixgbevf_dev_configure(struct rte_eth_dev *dev) {
    ...
    PMD_INIT_LOG(DEBUG, "Configured Virtual Function port id: %d", dev->data->port_id);
...
static uint32_t ixgbe_uta_vector(struct ixgbe_hw *hw, struct ether_addr *uc_addr) {
    uint32_t vector = 0;

    switch (hw->mac.mc_filter_type) {
    case 0: /* use bits [47:36] of the address */
        vector = ((uc_addr->addr_bytes[4] >> 4) | (((uint16_t)uc_addr->addr_bytes[5]) << 4));
        break;
    case 1: /* use bits [46:35] of the address */
        vector = ((uc_addr->addr_bytes[4] >> 3) | (((uint16_t)uc_addr->addr_bytes[5]) << 5));
        break;
    case 2: /* use bits [45:34] of the address */
        vector = ((uc_addr->addr_bytes[4] >> 2) | (((uint16_t)uc_addr->addr_bytes[5]) << 6));
        break;
    case 3: /* use bits [43:32] of the address */
        vector = ((uc_addr->addr_bytes[4]) | (((uint16_t)uc_addr->addr_bytes[5]) << 8));
        break;
...
static int ixgbe_uc_hash_table_set(struct rte_eth_dev *dev, struct ether_addr *mac_addr, uint8_t on) {
    ...
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbe_uta_info *uta_info = IXGBE_DEV_PRIVATE_TO_UTA(dev->data->dev_private);
    ...
    if (uta_info->uta_in_use > 0)
        IXGBE_WRITE_REG(hw, IXGBE_MCSTCTRL, IXGBE_MCSTCTRL_MFE | hw->mac.mc_filter_type);
    else
...
#define IXGBE_INVALID_MIRROR_TYPE(mirror_type)                                                                         \
    ((mirror_type) &                                                                                                   \
     ~(uint8_t)(ETH_MIRROR_VIRTUAL_POOL_UP | ETH_MIRROR_UPLINK_PORT | ETH_MIRROR_DOWNLINK_PORT | ETH_MIRROR_VLAN))
...
static int ixgbe_mirror_rule_set(struct rte_eth_dev *dev, struct rte_eth_mirror_conf *mirror_conf, uint8_t rule_id,
    ...
    struct ixgbe_mirror_info *mr_info = (IXGBE_DEV_PRIVATE_TO_PFDATA(dev->data->dev_private));
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    ...
    if (IXGBE_INVALID_MIRROR_TYPE(mirror_conf->rule_type)) {
        PMD_DRV_LOG(ERR, "unsupported mirror type 0x%x.", mirror_conf->rule_type);
        return -EINVAL;
    }
    ...
    if (mirror_conf->rule_type & ETH_MIRROR_VLAN) {
        ...
        for (i = 0; i < IXGBE_VLVF_ENTRIES; i++) {
            if (mirror_conf->vlan.vlan_mask & (1ULL << i)) {
                 ...
                 * index
                 */
                reg_index = ixgbe_find_vlvf_slot(hw, mirror_conf->vlan.vlan_id[i], false);
                if (reg_index < 0)
                    return -EINVAL;
                vlvf = IXGBE_READ_REG(hw, IXGBE_VLVF(reg_index));
                if ((vlvf & IXGBE_VLVF_VIEN) && ((vlvf & IXGBE_VLVF_VLANID_MASK) == mirror_conf->vlan.vlan_id[i]))
        ...
        if (on) {
            ...
            mr_info->mr_conf[rule_id].vlan.vlan_mask = mirror_conf->vlan.vlan_mask;
            for (i = 0; i < ETH_VMDQ_MAX_VLAN_FILTERS; i++) {
                if (mirror_conf->vlan.vlan_mask & (1ULL << i))
                    mr_info->mr_conf[rule_id].vlan.vlan_id[i] = mirror_conf->vlan.vlan_id[i];
            }
        } else {
            mv_lsb = 0;
            mv_msb = 0;
    ...
    if (mirror_conf->rule_type & ETH_MIRROR_VIRTUAL_POOL_UP) {
        ...
        if (on) {
            ...
            mr_info->mr_conf[rule_id].pool_mask = mirror_conf->pool_mask;
    ...
    if (mirror_conf->rule_type & ETH_MIRROR_VIRTUAL_POOL_UP) {
        ...
        IXGBE_WRITE_REG(hw, IXGBE_VMRVM(rule_id + rule_mr_offset), mp_msb);
    }
    /* write VLAN mirrror control  register */
    if (mirror_conf->rule_type & ETH_MIRROR_VLAN) {
        IXGBE_WRITE_REG(hw, IXGBE_VMRVLAN(rule_id), mv_lsb);
        IXGBE_WRITE_REG(hw, IXGBE_VMRVLAN(rule_id + rule_mr_offset), mv_msb);
    }
...
static int ixgbe_mirror_rule_reset(struct rte_eth_dev *dev, uint8_t rule_id) {
    ...
    memset(&mr_info->mr_conf[rule_id], 0, sizeof(struct rte_eth_mirror_conf));
...
static void ixgbe_set_ivar_map(struct ixgbe_hw *hw, int8_t direction, uint8_t queue, uint8_t msix_vector) {
    ...
    } else if ((hw->mac.type == ixgbe_mac_82599EB) || (hw->mac.type == ixgbe_mac_X540) ||
...
static void ixgbevf_configure_msix(struct rte_eth_dev *dev) {
    ...
    IXGBE_WRITE_REG(hw, IXGBE_VTEITR(IXGBE_MISC_VEC_ID),
                    IXGBE_EITR_INTERVAL_US(IXGBE_QUEUE_ITR_INTERVAL_DEFAULT) | IXGBE_EITR_CNT_WDIS);
}
...
static void ixgbe_configure_msix(struct rte_eth_dev *dev) {
     ...
     */
    gpie = IXGBE_READ_REG(hw, IXGBE_GPIE);
    if (!rte_intr_dp_is_en(intr_handle) && !(gpie & (IXGBE_GPIE_MSIX_MODE | IXGBE_GPIE_PBA_SUPPORT)))
        return;
    ...
    gpie = IXGBE_READ_REG(hw, IXGBE_GPIE);
    gpie |= IXGBE_GPIE_MSIX_MODE | IXGBE_GPIE_PBA_SUPPORT | IXGBE_GPIE_OCD | IXGBE_GPIE_EIAME;
    ...
    if (rte_intr_dp_is_en(intr_handle)) {
        ...
        switch (hw->mac.type) {
        case ixgbe_mac_82598EB:
            ixgbe_set_ivar_map(hw, -1, IXGBE_IVAR_OTHER_CAUSES_INDEX, IXGBE_MISC_VEC_ID);
            break;
        case ixgbe_mac_82599EB:
        case ixgbe_mac_X540:
        case ixgbe_mac_X550:
...
int ixgbe_set_queue_rate_limit(struct rte_eth_dev *dev, uint16_t queue_idx, uint16_t tx_rate) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct rte_eth_rxmode *rxmode;
    ...
    if (tx_rate != 0) {
        ...
        bcnrc_val |= ((rf_int << IXGBE_RTTBCNRC_RF_INT_SHIFT) & IXGBE_RTTBCNRC_RF_INT_MASK_M);
     ...
     * set as 0x4.
     */
    if ((rxmode->offloads & DEV_RX_OFFLOAD_JUMBO_FRAME) && (rxmode->max_rx_pkt_len >= IXGBE_MAX_JUMBO_FRAME_SIZE))
...
static int ixgbevf_add_mac_addr(struct rte_eth_dev *dev, struct ether_addr *mac_addr,
                                __attribute__((unused)) uint32_t index, __attribute__((unused)) uint32_t pool) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    int diag;
    ...
    if (diag != 0)
        PMD_DRV_LOG(ERR,
                    ...
                    mac_addr->addr_bytes[4], mac_addr->addr_bytes[5], diag);
    return diag;
}
...
static void ixgbevf_remove_mac_addr(struct rte_eth_dev *dev, uint32_t index) {
     ...
     */
    for (i = 0, mac_addr = dev->data->mac_addrs; i < hw->mac.num_rar_entries; i++, mac_addr++) {
        ...
        if (diag != 0)
            PMD_DRV_LOG(ERR,
                        ...
                        mac_addr->addr_bytes[3], mac_addr->addr_bytes[4], mac_addr->addr_bytes[5], diag);
    }
}

static int ixgbevf_set_default_mac_addr(struct rte_eth_dev *dev, struct ether_addr *addr) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
int ixgbe_syn_filter_set(struct rte_eth_dev *dev, struct rte_eth_syn_filter *filter, bool add) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbe_filter_info *filter_info = IXGBE_DEV_PRIVATE_TO_FILTER_INFO(dev->data->dev_private);
    uint32_t syn_info;
    uint32_t synqf;
    ...
    if (add) {
        if (syn_info & IXGBE_SYN_FILTER_ENABLE)
            return -EINVAL;
        synqf = (uint32_t)(((filter->queue << IXGBE_SYN_FILTER_QUEUE_SHIFT) & IXGBE_SYN_FILTER_QUEUE) |
...
static int ixgbe_syn_filter_handle(struct rte_eth_dev *dev, enum rte_filter_op filter_op, void *arg) {
    ...
    case RTE_ETH_FILTER_ADD:
        ret = ixgbe_syn_filter_set(dev, (struct rte_eth_syn_filter *)arg, TRUE);
        break;
    case RTE_ETH_FILTER_DELETE:
        ret = ixgbe_syn_filter_set(dev, (struct rte_eth_syn_filter *)arg, FALSE);
        break;
    case RTE_ETH_FILTER_GET:
        ret = ixgbe_syn_filter_get(dev, (struct rte_eth_syn_filter *)arg);
        break;
    default:
...
static inline enum ixgbe_5tuple_protocol convert_protocol_type(uint8_t protocol_value) {
...
static inline void ixgbe_inject_5tuple_filter(struct rte_eth_dev *dev, struct ixgbe_5tuple_filter *filter) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    int i;
    uint32_t ftqf, sdpqf;
    uint32_t l34timir = 0;
    ...
    ftqf |= (uint32_t)((filter->filter_info.priority & IXGBE_FTQF_PRIORITY_MASK) << IXGBE_FTQF_PRIORITY_SHIFT);
...
static void ixgbe_remove_5tuple_filter(struct rte_eth_dev *dev, struct ixgbe_5tuple_filter *filter) {
    ...
    filter_info->fivetuple_mask[index / (sizeof(uint32_t) * NBBY)] &= ~(1 << (index % (sizeof(uint32_t) * NBBY)));
...
static int ixgbevf_dev_set_mtu(struct rte_eth_dev *dev, uint16_t mtu) {
     ...
     */
    if (dev_data->dev_started && !dev_data->scattered_rx &&
        (max_frame + 2 * IXGBE_VLAN_TAG_SIZE > dev->data->min_rx_buf_size - RTE_PKTMBUF_HEADROOM)) {
        PMD_INIT_LOG(ERR, "Stop port first.");
        return -EINVAL;
    }
...
static inline struct ixgbe_5tuple_filter *ixgbe_5tuple_filter_lookup(struct ixgbe_5tuple_filter_list *filter_list,
                                                                     struct ixgbe_5tuple_filter_info *key) {
    struct ixgbe_5tuple_filter *it;

    TAILQ_FOREACH(it, filter_list, entries) {
        if (memcmp(key, &it->filter_info, sizeof(struct ixgbe_5tuple_filter_info)) == 0) {
            return it;
        }
    }
    return NULL;
...
static inline int ntuple_filter_to_5tuple(struct rte_eth_ntuple_filter *filter,
    ...
    if (filter->queue >= IXGBE_MAX_RX_QUEUE_NUM || filter->priority > IXGBE_5TUPLE_MAX_PRI ||
 ...
 *    - On failure, a negative value.
 */
int ixgbe_add_del_ntuple_filter(struct rte_eth_dev *dev, struct rte_eth_ntuple_filter *ntuple_filter, bool add) {
    struct ixgbe_filter_info *filter_info = IXGBE_DEV_PRIVATE_TO_FILTER_INFO(dev->data->dev_private);
    ...
    filter = ixgbe_5tuple_filter_lookup(&filter_info->fivetuple_list, &filter_5tuple);
    if (filter != NULL && add) {
    ...
    if (add) {
        filter = rte_zmalloc("ixgbe_5tuple_filter", sizeof(struct ixgbe_5tuple_filter), 0);
        if (filter == NULL)
            return -ENOMEM;
        rte_memcpy(&filter->filter_info, &filter_5tuple, sizeof(struct ixgbe_5tuple_filter_info));
 ...
 *    - On failure, a negative value.
 */
static int ixgbe_get_ntuple_filter(struct rte_eth_dev *dev, struct rte_eth_ntuple_filter *ntuple_filter) {
    struct ixgbe_filter_info *filter_info = IXGBE_DEV_PRIVATE_TO_FILTER_INFO(dev->data->dev_private);
    struct ixgbe_5tuple_filter_info filter_5tuple;
    struct ixgbe_5tuple_filter *filter;
    int ret;
...
static int ixgbe_ntuple_filter_handle(struct rte_eth_dev *dev, enum rte_filter_op filter_op, void *arg) {
    ...
    switch (filter_op) {
    case RTE_ETH_FILTER_ADD:
        ret = ixgbe_add_del_ntuple_filter(dev, (struct rte_eth_ntuple_filter *)arg, TRUE);
        break;
    case RTE_ETH_FILTER_DELETE:
        ret = ixgbe_add_del_ntuple_filter(dev, (struct rte_eth_ntuple_filter *)arg, FALSE);
        break;
    case RTE_ETH_FILTER_GET:
        ret = ixgbe_get_ntuple_filter(dev, (struct rte_eth_ntuple_filter *)arg);
        break;
    default:
        PMD_DRV_LOG(ERR, "unsupported operation %u.", filter_op);
        ret = -EINVAL;
...
int ixgbe_add_del_ethertype_filter(struct rte_eth_dev *dev, struct rte_eth_ethertype_filter *filter, bool add) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbe_filter_info *filter_info = IXGBE_DEV_PRIVATE_TO_FILTER_INFO(dev->data->dev_private);
    uint32_t etqf = 0;
    uint32_t etqs = 0;
    ...
    if (ret < 0 && !add) {
        PMD_DRV_LOG(ERR, "ethertype (0x%04x) filter doesn't exist.", filter->ether_type);
        return -ENOENT;
    }
    ...
    if (add) {
        ...
        etqs |= (uint32_t)((filter->queue << IXGBE_ETQS_RX_QUEUE_SHIFT) & IXGBE_ETQS_RX_QUEUE);
...
static int ixgbe_get_ethertype_filter(struct rte_eth_dev *dev, struct rte_eth_ethertype_filter *filter) {
    ...
    if (etqf & IXGBE_ETQF_FILTER_EN) {
        ...
        filter->flags = 0;
        filter->queue = (etqs & IXGBE_ETQS_RX_QUEUE) >> IXGBE_ETQS_RX_QUEUE_SHIFT;
        return 0;
    }
    return -ENOENT;
}
...
static int ixgbe_ethertype_filter_handle(struct rte_eth_dev *dev, enum rte_filter_op filter_op, void *arg) {
    ...
    switch (filter_op) {
    case RTE_ETH_FILTER_ADD:
        ret = ixgbe_add_del_ethertype_filter(dev, (struct rte_eth_ethertype_filter *)arg, TRUE);
        break;
    case RTE_ETH_FILTER_DELETE:
        ret = ixgbe_add_del_ethertype_filter(dev, (struct rte_eth_ethertype_filter *)arg, FALSE);
        break;
    case RTE_ETH_FILTER_GET:
        ret = ixgbe_get_ethertype_filter(dev, (struct rte_eth_ethertype_filter *)arg);
        break;
    default:
        PMD_DRV_LOG(ERR, "unsupported operation %u.", filter_op);
        ret = -EINVAL;
...
static u8 *ixgbe_dev_addr_list_itr(__attribute__((unused)) struct ixgbe_hw *hw, u8 **mc_addr_ptr, u32 *vmdq) {
    u8 *mc_addr;
...
static int ixgbe_dev_set_mc_addr_list(struct rte_eth_dev *dev, struct ether_addr *mc_addr_set, uint32_t nb_mc_addr) {
    ...
    return ixgbe_update_mc_addr_list(hw, mc_addr_list, nb_mc_addr, ixgbe_dev_addr_list_itr, TRUE);
}
...
static uint64_t ixgbe_read_systime_cyclecounter(struct rte_eth_dev *dev) {
    ...
    case ixgbe_mac_X550EM_a:
        ...
        systime_cycles += (uint64_t)IXGBE_READ_REG(hw, IXGBE_SYSTIMH) * NSEC_PER_SEC;
        break;
    default:
        systime_cycles = (uint64_t)IXGBE_READ_REG(hw, IXGBE_SYSTIML);
...
static uint64_t ixgbe_read_rx_tstamp_cyclecounter(struct rte_eth_dev *dev) {
    ...
    case ixgbe_mac_X550EM_a:
        ...
        rx_tstamp_cycles += (uint64_t)IXGBE_READ_REG(hw, IXGBE_RXSTMPH) * NSEC_PER_SEC;
        break;
    default:
        /* RXSTMPL stores ns and RXSTMPH stores seconds. */
        rx_tstamp_cycles = (uint64_t)IXGBE_READ_REG(hw, IXGBE_RXSTMPL);
...
static uint64_t ixgbe_read_tx_tstamp_cyclecounter(struct rte_eth_dev *dev) {
    ...
    case ixgbe_mac_X550EM_a:
        ...
        tx_tstamp_cycles += (uint64_t)IXGBE_READ_REG(hw, IXGBE_TXSTMPH) * NSEC_PER_SEC;
        break;
    default:
        /* TXSTMPL stores ns and TXSTMPH stores seconds. */
        tx_tstamp_cycles = (uint64_t)IXGBE_READ_REG(hw, IXGBE_TXSTMPL);
...
static void ixgbe_start_timecounters(struct rte_eth_dev *dev) {
    ...
    case ixgbe_mac_82599EB:
        ...
        IXGBE_WRITE_REG(hw, IXGBE_TIMINCA, (1 << IXGBE_INCPER_SHIFT_82599) | incval);
        break;
    default:
        /* Not supported. */
        return;
...
static int ixgbe_timesync_write_time(struct rte_eth_dev *dev, const struct timespec *ts) {
    uint64_t ns;
    struct ixgbe_adapter *adapter = (struct ixgbe_adapter *)dev->data->dev_private;
...
static int ixgbe_timesync_read_time(struct rte_eth_dev *dev, struct timespec *ts) {
...
static int ixgbe_timesync_read_rx_timestamp(struct rte_eth_dev *dev, struct timespec *timestamp,
...
static int ixgbe_get_reg_length(struct rte_eth_dev *dev) {
    ...
    const struct reg_info **reg_set = (hw->mac.type == ixgbe_mac_82598EB) ? ixgbe_regs_mac_82598EB : ixgbe_regs_others;
...
static int ixgbe_get_regs(struct rte_eth_dev *dev, struct rte_dev_reg_info *regs) {
    ...
    if ((regs->length == 0) || (regs->length == (uint32_t)ixgbe_get_reg_length(dev))) {
...
static int ixgbevf_get_regs(struct rte_eth_dev *dev, struct rte_dev_reg_info *regs) {
    ...
    /* Support only full register dump */
    if ((regs->length == 0) || (regs->length == (uint32_t)ixgbevf_get_reg_length(dev))) {
        regs->version = hw->mac.type << 24 | hw->revision_id << 16 | hw->device_id;
        while ((reg_group = ixgbevf_regs[g_ind++]))
            count += ixgbe_read_regs_group(dev, &data[count], reg_group);
        return 0;
...
static int ixgbe_get_eeprom(struct rte_eth_dev *dev, struct rte_dev_eeprom_info *in_eeprom) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    ...
    if ((first > hw->eeprom.word_size) || ((first + length) > hw->eeprom.word_size))
        return -EINVAL;
...
static int ixgbe_set_eeprom(struct rte_eth_dev *dev, struct rte_dev_eeprom_info *in_eeprom) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbe_eeprom_info *eeprom = &hw->eeprom;
    uint16_t *data = in_eeprom->data;
    int first, length;
...
static int ixgbe_get_module_info(struct rte_eth_dev *dev, struct rte_eth_dev_module_info *modinfo) {
    ...
    status = hw->phy.ops.read_i2c_eeprom(hw, IXGBE_SFF_SFF_8472_COMP, &sff8472_rev);
    if (status != 0)
        return -EIO;
...
static int ixgbe_dev_get_dcb_info(struct rte_eth_dev *dev, struct rte_eth_dcb_info *dcb_info) {
    ...
    if (dcb_config->vt_mode) { /* vt is enabled*/
        struct rte_eth_vmdq_dcb_conf *vmdq_rx_conf = &dev->data->dev_conf.rx_adv_conf.vmdq_dcb_conf;
...
static int ixgbe_update_e_tag_eth_type(struct ixgbe_hw *hw, uint16_t ether_type) {
    uint32_t etag_etype;
...
static int ixgbe_dev_l2_tunnel_eth_type_conf(struct rte_eth_dev *dev, struct rte_eth_l2_tunnel_conf *l2_tunnel) {
    int ret = 0;
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbe_l2_tn_info *l2_tn_info = IXGBE_DEV_PRIVATE_TO_L2_TN_INFO(dev->data->dev_private);
...
/* Enable l2 tunnel */
static int ixgbe_dev_l2_tunnel_enable(struct rte_eth_dev *dev, enum rte_eth_tunnel_type l2_tunnel_type) {
    int ret = 0;
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbe_l2_tn_info *l2_tn_info = IXGBE_DEV_PRIVATE_TO_L2_TN_INFO(dev->data->dev_private);
...
static int ixgbe_e_tag_disable(struct ixgbe_hw *hw) {
    uint32_t etag_etype;
...
/* Disable l2 tunnel */
static int ixgbe_dev_l2_tunnel_disable(struct rte_eth_dev *dev, enum rte_eth_tunnel_type l2_tunnel_type) {
    int ret = 0;
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbe_l2_tn_info *l2_tn_info = IXGBE_DEV_PRIVATE_TO_L2_TN_INFO(dev->data->dev_private);
...
static int ixgbe_e_tag_filter_del(struct rte_eth_dev *dev, struct rte_eth_l2_tunnel_conf *l2_tunnel) {
    int ret = 0;
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int ixgbe_e_tag_filter_add(struct rte_eth_dev *dev, struct rte_eth_l2_tunnel_conf *l2_tunnel) {
    int ret = 0;
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    uint32_t i, rar_entries;
    uint32_t rar_low, rar_high;
...
static inline struct ixgbe_l2_tn_filter *ixgbe_l2_tn_filter_lookup(struct ixgbe_l2_tn_info *l2_tn_info,
...
static inline int ixgbe_insert_l2_tn_filter(struct ixgbe_l2_tn_info *l2_tn_info,
                                            struct ixgbe_l2_tn_filter *l2_tn_filter) {
    int ret;
...
static inline int ixgbe_remove_l2_tn_filter(struct ixgbe_l2_tn_info *l2_tn_info, struct ixgbe_l2_tn_key *key) {
    int ret;
    struct ixgbe_l2_tn_filter *l2_tn_filter;
...
int ixgbe_dev_l2_tunnel_filter_add(struct rte_eth_dev *dev, struct rte_eth_l2_tunnel_conf *l2_tunnel, bool restore) {
    int ret;
    struct ixgbe_l2_tn_info *l2_tn_info = IXGBE_DEV_PRIVATE_TO_L2_TN_INFO(dev->data->dev_private);
    struct ixgbe_l2_tn_key key;
...
int ixgbe_dev_l2_tunnel_filter_del(struct rte_eth_dev *dev, struct rte_eth_l2_tunnel_conf *l2_tunnel) {
    int ret;
    struct ixgbe_l2_tn_info *l2_tn_info = IXGBE_DEV_PRIVATE_TO_L2_TN_INFO(dev->data->dev_private);
    struct ixgbe_l2_tn_key key;
...
static int ixgbe_dev_l2_tunnel_filter_handle(struct rte_eth_dev *dev, enum rte_filter_op filter_op, void *arg) {
    ...
    switch (filter_op) {
    case RTE_ETH_FILTER_ADD:
        ret = ixgbe_dev_l2_tunnel_filter_add(dev, (struct rte_eth_l2_tunnel_conf *)arg, FALSE);
        break;
    case RTE_ETH_FILTER_DELETE:
        ret = ixgbe_dev_l2_tunnel_filter_del(dev, (struct rte_eth_l2_tunnel_conf *)arg);
        break;
    default:
        PMD_DRV_LOG(ERR, "unsupported operation %u.", filter_op);
        ret = -EINVAL;
...
static int ixgbe_dev_l2_tunnel_forwarding_enable(struct rte_eth_dev *dev, enum rte_eth_tunnel_type l2_tunnel_type) {
    struct ixgbe_l2_tn_info *l2_tn_info = IXGBE_DEV_PRIVATE_TO_L2_TN_INFO(dev->data->dev_private);
    int ret = 0;
...
static int ixgbe_dev_l2_tunnel_forwarding_disable(struct rte_eth_dev *dev, enum rte_eth_tunnel_type l2_tunnel_type) {
    struct ixgbe_l2_tn_info *l2_tn_info = IXGBE_DEV_PRIVATE_TO_L2_TN_INFO(dev->data->dev_private);
    int ret = 0;
...
static int ixgbe_e_tag_insertion_en_dis(struct rte_eth_dev *dev, struct rte_eth_l2_tunnel_conf *l2_tunnel, bool en) {
    struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
    int ret = 0;
    ...
    if (l2_tunnel->vf_id >= pci_dev->max_vfs) {
        PMD_DRV_LOG(ERR, "VF id %u should be less than %u", l2_tunnel->vf_id, pci_dev->max_vfs);
        return -EINVAL;
    }
...
static int ixgbe_dev_l2_tunnel_insertion_enable(struct rte_eth_dev *dev, struct rte_eth_l2_tunnel_conf *l2_tunnel) {
    int ret = 0;
...
static int ixgbe_dev_l2_tunnel_insertion_disable(struct rte_eth_dev *dev, struct rte_eth_l2_tunnel_conf *l2_tunnel) {
    int ret = 0;
...
static int ixgbe_e_tag_stripping_en_dis(struct rte_eth_dev *dev, bool en) {
    int ret = 0;
    uint32_t qde;
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int ixgbe_dev_l2_tunnel_stripping_enable(struct rte_eth_dev *dev, enum rte_eth_tunnel_type l2_tunnel_type) {
    int ret = 0;
...
static int ixgbe_dev_l2_tunnel_stripping_disable(struct rte_eth_dev *dev, enum rte_eth_tunnel_type l2_tunnel_type) {
    int ret = 0;
...
static int ixgbe_dev_l2_tunnel_offload_set(struct rte_eth_dev *dev, struct rte_eth_l2_tunnel_conf *l2_tunnel,
    ...
    if (mask & ETH_L2_TUNNEL_ENABLE_MASK) {
        if (en)
            ret = ixgbe_dev_l2_tunnel_enable(dev, l2_tunnel->l2_tunnel_type);
        else
            ret = ixgbe_dev_l2_tunnel_disable(dev, l2_tunnel->l2_tunnel_type);
    }

    if (mask & ETH_L2_TUNNEL_INSERTION_MASK) {
        if (en)
            ret = ixgbe_dev_l2_tunnel_insertion_enable(dev, l2_tunnel);
        else
            ret = ixgbe_dev_l2_tunnel_insertion_disable(dev, l2_tunnel);
    }

    if (mask & ETH_L2_TUNNEL_STRIPPING_MASK) {
        if (en)
            ret = ixgbe_dev_l2_tunnel_stripping_enable(dev, l2_tunnel->l2_tunnel_type);
        else
            ret = ixgbe_dev_l2_tunnel_stripping_disable(dev, l2_tunnel->l2_tunnel_type);
    }

    if (mask & ETH_L2_TUNNEL_FORWARDING_MASK) {
        if (en)
            ret = ixgbe_dev_l2_tunnel_forwarding_enable(dev, l2_tunnel->l2_tunnel_type);
        else
            ret = ixgbe_dev_l2_tunnel_forwarding_disable(dev, l2_tunnel->l2_tunnel_type);
    }
...
static inline void ixgbe_ntuple_filter_restore(struct rte_eth_dev *dev) {
    ...
    TAILQ_FOREACH(node, &filter_info->fivetuple_list, entries) { ixgbe_inject_5tuple_filter(dev, node); }
}
...
static inline void ixgbe_ethertype_filter_restore(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i < IXGBE_MAX_ETQF_FILTERS; i++) {
        if (filter_info->ethertype_mask & (1 << i)) {
            IXGBE_WRITE_REG(hw, IXGBE_ETQF(i), filter_info->ethertype_filters[i].etqf);
            IXGBE_WRITE_REG(hw, IXGBE_ETQS(i), filter_info->ethertype_filters[i].etqs);
            IXGBE_WRITE_FLUSH(hw);
        }
    }
}
...
void ixgbe_clear_all_ethertype_filter(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i < IXGBE_MAX_ETQF_FILTERS; i++) {
        if (filter_info->ethertype_mask & (1 << i) && !filter_info->ethertype_filters[i].conf) {
