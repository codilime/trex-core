...
#define IXGBE_WAIT_RREAD                                                                                               \
    IXGBE_WRITE_REG_THEN_POLL_MASK(hw, IXGBE_IPSRXIDX, reg_val, IPSRXIDX_READ, RTE_IXGBE_REGISTER_POLL_WAIT_5_MS)
#define IXGBE_WAIT_RWRITE                                                                                              \
    IXGBE_WRITE_REG_THEN_POLL_MASK(hw, IXGBE_IPSRXIDX, reg_val, IPSRXIDX_WRITE, RTE_IXGBE_REGISTER_POLL_WAIT_5_MS)
#define IXGBE_WAIT_TREAD                                                                                               \
    IXGBE_WRITE_REG_THEN_POLL_MASK(hw, IXGBE_IPSTXIDX, reg_val, IPSRXIDX_READ, RTE_IXGBE_REGISTER_POLL_WAIT_5_MS)
#define IXGBE_WAIT_TWRITE                                                                                              \
    IXGBE_WRITE_REG_THEN_POLL_MASK(hw, IXGBE_IPSTXIDX, reg_val, IPSRXIDX_WRITE, RTE_IXGBE_REGISTER_POLL_WAIT_5_MS)

#define CMP_IP(a, b)                                                                                                   \
    ((a).ipv6[0] == (b).ipv6[0] && (a).ipv6[1] == (b).ipv6[1] && (a).ipv6[2] == (b).ipv6[2] &&                         \
     (a).ipv6[3] == (b).ipv6[3])
...
static int ixgbe_crypto_add_sa(struct ixgbe_crypto_session *ic_session) {
    ...
    if (ic_session->op == IXGBE_OP_AUTHENTICATED_DECRYPTION) {
        ...
        reg_val = IPSRXIDX_RX_EN | IPSRXIDX_WRITE | IPSRXIDX_TABLE_IP | (ip_index << 3);
        if (priv->rx_ip_tbl[ip_index].ip.type == IPv4) {
            IXGBE_WRITE_REG(hw, IXGBE_IPSRXIPADDR(0), 0);
            IXGBE_WRITE_REG(hw, IXGBE_IPSRXIPADDR(1), 0);
            IXGBE_WRITE_REG(hw, IXGBE_IPSRXIPADDR(2), 0);
            IXGBE_WRITE_REG(hw, IXGBE_IPSRXIPADDR(3), priv->rx_ip_tbl[ip_index].ip.ipv4);
        } else {
            IXGBE_WRITE_REG(hw, IXGBE_IPSRXIPADDR(0), priv->rx_ip_tbl[ip_index].ip.ipv6[0]);
            IXGBE_WRITE_REG(hw, IXGBE_IPSRXIPADDR(1), priv->rx_ip_tbl[ip_index].ip.ipv6[1]);
            IXGBE_WRITE_REG(hw, IXGBE_IPSRXIPADDR(2), priv->rx_ip_tbl[ip_index].ip.ipv6[2]);
            IXGBE_WRITE_REG(hw, IXGBE_IPSRXIPADDR(3), priv->rx_ip_tbl[ip_index].ip.ipv6[3]);
        }
        IXGBE_WAIT_RWRITE;

        /* write SPI table entry*/
        reg_val = IPSRXIDX_RX_EN | IPSRXIDX_WRITE | IPSRXIDX_TABLE_SPI | (sa_index << 3);
        ...
        reg_val = IPSRXIDX_RX_EN | IPSRXIDX_WRITE | IPSRXIDX_TABLE_KEY | (sa_index << 3);
        IXGBE_WRITE_REG(hw, IXGBE_IPSRXKEY(0), rte_cpu_to_be_32(*(uint32_t *)&ic_session->key[12]));
        IXGBE_WRITE_REG(hw, IXGBE_IPSRXKEY(1), rte_cpu_to_be_32(*(uint32_t *)&ic_session->key[8]));
        IXGBE_WRITE_REG(hw, IXGBE_IPSRXKEY(2), rte_cpu_to_be_32(*(uint32_t *)&ic_session->key[4]));
        IXGBE_WRITE_REG(hw, IXGBE_IPSRXKEY(3), rte_cpu_to_be_32(*(uint32_t *)&ic_session->key[0]));
        ...
        IXGBE_WRITE_REG(hw, IXGBE_IPSTXKEY(0), rte_cpu_to_be_32(*(uint32_t *)&ic_session->key[12]));
        IXGBE_WRITE_REG(hw, IXGBE_IPSTXKEY(1), rte_cpu_to_be_32(*(uint32_t *)&ic_session->key[8]));
        IXGBE_WRITE_REG(hw, IXGBE_IPSTXKEY(2), rte_cpu_to_be_32(*(uint32_t *)&ic_session->key[4]));
        IXGBE_WRITE_REG(hw, IXGBE_IPSTXKEY(3), rte_cpu_to_be_32(*(uint32_t *)&ic_session->key[0]));
...
static int ixgbe_crypto_remove_sa(struct rte_eth_dev *dev, struct ixgbe_crypto_session *ic_session) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbe_ipsec *priv = IXGBE_DEV_PRIVATE_TO_IPSEC(dev->data->dev_private);
    uint32_t reg_val;
    int sa_index = -1;
...
static int ixgbe_crypto_create_session(void *device, struct rte_security_session_conf *conf,
...
static int ixgbe_crypto_remove_session(void *device, struct rte_security_session *session) {
    struct rte_eth_dev *eth_dev = device;
    struct ixgbe_crypto_session *ic_session = (struct ixgbe_crypto_session *)get_sec_session_private_data(session);
...
static inline uint8_t ixgbe_crypto_compute_pad_len(struct rte_mbuf *m) {
    if (m->nb_segs == 1) {
         ...
         */
        uint8_t *esp_pad_len =
            rte_pktmbuf_mtod_offset(m, uint8_t *, rte_pktmbuf_pkt_len(m) - (ESP_TRAILER_SIZE + ESP_ICV_SIZE));
        return *esp_pad_len + ESP_TRAILER_SIZE + ESP_ICV_SIZE;
    }
    return 0;
}

static int ixgbe_crypto_update_mb(void *device __rte_unused, struct rte_security_session *session, struct rte_mbuf *m,
                                  void *params __rte_unused) {
    struct ixgbe_crypto_session *ic_session = get_sec_session_private_data(session);
    if (ic_session->op == IXGBE_OP_AUTHENTICATED_ENCRYPTION) {
        union ixgbe_crypto_tx_desc_md *mdata = (union ixgbe_crypto_tx_desc_md *)&m->udata64;
        mdata->enc = 1;
...
static const struct rte_security_capability *ixgbe_crypto_capabilities_get(void *device __rte_unused) {
    static const struct rte_cryptodev_capabilities aes_gcm_gmac_crypto_capabilities[] = {
        {/* AES GMAC (128-bit) */
...
int ixgbe_crypto_add_ingress_sa_from_flow(const void *sess, const void *ip_spec, uint8_t is_ipv6) {
...
static struct rte_security_ops ixgbe_security_ops = {.session_create = ixgbe_crypto_create_session,
                                                     .session_update = NULL,
...
int ixgbe_ipsec_ctx_create(struct rte_eth_dev *dev) {
    ...
    if (ixgbe_crypto_capable(dev)) {
        ctx = rte_malloc("rte_security_instances_ops", sizeof(struct rte_security_ctx), 0);
        if (ctx) {
            ctx->device = (void *)dev;
