...
static inline int ixgbe_vf_perm_addr_gen(struct rte_eth_dev *dev, uint16_t vf_num) {
    ...
    for (vfn = 0; vfn < vf_num; vfn++) {
        ...
        memcpy(vfinfo[vfn].vf_mac_addresses, vf_mac_addr, ETHER_ADDR_LEN);
    }
...
void ixgbe_pf_host_init(struct rte_eth_dev *eth_dev) {
    struct ixgbe_vf_info **vfinfo = IXGBE_DEV_PRIVATE_TO_P_VFDATA(eth_dev->data->dev_private);
    struct ixgbe_mirror_info *mirror_info = IXGBE_DEV_PRIVATE_TO_PFDATA(eth_dev->data->dev_private);
    struct ixgbe_uta_info *uta_info = IXGBE_DEV_PRIVATE_TO_UTA(eth_dev->data->dev_private);
...
static void ixgbe_add_tx_flow_control_drop_filter(struct rte_eth_dev *eth_dev) {
    ...
    ethertype_filter.etqf = IXGBE_ETQF_FILTER_EN | IXGBE_ETQF_TX_ANTISPOOF | IXGBE_ETHERTYPE_FLOW_CTRL;
    ...
    IXGBE_WRITE_REG(hw, IXGBE_ETQF(i), (IXGBE_ETQF_FILTER_EN | IXGBE_ETQF_TX_ANTISPOOF | IXGBE_ETHERTYPE_FLOW_CTRL));
...
static void set_rx_mode(struct rte_eth_dev *dev) {
    ...
    if (hw->mac.type != ixgbe_mac_82598EB) {
        vmolr |= IXGBE_READ_REG(hw, IXGBE_VMOLR(vfn)) & ~(IXGBE_VMOLR_MPE | IXGBE_VMOLR_ROMPE | IXGBE_VMOLR_ROPE);
...
static int ixgbe_vf_reset(struct rte_eth_dev *dev, uint16_t vf, uint32_t *msgbuf) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbe_vf_info *vfinfo = *(IXGBE_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
...
static int ixgbe_vf_set_mac_addr(struct rte_eth_dev *dev, uint32_t vf, uint32_t *msgbuf) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbe_vf_info *vfinfo = *(IXGBE_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
    int rar_entry = hw->mac.num_rar_entries - (vf + 1);
    uint8_t *new_mac = (uint8_t *)(&msgbuf[1]);
...
static int ixgbe_vf_set_multicast(struct rte_eth_dev *dev, uint32_t vf, uint32_t *msgbuf) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbe_vf_info *vfinfo = *(IXGBE_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
    int nb_entries = (msgbuf[0] & IXGBE_VT_MSGINFO_MASK) >> IXGBE_VT_MSGINFO_SHIFT;
    ...
    for (i = 0; i < vfinfo->num_vf_mc_hashes; i++) {
        mta_idx = (vfinfo->vf_mc_hashes[i] >> IXGBE_MTA_BIT_SHIFT) & IXGBE_MTA_INDEX_MASK;
...
static int ixgbe_vf_set_vlan(struct rte_eth_dev *dev, uint32_t vf, uint32_t *msgbuf) {
    int add, vid;
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    struct ixgbe_vf_info *vfinfo = *(IXGBE_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
...
static int ixgbe_set_vf_lpe(struct rte_eth_dev *dev, __rte_unused uint32_t vf, uint32_t *msgbuf) {
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    ...
    max_frs = (IXGBE_READ_REG(hw, IXGBE_MAXFRS) & IXGBE_MHADD_MFS_MASK) >> IXGBE_MHADD_MFS_SHIFT;
    if (max_frs < new_mtu) {
...
static int ixgbe_negotiate_vf_api(struct rte_eth_dev *dev, uint32_t vf, uint32_t *msgbuf) {
...
static int ixgbe_get_vf_queues(struct rte_eth_dev *dev, uint32_t vf, uint32_t *msgbuf) {
    struct ixgbe_vf_info *vfinfo = *IXGBE_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private);
...
#define VLAN_PRIO_SHIFT 13
    ...
    case ETH_MQ_TX_VMDQ_ONLY:
        ...
        if ((vlana == IXGBE_VMVIR_VLANA_DEFAULT) && ((vid != 0) || (user_priority != 0)))
            num_tcs = 1;
        else
            num_tcs = 0;
        break;
...
static int ixgbe_set_vf_mc_promisc(struct rte_eth_dev *dev, uint32_t vf, uint32_t *msgbuf) {
    struct ixgbe_vf_info *vfinfo = *(IXGBE_DEV_PRIVATE_TO_P_VFDATA(dev->data->dev_private));
    struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
    ...
    case IXGBEVF_XCAST_MODE_NONE:
        disable = IXGBE_VMOLR_BAM | IXGBE_VMOLR_ROMPE | IXGBE_VMOLR_MPE | IXGBE_VMOLR_UPE | IXGBE_VMOLR_VPE;
        enable = 0;
        break;
    ...
    case IXGBEVF_XCAST_MODE_PROMISC:
        ...
        if (!(fctrl & IXGBE_FCTRL_UPE)) {
            ...
            RTE_LOG(ERR, PMD, "Enabling VF promisc requires PF in promisc\n");
            return -1;
        }

        disable = 0;
        enable = IXGBE_VMOLR_BAM | IXGBE_VMOLR_ROMPE | IXGBE_VMOLR_MPE | IXGBE_VMOLR_UPE | IXGBE_VMOLR_VPE;
        break;
    default:
        return -1;
    }
