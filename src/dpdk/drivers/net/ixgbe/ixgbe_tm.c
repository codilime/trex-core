...
static int ixgbe_tm_capabilities_get(struct rte_eth_dev *dev, struct rte_tm_capabilities *cap,
                                     struct rte_tm_error *error);
static int ixgbe_shaper_profile_add(struct rte_eth_dev *dev, uint32_t shaper_profile_id,
                                    struct rte_tm_shaper_params *profile, struct rte_tm_error *error);
static int ixgbe_shaper_profile_del(struct rte_eth_dev *dev, uint32_t shaper_profile_id, struct rte_tm_error *error);
static int ixgbe_node_add(struct rte_eth_dev *dev, uint32_t node_id, uint32_t parent_node_id, uint32_t priority,
                          uint32_t weight, uint32_t level_id, struct rte_tm_node_params *params,
                          struct rte_tm_error *error);
static int ixgbe_node_delete(struct rte_eth_dev *dev, uint32_t node_id, struct rte_tm_error *error);
static int ixgbe_node_type_get(struct rte_eth_dev *dev, uint32_t node_id, int *is_leaf, struct rte_tm_error *error);
static int ixgbe_level_capabilities_get(struct rte_eth_dev *dev, uint32_t level_id,
                                        struct rte_tm_level_capabilities *cap, struct rte_tm_error *error);
static int ixgbe_node_capabilities_get(struct rte_eth_dev *dev, uint32_t node_id, struct rte_tm_node_capabilities *cap,
                                       struct rte_tm_error *error);
static int ixgbe_hierarchy_commit(struct rte_eth_dev *dev, int clear_on_fail, struct rte_tm_error *error);
...
static inline uint8_t ixgbe_tc_nb_get(struct rte_eth_dev *dev) {
    ...
    if (eth_conf->txmode.mq_mode == ETH_MQ_TX_DCB) {
        ...
        if (eth_conf->tx_adv_conf.vmdq_dcb_tx_conf.nb_queue_pools == ETH_32_POOLS)
            nb_tcs = ETH_4_TCS;
        else
            nb_tcs = ETH_8_TCS;
    } else {
...
static inline struct ixgbe_tm_shaper_profile *ixgbe_shaper_profile_search(struct rte_eth_dev *dev,
                                                                          uint32_t shaper_profile_id) {
    struct ixgbe_tm_conf *tm_conf = IXGBE_DEV_PRIVATE_TO_TM_CONF(dev->data->dev_private);
    struct ixgbe_shaper_profile_list *shaper_profile_list = &tm_conf->shaper_profile_list;
    struct ixgbe_tm_shaper_profile *shaper_profile;
...
static int ixgbe_shaper_profile_param_check(struct rte_tm_shaper_params *profile, struct rte_tm_error *error) {
...
static int ixgbe_shaper_profile_add(struct rte_eth_dev *dev, uint32_t shaper_profile_id,
    ...
    shaper_profile = rte_zmalloc("ixgbe_tm_shaper_profile", sizeof(struct ixgbe_tm_shaper_profile), 0);
    if (!shaper_profile)
        return -ENOMEM;
    shaper_profile->shaper_profile_id = shaper_profile_id;
    rte_memcpy(&shaper_profile->profile, profile, sizeof(struct rte_tm_shaper_params));
...
static inline struct ixgbe_tm_node *ixgbe_tm_node_search(struct rte_eth_dev *dev, uint32_t node_id,
...
static void ixgbe_queue_base_nb_get(struct rte_eth_dev *dev, uint16_t tc_node_no, uint16_t *base, uint16_t *nb) {
...
static int ixgbe_node_param_check(struct rte_eth_dev *dev, uint32_t node_id, uint32_t priority, uint32_t weight,
    ...
    if (node_id >= dev->data->nb_tx_queues) {
        ...
        if (params->nonleaf.n_sp_priorities != 1) {
            ...
            return -EINVAL;
        } else if (params->nonleaf.wfq_weight_mode && !(*params->nonleaf.wfq_weight_mode)) {
            error->type = RTE_TM_ERROR_TYPE_NODE_PARAMS_WFQ_WEIGHT_MODE;
...
static int ixgbe_node_add(struct rte_eth_dev *dev, uint32_t node_id, uint32_t parent_node_id, uint32_t priority,
    ...
    if (params->shaper_profile_id != RTE_TM_SHAPER_PROFILE_ID_NONE) {
        shaper_profile = ixgbe_shaper_profile_search(dev, params->shaper_profile_id);
        if (!shaper_profile) {
    ...
    if (parent_node_id == RTE_TM_NODE_ID_NULL) {
        /* check level */
        if (level_id != RTE_TM_NODE_LEVEL_ID_ANY && level_id > IXGBE_TM_NODE_TYPE_PORT) {
    ...
    if (parent_node_type != IXGBE_TM_NODE_TYPE_PORT && parent_node_type != IXGBE_TM_NODE_TYPE_TC) {
        error->type = RTE_TM_ERROR_TYPE_NODE_PARENT_NODE_ID;
        error->message = "parent is not port or TC";
        return -EINVAL;
    }
    /* check level */
    if (level_id != RTE_TM_NODE_LEVEL_ID_ANY && level_id != parent_node_type + 1) {
        error->type = RTE_TM_ERROR_TYPE_NODE_PARAMS;
        error->message = "Wrong level";
        return -EINVAL;
    }
...
static int ixgbe_hierarchy_commit(struct rte_eth_dev *dev, int clear_on_fail, struct rte_tm_error *error) {
    ...
    if (tm_conf->root->shaper_profile && tm_conf->root->shaper_profile->profile.peak.rate) {
        error->type = RTE_TM_ERROR_TYPE_SHAPER_PROFILE;
