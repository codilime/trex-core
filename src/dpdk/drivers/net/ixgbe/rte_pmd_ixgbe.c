...
int rte_pmd_ixgbe_set_all_queues_drop_en(uint16_t port, uint8_t on) {
    ...
    for (i = 0; i <= num_queues; i++) {
        reg_value = IXGBE_QDE_WRITE | (i << IXGBE_QDE_IDX_SHIFT) | (on & IXGBE_QDE_ENABLE);
...
int rte_pmd_ixgbe_set_vf_vlan_stripq(uint16_t port, uint16_t vf, uint8_t on) {
    ...
    if (hw->mac.type == ixgbe_mac_82598EB)
        queues_per_pool = (uint16_t)hw->mac.max_rx_queues / ETH_16_POOLS;
    else
        queues_per_pool = (uint16_t)hw->mac.max_rx_queues / ETH_64_POOLS;
...
int rte_pmd_ixgbe_macsec_select_txsa(uint16_t port, uint8_t idx, uint8_t an, uint32_t pn, uint8_t *key) {
    ...
    if (idx == 0) {
        ...
        for (i = 0; i < 4; i++) {
            ctrl = (key[i * 4 + 0] << 0) | (key[i * 4 + 1] << 8) | (key[i * 4 + 2] << 16) | (key[i * 4 + 3] << 24);
...
int rte_pmd_ixgbe_set_tc_bw_alloc(uint16_t port, uint8_t tc_num, uint8_t *bw_weight) {
    ...
    if (tc_num > IXGBE_DCB_MAX_TRAFFIC_CLASS) {
        PMD_DRV_LOG(ERR, "TCs should be no more than %d.", IXGBE_DCB_MAX_TRAFFIC_CLASS);
        return -EINVAL;
    }
...
int rte_pmd_ixgbe_bypass_event_show(uint16_t port_id, uint32_t event, uint32_t *state) {
    struct rte_eth_dev *dev;
...
int rte_pmd_ixgbe_bypass_event_store(uint16_t port_id, uint32_t event, uint32_t state) {
    struct rte_eth_dev *dev;
...
int rte_pmd_ixgbe_bypass_wd_timeout_show(uint16_t port_id, uint32_t *wd_timeout) {
    struct rte_eth_dev *dev;
...
int __rte_experimental rte_pmd_ixgbe_mdio_unlocked_read(uint16_t port, uint32_t reg_addr, uint32_t dev_type,
                                                        uint16_t *phy_data) {
    struct ixgbe_hw *hw;
    struct rte_eth_dev *dev;
    u32 i, data, command;
    ...
    command = (reg_addr << IXGBE_MSCA_DEV_TYPE_SHIFT) | (dev_type << IXGBE_MSCA_PHY_ADDR_SHIFT) |
              IXGBE_MSCA_OLD_PROTOCOL | IXGBE_MSCA_READ_AUTOINC | IXGBE_MSCA_MDI_COMMAND;
...
int __rte_experimental rte_pmd_ixgbe_mdio_unlocked_write(uint16_t port, uint32_t reg_addr, uint32_t dev_type,
                                                         uint16_t phy_data) {
    struct ixgbe_hw *hw;
    u32 i, command;
    struct rte_eth_dev *dev;
    ...
    command = (reg_addr << IXGBE_MSCA_DEV_TYPE_SHIFT) | (dev_type << IXGBE_MSCA_PHY_ADDR_SHIFT) |
              IXGBE_MSCA_OLD_PROTOCOL | IXGBE_MSCA_WRITE | IXGBE_MSCA_MDI_COMMAND;
