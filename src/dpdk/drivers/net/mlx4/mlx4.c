...
static int mlx4_init_shared_data(void) {
    ...
    if (mlx4_shared_data == NULL) {
        if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
            ...
            mz = rte_memzone_reserve(MZ_MLX4_PMD_SHARED_DATA, sizeof(*mlx4_shared_data), SOCKET_ID_ANY, 0);
            if (mz == NULL) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_proc_priv_init(struct rte_eth_dev *dev) {
     ...
     */
    ppriv_size = sizeof(struct mlx4_proc_priv) + dev->data->nb_tx_queues * sizeof(void *);
    ppriv = rte_malloc_socket("mlx4_proc_priv", ppriv_size, RTE_CACHE_LINE_SIZE, dev->device->numa_node);
    if (!ppriv) {
        rte_errno = ENOMEM;
        return -rte_errno;
    }
 ...
 *   Pointer to Ethernet device structure.
 */
static void mlx4_proc_priv_uninit(struct rte_eth_dev *dev) {
...
static int mlx4_dev_configure(struct rte_eth_dev *dev) {
    ...
    if (ret) {
        ERROR("cannot set up internal flow rules (code %d, \"%s\"),"
              ...
              -ret, strerror(-ret), error.type, error.cause, error.message ? error.message : "(unspecified)");
        goto exit;
    }
...
static int mlx4_dev_start(struct rte_eth_dev *dev) {
    ...
    if (ret) {
        ERROR("%p: cannot initialize RSS resources: %s", (void *)dev, strerror(-ret));
        goto err;
    }
#ifndef NDEBUG
 ...
 *   Pointer to Ethernet device structure.
 */
static void mlx4_dev_close(struct rte_eth_dev *dev) {
    struct mlx4_priv *priv = dev->data->dev_private;
    unsigned int i;

    DEBUG("%p: closing device \"%s\"", (void *)dev, ((priv->ctx != NULL) ? priv->ctx->device->name : ""));
    dev->rx_pkt_burst = mlx4_rx_burst_removed;
    dev->tx_pkt_burst = mlx4_tx_burst_removed;
    rte_wmb();
    /* Disable datapath on secondary process. */
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
static int mlx4_ibv_device_to_pci_addr(const struct ibv_device *device, struct rte_pci_addr *pci_addr) {
    FILE *file;
    char line[32];
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
static int mlx4_arg_parse(const char *key, const char *val, struct mlx4_conf *conf) {
    unsigned long tmp;
...
static int mlx4_args(struct rte_devargs *devargs, struct mlx4_conf *conf) {
    ...
    for (i = 0; pmd_mlx4_init_params[i]; ++i) {
        ...
        while (arg_count-- > 0) {
            ret = rte_kvargs_process(kvlist, pmd_mlx4_init_params[i],
 ...
 */
static uint64_t mlx4_hw_rss_sup(struct ibv_context *ctx, struct ibv_pd *pd, struct ibv_device_attr_ex *device_attr_ex) {
    ...
    if (!hw_rss_sup) {
        ...
        return IBV_RX_HASH_SRC_IPV4 | IBV_RX_HASH_DST_IPV4 | IBV_RX_HASH_SRC_IPV6 | IBV_RX_HASH_DST_IPV6 |
    ...
    wq = cq ? mlx4_glue->create_wq(ctx,
            ...
            : NULL;
    ind = wq ? mlx4_glue->create_rwq_ind_table(ctx,
    ...
    qp = ind ? mlx4_glue->create_qp_ex(
                   ...
                   &(struct ibv_qp_init_attr_ex){
                       .comp_mask = (IBV_QP_INIT_ATTR_PD | IBV_QP_INIT_ATTR_RX_HASH | IBV_QP_INIT_ATTR_IND_TABLE),
...
static int mlx4_init_once(void) {
    ...
    case RTE_PROC_PRIMARY:
        ...
        rte_mem_event_callback_register("MLX4_MEM_EVENT_CB", mlx4_mr_mem_event_cb, NULL);
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
static int mlx4_pci_probe(struct rte_pci_driver *pci_drv, struct rte_pci_device *pci_dev) {
    ...
    while (i != 0) {
        ...
        INFO("PCI information matches, using device \"%s\" (VF: %s)", list[i]->name, (vf ? "true" : "false"));
    ...
    for (i = 0; i < device_attr.phys_port_cnt; i++) {
        ...
        snprintf(name, sizeof(name), "%s port %u", mlx4_glue->get_device_name(ibv_dev), port);
        ...
        if (port_attr.state != IBV_PORT_ACTIVE)
            DEBUG("port %d is not active: \"%s\" (%d)", port, mlx4_glue->port_state_str(port_attr.state),
                  port_attr.state);
        ...
        priv->hw_csum_l2tun = priv->hw_csum && (device_attr.vendor_part_id == PCI_DEVICE_ID_MELLANOX_CONNECTX3PRO);
        DEBUG("L2 tunnel checksum offloads are %ssupported", priv->hw_csum_l2tun ? "" : "not ");
        ...
        priv->hw_rss_max_qps = device_attr_ex.rss_caps.max_rwq_indirection_table_size;
        DEBUG("MAX RSS queues %d", priv->hw_rss_max_qps);
        priv->hw_fcs_strip = !!(device_attr_ex.raw_packet_caps & IBV_RAW_PACKET_CAP_SCATTER_FCS);
        DEBUG("FCS stripping toggling is %ssupported", priv->hw_fcs_strip ? "" : "not ");
        ...
        INFO("port %u MAC address is %02x:%02x:%02x:%02x:%02x:%02x", priv->port, mac.addr_bytes[0], mac.addr_bytes[1],
             mac.addr_bytes[2], mac.addr_bytes[3], mac.addr_bytes[4], mac.addr_bytes[5]);
        ...
        err = mlx4_glue->dv_set_context_attr(ctx, MLX4DV_SET_CTX_ATTR_BUF_ALLOCATORS, (void *)((uintptr_t)&alctr));
        if (err)
         ...
         */
        err = mlx4_mr_btree_init(&priv->mr.cache, MLX4_MR_BTREE_CACHE_N * 2, eth_dev->device->numa_node);
        if (err) {
...
static const struct rte_pci_id mlx4_pci_id_map[] = {
    ...
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_MELLANOX, PCI_DEVICE_ID_MELLANOX_CONNECTX3PRO)},
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_MELLANOX, PCI_DEVICE_ID_MELLANOX_CONNECTX3VF)},
    {.vendor_id = 0}};
...
static int mlx4_glue_init(void) {
    ...
    const char *path[] = {
         ...
         */
        (geteuid() == getuid() && getegid() == getgid() ? getenv("MLX4_GLUE_PATH") : NULL),
        /*
...
RTE_INIT(rte_mlx4_pmd_init) {
    ...
    if (strcmp(mlx4_glue->version, MLX4_GLUE_VERSION)) {
        ERROR("rdma-core glue \"%s\" mismatch: \"%s\" is required", mlx4_glue->version, MLX4_GLUE_VERSION);
        return;
    }
    mlx4_glue->fork_init();
