...
int mlx4_get_ifname(const struct mlx4_priv *priv, char (*ifname)[IF_NAMESIZE]) {
    ...
    while ((dent = readdir(dir)) != NULL) {
        ...
        MKSTR(path, "%s/device/net/%s/%s", priv->ctx->device->ibdev_path, name, (dev_type ? "dev_id" : "dev_port"));
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
static int mlx4_ifreq(const struct mlx4_priv *priv, int req, struct ifreq *ifr) {
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
static int mlx4_set_flags(struct mlx4_priv *priv, unsigned int keep, unsigned int flags) {
    struct ifreq request;
 ...
 *   Toggle to set.
 */
static void mlx4_rxmode_toggle(struct rte_eth_dev *dev, enum rxmode_toggle toggle) {
    struct mlx4_priv *priv = dev->data->dev_private;
    const char *mode;
 ...
 *   Pointer to Ethernet device structure.
 */
void mlx4_promiscuous_enable(struct rte_eth_dev *dev) { mlx4_rxmode_toggle(dev, RXMODE_TOGGLE_PROMISC_ON); }
 ...
 *   Pointer to Ethernet device structure.
 */
void mlx4_promiscuous_disable(struct rte_eth_dev *dev) { mlx4_rxmode_toggle(dev, RXMODE_TOGGLE_PROMISC_OFF); }
 ...
 *   Pointer to Ethernet device structure.
 */
void mlx4_allmulticast_enable(struct rte_eth_dev *dev) { mlx4_rxmode_toggle(dev, RXMODE_TOGGLE_ALLMULTI_ON); }
 ...
 *   Pointer to Ethernet device structure.
 */
void mlx4_allmulticast_disable(struct rte_eth_dev *dev) { mlx4_rxmode_toggle(dev, RXMODE_TOGGLE_ALLMULTI_OFF); }
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
int mlx4_mac_addr_add(struct rte_eth_dev *dev, struct ether_addr *mac_addr, uint32_t index, uint32_t vmdq) {
    struct mlx4_priv *priv = dev->data->dev_private;
    struct rte_flow_error error;
    int ret;
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
int mlx4_set_mc_addr_list(struct rte_eth_dev *dev, struct ether_addr *list, uint32_t num) {
    struct mlx4_priv *priv = dev->data->dev_private;
    struct rte_flow_error error;
    int ret;
    ...
    if (num > priv->mac_mc) {
        ...
        for (i = RTE_DIM(priv->mac) - num; i != RTE_DIM(priv->mac) - priv->mac_mc; ++i)
        ...
        memset(priv->mac + RTE_DIM(priv->mac) - priv->mac_mc, 0, sizeof(priv->mac[0]) * (priv->mac_mc - num));
    }
    ...
    ERROR("failed to synchronize flow rules after modifying MC list,"
          ...
          rte_errno, strerror(rte_errno), error.type, error.cause, error.message ? error.message : "(unspecified)");
    return ret;
}
...
int mlx4_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id, int on) {
    ...
    *v |= (uint64_t) !!on << vbit;
    ...
    ERROR("failed to synchronize flow rules after %s VLAN filter on ID %u"
          " (code %d, \"%s\"), "
          " flow error type %d, cause %p, message: %s",
          on ? "enabling" : "disabling", vlan_id, rte_errno, strerror(rte_errno), error.type, error.cause,
          error.message ? error.message : "(unspecified)");
    return ret;
}
 ...
 */
void mlx4_dev_infos_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *info) {
    struct mlx4_priv *priv = dev->data->dev_private;
    unsigned int max;
     ...
     */
    max = ((priv->device_attr.max_cq > priv->device_attr.max_qp) ? priv->device_attr.max_qp : priv->device_attr.max_cq);
    ...
    info->rx_offload_capa = (mlx4_get_rx_port_offloads(priv) | info->rx_queue_offload_capa);
    if (mlx4_get_ifname(priv, &ifname) == 0)
        info->if_index = if_nametoindex(ifname);
    info->hash_key_size = MLX4_RSS_HASH_KEY_SIZE;
    info->speed_capa =
        ETH_LINK_SPEED_1G | ETH_LINK_SPEED_10G | ETH_LINK_SPEED_20G | ETH_LINK_SPEED_40G | ETH_LINK_SPEED_56G;
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
int mlx4_flow_ctrl_get(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf) {
    struct mlx4_priv *priv = dev->data->dev_private;
    struct ifreq ifr;
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
int mlx4_flow_ctrl_set(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf) {
    struct mlx4_priv *priv = dev->data->dev_private;
    struct ifreq ifr;
    struct ethtool_pauseparam ethpause = {
