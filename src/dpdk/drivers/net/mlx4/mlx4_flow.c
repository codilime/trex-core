...
struct mlx4_flow_proc_item {
    ...
    int (*merge)(struct rte_flow *flow, const struct rte_flow_item *item, const struct mlx4_flow_proc_item *proc,
 ...
 *   rte_errno is set.
 */
uint64_t mlx4_conv_rss_types(struct mlx4_priv *priv, uint64_t types, int verbs_to_dpdk) {
    enum {
        INNER,
        IPV4,
        IPV4_1,
 ...
 */
static int mlx4_flow_merge_eth(struct rte_flow *flow, const struct rte_flow_item *item,
                               const struct mlx4_flow_proc_item *proc, struct rte_flow_error *error) {
    const struct rte_flow_item_eth *spec = item->spec;
    const struct rte_flow_item_eth *mask = spec ? (item->mask ? item->mask : proc->mask_default) : NULL;
    ...
    *eth = (struct ibv_flow_spec_eth){
    ...
    return 0;
error:
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, item, msg);
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_merge_vlan(struct rte_flow *flow, const struct rte_flow_item *item,
                                const struct mlx4_flow_proc_item *proc, struct rte_flow_error *error) {
    const struct rte_flow_item_vlan *spec = item->spec;
    const struct rte_flow_item_vlan *mask = spec ? (item->mask ? item->mask : proc->mask_default) : NULL;
    struct ibv_flow_spec_eth *eth;
    const char *msg;
    ...
    if (!flow->ibv_attr)
        return 0;
    eth = (void *)((uintptr_t)flow->ibv_attr + flow->ibv_attr_size - sizeof(*eth));
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_merge_ipv4(struct rte_flow *flow, const struct rte_flow_item *item,
                                const struct mlx4_flow_proc_item *proc, struct rte_flow_error *error) {
    const struct rte_flow_item_ipv4 *spec = item->spec;
    const struct rte_flow_item_ipv4 *mask = spec ? (item->mask ? item->mask : proc->mask_default) : NULL;
    struct ibv_flow_spec_ipv4 *ipv4;
    const char *msg;

    if (mask &&
        ((uint32_t)(mask->hdr.src_addr + 1) > UINT32_C(1) || (uint32_t)(mask->hdr.dst_addr + 1) > UINT32_C(1))) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_merge_udp(struct rte_flow *flow, const struct rte_flow_item *item,
                               const struct mlx4_flow_proc_item *proc, struct rte_flow_error *error) {
    const struct rte_flow_item_udp *spec = item->spec;
    const struct rte_flow_item_udp *mask = spec ? (item->mask ? item->mask : proc->mask_default) : NULL;
    struct ibv_flow_spec_tcp_udp *udp;
    const char *msg;

    if (mask &&
        ((uint16_t)(mask->hdr.src_port + 1) > UINT16_C(1) || (uint16_t)(mask->hdr.dst_port + 1) > UINT16_C(1))) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_merge_tcp(struct rte_flow *flow, const struct rte_flow_item *item,
                               const struct mlx4_flow_proc_item *proc, struct rte_flow_error *error) {
    const struct rte_flow_item_tcp *spec = item->spec;
    const struct rte_flow_item_tcp *mask = spec ? (item->mask ? item->mask : proc->mask_default) : NULL;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_item_check(const struct rte_flow_item *item, const struct mlx4_flow_proc_item *proc,
                                struct rte_flow_error *error) {
    const uint8_t *mask;
    unsigned int i;
    ...
    if (!item->spec)
        return 0;
    mask = item->mask ? (const uint8_t *)item->mask : (const uint8_t *)proc->mask_default;
    assert(mask);
    /*
    ...
    for (i = 0; i != proc->mask_sz; ++i) {
        if (!mask[i])
            continue;
        if ((mask[i] | ((const uint8_t *)proc->mask_support)[i]) != ((const uint8_t *)proc->mask_support)[i])
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, item,
                                      "unsupported field found in \"mask\"");
        if (item->last && (((const uint8_t *)item->spec)[i] & mask[i]) != (((const uint8_t *)item->last)[i] & mask[i]))
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, item,
...
static const struct mlx4_flow_proc_item mlx4_flow_proc_item_list[] = {
    ...
    [RTE_FLOW_ITEM_TYPE_ETH] =
        {
            .next_item = NEXT_ITEM(RTE_FLOW_ITEM_TYPE_VLAN, RTE_FLOW_ITEM_TYPE_IPV4),
            .mask_support =
    ...
    [RTE_FLOW_ITEM_TYPE_IPV4] =
        {
            .next_item = NEXT_ITEM(RTE_FLOW_ITEM_TYPE_UDP, RTE_FLOW_ITEM_TYPE_TCP),
            .mask_support =
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_prepare(struct mlx4_priv *priv, const struct rte_flow_attr *attr,
                             const struct rte_flow_item pattern[], const struct rte_flow_action actions[],
                             struct rte_flow_error *error, struct rte_flow **addr) {
    ...
    if (attr->group)
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_GROUP, NULL, "groups are not supported");
    if (attr->priority > MLX4_FLOW_PRIORITY_LAST)
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_PRIORITY, NULL,
    ...
    if (!attr->ingress)
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_INGRESS, NULL, "only ingress is supported");
fill:
    overlap = 0;
    ...
    for (action = actions; action->type; ++action) {
        ...
        case RTE_FLOW_ACTION_TYPE_QUEUE:
            ...
            flow->rss = mlx4_rss_get(priv, 0, mlx4_rss_hash_key_default, 1, &queue->index);
            if (!flow->rss) {
        ...
        case RTE_FLOW_ACTION_TYPE_RSS:
            ...
            flow->rss = mlx4_rss_get(priv, fields, rss_key, rss->queue_num, rss->queue);
            if (!flow->rss) {
...
exit_item_not_supported:
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, item, msg ? msg : "item not supported");
exit_action_not_supported:
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, action, msg ? msg : "action not supported");
}
 ...
 * @see rte_flow_ops
 */
static int mlx4_flow_validate(struct rte_eth_dev *dev, const struct rte_flow_attr *attr,
                              const struct rte_flow_item pattern[], const struct rte_flow_action actions[],
                              struct rte_flow_error *error) {
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_toggle(struct mlx4_priv *priv, struct rte_flow *flow, int enable, struct rte_flow_error *error) {
    ...
    if (!flow->internal && !priv->isolated && flow->ibv_attr->priority == MLX4_FLOW_PRIORITY_LAST) {
        if (flow->ibv_flow) {
            claim_zero(mlx4_glue->destroy_flow(flow->ibv_flow));
            flow->ibv_flow = NULL;
            if (flow->drop)
        ...
        err = EACCES;
        msg = ("priority level " MLX4_STR_EXPAND(MLX4_FLOW_PRIORITY_LAST) " is reserved when not in isolated mode");
        goto error;
    }
    if (flow->rss) {
 ...
 * @see rte_flow_ops
 */
static struct rte_flow *mlx4_flow_create(struct rte_eth_dev *dev, const struct rte_flow_attr *attr,
                                         const struct rte_flow_item pattern[], const struct rte_flow_action actions[],
                                         struct rte_flow_error *error) {
    struct mlx4_priv *priv = dev->data->dev_private;
    ...
    if (flow->rss)
        mlx4_rss_put(flow->rss);
    rte_flow_error_set(error, -err, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, error->message);
    rte_free(flow);
    return NULL;
}
 ...
 * @see rte_flow_ops
 */
static int mlx4_flow_isolate(struct rte_eth_dev *dev, int enable, struct rte_flow_error *error) {
    struct mlx4_priv *priv = dev->data->dev_private;
 ...
 * @see rte_flow_ops
 */
static int mlx4_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow, struct rte_flow_error *error) {
    struct mlx4_priv *priv = dev->data->dev_private;
 ...
 * @see rte_flow_ops
 */
static int mlx4_flow_flush(struct rte_eth_dev *dev, struct rte_flow_error *error) {
    struct mlx4_priv *priv = dev->data->dev_private;
 ...
 */
static uint16_t mlx4_flow_internal_next_vlan(struct mlx4_priv *priv, uint16_t vlan) {
    while (vlan < 4096) {
        if (ETH_DEV(priv)->data->vlan_filter_conf.ids[vlan / 64] & (UINT64_C(1) << (vlan % 64)))
            return vlan;
        ++vlan;
    }
    return vlan;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx4_flow_internal(struct mlx4_priv *priv, struct rte_flow_error *error) {
    ...
    rte_be16_t *rule_vlan = (ETH_DEV(priv)->data->dev_conf.rxmode.offloads & DEV_RX_OFFLOAD_VLAN_FILTER) &&
    ...
    for (i = 0; i != RTE_DIM(priv->mac) + 1; ++i) {
        ...
        for (flow = LIST_FIRST(&priv->flows); flow && flow->internal; flow = LIST_NEXT(flow, next)) {
            const struct ibv_flow_spec_eth *eth = (const void *)((uintptr_t)flow->ibv_attr + sizeof(*flow->ibv_attr));
            unsigned int j;
            ...
            assert(flow->rss);
            if (rule_vlan && (eth->val.vlan_tag != *rule_vlan || eth->mask.vlan_tag != RTE_BE16(0x0fff)))
                continue;
            if (!rule_vlan && eth->mask.vlan_tag)
                continue;
            for (j = 0; j != sizeof(mac->addr_bytes); ++j)
                if (eth->val.dst_mac[j] != mac->addr_bytes[j] || eth->mask.dst_mac[j] != UINT8_C(0xff) ||
                    eth->val.src_mac[j] != UINT8_C(0x00) || eth->mask.src_mac[j] != UINT8_C(0x00))
                    break;
            if (j != sizeof(mac->addr_bytes))
                continue;
            if (flow->rss->queues != queues ||
                memcmp(flow->rss->queue_id, action_rss.queue, queues * sizeof(flow->rss->queue_id[0])))
                continue;
            break;
        }
...
int mlx4_flow_sync(struct mlx4_priv *priv, struct rte_flow_error *error) {
    ...
    if (priv->isolated) {
         ...
         */
        for (flow = LIST_FIRST(&priv->flows); flow && flow->internal; flow = LIST_FIRST(&priv->flows))
            claim_zero(mlx4_flow_destroy(ETH_DEV(priv), flow, error));
    } else {
 ...
 */
int mlx4_filter_ctrl(struct rte_eth_dev *dev, enum rte_filter_type filter_type, enum rte_filter_op filter_op,
                     void *arg) {
    switch (filter_type) {
    case RTE_ETH_FILTER_GENERIC:
        ...
        *(const void **)arg = &mlx4_flow_ops;
        return 0;
    default:
        ERROR("%p: filter type (%d) not supported", (void *)dev, filter_type);
        break;
    }
    rte_errno = ENOTSUP;
    return -rte_errno;
