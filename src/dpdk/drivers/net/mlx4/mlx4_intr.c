...
static void mlx4_link_status_alarm(struct mlx4_priv *priv) {
    const struct rte_intr_conf *const intr_conf = &ETH_DEV(priv)->data->dev_conf.intr_conf;

    assert(priv->intr_alarm == 1);
    priv->intr_alarm = 0;
    if (intr_conf->lsc && !mlx4_link_status_check(priv))
        _rte_eth_dev_callback_process(ETH_DEV(priv), RTE_ETH_EVENT_INTR_LSC, NULL);
}
...
static int mlx4_link_status_check(struct mlx4_priv *priv) {
    ...
    if (ret)
        return ret;
    if ((!link->link_speed && link->link_status) || (link->link_speed && !link->link_status)) {
        if (!priv->intr_alarm) {
            /* Inconsistent status, check again later. */
            ret = rte_eal_alarm_set(MLX4_INTR_ALARM_TIMEOUT, (void (*)(void *))mlx4_link_status_alarm, priv);
            if (ret)
                return ret;
            priv->intr_alarm = 1;
        }
...
static void mlx4_interrupt_handler(struct mlx4_priv *priv) {
    ...
    while (!mlx4_glue->get_async_event(priv->ctx, &event)) {
        ...
        case IBV_EVENT_DEVICE_FATAL:
            ...
            break;
        default:
            DEBUG("event type %d on physical port %d not handled", event.event_type, event.element.port_num);
        }
        mlx4_glue->ack_async_event(&event);
    }
    for (i = 0; i != RTE_DIM(caught); ++i)
        if (caught[i])
            _rte_eth_dev_callback_process(ETH_DEV(priv), type[i], NULL);
}
...
int mlx4_intr_uninstall(struct mlx4_priv *priv) {
    ...
    if (priv->intr_handle.fd != -1) {
        rte_intr_callback_unregister(&priv->intr_handle, (void (*)(void *))mlx4_interrupt_handler, priv);
...
int mlx4_intr_install(struct mlx4_priv *priv) {
    ...
    if (intr_conf->lsc | intr_conf->rmv) {
        ...
        rc = rte_intr_callback_register(&priv->intr_handle, (void (*)(void *))mlx4_interrupt_handler, priv);
        if (rc < 0) {
            rte_errno = -rc;
            goto error;
        }
