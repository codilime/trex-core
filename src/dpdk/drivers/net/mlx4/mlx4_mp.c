 ...
 *   Message type.
 */
static inline void mp_init_msg(struct rte_eth_dev *dev, struct rte_mp_msg *msg, enum mlx4_mp_req_type type) {
...
static int mp_primary_handle(const struct rte_mp_msg *mp_msg, const void *peer) {
    ...
    const struct mlx4_mp_param *param = (const struct mlx4_mp_param *)mp_msg->param;
 ...
 *   Request type.
 */
static void mp_req_on_rxtx(struct rte_eth_dev *dev, enum mlx4_mp_req_type type) {
    ...
    if (ret) {
        ERROR("port %u failed to request stop/start Rx/Tx (%d)", dev->data->port_id, type);
        goto exit;
    }
    if (mp_rep.nb_sent != mp_rep.nb_received) {
        ERROR("port %u not all secondaries responded (req_type %d)", dev->data->port_id, type);
        goto exit;
    }
 ...
 *   Pointer to Ethernet structure.
 */
void mlx4_mp_req_start_rxtx(struct rte_eth_dev *dev) { mp_req_on_rxtx(dev, MLX4_MP_REQ_START_RXTX); }
 ...
 *   Pointer to Ethernet structure.
 */
void mlx4_mp_req_stop_rxtx(struct rte_eth_dev *dev) { mp_req_on_rxtx(dev, MLX4_MP_REQ_STOP_RXTX); }
...
int mlx4_mp_req_verbs_cmd_fd(struct rte_eth_dev *dev) {
    ...
    if (res->result) {
        ...
        ERROR("port %u failed to get command FD from primary process", dev->data->port_id);
        ret = -rte_errno;
        goto exit;
    }
