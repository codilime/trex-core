 ...
 */
static uint32_t mr_btree_lookup(struct mlx4_mr_btree *bt, uint16_t *idx, uintptr_t addr) {
 ...
 *   0 on success, -1 on failure.
 */
static int mr_btree_insert(struct mlx4_mr_btree *bt, struct mlx4_mr_cache *entry) {
    struct mlx4_mr_cache *lkp_tbl;
    uint16_t idx = 0;
    size_t shift;
...
int mlx4_mr_btree_init(struct mlx4_mr_btree *bt, int n, int socket) {
    ...
    bt->table = rte_calloc_socket("B-tree table", n, sizeof(struct mlx4_mr_cache), 0, socket);
    if (bt->table == NULL) {
        rte_errno = ENOMEM;
 ...
 */
static int mr_find_next_chunk(struct mlx4_mr *mr, struct mlx4_mr_cache *entry, int base_idx) {
    uintptr_t start = 0;
    uintptr_t end = 0;
    uint32_t idx = 0;
...
static int mr_insert_dev_cache(struct rte_eth_dev *dev, struct mlx4_mr *mr) {
    ...
    DEBUG("port %u inserting MR(%p) to global cache", dev->data->port_id, (void *)mr);
 ...
 */
static struct mlx4_mr *mr_lookup_dev_list(struct rte_eth_dev *dev, struct mlx4_mr_cache *entry, uintptr_t addr) {
    struct mlx4_priv *priv = dev->data->dev_private;
    struct mlx4_mr *mr;
 ...
 */
static uint32_t mr_lookup_dev(struct rte_eth_dev *dev, struct mlx4_mr_cache *entry, uintptr_t addr) {
    struct mlx4_priv *priv = dev->data->dev_private;
    uint16_t idx;
 ...
 *   Searched LKey on success, UINT32_MAX on failure and rte_errno is set.
 */
static uint32_t mlx4_mr_create_secondary(struct rte_eth_dev *dev, struct mlx4_mr_cache *entry, uintptr_t addr) {
    struct mlx4_priv *priv = dev->data->dev_private;
    int ret;
 ...
 *   Searched LKey on success, UINT32_MAX on failure and rte_errno is set.
 */
uint32_t mlx4_mr_create_primary(struct rte_eth_dev *dev, struct mlx4_mr_cache *entry, uintptr_t addr) {
    struct mlx4_priv *priv = dev->data->dev_private;
    ...
    DEBUG("port %u creating a MR using address (%p)", dev->data->port_id, (void *)addr);
    /*
...
alloc_resources:
     ...
     */
    mr->ibv_mr = mlx4_glue->reg_mr(priv->pd, (void *)data.start, len, IBV_ACCESS_LOCAL_WRITE);
    if (mr->ibv_mr == NULL) {
        WARN("port %u fail to create a verbs MR for address (%p)", dev->data->port_id, (void *)addr);
        rte_errno = EINVAL;
        goto err_mrlock;
    }
 ...
 *   Searched LKey on success, UINT32_MAX on failure and rte_errno is set.
 */
static uint32_t mlx4_mr_create(struct rte_eth_dev *dev, struct mlx4_mr_cache *entry, uintptr_t addr) {
    uint32_t ret = 0;
 ...
 */
static void mlx4_mr_mem_event_free_cb(struct rte_eth_dev *dev, const void *addr, size_t len) {
    struct mlx4_priv *priv = dev->data->dev_private;
    const struct rte_memseg_list *msl;
    struct mlx4_mr *mr;
    int ms_n;
    ...
    DEBUG("port %u free callback: addr=%p, len=%zu", dev->data->port_id, addr, len);
    ...
    for (i = 0; i < ms_n; ++i) {
        ...
        DEBUG("port %u MR(%p): clear bitmap[%u] for addr %p", dev->data->port_id, (void *)mr, pos, (void *)start);
 ...
 *   Size of memory.
 */
void mlx4_mr_mem_event_cb(enum rte_mem_event event_type, const void *addr, size_t len, void *arg __rte_unused) {
 ...
 *   Searched LKey on success, UINT32_MAX on no match.
 */
static uint32_t mlx4_mr_lookup_dev(struct rte_eth_dev *dev, struct mlx4_mr_ctrl *mr_ctrl, struct mlx4_mr_cache *entry,
                                   uintptr_t addr) {
    struct mlx4_priv *priv = dev->data->dev_private;
 ...
 *   Searched LKey on success, UINT32_MAX on no match.
 */
static uint32_t mlx4_mr_addr2mr_bh(struct rte_eth_dev *dev, struct mlx4_mr_ctrl *mr_ctrl, uintptr_t addr) {
    uint32_t lkey;
    uint16_t bh_idx = 0;
 ...
 */
static void mlx4_mr_update_ext_mp_cb(struct rte_mempool *mp, void *opaque, struct rte_mempool_memhdr *memhdr,
    ...
    if (lkey != UINT32_MAX)
        return;
    mr = rte_zmalloc_socket(NULL, RTE_ALIGN_CEIL(sizeof(*mr), RTE_CACHE_LINE_SIZE), RTE_CACHE_LINE_SIZE, mp->socket_id);
    if (mr == NULL) {
        WARN("port %u unable to allocate memory for a new MR of"
             " mempool (%s).",
    ...
    DEBUG("port %u register MR for chunk #%d of mempool (%s)", dev->data->port_id, mem_idx, mp->name);
    mr->ibv_mr = mlx4_glue->reg_mr(priv->pd, (void *)addr, len, IBV_ACCESS_LOCAL_WRITE);
    if (mr->ibv_mr == NULL) {
        WARN("port %u fail to create a verbs MR for address (%p)", dev->data->port_id, (void *)addr);
        rte_free(mr);
        data->ret = -1;
 ...
 *   0 on success, -1 on failure.
 */
static uint32_t mlx4_mr_update_ext_mp(struct rte_eth_dev *dev, struct mlx4_mr_ctrl *mr_ctrl, struct rte_mempool *mp) {
 ...
 */
uint32_t mlx4_tx_update_ext_mp(struct txq *txq, uintptr_t addr, struct rte_mempool *mp) {
    struct mlx4_mr_ctrl *mr_ctrl = &txq->mr_ctrl;
    struct mlx4_priv *priv = txq->priv;
...
static void mlx4_mr_update_mp_cb(struct rte_mempool *mp __rte_unused, void *opaque, struct rte_mempool_memhdr *memhdr,
                                 unsigned mem_idx __rte_unused) {
    struct mr_update_mp_data *data = opaque;
    uint32_t lkey;
 ...
 *   0 on success, -1 on failure.
 */
int mlx4_mr_update_mp(struct rte_eth_dev *dev, struct mlx4_mr_ctrl *mr_ctrl, struct rte_mempool *mp) {
    struct mr_update_mp_data data = {
        .dev = dev,
        .mr_ctrl = mr_ctrl,
        .ret = 0,
...
void mlx4_mr_dump_dev(struct rte_eth_dev *dev) {
    ...
    LIST_FOREACH(mr, &priv->mr.mr_list, mr) {
        ...
        DEBUG("port %u MR[%u], LKey = 0x%x, ms_n = %u, ms_bmp_n = %u", dev->data->port_id, mr_n++,
