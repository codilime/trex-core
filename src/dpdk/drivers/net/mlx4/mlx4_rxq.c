 ...
 */
struct mlx4_rss *mlx4_rss_get(struct mlx4_priv *priv, uint64_t fields, const uint8_t key[MLX4_RSS_HASH_KEY_SIZE],
                              uint16_t queues, const uint16_t queue_id[]) {
    struct mlx4_rss *rss;
    size_t queue_id_size = sizeof(queue_id[0]) * queues;

    LIST_FOREACH(rss, &priv->rss, next)
    if (fields == rss->fields && queues == rss->queues && !memcmp(key, rss->key, MLX4_RSS_HASH_KEY_SIZE) &&
        !memcmp(queue_id, rss->queue_id, queue_id_size)) {
        ++rss->refcnt;
        return rss;
    }
    rss = rte_malloc(__func__, offsetof(struct mlx4_rss, queue_id) + queue_id_size, 0);
    if (!rss)
        goto error;
    *rss = (struct mlx4_rss){
        .priv = priv,
...
int mlx4_rss_attach(struct mlx4_rss *rss) {
    ...
    rss->ind = mlx4_glue->create_rwq_ind_table(priv->ctx, &(struct ibv_rwq_ind_table_init_attr){
    ...
    rss->qp = mlx4_glue->create_qp_ex(
        priv->ctx, &(struct ibv_qp_init_attr_ex){
...
int mlx4_rss_init(struct mlx4_priv *priv) {
    ...
    ret = mlx4_glue->dv_set_context_attr(priv->ctx, MLX4DV_SET_CTX_ATTR_LOG_WQS_RANGE_SZ, &log2_range);
    if (ret) {
    ...
    return 0;
error:
    ERROR("cannot initialize common RSS resources (queue %u): %s: %s", i, msg, strerror(ret));
    while (i--) {
        struct rxq *rxq = ETH_DEV(priv)->data->rx_queues[i];
...
int mlx4_rxq_attach(struct rxq *rxq) {
    ...
    DEBUG("port %u Rx queue %u registering mp %s having %u chunks", ETH_DEV(priv)->data->port_id, rxq->stats.idx,
          rxq->mp->name, rxq->mp->nb_mem_chunks);
    mlx4_mr_update_mp(dev, &rxq->mr_ctrl, rxq->mp);
    wqes = (volatile struct mlx4_wqe_data_seg(*)[])((uintptr_t)dv_rwq.buf.buf + dv_rwq.rq.offset);
    ...
    DEBUG("%p: allocated and configured %u segments (max %u packets)", (void *)rxq, elts_n, elts_n / sges_n);
    rxq->cq = cq;
    rxq->wq = wq;
    rxq->wqes = wqes;
...
error:
    ...
    --rxq->usecnt;
    rte_errno = ret;
    ERROR("error while attaching Rx queue %p: %s: %s", (void *)rxq, msg, strerror(ret));
    priv->verbs_alloc_ctx.type = MLX4_VERBS_ALLOC_TYPE_NONE;
    return -ret;
}
...
uint64_t mlx4_get_rx_queue_offloads(struct mlx4_priv *priv) {
    uint64_t offloads = DEV_RX_OFFLOAD_SCATTER | DEV_RX_OFFLOAD_KEEP_CRC | DEV_RX_OFFLOAD_JUMBO_FRAME;
 ...
 *   0 on success, negative errno value otherwise and rte_errno is set.
 */
int mlx4_rx_queue_setup(struct rte_eth_dev *dev, uint16_t idx, uint16_t desc, unsigned int socket,
    ...
    if (idx >= dev->data->nb_rx_queues) {
        rte_errno = EOVERFLOW;
        ERROR("%p: queue index out of range (%u >= %u)", (void *)dev, idx, dev->data->nb_rx_queues);
        return -rte_errno;
    }
    rxq = dev->data->rx_queues[idx];
    if (rxq) {
        rte_errno = EEXIST;
        ERROR("%p: Rx queue %u already configured, release it first", (void *)dev, idx);
        return -rte_errno;
    }
    if (!desc) {
        rte_errno = EINVAL;
    ...
    *rxq = (struct rxq){
        ...
        .csum = priv->hw_csum && (offloads & DEV_RX_OFFLOAD_CHECKSUM),
    ...
    if (dev->data->dev_conf.rxmode.max_rx_pkt_len <= (mb_len - RTE_PKTMBUF_HEADROOM)) {
        ;
    } else if (offloads & DEV_RX_OFFLOAD_SCATTER) {
        uint32_t size = RTE_PKTMBUF_HEADROOM + dev->data->dev_conf.rxmode.max_rx_pkt_len;
        uint32_t sges_n;
        ...
        if (size < dev->data->dev_conf.rxmode.max_rx_pkt_len) {
            ...
            ERROR("%p: too many SGEs (%u) needed to handle"
                  ...
                  (void *)dev, 1 << sges_n, dev->data->dev_conf.rxmode.max_rx_pkt_len);
            goto error;
        }
    } else {
        WARN("%p: the requested maximum Rx packet size (%u) is"
             ...
             (void *)dev, dev->data->dev_conf.rxmode.max_rx_pkt_len, mb_len - RTE_PKTMBUF_HEADROOM);
    }
    DEBUG("%p: maximum number of segments per packet: %u", (void *)dev, 1 << rxq->sges_n);
    ...
    if (mlx4_mr_btree_init(&rxq->mr_ctrl.cache_bh, MLX4_MR_BTREE_CACHE_N, socket)) {
...
void mlx4_rx_queue_release(void *dpdk_rxq) {
    ...
    for (i = 0; i != ETH_DEV(priv)->data->nb_rx_queues; ++i)
        if (ETH_DEV(priv)->data->rx_queues[i] == rxq) {
            DEBUG("%p: removing Rx queue %p from list", (void *)ETH_DEV(priv), (void *)rxq);
