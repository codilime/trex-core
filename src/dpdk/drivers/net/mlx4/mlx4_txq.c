...
static int txq_uar_init_secondary(struct txq *txq, int fd) {
    ...
    addr = mmap(NULL, page_size, PROT_WRITE, MAP_SHARED, fd, txq->msq.uar_mmap_offset);
    if (addr == MAP_FAILED) {
        ERROR("port %u mmap failed for BF reg of txq %u", txq->port_id, txq->stats.idx);
        rte_errno = ENXIO;
        return -rte_errno;
    }
...
#else
int mlx4_tx_uar_init_secondary(struct rte_eth_dev *dev __rte_unused, int fd __rte_unused) {
    assert(rte_eal_process_type() == RTE_PROC_SECONDARY);
 ...
 */
static void mlx4_txq_fill_dv_obj_info(struct txq *txq, struct mlx4dv_obj *mlxdv) {
 ...
 */
int mlx4_tx_queue_setup(struct rte_eth_dev *dev, uint16_t idx, uint16_t desc, unsigned int socket,
    ...
    if (idx >= dev->data->nb_tx_queues) {
        rte_errno = EOVERFLOW;
        ERROR("%p: queue index out of range (%u >= %u)", (void *)dev, idx, dev->data->nb_tx_queues);
        return -rte_errno;
    }
    txq = dev->data->tx_queues[idx];
    if (txq) {
        rte_errno = EEXIST;
        DEBUG("%p: Tx queue %u already configured, release it first", (void *)dev, idx);
        return -rte_errno;
    }
    if (!desc) {
        rte_errno = EINVAL;
    ...
    *txq = (struct txq){
        ...
        .csum = priv->hw_csum &&
                (offloads & (DEV_TX_OFFLOAD_IPV4_CKSUM | DEV_TX_OFFLOAD_UDP_CKSUM | DEV_TX_OFFLOAD_TCP_CKSUM)),
        .csum_l2tun = priv->hw_csum_l2tun && (offloads & DEV_TX_OFFLOAD_OUTER_IPV4_CKSUM),
    ...
    if (ret) {
        rte_errno = ret;
        ERROR("%p: QP state to IBV_QPS_INIT failed: %s", (void *)dev, strerror(rte_errno));
        goto error;
    }
    ret = mlx4_glue->modify_qp(txq->qp,
                               &(struct ibv_qp_attr){
    ...
    if (ret) {
        rte_errno = ret;
        ERROR("%p: QP state to IBV_QPS_RTR failed: %s", (void *)dev, strerror(rte_errno));
        goto error;
    }
    ret = mlx4_glue->modify_qp(txq->qp,
                               &(struct ibv_qp_attr){
    ...
    if (ret) {
        rte_errno = ret;
        ERROR("%p: QP state to IBV_QPS_RTS failed: %s", (void *)dev, strerror(rte_errno));
        goto error;
    }
    ...
    if (mlx4_mr_btree_init(&txq->mr_ctrl.cache_bh, MLX4_MR_BTREE_CACHE_N, socket)) {
...
void mlx4_tx_queue_release(void *dpdk_txq) {
    ...
    for (i = 0; i != ETH_DEV(priv)->data->nb_tx_queues; ++i)
        if (ETH_DEV(priv)->data->tx_queues[i] == txq) {
            DEBUG("%p: removing Tx queue %p from list", (void *)ETH_DEV(priv), (void *)txq);
