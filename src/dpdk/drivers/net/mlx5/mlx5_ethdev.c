...
int mlx5_get_master_ifname(const char *ibdev_path, char (*ifname)[IF_NAMESIZE]) {
    ...
    while ((dent = readdir(dir)) != NULL) {
        ...
        MKSTR(path, "%s/device/net/%s/%s", ibdev_path, name, (dev_type ? "dev_id" : "dev_port"));
...
int mlx5_get_ifname(const struct rte_eth_dev *dev, char (*ifname)[IF_NAMESIZE]) {
    ...
    assert(priv->sh);
    ifindex =
        priv->nl_socket_rdma >= 0 ? mlx5_nl_ifindex(priv->nl_socket_rdma, priv->sh->ibdev_name, priv->ibv_port) : 0;
    if (!ifindex) {
        if (!priv->representor)
            return mlx5_get_master_ifname(priv->sh->ibdev_path, ifname);
        rte_errno = ENXIO;
        return -rte_errno;
    }
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_set_mtu(struct rte_eth_dev *dev, uint16_t mtu) {
    struct ifreq request = {
        .ifr_mtu = mtu,
    };
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_set_flags(struct rte_eth_dev *dev, unsigned int keep, unsigned int flags) {
    struct ifreq request;
...
int mlx5_dev_configure(struct rte_eth_dev *dev) {
    ...
    const uint8_t use_app_rss_key = !!dev->data->dev_conf.rx_adv_conf.rss_conf.rss_key;
    int ret = 0;

    if (use_app_rss_key && (dev->data->dev_conf.rx_adv_conf.rss_conf.rss_key_len != MLX5_RSS_HASH_KEY_LEN)) {
        DRV_LOG(ERR, "port %u RSS key len must be %s Bytes long", dev->data->port_id, RTE_STR(MLX5_RSS_HASH_KEY_LEN));
        rte_errno = EINVAL;
        return -rte_errno;
    }
    priv->rss_conf.rss_key = rte_realloc(priv->rss_conf.rss_key, MLX5_RSS_HASH_KEY_LEN, 0);
    if (!priv->rss_conf.rss_key) {
        DRV_LOG(ERR, "port %u cannot allocate RSS hash key memory (%u)", dev->data->port_id, rxqs_n);
        rte_errno = ENOMEM;
        return -rte_errno;
    }
    memcpy(priv->rss_conf.rss_key,
           use_app_rss_key ? dev->data->dev_conf.rx_adv_conf.rss_conf.rss_key : rss_hash_default_key,
    ...
    if (txqs_n != priv->txqs_n) {
        DRV_LOG(INFO, "port %u Tx queues number update: %u -> %u", dev->data->port_id, priv->txqs_n, txqs_n);
        priv->txqs_n = txqs_n;
    }
    if (rxqs_n > priv->config.ind_table_max_size) {
        DRV_LOG(ERR, "port %u cannot handle this many Rx queues (%u)", dev->data->port_id, rxqs_n);
        rte_errno = EINVAL;
        return -rte_errno;
    }
    if (rxqs_n != priv->rxqs_n) {
        DRV_LOG(INFO, "port %u Rx queues number update: %u -> %u", dev->data->port_id, priv->rxqs_n, rxqs_n);
        priv->rxqs_n = rxqs_n;
         ...
         */
        reta_idx_n = (1 << log2above((rxqs_n & (rxqs_n - 1)) ? priv->config.ind_table_max_size : rxqs_n));
#endif
 ...
 */
static void mlx5_set_default_params(struct rte_eth_dev *dev, struct rte_eth_dev_info *info) {
    struct mlx5_priv *priv = dev->data->dev_private;
...
void mlx5_dev_infos_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *info) {
     ...
     */
    max = RTE_MIN(priv->sh->device_attr.orig_attr.max_cq, priv->sh->device_attr.orig_attr.max_qp);
    ...
    info->rx_offload_capa = (mlx5_get_rx_port_offloads() | info->rx_queue_offload_capa);
    info->tx_offload_capa = mlx5_get_tx_port_offloads(dev);
    if (mlx5_get_ifname(dev, &ifname) == 0)
        info->if_index = if_nametoindex(ifname);
    info->reta_size = priv->reta_idx_n ? priv->reta_idx_n : config->ind_table_max_size;
    ...
    if (priv->representor) {
        ...
        while (i--) {
            struct mlx5_priv *opriv = rte_eth_devices[port_id[i]].data->dev_private;

            if (!opriv || opriv->representor || opriv->domain_id != priv->domain_id)
                continue;
            /*
...
const uint32_t *mlx5_dev_supported_ptypes_get(struct rte_eth_dev *dev) {
    ...
    if (dev->rx_pkt_burst == mlx5_rx_burst || dev->rx_pkt_burst == mlx5_rx_burst_mprq ||
...
static struct rte_eth_dev *mlx5_find_master_dev(struct rte_eth_dev *dev) {
    ...
    RTE_ETH_FOREACH_DEV_OF(port_id, dev->device) {
        ...
        if (priv && priv->master && priv->domain_id == domain_id)
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_link_update_unlocked_gset(struct rte_eth_dev *dev, struct rte_eth_link *link) {
    struct mlx5_priv *priv = dev->data->dev_private;
    ...
    if (ret) {
        DRV_LOG(WARNING, "port %u ioctl(SIOCGIFFLAGS) failed: %s", dev->data->port_id, strerror(rte_errno));
        return ret;
    }
    dev_link = (struct rte_eth_link){
        .link_status = ((ifr.ifr_flags & IFF_UP) && (ifr.ifr_flags & IFF_RUNNING)),
    };
    ifr = (struct ifreq){
        .ifr_data = (void *)&edata,
    };
    ...
    if (edata.supported & (SUPPORTED_40000baseKR4_Full | SUPPORTED_40000baseCR4_Full | SUPPORTED_40000baseSR4_Full |
                           SUPPORTED_40000baseLR4_Full))
        priv->link_speed_capa |= ETH_LINK_SPEED_40G;
    dev_link.link_duplex = ((edata.duplex == DUPLEX_HALF) ? ETH_LINK_HALF_DUPLEX : ETH_LINK_FULL_DUPLEX);
    dev_link.link_autoneg = !(dev->data->dev_conf.link_speeds & ETH_LINK_SPEED_FIXED);
    if (((dev_link.link_speed && !dev_link.link_status) || (!dev_link.link_speed && dev_link.link_status))) {
        rte_errno = EAGAIN;
        return -rte_errno;
    }
    *link = dev_link;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_link_update_unlocked_gs(struct rte_eth_dev *dev, struct rte_eth_link *link)

{
    ...
    alignas(struct ethtool_link_settings) uint8_t data[offsetof(struct ethtool_link_settings, link_mode_masks) +
    ...
    *ecmd = gcmd;
    ...
    if (sc & MLX5_BITSHIFT(ETHTOOL_LINK_MODE_Autoneg_BIT))
        priv->link_speed_capa |= ETH_LINK_SPEED_AUTONEG;
    if (sc &
        (MLX5_BITSHIFT(ETHTOOL_LINK_MODE_1000baseT_Full_BIT) | MLX5_BITSHIFT(ETHTOOL_LINK_MODE_1000baseKX_Full_BIT)))
        priv->link_speed_capa |= ETH_LINK_SPEED_1G;
    if (sc &
        (MLX5_BITSHIFT(ETHTOOL_LINK_MODE_10000baseKX4_Full_BIT) |
         MLX5_BITSHIFT(ETHTOOL_LINK_MODE_10000baseKR_Full_BIT) | MLX5_BITSHIFT(ETHTOOL_LINK_MODE_10000baseR_FEC_BIT)))
        priv->link_speed_capa |= ETH_LINK_SPEED_10G;
    ...
    if (sc &
        (MLX5_BITSHIFT(ETHTOOL_LINK_MODE_25000baseCR_Full_BIT) | MLX5_BITSHIFT(ETHTOOL_LINK_MODE_25000baseKR_Full_BIT) |
    ...
    dev_link.link_duplex = ((ecmd->duplex == DUPLEX_HALF) ? ETH_LINK_HALF_DUPLEX : ETH_LINK_FULL_DUPLEX);
    dev_link.link_autoneg = !(dev->data->dev_conf.link_speeds & ETH_LINK_SPEED_FIXED);
    if (((dev_link.link_speed && !dev_link.link_status) || (!dev_link.link_speed && dev_link.link_status))) {
        rte_errno = EAGAIN;
        return -rte_errno;
...
int mlx5_link_update(struct rte_eth_dev *dev, int wait_to_complete) {
    ...
    do {
        ...
        if (wait_to_complete && ret == -EAGAIN) {
            if (abs((int)difftime(time(NULL), start_time)) < MLX5_LINK_STATUS_TIMEOUT) {
                usleep(0);
                continue;
            } else {
                rte_errno = EBUSY;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_dev_get_flow_ctrl(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf) {
    struct ifreq ifr;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_dev_set_flow_ctrl(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf) {
    struct ifreq ifr;
    ...
    if (((fc_conf->mode & RTE_FC_FULL) == RTE_FC_FULL) || (fc_conf->mode & RTE_FC_RX_PAUSE))
        ethpause.rx_pause = 1;
    else
        ethpause.rx_pause = 0;

    if (((fc_conf->mode & RTE_FC_FULL) == RTE_FC_FULL) || (fc_conf->mode & RTE_FC_TX_PAUSE))
        ethpause.tx_pause = 1;
    else
        ethpause.tx_pause = 0;
    ret = mlx5_ifreq(dev, SIOCETHTOOL, &ifr);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_ibv_device_to_pci_addr(const struct ibv_device *device, struct rte_pci_addr *pci_addr) {
    FILE *file;
    char line[32];
    ...
    while (fgets(line, sizeof(line), file) == line) {
        ...
        if (sscanf(line,
                   ...
                   &pci_addr->domain, &pci_addr->bus, &pci_addr->devid, &pci_addr->function) == 4) {
            ret = 0;
            break;
        }
    }
 ...
 */
static void mlx5_dev_interrupt_device_fatal(struct mlx5_ibv_shared *sh) {
    uint32_t i;
...
void mlx5_dev_interrupt_handler(void *cb_arg) {
    ...
    for (;;) {
        ...
        assert(dev);
        if ((event.event_type == IBV_EVENT_PORT_ACTIVE || event.event_type == IBV_EVENT_PORT_ERR) &&
        ...
        DRV_LOG(DEBUG, "port %u cannot handle an unknown event (type %d)", dev->data->port_id, event.event_type);
        mlx5_glue->ack_async_event(&event);
    }
}
 ...
 *   Pointer to Ethernet device.
 */
static void mlx5_dev_shared_handler_uninstall(struct rte_eth_dev *dev) {
    struct mlx5_priv *priv = dev->data->dev_private;
    ...
    if (sh->port[priv->ibv_port - 1].ih_port_id >= RTE_MAX_ETHPORTS)
        goto exit;
    assert(sh->port[priv->ibv_port - 1].ih_port_id == (uint32_t)dev->data->port_id);
    assert(sh->intr_cnt);
 ...
 *   Pointer to Ethernet device.
 */
static void mlx5_dev_shared_handler_install(struct rte_eth_dev *dev) {
    struct mlx5_priv *priv = dev->data->dev_private;
    struct mlx5_ibv_shared *sh = priv->sh;
    int ret;
    int flags;
    ...
    rte_intr_callback_register(&sh->intr_handle, mlx5_dev_interrupt_handler, sh);
    sh->intr_cnt++;
exit:
    pthread_mutex_unlock(&sh->intr_mutex);
}
 ...
 *   Pointer to Ethernet device.
 */
void mlx5_dev_interrupt_handler_uninstall(struct rte_eth_dev *dev) { mlx5_dev_shared_handler_uninstall(dev); }
 ...
 *   Pointer to Ethernet device.
 */
void mlx5_dev_interrupt_handler_install(struct rte_eth_dev *dev) { mlx5_dev_shared_handler_install(dev); }
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_set_link_down(struct rte_eth_dev *dev) { return mlx5_set_flags(dev, ~IFF_UP, ~IFF_UP); }
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_set_link_up(struct rte_eth_dev *dev) { return mlx5_set_flags(dev, ~IFF_UP, IFF_UP); }
...
eth_tx_burst_t mlx5_select_tx_function(struct rte_eth_dev *dev) {
    ...
    int tso = !!(tx_offloads & (DEV_TX_OFFLOAD_TCP_TSO | DEV_TX_OFFLOAD_VXLAN_TNL_TSO | DEV_TX_OFFLOAD_GRE_TNL_TSO |
                                DEV_TX_OFFLOAD_IP_TNL_TSO | DEV_TX_OFFLOAD_UDP_TNL_TSO));
    int swp =
        !!(tx_offloads & (DEV_TX_OFFLOAD_IP_TNL_TSO | DEV_TX_OFFLOAD_UDP_TNL_TSO | DEV_TX_OFFLOAD_OUTER_IPV4_CKSUM));
    ...
    if (config->mps == MLX5_MPW_ENHANCED) {
        if (mlx5_check_vec_tx_support(dev) > 0) {
            ...
            DRV_LOG(DEBUG, "port %u selected enhanced MPW Tx function", dev->data->port_id);
        }
    } else if (config->mps && (config->txq_inline > 0)) {
        tx_pkt_burst = mlx5_tx_burst_mpw_inline;
        DRV_LOG(DEBUG, "port %u selected MPW inline Tx function", dev->data->port_id);
    } else if (config->mps) {
        tx_pkt_burst = mlx5_tx_burst_mpw;
        DRV_LOG(DEBUG, "port %u selected MPW Tx function", dev->data->port_id);
    }
    return tx_pkt_burst;
}
...
eth_rx_burst_t mlx5_select_rx_function(struct rte_eth_dev *dev) {
    ...
    if (mlx5_check_vec_rx_support(dev) > 0) {
        ...
        DRV_LOG(DEBUG, "port %u selected Rx vectorized function", dev->data->port_id);
 ...
 */
unsigned int mlx5_dev_to_port_id(const struct rte_device *dev, uint16_t *port_list, unsigned int port_list_n) {
    uint16_t id;
    unsigned int n = 0;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_port_to_eswitch_info(uint16_t port, uint16_t *es_domain_id, uint16_t *es_port_id) {
    struct rte_eth_dev *dev;
    struct mlx5_priv *priv;
...
int mlx5_sysfs_switch_info(unsigned int ifindex, struct mlx5_switch_info *info) {
    ...
    port_switch_id_set = fscanf(file, "%" SCNx64 "%c", &data.switch_id, &c) == 2 && c == '\n';
    fclose(file);
 ...
 */
void mlx5_nl_check_switch_info(bool num_vf_set, struct mlx5_switch_info *switch_info) {
 ...
 *   recognized parameters (if any).
 */
void mlx5_sysfs_check_switch_info(bool device_dir, struct mlx5_switch_info *switch_info) {
    switch (switch_info->name_type) {
    case MLX5_PHYS_PORT_NAME_TYPE_UNKNOWN:
        /*
         * Name is not recognized, assume the master,
 ...
 */
void mlx5_translate_port_name(const char *port_name_in, struct mlx5_switch_info *port_info_out) {
     ...
     */
    sc_items = sscanf(port_name_in, "%c%c%d%c%c%d", &pf_c1, &pf_c2, &port_info_out->pf_num, &vf_c1, &vf_c2,
                      &port_info_out->port_name);
    if (sc_items == 6 && pf_c1 == 'p' && pf_c2 == 'f' && vf_c1 == 'v' && vf_c2 == 'f') {
     ...
     * (support kernel ver >= 5.0, or OFED ver >= 4.6).
     */
    sc_items = sscanf(port_name_in, "%c%d", &pf_c1, &port_info_out->port_name);
