...
enum { TCA_PEDIT_KEY_EX_HTYPE = 1, TCA_PEDIT_KEY_EX_CMD = 2, __TCA_PEDIT_KEY_EX_MAX };
...
__extension__ struct tc_pedit_sel {
    tc_gen;
    unsigned char nkeys;
    unsigned char flags;
...
#define MLX5_TCF_FATE_ACTIONS (MLX5_FLOW_ACTION_DROP | MLX5_FLOW_ACTION_PORT_ID | MLX5_FLOW_ACTION_JUMP)

#define MLX5_TCF_VLAN_ACTIONS                                                                                          \
    (MLX5_FLOW_ACTION_OF_POP_VLAN | MLX5_FLOW_ACTION_OF_PUSH_VLAN | MLX5_FLOW_ACTION_OF_SET_VLAN_VID |                 \
     MLX5_FLOW_ACTION_OF_SET_VLAN_PCP)

#define MLX5_TCF_VXLAN_ACTIONS (MLX5_FLOW_ACTION_VXLAN_ENCAP | MLX5_FLOW_ACTION_VXLAN_DECAP)

#define MLX5_TCF_PEDIT_ACTIONS                                                                                         \
    (MLX5_FLOW_ACTION_SET_IPV4_SRC | MLX5_FLOW_ACTION_SET_IPV4_DST | MLX5_FLOW_ACTION_SET_IPV6_SRC |                   \
     MLX5_FLOW_ACTION_SET_IPV6_DST | MLX5_FLOW_ACTION_SET_TP_SRC | MLX5_FLOW_ACTION_SET_TP_DST |                       \
     MLX5_FLOW_ACTION_SET_TTL | MLX5_FLOW_ACTION_DEC_TTL | MLX5_FLOW_ACTION_SET_MAC_SRC |                              \
     MLX5_FLOW_ACTION_SET_MAC_DST)

#define MLX5_TCF_CONFIG_ACTIONS                                                                                        \
    (MLX5_FLOW_ACTION_PORT_ID | MLX5_FLOW_ACTION_JUMP | MLX5_FLOW_ACTION_OF_PUSH_VLAN |                                \
...
#define NUM_OF_PEDIT_KEYS(sz) (((sz) / SZ_PEDIT_KEY_VAL) + (((sz) % SZ_PEDIT_KEY_VAL) ? 1 : 0))
 ...
 *   rte_errno is set.
 */
static struct mlx5_flow_counter *flow_tcf_counter_new(void) {
 ...
 */
static void flow_tcf_pedit_key_set_mac(const struct rte_flow_action *actions, struct pedit_parser *p_parser) {
    int idx = p_parser->sel.nkeys;
    uint32_t off = actions->type == RTE_FLOW_ACTION_TYPE_SET_MAC_SRC ? offsetof(struct ether_hdr, s_addr)
                                                                     : offsetof(struct ether_hdr, d_addr);
    const struct rte_flow_action_set_mac *conf = (const struct rte_flow_action_set_mac *)actions->conf;
    ...
    memcpy(&p_parser->keys[idx].val, conf->mac_addr, SZ_PEDIT_KEY_VAL);
    idx++;
    p_parser->keys[idx].off = off + SZ_PEDIT_KEY_VAL;
    p_parser->keys[idx].mask = 0xFFFF0000;
    p_parser->keys_ex[idx].htype = TCA_PEDIT_KEY_EX_HDR_TYPE_ETH;
    p_parser->keys_ex[idx].cmd = TCA_PEDIT_KEY_EX_CMD_SET;
    memcpy(&p_parser->keys[idx].val, conf->mac_addr + SZ_PEDIT_KEY_VAL, ETHER_ADDR_LEN - SZ_PEDIT_KEY_VAL);
 ...
 */
static void flow_tcf_pedit_key_set_dec_ttl(const struct rte_flow_action *actions, struct pedit_parser *p_parser,
                                           uint64_t item_flags) {
    int idx = p_parser->sel.nkeys;
    ...
    if (item_flags & MLX5_FLOW_LAYER_OUTER_L3_IPV4) {
        ...
        p_parser->keys[idx].off = offsetof(struct ipv4_hdr, time_to_live);
    }
    if (item_flags & MLX5_FLOW_LAYER_OUTER_L3_IPV6) {
        p_parser->keys_ex[idx].htype = TCA_PEDIT_KEY_EX_HDR_TYPE_IP6;
        p_parser->keys[idx].off = offsetof(struct ipv6_hdr, hop_limits);
    }
    if (actions->type == RTE_FLOW_ACTION_TYPE_DEC_TTL) {
        ...
        p_parser->keys_ex[idx].cmd = TCA_PEDIT_KEY_EX_CMD_SET;
        p_parser->keys[idx].val = (__u32)((const struct rte_flow_action_set_ttl *)actions->conf)->ttl_value;
    }
    p_parser->sel.nkeys = (++idx);
}
 ...
 *   flags of all items presented
 */
static void flow_tcf_pedit_key_set_tp_port(const struct rte_flow_action *actions, struct pedit_parser *p_parser,
                                           uint64_t item_flags) {
    int idx = p_parser->sel.nkeys;
    ...
    p_parser->keys[idx].off = actions->type == RTE_FLOW_ACTION_TYPE_SET_TP_SRC ? offsetof(struct tcp_hdr, src_port)
                                                                               : offsetof(struct tcp_hdr, dst_port);
    p_parser->keys[idx].mask = 0xFFFF0000;
    p_parser->keys[idx].val = (__u32)((const struct rte_flow_action_set_tp *)actions->conf)->port;
    p_parser->sel.nkeys = (++idx);
}
 ...
 *   pointer to pedit_parser
 */
static void flow_tcf_pedit_key_set_ipv6_addr(const struct rte_flow_action *actions, struct pedit_parser *p_parser) {
    int idx = p_parser->sel.nkeys;
    int keys = NUM_OF_PEDIT_KEYS(IPV6_ADDR_LEN);
    int off_base = actions->type == RTE_FLOW_ACTION_TYPE_SET_IPV6_SRC ? offsetof(struct ipv6_hdr, src_addr)
                                                                      : offsetof(struct ipv6_hdr, dst_addr);
    const struct rte_flow_action_set_ipv6 *conf = (const struct rte_flow_action_set_ipv6 *)actions->conf;
    ...
    for (int i = 0; i < keys; i++, idx++) {
        ...
        p_parser->keys[idx].mask = ~UINT32_MAX;
        memcpy(&p_parser->keys[idx].val, conf->ipv6_addr + i * SZ_PEDIT_KEY_VAL, SZ_PEDIT_KEY_VAL);
    }
 ...
 *   pointer to pedit_parser
 */
static void flow_tcf_pedit_key_set_ipv4_addr(const struct rte_flow_action *actions, struct pedit_parser *p_parser) {
    int idx = p_parser->sel.nkeys;

    p_parser->keys_ex[idx].htype = TCA_PEDIT_KEY_EX_HDR_TYPE_IP4;
    p_parser->keys_ex[idx].cmd = TCA_PEDIT_KEY_EX_CMD_SET;
    p_parser->keys[idx].off = actions->type == RTE_FLOW_ACTION_TYPE_SET_IPV4_SRC ? offsetof(struct ipv4_hdr, src_addr)
                                                                                 : offsetof(struct ipv4_hdr, dst_addr);
    p_parser->keys[idx].mask = ~UINT32_MAX;
    p_parser->keys[idx].val = ((const struct rte_flow_action_set_ipv4 *)actions->conf)->ipv4_addr;
    p_parser->sel.nkeys = (++idx);
}
 ...
 */
static void flow_tcf_create_pedit_mnl_msg(struct nlmsghdr *nl, const struct rte_flow_action **actions,
                                          uint64_t item_flags) {
    ...
    for (; (*actions)->type != RTE_FLOW_ACTION_TYPE_END; (*actions)++) {
        ...
        case RTE_FLOW_ACTION_TYPE_SET_TP_DST:
            flow_tcf_pedit_key_set_tp_port(*actions, &p_parser, item_flags);
            break;
        case RTE_FLOW_ACTION_TYPE_SET_TTL:
        case RTE_FLOW_ACTION_TYPE_DEC_TTL:
            flow_tcf_pedit_key_set_dec_ttl(*actions, &p_parser, item_flags);
            break;
...
pedit_mnl_msg_done:
    ...
    mnl_attr_put(nl, TCA_PEDIT_PARMS_EX, sizeof(p_parser.sel) + p_parser.sel.nkeys * sizeof(struct tc_pedit_key),
                 &p_parser);
    na_pedit_keys = mnl_attr_nest_start(nl, TCA_PEDIT_KEYS_EX | NLA_F_NESTED);
    for (int i = 0; i < p_parser.sel.nkeys; i++) {
        struct nlattr *na_pedit_key = mnl_attr_nest_start(nl, TCA_PEDIT_KEY_EX | NLA_F_NESTED);
        mnl_attr_put_u16(nl, TCA_PEDIT_KEY_EX_HTYPE, p_parser.keys_ex[i].htype);
        mnl_attr_put_u16(nl, TCA_PEDIT_KEY_EX_CMD, p_parser.keys_ex[i].cmd);
 ...
 */
static int flow_tcf_get_pedit_actions_size(const struct rte_flow_action **actions, uint64_t *action_flags) {
    int pedit_size = 0;
    int keys = 0;
    uint64_t flags = 0;

    pedit_size += SZ_NLATTR_NEST +                             /* na_act_index. */
                  SZ_NLATTR_STRZ_OF("pedit") + SZ_NLATTR_NEST; /* TCA_ACT_OPTIONS. */
    for (; (*actions)->type != RTE_FLOW_ACTION_TYPE_END; (*actions)++) {
        switch ((*actions)->type) {
        case RTE_FLOW_ACTION_TYPE_SET_IPV4_SRC:
...
get_pedit_action_size_done:
    /* TCA_PEDIT_PARAMS_EX */
    pedit_size += SZ_NLATTR_DATA_OF(sizeof(struct tc_pedit_sel) + keys * sizeof(struct tc_pedit_key));
 ...
 */
static const void *flow_tcf_item_mask(const struct rte_flow_item *item, const void *mask_default,
                                      const void *mask_supported, const void *mask_empty, size_t mask_size,
    ...
    for (i = 0; i != mask_size; ++i) {
        if (!mask[i])
            continue;
        if ((mask[i] | ((const uint8_t *)mask_supported)[i]) != ((const uint8_t *)mask_supported)[i]) {
            rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask,
        ...
        if (item->last &&
            ...
            rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM_LAST, item->last,
 ...
 */
static unsigned int flow_tcf_build_ptoi_table(struct rte_eth_dev *dev, struct flow_tcf_ptoi *ptoi, unsigned int len) {
 ...
 */
static int flow_tcf_validate_attributes(const struct rte_flow_attr *attr, struct rte_flow_error *error) {
    /*
    ...
    if (attr->group > MLX5_TCF_GROUP_ID_MAX)
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_GROUP, attr,
                                  "group ID larger than " RTE_STR(MLX5_TCF_GROUP_ID_MAX) " isn't supported");
        ...
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_PRIORITY, attr,
                                  "priority more than " RTE_STR(MLX5_TCF_GROUP_PRIORITY_MAX) " is not supported");
    if (!attr->ingress)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR_INGRESS, attr, "only ingress is supported");
    if (attr->egress)
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_INGRESS, attr, "egress is not supported");
    return 0;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 **/
static int flow_tcf_validate_vxlan_encap_eth(const struct rte_flow_item *item, struct rte_flow_error *error) {
    ...
    if (memcmp(&mask->dst, &flow_tcf_mask_empty.eth.dst, sizeof(flow_tcf_mask_empty.eth.dst))) {
        if (memcmp(&mask->dst, &rte_flow_item_eth_mask.dst, sizeof(rte_flow_item_eth_mask.dst)))
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask,
                                      "no support for partial mask on"
                                      " \"eth.dst\" field");
    }
    if (memcmp(&mask->src, &flow_tcf_mask_empty.eth.src, sizeof(flow_tcf_mask_empty.eth.src))) {
        if (memcmp(&mask->src, &rte_flow_item_eth_mask.src, sizeof(rte_flow_item_eth_mask.src)))
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask,
                                      "no support for partial mask on"
                                      " \"eth.src\" field");
    }
    if (mask->type != RTE_BE16(0x0000)) {
        if (mask->type != RTE_BE16(0xffff))
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask,
                                      "no support for partial mask on"
                                      " \"eth.type\" field");
        DRV_LOG(WARNING, "outer ethernet type field"
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 **/
static int flow_tcf_validate_vxlan_encap_ipv4(const struct rte_flow_item *item, struct rte_flow_error *error) {
    ...
    if (mask->hdr.type_of_service && mask->hdr.type_of_service != 0xff)
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask,
                                  "no support for partial mask on"
                                  " \"ipv4.hdr.type_of_service\" field"
                                  " for vxlan encapsulation");
    if (mask->hdr.time_to_live && mask->hdr.time_to_live != 0xff)
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask,
                                  "no support for partial mask on"
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 **/
static int flow_tcf_validate_vxlan_encap_ipv6(const struct rte_flow_item *item, struct rte_flow_error *error) {
    ...
    if (memcmp(&mask->hdr.dst_addr, &flow_tcf_mask_empty.ipv6.hdr.dst_addr, IPV6_ADDR_LEN)) {
        if (memcmp(&mask->hdr.dst_addr, &rte_flow_item_ipv6_mask.hdr.dst_addr, IPV6_ADDR_LEN))
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask,
                                      "no support for partial mask on"
    ...
    if (memcmp(&mask->hdr.src_addr, &flow_tcf_mask_empty.ipv6.hdr.src_addr, IPV6_ADDR_LEN)) {
        if (memcmp(&mask->hdr.src_addr, &rte_flow_item_ipv6_mask.hdr.src_addr, IPV6_ADDR_LEN))
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask,
                                      "no support for partial mask on"
    ...
    msk6 = (rte_be_to_cpu_32(mask->hdr.vtc_flow) >> IPV6_HDR_TC_SHIFT) & 0xff;
    if (msk6 && msk6 != 0xff)
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask,
                                  "no support for partial mask on"
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 **/
static int flow_tcf_validate_vxlan_encap_udp(const struct rte_flow_item *item, struct rte_flow_error *error) {
    ...
    if (mask->hdr.src_port != RTE_BE16(0x0000)) {
        if (mask->hdr.src_port != RTE_BE16(0xffff))
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask,
                                      ...
                                      " for vxlan encapsulation");
        DRV_LOG(WARNING, "outer UDP source port cannot be"
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 **/
static int flow_tcf_validate_vxlan_encap_vni(const struct rte_flow_item *item, struct rte_flow_error *error) {
    ...
    if (!mask->vni[0] && !mask->vni[1] && !mask->vni[2])
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
                                  ...
                                  "for vxlan encapsulation");
    if (mask->vni[0] != 0xff || mask->vni[1] != 0xff || mask->vni[2] != 0xff)
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask,
                                  "no support for partial mask on"
                                  " \"vxlan.vni\" field");

    if (!spec->vni[0] && !spec->vni[1] && !spec->vni[2])
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item, "vxlan vni cannot be 0");
    return 0;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 **/
static int flow_tcf_validate_vxlan_encap(const struct rte_flow_action *action, struct rte_flow_error *error) {
    ...
    items = ((const struct rte_flow_action_vxlan_encap *)action->conf)->definition;
    if (!items)
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, action,
                                  "Missing vxlan tunnel"
    ...
    for (; items->type != RTE_FLOW_ITEM_TYPE_END; items++) {
        ...
        case RTE_FLOW_ITEM_TYPE_ETH:
            ret = mlx5_flow_validate_item_eth(items, item_flags, error);
            if (ret < 0)
                return ret;
        ...
        case RTE_FLOW_ITEM_TYPE_IPV4:
            ret = mlx5_flow_validate_item_ipv4(items, item_flags, &flow_tcf_mask_supported.ipv4, error);
            if (ret < 0)
                return ret;
        ...
        case RTE_FLOW_ITEM_TYPE_IPV6:
            ret = mlx5_flow_validate_item_ipv6(items, item_flags, &flow_tcf_mask_supported.ipv6, error);
            if (ret < 0)
                return ret;
        ...
        case RTE_FLOW_ITEM_TYPE_UDP:
            ret = mlx5_flow_validate_item_udp(items, item_flags, 0xFF, error);
            if (ret < 0)
                return ret;
        ...
        case RTE_FLOW_ITEM_TYPE_VXLAN:
            ret = mlx5_flow_validate_item_vxlan(items, item_flags, error);
            if (ret < 0)
                return ret;
            ...
            break;
        default:
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, items,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 **/
static int flow_tcf_validate_vxlan_decap_udp(const struct rte_flow_item *udp, struct rte_flow_error *error) {
    ...
    if (!spec)
         ...
         */
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, udp,
                                  "NULL UDP port specification"
    ...
    if (mask->hdr.dst_port != RTE_BE16(0x0000)) {
        if (mask->hdr.dst_port != RTE_BE16(0xffff))
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask,
                                      ...
                                      " \"udp.hdr.dst_port\" field");
        if (!spec->hdr.dst_port)
            return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, udp, "zero decap local UDP port");
    } else {
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, udp,
    ...
    if (mask->hdr.src_port != RTE_BE16(0x0000)) {
        if (mask->hdr.src_port != RTE_BE16(0xffff))
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask,
                                      "no support for partial mask on"
                                      " \"udp.hdr.src_port\" field");
        DRV_LOG(WARNING, "outer UDP local port cannot be "
                         "forced for VXLAN encapsulation, "
                         "parameter ignored");
    }
    return 0;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_tcf_validate(struct rte_eth_dev *dev, const struct rte_flow_attr *attr,
    ...
    claim_nonzero(flow_tcf_build_ptoi_table(dev, ptoi, PTOI_TABLE_SZ_MAX(dev)));
    ...
    for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
            ...
            if (conf.port_id->original)
                ...
                for (i = 0; ptoi[i].ifindex; ++i)
                    if (ptoi[i].port_id == conf.port_id->id)
                        break;
            if (!ptoi[i].ifindex)
                return rte_flow_error_set(error, ENODEV, RTE_FLOW_ERROR_TYPE_ACTION_CONF, conf.port_id,
        ...
        case RTE_FLOW_ACTION_TYPE_JUMP:
            ...
            if (attr->group >= conf.jump->group)
                return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, actions,
        ...
        case RTE_FLOW_ACTION_TYPE_OF_PUSH_VLAN: {
            ...
            if (ethertype != RTE_BE16(ETH_P_8021Q) && ethertype != RTE_BE16(ETH_P_8021AD))
                return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, actions,
        ...
        case RTE_FLOW_ACTION_TYPE_SET_MAC_DST:
            ...
            break;
        default:
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, actions, "action not supported");
        }
        if (current_action_flag & MLX5_TCF_CONFIG_ACTIONS) {
            if (!actions->conf)
                return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF, actions,
                                          "action configuration not set");
        }
        if ((current_action_flag & MLX5_TCF_PEDIT_ACTIONS) && pedit_validated)
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, actions,
                                      "set actions should be "
                                      "listed successively");
        if ((current_action_flag & ~MLX5_TCF_PEDIT_ACTIONS) && (action_flags & MLX5_TCF_PEDIT_ACTIONS))
            pedit_validated = 1;
        if ((current_action_flag & MLX5_TCF_FATE_ACTIONS) && (action_flags & MLX5_TCF_FATE_ACTIONS))
            return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, actions,
                                      "can't have multiple fate"
                                      " actions");
        if ((current_action_flag & MLX5_TCF_VXLAN_ACTIONS) && (action_flags & MLX5_TCF_VXLAN_ACTIONS))
            return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, actions,
                                      "can't have multiple vxlan"
                                      " actions");
        if ((current_action_flag & MLX5_TCF_VXLAN_ACTIONS) && (action_flags & MLX5_TCF_VLAN_ACTIONS))
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, actions,
    ...
    for (; items->type != RTE_FLOW_ITEM_TYPE_END; items++) {
        ...
        case RTE_FLOW_ITEM_TYPE_PORT_ID:
            ...
            mask.port_id =
                flow_tcf_item_mask(items, &rte_flow_item_port_id_mask, &flow_tcf_mask_supported.port_id,
                                   &flow_tcf_mask_empty.port_id, sizeof(flow_tcf_mask_supported.port_id), error);
            if (!mask.port_id)
                return -rte_errno;
            ...
            if (mask.port_id->id && mask.port_id->id != 0xffffffff)
                return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask.port_id,
                                          "no support for partial mask on"
                                          " \"id\" field");
            if (!mask.port_id->id)
                i = 0;
                ...
                for (i = 0; ptoi[i].ifindex; ++i)
                    if (ptoi[i].port_id == spec.port_id->id)
                        break;
            if (!ptoi[i].ifindex)
                return rte_flow_error_set(error, ENODEV, RTE_FLOW_ERROR_TYPE_ITEM_SPEC, spec.port_id,
                                          "missing data to convert port ID to"
                                          " ifindex");
            if (in_port_id_set && ptoi[i].ifindex != tcm_ifindex)
                return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_SPEC, spec.port_id,
        ...
        case RTE_FLOW_ITEM_TYPE_ETH:
            ...
            if (ret < 0)
                return ret;
            item_flags |= (item_flags & MLX5_FLOW_LAYER_TUNNEL) ? MLX5_FLOW_LAYER_INNER_L2 : MLX5_FLOW_LAYER_OUTER_L2;
            /* TODO:
             * Redundant check due to different supported mask.
             * Same for the rest of items.
             */
            mask.eth = flow_tcf_item_mask(items, &rte_flow_item_eth_mask, &flow_tcf_mask_supported.eth,
                                          &flow_tcf_mask_empty.eth, sizeof(flow_tcf_mask_supported.eth), error);
            if (!mask.eth)
                return -rte_errno;
            if (mask.eth->type && mask.eth->type != RTE_BE16(0xffff))
                return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask.eth,
                                          "no support for partial mask on"
                                          " \"type\" field");
            assert(items->spec);
            spec.eth = items->spec;
            if (mask.eth->type && (item_flags & MLX5_FLOW_LAYER_TUNNEL) && inner_etype != RTE_BE16(ETH_P_ALL) &&
                inner_etype != spec.eth->type)
                return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, items, "inner eth_type conflict");
            if (mask.eth->type && !(item_flags & MLX5_FLOW_LAYER_TUNNEL) && outer_etype != RTE_BE16(ETH_P_ALL) &&
                outer_etype != spec.eth->type)
                return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, items, "outer eth_type conflict");
            if (mask.eth->type) {
                if (item_flags & MLX5_FLOW_LAYER_TUNNEL)
                    inner_etype = spec.eth->type;
                else
        ...
        case RTE_FLOW_ITEM_TYPE_VLAN:
            if (item_flags & MLX5_FLOW_LAYER_TUNNEL)
                return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, items,
                                          "inner tunnel VLAN"
                                          " is not supported");
            ret = mlx5_flow_validate_item_vlan(items, item_flags, error);
            if (ret < 0)
                return ret;
            item_flags |= MLX5_FLOW_LAYER_OUTER_VLAN;
            mask.vlan = flow_tcf_item_mask(items, &rte_flow_item_vlan_mask, &flow_tcf_mask_supported.vlan,
                                           &flow_tcf_mask_empty.vlan, sizeof(flow_tcf_mask_supported.vlan), error);
            if (!mask.vlan)
                return -rte_errno;
            if ((mask.vlan->tci & RTE_BE16(0xe000) && (mask.vlan->tci & RTE_BE16(0xe000)) != RTE_BE16(0xe000)) ||
                (mask.vlan->tci & RTE_BE16(0x0fff) && (mask.vlan->tci & RTE_BE16(0x0fff)) != RTE_BE16(0x0fff)) ||
                (mask.vlan->inner_type && mask.vlan->inner_type != RTE_BE16(0xffff)))
                return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask.vlan,
                                          "no support for partial masks on"
                                          " \"tci\" (PCP and VID parts) and"
                                          " \"inner_type\" fields");
            if (outer_etype != RTE_BE16(ETH_P_ALL) && outer_etype != RTE_BE16(ETH_P_8021Q))
                return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, items,
                                          "outer eth_type conflict,"
                                          " must be 802.1Q");
            outer_etype = RTE_BE16(ETH_P_8021Q);
            assert(items->spec);
            spec.vlan = items->spec;
            if (mask.vlan->inner_type && vlan_etype != RTE_BE16(ETH_P_ALL) && vlan_etype != spec.vlan->inner_type)
                return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, items, "vlan eth_type conflict");
        ...
        case RTE_FLOW_ITEM_TYPE_IPV4:
            ...
            if (ret < 0)
                return ret;
            item_flags |=
                (item_flags & MLX5_FLOW_LAYER_TUNNEL) ? MLX5_FLOW_LAYER_INNER_L3_IPV4 : MLX5_FLOW_LAYER_OUTER_L3_IPV4;
            mask.ipv4 = flow_tcf_item_mask(items, &rte_flow_item_ipv4_mask, &flow_tcf_mask_supported.ipv4,
                                           &flow_tcf_mask_empty.ipv4, sizeof(flow_tcf_mask_supported.ipv4), error);
            if (!mask.ipv4)
                return -rte_errno;
            if (mask.ipv4->hdr.next_proto_id && mask.ipv4->hdr.next_proto_id != 0xff)
                return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask.ipv4,
                                          "no support for partial mask on"
            ...
            if (item_flags & MLX5_FLOW_LAYER_TUNNEL) {
                if (inner_etype != RTE_BE16(ETH_P_ALL) && inner_etype != RTE_BE16(ETH_P_IP))
                    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, items,
                                              "inner eth_type conflict,"
                                              " IPv4 is required");
                inner_etype = RTE_BE16(ETH_P_IP);
            } else if (item_flags & MLX5_FLOW_LAYER_OUTER_VLAN) {
                if (vlan_etype != RTE_BE16(ETH_P_ALL) && vlan_etype != RTE_BE16(ETH_P_IP))
                    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, items,
                                              "vlan eth_type conflict,"
                                              " IPv4 is required");
                vlan_etype = RTE_BE16(ETH_P_IP);
            } else {
                if (outer_etype != RTE_BE16(ETH_P_ALL) && outer_etype != RTE_BE16(ETH_P_IP))
                    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, items,
                                              "eth_type conflict,"
                                              " IPv4 is required");
        ...
        case RTE_FLOW_ITEM_TYPE_IPV6:
            ...
            if (ret < 0)
                return ret;
            item_flags |=
                (item_flags & MLX5_FLOW_LAYER_TUNNEL) ? MLX5_FLOW_LAYER_INNER_L3_IPV6 : MLX5_FLOW_LAYER_OUTER_L3_IPV6;
            mask.ipv6 = flow_tcf_item_mask(items, &rte_flow_item_ipv6_mask, &flow_tcf_mask_supported.ipv6,
                                           &flow_tcf_mask_empty.ipv6, sizeof(flow_tcf_mask_supported.ipv6), error);
            if (!mask.ipv6)
                return -rte_errno;
            if (mask.ipv6->hdr.proto && mask.ipv6->hdr.proto != 0xff)
                return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask.ipv6,
                                          "no support for partial mask on"
                                          " \"hdr.proto\" field");
            else if (mask.ipv6->hdr.proto)
                next_protocol = ((const struct rte_flow_item_ipv6 *)(items->spec))->hdr.proto;
            if (item_flags & MLX5_FLOW_LAYER_TUNNEL) {
                if (inner_etype != RTE_BE16(ETH_P_ALL) && inner_etype != RTE_BE16(ETH_P_IPV6))
                    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, items,
                                              "inner eth_type conflict,"
                                              " IPv6 is required");
                inner_etype = RTE_BE16(ETH_P_IPV6);
            } else if (item_flags & MLX5_FLOW_LAYER_OUTER_VLAN) {
                if (vlan_etype != RTE_BE16(ETH_P_ALL) && vlan_etype != RTE_BE16(ETH_P_IPV6))
                    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, items,
                                              "vlan eth_type conflict,"
                                              " IPv6 is required");
                vlan_etype = RTE_BE16(ETH_P_IPV6);
            } else {
                if (outer_etype != RTE_BE16(ETH_P_ALL) && outer_etype != RTE_BE16(ETH_P_IPV6))
                    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, items,
                                              "eth_type conflict,"
                                              " IPv6 is required");
            ...
            break;
        case RTE_FLOW_ITEM_TYPE_UDP:
            ret = mlx5_flow_validate_item_udp(items, item_flags, next_protocol, error);
            if (ret < 0)
                return ret;
            item_flags |=
                (item_flags & MLX5_FLOW_LAYER_TUNNEL) ? MLX5_FLOW_LAYER_INNER_L4_UDP : MLX5_FLOW_LAYER_OUTER_L4_UDP;
            mask.udp = flow_tcf_item_mask(items, &rte_flow_item_udp_mask, &flow_tcf_mask_supported.udp,
                                          &flow_tcf_mask_empty.udp, sizeof(flow_tcf_mask_supported.udp), error);
            if (!mask.udp)
                return -rte_errno;
            /*
        ...
        case RTE_FLOW_ITEM_TYPE_TCP:
            ret = mlx5_flow_validate_item_tcp(items, item_flags, next_protocol, &flow_tcf_mask_supported.tcp, error);
            if (ret < 0)
                return ret;
            item_flags |=
                (item_flags & MLX5_FLOW_LAYER_TUNNEL) ? MLX5_FLOW_LAYER_INNER_L4_TCP : MLX5_FLOW_LAYER_OUTER_L4_TCP;
            mask.tcp = flow_tcf_item_mask(items, &rte_flow_item_tcp_mask, &flow_tcf_mask_supported.tcp,
                                          &flow_tcf_mask_empty.tcp, sizeof(flow_tcf_mask_supported.tcp), error);
            if (!mask.tcp)
                return -rte_errno;
            break;
        case RTE_FLOW_ITEM_TYPE_VXLAN:
            ...
            if (ret < 0)
                return ret;
            item_flags |= MLX5_FLOW_LAYER_VXLAN;
            mask.vxlan = flow_tcf_item_mask(items, &rte_flow_item_vxlan_mask, &flow_tcf_mask_supported.vxlan,
                                            &flow_tcf_mask_empty.vxlan, sizeof(flow_tcf_mask_supported.vxlan), error);
            if (!mask.vxlan)
                return -rte_errno;
            if (mask.vxlan->vni[0] != 0xff || mask.vxlan->vni[1] != 0xff || mask.vxlan->vni[2] != 0xff)
                return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM_MASK, mask.vxlan,
             ...
             */
            if (!(item_flags & (MLX5_FLOW_LAYER_OUTER_L3_IPV4 | MLX5_FLOW_LAYER_OUTER_L3_IPV6)))
             ...
             */
            assert(outer_udp);
            ret = flow_tcf_validate_vxlan_decap_udp(outer_udp, error);
            if (ret < 0)
                return ret;
            ...
            break;
        default:
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, items, "item not supported");
        }
    }
    if ((action_flags & MLX5_TCF_PEDIT_ACTIONS) && (action_flags & MLX5_FLOW_ACTION_DROP))
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, actions,
                                  "set action is not compatible with "
                                  "drop action");
    if ((action_flags & MLX5_TCF_PEDIT_ACTIONS) && !(action_flags & MLX5_FLOW_ACTION_PORT_ID))
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, actions,
                                  "set action must be followed by "
                                  "port_id action");
    if (action_flags & (MLX5_FLOW_ACTION_SET_IPV4_SRC | MLX5_FLOW_ACTION_SET_IPV4_DST)) {
        if (!(item_flags & MLX5_FLOW_LAYER_OUTER_L3_IPV4))
            return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, actions,
                                      "no ipv4 item found in"
                                      " pattern");
    }
    if (action_flags & (MLX5_FLOW_ACTION_SET_IPV6_SRC | MLX5_FLOW_ACTION_SET_IPV6_DST)) {
        if (!(item_flags & MLX5_FLOW_LAYER_OUTER_L3_IPV6))
            return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, actions,
                                      "no ipv6 item found in"
                                      " pattern");
    }
    if (action_flags & (MLX5_FLOW_ACTION_SET_TP_SRC | MLX5_FLOW_ACTION_SET_TP_DST)) {
        if (!(item_flags & (MLX5_FLOW_LAYER_OUTER_L4_UDP | MLX5_FLOW_LAYER_OUTER_L4_TCP)))
            return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, actions,
     ...
     */
    if ((action_flags & MLX5_FLOW_ACTION_OF_PUSH_VLAN) && (action_flags & MLX5_FLOW_ACTION_PORT_ID) &&
     ...
     */
    if ((action_flags & MLX5_TCF_VLAN_ACTIONS) && !(action_flags & MLX5_FLOW_ACTION_PORT_ID))
        return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, actions,
                                  "vlan actions are supported"
                                  " only with port_id action");
    if ((action_flags & MLX5_TCF_VXLAN_ACTIONS) && !(action_flags & MLX5_FLOW_ACTION_PORT_ID))
    ...
    if (!(action_flags & MLX5_TCF_FATE_ACTIONS))
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, actions, "no fate action is found");
    if (action_flags & (MLX5_FLOW_ACTION_SET_TTL | MLX5_FLOW_ACTION_DEC_TTL)) {
        if (!(item_flags & (MLX5_FLOW_LAYER_OUTER_L3_IPV4 | MLX5_FLOW_LAYER_OUTER_L3_IPV6)))
            return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, actions, "no IP found in pattern");
    }
    if (action_flags & (MLX5_FLOW_ACTION_SET_MAC_SRC | MLX5_FLOW_ACTION_SET_MAC_DST)) {
        if (!(item_flags & MLX5_FLOW_LAYER_OUTER_L2))
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, actions,
                                      "no ethernet found in"
                                      " pattern");
    }
    if ((action_flags & MLX5_FLOW_ACTION_VXLAN_DECAP) && !(item_flags & MLX5_FLOW_LAYER_VXLAN))
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                                  "no VNI pattern found"
                                  " for vxlan decap action");
    if ((action_flags & MLX5_FLOW_ACTION_VXLAN_ENCAP) && (item_flags & MLX5_FLOW_LAYER_TUNNEL))
        return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
 ...
 */
static int flow_tcf_get_items_size(const struct rte_flow_attr *attr, const struct rte_flow_item items[],
                                   uint64_t *action_flags) {
    int size = 0;

    size += SZ_NLATTR_STRZ_OF("flower") + SZ_NLATTR_TYPE_OF(uint16_t) + /* Outer ether type. */
    ...
    for (; items->type != RTE_FLOW_ITEM_TYPE_END; items++) {
        ...
        case RTE_FLOW_ITEM_TYPE_IPV6: {
            ...
            if (ipv6 && ipv6->hdr.hop_limits)
                size += SZ_NLATTR_TYPE_OF(uint8_t) * 2;
            if (ipv6 && (rte_be_to_cpu_32(ipv6->hdr.vtc_flow) & (0xfful << IPV6_HDR_TC_SHIFT)))
                size += SZ_NLATTR_TYPE_OF(uint8_t) * 2;
            break;
        }
        case RTE_FLOW_ITEM_TYPE_UDP:
 ...
 */
static int flow_tcf_vxlan_encap_size(const struct rte_flow_action *action) {
    const struct rte_flow_item *items;
    int size = 0;
 ...
 */
static int flow_tcf_get_actions_and_size(const struct rte_flow_action actions[], uint64_t *action_flags) {
    int size = 0;
    ...
    for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
        ...
        case RTE_FLOW_ACTION_TYPE_PORT_ID:
            size += SZ_NLATTR_NEST +                               /* na_act_index. */
                    SZ_NLATTR_STRZ_OF("mirred") + SZ_NLATTR_NEST + /* TCA_ACT_OPTIONS. */
                    SZ_NLATTR_TYPE_OF(struct tc_mirred);
            flags |= MLX5_FLOW_ACTION_PORT_ID;
            break;
        case RTE_FLOW_ACTION_TYPE_JUMP:
            size += SZ_NLATTR_NEST +                             /* na_act_index. */
                    SZ_NLATTR_STRZ_OF("gact") + SZ_NLATTR_NEST + /* TCA_ACT_OPTIONS. */
        ...
        action_of_vlan:
            size += SZ_NLATTR_NEST +                             /* na_act_index. */
                    SZ_NLATTR_STRZ_OF("vlan") + SZ_NLATTR_NEST + /* TCA_ACT_OPTIONS. */
                    SZ_NLATTR_TYPE_OF(struct tc_vlan) + SZ_NLATTR_TYPE_OF(uint16_t) +
                    /* VLAN protocol. */
                    SZ_NLATTR_TYPE_OF(uint16_t) + /* VLAN ID. */
                    SZ_NLATTR_TYPE_OF(uint8_t);   /* VLAN prio. */
            break;
        case RTE_FLOW_ACTION_TYPE_VXLAN_ENCAP:
            size += SZ_NLATTR_NEST +                                   /* na_act_index. */
                    SZ_NLATTR_STRZ_OF("tunnel_key") + SZ_NLATTR_NEST + /* TCA_ACT_OPTIONS. */
                    SZ_NLATTR_TYPE_OF(uint8_t);
            size += SZ_NLATTR_TYPE_OF(struct tc_tunnel_key);
            size += flow_tcf_vxlan_encap_size(actions) + RTE_ALIGN_CEIL /* preceding encap params. */
                    (sizeof(struct flow_tcf_vxlan_encap), MNL_ALIGNTO);
        ...
        case RTE_FLOW_ACTION_TYPE_VXLAN_DECAP:
            ...
            size += RTE_ALIGN_CEIL /* preceding decap params. */
                (sizeof(struct flow_tcf_vxlan_decap), MNL_ALIGNTO);
        ...
        case RTE_FLOW_ACTION_TYPE_SET_MAC_SRC:
        case RTE_FLOW_ACTION_TYPE_SET_MAC_DST:
            size += flow_tcf_get_pedit_actions_size(&actions, &flags);
            break;
        default:
            DRV_LOG(WARNING,
 ...
 */
static struct mlx5_flow *flow_tcf_prepare(const struct rte_flow_attr *attr, const struct rte_flow_item items[],
                                          const struct rte_flow_action actions[], struct rte_flow_error *error) {
    size_t size = RTE_ALIGN_CEIL(sizeof(struct mlx5_flow), alignof(struct flow_tcf_tunnel_hdr)) +
                  MNL_ALIGN(sizeof(struct nlmsghdr)) + MNL_ALIGN(sizeof(struct tcmsg));
    ...
    if (action_flags & MLX5_FLOW_ACTION_VXLAN_ENCAP) {
        sp = RTE_PTR_ALIGN(sp, alignof(struct flow_tcf_tunnel_hdr));
        tun = sp;
        sp += RTE_ALIGN_CEIL(sizeof(struct flow_tcf_vxlan_encap), MNL_ALIGNTO);
#ifndef NDEBUG
        size -= RTE_ALIGN_CEIL(sizeof(struct flow_tcf_vxlan_encap), MNL_ALIGNTO);
#endif
    } else if (action_flags & MLX5_FLOW_ACTION_VXLAN_DECAP) {
        sp = RTE_PTR_ALIGN(sp, alignof(struct flow_tcf_tunnel_hdr));
        tun = sp;
        sp += RTE_ALIGN_CEIL(sizeof(struct flow_tcf_vxlan_decap), MNL_ALIGNTO);
#ifndef NDEBUG
        size -= RTE_ALIGN_CEIL(sizeof(struct flow_tcf_vxlan_decap), MNL_ALIGNTO);
#endif
    } else {
...
#ifndef NDEBUG
                .nlsize = size - RTE_ALIGN_CEIL(sizeof(struct mlx5_flow), alignof(struct flow_tcf_tunnel_hdr)),
#endif
 ...
 */
static int flow_tcf_translate_action_count(struct rte_eth_dev *dev __rte_unused, struct mlx5_flow *dev_flow,
                                           struct rte_flow_error *error) {
    ...
    if (!flow->counter) {
        ...
        if (!flow->counter)
            return rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_ACTION, NULL,
                                      "cannot get counter"
                                      " context.");
    }
    return 0;
 ...
 */
static inline rte_be32_t vxlan_vni_as_be32(const uint8_t vni[3]) {
    union {
        uint8_t vni[4];
        rte_be32_t dword;
    } ret = {
 ...
 */
static void flow_tcf_parse_vxlan_encap_eth(const struct rte_flow_item_eth *spec, const struct rte_flow_item_eth *mask,
                                           struct flow_tcf_vxlan_encap *encap) {
    /* Item must be validated before. No redundant checks. */
    assert(spec);
    if (!mask || !memcmp(&mask->dst, &rte_flow_item_eth_mask.dst, sizeof(rte_flow_item_eth_mask.dst))) {
        /*
    ...
    if (!mask || !memcmp(&mask->src, &rte_flow_item_eth_mask.src, sizeof(rte_flow_item_eth_mask.src))) {
        /*
         * Ethernet addresses are not supported by
 ...
 */
static void flow_tcf_parse_vxlan_encap_ipv4(const struct rte_flow_item_ipv4 *spec,
                                            const struct rte_flow_item_ipv4 *mask, struct flow_tcf_vxlan_encap *encap) {
    /* Item must be validated before. No redundant checks. */
    assert(spec);
    encap->ipv4.dst = spec->hdr.dst_addr;
    encap->ipv4.src = spec->hdr.src_addr;
    encap->mask |= FLOW_TCF_ENCAP_IPV4_SRC | FLOW_TCF_ENCAP_IPV4_DST;
 ...
 */
static void flow_tcf_parse_vxlan_encap_ipv6(const struct rte_flow_item_ipv6 *spec,
                                            const struct rte_flow_item_ipv6 *mask, struct flow_tcf_vxlan_encap *encap) {
    /* Item must be validated before. No redundant checks. */
    assert(spec);
    memcpy(encap->ipv6.dst, spec->hdr.dst_addr, IPV6_ADDR_LEN);
    memcpy(encap->ipv6.src, spec->hdr.src_addr, IPV6_ADDR_LEN);
    encap->mask |= FLOW_TCF_ENCAP_IPV6_SRC | FLOW_TCF_ENCAP_IPV6_DST;
    if (mask) {
        if ((rte_be_to_cpu_32(mask->hdr.vtc_flow) >> IPV6_HDR_TC_SHIFT) & 0xff) {
            encap->mask |= FLOW_TCF_ENCAP_IP_TOS;
            encap->ip_tos = (rte_be_to_cpu_32(spec->hdr.vtc_flow) >> IPV6_HDR_TC_SHIFT) & 0xff;
        }
 ...
 */
static void flow_tcf_parse_vxlan_encap_udp(const struct rte_flow_item_udp *spec, const struct rte_flow_item_udp *mask,
                                           struct flow_tcf_vxlan_encap *encap) {
    assert(spec);
 ...
 */
static void flow_tcf_parse_vxlan_encap_vni(const struct rte_flow_item_vxlan *spec, struct flow_tcf_vxlan_encap *encap) {
 ...
 */
static void flow_tcf_vxlan_encap_parse(const struct rte_flow_action *action, struct flow_tcf_vxlan_encap *encap) {
    union {
        const struct rte_flow_item_eth *eth;
        const struct rte_flow_item_ipv4 *ipv4;
        const struct rte_flow_item_ipv6 *ipv6;
    ...
    for (; items->type != RTE_FLOW_ITEM_TYPE_END; items++) {
        ...
        case RTE_FLOW_ITEM_TYPE_ETH:
            mask.eth = items->mask;
            spec.eth = items->spec;
            flow_tcf_parse_vxlan_encap_eth(spec.eth, mask.eth, encap);
            break;
        case RTE_FLOW_ITEM_TYPE_IPV4:
            spec.ipv4 = items->spec;
            mask.ipv4 = items->mask;
            flow_tcf_parse_vxlan_encap_ipv4(spec.ipv4, mask.ipv4, encap);
            break;
        case RTE_FLOW_ITEM_TYPE_IPV6:
            spec.ipv6 = items->spec;
            mask.ipv6 = items->mask;
            flow_tcf_parse_vxlan_encap_ipv6(spec.ipv6, mask.ipv6, encap);
            break;
        case RTE_FLOW_ITEM_TYPE_UDP:
            mask.udp = items->mask;
            spec.udp = items->spec;
            flow_tcf_parse_vxlan_encap_udp(spec.udp, mask.udp, encap);
            break;
        case RTE_FLOW_ITEM_TYPE_VXLAN:
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_tcf_translate(struct rte_eth_dev *dev, struct mlx5_flow *dev_flow, const struct rte_flow_attr *attr,
                              const struct rte_flow_item items[], const struct rte_flow_action actions[],
                              struct rte_flow_error *error) {
    union {
        const struct rte_flow_item_port_id *port_id;
    ...
    for (; items->type != RTE_FLOW_ITEM_TYPE_END; items++) {
        ...
        case RTE_FLOW_ITEM_TYPE_ETH:
            ...
            if (tunnel_outer) {
                DRV_LOG(WARNING, "outer L2 addresses cannot be"
                                 " forced is outer ones for tunnel,"
                                 " parameter is ignored");
                break;
            }
            if (!is_zero_ether_addr(&mask.eth->dst)) {
                mnl_attr_put(nlh, TCA_FLOWER_KEY_ETH_DST, ETHER_ADDR_LEN, spec.eth->dst.addr_bytes);
                mnl_attr_put(nlh, TCA_FLOWER_KEY_ETH_DST_MASK, ETHER_ADDR_LEN, mask.eth->dst.addr_bytes);
            }
            if (!is_zero_ether_addr(&mask.eth->src)) {
                mnl_attr_put(nlh, TCA_FLOWER_KEY_ETH_SRC, ETHER_ADDR_LEN, spec.eth->src.addr_bytes);
                mnl_attr_put(nlh, TCA_FLOWER_KEY_ETH_SRC_MASK, ETHER_ADDR_LEN, mask.eth->src.addr_bytes);
            }
        ...
        case RTE_FLOW_ITEM_TYPE_VLAN:
            ...
            if (mask.vlan == &flow_tcf_mask_empty.vlan)
                break;
            spec.vlan = items->spec;
            assert(outer_etype == RTE_BE16(ETH_P_ALL) || outer_etype == RTE_BE16(ETH_P_8021Q));
            outer_etype = RTE_BE16(ETH_P_8021Q);
            if (mask.vlan->inner_type)
                vlan_etype = spec.vlan->inner_type;
            if (mask.vlan->tci & RTE_BE16(0xe000))
                mnl_attr_put_u8(nlh, TCA_FLOWER_KEY_VLAN_PRIO, (rte_be_to_cpu_16(spec.vlan->tci) >> 13) & 0x7);
            if (mask.vlan->tci & RTE_BE16(0x0fff))
                mnl_attr_put_u16(nlh, TCA_FLOWER_KEY_VLAN_ID, rte_be_to_cpu_16(spec.vlan->tci & RTE_BE16(0x0fff)));
            assert(dev_flow->tcf.nlsize >= nlh->nlmsg_len);
            break;
        case RTE_FLOW_ITEM_TYPE_IPV4:
            item_flags |=
            ...
            assert(mask.ipv4);
            if (item_flags & MLX5_FLOW_LAYER_TUNNEL) {
                assert(inner_etype == RTE_BE16(ETH_P_ALL) || inner_etype == RTE_BE16(ETH_P_IP));
                inner_etype = RTE_BE16(ETH_P_IP);
            } else if (outer_etype == RTE_BE16(ETH_P_8021Q)) {
                assert(vlan_etype == RTE_BE16(ETH_P_ALL) || vlan_etype == RTE_BE16(ETH_P_IP));
                vlan_etype = RTE_BE16(ETH_P_IP);
            } else {
                assert(outer_etype == RTE_BE16(ETH_P_ALL) || outer_etype == RTE_BE16(ETH_P_IP));
                outer_etype = RTE_BE16(ETH_P_IP);
            }
            spec.ipv4 = items->spec;
            ...
            if (mask.ipv4 == &flow_tcf_mask_empty.ipv4 || (!mask.ipv4->hdr.src_addr && !mask.ipv4->hdr.dst_addr)) {
                if (!tunnel_outer)
                    break;
                /*
                 ...
                 */
                mnl_attr_put_u32(nlh, TCA_FLOWER_KEY_ENC_IPV4_SRC, mask.ipv4->hdr.src_addr);
                mnl_attr_put_u32(nlh, TCA_FLOWER_KEY_ENC_IPV4_SRC_MASK, mask.ipv4->hdr.src_addr);
                assert(dev_flow->tcf.nlsize >= nlh->nlmsg_len);
                break;
            }
            if (mask.ipv4->hdr.src_addr) {
                mnl_attr_put_u32(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_IPV4_SRC : TCA_FLOWER_KEY_IPV4_SRC,
                                 spec.ipv4->hdr.src_addr);
                mnl_attr_put_u32(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_IPV4_SRC_MASK : TCA_FLOWER_KEY_IPV4_SRC_MASK,
                                 mask.ipv4->hdr.src_addr);
            }
            if (mask.ipv4->hdr.dst_addr) {
                mnl_attr_put_u32(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_IPV4_DST : TCA_FLOWER_KEY_IPV4_DST,
                                 spec.ipv4->hdr.dst_addr);
                mnl_attr_put_u32(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_IPV4_DST_MASK : TCA_FLOWER_KEY_IPV4_DST_MASK,
                                 mask.ipv4->hdr.dst_addr);
            }
            if (mask.ipv4->hdr.time_to_live) {
                mnl_attr_put_u8(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_IP_TTL : TCA_FLOWER_KEY_IP_TTL,
                                spec.ipv4->hdr.time_to_live);
                mnl_attr_put_u8(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_IP_TTL_MASK : TCA_FLOWER_KEY_IP_TTL_MASK,
                                mask.ipv4->hdr.time_to_live);
            }
            if (mask.ipv4->hdr.type_of_service) {
                mnl_attr_put_u8(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_IP_TOS : TCA_FLOWER_KEY_IP_TOS,
                                spec.ipv4->hdr.type_of_service);
                mnl_attr_put_u8(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_IP_TOS_MASK : TCA_FLOWER_KEY_IP_TOS_MASK,
        ...
        case RTE_FLOW_ITEM_TYPE_IPV6: {
            ...
            assert(mask.ipv6);
            if (item_flags & MLX5_FLOW_LAYER_TUNNEL) {
                assert(inner_etype == RTE_BE16(ETH_P_ALL) || inner_etype == RTE_BE16(ETH_P_IPV6));
                inner_etype = RTE_BE16(ETH_P_IPV6);
            } else if (outer_etype == RTE_BE16(ETH_P_8021Q)) {
                assert(vlan_etype == RTE_BE16(ETH_P_ALL) || vlan_etype == RTE_BE16(ETH_P_IPV6));
                vlan_etype = RTE_BE16(ETH_P_IPV6);
            } else {
                assert(outer_etype == RTE_BE16(ETH_P_ALL) || outer_etype == RTE_BE16(ETH_P_IPV6));
                outer_etype = RTE_BE16(ETH_P_IPV6);
            }
            spec.ipv6 = items->spec;
            if (!tunnel_outer && mask.ipv6->hdr.proto) {
                ...
                mnl_attr_put_u8(nlh, TCA_FLOWER_KEY_IP_PROTO, spec.ipv6->hdr.proto);
                ip_proto_set = 1;
            }
            ipv6_dst = !IN6_IS_ADDR_UNSPECIFIED(mask.ipv6->hdr.dst_addr);
            ipv6_src = !IN6_IS_ADDR_UNSPECIFIED(mask.ipv6->hdr.src_addr);
            if (mask.ipv6 == &flow_tcf_mask_empty.ipv6 || (!ipv6_dst && !ipv6_src)) {
                if (!tunnel_outer)
                    break;
                /*
                 * For tunnel outer we must set outer IP key
                 ...
                 * kernel about he outer layer protocol.
                 */
                mnl_attr_put(nlh, TCA_FLOWER_KEY_ENC_IPV6_SRC, IPV6_ADDR_LEN, mask.ipv6->hdr.src_addr);
                mnl_attr_put(nlh, TCA_FLOWER_KEY_ENC_IPV6_SRC_MASK, IPV6_ADDR_LEN, mask.ipv6->hdr.src_addr);
                assert(dev_flow->tcf.nlsize >= nlh->nlmsg_len);
                break;
            }
            if (ipv6_src) {
                mnl_attr_put(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_IPV6_SRC : TCA_FLOWER_KEY_IPV6_SRC, IPV6_ADDR_LEN,
                             spec.ipv6->hdr.src_addr);
                mnl_attr_put(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_IPV6_SRC_MASK : TCA_FLOWER_KEY_IPV6_SRC_MASK,
                             IPV6_ADDR_LEN, mask.ipv6->hdr.src_addr);
            }
            if (ipv6_dst) {
                mnl_attr_put(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_IPV6_DST : TCA_FLOWER_KEY_IPV6_DST, IPV6_ADDR_LEN,
                             spec.ipv6->hdr.dst_addr);
                mnl_attr_put(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_IPV6_DST_MASK : TCA_FLOWER_KEY_IPV6_DST_MASK,
                             IPV6_ADDR_LEN, mask.ipv6->hdr.dst_addr);
            }
            ...
            msk6 = (rte_be_to_cpu_32(mask.ipv6->hdr.vtc_flow) >> IPV6_HDR_TC_SHIFT) & 0xff;
            if (msk6) {
                tos6 = (rte_be_to_cpu_32(spec.ipv6->hdr.vtc_flow) >> IPV6_HDR_TC_SHIFT) & 0xff;
                mnl_attr_put_u8(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_IP_TOS : TCA_FLOWER_KEY_IP_TOS, tos6);
                mnl_attr_put_u8(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_IP_TOS_MASK : TCA_FLOWER_KEY_IP_TOS_MASK, msk6);
            }
            assert(dev_flow->tcf.nlsize >= nlh->nlmsg_len);
            break;
        }
        case RTE_FLOW_ITEM_TYPE_UDP:
            ...
            if (!tunnel_outer) {
                if (!ip_proto_set)
                    mnl_attr_put_u8(nlh, TCA_FLOWER_KEY_IP_PROTO, IPPROTO_UDP);
                if (mask.udp == &flow_tcf_mask_empty.udp)
                    break;
            } else {
                assert(mask.udp != &flow_tcf_mask_empty.udp);
                decap.vxlan->udp_port = rte_be_to_cpu_16(spec.udp->hdr.dst_port);
            }
            if (mask.udp->hdr.src_port) {
                mnl_attr_put_u16(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_UDP_SRC_PORT : TCA_FLOWER_KEY_UDP_SRC,
                                 spec.udp->hdr.src_port);
                mnl_attr_put_u16(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_UDP_SRC_PORT_MASK : TCA_FLOWER_KEY_UDP_SRC_MASK,
                                 mask.udp->hdr.src_port);
            }
            if (mask.udp->hdr.dst_port) {
                mnl_attr_put_u16(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_UDP_DST_PORT : TCA_FLOWER_KEY_UDP_DST,
                                 spec.udp->hdr.dst_port);
                mnl_attr_put_u16(nlh, tunnel_outer ? TCA_FLOWER_KEY_ENC_UDP_DST_PORT_MASK : TCA_FLOWER_KEY_UDP_DST_MASK,
        ...
        case RTE_FLOW_ITEM_TYPE_TCP:
            ...
            assert(mask.tcp);
            if (!ip_proto_set)
                mnl_attr_put_u8(nlh, TCA_FLOWER_KEY_IP_PROTO, IPPROTO_TCP);
            if (mask.tcp == &flow_tcf_mask_empty.tcp)
                break;
            spec.tcp = items->spec;
            if (mask.tcp->hdr.src_port) {
                mnl_attr_put_u16(nlh, TCA_FLOWER_KEY_TCP_SRC, spec.tcp->hdr.src_port);
                mnl_attr_put_u16(nlh, TCA_FLOWER_KEY_TCP_SRC_MASK, mask.tcp->hdr.src_port);
            }
            if (mask.tcp->hdr.dst_port) {
                mnl_attr_put_u16(nlh, TCA_FLOWER_KEY_TCP_DST, spec.tcp->hdr.dst_port);
                mnl_attr_put_u16(nlh, TCA_FLOWER_KEY_TCP_DST_MASK, mask.tcp->hdr.dst_port);
            }
            if (mask.tcp->hdr.tcp_flags) {
                mnl_attr_put_u16(nlh, TCA_FLOWER_KEY_TCP_FLAGS, rte_cpu_to_be_16(spec.tcp->hdr.tcp_flags));
                mnl_attr_put_u16(nlh, TCA_FLOWER_KEY_TCP_FLAGS_MASK, rte_cpu_to_be_16(mask.tcp->hdr.tcp_flags));
            }
            assert(dev_flow->tcf.nlsize >= nlh->nlmsg_len);
            break;
        case RTE_FLOW_ITEM_TYPE_VXLAN:
            ...
            item_flags |= MLX5_FLOW_LAYER_VXLAN;
            spec.vxlan = items->spec;
            mnl_attr_put_u32(nlh, TCA_FLOWER_KEY_ENC_KEY_ID, vxlan_vni_as_be32(spec.vxlan->vni));
            assert(dev_flow->tcf.nlsize >= nlh->nlmsg_len);
            break;
        default:
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, NULL, "item not supported");
        }
    }
    /*
    ...
    if (outer_etype != RTE_BE16(ETH_P_ALL)) {
        ...
        if (item_flags & MLX5_FLOW_LAYER_TUNNEL) {
            if (inner_etype != RTE_BE16(ETH_P_ALL))
                mnl_attr_put_u16(nlh, TCA_FLOWER_KEY_ETH_TYPE, inner_etype);
        } else {
            mnl_attr_put_u16(nlh, TCA_FLOWER_KEY_ETH_TYPE, outer_etype);
            if (outer_etype == RTE_BE16(ETH_P_8021Q) && vlan_etype != RTE_BE16(ETH_P_ALL))
                mnl_attr_put_u16(nlh, TCA_FLOWER_KEY_VLAN_ETH_TYPE, vlan_etype);
        }
        assert(dev_flow->tcf.nlsize >= nlh->nlmsg_len);
    }
    ...
    for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
        ...
        case RTE_FLOW_ACTION_TYPE_PORT_ID:
            ...
            if (conf.port_id->original)
                ...
                for (i = 0; ptoi[i].ifindex; ++i)
                    if (ptoi[i].port_id == conf.port_id->id)
                        break;
            assert(ptoi[i].ifindex);
            na_act_index = mnl_attr_nest_start(nlh, na_act_index_cur++);
            assert(na_act_index);
            ...
            if (encap.hdr) {
                ...
                dev_flow->tcf.tunnel->ifindex_ptr =
                    &((struct tc_mirred *)mnl_attr_get_payload(mnl_nlmsg_get_payload_tail(nlh)))->ifindex;
            } else if (decap.hdr) {
                assert(dev_flow->tcf.tunnel);
                dev_flow->tcf.tunnel->ifindex_ptr = (unsigned int *)&tcm->tcm_ifindex;
            }
            mnl_attr_put(nlh, TCA_MIRRED_PARMS, sizeof(struct tc_mirred),
                         &(struct tc_mirred){
        ...
        case RTE_FLOW_ACTION_TYPE_JUMP:
            ...
            na_act = mnl_attr_nest_start(nlh, TCA_ACT_OPTIONS);
            assert(na_act);
            mnl_attr_put(nlh, TCA_GACT_PARMS, sizeof(struct tc_gact),
                         &(struct tc_gact){
                             .action = TC_ACT_GOTO_CHAIN | conf.jump->group,
                         });
            mnl_attr_nest_end(nlh, na_act);
            mnl_attr_nest_end(nlh, na_act_index);
            break;
        ...
        case RTE_FLOW_ACTION_TYPE_COUNT:
             ...
             */
            ret = flow_tcf_translate_action_count(dev, dev_flow, error);
            if (ret < 0)
                return ret;
            break;
        case RTE_FLOW_ACTION_TYPE_OF_POP_VLAN:
        ...
        action_of_vlan:
            ...
            na_act = mnl_attr_nest_start(nlh, TCA_ACT_OPTIONS);
            assert(na_act);
            mnl_attr_put(nlh, TCA_VLAN_PARMS, sizeof(struct tc_vlan),
                         &(struct tc_vlan){
                             .action = TC_ACT_PIPE,
                             .v_action = vlan_act,
                         });
            ...
            if (vlan_act == TCA_VLAN_ACT_PUSH)
                mnl_attr_put_u16(nlh, TCA_VLAN_PUSH_VLAN_PROTOCOL, conf.of_push_vlan->ethertype);
            ...
            mnl_attr_nest_end(nlh, na_act);
            mnl_attr_nest_end(nlh, na_act_index);
            if (actions->type == RTE_FLOW_ACTION_TYPE_OF_SET_VLAN_VID) {
            override_na_vlan_id:
                na_vlan_id->nla_type = TCA_VLAN_PUSH_VLAN_ID;
                *(uint16_t *)mnl_attr_get_payload(na_vlan_id) = rte_be_to_cpu_16(conf.of_set_vlan_vid->vlan_vid);
            } else if (actions->type == RTE_FLOW_ACTION_TYPE_OF_SET_VLAN_PCP) {
            override_na_vlan_priority:
                na_vlan_priority->nla_type = TCA_VLAN_PUSH_VLAN_PRIORITY;
                *(uint8_t *)mnl_attr_get_payload(na_vlan_priority) = conf.of_set_vlan_pcp->vlan_pcp;
            }
            break;
        case RTE_FLOW_ACTION_TYPE_VXLAN_DECAP:
            assert(decap.vxlan);
            ...
            na_act = mnl_attr_nest_start(nlh, TCA_ACT_OPTIONS);
            assert(na_act);
            mnl_attr_put(nlh, TCA_TUNNEL_KEY_PARMS, sizeof(struct tc_tunnel_key),
                         &(struct tc_tunnel_key){
                             .action = TC_ACT_PIPE,
        ...
        case RTE_FLOW_ACTION_TYPE_VXLAN_ENCAP:
            ...
            if (encap.vxlan->mask & FLOW_TCF_ENCAP_UDP_DST)
                mnl_attr_put_u16(nlh, TCA_TUNNEL_KEY_ENC_DST_PORT, encap.vxlan->udp.dst);
            if (encap.vxlan->mask & FLOW_TCF_ENCAP_IPV4_SRC)
                mnl_attr_put_u32(nlh, TCA_TUNNEL_KEY_ENC_IPV4_SRC, encap.vxlan->ipv4.src);
            if (encap.vxlan->mask & FLOW_TCF_ENCAP_IPV4_DST)
                mnl_attr_put_u32(nlh, TCA_TUNNEL_KEY_ENC_IPV4_DST, encap.vxlan->ipv4.dst);
            if (encap.vxlan->mask & FLOW_TCF_ENCAP_IPV6_SRC)
                mnl_attr_put(nlh, TCA_TUNNEL_KEY_ENC_IPV6_SRC, sizeof(encap.vxlan->ipv6.src), &encap.vxlan->ipv6.src);
            if (encap.vxlan->mask & FLOW_TCF_ENCAP_IPV6_DST)
                mnl_attr_put(nlh, TCA_TUNNEL_KEY_ENC_IPV6_DST, sizeof(encap.vxlan->ipv6.dst), &encap.vxlan->ipv6.dst);
            if (encap.vxlan->mask & FLOW_TCF_ENCAP_IP_TTL)
                mnl_attr_put_u8(nlh, TCA_TUNNEL_KEY_ENC_TTL, encap.vxlan->ip_ttl_hop);
            if (encap.vxlan->mask & FLOW_TCF_ENCAP_IP_TOS)
                mnl_attr_put_u8(nlh, TCA_TUNNEL_KEY_ENC_TOS, encap.vxlan->ip_tos);
            if (encap.vxlan->mask & FLOW_TCF_ENCAP_VXLAN_VNI)
                mnl_attr_put_u32(nlh, TCA_TUNNEL_KEY_ENC_KEY_ID, vxlan_vni_as_be32(encap.vxlan->vxlan.vni));
        ...
        case RTE_FLOW_ACTION_TYPE_SET_MAC_DST:
            na_act_index = mnl_attr_nest_start(nlh, na_act_index_cur++);
            flow_tcf_create_pedit_mnl_msg(nlh, &actions, item_flags);
            mnl_attr_nest_end(nlh, na_act_index);
            break;
        default:
            return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, actions, "action not supported");
    ...
    dev_flow->tcf.ptc_flags = mnl_attr_get_payload(mnl_nlmsg_get_payload_tail(nlh));
    mnl_attr_put_u32(nlh, TCA_FLOWER_FLAGS, decap.vxlan ? 0 : TCA_CLS_FLAGS_SKIP_SW);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_tcf_nl_ack(struct mlx5_flow_tcf_context *tcf, struct nlmsghdr *nlh, mnl_cb_t cb, void *arg) {
    ...
    while (true) {
        ...
        if (ret < 0) {
             ...
             */
            if (err != ENOSPC || !(nlh->nlmsg_flags & NLM_F_MULTI) || nlh->nlmsg_type == NLMSG_DONE)
                break;
        } else {
...
#define MNL_REQUEST_SIZE RTE_MIN(RTE_MAX(sysconf(_SC_PAGESIZE), MNL_REQUEST_SIZE_MIN), MNL_REQUEST_SIZE_MAX)

/* Data structures used by flow_tcf_xxx_cb() routines. */
struct tcf_nlcb_buf {
    LIST_ENTRY(tcf_nlcb_buf) next;
    uint32_t size;
    alignas(struct nlmsghdr) uint8_t msg[]; /**< Netlink message data. */
};
 ...
 */
static struct nlmsghdr *flow_tcf_alloc_nlcmd(struct tcf_nlcb_context *ctx, uint32_t size) {
    ...
    buf = rte_malloc(__func__, ctx->bufsize + sizeof(struct tcf_nlcb_buf), alignof(struct tcf_nlcb_buf));
    if (!buf) {
 ...
 */
static int flow_tcf_send_nlcmd(struct mlx5_flow_tcf_context *tcf, struct tcf_nlcb_context *ctx) {
    ...
    while (bc) {
        ...
        while (msg < bc->size) {
            ...
            if (rc) {
                DRV_LOG(WARNING, "netlink: cleanup error %d", rc);
                if (!ret)
                    ret = rc;
            }
        }
 ...
 *   and rte_errno is set.
 */
static int flow_tcf_collect_local_cb(const struct nlmsghdr *nlh, void *arg) {
    ...
    if (ifa->ifa_index != ctx->ifindex || ifa->ifa_scope != RT_SCOPE_LINK || !(ifa->ifa_flags & IFA_F_PERMANENT) ||
    ...
    size = MNL_ALIGN(sizeof(struct nlmsghdr)) + MNL_ALIGN(sizeof(struct ifaddrmsg)) +
           (family == AF_INET6 ? 2 * SZ_NLATTR_DATA_OF(IPV6_ADDR_LEN) : 2 * SZ_NLATTR_TYPE_OF(uint32_t));
    ...
    if (family == AF_INET) {
        ...
        mnl_attr_put(cmd, IFA_LOCAL, IPV6_ADDR_LEN, mnl_attr_get_payload(na_local));
        mnl_attr_put(cmd, IFA_ADDRESS, IPV6_ADDR_LEN, mnl_attr_get_payload(na_peer));
    }
 ...
 */
static void flow_tcf_encap_local_cleanup(struct mlx5_flow_tcf_context *tcf, unsigned int ifindex) {
    struct nlmsghdr *nlh;
    struct ifaddrmsg *ifa;
 ...
 *   and rte_errno is set.
 */
static int flow_tcf_collect_neigh_cb(const struct nlmsghdr *nlh, void *arg) {
    struct tcf_nlcb_context *ctx = arg;
    struct nlmsghdr *cmd;
    struct ndmsg *ndm;
    struct nlattr *na;
    ...
    if (ndm->ndm_ifindex != (int)ctx->ifindex || !(ndm->ndm_state & NUD_PERMANENT) ||
        (family != AF_INET && family != AF_INET6))
        return 1;
    ...
    size = MNL_ALIGN(sizeof(struct nlmsghdr)) + MNL_ALIGN(sizeof(struct ndmsg)) + SZ_NLATTR_DATA_OF(ETHER_ADDR_LEN) +
           (family == AF_INET6 ? SZ_NLATTR_DATA_OF(IPV6_ADDR_LEN) : SZ_NLATTR_TYPE_OF(uint32_t));
    cmd = flow_tcf_alloc_nlcmd(ctx, size);
    if (!cmd) {
        rte_errno = ENOMEM;
        return -rte_errno;
    ...
    if (family == AF_INET) {
        ...
        mnl_attr_put(cmd, NDA_DST, IPV6_ADDR_LEN, mnl_attr_get_payload(na_ip));
    }
    mnl_attr_put(cmd, NDA_LLADDR, ETHER_ADDR_LEN, mnl_attr_get_payload(na_mac));
    assert(size == cmd->nlmsg_len);
    return 1;
}
 ...
 *   Network interface index to perform cleanup.
 */
static void flow_tcf_encap_neigh_cleanup(struct mlx5_flow_tcf_context *tcf, unsigned int ifindex) {
    struct nlmsghdr *nlh;
    struct ndmsg *ndm;
    struct tcf_nlcb_context ctx = {
        .ifindex = ifindex,
 ...
 *   and rte_errno is set.
 */
static int flow_tcf_collect_vxlan_cb(const struct nlmsghdr *nlh, void *arg) {
// ybrustin: no symbol IFLA_VXLAN_*, func is not used by TRex
    ...
    mnl_attr_for_each(na, nlh, sizeof(*ifm)) if (mnl_attr_get_type(na) == IFLA_LINKINFO) {
        na_info = na;
        break;
    }
    if (!na_info)
    ...
    mnl_attr_for_each_nested(na, na_info) {
        switch (mnl_attr_get_type(na)) {
        case IFLA_INFO_KIND:
            if (!strncmp("vxlan", mnl_attr_get_str(na), mnl_attr_get_len(na)))
                found = true;
            break;
        case IFLA_INFO_DATA:
            na_vxlan = na;
    ...
    mnl_attr_for_each_nested(na, na_vxlan) {
        if (mnl_attr_get_type(na) == IFLA_VXLAN_LINK && mnl_attr_get_u32(na) == ctx->ifindex) {
            found = true;
            break;
        }
    }
    ...
    size = MNL_ALIGN(sizeof(struct nlmsghdr)) + MNL_ALIGN(sizeof(struct ifinfomsg));
    cmd = flow_tcf_alloc_nlcmd(ctx, size);
    if (!cmd) {
        rte_errno = ENOMEM;
        return -rte_errno;
 ...
 */
static void flow_tcf_encap_iface_cleanup(struct mlx5_flow_tcf_context *tcf, unsigned int ifindex) {
    struct nlmsghdr *nlh;
    struct ifinfomsg *ifm;
    struct tcf_nlcb_context ctx = {
        .ifindex = ifindex,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_tcf_rule_local(struct mlx5_flow_tcf_context *tcf, const struct flow_tcf_vxlan_encap *encap,
                               unsigned int ifindex, bool enable, struct rte_flow_error *error) {
    struct nlmsghdr *nlh;
    struct ifaddrmsg *ifa;
    alignas(struct nlmsghdr) uint8_t buf[mnl_nlmsg_size(sizeof(*ifa) + 128)];

    nlh = mnl_nlmsg_put_header(buf);
    nlh->nlmsg_type = enable ? RTM_NEWADDR : RTM_DELADDR;
    nlh->nlmsg_flags = NLM_F_REQUEST | (enable ? NLM_F_CREATE | NLM_F_REPLACE : 0);
    nlh->nlmsg_seq = 0;
    ifa = mnl_nlmsg_put_extra_header(nlh, sizeof(*ifa));
    ifa->ifa_flags = IFA_F_PERMANENT;
    ifa->ifa_scope = RT_SCOPE_LINK;
    ...
    if (encap->mask & FLOW_TCF_ENCAP_IPV4_SRC) {
        ...
        if (encap->mask & FLOW_TCF_ENCAP_IPV4_DST)
            mnl_attr_put_u32(nlh, IFA_ADDRESS, encap->ipv4.dst);
    } else {
        assert(encap->mask & FLOW_TCF_ENCAP_IPV6_SRC);
        ifa->ifa_family = AF_INET6;
        ifa->ifa_prefixlen = 128;
        mnl_attr_put(nlh, IFA_LOCAL, sizeof(encap->ipv6.src), &encap->ipv6.src);
        if (encap->mask & FLOW_TCF_ENCAP_IPV6_DST)
            mnl_attr_put(nlh, IFA_ADDRESS, sizeof(encap->ipv6.dst), &encap->ipv6.dst);
    }
    if (!flow_tcf_nl_ack(tcf, nlh, NULL, NULL))
        return 0;
    return rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_tcf_rule_neigh(struct mlx5_flow_tcf_context *tcf, const struct flow_tcf_vxlan_encap *encap,
                               unsigned int ifindex, bool enable, struct rte_flow_error *error) {
    struct nlmsghdr *nlh;
    struct ndmsg *ndm;
    alignas(struct nlmsghdr) uint8_t buf[mnl_nlmsg_size(sizeof(*ndm) + 128)];
    ...
    if (encap->mask & FLOW_TCF_ENCAP_IPV4_DST) {
        ...
        ndm->ndm_family = AF_INET6;
        mnl_attr_put(nlh, NDA_DST, sizeof(encap->ipv6.dst), &encap->ipv6.dst);
    }
    if (encap->mask & FLOW_TCF_ENCAP_ETH_SRC && enable)
        DRV_LOG(WARNING, "outer ethernet source address cannot be "
                         "forced for VXLAN encapsulation");
    if (encap->mask & FLOW_TCF_ENCAP_ETH_DST)
        mnl_attr_put(nlh, NDA_LLADDR, sizeof(encap->eth.dst), &encap->eth.dst);
    if (!flow_tcf_nl_ack(tcf, nlh, NULL, NULL))
        return 0;
    return rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_tcf_encap_local(struct mlx5_flow_tcf_context *tcf, struct tcf_irule *iface, struct mlx5_flow *dev_flow,
    ...
    if (encap->mask & FLOW_TCF_ENCAP_IPV4_SRC) {
        ...
        LIST_FOREACH(rule, &iface->local, next) {
            if (rule->mask & FLOW_TCF_ENCAP_IPV4_SRC && encap->ipv4.src == rule->ipv4.src &&
        ...
        LIST_FOREACH(rule, &iface->local, next) {
            if (rule->mask & FLOW_TCF_ENCAP_IPV6_SRC &&
                !memcmp(&encap->ipv6.src, &rule->ipv6.src, sizeof(encap->ipv6.src)) &&
                !memcmp(&encap->ipv6.dst, &rule->ipv6.dst, sizeof(encap->ipv6.dst))) {
                break;
            }
        }
    }
    if (rule) {
        ...
        if (!rule->refcnt || !--rule->refcnt) {
            ...
            return flow_tcf_rule_local(tcf, encap, iface->ifouter, false, error);
        }
        return 0;
    }
    if (!enable) {
        DRV_LOG(WARNING, "disabling not existing local rule");
        rte_flow_error_set(error, ENOENT, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, "disabling not existing local rule");
        return -ENOENT;
    }
    rule = rte_zmalloc(__func__, sizeof(struct tcf_local_rule), alignof(struct tcf_local_rule));
    if (!rule) {
        rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                           "unable to allocate memory for local rule");
        return -rte_errno;
    }
    *rule = (struct tcf_local_rule){
        .refcnt = 0,
        .mask = 0,
    };
    if (encap->mask & FLOW_TCF_ENCAP_IPV4_SRC) {
        rule->mask = FLOW_TCF_ENCAP_IPV4_SRC | FLOW_TCF_ENCAP_IPV4_DST;
        rule->ipv4.src = encap->ipv4.src;
        rule->ipv4.dst = encap->ipv4.dst;
    } else {
        rule->mask = FLOW_TCF_ENCAP_IPV6_SRC | FLOW_TCF_ENCAP_IPV6_DST;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_tcf_encap_neigh(struct mlx5_flow_tcf_context *tcf, struct tcf_irule *iface, struct mlx5_flow *dev_flow,
                                bool enable, struct rte_flow_error *error) {
    const struct flow_tcf_vxlan_encap *encap = dev_flow->tcf.vxlan_encap;
    ...
    if (encap->mask & FLOW_TCF_ENCAP_IPV4_DST) {
        ...
        LIST_FOREACH(rule, &iface->neigh, next) {
            if (rule->mask & FLOW_TCF_ENCAP_IPV4_DST && encap->ipv4.dst == rule->ipv4.dst) {
                break;
            }
        }
    } else {
    ...
    if (rule) {
        if (memcmp(&encap->eth.dst, &rule->eth, sizeof(encap->eth.dst))) {
        ...
        if (!rule->refcnt || !--rule->refcnt) {
            LIST_REMOVE(rule, next);
            return flow_tcf_rule_neigh(tcf, encap, iface->ifouter, false, error);
        }
        return 0;
    }
    if (!enable) {
        DRV_LOG(WARNING, "Disabling not existing neigh rule");
        rte_flow_error_set(error, ENOENT, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                           "unable to allocate memory for neigh rule");
        return -ENOENT;
    }
    rule = rte_zmalloc(__func__, sizeof(struct tcf_neigh_rule), alignof(struct tcf_neigh_rule));
    if (!rule) {
        rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                           "unable to allocate memory for neigh rule");
        return -rte_errno;
    }
    *rule = (struct tcf_neigh_rule){
        .refcnt = 0,
        .mask = 0,
    };
    if (encap->mask & FLOW_TCF_ENCAP_IPV4_DST) {
 ...
 */
static struct tcf_irule *flow_tcf_encap_irule_acquire(struct mlx5_flow_tcf_context *tcf, unsigned int ifouter,
                                                      struct rte_flow_error *error) {
    struct tcf_irule *iface;
    ...
    iface = rte_zmalloc(__func__, sizeof(*iface), alignof(struct tcf_irule));
    if (!iface) {
        rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
 ...
 */
static void flow_tcf_encap_irule_release(struct tcf_irule *iface) {
    assert(iface->refcnt);
    if (--iface->refcnt == 0) {
 ...
 */
static void flow_tcf_vtep_delete(struct mlx5_flow_tcf_context *tcf, struct tcf_vtep *vtep) {
    struct nlmsghdr *nlh;
    struct ifinfomsg *ifm;
    alignas(struct nlmsghdr) uint8_t buf[mnl_nlmsg_size(MNL_ALIGN(sizeof(*ifm))) + MNL_BUF_EXTRA_SPACE];
    int ret;
 ...
 * NULL otherwise and rte_errno is set.
 */
static struct tcf_vtep *flow_tcf_vtep_create(struct mlx5_flow_tcf_context *tcf, uint16_t port,
                                             struct rte_flow_error *error) {
// ybrustin: no symbol IFLA_VXLAN_*, func is not used by TRex
#ifdef TREX_PATCH
    assert(0);
    ...
    alignas(struct nlmsghdr)
        uint8_t buf[mnl_nlmsg_size(sizeof(*ifm)) + SZ_NLATTR_DATA_OF(sizeof(name)) + SZ_NLATTR_NEST * 2 +
                    SZ_NLATTR_STRZ_OF("vxlan") + SZ_NLATTR_DATA_OF(sizeof(uint32_t)) +
                    SZ_NLATTR_DATA_OF(sizeof(uint16_t)) + SZ_NLATTR_DATA_OF(sizeof(uint8_t)) * 3 + MNL_BUF_EXTRA_SPACE];
    ...
    if (!vtep) {
        rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, "unable to allocate memory for VTEP");
        return NULL;
    }
    *vtep = (struct tcf_vtep){
    ...
    ret = flow_tcf_nl_ack(tcf, nlh, NULL, NULL);
    if (ret) {
        DRV_LOG(WARNING, "netlink: VTEP %s create failure (%d)", name, rte_errno);
        if (rte_errno != EEXIST)
            /*
    ...
    if (!ret) {
        DRV_LOG(WARNING, "VTEP %s failed to get index (%d)", name, errno);
    ...
    ret = flow_tcf_nl_ack(tcf, nlh, NULL, NULL);
    if (ret) {
        rte_flow_error_set(error, -errno, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, "netlink: failed to set VTEP link up");
        DRV_LOG(WARNING, "netlink: VTEP %s set link up failure (%d)", name, rte_errno);
        goto clean;
    }
 ...
 *   NULL otherwise and rte_errno is set.
 */
static struct tcf_vtep *flow_tcf_decap_vtep_acquire(struct mlx5_flow_tcf_context *tcf, struct mlx5_flow *dev_flow,
                                                    struct rte_flow_error *error) {
    struct tcf_vtep *vtep;
 ...
 *   NULL otherwise and rte_errno is set.
 */
static struct tcf_vtep *flow_tcf_encap_vtep_acquire(struct mlx5_flow_tcf_context *tcf, unsigned int ifouter,
    ...
    if (!ret) {
        ...
        if (ret)
            flow_tcf_encap_local(tcf, iface, dev_flow, false, error);
    }
    if (ret) {
 ...
 *   NULL otherwise and rte_errno is set.
 */
static struct tcf_vtep *flow_tcf_vtep_acquire(struct mlx5_flow_tcf_context *tcf, unsigned int ifouter,
                                              struct mlx5_flow *dev_flow, struct rte_flow_error *error) {
    ...
    switch (dev_flow->tcf.tunnel->type) {
    case FLOW_TCF_TUNACT_VXLAN_ENCAP:
        vtep = flow_tcf_encap_vtep_acquire(tcf, ifouter, dev_flow, error);
        break;
    case FLOW_TCF_TUNACT_VXLAN_DECAP:
        vtep = flow_tcf_decap_vtep_acquire(tcf, dev_flow, error);
        break;
    default:
        rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, "unsupported tunnel type");
        break;
    }
 ...
 *   Flow tcf object with tunnel structure pointer set.
 */
static void flow_tcf_vtep_release(struct mlx5_flow_tcf_context *tcf, struct tcf_vtep *vtep,
                                  struct mlx5_flow *dev_flow) {
    assert(dev_flow->tcf.tunnel);
    pthread_mutex_lock(&vtep_list_mutex);
    switch (dev_flow->tcf.tunnel->type) {
 ...
 */
static int flow_tcf_collect_query_cb(const struct nlmsghdr *nlh, void *arg) {
    struct tcf_nlcb_query *query = arg;
    struct tcmsg *tcm = mnl_nlmsg_get_payload(nlh);
    struct nlattr *na, *na_opt;
    bool flower = false;

    if (nlh->nlmsg_type != RTM_NEWTFILTER || tcm->tcm_handle != query->handle)
        return 1;
        ...
        case TCA_OPTIONS:
            ...
            mnl_attr_for_each_nested(na_opt, na) {
                ...
                case TCA_FLOWER_FLAGS:
                    query->flags_valid = 1;
                    query->tc_flags = mnl_attr_get_u32(na_opt);
                    break;
                }
            }
            break;
 ...
 */
static int flow_tcf_query_flags(struct mlx5_flow_tcf_context *tcf, struct mlx5_flow *dev_flow, uint32_t *pflags) {
    struct nlmsghdr *nlh;
    struct tcmsg *tcm;
 ...
 *   0 on success, a negative errno value otherwise.
 */
static int flow_tcf_check_inhw(struct mlx5_flow_tcf_context *tcf, struct mlx5_flow *dev_flow) {
    uint32_t flags;
    int ret;
 ...
 *   Pointer to the sub flow.
 */
static void flow_tcf_remove(struct rte_eth_dev *dev, struct rte_flow *flow) {
    ...
    if (dev_flow->tcf.applied) {
        ...
        if (dev_flow->tcf.tunnel) {
            ...
            flow_tcf_vtep_release(ctx, dev_flow->tcf.tunnel->vtep, dev_flow);
 ...
 *   to continue messages processing).
 */
static int flow_tcf_collect_apply_cb(const struct nlmsghdr *nlh, void *arg) {
    struct nlmsghdr *nlhrq = arg;
    struct tcmsg *tcmrq = mnl_nlmsg_get_payload(nlhrq);
    struct tcmsg *tcm = mnl_nlmsg_get_payload(nlh);
    struct nlattr *na;

    if (nlh->nlmsg_type != RTM_NEWTFILTER || nlh->nlmsg_seq != nlhrq->nlmsg_seq)
        return 1;
    mnl_attr_for_each(na, nlh, sizeof(*tcm)) {
        switch (mnl_attr_get_type(na)) {
        case TCA_KIND:
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_tcf_apply(struct rte_eth_dev *dev, struct rte_flow *flow, struct rte_flow_error *error) {
    struct mlx5_priv *priv = dev->data->dev_private;
    struct mlx5_flow_tcf_context *ctx = priv->tcf_context;
    struct mlx5_flow *dev_flow;
    struct nlmsghdr *nlh;
    ...
    nlh->nlmsg_flags = NLM_F_REQUEST | NLM_F_CREATE | NLM_F_EXCL | NLM_F_ECHO;
    tcm = mnl_nlmsg_get_payload(nlh);
    ...
    if (dev_flow->tcf.tunnel) {
        ...
        dev_flow->tcf.tunnel->vtep = flow_tcf_vtep_acquire(ctx, dev_flow->tcf.tunnel->ifindex_org, dev_flow, error);
        if (!dev_flow->tcf.tunnel->vtep)
            return -rte_errno;
        DRV_LOG(INFO, "Replace ifindex: %d->%d", dev_flow->tcf.tunnel->vtep->ifindex,
                dev_flow->tcf.tunnel->ifindex_org);
        *dev_flow->tcf.tunnel->ifindex_ptr = dev_flow->tcf.tunnel->vtep->ifindex;
    ...
    if (!ret) {
        if (!tcm->tcm_handle) {
            ...
            return rte_flow_error_set(error, ENOENT, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
 ...
 *   Pointer to the sub flow.
 */
static void flow_tcf_destroy(struct rte_eth_dev *dev, struct rte_flow *flow) {
    struct mlx5_flow *dev_flow;
 ...
 */
static uint16_t flow_tcf_arr_val_max(uint16_t array[], int idx, uint16_t value) {
 ...
 */
static void flow_tcf_nl_parse_rtattr(struct rtattr *tb[], int max, struct rtattr *rta, int len) {
    unsigned short type;
 ...
 */
static int flow_tcf_nl_action_stats_parse_and_get(struct rtattr *rta, uint16_t rta_type[], int idx,
                                                  struct gnet_stats_basic *data) {
    int tca_stats_max = flow_tcf_arr_val_max(rta_type, idx, TCA_STATS_BASIC);
    struct rtattr *tbs[tca_stats_max + 1];

    if (rta == NULL || idx < 0)
        return -1;
    flow_tcf_nl_parse_rtattr(tbs, tca_stats_max, RTA_DATA(rta), RTA_PAYLOAD(rta));
    switch (rta_type[idx]) {
    case TCA_STATS_BASIC:
        if (tbs[TCA_STATS_BASIC]) {
            memcpy(data, RTA_DATA(tbs[TCA_STATS_BASIC]), RTE_MIN(RTA_PAYLOAD(tbs[TCA_STATS_BASIC]), sizeof(*data)));
            return 0;
        }
        break;
    default:
 ...
 *   0 if data was found and retrieved, -1 otherwise.
 */
static int flow_tcf_nl_parse_one_action_and_get(struct rtattr *arg, uint16_t rta_type[], int idx, void *data) {
    int tca_act_max = flow_tcf_arr_val_max(rta_type, idx, TCA_ACT_STATS);
    struct rtattr *tb[tca_act_max + 1];

    if (arg == NULL || idx < 0)
        return -1;
    flow_tcf_nl_parse_rtattr(tb, tca_act_max, RTA_DATA(arg), RTA_PAYLOAD(arg));
    if (tb[TCA_ACT_KIND] == NULL)
        return -1;
    switch (rta_type[idx]) {
    case TCA_ACT_STATS:
        if (tb[TCA_ACT_STATS])
            return flow_tcf_nl_action_stats_parse_and_get(tb[TCA_ACT_STATS], rta_type, --idx,
                                                          (struct gnet_stats_basic *)data);
        break;
    default:
        break;
 ...
 *   0 if data was found and retrieved, -1 otherwise.
 */
static int flow_tcf_nl_action_parse_and_get(struct rtattr *arg, uint16_t rta_type[], int idx, void *data) {
    struct rtattr *tb[TCA_ACT_MAX_PRIO + 1];
    int i;

    if (arg == NULL || idx < 0)
        return -1;
    flow_tcf_nl_parse_rtattr(tb, TCA_ACT_MAX_PRIO, RTA_DATA(arg), RTA_PAYLOAD(arg));
    switch (rta_type[idx]) {
    /*
    ...
    case TCA_ACT_STATS:
        for (i = 0; i <= TCA_ACT_MAX_PRIO; i++) {
            if (tb[i] && !flow_tcf_nl_parse_one_action_and_get(tb[i], rta_type, idx, data))
                return 0;
        }
        break;
    default:
 ...
 *   0 if data was found and retrieved, -1 otherwise.
 */
static int flow_tcf_nl_opts_parse_and_get(struct rtattr *opt, uint16_t rta_type[], int idx, void *data) {
    int tca_flower_max = flow_tcf_arr_val_max(rta_type, idx, TCA_FLOWER_ACT);
    struct rtattr *tb[tca_flower_max + 1];

    if (!opt || idx < 0)
        return -1;
    flow_tcf_nl_parse_rtattr(tb, tca_flower_max, RTA_DATA(opt), RTA_PAYLOAD(opt));
    switch (rta_type[idx]) {
    case TCA_FLOWER_ACT:
        if (tb[TCA_FLOWER_ACT])
            return flow_tcf_nl_action_parse_and_get(tb[TCA_FLOWER_ACT], rta_type, --idx, data);
        break;
    default:
        break;
    }
 ...
 *   0 if data was found and retrieved, -1 otherwise.
 */
static int flow_tcf_nl_filter_parse_and_get(struct nlmsghdr *cnlh, uint16_t rta_type[], int idx, void *data) {
    ...
    if (idx < 0)
        return -1;
    if (nlh->nlmsg_type != RTM_NEWTFILTER && nlh->nlmsg_type != RTM_GETTFILTER && nlh->nlmsg_type != RTM_DELTFILTER)
        return -1;
    ...
    if (!tb[TCA_KIND] || strcmp(RTA_DATA(tb[TCA_KIND]), "flower"))
        return -1;
    switch (rta_type[idx]) {
    case TCA_OPTIONS:
        if (tb[TCA_OPTIONS])
            return flow_tcf_nl_opts_parse_and_get(tb[TCA_OPTIONS], rta_type, --idx, data);
        break;
    default:
        break;
    }
 ...
 *   MNL_CB_OK value.
 */
static int flow_tcf_nl_message_get_stats_basic(const struct nlmsghdr *nlh, void *data) {
    /*
     * The backward sequence of rta_types to pass in order to get
     *  to the counters.
     */
    uint16_t rta_type[] = {TCA_STATS_BASIC, TCA_ACT_STATS, TCA_FLOWER_ACT, TCA_OPTIONS};
    ...
    if (!flow_tcf_nl_filter_parse_and_get(tnlh.nc, rta_type, RTE_DIM(rta_type) - 1, (void *)&sb_data->counters))
        sb_data->valid = true;
    return MNL_CB_OK;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int flow_tcf_query_count(struct rte_eth_dev *dev, struct rte_flow *flow, void *data,
                                struct rte_flow_error *error) {
    ...
    do {
        ...
        if (ret <= 0)
            break;
        ret = mnl_cb_run(ctx->buf, ret, seq, mnl_socket_get_portid(nl), flow_tcf_nl_message_get_stats_basic,
                         (void *)&sb_data);
    } while (ret > 0);
    ...
    return rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, "flow does not have counter");
error_exit:
    return rte_flow_error_set(error, errno, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                              "netlink: failed to read flow rule counters");
notsup_exit:
    return rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, "counters are not available.");
}
 ...
 * @see rte_flow_ops
 */
static int flow_tcf_query(struct rte_eth_dev *dev, struct rte_flow *flow, const struct rte_flow_action *actions,
 ...
 *   rte_errno is set.
 */
static struct mnl_socket *flow_tcf_mnl_socket_create(void) {
 ...
 */
static void flow_tcf_mnl_socket_destroy(struct mnl_socket *nl) {
    if (nl)
        mnl_socket_close(nl);
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_flow_tcf_init(struct mlx5_flow_tcf_context *ctx, unsigned int ifindex, struct rte_flow_error *error) {
    struct nlmsghdr *nlh;
    struct tcmsg *tcm;
    alignas(struct nlmsghdr)
        uint8_t buf[mnl_nlmsg_size(sizeof(*tcm)) + SZ_NLATTR_STRZ_OF("ingress") + MNL_BUF_EXTRA_SPACE];
    ...
    if (flow_tcf_nl_ack(ctx, nlh, NULL, NULL) && rte_errno != EINVAL && rte_errno != ENOENT)
        return rte_flow_error_set(error, rte_errno, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
...
struct mlx5_flow_tcf_context *mlx5_flow_tcf_context_create(void) {
    struct mlx5_flow_tcf_context *ctx = rte_zmalloc(__func__, sizeof(*ctx), sizeof(uint32_t));
    if (!ctx)
        goto error;
