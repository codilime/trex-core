 ...
 *   MAC address index.
 */
static void mlx5_internal_mac_addr_remove(struct rte_eth_dev *dev, uint32_t index) {
    ...
    if (is_zero_ether_addr(&dev->data->mac_addrs[index]))
        return;
    if (vf)
        mlx5_nl_mac_addr_remove(dev, &dev->data->mac_addrs[index], index);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_internal_mac_addr_add(struct rte_eth_dev *dev, struct ether_addr *mac, uint32_t index) {
    struct mlx5_priv *priv = dev->data->dev_private;
    const int vf = priv->config.vf;
    unsigned int i;
...
void mlx5_mac_addr_remove(struct rte_eth_dev *dev, uint32_t index) {
    ...
    if (!dev->data->promiscuous) {
        ...
        if (ret)
            DRV_LOG(ERR, "port %u cannot restart traffic: %s", dev->data->port_id, strerror(rte_errno));
    }
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_mac_addr_add(struct rte_eth_dev *dev, struct ether_addr *mac, uint32_t index, uint32_t vmdq __rte_unused) {
    int ret;
 ...
 */
int mlx5_set_mc_addr_list(struct rte_eth_dev *dev, struct ether_addr *mc_addr_set, uint32_t nb_mc_addr) {
    uint32_t i;
    int ret;
