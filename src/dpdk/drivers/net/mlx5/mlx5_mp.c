 ...
 *   Message type.
 */
static inline void mp_init_msg(struct rte_eth_dev *dev, struct rte_mp_msg *msg, enum mlx5_mp_req_type type) {
...
static int mp_primary_handle(const struct rte_mp_msg *mp_msg, const void *peer) {
    ...
    const struct mlx5_mp_param *param = (const struct mlx5_mp_param *)mp_msg->param;
 ...
 *   Request type.
 */
static void mp_req_on_rxtx(struct rte_eth_dev *dev, enum mlx5_mp_req_type type) {
    ...
    if (type != MLX5_MP_REQ_START_RXTX && type != MLX5_MP_REQ_STOP_RXTX) {
        DRV_LOG(ERR, "port %u unknown request (req_type %d)", dev->data->port_id, type);
        return;
    }
    mp_init_msg(dev, &mp_req, type);
    ret = rte_mp_request_sync(&mp_req, &mp_rep, &ts);
    if (ret) {
        DRV_LOG(ERR, "port %u failed to request stop/start Rx/Tx (%d)", dev->data->port_id, type);
        goto exit;
    }
    if (mp_rep.nb_sent != mp_rep.nb_received) {
        DRV_LOG(ERR, "port %u not all secondaries responded (req_type %d)", dev->data->port_id, type);
        goto exit;
    }
    for (i = 0; i < mp_rep.nb_received; i++) {
        ...
        if (res->result) {
            DRV_LOG(ERR, "port %u request failed on secondary #%d", dev->data->port_id, i);
            goto exit;
        }
    }
exit:
 ...
 *   Pointer to Ethernet structure.
 */
void mlx5_mp_req_start_rxtx(struct rte_eth_dev *dev) { mp_req_on_rxtx(dev, MLX5_MP_REQ_START_RXTX); }
 ...
 *   Pointer to Ethernet structure.
 */
void mlx5_mp_req_stop_rxtx(struct rte_eth_dev *dev) { mp_req_on_rxtx(dev, MLX5_MP_REQ_STOP_RXTX); }
...
int mlx5_mp_req_mr_create(struct rte_eth_dev *dev, uintptr_t addr) {
    ...
    ret = rte_mp_request_sync(&mp_req, &mp_rep, &ts);
    if (ret) {
        DRV_LOG(ERR, "port %u request to primary process failed", dev->data->port_id);
        return -rte_errno;
    }
...
int mlx5_mp_req_verbs_cmd_fd(struct rte_eth_dev *dev) {
    ...
    if (res->result) {
        ...
        DRV_LOG(ERR, "port %u failed to get command FD from primary process", dev->data->port_id);
        ret = -rte_errno;
        goto exit;
    }
    assert(mp_res->num_fds == 1);
    ret = mp_res->fds[0];
    DRV_LOG(DEBUG, "port %u command FD from primary is %d", dev->data->port_id, ret);
exit:
    free(mp_rep.msgs);
    return ret;
}
