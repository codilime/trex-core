 ...
 */
static uint32_t mr_btree_lookup(struct mlx5_mr_btree *bt, uint16_t *idx, uintptr_t addr) {
    ...
    assert(bt->len > 0 || (lkp_tbl[0].start == 0 && lkp_tbl[0].lkey == UINT32_MAX));
    /* Binary search. */
    do {
 ...
 *   0 on success, -1 on failure.
 */
static int mr_btree_insert(struct mlx5_mr_btree *bt, struct mlx5_mr_cache *entry) {
    struct mlx5_mr_cache *lkp_tbl;
    uint16_t idx = 0;
    size_t shift;
...
int mlx5_mr_btree_init(struct mlx5_mr_btree *bt, int n, int socket) {
    ...
    bt->table = rte_calloc_socket("B-tree table", n, sizeof(struct mlx5_mr_cache), 0, socket);
    if (bt->table == NULL) {
        rte_errno = ENOMEM;
        DEBUG("failed to allocate memory for btree cache on socket %d", socket);
        return -rte_errno;
    }
    bt->size = n;
 ...
 */
static int mr_find_next_chunk(struct mlx5_mr *mr, struct mlx5_mr_cache *entry, int base_idx) {
    uintptr_t start = 0;
    uintptr_t end = 0;
    uint32_t idx = 0;
 ...
 *   0 on success, -1 on failure.
 */
static int mr_insert_dev_cache(struct mlx5_ibv_shared *sh, struct mlx5_mr *mr) {
    unsigned int n;

    DRV_LOG(DEBUG, "device %s inserting MR(%p) to global cache", sh->ibdev_name, (void *)mr);
 ...
 */
static struct mlx5_mr *mr_lookup_dev_list(struct mlx5_ibv_shared *sh, struct mlx5_mr_cache *entry, uintptr_t addr) {
    struct mlx5_mr *mr;
 ...
 */
static uint32_t mr_lookup_dev(struct mlx5_ibv_shared *sh, struct mlx5_mr_cache *entry, uintptr_t addr) {
    uint16_t idx;
    ...
    assert(lkey == UINT32_MAX || (addr >= entry->start && addr < entry->end));
    return lkey;
}
 ...
 */
static void mr_free(struct mlx5_mr *mr) {
    if (mr == NULL)
        return;
 ...
 *   Pointer to Ethernet device shared context.
 */
static void mlx5_mr_garbage_collect(struct mlx5_ibv_shared *sh) {
...
static int mr_find_contig_memsegs_cb(const struct rte_memseg_list *msl, const struct rte_memseg *ms, size_t len,
                                     void *arg) {
 ...
 *   Searched LKey on success, UINT32_MAX on failure and rte_errno is set.
 */
static uint32_t mlx5_mr_create_secondary(struct rte_eth_dev *dev, struct mlx5_mr_cache *entry, uintptr_t addr) {
    struct mlx5_priv *priv = dev->data->dev_private;
    int ret;

    DEBUG("port %u requesting MR creation for address (%p)", dev->data->port_id, (void *)addr);
    ret = mlx5_mp_req_mr_create(dev, addr);
    if (ret) {
        DEBUG("port %u fail to request MR creation for address (%p)", dev->data->port_id, (void *)addr);
        return UINT32_MAX;
    }
    ...
    DEBUG("port %u MR CREATED by primary process for %p:\n"
          "  [0x%" PRIxPTR ", 0x%" PRIxPTR "), lkey=0x%x",
          dev->data->port_id, (void *)addr, entry->start, entry->end, entry->lkey);
    return entry->lkey;
}
 ...
 *   Searched LKey on success, UINT32_MAX on failure and rte_errno is set.
 */
uint32_t mlx5_mr_create_primary(struct rte_eth_dev *dev, struct mlx5_mr_cache *entry, uintptr_t addr) {
    struct mlx5_priv *priv = dev->data->dev_private;
    ...
    DRV_LOG(DEBUG, "port %u creating a MR using address (%p)", dev->data->port_id, (void *)addr);
    /*
...
alloc_resources:
    ...
    DEBUG("port %u extending %p to [0x%" PRIxPTR ", 0x%" PRIxPTR "),"
          ...
          dev->data->port_id, (void *)addr, data.start, data.end, msl->page_sz, ms_n);
    /* Size of memory for bitmap. */
    bmp_size = rte_bitmap_get_memory_footprint(ms_n);
    mr = rte_zmalloc_socket(NULL, RTE_ALIGN_CEIL(sizeof(*mr), RTE_CACHE_LINE_SIZE) + bmp_size, RTE_CACHE_LINE_SIZE,
                            msl->socket_id);
    if (mr == NULL) {
    ...
    data_re = data;
    if (len > msl->page_sz && !rte_memseg_contig_walk(mr_find_contig_memsegs_cb, &data_re)) {
    ...
    if (mr_lookup_dev(sh, entry, addr) != UINT32_MAX) {
        ...
        DEBUG("port %u found MR for %p on final lookup, abort", dev->data->port_id, (void *)addr);
        rte_rwlock_write_unlock(&sh->mr.rwlock);
        rte_rwlock_read_unlock(&mcfg->memory_hotplug_lock);
        /*
     ...
     */
    mr->ibv_mr = mlx5_glue->reg_mr(sh->pd, (void *)data.start, len, IBV_ACCESS_LOCAL_WRITE);
    if (mr->ibv_mr == NULL) {
        DEBUG("port %u fail to create a verbs MR for address (%p)", dev->data->port_id, (void *)addr);
        rte_errno = EINVAL;
        goto err_mrlock;
    }
    ...
    DEBUG("port %u MR CREATED (%p) for %p:\n"
          ...
          dev->data->port_id, (void *)mr, (void *)addr, data.start, data.end, rte_cpu_to_be_32(mr->ibv_mr->lkey),
 ...
 *   Searched LKey on success, UINT32_MAX on failure and rte_errno is set.
 */
static uint32_t mlx5_mr_create(struct rte_eth_dev *dev, struct mlx5_mr_cache *entry, uintptr_t addr) {
    uint32_t ret = 0;
 ...
 *   Pointer to Ethernet device shared context.
 */
static void mr_rebuild_dev_cache(struct mlx5_ibv_shared *sh) {
    struct mlx5_mr *mr;
 ...
 */
static void mlx5_mr_mem_event_free_cb(struct mlx5_ibv_shared *sh, const void *addr, size_t len) {
    const struct rte_memseg_list *msl;
    struct mlx5_mr *mr;
    int ms_n;
    int i;
    ...
    DEBUG("device %s free callback: addr=%p, len=%zu", sh->ibdev_name, addr, len);
    ...
    for (i = 0; i < ms_n; ++i) {
        ...
        DEBUG("device %s MR(%p): clear bitmap[%u] for addr %p", sh->ibdev_name, (void *)mr, pos, (void *)start);
        rte_bitmap_clear(mr->ms_bmp, pos);
        if (--mr->ms_n == 0) {
            LIST_REMOVE(mr, mr);
            LIST_INSERT_HEAD(&sh->mr.mr_free_list, mr, mr);
            DEBUG("device %s remove MR(%p) from list", sh->ibdev_name, (void *)mr);
        }
        /*
 ...
 *   Size of memory.
 */
void mlx5_mr_mem_event_cb(enum rte_mem_event event_type, const void *addr, size_t len, void *arg __rte_unused) {
 ...
 *   Searched LKey on success, UINT32_MAX on no match.
 */
static uint32_t mlx5_mr_lookup_dev(struct rte_eth_dev *dev, struct mlx5_mr_ctrl *mr_ctrl, struct mlx5_mr_cache *entry,
                                   uintptr_t addr) {
    struct mlx5_priv *priv = dev->data->dev_private;
    struct mlx5_ibv_shared *sh = priv->sh;
 ...
 *   Searched LKey on success, UINT32_MAX on no match.
 */
static uint32_t mlx5_mr_addr2mr_bh(struct rte_eth_dev *dev, struct mlx5_mr_ctrl *mr_ctrl, uintptr_t addr) {
    uint32_t lkey;
    uint16_t bh_idx = 0;
 ...
 *   Searched LKey on success, UINT32_MAX on no match.
 */
static uint32_t mlx5_tx_addr2mr_bh(struct mlx5_txq_data *txq, uintptr_t addr) {
...
void mlx5_mr_flush_local_cache(struct mlx5_mr_ctrl *mr_ctrl) {
    ...
    DRV_LOG(DEBUG, "mr_ctrl(%p): flushed, cur_gen=%d", (void *)mr_ctrl, mr_ctrl->cur_gen);
}
 ...
 */
static struct mlx5_mr *mlx5_create_mr_ext(struct rte_eth_dev *dev, uintptr_t addr, size_t len, int socket_id) {
    struct mlx5_priv *priv = dev->data->dev_private;
    struct mlx5_mr *mr = NULL;

    mr = rte_zmalloc_socket(NULL, RTE_ALIGN_CEIL(sizeof(*mr), RTE_CACHE_LINE_SIZE), RTE_CACHE_LINE_SIZE, socket_id);
    if (mr == NULL)
        return NULL;
    mr->ibv_mr = mlx5_glue->reg_mr(priv->sh->pd, (void *)addr, len, IBV_ACCESS_LOCAL_WRITE);
    if (mr->ibv_mr == NULL) {
        DRV_LOG(WARNING, "port %u fail to create a verbs MR for address (%p)", dev->data->port_id, (void *)addr);
        rte_free(mr);
        return NULL;
    }
    ...
    DRV_LOG(DEBUG,
            ...
            "  [0x%" PRIxPTR ", 0x%" PRIxPTR "),"
            " lkey=0x%x base_idx=%u ms_n=%u, ms_bmp_n=%u",
            dev->data->port_id, (void *)mr, (void *)addr, addr, addr + len, rte_cpu_to_be_32(mr->ibv_mr->lkey),
            mr->ms_base_idx, mr->ms_n, mr->ms_bmp_n);
    return mr;
}
 ...
 */
static void mlx5_mr_update_ext_mp_cb(struct rte_mempool *mp, void *opaque, struct rte_mempool_memhdr *memhdr,
    ...
    if (lkey != UINT32_MAX)
        return;
    DRV_LOG(DEBUG, "port %u register MR for chunk #%d of mempool (%s)", dev->data->port_id, mem_idx, mp->name);
 ...
 */
static struct rte_eth_dev *pci_dev_to_eth_dev(struct rte_pci_device *pdev) {
    uint16_t port_id;
 ...
 */
int mlx5_dma_map(struct rte_pci_device *pdev, void *addr, uint64_t iova __rte_unused, size_t len) {
    struct rte_eth_dev *dev;
    struct mlx5_mr *mr;
    ...
    if (!dev) {
        DRV_LOG(WARNING,
                "unable to find matching ethdev "
                "to PCI device %p",
                (void *)pdev);
        rte_errno = ENODEV;
        return -1;
    ...
    if (!mr) {
        DRV_LOG(WARNING, "port %u unable to dma map", dev->data->port_id);
        rte_errno = EINVAL;
        return -1;
    }
    sh = priv->sh;
 ...
 *   0 on success, negative value on error.
 */
int mlx5_dma_unmap(struct rte_pci_device *pdev, void *addr, uint64_t iova __rte_unused, size_t len __rte_unused) {
    struct rte_eth_dev *dev;
    struct mlx5_priv *priv;
    struct mlx5_ibv_shared *sh;
    struct mlx5_mr *mr;
    ...
    if (!mr) {
        rte_rwlock_read_unlock(&sh->mr.rwlock);
        DRV_LOG(WARNING,
                "address 0x%" PRIxPTR " wasn't registered "
                "to PCI device %p",
                (uintptr_t)addr, (void *)pdev);
        rte_errno = EINVAL;
        return -1;
    }
    LIST_REMOVE(mr, mr);
 ...
 *   0 on success, -1 on failure.
 */
static uint32_t mlx5_mr_update_ext_mp(struct rte_eth_dev *dev, struct mlx5_mr_ctrl *mr_ctrl, struct rte_mempool *mp) {
 ...
 */
uint32_t mlx5_tx_update_ext_mp(struct mlx5_txq_data *txq, uintptr_t addr, struct rte_mempool *mp) {
    struct mlx5_txq_ctrl *txq_ctrl = container_of(txq, struct mlx5_txq_ctrl, txq);
    struct mlx5_mr_ctrl *mr_ctrl = &txq->mr_ctrl;
    struct mlx5_priv *priv = txq_ctrl->priv;
...
static void mlx5_mr_update_mp_cb(struct rte_mempool *mp __rte_unused, void *opaque, struct rte_mempool_memhdr *memhdr,
                                 unsigned mem_idx __rte_unused) {
    struct mr_update_mp_data *data = opaque;
    uint32_t lkey;
    ...
    lkey = mlx5_mr_addr2mr_bh(data->dev, data->mr_ctrl, (uintptr_t)memhdr->addr);
    if (lkey == UINT32_MAX)
        data->ret = -1;
}
 ...
 *   0 on success, -1 on failure.
 */
int mlx5_mr_update_mp(struct rte_eth_dev *dev, struct mlx5_mr_ctrl *mr_ctrl, struct rte_mempool *mp) {
    struct mr_update_mp_data data = {
        .dev = dev,
        .mr_ctrl = mr_ctrl,
        .ret = 0,
 ...
 *   Pointer to Ethernet device shared context.
 */
void mlx5_mr_dump_dev(struct mlx5_ibv_shared *sh __rte_unused) {
#ifndef NDEBUG
    struct mlx5_mr *mr;
    int mr_n = 0;
    int chunk_n = 0;
    ...
    LIST_FOREACH(mr, &sh->mr.mr_list, mr) {
        ...
        DEBUG("device %s MR[%u], LKey = 0x%x, ms_n = %u, ms_bmp_n = %u", sh->ibdev_name, mr_n++,
              rte_cpu_to_be_32(mr->ibv_mr->lkey), mr->ms_n, mr->ms_bmp_n);
        if (mr->ms_n == 0)
            continue;
        for (n = 0; n < mr->ms_bmp_n;) {
            struct mlx5_mr_cache ret = {
            ...
            if (!ret.end)
                break;
            DEBUG("  chunk[%u], [0x%" PRIxPTR ", 0x%" PRIxPTR ")", chunk_n++, ret.start, ret.end);
        }
    }
