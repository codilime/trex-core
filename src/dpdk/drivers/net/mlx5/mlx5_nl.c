 ...
 */
#ifndef MLX5_NDA_RTA
#define MLX5_NDA_RTA(r) ((struct rtattr *)(((char *)(r)) + NLMSG_ALIGN(sizeof(struct ndmsg))))
#endif
 ...
 *   rte_errno is set.
 */
static int mlx5_nl_request(int nlsk_fd, struct nlmsghdr *nh, uint32_t sn, void *req, int len) {
 ...
 */
static int mlx5_nl_recv(int nlsk_fd, uint32_t sn, int (*cb)(struct nlmsghdr *, void *arg), void *arg) {
    struct sockaddr_nl sa;
    ...
    do {
        ...
        for (; NLMSG_OK(nh, (unsigned int)recv_bytes); nh = NLMSG_NEXT(nh, recv_bytes)) {
...
static int mlx5_nl_mac_addr_cb(struct nlmsghdr *nh, void *arg) {
    ...
    for (attribute = MLX5_NDA_RTA(r); RTA_OK(attribute, len); attribute = RTA_NEXT(attribute, len)) {
...
#endif
            memcpy(&(*data->mac)[data->mac_n++], RTA_DATA(attribute), ETHER_ADDR_LEN);
        }
    }
    return 0;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_nl_mac_addr_list(struct rte_eth_dev *dev, struct ether_addr (*mac)[], int *mac_n) {
    ...
    ret = mlx5_nl_request(fd, &req.hdr, sn, &req.ifm, sizeof(struct ifinfomsg));
    if (ret < 0)
        goto error;
    ...
    *mac_n = data.mac_n;
    return 0;
error:
    DRV_LOG(DEBUG, "port %u cannot retrieve MAC address list %s", dev->data->port_id, strerror(rte_errno));
    return -rte_errno;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_nl_mac_addr_modify(struct rte_eth_dev *dev, struct ether_addr *mac, int add) {
    struct mlx5_priv *priv = dev->data->dev_private;
    unsigned int iface_idx = mlx5_ifindex(dev);
    struct {
        struct nlmsghdr hdr;
    ...
    req.hdr.nlmsg_len = NLMSG_ALIGN(req.hdr.nlmsg_len) + RTA_ALIGN(req.rta.rta_len);
...
error:
    DRV_LOG(DEBUG,
            ...
            " %s",
            dev->data->port_id, add ? "add" : "remove", mac->addr_bytes[0], mac->addr_bytes[1], mac->addr_bytes[2],
            mac->addr_bytes[3], mac->addr_bytes[4], mac->addr_bytes[5], strerror(rte_errno));
    return -rte_errno;
}
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_nl_mac_addr_add(struct rte_eth_dev *dev, struct ether_addr *mac, uint32_t index) {
    struct mlx5_priv *priv = dev->data->dev_private;
    int ret;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_nl_mac_addr_remove(struct rte_eth_dev *dev, struct ether_addr *mac, uint32_t index) {
    struct mlx5_priv *priv = dev->data->dev_private;
...
void mlx5_nl_mac_addr_sync(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i != macs_n; ++i) {
        ...
        for (j = 0; j != MLX5_MAX_MAC_ADDRESSES; ++j)
            if (is_same_ether_addr(&macs[i], &dev->data->mac_addrs[j]))
                break;
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
static int mlx5_nl_device_flags(struct rte_eth_dev *dev, uint32_t flags, int enable) {
    struct mlx5_priv *priv = dev->data->dev_private;
    unsigned int iface_idx = mlx5_ifindex(dev);
    struct {
        struct nlmsghdr hdr;
...
int mlx5_nl_promisc(struct rte_eth_dev *dev, int enable) {
    ...
    if (ret)
        DRV_LOG(DEBUG, "port %u cannot %s promisc mode: Netlink error %s", dev->data->port_id,
                enable ? "enable" : "disable", strerror(rte_errno));
    return ret;
}
...
int mlx5_nl_allmulti(struct rte_eth_dev *dev, int enable) {
    ...
    if (ret)
        DRV_LOG(DEBUG, "port %u cannot %s allmulti mode: Netlink error %s", dev->data->port_id,
                enable ? "enable" : "disable", strerror(rte_errno));
    return ret;
}
...
static int mlx5_nl_cmdget_cb(struct nlmsghdr *nh, void *arg) {
    ...
    if (nh->nlmsg_type != RDMA_NL_GET_TYPE(RDMA_NL_NLDEV, RDMA_NLDEV_CMD_GET) &&
        nh->nlmsg_type != RDMA_NL_GET_TYPE(RDMA_NL_NLDEV, RDMA_NLDEV_CMD_PORT_GET))
        goto error;
...
unsigned int mlx5_nl_ifindex(int nl, const char *name, uint32_t pindex) {
    ...
    union {
        struct nlmsghdr nh;
        uint8_t
            buf[NLMSG_HDRLEN + NLA_HDRLEN + NLA_ALIGN(sizeof(data.ibindex)) + NLA_HDRLEN + NLA_ALIGN(sizeof(pindex))];
    } req = {
        .nh =
            {
    ...
    memcpy((void *)((uintptr_t)na + NLA_HDRLEN), &data.ibindex, sizeof(data.ibindex));
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_nl_switch_info(int nl, unsigned int ifindex, struct mlx5_switch_info *info) {
    uint32_t seq = random();
    struct {
        struct nlmsghdr nh;
        ...
        .nh =
            {
                .nlmsg_len = NLMSG_LENGTH(sizeof(req.info) + RTA_LENGTH(sizeof(uint32_t))),
