...
uint8_t rss_hash_default_key[] = {
    0x2c, 0xc6, 0x81, 0xd1, 0x5b, 0xdb, 0xf4, 0xf7, 0xfc, 0xa2, 0x83, 0x19, 0xdb, 0x1a,
    0x3e, 0x94, 0x6b, 0x9e, 0x38, 0xd9, 0x2c, 0x9c, 0x03, 0xd1, 0xad, 0x99, 0x44, 0xa7,
    0xd9, 0x56, 0x3d, 0x59, 0x06, 0x3c, 0x25, 0xf3, 0xfc, 0x1f, 0xdc, 0x2a,
};

/* Length of the default RSS hash key. */
static_assert(MLX5_RSS_HASH_KEY_LEN == (unsigned int)sizeof(rss_hash_default_key), "wrong RSS default key size.");
 ...
 */
inline int mlx5_check_mprq_support(struct rte_eth_dev *dev) {
    struct mlx5_priv *priv = dev->data->dev_private;

    if (priv->config.mprq.enabled && priv->rxqs_n >= priv->config.mprq.min_rxqs_num)
        return 1;
    return -ENOTSUP;
}
 ...
 */
inline int mlx5_rxq_mprq_enabled(struct mlx5_rxq_data *rxq) { return rxq->strd_num_n > 0; }
 ...
 *   0 if disabled, otherwise enabled.
 */
inline int mlx5_mprq_enabled(struct rte_eth_dev *dev) {
    struct mlx5_priv *priv = dev->data->dev_private;
    uint16_t i;
    uint16_t n = 0;
...
static int rxq_alloc_elts_mprq(struct mlx5_rxq_ctrl *rxq_ctrl) {
    ...
    DRV_LOG(DEBUG, "port %u Rx queue %u allocated and configured %u segments", rxq->port_id, rxq->idx, wqe_n);
    return 0;
error:
    ...
    DRV_LOG(DEBUG, "port %u Rx queue %u failed, freed everything", rxq->port_id, rxq->idx);
...
static int rxq_alloc_elts_sprq(struct mlx5_rxq_ctrl *rxq_ctrl) {
    ...
    for (i = 0; (i != elts_n); ++i) {
        ...
        if (buf == NULL) {
            DRV_LOG(ERR, "port %u empty mbuf pool", PORT_ID(rxq_ctrl->priv));
            rte_errno = ENOMEM;
            goto error;
        }
    ...
    DRV_LOG(DEBUG,
            ...
            " (max %u packets)",
            PORT_ID(rxq_ctrl->priv), rxq_ctrl->rxq.idx, elts_n, elts_n / (1 << rxq_ctrl->rxq.sges_n));
    return 0;
error:
    err = rte_errno; /* Save rte_errno before cleanup. */
    elts_n = i;
    ...
    DRV_LOG(DEBUG, "port %u Rx queue %u failed, freed everything", PORT_ID(rxq_ctrl->priv), rxq_ctrl->rxq.idx);
    rte_errno = err; /* Restore rte_errno. */
    return -rte_errno;
}
...
int rxq_alloc_elts(struct mlx5_rxq_ctrl *rxq_ctrl) {
    return mlx5_rxq_mprq_enabled(&rxq_ctrl->rxq) ? rxq_alloc_elts_mprq(rxq_ctrl) : rxq_alloc_elts_sprq(rxq_ctrl);
}
...
static void rxq_free_elts_mprq(struct mlx5_rxq_ctrl *rxq_ctrl) {
    ...
    DRV_LOG(DEBUG, "port %u Multi-Packet Rx queue %u freeing WRs", rxq->port_id, rxq->idx);
...
static void rxq_free_elts_sprq(struct mlx5_rxq_ctrl *rxq_ctrl) {
    ...
    DRV_LOG(DEBUG, "port %u Rx queue %u freeing WRs", PORT_ID(rxq_ctrl->priv), rxq->idx);
    if (rxq->elts == NULL)
        return;
    /**
...
void mlx5_rxq_cleanup(struct mlx5_rxq_ctrl *rxq_ctrl) {
    DRV_LOG(DEBUG, "port %u cleaning up Rx queue %u", PORT_ID(rxq_ctrl->priv), rxq_ctrl->rxq.idx);
    if (rxq_ctrl->ibv)
...
uint64_t mlx5_get_rx_queue_offloads(struct rte_eth_dev *dev) {
    ...
    uint64_t offloads = (DEV_RX_OFFLOAD_SCATTER | DEV_RX_OFFLOAD_TIMESTAMP | DEV_RX_OFFLOAD_JUMBO_FRAME);
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 */
int mlx5_rx_queue_setup(struct rte_eth_dev *dev, uint16_t idx, uint16_t desc, unsigned int socket,
    ...
    DRV_LOG(DEBUG, "port %u configuring Rx queue %u for %u descriptors", dev->data->port_id, idx, desc);
    if (idx >= priv->rxqs_n) {
        DRV_LOG(ERR, "port %u Rx queue index out of range (%u >= %u)", dev->data->port_id, idx, priv->rxqs_n);
        rte_errno = EOVERFLOW;
        return -rte_errno;
    }
    if (!mlx5_rxq_releasable(dev, idx)) {
        DRV_LOG(ERR, "port %u unable to release queue index %u", dev->data->port_id, idx);
        rte_errno = EBUSY;
        return -rte_errno;
    }
    ...
    if (!rxq_ctrl) {
        DRV_LOG(ERR, "port %u unable to allocate queue index %u", dev->data->port_id, idx);
        rte_errno = ENOMEM;
        return -rte_errno;
    }
...
int mlx5_rx_intr_vec_enable(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i != n; ++i) {
        ...
        if (!rxq_ibv || !rxq_ibv->channel) {
            ...
            intr_handle->intr_vec[i] = RTE_INTR_VEC_RXTX_OFFSET + RTE_MAX_RXTX_INTR_VEC_ID;
            continue;
        }
...
void mlx5_rx_intr_vec_disable(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i != n; ++i) {
        ...
        if (intr_handle->intr_vec[i] == RTE_INTR_VEC_RXTX_OFFSET + RTE_MAX_RXTX_INTR_VEC_ID)
            continue;
        /**
...
static inline void mlx5_arm_cq(struct mlx5_rxq_data *rxq, int sq_n_rxq) {
    ...
    mlx5_uar_write64(rte_cpu_to_be_64(doorbell), cq_db_reg, rxq->uar_lock_cq);
}
...
exit:
    ...
    if (rxq_ibv)
        mlx5_rxq_ibv_release(rxq_ibv);
    DRV_LOG(WARNING, "port %u unable to disable interrupt on Rx queue %d", dev->data->port_id, rx_queue_id);
...
struct mlx5_rxq_ibv *mlx5_rxq_ibv_new(struct rte_eth_dev *dev, uint16_t idx) {
    ...
    struct mlx5_rxq_ctrl *rxq_ctrl = container_of(rxq_data, struct mlx5_rxq_ctrl, rxq);
    ...
    tmpl = rte_calloc_socket(__func__, 1, sizeof(*tmpl), 0, rxq_ctrl->socket);
    if (!tmpl) {
        DRV_LOG(ERR, "port %u Rx queue %u cannot allocate verbs resources", dev->data->port_id, rxq_data->idx);
        rte_errno = ENOMEM;
        goto error;
    }
    ...
    if (rxq_ctrl->irq) {
        ...
        if (!tmpl->channel) {
            DRV_LOG(ERR, "port %u: comp channel creation failure", dev->data->port_id);
            rte_errno = ENOMEM;
            goto error;
        }
    }
    ...
    if (config->cqe_comp && !rxq_data->hw_timestamp) {
        attr.cq.mlx5.comp_mask |= MLX5DV_CQ_INIT_ATTR_MASK_COMPRESSED_CQE;
#ifdef HAVE_IBV_DEVICE_STRIDING_RQ_SUPPORT
        attr.cq.mlx5.cqe_comp_res_format = mprq_en ? MLX5DV_CQE_RES_FORMAT_CSUM_STRIDX : MLX5DV_CQE_RES_FORMAT_HASH;
#else
...
#endif
    tmpl->cq = mlx5_glue->cq_ex_to_cq(mlx5_glue->dv_create_cq(priv->sh->ctx, &attr.cq.ibv, &attr.cq.mlx5));
    if (tmpl->cq == NULL) {
        DRV_LOG(ERR, "port %u Rx queue %u CQ creation failure", dev->data->port_id, idx);
        rte_errno = ENOMEM;
        goto error;
    }
    DRV_LOG(DEBUG, "port %u device_attr.max_qp_wr is %d", dev->data->port_id,
            priv->sh->device_attr.orig_attr.max_qp_wr);
    DRV_LOG(DEBUG, "port %u device_attr.max_sge is %d", dev->data->port_id, priv->sh->device_attr.orig_attr.max_sge);
    ...
    if (mprq_en) {
        struct mlx5dv_striding_rq_init_attr *mprq_attr = &attr.wq.mlx5.striding_rq_attrs;
    ...
    tmpl->wq = mlx5_glue->dv_create_wq(priv->sh->ctx, &attr.wq.ibv, &attr.wq.mlx5);
#else
    tmpl->wq = mlx5_glue->create_wq(priv->sh->ctx, &attr.wq.ibv);
#endif
    if (tmpl->wq == NULL) {
        DRV_LOG(ERR, "port %u Rx queue %u WQ creation failure", dev->data->port_id, idx);
        rte_errno = ENOMEM;
        goto error;
    }
    /*
     ...
     */
    if (attr.wq.ibv.max_wr != (wqe_n >> rxq_data->sges_n) || attr.wq.ibv.max_sge != (1u << rxq_data->sges_n)) {
        DRV_LOG(ERR,
                "port %u Rx queue %u requested %u*%u but got %u*%u"
                " WRs*SGEs",
                dev->data->port_id, idx, wqe_n >> rxq_data->sges_n, (1 << rxq_data->sges_n), attr.wq.ibv.max_wr,
                attr.wq.ibv.max_sge);
        rte_errno = EINVAL;
        goto error;
    }
    ...
    if (ret) {
        DRV_LOG(ERR, "port %u Rx queue %u WQ state to IBV_WQS_RDY failed", dev->data->port_id, idx);
        rte_errno = ret;
        goto error;
    }
    obj.cq.in = tmpl->cq;
    ...
    for (i = 0; (i != wqe_n); ++i) {
        ...
        if (mprq_en) {
            ...
            scat = &((volatile struct mlx5_wqe_mprq *)rxq_data->wqes)[i].dseg;
            addr = (uintptr_t)mlx5_mprq_buf_addr(buf);
            byte_count = (1 << rxq_data->strd_sz_n) * (1 << rxq_data->strd_num_n);
        } else {
            struct rte_mbuf *buf = (*rxq_data->elts)[i];

            scat = &((volatile struct mlx5_wqe_data_seg *)rxq_data->wqes)[i];
    ...
    *rxq_data->rq_db = rte_cpu_to_be_32(rxq_data->rq_ci);
    DRV_LOG(DEBUG, "port %u rxq %u updated with %p", dev->data->port_id, idx, (void *)&tmpl);
...
int mlx5_rxq_ibv_release(struct mlx5_rxq_ibv *rxq_ibv) {
    ...
    if (rte_atomic32_dec_and_test(&rxq_ibv->refcnt)) {
        ...
        if (rxq_ibv->channel)
            claim_zero(mlx5_glue->destroy_comp_channel(rxq_ibv->channel));
...
int mlx5_rxq_ibv_verify(struct rte_eth_dev *dev) {
    ...
    LIST_FOREACH(rxq_ibv, &priv->rxqsibv, next) {
        DRV_LOG(DEBUG, "port %u Verbs Rx queue %u still referenced", dev->data->port_id, rxq_ibv->rxq_ctrl->rxq.idx);
        ++ret;
    }
    return ret;
}
 ...
 */
static inline void mlx5_mprq_buf_init(struct rte_mempool *mp, void *opaque_arg __rte_unused, void *_m,
...
int mlx5_mprq_free_mp(struct rte_eth_dev *dev) {
    ...
    if (mp == NULL)
        return 0;
    DRV_LOG(DEBUG, "port %u freeing mempool (%s) for Multi-Packet RQ", dev->data->port_id, mp->name);
    /*
    ...
    if (!rte_mempool_full(mp)) {
        DRV_LOG(ERR, "port %u mempool for Multi-Packet RQ is still in use", dev->data->port_id);
        rte_errno = EBUSY;
        return -rte_errno;
    }
    rte_mempool_free(mp);
...
int mlx5_mprq_alloc_mp(struct rte_eth_dev *dev) {
    ...
    if (mp != NULL && mp->elt_size >= obj_size && mp->size >= obj_num) {
        DRV_LOG(DEBUG, "port %u mempool %s is being reused", dev->data->port_id, mp->name);
        /* Reuse. */
        goto exit;
    } else if (mp != NULL) {
        DRV_LOG(DEBUG, "port %u mempool %s should be resized, freeing it", dev->data->port_id, mp->name);
        /*
    ...
    mp = rte_mempool_create(name, obj_num, obj_size, MLX5_MPRQ_MP_CACHE_SZ, 0, NULL, NULL, mlx5_mprq_buf_init, NULL,
 ...
 */
struct mlx5_rxq_ctrl *mlx5_rxq_new(struct rte_eth_dev *dev, uint16_t idx, uint16_t desc, unsigned int socket,
                                   const struct rte_eth_rxconf *conf, struct rte_mempool *mp) {
    struct mlx5_priv *priv = dev->data->dev_private;
    ...
    tmpl = rte_calloc_socket("RXQ", 1, sizeof(*tmpl) + desc_n * sizeof(struct rte_mbuf *), 0, socket);
    if (!tmpl) {
        rte_errno = ENOMEM;
        return NULL;
    }
    if (mlx5_mr_btree_init(&tmpl->rxq.mr_ctrl.cache_bh, MLX5_MR_BTREE_CACHE_N, socket)) {
    ...
    mprq_stride_size =
        dev->data->dev_conf.rxmode.max_rx_pkt_len + sizeof(struct rte_mbuf_ext_shared_info) + RTE_PKTMBUF_HEADROOM;
    if (mprq_en && desc > (1U << config->mprq.stride_num_n) &&
        ...
        tmpl->rxq.strd_sz_n = RTE_MAX(log2above(mprq_stride_size), config->mprq.min_stride_size_n);
        tmpl->rxq.strd_shift_en = MLX5_MPRQ_TWO_BYTE_SHIFT;
        tmpl->rxq.mprq_max_memcpy_len = RTE_MIN(mb_len - RTE_PKTMBUF_HEADROOM, config->mprq.max_memcpy_len);
        DRV_LOG(DEBUG,
                "port %u Rx queue %u: Multi-Packet RQ is enabled"
                " strd_num_n = %u, strd_sz_n = %u",
                dev->data->port_id, idx, tmpl->rxq.strd_num_n, tmpl->rxq.strd_sz_n);
    } else if (dev->data->dev_conf.rxmode.max_rx_pkt_len <= (mb_len - RTE_PKTMBUF_HEADROOM)) {
        tmpl->rxq.sges_n = 0;
    } else if (offloads & DEV_RX_OFFLOAD_SCATTER) {
        unsigned int size = RTE_PKTMBUF_HEADROOM + dev->data->dev_conf.rxmode.max_rx_pkt_len;
        unsigned int sges_n;
        ...
        if (size < dev->data->dev_conf.rxmode.max_rx_pkt_len) {
            DRV_LOG(ERR,
                    ...
                    dev->data->port_id, 1 << sges_n, dev->data->dev_conf.rxmode.max_rx_pkt_len);
            rte_errno = EOVERFLOW;
            goto error;
        }
    } else {
        DRV_LOG(WARNING,
                ...
                " not been requested",
                dev->data->port_id, dev->data->dev_conf.rxmode.max_rx_pkt_len, mb_len - RTE_PKTMBUF_HEADROOM);
    }
    if (mprq_en && !mlx5_rxq_mprq_enabled(&tmpl->rxq))
        DRV_LOG(WARNING,
                ...
                dev->data->port_id, desc, mprq_stride_size, (1 << config->mprq.stride_num_n),
                (1 << config->mprq.max_stride_size_n));
    DRV_LOG(DEBUG, "port %u maximum number of segments per packet: %u", dev->data->port_id, 1 << tmpl->rxq.sges_n);
    ...
    DRV_LOG(DEBUG,
            ...
            dev->data->port_id, tmpl->rxq.crc_present ? "disabled" : "enabled", tmpl->rxq.crc_present << 2);
    /* Save port ID. */
    tmpl->rxq.rss_hash = !!priv->rss_conf.rss_hf && (!!(dev->data->dev_conf.rxmode.mq_mode & ETH_MQ_RX_RSS));
    tmpl->rxq.port_id = dev->data->port_id;
    tmpl->priv = priv;
    tmpl->rxq.mp = mp;
    tmpl->rxq.elts_n = log2above(desc);
    tmpl->rxq.rq_repl_thresh = MLX5_VPMD_RXQ_RPLNSH_THRESH(1 << tmpl->rxq.elts_n);
    tmpl->rxq.elts = (struct rte_mbuf * (*)[1 << tmpl->rxq.elts_n])(tmpl + 1);
#ifndef RTE_ARCH_64
...
int mlx5_rxq_verify(struct rte_eth_dev *dev) {
    ...
    LIST_FOREACH(rxq_ctrl, &priv->rxqsctrl, next) {
        DRV_LOG(DEBUG, "port %u Rx Queue %u still referenced", dev->data->port_id, rxq_ctrl->rxq.idx);
        ++ret;
    }
    return ret;
}
 ...
 *   The Verbs object initialised, NULL otherwise and rte_errno is set.
 */
struct mlx5_ind_table_ibv *mlx5_ind_table_ibv_new(struct rte_eth_dev *dev, const uint16_t *queues, uint32_t queues_n) {
    struct mlx5_priv *priv = dev->data->dev_private;
    struct mlx5_ind_table_ibv *ind_tbl;
    const unsigned int wq_n =
        rte_is_power_of_2(queues_n) ? log2above(queues_n) : log2above(priv->config.ind_table_max_size);
    struct ibv_wq *wq[1 << wq_n];
    unsigned int i;
    unsigned int j;

    ind_tbl = rte_calloc(__func__, 1, sizeof(*ind_tbl) + queues_n * sizeof(uint16_t), 0);
    if (!ind_tbl) {
        rte_errno = ENOMEM;
        return NULL;
    }
    ...
    for (j = 0; i != (unsigned int)(1 << wq_n); ++i, ++j)
        wq[i] = wq[j];
    ind_tbl->ind_table = mlx5_glue->create_rwq_ind_table(priv->sh->ctx, &(struct ibv_rwq_ind_table_init_attr){
 ...
 */
struct mlx5_ind_table_ibv *mlx5_ind_table_ibv_get(struct rte_eth_dev *dev, const uint16_t *queues, uint32_t queues_n) {
    struct mlx5_priv *priv = dev->data->dev_private;
    struct mlx5_ind_table_ibv *ind_tbl;

    LIST_FOREACH(ind_tbl, &priv->ind_tbls, next) {
        if ((ind_tbl->queues_n == queues_n) &&
            (memcmp(ind_tbl->queues, queues, ind_tbl->queues_n * sizeof(ind_tbl->queues[0])) == 0))
            break;
    }
    if (ind_tbl) {
        unsigned int i;
 ...
 *   1 while a reference on it exists, 0 when freed.
 */
int mlx5_ind_table_ibv_release(struct rte_eth_dev *dev, struct mlx5_ind_table_ibv *ind_tbl) {
    unsigned int i;
...
int mlx5_ind_table_ibv_verify(struct rte_eth_dev *dev) {
    ...
    LIST_FOREACH(ind_tbl, &priv->ind_tbls, next) {
        DRV_LOG(DEBUG, "port %u Verbs indirection table %p still referenced", dev->data->port_id, (void *)ind_tbl);
        ++ret;
    }
    return ret;
}
 ...
 *   The Verbs object initialised, NULL otherwise and rte_errno is set.
 */
struct mlx5_hrxq *mlx5_hrxq_new(struct rte_eth_dev *dev, const uint8_t *rss_key, uint32_t rss_key_len,
                                uint64_t hash_fields, const uint16_t *queues, uint32_t queues_n,
    ...
    qp = mlx5_glue->dv_create_qp(
        ...
        &(struct ibv_qp_init_attr_ex){
            ...
            .comp_mask = IBV_QP_INIT_ATTR_PD | IBV_QP_INIT_ATTR_IND_TABLE | IBV_QP_INIT_ATTR_RX_HASH,
            .rx_hash_conf =
    ...
    qp = mlx5_glue->create_qp_ex(
        priv->sh->ctx, &(struct ibv_qp_init_attr_ex){
                           .qp_type = IBV_QPT_RAW_PACKET,
                           .comp_mask = IBV_QP_INIT_ATTR_PD | IBV_QP_INIT_ATTR_IND_TABLE | IBV_QP_INIT_ATTR_RX_HASH,
                           .rx_hash_conf =
                               (struct ibv_rx_hash_conf){
 ...
 */
struct mlx5_hrxq *mlx5_hrxq_get(struct rte_eth_dev *dev, const uint8_t *rss_key, uint32_t rss_key_len,
                                uint64_t hash_fields, const uint16_t *queues, uint32_t queues_n) {
    struct mlx5_priv *priv = dev->data->dev_private;
    struct mlx5_hrxq *hrxq;
 ...
 *   1 while a reference on it exists, 0 when freed.
 */
int mlx5_hrxq_release(struct rte_eth_dev *dev, struct mlx5_hrxq *hrxq) {
...
int mlx5_hrxq_ibv_verify(struct rte_eth_dev *dev) {
    ...
    LIST_FOREACH(hrxq, &priv->hrxqs, next) {
        DRV_LOG(DEBUG, "port %u Verbs hash Rx queue %p still referenced", dev->data->port_id, (void *)hrxq);
        ++ret;
    }
    return ret;
}
...
struct mlx5_rxq_ibv *mlx5_rxq_ibv_drop_new(struct rte_eth_dev *dev) {
    ...
    if (!cq) {
        DEBUG("port %u cannot allocate CQ for drop queue", dev->data->port_id);
        rte_errno = errno;
        goto error;
    }
    ...
    if (!wq) {
        DEBUG("port %u cannot allocate WQ for drop queue", dev->data->port_id);
        rte_errno = errno;
        goto error;
    }
    rxq = rte_calloc(__func__, 1, sizeof(*rxq), 0);
    if (!rxq) {
        DEBUG("port %u cannot allocate drop Rx queue memory", dev->data->port_id);
        rte_errno = ENOMEM;
        goto error;
    }
    rxq->cq = cq;
...
struct mlx5_ind_table_ibv *mlx5_ind_table_ibv_drop_new(struct rte_eth_dev *dev) {
    ...
    if (!rxq)
        return NULL;
    tmpl.ind_table = mlx5_glue->create_rwq_ind_table(priv->sh->ctx, &(struct ibv_rwq_ind_table_init_attr){
                                                                        .log_ind_tbl_size = 0,
                                                                        .ind_tbl = &rxq->wq,
                                                                        .comp_mask = 0,
                                                                    });
...
struct mlx5_hrxq *mlx5_hrxq_drop_new(struct rte_eth_dev *dev) {
    ...
    if (!ind_tbl)
        return NULL;
    qp = mlx5_glue->create_qp_ex(
        priv->sh->ctx, &(struct ibv_qp_init_attr_ex){.qp_type = IBV_QPT_RAW_PACKET,
                                                     ...
                                                     .pd = priv->sh->pd});
    if (!qp) {
        DEBUG("port %u cannot allocate QP for drop queue", dev->data->port_id);
        rte_errno = errno;
        goto error;
    }
    hrxq = rte_calloc(__func__, 1, sizeof(*hrxq), 0);
    if (!hrxq) {
        DRV_LOG(WARNING, "port %u cannot allocate memory for drop queue", dev->data->port_id);
        rte_errno = ENOMEM;
        goto error;
    }
    hrxq->ind_table = ind_tbl;
