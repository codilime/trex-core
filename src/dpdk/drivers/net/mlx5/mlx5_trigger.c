...
static int mlx5_rxq_start(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i != priv->rxqs_n; ++i) {
        ...
        mp = mlx5_rxq_mprq_enabled(&rxq_ctrl->rxq) ? rxq_ctrl->rxq.mprq_mp : rxq_ctrl->rxq.mp;
        DRV_LOG(DEBUG,
                "port %u Rx queue %u registering"
                " mp %s having %u chunks",
                dev->data->port_id, rxq_ctrl->rxq.idx, mp->name, mp->nb_mem_chunks);
...
int mlx5_dev_start(struct rte_eth_dev *dev) {
    ...
    if (ret) {
        DRV_LOG(ERR, "port %u Tx queue allocation failed: %s", dev->data->port_id, strerror(rte_errno));
        return -rte_errno;
    }
    ret = mlx5_rxq_start(dev);
    if (ret) {
        DRV_LOG(ERR, "port %u Rx queue allocation failed: %s", dev->data->port_id, strerror(rte_errno));
        mlx5_txq_stop(dev);
        return -rte_errno;
    }
    ...
    if (ret) {
        DRV_LOG(ERR, "port %u Rx interrupt vector creation failed", dev->data->port_id);
        goto error;
    }
    mlx5_stats_init(dev);
    ret = mlx5_traffic_enable(dev);
    if (ret) {
        DRV_LOG(DEBUG, "port %u failed to set defaults flows", dev->data->port_id);
        goto error;
    }
    ret = mlx5_flow_start(dev, &priv->flows);
    if (ret) {
        DRV_LOG(DEBUG, "port %u failed to set flows", dev->data->port_id);
        goto error;
    }
    rte_wmb();
...
int mlx5_traffic_enable(struct rte_eth_dev *dev) {
    ...
    if (dev->data->all_multicast) {
        ...
        for (i = 0; i != vlan_filter_n; ++i) {
            ...
            if (ret)
                goto error;
            ret = mlx5_ctrl_flow_vlan(dev, &ipv6_multi_spec, &ipv6_multi_mask, &vlan_spec, &vlan_mask);
            if (ret)
                goto error;
        }
        if (!vlan_filter_n) {
    ...
    for (i = 0; i != MLX5_MAX_MAC_ADDRESSES; ++i) {
        ...
        if (!memcmp(mac, &cmp, sizeof(*mac)))
            continue;
        memcpy(&unicast.dst.addr_bytes, mac->addr_bytes, ETHER_ADDR_LEN);
        for (j = 0; j != vlan_filter_n; ++j) {
            ...
            ret = mlx5_ctrl_flow_vlan(dev, &unicast, &unicast_mask, &vlan_spec, &vlan_mask);
            if (ret)
                goto error;
        }
        if (!vlan_filter_n) {
