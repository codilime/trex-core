...
int mlx5_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id, int on) {
    ...
    DRV_LOG(DEBUG, "port %u %s VLAN filter ID %" PRIu16, dev->data->port_id, (on ? "enable" : "disable"), vlan_id);
...
void mlx5_vlan_strip_queue_set(struct rte_eth_dev *dev, uint16_t queue, int on) {
    ...
    if (queue >= priv->rxqs_n) {
        DRV_LOG(ERR, "port %u VLAN stripping, invalid queue number %d", dev->data->port_id, queue);
        return;
    }
    DRV_LOG(DEBUG, "port %u set VLAN offloads 0x%x for port %uqueue %d", dev->data->port_id, vlan_offloads,
            rxq->port_id, queue);
    if (!rxq_ctrl->ibv) {
    ...
    if (ret) {
        DRV_LOG(ERR, "port %u failed to modified stripping mode: %s", dev->data->port_id, strerror(rte_errno));
        return;
    }
    /* Update related bits in RX queue. */
    rxq->vlan_strip = !!on;
...
int mlx5_vlan_offload_set(struct rte_eth_dev *dev, int mask) {
    ...
    if (mask & ETH_VLAN_STRIP_MASK) {
        int hw_vlan_strip = !!(dev->data->dev_conf.rxmode.offloads & DEV_RX_OFFLOAD_VLAN_STRIP);
