...
void *ntacc_add_rules(uint8_t port_id, uint16_t type, uint8_t l4_proto, int queue, char *ntpl_str) {
    ...
    struct rte_flow_action_queue rte_flow_action_queue = {
        .index = queue,
    };
...
void ntacc_del_rules(uint8_t port_id, void *rte_flow) { rte_flow_destroy(port_id, (struct rte_flow *)rte_flow, NULL); }
