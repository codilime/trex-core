...
static const char *valid_arguments[] = {ETH_RING_NUMA_NODE_ACTION_ARG, ETH_RING_INTERNAL_ARG, NULL};
...
#define PMD_LOG(level, fmt, args...) rte_log(RTE_LOG_##level, eth_ring_logtype, "%s(): " fmt "\n", __func__, ##args)

static uint16_t eth_ring_rx(void *q, struct rte_mbuf **bufs, uint16_t nb_bufs) {
    void **ptrs = (void *)&bufs[0];
    struct ring_queue *r = q;
    const uint16_t nb_rx = (uint16_t)rte_ring_dequeue_burst(r->rng, ptrs, nb_bufs, NULL);
...
static uint16_t eth_ring_tx(void *q, struct rte_mbuf **bufs, uint16_t nb_bufs) {
    void **ptrs = (void *)&bufs[0];
    struct ring_queue *r = q;
    const uint16_t nb_tx = (uint16_t)rte_ring_enqueue_burst(r->rng, ptrs, nb_bufs, NULL);
...
static void eth_dev_stop(struct rte_eth_dev *dev) { dev->data->dev_link.link_status = ETH_LINK_DOWN; }
...
static int eth_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
    ...
    for (i = 0; i < RTE_ETHDEV_QUEUE_STAT_CNTRS && i < dev->data->nb_rx_queues; i++) {
        stats->q_ipackets[i] = internal->rx_ring_queues[i].rx_pkts.cnt;
        rx_total += stats->q_ipackets[i];
    }

    for (i = 0; i < RTE_ETHDEV_QUEUE_STAT_CNTRS && i < dev->data->nb_tx_queues; i++) {
...
static void eth_mac_addr_remove(struct rte_eth_dev *dev __rte_unused, uint32_t index __rte_unused) {}

static int eth_mac_addr_add(struct rte_eth_dev *dev __rte_unused, struct ether_addr *mac_addr __rte_unused,
                            uint32_t index __rte_unused, uint32_t vmdq __rte_unused) {
    return 0;
}

static void eth_queue_release(void *q __rte_unused) { ; }
static int eth_link_update(struct rte_eth_dev *dev __rte_unused, int wait_to_complete __rte_unused) { return 0; }
...
static int do_eth_dev_ring_create(const char *name, struct rte_ring *const rx_queues[], const unsigned int nb_rx_queues,
    ...
    rx_queues_local = rte_calloc_socket(name, nb_rx_queues, sizeof(void *), 0, numa_node);
    if (rx_queues_local == NULL) {
        rte_errno = ENOMEM;
        goto error;
    }

    tx_queues_local = rte_calloc_socket(name, nb_tx_queues, sizeof(void *), 0, numa_node);
...
int rte_eth_from_rings(const char *name, struct rte_ring *const rx_queues[], const unsigned int nb_rx_queues,
                       struct rte_ring *const tx_queues[], const unsigned int nb_tx_queues,
                       const unsigned int numa_node) {
...
int rte_eth_from_ring(struct rte_ring *r) {
    return rte_eth_from_rings(r->name, &r, 1, &r, 1, r->memzone ? r->memzone->socket_id : SOCKET_ID_ANY);
}

static int eth_dev_ring_create(const char *name, const unsigned int numa_node, enum dev_action action,
    ...
    unsigned int num_rings = RTE_MIN(RTE_PMD_RING_MAX_RX_RINGS, RTE_PMD_RING_MAX_TX_RINGS);
    ...
    for (i = 0; i < num_rings; i++) {
        ...
        rxtx[i] = (action == DEV_CREATE) ? rte_ring_create(rng_name, 1024, numa_node, RING_F_SP_ENQ | RING_F_SC_DEQ)
                                         : rte_ring_lookup(rng_name);
        if (rxtx[i] == NULL)
            return -1;
    }

    if (do_eth_dev_ring_create(name, rxtx, num_rings, rxtx, num_rings, numa_node, action, eth_dev) < 0)
        return -1;
...
static int parse_kvlist(const char *key __rte_unused, const char *value, void *data) {
    ...
    *action = '\0';
    ...
    strlcpy(info->list[info->count].name, name, sizeof(info->list[info->count].name));
...
static int parse_internal_args(const char *key __rte_unused, const char *value, void *data) {
...
static int rte_pmd_ring_probe(struct rte_vdev_device *dev) {
    ...
    if (params == NULL || params[0] == '\0') {
        ret = eth_dev_ring_create(name, rte_socket_id(), DEV_CREATE, &eth_dev);
        if (ret == -1) {
            PMD_LOG(INFO, "Attach to pmd_ring for %s", name);
            ret = eth_dev_ring_create(name, rte_socket_id(), DEV_ATTACH, &eth_dev);
        }
    } else {
        kvlist = rte_kvargs_parse(params, valid_arguments);

        if (!kvlist) {
            PMD_LOG(INFO, "Ignoring unsupported parameters when creatingrings-backed ethernet device");
            ret = eth_dev_ring_create(name, rte_socket_id(), DEV_CREATE, &eth_dev);
            if (ret == -1) {
                PMD_LOG(INFO, "Attach to pmd_ring for %s", name);
                ret = eth_dev_ring_create(name, rte_socket_id(), DEV_ATTACH, &eth_dev);
        ...
        if (rte_kvargs_count(kvlist, ETH_RING_INTERNAL_ARG) == 1) {
            ret = rte_kvargs_process(kvlist, ETH_RING_INTERNAL_ARG, parse_internal_args, &internal_args);
            if (ret < 0)
                goto out_free;

            ret = do_eth_dev_ring_create(name, internal_args->rx_queues, internal_args->nb_rx_queues,
            ...
            info = rte_zmalloc("struct node_action_list",
                               sizeof(struct node_action_list) + (sizeof(struct node_action_pair) * ret), 0);
            if (!info)
                goto out_free;
            ...
            ret = rte_kvargs_process(kvlist, ETH_RING_NUMA_NODE_ACTION_ARG, parse_kvlist, info);

            if (ret < 0)
                goto out_free;

            for (info->count = 0; info->count < info->total; info->count++) {
                ret = eth_dev_ring_create(info->list[info->count].name, info->list[info->count].node,
                                          info->list[info->count].action, &eth_dev);
                if ((ret == -1) && (info->list[info->count].action == DEV_CREATE)) {
                    PMD_LOG(INFO, "Attach to pmd_ring for %s", name);
                    ret = eth_dev_ring_create(name, info->list[info->count].node, DEV_ATTACH, &eth_dev);
                }
            }
        }
    }
...
RTE_PMD_REGISTER_PARAM_STRING(net_ring, ETH_RING_NUMA_NODE_ACTION_ARG "=name:node:action(ATTACH|CREATE)");
