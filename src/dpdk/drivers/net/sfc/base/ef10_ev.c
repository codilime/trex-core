...
static __checkReturn boolean_t ef10_ev_rx(__in efx_evq_t *eep, __in efx_qword_t *eqp,
                                          __in const efx_ev_callbacks_t *eecp, __in_opt void *arg);

static __checkReturn boolean_t ef10_ev_tx(__in efx_evq_t *eep, __in efx_qword_t *eqp,
                                          __in const efx_ev_callbacks_t *eecp, __in_opt void *arg);

static __checkReturn boolean_t ef10_ev_driver(__in efx_evq_t *eep, __in efx_qword_t *eqp,
                                              __in const efx_ev_callbacks_t *eecp, __in_opt void *arg);

static __checkReturn boolean_t ef10_ev_drv_gen(__in efx_evq_t *eep, __in efx_qword_t *eqp,
                                               __in const efx_ev_callbacks_t *eecp, __in_opt void *arg);

static __checkReturn boolean_t ef10_ev_mcdi(__in efx_evq_t *eep, __in efx_qword_t *eqp,
                                            __in const efx_ev_callbacks_t *eecp, __in_opt void *arg);

static __checkReturn efx_rc_t efx_mcdi_set_evq_tmr(__in efx_nic_t *enp, __in uint32_t instance, __in uint32_t mode,
                                                   __in uint32_t timer_ns) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_SET_EVQ_TMR_IN_LEN, MC_CMD_SET_EVQ_TMR_OUT_LEN);
    efx_rc_t rc;
...
static __checkReturn efx_rc_t efx_mcdi_init_evq(__in efx_nic_t *enp, __in unsigned int instance, __in efsys_mem_t *esmp,
                                                __in size_t nevs, __in uint32_t irq, __in uint32_t us,
                                                __in uint32_t flags, __in boolean_t low_latency) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_INIT_EVQ_IN_LEN(EF10_EVQ_MAXNBUFS), MC_CMD_INIT_EVQ_OUT_LEN);
    ...
    interrupting = ((flags & EFX_EVQ_FLAGS_NOTIFY_MASK) == EFX_EVQ_FLAGS_NOTIFY_INTERRUPT);
    ...
    MCDI_IN_POPULATE_DWORD_6(req, INIT_EVQ_IN_FLAGS, INIT_EVQ_IN_FLAG_INTERRUPTING, interrupting,
                             INIT_EVQ_IN_FLAG_RPTR_DOS, 0, INIT_EVQ_IN_FLAG_INT_ARMD, 0, INIT_EVQ_IN_FLAG_CUT_THRU,
                             ev_cut_through, INIT_EVQ_IN_FLAG_RX_MERGE, 1, INIT_EVQ_IN_FLAG_TX_MERGE, 1);

    /* If the value is zero then disable the timer */
    if (us == 0) {
        MCDI_IN_SET_DWORD(req, INIT_EVQ_IN_TMR_MODE, MC_CMD_INIT_EVQ_IN_TMR_MODE_DIS);
        ...
        MCDI_IN_SET_DWORD(req, INIT_EVQ_IN_TMR_MODE, MC_CMD_INIT_EVQ_IN_TMR_INT_HLDOFF);
        MCDI_IN_SET_DWORD(req, INIT_EVQ_IN_TMR_LOAD, ticks);
        MCDI_IN_SET_DWORD(req, INIT_EVQ_IN_TMR_RELOAD, ticks);
    }

    MCDI_IN_SET_DWORD(req, INIT_EVQ_IN_COUNT_MODE, MC_CMD_INIT_EVQ_IN_COUNT_MODE_DIS);
...
static __checkReturn efx_rc_t efx_mcdi_init_evq_v2(__in efx_nic_t *enp, __in unsigned int instance,
                                                   __in efsys_mem_t *esmp, __in size_t nevs, __in uint32_t irq,
                                                   __in uint32_t us, __in uint32_t flags) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_INIT_EVQ_V2_IN_LEN(EF10_EVQ_MAXNBUFS), MC_CMD_INIT_EVQ_V2_OUT_LEN);
    boolean_t interrupting;
    ...
    MCDI_IN_POPULATE_DWORD_4(req, INIT_EVQ_V2_IN_FLAGS, INIT_EVQ_V2_IN_FLAG_INTERRUPTING, interrupting,
                             INIT_EVQ_V2_IN_FLAG_RPTR_DOS, 0, INIT_EVQ_V2_IN_FLAG_INT_ARMD, 0, INIT_EVQ_V2_IN_FLAG_TYPE,
                             evq_type);

    /* If the value is zero then disable the timer */
    if (us == 0) {
        MCDI_IN_SET_DWORD(req, INIT_EVQ_V2_IN_TMR_MODE, MC_CMD_INIT_EVQ_V2_IN_TMR_MODE_DIS);
        ...
        MCDI_IN_SET_DWORD(req, INIT_EVQ_V2_IN_TMR_MODE, MC_CMD_INIT_EVQ_V2_IN_TMR_INT_HLDOFF);
        MCDI_IN_SET_DWORD(req, INIT_EVQ_V2_IN_TMR_LOAD, ticks);
        MCDI_IN_SET_DWORD(req, INIT_EVQ_V2_IN_TMR_RELOAD, ticks);
    }

    MCDI_IN_SET_DWORD(req, INIT_EVQ_V2_IN_COUNT_MODE, MC_CMD_INIT_EVQ_V2_IN_COUNT_MODE_DIS);
    ...
    EFSYS_PROBE1(mcdi_evq_flags, uint32_t, MCDI_OUT_DWORD(req, INIT_EVQ_V2_OUT_FLAGS));
...
static __checkReturn efx_rc_t efx_mcdi_fini_evq(__in efx_nic_t *enp, __in uint32_t instance) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_FINI_EVQ_IN_LEN, MC_CMD_FINI_EVQ_OUT_LEN);
    efx_rc_t rc;
...
__checkReturn efx_rc_t
    ef10_ev_qcreate(__in efx_nic_t *enp, __in unsigned int index, __in efsys_mem_t *esmp, __in size_t ndescs,
                    __in uint32_t id, __in uint32_t us, __in uint32_t flags, __in efx_evq_t *eep) {
    ...
    if ((flags & EFX_EVQ_FLAGS_NOTIFY_MASK) == EFX_EVQ_FLAGS_NOTIFY_INTERRUPT) {
        ...
        irq = index;
        flags = (flags & ~EFX_EVQ_FLAGS_NOTIFY_MASK) | EFX_EVQ_FLAGS_NOTIFY_INTERRUPT;
    } else {
    ...
    if (encp->enc_init_evq_v2_supported) {
         ...
         */
        rc = efx_mcdi_init_evq_v2(enp, index, esmp, ndescs, irq, us, flags);
        if (rc != 0)
            goto fail4;
    } else {
        /*
        ...
        rc = efx_mcdi_init_evq(enp, index, esmp, ndescs, irq, us, flags, low_latency);
        if (rc != 0)
            goto fail5;
    }
...
__checkReturn efx_rc_t ef10_ev_qprime(__in efx_evq_t *eep, __in unsigned int count) {
    efx_nic_t *enp = eep->ee_enp;
    uint32_t rptr;
    efx_dword_t dword;
    ...
    if (enp->en_nic_cfg.enc_bug35388_workaround) {
        ...
        EFX_POPULATE_DWORD_2(dword, ERF_DD_EVQ_IND_RPTR_FLAGS, EFE_DD_EVQ_IND_RPTR_FLAGS_HIGH, ERF_DD_EVQ_IND_RPTR,
                             (rptr >> ERF_DD_EVQ_IND_RPTR_WIDTH));
        EFX_BAR_VI_WRITED(enp, ER_DD_EVQ_INDIRECT, eep->ee_index, &dword, B_FALSE);

        EFX_POPULATE_DWORD_2(dword, ERF_DD_EVQ_IND_RPTR_FLAGS, EFE_DD_EVQ_IND_RPTR_FLAGS_LOW, ERF_DD_EVQ_IND_RPTR,
...
static __checkReturn efx_rc_t efx_mcdi_driver_event(__in efx_nic_t *enp, __in uint32_t evq, __in efx_qword_t data) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_DRIVER_EVENT_IN_LEN, MC_CMD_DRIVER_EVENT_OUT_LEN);
    efx_rc_t rc;
    ...
    MCDI_IN_SET_DWORD(req, DRIVER_EVENT_IN_DATA_LO, EFX_QWORD_FIELD(data, EFX_DWORD_0));
    MCDI_IN_SET_DWORD(req, DRIVER_EVENT_IN_DATA_HI, EFX_QWORD_FIELD(data, EFX_DWORD_1));
...
void ef10_ev_qpost(__in efx_evq_t *eep, __in uint16_t data) {
    ...
    EFX_POPULATE_QWORD_3(event, ESF_DZ_DRV_CODE, ESE_DZ_EV_CODE_DRV_GEN_EV, ESF_DZ_DRV_SUB_CODE, 0,
                         ESF_DZ_DRV_SUB_DATA_DW0, (uint32_t)data);

    (void)efx_mcdi_driver_event(enp, eep->ee_index, event);
}

__checkReturn efx_rc_t ef10_ev_qmoderate(__in efx_evq_t *eep, __in unsigned int us) {
    efx_nic_t *enp = eep->ee_enp;
    efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
    efx_dword_t dword;
    uint32_t mode;
    ...
    EFX_STATIC_ASSERT(FFE_CZ_TIMER_MODE_DIS == MC_CMD_SET_EVQ_TMR_IN_TIMER_MODE_DIS);
    EFX_STATIC_ASSERT(FFE_CZ_TIMER_MODE_IMMED_START == MC_CMD_SET_EVQ_TMR_IN_TIMER_MODE_IMMED_START);
    EFX_STATIC_ASSERT(FFE_CZ_TIMER_MODE_TRIG_START == MC_CMD_SET_EVQ_TMR_IN_TIMER_MODE_TRIG_START);
    EFX_STATIC_ASSERT(FFE_CZ_TIMER_MODE_INT_HLDOFF == MC_CMD_SET_EVQ_TMR_IN_TIMER_MODE_INT_HLDOFF);
    ...
    if (encp->enc_bug61265_workaround) {
        ...
        if (encp->enc_bug35388_workaround) {
            EFX_POPULATE_DWORD_3(dword, ERF_DD_EVQ_IND_TIMER_FLAGS, EFE_DD_EVQ_IND_TIMER_FLAGS,
             ...
             */
            EFX_POPULATE_DWORD_3(dword, ERF_DZ_TC_TIMER_MODE, mode, ERF_DZ_TC_TIMER_VAL, ticks, ERF_FZ_TC_TMR_REL_VAL,
                                 ticks);
...
#if EFSYS_OPT_QSTATS
void ef10_ev_qstats_update(__in efx_evq_t *eep, __inout_ecount(EV_NQSTATS) efsys_stat_t *stat) {
    unsigned int id;
...
static __checkReturn boolean_t ef10_ev_rx_packed_stream(__in efx_evq_t *eep, __in efx_qword_t *eqp,
                                                        __in const efx_ev_callbacks_t *eecp, __in_opt void *arg) {
    uint32_t label;
     ...
     */
    pkt_count = (pkt_count_lbits - eersp->eers_rx_stream_npackets) & EFX_MASK32(ESF_DZ_RX_DSC_PTR_LBITS);
...
static __checkReturn boolean_t ef10_ev_rx(__in efx_evq_t *eep, __in efx_qword_t *eqp,
    ...
    if (enp->en_reset_flags & (EFX_RESET_RXQ_ERR | EFX_RESET_TXQ_ERR | EFX_RESET_HW_UNAVAIL))
        return (B_FALSE);
     ...
     */
    desc_count = (next_read_lbits - eersp->eers_rx_read_ptr) & EFX_MASK32(ESF_DZ_RX_DSC_PTR_LBITS);
    ...
    if ((eth_tag_class == ESE_DZ_ETH_TAG_CLASS_VLAN1) || (eth_tag_class == ESE_DZ_ETH_TAG_CLASS_VLAN2)) {
...
static __checkReturn boolean_t ef10_ev_driver(__in efx_evq_t *eep, __in efx_qword_t *eqp,
    ...
    default:
        EFSYS_PROBE3(bad_event, unsigned int, eep->ee_index, uint32_t, EFX_QWORD_FIELD(*eqp, EFX_DWORD_1), uint32_t,
...
static __checkReturn boolean_t ef10_ev_mcdi(__in efx_evq_t *eep, __in efx_qword_t *eqp,
    ...
    case MCDI_EVENT_CODE_CMDDONE:
        efx_mcdi_ev_cpl(enp, MCDI_EV_FIELD(eqp, CMDDONE_SEQ), MCDI_EV_FIELD(eqp, CMDDONE_DATALEN),
    ...
    case MCDI_EVENT_CODE_SENSOREVT: {
        ...
        if ((rc = mcdi_mon_ev(enp, eqp, &id, &value)) == 0) {
            ...
            should_abort = eecp->eec_exception(arg, EFX_EXCEPTION_UNKNOWN_SENSOREVT, MCDI_EV_FIELD(eqp, DATA));
        } else {
    ...
    case MCDI_EVENT_CODE_FWALERT: {
        ...
        if (reason == MCDI_EVENT_FWALERT_REASON_SRAM_ACCESS)
            should_abort = eecp->eec_exception(arg, EFX_EXCEPTION_FWALERT_SRAM, MCDI_EV_FIELD(eqp, FWALERT_DATA));
        else
            should_abort = eecp->eec_exception(arg, EFX_EXCEPTION_UNKNOWN_FWALERT, MCDI_EV_FIELD(eqp, DATA));
        break;
    }
    ...
    case MCDI_EVENT_CODE_TX_ERR: {
        ...
        EFSYS_PROBE2(tx_descq_err, uint32_t, EFX_QWORD_FIELD(*eqp, EFX_DWORD_1), uint32_t,
                     EFX_QWORD_FIELD(*eqp, EFX_DWORD_0));

        /* Inform the driver that a reset is required. */
        eecp->eec_exception(arg, EFX_EXCEPTION_TX_ERROR, MCDI_EV_FIELD(eqp, TX_ERR_DATA));
        break;
    }
    ...
    case MCDI_EVENT_CODE_RX_ERR: {
        ...
        EFSYS_PROBE2(rx_descq_err, uint32_t, EFX_QWORD_FIELD(*eqp, EFX_DWORD_1), uint32_t,
                     EFX_QWORD_FIELD(*eqp, EFX_DWORD_0));

        /* Inform the driver that a reset is required. */
        eecp->eec_exception(arg, EFX_EXCEPTION_RX_ERROR, MCDI_EV_FIELD(eqp, RX_ERR_DATA));
        break;
    }
...
void ef10_ev_rxlabel_init(__in efx_evq_t *eep, __in efx_rxq_t *erp, __in unsigned int label, __in efx_rxq_type_t type) {
    efx_evq_rxq_state_t *eersp;
#if EFSYS_OPT_RX_PACKED_STREAM || EFSYS_OPT_RX_ES_SUPER_BUFFER
    ...
    if (packed_stream) {
        eersp->eers_rx_packed_stream_credits =
            (eep->ee_mask + 1) /
            EFX_DIV_ROUND_UP(EFX_RX_PACKED_STREAM_MEM_PER_CREDIT, EFX_RX_PACKED_STREAM_MIN_PACKET_SPACE);
         ...
         */
        eersp->eers_rx_packed_stream_credits--;
        EFSYS_ASSERT3U(eersp->eers_rx_packed_stream_credits, <=, EFX_RX_PACKED_STREAM_MAX_CREDITS);
    }
#endif
}
