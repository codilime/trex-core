...
static __checkReturn efx_rc_t efx_asn1_parse_header_match_tag(__inout efx_asn1_cursor_t *cursor, __in uint8_t tag) {
    efx_rc_t rc;
...
static __checkReturn efx_rc_t efx_asn1_enter_tag(__inout efx_asn1_cursor_t *cursor, __in uint8_t tag) {
    efx_rc_t rc;
 ...
 */
static __checkReturn efx_rc_t efx_asn1_match_tag_value(__inout efx_asn1_cursor_t *cursor, __in uint8_t tag,
...
static __checkReturn efx_rc_t efx_asn1_skip_tag(__inout efx_asn1_cursor_t *cursor, __in uint8_t tag) {
    efx_rc_t rc;
...
static __checkReturn efx_rc_t efx_asn1_get_tag_value(__inout efx_asn1_cursor_t *cursor, __in uint8_t tag,
...
static const uint8_t PKCS7_SignedData[] = {0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x07, 0x02};

/* OID 1.2.840.113549.1.7.1 */
static const uint8_t PKCS7_Data[] = {0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x07, 0x01};
 ...
 */
static __checkReturn efx_rc_t efx_check_signed_image_header(__in void *bufferp, __in uint32_t buffer_size,
    ...
    rc = efx_asn1_match_tag_value(&cursor, ASN1_TAG_OBJ_ID, PKCS7_SignedData, sizeof(PKCS7_SignedData));
    if (rc != 0)
        goto fail3;
    ...
    /* SignedData.version */
    rc = efx_asn1_match_tag_value(&cursor, ASN1_TAG_INTEGER, SignedData_Version, sizeof(SignedData_Version));
    if (rc != 0)
        goto fail6;
    ...
    rc = efx_asn1_match_tag_value(&cursor, ASN1_TAG_OBJ_ID, PKCS7_Data, sizeof(PKCS7_Data));
    if (rc != 0)
        goto fail9;
...
static __checkReturn efx_rc_t efx_check_unsigned_image(__in void *bufferp, __in uint32_t buffer_size) {
    ...
    if (buffer_size < (EFX_FIELD_OFFSET(efx_image_header_t, eih_size) + sizeof(header->eih_size))) {
        rc = ENOSPC;
        goto fail1;
    }
    ...
    if (buffer_size < (header->eih_size + header->eih_code_size + EFX_IMAGE_TRAILER_SIZE)) {
        rc = ENOSPC;
        goto fail4;
    }

    /* Check CRC from image buffer matches computed CRC. */
    trailer = (efx_image_trailer_t *)((uint8_t *)header + header->eih_size + header->eih_code_size);

    crc = efx_crc32_calculate(0, (uint8_t *)header, (header->eih_size + header->eih_code_size));
...
__checkReturn efx_rc_t efx_check_reflash_image(__in void *bufferp, __in uint32_t buffer_size,
     ...
     */
    rc = efx_check_signed_image_header(bufferp, buffer_size, &image_offset, &image_size);
    if (rc == 0) {
        /*
...
__checkReturn efx_rc_t efx_build_signed_image_write_buffer(__out_bcount(buffer_size) uint8_t *bufferp,
    ...
    if ((bufferp == NULL) || (buffer_size == 0) || (infop == NULL) || (headerpp == NULL)) {
        /* Invalid arguments */
        rc = EINVAL;
        goto fail1;
    }
    if ((infop->eii_format != EFX_IMAGE_FORMAT_SIGNED) || (infop->eii_imagep == NULL) || (infop->eii_headerp == NULL) ||
    ...
    cms_header.offset = 0;
    cms_header.size = (uint32_t)((uint8_t *)infop->eii_headerp - infop->eii_imagep);
    if ((cms_header.size > buffer_size) || (cms_header.offset > (buffer_size - cms_header.size))) {
        rc = EINVAL;
        goto fail3;
    }
    ...
    if ((image_header.size > buffer_size) || (image_header.offset > (buffer_size - image_header.size))) {
        rc = EINVAL;
        goto fail4;
    }
    ...
    if ((image_trailer.size > buffer_size) || (image_trailer.offset > (buffer_size - image_trailer.size))) {
        rc = EINVAL;
        goto fail6;
    }
    ...
    if ((signature.size > buffer_size) || (signature.offset > (buffer_size - signature.size))) {
        rc = EINVAL;
        goto fail7;
    }

    EFSYS_ASSERT3U(infop->eii_image_size, ==,
                   cms_header.size + image_header.size + code.size + image_trailer.size + signature.size);
    ...
    if ((chunk_hdr.len > buffer_size) || (chunk_hdr.offset > (buffer_size - chunk_hdr.len))) {
        rc = ENOSPC;
        goto fail9;
    }
    memcpy(bufferp + chunk_hdr.offset, infop->eii_imagep + cms_header.offset, cms_header.size);
    ...
    memcpy(bufferp + chunk_hdr.offset, infop->eii_imagep + image_header.offset, image_header.size);
    ...
    *headerpp = (efx_image_header_t *)(bufferp + chunk_hdr.offset);
    ...
    memcpy(bufferp + chunk_hdr.offset, infop->eii_imagep + code.offset, code.size);
    ...
    memcpy((uint8_t *)bufferp + chunk_hdr.offset, infop->eii_imagep + image_trailer.offset, image_trailer.size);
    ...
    memcpy(bufferp + chunk_hdr.offset, infop->eii_imagep + signature.offset, signature.size);
