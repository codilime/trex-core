...
__checkReturn efx_rc_t ef10_mac_poll(__in efx_nic_t *enp, __out efx_link_mode_t *link_modep) {
...
__checkReturn efx_rc_t ef10_mac_up(__in efx_nic_t *enp, __out boolean_t *mac_upp) {
    ef10_link_state_t els;
    efx_rc_t rc;
...
static __checkReturn efx_rc_t efx_mcdi_vadapter_set_mac(__in efx_nic_t *enp) {
    efx_port_t *epp = &(enp->en_port);
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_VADAPTOR_SET_MAC_IN_LEN, MC_CMD_VADAPTOR_SET_MAC_OUT_LEN);
    efx_rc_t rc;
    ...
    MCDI_IN_SET_DWORD(req, VADAPTOR_SET_MAC_IN_UPSTREAM_PORT_ID, enp->en_vport_id);
    EFX_MAC_ADDR_COPY(MCDI_IN2(req, uint8_t, VADAPTOR_SET_MAC_IN_MACADDR), epp->ep_mac_addr);
...
static __checkReturn efx_rc_t efx_mcdi_mtu_set(__in efx_nic_t *enp, __in uint32_t mtu) {
    efx_mcdi_req_t req;
    ...
    MCDI_IN_POPULATE_DWORD_1(req, SET_MAC_EXT_IN_CONTROL, SET_MAC_EXT_IN_CFG_MTU, 1);
...
static __checkReturn efx_rc_t efx_mcdi_mtu_get(__in efx_nic_t *enp, __out size_t *mtu) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_SET_MAC_EXT_IN_LEN, MC_CMD_SET_MAC_V2_OUT_LEN);
    efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_mac_pdu_get(__in efx_nic_t *enp, __out size_t *pdu) {
    efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_mac_reconfigure(__in efx_nic_t *enp) {
     ...
     */
    MCDI_IN_POPULATE_DWORD_2(req, SET_MAC_IN_REJECT, SET_MAC_IN_REJECT_UNCST, 0, SET_MAC_IN_REJECT_BRDCST, 0);
    ...
    MCDI_IN_POPULATE_DWORD_1(req, SET_MAC_IN_FLAGS, SET_MAC_IN_FLAG_INCLUDE_FCS, 0);
    ...
    rc = efx_filter_reconfigure(enp, epp->ep_mac_addr, epp->ep_all_unicst, epp->ep_mulcst, epp->ep_all_mulcst,
                                epp->ep_brdcst, epp->ep_mulcst_addr_list, epp->ep_mulcst_addr_count);
...
__checkReturn efx_rc_t ef10_mac_filter_default_rxq_set(__in efx_nic_t *enp, __in efx_rxq_t *erp,
...
void ef10_mac_filter_default_rxq_clear(__in efx_nic_t *enp) {
    ...
    (void)efx_filter_reconfigure(enp, epp->ep_mac_addr, epp->ep_all_unicst, epp->ep_mulcst, epp->ep_all_mulcst,
                                 epp->ep_brdcst, epp->ep_mulcst_addr_list, epp->ep_mulcst_addr_count);
}

#if EFSYS_OPT_LOOPBACK

__checkReturn efx_rc_t ef10_mac_loopback_set(__in efx_nic_t *enp, __in efx_link_mode_t link_mode,
...
__checkReturn efx_rc_t ef10_mac_stats_get_mask(__in efx_nic_t *enp, __inout_bcount(mask_size) uint32_t *maskp,
    ...
    if ((rc = efx_mac_stats_mask_add_ranges(maskp, mask_size, ef10_common, EFX_ARRAY_SIZE(ef10_common))) != 0)
        goto fail1;
    ...
    if (epp->ep_phy_cap_mask & (1 << MC_CMD_PHY_CAP_40000FDX_LBN)) {
        ...
        if ((rc = efx_mac_stats_mask_add_ranges(maskp, mask_size, ef10_40g_extra, EFX_ARRAY_SIZE(ef10_40g_extra))) != 0)
            goto fail2;

        if (encp->enc_mac_stats_40g_tx_size_bins) {
            if ((rc = efx_mac_stats_mask_add_ranges(maskp, mask_size, ef10_tx_size_bins,
    ...
    if (encp->enc_pm_and_rxdp_counters) {
        ...
        if ((rc = efx_mac_stats_mask_add_ranges(maskp, mask_size, ef10_pm_and_rxdp,
    ...
    if (encp->enc_datapath_cap_evb) {
        ...
        if ((rc = efx_mac_stats_mask_add_ranges(maskp, mask_size, ef10_vadaptor, EFX_ARRAY_SIZE(ef10_vadaptor))) != 0)
            goto fail6;
    }
    ...
    if (encp->enc_fec_counters) {
        ...
        if ((rc = efx_mac_stats_mask_add_ranges(maskp, mask_size, ef10_fec, EFX_ARRAY_SIZE(ef10_fec))) != 0)
            goto fail7;
    }

    if (encp->enc_mac_stats_nstats >= MC_CMD_MAC_NSTATS_V4) {
        const struct efx_mac_stats_range ef10_rxdp_sdt[] = {
            {EFX_MAC_RXDP_SCATTER_DISABLED_TRUNC, EFX_MAC_RXDP_SCATTER_DISABLED_TRUNC},
        };

        if ((rc = efx_mac_stats_mask_add_ranges(maskp, mask_size, ef10_rxdp_sdt, EFX_ARRAY_SIZE(ef10_rxdp_sdt))) != 0)
            goto fail8;
    }
    ...
    if (encp->enc_hlb_counters) {
        ...
        if ((rc = efx_mac_stats_mask_add_ranges(maskp, mask_size, ef10_hlb, EFX_ARRAY_SIZE(ef10_hlb))) != 0)
            goto fail9;
    }
...
#define EF10_MAC_STAT_READ(_esmp, _field, _eqp) EFSYS_MEM_READQ((_esmp), (_field) * sizeof(efx_qword_t), _eqp)

__checkReturn efx_rc_t ef10_mac_stats_update(__in efx_nic_t *enp, __in efsys_mem_t *esmp,
    ...
    if (EFSYS_MEM_SIZE(esmp) < (encp->enc_mac_stats_nstats * sizeof(efx_qword_t))) {
    ...
    EFSYS_STAT_SET_DWORD(&(stat[EFX_MAC_RX_LANE0_CHAR_ERR]), &(value.eq_dword[0]));
    EFSYS_STAT_SET_DWORD(&(stat[EFX_MAC_RX_LANE1_CHAR_ERR]), &(value.eq_dword[1]));

    EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_RX_LANES23_CHAR_ERR, &value);
    EFSYS_STAT_SET_DWORD(&(stat[EFX_MAC_RX_LANE2_CHAR_ERR]), &(value.eq_dword[0]));
    EFSYS_STAT_SET_DWORD(&(stat[EFX_MAC_RX_LANE3_CHAR_ERR]), &(value.eq_dword[1]));

    EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_RX_LANES01_DISP_ERR, &value);
    EFSYS_STAT_SET_DWORD(&(stat[EFX_MAC_RX_LANE0_DISP_ERR]), &(value.eq_dword[0]));
    EFSYS_STAT_SET_DWORD(&(stat[EFX_MAC_RX_LANE1_DISP_ERR]), &(value.eq_dword[1]));

    EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_RX_LANES23_DISP_ERR, &value);
    EFSYS_STAT_SET_DWORD(&(stat[EFX_MAC_RX_LANE2_DISP_ERR]), &(value.eq_dword[0]));
    EFSYS_STAT_SET_DWORD(&(stat[EFX_MAC_RX_LANE3_DISP_ERR]), &(value.eq_dword[1]));
