...
__checkReturn efx_rc_t efx_mcdi_get_port_assignment(__in efx_nic_t *enp, __out uint32_t *portp) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_PORT_ASSIGNMENT_IN_LEN, MC_CMD_GET_PORT_ASSIGNMENT_OUT_LEN);
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mcdi_get_port_modes(__in efx_nic_t *enp, __out uint32_t *modesp,
                                               __out_opt uint32_t *current_modep, __out_opt uint32_t *default_modep) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_PORT_MODES_IN_LEN, MC_CMD_GET_PORT_MODES_OUT_LEN);
    efx_rc_t rc;
    ...
    if ((current_modep != NULL) && (req.emr_out_length_used < MC_CMD_GET_PORT_MODES_OUT_CURRENT_MODE_OFST + 4)) {
        rc = EMSGSIZE;
        goto fail3;
    }
...
__checkReturn efx_rc_t ef10_nic_get_port_mode_bandwidth(__in efx_nic_t *enp, __out uint32_t *bandwidth_mbpsp) {
    uint32_t port_modes;
    ...
    if ((rc = efx_mcdi_get_port_modes(enp, &port_modes, &current_mode, NULL)) != 0) {
...
static __checkReturn efx_rc_t efx_mcdi_vadaptor_alloc(__in efx_nic_t *enp, __in uint32_t port_id) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_VADAPTOR_ALLOC_IN_LEN, MC_CMD_VADAPTOR_ALLOC_OUT_LEN);
    efx_rc_t rc;
    ...
    MCDI_IN_POPULATE_DWORD_1(req, VADAPTOR_ALLOC_IN_FLAGS, VADAPTOR_ALLOC_IN_FLAG_PERMIT_SET_MAC_WHEN_FILTERS_INSTALLED,
...
static __checkReturn efx_rc_t efx_mcdi_vadaptor_free(__in efx_nic_t *enp, __in uint32_t port_id) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_VADAPTOR_FREE_IN_LEN, MC_CMD_VADAPTOR_FREE_OUT_LEN);
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mcdi_get_mac_address_pf(__in efx_nic_t *enp, __out_ecount_opt(6) uint8_t mac_addrp[6]) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_MAC_ADDRESSES_IN_LEN, MC_CMD_GET_MAC_ADDRESSES_OUT_LEN);
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mcdi_get_mac_address_vf(__in efx_nic_t *enp, __out_ecount_opt(6) uint8_t mac_addrp[6]) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_VPORT_GET_MAC_ADDRESSES_IN_LEN, MC_CMD_VPORT_GET_MAC_ADDRESSES_OUT_LENMAX);
    efx_rc_t rc;
    ...
    MCDI_IN_SET_DWORD(req, VPORT_GET_MAC_ADDRESSES_IN_VPORT_ID, EVB_PORT_ID_ASSIGNED);
...
__checkReturn efx_rc_t efx_mcdi_get_clock(__in efx_nic_t *enp, __out uint32_t *sys_freqp, __out uint32_t *dpcpu_freqp) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_CLOCK_IN_LEN, MC_CMD_GET_CLOCK_OUT_LEN);
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mcdi_get_rxdp_config(__in efx_nic_t *enp, __out uint32_t *end_paddingp) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_RXDP_CONFIG_IN_LEN, MC_CMD_GET_RXDP_CONFIG_OUT_LEN);
    ...
    if (MCDI_OUT_DWORD_FIELD(req, GET_RXDP_CONFIG_OUT_DATA, GET_RXDP_CONFIG_OUT_PAD_HOST_DMA) == 0) {
        /* RX DMA end padding is disabled */
        end_padding = 0;
    } else {
        switch (MCDI_OUT_DWORD_FIELD(req, GET_RXDP_CONFIG_OUT_DATA, GET_RXDP_CONFIG_OUT_PAD_HOST_LEN)) {
...
__checkReturn efx_rc_t efx_mcdi_get_vector_cfg(__in efx_nic_t *enp, __out_opt uint32_t *vec_basep,
                                               __out_opt uint32_t *pf_nvecp, __out_opt uint32_t *vf_nvecp) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_VECTOR_CFG_IN_LEN, MC_CMD_GET_VECTOR_CFG_OUT_LEN);
    efx_rc_t rc;
...
static __checkReturn efx_rc_t efx_mcdi_alloc_vis(__in efx_nic_t *enp, __in uint32_t min_vi_count,
                                                 __in uint32_t max_vi_count, __out uint32_t *vi_basep,
                                                 __out uint32_t *vi_countp, __out uint32_t *vi_shiftp) {
    efx_mcdi_req_t req;
...
static __checkReturn efx_rc_t efx_mcdi_free_vis(__in efx_nic_t *enp) {
    efx_mcdi_req_t req;
    efx_rc_t rc;
...
static __checkReturn efx_rc_t efx_mcdi_alloc_piobuf(__in efx_nic_t *enp, __out efx_piobuf_handle_t *handlep) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_ALLOC_PIOBUF_IN_LEN, MC_CMD_ALLOC_PIOBUF_OUT_LEN);
    efx_rc_t rc;
...
static __checkReturn efx_rc_t efx_mcdi_free_piobuf(__in efx_nic_t *enp, __in efx_piobuf_handle_t handle) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_FREE_PIOBUF_IN_LEN, MC_CMD_FREE_PIOBUF_OUT_LEN);
    efx_rc_t rc;
...
static __checkReturn efx_rc_t efx_mcdi_link_piobuf(__in efx_nic_t *enp, __in uint32_t vi_index,
                                                   __in efx_piobuf_handle_t handle) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_LINK_PIOBUF_IN_LEN, MC_CMD_LINK_PIOBUF_OUT_LEN);
    efx_rc_t rc;
...
static __checkReturn efx_rc_t efx_mcdi_unlink_piobuf(__in efx_nic_t *enp, __in uint32_t vi_index) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_UNLINK_PIOBUF_IN_LEN, MC_CMD_UNLINK_PIOBUF_OUT_LEN);
    efx_rc_t rc;
...
static void ef10_nic_alloc_piobufs(__in efx_nic_t *enp, __in uint32_t max_piobuf_count) {
    efx_piobuf_handle_t *handlep;
    unsigned int i;

    EFSYS_ASSERT3U(max_piobuf_count, <=, EFX_ARRAY_SIZE(enp->en_arch.ef10.ena_piobuf_handle));
...
__checkReturn efx_rc_t ef10_nic_pio_alloc(__inout efx_nic_t *enp, __out uint32_t *bufnump,
    ...
    if ((edcp->edc_pio_alloc_size == 0) || (enp->en_arch.ef10.ena_piobuf_count == 0)) {
        rc = ENOMEM;
        goto fail1;
    }
...
__checkReturn efx_rc_t ef10_nic_pio_free(__inout efx_nic_t *enp, __in uint32_t bufnum, __in uint32_t blknum) {
    uint32_t *map;
    efx_rc_t rc;

    if ((bufnum >= enp->en_arch.ef10.ena_piobuf_count) || (blknum >= (8 * sizeof(*map)))) {
        rc = EINVAL;
        goto fail1;
    }
...
__checkReturn efx_rc_t ef10_nic_pio_link(__inout efx_nic_t *enp, __in uint32_t vi_index,
                                         __in efx_piobuf_handle_t handle) {
    return (efx_mcdi_link_piobuf(enp, vi_index, handle));
}

__checkReturn efx_rc_t ef10_nic_pio_unlink(__inout efx_nic_t *enp, __in uint32_t vi_index) {
    return (efx_mcdi_unlink_piobuf(enp, vi_index));
}

static __checkReturn efx_rc_t ef10_mcdi_get_pf_count(__in efx_nic_t *enp, __out uint32_t *pf_countp) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_PF_COUNT_IN_LEN, MC_CMD_GET_PF_COUNT_OUT_LEN);
    efx_rc_t rc;
...
static __checkReturn efx_rc_t ef10_get_datapath_caps(__in efx_nic_t *enp) {
    efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_CAPABILITIES_IN_LEN, MC_CMD_GET_CAPABILITIES_V5_OUT_LEN);
    efx_rc_t rc;
...
#define CAP_FLAGS1(_req, _flag)                                                                                        \
    (MCDI_OUT_DWORD((_req), GET_CAPABILITIES_OUT_FLAGS1) & (1u << (MC_CMD_GET_CAPABILITIES_V2_OUT_##_flag##_LBN)))
...
#define CAP_FLAGS2(_req, _flag)                                                                                        \
    ...
    if (CAP_FLAGS2(req, TX_TSO_V2)) {
        ...
        encp->enc_fw_assisted_tso_v2_n_contexts = MCDI_OUT_WORD(req, GET_CAPABILITIES_V2_OUT_TX_TSO_V2_N_CONTEXTS);
    } else {
    ...
    if (CAP_FLAGS1(req, VXLAN_NVGRE)) {
        encp->enc_tunnel_encapsulations_supported =
            (1u << EFX_TUNNEL_PROTOCOL_VXLAN) | (1u << EFX_TUNNEL_PROTOCOL_GENEVE) | (1u << EFX_TUNNEL_PROTOCOL_NVGRE);

        EFX_STATIC_ASSERT(EFX_TUNNEL_MAXNENTRIES == MC_CMD_SET_TUNNEL_ENCAP_UDP_PORTS_IN_ENTRIES_MAXNUM);
    ...
    } else if ((enp->en_family == EFX_FAMILY_HUNTINGTON) || (enp->en_family == EFX_FAMILY_MEDFORD)) {
    ...
    if (req.emr_out_length_used >= MC_CMD_GET_CAPABILITIES_V4_OUT_LEN) {
        ...
        encp->enc_mac_stats_nstats = MCDI_OUT_WORD(req, GET_CAPABILITIES_V4_OUT_MAC_STATS_NUM_STATS);
    } else {
    ...
    if (req.emr_out_length_used >= MC_CMD_GET_CAPABILITIES_V5_OUT_LEN)
        encp->enc_filter_action_mark_max = MCDI_OUT_DWORD(req, GET_CAPABILITIES_V5_OUT_FILTER_ACTION_MARK_MAX);
    else
...
#define EF10_LEGACY_PF_PRIVILEGE_MASK                                                                                  \
    (MC_CMD_PRIVILEGE_MASK_IN_GRP_ADMIN | MC_CMD_PRIVILEGE_MASK_IN_GRP_LINK | MC_CMD_PRIVILEGE_MASK_IN_GRP_ONLOAD |    \
     MC_CMD_PRIVILEGE_MASK_IN_GRP_PTP | MC_CMD_PRIVILEGE_MASK_IN_GRP_INSECURE_FILTERS |                                \
     MC_CMD_PRIVILEGE_MASK_IN_GRP_MAC_SPOOFING | MC_CMD_PRIVILEGE_MASK_IN_GRP_UNICAST |                                \
     MC_CMD_PRIVILEGE_MASK_IN_GRP_MULTICAST | MC_CMD_PRIVILEGE_MASK_IN_GRP_BROADCAST |                                 \
     MC_CMD_PRIVILEGE_MASK_IN_GRP_ALL_MULTICAST | MC_CMD_PRIVILEGE_MASK_IN_GRP_PROMISCUOUS)

#define EF10_LEGACY_VF_PRIVILEGE_MASK 0

__checkReturn efx_rc_t ef10_get_privilege_mask(__in efx_nic_t *enp, __out uint32_t *maskp) {
    efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
    uint32_t mask;
    efx_rc_t rc;

    if ((rc = efx_mcdi_privilege_mask(enp, encp->enc_pf, encp->enc_vf, &mask)) != 0) {
        if (rc != ENOTSUP)
            goto fail1;
...
static __checkReturn efx_rc_t ef10_external_port_mapping(__in efx_nic_t *enp, __in uint32_t port,
...
static __checkReturn efx_rc_t ef10_nic_board_cfg(__in efx_nic_t *enp) {
    ...
    if ((rc = ef10_external_port_mapping(enp, port, &encp->enc_external_port)) != 0)
        goto fail2;
...
__checkReturn efx_rc_t ef10_nic_probe(__in efx_nic_t *enp) {
     ...
     * port.
     */
    edcp->edc_min_vi_count = edcp->edc_max_vi_count = MIN(128, MAX(encp->enc_rxq_limit, encp->enc_txq_limit));
...
__checkReturn efx_rc_t ef10_nic_set_drv_limits(__inout efx_nic_t *enp, __in efx_drv_limits_t *edlp) {
    efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
    efx_drv_cfg_t *edcp = &(enp->en_drv_cfg);
    ...
    edcp->edc_min_vi_count = MAX(min_evq_count, MAX(min_rxq_count, min_txq_count));
    ...
    edcp->edc_max_vi_count = MAX(max_evq_count, MAX(max_rxq_count, max_txq_count));

    /*
     * Check limits for sub-allocated piobuf blocks.
     * PIO is optional, so don't fail if the limits are incorrect.
     */
    if ((encp->enc_piobuf_size == 0) || (encp->enc_piobuf_limit == 0) || (edlp->edl_min_pio_alloc_size == 0) ||
        ...
        blk_size = MAX(edlp->edl_min_pio_alloc_size, encp->enc_piobuf_min_alloc_size);
        ...
        if ((edlp->edl_max_pio_alloc_count > 0) && (edlp->edl_max_pio_alloc_count < blk_count)) {
            blk_count = edlp->edl_max_pio_alloc_count;
        }

        edcp->edc_pio_alloc_size = blk_size;
        edcp->edc_max_piobuf_count = (blk_count + (blks_per_piobuf - 1)) / blks_per_piobuf;
    }
...
__checkReturn efx_rc_t ef10_nic_reset(__in efx_nic_t *enp) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_ENTITY_RESET_IN_LEN, MC_CMD_ENTITY_RESET_OUT_LEN);
    efx_rc_t rc;
    ...
    MCDI_IN_POPULATE_DWORD_1(req, ENTITY_RESET_IN_FLAG, ENTITY_RESET_IN_FUNCTION_RESOURCE_RESET, 1);
...
__checkReturn efx_rc_t ef10_nic_init(__in efx_nic_t *enp) {
     ...
     */
    vi_count = 0;
    if ((rc = efx_mcdi_alloc_vis(enp, min_vi_count, max_vi_count, &vi_base, &vi_count, &vi_shift)) != 0)
        goto fail3;
    ...
    enp->en_arch.ef10.ena_pio_write_vi_base = vi_count - enp->en_arch.ef10.ena_piobuf_count;

    EFSYS_ASSERT3U(enp->en_nic_cfg.enc_vi_window_shift, !=, EFX_VI_WINDOW_SHIFT_INVALID);
    EFSYS_ASSERT3U(enp->en_nic_cfg.enc_vi_window_shift, <=, EFX_VI_WINDOW_SHIFT_64K);
    ...
    if (enp->en_arch.ef10.ena_piobuf_count > 0) {
        enp->en_arch.ef10.ena_uc_mem_map_size = (vi_window_size * enp->en_arch.ef10.ena_pio_write_vi_base);
    } else {
        enp->en_arch.ef10.ena_uc_mem_map_size = (vi_window_size * enp->en_arch.ef10.ena_vi_count);
    }

    /* Save WC memory mapping details */
    enp->en_arch.ef10.ena_wc_mem_map_offset =
        enp->en_arch.ef10.ena_uc_mem_map_offset + enp->en_arch.ef10.ena_uc_mem_map_size;

    enp->en_arch.ef10.ena_wc_mem_map_size = (vi_window_size * enp->en_arch.ef10.ena_piobuf_count);

    /* Link piobufs to extra VIs in WC mapping */
    if (enp->en_arch.ef10.ena_piobuf_count > 0) {
        for (i = 0; i < enp->en_arch.ef10.ena_piobuf_count; i++) {
            rc = efx_mcdi_link_piobuf(enp, enp->en_arch.ef10.ena_pio_write_vi_base + i,
...
__checkReturn efx_rc_t ef10_nic_get_vi_pool(__in efx_nic_t *enp, __out uint32_t *vi_countp) {
    EFSYS_ASSERT(EFX_FAMILY_IS_EF10(enp));
...
__checkReturn efx_rc_t ef10_nic_get_bar_region(__in efx_nic_t *enp, __in efx_nic_region_t region,
                                               __out uint32_t *offsetp, __out size_t *sizep) {
    efx_rc_t rc;
...
void ef10_nic_fini(__in efx_nic_t *enp) {
    ...
    if (enp->en_arch.ef10.ena_piobuf_count > 0) {
        for (i = 0; i < enp->en_arch.ef10.ena_piobuf_count; i++) {
            rc = efx_mcdi_unlink_piobuf(enp, enp->en_arch.ef10.ena_pio_write_vi_base + i);
            if (rc != 0)
                break;
        }
    }
...
__checkReturn efx_rc_t efx_mcdi_get_nic_global(__in efx_nic_t *enp, __in uint32_t key, __out uint32_t *valuep) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_NIC_GLOBAL_IN_LEN, MC_CMD_GET_NIC_GLOBAL_OUT_LEN);
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mcdi_set_nic_global(__in efx_nic_t *enp, __in uint32_t key, __in uint32_t value) {
    efx_mcdi_req_t req;
