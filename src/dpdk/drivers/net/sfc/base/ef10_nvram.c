...
static void tlv_init_block(__out uint32_t *block) { *block = __CPU_TO_LE_32(TLV_TAG_END); }
 ...
 */
#define TLV_DWORD_COUNT(length) (1 + 1 + (((length) + sizeof(uint32_t) - 1) / sizeof(uint32_t)))
...
static efx_rc_t tlv_init_cursor(__out tlv_cursor_t *cursor, __in uint32_t *block, __in uint32_t *limit,
...
static __checkReturn efx_rc_t tlv_init_cursor_from_size(__out tlv_cursor_t *cursor, __in_bcount(size) uint8_t *block,
                                                        __in size_t size) {
    uint32_t *limit;
...
static __checkReturn efx_rc_t tlv_init_cursor_at_offset(__out tlv_cursor_t *cursor, __in_bcount(size) uint8_t *block,
...
static uint32_t *tlv_last_segment_end(__in tlv_cursor_t *cursor) {
     ...
     */
    for (;;) {
        if (tlv_init_cursor(&segment_cursor, segment_start, cursor->limit, segment_start) != 0)
            break;
...
static uint32_t *tlv_write(__in tlv_cursor_t *cursor, __in uint32_t tag, __in_bcount(size) uint8_t *data,
                           __in size_t size) {
    uint32_t len = size;
    uint32_t *ptr;
...
static __checkReturn efx_rc_t tlv_insert(__inout tlv_cursor_t *cursor, __in uint32_t tag,
    ...
    memmove(cursor->current + delta, cursor->current, (last_segment_end + 1 - cursor->current) * sizeof(uint32_t));
...
static __checkReturn efx_rc_t tlv_delete(__inout tlv_cursor_t *cursor) {
    ...
    memmove(cursor->current, cursor->current + delta, (last_segment_end + 1 - cursor->current) * sizeof(uint32_t));
...
static __checkReturn efx_rc_t tlv_modify(__inout tlv_cursor_t *cursor, __in uint32_t tag,
                                         __in_bcount(size) uint8_t *data, __in size_t size) {
    uint32_t *pos;
...
static __checkReturn efx_rc_t tlv_update_partition_len_and_cks(__in tlv_cursor_t *cursor) {
    efx_rc_t rc;
    ...
    trailer = (struct tlv_partition_trailer *)((uint8_t *)header + new_len - sizeof(*trailer) - sizeof(uint32_t));
    trailer->generation = header->generation;
    trailer->checksum = __CPU_TO_LE_32(__LE_TO_CPU_32(trailer->checksum) - checksum_tlv_partition(&partition));
...
__checkReturn efx_rc_t ef10_nvram_buffer_validate(__in uint32_t partn, __in_bcount(partn_size) caddr_t partn_data,
    ...
    if ((rc = tlv_init_cursor_from_size(&cursor, (uint8_t *)partn_data, partn_size)) != 0) {
        rc = EFAULT;
        goto fail2;
    }
...
void ef10_nvram_buffer_init(__out_bcount(buffer_size) caddr_t bufferp, __in size_t buffer_size) {
...
__checkReturn efx_rc_t ef10_nvram_buffer_create(__in uint32_t partn_type, __out_bcount(partn_size) caddr_t partn_data,
                                                __in size_t partn_size) {
    ...
    unsigned int min_buf_size = sizeof(struct tlv_partition_header) + sizeof(struct tlv_partition_trailer);
    ...
    if ((rc = tlv_init_cursor(&cursor, buf, (uint32_t *)((uint8_t *)buf + partn_size), buf)) != 0) {
        goto fail2;
    }
    ...
    if ((rc = tlv_insert(&cursor, TLV_TAG_PARTITION_HEADER, (uint8_t *)&header.type_id, sizeof(header) - 8)) != 0)
        goto fail3;
    if ((rc = tlv_advance(&cursor)) != 0)
        goto fail4;
    ...
    if ((rc = tlv_insert(&cursor, TLV_TAG_PARTITION_TRAILER, (uint8_t *)&trailer.generation, sizeof(trailer) - 8)) != 0)
        goto fail5;
    ...
    if ((rc = ef10_nvram_buffer_validate(partn_type, partn_data, partn_size)) != 0)
        goto fail7;
...
__checkReturn efx_rc_t ef10_nvram_buffer_find_item_start(__in_bcount(buffer_size) caddr_t bufferp,
                                                         __in size_t buffer_size, __out uint32_t *startp) {
    ...
    if ((rc = tlv_init_cursor_from_size(&cursor, (uint8_t *)bufferp, buffer_size)) != 0) {
        rc = EFAULT;
        goto fail1;
    }
    if (tlv_tag(&cursor) != TLV_TAG_PARTITION_HEADER) {
...
__checkReturn efx_rc_t ef10_nvram_buffer_find_end(__in_bcount(buffer_size) caddr_t bufferp, __in size_t buffer_size,
...
__checkReturn __success(return != B_FALSE) boolean_t
    ef10_nvram_buffer_find_item(__in_bcount(buffer_size) caddr_t bufferp, __in size_t buffer_size, __in uint32_t offset,
    ...
    if (tlv_init_cursor_at_offset(&cursor, (uint8_t *)bufferp, buffer_size, offset) != 0) {
        return (B_FALSE);
    }
...
__checkReturn efx_rc_t ef10_nvram_buffer_peek_item(__in_bcount(buffer_size) caddr_t bufferp, __in size_t buffer_size,
    ...
    if ((rc = tlv_init_cursor_at_offset(&cursor, (uint8_t *)bufferp, buffer_size, offset)) != 0) {
        goto fail1;
    }
    ...
    *tagp = tag;
    if (tag == TLV_TAG_END) {
        ...
        *lengthp = byte_offset(tlv_next_item_ptr(&cursor), cursor.current);
        *value_offsetp = byte_offset((uint32_t *)tlv_value(&cursor), cursor.current);
    }
    return (0);
...
__checkReturn efx_rc_t ef10_nvram_buffer_get_item(__in_bcount(buffer_size) caddr_t bufferp, __in size_t buffer_size,
...
__checkReturn efx_rc_t ef10_nvram_buffer_insert_item(__in_bcount(buffer_size) caddr_t bufferp, __in size_t buffer_size,
                                                     __in uint32_t offset, __in uint32_t tag,
                                                     __in_bcount(length) caddr_t valuep, __in uint32_t length,
...
__checkReturn efx_rc_t ef10_nvram_buffer_modify_item(__in_bcount(buffer_size) caddr_t bufferp, __in size_t buffer_size,
                                                     __in uint32_t offset, __in uint32_t tag,
                                                     __in_bcount(length) caddr_t valuep, __in uint32_t length,
                                                     __out uint32_t *lengthp) {
    efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_nvram_buffer_delete_item(__in_bcount(buffer_size) caddr_t bufferp, __in size_t buffer_size,
...
__checkReturn efx_rc_t ef10_nvram_buffer_finish(__in_bcount(buffer_size) caddr_t bufferp, __in size_t buffer_size) {
    efx_rc_t rc;
    tlv_cursor_t cursor;
 ...
 */
static __checkReturn efx_rc_t ef10_nvram_read_tlv_segment(__in efx_nic_t *enp, __in uint32_t partn,
    ...
    if ((rc = ef10_nvram_partn_read_mode(enp, partn, seg_offset, seg_data, EF10_NVRAM_CHUNK,
                                         MC_CMD_NVRAM_READ_IN_V2_TARGET_CURRENT)) != 0) {
        goto fail2;
    }

    /* A PARTITION_HEADER tag must be the first item at the given offset */
    if ((rc = tlv_init_cursor_from_size(&cursor, (uint8_t *)seg_data, max_seg_size)) != 0) {
        rc = EFAULT;
        goto fail3;
    }
    if (tlv_tag(&cursor) != TLV_TAG_PARTITION_HEADER) {
    ...
    if (total_length > EF10_NVRAM_CHUNK) {
        if ((rc = ef10_nvram_partn_read_mode(enp, partn, seg_offset + EF10_NVRAM_CHUNK, seg_data + EF10_NVRAM_CHUNK,
 ...
 */
__checkReturn efx_rc_t ef10_nvram_buf_read_tlv(__in efx_nic_t *enp, __in_bcount(max_seg_size) caddr_t seg_data,
...
__checkReturn efx_rc_t ef10_nvram_partn_read_tlv(__in efx_nic_t *enp, __in uint32_t partn, __in uint32_t tag,
    ...
    retry = 10;
    do {
        if ((rc = ef10_nvram_read_tlv_segment(enp, partn, 0, seg_data, partn_size)) != 0)
            --retry;
    ...
    if ((rc = ef10_nvram_buf_read_tlv(enp, seg_data, partn_size, tag, &data, &length)) != 0)
        goto fail5;
...
static __checkReturn efx_rc_t ef10_nvram_buf_segment_size(__in caddr_t seg_data, __in size_t max_seg_size,
                                                          __out size_t *seg_sizep) {
    efx_rc_t rc;
    tlv_cursor_t cursor;
    struct tlv_partition_header *header;
 ...
 */
__checkReturn efx_rc_t ef10_nvram_buf_write_tlv(__inout_bcount(max_seg_size) caddr_t seg_data, __in size_t max_seg_size,
 ...
 * configuration.
 */
__checkReturn efx_rc_t ef10_nvram_partn_write_tlv(__in efx_nic_t *enp, __in uint32_t partn, __in uint32_t tag,
                                                  __in_bcount(size) caddr_t data, __in size_t size) {
    return ef10_nvram_partn_write_segment_tlv(enp, partn, tag, data, size, B_FALSE);
}
 ...
 */
static __checkReturn efx_rc_t ef10_nvram_segment_write_tlv(__in efx_nic_t *enp, __in uint32_t partn, __in uint32_t tag,
                                                           __in_bcount(size) caddr_t data, __in size_t size,
                                                           __inout caddr_t *seg_datap, __inout size_t *partn_offsetp,
                                                           __inout size_t *src_remain_lenp,
                                                           __inout size_t *dest_remain_lenp, __in boolean_t write) {
    efx_rc_t rc;
    efx_rc_t status;
     ...
     */
    status = ef10_nvram_read_tlv_segment(enp, partn, *partn_offsetp, *seg_datap, *src_remain_lenp);
    if (status != 0) {
        rc = EINVAL;
        goto fail1;
    }

    status = ef10_nvram_buf_segment_size(*seg_datap, *src_remain_lenp, &original_segment_size);
    if (status != 0) {
        rc = EINVAL;
        goto fail2;
    }
    ...
    if (write) {
        ...
        if ((rc = ef10_nvram_buf_write_tlv(*seg_datap, *dest_remain_lenp, tag, data, size, &modified_segment_size)) !=
            0) {
            goto fail3;
        }
 ...
 * invalidate them.
 */
__checkReturn efx_rc_t ef10_nvram_partn_write_segment_tlv(__in efx_nic_t *enp, __in uint32_t partn, __in uint32_t tag,
                                                          __in_bcount(size) caddr_t data, __in size_t size,
    ...
    do {
        ...
        rc = ef10_nvram_segment_write_tlv(enp, partn, tag, data, size, &segment_data, &current_offset,
    ...
    if ((rc = ef10_nvram_partn_write(enp, partn, 0, partn_data, total_length)) != 0)
        goto fail7;
 ...
 */
__checkReturn efx_rc_t ef10_nvram_partn_size(__in efx_nic_t *enp, __in uint32_t partn, __out size_t *sizep) {
    efx_rc_t rc;

    if ((rc = efx_mcdi_nvram_info(enp, partn, sizep, NULL, NULL, NULL)) != 0)
        goto fail1;
...
__checkReturn efx_rc_t ef10_nvram_partn_lock(__in efx_nic_t *enp, __in uint32_t partn) {
    efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_nvram_partn_read_mode(__in efx_nic_t *enp, __in uint32_t partn, __in unsigned int offset,
    ...
    while (size > 0) {
        ...
        if ((rc = efx_mcdi_nvram_read(enp, partn, offset, data, chunk, mode)) != 0) {
            goto fail1;
        }
...
__checkReturn efx_rc_t ef10_nvram_partn_read(__in efx_nic_t *enp, __in uint32_t partn, __in unsigned int offset,
                                             __out_bcount(size) caddr_t data, __in size_t size) {
    /*
     ...
     */
    return ef10_nvram_partn_read_mode(enp, partn, offset, data, size, MC_CMD_NVRAM_READ_IN_V2_TARGET_CURRENT);
}

__checkReturn efx_rc_t ef10_nvram_partn_read_backup(__in efx_nic_t *enp, __in uint32_t partn, __in unsigned int offset,
                                                    __out_bcount(size) caddr_t data, __in size_t size) {
    /*
     * An A/B partition has two data stores (current and backup).
     ...
     */
    return ef10_nvram_partn_read_mode(enp, partn, offset, data, size, MC_CMD_NVRAM_READ_IN_V2_TARGET_BACKUP);
}

__checkReturn efx_rc_t ef10_nvram_partn_erase(__in efx_nic_t *enp, __in uint32_t partn, __in unsigned int offset,
                                              __in size_t size) {
    efx_rc_t rc;
    uint32_t erase_size;

    if ((rc = efx_mcdi_nvram_info(enp, partn, NULL, NULL, &erase_size, NULL)) != 0)
        goto fail1;
...
__checkReturn efx_rc_t ef10_nvram_partn_write(__in efx_nic_t *enp, __in uint32_t partn, __in unsigned int offset,
                                              __in_bcount(size) caddr_t data, __in size_t size) {
    size_t chunk;
    uint32_t write_size;
    efx_rc_t rc;

    if ((rc = efx_mcdi_nvram_info(enp, partn, NULL, NULL, NULL, &write_size)) != 0)
        goto fail1;
...
__checkReturn efx_rc_t ef10_nvram_partn_unlock(__in efx_nic_t *enp, __in uint32_t partn,
...
__checkReturn efx_rc_t ef10_nvram_partn_set_version(__in efx_nic_t *enp, __in uint32_t partn,
    ...
    if ((rc = ef10_nvram_partn_write_segment_tlv(enp, NVRAM_PARTITION_TYPE_DYNAMIC_CONFIG,
...
static ef10_parttbl_entry_t medford_parttbl[] = {
    /*		partn			ports	nvtype */
    PARTN_MAP_ENTRY(MC_FIRMWARE, ALL, MC_FIRMWARE),    PARTN_MAP_ENTRY(MC_FIRMWARE_BACKUP, ALL, MC_GOLDEN),
    PARTN_MAP_ENTRY(EXPANSION_ROM, ALL, BOOTROM),      PARTN_MAP_ENTRY(EXPROM_CONFIG, ALL, BOOTROM_CFG),
    PARTN_MAP_ENTRY(DYNAMIC_CONFIG, ALL, DYNAMIC_CFG), PARTN_MAP_ENTRY(FPGA, ALL, FPGA),
    PARTN_MAP_ENTRY(FPGA_BACKUP, ALL, FPGA_BACKUP),    PARTN_MAP_ENTRY(LICENSE, ALL, LICENSE),
    PARTN_MAP_ENTRY(EXPANSION_UEFI, ALL, UEFIROM),     PARTN_MAP_ENTRY(MUM_FIRMWARE, ALL, MUM_FIRMWARE),
};
...
static __checkReturn efx_rc_t ef10_parttbl_get(__in efx_nic_t *enp, __out ef10_parttbl_entry_t **parttblp,
...
__checkReturn efx_rc_t ef10_nvram_type_to_partn(__in efx_nic_t *enp, __in efx_nvram_type_t type,
    ...
    if (ef10_parttbl_get(enp, &parttbl, &parttbl_rows) == 0) {
        for (i = 0; i < parttbl_rows; i++) {
            ...
            if ((entry->nvtype == type) && (entry->port_mask & (1u << emip->emi_port))) {
                *partnp = entry->partn;
                return (0);
            }
        }
...
static __checkReturn efx_rc_t ef10_nvram_partn_to_type(__in efx_nic_t *enp, __in uint32_t partn,
    ...
    if (ef10_parttbl_get(enp, &parttbl, &parttbl_rows) == 0) {
        for (i = 0; i < parttbl_rows; i++) {
            ...
            if ((entry->partn == partn) && (entry->port_mask & (1u << emip->emi_port))) {
                *typep = entry->nvtype;
                return (0);
            }
        }
...
__checkReturn efx_rc_t ef10_nvram_test(__in efx_nic_t *enp) {
    ...
    if ((rc = efx_mcdi_nvram_partitions(enp, (caddr_t)partns, size, &npartns)) != 0) {
        goto fail2;
    }
...
__checkReturn efx_rc_t ef10_nvram_partn_get_version(__in efx_nic_t *enp, __in uint32_t partn, __out uint32_t *subtypep,
    ...
    if ((rc = efx_mcdi_nvram_metadata(enp, partn, subtypep, version, NULL, 0)) != 0)
        goto fail1;
...
__checkReturn efx_rc_t ef10_nvram_partn_rw_start(__in efx_nic_t *enp, __in uint32_t partn, __out size_t *chunk_sizep) {
...
__checkReturn efx_rc_t ef10_nvram_partn_rw_finish(__in efx_nic_t *enp, __in uint32_t partn,
                                                  __out_opt uint32_t *verify_resultp) {
    efx_rc_t rc;
