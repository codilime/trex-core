...
#define CHECK_CAP(_cap) EFX_STATIC_ASSERT(EFX_PHY_CAP_##_cap == MC_CMD_PHY_CAP_##_cap##_LBN)
...
static void mcdi_phy_decode_link_mode(__in efx_nic_t *enp, __in uint32_t link_flags, __in unsigned int speed,
                                      __in unsigned int fcntl, __in uint32_t fec, __out efx_link_mode_t *link_modep,
                                      __out unsigned int *fcntlp, __out efx_phy_fec_type_t *fecp) {
...
void ef10_phy_link_ev(__in efx_nic_t *enp, __in efx_qword_t *eqp, __out efx_link_mode_t *link_modep) {
    ...
    mcdi_phy_decode_link_mode(enp, link_flags, speed, MCDI_EV_FIELD(eqp, LINKCHANGE_FCNTL), MC_CMD_FEC_NONE, &link_mode,
                              &fcntl, &fec);
...
__checkReturn efx_rc_t ef10_phy_power(__in efx_nic_t *enp, __in boolean_t power) {
    efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_phy_get_link(__in efx_nic_t *enp, __out ef10_link_state_t *elsp) {
    efx_mcdi_req_t req;
    uint32_t fec;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_LINK_IN_LEN, MC_CMD_GET_LINK_OUT_V2_LEN);
    efx_rc_t rc;
    ...
    mcdi_phy_decode_cap(MCDI_OUT_DWORD(req, GET_LINK_OUT_CAP), &elsp->epls.epls_adv_cap_mask);
    mcdi_phy_decode_cap(MCDI_OUT_DWORD(req, GET_LINK_OUT_LP_CAP), &elsp->epls.epls_lp_cap_mask);
    ...
    mcdi_phy_decode_link_mode(enp, MCDI_OUT_DWORD(req, GET_LINK_OUT_FLAGS),
                              ...
                              fec, &elsp->epls.epls_link_mode, &elsp->epls.epls_fcntl, &elsp->epls.epls_fec);

    if (req.emr_out_length_used < MC_CMD_GET_LINK_OUT_V2_LEN) {
        elsp->epls.epls_ld_cap_mask = 0;
    } else {
        mcdi_phy_decode_cap(MCDI_OUT_DWORD(req, GET_LINK_OUT_V2_LD_CAP), &elsp->epls.epls_ld_cap_mask);
    }
...
__checkReturn efx_rc_t ef10_phy_reconfigure(__in efx_nic_t *enp) {
    efx_port_t *epp = &(enp->en_port);
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_SET_LINK_IN_LEN, MC_CMD_SET_LINK_OUT_LEN);
    uint32_t cap_mask;
    ...
    MCDI_IN_SET_DWORD_FIELD(req, SET_LINK_IN_CAP, PHY_CAP_25000FDX, (cap_mask >> EFX_PHY_CAP_25000FDX) & 0x1);
    MCDI_IN_SET_DWORD_FIELD(req, SET_LINK_IN_CAP, PHY_CAP_40000FDX, (cap_mask >> EFX_PHY_CAP_40000FDX) & 0x1);
    MCDI_IN_SET_DWORD_FIELD(req, SET_LINK_IN_CAP, PHY_CAP_50000FDX, (cap_mask >> EFX_PHY_CAP_50000FDX) & 0x1);
    MCDI_IN_SET_DWORD_FIELD(req, SET_LINK_IN_CAP, PHY_CAP_100000FDX, (cap_mask >> EFX_PHY_CAP_100000FDX) & 0x1);

    MCDI_IN_SET_DWORD_FIELD(req, SET_LINK_IN_CAP, PHY_CAP_BASER_FEC, (cap_mask >> EFX_PHY_CAP_BASER_FEC) & 0x1);
    MCDI_IN_SET_DWORD_FIELD(req, SET_LINK_IN_CAP, PHY_CAP_BASER_FEC_REQUESTED,
                            (cap_mask >> EFX_PHY_CAP_BASER_FEC_REQUESTED) & 0x1);

    MCDI_IN_SET_DWORD_FIELD(req, SET_LINK_IN_CAP, PHY_CAP_RS_FEC, (cap_mask >> EFX_PHY_CAP_RS_FEC) & 0x1);
    MCDI_IN_SET_DWORD_FIELD(req, SET_LINK_IN_CAP, PHY_CAP_RS_FEC_REQUESTED,
                            (cap_mask >> EFX_PHY_CAP_RS_FEC_REQUESTED) & 0x1);

    MCDI_IN_SET_DWORD_FIELD(req, SET_LINK_IN_CAP, PHY_CAP_25G_BASER_FEC, (cap_mask >> EFX_PHY_CAP_25G_BASER_FEC) & 0x1);
...
__checkReturn efx_rc_t ef10_phy_oui_get(__in efx_nic_t *enp, __out uint32_t *ouip) {
    _NOTE(ARGUNUSED(enp, ouip))

    return (ENOTSUP);
}

__checkReturn efx_rc_t ef10_phy_link_state_get(__in efx_nic_t *enp, __out efx_phy_link_state_t *eplsp) {
    efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_phy_stats_update(__in efx_nic_t *enp, __in efsys_mem_t *esmp,
...
__checkReturn efx_rc_t ef10_bist_start(__in efx_nic_t *enp, __in efx_bist_type_t type) {
    efx_rc_t rc;
...
__checkReturn efx_rc_t ef10_bist_poll(__in efx_nic_t *enp, __in efx_bist_type_t type, __out efx_bist_result_t *resultp,
                                      __out_opt __drv_when(count > 0, __notnull) uint32_t *value_maskp,
                                      __out_ecount_opt(count) __drv_when(count > 0, __notnull) unsigned long *valuesp,
                                      __in size_t count) {
    /*
     * MCDI_CTL_SDU_LEN_MAX_V1 is large enough cover all BIST results,
     * whilst not wasting stack.
     */
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_POLL_BIST_IN_LEN, MCDI_CTL_SDU_LEN_MAX_V1);
    ...
    EFX_STATIC_ASSERT(MC_CMD_POLL_BIST_OUT_SFT9001_LEN <= MCDI_CTL_SDU_LEN_MAX_V1);
    ...
    if (result == MC_CMD_POLL_BIST_FAILED && req.emr_out_length >= MC_CMD_POLL_BIST_OUT_MEM_LEN &&
        ...
        if (valuesp != NULL) {
            ...
            valuesp[EFX_BIST_MEM_EXPECT] = MCDI_OUT_DWORD(req, POLL_BIST_OUT_MEM_EXPECT);
            valuesp[EFX_BIST_MEM_ACTUAL] = MCDI_OUT_DWORD(req, POLL_BIST_OUT_MEM_ACTUAL);
            valuesp[EFX_BIST_MEM_ECC] = MCDI_OUT_DWORD(req, POLL_BIST_OUT_MEM_ECC);
            valuesp[EFX_BIST_MEM_ECC_PARITY] = MCDI_OUT_DWORD(req, POLL_BIST_OUT_MEM_ECC_PARITY);
            valuesp[EFX_BIST_MEM_ECC_FATAL] = MCDI_OUT_DWORD(req, POLL_BIST_OUT_MEM_ECC_FATAL);
        }
        value_mask |= (1 << EFX_BIST_MEM_TEST) | (1 << EFX_BIST_MEM_ADDR) | (1 << EFX_BIST_MEM_BUS) |
                      (1 << EFX_BIST_MEM_EXPECT) | (1 << EFX_BIST_MEM_ACTUAL) | (1 << EFX_BIST_MEM_ECC) |
                      (1 << EFX_BIST_MEM_ECC_PARITY) | (1 << EFX_BIST_MEM_ECC_FATAL);
    } else if (result == MC_CMD_POLL_BIST_FAILED && encp->enc_phy_type == EFX_PHY_XFI_FARMI &&
