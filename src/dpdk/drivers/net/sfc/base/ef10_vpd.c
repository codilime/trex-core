...
__checkReturn efx_rc_t ef10_vpd_init(__in efx_nic_t *enp) {
    ...
    svpd = NULL;
    svpd_size = 0;
    rc = ef10_nvram_partn_read_tlv(enp, NVRAM_PARTITION_TYPE_STATIC_CONFIG, tag, &svpd, &svpd_size);
    if (rc != 0) {
        if (rc == EACCES) {
...
__checkReturn efx_rc_t ef10_vpd_size(__in efx_nic_t *enp, __out size_t *sizep) {
     ...
     */
    if ((rc = efx_mcdi_nvram_info(enp, NVRAM_PARTITION_TYPE_DYNAMIC_CONFIG, sizep, NULL, NULL, NULL)) != 0)
        goto fail1;
...
__checkReturn efx_rc_t ef10_vpd_read(__in efx_nic_t *enp, __out_bcount(size) caddr_t data, __in size_t size) {
    caddr_t dvpd;
    size_t dvpd_size;
    uint32_t pci_pf;
    uint32_t tag;
    ...
    if ((rc = ef10_nvram_partn_read_tlv(enp, NVRAM_PARTITION_TYPE_DYNAMIC_CONFIG, tag, &dvpd, &dvpd_size)) != 0)
        goto fail1;
...
__checkReturn efx_rc_t ef10_vpd_verify(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size) {
    efx_vpd_tag_t stag;
    efx_vpd_tag_t dtag;
    ...
    while (1) {
        ...
        _NOTE(CONSTANTCONDITION)
        while (1) {
            if ((rc = efx_vpd_hunk_next(enp->en_arch.ef10.ena_svpd, enp->en_arch.ef10.ena_svpd_length, &stag, &skey,
                                        NULL, NULL, &scont)) != 0)
                goto fail3;
            if (scont == 0)
                break;
...
__checkReturn efx_rc_t ef10_vpd_reinit(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size) {
    boolean_t wantpid;
    efx_rc_t rc;
    ...
    if (enp->en_arch.ef10.ena_svpd_length == 0)
        ...
        rc = efx_vpd_hunk_get(enp->en_arch.ef10.ena_svpd, enp->en_arch.ef10.ena_svpd_length, EFX_VPD_ID, 0, &offset,
                              &length);
        if (rc == 0)
            wantpid = B_FALSE;
        else if (rc == ENOENT)
...
__checkReturn efx_rc_t ef10_vpd_get(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size,
    ...
    if (enp->en_arch.ef10.ena_svpd_length > 0) {
        if ((rc = efx_vpd_hunk_get(enp->en_arch.ef10.ena_svpd, enp->en_arch.ef10.ena_svpd_length, evvp->evv_tag,
...
__checkReturn efx_rc_t ef10_vpd_set(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size,
...
__checkReturn efx_rc_t ef10_vpd_next(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size,
...
__checkReturn efx_rc_t ef10_vpd_write(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size) {
    size_t vpd_length;
    uint32_t pci_pf;
    uint32_t tag;
    efx_rc_t rc;
    ...
    if ((rc = ef10_nvram_partn_write_segment_tlv(enp, NVRAM_PARTITION_TYPE_DYNAMIC_CONFIG, tag, data, vpd_length,
                                                 B_TRUE)) != 0) {
        goto fail2;
    }
...
void ef10_vpd_fini(__in efx_nic_t *enp) {
    ...
    if (enp->en_arch.ef10.ena_svpd_length > 0) {
        EFSYS_KMEM_FREE(enp->en_esip, enp->en_arch.ef10.ena_svpd_length, enp->en_arch.ef10.ena_svpd);
