...
#define DHCP_OPT_HAS_VALUE(opt) (((opt) > EFX_DHCP_PAD) && ((opt) < EFX_DHCP_END))
...
#define DHCP_NEXT_TAG(hdr) ((efx_dhcp_tag_hdr_t *)(((uint8_t *)(hdr)) + DHCP_FULL_TAG_LENGTH((hdr))))

#define DHCP_CALC_TAG_LENGTH(payload_len) ((payload_len) + sizeof(efx_dhcp_tag_hdr_t))

/* Report the layout of bootcfg sectors in NVRAM partition. */
__checkReturn efx_rc_t efx_bootcfg_sector_info(__in efx_nic_t *enp, __in uint32_t pf, __out_opt uint32_t *sector_countp,
...
__checkReturn uint8_t efx_dhcp_csum(__in_bcount(size) uint8_t const *data, __in size_t size) {
    unsigned int pos;
    uint8_t checksum = 0;
...
__checkReturn efx_rc_t efx_dhcp_verify(__in_bcount(size) uint8_t const *data, __in size_t size,
 ...
 * ENOSPC.
 */
static efx_rc_t efx_dhcp_walk_tags(__deref_inout uint8_t **tagpp, __inout size_t *buffer_sizep, __in uint16_t opt) {
    efx_rc_t rc = 0;
 ...
 */
__checkReturn efx_rc_t efx_dhcp_find_tag(__in_bcount(buffer_length) uint8_t *bufferp, __in size_t buffer_length,
                                         __in uint16_t opt, __deref_out uint8_t **valuepp,
 ...
 */
__checkReturn efx_rc_t efx_dhcp_find_end(__in_bcount(buffer_length) uint8_t *bufferp, __in size_t buffer_length,
 ...
 * necessary.
 */
__checkReturn efx_rc_t efx_dhcp_delete_tag(__inout_bcount(buffer_length) uint8_t *bufferp, __in size_t buffer_length,
                                           __in uint16_t opt) {
    efx_rc_t rc;
    efx_dhcp_tag_hdr_t *hdrp;
    size_t len;
 ...
 * necessary.
 */
__checkReturn efx_rc_t efx_dhcp_add_tag(__inout_bcount(buffer_length) uint8_t *bufferp, __in size_t buffer_length,
    ...
    if (DHCP_IS_ENCAP_OPT(opt)) {
        ...
        if (rc == 0) {
            ...
            if (((size_t)encap_hdrp->length + DHCP_CALC_TAG_LENGTH(value_length)) > DHCP_MAX_VALUE) {
                rc = E2BIG;
                goto fail6;
            }
    ...
    added_length = DHCP_CALC_TAG_LENGTH(value_length) + (DHCP_IS_ENCAP_OPT(opt) ? sizeof(efx_dhcp_tag_hdr_t) : 0);
    ...
    memmove(insert_pointp + added_length, insert_pointp, available_space - added_length);

    if (DHCP_IS_ENCAP_OPT(opt)) {
        /* Create new encapsulator header */
        added_length -= sizeof(efx_dhcp_tag_hdr_t);
        efx_dhcp_write_tag(insert_pointp, DHCP_ENCAPSULATOR(opt), NULL, added_length);
 ...
 */
__checkReturn efx_rc_t efx_dhcp_update_tag(__inout_bcount(buffer_length) uint8_t *bufferp, __in size_t buffer_length,
 ...
 */
efx_rc_t efx_bootcfg_copy_sector(__in efx_nic_t *enp, __inout_bcount(sector_length) uint8_t *sector,
...
efx_rc_t efx_bootcfg_read(__in efx_nic_t *enp, __out_bcount(size) uint8_t *data, __in size_t size) {
    ...
    rc = efx_bootcfg_sector_info(enp, sector_number, NULL, &sector_offset, &sector_length);
    if (rc != 0)
        goto fail3;
    ...
    if ((rc = efx_nvram_read_chunk(enp, EFX_NVRAM_BOOTROM_CFG, sector_offset, (caddr_t)payload, sector_length)) != 0) {
...
efx_rc_t efx_bootcfg_write(__in efx_nic_t *enp, __in_bcount(size) uint8_t *data, __in size_t size) {
    uint8_t *partn_data;
    uint8_t checksum;
    size_t partn_length;
    size_t sector_length;
    ...
    rc = efx_nvram_read_chunk(enp, EFX_NVRAM_BOOTROM_CFG, 0, (caddr_t)partn_data, partn_length);
    if (rc != 0)
        goto fail9;
    ...
    if ((rc = efx_nvram_write_chunk(enp, EFX_NVRAM_BOOTROM_CFG, 0, (caddr_t)partn_data, partn_length)) != 0)
        goto fail11;
