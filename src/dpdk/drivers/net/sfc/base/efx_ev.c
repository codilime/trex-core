...
#define EFX_EV_PRESENT(_qword)                                                                                         \
    (EFX_QWORD_FIELD((_qword), EFX_DWORD_0) != 0xffffffff && EFX_QWORD_FIELD((_qword), EFX_DWORD_1) != 0xffffffff)
...
static __checkReturn efx_rc_t siena_ev_qcreate(__in efx_nic_t *enp, __in unsigned int index, __in efsys_mem_t *esmp,
                                               __in size_t ndescs, __in uint32_t id, __in uint32_t us,
                                               __in uint32_t flags, __in efx_evq_t *eep);

static void siena_ev_qdestroy(__in efx_evq_t *eep);

static __checkReturn efx_rc_t siena_ev_qprime(__in efx_evq_t *eep, __in unsigned int count);

static void siena_ev_qpost(__in efx_evq_t *eep, __in uint16_t data);

static __checkReturn efx_rc_t siena_ev_qmoderate(__in efx_evq_t *eep, __in unsigned int us);

#if EFSYS_OPT_QSTATS
static void siena_ev_qstats_update(__in efx_evq_t *eep, __inout_ecount(EV_NQSTATS) efsys_stat_t *stat);
...
__checkReturn size_t efx_evq_size(__in const efx_nic_t *enp, __in unsigned int ndescs) {
    const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);

    return (ndescs * encp->enc_ev_desc_size);
}

__checkReturn unsigned int efx_evq_nbufs(__in const efx_nic_t *enp, __in unsigned int ndescs) {
...
__checkReturn efx_rc_t efx_ev_qcreate(__in efx_nic_t *enp, __in unsigned int index, __in efsys_mem_t *esmp,
    ...
    if (!ISP2(ndescs) || ndescs < encp->enc_evq_min_nevs || ndescs > encp->enc_evq_max_nevs) {
        rc = EINVAL;
        goto fail3;
    }
    ...
    *eepp = eep;
    ...
    if ((rc = eevop->eevo_qcreate(enp, index, esmp, ndescs, id, us, flags, eep)) != 0)
        goto fail5;
...
__checkReturn efx_rc_t efx_ev_qprime(__in efx_evq_t *eep, __in unsigned int count) {
    efx_nic_t *enp = eep->ee_enp;
    const efx_ev_ops_t *eevop = enp->en_eevop;
    efx_rc_t rc;
...
__checkReturn boolean_t efx_ev_qpending(__in efx_evq_t *eep, __in unsigned int count) {
    size_t offset;
    efx_qword_t qword;
...
void efx_ev_qpoll(__in efx_evq_t *eep, __inout unsigned int *countp, __in const efx_ev_callbacks_t *eecp,
                  __in_opt void *arg) {
    ...
    do {
        ...
        for (total = 0; total < batch; ++total) {
            ...
            EFSYS_PROBE3(event, unsigned int, eep->ee_index, uint32_t, EFX_QWORD_FIELD(ev[total], EFX_DWORD_1),
        ...
        for (index = 0; index < total; ++index) {
            ...
            case FSE_AZ_EV_CODE_GLOBAL_EV:
                ...
                /* else fallthrough */
            default:
                EFSYS_PROBE3(bad_event, unsigned int, eep->ee_index, uint32_t, EFX_QWORD_FIELD(ev[index], EFX_DWORD_1),
                             uint32_t, EFX_QWORD_FIELD(ev[index], EFX_DWORD_0));
...
__checkReturn efx_rc_t efx_ev_usecs_to_ticks(__in efx_nic_t *enp, __in unsigned int us, __out unsigned int *ticksp) {
...
__checkReturn efx_rc_t efx_ev_qmoderate(__in efx_evq_t *eep, __in unsigned int us) {
    efx_nic_t *enp = eep->ee_enp;
    const efx_ev_ops_t *eevop = enp->en_eevop;
    efx_rc_t rc;
    ...
    if ((eep->ee_flags & EFX_EVQ_FLAGS_NOTIFY_MASK) == EFX_EVQ_FLAGS_NOTIFY_DISABLED) {
        rc = EINVAL;
        goto fail1;
    }
...
#if EFSYS_OPT_QSTATS
void efx_ev_qstats_update(__in efx_evq_t *eep, __inout_ecount(EV_NQSTATS) efsys_stat_t *stat)
...
static __checkReturn boolean_t siena_ev_rx_not_ok(__in efx_evq_t *eep, __in efx_qword_t *eqp, __in uint32_t label,
                                                  __in uint32_t id, __inout uint16_t *flagsp) {
    ...
    if (EFX_QWORD_FIELD(*eqp, FSF_AZ_RX_EV_FRM_TRUNC) != 0) {
         ...
         */
        if ((EFX_QWORD_FIELD(*eqp, FSF_AZ_RX_EV_SOP) == 0) && (EFX_QWORD_FIELD(*eqp, FSF_AZ_RX_EV_JUMBO_CONT) == 0) &&
...
static __checkReturn boolean_t siena_ev_rx(__in efx_evq_t *eep, __in efx_qword_t *eqp,
    ...
    if (!ok) {
        ...
        if (ignore) {
            EFSYS_PROBE4(rx_complete, uint32_t, label, uint32_t, id, uint32_t, size, uint16_t, flags);
...
static __checkReturn boolean_t siena_ev_tx(__in efx_evq_t *eep, __in efx_qword_t *eqp,
                                           __in const efx_ev_callbacks_t *eecp, __in_opt void *arg) {
    uint32_t id;
    uint32_t label;
    boolean_t should_abort;
    ...
    if (EFX_QWORD_FIELD(*eqp, FSF_AZ_TX_EV_COMP) != 0 && EFX_QWORD_FIELD(*eqp, FSF_AZ_TX_EV_PKT_ERR) == 0 &&
        EFX_QWORD_FIELD(*eqp, FSF_AZ_TX_EV_PKT_TOO_BIG) == 0 && EFX_QWORD_FIELD(*eqp, FSF_AZ_TX_EV_WQ_FF_FULL) == 0) {
...
static __checkReturn boolean_t siena_ev_global(__in efx_evq_t *eep, __in efx_qword_t *eqp,
                                               __in const efx_ev_callbacks_t *eecp, __in_opt void *arg) {
...
static __checkReturn boolean_t siena_ev_driver(__in efx_evq_t *eep, __in efx_qword_t *eqp,
                                               __in const efx_ev_callbacks_t *eecp, __in_opt void *arg) {
    boolean_t should_abort;
...
static __checkReturn boolean_t siena_ev_drv_gen(__in efx_evq_t *eep, __in efx_qword_t *eqp,
                                                __in const efx_ev_callbacks_t *eecp, __in_opt void *arg) {
    uint32_t data;
    boolean_t should_abort;
...
static __checkReturn boolean_t siena_ev_mcdi(__in efx_evq_t *eep, __in efx_qword_t *eqp,
                                             __in const efx_ev_callbacks_t *eecp, __in_opt void *arg) {
    efx_nic_t *enp = eep->ee_enp;
    unsigned int code;
...
static void siena_ev_qpost(__in efx_evq_t *eep, __in uint16_t data) {
    ...
    EFX_POPULATE_QWORD_2(ev, FSF_AZ_EV_CODE, FSE_AZ_EV_CODE_DRV_GEN_EV, FSF_AZ_EV_DATA_DW0, (uint32_t)data);

    EFX_POPULATE_OWORD_3(oword, FRF_AZ_DRV_EV_QID, eep->ee_index, EFX_DWORD_0, EFX_QWORD_FIELD(ev, EFX_DWORD_0),
...
static __checkReturn efx_rc_t siena_ev_qmoderate(__in efx_evq_t *eep, __in unsigned int us) {
    ...
    if (us == 0) {
        EFX_POPULATE_DWORD_2(dword, FRF_CZ_TC_TIMER_MODE, FFE_CZ_TIMER_MODE_DIS, FRF_CZ_TC_TIMER_VAL, 0);
    } else {
        unsigned int ticks;
        ...
        EFX_POPULATE_DWORD_2(dword, FRF_CZ_TC_TIMER_MODE, FFE_CZ_TIMER_MODE_INT_HLDOFF, FRF_CZ_TC_TIMER_VAL, ticks - 1);
    }

    locked = (eep->ee_index == 0) ? 1 : 0;

    EFX_BAR_TBL_WRITED(enp, FR_BZ_TIMER_COMMAND_REGP0, eep->ee_index, &dword, locked);
...
static __checkReturn efx_rc_t siena_ev_qcreate(__in efx_nic_t *enp, __in unsigned int index, __in efsys_mem_t *esmp,
...
#endif
    for (size = 0; (1U << size) <= encp->enc_evq_max_nevs / encp->enc_evq_min_nevs; size++)
    ...
    notify_mode = ((flags & EFX_EVQ_FLAGS_NOTIFY_MASK) != EFX_EVQ_FLAGS_NOTIFY_INTERRUPT);

    /* Set up the new event queue */
    EFX_POPULATE_OWORD_3(oword, FRF_CZ_TIMER_Q_EN, 1, FRF_CZ_HOST_NOTIFY_MODE, notify_mode, FRF_CZ_TIMER_MODE,
                         FFE_CZ_TIMER_MODE_DIS);
    EFX_BAR_TBL_WRITEO(enp, FR_AZ_TIMER_TBL, index, &oword, B_TRUE);

    EFX_POPULATE_OWORD_3(oword, FRF_AZ_EVQ_EN, 1, FRF_AZ_EVQ_SIZE, size, FRF_AZ_EVQ_BUF_BASE_ID, id);
