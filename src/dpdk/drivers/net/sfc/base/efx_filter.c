...
static __checkReturn efx_rc_t siena_filter_add(__in efx_nic_t *enp, __inout efx_filter_spec_t *spec,
                                               __in boolean_t may_replace);

static __checkReturn efx_rc_t siena_filter_delete(__in efx_nic_t *enp, __inout efx_filter_spec_t *spec);

static __checkReturn efx_rc_t siena_filter_supported_filters(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t efx_filter_insert(__in efx_nic_t *enp, __inout efx_filter_spec_t *spec) {
    ...
    if ((spec->efs_flags & EFX_FILTER_FLAG_ACTION_MARK) && !encp->enc_filter_action_mark_supported) {
        rc = ENOTSUP;
        goto fail1;
    }

    if ((spec->efs_flags & EFX_FILTER_FLAG_ACTION_FLAG) && !encp->enc_filter_action_flag_supported) {
        rc = ENOTSUP;
        goto fail2;
    }
...
__checkReturn efx_rc_t efx_filter_remove(__in efx_nic_t *enp, __inout efx_filter_spec_t *spec) {
    const efx_filter_ops_t *efop = enp->en_efop;
 ...
 */
__checkReturn efx_rc_t efx_filter_supported_filters(__in efx_nic_t *enp, __out_ecount(buffer_length) uint32_t *buffer,
                                                    __in size_t buffer_length, __out size_t *list_lengthp) {
    efx_rc_t rc;
    ...
    rc = enp->en_efop->efo_supported_filters(enp, buffer, buffer_length, list_lengthp);
    if (rc != 0)
        goto fail2;
...
__checkReturn efx_rc_t efx_filter_reconfigure(__in efx_nic_t *enp, __in_ecount(6) uint8_t const *mac_addr,
    ...
    if (enp->en_efop->efo_reconfigure != NULL) {
        if ((rc = enp->en_efop->efo_reconfigure(enp, mac_addr, all_unicst, mulcst, all_mulcst, brdcst, addrs, count)) !=
            0)
            goto fail1;
    }
...
void efx_filter_spec_init_rx(__out efx_filter_spec_t *spec, __in efx_filter_priority_t priority,
                             __in efx_filter_flags_t flags, __in efx_rxq_t *erp) {
    EFSYS_ASSERT3P(spec, !=, NULL);
    EFSYS_ASSERT3P(erp, !=, NULL);
    EFSYS_ASSERT((flags & ~(EFX_FILTER_FLAG_RX_RSS | EFX_FILTER_FLAG_RX_SCATTER)) == 0);
...
void efx_filter_spec_init_tx(__out efx_filter_spec_t *spec, __in efx_txq_t *etp) {
    EFSYS_ASSERT3P(spec, !=, NULL);
 ...
 */
__checkReturn efx_rc_t efx_filter_spec_set_ipv4_local(__inout efx_filter_spec_t *spec, __in uint8_t proto,
                                                      __in uint32_t host, __in uint16_t port) {
    EFSYS_ASSERT3P(spec, !=, NULL);

    spec->efs_match_flags |=
        EFX_FILTER_MATCH_ETHER_TYPE | EFX_FILTER_MATCH_IP_PROTO | EFX_FILTER_MATCH_LOC_HOST | EFX_FILTER_MATCH_LOC_PORT;
 ...
 */
__checkReturn efx_rc_t efx_filter_spec_set_ipv4_full(__inout efx_filter_spec_t *spec, __in uint8_t proto,
                                                     __in uint32_t lhost, __in uint16_t lport, __in uint32_t rhost,
                                                     __in uint16_t rport) {
    EFSYS_ASSERT3P(spec, !=, NULL);

    spec->efs_match_flags |= EFX_FILTER_MATCH_ETHER_TYPE | EFX_FILTER_MATCH_IP_PROTO | EFX_FILTER_MATCH_LOC_HOST |
                             EFX_FILTER_MATCH_LOC_PORT | EFX_FILTER_MATCH_REM_HOST | EFX_FILTER_MATCH_REM_PORT;
    spec->efs_ether_type = EFX_ETHER_TYPE_IPV4;
    spec->efs_ip_proto = proto;
 ...
 */
__checkReturn efx_rc_t efx_filter_spec_set_eth_local(__inout efx_filter_spec_t *spec, __in uint16_t vid,
...
void efx_filter_spec_set_ether_type(__inout efx_filter_spec_t *spec, __in uint16_t ether_type) {
    EFSYS_ASSERT3P(spec, !=, NULL);
 ...
 */
__checkReturn efx_rc_t efx_filter_spec_set_uc_def(__inout efx_filter_spec_t *spec) {
    EFSYS_ASSERT3P(spec, !=, NULL);
 ...
 */
__checkReturn efx_rc_t efx_filter_spec_set_mc_def(__inout efx_filter_spec_t *spec) {
    EFSYS_ASSERT3P(spec, !=, NULL);
...
__checkReturn efx_rc_t efx_filter_spec_set_encap_type(__inout efx_filter_spec_t *spec,
 ...
 * specification.
 */
static __checkReturn efx_rc_t efx_filter_spec_set_tunnel(__inout efx_filter_spec_t *spec,
                                                         __in efx_tunnel_protocol_t encap_type,
 ...
 * specification.
 */
__checkReturn efx_rc_t efx_filter_spec_set_vxlan(__inout efx_filter_spec_t *spec, __in const uint8_t *vni,
                                                 __in const uint8_t *inner_addr, __in const uint8_t *outer_addr) {
    return efx_filter_spec_set_tunnel(spec, EFX_TUNNEL_PROTOCOL_VXLAN, vni, inner_addr, outer_addr);
}
 ...
 * specification.
 */
__checkReturn efx_rc_t efx_filter_spec_set_geneve(__inout efx_filter_spec_t *spec, __in const uint8_t *vni,
                                                  __in const uint8_t *inner_addr, __in const uint8_t *outer_addr) {
    return efx_filter_spec_set_tunnel(spec, EFX_TUNNEL_PROTOCOL_GENEVE, vni, inner_addr, outer_addr);
}
 ...
 * specification.
 */
__checkReturn efx_rc_t efx_filter_spec_set_nvgre(__inout efx_filter_spec_t *spec, __in const uint8_t *vsid,
                                                 __in const uint8_t *inner_addr, __in const uint8_t *outer_addr) {
    return efx_filter_spec_set_tunnel(spec, EFX_TUNNEL_PROTOCOL_NVGRE, vsid, inner_addr, outer_addr);
}

#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t efx_filter_spec_set_rss_context(__inout efx_filter_spec_t *spec, __in uint32_t rss_context) {
    efx_rc_t rc;
...
static __checkReturn efx_rc_t siena_filter_spec_from_gen_spec(__out siena_filter_spec_t *sf_spec,
    ...
    if ((gen_spec->efs_flags & EFX_FILTER_FLAG_RX_RSS) && gen_spec->efs_rss_context != EFX_RSS_CONTEXT_DEFAULT) {
        rc = EINVAL;
        goto fail1;
    }
    ...
    case EFX_FILTER_MATCH_ETHER_TYPE | EFX_FILTER_MATCH_IP_PROTO | EFX_FILTER_MATCH_LOC_HOST |
        EFX_FILTER_MATCH_LOC_PORT | EFX_FILTER_MATCH_REM_HOST | EFX_FILTER_MATCH_REM_PORT:
        is_full = B_TRUE;
        /* Fall through */
    case EFX_FILTER_MATCH_ETHER_TYPE | EFX_FILTER_MATCH_IP_PROTO | EFX_FILTER_MATCH_LOC_HOST |
        ...
        if (gen_spec->efs_loc_port == 0 || (is_full && gen_spec->efs_rem_port == 0)) {
            rc = EINVAL;
            goto fail3;
        }
        ...
        case EFX_IPPROTO_TCP:
            if (gen_spec->efs_flags & EFX_FILTER_FLAG_TX) {
                sf_spec->sfs_type = (is_full ? EFX_SIENA_FILTER_TX_TCP_FULL : EFX_SIENA_FILTER_TX_TCP_WILD);
            } else {
        ...
        case EFX_IPPROTO_UDP:
            if (gen_spec->efs_flags & EFX_FILTER_FLAG_TX) {
                sf_spec->sfs_type = (is_full ? EFX_SIENA_FILTER_TX_UDP_FULL : EFX_SIENA_FILTER_TX_UDP_WILD);
            } else {
                sf_spec->sfs_type = (is_full ? EFX_SIENA_FILTER_RX_UDP_FULL : EFX_SIENA_FILTER_RX_UDP_WILD);
            }
            break;
        default:
            rc = ENOTSUP;
    ...
    case EFX_FILTER_MATCH_LOC_MAC:
        if (gen_spec->efs_flags & EFX_FILTER_FLAG_TX) {
            sf_spec->sfs_type = (is_full ? EFX_SIENA_FILTER_TX_MAC_FULL : EFX_SIENA_FILTER_TX_MAC_WILD);
        } else {
            sf_spec->sfs_type = (is_full ? EFX_SIENA_FILTER_RX_MAC_FULL : EFX_SIENA_FILTER_RX_MAC_WILD);
        }
        sf_spec->sfs_dword[0] = is_full ? gen_spec->efs_outer_vid : 0;
        sf_spec->sfs_dword[1] = gen_spec->efs_loc_mac[2] << 24 | gen_spec->efs_loc_mac[3] << 16 |
                                gen_spec->efs_loc_mac[4] << 8 | gen_spec->efs_loc_mac[5];
        sf_spec->sfs_dword[2] = gen_spec->efs_loc_mac[0] << 8 | gen_spec->efs_loc_mac[1];
        break;
 ...
 */
static uint16_t siena_filter_tbl_increment(__in uint32_t key) { return ((uint16_t)(key * 2 - 1)); }

static __checkReturn boolean_t siena_filter_test_used(__in siena_filter_tbl_t *sftp, __in unsigned int index) {
    EFSYS_ASSERT3P(sftp->sft_bitmap, !=, NULL);
    return ((sftp->sft_bitmap[index / 32] & (1 << (index % 32))) != 0);
}

static void siena_filter_set_used(__in siena_filter_tbl_t *sftp, __in unsigned int index) {
    EFSYS_ASSERT3P(sftp->sft_bitmap, !=, NULL);
    sftp->sft_bitmap[index / 32] |= (1 << (index % 32));
    ++sftp->sft_used;
}

static void siena_filter_clear_used(__in siena_filter_tbl_t *sftp, __in unsigned int index) {
    EFSYS_ASSERT3P(sftp->sft_bitmap, !=, NULL);
    sftp->sft_bitmap[index / 32] &= ~(1 << (index % 32));
...
static siena_filter_tbl_id_t siena_filter_tbl_id(__in siena_filter_type_t type) {
...
static void siena_filter_reset_search_depth(__inout siena_filter_t *sfp, __in siena_filter_tbl_id_t tbl_id) {
    switch (tbl_id) {
...
static void siena_filter_push_rx_limits(__in efx_nic_t *enp) {
    ...
    EFX_SET_OWORD_FIELD(oword, FRF_AZ_TCP_FULL_SRCH_LIMIT,
                        sfp->sf_depth[EFX_SIENA_FILTER_RX_TCP_FULL] + FILTER_CTL_SRCH_FUDGE_FULL);
    ...
    EFX_SET_OWORD_FIELD(oword, FRF_AZ_UDP_WILD_SRCH_LIMIT,
                        sfp->sf_depth[EFX_SIENA_FILTER_RX_UDP_WILD] + FILTER_CTL_SRCH_FUDGE_WILD);

    if (sfp->sf_tbl[EFX_SIENA_FILTER_TBL_RX_MAC].sft_size) {
        EFX_SET_OWORD_FIELD(oword, FRF_CZ_ETHERNET_FULL_SEARCH_LIMIT,
                            sfp->sf_depth[EFX_SIENA_FILTER_RX_MAC_FULL] + FILTER_CTL_SRCH_FUDGE_FULL);
        EFX_SET_OWORD_FIELD(oword, FRF_CZ_ETHERNET_WILDCARD_SEARCH_LIMIT,
                            sfp->sf_depth[EFX_SIENA_FILTER_RX_MAC_WILD] + FILTER_CTL_SRCH_FUDGE_WILD);
    }
...
static void siena_filter_push_tx_limits(__in efx_nic_t *enp) {
    ...
    if (sfp->sf_tbl[EFX_SIENA_FILTER_TBL_TX_IP].sft_size != 0) {
        EFX_SET_OWORD_FIELD(oword, FRF_CZ_TX_TCPIP_FILTER_FULL_SEARCH_RANGE,
                            sfp->sf_depth[EFX_SIENA_FILTER_TX_TCP_FULL] + FILTER_CTL_SRCH_FUDGE_FULL);
        EFX_SET_OWORD_FIELD(oword, FRF_CZ_TX_TCPIP_FILTER_WILD_SEARCH_RANGE,
                            sfp->sf_depth[EFX_SIENA_FILTER_TX_TCP_WILD] + FILTER_CTL_SRCH_FUDGE_WILD);
        EFX_SET_OWORD_FIELD(oword, FRF_CZ_TX_UDPIP_FILTER_FULL_SEARCH_RANGE,
                            sfp->sf_depth[EFX_SIENA_FILTER_TX_UDP_FULL] + FILTER_CTL_SRCH_FUDGE_FULL);
        EFX_SET_OWORD_FIELD(oword, FRF_CZ_TX_UDPIP_FILTER_WILD_SEARCH_RANGE,
                            sfp->sf_depth[EFX_SIENA_FILTER_TX_UDP_WILD] + FILTER_CTL_SRCH_FUDGE_WILD);
    }

    if (sfp->sf_tbl[EFX_SIENA_FILTER_TBL_TX_MAC].sft_size != 0) {
        EFX_SET_OWORD_FIELD(oword, FRF_CZ_TX_ETH_FILTER_FULL_SEARCH_RANGE,
                            sfp->sf_depth[EFX_SIENA_FILTER_TX_MAC_FULL] + FILTER_CTL_SRCH_FUDGE_FULL);
        EFX_SET_OWORD_FIELD(oword, FRF_CZ_TX_ETH_FILTER_WILD_SEARCH_RANGE,
                            sfp->sf_depth[EFX_SIENA_FILTER_TX_MAC_WILD] + FILTER_CTL_SRCH_FUDGE_WILD);
    }
...
static __checkReturn uint32_t siena_filter_build(__out efx_oword_t *filter, __in siena_filter_spec_t *spec) {
    uint32_t dword3;
    uint32_t key;
    ...
    case EFX_SIENA_FILTER_TBL_RX_IP: {
        boolean_t is_udp = (type == EFX_SIENA_FILTER_RX_UDP_FULL || type == EFX_SIENA_FILTER_RX_UDP_WILD);
        EFX_POPULATE_OWORD_7(*filter, FRF_BZ_RSS_EN, (flags & EFX_FILTER_FLAG_RX_RSS) ? 1 : 0, FRF_BZ_SCATTER_EN,
                             (flags & EFX_FILTER_FLAG_RX_SCATTER) ? 1 : 0, FRF_AZ_TCP_UDP, is_udp, FRF_AZ_RXQ_ID,
                             spec->sfs_dmaq_id, EFX_DWORD_2, spec->sfs_dword[2], EFX_DWORD_1, spec->sfs_dword[1],
    ...
    case EFX_SIENA_FILTER_TBL_RX_MAC: {
        ...
        EFX_POPULATE_OWORD_7(*filter, FRF_CZ_RMFT_RSS_EN, (flags & EFX_FILTER_FLAG_RX_RSS) ? 1 : 0,
                             FRF_CZ_RMFT_SCATTER_EN, (flags & EFX_FILTER_FLAG_RX_SCATTER) ? 1 : 0, FRF_CZ_RMFT_RXQ_ID,
                             spec->sfs_dmaq_id, FRF_CZ_RMFT_WILDCARD_MATCH, is_wild, FRF_CZ_RMFT_DEST_MAC_DW1,
                             spec->sfs_dword[2], FRF_CZ_RMFT_DEST_MAC_DW0, spec->sfs_dword[1], FRF_CZ_RMFT_VLAN_ID,
                             spec->sfs_dword[0]);
        dword3 = is_wild;
        break;
    }
    ...
    case EFX_SIENA_FILTER_TBL_TX_IP: {
        boolean_t is_udp = (type == EFX_SIENA_FILTER_TX_UDP_FULL || type == EFX_SIENA_FILTER_TX_UDP_WILD);
        EFX_POPULATE_OWORD_5(*filter, FRF_CZ_TIFT_TCP_UDP, is_udp, FRF_CZ_TIFT_TXQ_ID, spec->sfs_dmaq_id, EFX_DWORD_2,
                             spec->sfs_dword[2], EFX_DWORD_1, spec->sfs_dword[1], EFX_DWORD_0, spec->sfs_dword[0]);
    ...
    case EFX_SIENA_FILTER_TBL_TX_MAC: {
        ...
        EFX_POPULATE_OWORD_5(*filter, FRF_CZ_TMFT_TXQ_ID, spec->sfs_dmaq_id, FRF_CZ_TMFT_WILDCARD_MATCH, is_wild,
                             FRF_CZ_TMFT_SRC_MAC_DW1, spec->sfs_dword[2], FRF_CZ_TMFT_SRC_MAC_DW0, spec->sfs_dword[1],
...
static __checkReturn efx_rc_t siena_filter_push_entry(__inout efx_nic_t *enp, __in siena_filter_type_t type,
...
static __checkReturn boolean_t siena_filter_equal(__in const siena_filter_spec_t *left,
    ...
    if ((tbl_id == EFX_SIENA_FILTER_TBL_TX_IP || tbl_id == EFX_SIENA_FILTER_TBL_TX_MAC) &&
...
static __checkReturn efx_rc_t siena_filter_search(__in siena_filter_tbl_t *sftp, __in siena_filter_spec_t *spec,
    ...
    for (;;) {
         ...
         */
        if (siena_filter_test_used(sftp, filter_idx) ? siena_filter_equal(spec, &sftp->sft_spec[filter_idx])
                                                     : for_insert) {
...
static void siena_filter_clear_entry(__in efx_nic_t *enp, __in siena_filter_tbl_t *sftp, __in int index) {
    efx_oword_t filter;
    ...
    if (siena_filter_test_used(sftp, index)) {
        ...
        EFX_ZERO_OWORD(filter);
        siena_filter_push_entry(enp, sftp->sft_spec[index].sfs_type, index, &filter);

        memset(&sftp->sft_spec[index], 0, sizeof(sftp->sft_spec[0]));
    }
}

void siena_filter_tbl_clear(__in efx_nic_t *enp, __in siena_filter_tbl_id_t tbl_id) {
    siena_filter_t *sfp = enp->en_filter.ef_siena_filter;
...
static __checkReturn efx_rc_t siena_filter_init(__in efx_nic_t *enp) {
    ...
    for (tbl_id = 0; tbl_id < EFX_SIENA_FILTER_NTBLS; tbl_id++) {
        ...
        EFSYS_KMEM_ALLOC(enp->en_esip, sftp->sft_size * sizeof(*sftp->sft_spec), sftp->sft_spec);
        if (!sftp->sft_spec) {
            rc = ENOMEM;
            goto fail4;
        }
...
static void siena_filter_fini(__in efx_nic_t *enp) {
    ...
    for (tbl_id = 0; tbl_id < EFX_SIENA_FILTER_NTBLS; tbl_id++) {
        ...
        if (sftp->sft_spec != NULL) {
            EFSYS_KMEM_FREE(enp->en_esip, sftp->sft_size * sizeof(*sftp->sft_spec), sftp->sft_spec);
            sftp->sft_spec = NULL;
        }
    }

    EFSYS_KMEM_FREE(enp->en_esip, sizeof(siena_filter_t), enp->en_filter.ef_siena_filter);
}
...
static __checkReturn efx_rc_t siena_filter_restore(__in efx_nic_t *enp) {
    ...
    for (tbl_id = 0; tbl_id < EFX_SIENA_FILTER_NTBLS; tbl_id++) {
        ...
        for (filter_idx = 0; filter_idx < sftp->sft_size; filter_idx++) {
            ...
            if ((rc = siena_filter_push_entry(enp, spec->sfs_type, filter_idx, &filter)) != 0)
                goto fail2;
        }
    }
...
static __checkReturn efx_rc_t siena_filter_supported_filters(__in efx_nic_t *enp,
    ...
    rx_matches[index++] = EFX_FILTER_MATCH_ETHER_TYPE | EFX_FILTER_MATCH_IP_PROTO | EFX_FILTER_MATCH_LOC_HOST |
                          EFX_FILTER_MATCH_LOC_PORT | EFX_FILTER_MATCH_REM_HOST | EFX_FILTER_MATCH_REM_PORT;
