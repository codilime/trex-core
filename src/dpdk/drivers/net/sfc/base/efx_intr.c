...
static __checkReturn efx_rc_t siena_intr_init(__in efx_nic_t *enp, __in efx_intr_type_t type, __in efsys_mem_t *esmp);
...
static __checkReturn efx_rc_t siena_intr_trigger(__in efx_nic_t *enp, __in unsigned int level);

static void siena_intr_fini(__in efx_nic_t *enp);

static void siena_intr_status_line(__in efx_nic_t *enp, __out boolean_t *fatalp, __out uint32_t *qmaskp);

static void siena_intr_status_message(__in efx_nic_t *enp, __in unsigned int message, __out boolean_t *fatalp);
...
__checkReturn efx_rc_t efx_intr_init(__in efx_nic_t *enp, __in efx_intr_type_t type, __in_opt efsys_mem_t *esmp) {
...
__checkReturn efx_rc_t efx_intr_trigger(__in efx_nic_t *enp, __in unsigned int level) {
    efx_intr_t *eip = &(enp->en_intr);
    const efx_intr_ops_t *eiop = eip->ei_eiop;
...
void efx_intr_status_line(__in efx_nic_t *enp, __out boolean_t *fatalp, __out uint32_t *qmaskp) {
    efx_intr_t *eip = &(enp->en_intr);
    const efx_intr_ops_t *eiop = eip->ei_eiop;
...
void efx_intr_status_message(__in efx_nic_t *enp, __in unsigned int message, __out boolean_t *fatalp) {
    efx_intr_t *eip = &(enp->en_intr);
    const efx_intr_ops_t *eiop = eip->ei_eiop;
...
static __checkReturn efx_rc_t siena_intr_init(__in efx_nic_t *enp, __in efx_intr_type_t type, __in efsys_mem_t *esmp) {
    ...
    EFX_POPULATE_OWORD_3(oword, FRF_AZ_NORM_INT_VEC_DIS_KER, (type == EFX_INTR_MESSAGE) ? 1 : 0, FRF_AZ_INT_ADR_KER_DW0,
                         EFSYS_MEM_ADDR(esmp) & 0xffffffff, FRF_AZ_INT_ADR_KER_DW1, EFSYS_MEM_ADDR(esmp) >> 32);
...
static void siena_intr_fatal(__in efx_nic_t *enp) {
    ...
    if (EFX_OWORD_FIELD(fatal, FRF_AZ_SRM_PERR_INT_KER) != 0 || EFX_OWORD_FIELD(fatal, FRF_AZ_MEM_PERR_INT_KER) != 0)
    ...
    if (EFX_OWORD_FIELD(fatal, FRF_AZ_MEM_PERR_INT_KER) != 0)
        EFSYS_ERR(enp->en_esip, EFX_ERR_MEM_PERR, EFX_OWORD_FIELD(mem_per, EFX_DWORD_0),
    ...
    if (EFX_OWORD_FIELD(fatal, FRF_AZ_SRM_PERR_INT_KER) != 0)
        EFSYS_ERR(enp->en_esip, EFX_ERR_SRAM_PERR, EFX_OWORD_FIELD(mem_per, EFX_DWORD_0),
                  EFX_OWORD_FIELD(mem_per, EFX_DWORD_1));
#else
    EFSYS_ASSERT(0);
#endif
