...
__checkReturn efx_rc_t efx_mac_addr_set(__in efx_nic_t *enp, __in uint8_t *addr) {
    efx_port_t *epp = &(enp->en_port);
    const efx_mac_ops_t *emop = epp->ep_emop;
    uint8_t old_addr[6];
    uint32_t oui;
...
__checkReturn efx_rc_t efx_mac_filter_set(__in efx_nic_t *enp, __in boolean_t all_unicst, __in boolean_t mulcst,
...
__checkReturn efx_rc_t efx_mac_drain(__in efx_nic_t *enp, __in boolean_t enabled) {
    efx_port_t *epp = &(enp->en_port);
    const efx_mac_ops_t *emop = epp->ep_emop;
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mac_up(__in efx_nic_t *enp, __out boolean_t *mac_upp) {
    efx_port_t *epp = &(enp->en_port);
    const efx_mac_ops_t *emop = epp->ep_emop;
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mac_fcntl_set(__in efx_nic_t *enp, __in unsigned int fcntl, __in boolean_t autoneg) {
    efx_port_t *epp = &(enp->en_port);
    const efx_mac_ops_t *emop = epp->ep_emop;
...
void efx_mac_fcntl_get(__in efx_nic_t *enp, __out unsigned int *fcntl_wantedp, __out unsigned int *fcntl_linkp) {
    efx_port_t *epp = &(enp->en_port);
...
__checkReturn efx_rc_t efx_mac_multicast_list_set(__in efx_nic_t *enp, __in_ecount(6 * count) uint8_t const *addrs,
                                                  __in int count) {
    efx_port_t *epp = &(enp->en_port);
    const efx_mac_ops_t *emop = epp->ep_emop;
    ...
    if (old_mulcst_addr_count > 0) {
        ...
        EFSYS_KMEM_ALLOC(enp->en_esip, old_mulcst_addr_count * EFX_MAC_ADDR_LEN, old_mulcst_addr_list);
        ...
        memcpy(old_mulcst_addr_list, epp->ep_mulcst_addr_list, old_mulcst_addr_count * EFX_MAC_ADDR_LEN);
    }
    ...
    if (old_mulcst_addr_count > 0) {
        EFSYS_KMEM_FREE(enp->en_esip, old_mulcst_addr_count * EFX_MAC_ADDR_LEN, old_mulcst_addr_list);
    }
...
fail3:
    ...
    if (old_mulcst_addr_count > 0) {
        memcpy(epp->ep_mulcst_addr_list, old_mulcst_addr_list, old_mulcst_addr_count * EFX_MAC_ADDR_LEN);
...
__checkReturn efx_rc_t efx_mac_filter_default_rxq_set(__in efx_nic_t *enp, __in efx_rxq_t *erp,
...
__checkReturn const char *efx_mac_stat_name(__in efx_nic_t *enp, __in unsigned int id) {
    _NOTE(ARGUNUSED(enp))
    EFSYS_ASSERT3U(enp->en_magic, ==, EFX_NIC_MAGIC);
...
static efx_rc_t efx_mac_stats_mask_add_range(__inout_bcount(mask_size) uint32_t *maskp, __in size_t mask_size,
    ...
    if ((mask_npages * EFX_MAC_STATS_MASK_BITS_PER_PAGE) <= (unsigned int)rngp->last) {
        rc = EINVAL;
        goto fail1;
    }
    ...
    for (el = 0; el < mask_npages; ++el) {
        ...
        width = high - low + 1;
        maskp[el] |= (width == EFX_MAC_STATS_MASK_BITS_PER_PAGE) ? (~0ULL) : (((1ULL << width) - 1) << (low - el_min));
    }
...
efx_rc_t efx_mac_stats_mask_add_ranges(__inout_bcount(mask_size) uint32_t *maskp, __in size_t mask_size,
...
__checkReturn efx_rc_t efx_mac_stats_get_mask(__in efx_nic_t *enp, __out_bcount(mask_size) uint32_t *maskp,
...
__checkReturn efx_rc_t efx_mac_stats_upload(__in efx_nic_t *enp, __in efsys_mem_t *esmp) {
    efx_port_t *epp = &(enp->en_port);
    const efx_mac_ops_t *emop = epp->ep_emop;
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mac_stats_periodic(__in efx_nic_t *enp, __in efsys_mem_t *esmp, __in uint16_t period_ms,
...
__checkReturn efx_rc_t efx_mac_stats_update(__in efx_nic_t *enp, __in efsys_mem_t *esmp,
...
static void siena_mac_multicast_hash_compute(__in_ecount(6 * count) uint8_t const *addrs, __in int count,
                                             __out efx_oword_t *hash_low, __out efx_oword_t *hash_high) {
    uint32_t crc, index;
    int i;
...
static __checkReturn efx_rc_t siena_mac_multicast_list_set(__in efx_nic_t *enp) {
    ...
    siena_mac_multicast_hash_compute(epp->ep_mulcst_addr_list, epp->ep_mulcst_addr_count, &epp->ep_multicst_hash[0],
