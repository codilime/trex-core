...
static const char *const __efx_mon_name[] = {"", "sfx90x0", "sfx91x0", "sfx92x0"};
...
const char *efx_mon_stat_description(__in efx_nic_t *enp, __in efx_mon_stat_t id) {
    _NOTE(ARGUNUSED(enp))
    EFSYS_ASSERT3U(enp->en_magic, ==, EFX_NIC_MAGIC);
...
__checkReturn boolean_t efx_mon_mcdi_to_efx_stat(__in int mcdi_index, __out efx_mon_stat_t *statp) {

    if ((mcdi_index % (MC_CMD_SENSOR_PAGE0_NEXT + 1)) == MC_CMD_SENSOR_PAGE0_NEXT) {
...
__checkReturn boolean_t efx_mon_get_stat_unit(__in efx_mon_stat_t stat, __out efx_mon_stat_unit_t *unitp) {
    switch (stat) {
...
__checkReturn boolean_t efx_mon_get_stat_portmap(__in efx_mon_stat_t stat, __out efx_mon_stat_portmask_t *maskp) {
...
__checkReturn efx_rc_t efx_mon_stats_update(__in efx_nic_t *enp, __in efsys_mem_t *esmp,
...
__checkReturn efx_rc_t efx_mon_limits_update(__in efx_nic_t *enp,
