...
__checkReturn efx_rc_t efx_family(__in uint16_t venid, __in uint16_t devid, __out efx_family_t *efp,
...
__checkReturn efx_rc_t efx_nic_create(__in efx_family_t family, __in efsys_identifier_t *esip, __in efsys_bar_t *esbp,
                                      __in efsys_lock_t *eslp, __deref_out efx_nic_t **enpp) {
    efx_nic_t *enp;
    efx_rc_t rc;
    ...
    case EFX_FAMILY_SIENA:
        ...
        enp->en_features = EFX_FEATURE_IPV6 | EFX_FEATURE_LFSR_HASH_INSERT | EFX_FEATURE_LINK_EVENTS |
                           EFX_FEATURE_PERIODIC_MAC_STATS | EFX_FEATURE_MCDI | EFX_FEATURE_LOOKAHEAD_SPLIT |
    ...
    case EFX_FAMILY_HUNTINGTON:
        ...
        enp->en_features = EFX_FEATURE_IPV6 | EFX_FEATURE_LINK_EVENTS | EFX_FEATURE_PERIODIC_MAC_STATS |
                           EFX_FEATURE_MCDI | EFX_FEATURE_MAC_HEADER_FILTERS | EFX_FEATURE_MCDI_DMA |
                           EFX_FEATURE_PIO_BUFFERS | EFX_FEATURE_FW_ASSISTED_TSO | EFX_FEATURE_FW_ASSISTED_TSO_V2 |
    ...
    case EFX_FAMILY_MEDFORD:
        ...
        enp->en_features = EFX_FEATURE_IPV6 | EFX_FEATURE_LINK_EVENTS | EFX_FEATURE_PERIODIC_MAC_STATS |
                           EFX_FEATURE_MCDI | EFX_FEATURE_MAC_HEADER_FILTERS | EFX_FEATURE_MCDI_DMA |
                           EFX_FEATURE_PIO_BUFFERS | EFX_FEATURE_FW_ASSISTED_TSO_V2 | EFX_FEATURE_PACKED_STREAM |
...
__checkReturn efx_rc_t efx_nic_probe(__in efx_nic_t *enp, __in efx_fw_variant_t efv) {
    ...
    EFX_STATIC_ASSERT(EFX_FW_VARIANT_PACKED_STREAM_HASH_MODE_1 == MC_CMD_FW_PACKED_STREAM_HASH_MODE_1);
...
__checkReturn efx_rc_t efx_nic_set_drv_limits(__inout efx_nic_t *enp, __in efx_drv_limits_t *edlp) {
...
__checkReturn efx_rc_t efx_nic_get_bar_region(__in efx_nic_t *enp, __in efx_nic_region_t region,
...
__checkReturn efx_rc_t efx_nic_get_vi_pool(__in efx_nic_t *enp, __out uint32_t *evq_countp, __out uint32_t *rxq_countp,
...
__checkReturn efx_rc_t efx_nic_reset(__in efx_nic_t *enp) {
    ...
    mod_flags &= ~(EFX_MOD_MCDI | EFX_MOD_PROBE | EFX_MOD_NVRAM | EFX_MOD_VPD | EFX_MOD_MON);
#if EFSYS_OPT_TUNNEL
...
__checkReturn efx_rc_t efx_nic_get_fw_version(__in efx_nic_t *enp, __out efx_nic_fw_info_t *enfip) {
    ...
    EFX_STATIC_ASSERT(EFX_RXDP_FULL_FEATURED_FW_ID == MC_CMD_GET_CAPABILITIES_OUT_RXDP);
    EFX_STATIC_ASSERT(EFX_RXDP_LOW_LATENCY_FW_ID == MC_CMD_GET_CAPABILITIES_OUT_RXDP_LOW_LATENCY);
    EFX_STATIC_ASSERT(EFX_RXDP_PACKED_STREAM_FW_ID == MC_CMD_GET_CAPABILITIES_OUT_RXDP_PACKED_STREAM);
    EFX_STATIC_ASSERT(EFX_RXDP_RULES_ENGINE_FW_ID == MC_CMD_GET_CAPABILITIES_OUT_RXDP_RULES_ENGINE);
    EFX_STATIC_ASSERT(EFX_RXDP_DPDK_FW_ID == MC_CMD_GET_CAPABILITIES_OUT_RXDP_DPDK);

    rc = efx_mcdi_version(enp, mc_fw_version, NULL, NULL);
    if (rc != 0)
        goto fail2;

    rc = efx_mcdi_get_capabilities(enp, NULL, &enfip->enfi_rx_dpcpu_fw_id, &enfip->enfi_tx_dpcpu_fw_id, NULL, NULL);
    if (rc == 0) {
...
extern void efx_loopback_mask(__in efx_loopback_kind_t loopback_kind, __out efx_qword_t *maskp) {
    efx_qword_t mask;
...
#define LOOPBACK_CHECK(_mcdi, _efx) EFX_STATIC_ASSERT(MC_CMD_LOOPBACK_##_mcdi == EFX_LOOPBACK_##_efx)
...
#undef LOOPBACK_CHECK
    ...
    if ((loopback_kind == EFX_LOOPBACK_KIND_OFF) || (loopback_kind == EFX_LOOPBACK_KIND_ALL)) {
        EFX_SET_QWORD_BIT(mask, EFX_LOOPBACK_OFF);
    }

    if ((loopback_kind == EFX_LOOPBACK_KIND_MAC) || (loopback_kind == EFX_LOOPBACK_KIND_ALL)) {
        /*
    ...
    if ((loopback_kind == EFX_LOOPBACK_KIND_PHY) || (loopback_kind == EFX_LOOPBACK_KIND_ALL)) {
        /*
...
__checkReturn efx_rc_t efx_mcdi_get_loopback_modes(__in efx_nic_t *enp) {
    efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_LOOPBACK_MODES_IN_LEN, MC_CMD_GET_LOOPBACK_MODES_OUT_V2_LEN);
    efx_qword_t mask;
    efx_qword_t modes;
    efx_rc_t rc;
    ...
    if (req.emr_out_length_used <
        MC_CMD_GET_LOOPBACK_MODES_OUT_SUGGESTED_OFST + MC_CMD_GET_LOOPBACK_MODES_OUT_SUGGESTED_LEN) {
        rc = EMSGSIZE;
        goto fail2;
    }
    ...
    EFX_AND_QWORD(mask, *MCDI_OUT2(req, efx_qword_t, GET_LOOPBACK_MODES_OUT_SUGGESTED));
    ...
    if (req.emr_out_length_used >= MC_CMD_GET_LOOPBACK_MODES_OUT_40G_OFST + MC_CMD_GET_LOOPBACK_MODES_OUT_40G_LEN) {
    ...
    if (req.emr_out_length_used >=
        MC_CMD_GET_LOOPBACK_MODES_OUT_V2_25G_OFST + MC_CMD_GET_LOOPBACK_MODES_OUT_V2_25G_LEN) {
    ...
    if (req.emr_out_length_used >=
        MC_CMD_GET_LOOPBACK_MODES_OUT_V2_50G_OFST + MC_CMD_GET_LOOPBACK_MODES_OUT_V2_50G_LEN) {
    ...
    if (req.emr_out_length_used >=
        MC_CMD_GET_LOOPBACK_MODES_OUT_V2_100G_OFST + MC_CMD_GET_LOOPBACK_MODES_OUT_V2_100G_LEN) {
...
__checkReturn efx_rc_t efx_nic_calculate_pcie_link_bandwidth(__in uint32_t pcie_link_width, __in uint32_t pcie_link_gen,
                                                             __out uint32_t *bandwidth_mbpsp) {
    uint32_t lane_bandwidth;
    uint32_t total_bandwidth;
    efx_rc_t rc;

    if ((pcie_link_width == 0) || (pcie_link_width > 16) || !ISP2(pcie_link_width)) {
        rc = EINVAL;
        goto fail1;
    }
...
__checkReturn efx_rc_t efx_nic_get_fw_subvariant(__in efx_nic_t *enp, __out efx_nic_fw_subvariant_t *subvariantp) {
    efx_rc_t rc;
    uint32_t value;

    rc = efx_mcdi_get_nic_global(enp, MC_CMD_SET_NIC_GLOBAL_IN_FIRMWARE_SUBVARIANT, &value);
    if (rc != 0)
        goto fail1;

    /* Mapping is not required since values match MCDI */
    EFX_STATIC_ASSERT(EFX_NIC_FW_SUBVARIANT_DEFAULT == MC_CMD_SET_NIC_GLOBAL_IN_FW_SUBVARIANT_DEFAULT);
    EFX_STATIC_ASSERT(EFX_NIC_FW_SUBVARIANT_NO_TX_CSUM == MC_CMD_SET_NIC_GLOBAL_IN_FW_SUBVARIANT_NO_TX_CSUM);
...
__checkReturn efx_rc_t efx_nic_set_fw_subvariant(__in efx_nic_t *enp, __in efx_nic_fw_subvariant_t subvariant) {
    efx_rc_t rc;
    ...
    rc = efx_mcdi_set_nic_global(enp, MC_CMD_SET_NIC_GLOBAL_IN_FIRMWARE_SUBVARIANT, subvariant);
    if (rc != 0)
        goto fail2;
...
__checkReturn efx_rc_t efx_nic_check_pcie_link_speed(__in efx_nic_t *enp, __in uint32_t pcie_link_width,
    ...
    if ((encp->enc_required_pcie_bandwidth_mbps == 0) || (pcie_link_width == 0) || (pcie_link_width == 32) ||
        (pcie_link_gen == 0)) {
        /*
    ...
    rc = efx_nic_calculate_pcie_link_bandwidth(pcie_link_width, pcie_link_gen, &bandwidth);
    if (rc != 0)
        goto fail1;
