...
__checkReturn efx_rc_t efx_nvram_size(__in efx_nic_t *enp, __in efx_nvram_type_t type, __out size_t *sizep) {
    const efx_nvram_ops_t *envop = enp->en_envop;
    uint32_t partn;
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_nvram_get_version(__in efx_nic_t *enp, __in efx_nvram_type_t type, __out uint32_t *subtypep,
...
__checkReturn efx_rc_t efx_nvram_rw_start(__in efx_nic_t *enp, __in efx_nvram_type_t type,
...
__checkReturn efx_rc_t efx_nvram_read_chunk(__in efx_nic_t *enp, __in efx_nvram_type_t type, __in unsigned int offset,
 ...
 */
__checkReturn efx_rc_t efx_nvram_read_backup(__in efx_nic_t *enp, __in efx_nvram_type_t type, __in unsigned int offset,
                                             __out_bcount(size) caddr_t data, __in size_t size) {
    const efx_nvram_ops_t *envop = enp->en_envop;
    uint32_t partn;
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_nvram_erase(__in efx_nic_t *enp, __in efx_nvram_type_t type) {
    const efx_nvram_ops_t *envop = enp->en_envop;
...
__checkReturn efx_rc_t efx_nvram_write_chunk(__in efx_nic_t *enp, __in efx_nvram_type_t type, __in unsigned int offset,
...
__checkReturn efx_rc_t efx_nvram_set_version(__in efx_nic_t *enp, __in efx_nvram_type_t type,
...
__checkReturn efx_rc_t efx_nvram_validate(__in efx_nic_t *enp, __in efx_nvram_type_t type,
                                          __in_bcount(partn_size) caddr_t partn_data, __in size_t partn_size) {
    const efx_nvram_ops_t *envop = enp->en_envop;
    uint32_t partn;
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mcdi_nvram_partitions(__in efx_nic_t *enp, __out_bcount(size) caddr_t data, __in size_t size,
                                                 __out unsigned int *npartnp) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_NVRAM_PARTITIONS_IN_LEN, MC_CMD_NVRAM_PARTITIONS_OUT_LENMAX);
    unsigned int npartn;
    efx_rc_t rc;
    ...
    *npartnp = npartn;
    ...
    memcpy(data, MCDI_OUT2(req, uint32_t, NVRAM_PARTITIONS_OUT_TYPE_ID), (npartn * sizeof(uint32_t)));
...
__checkReturn efx_rc_t efx_mcdi_nvram_metadata(__in efx_nic_t *enp, __in uint32_t partn, __out uint32_t *subtypep,
                                               __out_ecount(4) uint16_t version[4], __out_bcount_opt(size) char *descp,
                                               __in size_t size) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_NVRAM_METADATA_IN_LEN, MC_CMD_NVRAM_METADATA_OUT_LENMAX);
    efx_rc_t rc;
    ...
    if (MCDI_OUT_DWORD_FIELD(req, NVRAM_METADATA_OUT_FLAGS, NVRAM_METADATA_OUT_SUBTYPE_VALID)) {
        *subtypep = MCDI_OUT_DWORD(req, NVRAM_METADATA_OUT_SUBTYPE);
    } else {
        *subtypep = 0;
    }

    if (MCDI_OUT_DWORD_FIELD(req, NVRAM_METADATA_OUT_FLAGS, NVRAM_METADATA_OUT_VERSION_VALID)) {
    ...
    if (MCDI_OUT_DWORD_FIELD(req, NVRAM_METADATA_OUT_FLAGS, NVRAM_METADATA_OUT_DESCRIPTION_VALID)) {
        ...
        if ((descp != NULL) && (size > 0)) {
            ...
            memcpy(descp, MCDI_OUT2(req, char, NVRAM_METADATA_OUT_DESCRIPTION), desclen);
...
__checkReturn efx_rc_t efx_mcdi_nvram_info(__in efx_nic_t *enp, __in uint32_t partn, __out_opt size_t *sizep,
                                           __out_opt uint32_t *addressp, __out_opt uint32_t *erase_sizep,
                                           __out_opt uint32_t *write_sizep) {
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_NVRAM_INFO_IN_LEN, MC_CMD_NVRAM_INFO_V2_OUT_LEN);
    efx_mcdi_req_t req;
    efx_rc_t rc;
    ...
    if (write_sizep) {
        *write_sizep = (req.emr_out_length_used < MC_CMD_NVRAM_INFO_V2_OUT_LEN)
                           ? 0
 ...
 */
__checkReturn efx_rc_t efx_mcdi_nvram_update_start(__in efx_nic_t *enp, __in uint32_t partn) {
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_NVRAM_UPDATE_START_V2_IN_LEN, MC_CMD_NVRAM_UPDATE_START_OUT_LEN);
    efx_mcdi_req_t req;
    efx_rc_t rc;
    ...
    MCDI_IN_POPULATE_DWORD_1(req, NVRAM_UPDATE_START_V2_IN_FLAGS, NVRAM_UPDATE_START_V2_IN_FLAG_REPORT_VERIFY_RESULT,
                             1);
...
__checkReturn efx_rc_t efx_mcdi_nvram_read(__in efx_nic_t *enp, __in uint32_t partn, __in uint32_t offset,
                                           __out_bcount(size) caddr_t data, __in size_t size, __in uint32_t mode) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_NVRAM_READ_IN_V2_LEN, MC_CMD_NVRAM_READ_OUT_LENMAX);
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_mcdi_nvram_erase(__in efx_nic_t *enp, __in uint32_t partn, __in uint32_t offset,
                                            __in size_t size) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_NVRAM_ERASE_IN_LEN, MC_CMD_NVRAM_ERASE_OUT_LEN);
    efx_rc_t rc;
 ...
 */
__checkReturn efx_rc_t efx_mcdi_nvram_write(__in efx_nic_t *enp, __in uint32_t partn, __in uint32_t offset,
                                            __in_bcount(size) caddr_t data, __in size_t size) {
    efx_mcdi_req_t req;
    uint8_t *payload;
    efx_rc_t rc;
 ...
 * NVRAM updates. Older firmware will ignore the flags field in the request.
 */
__checkReturn efx_rc_t efx_mcdi_nvram_update_finish(__in efx_nic_t *enp, __in uint32_t partn, __in boolean_t reboot,
                                                    __out_opt uint32_t *verify_resultp) {
    const efx_nic_cfg_t *encp = &enp->en_nic_cfg;
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_NVRAM_UPDATE_FINISH_V2_IN_LEN, MC_CMD_NVRAM_UPDATE_FINISH_V2_OUT_LEN);
    ...
    MCDI_IN_POPULATE_DWORD_1(req, NVRAM_UPDATE_FINISH_V2_IN_FLAGS, NVRAM_UPDATE_FINISH_V2_IN_FLAG_REPORT_VERIFY_RESULT,
                             1);
    ...
    if ((encp->enc_nvram_update_verify_result_supported) && (verify_result != MC_CMD_NVRAM_VERIFY_RC_SUCCESS)) {
...
__checkReturn efx_rc_t efx_mcdi_nvram_test(__in efx_nic_t *enp, __in uint32_t partn) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_NVRAM_TEST_IN_LEN, MC_CMD_NVRAM_TEST_OUT_LEN);
    int result;
    efx_rc_t rc;
