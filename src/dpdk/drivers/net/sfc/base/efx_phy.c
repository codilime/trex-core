...
__checkReturn efx_rc_t efx_phy_led_set(__in efx_nic_t *enp, __in efx_phy_led_mode_t mode) {
    efx_nic_cfg_t *encp = (&enp->en_nic_cfg);
    efx_port_t *epp = &(enp->en_port);
    const efx_phy_ops_t *epop = epp->ep_epop;
    uint32_t mask;
...
void efx_phy_adv_cap_get(__in efx_nic_t *enp, __in uint32_t flag, __out uint32_t *maskp) {
    efx_port_t *epp = &(enp->en_port);
...
__checkReturn efx_rc_t efx_phy_adv_cap_set(__in efx_nic_t *enp, __in uint32_t mask) {
    efx_port_t *epp = &(enp->en_port);
    const efx_phy_ops_t *epop = epp->ep_epop;
    uint32_t old_mask;
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_phy_oui_get(__in efx_nic_t *enp, __out uint32_t *ouip) {
    efx_port_t *epp = &(enp->en_port);
    const efx_phy_ops_t *epop = epp->ep_epop;
...
void efx_phy_media_type_get(__in efx_nic_t *enp, __out efx_phy_media_type_t *typep) {
    efx_port_t *epp = &(enp->en_port);
...
__checkReturn efx_rc_t efx_phy_module_get_info(__in efx_nic_t *enp, __in uint8_t dev_addr, __in size_t offset,
    ...
    if ((offset > EFX_PHY_MEDIA_INFO_MAX_OFFSET) || ((offset + len) > EFX_PHY_MEDIA_INFO_MAX_OFFSET)) {
        rc = EINVAL;
        goto fail1;
    }

    if ((rc = efx_mcdi_phy_module_get_info(enp, dev_addr, offset, len, data)) != 0)
        goto fail2;
...
__checkReturn efx_rc_t efx_phy_fec_type_get(__in efx_nic_t *enp, __out efx_phy_fec_type_t *typep) {
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_phy_link_state_get(__in efx_nic_t *enp, __out efx_phy_link_state_t *eplsp) {
    efx_port_t *epp = &(enp->en_port);
    const efx_phy_ops_t *epop = epp->ep_epop;
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_phy_stats_update(__in efx_nic_t *enp, __in efsys_mem_t *esmp,
...
__checkReturn efx_rc_t efx_bist_start(__in efx_nic_t *enp, __in efx_bist_type_t type) {
    efx_port_t *epp = &(enp->en_port);
    const efx_phy_ops_t *epop = epp->ep_epop;
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_bist_poll(__in efx_nic_t *enp, __in efx_bist_type_t type, __out efx_bist_result_t *resultp,
                                     __out_opt uint32_t *value_maskp, __out_ecount_opt(count) unsigned long *valuesp,
                                     __in size_t count) {
    efx_port_t *epp = &(enp->en_port);
    const efx_phy_ops_t *epop = epp->ep_epop;
    efx_rc_t rc;
    ...
    if ((rc = epop->epo_bist_poll(enp, type, resultp, value_maskp, valuesp, count)) != 0)
        goto fail2;
