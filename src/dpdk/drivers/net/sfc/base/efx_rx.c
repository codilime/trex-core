...
static __checkReturn efx_rc_t siena_rx_scatter_enable(__in efx_nic_t *enp, __in unsigned int buf_size);
#endif /* EFSYS_OPT_RX_SCATTER */

#if EFSYS_OPT_RX_SCALE
static __checkReturn efx_rc_t siena_rx_scale_mode_set(__in efx_nic_t *enp, __in uint32_t rss_context,
                                                      __in efx_rx_hash_alg_t alg, __in efx_rx_hash_type_t type,
                                                      __in boolean_t insert);

static __checkReturn efx_rc_t siena_rx_scale_key_set(__in efx_nic_t *enp, __in uint32_t rss_context,
                                                     __in_ecount(n) uint8_t *key, __in size_t n);

static __checkReturn efx_rc_t siena_rx_scale_tbl_set(__in efx_nic_t *enp, __in uint32_t rss_context,
                                                     __in_ecount(n) unsigned int *table, __in size_t n);

static __checkReturn uint32_t siena_rx_prefix_hash(__in efx_nic_t *enp, __in efx_rx_hash_alg_t func,
                                                   __in uint8_t *buffer);

#endif /* EFSYS_OPT_RX_SCALE */

static __checkReturn efx_rc_t siena_rx_prefix_pktlen(__in efx_nic_t *enp, __in uint8_t *buffer,
                                                     __out uint16_t *lengthp);

static void siena_rx_qpost(__in efx_rxq_t *erp, __in_ecount(ndescs) efsys_dma_addr_t *addrp, __in size_t size,
                           __in unsigned int ndescs, __in unsigned int completed, __in unsigned int added);

static void siena_rx_qpush(__in efx_rxq_t *erp, __in unsigned int added, __inout unsigned int *pushedp);

#if EFSYS_OPT_RX_PACKED_STREAM
static void siena_rx_qpush_ps_credits(__in efx_rxq_t *erp);

static __checkReturn uint8_t *siena_rx_qps_packet_info(__in efx_rxq_t *erp, __in uint8_t *buffer,
                                                       __in uint32_t buffer_length, __in uint32_t current_offset,
                                                       __out uint16_t *lengthp, __out uint32_t *next_offsetp,
...
static __checkReturn efx_rc_t siena_rx_qcreate(__in efx_nic_t *enp, __in unsigned int index, __in unsigned int label,
...
#if EFSYS_OPT_RX_SCATTER
__checkReturn efx_rc_t efx_rx_scatter_enable(__in efx_nic_t *enp, __in unsigned int buf_size) {
    const efx_rx_ops_t *erxop = enp->en_erxop;
    efx_rc_t rc;
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t efx_rx_scale_hash_flags_get(__in efx_nic_t *enp, __in efx_rx_hash_alg_t hash_alg,
...
__checkReturn efx_rc_t efx_rx_hash_default_support_get(__in efx_nic_t *enp, __out efx_rx_hash_support_t *supportp) {
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_rx_scale_default_support_get(__in efx_nic_t *enp, __out efx_rx_scale_context_type_t *typep) {
    efx_rc_t rc;
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t efx_rx_scale_context_alloc(__in efx_nic_t *enp, __in efx_rx_scale_context_type_t type,
    ...
    if ((rc = erxop->erxo_scale_context_alloc(enp, type, num_queues, rss_contextp)) != 0) {
        goto fail2;
    }
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t efx_rx_scale_context_free(__in efx_nic_t *enp, __in uint32_t rss_context) {
    const efx_rx_ops_t *erxop = enp->en_erxop;
    efx_rc_t rc;
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t efx_rx_scale_mode_set(__in efx_nic_t *enp, __in uint32_t rss_context, __in efx_rx_hash_alg_t alg,
    ...
    if (type_check != 0) {
        ...
        rc = efx_rx_scale_hash_flags_get(enp, alg, type_flags, EFX_ARRAY_SIZE(type_flags), &type_nflags);
        if (rc != 0)
            goto fail2;
     ...
     */
    if (encp->enc_rx_scale_additional_modes_supported == B_FALSE) {
        efx_rx_hash_type_t t_ipv4 = EFX_RX_HASH(IPV4, 2TUPLE) | EFX_RX_HASH(IPV4_TCP, 2TUPLE);
        efx_rx_hash_type_t t_ipv6 = EFX_RX_HASH(IPV6, 2TUPLE) | EFX_RX_HASH(IPV6_TCP, 2TUPLE);
    ...
    if (erxop->erxo_scale_mode_set != NULL) {
        if ((rc = erxop->erxo_scale_mode_set(enp, rss_context, alg, type, insert)) != 0)
            goto fail4;
    }
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t efx_rx_scale_key_set(__in efx_nic_t *enp, __in uint32_t rss_context, __in_ecount(n) uint8_t *key,
                                            __in size_t n) {
    const efx_rx_ops_t *erxop = enp->en_erxop;
    efx_rc_t rc;
...
#if EFSYS_OPT_RX_SCALE
__checkReturn efx_rc_t efx_rx_scale_tbl_set(__in efx_nic_t *enp, __in uint32_t rss_context,
                                            __in_ecount(n) unsigned int *table, __in size_t n) {
    const efx_rx_ops_t *erxop = enp->en_erxop;
    efx_rc_t rc;
...
void efx_rx_qpost(__in efx_rxq_t *erp, __in_ecount(ndescs) efsys_dma_addr_t *addrp, __in size_t size,
                  __in unsigned int ndescs, __in unsigned int completed, __in unsigned int added) {
...
__checkReturn uint8_t *efx_rx_qps_packet_info(__in efx_rxq_t *erp, __in uint8_t *buffer, __in uint32_t buffer_length,
                                              __in uint32_t current_offset, __out uint16_t *lengthp,
                                              __out uint32_t *next_offsetp, __out uint32_t *timestamp) {
    efx_nic_t *enp = erp->er_enp;
    const efx_rx_ops_t *erxop = enp->en_erxop;

    return (erxop->erxo_qps_packet_info(erp, buffer, buffer_length, current_offset, lengthp, next_offsetp, timestamp));
}

#endif /* EFSYS_OPT_RX_PACKED_STREAM */

void efx_rx_qpush(__in efx_rxq_t *erp, __in unsigned int added, __inout unsigned int *pushedp) {
    efx_nic_t *enp = erp->er_enp;
    const efx_rx_ops_t *erxop = enp->en_erxop;
...
__checkReturn size_t efx_rxq_size(__in const efx_nic_t *enp, __in unsigned int ndescs) {
    const efx_nic_cfg_t *encp = efx_nic_cfg_get(enp);

    return (ndescs * encp->enc_rx_desc_size);
}

__checkReturn unsigned int efx_rxq_nbufs(__in const efx_nic_t *enp, __in unsigned int ndescs) {
...
static __checkReturn efx_rc_t efx_rx_qcreate_internal(__in efx_nic_t *enp, __in unsigned int index,
    ...
    if (!ISP2(ndescs) || ndescs < encp->enc_rxq_min_ndescs || ndescs > encp->enc_rxq_max_ndescs) {
        rc = EINVAL;
        goto fail1;
    }
    ...
    if ((rc = erxop->erxo_qcreate(enp, index, label, type, type_data, esmp, ndescs, id, flags, eep, erp)) != 0)
        goto fail3;
...
__checkReturn efx_rc_t efx_rx_qcreate(__in efx_nic_t *enp, __in unsigned int index, __in unsigned int label,
    ...
    return efx_rx_qcreate_internal(enp, index, label, type, &type_data, esmp, ndescs, id, flags, eep, erpp);
}

#if EFSYS_OPT_RX_PACKED_STREAM

__checkReturn efx_rc_t efx_rx_qcreate_packed_stream(__in efx_nic_t *enp, __in unsigned int index,
                                                    __in unsigned int label, __in uint32_t ps_buf_size,
                                                    __in efsys_mem_t *esmp, __in size_t ndescs, __in efx_evq_t *eep,
                                                    __deref_out efx_rxq_t **erpp) {
    efx_rxq_type_data_t type_data;
    ...
    return efx_rx_qcreate_internal(enp, index, label, EFX_RXQ_TYPE_PACKED_STREAM, &type_data, esmp, ndescs,
...
__checkReturn efx_rc_t efx_rx_qcreate_es_super_buffer(__in efx_nic_t *enp, __in unsigned int index,
                                                      __in unsigned int label, __in uint32_t n_bufs_per_desc,
                                                      __in uint32_t max_dma_len, __in uint32_t buf_stride,
                                                      __in uint32_t hol_block_timeout, __in efsys_mem_t *esmp,
                                                      __in size_t ndescs, __in unsigned int flags, __in efx_evq_t *eep,
                                                      __deref_out efx_rxq_t **erpp) {
    efx_rc_t rc;
    efx_rxq_type_data_t type_data;
    ...
    rc = efx_rx_qcreate_internal(enp, index, label, EFX_RXQ_TYPE_ES_SUPER_BUFFER, &type_data, esmp, ndescs,
...
__checkReturn efx_rc_t efx_pseudo_hdr_pkt_length_get(__in efx_rxq_t *erp, __in uint8_t *buffer,
                                                     __out uint16_t *lengthp) {
    efx_nic_t *enp = erp->er_enp;
    const efx_rx_ops_t *erxop = enp->en_erxop;
...
#if EFSYS_OPT_RX_SCALE
__checkReturn uint32_t efx_pseudo_hdr_hash_get(__in efx_rxq_t *erp, __in efx_rx_hash_alg_t func, __in uint8_t *buffer) {
    efx_nic_t *enp = erp->er_enp;
    const efx_rx_ops_t *erxop = enp->en_erxop;
...
static __checkReturn efx_rc_t siena_rx_scatter_enable(__in efx_nic_t *enp, __in unsigned int buf_size) {
    ...
    nbuf32 = buf_size / 32;
    if ((nbuf32 == 0) || (nbuf32 >= (1 << FRF_BZ_RX_USR_BUF_SIZE_WIDTH)) || ((buf_size % 32) != 0)) {
        rc = EINVAL;
        goto fail1;
    }
...
#define EFX_RX_TOEPLITZ_IPV6_HASH(_enp, _ip, _tcp, _rc)                                                                \
    do {                                                                                                               \
        ...
        EFX_SET_OWORD_FIELD(oword, FRF_CZ_RX_RSS_IPV6_IP_THASH_ENABLE, (_ip) ? 1 : 0);                                 \
        EFX_SET_OWORD_FIELD(oword, FRF_CZ_RX_RSS_IPV6_TCP_SUPPRESS, (_tcp) ? 0 : 1);                                   \
        EFX_BAR_WRITEO((_enp), FR_CZ_RX_RSS_IPV6_REG3, &oword);                                                        \
                                                                                                                       \
        (_rc) = 0;                                                                                                     \
                                                                                                                       \
...
static __checkReturn efx_rc_t siena_rx_scale_mode_set(__in efx_nic_t *enp, __in uint32_t rss_context,
    ...
    case EFX_RX_HASHALG_TOEPLITZ:
        EFX_RX_TOEPLITZ_IPV4_HASH(enp, insert, (type & EFX_RX_HASH_IPV4) ? B_TRUE : B_FALSE,
                                  (type & EFX_RX_HASH_TCPIPV4) ? B_TRUE : B_FALSE);

        EFX_RX_TOEPLITZ_IPV6_HASH(enp, (type & EFX_RX_HASH_IPV6) ? B_TRUE : B_FALSE,
                                  (type & EFX_RX_HASH_TCPIPV6) ? B_TRUE : B_FALSE, rc);
        if (rc != 0)
            goto fail2;
...
static __checkReturn efx_rc_t siena_rx_scale_key_set(__in efx_nic_t *enp, __in uint32_t rss_context,
    ...
    EFX_ZERO_OWORD(oword);
    for (offset = (FRF_BZ_RX_RSS_TKEY_LBN + FRF_BZ_RX_RSS_TKEY_WIDTH) / 8; offset > 0 && byte < n; --offset)
    ...
    for (offset = (FRF_CZ_RX_RSS_IPV6_TKEY_HI_LBN + FRF_CZ_RX_RSS_IPV6_TKEY_HI_WIDTH) / 8; offset > 0 && byte < n;
         --offset)
        oword.eo_u8[offset - 1] = key[byte++];
    ...
    EFX_ZERO_OWORD(oword);
    for (offset = (FRF_CZ_RX_RSS_IPV6_TKEY_MID_LBN + FRF_CZ_RX_RSS_IPV6_TKEY_MID_WIDTH) / 8; offset > 0 && byte < n;
         --offset)
        oword.eo_u8[offset - 1] = key[byte++];
    ...
    EFX_ZERO_OWORD(oword);
    for (offset = (FRF_CZ_RX_RSS_IPV6_TKEY_LO_LBN + FRF_CZ_RX_RSS_IPV6_TKEY_LO_WIDTH) / 8; offset > 0 && byte < n;
         --offset)
        oword.eo_u8[offset - 1] = key[byte++];
...
static __checkReturn uint32_t siena_rx_prefix_hash(__in efx_nic_t *enp, __in efx_rx_hash_alg_t func,
    ...
    switch (func) {
    case EFX_RX_HASHALG_TOEPLITZ:
        return ((buffer[12] << 24) | (buffer[13] << 16) | (buffer[14] << 8) | buffer[15]);
...
static void siena_rx_qpost(__in efx_rxq_t *erp, __in_ecount(ndescs) efsys_dma_addr_t *addrp, __in size_t size,
    ...
    for (i = 0; i < ndescs; i++) {
        ...
        EFX_POPULATE_QWORD_3(qword, FSF_AZ_RX_KER_BUF_SIZE, (uint32_t)(size), FSF_AZ_RX_KER_BUF_ADDR_DW0,
                             (uint32_t)(addrp[i] & 0xffffffff), FSF_AZ_RX_KER_BUF_ADDR_DW1, (uint32_t)(addrp[i] >> 32));
...
static void siena_rx_qpush(__in efx_rxq_t *erp, __in unsigned int added, __inout unsigned int *pushedp) {
    ...
    *pushedp = added;
    ...
    EFX_BAR_TBL_WRITED3(enp, FR_BZ_RX_DESC_UPD_REGP0, erp->er_index, &dword, B_FALSE);
}
...
static uint8_t *siena_rx_qps_packet_info(__in efx_rxq_t *erp, __in uint8_t *buffer, __in uint32_t buffer_length,
                                         __in uint32_t current_offset, __out uint16_t *lengthp,
                                         __out uint32_t *next_offsetp, __out uint32_t *timestamp) {
    /* Not supported by Siena hardware */
    EFSYS_ASSERT(0);
...
static __checkReturn efx_rc_t siena_rx_qflush(__in efx_rxq_t *erp) {
    ...
    /* Flush the queue */
    EFX_POPULATE_OWORD_2(oword, FRF_AZ_RX_FLUSH_DESCQ_CMD, 1, FRF_AZ_RX_FLUSH_DESCQ, label);
...
static __checkReturn efx_rc_t siena_rx_qcreate(__in efx_nic_t *enp, __in unsigned int index, __in unsigned int label,
    ...
    for (size = 0; (1U << size) <= encp->enc_rxq_max_ndescs / encp->enc_rxq_min_ndescs; size++)
    ...
    EFX_POPULATE_OWORD_7(oword, FRF_AZ_RX_DESCQ_BUF_BASE_ID, id, FRF_AZ_RX_DESCQ_EVQ_ID, eep->ee_index,
                         FRF_AZ_RX_DESCQ_OWNER_ID, 0, FRF_AZ_RX_DESCQ_LABEL, label, FRF_AZ_RX_DESCQ_SIZE, size,
                         FRF_AZ_RX_DESCQ_TYPE, 0, FRF_AZ_RX_DESCQ_JUMBO, jumbo);
