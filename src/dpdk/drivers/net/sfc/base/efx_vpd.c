...
__checkReturn efx_rc_t efx_vpd_read(__in efx_nic_t *enp, __out_bcount(size) caddr_t data, __in size_t size) {
    const efx_vpd_ops_t *evpdop = enp->en_evpdop;
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_vpd_verify(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size) {
    const efx_vpd_ops_t *evpdop = enp->en_evpdop;
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_vpd_reinit(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size) {
    const efx_vpd_ops_t *evpdop = enp->en_evpdop;
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_vpd_get(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size,
...
__checkReturn efx_rc_t efx_vpd_set(__in efx_nic_t *enp, __inout_bcount(size) caddr_t data, __in size_t size,
...
__checkReturn efx_rc_t efx_vpd_next(__in efx_nic_t *enp, __inout_bcount(size) caddr_t data, __in size_t size,
...
__checkReturn efx_rc_t efx_vpd_write(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size) {
    const efx_vpd_ops_t *evpdop = enp->en_evpdop;
    efx_rc_t rc;
...
static __checkReturn efx_rc_t efx_vpd_next_tag(__in caddr_t data, __in size_t size, __inout unsigned int *offsetp,
                                               __out efx_vpd_tag_t *tagp, __out uint16_t *lengthp) {
    efx_byte_t byte;
    efx_word_t word;
    uint8_t name;
    uint16_t length;
    ...
    case TAG_TYPE_LARGE_ITEM_DECODE:
        ...
        EFX_POPULATE_WORD_2(word, EFX_BYTE_0, data[*offsetp + 1], EFX_BYTE_1, data[*offsetp + 2]);
...
static __checkReturn efx_rc_t efx_vpd_next_keyword(__in_bcount(size) caddr_t tag, __in size_t size,
...
__checkReturn efx_rc_t efx_vpd_hunk_length(__in_bcount(size) caddr_t data, __in size_t size, __out size_t *lengthp) {
    efx_vpd_tag_t tag;
    unsigned int offset;
    uint16_t taglen;
    efx_rc_t rc;
...
__checkReturn efx_rc_t efx_vpd_hunk_verify(__in_bcount(size) caddr_t data, __in size_t size,
    ...
    while (1) {
        ...
        for (pos = 0; pos != taglen; pos += 3 + keylen) {
            ...
            if ((rc = efx_vpd_next_keyword(data + offset, taglen, pos, &keyword, &keylen)) != 0)
                goto fail3;
...
__checkReturn efx_rc_t efx_vpd_hunk_reinit(__in_bcount(size) caddr_t data, __in size_t size, __in boolean_t wantpid) {
    ...
    /* Append trailing tag */
    EFX_POPULATE_BYTE_3(byte, TAG_TYPE, TAG_TYPE_SMALL_ITEM_DECODE, TAG_SMALL_ITEM_NAME, TAG_NAME_END_DECODE,
...
__checkReturn efx_rc_t efx_vpd_hunk_next(__in_bcount(size) caddr_t data, __in size_t size, __out efx_vpd_tag_t *tagp,
...
__checkReturn efx_rc_t efx_vpd_hunk_get(__in_bcount(size) caddr_t data, __in size_t size, __in efx_vpd_tag_t tag,
                                        __in efx_vpd_keyword_t keyword, __out unsigned int *payloadp,
    ...
    while (1) {
        ...
        if (itag == tag) {
            ...
            for (pos = 0; pos != taglen; pos += 3 + keylen) {
                if ((rc = efx_vpd_next_keyword(data + offset, taglen, pos, &ikeyword, &keylen)) != 0)
                    goto fail2;
...
__checkReturn efx_rc_t efx_vpd_hunk_set(__in_bcount(size) caddr_t data, __in size_t size, __in efx_vpd_value_t *evvp) {
    efx_word_t word;
    efx_vpd_tag_t tag;
    efx_vpd_keyword_t keyword;
    unsigned int offset;
