...
#define MCDI_STATIC_SENSOR_ASSERT(_field) EFX_STATIC_ASSERT(MC_CMD_SENSOR_STATE_##_field == EFX_MON_STAT_STATE_##_field)

static void mcdi_mon_decode_stats(__in efx_nic_t *enp, __in_bcount(sensor_mask_size) uint32_t *sensor_mask,
    ...
    for (sensor = 0; sensor < sensor_max; ++sensor) {
        ...
        decode_ok = decode_ok && efx_mon_get_stat_portmap(id, &stat_portmask);
         ...
         * monitor statistic.
         */
        stat_mask[id / EFX_MON_MASK_ELEMENT_SIZE] |= (1U << (id % EFX_MON_MASK_ELEMENT_SIZE));
        ...
        if (stat != NULL && esmp != NULL && !EFSYS_MEM_IS_NULL(esmp)) {
            ...
            stat[id].emsv_value = (uint16_t)EFX_DWORD_FIELD(dword, MC_CMD_SENSOR_VALUE_ENTRY_TYPEDEF_VALUE);

            stat[id].emsv_state = (uint16_t)EFX_DWORD_FIELD(dword, MC_CMD_SENSOR_VALUE_ENTRY_TYPEDEF_STATE);

            stat[id].emsv_unit = efx_mon_get_stat_unit(id, &stat_unit) ? stat_unit : EFX_MON_STAT_UNIT_UNKNOWN;
        }
    }
...
__checkReturn efx_rc_t mcdi_mon_ev(__in efx_nic_t *enp, __in efx_qword_t *eqp, __out efx_mon_stat_t *idp,
    ...
    EFSYS_ASSERT((sensor % (MC_CMD_SENSOR_PAGE0_NEXT + 1)) != MC_CMD_SENSOR_PAGE0_NEXT);
    EFSYS_ASSERT(enp->en_nic_cfg.enc_mcdi_sensor_maskp != NULL);
    EFSYS_ASSERT((enp->en_nic_cfg.enc_mcdi_sensor_maskp[sensor / (MC_CMD_SENSOR_PAGE0_NEXT + 1)] &
    ...
    if (!(efx_mon_get_stat_portmap(id, &sensor_port_mask) && (port_mask && sensor_port_mask))) {
        return (ENODEV);
    }
    EFSYS_ASSERT(id < EFX_MON_NSTATS);
...
static __checkReturn efx_rc_t efx_mcdi_read_sensors(__in efx_nic_t *enp, __in efsys_mem_t *esmp, __in uint32_t size) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_READ_SENSORS_EXT_IN_LEN, MC_CMD_READ_SENSORS_EXT_OUT_LEN);
...
static __checkReturn efx_rc_t efx_mcdi_sensor_info_npages(__in efx_nic_t *enp, __out uint32_t *npagesp) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_SENSOR_INFO_EXT_IN_LEN, MC_CMD_SENSOR_INFO_OUT_LENMAX);
    int page;
    efx_rc_t rc;
    ...
    } while (MCDI_OUT_DWORD(req, SENSOR_INFO_OUT_MASK) & (1U << MC_CMD_SENSOR_PAGE0_NEXT));
...
static __checkReturn efx_rc_t efx_mcdi_sensor_info(__in efx_nic_t *enp, __out_ecount(npages) uint32_t *sensor_maskp,
                                                   __in size_t npages) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_SENSOR_INFO_EXT_IN_LEN, MC_CMD_SENSOR_INFO_OUT_LENMAX);
    uint32_t page;
    ...
    for (page = 0; page < npages; page++) {
        ...
        if ((page != (npages - 1)) && ((mask & (1U << MC_CMD_SENSOR_PAGE0_NEXT)) == 0)) {
            rc = EINVAL;
            goto fail3;
        }
...
static __checkReturn efx_rc_t efx_mcdi_sensor_info_page(__in efx_nic_t *enp, __in uint32_t page,
    ...
    memset(limits, 0, ((sizeof(*mask_part) * 8) - 1) * sizeof(efx_mon_stat_limits_t));
    ...
    *mask_part = maskp->ed_u32[0];
    ...
    while (mask_copy) {
        ...
        if (mask_copy == (1U << MC_CMD_SENSOR_PAGE0_NEXT)) {
            ...
            limits->emlv_warning_min = EFX_QWORD_FIELD(*limit_info, MC_CMD_SENSOR_INFO_ENTRY_TYPEDEF_MIN1);

            limits->emlv_warning_max = EFX_QWORD_FIELD(*limit_info, MC_CMD_SENSOR_INFO_ENTRY_TYPEDEF_MAX1);

            limits->emlv_fatal_min = EFX_QWORD_FIELD(*limit_info, MC_CMD_SENSOR_INFO_ENTRY_TYPEDEF_MIN2);

            limits->emlv_fatal_max = EFX_QWORD_FIELD(*limit_info, MC_CMD_SENSOR_INFO_ENTRY_TYPEDEF_MAX2);
...
__checkReturn efx_rc_t mcdi_mon_stats_update(__in efx_nic_t *enp, __in efsys_mem_t *esmp,
    ...
    mcdi_mon_decode_stats(enp, encp->enc_mcdi_sensor_maskp, encp->enc_mcdi_sensor_mask_size, esmp, NULL, values);
...
static void lowest_set_bit(__in uint32_t input_mask, __out uint32_t *lowest_bit_mask, __out uint32_t *lowest_bit_num) {
    uint32_t x;
...
__checkReturn efx_rc_t mcdi_mon_limits_update(__in efx_nic_t *enp,
...
__checkReturn efx_rc_t mcdi_mon_cfg_build(__in efx_nic_t *enp) {
    ...
    EFSYS_KMEM_ALLOC(enp->en_esip, encp->enc_mcdi_sensor_mask_size, encp->enc_mcdi_sensor_maskp);
    ...
    /* Read mc sensor mask */
    if ((rc = efx_mcdi_sensor_info(enp, encp->enc_mcdi_sensor_maskp, npages)) != 0)
        goto fail4;

    /* Build monitor statistics mask */
    mcdi_mon_decode_stats(enp, encp->enc_mcdi_sensor_maskp, encp->enc_mcdi_sensor_mask_size, NULL,
...
fail4:
    EFSYS_PROBE(fail4);
    EFSYS_KMEM_FREE(enp->en_esip, encp->enc_mcdi_sensor_mask_size, encp->enc_mcdi_sensor_maskp);
