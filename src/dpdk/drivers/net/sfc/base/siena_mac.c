...
__checkReturn efx_rc_t siena_mac_poll(__in efx_nic_t *enp, __out efx_link_mode_t *link_modep) {
...
__checkReturn efx_rc_t siena_mac_up(__in efx_nic_t *enp, __out boolean_t *mac_upp) {
    siena_link_state_t sls;
    efx_rc_t rc;
...
__checkReturn efx_rc_t siena_mac_reconfigure(__in efx_nic_t *enp) {
    ...
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MAX(MC_CMD_SET_MAC_IN_LEN, MC_CMD_SET_MCAST_HASH_IN_LEN),
    ...
    MCDI_IN_POPULATE_DWORD_2(req, SET_MAC_IN_REJECT, SET_MAC_IN_REJECT_UNCST, !epp->ep_all_unicst,
    ...
    if (epp->ep_fcntl_autoneg)
        ...
        fcntl = (epp->ep_fcntl & EFX_FCNTL_GENERATE) ? MC_CMD_FCNTL_BIDIR : MC_CMD_FCNTL_RESPOND;
    else
    ...
    memcpy(MCDI_IN2(req, uint8_t, SET_MCAST_HASH_IN_HASH0), multicast_hash, sizeof(multicast_hash));
...
__checkReturn efx_rc_t siena_mac_loopback_set(__in efx_nic_t *enp, __in efx_link_mode_t link_mode,
...
__checkReturn efx_rc_t siena_mac_stats_get_mask(__in efx_nic_t *enp, __inout_bcount(mask_size) uint32_t *maskp,
    ...
    if ((rc = efx_mac_stats_mask_add_ranges(maskp, mask_size, siena_stats, EFX_ARRAY_SIZE(siena_stats))) != 0)
        goto fail1;
...
#define SIENA_MAC_STAT_READ(_esmp, _field, _eqp) EFSYS_MEM_READQ((_esmp), (_field) * sizeof(efx_qword_t), _eqp)

__checkReturn efx_rc_t siena_mac_stats_update(__in efx_nic_t *enp, __in efsys_mem_t *esmp,
...
__checkReturn efx_rc_t siena_mac_pdu_get(__in efx_nic_t *enp, __out size_t *pdu) { return (ENOTSUP); }
