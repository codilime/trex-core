...
#define SIENA_MCDI_PDU(_emip) (((emip)->emi_port == 1) ? MC_SMEM_P0_PDU_OFST >> 2 : MC_SMEM_P1_PDU_OFST >> 2)

#define SIENA_MCDI_DOORBELL(_emip)                                                                                     \
    (((emip)->emi_port == 1) ? MC_SMEM_P0_DOORBELL_OFST >> 2 : MC_SMEM_P1_DOORBELL_OFST >> 2)

#define SIENA_MCDI_STATUS(_emip) (((emip)->emi_port == 1) ? MC_SMEM_P0_STATUS_OFST >> 2 : MC_SMEM_P1_STATUS_OFST >> 2)

void siena_mcdi_send_request(__in efx_nic_t *enp, __in_bcount(hdr_len) void *hdrp, __in size_t hdr_len,
                             __in_bcount(sdu_len) void *sdup, __in size_t sdu_len) {
    ...
    for (pos = 0; pos < sdu_len; pos += sizeof(efx_dword_t)) {
        ...
        EFX_BAR_TBL_WRITED(enp, FR_CZ_MC_TREG_SMEM, pdur + 1 + (pos >> 2), &dword, B_FALSE);
    }
...
void siena_mcdi_read_response(__in efx_nic_t *enp, __out_bcount(length) void *bufferp, __in size_t offset,
                              __in size_t length) {
    efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
    unsigned int pdur;
    unsigned int pos = 0;
    ...
    while (remaining > 0) {
        ...
        EFX_BAR_TBL_READD(enp, FR_CZ_MC_TREG_SMEM, pdur + ((offset + pos) >> 2), &data, B_FALSE);
...
__checkReturn efx_rc_t siena_mcdi_init(__in efx_nic_t *enp, __in const efx_mcdi_transport_t *mtp) {
    efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
    efx_oword_t oword;
...
__checkReturn efx_rc_t
    siena_mcdi_feature_supported(__in efx_nic_t *enp, __in efx_mcdi_feature_id_t id, __out boolean_t *supportedp) {
    efx_rc_t rc;
...
void siena_mcdi_get_timeout(__in efx_nic_t *enp, __in efx_mcdi_req_t *emrp, __out uint32_t *timeoutp) {
