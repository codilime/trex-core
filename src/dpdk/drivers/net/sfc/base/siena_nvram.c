...
__checkReturn efx_rc_t siena_nvram_partn_size(__in efx_nic_t *enp, __in uint32_t partn, __out size_t *sizep) {
    efx_rc_t rc;
...
__checkReturn efx_rc_t siena_nvram_partn_lock(__in efx_nic_t *enp, __in uint32_t partn) {
    efx_rc_t rc;
...
__checkReturn efx_rc_t siena_nvram_partn_read(__in efx_nic_t *enp, __in uint32_t partn, __in unsigned int offset,
    ...
    while (size > 0) {
        ...
        if ((rc = efx_mcdi_nvram_read(enp, partn, offset, data, chunk, MC_CMD_NVRAM_READ_IN_V2_DEFAULT)) != 0) {
            goto fail1;
        }
...
__checkReturn efx_rc_t siena_nvram_partn_erase(__in efx_nic_t *enp, __in uint32_t partn, __in unsigned int offset,
                                               __in size_t size) {
    efx_rc_t rc;
...
__checkReturn efx_rc_t siena_nvram_partn_write(__in efx_nic_t *enp, __in uint32_t partn, __in unsigned int offset,
                                               __out_bcount(size) caddr_t data, __in size_t size) {
    size_t chunk;
    efx_rc_t rc;
...
__checkReturn efx_rc_t siena_nvram_partn_unlock(__in efx_nic_t *enp, __in uint32_t partn,
     ...
     */
    reboot = (partn == MC_CMD_NVRAM_TYPE_PHY_PORT0 || partn == MC_CMD_NVRAM_TYPE_PHY_PORT1 ||
...
static siena_parttbl_entry_t siena_parttbl[] = {{MC_CMD_NVRAM_TYPE_DISABLED_CALLISTO, 1, EFX_NVRAM_NULLPHY},
...
__checkReturn efx_rc_t siena_nvram_test(__in efx_nic_t *enp) {
    ...
    for (i = 0; i < EFX_ARRAY_SIZE(siena_parttbl); i++) {
        ...
        if (entry->port != emip->emi_port || !(enp->en_u.siena.enu_partn_mask & (1 << entry->partn)))
            continue;
...
#define SIENA_DYNAMIC_CFG_SIZE(_nitems)                                                                                \
    (sizeof(siena_mc_dynamic_config_hdr_t) +                                                                           \
     ((_nitems) * sizeof(((siena_mc_dynamic_config_hdr_t *)NULL)->fw_version[0])))

__checkReturn efx_rc_t siena_nvram_get_dynamic_cfg(__in efx_nic_t *enp, __in uint32_t partn, __in boolean_t vpd,
    ...
    EFSYS_ASSERT(partn == MC_CMD_NVRAM_TYPE_DYNAMIC_CFG_PORT0 || partn == MC_CMD_NVRAM_TYPE_DYNAMIC_CFG_PORT1);
    ...
    if ((rc = siena_nvram_partn_read(enp, partn, 0, (caddr_t)dcfg, SIENA_NVRAM_CHUNK)) != 0)
        goto fail4;

    /* Verify the magic */
    if (EFX_DWORD_FIELD(dcfg->magic, EFX_DWORD_0) != SIENA_MC_DYNAMIC_CONFIG_MAGIC)
        goto invalid1;
    ...
    if (hdr_length > size || vpd_offset > size || vpd_length > size || vpd_length + vpd_offset > size)
        goto invalid2;

    /* Verify the header has room for all it's versions */
    if (hdr_length < SIENA_DYNAMIC_CFG_SIZE(0) || hdr_length < SIENA_DYNAMIC_CFG_SIZE(nversions))
        goto invalid3;
    ...
    if (region > SIENA_NVRAM_CHUNK) {
        if ((rc = siena_nvram_partn_read(enp, partn, SIENA_NVRAM_CHUNK, (caddr_t)dcfg + SIENA_NVRAM_CHUNK,
...
invalid1:
    ...
    EFX_POPULATE_BYTE_1(dcfg->version, EFX_BYTE_0, SIENA_MC_DYNAMIC_CONFIG_VERSION);
...
__checkReturn efx_rc_t siena_nvram_get_subtype(__in efx_nic_t *enp, __in uint32_t partn, __out uint32_t *subtypep) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_BOARD_CFG_IN_LEN, MC_CMD_GET_BOARD_CFG_OUT_LENMAX);
    efx_word_t *fw_list;
    efx_rc_t rc;
    ...
    if (req.emr_out_length_used < MC_CMD_GET_BOARD_CFG_OUT_FW_SUBTYPE_LIST_OFST + (partn + 1) * sizeof(efx_word_t)) {
        rc = ENOENT;
        goto fail3;
    }
...
__checkReturn efx_rc_t siena_nvram_partn_get_version(__in efx_nic_t *enp, __in uint32_t partn, __out uint32_t *subtypep,
    ...
    for (i = 0; i < EFX_ARRAY_SIZE(siena_parttbl); i++) {
        ...
        dcfg_partn = (entry->port == 1) ? MC_CMD_NVRAM_TYPE_DYNAMIC_CFG_PORT0 : MC_CMD_NVRAM_TYPE_DYNAMIC_CFG_PORT1;
        /*
        ...
        if ((rc = siena_nvram_get_dynamic_cfg(enp, dcfg_partn, B_FALSE, &dcfg, &length)) != 0)
            goto fail3;
...
__checkReturn efx_rc_t siena_nvram_partn_rw_start(__in efx_nic_t *enp, __in uint32_t partn, __out size_t *chunk_sizep) {
    efx_rc_t rc;
...
__checkReturn efx_rc_t siena_nvram_partn_rw_finish(__in efx_nic_t *enp, __in uint32_t partn,
                                                   __out_opt uint32_t *verify_resultp) {
    efx_rc_t rc;
...
__checkReturn efx_rc_t siena_nvram_partn_set_version(__in efx_nic_t *enp, __in uint32_t partn,
    ...
    dcfg_partn = (emip->emi_port == 1) ? MC_CMD_NVRAM_TYPE_DYNAMIC_CFG_PORT0 : MC_CMD_NVRAM_TYPE_DYNAMIC_CFG_PORT1;
    ...
    if ((rc = siena_nvram_get_dynamic_cfg(enp, dcfg_partn, B_TRUE, &dcfg, &length)) != 0)
        goto fail3;
    ...
    if (vpd_offset < required_hdr_length) {
        (void)memmove((caddr_t)dcfg + required_hdr_length, (caddr_t)dcfg + vpd_offset, vpd_length);
    ...
    if (hdr_length < required_hdr_length) {
        (void)memset((caddr_t)dcfg + hdr_length, 0, required_hdr_length - hdr_length);
    ...
    if ((rc = siena_nvram_partn_write(enp, dcfg_partn, 0, (caddr_t)dcfg, vpd_offset + vpd_length)) != 0)
        goto fail7;
