...
static void siena_phy_decode_cap(__in uint32_t mcdi_cap, __out uint32_t *maskp) {
    uint32_t mask;
...
static void siena_phy_decode_link_mode(__in efx_nic_t *enp, __in uint32_t link_flags, __in unsigned int speed,
                                       __in unsigned int fcntl, __out efx_link_mode_t *link_modep,
...
void siena_phy_link_ev(__in efx_nic_t *enp, __in efx_qword_t *eqp, __out efx_link_mode_t *link_modep) {
    ...
    siena_phy_decode_link_mode(enp, link_flags, speed, MCDI_EV_FIELD(eqp, LINKCHANGE_FCNTL), &link_mode, &fcntl);
...
__checkReturn efx_rc_t siena_phy_power(__in efx_nic_t *enp, __in boolean_t power) {
    efx_rc_t rc;
...
__checkReturn efx_rc_t siena_phy_get_link(__in efx_nic_t *enp, __out siena_link_state_t *slsp) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_LINK_IN_LEN, MC_CMD_GET_LINK_OUT_LEN);
    efx_rc_t rc;
    ...
    siena_phy_decode_cap(MCDI_OUT_DWORD(req, GET_LINK_OUT_CAP), &slsp->sls_adv_cap_mask);
    siena_phy_decode_cap(MCDI_OUT_DWORD(req, GET_LINK_OUT_LP_CAP), &slsp->sls_lp_cap_mask);

    siena_phy_decode_link_mode(enp, MCDI_OUT_DWORD(req, GET_LINK_OUT_FLAGS),
                               MCDI_OUT_DWORD(req, GET_LINK_OUT_LINK_SPEED), MCDI_OUT_DWORD(req, GET_LINK_OUT_FCNTL),
...
__checkReturn efx_rc_t siena_phy_reconfigure(__in efx_nic_t *enp) {
    efx_port_t *epp = &(enp->en_port);
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MAX(MC_CMD_SET_ID_LED_IN_LEN, MC_CMD_SET_LINK_IN_LEN),
    ...
    MCDI_IN_POPULATE_DWORD_10(
        req, SET_LINK_IN_CAP, PHY_CAP_10HDX, (cap_mask >> EFX_PHY_CAP_10HDX) & 0x1, PHY_CAP_10FDX,
        (cap_mask >> EFX_PHY_CAP_10FDX) & 0x1, PHY_CAP_100HDX, (cap_mask >> EFX_PHY_CAP_100HDX) & 0x1, PHY_CAP_100FDX,
        (cap_mask >> EFX_PHY_CAP_100FDX) & 0x1, PHY_CAP_1000HDX, (cap_mask >> EFX_PHY_CAP_1000HDX) & 0x1,
        PHY_CAP_1000FDX, (cap_mask >> EFX_PHY_CAP_1000FDX) & 0x1, PHY_CAP_10000FDX,
        (cap_mask >> EFX_PHY_CAP_10000FDX) & 0x1, PHY_CAP_PAUSE, (cap_mask >> EFX_PHY_CAP_PAUSE) & 0x1, PHY_CAP_ASYM,
        (cap_mask >> EFX_PHY_CAP_ASYM) & 0x1, PHY_CAP_AN, (cap_mask >> EFX_PHY_CAP_AN) & 0x1);
...
__checkReturn efx_rc_t siena_phy_verify(__in efx_nic_t *enp) {
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_GET_PHY_STATE_IN_LEN, MC_CMD_GET_PHY_STATE_OUT_LEN);
    uint32_t state;
    efx_rc_t rc;
...
__checkReturn efx_rc_t siena_phy_oui_get(__in efx_nic_t *enp, __out uint32_t *ouip) {
...
#define SIENA_SIMPLE_STAT_SET(_vmask, _esmp, _smask, _stat, _mc_record, _efx_record)                                   \
...
#define SIENA_SIMPLE_STAT_SET2(_vmask, _esmp, _smask, _stat, _record)                                                  \
    SIENA_SIMPLE_STAT_SET(_vmask, _esmp, _smask, _stat, MC_CMD_##_record, EFX_PHY_STAT_##_record)

void siena_phy_decode_stats(__in efx_nic_t *enp, __in uint32_t vmask, __in_opt efsys_mem_t *esmp,
                            __out_opt uint64_t *smaskp, __inout_ecount_opt(EFX_PHY_NSTATS) uint32_t *stat) {
    uint64_t smask = 0;
    ...
    if (vmask & (1 << MC_CMD_PMA_PMD_SIGNAL)) {
        smask |= ((1ULL << EFX_PHY_STAT_PMA_PMD_SIGNAL_A) | (1ULL << EFX_PHY_STAT_PMA_PMD_SIGNAL_B) |
                  (1ULL << EFX_PHY_STAT_PMA_PMD_SIGNAL_C) | (1ULL << EFX_PHY_STAT_PMA_PMD_SIGNAL_D));
    ...
    SIENA_SIMPLE_STAT_SET(vmask, esmp, smask, stat, MC_CMD_PMA_PMD_SNR_A, EFX_PHY_STAT_SNR_A);
    SIENA_SIMPLE_STAT_SET(vmask, esmp, smask, stat, MC_CMD_PMA_PMD_SNR_B, EFX_PHY_STAT_SNR_B);
    SIENA_SIMPLE_STAT_SET(vmask, esmp, smask, stat, MC_CMD_PMA_PMD_SNR_C, EFX_PHY_STAT_SNR_C);
    SIENA_SIMPLE_STAT_SET(vmask, esmp, smask, stat, MC_CMD_PMA_PMD_SNR_D, EFX_PHY_STAT_SNR_D);
    ...
    SIENA_SIMPLE_STAT_SET(vmask, esmp, smask, stat, MC_CMD_PHYXS_LINK_UP, EFX_PHY_STAT_PHY_XS_LINK_UP);
    SIENA_SIMPLE_STAT_SET(vmask, esmp, smask, stat, MC_CMD_PHYXS_RX_FAULT, EFX_PHY_STAT_PHY_XS_RX_FAULT);
    SIENA_SIMPLE_STAT_SET(vmask, esmp, smask, stat, MC_CMD_PHYXS_TX_FAULT, EFX_PHY_STAT_PHY_XS_TX_FAULT);
    SIENA_SIMPLE_STAT_SET(vmask, esmp, smask, stat, MC_CMD_PHYXS_ALIGN, EFX_PHY_STAT_PHY_XS_ALIGN);

    if (vmask & (1 << MC_CMD_PHYXS_SYNC)) {
        smask |= ((1 << EFX_PHY_STAT_PHY_XS_SYNC_A) | (1 << EFX_PHY_STAT_PHY_XS_SYNC_B) |
    ...
    SIENA_SIMPLE_STAT_SET(vmask, esmp, smask, stat, MC_CMD_CL22_LINK_UP, EFX_PHY_STAT_CL22EXT_LINK_UP);

    if (smaskp != NULL)
        *smaskp = smask;
}

__checkReturn efx_rc_t siena_phy_stats_update(__in efx_nic_t *enp, __in efsys_mem_t *esmp,
                                              __inout_ecount(EFX_PHY_NSTATS) uint32_t *stat) {
    efx_nic_cfg_t *encp = &(enp->en_nic_cfg);
    uint32_t vmask = encp->enc_mcdi_phy_stat_mask;
    uint64_t smask;
    efx_mcdi_req_t req;
    EFX_MCDI_DECLARE_BUF(payload, MC_CMD_PHY_STATS_IN_LEN, MC_CMD_PHY_STATS_OUT_DMA_LEN);
    efx_rc_t rc;
    ...
    MCDI_IN_SET_DWORD(req, PHY_STATS_IN_DMA_ADDR_LO, EFSYS_MEM_ADDR(esmp) & 0xffffffff);
...
__checkReturn efx_rc_t siena_phy_bist_start(__in efx_nic_t *enp, __in efx_bist_type_t type) {
    efx_rc_t rc;
...
static __checkReturn unsigned long siena_phy_sft9001_bist_status(__in uint16_t code) {
    switch (code) {
...
__checkReturn efx_rc_t siena_phy_bist_poll(__in efx_nic_t *enp, __in efx_bist_type_t type,
    ...
    if (result == MC_CMD_POLL_BIST_PASSED && encp->enc_phy_type == EFX_PHY_SFT9001B &&
        req.emr_out_length_used >= MC_CMD_POLL_BIST_OUT_SFT9001_LEN &&
        (type == EFX_BIST_TYPE_PHY_CABLE_SHORT || type == EFX_BIST_TYPE_PHY_CABLE_LONG)) {
        uint16_t word;

        if (count > EFX_BIST_PHY_CABLE_LENGTH_A) {
            if (valuesp != NULL)
                valuesp[EFX_BIST_PHY_CABLE_LENGTH_A] = MCDI_OUT_DWORD(req, POLL_BIST_OUT_SFT9001_CABLE_LENGTH_A);
            value_mask |= (1 << EFX_BIST_PHY_CABLE_LENGTH_A);
        }

        if (count > EFX_BIST_PHY_CABLE_LENGTH_B) {
            if (valuesp != NULL)
                valuesp[EFX_BIST_PHY_CABLE_LENGTH_B] = MCDI_OUT_DWORD(req, POLL_BIST_OUT_SFT9001_CABLE_LENGTH_B);
            value_mask |= (1 << EFX_BIST_PHY_CABLE_LENGTH_B);
        }

        if (count > EFX_BIST_PHY_CABLE_LENGTH_C) {
            if (valuesp != NULL)
                valuesp[EFX_BIST_PHY_CABLE_LENGTH_C] = MCDI_OUT_DWORD(req, POLL_BIST_OUT_SFT9001_CABLE_LENGTH_C);
            value_mask |= (1 << EFX_BIST_PHY_CABLE_LENGTH_C);
        }

        if (count > EFX_BIST_PHY_CABLE_LENGTH_D) {
            if (valuesp != NULL)
                valuesp[EFX_BIST_PHY_CABLE_LENGTH_D] = MCDI_OUT_DWORD(req, POLL_BIST_OUT_SFT9001_CABLE_LENGTH_D);
        ...
        if (count > EFX_BIST_PHY_CABLE_STATUS_A) {
            if (valuesp != NULL) {
                ...
                valuesp[EFX_BIST_PHY_CABLE_STATUS_A] = siena_phy_sft9001_bist_status(word);
            }
        ...
        if (count > EFX_BIST_PHY_CABLE_STATUS_B) {
            if (valuesp != NULL) {
                ...
                valuesp[EFX_BIST_PHY_CABLE_STATUS_B] = siena_phy_sft9001_bist_status(word);
            }
        ...
        if (count > EFX_BIST_PHY_CABLE_STATUS_C) {
            if (valuesp != NULL) {
                ...
                valuesp[EFX_BIST_PHY_CABLE_STATUS_C] = siena_phy_sft9001_bist_status(word);
            }
        ...
        if (count > EFX_BIST_PHY_CABLE_STATUS_D) {
            if (valuesp != NULL) {
                ...
                valuesp[EFX_BIST_PHY_CABLE_STATUS_D] = siena_phy_sft9001_bist_status(word);
            }
            value_mask |= (1 << EFX_BIST_PHY_CABLE_STATUS_D);
        }

    } else if (result == MC_CMD_POLL_BIST_FAILED && encp->enc_phy_type == EFX_PHY_QLX111V &&
               req.emr_out_length >= MC_CMD_POLL_BIST_OUT_MRSFP_LEN && count > EFX_BIST_FAULT_CODE) {
        if (valuesp != NULL)
            valuesp[EFX_BIST_FAULT_CODE] = MCDI_OUT_DWORD(req, POLL_BIST_OUT_MRSFP_TEST);
