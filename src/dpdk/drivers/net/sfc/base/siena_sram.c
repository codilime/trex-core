...
void siena_sram_init(__in efx_nic_t *enp) {
    ...
    tx_base = rx_base + (encp->enc_rxq_limit * EFX_RXQ_DC_NDESCS(EFX_RXQ_DC_SIZE));
...
__checkReturn efx_rc_t siena_sram_test(__in efx_nic_t *enp, __in efx_sram_pattern_fn_t func) {
    efx_oword_t oword;
    efx_qword_t qword;
    efx_qword_t verify;
    size_t rows;
