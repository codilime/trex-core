...
static __checkReturn efx_rc_t siena_vpd_get_static(__in efx_nic_t *enp, __in uint32_t partn,
    ...
    EFSYS_ASSERT(partn == MC_CMD_NVRAM_TYPE_STATIC_CFG_PORT0 || partn == MC_CMD_NVRAM_TYPE_STATIC_CFG_PORT1);
    ...
    if ((rc = siena_nvram_partn_read(enp, partn, 0, (caddr_t)scfg, SIENA_NVRAM_CHUNK)) != 0)
        goto fail4;

    /* Verify the magic number */
    if (EFX_DWORD_FIELD(scfg->magic, EFX_DWORD_0) != SIENA_MC_STATIC_CONFIG_MAGIC) {
        rc = EINVAL;
        goto fail5;
    }
    ...
    if (region > SIENA_NVRAM_CHUNK) {
        if ((rc = siena_nvram_partn_read(enp, partn, SIENA_NVRAM_CHUNK, (caddr_t)scfg + SIENA_NVRAM_CHUNK,
...
__checkReturn efx_rc_t siena_vpd_init(__in efx_nic_t *enp) {
    ...
    partn = (emip->emi_port == 1) ? MC_CMD_NVRAM_TYPE_STATIC_CFG_PORT0 : MC_CMD_NVRAM_TYPE_STATIC_CFG_PORT1;
...
__checkReturn efx_rc_t siena_vpd_size(__in efx_nic_t *enp, __out size_t *sizep) {
    efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
    uint32_t partn;
    efx_rc_t rc;
     ...
     */
    partn = (emip->emi_port == 1) ? MC_CMD_NVRAM_TYPE_DYNAMIC_CFG_PORT0 : MC_CMD_NVRAM_TYPE_DYNAMIC_CFG_PORT1;
...
__checkReturn efx_rc_t siena_vpd_read(__in efx_nic_t *enp, __out_bcount(size) caddr_t data, __in size_t size) {
    efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
    ...
    if ((rc = siena_nvram_get_dynamic_cfg(enp, dcfg_partn, B_TRUE, &dcfg, &dcfg_size)) != 0)
        goto fail1;
...
__checkReturn efx_rc_t siena_vpd_verify(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size) {
    efx_vpd_tag_t stag;
    efx_vpd_tag_t dtag;
    ...
    while (1) {
        if ((rc = efx_vpd_hunk_next(data, size, &dtag, &dkey, NULL, NULL, &dcont)) != 0)
            goto fail2;
        if (dcont == 0)
            break;
        ...
        _NOTE(CONSTANTCONDITION)
        while (1) {
            if ((rc = efx_vpd_hunk_next(enp->en_u.siena.enu_svpd, enp->en_u.siena.enu_svpd_length, &stag, &skey, NULL,
                                        NULL, &scont)) != 0)
                goto fail3;
            if (scont == 0)
                break;
...
__checkReturn efx_rc_t siena_vpd_reinit(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size) {
    boolean_t wantpid;
    efx_rc_t rc;
    ...
    if (enp->en_u.siena.enu_svpd_length == 0)
        ...
        rc = efx_vpd_hunk_get(enp->en_u.siena.enu_svpd, enp->en_u.siena.enu_svpd_length, EFX_VPD_ID, 0, &offset,
                              &length);
        if (rc == 0)
            wantpid = B_FALSE;
        else if (rc == ENOENT)
...
__checkReturn efx_rc_t siena_vpd_get(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size,
    ...
    if (enp->en_u.siena.enu_svpd_length > 0) {
        if ((rc = efx_vpd_hunk_get(enp->en_u.siena.enu_svpd, enp->en_u.siena.enu_svpd_length, evvp->evv_tag,
    ...
    if ((rc = efx_vpd_hunk_get(data, size, evvp->evv_tag, evvp->evv_keyword, &offset, &length)) != 0) {
        if (rc == ENOENT)
            return (rc);
...
__checkReturn efx_rc_t siena_vpd_set(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size,
...
__checkReturn efx_rc_t siena_vpd_next(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size,
                                      __out efx_vpd_value_t *evvp, __inout unsigned int *contp) {
    _NOTE(ARGUNUSED(enp, data, size, evvp, contp))

    return (ENOTSUP);
}

__checkReturn efx_rc_t siena_vpd_write(__in efx_nic_t *enp, __in_bcount(size) caddr_t data, __in size_t size) {
    efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
    siena_mc_dynamic_config_hdr_t *dcfg = NULL;
    unsigned int vpd_offset;
    unsigned int dcfg_partn;
    ...
    if ((rc = siena_nvram_get_dynamic_cfg(enp, dcfg_partn, B_FALSE, &dcfg, &dcfg_size)) != 0)
        goto fail4;
...
void siena_vpd_fini(__in efx_nic_t *enp) {
    ...
    if (enp->en_u.siena.enu_svpd_length > 0) {
        EFSYS_KMEM_FREE(enp->en_esip, enp->en_u.siena.enu_svpd_length, enp->en_u.siena.enu_svpd);
