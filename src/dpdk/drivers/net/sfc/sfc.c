...
int sfc_dma_alloc(const struct sfc_adapter *sa, const char *name, uint16_t id, size_t len, int socket_id,
                  efsys_mem_t *esmp) {
    const struct rte_memzone *mz;

    sfc_log_init(sa, "name=%s id=%u len=%lu socket_id=%d", name, id, len, socket_id);

    mz = rte_eth_dma_zone_reserve(sa->eth_dev, name, id, len, sysconf(_SC_PAGESIZE), socket_id);
    if (mz == NULL) {
        sfc_err(sa, "cannot reserve DMA zone for %s:%u %#x@%d: %s", name, (unsigned int)id, (unsigned int)len,
...
static uint32_t sfc_phy_cap_from_link_speeds(uint32_t speeds) {
    ...
    if (~speeds & ETH_LINK_SPEED_FIXED) {
        ...
        if (speeds == ETH_LINK_SPEED_AUTONEG)
            phy_caps |= (1 << EFX_PHY_CAP_1000FDX) | (1 << EFX_PHY_CAP_10000FDX) | (1 << EFX_PHY_CAP_25000FDX) |
                        (1 << EFX_PHY_CAP_40000FDX) | (1 << EFX_PHY_CAP_50000FDX) | (1 << EFX_PHY_CAP_100000FDX);
    }
...
static int sfc_check_conf(struct sfc_adapter *sa) {
    ...
    sa->port.phy_adv_cap = sfc_phy_cap_from_link_speeds(conf->link_speeds) & sa->port.phy_adv_cap_mask;
    if ((sa->port.phy_adv_cap & ~(1 << EFX_PHY_CAP_AN)) == 0) {
        sfc_err(sa, "No link speeds from mask %#x are supported", conf->link_speeds);
        rc = EINVAL;
    }
    ...
    if ((conf->intr_conf.lsc != 0) && (sa->intr.type != EFX_INTR_LINE) && (sa->intr.type != EFX_INTR_MESSAGE)) {
...
static int sfc_estimate_resource_limits(struct sfc_adapter *sa) {
    ...
    limits.edl_min_evq_count = 1 + limits.edl_min_rxq_count + limits.edl_min_txq_count;
    ...
    limits.edl_max_rxq_count = MIN(encp->enc_rxq_limit, (limits.edl_max_evq_count - 1) / 2);
    SFC_ASSERT(limits.edl_max_rxq_count >= limits.edl_min_rxq_count);

    limits.edl_max_txq_count = MIN(encp->enc_txq_limit, limits.edl_max_evq_count - 1 - limits.edl_max_rxq_count);

    if (sa->tso)
        limits.edl_max_txq_count =
            MIN(limits.edl_max_txq_count, encp->enc_fw_assisted_tso_v2_n_contexts / encp->enc_hw_pf_count);
    ...
    rc = efx_nic_get_vi_pool(sa->nic, &evq_allocated, &rxq_allocated, &txq_allocated);
    if (rc != 0)
...
static int sfc_set_drv_limits(struct sfc_adapter *sa) {
    ...
    lim.edl_min_evq_count = lim.edl_max_evq_count = 1 + data->nb_rx_queues + data->nb_tx_queues;
...
static int sfc_set_fw_subvariant(struct sfc_adapter *sa) {
    ...
    if (tx_offloads & (DEV_TX_OFFLOAD_IPV4_CKSUM | DEV_TX_OFFLOAD_TCP_CKSUM | DEV_TX_OFFLOAD_UDP_CKSUM |
    ...
    sfc_info(sa, "FW subvariant is %u vs required %u", cur_fw_subvariant, req_fw_subvariant);
...
int sfc_start(struct sfc_adapter *sa) {
    ...
    do {
        rc = sfc_try_start(sa);
    } while ((--start_tries > 0) && (rc == EIO || rc == EAGAIN || rc == ENOENT || rc == EINVAL));
...
int sfc_configure(struct sfc_adapter *sa) {
    ...
    SFC_ASSERT(sa->state == SFC_ADAPTER_INITIALIZED || sa->state == SFC_ADAPTER_CONFIGURED);
...
static int sfc_mem_bar_init(struct sfc_adapter *sa, unsigned int membar) {
...
static int sfc_kvarg_fv_variant_handler(__rte_unused const char *key, const char *value_str, void *opaque) {
...
static int sfc_kvarg_rxd_wait_timeout_ns(struct sfc_adapter *sa) {
    ...
    rc = sfc_kvargs_process(sa, SFC_KVARG_RXD_WAIT_TIMEOUT_NS, sfc_kvarg_long_handler, &value);
    if (rc != 0)
        return rc;

    if (value < 0 || (unsigned long)value > EFX_RXQ_ES_SUPER_BUFFER_HOL_BLOCK_MAX) {
        sfc_err(sa,
                "wrong '" SFC_KVARG_RXD_WAIT_TIMEOUT_NS "' "
                "was set (%ld);",
                value);
        sfc_err(sa, "it must not be less than 0 or greater than %u", EFX_RXQ_ES_SUPER_BUFFER_HOL_BLOCK_MAX);
        return EINVAL;
    }
...
static int sfc_nic_probe(struct sfc_adapter *sa) {
    ...
    rc = sfc_kvargs_process(sa, SFC_KVARG_FW_VARIANT, sfc_kvarg_fv_variant_handler, &preferred_efv);
    if (rc != 0) {
    ...
    if (preferred_efv != EFX_FW_VARIANT_DONT_CARE && preferred_efv != efv) {
        sfc_warn(sa, "FW variant has not changed to the requested %s", sfc_fw_variant2str(preferred_efv));
    }
...
int sfc_probe(struct sfc_adapter *sa) {
    ...
    rc = efx_family(pci_dev->id.vendor_id, pci_dev->id.device_id, &sa->family, &membar);
    if (rc != 0)
        goto fail_family;
    ...
    rc = efx_nic_create(sa->family, (efsys_identifier_t *)sa, &sa->mem_bar, &sa->nic_lock, &enp);
    if (rc != 0)
...
uint32_t sfc_register_logtype(const struct rte_pci_addr *pci_addr, const char *lt_prefix_str, uint32_t ll_default) {
    ...
    rte_pci_device_name(pci_addr, lt_str + lt_prefix_str_size, lt_str_size_max - lt_prefix_str_size);
