...
#define sfc_ef10_essb_rx_err(dpq, ...) SFC_DP_LOG(SFC_KVARG_DATAPATH_EF10_ESSB, ERR, dpq, __VA_ARGS__)

#define sfc_ef10_essb_rx_info(dpq, ...) SFC_DP_LOG(SFC_KVARG_DATAPATH_EF10_ESSB, INFO, dpq, __VA_ARGS__)
...
#define SFC_EF10_ESSB_RXQ_LIMIT(_nevs)                                                                                 \
    ((_nevs)-1 /* head must not step on tail */ - (SFC_EF10_EV_PER_CACHE_LINE - 1) /* max unused EvQ entries */ -      \
     1 /* Rx error */ - 1 /* flush */)
...
static inline struct sfc_ef10_essb_rxq *sfc_ef10_essb_rxq_by_dp_rxq(struct sfc_dp_rxq *dp_rxq) {
    return container_of(dp_rxq, struct sfc_ef10_essb_rxq, dp);
}

static struct rte_mbuf *sfc_ef10_essb_next_mbuf(const struct sfc_ef10_essb_rxq *rxq, struct rte_mbuf *mbuf) {
    struct rte_mbuf *m;
...
static struct rte_mbuf *sfc_ef10_essb_mbuf_by_index(const struct sfc_ef10_essb_rxq *rxq, struct rte_mbuf *mbuf,
                                                    unsigned int idx) {
    struct rte_mbuf *m;
...
static struct rte_mbuf *sfc_ef10_essb_maybe_next_completed(struct sfc_ef10_essb_rxq *rxq) {
    const struct sfc_ef10_essb_rx_sw_desc *rxd;

    if (rxq->left_in_completed != 0) {
        rxd = &rxq->sw_ring[rxq->completed & rxq->rxq_ptr_mask];
        return sfc_ef10_essb_mbuf_by_index(rxq, rxd->first_mbuf, rxq->block_size - rxq->left_in_completed);
    } else {
        rxq->completed++;
        rxd = &rxq->sw_ring[rxq->completed & rxq->rxq_ptr_mask];
...
static void sfc_ef10_essb_rx_qrefill(struct sfc_ef10_essb_rxq *rxq) {
    ...
    do {
        ...
        if (unlikely(rte_mempool_get_contig_blocks(rxq->refill_mb_pool, mbuf_blocks, RTE_DIM(mbuf_blocks)) < 0)) {
        ...
        for (i = 0, id = added & rxq_ptr_mask; i < RTE_DIM(mbuf_blocks); ++i, ++id) {
            ...
            EFX_POPULATE_QWORD_2(rxq->rxq_hw_ring[id], ESF_DZ_RX_KER_BYTE_CNT, SFC_EF10_ESSB_RX_FAKE_BUF_SIZE,
                                 ESF_DZ_RX_KER_BUF_ADDR, rte_mbuf_data_iova_default(m));
        }
...
static bool sfc_ef10_essb_rx_event_get(struct sfc_ef10_essb_rxq *rxq, efx_qword_t *rx_ev) {
    *rx_ev = rxq->evq_hw_ring[rxq->evq_read_ptr & rxq->evq_ptr_mask];

    if (!sfc_ef10_ev_present(*rx_ev))
        return false;

    if (unlikely(EFX_QWORD_FIELD(*rx_ev, FSF_AZ_EV_CODE) != FSE_AZ_EV_CODE_RX_EV)) {
        /*
         * Do not move read_ptr to keep the event for exception
         * handling
         */
        rxq->flags |= SFC_EF10_ESSB_RXQ_EXCEPTION;
        sfc_ef10_essb_rx_err(&rxq->dp.dpq, "RxQ exception at EvQ read ptr %#x", rxq->evq_read_ptr);
        return false;
    }
...
static void sfc_ef10_essb_rx_process_ev(struct sfc_ef10_essb_rxq *rxq, efx_qword_t rx_ev) {
    unsigned int ready;

    ready = (EFX_QWORD_FIELD(rx_ev, ESF_DZ_RX_DSC_PTR_LBITS) - rxq->bufs_ptr) & EFX_MASK32(ESF_DZ_RX_DSC_PTR_LBITS);
    ...
    do {
        ...
        m = sfc_ef10_essb_mbuf_by_index(rxq, rxd->first_mbuf, rxq->block_size - rxq->left_in_pending);
...
static unsigned int sfc_ef10_essb_rx_get_pending(struct sfc_ef10_essb_rxq *rxq, struct rte_mbuf **rx_pkts,
                                                 uint16_t nb_pkts) {
    ...
    while ((todo_bufs = RTE_MIN(nb_pkts - n_rx_pkts, rxq->bufs_pending)) > 0) {
        ...
        do {
            ...
            /* Parse pseudo-header */
            qwordp = (const efx_qword_t *)((uint8_t *)m->buf_addr + RTE_PKTMBUF_HEADROOM);
            ...
            m->ol_flags |= (PKT_RX_RSS_HASH * !!EFX_TEST_QWORD_BIT(*qwordp, ES_EZ_ESSB_RX_PREFIX_HASH_VALID_LBN)) |
                           (PKT_RX_FDIR_ID * !!EFX_TEST_QWORD_BIT(*qwordp, ES_EZ_ESSB_RX_PREFIX_MARK_VALID_LBN)) |
                           (PKT_RX_FDIR * !!EFX_TEST_QWORD_BIT(*qwordp, ES_EZ_ESSB_RX_PREFIX_MATCH_FLAG_LBN));
...
static uint16_t sfc_ef10_essb_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    struct sfc_ef10_essb_rxq *rxq = sfc_ef10_essb_rxq_by_dp_rxq(rx_queue);
    const unsigned int evq_old_read_ptr = rxq->evq_read_ptr;
    uint16_t n_rx_pkts;
    efx_qword_t rx_ev;

    if (unlikely(rxq->flags & (SFC_EF10_ESSB_RXQ_NOT_RUNNING | SFC_EF10_ESSB_RXQ_EXCEPTION)))
        return 0;
    ...
    while (n_rx_pkts != nb_pkts && sfc_ef10_essb_rx_event_get(rxq, &rx_ev)) {
        ...
        n_rx_pkts += sfc_ef10_essb_rx_get_pending(rxq, rx_pkts + n_rx_pkts, nb_pkts - n_rx_pkts);
    }

    sfc_ef10_ev_qclear(rxq->evq_hw_ring, rxq->evq_ptr_mask, evq_old_read_ptr, rxq->evq_read_ptr);
...
static int sfc_ef10_essb_rx_qdesc_status(struct sfc_dp_rxq *dp_rxq, uint16_t offset) {
    struct sfc_ef10_essb_rxq *rxq = sfc_ef10_essb_rxq_by_dp_rxq(dp_rxq);
    ...
    if (offset < (rxq->added - rxq->completed) * rxq->block_size + rxq->left_in_completed - rxq->block_size)
...
static int sfc_ef10_essb_rx_qsize_up_rings(uint16_t nb_rx_desc, struct sfc_dp_rx_hw_limits *limits,
                                           struct rte_mempool *mb_pool, unsigned int *rxq_entries,
     ...
     * empty.
     */
    nb_hw_rx_desc = RTE_MAX(SFC_DIV_ROUND_UP(nb_rx_desc, mp_info.contig_block_size), SFC_EF10_RX_WPTR_ALIGN + 1);
    ...
    max_events = RTE_ALIGN_FLOOR(nb_hw_rx_desc, SFC_EF10_RX_WPTR_ALIGN) * mp_info.contig_block_size +
                 (SFC_EF10_EV_PER_CACHE_LINE - 1) /* max unused EvQ entries */ + 1 /* Rx error */ + 1 /* flush */ +
                 1 /* head-tail space */;
    ...
    *evq_entries = RTE_MIN(*evq_entries, limits->evq_max_entries);
     ...
     */
    *rxq_max_fill_level = RTE_MIN(nb_rx_desc, SFC_EF10_ESSB_RXQ_LIMIT(*evq_entries));
    return 0;
}

static sfc_dp_rx_qcreate_t sfc_ef10_essb_rx_qcreate;
static int sfc_ef10_essb_rx_qcreate(uint16_t port_id, uint16_t queue_id, const struct rte_pci_addr *pci_addr,
                                    int socket_id, const struct sfc_dp_rx_qcreate_info *info,
    ...
    rc = ENOMEM;
    rxq = rte_zmalloc_socket("sfc-ef10-rxq", sizeof(*rxq), RTE_CACHE_LINE_SIZE, socket_id);
    if (rxq == NULL)
        goto fail_rxq_alloc;
    ...
    rc = ENOMEM;
    rxq->sw_ring = rte_calloc_socket("sfc-ef10-rxq-sw_ring", info->rxq_entries, sizeof(*rxq->sw_ring),
    ...
    rxq->refill_threshold = RTE_MAX(info->refill_threshold / mp_info.contig_block_size, SFC_EF10_RX_WPTR_ALIGN);
    rxq->refill_mb_pool = mp;
    rxq->rxq_hw_ring = info->rxq_hw_ring;

    rxq->doorbell =
        (volatile uint8_t *)info->mem_bar + ER_DZ_RX_DESC_UPD_REG_OFST + (info->hw_index << info->vi_window_shift);

    sfc_ef10_essb_rx_info(&rxq->dp.dpq, "block size is %u, buf stride is %u", rxq->block_size, rxq->buf_stride);
    sfc_ef10_essb_rx_info(&rxq->dp.dpq,
                          "max fill level is %u descs (%u bufs), "
                          "refill threashold %u descs (%u bufs)",
                          rxq->max_fill_level, rxq->max_fill_level * rxq->block_size, rxq->refill_threshold,
...
static int sfc_ef10_essb_rx_qstart(struct sfc_dp_rxq *dp_rxq, unsigned int evq_read_ptr) {
    struct sfc_ef10_essb_rxq *rxq = sfc_ef10_essb_rxq_by_dp_rxq(dp_rxq);
...
static void sfc_ef10_essb_rx_qstop(struct sfc_dp_rxq *dp_rxq, unsigned int *evq_read_ptr) {
    struct sfc_ef10_essb_rxq *rxq = sfc_ef10_essb_rxq_by_dp_rxq(dp_rxq);
...
static bool sfc_ef10_essb_rx_qrx_ev(struct sfc_dp_rxq *dp_rxq, __rte_unused unsigned int id) {
...
static void sfc_ef10_essb_rx_qpurge(struct sfc_dp_rxq *dp_rxq) {
    ...
    for (i = rxq->completed; i != rxq->added; ++i) {
        ...
        m = sfc_ef10_essb_mbuf_by_index(rxq, rxd->first_mbuf, rxq->block_size - rxq->left_in_completed);
    ...
    .dp =
        {
            ...
            .type = SFC_DP_RX,
            .hw_fw_caps = SFC_DP_HW_FW_CAP_EF10 | SFC_DP_HW_FW_CAP_RX_ES_SUPER_BUFFER,
        },
    .features = SFC_DP_RX_FEAT_FLOW_FLAG | SFC_DP_RX_FEAT_FLOW_MARK | SFC_DP_RX_FEAT_CHECKSUM,
