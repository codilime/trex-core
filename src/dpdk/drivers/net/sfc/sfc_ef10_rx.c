...
#define sfc_ef10_rx_err(dpq, ...) SFC_DP_LOG(SFC_KVARG_DATAPATH_EF10, ERR, dpq, __VA_ARGS__)
...
static inline struct sfc_ef10_rxq *sfc_ef10_rxq_by_dp_rxq(struct sfc_dp_rxq *dp_rxq) {
...
static void sfc_ef10_rx_qrefill(struct sfc_ef10_rxq *rxq) {
    ...
    do {
        ...
        if (unlikely(rte_mempool_get_bulk(rxq->refill_mb_pool, objs, RTE_DIM(objs)) < 0)) {
            struct rte_eth_dev_data *dev_data = rte_eth_devices[rxq->dp.dpq.port_id].data;
        ...
        for (i = 0, id = added & ptr_mask; i < RTE_DIM(objs); ++i, ++id) {
            ...
            EFX_POPULATE_QWORD_2(rxq->rxq_hw_ring[id], ESF_DZ_RX_KER_BYTE_CNT, buf_size, ESF_DZ_RX_KER_BUF_ADDR,
                                 phys_addr);
        }
...
static void sfc_ef10_rx_prefetch_next(struct sfc_ef10_rxq *rxq, unsigned int next_id) {
...
static struct rte_mbuf **sfc_ef10_rx_pending(struct sfc_ef10_rxq *rxq, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
...
static struct rte_mbuf **sfc_ef10_rx_process_event(struct sfc_ef10_rxq *rxq, efx_qword_t rx_ev,
                                                   struct rte_mbuf **rx_pkts, struct rte_mbuf **const rx_pkts_end) {
    const unsigned int ptr_mask = rxq->ptr_mask;
    ...
    ready = (EFX_QWORD_FIELD(rx_ev, ESF_DZ_RX_DSC_PTR_LBITS) - pending) & EFX_MASK32(ESF_DZ_RX_DSC_PTR_LBITS);
    ...
    if (rx_ev.eq_u64[0] & rte_cpu_to_le_64((1ull << ESF_DZ_RX_ECC_ERR_LBN) | (1ull << ESF_DZ_RX_ECRC_ERR_LBN))) {
    ...
    sfc_ef10_rx_ev_to_offloads(rx_ev, m, (rxq->flags & SFC_EF10_RXQ_RSS_HASH) ? ~0ull : ~PKT_RX_RSS_HASH);
...
static bool sfc_ef10_rx_get_event(struct sfc_ef10_rxq *rxq, efx_qword_t *rx_ev) {
    *rx_ev = rxq->evq_hw_ring[rxq->evq_read_ptr & rxq->ptr_mask];
    ...
    if (unlikely(EFX_QWORD_FIELD(*rx_ev, FSF_AZ_EV_CODE) != FSE_AZ_EV_CODE_RX_EV)) {
        ...
        sfc_ef10_rx_err(&rxq->dp.dpq, "RxQ exception at EvQ read ptr %#x", rxq->evq_read_ptr);
        return false;
    }
...
static uint16_t sfc_ef10_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    ...
    if (unlikely(rxq->flags & (SFC_EF10_RXQ_NOT_RUNNING | SFC_EF10_RXQ_EXCEPTION)))
        goto done;
    ...
    sfc_ef10_ev_qclear(rxq->evq_hw_ring, rxq->ptr_mask, evq_old_read_ptr, rxq->evq_read_ptr);
...
const uint32_t *sfc_ef10_supported_ptypes_get(uint32_t tunnel_encaps) {
    static const uint32_t ef10_native_ptypes[] = {RTE_PTYPE_L2_ETHER,
                                                  ...
                                                  RTE_PTYPE_L4_UDP,
                                                  RTE_PTYPE_UNKNOWN};
    static const uint32_t ef10_overlay_ptypes[] = {RTE_PTYPE_L2_ETHER,
                                                   RTE_PTYPE_L2_ETHER_ARP,
                                                   RTE_PTYPE_L2_ETHER_VLAN,
                                                   RTE_PTYPE_L2_ETHER_QINQ,
                                                   RTE_PTYPE_L3_IPV4_EXT_UNKNOWN,
    ...
    case (1u << EFX_TUNNEL_PROTOCOL_VXLAN | 1u << EFX_TUNNEL_PROTOCOL_GENEVE | 1u << EFX_TUNNEL_PROTOCOL_NVGRE):
        return ef10_overlay_ptypes;
    default:
        SFC_GENERIC_LOG(ERR, "Unexpected set of supported tunnel encapsulations: %#x", tunnel_encaps);
        /* FALLTHROUGH */
    case 0:
...
static int sfc_ef10_rx_qdesc_status(struct sfc_dp_rxq *dp_rxq, uint16_t offset) {
    struct sfc_ef10_rxq *rxq = sfc_ef10_rxq_by_dp_rxq(dp_rxq);
...
static int sfc_ef10_rx_qsize_up_rings(uint16_t nb_rx_desc, struct sfc_dp_rx_hw_limits *limits,
...
static uint64_t sfc_ef10_mk_mbuf_rearm_data(uint16_t port_id, uint16_t prefix_size) {
    struct rte_mbuf m;
...
static int sfc_ef10_rx_qcreate(uint16_t port_id, uint16_t queue_id, const struct rte_pci_addr *pci_addr, int socket_id,
...
static int sfc_ef10_rx_qstart(struct sfc_dp_rxq *dp_rxq, unsigned int evq_read_ptr) {
    struct sfc_ef10_rxq *rxq = sfc_ef10_rxq_by_dp_rxq(dp_rxq);
...
static void sfc_ef10_rx_qstop(struct sfc_dp_rxq *dp_rxq, unsigned int *evq_read_ptr) {
    struct sfc_ef10_rxq *rxq = sfc_ef10_rxq_by_dp_rxq(dp_rxq);
...
static bool sfc_ef10_rx_qrx_ev(struct sfc_dp_rxq *dp_rxq, __rte_unused unsigned int id) {
