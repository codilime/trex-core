...
#define sfc_ef10_tx_err(dpq, ...) SFC_DP_LOG(SFC_KVARG_DATAPATH_EF10, ERR, dpq, __VA_ARGS__)
...
#define SFC_EF10_TXQ_LIMIT(_ndesc)                                                                                     \
    ((_ndesc)-1 /* head must not step on tail */ - (SFC_EF10_EV_PER_CACHE_LINE - 1) /* max unused EvQ entries */ -     \
     1 /* Rx error */ - 1 /* flush */)
...
static inline struct sfc_ef10_txq *sfc_ef10_txq_by_dp_txq(struct sfc_dp_txq *dp_txq) {
    return container_of(dp_txq, struct sfc_ef10_txq, dp);
}

static bool sfc_ef10_tx_get_event(struct sfc_ef10_txq *txq, efx_qword_t *tx_ev) {
    ...
    *tx_ev = evq_hw_ring[txq->evq_read_ptr & txq->ptr_mask];
    ...
    if (unlikely(EFX_QWORD_FIELD(*tx_ev, FSF_AZ_EV_CODE) != FSE_AZ_EV_CODE_TX_EV)) {
        /*
         * Do not move read_ptr to keep the event for exception
         * handling by the control path.
         */
        txq->flags |= SFC_EF10_TXQ_EXCEPTION;
        sfc_ef10_tx_err(&txq->dp.dpq, "TxQ exception at EvQ read ptr %#x", txq->evq_read_ptr);
        return false;
    }
...
static void sfc_ef10_tx_reap(struct sfc_ef10_txq *txq) {
    ...
    sfc_ef10_ev_qclear(txq->evq_hw_ring, ptr_mask, old_read_ptr, txq->evq_read_ptr);
}

static void sfc_ef10_tx_qdesc_dma_create(rte_iova_t addr, uint16_t size, bool eop, efx_qword_t *edp) {
    EFX_POPULATE_QWORD_4(*edp, ESF_DZ_TX_KER_TYPE, 0, ESF_DZ_TX_KER_CONT, !eop, ESF_DZ_TX_KER_BYTE_CNT, size,
                         ESF_DZ_TX_KER_BUF_ADDR, addr);
}

static void sfc_ef10_tx_qdesc_tso2_create(struct sfc_ef10_txq *const txq, unsigned int added, uint16_t ipv4_id,
                                          uint16_t outer_ipv4_id, uint32_t tcp_seq, uint16_t tcp_mss) {
    EFX_POPULATE_QWORD_5(txq->txq_hw_ring[added & txq->ptr_mask], ESF_DZ_TX_DESC_IS_OPT, 1, ESF_DZ_TX_OPTION_TYPE,
                         ESE_DZ_TX_OPTION_DESC_TSO, ESF_DZ_TX_TSO_OPTION_TYPE, ESE_DZ_TX_TSO_OPTION_DESC_FATSO2A,
                         ESF_DZ_TX_TSO_IP_ID, ipv4_id, ESF_DZ_TX_TSO_TCP_SEQNO, tcp_seq);
    EFX_POPULATE_QWORD_5(txq->txq_hw_ring[(added + 1) & txq->ptr_mask], ESF_DZ_TX_DESC_IS_OPT, 1, ESF_DZ_TX_OPTION_TYPE,
                         ESE_DZ_TX_OPTION_DESC_TSO, ESF_DZ_TX_TSO_OPTION_TYPE, ESE_DZ_TX_TSO_OPTION_DESC_FATSO2B,
                         ESF_DZ_TX_TSO_TCP_MSS, tcp_mss, ESF_DZ_TX_TSO_OUTER_IPID, outer_ipv4_id);
}

static inline void sfc_ef10_tx_qpush(struct sfc_ef10_txq *txq, unsigned int added, unsigned int pushed) {
    efx_qword_t desc;
    efx_oword_t oword;
    ...
    EFX_POPULATE_OWORD_3(oword, ERF_DZ_TX_DESC_WPTR, added & txq->ptr_mask, ERF_DZ_TX_DESC_HWORD,
                         EFX_QWORD_FIELD(desc, EFX_DWORD_1), ERF_DZ_TX_DESC_LWORD, EFX_QWORD_FIELD(desc, EFX_DWORD_0));
...
#define SFC_MBUF_SEG_LEN_MAX UINT16_MAX
     ...
     */
    extra_descs_per_seg = (SFC_MBUF_SEG_LEN_MAX - 1) / SFC_EF10_TX_DMA_DESC_LEN_MAX;
...
#define SFC_MBUF_PKT_LEN_MAX UINT32_MAX
     ...
     */
    extra_descs_per_pkt =
        (RTE_MIN((unsigned int)EFX_MAC_PDU_MAX, SFC_MBUF_PKT_LEN_MAX) - 1) / SFC_EF10_TX_DMA_DESC_LEN_MAX;

    return m->nb_segs + RTE_MIN(m->nb_segs * extra_descs_per_seg, extra_descs_per_pkt);
}

static bool sfc_ef10_try_reap(struct sfc_ef10_txq *const txq, unsigned int added, unsigned int needed_desc,
...
static uint16_t sfc_ef10_prepare_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
...
#endif
        ret = sfc_dp_tx_prepare_pkt(m, txq->tso_tcp_header_offset_limit, txq->max_fill_level,
...
static int sfc_ef10_xmit_tso_pkt(struct sfc_ef10_txq *const txq, struct rte_mbuf *m_seg, unsigned int *added,
                                 unsigned int *dma_desc_space, bool *reap_done) {
    size_t iph_off =
        ((m_seg->ol_flags & PKT_TX_TUNNEL_MASK) ? m_seg->outer_l2_len + m_seg->outer_l3_len : 0) + m_seg->l2_len;
     ...
     * several descriptors.
     */
    needed_desc = m_seg->nb_segs + (unsigned int)SFC_EF10_TSO_OPT_DESCS_NUM + (unsigned int)SFC_EF10_TSO_HDR_DESCS_NUM;

    if (needed_desc > *dma_desc_space && !sfc_ef10_try_reap(txq, pkt_start, needed_desc, dma_desc_space, reap_done)) {
        /*
         ...
         * segment descriptor.
         */
        if (*dma_desc_space < SFC_EF10_TSO_OPT_DESCS_NUM + SFC_EF10_TSO_HDR_DESCS_NUM + 1)
            return EMSGSIZE;
    }
    ...
    if (rte_pktmbuf_data_len(m_seg) >= header_len) {
        ...
        if (rte_pktmbuf_data_len(m_seg) == header_len) {
             ...
             */
            txq->sw_ring[(pkt_start + SFC_EF10_TSO_OPT_DESCS_NUM) & txq->ptr_mask].mbuf = m_seg;
            m_seg = m_seg->next;
            in_off = 0;
    ...
    sfc_ef10_tx_qdesc_tso2_create(txq, *added, packet_id, outer_packet_id, sent_seq, first_m_seg->tso_segsz);
    (*added) += SFC_EF10_TSO_OPT_DESCS_NUM;

    sfc_ef10_tx_qdesc_dma_create(hdr_iova, header_len, false, &txq->txq_hw_ring[(*added) & txq->ptr_mask]);
    (*added)++;
    ...
    do {
        ...
        do {
            ...
            if (*added - pkt_start == *dma_desc_space && !eop &&
                !sfc_ef10_try_reap(txq, pkt_start, needed_desc, dma_desc_space, reap_done)) {
                struct rte_mbuf *m;
            ...
            sfc_ef10_tx_qdesc_dma_create(frag_addr, frag_len, eop, &txq->txq_hw_ring[id]);
...
static uint16_t sfc_ef10_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    struct sfc_ef10_txq *const txq = sfc_ef10_txq_by_dp_txq(tx_queue);
    unsigned int added;
    ...
    if (unlikely(txq->flags & (SFC_EF10_TXQ_NOT_RUNNING | SFC_EF10_TXQ_EXCEPTION)))
        return 0;
    ...
    for (pktp = &tx_pkts[0], pktp_end = &tx_pkts[nb_pkts]; pktp != pktp_end; ++pktp) {
        ...
        if (m_seg->ol_flags & PKT_TX_TCP_SEG) {
            ...
            rc = sfc_ef10_xmit_tso_pkt(txq, m_seg, &added, &dma_desc_space, &reap_done);
            if (rc != 0) {
                added = pkt_start;
        ...
        do {
            ...
            sfc_ef10_tx_qdesc_dma_create(seg_addr, seg_len, (pkt_len == 0), &txq->txq_hw_ring[id]);
...
#ifdef RTE_LIBRTE_SFC_EFX_DEBUG
static uint16_t sfc_ef10_simple_prepare_pkts(__rte_unused void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    uint16_t i;
    ...
    for (i = 0; i < nb_pkts; i++) {
        ...
        if (unlikely(m->ol_flags & (PKT_TX_TCP_SEG | PKT_TX_VLAN_PKT))) {
            rte_errno = ENOTSUP;
            break;
        }
...
static uint16_t sfc_ef10_simple_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    struct sfc_ef10_txq *const txq = sfc_ef10_txq_by_dp_txq(tx_queue);
    unsigned int ptr_mask;
    unsigned int added;
    unsigned int dma_desc_space;
    ...
    for (pktp = &tx_pkts[0]; pktp != pktp_end; ++pktp) {
        ...
        sfc_ef10_tx_qdesc_dma_create(rte_mbuf_data_iova(pkt), rte_pktmbuf_data_len(pkt), true, &txq->txq_hw_ring[id]);
...
static int sfc_ef10_tx_qsize_up_rings(uint16_t nb_tx_desc, struct sfc_dp_tx_hw_limits *limits,
...
static int sfc_ef10_tx_qcreate(uint16_t port_id, uint16_t queue_id, const struct rte_pci_addr *pci_addr, int socket_id,
                               const struct sfc_dp_tx_qcreate_info *info, struct sfc_dp_txq **dp_txqp) {
    ...
    rc = ENOMEM;
    txq = rte_zmalloc_socket("sfc-ef10-txq", sizeof(*txq), RTE_CACHE_LINE_SIZE, socket_id);
    if (txq == NULL)
        goto fail_txq_alloc;
    ...
    rc = ENOMEM;
    txq->sw_ring = rte_calloc_socket("sfc-ef10-txq-sw_ring", info->txq_entries, sizeof(*txq->sw_ring),
                                     RTE_CACHE_LINE_SIZE, socket_id);
    if (txq->sw_ring == NULL)
        goto fail_sw_ring_alloc;

    if (info->offloads & (DEV_TX_OFFLOAD_TCP_TSO | DEV_TX_OFFLOAD_VXLAN_TNL_TSO | DEV_TX_OFFLOAD_GENEVE_TNL_TSO)) {
        txq->tsoh =
            rte_calloc_socket("sfc-ef10-txq-tsoh", info->txq_entries, SFC_TSOH_STD_LEN, RTE_CACHE_LINE_SIZE, socket_id);
        if (txq->tsoh == NULL)
    ...
    txq->doorbell =
        (volatile uint8_t *)info->mem_bar + ER_DZ_TX_DESC_UPD_REG_OFST + (info->hw_index << info->vi_window_shift);
...
static int sfc_ef10_tx_qstart(struct sfc_dp_txq *dp_txq, unsigned int evq_read_ptr, unsigned int txq_desc_index) {
    struct sfc_ef10_txq *txq = sfc_ef10_txq_by_dp_txq(dp_txq);
...
static void sfc_ef10_tx_qstop(struct sfc_dp_txq *dp_txq, unsigned int *evq_read_ptr) {
    struct sfc_ef10_txq *txq = sfc_ef10_txq_by_dp_txq(dp_txq);
...
static bool sfc_ef10_tx_qtx_ev(struct sfc_dp_txq *dp_txq, __rte_unused unsigned int id) {
...
static int sfc_ef10_tx_qdesc_status(struct sfc_dp_txq *dp_txq, uint16_t offset) {
    struct sfc_ef10_txq *txq = sfc_ef10_txq_by_dp_txq(dp_txq);
...
struct sfc_dp_tx sfc_ef10_tx = {
    ...
    .features = SFC_DP_TX_FEAT_TSO | SFC_DP_TX_FEAT_TSO_ENCAP | SFC_DP_TX_FEAT_MULTI_SEG | SFC_DP_TX_FEAT_MULTI_POOL |
                SFC_DP_TX_FEAT_REFCNT | SFC_DP_TX_FEAT_MULTI_PROCESS,
