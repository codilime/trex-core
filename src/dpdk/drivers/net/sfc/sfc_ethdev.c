...
static int sfc_fw_version_get(struct rte_eth_dev *dev, char *fw_version, size_t fw_size) {
    ...
    ret = snprintf(fw_version, fw_size, "%" PRIu16 ".%" PRIu16 ".%" PRIu16 ".%" PRIu16, enfi.enfi_mc_fw_version[0],
                   enfi.enfi_mc_fw_version[1], enfi.enfi_mc_fw_version[2], enfi.enfi_mc_fw_version[3]);
    if (ret < 0)
        return ret;
    ...
    if (enfi.enfi_dpcpu_fw_ids_valid) {
        ...
        ret_extra = snprintf(fw_version + dpcpu_fw_ids_offset, fw_size - dpcpu_fw_ids_offset,
                             " rx%" PRIx16 " tx%" PRIx16, enfi.enfi_rx_dpcpu_fw_id, enfi.enfi_tx_dpcpu_fw_id);
        if (ret_extra < 0)
            return ret_extra;
...
static void sfc_dev_infos_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info) {
     ...
     */
    dev_info->rx_offload_capa = sfc_rx_get_dev_offload_caps(sa) | dev_info->rx_queue_offload_capa;
     ...
     * sense when some offloads are needed to be set on all queues.
     */
    dev_info->tx_offload_capa = sfc_tx_get_dev_offload_caps(sa) | dev_info->tx_queue_offload_capa;
...
static int sfc_dev_configure(struct rte_eth_dev *dev) {
    ...
    sfc_log_init(sa, "entry n_rxq=%u n_txq=%u", dev_data->nb_rx_queues, dev_data->nb_tx_queues);
...
static int sfc_dev_link_update(struct rte_eth_dev *dev, int wait_to_complete) {
    ...
    if (ret == 0)
        sfc_notice(sa, "Link status is %s", current_link.link_status ? "UP" : "DOWN");

    return ret;
}

static void sfc_dev_stop(struct rte_eth_dev *dev) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
...
static void sfc_dev_close(struct rte_eth_dev *dev) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
...
static void sfc_dev_filter_set(struct rte_eth_dev *dev, enum sfc_dev_filter_mode mode, boolean_t enabled) {
    ...
    if (*toggle != enabled) {
        *toggle = enabled;
        ...
        } else if ((sa->state == SFC_ADAPTER_STARTED) && (sfc_set_rx_mode(sa) != 0)) {
            *toggle = !(enabled);
            sfc_warn(sa, "Failed to %s %s mode", ((enabled) ? "enable" : "disable"), desc);
        }
    }
...
static void sfc_dev_promisc_enable(struct rte_eth_dev *dev) {
    sfc_dev_filter_set(dev, SFC_DEV_FILTER_MODE_PROMISC, B_TRUE);
}

static void sfc_dev_promisc_disable(struct rte_eth_dev *dev) {
    sfc_dev_filter_set(dev, SFC_DEV_FILTER_MODE_PROMISC, B_FALSE);
}

static void sfc_dev_allmulti_enable(struct rte_eth_dev *dev) {
    sfc_dev_filter_set(dev, SFC_DEV_FILTER_MODE_ALLMULTI, B_TRUE);
}

static void sfc_dev_allmulti_disable(struct rte_eth_dev *dev) {
    sfc_dev_filter_set(dev, SFC_DEV_FILTER_MODE_ALLMULTI, B_FALSE);
}

static int sfc_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id, uint16_t nb_rx_desc,
    ...
    sfc_log_init(sa, "RxQ=%u nb_rx_desc=%u socket_id=%u", rx_queue_id, nb_rx_desc, socket_id);

    sfc_adapter_lock(sa);

    rc = sfc_rx_qinit(sa, rx_queue_id, nb_rx_desc, socket_id, rx_conf, mb_pool);
    if (rc != 0)
        goto fail_rx_qinit;
...
static void sfc_rx_queue_release(void *queue) {
...
static int sfc_tx_queue_setup(struct rte_eth_dev *dev, uint16_t tx_queue_id, uint16_t nb_tx_desc,
                              unsigned int socket_id, const struct rte_eth_txconf *tx_conf) {
    struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    int rc;

    sfc_log_init(sa, "TxQ = %u, nb_tx_desc = %u, socket_id = %u", tx_queue_id, nb_tx_desc, socket_id);
...
static void sfc_tx_queue_release(void *queue) {
...
static int sfc_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
    ...
    if (EFX_MAC_STAT_SUPPORTED(port->mac_stats_mask, EFX_MAC_VADAPTER_RX_UNICAST_PACKETS)) {
        ...
        stats->ibytes = mac_stats[EFX_MAC_VADAPTER_RX_UNICAST_BYTES] + mac_stats[EFX_MAC_VADAPTER_RX_MULTICAST_BYTES] +
         ...
         */
        stats->imissed = mac_stats[EFX_MAC_RX_NODESC_DROP_CNT] + mac_stats[EFX_MAC_PM_TRUNC_BB_OVERFLOW] +
                         mac_stats[EFX_MAC_PM_DISCARD_BB_OVERFLOW] + mac_stats[EFX_MAC_PM_TRUNC_VFIFO_FULL] +
                         mac_stats[EFX_MAC_PM_DISCARD_VFIFO_FULL] + mac_stats[EFX_MAC_PM_TRUNC_QBB] +
                         mac_stats[EFX_MAC_PM_DISCARD_QBB] + mac_stats[EFX_MAC_PM_DISCARD_MAPPING] +
                         mac_stats[EFX_MAC_RXDP_Q_DISABLED_PKTS] + mac_stats[EFX_MAC_RXDP_DI_DROPPED_PKTS];
        stats->ierrors =
            mac_stats[EFX_MAC_RX_FCS_ERRORS] + mac_stats[EFX_MAC_RX_ALIGN_ERRORS] + mac_stats[EFX_MAC_RX_JABBER_PKTS];
        /* no oerrors counters supported on EF10 */

        /* Exclude missed, errors and pauses from Rx packets */
        sfc_update_diff_stat(&port->ipackets, mac_stats[EFX_MAC_RX_PKTS] - mac_stats[EFX_MAC_RX_PAUSE_PKTS] -
...
static void sfc_stats_reset(struct rte_eth_dev *dev) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    struct sfc_port *port = &sa->port;
    int rc;
...
static int sfc_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats, unsigned int xstats_count) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    struct sfc_port *port = &sa->port;
    uint64_t *mac_stats;
    int rc;
...
static int sfc_xstats_get_names(struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats_names,
    ...
    for (i = 0; i < EFX_MAC_NSTATS; ++i) {
        if (EFX_MAC_STAT_SUPPORTED(port->mac_stats_mask, i)) {
            if (xstats_names != NULL && nstats < xstats_count)
                strlcpy(xstats_names[nstats].name, efx_mac_stat_name(sa->nic, i), sizeof(xstats_names[0].name));
            nstats++;
        }
    }
...
static int sfc_xstats_get_by_id(struct rte_eth_dev *dev, const uint64_t *ids, uint64_t *values, unsigned int n) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    struct sfc_port *port = &sa->port;
    uint64_t *mac_stats;
    ...
    if (unlikely(values == NULL) || unlikely((ids == NULL) && (n < port->mac_stats_nb_supported)))
...
static int sfc_xstats_get_names_by_id(struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats_names,
    ...
    if (unlikely(xstats_names == NULL) || unlikely((ids == NULL) && (size < port->mac_stats_nb_supported)))
        return port->mac_stats_nb_supported;
    ...
    for (i = 0; (i < EFX_MAC_NSTATS) && (nb_written < size); ++i) {
        ...
        if ((ids == NULL) || (ids[nb_written] == nb_supported)) {
            ...
            strlcpy(name, efx_mac_stat_name(sa->nic, i), sizeof(xstats_names[0].name));
        }
...
static int sfc_flow_ctrl_get(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
...
static int sfc_flow_ctrl_set(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    struct sfc_port *port = &sa->port;
    unsigned int fcntl;
    int rc;

    if (fc_conf->high_water != 0 || fc_conf->low_water != 0 || fc_conf->pause_time != 0 || fc_conf->send_xon != 0 ||
...
static int sfc_check_scatter_on_all_rx_queues(struct sfc_adapter *sa, size_t pdu) {
    ...
    for (i = 0; i < sas->rxq_count; i++) {
        ...
        if (!sfc_rx_check_scatter(pdu, sa->rxq_ctrl[i].buf_size, encp->enc_rx_prefix_size, scatter_enabled, &error)) {
...
static int sfc_dev_set_mtu(struct rte_eth_dev *dev, uint16_t mtu) {
    ...
    if (pdu < EFX_MAC_PDU_MIN) {
        sfc_err(sa, "too small MTU %u (PDU size %u less than min %u)", (unsigned int)mtu, (unsigned int)pdu,
                EFX_MAC_PDU_MIN);
        goto fail_inval;
    }
    if (pdu > EFX_MAC_PDU_MAX) {
        sfc_err(sa, "too big MTU %u (PDU size %u greater than max %u)", (unsigned int)mtu, (unsigned int)pdu,
                EFX_MAC_PDU_MAX);
        goto fail_inval;
    }
...
static int sfc_mac_addr_set(struct rte_eth_dev *dev, struct ether_addr *mac_addr) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    const efx_nic_cfg_t *encp = efx_nic_cfg_get(sa->nic);
    struct sfc_port *port = &sa->port;
...
static int sfc_set_mc_addr_list(struct rte_eth_dev *dev, struct ether_addr *mc_addr_set, uint32_t nb_mc_addr) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    struct sfc_port *port = &sa->port;
    ...
    if (nb_mc_addr > port->max_mcast_addrs) {
        sfc_err(sa, "too many multicast addresses: %u > %u", nb_mc_addr, port->max_mcast_addrs);
        return -EINVAL;
    }
    ...
    rc = efx_mac_multicast_list_set(sa->nic, port->mcast_addrs, port->nb_mcast_addrs);
    if (rc != 0)
 ...
 */
static void sfc_rx_queue_info_get(struct rte_eth_dev *dev, uint16_t rx_queue_id, struct rte_eth_rxq_info *qinfo) {
    struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
 ...
 * use any process-local pointers from the adapter data.
 */
static void sfc_tx_queue_info_get(struct rte_eth_dev *dev, uint16_t tx_queue_id, struct rte_eth_txq_info *qinfo) {
    struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
 ...
 * use any process-local pointers from the adapter data.
 */
static uint32_t sfc_rx_queue_count(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
    const struct sfc_adapter_priv *sap = sfc_adapter_priv_by_eth_dev(dev);
    struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
    struct sfc_rxq_info *rxq_info;
 ...
 * use any process-local pointers from the adapter data.
 */
static int sfc_rx_descriptor_done(void *queue, uint16_t offset) {
    struct sfc_dp_rxq *dp_rxq = queue;
...
static int sfc_rx_queue_start(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
    struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    int rc;
...
static int sfc_rx_queue_stop(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
    struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
...
static int sfc_tx_queue_start(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
    struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    int rc;
...
static int sfc_tx_queue_stop(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
    struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
...
static efx_tunnel_protocol_t sfc_tunnel_rte_type_to_efx_udp_proto(enum rte_eth_tunnel_type rte_type) {
    switch (rte_type) {
...
static int sfc_dev_udp_tunnel_op(struct rte_eth_dev *dev, struct rte_eth_udp_tunnel *tunnel_udp,
    ...
    sfc_log_init(sa, "%s udp_port=%u prot_type=%u",
                 (op == SFC_UDP_TUNNEL_ADD_PORT) ? "add" : (op == SFC_UDP_TUNNEL_DEL_PORT) ? "delete" : "unknown",
    ...
    case SFC_UDP_TUNNEL_DEL_PORT:
        rc = efx_tunnel_config_udp_remove(sa->nic, tunnel_udp->udp_port, tunnel_proto);
        break;
    default:
        rc = EINVAL;
        goto fail_bad_op;
...
fail_reconfigure:
    ...
    switch (op) {
    case SFC_UDP_TUNNEL_ADD_PORT:
        (void)efx_tunnel_config_udp_remove(sa->nic, tunnel_udp->udp_port, tunnel_proto);
        break;
    case SFC_UDP_TUNNEL_DEL_PORT:
        (void)efx_tunnel_config_udp_add(sa->nic, tunnel_udp->udp_port, tunnel_proto);
        break;
    }
...
static int sfc_dev_udp_tunnel_port_add(struct rte_eth_dev *dev, struct rte_eth_udp_tunnel *tunnel_udp) {
    return sfc_dev_udp_tunnel_op(dev, tunnel_udp, SFC_UDP_TUNNEL_ADD_PORT);
}

static int sfc_dev_udp_tunnel_port_del(struct rte_eth_dev *dev, struct rte_eth_udp_tunnel *tunnel_udp) {
 ...
 * use any process-local pointers from the adapter data.
 */
static int sfc_dev_rss_hash_conf_get(struct rte_eth_dev *dev, struct rte_eth_rss_conf *rss_conf) {
    struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
    struct sfc_rss *rss = &sas->rss;
...
static int sfc_dev_rss_hash_update(struct rte_eth_dev *dev, struct rte_eth_rss_conf *rss_conf) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    ...
    if ((rss_conf->rss_key != NULL) && (rss_conf->rss_key_len != sizeof(rss->key))) {
        sfc_err(sa, "RSS key size is wrong (should be %lu)", sizeof(rss->key));
        return -EINVAL;
    }
    ...
    rc = efx_rx_scale_mode_set(sa->nic, EFX_RSS_CONTEXT_DEFAULT, rss->hash_alg, efx_hash_types, B_TRUE);
    if (rc != 0)
        goto fail_scale_mode_set;

    if (rss_conf->rss_key != NULL) {
        if (sa->state == SFC_ADAPTER_STARTED) {
            rc = efx_rx_scale_key_set(sa->nic, EFX_RSS_CONTEXT_DEFAULT, rss_conf->rss_key, sizeof(rss->key));
            if (rc != 0)
...
fail_scale_key_set:
    if (efx_rx_scale_mode_set(sa->nic, EFX_RSS_CONTEXT_DEFAULT, EFX_RX_HASHALG_TOEPLITZ, rss->hash_types, B_TRUE) != 0)
 ...
 * use any process-local pointers from the adapter data.
 */
static int sfc_dev_rss_reta_query(struct rte_eth_dev *dev, struct rte_eth_rss_reta_entry64 *reta_conf,
                                  uint16_t reta_size) {
    struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
    struct sfc_rss *rss = &sas->rss;
    int entry;
...
static int sfc_dev_rss_reta_update(struct rte_eth_dev *dev, struct rte_eth_rss_reta_entry64 *reta_conf,
                                   uint16_t reta_size) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    struct sfc_rss *rss = &sfc_sa2shared(sa)->rss;
    ...
    if (sa->state == SFC_ADAPTER_STARTED) {
        rc = efx_rx_scale_tbl_set(sa->nic, EFX_RSS_CONTEXT_DEFAULT, rss_tbl_new, EFX_RSS_TBL_SIZE);
        if (rc != 0)
...
static int sfc_dev_filter_ctrl(struct rte_eth_dev *dev, enum rte_filter_type filter_type, enum rte_filter_op filter_op,
                               void *arg) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    int rc = ENOTSUP;
...
static int sfc_eth_dev_set_ops(struct rte_eth_dev *dev) {
    ...
    rc = sfc_kvargs_process(sa, SFC_KVARG_RX_DATAPATH, sfc_kvarg_string_handler, &rx_name);
    if (rc != 0)
    ...
    if (rx_name != NULL) {
        ...
        if (!sfc_dp_match_hw_fw_caps(&dp_rx->dp, avail_caps)) {
            sfc_err(sa, "Insufficient Hw/FW capabilities to use Rx datapath %s", rx_name);
            rc = EINVAL;
    ...
    rc = sfc_kvargs_process(sa, SFC_KVARG_TX_DATAPATH, sfc_kvarg_string_handler, &tx_name);
    if (rc != 0)
    ...
    if (tx_name != NULL) {
        ...
        if (!sfc_dp_match_hw_fw_caps(&dp_tx->dp, avail_caps)) {
            sfc_err(sa, "Insufficient Hw/FW capabilities to use Tx datapath %s", tx_name);
            rc = EINVAL;
...
static int sfc_eth_dev_secondary_init(struct rte_eth_dev *dev, uint32_t logtype_main) {
    struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
    ...
    if (dp_rx == NULL) {
        SFC_LOG(sas, RTE_LOG_ERR, logtype_main, "cannot find %s Rx datapath", sas->dp_rx_name);
        rc = ENOENT;
        goto fail_dp_rx;
    }
    if (~dp_rx->features & SFC_DP_RX_FEAT_MULTI_PROCESS) {
        SFC_LOG(sas, RTE_LOG_ERR, logtype_main, "%s Rx datapath does not support multi-process", sas->dp_rx_name);
        rc = EINVAL;
    ...
    if (dp_tx == NULL) {
        SFC_LOG(sas, RTE_LOG_ERR, logtype_main, "cannot find %s Tx datapath", sas->dp_tx_name);
        rc = ENOENT;
        goto fail_dp_tx;
    }
    if (~dp_tx->features & SFC_DP_TX_FEAT_MULTI_PROCESS) {
        SFC_LOG(sas, RTE_LOG_ERR, logtype_main, "%s Tx datapath does not support multi-process", sas->dp_tx_name);
        rc = EINVAL;
...
static int sfc_eth_dev_init(struct rte_eth_dev *dev) {
    struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
    ...
    logtype_main = sfc_register_logtype(&pci_dev->addr, SFC_LOGTYPE_MAIN_STR, RTE_LOG_NOTICE);
...
static int sfc_eth_dev_pci_probe(struct rte_pci_driver *pci_drv __rte_unused, struct rte_pci_device *pci_dev) {
    return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct sfc_adapter_shared), sfc_eth_dev_init);
}
...
RTE_PMD_REGISTER_PARAM_STRING(net_sfc_efx, SFC_KVARG_RX_DATAPATH
                              "=" SFC_KVARG_VALUES_RX_DATAPATH " " SFC_KVARG_TX_DATAPATH
                              "=" SFC_KVARG_VALUES_TX_DATAPATH " " SFC_KVARG_PERF_PROFILE
                              "=" SFC_KVARG_VALUES_PERF_PROFILE " " SFC_KVARG_FW_VARIANT "=" SFC_KVARG_VALUES_FW_VARIANT
                              " " SFC_KVARG_RXD_WAIT_TIMEOUT_NS "=<long> " SFC_KVARG_STATS_UPDATE_PERIOD_MS "=<long>");

RTE_INIT(sfc_driver_register_logtype) {
    int ret;

    ret = rte_log_register_type_and_pick_level(SFC_LOGTYPE_PREFIX "driver", RTE_LOG_NOTICE);
