...
static boolean_t sfc_ev_initialized(void *arg) {
    ...
    SFC_ASSERT(evq->init_state == SFC_EVQ_STARTING || evq->init_state == SFC_EVQ_STARTED);
...
static boolean_t sfc_ev_nop_rx(void *arg, uint32_t label, uint32_t id, uint32_t size, uint16_t flags) {
    struct sfc_evq *evq = arg;

    sfc_err(evq->sa, "EVQ %u unexpected Rx event label=%u id=%#x size=%u flags=%#x", evq->evq_index, label, id, size,
            flags);
    return B_TRUE;
}

static boolean_t sfc_ev_efx_rx(void *arg, __rte_unused uint32_t label, uint32_t id, uint32_t size, uint16_t flags) {
    struct sfc_evq *evq = arg;
    ...
    delta = (stop >= pending_id) ? (stop - pending_id) : (rxq->ptr_mask + 1 - pending_id + stop);
...
static boolean_t sfc_ev_dp_rx(void *arg, __rte_unused uint32_t label, uint32_t id, __rte_unused uint32_t size,
...
static boolean_t sfc_ev_nop_rx_ps(void *arg, uint32_t label, uint32_t id, uint32_t pkt_count, uint16_t flags) {
    struct sfc_evq *evq = arg;

    sfc_err(evq->sa, "EVQ %u unexpected packed stream Rx event label=%u id=%#x pkt_count=%u flags=%#x", evq->evq_index,
            label, id, pkt_count, flags);
    return B_TRUE;
}

/* It is not actually used on datapath, but required on RxQ flush */
static boolean_t sfc_ev_dp_rx_ps(void *arg, __rte_unused uint32_t label, uint32_t id, __rte_unused uint32_t pkt_count,
                                 __rte_unused uint16_t flags) {
    struct sfc_evq *evq = arg;
    struct sfc_dp_rxq *dp_rxq;
...
static boolean_t sfc_ev_nop_tx(void *arg, uint32_t label, uint32_t id) {
    ...
    sfc_err(evq->sa, "EVQ %u unexpected Tx event label=%u id=%#x", evq->evq_index, label, id);
    return B_TRUE;
}

static boolean_t sfc_ev_tx(void *arg, __rte_unused uint32_t label, uint32_t id) {
    struct sfc_evq *evq = arg;
...
static boolean_t sfc_ev_dp_tx(void *arg, __rte_unused uint32_t label, uint32_t id) {
    struct sfc_evq *evq = arg;
    struct sfc_dp_txq *dp_txq;
...
static boolean_t sfc_ev_exception(void *arg, uint32_t code, __rte_unused uint32_t data) {
    struct sfc_evq *evq = arg;
    ...
    sfc_warn(evq->sa,
             ...
             " needs recovery",
             (code == EFX_EXCEPTION_RX_RECOVERY)
                 ? "RX_RECOVERY"
                 : (code == EFX_EXCEPTION_RX_DSC_ERROR)
                       ? "RX_DSC_ERROR"
                       : (code == EFX_EXCEPTION_TX_DSC_ERROR)
                             ? "TX_DSC_ERROR"
                             : (code == EFX_EXCEPTION_FWALERT_SRAM)
                                   ? "FWALERT_SRAM"
                                   : (code == EFX_EXCEPTION_UNKNOWN_FWALERT)
                                         ? "UNKNOWN_FWALERT"
                                         : (code == EFX_EXCEPTION_RX_ERROR)
                                               ? "RX_ERROR"
                                               : (code == EFX_EXCEPTION_TX_ERROR)
                                                     ? "TX_ERROR"
                                                     : (code == EFX_EXCEPTION_EV_ERROR) ? "EV_ERROR" : "UNKNOWN",
...
static boolean_t sfc_ev_nop_rxq_flush_done(void *arg, uint32_t rxq_hw_index) {
    ...
    sfc_err(evq->sa, "EVQ %u unexpected RxQ %u flush done", evq->evq_index, rxq_hw_index);
    return B_TRUE;
}

static boolean_t sfc_ev_rxq_flush_done(void *arg, __rte_unused uint32_t rxq_hw_index) {
    struct sfc_evq *evq = arg;
    struct sfc_dp_rxq *dp_rxq;
    struct sfc_rxq *rxq;
...
static boolean_t sfc_ev_nop_rxq_flush_failed(void *arg, uint32_t rxq_hw_index) {
    ...
    sfc_err(evq->sa, "EVQ %u unexpected RxQ %u flush failed", evq->evq_index, rxq_hw_index);
    return B_TRUE;
}

static boolean_t sfc_ev_rxq_flush_failed(void *arg, __rte_unused uint32_t rxq_hw_index) {
    struct sfc_evq *evq = arg;
    struct sfc_dp_rxq *dp_rxq;
    struct sfc_rxq *rxq;
...
static boolean_t sfc_ev_nop_txq_flush_done(void *arg, uint32_t txq_hw_index) {
    ...
    sfc_err(evq->sa, "EVQ %u unexpected TxQ %u flush done", evq->evq_index, txq_hw_index);
    return B_TRUE;
}

static boolean_t sfc_ev_txq_flush_done(void *arg, __rte_unused uint32_t txq_hw_index) {
    struct sfc_evq *evq = arg;
    struct sfc_dp_txq *dp_txq;
    struct sfc_txq *txq;
...
static boolean_t sfc_ev_software(void *arg, uint16_t magic) {
    ...
    sfc_err(evq->sa, "EVQ %u unexpected software event magic=%#.4x", evq->evq_index, magic);
    return B_TRUE;
}
...
static boolean_t sfc_ev_sram(void *arg, uint32_t code) {
    ...
    sfc_err(evq->sa, "EVQ %u unexpected SRAM event code=%u", evq->evq_index, code);
    return B_TRUE;
}
...
static boolean_t sfc_ev_wake_up(void *arg, uint32_t index) {
    ...
    sfc_err(evq->sa, "EVQ %u unexpected wake up event index=%u", evq->evq_index, index);
    return B_TRUE;
}
...
static boolean_t sfc_ev_timer(void *arg, uint32_t index) {
    ...
    sfc_err(evq->sa, "EVQ %u unexpected timer event index=%u", evq->evq_index, index);
    return B_TRUE;
}

static boolean_t sfc_ev_nop_link_change(void *arg, __rte_unused efx_link_mode_t link_mode) {
    struct sfc_evq *evq = arg;
...
void sfc_ev_qpoll(struct sfc_evq *evq) {
    SFC_ASSERT(evq->init_state == SFC_EVQ_STARTED || evq->init_state == SFC_EVQ_STARTING);
    ...
    if (unlikely(evq->exception) && sfc_adapter_trylock(evq->sa)) {
        ...
        if (evq->dp_rxq != NULL) {
            ...
            sfc_warn(sa, "restart RxQ %u because of exception on its EvQ %u", rxq_sw_index, evq->evq_index);
        ...
        if (evq->dp_txq != NULL) {
            ...
            sfc_warn(sa, "restart TxQ %u because of exception on its EvQ %u", txq_sw_index, evq->evq_index);
...
int sfc_ev_qstart(struct sfc_evq *evq, unsigned int hw_index) {
    ...
    /* Clear all events */
    (void)memset((void *)esmp->esm_base, 0xff, efx_evq_size(sa->nic, evq->entries));
    ...
    rc = efx_ev_qcreate(sa->nic, hw_index, esmp, evq->entries, 0 /* unused on EF10 */, 0, evq_flags, &evq->common);
    if (rc != 0)
...
static void sfc_ev_mgmt_periodic_qpoll(void *arg) {
    ...
    rc = rte_eal_alarm_set(SFC_MGMT_EV_QPOLL_PERIOD_US, sfc_ev_mgmt_periodic_qpoll, sa);
    if (rc == -ENOTSUP) {
...
static void sfc_ev_mgmt_periodic_qpoll_start(struct sfc_adapter *sa) { sfc_ev_mgmt_periodic_qpoll(sa); }
...
int sfc_ev_qinit(struct sfc_adapter *sa, enum sfc_evq_type type, unsigned int type_index, unsigned int entries,
                 int socket_id, struct sfc_evq **evqp) {
    struct sfc_evq *evq;
    int rc;
    ...
    rc = ENOMEM;
    evq = rte_zmalloc_socket("sfc-evq", sizeof(*evq), RTE_CACHE_LINE_SIZE, socket_id);
    if (evq == NULL)
        goto fail_evq_alloc;
    ...
    /* Allocate DMA space */
    rc = sfc_dma_alloc(sa, sfc_evq_type2str(type), type_index, efx_evq_size(sa->nic, evq->entries), socket_id,
                       &evq->mem);
    if (rc != 0)
        goto fail_dma_alloc;
...
static int sfc_kvarg_perf_profile_handler(__rte_unused const char *key, const char *value_str, void *opaque) {
...
int sfc_ev_attach(struct sfc_adapter *sa) {
    ...
    rc = sfc_kvargs_process(sa, SFC_KVARG_PERF_PROFILE, sfc_kvarg_perf_profile_handler, &sa->evq_flags);
    if (rc != 0) {
    ...
    rc = sfc_ev_qinit(sa, SFC_EVQ_TYPE_MGMT, 0, sa->evq_min_entries, sa->socket_id, &sa->mgmt_evq);
    if (rc != 0)
