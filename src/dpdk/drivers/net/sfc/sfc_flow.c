...
typedef int(sfc_flow_item_parse)(const struct rte_flow_item *item, efx_filter_spec_t *spec,
...
typedef int(sfc_flow_spec_set_vals)(struct sfc_flow_spec *spec, unsigned int filters_count_for_one_val,
                                    struct rte_flow_error *error);

typedef boolean_t(sfc_flow_spec_check)(efx_filter_match_flags_t match, efx_filter_spec_t *spec,
 ...
 */
static int sfc_flow_parse_init(const struct rte_flow_item *item, const void **spec_ptr, const void **mask_ptr,
                               const void *supp_mask, const void *def_mask, unsigned int size,
                               struct rte_flow_error *error) {
    const uint8_t *spec;
    const uint8_t *mask;
    const uint8_t *last;
    ...
    if ((item->last != NULL || item->mask != NULL) && item->spec == NULL) {
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item, "Mask or last is set without spec");
        return -rte_errno;
    }
    ...
    if (item->mask == NULL) {
        if (def_mask == NULL) {
            rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, NULL, "Mask should be specified");
            return -rte_errno;
        }
     ...
     */
    if (last != NULL && !sfc_flow_is_zero(last, size) && memcmp(last, spec, size) != 0) {
        rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, item, "Ranging is not supported");
        return -rte_errno;
    }
    ...
    for (i = 0; i < size; i++) {
        ...
        if (~supp & mask[i]) {
            rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, item, "Item's field is not supported");
            return -rte_errno;
        }
    }
...
static int sfc_flow_parse_void(__rte_unused const struct rte_flow_item *item, __rte_unused efx_filter_spec_t *efx_spec,
 ...
 */
static int sfc_flow_parse_eth(const struct rte_flow_item *item, efx_filter_spec_t *efx_spec,
                              struct rte_flow_error *error) {
    int rc;
    ...
    const uint8_t ig_mask[EFX_MAC_ADDR_LEN] = {0x01, 0x00, 0x00, 0x00, 0x00, 0x00};
    ...
    rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask, supp_mask_p, def_mask_p,
    ...
    if (is_same_ether_addr(&mask->dst, &supp_mask.dst)) {
        efx_spec->efs_match_flags |= is_ifrm ? EFX_FILTER_MATCH_IFRM_LOC_MAC : EFX_FILTER_MATCH_LOC_MAC;
        ...
        if (is_unicast_ether_addr(&spec->dst))
            efx_spec->efs_match_flags |=
                is_ifrm ? EFX_FILTER_MATCH_IFRM_UNKNOWN_UCAST_DST : EFX_FILTER_MATCH_UNKNOWN_UCAST_DST;
        else
            efx_spec->efs_match_flags |=
                is_ifrm ? EFX_FILTER_MATCH_IFRM_UNKNOWN_MCAST_DST : EFX_FILTER_MATCH_UNKNOWN_MCAST_DST;
...
fail_bad_mask:
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item, "Bad mask in the ETH pattern item");
    return -rte_errno;
}
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_vlan(const struct rte_flow_item *item, efx_filter_spec_t *efx_spec,
                               struct rte_flow_error *error) {
    int rc;
    uint16_t vid;
    ...
    rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask, &supp_mask, NULL,
    ...
    if (mask->tci == supp_mask.tci) {
        ...
        if (!(efx_spec->efs_match_flags & EFX_FILTER_MATCH_OUTER_VID)) {
            ...
            rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item, "More than two VLAN items");
            return -rte_errno;
        }
    } else {
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item, "VLAN ID in TCI match is required");
        return -rte_errno;
    }

    if (efx_spec->efs_match_flags & EFX_FILTER_MATCH_ETHER_TYPE) {
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item, "VLAN TPID matching is not supported");
        return -rte_errno;
    }
    if (mask->inner_type == supp_mask.inner_type) {
        ...
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item, "Bad mask for VLAN inner_type");
        return -rte_errno;
    }
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_ipv4(const struct rte_flow_item *item, efx_filter_spec_t *efx_spec,
                               struct rte_flow_error *error) {
    int rc;
    ...
    rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask, &supp_mask, &rte_flow_item_ipv4_mask,
...
fail_bad_mask:
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item, "Bad mask in the IPV4 pattern item");
    return -rte_errno;
}
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_ipv6(const struct rte_flow_item *item, efx_filter_spec_t *efx_spec,
                               struct rte_flow_error *error) {
    int rc;
    ...
    const struct rte_flow_item_ipv6 supp_mask = {.hdr = {
    ...
    rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask, &supp_mask, &rte_flow_item_ipv6_mask,
     ...
     * efx_spec
     */
    if (memcmp(mask->hdr.src_addr, supp_mask.hdr.src_addr, sizeof(mask->hdr.src_addr)) == 0) {
        efx_spec->efs_match_flags |= EFX_FILTER_MATCH_REM_HOST;

        RTE_BUILD_BUG_ON(sizeof(efx_spec->efs_rem_host) != sizeof(spec->hdr.src_addr));
        rte_memcpy(&efx_spec->efs_rem_host, spec->hdr.src_addr, sizeof(efx_spec->efs_rem_host));
    } else if (!sfc_flow_is_zero(mask->hdr.src_addr, sizeof(mask->hdr.src_addr))) {
        goto fail_bad_mask;
    }

    if (memcmp(mask->hdr.dst_addr, supp_mask.hdr.dst_addr, sizeof(mask->hdr.dst_addr)) == 0) {
        efx_spec->efs_match_flags |= EFX_FILTER_MATCH_LOC_HOST;

        RTE_BUILD_BUG_ON(sizeof(efx_spec->efs_loc_host) != sizeof(spec->hdr.dst_addr));
        rte_memcpy(&efx_spec->efs_loc_host, spec->hdr.dst_addr, sizeof(efx_spec->efs_loc_host));
    } else if (!sfc_flow_is_zero(mask->hdr.dst_addr, sizeof(mask->hdr.dst_addr))) {
        goto fail_bad_mask;
    }
...
fail_bad_mask:
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item, "Bad mask in the IPV6 pattern item");
    return -rte_errno;
}
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_tcp(const struct rte_flow_item *item, efx_filter_spec_t *efx_spec,
                              struct rte_flow_error *error) {
    int rc;
    ...
    rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask, &supp_mask, &rte_flow_item_tcp_mask,
...
fail_bad_mask:
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item, "Bad mask in the TCP pattern item");
    return -rte_errno;
}
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_udp(const struct rte_flow_item *item, efx_filter_spec_t *efx_spec,
                              struct rte_flow_error *error) {
    int rc;
    ...
    rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask, &supp_mask, &rte_flow_item_udp_mask,
...
fail_bad_mask:
    rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item, "Bad mask in the UDP pattern item");
    return -rte_errno;
}
 ...
 */
static int sfc_flow_set_match_flags_for_encap_pkts(const struct rte_flow_item *item, efx_filter_spec_t *efx_spec,
    ...
    if (efx_spec->efs_match_flags & EFX_FILTER_MATCH_ETHER_TYPE && efx_spec->efs_ether_type != EFX_ETHER_TYPE_IPV4 &&
...
static int sfc_flow_set_efx_spec_vni_or_vsid(efx_filter_spec_t *efx_spec, const uint8_t *vni_or_vsid_val,
                                             const uint8_t *vni_or_vsid_mask, const struct rte_flow_item *item,
                                             struct rte_flow_error *error) {
    const uint8_t vni_or_vsid_full_mask[EFX_VNI_OR_VSID_LEN] = {0xff, 0xff, 0xff};

    if (memcmp(vni_or_vsid_mask, vni_or_vsid_full_mask, EFX_VNI_OR_VSID_LEN) == 0) {
        ...
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ITEM, item, "Unsupported VNI/VSID mask");
        return -rte_errno;
    }
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_vxlan(const struct rte_flow_item *item, efx_filter_spec_t *efx_spec,
                                struct rte_flow_error *error) {
    int rc;
    ...
    rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask, &supp_mask, &rte_flow_item_vxlan_mask,
                             sizeof(struct rte_flow_item_vxlan), error);
    if (rc != 0)
        return rc;

    rc = sfc_flow_set_match_flags_for_encap_pkts(item, efx_spec, EFX_IPPROTO_UDP, error);
    if (rc != 0)
        return rc;
    ...
    rc = sfc_flow_set_efx_spec_vni_or_vsid(efx_spec, spec->vni, mask->vni, item, error);
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_geneve(const struct rte_flow_item *item, efx_filter_spec_t *efx_spec,
                                 struct rte_flow_error *error) {
    int rc;
    const struct rte_flow_item_geneve *spec = NULL;
    const struct rte_flow_item_geneve *mask = NULL;
    const struct rte_flow_item_geneve supp_mask = {.protocol = RTE_BE16(0xffff), .vni = {0xff, 0xff, 0xff}};

    rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask, &supp_mask, &rte_flow_item_geneve_mask,
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_parse_nvgre(const struct rte_flow_item *item, efx_filter_spec_t *efx_spec,
                                struct rte_flow_error *error) {
    int rc;
    ...
    rc = sfc_flow_parse_init(item, (const void **)&spec, (const void **)&mask, &supp_mask, &rte_flow_item_nvgre_mask,
                             sizeof(struct rte_flow_item_nvgre), error);
    if (rc != 0)
        return rc;

    rc = sfc_flow_set_match_flags_for_encap_pkts(item, efx_spec, EFX_IPPROTO_GRE, error);
    if (rc != 0)
        return rc;
    ...
    rc = sfc_flow_set_efx_spec_vni_or_vsid(efx_spec, spec->tni, mask->tni, item, error);
 ...
 */
static int sfc_flow_parse_attr(const struct rte_flow_attr *attr, struct rte_flow *flow, struct rte_flow_error *error) {
    if (attr == NULL) {
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ATTR, NULL, "NULL attribute");
        return -rte_errno;
    }
    if (attr->group != 0) {
        rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_GROUP, attr, "Groups are not supported");
        return -rte_errno;
    }
    if (attr->priority != 0) {
        rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_PRIORITY, attr, "Priorities are not supported");
        return -rte_errno;
    }
    if (attr->egress != 0) {
        rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_EGRESS, attr, "Egress is not supported");
        return -rte_errno;
    }
    if (attr->transfer != 0) {
        rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_TRANSFER, attr, "Transfer is not supported");
        return -rte_errno;
    }
    if (attr->ingress == 0) {
        rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ATTR_INGRESS, attr, "Only ingress is supported");
        return -rte_errno;
    }
...
static const struct sfc_flow_item *sfc_flow_get_item(enum rte_flow_item_type type) {
    unsigned int i;
...
static int sfc_flow_parse_pattern(const struct rte_flow_item pattern[], struct rte_flow *flow,
                                  struct rte_flow_error *error) {
    int rc;
    ...
    for (; pattern->type != RTE_FLOW_ITEM_TYPE_END; pattern++) {
        ...
        if (item == NULL) {
            rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, pattern, "Unsupported pattern item");
            return -rte_errno;
        }
         ...
         */
        if (item->prev_layer != SFC_FLOW_ITEM_ANY_LAYER && prev_layer != SFC_FLOW_ITEM_ANY_LAYER &&
...
static int sfc_flow_parse_queue(struct sfc_adapter *sa, const struct rte_flow_action_queue *queue,
...
static int sfc_flow_parse_rss(struct sfc_adapter *sa, const struct rte_flow_action_rss *action_rss,
                              struct rte_flow *flow) {
     ...
     */
    if (action_rss->queue_num == 1 && action_rss->types == 0 && action_rss->key_len == 0) {
    ...
    if (action_rss->types) {
        ...
        rc = sfc_rx_hf_rte_to_efx(sa, action_rss->types, &efx_hash_types);
        if (rc != 0)
            return -rc;
    } else {
        unsigned int i;
...
static int sfc_flow_spec_flush(struct sfc_adapter *sa, struct sfc_flow_spec *spec, unsigned int filters_count) {
    unsigned int i;
    int ret = 0;
...
static int sfc_flow_spec_insert(struct sfc_adapter *sa, struct sfc_flow_spec *spec) {
    unsigned int i;
    int rc = 0;
...
static int sfc_flow_spec_remove(struct sfc_adapter *sa, struct sfc_flow_spec *spec) {
    return sfc_flow_spec_flush(sa, spec, spec->count);
}

static int sfc_flow_filter_insert(struct sfc_adapter *sa, struct rte_flow *flow) {
    struct sfc_adapter_shared *const sas = sfc_sa2shared(sa);
    struct sfc_rss *rss = &sas->rss;
    ...
    if (flow->rss) {
        unsigned int rss_spread = MIN(flow_rss->rxq_hw_index_max - flow_rss->rxq_hw_index_min + 1, EFX_MAXRSS);

        rc = efx_rx_scale_context_alloc(sa->nic, EFX_RX_SCALE_EXCLUSIVE, rss_spread, &efs_rss_context);
        if (rc != 0)
            goto fail_scale_context_alloc;

        rc = efx_rx_scale_mode_set(sa->nic, efs_rss_context, rss->hash_alg, flow_rss->rss_hash_types, B_TRUE);
        if (rc != 0)
            goto fail_scale_mode_set;

        rc = efx_rx_scale_key_set(sa->nic, efs_rss_context, flow_rss->rss_key, sizeof(rss->key));
        if (rc != 0)
    ...
    if (flow->rss) {
         ...
         */
        rc = efx_rx_scale_tbl_set(sa->nic, efs_rss_context, flow_rss->rss_tbl, RTE_DIM(flow_rss->rss_tbl));
        if (rc != 0)
...
static int sfc_flow_filter_remove(struct sfc_adapter *sa, struct rte_flow *flow) {
    int rc = 0;
...
static int sfc_flow_parse_mark(struct sfc_adapter *sa, const struct rte_flow_action_mark *mark, struct rte_flow *flow) {
...
static int sfc_flow_parse_actions(struct sfc_adapter *sa, const struct rte_flow_action actions[], struct rte_flow *flow,
                                  struct rte_flow_error *error) {
    int rc;
    const unsigned int dp_rx_features = sa->priv.dp_rx->features;
    uint32_t actions_set = 0;
    const uint32_t fate_actions_mask =
        (1UL << RTE_FLOW_ACTION_TYPE_QUEUE) | (1UL << RTE_FLOW_ACTION_TYPE_RSS) | (1UL << RTE_FLOW_ACTION_TYPE_DROP);
    const uint32_t mark_actions_mask = (1UL << RTE_FLOW_ACTION_TYPE_MARK) | (1UL << RTE_FLOW_ACTION_TYPE_FLAG);

    if (actions == NULL) {
        rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_NUM, NULL, "NULL actions");
        return -rte_errno;
    }

#define SFC_BUILD_SET_OVERFLOW(_action, _set) RTE_BUILD_BUG_ON(_action >= sizeof(_set) * CHAR_BIT)
    ...
    for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
        ...
        case RTE_FLOW_ACTION_TYPE_QUEUE:
            ...
            if (rc != 0) {
                rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION, actions, "Bad QUEUE action");
                return -rte_errno;
            }
            break;
        ...
        case RTE_FLOW_ACTION_TYPE_RSS:
            ...
            if (rc != 0) {
                rte_flow_error_set(error, -rc, RTE_FLOW_ERROR_TYPE_ACTION, actions, "Bad RSS action");
                return -rte_errno;
            }
            break;
        ...
        case RTE_FLOW_ACTION_TYPE_DROP:
            ...
            flow->spec.template.efs_dmaq_id = EFX_FILTER_SPEC_RX_DMAQ_ID_DROP;
            break;
        ...
        case RTE_FLOW_ACTION_TYPE_FLAG:
            ...
            flow->spec.template.efs_flags |= EFX_FILTER_FLAG_ACTION_FLAG;
            break;
        ...
        case RTE_FLOW_ACTION_TYPE_MARK:
            ...
            if (rc != 0) {
                rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_ACTION, actions, "Bad MARK action");
                return -rte_errno;
            }
            break;

        default:
            rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, actions, "Action is not supported");
            return -rte_errno;
        }
...
fail_actions_overlap:
    rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION, actions, "Overlapping actions are not supported");
    return -rte_errno;
}
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_set_unknown_dst_flags(struct sfc_flow_spec *spec, unsigned int filters_count_for_one_val,
                                          struct rte_flow_error *error) {
    unsigned int i;
    static const efx_filter_match_flags_t vals[] = {EFX_FILTER_MATCH_UNKNOWN_UCAST_DST,
    ...
    for (i = 0; i < spec->count; i++) {
        ...
        spec->filters[i].efs_match_flags |= vals[i / filters_count_for_one_val];
    }
 ...
 */
static boolean_t sfc_flow_check_unknown_dst_flags(efx_filter_match_flags_t match, __rte_unused efx_filter_spec_t *spec,
                                                  struct sfc_filter *filter) {
    unsigned int i;
    efx_filter_match_flags_t match_mcast_dst;

    match_mcast_dst = (match & ~EFX_FILTER_MATCH_UNKNOWN_UCAST_DST) | EFX_FILTER_MATCH_UNKNOWN_MCAST_DST;
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_set_ethertypes(struct sfc_flow_spec *spec, unsigned int filters_count_for_one_val,
                                   struct rte_flow_error *error) {
    unsigned int i;
    ...
    for (i = 0; i < spec->count; i++) {
        spec->filters[i].efs_match_flags |= EFX_FILTER_MATCH_ETHER_TYPE;

        /*
         * The check above ensures that
         * filters_count_for_one_val is not 0
         */
        spec->filters[i].efs_ether_type = vals[i / filters_count_for_one_val];
    }
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_set_outer_vid_flag(struct sfc_flow_spec *spec, unsigned int filters_count_for_one_val,
                                       struct rte_flow_error *error) {
    unsigned int i;
    ...
    for (i = 0; i < spec->count; i++) {
        spec->filters[i].efs_match_flags |= EFX_FILTER_MATCH_OUTER_VID;
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_set_ifrm_unknown_dst_flags(struct sfc_flow_spec *spec, unsigned int filters_count_for_one_val,
                                               struct rte_flow_error *error) {
    unsigned int i;
    static const efx_filter_match_flags_t vals[] = {EFX_FILTER_MATCH_IFRM_UNKNOWN_UCAST_DST,
 ...
 *   SFC filter with list of supported filters.
 */
static boolean_t sfc_flow_check_ifrm_unknown_dst_flags(efx_filter_match_flags_t match, efx_filter_spec_t *spec,
                                                       struct sfc_filter *filter) {
    unsigned int i;
    ...
    match_mcast_dst = (match & ~EFX_FILTER_MATCH_IFRM_UNKNOWN_UCAST_DST) | EFX_FILTER_MATCH_IFRM_UNKNOWN_MCAST_DST;
    for (i = 0; i < filter->supported_match_num; i++) {
        if (match_mcast_dst == filter->supported_match[i])
            return B_TRUE;
    }
 ...
 *   SFC filter with list of supported filters.
 */
static boolean_t sfc_flow_check_outer_vid_flag(efx_filter_match_flags_t match, __rte_unused efx_filter_spec_t *spec,
                                               struct sfc_filter *filter) {
    unsigned int i;
    efx_filter_match_flags_t match_without_vid = match & ~EFX_FILTER_MATCH_OUTER_VID;
...
static const struct sfc_flow_copy_flag *sfc_flow_get_copy_flag(efx_filter_match_flags_t flag) {
    unsigned int i;
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_spec_add_match_flag(struct sfc_flow_spec *spec, efx_filter_match_flags_t flag,
                                        struct rte_flow_error *error) {
    unsigned int i;
    ...
    if (copy_flag == NULL) {
        rte_flow_error_set(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, "Unsupported spec field for copying");
        return -rte_errno;
    }
 ...
 */
static unsigned int sfc_flow_check_missing_flags(efx_filter_match_flags_t miss_flags, efx_filter_spec_t *spec,
                                                 struct sfc_filter *filter) {
    unsigned int i;
 ...
 *   Perform verbose error reporting if not NULL.
 */
static int sfc_flow_spec_filters_complete(struct sfc_adapter *sa, struct sfc_flow_spec *spec,
                                          struct rte_flow_error *error) {
    ...
    for (i = 0; i < filter->supported_match_num; i++) {
        if ((match & filter->supported_match[i]) == match) {
            ...
            multiplier = sfc_flow_check_missing_flags(miss_flags, &spec->template, filter);
            if (multiplier > 0) {
 ...
 */
static boolean_t sfc_flow_is_match_with_vids(efx_filter_match_flags_t match_flags,
 ...
 */
static boolean_t sfc_flow_is_match_flags_exception(struct sfc_filter *filter, struct sfc_flow_spec *spec) {
    unsigned int i;
    uint16_t ether_type;
    uint8_t ip_proto;
    ...
    for (i = 0; i < spec->count; i++) {
        ...
        if (sfc_flow_is_match_with_vids(match_flags, EFX_FILTER_MATCH_ETHER_TYPE) ||
            sfc_flow_is_match_with_vids(match_flags, EFX_FILTER_MATCH_ETHER_TYPE | EFX_FILTER_MATCH_LOC_MAC)) {
            ether_type = spec->filters[i].efs_ether_type;
            if (filter->supports_ip_proto_or_addr_filter &&
                (ether_type == EFX_ETHER_TYPE_IPV4 || ether_type == EFX_ETHER_TYPE_IPV6))
                return B_TRUE;
        } else if (sfc_flow_is_match_with_vids(match_flags, EFX_FILTER_MATCH_ETHER_TYPE | EFX_FILTER_MATCH_IP_PROTO) ||
                   sfc_flow_is_match_with_vids(match_flags, EFX_FILTER_MATCH_ETHER_TYPE | EFX_FILTER_MATCH_IP_PROTO |
...
static int sfc_flow_validate_match_flags(struct sfc_adapter *sa, struct rte_flow *flow, struct rte_flow_error *error) {
...
static int sfc_flow_parse(struct rte_eth_dev *dev, const struct rte_flow_attr *attr,
...
static int sfc_flow_validate(struct rte_eth_dev *dev, const struct rte_flow_attr *attr,
                             const struct rte_flow_item pattern[], const struct rte_flow_action actions[],
                             struct rte_flow_error *error) {
    struct rte_flow flow;
...
static struct rte_flow *sfc_flow_create(struct rte_eth_dev *dev, const struct rte_flow_attr *attr,
                                        const struct rte_flow_item pattern[], const struct rte_flow_action actions[],
                                        struct rte_flow_error *error) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    ...
    if (flow == NULL) {
        rte_flow_error_set(error, ENOMEM, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, "Failed to allocate memory");
        goto fail_no_mem;
    }
    ...
    if (sa->state == SFC_ADAPTER_STARTED) {
        ...
        if (rc != 0) {
            rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, "Failed to insert filter");
            goto fail_filter_insert;
        }
    }
...
static int sfc_flow_remove(struct sfc_adapter *sa, struct rte_flow *flow, struct rte_flow_error *error) {
    int rc = 0;
    ...
    if (sa->state == SFC_ADAPTER_STARTED) {
        ...
        if (rc != 0)
            rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, "Failed to destroy flow rule");
    }
...
static int sfc_flow_destroy(struct rte_eth_dev *dev, struct rte_flow *flow, struct rte_flow_error *error) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    ...
    if (rc != 0) {
        rte_flow_error_set(error, rc, RTE_FLOW_ERROR_TYPE_HANDLE, NULL, "Failed to find flow rule to destroy");
        goto fail_bad_value;
    }
...
static int sfc_flow_flush(struct rte_eth_dev *dev, struct rte_flow_error *error) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    struct rte_flow *flow;
    int rc = 0;
    int ret = 0;
...
static int sfc_flow_isolate(struct rte_eth_dev *dev, int enable, struct rte_flow_error *error) {
    struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
    int ret = 0;

    sfc_adapter_lock(sa);
    if (sa->state != SFC_ADAPTER_INITIALIZED) {
        rte_flow_error_set(error, EBUSY, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, "please close the port first");
        ret = -rte_errno;
    } else {
