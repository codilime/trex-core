...
static void sfc_intr_handle_mgmt_evq(struct sfc_adapter *sa) {
    ...
    if (!sa->mgmt_evq_running) {
        sfc_log_init(sa, "interrupt on not running management EVQ %u", evq->evq_index);
    } else {
        sfc_ev_qpoll(evq);
...
static void sfc_intr_line_handler(void *cb_arg) {
    ...
    if (sa->state != SFC_ADAPTER_STARTED && sa->state != SFC_ADAPTER_STARTING && sa->state != SFC_ADAPTER_STOPPING) {
...
exit:
    if (lsc_seq != sa->port.lsc_seq) {
        sfc_notice(sa, "link status change event: link %s", sa->eth_dev->data->dev_link.link_status ? "UP" : "DOWN");
        _rte_eth_dev_callback_process(sa->eth_dev, RTE_ETH_EVENT_INTR_LSC, NULL);
    }
}
...
int sfc_intr_start(struct sfc_adapter *sa) {
    ...
    sfc_log_init(sa, "done type=%u max_intr=%d nb_efd=%u vec=%p", intr_handle->type, intr_handle->max_intr,
...
void sfc_intr_stop(struct sfc_adapter *sa) {
    ...
    if (intr->handler != NULL) {
        ...
        while ((rc = rte_intr_callback_unregister(intr_handle, intr->handler, (void *)sa)) == -EAGAIN)
            ;
        if (rc != 1)
