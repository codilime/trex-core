...
static void sfc_mcdi_timeout(struct sfc_adapter *sa) {
    sfc_warn(sa, "MC TIMEOUT");

    sfc_panic(sa, "MCDI timeout handling is not implemented\n");
}

static inline boolean_t sfc_mcdi_proxy_event_available(struct sfc_adapter *sa) {
...
static void sfc_mcdi_poll(struct sfc_adapter *sa, boolean_t proxy) {
    efx_nic_t *enp;
    ...
    do {
        ...
        poll_completed = (proxy) ? sfc_mcdi_proxy_event_available(sa) : efx_mcdi_request_poll(enp);
        if (poll_completed)
            return;
...
static void sfc_mcdi_execute(void *arg, efx_mcdi_req_t *emrp) {
    ...
    if (efx_mcdi_get_proxy_handle(sa->nic, emrp, &proxy_handle) == 0) {
        ...
        if ((mcdi->proxy_handle != 0) && (mcdi->proxy_handle != proxy_handle)) {
            ...
            sfc_err(sa,
                    "MCDI proxy authorization failed "
                    "(handle=%08x, result=%d)",
...
static void sfc_mcdi_ev_cpl(void *arg) {
    struct sfc_adapter *sa = (struct sfc_adapter *)arg;
...
static void sfc_mcdi_exception(void *arg, efx_mcdi_exception_t eme) {
    struct sfc_adapter *sa = (struct sfc_adapter *)arg;
...
static size_t sfc_mcdi_do_log(const struct sfc_adapter *sa, char *buffer, void *data, size_t data_size, size_t pfxsize,
                              size_t position) {
    uint32_t *words = data;
    ...
    for (i = 0; i < data_size; i += sizeof(*words)) {
        ...
        position += snprintf(buffer + position, SFC_MCDI_LOG_BUF_SIZE - position, " %08x", *words);
        words++;
    }
    return position;
}

static void sfc_mcdi_logger(void *arg, efx_log_msg_t type, void *header, size_t header_size, void *data,
                            size_t data_size) {
    struct sfc_adapter *sa = (struct sfc_adapter *)arg;
    ...
    pfxsize =
        snprintf(buffer, sizeof(buffer),
                 "MCDI RPC %s:", type == EFX_LOG_MCDI_REQUEST ? "REQ" : type == EFX_LOG_MCDI_RESPONSE ? "RESP" : "???");
    start = sfc_mcdi_do_log(sa, buffer, header, header_size, pfxsize, pfxsize);
...
static void sfc_mcdi_ev_proxy_response(void *arg, uint32_t handle, efx_rc_t result) {
    struct sfc_adapter *sa = (struct sfc_adapter *)arg;
    struct sfc_mcdi *mcdi = &sa->mcdi;
...
int sfc_mcdi_init(struct sfc_adapter *sa) {
    ...
    rc = sfc_dma_alloc(sa, "mcdi", 0, max_msg_size, sa->socket_id, &mcdi->mem);
    if (rc != 0)
        goto fail_dma_alloc;

    mcdi->logtype = sfc_register_logtype(&sa->priv.shared->pci_addr, SFC_LOGTYPE_MCDI_STR, RTE_LOG_NOTICE);
