...
int sfc_port_update_mac_stats(struct sfc_adapter *sa) {
     ...
     */
    if (!port->mac_stats_periodic_dma_supported || (port->mac_stats_update_period_ms == 0)) {
        if (port->mac_stats_update_period_ms != 0) {
            uint64_t timestamp = sfc_get_system_msecs();

            if ((timestamp - port->mac_stats_last_request_timestamp) < port->mac_stats_update_period_ms)
                return 0;
    ...
    } while ((genp != NULL) && (*genp == gen_old) && (++nb_attempts < SFC_MAC_STATS_UPDATE_NB_ATTEMPTS));
...
int sfc_port_start(struct sfc_adapter *sa) {
    ...
    const uint32_t phy_pause_caps = ((1u << EFX_PHY_CAP_PAUSE) | (1u << EFX_PHY_CAP_ASYM));
    unsigned int i;
    ...
    if (sa->eth_dev->data->dev_conf.lpbk_mode != 0) {
        ...
        sfc_log_init(sa, "set loopback link_mode=%u type=%u", link_mode, sa->eth_dev->data->dev_conf.lpbk_mode);
        rc = efx_port_loopback_set(sa->nic, link_mode, sa->eth_dev->data->dev_conf.lpbk_mode);
        if (rc != 0)
            goto fail_loopback_set;
    }
#endif

    sfc_log_init(sa, "set flow control to %#x autoneg=%u", port->flow_ctrl, port->flow_ctrl_autoneg);
    ...
    phy_adv_cap |= port->phy_adv_cap_mask &
                   (1u << EFX_PHY_CAP_BASER_FEC | 1u << EFX_PHY_CAP_RS_FEC | 1u << EFX_PHY_CAP_25G_BASER_FEC);
    ...
    efx_mac_stats_get_mask(sa->nic, port->mac_stats_mask, sizeof(port->mac_stats_mask));
    ...
    if (port->mac_stats_update_period_ms != 0) {
        ...
        rc = efx_mac_stats_periodic(sa->nic, &port->mac_stats_dma_mem, port->mac_stats_update_period_ms, B_FALSE);
        if (rc == 0) {
    ...
    if ((port->mac_stats_update_period_ms != 0) && port->mac_stats_periodic_dma_supported) {
        /*
...
void sfc_port_stop(struct sfc_adapter *sa) {
    ...
    (void)efx_mac_stats_periodic(sa->nic, &sa->port.mac_stats_dma_mem, 0, B_FALSE);
...
int sfc_port_attach(struct sfc_adapter *sa) {
    ...
    port->mcast_addrs =
        rte_calloc_socket("mcast_addr_list_buf", port->max_mcast_addrs, EFX_MAC_ADDR_LEN, 0, sa->socket_id);
    ...
    rc = ENOMEM;
    port->mac_stats_buf = rte_calloc_socket("mac_stats_buf", EFX_MAC_NSTATS, sizeof(uint64_t), 0, sa->socket_id);
    ...
    rc = sfc_dma_alloc(sa, "mac_stats", 0, mac_stats_size, sa->socket_id, &port->mac_stats_dma_mem);
    if (rc != 0)
    ...
    rc =
        sfc_kvargs_process(sa, SFC_KVARG_STATS_UPDATE_PERIOD_MS, sfc_kvarg_long_handler, &kvarg_stats_update_period_ms);
    if ((rc == 0) && ((kvarg_stats_update_period_ms < 0) || (kvarg_stats_update_period_ms > UINT16_MAX))) {
        sfc_err(sa,
...
int sfc_set_rx_mode(struct sfc_adapter *sa) {
    ...
    rc = efx_mac_filter_set(sa->nic, port->promisc, B_TRUE, port->promisc || port->allmulti, B_TRUE);

    return rc;
}

void sfc_port_link_mode_to_info(efx_link_mode_t link_mode, struct rte_eth_link *link_info) {
