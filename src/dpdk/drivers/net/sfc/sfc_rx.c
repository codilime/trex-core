...
static void sfc_efx_rx_qrefill(struct sfc_efx_rxq *rxq) {
    ...
    do {
        if (unlikely(rte_mempool_get_bulk(rxq->refill_mb_pool, objs, RTE_DIM(objs)) < 0)) {
            /*
        ...
        efx_rx_qpost(rxq->common, addr, rxq->buf_size, RTE_DIM(objs), rxq->completed, added);
        added += RTE_DIM(objs);
    } while (--bulks > 0);
...
static uint64_t sfc_efx_rx_desc_flags_to_offload_flags(const unsigned int desc_flags) {
    ...
    default:
        ...
        SFC_ASSERT((mbuf_flags & PKT_RX_IP_CKSUM_MASK) == PKT_RX_IP_CKSUM_UNKNOWN);
        break;
    }
    ...
    default:
        ...
        SFC_ASSERT((mbuf_flags & PKT_RX_L4_CKSUM_MASK) == PKT_RX_L4_CKSUM_UNKNOWN);
        break;
    }
...
static uint32_t sfc_efx_rx_desc_flags_to_packet_type(const unsigned int desc_flags) {
    return RTE_PTYPE_L2_ETHER | ((desc_flags & EFX_PKT_IPV4) ? RTE_PTYPE_L3_IPV4_EXT_UNKNOWN : 0) |
           ((desc_flags & EFX_PKT_IPV6) ? RTE_PTYPE_L3_IPV6_EXT_UNKNOWN : 0) |
           ((desc_flags & EFX_PKT_TCP) ? RTE_PTYPE_L4_TCP : 0) | ((desc_flags & EFX_PKT_UDP) ? RTE_PTYPE_L4_UDP : 0);
}

static const uint32_t *sfc_efx_supported_ptypes_get(__rte_unused uint32_t tunnel_encaps) {
...
static void sfc_efx_rx_set_rss_hash(struct sfc_efx_rxq *rxq, unsigned int flags, struct rte_mbuf *m) {
    uint8_t *mbuf_data;
    ...
    if (flags & (EFX_PKT_IPV4 | EFX_PKT_IPV6)) {
        m->hash.rss = efx_pseudo_hdr_hash_get(rxq->common, EFX_RX_HASHALG_TOEPLITZ, mbuf_data);

        m->ol_flags |= PKT_RX_RSS_HASH;
    }
}

static uint16_t sfc_efx_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    ...
    while (completed != rxq->pending && done_pkts < nb_pkts) {
        ...
        if (desc_flags & EFX_PKT_PREFIX_LEN) {
            ...
            rc = efx_pseudo_hdr_pkt_length_get(rxq->common, rte_pktmbuf_mtod(m, uint8_t *), &tmp_size);
            SFC_ASSERT(rc == 0);
            seg_len = tmp_size;
        } else {
...
static int sfc_efx_rx_qdesc_status(struct sfc_dp_rxq *dp_rxq, uint16_t offset) {
     ...
     */
    SFC_ASSERT((rxq->flags & SFC_EFX_RXQ_FLAG_RUNNING) == SFC_EFX_RXQ_FLAG_RUNNING);
    sfc_ev_qpoll(rxq->evq);
...
boolean_t sfc_rx_check_scatter(size_t pdu, size_t rx_buf_size, uint32_t rx_prefix_size, boolean_t rx_scatter_enabled,
                               const char **error) {
...
static int sfc_efx_rx_qsize_up_rings(uint16_t nb_rx_desc, __rte_unused struct sfc_dp_rx_hw_limits *limits,
...
static int sfc_efx_rx_qcreate(uint16_t port_id, uint16_t queue_id, const struct rte_pci_addr *pci_addr, int socket_id,
                              const struct sfc_dp_rx_qcreate_info *info, struct sfc_dp_rxq **dp_rxqp) {
    struct sfc_efx_rxq *rxq;
    int rc;

    rc = ENOMEM;
    rxq = rte_zmalloc_socket("sfc-efx-rxq", sizeof(*rxq), RTE_CACHE_LINE_SIZE, socket_id);
    if (rxq == NULL)
        goto fail_rxq_alloc;
    ...
    rc = ENOMEM;
    rxq->sw_desc = rte_calloc_socket("sfc-efx-rxq-sw_desc", info->rxq_entries, sizeof(*rxq->sw_desc),
...
static int sfc_efx_rx_qstart(struct sfc_dp_rxq *dp_rxq, __rte_unused unsigned int evq_read_ptr) {
...
static void sfc_efx_rx_qstop(struct sfc_dp_rxq *dp_rxq, __rte_unused unsigned int *evq_read_ptr) {
    struct sfc_efx_rxq *rxq = sfc_efx_rxq_by_dp_rxq(dp_rxq);
...
struct sfc_dp_rx sfc_efx_rx = {
    ...
    .features = SFC_DP_RX_FEAT_SCATTER | SFC_DP_RX_FEAT_CHECKSUM,
...
static void sfc_rx_qflush(struct sfc_adapter *sa, unsigned int sw_index) {
     ...
     */
    for (retry_count = 0; ((rxq_info->state & SFC_RXQ_FLUSHED) == 0) && (retry_count < SFC_RX_QFLUSH_ATTEMPTS);
         ++retry_count) {
        rc = efx_rx_qflush(rxq->common);
        if (rc != 0) {
            rxq_info->state |= (rc == EALREADY) ? SFC_RXQ_FLUSHED : SFC_RXQ_FLUSH_FAILED;
            break;
        }
        rxq_info->state &= ~SFC_RXQ_FLUSH_FAILED;
        rxq_info->state |= SFC_RXQ_FLUSHING;
        ...
        do {
            ...
            sfc_ev_qpoll(rxq->evq);
        } while ((rxq_info->state & SFC_RXQ_FLUSHING) && (wait_count++ < SFC_RX_QFLUSH_POLL_ATTEMPTS));
...
static int sfc_rx_default_rxq_set_filter(struct sfc_adapter *sa, struct sfc_rxq *rxq) {
...
int sfc_rx_qstart(struct sfc_adapter *sa, unsigned int sw_index) {
    ...
    case EFX_RXQ_TYPE_DEFAULT:
        rc = efx_rx_qcreate(sa->nic, rxq->hw_index, 0, rxq_info->type, rxq->buf_size, &rxq->mem, rxq_info->entries,
                            0 /* not used on EF10 */, rxq_info->type_flags, evq->common, &rxq->common);
        break;
    case EFX_RXQ_TYPE_ES_SUPER_BUFFER: {
        ...
        rc = efx_rx_qcreate_es_super_buffer(sa->nic, rxq->hw_index, 0, mp_info.contig_block_size, rxq->buf_size,
                                            mp->header_size + mp->elt_size + mp->trailer_size, sa->rxd_wait_timeout_ns,
                                            &rxq->mem, rxq_info->entries, rxq_info->type_flags, evq->common,
                                            &rxq->common);
        break;
    }
    default:
    ...
    sa->eth_dev->data->rx_queue_state[sw_index] = RTE_ETH_QUEUE_STATE_STARTED;
...
void sfc_rx_qstop(struct sfc_adapter *sa, unsigned int sw_index) {
    ...
    /* It seems to be used by DPDK for debug purposes only ('rte_ether') */
    sa->eth_dev->data->rx_queue_state[sw_index] = RTE_ETH_QUEUE_STATE_STOPPED;
...
uint64_t sfc_rx_get_dev_offload_caps(struct sfc_adapter *sa) {
    ...
    if (encp->enc_tunnel_encapsulations_supported && (sa->priv.dp_rx->features & SFC_DP_RX_FEAT_TUNNELS))
...
static int sfc_rx_qcheck_conf(struct sfc_adapter *sa, unsigned int rxq_max_fill_level,
                              const struct rte_eth_rxconf *rx_conf, __rte_unused uint64_t offloads) {
    int rc = 0;

    if (rx_conf->rx_thresh.pthresh != 0 || rx_conf->rx_thresh.hthresh != 0 || rx_conf->rx_thresh.wthresh != 0) {
        sfc_err(sa, "RxQ prefetch/host/writeback thresholds are not supported");
        rc = EINVAL;
    }

    if (rx_conf->rx_free_thresh > rxq_max_fill_level) {
        sfc_err(sa, "RxQ free threshold too large: %u vs maximum %u", rx_conf->rx_free_thresh, rxq_max_fill_level);
        rc = EINVAL;
    }
...
static unsigned int sfc_rx_mbuf_data_alignment(struct rte_mempool *mb_pool) {
    ...
    data_off = sizeof(struct rte_mbuf) + rte_pktmbuf_priv_size(mb_pool) + RTE_PKTMBUF_HEADROOM;
...
static uint16_t sfc_rx_mb_pool_buf_size(struct sfc_adapter *sa, struct rte_mempool *mb_pool) {
    const efx_nic_cfg_t *encp = efx_nic_cfg_get(sa->nic);
    ...
    if (buf_size <= RTE_PKTMBUF_HEADROOM) {
        sfc_err(sa, "RxQ mbuf pool %s object data room size %u is smaller than headroom %u", mb_pool->name, buf_size,
    ...
    if (buf_aligned < nic_align_start) {
        ...
        if (buf_size <= start_alignment) {
            sfc_err(sa,
                    "RxQ mbuf pool %s object data room size %u is insufficient for headroom %u and buffer start "
                    "alignment %u required by NIC",
                    mb_pool->name, rte_pktmbuf_data_room_size(mb_pool), RTE_PKTMBUF_HEADROOM, start_alignment);
            return 0;
        }
    ...
    if (buf_aligned < nic_align_end) {
         ...
         */
        end_padding_alignment = nic_align_end - MIN(buf_aligned, 1u << (rte_bsf32(buf_size) - 1));
        if (buf_size <= end_padding_alignment) {
            sfc_err(sa,
                    "RxQ mbuf pool %s object data room size %u is insufficient for headroom %u, buffer start alignment "
                    "%u and end padding alignment %u required by NIC",
                    mb_pool->name, rte_pktmbuf_data_room_size(mb_pool), RTE_PKTMBUF_HEADROOM, start_alignment,
...
int sfc_rx_qinit(struct sfc_adapter *sa, unsigned int sw_index, uint16_t nb_rx_desc, unsigned int socket_id,
    ...
    rc = sa->priv.dp_rx->qsize_up_rings(nb_rx_desc, &hw_limits, mb_pool, &rxq_entries, &evq_entries,
                                        &rxq_max_fill_level);
    if (rc != 0)
    ...
    offloads = rx_conf->offloads | sa->eth_dev->data->dev_conf.rxmode.offloads;
    ...
    if (buf_size == 0) {
        sfc_err(sa, "RxQ %u mbuf pool object size is too small", sw_index);
        rc = EINVAL;
        goto fail_bad_conf;
    }

    if (!sfc_rx_check_scatter(sa->port.pdu, buf_size, encp->enc_rx_prefix_size, (offloads & DEV_RX_OFFLOAD_SCATTER),
                              &error)) {
        ...
        sfc_err(sa,
                ...
                sw_index, buf_size, (unsigned int)sa->port.pdu, encp->enc_rx_prefix_size);
        rc = EINVAL;
        goto fail_bad_conf;
    }
    ...
    rxq_info->type_flags = (offloads & DEV_RX_OFFLOAD_SCATTER) ? EFX_RXQ_FLAG_SCATTER : EFX_RXQ_FLAG_NONE;

    if ((encp->enc_tunnel_encapsulations_supported != 0) && (sa->priv.dp_rx->features & SFC_DP_RX_FEAT_TUNNELS))
        rxq_info->type_flags |= EFX_RXQ_FLAG_INNER_CLASSES;

    rc = sfc_ev_qinit(sa, SFC_EVQ_TYPE_RX, sw_index, evq_entries, socket_id, &evq);
    if (rc != 0)
        goto fail_ev_qinit;
     ...
     */
    rx_free_thresh = (rx_conf->rx_free_thresh != 0) ? rx_conf->rx_free_thresh : EFX_DIV_ROUND_UP(nb_rx_desc, 8);
    ...
    rc = sfc_dma_alloc(sa, "rxq", sw_index, efx_rxq_size(sa->nic, rxq_info->entries), socket_id, &rxq->mem);
    if (rc != 0)
        goto fail_dma_alloc;
    ...
    rc = sa->priv.dp_rx->qcreate(sa->eth_dev->data->port_id, sw_index, &RTE_ETH_DEV_TO_PCI(sa->eth_dev)->addr,
...
static const struct sfc_rss_hf_rte_to_efx sfc_rss_hf_map[] = {
    {ETH_RSS_NONFRAG_IPV4_TCP, EFX_RX_HASH(IPV4_TCP, 4TUPLE)},
    {ETH_RSS_NONFRAG_IPV4_UDP, EFX_RX_HASH(IPV4_UDP, 4TUPLE)},
    {ETH_RSS_NONFRAG_IPV6_TCP | ETH_RSS_IPV6_TCP_EX, EFX_RX_HASH(IPV6_TCP, 4TUPLE)},
    {ETH_RSS_NONFRAG_IPV6_UDP | ETH_RSS_IPV6_UDP_EX, EFX_RX_HASH(IPV6_UDP, 4TUPLE)},
    {ETH_RSS_IPV4 | ETH_RSS_FRAG_IPV4 | ETH_RSS_NONFRAG_IPV4_OTHER,
     EFX_RX_HASH(IPV4_TCP, 2TUPLE) | EFX_RX_HASH(IPV4_UDP, 2TUPLE) | EFX_RX_HASH(IPV4, 2TUPLE)},
    {ETH_RSS_IPV6 | ETH_RSS_FRAG_IPV6 | ETH_RSS_NONFRAG_IPV6_OTHER | ETH_RSS_IPV6_EX,
     EFX_RX_HASH(IPV6_TCP, 2TUPLE) | EFX_RX_HASH(IPV6_UDP, 2TUPLE) | EFX_RX_HASH(IPV6, 2TUPLE)}};

static efx_rx_hash_type_t sfc_rx_hash_types_mask_supp(efx_rx_hash_type_t hash_type,
    ...
    for (i = 0; i < nb_hash_type_flags_supported; ++i) {
        unsigned int class_tuple_lbn[] = {EFX_RX_CLASS_IPV4_TCP_LBN, EFX_RX_CLASS_IPV4_UDP_LBN, EFX_RX_CLASS_IPV4_LBN,
                                          EFX_RX_CLASS_IPV6_TCP_LBN, EFX_RX_CLASS_IPV6_UDP_LBN, EFX_RX_CLASS_IPV6_LBN};
...
int sfc_rx_hash_init(struct sfc_adapter *sa) {
    ...
    rc = efx_rx_scale_hash_flags_get(sa->nic, alg, flags_supp, RTE_DIM(flags_supp), &nb_flags_supp);
    if (rc != 0)
        return rc;

    hf_map = rte_calloc_socket("sfc-rss-hf-map", RTE_DIM(sfc_rss_hf_map), sizeof(*hf_map), 0, sa->socket_id);
    if (hf_map == NULL)
        return ENOMEM;
    ...
    for (i = 0; i < RTE_DIM(sfc_rss_hf_map); ++i) {
        ...
        ht = sfc_rx_hash_types_mask_supp(sfc_rss_hf_map[i].efx, flags_supp, nb_flags_supp);
        if (ht != 0) {
...
int sfc_rx_hf_rte_to_efx(struct sfc_adapter *sa, uint64_t rte, efx_rx_hash_type_t *efx) {
    struct sfc_rss *rss = &sfc_sa2shared(sa)->rss;
...
static int sfc_rx_process_adv_conf_rss(struct sfc_adapter *sa, struct rte_eth_rss_conf *conf) {
    struct sfc_rss *rss = &sfc_sa2shared(sa)->rss;
...
static int sfc_rx_rss_config(struct sfc_adapter *sa) {
    ...
    if (rss->channels > 0) {
        rc = efx_rx_scale_mode_set(sa->nic, EFX_RSS_CONTEXT_DEFAULT, rss->hash_alg, rss->hash_types, B_TRUE);
        if (rc != 0)
            goto finish;

        rc = efx_rx_scale_key_set(sa->nic, EFX_RSS_CONTEXT_DEFAULT, rss->key, sizeof(rss->key));
        if (rc != 0)
            goto finish;

        rc = efx_rx_scale_tbl_set(sa->nic, EFX_RSS_CONTEXT_DEFAULT, rss->tbl, RTE_DIM(rss->tbl));
    }
...
int sfc_rx_start(struct sfc_adapter *sa) {
    ...
    for (sw_index = 0; sw_index < sas->rxq_count; ++sw_index) {
        if (sas->rxq_info[sw_index].state == SFC_RXQ_INITIALIZED &&
            (!sas->rxq_info[sw_index].deferred_start || sas->rxq_info[sw_index].deferred_started)) {
...
static int sfc_rx_qinit_info(struct sfc_adapter *sa, unsigned int sw_index) {
    struct sfc_adapter_shared *const sas = sfc_sa2shared(sa);
...
static int sfc_rx_check_mode(struct sfc_adapter *sa, struct rte_eth_rxmode *rxmode) {
    struct sfc_adapter_shared *const sas = sfc_sa2shared(sa);
    uint64_t offloads_supported = sfc_rx_get_dev_offload_caps(sa) | sfc_rx_get_queue_offload_caps(sa);
     ...
     * below check.
     */
    if ((rxmode->offloads & DEV_RX_OFFLOAD_CHECKSUM) != (offloads_supported & DEV_RX_OFFLOAD_CHECKSUM)) {
 ...
 * or complete close.
 */
static void sfc_rx_fini_queues(struct sfc_adapter *sa, unsigned int nb_rx_queues) {
    struct sfc_adapter_shared *const sas = sfc_sa2shared(sa);
    int sw_index;
...
int sfc_rx_configure(struct sfc_adapter *sa) {
    ...
    sfc_log_init(sa, "nb_rx_queues=%u (old %u)", nb_rx_queues, sas->rxq_count);
    ...
    if (sas->rxq_info == NULL) {
        rc = ENOMEM;
        sas->rxq_info = rte_calloc_socket("sfc-rxqs", nb_rx_queues, sizeof(sas->rxq_info[0]), 0, sa->socket_id);
        if (sas->rxq_info == NULL)
        ...
        rc = ENOMEM;
        new_rxq_info = rte_realloc(sas->rxq_info, nb_rx_queues * sizeof(sas->rxq_info[0]), 0);
        if (new_rxq_info == NULL && nb_rx_queues > 0)
            goto fail_rxqs_realloc;

        rc = ENOMEM;
        new_rxq_ctrl = realloc(sa->rxq_ctrl, nb_rx_queues * sizeof(sa->rxq_ctrl[0]));
        ...
        if (nb_rx_queues > sas->rxq_count) {
            memset(&sas->rxq_info[sas->rxq_count], 0, (nb_rx_queues - sas->rxq_count) * sizeof(sas->rxq_info[0]));
            memset(&sa->rxq_ctrl[sas->rxq_count], 0, (nb_rx_queues - sas->rxq_count) * sizeof(sa->rxq_ctrl[0]));
        }
    }
...
configure_rss:
    rss->channels = (dev_conf->rxmode.mq_mode == ETH_MQ_RX_RSS) ? MIN(sas->rxq_count, EFX_MAXRSS) : 0;
