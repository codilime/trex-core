...
int sfc_efx_tso_alloc_tsoh_objs(struct sfc_efx_tx_sw_desc *sw_ring, unsigned int txq_entries, unsigned int socket_id) {
    unsigned int i;

    for (i = 0; i < txq_entries; ++i) {
        sw_ring[i].tsoh = rte_malloc_socket("sfc-efx-txq-tsoh-obj", SFC_TSOH_STD_LEN, RTE_CACHE_LINE_SIZE, socket_id);
...
void sfc_efx_tso_free_tsoh_objs(struct sfc_efx_tx_sw_desc *sw_ring, unsigned int txq_entries) {
    unsigned int i;
...
unsigned int sfc_tso_prepare_header(uint8_t *tsoh, size_t header_len, struct rte_mbuf **in_seg, size_t *in_off) {
...
int sfc_efx_tso_do(struct sfc_efx_txq *txq, unsigned int idx, struct rte_mbuf **in_seg, size_t *in_off,
                   efx_desc_t **pend, unsigned int *pkt_descs, size_t *pkt_len) {
    uint8_t *tsoh;
    ...
    efx_tx_qdesc_tso2_create(txq->common, packet_id, 0, sent_seq, m->tso_segsz, *pend, EFX_TX_FATSOV2_OPT_NDESCS);

    *pend += EFX_TX_FATSOV2_OPT_NDESCS;
    *pkt_descs += EFX_TX_FATSOV2_OPT_NDESCS;

    efx_tx_qdesc_dma_create(txq->common, header_paddr, header_len, B_FALSE, (*pend)++);
    (*pkt_descs)++;
    *pkt_len -= header_len;
