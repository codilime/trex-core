...
uint64_t sfc_tx_get_dev_offload_caps(struct sfc_adapter *sa) {
    ...
    if ((sa->priv.dp_tx->features & SFC_DP_TX_FEAT_VLAN_INSERT) && encp->enc_hw_tx_insert_vlan_enabled)
        caps |= DEV_TX_OFFLOAD_VLAN_INSERT;

    if (sa->priv.dp_tx->features & SFC_DP_TX_FEAT_MULTI_SEG)
        caps |= DEV_TX_OFFLOAD_MULTI_SEGS;

    if ((~sa->priv.dp_tx->features & SFC_DP_TX_FEAT_MULTI_POOL) && (~sa->priv.dp_tx->features & SFC_DP_TX_FEAT_REFCNT))
...
uint64_t sfc_tx_get_queue_offload_caps(struct sfc_adapter *sa) {
    ...
    if (sa->tso_encap)
        caps |= (DEV_TX_OFFLOAD_VXLAN_TNL_TSO | DEV_TX_OFFLOAD_GENEVE_TNL_TSO);

    return caps;
}

static int sfc_tx_qcheck_conf(struct sfc_adapter *sa, unsigned int txq_max_fill_level,
                              const struct rte_eth_txconf *tx_conf, uint64_t offloads) {
    int rc = 0;
    ...
    if (tx_conf->tx_free_thresh > txq_max_fill_level) {
        sfc_err(sa, "TxQ free threshold too large: %u vs maximum %u", tx_conf->tx_free_thresh, txq_max_fill_level);
        rc = EINVAL;
    }

    if (tx_conf->tx_thresh.pthresh != 0 || tx_conf->tx_thresh.hthresh != 0 || tx_conf->tx_thresh.wthresh != 0) {
        sfc_err(sa, "prefetch/host/writeback thresholds are not supported");
        rc = EINVAL;
    }

    /* We either perform both TCP and UDP offload, or no offload at all */
    if (((offloads & DEV_TX_OFFLOAD_TCP_CKSUM) == 0) != ((offloads & DEV_TX_OFFLOAD_UDP_CKSUM) == 0)) {
...
int sfc_tx_qinit(struct sfc_adapter *sa, unsigned int sw_index, uint16_t nb_tx_desc, unsigned int socket_id,
    ...
    rc = sa->priv.dp_tx->qsize_up_rings(nb_tx_desc, &hw_limits, &txq_entries, &evq_entries, &txq_max_fill_level);
    if (rc != 0)
    ...
    offloads = tx_conf->offloads | sa->eth_dev->data->dev_conf.txmode.offloads;
    ...
    rc = sfc_ev_qinit(sa, SFC_EVQ_TYPE_TX, sw_index, evq_entries, socket_id, &evq);
    if (rc != 0)
        goto fail_ev_qinit;
    ...
    txq->evq = evq;
    txq_info->free_thresh = (tx_conf->tx_free_thresh) ? tx_conf->tx_free_thresh : SFC_TX_DEFAULT_FREE_THRESH;
    txq_info->offloads = offloads;

    rc = sfc_dma_alloc(sa, "txq", sw_index, efx_txq_size(sa->nic, txq_info->entries), socket_id, &txq->mem);
    if (rc != 0)
        goto fail_dma_alloc;
    ...
    rc = sa->priv.dp_tx->qcreate(sa->eth_dev->data->port_id, sw_index, &RTE_ETH_DEV_TO_PCI(sa->eth_dev)->addr,
...
static int sfc_tx_qinit_info(struct sfc_adapter *sa, unsigned int sw_index) {
    sfc_log_init(sa, "TxQ = %u", sw_index);

    return 0;
}

static int sfc_tx_check_mode(struct sfc_adapter *sa, const struct rte_eth_txmode *txmode) {
    int rc = 0;
 ...
 * or complete close.
 */
static void sfc_tx_fini_queues(struct sfc_adapter *sa, unsigned int nb_tx_queues) {
...
int sfc_tx_configure(struct sfc_adapter *sa) {
    ...
    sfc_log_init(sa, "nb_tx_queues=%u (old %u)", nb_tx_queues, sas->txq_count);
    ...
    if (sas->txq_info == NULL) {
        sas->txq_info = rte_calloc_socket("sfc-txqs", nb_tx_queues, sizeof(sas->txq_info[0]), 0, sa->socket_id);
        if (sas->txq_info == NULL)
        ...
        new_txq_info = rte_realloc(sas->txq_info, nb_tx_queues * sizeof(sas->txq_info[0]), 0);
        if (new_txq_info == NULL && nb_tx_queues > 0)
            goto fail_txqs_realloc;

        new_txq_ctrl = realloc(sa->txq_ctrl, nb_tx_queues * sizeof(sa->txq_ctrl[0]));
        ...
        if (nb_tx_queues > sas->txq_count) {
            memset(&sas->txq_info[sas->txq_count], 0, (nb_tx_queues - sas->txq_count) * sizeof(sas->txq_info[0]));
            memset(&sa->txq_ctrl[sas->txq_count], 0, (nb_tx_queues - sas->txq_count) * sizeof(sa->txq_ctrl[0]));
        }
    }
...
int sfc_tx_qstart(struct sfc_adapter *sa, unsigned int sw_index) {
    struct sfc_adapter_shared *const sas = sfc_sa2shared(sa);
    uint64_t offloads_supported = sfc_tx_get_dev_offload_caps(sa) | sfc_tx_get_queue_offload_caps(sa);
    ...
    if ((txq_info->offloads & DEV_TX_OFFLOAD_TCP_CKSUM) || (txq_info->offloads & DEV_TX_OFFLOAD_UDP_CKSUM)) {
    ...
    if (txq_info->offloads & (DEV_TX_OFFLOAD_TCP_TSO | DEV_TX_OFFLOAD_VXLAN_TNL_TSO | DEV_TX_OFFLOAD_GENEVE_TNL_TSO))
        flags |= EFX_TXQ_FATSOV2;

    rc = efx_tx_qcreate(sa->nic, txq->hw_index, 0, &txq->mem, txq_info->entries, 0 /* not used on EF10 */, flags,
                        evq->common, &txq->common, &desc_index);
    if (rc != 0) {
        if (sa->tso && (rc == ENOSPC))
...
void sfc_tx_qstop(struct sfc_adapter *sa, unsigned int sw_index) {
     ...
     */
    for (retry_count = 0; ((txq_info->state & SFC_TXQ_FLUSHED) == 0) && (retry_count < SFC_TX_QFLUSH_ATTEMPTS);
         ++retry_count) {
        rc = efx_tx_qflush(txq->common);
        if (rc != 0) {
            txq_info->state |= (rc == EALREADY) ? SFC_TXQ_FLUSHED : SFC_TXQ_FLUSH_FAILED;
            break;
        }
        ...
        } while ((txq_info->state & SFC_TXQ_FLUSHING) && wait_count++ < SFC_TX_QFLUSH_POLL_ATTEMPTS);
...
int sfc_tx_start(struct sfc_adapter *sa) {
    ...
    for (sw_index = 0; sw_index < sas->txq_count; ++sw_index) {
        if (sas->txq_info[sw_index].state == SFC_TXQ_INITIALIZED &&
            (!(sas->txq_info[sw_index].deferred_start) || sas->txq_info[sw_index].deferred_started)) {
 ...
 */
static unsigned int sfc_efx_tx_maybe_insert_tag(struct sfc_efx_txq *txq, struct rte_mbuf *m, efx_desc_t **pend) {
    uint16_t this_tag = ((m->ol_flags & PKT_TX_VLAN_PKT) ? m->vlan_tci : 0);
...
static uint16_t sfc_efx_prepare_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    ...
    for (i = 0; i < nb_pkts; i++) {
         ...
         * requested/supported.
         */
        ret = sfc_dp_tx_prepare_pkt(tx_pkts[i], encp->enc_tx_tso_tcp_header_offset_limit, txq->max_fill_level,
                                    EFX_TX_FATSOV2_OPT_NDESCS, 1);
        if (unlikely(ret != 0)) {
            rte_errno = ret;
            break;
        }
...
static uint16_t sfc_efx_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    ...
    for (pkts_sent = 0, pktp = &tx_pkts[0]; (pkts_sent < nb_pkts) && (fill_level <= soft_max_fill);
         pkts_sent++, pktp++) {
        ...
        if (m_seg->ol_flags & PKT_TX_TCP_SEG) {
             ...
             */
            if (sfc_efx_tso_do(txq, added, &m_seg, &in_off, &pend, &pkt_descs, &pkt_len) != 0) {
        ...
        for (; m_seg != NULL; m_seg = m_seg->next) {
            ...
            do {
                ...
                efx_tx_qdesc_dma_create(txq->common, frag_addr, frag_len, (pkt_len == 0), pend++);
    ...
    if (likely(pkts_sent > 0)) {
        rc = efx_tx_qdesc_post(txq->common, txq->pend_desc, pend - &txq->pend_desc[0], txq->completed, &txq->added);
        SFC_ASSERT(rc == 0);
...
static int sfc_efx_tx_qsize_up_rings(uint16_t nb_tx_desc, __rte_unused struct sfc_dp_tx_hw_limits *limits,
...
static int sfc_efx_tx_qcreate(uint16_t port_id, uint16_t queue_id, const struct rte_pci_addr *pci_addr, int socket_id,
    ...
    rc = ENOMEM;
    txq = rte_zmalloc_socket("sfc-efx-txq", sizeof(*txq), RTE_CACHE_LINE_SIZE, socket_id);
    if (txq == NULL)
        goto fail_txq_alloc;
    ...
    rc = ENOMEM;
    txq->pend_desc = rte_calloc_socket("sfc-efx-txq-pend-desc", EFX_TXQ_LIMIT(info->txq_entries),
                                       sizeof(*txq->pend_desc), 0, socket_id);
    if (txq->pend_desc == NULL)
        goto fail_pend_desc_alloc;

    rc = ENOMEM;
    txq->sw_ring = rte_calloc_socket("sfc-efx-txq-sw_ring", info->txq_entries, sizeof(*txq->sw_ring),
    ...
    if (ctrl_txq->evq->sa->tso) {
        rc = sfc_efx_tso_alloc_tsoh_objs(txq->sw_ring, info->txq_entries, socket_id);
        if (rc != 0)
...
static int sfc_efx_tx_qstart(struct sfc_dp_txq *dp_txq, __rte_unused unsigned int evq_read_ptr,
...
static void sfc_efx_tx_qstop(struct sfc_dp_txq *dp_txq, __rte_unused unsigned int *evq_read_ptr) {
    struct sfc_efx_txq *txq = sfc_efx_txq_by_dp_txq(dp_txq);
...
static int sfc_efx_tx_qdesc_status(struct sfc_dp_txq *dp_txq, uint16_t offset) {
     ...
     */
    SFC_ASSERT((txq->flags & SFC_EFX_TXQ_FLAG_RUNNING) == SFC_EFX_TXQ_FLAG_RUNNING);
    sfc_ev_qpoll(txq->evq);
...
struct sfc_dp_tx sfc_efx_tx = {
    ...
    .features = SFC_DP_TX_FEAT_VLAN_INSERT | SFC_DP_TX_FEAT_TSO | SFC_DP_TX_FEAT_MULTI_POOL | SFC_DP_TX_FEAT_REFCNT |
