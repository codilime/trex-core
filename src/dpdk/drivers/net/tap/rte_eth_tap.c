...
static const char *valid_arguments[] = {ETH_TAP_IFACE_ARG, ETH_TAP_REMOTE_ARG, ETH_TAP_MAC_ARG, NULL};
...
static int tun_alloc(struct pmd_internals *pmd, int is_keepalive) {
    ...
    if (features & IFF_MULTI_QUEUE) {
        TAP_LOG(DEBUG, "  Multi-queue support for %d queues", RTE_PMD_TAP_MAX_QUEUES);
    ...
    if (ioctl(fd, TUNSETIFF, (void *)&ifr) < 0) {
        TAP_LOG(WARNING, "Unable to set TUNSETIFF for %s: %s", ifr.ifr_name, strerror(errno));
        goto error;
    }
    ...
    if (is_keepalive) {
        ...
        if (ioctl(fd, TUNSETQUEUE, (void *)&ifr) < 0) {
            TAP_LOG(WARNING, "Unable to detach keep-alive queue for %s: %s", ifr.ifr_name, strerror(errno));
            goto error;
        }
    }
    ...
    if (fcntl(fd, F_SETFL, O_NONBLOCK) < 0) {
        TAP_LOG(WARNING, "Unable to set %s to nonblocking: %s", ifr.ifr_name, strerror(errno));
        goto error;
    }
    ...
    if (errno) {
        ...
        tap_trigger = 0;
        TAP_LOG(WARNING, "Rx trigger disabled: %s", strerror(errno));
    }
...
static void tap_verify_csum(struct rte_mbuf *mbuf) {
    ...
    if (unlikely(l2_len + sizeof(struct ipv4_hdr) > rte_pktmbuf_data_len(mbuf)))
        return;
    ...
    if (l3 == RTE_PTYPE_L3_IPV4 || l3 == RTE_PTYPE_L3_IPV4_EXT) {
         ...
         */
        if (l2_len + rte_be_to_cpu_16(iph->total_length) > rte_pktmbuf_data_len(mbuf))
            return;

        cksum = ~rte_raw_cksum(iph, l3_len);
        mbuf->ol_flags |= cksum ? PKT_RX_IP_CKSUM_BAD : PKT_RX_IP_CKSUM_GOOD;
         ...
         * greater than the total received size
         */
        if (l2_len + l3_len + rte_be_to_cpu_16(iph->payload_len) > rte_pktmbuf_data_len(mbuf))
            return;
    } else {
...
static uint64_t tap_rx_offload_get_port_capa(void) {
    /*
...
static uint64_t tap_rx_offload_get_queue_capa(void) {
    return DEV_RX_OFFLOAD_SCATTER | DEV_RX_OFFLOAD_IPV4_CKSUM | DEV_RX_OFFLOAD_UDP_CKSUM | DEV_RX_OFFLOAD_TCP_CKSUM;
}

/* Callback to handle the rx burst of packets to the correct interface and
 * file descriptor(s) in a multi-queue setup.
 */
static uint16_t pmd_rx_burst(void *queue, struct rte_mbuf **bufs, uint16_t nb_pkts) {
    ...
    for (num_rx = 0; num_rx < nb_pkts;) {
        ...
        len = readv(process_private->rxq_fds[rxq->queue_id], *rxq->iovecs,
                    1 + (rxq->rxmode->offloads & DEV_RX_OFFLOAD_SCATTER ? rxq->nb_rx_desc : 1));
...
static uint64_t tap_tx_offload_get_port_capa(void) {
    /*
...
static uint64_t tap_tx_offload_get_queue_capa(void) {
    return DEV_TX_OFFLOAD_MULTI_SEGS | DEV_TX_OFFLOAD_IPV4_CKSUM | DEV_TX_OFFLOAD_UDP_CKSUM | DEV_TX_OFFLOAD_TCP_CKSUM |
           DEV_TX_OFFLOAD_TCP_TSO;
}

/* Finalize l4 checksum calculation */
static void tap_tx_l4_cksum(uint16_t *l4_cksum, uint16_t l4_phdr_cksum, uint32_t l4_raw_cksum) {
    if (l4_cksum) {
        uint32_t cksum;
...
static void tap_tx_l4_add_rcksum(char *l4_data, unsigned int l4_len, uint16_t *l4_cksum, uint32_t *l4_raw_cksum) {
    if (l4_cksum == NULL)
        return;
...
static void tap_tx_l3_cksum(char *packet, uint64_t ol_flags, unsigned int l2_len, unsigned int l3_len,
                            unsigned int l4_len, uint16_t **l4_cksum, uint16_t *l4_phdr_cksum, uint32_t *l4_raw_cksum) {
...
static inline void tap_write_mbufs(struct tx_queue *txq, uint16_t num_mbufs, struct rte_mbuf **pmbufs,
    ...
    for (i = 0; i < num_mbufs; i++) {
        ...
        if (txq->type == ETH_TUNTAP_TYPE_TUN) {
            ...
            pi.proto = (proto == 0x40) ? rte_cpu_to_be_16(ETHER_TYPE_IPv4)
                                       : ((proto == 0x60) ? rte_cpu_to_be_16(ETHER_TYPE_IPv6) : 0x00);
        }
        ...
        if (txq->csum && ((mbuf->ol_flags & (PKT_TX_IP_CKSUM | PKT_TX_IPV4) ||
            ...
            tap_tx_l3_cksum(m_copy, mbuf->ol_flags, mbuf->l2_len, mbuf->l3_len, mbuf->l4_len, &l4_cksum, &l4_phdr_cksum,
                            &l4_raw_cksum);
            ...
            if (seg_len > l234_hlen) {
                ...
                tap_tx_l4_add_rcksum(iovecs[k].iov_base, iovecs[k].iov_len, l4_cksum, &l4_raw_cksum);
                k++;
                nb_segs++;
            }
            seg = seg->next;
        ...
        for (j = k; j <= nb_segs; j++) {
            ...
            if (is_cksum)
                tap_tx_l4_add_rcksum(iovecs[j].iov_base, iovecs[j].iov_len, l4_cksum, &l4_raw_cksum);
            seg = seg->next;
        }
 ...
 */
static uint16_t pmd_tx_burst(void *queue, struct rte_mbuf **bufs, uint16_t nb_pkts) {
    ...
    for (i = 0; i < nb_pkts; i++) {
        ...
        tap_write_mbufs(txq, num_mbufs, mbuf, &num_packets, &num_tx_bytes);
        num_tx++;
        /* free original mbuf */
...
static int tap_ioctl(struct pmd_internals *pmd, unsigned long request, struct ifreq *ifr, int set,
                     enum ioctl_mode mode) {
    short req_flags = ifr->ifr_flags;
    int remote = pmd->remote_if_index && (mode == REMOTE_ONLY || mode == LOCAL_AND_REMOTE);
...
error:
    TAP_LOG(DEBUG, "%s(%s) failed: %s(%d)", ifr->ifr_name, tap_ioctl_req2str(request), strerror(errno), errno);
    return -errno;
}
 ...
 */
static void tap_dev_stop(struct rte_eth_dev *dev) {
    int i;
...
static int tap_dev_configure(struct rte_eth_dev *dev) {
    ...
    if (dev->data->nb_rx_queues > RTE_PMD_TAP_MAX_QUEUES) {
        TAP_LOG(ERR, "%s: number of rx queues %d exceeds max num of queues %d", dev->device->name,
                dev->data->nb_rx_queues, RTE_PMD_TAP_MAX_QUEUES);
        return -1;
    }
    if (dev->data->nb_tx_queues > RTE_PMD_TAP_MAX_QUEUES) {
        TAP_LOG(ERR, "%s: number of tx queues %d exceeds max num of queues %d", dev->device->name,
                dev->data->nb_tx_queues, RTE_PMD_TAP_MAX_QUEUES);
        return -1;
    }

    TAP_LOG(INFO, "%s: %s: TX configured queues number: %u", dev->device->name, pmd->name, dev->data->nb_tx_queues);

    TAP_LOG(INFO, "%s: %s: RX configured queues number: %u", dev->device->name, pmd->name, dev->data->nb_rx_queues);
...
static void tap_dev_info(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info) {
    ...
    dev_info->rx_offload_capa = tap_rx_offload_get_port_capa() | dev_info->rx_queue_offload_capa;
    dev_info->tx_queue_offload_capa = tap_tx_offload_get_queue_capa();
    dev_info->tx_offload_capa = tap_tx_offload_get_port_capa() | dev_info->tx_queue_offload_capa;
...
static int tap_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *tap_stats) {
    unsigned int i, imax;
    ...
    /* rx queue statistics */
    imax =
        (dev->data->nb_rx_queues < RTE_ETHDEV_QUEUE_STAT_CNTRS) ? dev->data->nb_rx_queues : RTE_ETHDEV_QUEUE_STAT_CNTRS;
    for (i = 0; i < imax; i++) {
    ...
    /* tx queue statistics */
    imax =
        (dev->data->nb_tx_queues < RTE_ETHDEV_QUEUE_STAT_CNTRS) ? dev->data->nb_tx_queues : RTE_ETHDEV_QUEUE_STAT_CNTRS;
...
static void tap_stats_reset(struct rte_eth_dev *dev) {
    int i;
    struct pmd_internals *pmd = dev->data->dev_private;
...
static void tap_dev_close(struct rte_eth_dev *dev) {
    int i;
    struct pmd_internals *internals = dev->data->dev_private;
    ...
    if (internals->remote_if_index) {
        ...
        ioctl(internals->ioctl_sock, SIOCSIFFLAGS, &internals->remote_initial_flags);
    }
...
static void tap_rx_queue_release(void *queue) {
    struct rx_queue *rxq = queue;
    struct pmd_process_private *process_private;
...
static void tap_tx_queue_release(void *queue) {
    struct tx_queue *txq = queue;
    struct pmd_process_private *process_private;
...
static int tap_link_update(struct rte_eth_dev *dev, int wait_to_complete __rte_unused) {
...
static void tap_promisc_enable(struct rte_eth_dev *dev) {
    struct pmd_internals *pmd = dev->data->dev_private;
...
static void tap_promisc_disable(struct rte_eth_dev *dev) {
    struct pmd_internals *pmd = dev->data->dev_private;
    struct ifreq ifr = {.ifr_flags = IFF_PROMISC};
...
static void tap_allmulti_enable(struct rte_eth_dev *dev) {
    struct pmd_internals *pmd = dev->data->dev_private;
...
static void tap_allmulti_disable(struct rte_eth_dev *dev) {
    struct pmd_internals *pmd = dev->data->dev_private;
    struct ifreq ifr = {.ifr_flags = IFF_ALLMULTI};
...
static int tap_mac_set(struct rte_eth_dev *dev, struct ether_addr *mac_addr) {
    struct pmd_internals *pmd = dev->data->dev_private;
    ...
    if (ret < 0)
        return ret;
    if (is_same_ether_addr((struct ether_addr *)&ifr.ifr_hwaddr.sa_data, mac_addr))
        return 0;
    ...
    if (pmd->remote_if_index && !pmd->flow_isolate) {
        ...
        if (ret < 0) {
            TAP_LOG(ERR, "%s: Couldn't delete MAC redirection rule", dev->device->name);
            return ret;
        }
...
static int tap_gso_ctx_setup(struct rte_gso_ctx *gso_ctx, struct rte_eth_dev *dev) {
    uint32_t gso_types;
    char pool_name[64];
    ...
    if (!mp) {
        mp = rte_pktmbuf_pool_create(pool_name, TAP_GSO_MBUFS_NUM, TAP_GSO_MBUF_CACHE_SIZE, 0,
...
static int tap_setup_queue(struct rte_eth_dev *dev, struct pmd_internals *internals, uint16_t qid, int is_rx) {
    int ret;
    int *fd;
    int *other_fd;
    const char *dir;
    ...
    if (*fd != -1) {
        ...
        TAP_LOG(DEBUG, "%s: fd %d for %s queue qid %d exists", pmd->name, *fd, dir, qid);
        gso_ctx = NULL;
    } else if (*other_fd != -1) {
        ...
        *fd = dup(*other_fd);
        ...
        TAP_LOG(DEBUG, "%s: dup fd %d for %s queue qid %d (%d)", pmd->name, *other_fd, dir, qid, *fd);
    } else {
        ...
        *fd = tun_alloc(pmd, 0);
        ...
        TAP_LOG(DEBUG, "%s: add %s queue for qid %d fd %d", pmd->name, dir, qid, *fd);
    }
...
static int tap_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id, uint16_t nb_rx_desc,
                              unsigned int socket_id, const struct rte_eth_rxconf *rx_conf __rte_unused,
    ...
    if (iov_max <= 0) {
        TAP_LOG(WARNING, "_SC_IOV_MAX is not defined. Using %d as default", TAP_IOV_DEFAULT_MAX);
    ...
    if (rx_queue_id >= dev->data->nb_rx_queues || !mp) {
        TAP_LOG(WARNING, "nb_rx_queues %d too small or mempool NULL", dev->data->nb_rx_queues);
        return -1;
    }
    ...
    if (!iovecs) {
        TAP_LOG(WARNING, "%s: Couldn't allocate %d RX descriptors", dev->device->name, nb_desc);
        return -ENOMEM;
    }
    rxq->iovecs = iovecs;
    ...
    for (i = 1; i <= nb_desc; i++) {
        *tmp = rte_pktmbuf_alloc(rxq->mp);
        if (!*tmp) {
            TAP_LOG(WARNING, "%s: couldn't allocate memory for queue %d", dev->device->name, rx_queue_id);
            ret = -ENOMEM;
            goto error;
        }
    ...
    TAP_LOG(DEBUG, "  RX TUNTAP device name %s, qid %d on fd %d", internals->name, rx_queue_id,
...
static int tap_tx_queue_setup(struct rte_eth_dev *dev, uint16_t tx_queue_id, uint16_t nb_tx_desc __rte_unused,
    ...
    txq->csum = !!(offloads & (DEV_TX_OFFLOAD_IPV4_CKSUM | DEV_TX_OFFLOAD_UDP_CKSUM | DEV_TX_OFFLOAD_TCP_CKSUM));

    ret = tap_setup_queue(dev, internals, tx_queue_id, 0);
    if (ret == -1)
        return -1;
    TAP_LOG(DEBUG, "  TX TUNTAP device name %s, qid %d on fd %d csum %s", internals->name, tx_queue_id,
            process_private->txq_fds[tx_queue_id], txq->csum ? "on" : "off");
...
static int tap_set_mc_addr_list(struct rte_eth_dev *dev __rte_unused, struct ether_addr *mc_addr_set __rte_unused,
...
static int tap_nl_msg_handler(struct nlmsghdr *nh, void *arg) {
    ...
    if (nh->nlmsg_type != RTM_NEWLINK || (info->ifi_index != pmd->if_index && info->ifi_index != pmd->remote_if_index))
        return 0;
...
static int tap_lsc_intr_handle_set(struct rte_eth_dev *dev, int set) {
    ...
    if (!dev->data->dev_conf.intr_conf.lsc) {
        if (pmd->intr_handle.fd != -1) {
            ...
            rte_intr_callback_unregister(&pmd->intr_handle, tap_dev_intr_handler, dev);
        }
        return 0;
    }
    if (set) {
        ...
        if (unlikely(pmd->intr_handle.fd == -1))
            return -EBADF;
        return rte_intr_callback_register(&pmd->intr_handle, tap_dev_intr_handler, dev);
    }
    tap_nl_final(pmd->intr_handle.fd);
    return rte_intr_callback_unregister(&pmd->intr_handle, tap_dev_intr_handler, dev);
}
...
static int tap_flow_ctrl_get(struct rte_eth_dev *dev __rte_unused, struct rte_eth_fc_conf *fc_conf) {
    fc_conf->mode = RTE_FC_NONE;
    return 0;
}

static int tap_flow_ctrl_set(struct rte_eth_dev *dev __rte_unused, struct rte_eth_fc_conf *fc_conf) {
 ...
 */
static int tap_rss_hash_update(struct rte_eth_dev *dev, struct rte_eth_rss_conf *rss_conf) {
...
static int eth_dev_tap_create(struct rte_vdev_device *vdev, const char *tap_name, char *remote_iface,
                              struct ether_addr *mac_addr, enum rte_tuntap_type type) {
    ...
    process_private = (struct pmd_process_private *)rte_zmalloc_socket(tap_name, sizeof(struct pmd_process_private),
    ...
    if (pmd->ioctl_sock == -1) {
        TAP_LOG(ERR, "%s Unable to get a socket for management: %s", tuntap_name, strerror(errno));
        goto error_exit;
    }
    ...
    if (strlen(remote_iface)) {
        ...
        if (tap_ioctl(pmd, SIOCGIFHWADDR, &ifr, 0, REMOTE_ONLY) < 0) {
            TAP_LOG(ERR, "%s: failed to get %s MAC address.", pmd->name, pmd->remote_iface);
            goto error_remote;
        }
        rte_memcpy(&pmd->eth_addr, ifr.ifr_hwaddr.sa_data, ETHER_ADDR_LEN);
        /* The desired MAC is already in ifreq after SIOCGIFHWADDR. */
        if (tap_ioctl(pmd, SIOCSIFHWADDR, &ifr, 0, LOCAL_ONLY) < 0) {
            TAP_LOG(ERR, "%s: failed to get %s MAC address.", pmd->name, remote_iface);
            goto error_remote;
        }
...
error_exit_nodev:
    TAP_LOG(ERR, "%s Unable to initialize %s", tuntap_name, rte_vdev_device_name(vdev));
...
static int set_interface_name(const char *key __rte_unused, const char *value, void *extra_args) {
...
static int set_remote_iface(const char *key __rte_unused, const char *value, void *extra_args) {
    char *name = (char *)extra_args;
...
static int parse_user_mac(struct ether_addr *user_mac, const char *value) {
...
static int set_mac_type(const char *key __rte_unused, const char *value, void *extra_args) {
...
error:
    TAP_LOG(ERR, "TAP user MAC (%s) is not in format (%s|%s)", value, ETH_TAP_MAC_FIXED, ETH_TAP_USR_MAC_FMT);
    return -1;
}
    ...
    if (params && (params[0] != '\0')) {
        ...
        if (kvlist) {
            if (rte_kvargs_count(kvlist, ETH_TAP_IFACE_ARG) == 1) {
                ret = rte_kvargs_process(kvlist, ETH_TAP_IFACE_ARG, &set_interface_name, tun_name);
...
static int tap_mp_attach_queues(const char *port_name, struct rte_eth_dev *dev) {
    int ret;
    ...
    strlcpy(request_param->port_name, port_name, sizeof(request_param->port_name));
...
static int tap_mp_sync_queues(const struct rte_mp_msg *request, const void *peer) {
    struct rte_eth_dev *dev;
    struct pmd_process_private *process_private;
    ...
    strlcpy(reply_param->port_name, request_param->port_name, sizeof(reply_param->port_name));
...
static int rte_pmd_tap_probe(struct rte_vdev_device *dev) {
    ...
    if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
        ...
        eth_dev->process_private = (struct pmd_process_private *)rte_zmalloc_socket(
            name, sizeof(struct pmd_process_private), RTE_CACHE_LINE_SIZE, eth_dev->device->numa_node);
    ...
    if (params && (params[0] != '\0')) {
        ...
        if (kvlist) {
            if (rte_kvargs_count(kvlist, ETH_TAP_IFACE_ARG) == 1) {
                ret = rte_kvargs_process(kvlist, ETH_TAP_IFACE_ARG, &set_interface_name, tap_name);
                if (ret == -1)
                    goto leave;
            }

            if (rte_kvargs_count(kvlist, ETH_TAP_REMOTE_ARG) == 1) {
                ret = rte_kvargs_process(kvlist, ETH_TAP_REMOTE_ARG, &set_remote_iface, remote_iface);
                if (ret == -1)
                    goto leave;
            }

            if (rte_kvargs_count(kvlist, ETH_TAP_MAC_ARG) == 1) {
                ret = rte_kvargs_process(kvlist, ETH_TAP_MAC_ARG, &set_mac_type, &user_mac);
                if (ret == -1)
                    goto leave;
            }
        }
    ...
    if (!tap_devices_count) {
        ...
        if (ret < 0) {
            TAP_LOG(ERR, "tap: Failed to register IPC callback: %s", strerror(rte_errno));
            goto leave;
        }
    }
    tap_devices_count++;
    tap_devices_count_increased = 1;
    ret = eth_dev_tap_create(dev, tap_name, remote_iface, &user_mac, ETH_TUNTAP_TYPE_TAP);
...
static int rte_pmd_tap_remove(struct rte_vdev_device *dev) {
    ...
    TAP_LOG(DEBUG, "Closing %s Ethernet device on numa %u", tuntap_types[internals->type], rte_socket_id());
...
RTE_PMD_REGISTER_PARAM_STRING(net_tap, ETH_TAP_IFACE_ARG "=<string> " ETH_TAP_MAC_ARG "=" ETH_TAP_MAC_ARG_FMT
