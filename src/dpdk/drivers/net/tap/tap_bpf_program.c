...
/** Create IPv4 address */
#define IPv4(a, b, c, d) ((__u32)(((a)&0xff) << 24) | (((b)&0xff) << 16) | (((c)&0xff) << 8) | ((d)&0xff))
...
rte_softrss_be(const __u32 *input_tuple, const uint8_t *rss_key, __u8 input_len) {
    __u32 i, j, hash = 0;
#pragma unroll
        ...
        for (i = 0; i < 32; i++) {
            if (input_tuple[j] & (1U << (31 - i))) {
                hash ^= ((const __u32 *)def_rss_key)[j] << i |
                        (__u32)((uint64_t)(((const __u32 *)def_rss_key)[j + 1]) >> (32 - i));
            }
        }
    }
    return hash;
...
static int __attribute__((always_inline)) rss_l3_l4(struct __sk_buff *skb) {
    ...
    if (skb->vlan_present && skb->vlan_proto == htons(ETH_P_8021AD)) {
        if (data + ETH_ALEN * 2 + sizeof(struct vlan_hdr) + sizeof(proto) > data_end)
            return TC_ACT_OK;
    ...
    if (proto == htons(ETH_P_IP)) {
        ...
        struct ipv4_l3_l4_tuple v4_tuple = {
            .src_addr = IPv4(*(src_dst_addr + 0), *(src_dst_addr + 1), *(src_dst_addr + 2), *(src_dst_addr + 3)),
            .dst_addr = IPv4(*(src_dst_addr + 4), *(src_dst_addr + 5), *(src_dst_addr + 6), *(src_dst_addr + 7)),
        ...
        for (j = 0; j < 4; j++)
            *((uint32_t *)&v6_tuple.src_addr + j) = __builtin_bswap32(*((uint32_t *)src_dst_addr + j));
        for (j = 0; j < 4; j++)
            *((uint32_t *)&v6_tuple.dst_addr + j) = __builtin_bswap32(*((uint32_t *)src_dst_addr + 4 + j));
