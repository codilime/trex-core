...
int tap_nl_final(int nlsk_fd) {
    if (close(nlsk_fd)) {
        TAP_LOG(ERR, "Failed to close netlink socket: %s (%d)", strerror(errno), errno);
        return -1;
    }
    return 0;
}
...
int tap_nl_send(int nlsk_fd, struct nlmsghdr *nh) {
    ...
    if (send_bytes < 0) {
        TAP_LOG(ERR, "Failed to send netlink message: %s (%d)", strerror(errno), errno);
        return -1;
    }
    return send_bytes;
}
 ...
 *   0 on success, -1 otherwise with errno set.
 */
int tap_nl_recv(int nlsk_fd, int (*cb)(struct nlmsghdr *, void *arg), void *arg) {
    /* man 7 netlink EXAMPLE */
    struct sockaddr_nl sa;
    char buf[BUF_SIZE];
    struct iovec iov = {
    ...
    do {
        ...
        if (recv_bytes < 0)
            return -1;
        for (nh = (struct nlmsghdr *)buf; NLMSG_OK(nh, (unsigned int)recv_bytes); nh = NLMSG_NEXT(nh, recv_bytes)) {
 ...
 *   The data to append.
 */
void tap_nlattr_add(struct nlmsghdr *nh, unsigned short type, unsigned int data_len, const void *data) {
    /* see man 3 rtnetlink */
    struct rtattr *rta;
