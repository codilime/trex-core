 ...
 */
void tc_init_msg(struct nlmsg *msg, uint16_t ifindex, uint16_t type, uint16_t flags) {
...
int qdisc_add_multiq(int nlsk_fd, uint16_t ifindex) {
    ...
    tc_init_msg(&msg, ifindex, RTM_NEWQDISC, NLM_F_REQUEST | NLM_F_ACK | NLM_F_EXCL | NLM_F_CREATE);
 ...
 *   0 on success, -1 otherwise with errno set.
 */
static int qdisc_iterate(int nlsk_fd, uint16_t ifindex, int (*callback)(struct nlmsghdr *, void *), void *arg) {
    struct nlmsg msg;
 ...
 *   0 on success, -1 otherwise with errno set.
 */
int qdisc_flush(int nlsk_fd, uint16_t ifindex) { return qdisc_iterate(nlsk_fd, ifindex, qdisc_del_cb, NULL); }
...
int qdisc_create_ingress(int nlsk_fd, uint16_t ifindex) {
    ...
    if (err < 0 && errno != -EEXIST) {
        TAP_LOG(ERR, "Could not add ingress qdisc (%d): %s", errno, strerror(errno));
        return -1;
    }
    return 0;
}
