...
#define DRV_LOG(level, ...)                                                                                            \
    rte_log(RTE_LOG_##level, vdev_netvsc_logtype,                                                                      \
            RTE_FMT(VDEV_NETVSC_DRIVER_NAME ": " RTE_FMT_HEAD(__VA_ARGS__, ) "\n", RTE_FMT_TAIL(__VA_ARGS__, )))
...
static LIST_HEAD(, vdev_netvsc_ctx) vdev_netvsc_ctx_list = LIST_HEAD_INITIALIZER(vdev_netvsc_ctx_list);
 ...
 */
static int vdev_netvsc_foreach_iface(int (*func)(const struct if_nameindex *iface, const struct ether_addr *eth_addr,
                                                 va_list ap),
                                     int is_netvsc, ...) {
 ...
 *   0 otherwise.
 */
static int vdev_netvsc_has_route(const struct if_nameindex *iface, const unsigned char family) {
    /*
    ...
    while (NLMSG_OK(retmsg, (unsigned int)len)) {
        ...
        if (retaddr->ifa_family == family && retaddr->ifa_index == iface->if_index) {
 ...
 */
static int vdev_netvsc_sysfs_readlink(char *buf, size_t size, const char *if_name, const char *relpath) {
    struct vdev_netvsc_ctx *ctx;
    char in[RTE_MAX(sizeof(ctx->yield), 256u)];
    int ret;

    ret = snprintf(in, sizeof(in) - 1, "/sys/class/net/%s/%s", if_name, relpath);
 ...
 *   error.
 */
static int vdev_netvsc_device_probe(const struct if_nameindex *iface, const struct ether_addr *eth_addr, va_list ap) {
    ...
    if (ctx->if_index == iface->if_index) {
        if (!strcmp(ctx->if_name, iface->if_name))
            return 0;
        DRV_LOG(DEBUG, "NetVSC interface \"%s\" (index %u) renamed \"%s\"", ctx->if_name, ctx->if_index,
                iface->if_name);
    ...
    ret = vdev_netvsc_sysfs_readlink(buf, sizeof(buf), iface->if_name, "device/subsystem");
    if (ret)
        return 0;
    addr = strrchr(buf, '/');
 ...
 *   error.
 */
static int vdev_netvsc_netvsc_probe(const struct if_nameindex *iface, const struct ether_addr *eth_addr, va_list ap) {
        ...
        for (i = 0; i != kvargs->count; ++i) {
            ...
            if (!strcmp(pair->key, VDEV_NETVSC_ARG_IFACE)) {
                ...
                if (sscanf(pair->value,
                           "%" SCNx8 ":%" SCNx8 ":%" SCNx8 ":"
                           "%" SCNx8 ":%" SCNx8 ":%" SCNx8,
                           &tmp.addr_bytes[0], &tmp.addr_bytes[1], &tmp.addr_bytes[2], &tmp.addr_bytes[3],
    ...
    if (vdev_netvsc_has_route(iface, AF_INET) || vdev_netvsc_has_route(iface, AF_INET6)) {
        if (!specified)
            return 0;
        DRV_LOG(WARNING,
    ...
    if (!ctx) {
        ret = -errno;
        DRV_LOG(ERR, "cannot allocate context for interface \"%s\": %s", iface->if_name, rte_strerror(errno));
        goto error;
    }
    ...
    if (pipe(ctx->pipe) == -1) {
        ret = -errno;
        DRV_LOG(ERR, "cannot allocate control pipe for interface \"%s\": %s", ctx->if_name, rte_strerror(errno));
        goto error;
    }
    ...
    if (ret == -1 || (size_t)ret >= sizeof(ctx->name))
        ++i;
    ret = snprintf(ctx->devname, sizeof(ctx->devname), "net_failsafe_vsc%u", ctx->id);
    if (ret == -1 || (size_t)ret >= sizeof(ctx->devname))
        ++i;
    ret = snprintf(ctx->devargs, sizeof(ctx->devargs), "fd(%d),dev(net_tap_vsc%u,remote=%s)", ctx->pipe[0], ctx->id,
                   ctx->if_name);
    ...
    DRV_LOG(DEBUG, "generating virtual device \"%s\" with arguments \"%s\"", ctx->devname, ctx->devargs);
...
static int vdev_netvsc_vdev_probe(struct rte_vdev_device *dev) {
    static const char *const vdev_netvsc_arg[] = {
        VDEV_NETVSC_ARG_IFACE, VDEV_NETVSC_ARG_MAC, VDEV_NETVSC_ARG_FORCE, VDEV_NETVSC_ARG_IGNORE, NULL,
    };
    const char *name = rte_vdev_device_name(dev);
    const char *args = rte_vdev_device_args(dev);
    struct rte_kvargs *kvargs = rte_kvargs_parse(args ? args : "", vdev_netvsc_arg);
    ...
    for (i = 0; i != kvargs->count; ++i) {
        ...
        else if (!strcmp(pair->key, VDEV_NETVSC_ARG_IFACE) || !strcmp(pair->key, VDEV_NETVSC_ARG_MAC))
            ++specified;
    }
    if (ignore) {
        if (kvargs)
    ...
    /* Gather interfaces. */
    ret = vdev_netvsc_foreach_iface(vdev_netvsc_netvsc_probe, 1, name, kvargs, specified, &matched);
    if (ret < 0)
        goto error;
    if (specified && matched < specified) {
        ...
        if (vdev_netvsc_foreach_iface(vdev_netvsc_netvsc_probe, 0, name, kvargs, specified, &matched) < 0)
            goto error;
...
RTE_PMD_REGISTER_PARAM_STRING(net_vdev_netvsc,
                              VDEV_NETVSC_ARG_IFACE "=<string> " VDEV_NETVSC_ARG_MAC "=<string> " VDEV_NETVSC_ARG_FORCE
...
static int vdev_netvsc_cmp_rte_device(const struct rte_device *dev1, __rte_unused const void *_dev2) {
    return strncmp(dev1->devargs->name, VDEV_NETVSC_DRIVER_NAME, VDEV_NETVSC_DRIVER_NAME_LEN);
}
...
static void vdev_netvsc_scan_callback(__rte_unused void *arg) {
    ...
    if (!strncmp(devargs->name, VDEV_NETVSC_DRIVER_NAME, VDEV_NETVSC_DRIVER_NAME_LEN))
        return;
