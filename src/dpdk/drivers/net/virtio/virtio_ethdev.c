...
static int virtio_dev_link_update(struct rte_eth_dev *dev, int wait_to_complete);
...
static int virtio_dev_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats, unsigned n);
...
static int virtio_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id, int on);
static int virtio_mac_addr_add(struct rte_eth_dev *dev, struct ether_addr *mac_addr, uint32_t index, uint32_t vmdq);
static void virtio_mac_addr_remove(struct rte_eth_dev *dev, uint32_t index);
static int virtio_mac_addr_set(struct rte_eth_dev *dev, struct ether_addr *mac_addr);

static int virtio_intr_disable(struct rte_eth_dev *dev);

static int virtio_dev_queue_stats_mapping_set(struct rte_eth_dev *eth_dev, uint16_t queue_id, uint8_t stat_idx,
                                              uint8_t is_rx);
...
#define VIRTIO_NB_RXQ_XSTATS (sizeof(rte_virtio_rxq_stat_strings) / sizeof(rte_virtio_rxq_stat_strings[0]))
#define VIRTIO_NB_TXQ_XSTATS (sizeof(rte_virtio_txq_stat_strings) / sizeof(rte_virtio_txq_stat_strings[0]))

struct virtio_hw_internal virtio_hw_internal[RTE_MAX_ETHPORTS];

static struct virtio_pmd_ctrl *virtio_send_command_packed(struct virtnet_ctl *cvq, struct virtio_pmd_ctrl *ctrl,
    ...
    for (k = 0; k < pkt_num; k++) {
        desc[vq->vq_avail_idx].addr =
            cvq->virtio_net_hdr_mem + sizeof(struct virtio_net_ctrl_hdr) + sizeof(ctrl->status) + sizeof(uint8_t) * sum;
        desc[vq->vq_avail_idx].len = dlen[k];
        desc[vq->vq_avail_idx].flags = VRING_DESC_F_NEXT | vq->vq_packed.cached_flags;
        sum += dlen[k];
        vq->vq_free_cnt--;
        nb_descs++;
        if (++vq->vq_avail_idx >= vq->vq_nentries) {
    ...
    desc[vq->vq_avail_idx].addr = cvq->virtio_net_hdr_mem + sizeof(struct virtio_net_ctrl_hdr);
    desc[vq->vq_avail_idx].len = sizeof(ctrl->status);
    desc[vq->vq_avail_idx].flags = VRING_DESC_F_WRITE | vq->vq_packed.cached_flags;
    vq->vq_free_cnt--;
    nb_descs++;
    if (++vq->vq_avail_idx >= vq->vq_nentries) {
        vq->vq_avail_idx -= vq->vq_nentries;
    ...
    PMD_INIT_LOG(DEBUG,
                 ...
                 vq->vq_free_cnt, vq->vq_avail_idx, vq->vq_used_cons_idx, vq->vq_packed.cached_flags,
...
static struct virtio_pmd_ctrl *virtio_send_command_split(struct virtnet_ctl *cvq, struct virtio_pmd_ctrl *ctrl,
                                                         int *dlen, int pkt_num) {
    struct virtio_pmd_ctrl *result;
    struct virtqueue *vq = cvq->vq;
    uint32_t head, i;
    ...
    vq->vq_split.ring.desc[i].addr = cvq->virtio_net_hdr_mem + sizeof(struct virtio_net_ctrl_hdr);
    ...
    PMD_INIT_LOG(DEBUG, "vq->vq_free_cnt=%d\nvq->vq_desc_head_idx=%d", vq->vq_free_cnt, vq->vq_desc_head_idx);

    result = cvq->virtio_net_hdr_mz->addr;
    return result;
}

static int virtio_send_command(struct virtnet_ctl *cvq, struct virtio_pmd_ctrl *ctrl, int *dlen, int pkt_num) {
...
static int virtio_set_multiple_queues(struct rte_eth_dev *dev, uint16_t nb_queues) {
...
static void virtio_dev_queue_release(void *queue __rte_unused) { /* do nothing */ }
...
static void virtio_init_vring(struct virtqueue *vq) {
    ...
    if (vtpci_packed_queue(vq->hw)) {
        vring_init_packed(&vq->vq_packed.ring, ring_mem, VIRTIO_PCI_VRING_ALIGN, size);
...
static int virtio_init_queue(struct rte_eth_dev *dev, uint16_t vtpci_queue_idx) {
    ...
    PMD_INIT_LOG(INFO, "setting up queue: %u on NUMA node %d", vtpci_queue_idx, numa_node);
    ...
    mz = rte_memzone_reserve_aligned(vq_name, vq->vq_ring_size, numa_node, RTE_MEMZONE_IOVA_CONTIG,
    ...
    if (sz_hdr_mz) {
        snprintf(vq_hdr_name, sizeof(vq_hdr_name), "port%d_vq%d_hdr", dev->data->port_id, vtpci_queue_idx);
        hdr_mz = rte_memzone_reserve_aligned(vq_hdr_name, sz_hdr_mz, numa_node, RTE_MEMZONE_IOVA_CONTIG,
                                             RTE_CACHE_LINE_SIZE);
        if (hdr_mz == NULL) {
            if (rte_errno == EEXIST)
    ...
    if (queue_type == VTNET_RQ) {
        size_t sz_sw = (RTE_PMD_VIRTIO_RX_MAX_BURST + vq_size) * sizeof(vq->sw_ring[0]);
    ...
    if (queue_type == VTNET_TQ) {
        ...
        for (i = 0; i < vq_size; i++) {
            ...
            if (!vtpci_packed_queue(hw)) {
                ...
                start_dp->addr =
                    txvq->virtio_net_hdr_mem + i * sizeof(*txr) + offsetof(struct virtio_tx_region, tx_hdr);
...
static int virtio_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
    struct virtio_hw *hw = dev->data->dev_private;
    uint32_t ether_hdr_len = ETHER_HDR_LEN + VLAN_TAG_LEN + hw->vtnet_hdr_size;
    ...
    if (mtu < ETHER_MIN_MTU || frame_size > max_frame_size) {
        PMD_INIT_LOG(ERR, "MTU should be between %d and %d", ETHER_MIN_MTU, max_frame_size - ether_hdr_len);
        return -EINVAL;
    }
    return 0;
}

static int virtio_dev_rx_queue_intr_enable(struct rte_eth_dev *dev, uint16_t queue_id) {
    struct virtio_hw *hw = dev->data->dev_private;
...
static int virtio_dev_rx_queue_intr_disable(struct rte_eth_dev *dev, uint16_t queue_id) {
    struct virtnet_rx *rxvq = dev->data->rx_queues[queue_id];
    struct virtqueue *vq = rxvq->vq;
...
static void virtio_update_stats(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
    unsigned i;
...
static int virtio_dev_xstats_get_names(struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats_names,
    ...
    unsigned nstats = dev->data->nb_tx_queues * VIRTIO_NB_TXQ_XSTATS + dev->data->nb_rx_queues * VIRTIO_NB_RXQ_XSTATS;
    ...
    if (xstats_names != NULL) {
        ...
        for (i = 0; i < dev->data->nb_rx_queues; i++) {
            ...
            for (t = 0; t < VIRTIO_NB_RXQ_XSTATS; t++) {
                snprintf(xstats_names[count].name, sizeof(xstats_names[count].name), "rx_q%u_%s", i,
        ...
        for (i = 0; i < dev->data->nb_tx_queues; i++) {
            ...
            for (t = 0; t < VIRTIO_NB_TXQ_XSTATS; t++) {
                snprintf(xstats_names[count].name, sizeof(xstats_names[count].name), "tx_q%u_%s", i,
...
static int virtio_dev_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats, unsigned n) {
    ...
    for (i = 0; i < dev->data->nb_rx_queues; i++) {
        ...
        for (t = 0; t < VIRTIO_NB_RXQ_XSTATS; t++) {
            xstats[count].value = *(uint64_t *)(((char *)rxvq) + rte_virtio_rxq_stat_strings[t].offset);
    ...
    for (i = 0; i < dev->data->nb_tx_queues; i++) {
        ...
        for (t = 0; t < VIRTIO_NB_TXQ_XSTATS; t++) {
            xstats[count].value = *(uint64_t *)(((char *)txvq) + rte_virtio_txq_stat_strings[t].offset);
            xstats[count].id = count;
            count++;
        }
    }
...
static void virtio_set_hwaddr(struct virtio_hw *hw) {
    vtpci_write_dev_config(hw, offsetof(struct virtio_net_config, mac), &hw->mac_addr, ETHER_ADDR_LEN);
}

static void virtio_get_hwaddr(struct virtio_hw *hw) {
    if (vtpci_with_feature(hw, VIRTIO_NET_F_MAC)) {
        vtpci_read_dev_config(hw, offsetof(struct virtio_net_config, mac), &hw->mac_addr, ETHER_ADDR_LEN);
    } else {
...
static int virtio_mac_table_set(struct virtio_hw *hw, const struct virtio_net_ctrl_mac *uc,
...
static int virtio_mac_addr_add(struct rte_eth_dev *dev, struct ether_addr *mac_addr, uint32_t index,
    ...
    for (i = 0; i < VIRTIO_MAX_MAC_ADDRS; i++) {
        const struct ether_addr *addr = (i == index) ? mac_addr : addrs + i;
        struct virtio_net_ctrl_mac *tbl = is_multicast_ether_addr(addr) ? mc : uc;
...
static int virtio_negotiate_features(struct virtio_hw *hw, uint64_t req_features) {
    uint64_t host_features;

    /* Prepare guest_features: feature that driver wants to support */
    PMD_INIT_LOG(DEBUG, "guest_features before negotiate = %" PRIx64, req_features);

    /* Read device(host) feature bits */
    host_features = VTPCI_OPS(hw)->get_features(hw);
    PMD_INIT_LOG(DEBUG, "host_features before negotiate = %" PRIx64, host_features);

    /* If supported, ensure MTU value is valid before acknowledging it. */
    if (host_features & req_features & (1ULL << VIRTIO_NET_F_MTU)) {
        struct virtio_net_config config;

        vtpci_read_dev_config(hw, offsetof(struct virtio_net_config, mtu), &config.mtu, sizeof(config.mtu));
    ...
    PMD_INIT_LOG(DEBUG, "features after negotiate = %" PRIx64, hw->guest_features);

    if (hw->modern) {
        if (!vtpci_with_feature(hw, VIRTIO_F_VERSION_1)) {
            PMD_INIT_LOG(ERR, "VIRTIO_F_VERSION_1 features is not enabled.");
            return -1;
        }
 ...
 */
int virtio_inject_pkts(struct rte_eth_dev *dev, struct rte_mbuf **tx_pkts, int nb_pkts) {
    struct virtio_hw *hw = dev->data->dev_private;
...
static void virtio_notify_peers(struct rte_eth_dev *dev) {
    ...
    rarp_mbuf = rte_net_make_rarp_packet(rxvq->mpool, (struct ether_addr *)hw->mac_addr);
    if (rarp_mbuf == NULL) {
...
void virtio_interrupt_handler(void *param) {
    ...
    if (isr & VIRTIO_PCI_ISR_CONFIG) {
        ...
        if (vtpci_with_feature(hw, VIRTIO_NET_F_STATUS)) {
            vtpci_read_dev_config(hw, offsetof(struct virtio_net_config, status), &status, sizeof(status));
...
static void set_rxtx_funcs(struct rte_eth_dev *eth_dev) {
    ...
    if (vtpci_packed_queue(hw)) {
        PMD_INIT_LOG(INFO, "virtio: using packed ring %s Tx path on port %u",
                     hw->use_inorder_tx ? "inorder" : "standard", eth_dev->data->port_id);
        eth_dev->tx_pkt_burst = virtio_xmit_pkts_packed;
    } else {
        if (hw->use_inorder_tx) {
            PMD_INIT_LOG(INFO, "virtio: using inorder Tx path on port %u", eth_dev->data->port_id);
            eth_dev->tx_pkt_burst = virtio_xmit_pkts_inorder;
        } else {
            PMD_INIT_LOG(INFO, "virtio: using standard Tx path on port %u", eth_dev->data->port_id);
    ...
    if (vtpci_packed_queue(hw)) {
        if (vtpci_with_feature(hw, VIRTIO_NET_F_MRG_RXBUF)) {
            PMD_INIT_LOG(INFO, "virtio: using packed ring mergeable buffer Rx path on port %u", eth_dev->data->port_id);
            eth_dev->rx_pkt_burst = &virtio_recv_mergeable_pkts_packed;
        } else {
            PMD_INIT_LOG(INFO, "virtio: using packed ring standard Rx path on port %u", eth_dev->data->port_id);
            eth_dev->rx_pkt_burst = &virtio_recv_pkts_packed;
        }
    } else {
        if (hw->use_simple_rx) {
            PMD_INIT_LOG(INFO, "virtio: using simple Rx path on port %u", eth_dev->data->port_id);
            eth_dev->rx_pkt_burst = virtio_recv_pkts_vec;
        } else if (hw->use_inorder_rx) {
            PMD_INIT_LOG(INFO, "virtio: using inorder Rx path on port %u", eth_dev->data->port_id);
            eth_dev->rx_pkt_burst = &virtio_recv_pkts_inorder;
        } else if (vtpci_with_feature(hw, VIRTIO_NET_F_MRG_RXBUF)) {
            PMD_INIT_LOG(INFO, "virtio: using mergeable buffer Rx path on port %u", eth_dev->data->port_id);
            eth_dev->rx_pkt_burst = &virtio_recv_mergeable_pkts;
        } else {
            PMD_INIT_LOG(INFO, "virtio: using standard Rx path on port %u", eth_dev->data->port_id);
...
static int virtio_queues_bind_intr(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i < dev->data->nb_rx_queues; ++i) {
        ...
        if (VTPCI_OPS(hw)->set_queue_irq(hw, hw->vqs[i * 2], i + 1) == VIRTIO_MSI_NO_VECTOR) {
...
static void virtio_queues_unbind_intr(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i < dev->data->nb_rx_queues; ++i)
        VTPCI_OPS(hw)->set_queue_irq(hw, hw->vqs[i * VTNET_CQ], VIRTIO_MSI_NO_VECTOR);
}
...
static int virtio_configure_intr(struct rte_eth_dev *dev) {
    ...
    if (!dev->intr_handle->intr_vec) {
        dev->intr_handle->intr_vec = rte_zmalloc("intr_vec", hw->max_queue_pairs * sizeof(int), 0);
        if (!dev->intr_handle->intr_vec) {
    ...
    rte_intr_callback_unregister(dev->intr_handle, virtio_interrupt_handler, dev);
...
static int virtio_init_device(struct rte_eth_dev *eth_dev, uint64_t req_features) {
    struct virtio_hw *hw = eth_dev->data->dev_private;
    struct virtio_net_config *config;
    ...
    if (vtpci_with_feature(hw, VIRTIO_NET_F_STATUS) && hw->use_msix != VIRTIO_MSIX_NONE)
        eth_dev->data->dev_flags |= RTE_ETH_DEV_INTR_LSC;
    else
        eth_dev->data->dev_flags &= ~RTE_ETH_DEV_INTR_LSC;

    /* Setting up rx_header size for the device */
    if (vtpci_with_feature(hw, VIRTIO_NET_F_MRG_RXBUF) || vtpci_with_feature(hw, VIRTIO_F_VERSION_1) ||
    ...
    ether_addr_copy((struct ether_addr *)hw->mac_addr, &eth_dev->data->mac_addrs[0]);
    PMD_INIT_LOG(DEBUG, "PORT MAC: %02X:%02X:%02X:%02X:%02X:%02X", hw->mac_addr[0], hw->mac_addr[1], hw->mac_addr[2],
                 hw->mac_addr[3], hw->mac_addr[4], hw->mac_addr[5]);

    if (vtpci_with_feature(hw, VIRTIO_NET_F_CTRL_VQ)) {
        config = &local_config;

        vtpci_read_dev_config(hw, offsetof(struct virtio_net_config, mac), &config->mac, sizeof(config->mac));

        if (vtpci_with_feature(hw, VIRTIO_NET_F_STATUS)) {
            vtpci_read_dev_config(hw, offsetof(struct virtio_net_config, status), &config->status,
        ...
        if (vtpci_with_feature(hw, VIRTIO_NET_F_MTU)) {
            ...
            hw->max_mtu = VIRTIO_MAX_RX_PKTLEN - ETHER_HDR_LEN - VLAN_TAG_LEN - hw->vtnet_hdr_size;
        }

        PMD_INIT_LOG(DEBUG, "config->max_virtqueue_pairs=%d", config->max_virtqueue_pairs);
        PMD_INIT_LOG(DEBUG, "config->status=%d", config->status);
        PMD_INIT_LOG(DEBUG, "PORT MAC: %02X:%02X:%02X:%02X:%02X:%02X", config->mac[0], config->mac[1], config->mac[2],
 ...
 */
static int virtio_remap_pci(struct rte_pci_device *pci_dev, struct virtio_hw *hw) {
    if (hw->modern) {
        /*
...
static void virtio_set_vtpci_ops(struct virtio_hw *hw) {
...
static int vdpa_check_handler(__rte_unused const char *key, const char *value, __rte_unused void *opaque) {
    if (strcmp(value, "1"))
        return -1;
...
static int vdpa_mode_selected(struct rte_devargs *devargs) {
    ...
    if (rte_kvargs_process(kvlist, key, vdpa_check_handler, NULL) < 0) {
        goto exit;
    }
    ret = 1;
...
static int eth_virtio_pci_probe(struct rte_pci_driver *pci_drv __rte_unused, struct rte_pci_device *pci_dev) {
...
static bool rx_offload_enabled(struct virtio_hw *hw) {
    return vtpci_with_feature(hw, VIRTIO_NET_F_GUEST_CSUM) || vtpci_with_feature(hw, VIRTIO_NET_F_GUEST_TSO4) ||
           vtpci_with_feature(hw, VIRTIO_NET_F_GUEST_TSO6);
}

static bool tx_offload_enabled(struct virtio_hw *hw) {
    return vtpci_with_feature(hw, VIRTIO_NET_F_CSUM) || vtpci_with_feature(hw, VIRTIO_NET_F_HOST_TSO4) ||
...
static int virtio_dev_configure(struct rte_eth_dev *dev) {
    ...
    if (rx_offloads & DEV_RX_OFFLOAD_TCP_LRO)
        req_features |= (1ULL << VIRTIO_NET_F_GUEST_TSO4) | (1ULL << VIRTIO_NET_F_GUEST_TSO6);

    if (tx_offloads & (DEV_TX_OFFLOAD_UDP_CKSUM | DEV_TX_OFFLOAD_TCP_CKSUM))
        req_features |= (1ULL << VIRTIO_NET_F_CSUM);

    if (tx_offloads & DEV_TX_OFFLOAD_TCP_TSO)
        req_features |= (1ULL << VIRTIO_NET_F_HOST_TSO4) | (1ULL << VIRTIO_NET_F_HOST_TSO6);
    ...
    if ((rx_offloads & DEV_RX_OFFLOAD_TCP_LRO) &&
        (!vtpci_with_feature(hw, VIRTIO_NET_F_GUEST_TSO4) || !vtpci_with_feature(hw, VIRTIO_NET_F_GUEST_TSO6))) {
    ...
    if ((rx_offloads & DEV_RX_OFFLOAD_VLAN_FILTER) && !vtpci_with_feature(hw, VIRTIO_NET_F_CTRL_VLAN)) {
    ...
    if (rx_offloads &
        (DEV_RX_OFFLOAD_UDP_CKSUM | DEV_RX_OFFLOAD_TCP_CKSUM | DEV_RX_OFFLOAD_TCP_LRO | DEV_RX_OFFLOAD_VLAN_STRIP))
        hw->use_simple_rx = 0;
...
static int virtio_dev_link_update(struct rte_eth_dev *dev, __rte_unused int wait_to_complete) {
    struct rte_eth_link link;
    uint16_t status;
    struct virtio_hw *hw = dev->data->dev_private;
...
static int virtio_dev_vlan_offload_set(struct rte_eth_dev *dev, int mask) {
    ...
    if (mask & ETH_VLAN_FILTER_MASK) {
        if ((offloads & DEV_RX_OFFLOAD_VLAN_FILTER) && !vtpci_with_feature(hw, VIRTIO_NET_F_CTRL_VLAN)) {
...
static void virtio_dev_info_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info) {
    ...
    if (host_features & (1ULL << VIRTIO_NET_F_GUEST_CSUM)) {
        dev_info->rx_offload_capa |= DEV_RX_OFFLOAD_TCP_CKSUM | DEV_RX_OFFLOAD_UDP_CKSUM;
    }
    if (host_features & (1ULL << VIRTIO_NET_F_CTRL_VLAN))
        dev_info->rx_offload_capa |= DEV_RX_OFFLOAD_VLAN_FILTER;
    tso_mask = (1ULL << VIRTIO_NET_F_GUEST_TSO4) | (1ULL << VIRTIO_NET_F_GUEST_TSO6);
    if ((host_features & tso_mask) == tso_mask)
        dev_info->rx_offload_capa |= DEV_RX_OFFLOAD_TCP_LRO;

    dev_info->tx_offload_capa = DEV_TX_OFFLOAD_MULTI_SEGS | DEV_TX_OFFLOAD_VLAN_INSERT;
    if (host_features & (1ULL << VIRTIO_NET_F_CSUM)) {
        dev_info->tx_offload_capa |= DEV_TX_OFFLOAD_UDP_CKSUM | DEV_TX_OFFLOAD_TCP_CKSUM;
    }
    tso_mask = (1ULL << VIRTIO_NET_F_HOST_TSO4) | (1ULL << VIRTIO_NET_F_HOST_TSO6);
    if ((host_features & tso_mask) == tso_mask)
 ...
 */
static int virtio_dev_queue_stats_mapping_set(__rte_unused struct rte_eth_dev *eth_dev, __rte_unused uint16_t queue_id,
