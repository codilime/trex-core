...
void vq_ring_free_inorder(struct virtqueue *vq, uint16_t desc_idx, uint16_t num) {
    vq->vq_free_cnt += num;
...
static uint16_t virtqueue_dequeue_burst_rx_packed(struct virtqueue *vq, struct rte_mbuf **rx_pkts, uint32_t *len,
                                                  uint16_t num) {
    ...
    for (i = 0; i < num; i++) {
        ...
        if (unlikely(cookie == NULL)) {
            PMD_DRV_LOG(ERR, "vring descriptor with no mbuf cookie at %u", vq->vq_used_cons_idx);
            break;
        }
...
static uint16_t virtqueue_dequeue_burst_rx(struct virtqueue *vq, struct rte_mbuf **rx_pkts, uint32_t *len,
                                           uint16_t num) {
...
static uint16_t virtqueue_dequeue_rx_inorder(struct virtqueue *vq, struct rte_mbuf **rx_pkts, uint32_t *len,
                                             uint16_t num) {
    struct vring_used_elem *uep;
    struct rte_mbuf *cookie;
    uint16_t used_idx = 0;
...
static void virtio_xmit_cleanup_inorder_packed(struct virtqueue *vq, int num) {
...
static void virtio_xmit_cleanup_normal_packed(struct virtqueue *vq, int num) {
...
static inline void virtio_xmit_cleanup_packed(struct virtqueue *vq, int num, int in_order) {
    if (in_order)
...
static void virtio_xmit_cleanup(struct virtqueue *vq, uint16_t num) {
...
static void virtio_xmit_cleanup_inorder(struct virtqueue *vq, uint16_t num) {
...
static inline int virtqueue_enqueue_refill_inorder(struct virtqueue *vq, struct rte_mbuf **cookies, uint16_t num) {
    struct vq_desc_extra *dxp;
...
static inline int virtqueue_enqueue_recv_refill(struct virtqueue *vq, struct rte_mbuf **cookie, uint16_t num) {
    struct vq_desc_extra *dxp;
    struct virtio_hw *hw = vq->hw;
...
static inline int virtqueue_enqueue_recv_refill_packed(struct virtqueue *vq, struct rte_mbuf **cookie, uint16_t num) {
...
#define virtqueue_clear_net_hdr(_hdr)                                                                                  \
    do {                                                                                                               \
...
static inline void virtqueue_xmit_offload(struct virtio_net_hdr *hdr, struct rte_mbuf *cookie, bool offload) {
    if (offload) {
        ...
        case PKT_TX_UDP_CKSUM:
            ...
            hdr->csum_offset = offsetof(struct udp_hdr, dgram_cksum);
        ...
        if (cookie->ol_flags & PKT_TX_TCP_SEG) {
            ...
            hdr->gso_type = (cookie->ol_flags & PKT_TX_IPV6) ? VIRTIO_NET_HDR_GSO_TCPV6 : VIRTIO_NET_HDR_GSO_TCPV4;
...
static inline void virtqueue_enqueue_xmit_inorder(struct virtnet_tx *txvq, struct rte_mbuf **cookies, uint16_t num) {
    struct vq_desc_extra *dxp;
    ...
    while (i < num) {
        ...
        hdr = (struct virtio_net_hdr *)rte_pktmbuf_prepend(cookies[i], head_size);
...
static inline void virtqueue_enqueue_xmit_packed_fast(struct virtnet_tx *txvq, struct rte_mbuf *cookie, int in_order) {
    struct virtqueue *vq = txvq->vq;
    ...
    hdr = (struct virtio_net_hdr *)rte_pktmbuf_prepend(cookie, head_size);
...
static inline void virtqueue_enqueue_xmit_packed(struct virtnet_tx *txvq, struct rte_mbuf *cookie, uint16_t needed,
...
static inline void virtqueue_enqueue_xmit(struct virtnet_tx *txvq, struct rte_mbuf *cookie, uint16_t needed,
                                          int use_indirect, int can_push, int in_order) {
    struct virtio_tx_region *txr = txvq->virtio_net_hdr_mz->addr;
    struct vq_desc_extra *dxp;
    struct virtqueue *vq = txvq->vq;
    struct vring_desc *start_dp;
    ...
    if (can_push) {
         ...
         */
        start_dp[idx].addr = txvq->virtio_net_hdr_mem + RTE_PTR_DIFF(&txr[idx].tx_indir, txr);
...
int virtio_dev_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx, uint16_t nb_desc,
    ...
    rxvq->mpool = mp;
    if (rxvq->mpool == NULL) {
        rte_exit(EXIT_FAILURE, "Cannot allocate mbufs for rx virtqueue");
    }
...
int virtio_dev_rx_queue_setup_finish(struct rte_eth_dev *dev, uint16_t queue_idx) {
    uint16_t vtpci_queue_idx = 2 * queue_idx + VTNET_SQ_RQ_QUEUE_IDX;
    struct virtio_hw *hw = dev->data->dev_private;
    struct virtqueue *vq = hw->vqs[vtpci_queue_idx];
 ...
 */
int virtio_dev_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx, uint16_t nb_desc,
...
int virtio_dev_tx_queue_setup_finish(struct rte_eth_dev *dev, uint16_t queue_idx) {
    uint8_t vtpci_queue_idx = 2 * queue_idx + VTNET_SQ_TQ_QUEUE_IDX;
    struct virtio_hw *hw = dev->data->dev_private;
    struct virtqueue *vq = hw->vqs[vtpci_queue_idx];
...
static inline void virtio_discard_rxbuf(struct virtqueue *vq, struct rte_mbuf *m) {
    int error;
    /*
...
static inline void virtio_discard_rxbuf_inorder(struct virtqueue *vq, struct rte_mbuf *m) {
    int error;
...
static inline void virtio_update_packet_stats(struct virtnet_stats *stats, struct rte_mbuf *mbuf) {
...
static inline void virtio_rx_stats_updated(struct virtnet_rx *rxvq, struct rte_mbuf *m) {
...
static inline int virtio_rx_offload(struct rte_mbuf *m, struct virtio_net_hdr *hdr) {
    ...
    m->packet_type = ptype;
    if ((ptype & RTE_PTYPE_L4_MASK) == RTE_PTYPE_L4_TCP || (ptype & RTE_PTYPE_L4_MASK) == RTE_PTYPE_L4_UDP ||
    ...
    if (hdr->flags & VIRTIO_NET_HDR_F_NEEDS_CSUM) {
        ...
        if (hdr->csum_start <= hdrlen && l4_supported) {
            ...
            rte_raw_cksum_mbuf(m, hdr->csum_start, rte_pktmbuf_pkt_len(m) - hdr->csum_start, &csum);
...
uint16_t virtio_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    ...
    for (i = 0; i < num; i++) {
        ...
        hdr = (struct virtio_net_hdr *)((char *)rxm->buf_addr + RTE_PKTMBUF_HEADROOM - hdr_size);
...
uint16_t virtio_recv_pkts_inorder(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    struct virtnet_rx *rxvq = rx_queue;
    struct virtqueue *vq = rxvq->vq;
    struct virtio_hw *hw = vq->hw;
    struct rte_mbuf *rxm;
    ...
    for (i = 0; i < num; i++) {
        ...
        header = (struct virtio_net_hdr_mrg_rxbuf *)((char *)rxm->buf_addr + RTE_PKTMBUF_HEADROOM - hdr_size);
...
uint16_t virtio_recv_mergeable_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    struct virtnet_rx *rxvq = rx_queue;
    struct virtqueue *vq = rxvq->vq;
    struct virtio_hw *hw = vq->hw;
    struct rte_mbuf *rxm;
...
uint16_t virtio_recv_mergeable_pkts_packed(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    struct virtnet_rx *rxvq = rx_queue;
    struct virtqueue *vq = rxvq->vq;
    struct virtio_hw *hw = vq->hw;
    struct rte_mbuf *rxm;
    ...
    while (seg_res != 0) {
        ...
        if (likely(vq->vq_free_cnt >= rcv_cnt)) {
            num = virtqueue_dequeue_burst_rx_packed(vq, rcv_pkts, len, rcv_cnt);
            uint16_t extra_idx = 0;
...
uint16_t virtio_xmit_pkts_packed(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    ...
    for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
        ...
        /* optimize ring usage */
        if ((vtpci_with_feature(hw, VIRTIO_F_ANY_LAYOUT) || vtpci_with_feature(hw, VIRTIO_F_VERSION_1)) &&
            rte_mbuf_refcnt_read(txm) == 1 && RTE_MBUF_DIRECT(txm) && txm->nb_segs == 1 &&
            rte_pktmbuf_headroom(txm) >= hdr_size &&
            rte_is_aligned(rte_pktmbuf_mtod(txm, char *), __alignof__(struct virtio_net_hdr_mrg_rxbuf)))
            can_push = 1;
        ...
        if (can_push)
            ...
            virtqueue_enqueue_xmit_packed(txvq, txm, slots, 0, in_order);
...
uint16_t virtio_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    struct virtnet_tx *txvq = tx_queue;
    struct virtqueue *vq = txvq->vq;
    struct virtio_hw *hw = vq->hw;
    uint16_t hdr_size = hw->vtnet_hdr_size;
    ...
    for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
        ...
        if ((vtpci_with_feature(hw, VIRTIO_F_ANY_LAYOUT) || vtpci_with_feature(hw, VIRTIO_F_VERSION_1)) &&
            ...
            rte_is_aligned(rte_pktmbuf_mtod(txm, char *), __alignof__(struct virtio_net_hdr_mrg_rxbuf)))
            can_push = 1;
        else if (vtpci_with_feature(hw, VIRTIO_RING_F_INDIRECT_DESC) && txm->nb_segs < VIRTIO_MAX_TX_INDIRECT)
            use_indirect = 1;
...
uint16_t virtio_xmit_pkts_inorder(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    struct virtnet_tx *txvq = tx_queue;
    struct virtqueue *vq = txvq->vq;
    struct virtio_hw *hw = vq->hw;
    uint16_t hdr_size = hw->vtnet_hdr_size;
