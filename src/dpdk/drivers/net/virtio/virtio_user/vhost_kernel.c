...
static uint64_t vhost_req_user_to_kernel[] = {
    [VHOST_USER_SET_OWNER] = VHOST_SET_OWNER,           [VHOST_USER_RESET_OWNER] = VHOST_RESET_OWNER,
    [VHOST_USER_SET_FEATURES] = VHOST_SET_FEATURES,     [VHOST_USER_GET_FEATURES] = VHOST_GET_FEATURES,
    [VHOST_USER_SET_VRING_CALL] = VHOST_SET_VRING_CALL, [VHOST_USER_SET_VRING_NUM] = VHOST_SET_VRING_NUM,
    [VHOST_USER_SET_VRING_BASE] = VHOST_SET_VRING_BASE, [VHOST_USER_GET_VRING_BASE] = VHOST_GET_VRING_BASE,
    [VHOST_USER_SET_VRING_ADDR] = VHOST_SET_VRING_ADDR, [VHOST_USER_SET_VRING_KICK] = VHOST_SET_VRING_KICK,
...
static int add_memseg_list(const struct rte_memseg_list *msl, void *arg) {
    ...
    PMD_DRV_LOG(DEBUG, "index=%u addr=%p len=%" PRIu64, vm->nregions - 1, start_addr, len);
 ...
 */
static struct vhost_memory_kernel *prepare_vhost_memory_kernel(void) {
    struct vhost_memory_kernel *vm;

    vm = malloc(sizeof(struct vhost_memory_kernel) + max_regions * sizeof(struct vhost_memory_region));
    if (!vm)
        return NULL;
...
#define VHOST_KERNEL_GUEST_OFFLOADS_MASK                                                                               \
    ((1ULL << VIRTIO_NET_F_GUEST_CSUM) | (1ULL << VIRTIO_NET_F_GUEST_TSO4) | (1ULL << VIRTIO_NET_F_GUEST_TSO6) |       \
     (1ULL << VIRTIO_NET_F_GUEST_ECN) | (1ULL << VIRTIO_NET_F_GUEST_UFO))
...
#define VHOST_KERNEL_HOST_OFFLOADS_MASK                                                                                \
    ((1ULL << VIRTIO_NET_F_HOST_TSO4) | (1ULL << VIRTIO_NET_F_HOST_TSO6) | (1ULL << VIRTIO_NET_F_CSUM))

static unsigned int tap_support_features(void) {
    int tapfd;
...
static int vhost_kernel_ioctl(struct virtio_user_dev *dev, enum vhost_user_request req, void *arg) {
    int ret = -1;
    unsigned int i;
    uint64_t req_kernel;
    ...
    case VHOST_SET_VRING_CALL:
        ...
        *(unsigned int *)arg = queue_sel % 2;
        PMD_DRV_LOG(DEBUG, "vhostfd=%d, index=%u", vhostfd, *(unsigned int *)arg);
        break;
    default:
        vhostfd = -1;
    }
    ...
    if (ret < 0)
        PMD_DRV_LOG(ERR, "%s failed: %s", vhost_msg_strings[req], strerror(errno));
...
static int vhost_kernel_enable_queue_pair(struct virtio_user_dev *dev, uint16_t pair_idx, int enable) {
    int hdr_size;
    int vhostfd;
    int tapfd;
    ...
    if ((dev->features & (1ULL << VIRTIO_NET_F_MRG_RXBUF)) || (dev->features & (1ULL << VIRTIO_F_VERSION_1)))
        hdr_size = sizeof(struct virtio_net_hdr_mrg_rxbuf);
    else
        hdr_size = sizeof(struct virtio_net_hdr);

    tapfd = vhost_kernel_open_tap(&dev->ifname, hdr_size, req_mq, (char *)dev->mac_addr, dev->features);
    if (tapfd < 0) {
...
struct virtio_user_backend_ops virtio_ops_kernel = {
    .setup = vhost_kernel_setup, .send_request = vhost_kernel_ioctl, .enable_qp = vhost_kernel_enable_queue_pair};
