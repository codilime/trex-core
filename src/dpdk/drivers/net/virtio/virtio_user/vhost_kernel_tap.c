...
static int vhost_kernel_tap_set_offload(int fd, uint64_t features) {
    ...
    if (features & (1ULL << VIRTIO_NET_F_GUEST_CSUM)) {
        ...
        if (features & (1ULL << VIRTIO_NET_F_GUEST_TSO6))
            offload |= TUN_F_TSO6;
        if (features & ((1ULL << VIRTIO_NET_F_GUEST_TSO4) | (1ULL << VIRTIO_NET_F_GUEST_TSO6)) &&
...
int vhost_kernel_open_tap(char **p_ifname, int hdr_size, int req_mq, const char *mac, uint64_t features) {
