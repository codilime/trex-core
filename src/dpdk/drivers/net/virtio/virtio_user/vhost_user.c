...
static int vhost_user_read(int fd, struct vhost_user_msg *msg) {
    ...
    if (sz_payload) {
        ...
        if (ret < sz_payload) {
            PMD_DRV_LOG(ERR, "Failed to recv msg payload: %d instead of %d.", ret, msg->size);
            goto fail;
        }
    }
...
static int update_memory_region(const struct rte_memseg_list *msl __rte_unused, const struct rte_memseg *ms,
                                void *arg) {
    ...
    for (i = 0; i < wa->region_nr; i++) {
        ...
        PMD_DRV_LOG(DEBUG, "index=%d fd=%d offset=0x%" PRIx64 " addr=0x%" PRIx64 " len=%" PRIu64, i, fd,
                    mr->mmap_offset, mr->userspace_addr, mr->memory_size);
    ...
    PMD_DRV_LOG(DEBUG, "index=%d fd=%d offset=0x%" PRIx64 " addr=0x%" PRIx64 " len=%" PRIu64, i, fd, mr->mmap_offset,
...
static int prepare_vhost_memory_user(struct vhost_user_msg *msg, int fds[]) {
    struct walk_arg wa;
...
const char *const vhost_msg_strings[] = {
    ...
    [VHOST_USER_SET_MEM_TABLE] = "VHOST_SET_MEM_TABLE",   [VHOST_USER_SET_VRING_ENABLE] = "VHOST_SET_VRING_ENABLE",
};

static int vhost_user_sock(struct virtio_user_dev *dev, enum vhost_user_request req, void *arg) {
    ...
    if (need_reply) {
        if (vhost_user_read(vhostfd, &msg) < 0) {
            PMD_DRV_LOG(ERR, "Received msg failed: %s", strerror(errno));
            return -1;
        }
...
static int virtio_user_start_server(struct virtio_user_dev *dev, struct sockaddr_un *un) {
    int ret;
    int flag;
    int fd = dev->listenfd;
...
static int vhost_user_enable_queue_pair(struct virtio_user_dev *dev, uint16_t pair_idx, int enable) {
    int i;
...
struct virtio_user_backend_ops virtio_ops_user = {
    .setup = vhost_user_setup, .send_request = vhost_user_sock, .enable_qp = vhost_user_enable_queue_pair};
