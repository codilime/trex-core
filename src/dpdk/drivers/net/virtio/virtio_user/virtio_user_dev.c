...
static int virtio_user_create_queue(struct virtio_user_dev *dev, uint32_t queue_sel) {
...
static int virtio_user_kick_queue(struct virtio_user_dev *dev, uint32_t queue_sel) {
    struct vhost_vring_file file;
...
static int virtio_user_queue_setup(struct virtio_user_dev *dev, int (*fn)(struct virtio_user_dev *, uint32_t)) {
...
int virtio_user_stop_device(struct virtio_user_dev *dev) {
    ...
    for (i = 0; i < dev->max_queue_pairs * 2; ++i) {
        state.index = i;
        if (dev->ops->send_request(dev, VHOST_USER_GET_VRING_BASE, &state) < 0) {
...
static inline void parse_mac(struct virtio_user_dev *dev, const char *mac) {
    ...
    r = sscanf(mac, "%x:%x:%x:%x:%x:%x", &tmp[0], &tmp[1], &tmp[2], &tmp[3], &tmp[4], &tmp[5]);
    if (r == ETHER_ADDR_LEN) {
...
static void virtio_user_mem_event_cb(enum rte_mem_event type __rte_unused, const void *addr __rte_unused,
                                     size_t len __rte_unused, void *arg) {
...
static int virtio_user_dev_setup(struct virtio_user_dev *dev) {
    ...
    if (dev->is_server) {
        if (access(dev->path, F_OK) == 0 && !is_vhost_user_by_type(dev->path)) {
...
#define VIRTIO_USER_SUPPORTED_FEATURES                                                                                 \
    (1ULL << VIRTIO_NET_F_MAC | 1ULL << VIRTIO_NET_F_STATUS | 1ULL << VIRTIO_NET_F_MQ |                                \
     1ULL << VIRTIO_NET_F_CTRL_MAC_ADDR | 1ULL << VIRTIO_NET_F_CTRL_VQ | 1ULL << VIRTIO_NET_F_CTRL_RX |                \
     1ULL << VIRTIO_NET_F_CTRL_VLAN | 1ULL << VIRTIO_NET_F_CSUM | 1ULL << VIRTIO_NET_F_HOST_TSO4 |                     \
     1ULL << VIRTIO_NET_F_HOST_TSO6 | 1ULL << VIRTIO_NET_F_MRG_RXBUF | 1ULL << VIRTIO_RING_F_INDIRECT_DESC |           \
     1ULL << VIRTIO_NET_F_GUEST_CSUM | 1ULL << VIRTIO_NET_F_GUEST_TSO4 | 1ULL << VIRTIO_NET_F_GUEST_TSO6 |             \
     1ULL << VIRTIO_F_IN_ORDER | 1ULL << VIRTIO_F_VERSION_1 | 1ULL << VIRTIO_F_RING_PACKED)

int virtio_user_dev_init(struct virtio_user_dev *dev, char *path, int queues, int cq, int queue_size, const char *mac,
                         char **ifname, int server, int mrg_rxbuf, int in_order, int packed_vq) {
    ...
    if (rte_mem_event_callback_register(VIRTIO_USER_MEM_EVENT_CLB_NAME, virtio_user_mem_event_cb, dev)) {
...
uint8_t virtio_user_handle_mq(struct virtio_user_dev *dev, uint16_t q_pairs) {
    ...
    if (q_pairs > dev->max_queue_pairs) {
        PMD_INIT_LOG(ERR, "multi-q config %u, but only %u supported", q_pairs, dev->max_queue_pairs);
        return -1;
    }
...
static uint32_t virtio_user_handle_ctrl_msg(struct virtio_user_dev *dev, struct vring *vring, uint16_t idx_hdr) {
    ...
    if (hdr->class == VIRTIO_NET_CTRL_MQ && hdr->cmd == VIRTIO_NET_CTRL_MQ_VQ_PAIRS_SET) {
        uint16_t queues;
...
static inline int desc_is_avail(struct vring_packed_desc *desc, bool wrap_counter) {
...
static uint32_t virtio_user_handle_ctrl_msg_packed(struct virtio_user_dev *dev, struct vring_packed *vring,
                                                   uint16_t idx_hdr) {
    struct virtio_net_ctrl_hdr *hdr;
    virtio_net_ctrl_ack status = ~0;
