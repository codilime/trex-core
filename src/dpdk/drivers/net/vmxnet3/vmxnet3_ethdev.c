...
#define VMXNET3_TX_OFFLOAD_CAP                                                                                         \
    (DEV_TX_OFFLOAD_VLAN_INSERT | DEV_TX_OFFLOAD_TCP_CKSUM | DEV_TX_OFFLOAD_UDP_CKSUM | DEV_TX_OFFLOAD_TCP_TSO |       \
     DEV_TX_OFFLOAD_MULTI_SEGS)

#define VMXNET3_RX_OFFLOAD_CAP                                                                                         \
    (DEV_RX_OFFLOAD_VLAN_STRIP | DEV_RX_OFFLOAD_VLAN_FILTER | DEV_RX_OFFLOAD_SCATTER | DEV_RX_OFFLOAD_IPV4_CKSUM |     \
     DEV_RX_OFFLOAD_UDP_CKSUM | DEV_RX_OFFLOAD_TCP_CKSUM | DEV_RX_OFFLOAD_TCP_LRO | DEV_RX_OFFLOAD_JUMBO_FRAME)
...
static int __vmxnet3_dev_link_update(struct rte_eth_dev *dev, int wait_to_complete);
static int vmxnet3_dev_link_update(struct rte_eth_dev *dev, int wait_to_complete);
static void vmxnet3_hw_stats_save(struct vmxnet3_hw *hw);
static int vmxnet3_dev_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats);
static void vmxnet3_dev_stats_reset(struct rte_eth_dev *dev);
static int vmxnet3_dev_xstats_get_names(struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats, unsigned int n);
static int vmxnet3_dev_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats, unsigned int n);
static void vmxnet3_dev_info_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info);
static const uint32_t *vmxnet3_dev_supported_ptypes_get(struct rte_eth_dev *dev);
static int vmxnet3_dev_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vid, int on);
static int vmxnet3_dev_vlan_offload_set(struct rte_eth_dev *dev, int mask);
static int vmxnet3_mac_addr_set(struct rte_eth_dev *dev, struct ether_addr *mac_addr);
...
static const struct rte_memzone *gpa_zone_reserve(struct rte_eth_dev *dev, uint32_t size, const char *post_string,
                                                  int socket_id, uint16_t align, bool reuse) {
    char z_name[RTE_MEMZONE_NAMESIZE];
    const struct rte_memzone *mz;

    snprintf(z_name, sizeof(z_name), "eth_p%d_%s", dev->data->port_id, post_string);
 ...
 */
static void vmxnet3_disable_intr(struct vmxnet3_hw *hw) {
    int i;
...
static void vmxnet3_enable_intr(struct vmxnet3_hw *hw) {
    int i;
...
static uint16_t eth_vmxnet3_txdata_get(struct vmxnet3_hw *hw) {
    ...
    return (txdata_desc_size < VMXNET3_TXDATA_DESC_MIN_SIZE || txdata_desc_size > VMXNET3_TXDATA_DESC_MAX_SIZE ||
...
static int eth_vmxnet3_dev_init(struct rte_eth_dev *eth_dev) {
    ...
    eth_dev->data->mac_addrs = rte_zmalloc("vmxnet3", ETHER_ADDR_LEN * VMXNET3_MAX_MAC_ADDRS, 0);
    ...
    ether_addr_copy((struct ether_addr *)hw->perm_addr, &eth_dev->data->mac_addrs[0]);

    PMD_INIT_LOG(DEBUG, "MAC Address : %02x:%02x:%02x:%02x:%02x:%02x", hw->perm_addr[0], hw->perm_addr[1],
                 hw->perm_addr[2], hw->perm_addr[3], hw->perm_addr[4], hw->perm_addr[5]);
    ...
    hw->txdata_desc_size = VMXNET3_VERSION_GE_3(hw) ? eth_vmxnet3_txdata_get(hw) : sizeof(struct Vmxnet3_TxDataDesc);

    hw->rxdata_desc_size = VMXNET3_VERSION_GE_3(hw) ? VMXNET3_DEF_RXDATA_DESC_SIZE : 0;
    RTE_ASSERT((hw->rxdata_desc_size & ~VMXNET3_RXDATA_DESC_SIZE_MASK) == hw->rxdata_desc_size);
...
static int eth_vmxnet3_pci_probe(struct rte_pci_driver *pci_drv __rte_unused, struct rte_pci_device *pci_dev) {
...
static int vmxnet3_dev_configure(struct rte_eth_dev *dev) {
    ...
    if (dev->data->nb_tx_queues > VMXNET3_MAX_TX_QUEUES || dev->data->nb_rx_queues > VMXNET3_MAX_RX_QUEUES) {
     ...
     * on current socket
     */
    mz = gpa_zone_reserve(dev, sizeof(struct Vmxnet3_DriverShared), "shared", rte_socket_id(), 8, 1);
     ...
     */
    mz = gpa_zone_reserve(dev, size, "queuedesc", rte_socket_id(), VMXNET3_QUEUE_DESC_ALIGN, 0);
    if (mz == NULL) {
    ...
    if (dev->data->dev_conf.rxmode.mq_mode == ETH_MQ_RX_RSS) {
        ...
        mz = gpa_zone_reserve(dev, sizeof(struct VMXNET3_RSSConf), "rss_conf", rte_socket_id(), RTE_CACHE_LINE_SIZE, 1);
        if (mz == NULL) {
...
static void vmxnet3_write_mac(struct vmxnet3_hw *hw, const uint8_t *addr) {
    ...
    PMD_INIT_LOG(DEBUG, "Writing MAC Address : %02x:%02x:%02x:%02x:%02x:%02x", addr[0], addr[1], addr[2], addr[3],
                 addr[4], addr[5]);
...
static int vmxnet3_dev_setup_memreg(struct rte_eth_dev *dev) {
    ...
    if (hw->memRegsPA == 0) {
        ...
        size = sizeof(Vmxnet3_MemRegs) + (VMXNET3_MAX_RX_QUEUES + VMXNET3_MAX_TX_QUEUES) * sizeof(Vmxnet3_MemoryRegion);
    ...
    for (i = 0; i < num; i++) {
        ...
        mr->length = STAILQ_FIRST(&mp[i]->mem_list)->len <= INT32_MAX ? STAILQ_FIRST(&mp[i]->mem_list)->len : INT32_MAX;
...
static int vmxnet3_setup_driver_shared(struct rte_eth_dev *dev) {
    ...
    devRead->misc.driverInfo.gos.gosBits = sizeof(void *) == 4 ? VMXNET3_GOS_BITS_32 : VMXNET3_GOS_BITS_64;
    ...
    ret = vmxnet3_dev_vlan_offload_set(dev, ETH_VLAN_STRIP_MASK | ETH_VLAN_FILTER_MASK);
    if (ret)
        return ret;
...
static int vmxnet3_dev_start(struct rte_eth_dev *dev) {
    ...
    if (dev->data->dev_conf.intr_conf.lsc) {
        ...
        rte_intr_callback_register(&pci_dev->intr_handle, vmxnet3_interrupt_handler, dev);
    ...
    VMXNET3_WRITE_BAR1_REG(hw, VMXNET3_REG_DSAL, VMXNET3_GET_ADDR_LO(hw->sharedPA));
    VMXNET3_WRITE_BAR1_REG(hw, VMXNET3_REG_DSAH, VMXNET3_GET_ADDR_HI(hw->sharedPA));
...
static void vmxnet3_dev_stop(struct rte_eth_dev *dev) {
    ...
    if (dev->data->dev_conf.intr_conf.lsc) {
        ...
        rte_intr_callback_unregister(&pci_dev->intr_handle, vmxnet3_interrupt_handler, dev);
    }
...
static void vmxnet3_hw_tx_stats_get(struct vmxnet3_hw *hw, unsigned int q, struct UPT1_TxStats *res) {
#define VMXNET3_UPDATE_TX_STAT(h, i, f, r) ((r)->f = (h)->tqd_start[(i)].stats.f + (h)->saved_tx_stats[(i)].f)
...
static void vmxnet3_hw_rx_stats_get(struct vmxnet3_hw *hw, unsigned int q, struct UPT1_RxStats *res) {
#define VMXNET3_UPDATE_RX_STAT(h, i, f, r) ((r)->f = (h)->rqd_start[(i)].stats.f + (h)->saved_rx_stats[(i)].f)
...
static void vmxnet3_tx_stats_get(struct vmxnet3_hw *hw, unsigned int q, struct UPT1_TxStats *res) {
    vmxnet3_hw_tx_stats_get(hw, q, res);

#define VMXNET3_REDUCE_SNAPSHOT_TX_STAT(h, i, f, r) ((r)->f -= (h)->snapshot_tx_stats[(i)].f)
...
static void vmxnet3_rx_stats_get(struct vmxnet3_hw *hw, unsigned int q, struct UPT1_RxStats *res) {
    vmxnet3_hw_rx_stats_get(hw, q, res);

#define VMXNET3_REDUCE_SNAPSHOT_RX_STAT(h, i, f, r) ((r)->f -= (h)->snapshot_rx_stats[(i)].f)
...
static int vmxnet3_dev_xstats_get_names(struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats_names,
                                        unsigned int n) {
    unsigned int i, t, count = 0;
    unsigned int nstats = dev->data->nb_tx_queues * RTE_DIM(vmxnet3_txq_stat_strings) +
...
static int vmxnet3_dev_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats, unsigned int n) {
    ...
    for (i = 0; i < dev->data->nb_rx_queues; i++) {
        ...
        for (t = 0; t < RTE_DIM(vmxnet3_rxq_stat_strings); t++) {
            xstats[count].value = *(uint64_t *)(((char *)&rxq->stats) + vmxnet3_rxq_stat_strings[t].offset);
    ...
    for (i = 0; i < dev->data->nb_tx_queues; i++) {
        ...
        for (t = 0; t < RTE_DIM(vmxnet3_txq_stat_strings); t++) {
            xstats[count].value = *(uint64_t *)(((char *)&txq->stats) + vmxnet3_txq_stat_strings[t].offset);
            xstats[count].id = count;
            count++;
        }
    }
...
static int vmxnet3_dev_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
    ...
    for (i = 0; i < hw->num_tx_queues; i++) {
        ...
        stats->q_opackets[i] = txStats.ucastPktsTxOK + txStats.mcastPktsTxOK + txStats.bcastPktsTxOK;

        stats->q_obytes[i] = txStats.ucastBytesTxOK + txStats.mcastBytesTxOK + txStats.bcastBytesTxOK;
    ...
    for (i = 0; i < hw->num_rx_queues; i++) {
        ...
        stats->q_ipackets[i] = rxStats.ucastPktsRxOK + rxStats.mcastPktsRxOK + rxStats.bcastPktsRxOK;

        stats->q_ibytes[i] = rxStats.ucastBytesRxOK + rxStats.mcastBytesRxOK + rxStats.bcastBytesRxOK;
...
static void vmxnet3_dev_stats_reset(struct rte_eth_dev *dev) {
    ...
    for (i = 0; i < hw->num_tx_queues; i++) {
        ...
        memcpy(&hw->snapshot_tx_stats[i], &txStats, sizeof(hw->snapshot_tx_stats[0]));
    }
    for (i = 0; i < hw->num_rx_queues; i++) {
        vmxnet3_hw_rx_stats_get(hw, i, &rxStats);
        memcpy(&hw->snapshot_rx_stats[i], &rxStats, sizeof(hw->snapshot_rx_stats[0]));
    }
}

static void vmxnet3_dev_info_get(struct rte_eth_dev *dev __rte_unused, struct rte_eth_dev_info *dev_info) {
    struct vmxnet3_hw *hw = dev->data->dev_private;
...
static const uint32_t *vmxnet3_dev_supported_ptypes_get(struct rte_eth_dev *dev) {
    static const uint32_t ptypes[] = {RTE_PTYPE_L3_IPV4_EXT, RTE_PTYPE_L3_IPV4, RTE_PTYPE_UNKNOWN};
...
static int __vmxnet3_dev_link_update(struct rte_eth_dev *dev, __rte_unused int wait_to_complete) {
    struct vmxnet3_hw *hw = dev->data->dev_private;
    struct rte_eth_link link;
    uint32_t ret;
