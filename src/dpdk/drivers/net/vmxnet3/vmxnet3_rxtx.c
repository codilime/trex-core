...
#define VMXNET3_TX_OFFLOAD_MASK (PKT_TX_VLAN_PKT | PKT_TX_IPV6 | PKT_TX_IPV4 | PKT_TX_L4_MASK | PKT_TX_TCP_SEG)

#define VMXNET3_TX_OFFLOAD_NOTSUP_MASK (PKT_TX_OFFLOAD_MASK ^ VMXNET3_TX_OFFLOAD_MASK)
...
static void vmxnet3_rxq_dump(struct vmxnet3_rx_queue *rxq) {
    ...
    PMD_RX_LOG(DEBUG, "RXQ: cmd0 base : %p cmd1 base : %p comp ring base : %p.", rxq->cmd_ring[0].base,
               rxq->cmd_ring[1].base, rxq->comp_ring.base);
    ...
    PMD_RX_LOG(DEBUG, "RXQ:cmd0: size=%u; free=%u; next2proc=%u; queued=%u", (uint32_t)rxq->cmd_ring[0].size, avail,
               rxq->comp_ring.next2proc, rxq->cmd_ring[0].size - avail);

    avail = vmxnet3_cmd_ring_desc_avail(&rxq->cmd_ring[1]);
    PMD_RX_LOG(DEBUG, "RXQ:cmd1 size=%u; free=%u; next2proc=%u; queued=%u", (uint32_t)rxq->cmd_ring[1].size, avail,
               rxq->comp_ring.next2proc, rxq->cmd_ring[1].size - avail);
}
...
static void vmxnet3_txq_dump(struct vmxnet3_tx_queue *txq) {
    ...
    PMD_TX_LOG(DEBUG, "TXQ: cmd base : %p comp ring base : %p data ring base : %p.", txq->cmd_ring.base,
               txq->comp_ring.base, txq->data_ring.base);
    ...
    PMD_TX_LOG(DEBUG, "TXQ: size=%u; free=%u; next2proc=%u; queued=%u", (uint32_t)txq->cmd_ring.size, avail,
...
static void vmxnet3_tq_tx_complete(vmxnet3_tx_queue_t *txq) {
    ...
    struct Vmxnet3_TxCompDesc *tcd = (struct Vmxnet3_TxCompDesc *)(comp_ring->base + comp_ring->next2proc);
...
uint16_t vmxnet3_prep_pkts(__rte_unused void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    int32_t ret;
    uint32_t i;
    uint64_t ol_flags;
    struct rte_mbuf *m;
    ...
    for (i = 0; i != nb_pkts; i++) {
         ...
         */
        if ((ol_flags & PKT_TX_TCP_SEG) == 0 && m->nb_segs > VMXNET3_MAX_TXD_PER_PKT) {
            rte_errno = -EINVAL;
            return i;
        }

        /* check that only supported TX offloads are requested. */
        if ((ol_flags & VMXNET3_TX_OFFLOAD_NOTSUP_MASK) != 0 || (ol_flags & PKT_TX_L4_MASK) == PKT_TX_SCTP_CKSUM) {
            rte_errno = -ENOTSUP;
            return i;
        }
...
uint16_t vmxnet3_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    uint16_t nb_tx;
    ...
    while (nb_tx < nb_pkts) {
        ...
        if (txm->nb_segs == 1 && rte_pktmbuf_pkt_len(txm) <= txq->txdata_desc_size) {
            ...
            tdd = (struct Vmxnet3_TxDataDesc *)((uint8 *)txq->data_ring.base +
                                                txq->cmd_ring.next2fill * txq->txdata_desc_size);
        ...
        if (tso) {
            ...
            case PKT_TX_UDP_CKSUM:
                ...
                break;
            default:
                PMD_TX_LOG(WARNING, "requested cksum offload not supported %#llx", txm->ol_flags & PKT_TX_L4_MASK);
                abort();
            }
            deferred++;
        } else {
    ...
    if (deferred >= rte_le_to_cpu_32(txq_ctrl->txThreshold)) {
        ...
        VMXNET3_WRITE_BAR0_REG(hw, (VMXNET3_REG_TXPROD + txq->queue_id * VMXNET3_REG_ALIGN), txq->cmd_ring.next2fill);
    }

    return nb_tx;
}

static inline void vmxnet3_renew_desc(vmxnet3_rx_queue_t *rxq, uint8_t ring_id, struct rte_mbuf *mbuf) {
    uint32_t val;
    struct vmxnet3_cmd_ring *ring = &rxq->cmd_ring[ring_id];
    struct Vmxnet3_RxDesc *rxd = (struct Vmxnet3_RxDesc *)(ring->base + ring->next2fill);
...
static uint16_t vmxnet3_guess_mss(struct vmxnet3_hw *hw, const Vmxnet3_RxCompDesc *rcd, struct rte_mbuf *rxm) {
    uint32_t hlen, slen;
    ...
    if (rcd->v4) {
        if (unlikely(slen < hlen + sizeof(struct ipv4_hdr)))
            return hw->mtu - sizeof(struct ipv4_hdr) - sizeof(struct tcp_hdr);

        ipv4_hdr = (struct ipv4_hdr *)(ptr + hlen);
        hlen += (ipv4_hdr->version_ihl & IPV4_HDR_IHL_MASK) * IPV4_IHL_MULTIPLIER;
    } else if (rcd->v6) {
        if (unlikely(slen < hlen + sizeof(struct ipv6_hdr)))
            return hw->mtu - sizeof(struct ipv6_hdr) - sizeof(struct tcp_hdr);
    ...
    if (unlikely(slen < hlen + sizeof(struct tcp_hdr)))
        return hw->mtu - hlen - sizeof(struct tcp_hdr) + sizeof(struct ether_hdr);

    tcp_hdr = (struct tcp_hdr *)(ptr + hlen);
    hlen += (tcp_hdr->data_off & 0xf0) >> 2;

    if (rxm->udata64 > 1)
        return (rte_pktmbuf_pkt_len(rxm) - hlen + rxm->udata64 - 1) / rxm->udata64;
    else
        return hw->mtu - hlen + sizeof(struct ether_hdr);
}

/* Receive side checksum and other offloads */
static inline void vmxnet3_rx_offload(struct vmxnet3_hw *hw, const Vmxnet3_RxCompDesc *rcd, struct rte_mbuf *rxm,
                                      const uint8_t sop) {
 ...
 */
uint16_t vmxnet3_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    uint16_t nb_rx;
    uint32_t nb_rxd, idx;
    uint8_t ring_idx;
    ...
    while (rcd->gen == rxq->comp_ring.gen) {
        ...
        if (unlikely(rcd->eop && rcd->err)) {
            ...
            PMD_RX_LOG(ERR, "Error in received packet rcd#:%d rxd:%d",
                       (int)(rcd - (struct Vmxnet3_RxCompDesc *)rxq->comp_ring.base), rcd->rxdIdx);
...
int vmxnet3_dev_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx, uint16_t nb_desc, unsigned int socket_id,
    ...
    mz = rte_eth_dma_zone_reserve(dev, "txdesc", queue_idx, size, VMXNET3_RING_BA_ALIGN, socket_id);
    if (mz == NULL) {
    ...
    comp_ring->basePA = ring->basePA + (sizeof(struct Vmxnet3_TxDesc) * ring->size);

    /* data_ring initialization */
    data_ring->base = (Vmxnet3_TxDataDesc *)(comp_ring->base + comp_ring->size);
    data_ring->basePA = comp_ring->basePA + (sizeof(struct Vmxnet3_TxCompDesc) * comp_ring->size);

    /* cmd_ring0 buf_info allocation */
    ring->buf_info = rte_zmalloc("tx_ring_buf_info", ring->size * sizeof(vmxnet3_buf_info_t), RTE_CACHE_LINE_SIZE);
...
int vmxnet3_dev_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx, uint16_t nb_desc, unsigned int socket_id,
                               __rte_unused const struct rte_eth_rxconf *rx_conf, struct rte_mempool *mp) {
    const struct rte_memzone *mz;
    ...
    mz = rte_eth_dma_zone_reserve(dev, "rxdesc", queue_idx, size, VMXNET3_RING_BA_ALIGN, socket_id);
    if (mz == NULL) {
        PMD_INIT_LOG(ERR, "ERROR: Creating queue descriptors zone");
        return -ENOMEM;
    }
    ...
    if (VMXNET3_VERSION_GE_3(hw) && rxq->data_desc_size) {
        ...
        data_ring->basePA = comp_ring->basePA + sizeof(struct Vmxnet3_RxCompDesc) * comp_ring->size;
    }
...
int vmxnet3_v4_rss_configure(struct rte_eth_dev *dev) {
    ...
    rss_hf = port_rss_conf->rss_hf & (VMXNET3_V4_RSS_MASK | VMXNET3_RSS_OFFLOAD_ALL);
