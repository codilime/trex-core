...
static struct rte_cfgfile_section *_get_section(struct rte_cfgfile *cfg, const char *sectionname) {
    int i;

    for (i = 0; i < cfg->num_sections; i++) {
        if (strncmp(cfg->sections[i].name, sectionname, sizeof(cfg->sections[0].name)) == 0)
            return &cfg->sections[i];
    }
    return NULL;
}

static int _add_entry(struct rte_cfgfile_section *section, const char *entryname, const char *entryvalue) {
    ...
    if (section->num_entries == section->allocated_entries) {
        struct rte_cfgfile_entry *n_entries =
            realloc(section->entries,
                    sizeof(struct rte_cfgfile_entry) * ((section->allocated_entries) + CFG_ALLOC_ENTRY_BATCH));
    ...
    struct rte_cfgfile_entry *curr_entry = &section->entries[section->num_entries];
...
static int rte_cfgfile_check_params(const struct rte_cfgfile_parameters *params) {
...
struct rte_cfgfile *rte_cfgfile_load_with_params(const char *filename, int flags,
...
struct rte_cfgfile *rte_cfgfile_create(int flags) {
    ...
    cfg->sections = malloc(sizeof(struct rte_cfgfile_section) * CFG_ALLOC_SECTION_BATCH);
    ...
    for (i = 0; i < CFG_ALLOC_SECTION_BATCH; i++) {
        cfg->sections[i].entries = malloc(sizeof(struct rte_cfgfile_entry) * CFG_ALLOC_ENTRY_BATCH);
...
int rte_cfgfile_add_section(struct rte_cfgfile *cfg, const char *sectionname) {
    ...
    if (cfg->num_sections == cfg->allocated_sections) {
        ...
        struct rte_cfgfile_section *n_sections = realloc(
            cfg->sections, sizeof(struct rte_cfgfile_section) * ((cfg->allocated_sections) + CFG_ALLOC_SECTION_BATCH));
    ...
    strlcpy(cfg->sections[cfg->num_sections].name, sectionname, sizeof(cfg->sections[0].name));
...
int rte_cfgfile_add_entry(struct rte_cfgfile *cfg, const char *sectionname, const char *entryname,
                          const char *entryvalue) {
    int ret;

    if ((cfg == NULL) || (sectionname == NULL) || (entryname == NULL) || (entryvalue == NULL))
        return -EINVAL;
...
int rte_cfgfile_set_entry(struct rte_cfgfile *cfg, const char *sectionname, const char *entryname,
                          const char *entryvalue) {
    int i;
    ...
    for (i = 0; i < curr_section->num_entries; i++)
        if (!strcmp(curr_section->entries[i].name, entryname)) {
            strlcpy(curr_section->entries[i].value, entryvalue, sizeof(curr_section->entries[i].value));
            return 0;
        }
...
int rte_cfgfile_save(struct rte_cfgfile *cfg, const char *filename) {
    ...
    for (i = 0; i < cfg->num_sections; i++) {
        ...
        for (j = 0; j < cfg->sections[i].num_entries; j++) {
            fprintf(f, "%s=%s\n", cfg->sections[i].entries[j].name, cfg->sections[i].entries[j].value);
        }
    }
    return fclose(f);
}
...
int rte_cfgfile_num_sections(struct rte_cfgfile *cfg, const char *sectionname, size_t length) {
    int i;
    int num_sections = 0;
    for (i = 0; i < cfg->num_sections; i++) {
...
int rte_cfgfile_section_entries(struct rte_cfgfile *cfg, const char *sectionname, struct rte_cfgfile_entry *entries,
                                int max_entries) {
    int i;
...
int rte_cfgfile_section_entries_by_index(struct rte_cfgfile *cfg, int index, char *sectionname,
