...
static struct rte_cryptodev_global cryptodev_globals = {
    .devs = rte_crypto_devices, .data = {NULL}, .nb_devs = 0, .max_devs = RTE_CRYPTO_MAX_DEVS};
 ...
 */
const char *rte_crypto_cipher_algorithm_strings[] = {[RTE_CRYPTO_CIPHER_3DES_CBC] = "3des-cbc",
...
const char *rte_crypto_cipher_operation_strings[] = {
    [RTE_CRYPTO_CIPHER_OP_ENCRYPT] = "encrypt", [RTE_CRYPTO_CIPHER_OP_DECRYPT] = "decrypt"};

/**
 * The crypto auth algorithm strings identifiers.
 * It could be used in application command line.
 */
const char *rte_crypto_auth_algorithm_strings[] = {[RTE_CRYPTO_AUTH_AES_CBC_MAC] = "aes-cbc-mac",
...
const char *rte_crypto_aead_operation_strings[] = {
    [RTE_CRYPTO_AEAD_OP_ENCRYPT] = "encrypt", [RTE_CRYPTO_AEAD_OP_DECRYPT] = "decrypt"};

/**
 * Asymmetric crypto transform operation strings identifiers.
 */
const char *rte_crypto_asym_xform_strings[] = {
    [RTE_CRYPTO_ASYM_XFORM_NONE] = "none",    [RTE_CRYPTO_ASYM_XFORM_RSA] = "rsa",
    [RTE_CRYPTO_ASYM_XFORM_MODEX] = "modexp", [RTE_CRYPTO_ASYM_XFORM_MODINV] = "modinv",
    [RTE_CRYPTO_ASYM_XFORM_DH] = "dh",        [RTE_CRYPTO_ASYM_XFORM_DSA] = "dsa",
};
...
const char *rte_crypto_auth_operation_strings[] = {
    [RTE_CRYPTO_AUTH_OP_VERIFY] = "verify", [RTE_CRYPTO_AUTH_OP_GENERATE] = "generate"};
...
rte_cryptodev_sym_capability_get(uint8_t dev_id, const struct rte_cryptodev_sym_capability_idx *idx) {
    ...
    while ((capability = &dev_info.capabilities[i++])->op != RTE_CRYPTO_OP_TYPE_UNDEFINED) {
        ...
        if (idx->type == RTE_CRYPTO_SYM_XFORM_AUTH && capability->sym.auth.algo == idx->algo.auth)
            return &capability->sym;

        if (idx->type == RTE_CRYPTO_SYM_XFORM_CIPHER && capability->sym.cipher.algo == idx->algo.cipher)
            return &capability->sym;

        if (idx->type == RTE_CRYPTO_SYM_XFORM_AEAD && capability->sym.aead.algo == idx->algo.aead)
            return &capability->sym;
    }
...
static int param_range_check(uint16_t size, const struct rte_crypto_param_range *range) {
    ...
    for (next_size = range->min; next_size <= range->max; next_size += range->increment)
        if (size == next_size)
            return 0;
...
int __rte_experimental rte_cryptodev_asym_xform_capability_check_optype(
    const struct rte_cryptodev_asymmetric_xform_capability *capability, enum rte_crypto_asym_op_type op_type) {
...
int __rte_experimental rte_cryptodev_asym_xform_capability_check_modlen(
    const struct rte_cryptodev_asymmetric_xform_capability *capability, uint16_t modlen) {
...
struct rte_cryptodev *rte_cryptodev_pmd_get_named_dev(const char *name) {
    ...
    for (i = 0; i < cryptodev_globals.max_devs; i++) {
        ...
        if ((dev->attached == RTE_CRYPTODEV_ATTACHED) && (strcmp(dev->data->name, name) == 0))
            return dev;
    }
...
uint8_t rte_cryptodev_devices_get(const char *driver_name, uint8_t *devices, uint8_t nb_devices) {
    ...
    for (i = 0; i < max_devs && count < nb_devices; i++) {
        ...
        if (devs[i].attached == RTE_CRYPTODEV_ATTACHED) {
            ...
            cmp = strncmp(devs[i].device->driver->name, driver_name, strlen(driver_name) + 1);
...
void *rte_cryptodev_get_sec_ctx(uint8_t dev_id) {
    if (rte_crypto_devices[dev_id].feature_flags & RTE_CRYPTODEV_FF_SECURITY)
...
static inline int rte_cryptodev_data_alloc(uint8_t dev_id, struct rte_cryptodev_data **data, int socket_id) {
    ...
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
        mz = rte_memzone_reserve(mz_name, sizeof(struct rte_cryptodev_data), socket_id, 0);
    } else
...
static int rte_cryptodev_queue_pairs_config(struct rte_cryptodev *dev, uint16_t nb_qpairs, int socket_id) {
    struct rte_cryptodev_info dev_info;
    void **qp;
    unsigned i;
    ...
    CDEV_LOG_DEBUG("Setup %d queues pairs on device %u", nb_qpairs, dev->data->dev_id);
    ...
    if (nb_qpairs > (dev_info.max_nb_queue_pairs)) {
        CDEV_LOG_ERR("Invalid num queue_pairs (%u) for dev %u", nb_qpairs, dev->data->dev_id);
        return -EINVAL;
    }

    if (dev->data->queue_pairs == NULL) { /* first time configuration */
        dev->data->queue_pairs = rte_zmalloc_socket(
            "cryptodev->queue_pairs", sizeof(dev->data->queue_pairs[0]) * nb_qpairs, RTE_CACHE_LINE_SIZE, socket_id);
...
int rte_cryptodev_configure(uint8_t dev_id, struct rte_cryptodev_config *config) {
    ...
    diag = rte_cryptodev_queue_pairs_config(dev, config->nb_queue_pairs, config->socket_id);
    if (diag != 0) {
...
int rte_cryptodev_queue_pair_setup(uint8_t dev_id, uint16_t queue_pair_id, const struct rte_cryptodev_qp_conf *qp_conf,
                                   int socket_id)

{
    ...
    if (qp_conf->mp_session) {
        ...
        if (!pool_priv || qp_conf->mp_session->private_data_size < sizeof(*pool_priv)) {
        ...
        if ((rte_cryptodev_sym_get_existing_header_session_size(&s) > obj_size) || (s.nb_drivers <= dev->driver_id) ||
...
int rte_cryptodev_callback_register(uint8_t dev_id, enum rte_cryptodev_event_type event, rte_cryptodev_cb_fn cb_fn,
                                    void *cb_arg) {
    struct rte_cryptodev *dev;
    ...
    if (user_cb == NULL) {
        user_cb = rte_zmalloc("INTR_USER_CALLBACK", sizeof(struct rte_cryptodev_callback), 0);
        if (user_cb != NULL) {
            user_cb->cb_fn = cb_fn;
...
int rte_cryptodev_callback_unregister(uint8_t dev_id, enum rte_cryptodev_event_type event, rte_cryptodev_cb_fn cb_fn,
                                      void *cb_arg) {
    int ret;
    struct rte_cryptodev *dev;
...
int rte_cryptodev_sym_session_init(uint8_t dev_id, struct rte_cryptodev_sym_session *sess,
                                   struct rte_crypto_sym_xform *xforms, struct rte_mempool *mp) {
    struct rte_cryptodev *dev;
...
int __rte_experimental rte_cryptodev_asym_session_init(uint8_t dev_id, struct rte_cryptodev_asym_session *sess,
    ...
    if (sess->sess_private_data[index] == NULL) {
        ret = dev->dev_ops->asym_session_configure(dev, xforms, sess, mp);
        if (ret < 0) {
            CDEV_LOG_ERR("dev_id %d failed to configure session details", dev_id);
            return ret;
        }
...
struct rte_mempool *__rte_experimental rte_cryptodev_sym_session_pool_create(const char *name, uint32_t nb_elts,
    ...
    mp = rte_mempool_create(name, nb_elts, obj_sz, cache_size, (uint32_t)(sizeof(*pool_priv)), NULL, NULL, NULL, NULL,
                            socket_id, 0);
    if (mp == NULL) {
...
static unsigned int rte_cryptodev_sym_session_data_size(struct rte_cryptodev_sym_session *sess) {
    return (sizeof(sess->sess_data[0]) * sess->nb_drivers) + sess->user_data_sz;
}

struct rte_cryptodev_sym_session *rte_cryptodev_sym_session_create(struct rte_mempool *mp) {
...
struct rte_cryptodev_asym_session *__rte_experimental rte_cryptodev_asym_session_create(struct rte_mempool *mp) {
...
int __rte_experimental rte_cryptodev_asym_session_clear(uint8_t dev_id, struct rte_cryptodev_asym_session *sess) {
    struct rte_cryptodev *dev;
...
rte_cryptodev_sym_get_existing_header_session_size(struct rte_cryptodev_sym_session *sess) {
    if (!sess)
        return 0;
    else
        return (unsigned int)(sizeof(*sess) + rte_cryptodev_sym_session_data_size(sess));
}
...
int __rte_experimental rte_cryptodev_sym_session_set_user_data(struct rte_cryptodev_sym_session *sess, void *data,
                                                               uint16_t size) {
    if (sess == NULL)
        return -EINVAL;
...
static void rte_crypto_op_init(struct rte_mempool *mempool, void *opaque_arg, void *_op_data, __rte_unused unsigned i) {
...
struct rte_mempool *rte_crypto_op_pool_create(const char *name, enum rte_crypto_op_type type, unsigned nb_elts,
    ...
    mp = rte_mempool_create(name, nb_elts, elt_size, cache_size, sizeof(struct rte_crypto_op_pool_private), NULL, NULL,
...
int rte_cryptodev_pmd_create_dev_name(char *name, const char *dev_name_prefix) {
    ...
    for (i = 0; i < RTE_CRYPTO_MAX_DEVS; i++) {
        int ret = snprintf(name, RTE_CRYPTODEV_NAME_MAX_LEN, "%s_%u", dev_name_prefix, i);
...
static struct cryptodev_driver_list cryptodev_driver_list = TAILQ_HEAD_INITIALIZER(cryptodev_driver_list);
...
uint8_t rte_cryptodev_allocate_driver(struct cryptodev_driver *crypto_drv, const struct rte_driver *drv) {
