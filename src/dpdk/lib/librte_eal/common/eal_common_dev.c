...
static int build_devargs(const char *busname, const char *devname, const char *drvargs, char **devargs) {
    int length;
    ...
    *devargs = malloc(length + 1);
    ...
    length = snprintf(*devargs, length + 1, "%s:%s,%s", busname, devname, drvargs);
    if (length < 0) {
        free(*devargs);
        return -EINVAL;
    }
...
int __rte_experimental rte_dev_event_callback_register(const char *device_name, rte_dev_event_cb_fn cb_fn,
                                                       void *cb_arg) {
...
int __rte_experimental rte_dev_event_callback_unregister(const char *device_name, rte_dev_event_cb_fn cb_fn,
                                                         void *cb_arg) {
    int ret = 0;
    ...
    for (event_cb = TAILQ_FIRST(&dev_event_cbs); event_cb != NULL; event_cb = next) {

        next = TAILQ_NEXT(event_cb, next);

        if (device_name != NULL && event_cb->dev_name != NULL) {
            if (!strcmp(event_cb->dev_name, device_name)) {
                if (event_cb->cb_fn != cb_fn || (cb_arg != (void *)-1 && event_cb->cb_arg != cb_arg))
                    continue;
            }
...
__rte_experimental int rte_dev_iterator_init(struct rte_dev_iterator *it, const char *dev_str) {
...
__rte_experimental struct rte_device *rte_dev_iterator_next(struct rte_dev_iterator *it) {
    ...
    while ((bus = rte_bus_find(bus, bus_next_dev_cmp, CTX(it, bus_str, cls_str)))) {
        if (it->device != NULL) {
            it->bus = bus;
            goto out;
        }
