...
static struct rte_devargs_list devargs_list = TAILQ_HEAD_INITIALIZER(devargs_list);

static size_t devargs_layer_count(const char *s) {
    size_t i = s ? 1 : 0;
...
int rte_devargs_layers_parse(struct rte_devargs *devargs, const char *devstr) {
    ...
    while (s != NULL) {
         ...
         */
        if (strncmp(layers[i].key, s, strlen(layers[i].key)) && i != RTE_DIM(layers) - 1)
            goto next_layer;
        layers[i].str = s;
...
int rte_devargs_insert(struct rte_devargs **da) {
    ...
    TAILQ_FOREACH_SAFE(listed_da, &devargs_list, next, tmp) {
        if (listed_da == *da)
            ...
            return 0;
        if (strcmp(listed_da->bus->name, (*da)->bus->name) == 0 && strcmp(listed_da->name, (*da)->name) == 0) {
...
int rte_devargs_remove(struct rte_devargs *devargs) {
    ...
    TAILQ_FOREACH_SAFE(d, &devargs_list, next, tmp) {
        if (strcmp(d->bus->name, devargs->bus->name) == 0 && strcmp(d->name, devargs->name) == 0) {
...
void rte_devargs_dump(FILE *f) {
    ...
    TAILQ_FOREACH(devargs, &devargs_list, next) {
        fprintf(f, "  [%s]: %s %s\n", (devargs->bus ? devargs->bus->name : "??"), devargs->name, devargs->args);
    }
}
