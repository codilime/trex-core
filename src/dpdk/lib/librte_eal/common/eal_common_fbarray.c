...
static struct mem_area_head mem_area_tailq = TAILQ_HEAD_INITIALIZER(mem_area_tailq);
...
static int find_prev_n(const struct rte_fbarray *arr, unsigned int start, unsigned int n, bool used) {
    const struct used_mask *msk = get_used_mask(arr->data, arr->elt_sz, arr->len);
    ...
    ignore_msk = first_mod == MASK_ALIGN - 1 ? -1ULL : /* prevent overflow */
                     ~(-1ULL << (first_mod + 1));
...
static int find_prev(const struct rte_fbarray *arr, unsigned int start, bool used) {
    const struct used_mask *msk = get_used_mask(arr->data, arr->elt_sz, arr->len);
    unsigned int idx, first, first_mod;
    uint64_t ignore_msk;
...
static int find_rev_contig(const struct rte_fbarray *arr, unsigned int start, bool used) {
    const struct used_mask *msk = get_used_mask(arr->data, arr->elt_sz, arr->len);
    unsigned int idx, first, first_mod;
    unsigned int need_len, result = 0;
...
int __rte_experimental rte_fbarray_init(struct rte_fbarray *arr, const char *name, unsigned int len,
                                        unsigned int elt_sz) {
    ...
    if (internal_config.no_shconf) {
        ...
        void *new_data = mmap(data, mmap_len, PROT_READ | PROT_WRITE, MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS, fd, 0);
...
static int fbarray_find(struct rte_fbarray *arr, unsigned int start, bool next, bool used) {
    int ret = -1;
...
static int fbarray_find_n(struct rte_fbarray *arr, unsigned int start, unsigned int n, bool next, bool used) {
    int ret = -1;
...
static int fbarray_find_contig(struct rte_fbarray *arr, unsigned int start, bool next, bool used) {
    int ret = -1;
...
static int fbarray_find_biggest(struct rte_fbarray *arr, unsigned int start, bool used, bool rev) {
...
int __rte_experimental rte_fbarray_find_biggest_free(struct rte_fbarray *arr, unsigned int start) {
    return fbarray_find_biggest(arr, start, false, false);
}

int __rte_experimental rte_fbarray_find_biggest_used(struct rte_fbarray *arr, unsigned int start) {
    return fbarray_find_biggest(arr, start, true, false);
}

int __rte_experimental rte_fbarray_find_rev_biggest_free(struct rte_fbarray *arr, unsigned int start) {
    return fbarray_find_biggest(arr, start, false, true);
}

int __rte_experimental rte_fbarray_find_rev_biggest_used(struct rte_fbarray *arr, unsigned int start) {
