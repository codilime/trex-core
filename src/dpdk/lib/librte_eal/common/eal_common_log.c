...
static struct rte_eal_opt_loglevel_list opt_loglevel_list = TAILQ_HEAD_INITIALIZER(opt_loglevel_list);
...
void rte_log_set_global_level(uint32_t level) { rte_logs.level = (uint32_t)level; }
 ...
 */
static int rte_log_save_level(int priority, const char *regex, const char *pattern) {
...
int rte_log_save_regexp(const char *regex, int tmp) { return rte_log_save_level(tmp, regex, NULL); }
...
int rte_log_save_pattern(const char *pattern, int priority) { return rte_log_save_level(priority, NULL, pattern); }

/* get the current loglevel for the message being processed */
int rte_log_cur_msg_loglevel(void) { return RTE_PER_LCORE(log_cur_msg).loglevel; }
...
int rte_log_register(const char *name) {
    ...
    new_dynamic_types =
        realloc(rte_logs.dynamic_types, sizeof(struct rte_log_dynamic_type) * (rte_logs.dynamic_types_len + 1));
...
int __rte_experimental rte_log_register_type_and_pick_level(const char *name, uint32_t level_def) {
...
static const char *loglevel_to_string(uint32_t level) {
    switch (level) {
    case 0:
        return "disabled";
    case RTE_LOG_EMERG:
