...
static struct mem_event_callback_entry_list mem_event_callback_list = TAILQ_HEAD_INITIALIZER(mem_event_callback_list);
...
static struct mem_event_callback_entry *find_mem_event_callback(const char *name, void *arg) {
...
static struct mem_alloc_validator_entry *find_mem_alloc_validator(const char *name, int socket_id) {
...
bool eal_memalloc_is_contig(const struct rte_memseg_list *msl, void *start, size_t len) {
    ...
    if (!rte_eal_has_hugepages()) {
        ...
        for (cur_seg = start_seg + 1; cur_seg < end_seg; cur_seg++, expected += pgsz) {
...
int eal_memalloc_mem_event_callback_register(const char *name, rte_mem_event_callback_t clb, void *arg) {
...
void eal_memalloc_mem_event_notify(enum rte_mem_event event, const void *start, size_t len) {
    struct mem_event_callback_entry *entry;
...
int eal_memalloc_mem_alloc_validator_register(const char *name, rte_mem_alloc_validator_t clb, int socket_id,
                                              size_t limit) {
...
int eal_memalloc_mem_alloc_validator_unregister(const char *name, int socket_id) {
    struct mem_alloc_validator_entry *entry;
    int ret, len;
