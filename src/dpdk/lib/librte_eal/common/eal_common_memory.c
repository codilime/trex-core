...
void *eal_get_virtual_area(void *requested_addr, size_t *size, size_t page_sz, int flags, int mmap_flags) {
    ...
    if (next_baseaddr == NULL && internal_config.base_virtaddr != 0 && rte_eal_process_type() == RTE_PROC_PRIMARY)
     ...
     */
    no_align = (requested_addr != NULL && requested_addr == RTE_PTR_ALIGN(requested_addr, page_sz) && !addr_is_hint) ||
    ...
    do {
        ...
        if (mapped_addr != MAP_FAILED && addr_is_hint && mapped_addr != requested_addr) {
            try
                ++;
    ...
    } while ((allow_shrink || addr_is_hint) && mapped_addr == MAP_FAILED && *size > 0);
...
static struct rte_memseg *virt2memseg(const void *addr, const struct rte_memseg_list *msl) {
...
__rte_experimental struct rte_memseg_list *rte_mem_virt2memseg_list(const void *addr) { return virt2memseg_list(addr); }

struct virtiova {
    rte_iova_t iova;
    void *virt;
};
static int find_virt(const struct rte_memseg_list *msl __rte_unused, const struct rte_memseg *ms, void *arg) {
...
static int find_virt_legacy(const struct rte_memseg_list *msl __rte_unused, const struct rte_memseg *ms, size_t len,
                            void *arg) {
    struct virtiova *vi = arg;
...
__rte_experimental struct rte_memseg *rte_mem_virt2memseg(const void *addr, const struct rte_memseg_list *msl) {
...
static int dump_memseg(const struct rte_memseg_list *msl, const struct rte_memseg *ms, void *arg) {
    struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
    ...
    fprintf(f,
            ...
            "nrank:%" PRIx32 " fd:%i\n",
            msl_idx, ms_idx, ms->iova, ms->len, ms->addr, ms->socket_id, ms->hugepage_sz, ms->nchannel, ms->nrank, fd);
...
int __rte_experimental rte_mem_alloc_validator_register(const char *name, rte_mem_alloc_validator_t clb, int socket_id,
                                                        size_t limit) {
    /* FreeBSD boots with legacy mem enabled by default */
    if (internal_config.legacy_mem) {
...
static int check_iova(const struct rte_memseg_list *msl __rte_unused, const struct rte_memseg *ms, void *arg) {
    uint64_t *mask = arg;
    rte_iova_t iova;
...
static int __rte_experimental check_dma_mask(uint8_t maskbits, bool thread_unsafe) {
    struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
    uint64_t mask;
    int ret;
     ...
     */
    mcfg->dma_maskbits = mcfg->dma_maskbits == 0 ? maskbits : RTE_MIN(mcfg->dma_maskbits, maskbits);

    return 0;
}

int __rte_experimental rte_mem_check_dma_mask(uint8_t maskbits) { return check_dma_mask(maskbits, false); }

int __rte_experimental rte_mem_check_dma_mask_thread_unsafe(uint8_t maskbits) { return check_dma_mask(maskbits, true); }
...
unsigned rte_memory_get_nchannel(void) { return rte_eal_get_configuration()->mem_config->nchannel; }

/* return the number of memory rank */
unsigned rte_memory_get_nrank(void) { return rte_eal_get_configuration()->mem_config->nrank; }
...
int __rte_experimental rte_extmem_register(void *va_addr, size_t len, rte_iova_t iova_addrs[], unsigned int n_pages,
                                           size_t page_sz) {
    struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
    unsigned int socket_id, n;
    int ret = 0;

    if (va_addr == NULL || page_sz == 0 || len == 0 || !rte_is_power_of_2(page_sz) || RTE_ALIGN(len, page_sz) != len ||
        ((len / page_sz) != n_pages && iova_addrs != NULL) || !rte_is_aligned(va_addr, page_sz)) {
        rte_errno = EINVAL;
        return -1;
    }
    ...
    n = len / page_sz;
    if (malloc_heap_create_external_seg(va_addr, iova_addrs, n, page_sz, "extmem", socket_id) == NULL) {
        ret = -1;
        goto unlock;
    }
...
int __rte_experimental rte_extmem_attach(void *va_addr, size_t len) { return sync_memory(va_addr, len, true); }

int __rte_experimental rte_extmem_detach(void *va_addr, size_t len) { return sync_memory(va_addr, len, false); }
...
int rte_eal_memory_init(void) {
    ...
    retval = rte_eal_process_type() == RTE_PROC_PRIMARY ? rte_eal_hugepage_init() : rte_eal_hugepage_attach();
    if (retval < 0)
        goto fail;
