...
static inline const struct rte_memzone *memzone_lookup_thread_unsafe(const char *name) {
...
static const struct rte_memzone *memzone_reserve_aligned_thread_unsafe(const char *name, size_t len, int socket_id,
    ...
    mz->addr = mz_addr;
    mz->len = requested_len == 0 ? elem->size - elem->pad - MALLOC_ELEM_OVERHEAD : requested_len;
...
static const struct rte_memzone *rte_memzone_reserve_thread_safe(const char *name, size_t len, int socket_id,
                                                                 unsigned int flags, unsigned int align,
                                                                 unsigned int bound) {
    struct rte_mem_config *mcfg;
    ...
    mz = memzone_reserve_aligned_thread_unsafe(name, len, socket_id, flags, align, bound);
 ...
 * return NULL.
 */
const struct rte_memzone *rte_memzone_reserve_bounded(const char *name, size_t len, int socket_id, unsigned flags,
                                                      unsigned align, unsigned bound) {
    return rte_memzone_reserve_thread_safe(name, len, socket_id, flags, align, bound);
}
 ...
 */
const struct rte_memzone *rte_memzone_reserve_aligned(const char *name, size_t len, int socket_id, unsigned flags,
                                                      unsigned align) {
...
int rte_eal_memzone_init(void) {
    ...
    if (rte_eal_process_type() == RTE_PROC_PRIMARY &&
        ...
        ret = -1;
    } else if (rte_eal_process_type() == RTE_PROC_SECONDARY && rte_fbarray_attach(&mcfg->memzones)) {
