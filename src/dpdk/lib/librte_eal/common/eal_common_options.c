...
const char eal_short_options[] = "b:" /* pci-blacklist */
                                 "c:" /* coremask */
                                 "s:" /* service coremask */
                                 "d:" /* driver */
                                 "h"  /* help */
...
const struct option eal_long_options[] = {{OPT_BASE_VIRTADDR, 1, NULL, OPT_BASE_VIRTADDR_NUM},
...
static struct shared_driver_list solib_list = TAILQ_HEAD_INITIALIZER(solib_list);
 ...
 */
static const char dpdk_solib_path[] __attribute__((used)) = "DPDK_PLUGIN_PATH=" RTE_EAL_PMD_PATH;
...
static struct device_option_list devopt_list = TAILQ_HEAD_INITIALIZER(devopt_list);
...
void eal_reset_internal_config(struct internal_config *internal_cfg) {
    ...
    for (i = 0; i < MAX_HUGEPAGE_SIZES; i++) {
        memset(&internal_cfg->hugepage_info[i], 0, sizeof(internal_cfg->hugepage_info[0]));
...
int eal_plugins_init(void) {
    ...
    if (*default_solib_dir != '\0' && stat(default_solib_dir, &sb) == 0 && S_ISDIR(sb.st_mode))
    ...
    for (i = i - 1; i >= 0 && idx < RTE_MAX_LCORE; i--) {
        ...
        for (j = 0; j < BITS_PER_HEX && idx < RTE_MAX_LCORE; j++, idx++) {
            if ((1 << j) & val) {
                ...
                uint32_t lcore = idx;
                if (master_lcore_parsed && cfg->master_lcore == lcore) {
                    RTE_LOG(ERR, EAL, "lcore %u is master lcore, cannot use as service core\n", idx);
                    return -1;
                }
        ...
        if (*end == '-') {
            ...
            for (idx = min; idx <= max; idx++) {
                if (cfg->lcore_role[idx] != ROLE_SERVICE) {
                    /* handle master lcore already parsed */
                    uint32_t lcore = idx;
                    if (cfg->master_lcore == lcore && master_lcore_parsed) {
                        RTE_LOG(ERR, EAL, "Error: lcore %u is master lcore, cannot use as service core\n", idx);
                        return -1;
                    }
                    if (cfg->lcore_role[idx] == ROLE_RTE)
                        taken_lcore_count++;
 ...
 */
static int eal_parse_set(const char *input, uint16_t set[], unsigned num) {
    unsigned idx;
...
static int convert_to_cpuset(rte_cpuset_t *cpusetp, uint16_t *set, unsigned num) {
    unsigned idx;
...
static int eal_parse_lcores(const char *lcores) {
    ...
    do {
        ...
        if (*lcores != '@' && 0 > convert_to_cpuset(&cpuset, set, RTE_DIM(set)))
            goto err;
...
static int eal_parse_syslog(const char *facility, struct internal_config *conf) {
    int i;
    static const struct {
        const char *name;
        int value;
    } map[] = {{"auth", LOG_AUTH},     {"cron", LOG_CRON},     {"daemon", LOG_DAEMON}, {"ftp", LOG_FTP},
               ...
               {"local5", LOG_LOCAL5}, {"local6", LOG_LOCAL6}, {"local7", LOG_LOCAL7}, {NULL, 0}};
...
static int eal_parse_log_priority(const char *level) {
    static const char *const levels[] = {
        [RTE_LOG_EMERG] = "emergency", [RTE_LOG_ALERT] = "alert",     [RTE_LOG_CRIT] = "critical",
        [RTE_LOG_ERR] = "error",       [RTE_LOG_WARNING] = "warning", [RTE_LOG_NOTICE] = "notice",
        [RTE_LOG_INFO] = "info",       [RTE_LOG_DEBUG] = "debug",
    };
...
int eal_parse_common_option(int opt, const char *optarg, struct internal_config *conf) {
    ...
    case 'b':
        if (w_used)
            goto bw_used;
        if (eal_option_device_add(RTE_DEVTYPE_BLACKLISTED_PCI, optarg) < 0) {
            return -1;
        }
        b_used = 1;
        break;
    ...
    case 'w':
        if (b_used)
            goto bw_used;
        if (eal_option_device_add(RTE_DEVTYPE_WHITELISTED_PCI, optarg) < 0) {
            return -1;
        }
        w_used = 1;
        break;
    ...
    case 'c': {
        ...
        if (eal_service_cores_parsed())
            RTE_LOG(WARNING, EAL, "Service cores parsed before dataplane cores. Please ensure -c is before -s or -S\n");
        ...
        if (update_lcore_config(lcore_indexes) < 0) {
            ...
            RTE_LOG(ERR, EAL, "invalid coremask, please check specified cores are part of %s\n", available);
            free(available);
            return -1;
        }
        ...
        if (core_parsed) {
            RTE_LOG(ERR, EAL, "Option -c is ignored, because (%s) is set!\n",
                    (core_parsed == LCORE_OPT_LST) ? "-l" : (core_parsed == LCORE_OPT_MAP) ? "--lcore" : "-c");
            return -1;
        }
    ...
    case 'l': {
        ...
        if (eal_service_cores_parsed())
            RTE_LOG(WARNING, EAL, "Service cores parsed before dataplane cores. Please ensure -l is before -s or -S\n");
        ...
        if (update_lcore_config(lcore_indexes) < 0) {
            ...
            RTE_LOG(ERR, EAL, "invalid core list, please check specified cores are part of %s\n", available);
            free(available);
            return -1;
        }
        ...
        if (core_parsed) {
            RTE_LOG(ERR, EAL, "Option -l is ignored, because (%s) is set!\n",
                    (core_parsed == LCORE_OPT_MSK) ? "-c" : (core_parsed == LCORE_OPT_MAP) ? "--lcore" : "-l");
            return -1;
        }
    ...
    case OPT_LCORES_NUM:
        ...
        if (core_parsed) {
            RTE_LOG(ERR, EAL, "Option --lcore is ignored, because (%s) is set!\n",
                    (core_parsed == LCORE_OPT_LST) ? "-l" : (core_parsed == LCORE_OPT_MSK) ? "-c" : "--lcore");
            return -1;
        }
...
static void eal_auto_detect_cores(struct rte_config *cfg) {
    ...
    for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
        if (cfg->lcore_role[lcore_id] == ROLE_RTE && !CPU_ISSET(lcore_id, &affinity_set)) {
...
static void compute_ctrl_threads_cpuset(struct internal_config *internal_cfg) {
    ...
    for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
        if (eal_cpu_detected(lcore_id) && rte_lcore_has_role(lcore_id, ROLE_OFF)) {
            CPU_SET(lcore_id, cpuset);
        }
    }

    if (pthread_getaffinity_np(pthread_self(), sizeof(rte_cpuset_t), &default_set))
...
int eal_check_common_options(struct internal_config *internal_cfg) {
    ...
    if (internal_cfg->hugefile_prefix != NULL && strlen(internal_cfg->hugefile_prefix) < 1) {
        RTE_LOG(ERR, EAL, "Invalid length of --" OPT_FILE_PREFIX " option\n");
        return -1;
    }
    if (internal_cfg->hugepage_dir != NULL && strlen(internal_cfg->hugepage_dir) < 1) {
        RTE_LOG(ERR, EAL, "Invalid length of --" OPT_HUGE_DIR " option\n");
        return -1;
    }
    if (internal_cfg->user_mbuf_pool_ops_name != NULL && strlen(internal_cfg->user_mbuf_pool_ops_name) < 1) {
    ...
    if (internal_cfg->no_hugetlbfs && internal_cfg->hugepage_unlink && !internal_cfg->in_memory) {
        RTE_LOG(ERR, EAL,
    ...
    if (internal_config.force_socket_limits && internal_config.legacy_mem) {
        RTE_LOG(ERR, EAL, "Option --" OPT_SOCKET_LIMIT " is only supported in non-legacy memory mode\n");
    }
    if (internal_cfg->single_file_segments && internal_cfg->hugepage_unlink && !internal_cfg->in_memory) {
        RTE_LOG(ERR, EAL,
