...
static struct action_entry_list action_entry_list = TAILQ_HEAD_INITIALIZER(action_entry_list);
...
static struct {
    ...
    .requests = TAILQ_HEAD_INITIALIZER(pending_requests.requests), .lock = PTHREAD_MUTEX_INITIALIZER,
...
static struct pending_request *find_pending_request(const char *dst, const char *act_name) {
...
static enum async_action process_async_request(struct pending_request *sr, const struct timespec *now) {
    struct async_request_param *param;
...
static int open_socket_fd(void) {
    ...
    if (rte_eal_process_type() == RTE_PROC_SECONDARY)
        snprintf(peer_name, sizeof(peer_name), "%d_%" PRIx64, getpid(), rte_rdtsc());
...
static void close_socket_fd(void) {
    char path[PATH_MAX];
...
int rte_mp_channel_init(void) {
    ...
    if (rte_ctrl_thread_create(&mp_handle_tid, "rte_mp_handle", NULL, mp_handle, NULL) < 0) {
...
void rte_mp_channel_cleanup(void) { close_socket_fd(); }
...
static int mp_request_async(const char *dst, struct rte_mp_msg *req, struct async_request_param *param,
    ...
    if (rte_eal_alarm_set(ts->tv_sec * 1000000 + ts->tv_nsec / 1000, async_reply_handle, pending_req) < 0) {
...
static int mp_request_sync(const char *dst, struct rte_mp_msg *req, struct rte_mp_reply *reply,
                           const struct timespec *ts) {
    int ret;
    ...
    do {
        ret = pthread_cond_timedwait(&pending_req.sync.cond, &pending_requests.lock, ts);
...
int __rte_experimental rte_mp_request_sync(struct rte_mp_msg *req, struct rte_mp_reply *reply,
    ...
    end.tv_sec = now.tv_sec + ts->tv_sec + (now.tv_usec * 1000 + ts->tv_nsec) / 1000000000;
...
int __rte_experimental rte_mp_request_async(struct rte_mp_msg *req, const struct timespec *ts,
                                            rte_mp_async_reply_t clb) {
