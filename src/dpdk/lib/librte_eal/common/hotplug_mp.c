...
static int send_response_to_secondary(const struct eal_dev_mp_req *req, int result, const void *peer) {
...
int eal_dev_hotplug_request_to_secondary(struct eal_dev_mp_req *req) {
    ...
    for (i = 0; i < mp_reply.nb_received; i++) {
        struct eal_dev_mp_req *resp = (struct eal_dev_mp_req *)mp_reply.msgs[i].param;
        if (resp->result != 0) {
...
int rte_mp_dev_hotplug_init(void) {
    ...
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
        ret = rte_mp_action_register(EAL_DEV_MP_ACTION_REQUEST, handle_secondary_request);
        if (ret != 0) {
            RTE_LOG(ERR, EAL, "Couldn't register '%s' action\n", EAL_DEV_MP_ACTION_REQUEST);
            return ret;
        }
    } else {
        ret = rte_mp_action_register(EAL_DEV_MP_ACTION_REQUEST, handle_primary_request);
        if (ret != 0) {
            RTE_LOG(ERR, EAL, "Couldn't register '%s' action\n", EAL_DEV_MP_ACTION_REQUEST);
            return ret;
        }
