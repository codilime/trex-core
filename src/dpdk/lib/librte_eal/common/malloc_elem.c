 ...
 */
void malloc_elem_init(struct malloc_elem *elem, struct malloc_heap *heap, struct rte_memseg_list *msl, size_t size,
 ...
 */
static bool elem_check_phys_contig(const struct rte_memseg_list *msl, void *start, size_t size) {
 ...
 * fit, return NULL.
 */
static void *elem_start_pt(struct malloc_elem *elem, size_t size, unsigned align, size_t bound, bool contig) {
    ...
    while (elem_size >= size) {
        ...
        if (contig) {
             ...
             */
            if (!elem_check_phys_contig(elem->msl, (void *)new_data_start, new_data_size)) {
                elem_size -= align;
                continue;
            }
        }
 ...
 */
int malloc_elem_can_hold(struct malloc_elem *elem, size_t size, unsigned align, size_t bound, bool contig) {
...
static void split_elem(struct malloc_elem *elem, struct malloc_elem *split_pt) {
    ...
    malloc_elem_init(split_pt, elem->heap, elem->msl, new_elem_size, elem->orig_elem, elem->orig_size);
    split_pt->prev = elem;
...
static int next_elem_is_adjacent(struct malloc_elem *elem) {
    return elem->next == RTE_PTR_ADD(elem, elem->size) && elem->next->msl == elem->msl &&
           (!internal_config.match_allocations || elem->orig_elem == elem->next->orig_elem);
}

static int prev_elem_is_adjacent(struct malloc_elem *elem) {
    return elem == RTE_PTR_ADD(elem->prev, elem->prev->size) && elem->prev->msl == elem->msl &&
           (!internal_config.match_allocations || elem->orig_elem == elem->prev->orig_elem);
}
 ...
 */
void malloc_elem_free_list_remove(struct malloc_elem *elem) { LIST_REMOVE(elem, free_list); }
 ...
 */
struct malloc_elem *malloc_elem_alloc(struct malloc_elem *elem, size_t size, unsigned align, size_t bound,
                                      bool contig) {
    struct malloc_elem *new_elem = elem_start_pt(elem, size, align, bound, contig);
 ...
 */
static inline void join_elem(struct malloc_elem *elem1, struct malloc_elem *elem2) {
...
struct malloc_elem *malloc_elem_join_adjacent_free(struct malloc_elem *elem) {
     ...
     */
    if (elem->next != NULL && elem->next->state == ELEM_FREE && next_elem_is_adjacent(elem)) {
        void *erase;
        size_t erase_len;
     ...
     * with it, need to remove from free list.
     */
    if (elem->prev != NULL && elem->prev->state == ELEM_FREE && prev_elem_is_adjacent(elem)) {
...
int malloc_elem_resize(struct malloc_elem *elem, size_t size) {
    ...
    if (!elem->next || elem->next->state != ELEM_FREE || !next_elem_is_adjacent(elem))
        return -1;
...
void malloc_elem_dump(const struct malloc_elem *elem, FILE *f) {
    fprintf(f, "Malloc element at %p (%s)\n", elem, elem_state_to_str(elem->state));
