 ...
 */
static struct malloc_elem *malloc_heap_add_memory(struct malloc_heap *heap, struct rte_memseg_list *msl, void *start,
                                                  size_t len) {
...
static int malloc_add_seg(const struct rte_memseg_list *msl, const struct rte_memseg *ms, size_t len,
 ...
 */
static struct malloc_elem *find_suitable_element(struct malloc_heap *heap, size_t size, unsigned int flags,
 ...
 * Returns null on failure, or pointer to element on success.
 */
static struct malloc_elem *find_biggest_element(struct malloc_heap *heap, size_t *size, unsigned int flags,
    ...
    for (idx = 0; idx < RTE_HEAP_NUM_FREELISTS; idx++) {
        for (elem = LIST_FIRST(&heap->free_head[idx]); !!elem; elem = LIST_NEXT(elem, free_list)) {
            size_t cur_size;
            if ((flags & RTE_MEMZONE_SIZE_HINT_ONLY) == 0 && !check_hugepage_sz(flags, elem->msl->page_sz))
                continue;
            if (contig) {
                cur_size = malloc_elem_find_max_iova_contig(elem, align);
            } else {
                void *data_start = RTE_PTR_ADD(elem, MALLOC_ELEM_HEADER_LEN);
                void *data_end = RTE_PTR_ADD(elem, elem->size - MALLOC_ELEM_TRAILER_LEN);
 ...
 */
static void *heap_alloc(struct malloc_heap *heap, const char *type __rte_unused, size_t size, unsigned int flags,
                        size_t align, size_t bound, bool contig) {
...
static void *heap_alloc_biggest(struct malloc_heap *heap, const char *type __rte_unused, unsigned int flags,
                                size_t align, bool contig) {
    struct malloc_elem *elem;
    size_t size;
...
void rollback_expand_heap(struct rte_memseg **ms, int n_segs, struct malloc_elem *elem, void *map_addr,
                          size_t map_len) {
    if (elem != NULL) {
...
/* this function is exposed in malloc_mp.h */
struct malloc_elem *alloc_pages_on_heap(struct malloc_heap *heap, uint64_t pg_sz, size_t elt_size, int socket,
    ...
    if (eal_memalloc_mem_alloc_validate(socket, heap->total_size + alloc_sz) < 0) {
...
static int try_expand_heap_primary(struct malloc_heap *heap, uint64_t pg_sz, size_t elt_size, int socket,
                                   unsigned int flags, size_t align, size_t bound, bool contig) {
    struct malloc_elem *elem;
    ...
    elem = alloc_pages_on_heap(heap, pg_sz, elt_size, socket, flags, align, bound, contig, ms, n_segs);
...
static int try_expand_heap_secondary(struct malloc_heap *heap, uint64_t pg_sz, size_t elt_size, int socket,
                                     unsigned int flags, size_t align, size_t bound, bool contig) {
...
static int try_expand_heap(struct malloc_heap *heap, uint64_t pg_sz, size_t elt_size, int socket, unsigned int flags,
                           size_t align, size_t bound, bool contig) {
    struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
    int ret;
    ...
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
        ret = try_expand_heap_primary(heap, pg_sz, elt_size, socket, flags, align, bound, contig);
    } else {
        ret = try_expand_heap_secondary(heap, pg_sz, elt_size, socket, flags, align, bound, contig);
    }
...
static int alloc_more_mem_on_socket(struct malloc_heap *heap, size_t size, int socket, unsigned int flags, size_t align,
    ...
    qsort(requested_msls, n_requested_msls, sizeof(requested_msls[0]), compare_pagesz);
    ...
    for (i = 0; i < n_requested_pg_sz; i++) {
         ...
         */
        if (!try_expand_heap(heap, pg_sz, size, socket, size_flags, align, bound, contig))
            return 0;
    }
    if (n_other_pg_sz == 0)
        return -1;
    ...
    for (i = 0; i < n_other_pg_sz; i++) {
        ...
        if (!try_expand_heap(heap, pg_sz, size, socket, flags, align, bound, contig))
            return 0;
    }
    return -1;
}
...
static void *malloc_heap_alloc_on_heap_id(const char *type, size_t size, unsigned int heap_id, unsigned int flags,
                                          size_t align, size_t bound, bool contig) {
    struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
    ...
    if (!alloc_more_mem_on_socket(heap, size, socket_id, flags, align, bound, contig)) {
        ret = heap_alloc(heap, type, size, flags, align, bound, contig);
...
void *malloc_heap_alloc(const char *type, size_t size, int socket_arg, unsigned int flags, size_t align, size_t bound,
                        bool contig) {
    ...
    ret = malloc_heap_alloc_on_heap_id(type, size, heap_id, flags, align, bound, contig);
    ...
    for (i = 0; i < (int)rte_socket_count(); i++) {
        if (i == heap_id)
            continue;
        ret = malloc_heap_alloc_on_heap_id(type, size, i, flags, align, bound, contig);
        if (ret != NULL)
            return ret;
    }
    return NULL;
...
static void *heap_alloc_biggest_on_heap_id(const char *type, unsigned int heap_id, unsigned int flags, size_t align,
                                           bool contig) {
    struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
    struct malloc_heap *heap = &mcfg->malloc_heaps[heap_id];
    void *ret;
...
void *malloc_heap_alloc_biggest(const char *type, int socket_arg, unsigned int flags, size_t align, bool contig) {
...
int malloc_heap_free(struct malloc_elem *elem) {
    ...
    if (before_space != 0 && before_space < MALLOC_ELEM_OVERHEAD + MIN_DATA_SIZE) {
    ...
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
        /* notify user about changes in memory map */
        eal_memalloc_mem_event_notify(RTE_MEM_EVENT_FREE, aligned_start, aligned_len);
 ...
 */
int malloc_heap_get_stats(struct malloc_heap *heap, struct rte_malloc_socket_stats *socket_stats) {
    size_t idx;
    struct malloc_elem *elem;
...
struct rte_memseg_list *malloc_heap_create_external_seg(void *va_addr, rte_iova_t iova_addrs[], unsigned int n_pages,
                                                        size_t page_sz, const char *seg_name, unsigned int socket_id) {
    struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
    ...
    snprintf(fbarray_name, sizeof(fbarray_name) - 1, "%s_%p", seg_name, va_addr);

    /* create the backing fbarray */
    if (rte_fbarray_init(&msl->memseg_arr, fbarray_name, n_pages, sizeof(struct rte_memseg)) < 0) {
...
struct rte_memseg_list *malloc_heap_find_external_seg(void *va_addr, size_t len) {
...
int malloc_heap_add_external_memory(struct malloc_heap *heap, struct rte_memseg_list *msl) {
...
int malloc_heap_remove_external_memory(struct malloc_heap *heap, void *va_addr, size_t len) {
...
int rte_eal_malloc_heap_init(void) {
    ...
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
        ...
        for (i = 0; i < rte_socket_count(); i++) {
            ...
            snprintf(heap_name, sizeof(heap_name) - 1, "socket_%i", socket_id);
            strlcpy(heap->name, heap_name, RTE_HEAP_NAME_MAX_LEN);
