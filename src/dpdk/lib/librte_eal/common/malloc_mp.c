...
static int handle_sync_response(const struct rte_mp_msg *request, const struct rte_mp_reply *reply);
...
} mp_request_list = {.list = TAILQ_HEAD_INITIALIZER(mp_request_list.list), .lock = PTHREAD_MUTEX_INITIALIZER};
...
static int handle_alloc_request(const struct malloc_mp_req *m, struct mp_request *req) {
    ...
    alloc_sz = RTE_ALIGN_CEIL(ar->align + ar->elt_size + MALLOC_ELEM_TRAILER_LEN, ar->page_sz);
    ...
    elem = alloc_pages_on_heap(heap, ar->page_sz, ar->elt_size, ar->socket, ar->flags, ar->align, ar->bound, ar->contig,
                               ms, n_segs);
...
static int handle_request(const struct rte_mp_msg *msg, const void *peer __rte_unused) {
    ...
    if (m->t == REQ_TYPE_ALLOC) {
        ...
        eal_memalloc_mem_event_notify(RTE_MEM_EVENT_FREE, m->free_req.addr, m->free_req.len);

        ret = malloc_heap_free_pages(m->free_req.addr, m->free_req.len);
    } else {
...
static int handle_sync_response(const struct rte_mp_msg *request, const struct rte_mp_reply *reply) {
    ...
    struct mp_request *entry;
    const struct malloc_mp_req *mpreq = (const struct malloc_mp_req *)request->param;
    int i;
    ...
    if (entry->user_req.t == REQ_TYPE_FREE) {
        ...
        free(entry);
    } else if (entry->user_req.t == REQ_TYPE_ALLOC && result == REQ_RESULT_SUCCESS) {
        ...
        eal_memalloc_mem_event_notify(RTE_MEM_EVENT_FREE, state->map_addr, state->map_len);

        rollback_expand_heap(state->ms, state->ms_len, state->elem, state->map_addr, state->map_len);
...
static int handle_rollback_response(const struct rte_mp_msg *request, const struct rte_mp_reply *reply __rte_unused) {
    struct rte_mp_msg msg;
    struct malloc_mp_req *resp = (struct malloc_mp_req *)msg.param;
    const struct malloc_mp_req *mpreq = (const struct malloc_mp_req *)request->param;
    struct mp_request *entry;
