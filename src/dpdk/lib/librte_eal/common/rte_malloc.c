...
void *rte_malloc_socket(const char *type, size_t size, unsigned int align, int socket_arg) {
     ...
     */
    if (rte_malloc_heap_socket_is_external(socket_arg) != 1 && !rte_eal_has_hugepages())
        socket_arg = SOCKET_ID_ANY;

    return malloc_heap_alloc(type, size, socket_arg, 0, align == 0 ? 1 : align, 0, false);
}
...
void *rte_realloc_socket(void *ptr, size_t size, unsigned int align, int socket) {
     ...
     */
    if ((socket == SOCKET_ID_ANY || (unsigned int)socket == elem->heap->socket_id) &&
 ...
 */
int rte_malloc_set_limit(__rte_unused const char *type, __rte_unused size_t max) { return 0; }
...
int rte_malloc_heap_memory_add(const char *heap_name, void *va_addr, size_t len, rte_iova_t iova_addrs[],
    ...
    if (heap_name == NULL || va_addr == NULL || page_sz == 0 || !rte_is_power_of_2(page_sz) ||
        RTE_ALIGN(len, page_sz) != len || !rte_is_aligned(va_addr, page_sz) ||
        ((len / page_sz) != n_pages && iova_addrs != NULL) || strnlen(heap_name, RTE_HEAP_NAME_MAX_LEN) == 0 ||
    ...
    msl = malloc_heap_create_external_seg(va_addr, iova_addrs, n, page_sz, heap_name, heap->socket_id);
    if (msl == NULL) {
        ret = -1;
        goto unlock;
    }
...
int rte_malloc_heap_memory_remove(const char *heap_name, void *va_addr, size_t len) {
    ...
    if (heap_name == NULL || va_addr == NULL || len == 0 || strnlen(heap_name, RTE_HEAP_NAME_MAX_LEN) == 0 ||
        strnlen(heap_name, RTE_HEAP_NAME_MAX_LEN) == RTE_HEAP_NAME_MAX_LEN) {
        rte_errno = EINVAL;
        return -1;
    }
...
static int sync_memory(const char *heap_name, void *va_addr, size_t len, bool attach) {
    struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
    struct malloc_heap *heap = NULL;
    struct rte_memseg_list *msl;
    int ret;
