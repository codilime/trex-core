...
int32_t rte_service_init(void) {
    ...
    rte_services =
        rte_calloc("rte_services", RTE_SERVICE_NUM_MAX, sizeof(struct rte_service_spec_impl), RTE_CACHE_LINE_SIZE);
    if (!rte_services) {
        printf("error allocating rte services array\n");
        goto fail_mem;
    }

    lcore_states = rte_calloc("rte_service_core_states", RTE_MAX_LCORE, sizeof(struct core_state), RTE_CACHE_LINE_SIZE);
    if (!lcore_states) {
 ...
 */
static inline int service_valid(uint32_t id) { return !!(rte_services[id].internal_flags & SERVICE_F_REGISTERED); }
...
int32_t rte_service_component_register(const struct rte_service_spec *spec, uint32_t *id_ptr) {
    uint32_t i;
    int32_t free_slot = -1;
...
int32_t rte_service_runstate_get(uint32_t id) {
    ...
    return (s->app_runstate == RUNSTATE_RUNNING) && (s->comp_runstate == RUNSTATE_RUNNING) &&
           (check_disabled | lcore_mapped);
}

static inline void rte_service_runner_do_callback(struct rte_service_spec_impl *s, struct core_state *cs,
...
static inline int32_t service_run(uint32_t i, int lcore, struct core_state *cs, uint64_t service_mask) {
    if (!service_valid(i))
        return -EINVAL;
    struct rte_service_spec_impl *s = &rte_services[i];
    if (s->comp_runstate != RUNSTATE_RUNNING || s->app_runstate != RUNSTATE_RUNNING ||
     ...
     */
    const int use_atomics = (service_mt_safe(s) == 0) && (rte_atomic32_read(&s->num_mapped_cores) > 1);
    if (use_atomics) {
...
int32_t __rte_experimental rte_service_may_be_active(uint32_t id) {
...
int32_t rte_service_run_iter_on_app_lcore(uint32_t id, uint32_t serialize_mt_unsafe) {
    ...
    if (service_mt_safe(s) == 0 && rte_atomic32_read(&s->num_mapped_cores) > 1) {
        if (serialize_mt_unsafe)
...
static int32_t rte_service_runner_func(void *arg) {
    RTE_SET_USED(arg);
    uint32_t i;
...
static int32_t service_update(struct rte_service_spec *service, uint32_t lcore, uint32_t *set, uint32_t *enabled) {
    uint32_t i;
    int32_t sid = -1;

    for (i = 0; i < RTE_SERVICE_NUM_MAX; i++) {
        if ((struct rte_service_spec *)&rte_services[i] == service && service_valid(i)) {
            sid = i;
            break;
        }
    }
...
int32_t rte_service_map_lcore_set(uint32_t id, uint32_t lcore, uint32_t enabled) {
    struct rte_service_spec_impl *s;
    SERVICE_VALID_GET_OR_ERR_RET(id, s, -EINVAL);
...
int32_t __rte_experimental rte_service_lcore_attr_get(uint32_t lcore, uint32_t attr_id, uint64_t *attr_value) {
...
static void rte_service_dump_one(FILE *f, struct rte_service_spec_impl *s, uint64_t all_cycles, uint32_t reset) {
    ...
    fprintf(f, "  %s: stats %d\tcalls %" PRIu64 "\tcycles %" PRIu64 "\tavg: %" PRIu64 "\n", s->spec.name,
            service_stats_enabled(s), s->calls, s->cycles_spent, s->cycles_spent / calls);
}
...
int32_t __rte_experimental rte_service_lcore_attr_reset_all(uint32_t lcore) {
    struct core_state *cs;
...
static void service_dump_calls_per_lcore(FILE *f, uint32_t lcore, uint32_t reset) {
    uint32_t i;
    struct core_state *cs = &lcore_states[lcore];
