...
static inline int timespec_cmp(const struct timespec *now, const struct timespec *at) {
...
static int unregister_current_callback(void) {
    ...
    if (!LIST_EMPTY(&alarm_list)) {
        ...
        do {
            ret = rte_intr_callback_unregister(&intr_handle, eal_alarm_callback, &ap->time);
        } while (ret == -EAGAIN);
    }
...
static int register_first_callback(void) {
    ...
    if (!LIST_EMPTY(&alarm_list)) {
        ...
        ret = rte_intr_callback_register(&intr_handle, eal_alarm_callback, &ap->time);
    }
    return ret;
}
...
int rte_eal_alarm_cancel(rte_eal_alarm_callback cb_fn, void *cb_arg) {
    ...
    do {
        ...
        LIST_FOREACH(ap, &alarm_list, next) {
            ...
            if (cb_fn == ap->cb_fn && (cb_arg == (void *)-1 || cb_arg == ap->cb_arg)) {
                if (ap->executing == 0) {
                    LIST_REMOVE(ap, next);
                    free(ap);
                    count++;
