...
rte_iova_t rte_mem_virt2iova(const void *virtaddr) { return rte_mem_virt2phy(virtaddr); }
...
int rte_eal_hugepage_init(void) {
    ...
    if (internal_config.no_hugetlbfs) {
        ...
        addr = mmap(NULL, internal_config.memory, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
        if (addr == MAP_FAILED) {
    ...
    for (i = 0; i < internal_config.num_hugepage_sizes; i++) {
        ...
        mem_needed = RTE_ALIGN_CEIL(internal_config.memory - total_mem, page_sz);
        ...
        for (j = 0; j < n_pages; j++) {
             ...
             * the previous one.
             */
            snprintf(physaddr_str, sizeof(physaddr_str), "hw.contigmem.physaddr.%d", j);
             ...
             */
            addr = mmap(addr, page_sz, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, hpi->lock_descriptor,
                        j * EAL_PAGE_SIZE);
            if (addr == MAP_FAILED) {
            ...
            RTE_LOG(INFO, EAL, "Mapped memory segment %u @ %p: physaddr:0x%" PRIx64 ", len %zu\n", seg_idx++, addr,
                    physaddr, page_sz);
...
static int attach_segment(const struct rte_memseg_list *msl, const struct rte_memseg *ms, void *arg) {
    ...
    addr = mmap(ms->addr, ms->len, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, wa->fd_hugepage,
...
static int memseg_primary_init(void) {
    ...
    for (hpi_idx = 0; hpi_idx < (int)internal_config.num_hugepage_sizes; hpi_idx++) {
        ...
        max_type_mem = RTE_MIN(max_mem - total_mem, (uint64_t)RTE_MAX_MEM_MB_PER_TYPE << 20);
        ...
        while (total_type_mem < max_type_mem && total_segs < max_segs) {
            ...
            if (alloc_memseg_list(msl, hugepage_sz, n_segs, 0, type_msl_idx))
                return -1;
