...
static void rte_eal_config_create(void) {
    ...
    if (internal_config.base_virtaddr != 0)
        rte_mem_cfg_addr = (void *)RTE_ALIGN_FLOOR(internal_config.base_virtaddr - sizeof(struct rte_mem_config),
    ...
    rte_mem_cfg_addr =
        mmap(rte_mem_cfg_addr, sizeof(*rte_config.mem_config), PROT_READ | PROT_WRITE, MAP_SHARED, mem_cfg_fd, 0);
...
static void rte_eal_config_reattach(void) {
    struct rte_mem_config *mem_config;
    void *rte_mem_cfg_addr;
    ...
    mem_config = (struct rte_mem_config *)mmap(rte_mem_cfg_addr, sizeof(*mem_config), PROT_READ | PROT_WRITE,
...
static void eal_update_internal_config(void) {
    struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
...
static int eal_parse_args(int argc, char **argv) {
    ...
    while ((opt = getopt_long(argc, argvopt, eal_short_options, eal_long_options, &option_index)) != EOF) {
        ...
        case OPT_SOCKET_MEM_NUM:
            if (eal_parse_socket_arg(optarg, internal_config.socket_mem) < 0) {
        ...
        case OPT_SOCKET_LIMIT_NUM:
            if (eal_parse_socket_arg(optarg, internal_config.socket_limit) < 0) {
...
int rte_eal_init(int argc, char **argv) {
    ...
    if (internal_config.iova_mode == RTE_IOVA_DC) {
        ...
        if (rte_eal_get_configuration()->iova_mode == RTE_IOVA_VA && rte_eal_check_module("rte_kni") == 1) {
...
static int mark_freeable(const struct rte_memseg_list *msl, const struct rte_memseg *ms, void *arg __rte_unused) {
...
enum rte_intr_mode rte_eal_vfio_intr_mode(void) { return internal_config.vfio_intr_mode; }
