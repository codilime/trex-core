...
static void sigbus_handler(int signum, siginfo_t *info, void *ctx __rte_unused) {
    int ret;
    ...
    if (ret == -1) {
        ...
        if (sigbus_action_old.sa_flags == SA_SIGINFO && sigbus_action_old.sa_sigaction) {
            (*(sigbus_action_old.sa_sigaction))(signum, info, ctx);
        } else if (sigbus_action_old.sa_flags != SA_SIGINFO && sigbus_action_old.sa_handler) {
...
static int dev_uev_socket_fd_create(void) {
    ...
    intr_handle.fd = socket(PF_NETLINK, SOCK_RAW | SOCK_CLOEXEC | SOCK_NONBLOCK, NETLINK_KOBJECT_UEVENT);
    if (intr_handle.fd < 0) {
...
static int dev_uev_parse(const char *buf, struct rte_dev_event *event, int length) {
