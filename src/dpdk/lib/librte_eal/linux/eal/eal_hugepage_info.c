 ...
 */
static void *map_shared_memory(const char *filename, const size_t mem_size, int flags) {
    void *retval;
...
static uint32_t get_num_hugepages_on_node(const char *subdir, unsigned int socket) {
    char path[PATH_MAX], socketpath[PATH_MAX];
    DIR *socketdir;
    unsigned long num_pages = 0;
    const char *nr_hp_file = "free_hugepages";

    snprintf(socketpath, sizeof(socketpath), "%s/node%u/hugepages", sys_pages_numa_dir_path, socket);
...
static void calc_num_pages(struct hugepage_info *hpi, struct dirent *dirent) {
    ...
    if (total_pages == 0) {
         ...
         * 1GB per page size */
        hpi->num_pages[0] = RTE_MIN(hpi->num_pages[0], RTE_PGSIZE_1G / hpi->hugepage_sz);
#endif
    }
}
...
int eal_hugepage_info_read(void) {
    ...
    tmp_hpi = open_shared_memory(eal_hugepage_info_path(), sizeof(internal_config.hugepage_info));
    if (tmp_hpi == NULL) {
