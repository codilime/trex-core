...
#define MSIX_IRQ_SET_BUF_LEN (sizeof(struct vfio_irq_set) + sizeof(int) * (RTE_MAX_RXTX_INTR_VEC_ID + 1))
...
static int vfio_enable_msix(const struct rte_intr_handle *intr_handle) {
    ...
    irq_set->count = intr_handle->max_intr
                         ? (intr_handle->max_intr > RTE_MAX_RXTX_INTR_VEC_ID + 1 ? RTE_MAX_RXTX_INTR_VEC_ID + 1
    ...
    memcpy(&fd_ptr[RTE_INTR_VEC_RXTX_OFFSET], intr_handle->efds, sizeof(*intr_handle->efds) * intr_handle->nb_efd);
...
int rte_intr_callback_unregister(const struct rte_intr_handle *intr_handle, rte_intr_callback_fn cb_fn, void *cb_arg) {
    int ret;
    struct rte_intr_source *src;
    struct rte_intr_callback *cb, *next;
...
int rte_eal_intr_init(void) {
    ...
    ret = rte_ctrl_thread_create(&intr_thread, "eal-intr-thread", NULL, eal_intr_thread_main, NULL);
    if (ret != 0) {
        rte_errno = -ret;
...
static int eal_epoll_process_event(struct epoll_event *evs, unsigned int n, struct rte_epoll_event *events) {
...
static inline void eal_epoll_data_safe_free(struct rte_epoll_event *ev) {
    while (!rte_atomic32_cmpset(&ev->status, RTE_EPOLL_VALID, RTE_EPOLL_INVALID))
...
int rte_intr_rx_ctl(struct rte_intr_handle *intr_handle, int epfd, int op, unsigned int vec, void *data) {
    ...
    if (!intr_handle || intr_handle->nb_efd == 0 || efd_idx >= intr_handle->nb_efd) {
...
int rte_intr_efd_enable(struct rte_intr_handle *intr_handle, uint32_t nb_efd) {
    ...
    if (intr_handle->type == RTE_INTR_HANDLE_VFIO_MSIX) {
        ...
        if (intr_handle->efd_counter_size > sizeof(union rte_intr_read_buffer)) {
...
int rte_intr_dp_is_en(struct rte_intr_handle *intr_handle) { return !(!intr_handle->nb_efd); }
