...
static void __rte_unused huge_sigbus_handler(int signo __rte_unused) { siglongjmp(huge_jmpenv, 1); }

/* Put setjmp into a wrap method to avoid compiling error. Any non-volatile,
 * non-static local variable in the stack frame calling sigsetjmp might be
 * clobbered by a call to longjmp.
 */
static int __rte_unused huge_wrap_sigsetjmp(void) { return sigsetjmp(huge_jmpenv, 1); }

static struct sigaction huge_action_old;
static int huge_need_recover;

static void __rte_unused huge_register_sigbus(void) {
    sigset_t mask;
...
static void __rte_unused huge_recover_sigbus(void) {
...
static void prepare_numa(int *oldpolicy, struct bitmask *oldmask, int socket_id) {
...
static int get_seg_memfd(struct hugepage_info *hi __rte_unused, unsigned int list_idx __rte_unused,
...
static int get_seg_fd(char *path, int buflen, struct hugepage_info *hi, unsigned int list_idx, unsigned int seg_idx) {
    int fd;
...
static int resize_hugefile_in_memory(int fd, uint64_t fa_offset, uint64_t page_sz, bool grow) {
...
static int resize_hugefile_in_filesystem(int fd, uint64_t fa_offset, uint64_t page_sz, bool grow) {
    bool again = false;
...
static void close_hugefile(int fd, char *path, int list_idx) {
     ...
     */
    if (!internal_config.in_memory && rte_eal_process_type() == RTE_PROC_PRIMARY && unlink(path))
...
static int resize_hugefile(int fd, uint64_t fa_offset, uint64_t page_sz, bool grow) {
    ...
    return resize_hugefile_in_filesystem(fd, fa_offset, page_sz, grow);
}

static int alloc_seg(struct rte_memseg *ms, void *addr, int socket_id, struct hugepage_info *hi, unsigned int list_idx,
    ...
    if (internal_config.in_memory && !memfd_create_supported && internal_config.single_file_segments) {
    ...
    if (internal_config.in_memory && !memfd_create_supported) {
        const int in_memory_flags = MAP_HUGETLB | MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS;
        int pagesz_flag;
        ...
        if (internal_config.single_file_segments) {
            ...
            if (internal_config.hugepage_unlink && !internal_config.in_memory) {
                if (unlink(path)) {
    ...
    *(volatile int *)addr = *(volatile int *)addr;
    ...
    if (cur_socket_id != socket_id) {
        RTE_LOG(DEBUG, EAL, "%s(): allocation happened on wrong socket (wanted %d, got %d)\n", __func__, socket_id,
                cur_socket_id);
        goto mapped;
    }
#else
...
resized:
    ...
    if (internal_config.single_file_segments) {
        ...
        if (internal_config.hugepage_unlink == 0 && internal_config.in_memory == 0 && lock(fd, LOCK_EX) == 1)
            unlink(path);
        close(fd);
...
static int free_seg(struct rte_memseg *ms, struct hugepage_info *hi, unsigned int list_idx, unsigned int seg_idx) {
    uint64_t map_offset;
    char path[PATH_MAX];
    int fd, ret = 0;
    bool exit_early;
    ...
    if (mmap(ms->addr, ms->len, PROT_READ, MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0) == MAP_FAILED) {
...
static int free_seg_walk(const struct rte_memseg_list *msl, void *arg) {
    ...
    if ((uintptr_t)wa->ms->addr < start_addr || (uintptr_t)wa->ms->addr >= end_addr)
        return 0;
...
int eal_memalloc_alloc_seg_bulk(struct rte_memseg **ms, int n_segs, size_t page_sz, int socket, bool exact) {
    int i, ret = -1;
#ifdef RTE_EAL_NUMA_AWARE_HUGEPAGES
    ...
    for (i = 0; i < (int)RTE_DIM(internal_config.hugepage_info); i++) {
        if (page_sz == internal_config.hugepage_info[i].hugepage_sz) {
...
static int sync_chunk(struct rte_memseg_list *primary_msl, struct rte_memseg_list *local_msl, struct hugepage_info *hi,
    ...
    diff_len = used ? rte_fbarray_find_contig_free(l_arr, start) : rte_fbarray_find_contig_used(l_arr, start);
    ...
    diff_len = used ? rte_fbarray_find_contig_used(l_arr, start) : rte_fbarray_find_contig_free(l_arr, start);
    ret = RTE_MIN(chunk_len, diff_len);

    return ret;
}

static int sync_status(struct rte_memseg_list *primary_msl, struct rte_memseg_list *local_msl, struct hugepage_info *hi,
...
static int sync_existing(struct rte_memseg_list *primary_msl, struct rte_memseg_list *local_msl,
    ...
    if (dir_fd < 0) {
        RTE_LOG(ERR, EAL, "%s(): Cannot open '%s': %s\n", __func__, hi->hugedir, strerror(errno));
        return -1;
    }
    /* blocking writelock */
    if (flock(dir_fd, LOCK_EX)) {
...
static int sync_walk(const struct rte_memseg_list *msl, void *arg __rte_unused) {
    struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
    ...
    for (i = 0; i < RTE_DIM(internal_config.hugepage_info); i++) {
        uint64_t cur_sz = internal_config.hugepage_info[i].hugepage_sz;
    ...
    if (local_msl->version != primary_msl->version && sync_existing(primary_msl, local_msl, hi, msl_idx))
        return -1;
    return 0;
}
...
static int secondary_msl_create_walk(const struct rte_memseg_list *msl, void *arg __rte_unused) {
    struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
    struct rte_memseg_list *primary_msl, *local_msl;
    char name[PATH_MAX];
    int msl_idx, ret;
    ...
    snprintf(name, RTE_FBARRAY_NAME_LEN, "%s_%i", primary_msl->memseg_arr.name, getpid());

    ret = rte_fbarray_init(&local_msl->memseg_arr, name, primary_msl->memseg_arr.len, primary_msl->memseg_arr.elt_sz);
    if (ret < 0) {
...
static int fd_list_create_walk(const struct rte_memseg_list *msl, void *arg __rte_unused) {
    struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
    unsigned int len;
    int msl_idx;
...
int eal_memalloc_init(void) {
    if (rte_eal_process_type() == RTE_PROC_SECONDARY)
        if (rte_memseg_list_walk(secondary_msl_create_walk, NULL) < 0)
            return -1;
    if (rte_eal_process_type() == RTE_PROC_PRIMARY && internal_config.in_memory) {
         ...
         * test if we do.
         */
        if (internal_config.single_file_segments && mfd_res != 1) {
