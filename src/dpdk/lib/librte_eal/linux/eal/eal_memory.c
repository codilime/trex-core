...
static void test_phys_addrs_available(void) {
    uint64_t tmp = 0;
 ...
 */
static int find_physaddrs(struct hugepage_file *hugepg_tbl, struct hugepage_info *hpi) {
    unsigned int i;
    phys_addr_t addr;
 ...
 */
static int set_physaddrs(struct hugepage_file *hugepg_tbl, struct hugepage_info *hpi) {
    unsigned int i;
...
static void huge_sigbus_handler(int signo __rte_unused) { siglongjmp(huge_jmpenv, 1); }
...
void numa_error(char *where) { RTE_LOG(ERR, EAL, "%s failed: %s\n", where, strerror(errno)); }
#endif
 ...
 */
static unsigned map_all_hugepages(struct hugepage_file *hugepg_tbl, struct hugepage_info *hpi,
    ...
    for (i = 0; i < hpi->num_pages[0]; i++) {
        ...
        hf->file_id = i;
        hf->size = hugepage_sz;
        eal_get_hugefile_path(hf->filepath, sizeof(hf->filepath), hpi->hugedir, hf->file_id);
         ...
         */
        virtaddr = mmap(NULL, hugepage_sz, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE, fd, 0);
 ...
 * page.
 */
static int find_numasocket(struct hugepage_file *hugepg_tbl, struct hugepage_info *hpi) {
    int socket_id;
    char *end, *nodestr;
    ...
    snprintf(hugedir_str, sizeof(hugedir_str), "%s/%s", hpi->hugedir, eal_get_hugefile_prefix());
...
static void *create_shared_memory(const char *filename, const size_t mem_size) {
    ...
    if (internal_config.no_shconf) {
        retval = mmap(NULL, mem_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
 ...
 */
static int copy_hugepages_to_shared_mem(struct hugepage_file *dst, int dest_size, const struct hugepage_file *src,
                                        int src_size) {
...
static int unlink_hugepage_files(struct hugepage_file *hugepg_tbl, unsigned num_hp_info) {
 ...
 */
static int unmap_unneeded_hugepages(struct hugepage_file *hugepg_tbl, struct hugepage_info *hpi, unsigned num_hp_info) {
    unsigned socket, size;
    int page, nrpages = 0;
    ...
    for (size = 0; size < num_hp_info; size++) {
        for (socket = 0; socket < RTE_MAX_NUMA_NODES; socket++) {
            ...
            for (page = 0; page < nrpages; page++) {
                ...
                if ((hp->size == hpi[size].hugepage_sz) && (hp->socket_id == (int)socket)) {
...
static int remap_segment(struct hugepage_file *hugepages, int seg_start, int seg_end) {
    ...
    if (msl_idx == RTE_MAX_MEMSEG_LISTS) {
        RTE_LOG(ERR, EAL, "Could not find space for memseg. Please increase %s and/or %s in configuration.\n",
                RTE_STR(CONFIG_RTE_MAX_MEMSEG_PER_TYPE), RTE_STR(CONFIG_RTE_MAX_MEM_PER_TYPE));
        return -1;
    }
    ...
    for (cur_page = seg_start; cur_page < seg_end; cur_page++, ms_idx++) {
         ...
         */
        addr = mmap(addr, page_sz, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE | MAP_FIXED, fd, 0);
        if (addr == MAP_FAILED) {
...
static int free_memseg_list(struct rte_memseg_list *msl) {
...
static int alloc_memseg_list(struct rte_memseg_list *msl, uint64_t page_sz, int n_segs, int socket_id,
                             int type_msl_idx) {
    char name[RTE_FBARRAY_NAME_LEN];

    snprintf(name, sizeof(name), MEMSEG_LIST_FMT, page_sz >> 10, socket_id, type_msl_idx);
    if (rte_fbarray_init(&msl->memseg_arr, name, n_segs, sizeof(struct rte_memseg))) {
...
static int alloc_va_space(struct rte_memseg_list *msl) {
    uint64_t page_sz;
    size_t mem_sz;
    void *addr;
    int flags = 0;
 ...
 */
static int __rte_unused prealloc_segments(struct hugepage_file *hugepages, int n_pages) {
    struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
     ...
     */
    for (hpi_idx = 0; hpi_idx < internal_config.num_hugepage_sizes; hpi_idx++) {
        uint64_t page_sz = internal_config.hugepage_info[hpi_idx].hugepage_sz;
        ...
        for (i = 0; i < rte_socket_count(); i++) {
            ...
            if (alloc_memseg_list(msl, page_sz, n_segs, socket, msl_idx) < 0)
                return -1;
 ...
 */
static int remap_needed_hugepages(struct hugepage_file *hugepages, int n_pages) {
 ...
 */
static int calc_num_pages_per_socket(uint64_t *memory, struct hugepage_info *hp_info, struct hugepage_info *hp_used,
    ...
    if (internal_config.force_sockets == 0) {
        ...
        RTE_LCORE_FOREACH(lcore_id) { cpu_per_socket[rte_lcore_to_socket_id(lcore_id)]++; }
        ...
        for (socket = 0; socket < RTE_MAX_NUMA_NODES && total_size != 0; socket++) {
            ...
            default_size = (internal_config.memory * cpu_per_socket[socket]) / rte_lcore_count();
        ...
        for (socket = 0; socket < RTE_MAX_NUMA_NODES && total_size != 0; socket++) {
            ...
            default_size = RTE_MIN(get_socket_mem_size(socket) - memory[socket], total_size);
    ...
    for (socket = 0; socket < RTE_MAX_NUMA_NODES && total_mem != 0; socket++) {
        ...
        for (i = 0; i < num_hp_info && memory[socket] != 0; i++) {
            strlcpy(hp_used[i].hugedir, hp_info[i].hugedir, sizeof(hp_used[i].hugedir));
            hp_used[i].num_pages[socket] =
                RTE_MIN(memory[socket] / hp_info[i].hugepage_sz, hp_info[i].num_pages[socket]);
        ...
        if (memory[socket] > 0 && internal_config.socket_mem[socket] != 0) {
...
static int eal_legacy_hugepage_init(void) {
    ...
    if (internal_config.no_hugetlbfs) {
        ...
        if (rte_fbarray_init(&msl->memseg_arr, "nohugemem", n_segs, sizeof(struct rte_memseg))) {
...
#endif
        addr = mmap(NULL, internal_config.memory, PROT_READ | PROT_WRITE, flags, fd, 0);
        if (addr == MAP_FAILED) {
        ...
        if (mcfg->dma_maskbits && rte_mem_check_dma_mask_thread_unsafe(mcfg->dma_maskbits)) {
            RTE_LOG(ERR, EAL, "%s(): couldn't allocate memory due to IOVA exceeding limits of current DMA mask.\n",
                    __func__);
            if (rte_eal_iova_mode() == RTE_IOVA_VA && rte_eal_using_phys_addrs())
                RTE_LOG(ERR, EAL, "%s(): Please try initializing EAL with --iova-mode=pa parameter.\n", __func__);
            goto fail;
        }
        return 0;
    }
    ...
    for (i = 0; i < (int)internal_config.num_hugepage_sizes; i++) {
        ...
        if (phys_addrs_available && rte_eal_iova_mode() != RTE_IOVA_VA) {
    ...
    for (i = 0; i < nr_hugefiles; i++) {
        ...
        const int nb_hpsizes = RTE_MIN(MAX_HUGEPAGE_SIZES, (int)internal_config.num_hugepage_sizes);
    ...
    nr_hugepages =
        calc_num_pages_per_socket(memory, internal_config.hugepage_info, used_hp, internal_config.num_hugepage_sizes);
    ...
    hugepage = create_shared_memory(eal_hugepage_data_path(), nr_hugefiles * sizeof(struct hugepage_file));
     ...
     */
    if (unmap_unneeded_hugepages(tmp_hp, used_hp, internal_config.num_hugepage_sizes) < 0) {
     ...
     */
    if (copy_hugepages_to_shared_mem(hugepage, nr_hugefiles, tmp_hp, nr_hugefiles) < 0) {
    ...
    if (internal_config.legacy_mem && prealloc_segments(hugepage, nr_hugefiles)) {
    ...
    if (internal_config.hugepage_unlink && unlink_hugepage_files(tmp_hp, internal_config.num_hugepage_sizes) < 0) {
...
static int __rte_unused hugepage_count_walk(const struct rte_memseg_list *msl, void *arg) {
...
static int eal_hugepage_init(void) {
    ...
    /* calculate final number of pages */
    if (calc_num_pages_per_socket(memory, internal_config.hugepage_info, used_hp, internal_config.num_hugepage_sizes) <
        0)
        return -1;
    ...
    if (internal_config.force_socket_limits) {
        ...
        for (i = 0; i < RTE_MAX_NUMA_NODES; i++) {
            ...
            if (limit == 0)
                continue;
            if (rte_mem_alloc_validator_register("socket-limit", limits_callback, i, limit))
...
static int eal_legacy_hugepage_attach(void) {
    ...
    for (i = 0; i < num_hp; i++) {
        ...
        map_addr = mmap(map_addr, map_sz, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, fd, 0);
...
static int eal_hugepage_attach(void) {
    if (eal_memalloc_sync_with_primary()) {
        ...
        if (aslr_enabled() > 0)
            RTE_LOG(ERR, EAL,
                    "It is recommended to disable ASLR in the kernel and retry running both primary and secondary "
                    "processes\n");
        return -1;
    }
    return 0;
...
int rte_eal_hugepage_attach(void) {
    return internal_config.legacy_mem ? eal_legacy_hugepage_attach() : eal_hugepage_attach();
}
...
static int __rte_unused memseg_primary_init_32(void) {
    ...
    extra_mem_per_socket = active_sockets == 0 ? total_extra_mem : total_extra_mem / active_sockets;
    ...
    for (i = 0; i < rte_socket_count(); i++) {
        ...
        skip = active_sockets != 0 && internal_config.socket_mem[socket_id] == 0;
        ...
        max_socket_mem = (active_sockets != 0 ? internal_config.socket_mem[socket_id] : internal_config.memory) +
        ...
        for (hpi_idx = 0; hpi_idx < hp_sizes; hpi_idx++) {
            ...
            while (cur_pagesz_mem < max_pagesz_mem && total_segs < max_segs) {
                ...
                if (alloc_memseg_list(msl, hugepage_sz, n_segs, socket_id, type_msl_idx)) {
...
static int __rte_unused memseg_primary_init(void) {
    ...
    /* populate mem types */
    cur_type = 0;
    for (hpi_idx = 0; hpi_idx < (int)internal_config.num_hugepage_sizes; hpi_idx++) {
        struct hugepage_info *hpi;
        uint64_t hugepage_sz;
    ...
    max_mem = (uint64_t)RTE_MAX_MEM_MB << 20;
    max_mem_per_type = RTE_MIN((uint64_t)RTE_MAX_MEM_MB_PER_TYPE << 20, max_mem / n_memtypes);
    /*
    ...
    for (cur_type = 0; cur_type < n_memtypes; cur_type++) {
        ...
        max_segs_per_type = RTE_MIN(max_segs_per_type, (unsigned int)RTE_MAX_MEMSEG_PER_TYPE);
        /* limit number of segments to maximum allowed per list */
        max_segs_per_list = RTE_MIN(max_segs_per_type, (unsigned int)RTE_MAX_MEMSEG_PER_LIST);

        /* calculate how much memory we can have per segment list */
        max_mem_per_list = RTE_MIN(max_segs_per_list * pagesz, (uint64_t)RTE_MAX_MEM_MB_PER_LIST << 20);
        ...
        n_seglists = RTE_MIN(max_segs_per_type / n_segs, max_mem_per_type / max_mem_per_list);
        ...
        for (cur_seglist = 0; cur_seglist < n_seglists; cur_seglist++) {
            ...
            if (alloc_memseg_list(msl, pagesz, n_segs, socket_id, cur_seglist))
                goto out;
...
int rte_eal_memseg_init(void) {
    ...
    if (getrlimit(RLIMIT_NOFILE, &lim) == 0) {
        ...
        if (setrlimit(RLIMIT_NOFILE, &lim) < 0) {
            ...
            RTE_LOG(DEBUG, EAL, "Setting maximum number of open files to %" PRIu64 "\n", (uint64_t)lim.rlim_cur);
        }
    } else {
