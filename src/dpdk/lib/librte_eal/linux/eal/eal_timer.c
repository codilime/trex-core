 ...
 */
static void *hpet_msb_inc(__attribute__((unused)) void *arg) {
    uint32_t t;
...
int rte_eal_hpet_init(int make_default) {
    ...
    eal_hpet_resolution_fs = (uint32_t)((eal_hpet->capabilities & CLK_PERIOD_MASK) >> CLK_PERIOD_SHIFT);

    eal_hpet_resolution_hz = (1000ULL * 1000ULL * 1000ULL * 1000ULL * 1000ULL) / (uint64_t)eal_hpet_resolution_fs;
    ...
    ret = rte_ctrl_thread_create(&msb_inc_thread_id, "hpet-msb-inc", NULL, hpet_msb_inc, NULL);
    if (ret != 0) {
