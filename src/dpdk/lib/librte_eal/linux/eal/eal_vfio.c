...
static int vfio_dma_mem_map(struct vfio_config *vfio_cfg, uint64_t vaddr, uint64_t iova, uint64_t len, int do_map);
...
static int is_null_map(const struct user_mem_map *map) { return map->addr == 0 && map->iova == 0 && map->len == 0; }
 ...
 */
static void adjust_map(struct user_mem_map *src, struct user_mem_map *end, uint64_t remove_va_start,
                       uint64_t remove_len) {
...
static struct user_mem_map *find_user_mem_map(struct user_mem_maps *user_mem_maps, uint64_t addr, uint64_t iova,
                                              uint64_t len) {
...
static void compact_user_maps(struct user_mem_maps *user_mem_maps) {
    ...
    qsort(user_mem_maps->maps, user_mem_maps->n_maps, sizeof(user_mem_maps->maps[0]), user_mem_map_cmp);
...
static int vfio_open_group_fd(int iommu_group_num) {
    ...
    if (internal_config.process_type == RTE_PROC_PRIMARY) {
        ...
        if (vfio_group_fd < 0) {
            ...
            snprintf(filename, sizeof(filename), VFIO_NOIOMMU_GROUP_FMT, iommu_group_num);
            vfio_group_fd = open(filename, O_RDWR);
            if (vfio_group_fd < 0) {
                if (errno != ENOENT) {
                    RTE_LOG(ERR, EAL, "Cannot open %s: %s\n", filename, strerror(errno));
...
static int vfio_get_group_fd(struct vfio_config *vfio_cfg, int iommu_group_num) {
    int i;
    int vfio_group_fd;
...
static void vfio_mem_event_callback(enum rte_mem_event type, const void *addr, size_t len, void *arg __rte_unused) {
    ...
    if (rte_eal_iova_mode() == RTE_IOVA_VA && msl->external == 0) {
        ...
        if (type == RTE_MEM_EVENT_ALLOC)
            vfio_dma_mem_map(default_vfio_cfg, vfio_va, vfio_va, len, 1);
        else
            vfio_dma_mem_map(default_vfio_cfg, vfio_va, vfio_va, len, 0);
        return;
    }
...
static int vfio_sync_default_container(void) {
    ...
    if (!default_vfio_cfg->vfio_enabled || default_vfio_cfg->vfio_container_fd < 0) {
...
int rte_vfio_setup_device(const char *sysfs_base, const char *dev_addr, int *vfio_dev_fd,
    ...
    if (!(group_status.flags & VFIO_GROUP_FLAGS_CONTAINER_SET)) {
         ...
         * functionality.
         */
        if (internal_config.process_type == RTE_PROC_PRIMARY && vfio_cfg->vfio_active_groups == 1 &&
            ...
            if (vfio_cfg == default_vfio_cfg)
                ...
                ret = 0;
            if (ret) {
                RTE_LOG(ERR, EAL, "  %s DMA remapping failed, error %i (%s)\n", dev_addr, errno, strerror(errno));
                close(vfio_group_fd);
                rte_vfio_clear_group(vfio_group_fd);
            ...
            for (i = 0; i < user_mem_maps->n_maps; i++) {
                ...
                ret = t->dma_user_map_func(vfio_container_fd, map->addr, map->iova, map->len, 1);
                if (ret) {
                    RTE_LOG(ERR, EAL,
            ...
            if (vfio_cfg == default_vfio_cfg)
                ret = rte_mem_event_callback_register(VFIO_MEM_EVENT_CLB_NAME, vfio_mem_event_callback, NULL);
            else
                ret = 0;
    ...
    } else if (rte_eal_process_type() != RTE_PROC_PRIMARY && vfio_cfg == default_vfio_cfg &&
...
int rte_vfio_release_device(const char *sysfs_base, const char *dev_addr, int vfio_dev_fd) {
    struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
    rte_rwlock_t *mem_lock = &mcfg->memory_hotplug_lock;
    struct vfio_group_status group_status = {.argsz = sizeof(group_status)};
    struct vfio_config *vfio_cfg;
...
static int type1_map(const struct rte_memseg_list *msl, const struct rte_memseg *ms, void *arg) {
    int *vfio_container_fd = arg;

    if (msl->external)
        return 0;

    return vfio_type1_dma_mem_map(*vfio_container_fd, ms->addr_64, ms->iova, ms->len, 1);
}

static int vfio_type1_dma_mem_map(int vfio_container_fd, uint64_t vaddr, uint64_t iova, uint64_t len, int do_map) {
...
static int vfio_type1_dma_map(int vfio_container_fd) { return rte_memseg_walk(type1_map, &vfio_container_fd); }

static int vfio_spapr_dma_do_map(int vfio_container_fd, uint64_t vaddr, uint64_t iova, uint64_t len, int do_map) {
    struct vfio_iommu_type1_dma_map dma_map;
    struct vfio_iommu_type1_dma_unmap dma_unmap;
    int ret;
    struct vfio_iommu_spapr_register_memory reg = {.argsz = sizeof(reg), .flags = 0};
    ...
    if (do_map != 0) {
        ...
        if (ret) {
            RTE_LOG(ERR, EAL,
                    "  cannot register vaddr for IOMMU, "
                    "error %i (%s)\n",
                    errno, strerror(errno));
            return -1;
        }
...
static int vfio_spapr_map_walk(const struct rte_memseg_list *msl, const struct rte_memseg *ms, void *arg) {
    int *vfio_container_fd = arg;

    if (msl->external)
        return 0;

    return vfio_spapr_dma_do_map(*vfio_container_fd, ms->addr_64, ms->iova, ms->len, 1);
}
...
static int vfio_spapr_window_size_walk(const struct rte_memseg_list *msl, const struct rte_memseg *ms, void *arg) {
...
static int vfio_spapr_create_new_dma_window(int vfio_container_fd, struct vfio_iommu_spapr_tce_create *create) {
    ...
    if (ret) {
        RTE_LOG(ERR, EAL,
                "  cannot remove default DMA window, "
                "error %i (%s)\n",
                errno, strerror(errno));
        return -1;
    }
...
static int vfio_spapr_dma_mem_map(int vfio_container_fd, uint64_t vaddr, uint64_t iova, uint64_t len, int do_map) {
    ...
    if (rte_memseg_walk_thread_unsafe(vfio_spapr_window_size_walk, &param) < 0) {
    ...
    for (i = 0; i < user_mem_maps->n_maps; i++) {
        uint64_t max = user_mem_maps->maps[i].iova + user_mem_maps->maps[i].len;
    ...
    if (do_map) {
        ...
        if (iova > create.window_size) {
            ...
            /* we're inside a callback, so use thread-unsafe version
             */
            if (rte_memseg_walk_thread_unsafe(vfio_spapr_map_walk, &vfio_container_fd) < 0) {
            ...
            for (i = 0; i < user_mem_maps->n_maps; i++) {
                struct user_mem_map *map = &user_mem_maps->maps[i];
                if (vfio_spapr_dma_do_map(vfio_container_fd, map->addr, map->iova, map->len, 1)) {
        ...
        if (rte_mem_virt2memseg_list(addr) == NULL &&
            vfio_spapr_dma_do_map(vfio_container_fd, vaddr, iova, len, 1) < 0) {
...
static int vfio_noiommu_dma_mem_map(int __rte_unused vfio_container_fd, uint64_t __rte_unused vaddr,
                                    uint64_t __rte_unused iova, uint64_t __rte_unused len, int __rte_unused do_map) {
    /* No-IOMMU mode does not need DMA mapping */
    return 0;
}
...
static int vfio_dma_mem_map(struct vfio_config *vfio_cfg, uint64_t vaddr, uint64_t iova, uint64_t len, int do_map) {
    ...
    if (!t->dma_user_map_func) {
        RTE_LOG(ERR, EAL, "  VFIO custom DMA region maping not supported by IOMMU %s\n", t->name);
        rte_errno = ENOTSUP;
        return -1;
    }

    return t->dma_user_map_func(vfio_cfg->vfio_container_fd, vaddr, iova, len, do_map);
}

static int container_dma_map(struct vfio_config *vfio_cfg, uint64_t vaddr, uint64_t iova, uint64_t len) {
...
static int container_dma_unmap(struct vfio_config *vfio_cfg, uint64_t vaddr, uint64_t iova, uint64_t len) {
...
int rte_vfio_noiommu_is_enabled(void) {
    ...
    if (cnt != 1) {
        RTE_LOG(ERR, EAL,
                "  unable to read from vfio noiommu "
                "file %i (%s)\n",
                errno, strerror(errno));
        return -1;
    }
...
int __rte_experimental rte_vfio_container_destroy(int container_fd) {
    struct vfio_config *vfio_cfg;
    int i;
    ...
    for (i = 0; i < VFIO_MAX_GROUPS; i++)
        if (vfio_cfg->vfio_groups[i].group_num != -1)
            rte_vfio_container_group_unbind(container_fd, vfio_cfg->vfio_groups[i].group_num);
...
int rte_vfio_container_group_unbind(int container_fd, int iommu_group_num) {
    ...
    if (cur_grp->fd >= 0 && close(cur_grp->fd) < 0) {
        RTE_LOG(ERR, EAL,
                "Error when closing vfio_group_fd for"
                " iommu_group_num %d\n",
                iommu_group_num);
        return -1;
    }
...
int rte_vfio_dma_map(uint64_t __rte_unused vaddr, __rte_unused uint64_t iova, __rte_unused uint64_t len) { return -1; }

int rte_vfio_dma_unmap(uint64_t __rte_unused vaddr, uint64_t __rte_unused iova, __rte_unused uint64_t len) {
    return -1;
}

int rte_vfio_setup_device(__rte_unused const char *sysfs_base, __rte_unused const char *dev_addr,
                          __rte_unused int *vfio_dev_fd, __rte_unused struct vfio_device_info *device_info) {
    return -1;
}

int rte_vfio_release_device(__rte_unused const char *sysfs_base, __rte_unused const char *dev_addr,
                            __rte_unused int fd) {
    return -1;
}

int rte_vfio_enable(__rte_unused const char *modname) { return -1; }

int rte_vfio_is_enabled(__rte_unused const char *modname) { return -1; }

int rte_vfio_noiommu_is_enabled(void) { return -1; }

int rte_vfio_clear_group(__rte_unused int vfio_group_fd) { return -1; }
...
int rte_vfio_get_container_fd(void) { return -1; }

int rte_vfio_get_group_fd(__rte_unused int iommu_group_num) { return -1; }

int rte_vfio_container_create(void) { return -1; }

int rte_vfio_container_destroy(__rte_unused int container_fd) { return -1; }

int rte_vfio_container_group_bind(__rte_unused int container_fd, __rte_unused int iommu_group_num) { return -1; }

int rte_vfio_container_group_unbind(__rte_unused int container_fd, __rte_unused int iommu_group_num) { return -1; }

int rte_vfio_container_dma_map(__rte_unused int container_fd, __rte_unused uint64_t vaddr, __rte_unused uint64_t iova,
                               __rte_unused uint64_t len) {
    return -1;
}

int rte_vfio_container_dma_unmap(__rte_unused int container_fd, __rte_unused uint64_t vaddr, __rte_unused uint64_t iova,
                                 __rte_unused uint64_t len) {
    return -1;
}
