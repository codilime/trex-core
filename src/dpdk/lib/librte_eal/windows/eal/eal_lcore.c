...
void eal_create_cpu_map() {
    wcpu_map.total_procs = GetActiveProcessorCount(ALL_PROCESSOR_GROUPS);
    ...
    for (unsigned int socket = 0; socket < wcpu_map.proc_sockets; ++socket) {
        for (unsigned int core = 0; core < (wcpu_map.proc_cores / wcpu_map.proc_sockets); ++core) {
            wcpu_map.wlcore_map[lcore].socket_id = socket;
            wcpu_map.wlcore_map[lcore].core_id = core;
            lcore++;
            if (ht_enabled) {
                wcpu_map.wlcore_map[lcore].socket_id = socket;
                wcpu_map.wlcore_map[lcore].core_id = core;
 ...
 */
int eal_cpu_detected(unsigned int lcore_id) { return (lcore_id < wcpu_map.total_procs); }

/*
 * Get CPU socket id for a logical core
 */
unsigned eal_cpu_socket_id(unsigned int lcore_id) { return wcpu_map.wlcore_map[lcore_id].socket_id; }

/*
 * Get CPU socket id (NUMA node) for a logical core
 */
unsigned eal_cpu_core_id(unsigned int lcore_id) { return wcpu_map.wlcore_map[lcore_id].core_id; }
