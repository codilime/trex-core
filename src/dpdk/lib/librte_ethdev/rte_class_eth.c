...
static int eth_mac_cmp(const char *key __rte_unused, const char *value, void *opaque) {
    int ret;
...
static int eth_representor_cmp(const char *key __rte_unused, const char *value, void *opaque) {
    int ret;
    char *values;
    const struct rte_eth_dev_data *data = opaque;
    ...
    ret = rte_eth_devargs_parse_list(values, rte_eth_devargs_parse_representor_ports, &representors);
    free(values);
    if (ret != 0)
        return -1; /* invalid devargs value */
    ...
    for (index = 0; index < representors.nb_representor_ports; index++)
        if (data->representor_id == representors.representor_ports[index])
            return 0;
    return -1; /* no match */
}
...
static int eth_dev_match(const struct rte_eth_dev *edev, const void *_arg) {
    ...
    ret = rte_kvargs_process(kvlist, eth_params_keys[RTE_ETH_PARAM_MAC], eth_mac_cmp, edev->data);
    if (ret != 0)
        return -1;

    ret = rte_kvargs_process(kvlist, eth_params_keys[RTE_ETH_PARAM_REPRESENTOR], eth_representor_cmp, edev->data);
    if (ret != 0)
        return -1;
    /* search for representor key */
    for (pair = 0; pair < kvlist->count; pair++) {
        ret = strcmp(kvlist->pairs[pair].key, eth_params_keys[RTE_ETH_PARAM_REPRESENTOR]);
        if (ret == 0)
...
static void *eth_dev_iterate(const void *start, const char *str, const struct rte_dev_iterator *it) {
    ...
    edev = eth_find_device(start, eth_dev_match, eth_dev_match_arg(it->device, kvargs));
