...
#define RTE_NB_RXQ_STATS (sizeof(rte_rxq_stats_strings) / sizeof(rte_rxq_stats_strings[0]))

static const struct rte_eth_xstats_name_off rte_txq_stats_strings[] = {
    {"packets", offsetof(struct rte_eth_stats, q_opackets)},
    {"bytes", offsetof(struct rte_eth_stats, q_obytes)},
};
#define RTE_NB_TXQ_STATS (sizeof(rte_txq_stats_strings) / sizeof(rte_txq_stats_strings[0]))
...
static const struct {
    ...
    RTE_RX_OFFLOAD_BIT2STR(VLAN_STRIP),       RTE_RX_OFFLOAD_BIT2STR(IPV4_CKSUM),
    RTE_RX_OFFLOAD_BIT2STR(UDP_CKSUM),        RTE_RX_OFFLOAD_BIT2STR(TCP_CKSUM),
    RTE_RX_OFFLOAD_BIT2STR(TCP_LRO),          RTE_RX_OFFLOAD_BIT2STR(QINQ_STRIP),
    RTE_RX_OFFLOAD_BIT2STR(OUTER_IPV4_CKSUM), RTE_RX_OFFLOAD_BIT2STR(MACSEC_STRIP),
    RTE_RX_OFFLOAD_BIT2STR(HEADER_SPLIT),     RTE_RX_OFFLOAD_BIT2STR(VLAN_FILTER),
    RTE_RX_OFFLOAD_BIT2STR(VLAN_EXTEND),      RTE_RX_OFFLOAD_BIT2STR(JUMBO_FRAME),
    RTE_RX_OFFLOAD_BIT2STR(SCATTER),          RTE_RX_OFFLOAD_BIT2STR(TIMESTAMP),
    RTE_RX_OFFLOAD_BIT2STR(SECURITY),         RTE_RX_OFFLOAD_BIT2STR(KEEP_CRC),
    RTE_RX_OFFLOAD_BIT2STR(SCTP_CKSUM),       RTE_RX_OFFLOAD_BIT2STR(OUTER_UDP_CKSUM),
};
...
static const struct {
    ...
    RTE_TX_OFFLOAD_BIT2STR(VLAN_INSERT),     RTE_TX_OFFLOAD_BIT2STR(IPV4_CKSUM),
    RTE_TX_OFFLOAD_BIT2STR(UDP_CKSUM),       RTE_TX_OFFLOAD_BIT2STR(TCP_CKSUM),
    RTE_TX_OFFLOAD_BIT2STR(SCTP_CKSUM),      RTE_TX_OFFLOAD_BIT2STR(TCP_TSO),
    RTE_TX_OFFLOAD_BIT2STR(UDP_TSO),         RTE_TX_OFFLOAD_BIT2STR(OUTER_IPV4_CKSUM),
    RTE_TX_OFFLOAD_BIT2STR(QINQ_INSERT),     RTE_TX_OFFLOAD_BIT2STR(VXLAN_TNL_TSO),
    RTE_TX_OFFLOAD_BIT2STR(GRE_TNL_TSO),     RTE_TX_OFFLOAD_BIT2STR(IPIP_TNL_TSO),
    RTE_TX_OFFLOAD_BIT2STR(GENEVE_TNL_TSO),  RTE_TX_OFFLOAD_BIT2STR(MACSEC_INSERT),
    RTE_TX_OFFLOAD_BIT2STR(MT_LOCKFREE),     RTE_TX_OFFLOAD_BIT2STR(MULTI_SEGS),
    RTE_TX_OFFLOAD_BIT2STR(MBUF_FAST_FREE),  RTE_TX_OFFLOAD_BIT2STR(SECURITY),
    RTE_TX_OFFLOAD_BIT2STR(UDP_TNL_TSO),     RTE_TX_OFFLOAD_BIT2STR(IP_TNL_TSO),
    RTE_TX_OFFLOAD_BIT2STR(OUTER_UDP_CKSUM), RTE_TX_OFFLOAD_BIT2STR(MATCH_METADATA),
};
...
int rte_eth_iterator_init(struct rte_dev_iterator *iter, const char *devargs_str) {
    int ret;
...
uint16_t rte_eth_iterator_next(struct rte_dev_iterator *iter) {
    ...
    do { /* loop to try all matching rte_device */
        ...
        iter->class_device = iter->cls->dev_iterate(iter->class_device, iter->cls_str, iter);
...
uint16_t rte_eth_find_next(uint16_t port_id) {
    while (port_id < RTE_MAX_ETHPORTS && rte_eth_devices[port_id].state == RTE_ETH_DEV_UNUSED)
        port_id++;
...
#define RTE_ETH_FOREACH_VALID_DEV(port_id)                                                                             \
    for (port_id = rte_eth_find_next(0); port_id < RTE_MAX_ETHPORTS; port_id = rte_eth_find_next(port_id + 1))

uint16_t rte_eth_find_next_of(uint16_t port_id, const struct rte_device *parent) {
    port_id = rte_eth_find_next(port_id);
    while (port_id < RTE_MAX_ETHPORTS && rte_eth_devices[port_id].device != parent)
...
static void rte_eth_dev_shared_data_prepare(void) {
    ...
    if (rte_eth_dev_shared_data == NULL) {
        if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
            ...
            mz = rte_memzone_reserve(MZ_RTE_ETH_DEV_DATA, sizeof(*rte_eth_dev_shared_data), rte_socket_id(), flags);
        } else
        ...
        if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
            rte_eth_dev_shared_data->next_owner_id = RTE_ETH_DEV_NO_OWNER + 1;
            rte_spinlock_init(&rte_eth_dev_shared_data->ownership_lock);
            memset(rte_eth_dev_shared_data->data, 0, sizeof(rte_eth_dev_shared_data->data));
        }
    }
...
static bool is_allocated(const struct rte_eth_dev *ethdev) { return ethdev->data->name[0] != '\0'; }

static struct rte_eth_dev *_rte_eth_dev_allocated(const char *name) {
    unsigned i;

    for (i = 0; i < RTE_MAX_ETHPORTS; i++) {
        if (rte_eth_devices[i].data != NULL && strcmp(rte_eth_devices[i].data->name, name) == 0)
...
static uint16_t rte_eth_dev_find_free_port(void) {
    unsigned i;
...
int rte_eth_dev_is_valid_port(uint16_t port_id) {
    if (port_id >= RTE_MAX_ETHPORTS || (rte_eth_devices[port_id].state == RTE_ETH_DEV_UNUSED))
        return 0;
    else
        return 1;
}

static int rte_eth_is_valid_owner_id(uint64_t owner_id) {
    if (owner_id == RTE_ETH_DEV_NO_OWNER || rte_eth_dev_shared_data->next_owner_id <= owner_id)
        return 0;
    return 1;
}
...
uint64_t rte_eth_find_next_owned_by(uint16_t port_id, const uint64_t owner_id) {
    port_id = rte_eth_find_next(port_id);
    while (port_id < RTE_MAX_ETHPORTS && rte_eth_devices[port_id].data->owner.id != owner_id)
        port_id = rte_eth_find_next(port_id + 1);
...
static int _rte_eth_dev_owner_set(const uint16_t port_id, const uint64_t old_owner_id,
    ...
    if (!rte_eth_is_valid_owner_id(new_owner->id) && !rte_eth_is_valid_owner_id(old_owner_id)) {
        RTE_ETHDEV_LOG(ERR, "Invalid owner old_id=%016" PRIx64 " new_id=%016" PRIx64 "\n", old_owner_id, new_owner->id);
        return -EINVAL;
    }
...
int __rte_experimental rte_eth_dev_owner_unset(const uint16_t port_id, const uint64_t owner_id) {
    const struct rte_eth_dev_owner new_owner = (struct rte_eth_dev_owner){.id = RTE_ETH_DEV_NO_OWNER, .name = ""};
    int ret;
    ...
    if (rte_eth_is_valid_owner_id(owner_id)) {
        for (port_id = 0; port_id < RTE_MAX_ETHPORTS; port_id++)
            if (rte_eth_devices[port_id].data->owner.id == owner_id)
                memset(&rte_eth_devices[port_id].data->owner, 0, sizeof(struct rte_eth_dev_owner));
...
uint16_t rte_eth_dev_count(void) { return rte_eth_dev_count_avail(); }
...
static int rte_eth_dev_rx_queue_config(struct rte_eth_dev *dev, uint16_t nb_queues) {
    ...
    if (dev->data->rx_queues == NULL && nb_queues != 0) { /* first time configuration */
        dev->data->rx_queues =
            rte_zmalloc("ethdev->rx_queues", sizeof(dev->data->rx_queues[0]) * nb_queues, RTE_CACHE_LINE_SIZE);
...
static int rte_eth_dev_tx_queue_config(struct rte_eth_dev *dev, uint16_t nb_queues) {
    ...
    if (dev->data->tx_queues == NULL && nb_queues != 0) { /* first time configuration */
        dev->data->tx_queues =
            rte_zmalloc("ethdev->tx_queues", sizeof(dev->data->tx_queues[0]) * nb_queues, RTE_CACHE_LINE_SIZE);
...
int rte_eth_dev_configure(uint16_t port_id, uint16_t nb_rx_q, uint16_t nb_tx_q, const struct rte_eth_conf *dev_conf) {
    ...
    if (nb_rx_q > RTE_MAX_QUEUES_PER_PORT) {
        RTE_ETHDEV_LOG(ERR, "Number of RX queues requested (%u) is greater than max supported(%d)\n", nb_rx_q,
    ...
    if (nb_tx_q > RTE_MAX_QUEUES_PER_PORT) {
        RTE_ETHDEV_LOG(ERR, "Number of TX queues requested (%u) is greater than max supported(%d)\n", nb_tx_q,
                       RTE_MAX_QUEUES_PER_PORT);
        ret = -EINVAL;
        goto rollback;
    }
    ...
    if ((dev_conf->intr_conf.rmv == 1) && (!(dev->data->dev_flags & RTE_ETH_DEV_INTR_RMV))) {
    ...
    if (dev_conf->rxmode.offloads & DEV_RX_OFFLOAD_JUMBO_FRAME) {
        if (dev_conf->rxmode.max_rx_pkt_len > dev_info.max_rx_pktlen) {
            RTE_ETHDEV_LOG(ERR, "Ethdev port_id=%u max_rx_pkt_len %u > max valid value %u\n", port_id,
                           dev_conf->rxmode.max_rx_pkt_len, dev_info.max_rx_pktlen);
            ret = -EINVAL;
            goto rollback;
        } else if (dev_conf->rxmode.max_rx_pkt_len < ETHER_MIN_LEN) {
            RTE_ETHDEV_LOG(ERR, "Ethdev port_id=%u max_rx_pkt_len %u < min valid value %u\n", port_id,
                           dev_conf->rxmode.max_rx_pkt_len, (unsigned)ETHER_MIN_LEN);
            ret = -EINVAL;
            goto rollback;
        }
    } else {
    ...
    if ((dev_conf->rxmode.offloads & dev_info.rx_offload_capa) != dev_conf->rxmode.offloads) {
        RTE_ETHDEV_LOG(ERR,
                       ...
                       port_id, dev_conf->rxmode.offloads, dev_info.rx_offload_capa, __func__);
        ret = -EINVAL;
        goto rollback;
    }
    if ((dev_conf->txmode.offloads & dev_info.tx_offload_capa) != dev_conf->txmode.offloads) {
        RTE_ETHDEV_LOG(ERR,
                       ...
                       "capabilities 0x%" PRIx64 " in %s()\n",
                       port_id, dev_conf->txmode.offloads, dev_info.tx_offload_capa, __func__);
        ret = -EINVAL;
        goto rollback;
    }
...
static void rte_eth_dev_mac_restore(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info) {
...
static void rte_eth_dev_config_restore(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info, uint16_t port_id) {
...
int rte_eth_rx_queue_setup(uint16_t port_id, uint16_t rx_queue_id, uint16_t nb_rx_desc, unsigned int socket_id,
    ...
    if ((mbp_buf_size - RTE_PKTMBUF_HEADROOM) < dev_info.min_rx_bufsize) {
        RTE_ETHDEV_LOG(ERR, "%s mbuf_data_room_size %d < %d (RTE_PKTMBUF_HEADROOM=%d + min_rx_bufsize(dev)=%d)\n",
    ...
    if (nb_rx_desc > dev_info.rx_desc_lim.nb_max || nb_rx_desc < dev_info.rx_desc_lim.nb_min ||
        nb_rx_desc % dev_info.rx_desc_lim.nb_align != 0) {

        RTE_ETHDEV_LOG(ERR, "Invalid value for nb_rx_desc(=%hu), should be: <= %hu, >= %hu, and a product of %hu\n",
                       nb_rx_desc, dev_info.rx_desc_lim.nb_max, dev_info.rx_desc_lim.nb_min,
                       dev_info.rx_desc_lim.nb_align);
        return -EINVAL;
    }

    if (dev->data->dev_started && !(dev_info.dev_capa & RTE_ETH_DEV_CAPA_RUNTIME_RX_QUEUE_SETUP))
        return -EBUSY;

    if (dev->data->dev_started && (dev->data->rx_queue_state[rx_queue_id] != RTE_ETH_QUEUE_STATE_STOPPED))
        return -EBUSY;
     ...
     */
    if ((local_conf.offloads & dev_info.rx_queue_offload_capa) != local_conf.offloads) {
        RTE_ETHDEV_LOG(ERR,
                       "Ethdev port_id=%d rx_queue_id=%d, new added offloads 0x%" PRIx64 " must be "
                       "within per-queue offload capabilities 0x%" PRIx64 " in %s()\n",
                       port_id, rx_queue_id, local_conf.offloads, dev_info.rx_queue_offload_capa, __func__);
        return -EINVAL;
    }
...
int rte_eth_tx_queue_setup(uint16_t port_id, uint16_t tx_queue_id, uint16_t nb_tx_desc, unsigned int socket_id,
    ...
    if (nb_tx_desc > dev_info.tx_desc_lim.nb_max || nb_tx_desc < dev_info.tx_desc_lim.nb_min ||
        nb_tx_desc % dev_info.tx_desc_lim.nb_align != 0) {
        RTE_ETHDEV_LOG(ERR, "Invalid value for nb_tx_desc(=%hu), should be: <= %hu, >= %hu, and a product of %hu\n",
                       nb_tx_desc, dev_info.tx_desc_lim.nb_max, dev_info.tx_desc_lim.nb_min,
                       dev_info.tx_desc_lim.nb_align);
        return -EINVAL;
    }

    if (dev->data->dev_started && !(dev_info.dev_capa & RTE_ETH_DEV_CAPA_RUNTIME_TX_QUEUE_SETUP))
        return -EBUSY;

    if (dev->data->dev_started && (dev->data->tx_queue_state[tx_queue_id] != RTE_ETH_QUEUE_STATE_STOPPED))
        return -EBUSY;
     ...
     * enabled in rte_eth_dev_configure().
     */
    if ((local_conf.offloads & dev_info.tx_queue_offload_capa) != local_conf.offloads) {
        RTE_ETHDEV_LOG(ERR,
    ...
    return eth_err(port_id, (*dev->dev_ops->tx_queue_setup)(dev, tx_queue_id, nb_tx_desc, socket_id, &local_conf));
}

void rte_eth_tx_buffer_drop_callback(struct rte_mbuf **pkts, uint16_t unsent, void *userdata __rte_unused) {
    unsigned i;
...
int rte_eth_tx_buffer_set_err_callback(struct rte_eth_dev_tx_buffer *buffer, buffer_tx_error_fn cbfn, void *userdata) {
...
int rte_eth_tx_buffer_init(struct rte_eth_dev_tx_buffer *buffer, uint16_t size) {
    ...
    if (buffer->error_callback == NULL) {
        ret = rte_eth_tx_buffer_set_err_callback(buffer, rte_eth_tx_buffer_drop_callback, NULL);
    }
...
void rte_eth_link_get(uint16_t port_id, struct rte_eth_link *eth_link) {
    struct rte_eth_dev *dev;
...
void rte_eth_link_get_nowait(uint16_t port_id, struct rte_eth_link *eth_link) {
    struct rte_eth_dev *dev;
...
static inline int get_xstats_basic_count(struct rte_eth_dev *dev) {
...
static int get_xstats_count(uint16_t port_id) {
    struct rte_eth_dev *dev;
    int count;
    ...
    if (dev->dev_ops->xstats_get_names_by_id != NULL) {
        count = (*dev->dev_ops->xstats_get_names_by_id)(dev, NULL, NULL, 0);
        if (count < 0)
...
int rte_eth_xstats_get_id_by_name(uint16_t port_id, const char *xstat_name, uint64_t *id) {
    ...
    if (cnt_xstats != rte_eth_xstats_get_names_by_id(port_id, xstats_names, cnt_xstats, NULL)) {
...
static int rte_eth_basic_stats_get_names(struct rte_eth_dev *dev, struct rte_eth_xstat_name *xstats_names) {
    int cnt_used_entries = 0;
    uint32_t idx, id_queue;
    uint16_t num_q;

    for (idx = 0; idx < RTE_NB_STATS; idx++) {
        strlcpy(xstats_names[cnt_used_entries].name, rte_stats_strings[idx].name, sizeof(xstats_names[0].name));
        cnt_used_entries++;
    }
    num_q = RTE_MIN(dev->data->nb_rx_queues, RTE_ETHDEV_QUEUE_STAT_CNTRS);
    for (id_queue = 0; id_queue < num_q; id_queue++) {
        for (idx = 0; idx < RTE_NB_RXQ_STATS; idx++) {
            snprintf(xstats_names[cnt_used_entries].name, sizeof(xstats_names[0].name), "rx_q%u%s", id_queue,
    ...
    for (id_queue = 0; id_queue < num_q; id_queue++) {
        for (idx = 0; idx < RTE_NB_TXQ_STATS; idx++) {
            snprintf(xstats_names[cnt_used_entries].name, sizeof(xstats_names[0].name), "tx_q%u%s", id_queue,
...
int rte_eth_xstats_get_names_by_id(uint16_t port_id, struct rte_eth_xstat_name *xstats_names, unsigned int size,
                                   uint64_t *ids) {
    ...
    if (ids && dev->dev_ops->xstats_get_names_by_id != NULL && size > 0) {
        ...
        if (no_basic_stat_requested)
            return (*dev->dev_ops->xstats_get_names_by_id)(dev, xstats_names, ids_copy, size);
    }

    /* Retrieve all stats */
    if (!ids) {
        int num_stats = rte_eth_xstats_get_names(port_id, xstats_names, expected_entries);
    ...
    xstats_names_copy = calloc(expected_entries, sizeof(struct rte_eth_xstat_name));
    ...
    if (ids && no_ext_stat_requested) {
        ...
        ret = rte_eth_xstats_get_names(port_id, xstats_names_copy, expected_entries);
        if (ret < 0) {
...
int rte_eth_xstats_get_names(uint16_t port_id, struct rte_eth_xstat_name *xstats_names, unsigned int size) {
    ...
    if (dev->dev_ops->xstats_get_names != NULL) {
         ...
         */
        cnt_driver_entries =
            (*dev->dev_ops->xstats_get_names)(dev, xstats_names + cnt_used_entries, size - cnt_used_entries);
...
static int rte_eth_basic_stats_get(uint16_t port_id, struct rte_eth_xstat *xstats) {
    struct rte_eth_dev *dev;
    ...
    for (i = 0; i < RTE_NB_STATS; i++) {
        stats_ptr = RTE_PTR_ADD(&eth_stats, rte_stats_strings[i].offset);
        val = *stats_ptr;
    ...
    for (q = 0; q < nb_rxqs; q++) {
        for (i = 0; i < RTE_NB_RXQ_STATS; i++) {
            stats_ptr = RTE_PTR_ADD(&eth_stats, rte_rxq_stats_strings[i].offset + q * sizeof(uint64_t));
            val = *stats_ptr;
            xstats[count++].value = val;
        }
    }
    ...
    for (q = 0; q < nb_txqs; q++) {
        for (i = 0; i < RTE_NB_TXQ_STATS; i++) {
            stats_ptr = RTE_PTR_ADD(&eth_stats, rte_txq_stats_strings[i].offset + q * sizeof(uint64_t));
            val = *stats_ptr;
            xstats[count++].value = val;
        }
    }
...
int rte_eth_xstats_get(uint16_t port_id, struct rte_eth_xstat *xstats, unsigned int n) {
    ...
    count = RTE_NB_STATS + (nb_rxqs * RTE_NB_RXQ_STATS) + (nb_txqs * RTE_NB_TXQ_STATS);
    ...
    if (dev->dev_ops->xstats_get != NULL) {
         ...
         * xstats struct.
         */
        xcount = (*dev->dev_ops->xstats_get)(dev, xstats ? xstats + count : NULL, (n > count) ? n - count : 0);
...
static int set_queue_stats_mapping(uint16_t port_id, uint16_t queue_id, uint8_t stat_idx, uint8_t is_rx) {
    struct rte_eth_dev *dev;
...
int rte_eth_dev_set_rx_queue_stats_mapping(uint16_t port_id, uint16_t rx_queue_id, uint8_t stat_idx) {
    return eth_err(port_id, set_queue_stats_mapping(port_id, rx_queue_id, stat_idx, STAT_QMAP_RX));
}
...
int rte_eth_dev_fw_version_get(uint16_t port_id, char *fw_version, size_t fw_size) {
    ...
    return eth_err(port_id, (*dev->dev_ops->fw_version_get)(dev, fw_version, fw_size));
}
...
int rte_eth_dev_set_vlan_ether_type(uint16_t port_id, enum rte_vlan_type vlan_type, uint16_t tpid) {
    struct rte_eth_dev *dev;
...
int rte_eth_dev_set_vlan_offload(uint16_t port_id, int offload_mask) {
    ...
    org = !!(dev->data->dev_conf.rxmode.offloads & DEV_RX_OFFLOAD_VLAN_STRIP);
    if (cur != org) {
        if (cur)
            dev->data->dev_conf.rxmode.offloads |= DEV_RX_OFFLOAD_VLAN_STRIP;
        else
            dev->data->dev_conf.rxmode.offloads &= ~DEV_RX_OFFLOAD_VLAN_STRIP;
    ...
    org = !!(dev->data->dev_conf.rxmode.offloads & DEV_RX_OFFLOAD_VLAN_FILTER);
    if (cur != org) {
        if (cur)
            dev->data->dev_conf.rxmode.offloads |= DEV_RX_OFFLOAD_VLAN_FILTER;
        else
            dev->data->dev_conf.rxmode.offloads &= ~DEV_RX_OFFLOAD_VLAN_FILTER;
    ...
    org = !!(dev->data->dev_conf.rxmode.offloads & DEV_RX_OFFLOAD_VLAN_EXTEND);
    if (cur != org) {
        if (cur)
            dev->data->dev_conf.rxmode.offloads |= DEV_RX_OFFLOAD_VLAN_EXTEND;
        else
            dev->data->dev_conf.rxmode.offloads &= ~DEV_RX_OFFLOAD_VLAN_EXTEND;
...
int rte_eth_dev_get_vlan_offload(uint16_t port_id) {
    ...
    if (dev->data->dev_conf.rxmode.offloads & DEV_RX_OFFLOAD_VLAN_EXTEND)
...
int rte_eth_dev_priority_flow_ctrl_set(uint16_t port_id, struct rte_eth_pfc_conf *pfc_conf) {
    ...
    if (*dev->dev_ops->priority_flow_ctrl_set)
        return eth_err(port_id, (*dev->dev_ops->priority_flow_ctrl_set)(dev, pfc_conf));
    return -ENOTSUP;
}

static int rte_eth_check_reta_mask(struct rte_eth_rss_reta_entry64 *reta_conf, uint16_t reta_size) {
    uint16_t i, num;
...
static int rte_eth_check_reta_entry(struct rte_eth_rss_reta_entry64 *reta_conf, uint16_t reta_size, uint16_t max_rxq) {
    ...
    for (i = 0; i < reta_size; i++) {
        ...
        if ((reta_conf[idx].mask & (1ULL << shift)) && (reta_conf[idx].reta[shift] >= max_rxq)) {
            RTE_ETHDEV_LOG(ERR, "reta_conf[%u]->reta[%u]: %u exceeds the maximum rxq index: %u\n", idx, shift,
...
int rte_eth_dev_rss_reta_update(uint16_t port_id, struct rte_eth_rss_reta_entry64 *reta_conf, uint16_t reta_size) {
    ...
    /* Check entry value */
    ret = rte_eth_check_reta_entry(reta_conf, reta_size, dev->data->nb_rx_queues);
    if (ret < 0)
        return ret;

    RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->reta_update, -ENOTSUP);
    return eth_err(port_id, (*dev->dev_ops->reta_update)(dev, reta_conf, reta_size));
}
...
int rte_eth_dev_rss_reta_query(uint16_t port_id, struct rte_eth_rss_reta_entry64 *reta_conf, uint16_t reta_size) {
    ...
    return eth_err(port_id, (*dev->dev_ops->reta_query)(dev, reta_conf, reta_size));
}
...
int rte_eth_dev_rss_hash_update(uint16_t port_id, struct rte_eth_rss_conf *rss_conf) {
    ...
    dev = &rte_eth_devices[port_id];
    rte_eth_dev_info_get(port_id, &dev_info);
    if ((dev_info.flow_type_rss_offloads | rss_conf->rss_hf) != dev_info.flow_type_rss_offloads) {
        RTE_ETHDEV_LOG(ERR, "Ethdev port_id=%u invalid rss_hf: 0x%" PRIx64 ", valid value: 0x%" PRIx64 "\n", port_id,
...
int rte_eth_dev_udp_tunnel_port_add(uint16_t port_id, struct rte_eth_udp_tunnel *udp_tunnel) {
    struct rte_eth_dev *dev;
    ...
    return eth_err(port_id, (*dev->dev_ops->udp_tunnel_port_add)(dev, udp_tunnel));
}

int rte_eth_dev_udp_tunnel_port_delete(uint16_t port_id, struct rte_eth_udp_tunnel *udp_tunnel) {
    struct rte_eth_dev *dev;
    ...
    return eth_err(port_id, (*dev->dev_ops->udp_tunnel_port_del)(dev, udp_tunnel));
}
 ...
 * an empty spot.
 */
static int get_mac_addr_index(uint16_t port_id, const struct ether_addr *addr) {
    struct rte_eth_dev_info dev_info;
    struct rte_eth_dev *dev = &rte_eth_devices[port_id];
    unsigned i;
...
int rte_eth_dev_mac_addr_add(uint16_t port_id, struct ether_addr *addr, uint32_t pool) {
    struct rte_eth_dev *dev;
    int index;
    uint64_t pool_mask;
    int ret;
...
int rte_eth_dev_mac_addr_remove(uint16_t port_id, struct ether_addr *addr) {
    struct rte_eth_dev *dev;
    int index;
...
int rte_eth_dev_default_mac_addr_set(uint16_t port_id, struct ether_addr *addr) {
    struct rte_eth_dev *dev;
    int ret;
 ...
 * an empty spot.
 */
static int get_hash_mac_addr_index(uint16_t port_id, const struct ether_addr *addr) {
    struct rte_eth_dev_info dev_info;
    struct rte_eth_dev *dev = &rte_eth_devices[port_id];
    unsigned i;
    ...
    for (i = 0; i < dev_info.max_hash_mac_addrs; i++)
        if (memcmp(addr, &dev->data->hash_mac_addrs[i], ETHER_ADDR_LEN) == 0)
            return i;
...
int rte_eth_dev_uc_hash_table_set(uint16_t port_id, struct ether_addr *addr, uint8_t on) {
    ...
    if (ret == 0) {
        /* Update address in NIC data structure */
        if (on)
            ether_addr_copy(addr, &dev->data->hash_mac_addrs[index]);
        else
            ether_addr_copy(&null_mac_addr, &dev->data->hash_mac_addrs[index]);
    }
...
int rte_eth_set_queue_rate_limit(uint16_t port_id, uint16_t queue_idx, uint16_t tx_rate) {
    ...
    return eth_err(port_id, (*dev->dev_ops->set_queue_rate_limit)(dev, queue_idx, tx_rate));
}
...
int rte_eth_mirror_rule_set(uint16_t port_id, struct rte_eth_mirror_conf *mirror_conf, uint8_t rule_id, uint8_t on) {
    ...
    if ((mirror_conf->rule_type & (ETH_MIRROR_VIRTUAL_POOL_UP | ETH_MIRROR_VIRTUAL_POOL_DOWN)) &&
        (mirror_conf->pool_mask == 0)) {
        RTE_ETHDEV_LOG(ERR, "Invalid mirror pool, pool mask can not be 0\n");
        return -EINVAL;
    }

    if ((mirror_conf->rule_type & ETH_MIRROR_VLAN) && mirror_conf->vlan.vlan_mask == 0) {
    ...
    return eth_err(port_id, (*dev->dev_ops->mirror_rule_set)(dev, mirror_conf, rule_id, on));
}
...
int rte_eth_dev_callback_register(uint16_t port_id, enum rte_eth_event_type event, rte_eth_dev_cb_fn cb_fn,
    ...
    do {
        ...
        TAILQ_FOREACH(user_cb, &(dev->link_intr_cbs), next) {
            if (user_cb->cb_fn == cb_fn && user_cb->cb_arg == cb_arg && user_cb->event == event) {
                break;
            }
        }
        ...
        if (user_cb == NULL) {
            user_cb = rte_zmalloc("INTR_USER_CALLBACK", sizeof(struct rte_eth_dev_callback), 0);
            if (user_cb != NULL) {
                user_cb->cb_fn = cb_fn;
...
int rte_eth_dev_callback_unregister(uint16_t port_id, enum rte_eth_event_type event, rte_eth_dev_cb_fn cb_fn,
                                    void *cb_arg) {
    int ret;
    struct rte_eth_dev *dev;
    ...
    do {
        ...
        for (cb = TAILQ_FIRST(&dev->link_intr_cbs); cb != NULL; cb = next) {
            ...
            if (cb->cb_fn != cb_fn || cb->event != event || (cb->cb_arg != (void *)-1 && cb->cb_arg != cb_arg))
                continue;
...
int _rte_eth_dev_callback_process(struct rte_eth_dev *dev, enum rte_eth_event_type event, void *ret_param) {
    ...
    TAILQ_FOREACH(cb_lst, &(dev->link_intr_cbs), next) {
        ...
        rte_spinlock_unlock(&rte_eth_dev_cb_lock);
        rc = dev_cb.cb_fn(dev->data->port_id, dev_cb.event, dev_cb.cb_arg, dev_cb.ret_param);
        rte_spinlock_lock(&rte_eth_dev_cb_lock);
        cb_lst->active = 0;
    }
    rte_spinlock_unlock(&rte_eth_dev_cb_lock);
...
int __rte_experimental rte_eth_dev_rx_intr_ctl_q_get_fd(uint16_t port_id, uint16_t queue_id) {
    ...
    efd_idx = (vec >= RTE_INTR_VEC_RXTX_OFFSET) ? (vec - RTE_INTR_VEC_RXTX_OFFSET) : vec;
    fd = intr_handle->efds[efd_idx];

    return fd;
}

const struct rte_memzone *rte_eth_dma_zone_reserve(const struct rte_eth_dev *dev, const char *ring_name,
                                                   uint16_t queue_id, size_t size, unsigned align, int socket_id) {
    char z_name[RTE_MEMZONE_NAMESIZE];
    const struct rte_memzone *mz;
    int rc;

    rc = snprintf(z_name, sizeof(z_name), "eth_p%d_q%d_%s", dev->data->port_id, queue_id, ring_name);
    ...
    return rte_memzone_reserve_aligned(z_name, size, socket_id, RTE_MEMZONE_IOVA_CONTIG, align);
}

int __rte_experimental rte_eth_dev_create(struct rte_device *device, const char *name, size_t priv_data_size,
                                          ethdev_bus_specific_init ethdev_bus_specific_init, void *bus_init_params,
    ...
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
        ...
        if (priv_data_size) {
            ethdev->data->dev_private =
                rte_zmalloc_socket(name, priv_data_size, RTE_CACHE_LINE_SIZE, device->numa_node);
    ...
    if (ethdev_bus_specific_init) {
        ...
        if (retval) {
            RTE_LOG(ERR, EAL, "ethdev bus specific initialisation failed");
            goto probe_failed;
        }
    }
...
int rte_eth_dev_filter_supported(uint16_t port_id, enum rte_filter_type filter_type) {
    ...
    return (*dev->dev_ops->filter_ctrl)(dev, filter_type, RTE_ETH_FILTER_NOP, NULL);
}

int rte_eth_dev_filter_ctrl(uint16_t port_id, enum rte_filter_type filter_type, enum rte_filter_op filter_op,
                            void *arg) {
    struct rte_eth_dev *dev;
    ...
    dev = &rte_eth_devices[port_id];
    RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->filter_ctrl, -ENOTSUP);
    return eth_err(port_id, (*dev->dev_ops->filter_ctrl)(dev, filter_type, filter_op, arg));
}

const struct rte_eth_rxtx_callback *rte_eth_add_rx_callback(uint16_t port_id, uint16_t queue_id, rte_rx_callback_fn fn,
                                                            void *user_param) {
    ...
    if (!rte_eth_dev_is_valid_port(port_id) || fn == NULL || queue_id >= rte_eth_devices[port_id].data->nb_rx_queues) {
        rte_errno = EINVAL;
        return NULL;
    }
...
const struct rte_eth_rxtx_callback *rte_eth_add_tx_callback(uint16_t port_id, uint16_t queue_id, rte_tx_callback_fn fn,
                                                            void *user_param) {
#ifndef RTE_ETHDEV_RXTX_CALLBACKS
    rte_errno = ENOTSUP;
    return NULL;
#endif
    /* check input parameters */
    if (!rte_eth_dev_is_valid_port(port_id) || fn == NULL || queue_id >= rte_eth_devices[port_id].data->nb_tx_queues) {
        rte_errno = EINVAL;
        return NULL;
    }
    ...
    /* Add the callbacks in fifo order. */
    struct rte_eth_rxtx_callback *tail = rte_eth_devices[port_id].pre_tx_burst_cbs[queue_id];
...
int rte_eth_remove_rx_callback(uint16_t port_id, uint16_t queue_id, const struct rte_eth_rxtx_callback *user_cb) {
    ...
    /* Check input parameters. */
    RTE_ETH_VALID_PORTID_OR_ERR_RET(port_id, -EINVAL);
    if (user_cb == NULL || queue_id >= rte_eth_devices[port_id].data->nb_rx_queues)
        return -EINVAL;
...
int rte_eth_remove_tx_callback(uint16_t port_id, uint16_t queue_id, const struct rte_eth_rxtx_callback *user_cb) {
    ...
    /* Check input parameters. */
    RTE_ETH_VALID_PORTID_OR_ERR_RET(port_id, -EINVAL);
    if (user_cb == NULL || queue_id >= rte_eth_devices[port_id].data->nb_tx_queues)
        return -EINVAL;
...
int rte_eth_dev_set_mc_addr_list(uint16_t port_id, struct ether_addr *mc_addr_set, uint32_t nb_mc_addr) {
    ...
    return eth_err(port_id, dev->dev_ops->set_mc_addr_list(dev, mc_addr_set, nb_mc_addr));
}
...
int rte_eth_timesync_read_tx_timestamp(uint16_t port_id, struct timespec *timestamp) {
    ...
    return eth_err(port_id, (*dev->dev_ops->timesync_read_tx_timestamp)(dev, timestamp));
}
...
int rte_eth_timesync_write_time(uint16_t port_id, const struct timespec *timestamp) {
    struct rte_eth_dev *dev;
...
int rte_eth_dev_l2_tunnel_eth_type_conf(uint16_t port_id, struct rte_eth_l2_tunnel_conf *l2_tunnel) {
    ...
    dev = &rte_eth_devices[port_id];
    RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->l2_tunnel_eth_type_conf, -ENOTSUP);
    return eth_err(port_id, (*dev->dev_ops->l2_tunnel_eth_type_conf)(dev, l2_tunnel));
}

int rte_eth_dev_l2_tunnel_offload_set(uint16_t port_id, struct rte_eth_l2_tunnel_conf *l2_tunnel, uint32_t mask,
                                      uint8_t en) {
    struct rte_eth_dev *dev;
    ...
    dev = &rte_eth_devices[port_id];
    RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->l2_tunnel_offload_set, -ENOTSUP);
    return eth_err(port_id, (*dev->dev_ops->l2_tunnel_offload_set)(dev, l2_tunnel, mask, en));
}

static void rte_eth_dev_adjust_nb_desc(uint16_t *nb_desc, const struct rte_eth_desc_lim *desc_lim) {
 ...
 */
enum rte_eth_switch_domain_state { RTE_ETH_SWITCH_DOMAIN_UNUSED = 0, RTE_ETH_SWITCH_DOMAIN_ALLOCATED };
 ...
 */
static struct rte_eth_dev_switch { enum rte_eth_switch_domain_state state; } rte_eth_switch_domains[RTE_MAX_ETHPORTS];
...
int __rte_experimental rte_eth_switch_domain_alloc(uint16_t *domain_id) {
    ...
    *domain_id = RTE_ETH_DEV_SWITCH_DOMAIN_ID_INVALID;
    ...
    for (i = RTE_ETH_DEV_SWITCH_DOMAIN_ID_INVALID + 1; i < RTE_MAX_ETHPORTS; i++) {
        if (rte_eth_switch_domains[i].state == RTE_ETH_SWITCH_DOMAIN_UNUSED) {
            rte_eth_switch_domains[i].state = RTE_ETH_SWITCH_DOMAIN_ALLOCATED;
            *domain_id = i;
            return 0;
        }
    }
...
int __rte_experimental rte_eth_switch_domain_free(uint16_t domain_id) {
    if (domain_id == RTE_ETH_DEV_SWITCH_DOMAIN_ID_INVALID || domain_id >= RTE_MAX_ETHPORTS)
        return -EINVAL;

    if (rte_eth_switch_domains[domain_id].state != RTE_ETH_SWITCH_DOMAIN_ALLOCATED)
        return -EINVAL;
...
static int rte_eth_devargs_tokenise(struct rte_kvargs *arglist, const char *str_in) {
    int state;
...
int __rte_experimental rte_eth_devargs_parse(const char *dargs, struct rte_eth_devargs *eth_da) {
    ...
    for (i = 0; i < args.count; i++) {
        ...
        if (strcmp("representor", pair->key) == 0) {
            result = rte_eth_devargs_parse_list(pair->value, rte_eth_devargs_parse_representor_ports, eth_da);
            if (result < 0)
                goto parse_cleanup;
        }
    }
