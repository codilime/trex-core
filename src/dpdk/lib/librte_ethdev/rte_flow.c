...
static const struct rte_flow_desc_data rte_flow_desc_item[] = {
    ...
    MK_FLOW_ITEM(ICMP6_ND_OPT_SLA_ETH, sizeof(struct rte_flow_item_icmp6_nd_opt_sla_eth)),
    MK_FLOW_ITEM(ICMP6_ND_OPT_TLA_ETH, sizeof(struct rte_flow_item_icmp6_nd_opt_tla_eth)),
...
static const struct rte_flow_desc_data rte_flow_desc_action[] = {
    ...
    MK_FLOW_ACTION(OF_SET_MPLS_TTL, sizeof(struct rte_flow_action_of_set_mpls_ttl)),
    ...
    MK_FLOW_ACTION(OF_SET_VLAN_VID, sizeof(struct rte_flow_action_of_set_vlan_vid)),
    MK_FLOW_ACTION(OF_SET_VLAN_PCP, sizeof(struct rte_flow_action_of_set_vlan_pcp)),
...
static int flow_err(uint16_t port_id, int ret, struct rte_flow_error *error) {
    ...
    if (rte_eth_dev_is_removed(port_id))
        return rte_flow_error_set(error, EIO, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, rte_strerror(EIO));
    return ret;
}
...
const struct rte_flow_ops *rte_flow_ops_get(uint16_t port_id, struct rte_flow_error *error) {
    ...
    if (unlikely(!rte_eth_dev_is_valid_port(port_id)))
        code = ENODEV;
                      ...
                      dev->dev_ops->filter_ctrl(dev, RTE_ETH_FILTER_GENERIC, RTE_ETH_FILTER_GET, &ops) || !ops))
        code = ENOSYS;
    else
        return ops;
    rte_flow_error_set(error, code, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, rte_strerror(code));
    return NULL;
}
...
struct rte_flow *rte_flow_create(uint16_t port_id, const struct rte_flow_attr *attr,
...
int rte_flow_destroy(uint16_t port_id, struct rte_flow *flow, struct rte_flow_error *error) {
    ...
    if (likely(!!ops->destroy))
        return flow_err(port_id, ops->destroy(dev, flow, error), error);
    return rte_flow_error_set(error, ENOSYS, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, rte_strerror(ENOSYS));
}
...
int rte_flow_query(uint16_t port_id, struct rte_flow *flow, const struct rte_flow_action *action, void *data,
                   struct rte_flow_error *error) {
    struct rte_eth_dev *dev = &rte_eth_devices[port_id];
    const struct rte_flow_ops *ops = rte_flow_ops_get(port_id, error);
    ...
    if (!ops)
        return -rte_errno;
    if (likely(!!ops->query))
        return flow_err(port_id, ops->query(dev, flow, action, data, error), error);
    return rte_flow_error_set(error, ENOSYS, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL, rte_strerror(ENOSYS));
}
...
int rte_flow_error_set(struct rte_flow_error *error, int code, enum rte_flow_error_type type, const void *cause,
                       const char *message) {
    if (error) {
 ...
 *   enough.
 */
static size_t rte_flow_conv_item_spec(void *buf, const size_t size, const struct rte_flow_item *item,
                                      enum rte_flow_conv_item_spec_type type) {
    size_t off;
    const void *data =
        type == RTE_FLOW_CONV_ITEM_SPEC
            ? item->spec
            : type == RTE_FLOW_CONV_ITEM_LAST ? item->last : type == RTE_FLOW_CONV_ITEM_MASK ? item->mask : NULL;
    ...
    case RTE_FLOW_ITEM_TYPE_RAW:
        ...
        if (type == RTE_FLOW_CONV_ITEM_SPEC ||
            (type == RTE_FLOW_CONV_ITEM_MASK &&
             ((spec.raw->length & mask.raw->length) >= (last.raw->length & mask.raw->length))))
        ...
        if (tmp) {
            ...
            if (size >= off + tmp)
                dst.raw->pattern = rte_memcpy((void *)((uintptr_t)dst.raw + off), src.raw->pattern, tmp);
            off += tmp;
        }
        break;
    default:
 ...
 *   enough.
 */
static size_t rte_flow_conv_action_conf(void *buf, const size_t size, const struct rte_flow_action *action) {
    size_t off;
    ...
    case RTE_FLOW_ACTION_TYPE_RSS:
        ...
        if (src.rss->key_len) {
            ...
            if (size >= off + tmp)
                dst.rss->key = rte_memcpy((void *)((uintptr_t)dst.rss + off), src.rss->key, tmp);
            off += tmp;
        }
        if (src.rss->queue_num) {
            ...
            if (size >= off + tmp)
                dst.rss->queue = rte_memcpy((void *)((uintptr_t)dst.rss + off), src.rss->queue, tmp);
            off += tmp;
        }
        break;
    ...
    case RTE_FLOW_ACTION_TYPE_NVGRE_ENCAP:
        ...
        dst.vxlan_encap = buf;
        RTE_BUILD_BUG_ON(sizeof(*src.vxlan_encap) != sizeof(*src.nvgre_encap) ||
        ...
        if (src.vxlan_encap->definition) {
            ...
            ret = rte_flow_conv(RTE_FLOW_CONV_OP_PATTERN, (void *)((uintptr_t)dst.vxlan_encap + off),
                                size > off ? size - off : 0, src.vxlan_encap->definition, NULL);
            if (ret < 0)
                return 0;
            if (size >= off + ret)
                dst.vxlan_encap->definition = (void *)((uintptr_t)dst.vxlan_encap + off);
            off += ret;
        }
        break;
    default:
 ...
 */
static int rte_flow_conv_pattern(struct rte_flow_item *dst, const size_t size, const struct rte_flow_item *src,
                                 unsigned int num, struct rte_flow_error *error) {
    ...
    for (i = 0, off = 0; !num || i != num; ++i, ++src, ++dst) {
        if ((size_t)src->type >= RTE_DIM(rte_flow_desc_item) || !rte_flow_desc_item[src->type].name)
    ...
    do {
        if (src->spec) {
            ...
            ret = rte_flow_conv_item_spec((void *)(data + off), size > off ? size - off : 0, src,
                                          RTE_FLOW_CONV_ITEM_SPEC);
 ...
 *   rte_errno is set.
 */
static int rte_flow_conv_actions(struct rte_flow_action *dst, const size_t size, const struct rte_flow_action *src,
                                 unsigned int num, struct rte_flow_error *error) {
    uintptr_t data = (uintptr_t)dst;
    size_t off;
    size_t ret;
    ...
    for (i = 0, off = 0; !num || i != num; ++i, ++src, ++dst) {
        if ((size_t)src->type >= RTE_DIM(rte_flow_desc_action) || !rte_flow_desc_action[src->type].name)
    ...
    do {
        if (src->conf) {
            off = RTE_ALIGN_CEIL(off, sizeof(double));
            ret = rte_flow_conv_action_conf((void *)(data + off), size > off ? size - off : 0, src);
            if (size && size >= off + ret)
 ...
 */
static int rte_flow_conv_rule(struct rte_flow_conv_rule *dst, const size_t size, const struct rte_flow_conv_rule *src,
                              struct rte_flow_error *error) {
    size_t off;
    int ret;
    ...
    if (src->attr_ro) {
        ...
        if (size && size >= off + sizeof(*dst->attr))
            dst->attr = rte_memcpy((void *)((uintptr_t)dst + off), src->attr_ro, sizeof(*dst->attr));
        off += sizeof(*dst->attr);
    }
    if (src->pattern_ro) {
        off = RTE_ALIGN_CEIL(off, sizeof(double));
        ret = rte_flow_conv_pattern((void *)((uintptr_t)dst + off), size > off ? size - off : 0, src->pattern_ro, 0,
                                    error);
        if (ret < 0)
            return ret;
    ...
    if (src->actions_ro) {
        off = RTE_ALIGN_CEIL(off, sizeof(double));
        ret = rte_flow_conv_actions((void *)((uintptr_t)dst + off), size > off ? size - off : 0, src->actions_ro, 0,
                                    error);
        if (ret < 0)
            return ret;
 ...
 *   otherwise and rte_errno is set.
 */
static int rte_flow_conv_name(int is_action, int is_ptr, char *dst, const size_t size, const void *src,
                              struct rte_flow_error *error) {
    struct desc_info {
...
int rte_flow_conv(enum rte_flow_conv_op op, void *dst, size_t size, const void *src, struct rte_flow_error *error) {
    switch (op) {
...
size_t rte_flow_copy(struct rte_flow_desc *desc, size_t len, const struct rte_flow_attr *attr,
     ...
     */
    struct rte_flow_conv_rule *dst = len ? (void *)((uintptr_t)desc + (offsetof(struct rte_flow_desc, actions) -
                                                                       offsetof(struct rte_flow_conv_rule, actions)))
                                         : NULL;
    size_t dst_size = len > sizeof(*desc) - sizeof(*dst) ? len - (sizeof(*desc) - sizeof(*dst)) : 0;
    ...
    RTE_BUILD_BUG_ON(sizeof(struct rte_flow_desc) < sizeof(struct rte_flow_conv_rule));
    if (dst_size && (&dst->pattern != &desc->items || &dst->actions != &desc->actions ||
 ...
 */
int __rte_experimental rte_flow_expand_rss(struct rte_flow_expand_rss *buf, size_t size,
                                           const struct rte_flow_item *pattern, uint64_t types,
                                           const struct rte_flow_expand_node graph[], int graph_root_index) {
    const int elt_n = 8;
    ...
    lsize = offsetof(struct rte_flow_expand_rss, entry) + elt_n * sizeof(buf->entry[0]);
    if (lsize <= size) {
    ...
    while (node) {
        ...
        if (node->rss_types & types) {
            ...
            if (lsize <= size) {
                ...
                buf->entry[buf->entries].priority = stack_pos + 1;
