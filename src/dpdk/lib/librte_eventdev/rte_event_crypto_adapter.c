...
static inline int eca_valid_id(uint8_t id) { return id < RTE_EVENT_CRYPTO_ADAPTER_MAX_INSTANCE; }
...
static inline struct rte_event_crypto_adapter *eca_id_to_adapter(uint8_t id) {
    return event_crypto_adapter ? event_crypto_adapter[id] : NULL;
}

static int eca_default_config_cb(uint8_t id, uint8_t dev_id, struct rte_event_crypto_adapter_conf *conf, void *arg) {
...
int rte_event_crypto_adapter_create_ext(uint8_t id, uint8_t dev_id, rte_event_crypto_adapter_conf_cb conf_cb,
    ...
    snprintf(mem_name, CRYPTO_ADAPTER_MEM_NAME_LEN, "rte_event_crypto_adapter_%d", id);

    adapter = rte_zmalloc_socket(mem_name, sizeof(*adapter), RTE_CACHE_LINE_SIZE, socket_id);
    if (adapter == NULL) {
    ...
    if (ret < 0) {
        RTE_EDEV_LOG_ERR("Failed to get info for eventdev %d: %s!", dev_id, dev_info.driver_name);
        return ret;
    }

    adapter->implicit_release_disabled = (dev_info.event_dev_cap & RTE_EVENT_DEV_CAP_IMPLICIT_RELEASE_DISABLE);
    ...
    adapter->cdevs =
        rte_zmalloc_socket(adapter->mem_name, rte_cryptodev_count() * sizeof(struct crypto_device_info), 0, socket_id);
...
int rte_event_crypto_adapter_create(uint8_t id, uint8_t dev_id, struct rte_event_port_conf *port_config,
    ...
    if (pc == NULL)
        return -ENOMEM;
    *pc = *port_config;
    ret = rte_event_crypto_adapter_create_ext(id, dev_id, eca_default_config_cb, mode, pc);
    if (ret)
        rte_free(pc);
...
static inline unsigned int eca_enq_to_cryptodev(struct rte_event_crypto_adapter *adapter, struct rte_event *ev,
                                                unsigned int cnt) {
    ...
    for (i = 0; i < cnt; i++) {
        ...
        if (crypto_op->sess_type == RTE_CRYPTO_OP_WITH_SESSION) {
            ...
            len++;
        } else if (crypto_op->sess_type == RTE_CRYPTO_OP_SESSIONLESS && crypto_op->private_data_offset) {
            m_data = (union rte_event_crypto_metadata *)((uint8_t *)crypto_op + crypto_op->private_data_offset);
            cdev_id = m_data->request_info.cdev_id;
            qp_id = m_data->request_info.queue_pair_id;
            qp_info = &adapter->cdevs[cdev_id].qpairs[qp_id];
            if (!qp_info->qp_enabled) {
        ...
        if (len == BATCH_SIZE) {
            ...
            ret = rte_cryptodev_enqueue_burst(cdev_id, qp_id, op_buffer, BATCH_SIZE);
...
static unsigned int eca_crypto_enq_flush(struct rte_event_crypto_adapter *adapter) {
    struct rte_event_crypto_adapter_stats *stats = &adapter->crypto_stats;
    ...
    for (cdev_id = 0; cdev_id < num_cdev; cdev_id++) {
        ...
        for (qp = 0; qp < dev->data->nb_queue_pairs; qp++) {
            ...
            ret = rte_cryptodev_enqueue_burst(cdev_id, qp, op_buffer, curr_queue->len);
            stats->crypto_enq_count += ret;
...
static int eca_crypto_adapter_enq_run(struct rte_event_crypto_adapter *adapter, unsigned int max_enq) {
    struct rte_event_crypto_adapter_stats *stats = &adapter->crypto_stats;
    ...
    if ((++adapter->transmit_loop_count & (CRYPTO_ENQ_FLUSH_THRESHOLD - 1)) == 0) {
...
static inline void eca_ops_enqueue_burst(struct rte_event_crypto_adapter *adapter, struct rte_crypto_op **ops,
                                         uint16_t num) {
    struct rte_event_crypto_adapter_stats *stats = &adapter->crypto_stats;
    union rte_event_crypto_metadata *m_data = NULL;
    uint8_t event_dev_id = adapter->eventdev_id;
    ...
    for (i = 0; i < num; i++) {
        ...
        } else if (ops[i]->sess_type == RTE_CRYPTO_OP_SESSIONLESS && ops[i]->private_data_offset) {
            m_data = (union rte_event_crypto_metadata *)((uint8_t *)ops[i] + ops[i]->private_data_offset);
        }
    ...
    do {
        nb_enqueued += rte_event_enqueue_burst(event_dev_id, event_port_id, &events[nb_enqueued], nb_ev - nb_enqueued);
...
static inline unsigned int eca_crypto_adapter_deq_run(struct rte_event_crypto_adapter *adapter, unsigned int max_deq) {
    struct rte_event_crypto_adapter_stats *stats = &adapter->crypto_stats;
    struct crypto_device_info *curr_dev;
    struct crypto_queue_pair_info *curr_queue;
    ...
    do {
        ...
        for (cdev_id = adapter->next_cdev_id; cdev_id < num_cdev; cdev_id++) {
            ...
            for (qp = curr_dev->next_queue_pair_id; queues < dev_qps; qp = (qp + 1) % dev_qps, queues++) {
...
static void eca_crypto_adapter_run(struct rte_event_crypto_adapter *adapter, unsigned int max_ops) {
    while (max_ops) {
...
static int eca_init_service(struct rte_event_crypto_adapter *adapter, uint8_t id) {
    ...
    snprintf(service.name, CRYPTO_ADAPTER_NAME_LEN, "rte_event_crypto_adapter_%d", id);
...
static void eca_update_qp_info(struct rte_event_crypto_adapter *adapter, struct crypto_device_info *dev_info,
...
static int eca_add_queue_pair(struct rte_event_crypto_adapter *adapter, uint8_t cdev_id, int queue_pair_id) {
    struct crypto_device_info *dev_info = &adapter->cdevs[cdev_id];
    struct crypto_queue_pair_info *qpairs;
    uint32_t i;

    if (dev_info->qpairs == NULL) {
        dev_info->qpairs = rte_zmalloc_socket(
            adapter->mem_name, dev_info->dev->data->nb_queue_pairs * sizeof(struct crypto_queue_pair_info), 0,
            adapter->socket_id);
        if (dev_info->qpairs == NULL)
            return -ENOMEM;
        ...
        qpairs->op_buffer =
            rte_zmalloc_socket(adapter->mem_name, BATCH_SIZE * sizeof(struct rte_crypto_op *), 0, adapter->socket_id);
        if (!qpairs->op_buffer) {
            rte_free(qpairs);
            return -ENOMEM;
        }
...
int rte_event_crypto_adapter_queue_pair_add(uint8_t id, uint8_t cdev_id, int32_t queue_pair_id,
    ...
    if (ret) {
        RTE_EDEV_LOG_ERR("Failed to get adapter caps dev %" PRIu8 " cdev %" PRIu8, id, cdev_id);
        return ret;
    }

    if ((cap & RTE_EVENT_CRYPTO_ADAPTER_CAP_INTERNAL_PORT_QP_EV_BIND) && (event == NULL)) {
    ...
    if ((cap & RTE_EVENT_CRYPTO_ADAPTER_CAP_INTERNAL_PORT_OP_FWD) ||
        ...
        (cap & RTE_EVENT_CRYPTO_ADAPTER_CAP_INTERNAL_PORT_OP_NEW && adapter->mode == RTE_EVENT_CRYPTO_ADAPTER_OP_NEW)) {
        RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->crypto_adapter_queue_pair_add, -ENOTSUP);
        if (dev_info->qpairs == NULL) {
            dev_info->qpairs = rte_zmalloc_socket(
                adapter->mem_name, dev_info->dev->data->nb_queue_pairs * sizeof(struct crypto_queue_pair_info), 0,
                adapter->socket_id);
        ...
        ret = (*dev->dev_ops->crypto_adapter_queue_pair_add)(dev, dev_info->dev, queue_pair_id, event);
        if (ret)
            return ret;
...
int rte_event_crypto_adapter_queue_pair_del(uint8_t id, uint8_t cdev_id, int32_t queue_pair_id) {
    ...
    if ((cap & RTE_EVENT_CRYPTO_ADAPTER_CAP_INTERNAL_PORT_OP_FWD) ||
        (cap & RTE_EVENT_CRYPTO_ADAPTER_CAP_INTERNAL_PORT_OP_NEW && adapter->mode == RTE_EVENT_CRYPTO_ADAPTER_OP_NEW)) {
        RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->crypto_adapter_queue_pair_del, -ENOTSUP);
...
int rte_event_crypto_adapter_stop(uint8_t id) { return eca_adapter_ctrl(id, 0); }
...
int rte_event_crypto_adapter_stats_get(uint8_t id, struct rte_event_crypto_adapter_stats *stats) {
    ...
    for (i = 0; i < rte_cryptodev_count(); i++) {
        dev_info = &adapter->cdevs[i];
        if (dev_info->internal_event_port == 0 || dev->dev_ops->crypto_adapter_stats_get == NULL)
            continue;
...
int rte_event_crypto_adapter_stats_reset(uint8_t id) {
    ...
    for (i = 0; i < rte_cryptodev_count(); i++) {
        dev_info = &adapter->cdevs[i];
        if (dev_info->internal_event_port == 0 || dev->dev_ops->crypto_adapter_stats_reset == NULL)
            continue;
