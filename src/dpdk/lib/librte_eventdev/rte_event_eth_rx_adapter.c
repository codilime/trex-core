...
static inline int rxa_sw_adapter_queue_count(struct rte_event_eth_rx_adapter *rx_adapter) {
 ...
 */
static int rxa_wrr_next(struct rte_event_eth_rx_adapter *rx_adapter, unsigned int n, int *cw,
                        struct eth_rx_poll_entry *eth_rx_poll, uint16_t max_wt, uint16_t gcd, int prev) {
    int i = prev;
    uint16_t w;
...
static inline int rxa_shared_intr(struct eth_device_info *dev_info, int rx_queue_id) {
    int multi_intr_cap;
...
static inline int rxa_intr_queue(struct eth_device_info *dev_info, int rx_queue_id) {
    struct eth_rx_queue_info *queue_info;

    queue_info = &dev_info->rx_queue[rx_queue_id];
    return dev_info->rx_queue && !dev_info->internal_event_port && queue_info->queue_enabled && queue_info->wt == 0;
}

static inline int rxa_polled_queue(struct eth_device_info *dev_info, int rx_queue_id) {
    struct eth_rx_queue_info *queue_info;

    queue_info = &dev_info->rx_queue[rx_queue_id];
    return !dev_info->internal_event_port && dev_info->rx_queue && queue_info->queue_enabled && queue_info->wt != 0;
}

/* Calculate change in number of vectors after Rx queue ID is add/deleted */
static int rxa_nb_intr_vect(struct eth_device_info *dev_info, int rx_queue_id, int add) {
    uint16_t i;
    int n, s;
    uint16_t nbq;
    ...
    if (rx_queue_id == -1) {
        ...
        if (s > 0) {
            if ((add && dev_info->nb_shared_intr == 0) || (!add && dev_info->nb_shared_intr))
                n += 1;
        }
    } else {
        if (!rxa_shared_intr(dev_info, rx_queue_id))
            n = add ? !rxa_intr_queue(dev_info, rx_queue_id) : rxa_intr_queue(dev_info, rx_queue_id);
        else
 ...
 */
static void rxa_calc_nb_post_intr_del(struct rte_event_eth_rx_adapter *rx_adapter, struct eth_device_info *dev_info,
 ...
 */
static void rxa_calc_nb_post_add_intr(struct rte_event_eth_rx_adapter *rx_adapter, struct eth_device_info *dev_info,
                                      int rx_queue_id, uint32_t *nb_rx_poll, uint32_t *nb_rx_intr, uint32_t *nb_wrr) {
    uint32_t intr_diff;
    uint32_t poll_diff;
 ...
 */
static void rxa_calc_nb_post_poll_del(struct rte_event_eth_rx_adapter *rx_adapter, struct eth_device_info *dev_info,
                                      int rx_queue_id, uint32_t *nb_rx_poll, uint32_t *nb_wrr) {
    uint32_t poll_diff;
    uint32_t wrr_len_diff;
 ...
 */
static void rxa_calc_nb_post_add_poll(struct rte_event_eth_rx_adapter *rx_adapter, struct eth_device_info *dev_info,
                                      int rx_queue_id, uint16_t wt, uint32_t *nb_rx_poll, uint32_t *nb_rx_intr,
                                      uint32_t *nb_wrr) {
    uint32_t intr_diff;
    uint32_t poll_diff;
    uint32_t wrr_len_diff;
    ...
    if (rx_queue_id == -1) {
        ...
        intr_diff = rxa_intr_queue(dev_info, rx_queue_id);
        poll_diff = !rxa_polled_queue(dev_info, rx_queue_id);
        wrr_len_diff = rxa_polled_queue(dev_info, rx_queue_id) ? wt - dev_info->rx_queue[rx_queue_id].wt : wt;
    }
...
static void rxa_calc_nb_post_add(struct rte_event_eth_rx_adapter *rx_adapter, struct eth_device_info *dev_info,
                                 int rx_queue_id, uint16_t wt, uint32_t *nb_rx_poll, uint32_t *nb_rx_intr,
                                 uint32_t *nb_wrr) {
    if (wt != 0)
        rxa_calc_nb_post_add_poll(rx_adapter, dev_info, rx_queue_id, wt, nb_rx_poll, nb_rx_intr, nb_wrr);
    else
        rxa_calc_nb_post_add_intr(rx_adapter, dev_info, rx_queue_id, nb_rx_poll, nb_rx_intr, nb_wrr);
}

/* Calculate nb_rx_* after deleting rx_queue_id */
static void rxa_calc_nb_post_del(struct rte_event_eth_rx_adapter *rx_adapter, struct eth_device_info *dev_info,
                                 int rx_queue_id, uint32_t *nb_rx_poll, uint32_t *nb_rx_intr, uint32_t *nb_wrr) {
    rxa_calc_nb_post_poll_del(rx_adapter, dev_info, rx_queue_id, nb_rx_poll, nb_wrr);
 ...
 */
static struct eth_rx_poll_entry *rxa_alloc_poll(struct rte_event_eth_rx_adapter *rx_adapter, uint32_t num_rx_polled) {
    size_t len;

    len = RTE_ALIGN(num_rx_polled * sizeof(*rx_adapter->eth_rx_poll), RTE_CACHE_LINE_SIZE);
    return rte_zmalloc_socket(rx_adapter->mem_name, len, RTE_CACHE_LINE_SIZE, rx_adapter->socket_id);
}

/*
 * Allocate the WRR array
 */
static uint32_t *rxa_alloc_wrr(struct rte_event_eth_rx_adapter *rx_adapter, int nb_wrr) {
    size_t len;
...
static int rxa_alloc_poll_arrays(struct rte_event_eth_rx_adapter *rx_adapter, uint32_t nb_poll, uint32_t nb_wrr,
                                 struct eth_rx_poll_entry **rx_poll, uint32_t **wrr_sched) {
...
static void rxa_calc_wrr_sequence(struct rte_event_eth_rx_adapter *rx_adapter, struct eth_rx_poll_entry *rx_poll,
                                  uint32_t *rx_wrr) {
    uint16_t d;
    uint16_t q;
    unsigned int i;
    ...
    for (i = 0; i < max_wrr_pos; i++) {
        rx_wrr[i] = rxa_wrr_next(rx_adapter, poll_q, &cw, rx_poll, max_wt, gcd, prev);
        prev = rx_wrr[i];
    }
}

static inline void rxa_mtoip(struct rte_mbuf *m, struct ipv4_hdr **ipv4_hdr, struct ipv6_hdr **ipv6_hdr) {
...
static inline uint32_t rxa_do_softrss(struct rte_mbuf *m, const uint8_t *rss_key_be) {
    uint32_t input_len;
    void *tuple;
...
static inline int rxa_enq_blocked(struct rte_event_eth_rx_adapter *rx_adapter) { return !!rx_adapter->enq_block_count; }

static inline void rxa_enq_block_start_ts(struct rte_event_eth_rx_adapter *rx_adapter) {
...
static inline void rxa_enq_block_end_ts(struct rte_event_eth_rx_adapter *rx_adapter,
                                        struct rte_event_eth_rx_adapter_stats *stats) {
    ...
    if (rx_adapter->rx_enq_block_start_ts) {
        ...
        stats->rx_enq_block_cycles += stats->rx_enq_end_ts - rx_adapter->rx_enq_block_start_ts;
 ...
 * this function
 */
static inline void rxa_buffer_event(struct rte_event_eth_rx_adapter *rx_adapter, struct rte_event *ev) {
    struct rte_eth_event_enqueue_buffer *buf = &rx_adapter->event_enqueue_buffer;
    rte_memcpy(&buf->events[buf->count++], ev, sizeof(struct rte_event));
}

/* Enqueue buffered events to event device */
static inline uint16_t rxa_flush_event_buffer(struct rte_event_eth_rx_adapter *rx_adapter) {
    struct rte_eth_event_enqueue_buffer *buf = &rx_adapter->event_enqueue_buffer;
    struct rte_event_eth_rx_adapter_stats *stats = &rx_adapter->stats;

    uint16_t n =
        rte_event_enqueue_new_burst(rx_adapter->eventdev_id, rx_adapter->event_port_id, buf->events, buf->count);
    if (n != buf->count) {
        memmove(buf->events, &buf->events[n], (buf->count - n) * sizeof(struct rte_event));
        stats->rx_enq_retry++;
    }

    n ? rxa_enq_block_end_ts(rx_adapter, stats) : rxa_enq_block_start_ts(rx_adapter);
...
static inline void rxa_buffer_mbufs(struct rte_event_eth_rx_adapter *rx_adapter, uint16_t eth_dev_id,
                                    uint16_t rx_queue_id, struct rte_mbuf **mbufs, uint16_t num) {
    uint32_t i;
    struct eth_device_info *dev_info = &rx_adapter->eth_devices[eth_dev_id];
    struct eth_rx_queue_info *eth_rx_queue_info = &dev_info->rx_queue[rx_queue_id];
    struct rte_eth_event_enqueue_buffer *buf = &rx_adapter->event_enqueue_buffer;
    ...
    nb_cb = dev_info->cb_fn ? dev_info->cb_fn(eth_dev_id, rx_queue_id, ETH_EVENT_BUFFER_SIZE, buf->count, mbufs, num,
                                              dev_info->cb_arg, cb_mbufs)
                            : num;
    if (nb_cb < num) {
        mbufs = cb_mbufs;
        num = nb_cb;
    ...
    for (i = 0; i < num; i++) {
        ...
        flow_id = eth_rx_queue_info->flow_id & eth_rx_queue_info->flow_id_mask;
...
static inline uint32_t rxa_eth_rx(struct rte_event_eth_rx_adapter *rx_adapter, uint16_t port_id, uint16_t queue_id,
                                  uint32_t rx_count, uint32_t max_rx, int *rxq_empty) {
...
static inline void rxa_intr_ring_enqueue(struct rte_event_eth_rx_adapter *rx_adapter, void *data) {
    uint16_t port_id;
    uint16_t queue;
    int err;
    union queue_data qd;
...
static int rxa_intr_ring_check_avail(struct rte_event_eth_rx_adapter *rx_adapter, uint32_t num_intr_vec) {
    if (rx_adapter->num_intr_vec + num_intr_vec > RTE_EVENT_ETH_INTR_RING_SIZE) {
        RTE_EDEV_LOG_ERR("Exceeded intr ring slots current"
                         " %d needed %d limit %d",
                         rx_adapter->num_intr_vec, num_intr_vec, RTE_EVENT_ETH_INTR_RING_SIZE);
        return -ENOSPC;
    }
...
static void rxa_intr_ring_del_entries(struct rte_event_eth_rx_adapter *rx_adapter, struct eth_device_info *dev_info,
 ...
 * mbufs to eventdev
 */
static inline uint32_t rxa_intr_ring_dequeue(struct rte_event_eth_rx_adapter *rx_adapter) {
    uint32_t n;
    uint32_t nb_rx = 0;
    int rxq_empty;
        ...
        if (rxa_shared_intr(dev_info, queue)) {
            ...
            for (i = dev_info->next_q_idx; i < nb_queues; i++) {
                ...
                if (!rxa_intr_queue(dev_info, i))
                    continue;
                n = rxa_eth_rx(rx_adapter, port, i, nb_rx, rx_adapter->max_nb_rx, &rxq_empty);
                nb_rx += n;
            ...
            dev_info->next_q_idx = dev_info->multi_intr_cap ? RTE_MAX_RXTX_INTR_VEC_ID - 1 : 0;
        } else {
            n = rxa_eth_rx(rx_adapter, port, queue, nb_rx, rx_adapter->max_nb_rx, &rxq_empty);
 ...
 * it.
 */
static inline uint32_t rxa_poll(struct rte_event_eth_rx_adapter *rx_adapter) {
    uint32_t num_queue;
    uint32_t nb_rx = 0;
    struct rte_eth_event_enqueue_buffer *buf;
    uint32_t wrr_pos;
    ...
    for (num_queue = 0; num_queue < rx_adapter->wrr_len; num_queue++) {
        ...
        nb_rx += rxa_eth_rx(rx_adapter, d, qid, nb_rx, max_nb_rx, NULL);
        if (nb_rx > max_nb_rx) {
...
static int rte_event_eth_rx_adapter_init(void) {
    ...
    if (mz == NULL) {
        mz = rte_memzone_reserve_aligned(name, sz, rte_socket_id(), 0, RTE_CACHE_LINE_SIZE);
        if (mz == NULL) {
...
static inline struct rte_event_eth_rx_adapter *rxa_id_to_adapter(uint8_t id) {
    return event_eth_rx_adapter ? event_eth_rx_adapter[id] : NULL;
}

static int rxa_default_conf_cb(uint8_t id, uint8_t dev_id, struct rte_event_eth_rx_adapter_conf *conf, void *arg) {
    int ret;
...
static int rxa_create_intr_thread(struct rte_event_eth_rx_adapter *rx_adapter) {
    ...
    rx_adapter->intr_ring = rte_ring_create("intr_ring", RTE_EVENT_ETH_INTR_RING_SIZE, rte_socket_id(), 0);
    if (!rx_adapter->intr_ring)
        return -ENOMEM;

    rx_adapter->epoll_events =
        rte_zmalloc_socket(rx_adapter->mem_name, RTE_EVENT_ETH_INTR_RING_SIZE * sizeof(struct rte_epoll_event),
    ...
    snprintf(thread_name, RTE_MAX_THREAD_NAME_LEN, "rx-intr-thread-%d", rx_adapter->id);

    err = rte_ctrl_thread_create(&rx_adapter->rx_intr_thread, thread_name, NULL, rxa_intr_thread, rx_adapter);
    if (!err) {
...
static int rxa_disable_intr(struct rte_event_eth_rx_adapter *rx_adapter, struct eth_device_info *dev_info,
                            uint16_t rx_queue_id) {
    int err;
    ...
    if (err) {
        RTE_EDEV_LOG_ERR("Could not disable interrupt for Rx queue %u", rx_queue_id);
        return err;
    }

    err = rte_eth_dev_rx_intr_ctl_q(eth_dev_id, rx_queue_id, rx_adapter->epd, RTE_INTR_EVENT_DEL, 0);
    if (err)
...
static int rxa_del_intr_queue(struct rte_event_eth_rx_adapter *rx_adapter, struct eth_device_info *dev_info,
                              int rx_queue_id) {
    int err;
    int i;
    int s;
    ...
    if (rx_queue_id == -1) {
        ...
        if (!rxa_intr_queue(dev_info, rx_queue_id))
            return 0;
        if (!rxa_shared_intr(dev_info, rx_queue_id) || dev_info->nb_shared_intr == 1) {
...
static int rxa_config_intr(struct rte_event_eth_rx_adapter *rx_adapter, struct eth_device_info *dev_info,
                           uint16_t rx_queue_id) {
    int err, err1;
    uint16_t eth_dev_id = dev_info->dev->data->port_id;
    union queue_data qd;
    ...
    if (dev_info->intr_queue == NULL) {
        ...
        dev_info->intr_queue = rte_zmalloc_socket(rx_adapter->mem_name, len, 0, rx_adapter->socket_id);
        if (dev_info->intr_queue == NULL)
            return -ENOMEM;
    }
    ...
    err = rte_eth_dev_rx_intr_ctl_q(eth_dev_id, rx_queue_id, rx_adapter->epd, RTE_INTR_EVENT_ADD, qd.ptr);
    if (err) {
    ...
    if (err)
        RTE_EDEV_LOG_ERR("Could not disable interrupt for"
                         ...
                         rx_queue_id, err);
err_del_event:
    err1 = rte_eth_dev_rx_intr_ctl_q(eth_dev_id, rx_queue_id, rx_adapter->epd, RTE_INTR_EVENT_DEL, 0);
    if (err1) {
...
static int rxa_add_intr_queue(struct rte_event_eth_rx_adapter *rx_adapter, struct eth_device_info *dev_info,
                              int rx_queue_id)
...
static int rxa_init_service(struct rte_event_eth_rx_adapter *rx_adapter, uint8_t id) {
    int ret;
    ...
    snprintf(service.name, ETH_RX_ADAPTER_SERVICE_NAME_LEN, "rte_event_eth_rx_adapter_%d", id);
    ...
    ret = rx_adapter->conf_cb(id, rx_adapter->eventdev_id, &rx_adapter_conf, rx_adapter->conf_arg);
    if (ret) {
...
static void rxa_update_queue(struct rte_event_eth_rx_adapter *rx_adapter, struct eth_device_info *dev_info,
...
static void rxa_sw_del(struct rte_event_eth_rx_adapter *rx_adapter, struct eth_device_info *dev_info,
                       int32_t rx_queue_id) {
    int pollq;
    int intrq;
    int sintrq;
...
static void rxa_add_queue(struct rte_event_eth_rx_adapter *rx_adapter, struct eth_device_info *dev_info,
                          int32_t rx_queue_id, const struct rte_event_eth_rx_adapter_queue_conf *conf) {
    struct eth_rx_queue_info *queue_info;
...
static int rxa_sw_add(struct rte_event_eth_rx_adapter *rx_adapter, uint16_t eth_dev_id, int rx_queue_id,
    ...
    if (dev_info->rx_queue == NULL) {
        dev_info->rx_queue = rte_zmalloc_socket(rx_adapter->mem_name, nb_rx_queues * sizeof(struct eth_rx_queue_info),
    ...
    rxa_calc_nb_post_add(rx_adapter, dev_info, rx_queue_id, queue_conf->servicing_weight, &nb_rx_poll, &nb_rx_intr,
                         &nb_wrr);

    if (dev_info->dev->intr_handle)
        dev_info->multi_intr_cap = rte_intr_cap_multiple(dev_info->dev->intr_handle);

    ret = rxa_alloc_poll_arrays(rx_adapter, nb_rx_poll, nb_wrr, &rx_poll, &rx_wrr);
    if (ret)
...
int rte_event_eth_rx_adapter_create_ext(uint8_t id, uint8_t dev_id, rte_event_eth_rx_adapter_conf_cb conf_cb,
                                        void *conf_arg) {
    struct rte_event_eth_rx_adapter *rx_adapter;
    int ret;
    int socket_id;
    ...
    snprintf(mem_name, ETH_RX_ADAPTER_MEM_NAME_LEN, "rte_event_eth_rx_adapter_%d", id);

    rx_adapter = rte_zmalloc_socket(mem_name, sizeof(*rx_adapter), RTE_CACHE_LINE_SIZE, socket_id);
    if (rx_adapter == NULL) {
    ...
    rx_adapter->eth_devices =
        rte_zmalloc_socket(rx_adapter->mem_name, RTE_MAX_ETHPORTS * sizeof(struct eth_device_info), 0, socket_id);
    rte_convert_rss_key((const uint32_t *)default_rss_key, (uint32_t *)rx_adapter->rss_key_be,
...
int rte_event_eth_rx_adapter_create(uint8_t id, uint8_t dev_id, struct rte_event_port_conf *port_config) {
    ...
    if (pc == NULL)
        return -ENOMEM;
    *pc = *port_config;
    ret = rte_event_eth_rx_adapter_create_ext(id, dev_id, rxa_default_conf_cb, pc);
    if (ret)
        rte_free(pc);
    return ret;
}
...
int rte_event_eth_rx_adapter_free(uint8_t id) {
    ...
    if (rx_adapter->nb_queues) {
        RTE_EDEV_LOG_ERR("%" PRIu16 " Rx queues not deleted", rx_adapter->nb_queues);
        return -EBUSY;
    }
...
int rte_event_eth_rx_adapter_queue_add(uint8_t id, uint16_t eth_dev_id, int32_t rx_queue_id,
                                       const struct rte_event_eth_rx_adapter_queue_conf *queue_conf) {
    int ret;
    uint32_t cap;
    struct rte_event_eth_rx_adapter *rx_adapter;
    ...
    if (ret) {
        RTE_EDEV_LOG_ERR("Failed to get adapter caps edev %" PRIu8 "eth port %" PRIu16, id, eth_dev_id);
        return ret;
    }
    ...
    if ((cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_MULTI_EVENTQ) == 0 && (rx_queue_id != -1)) {
    ...
    if (rx_queue_id != -1 && (uint16_t)rx_queue_id >= rte_eth_devices[eth_dev_id].data->nb_rx_queues) {
    ...
    if (cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_INTERNAL_PORT) {
        ...
        if (dev_info->rx_queue == NULL) {
            dev_info->rx_queue = rte_zmalloc_socket(
                rx_adapter->mem_name, dev_info->dev->data->nb_rx_queues * sizeof(struct eth_rx_queue_info), 0,
                rx_adapter->socket_id);
            if (dev_info->rx_queue == NULL)
                return -ENOMEM;
        }

        ret = (*dev->dev_ops->eth_rx_adapter_queue_add)(dev, &rte_eth_devices[eth_dev_id], rx_queue_id, queue_conf);
        if (ret == 0) {
            dev_info->internal_event_port = 1;
            rxa_update_queue(rx_adapter, &rx_adapter->eth_devices[eth_dev_id], rx_queue_id, 1);
        }
    } else {
        rte_spinlock_lock(&rx_adapter->rx_lock);
        ...
        if (ret == 0) {
            ...
            rte_service_component_runstate_set(service_id, rxa_sw_adapter_queue_count(rx_adapter));
        }
        rte_spinlock_unlock(&rx_adapter->rx_lock);
    }
...
int rte_event_eth_rx_adapter_queue_del(uint8_t id, uint16_t eth_dev_id, int32_t rx_queue_id) {
    ...
    if (cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_INTERNAL_PORT) {
        ...
        ret = (*dev->dev_ops->eth_rx_adapter_queue_del)(dev, &rte_eth_devices[eth_dev_id], rx_queue_id);
        if (ret == 0) {
            rxa_update_queue(rx_adapter, &rx_adapter->eth_devices[eth_dev_id], rx_queue_id, 0);
        ...
        rxa_calc_nb_post_del(rx_adapter, dev_info, rx_queue_id, &nb_rx_poll, &nb_rx_intr, &nb_wrr);
    ...
    unlock_ret:
        ...
        rte_service_component_runstate_set(rx_adapter->service_id, rxa_sw_adapter_queue_count(rx_adapter));
    }

    return ret;
}

int rte_event_eth_rx_adapter_start(uint8_t id) { return rxa_ctrl(id, 1); }

int rte_event_eth_rx_adapter_stop(uint8_t id) { return rxa_ctrl(id, 0); }

int __rte_experimental rte_event_eth_rx_adapter_stats_get(uint8_t id, struct rte_event_eth_rx_adapter_stats *stats) {
    struct rte_event_eth_rx_adapter *rx_adapter;
    ...
    RTE_ETH_FOREACH_DEV(i) {
        dev_info = &rx_adapter->eth_devices[i];
        if (dev_info->internal_event_port == 0 || dev->dev_ops->eth_rx_adapter_stats_get == NULL)
            continue;
        ret = (*dev->dev_ops->eth_rx_adapter_stats_get)(dev, &rte_eth_devices[i], &dev_stats);
        if (ret)
            continue;
...
int rte_event_eth_rx_adapter_stats_reset(uint8_t id) {
    ...
    RTE_ETH_FOREACH_DEV(i) {
        dev_info = &rx_adapter->eth_devices[i];
        if (dev_info->internal_event_port == 0 || dev->dev_ops->eth_rx_adapter_stats_reset == NULL)
            continue;
...
int __rte_experimental rte_event_eth_rx_adapter_cb_register(uint8_t id, uint16_t eth_dev_id,
    ...
    if (cap & RTE_EVENT_ETH_RX_ADAPTER_CAP_INTERNAL_PORT) {
        RTE_EDEV_LOG_ERR("Rx callback not supported for eth port %" PRIu16, eth_dev_id);
        return -EINVAL;
    }
