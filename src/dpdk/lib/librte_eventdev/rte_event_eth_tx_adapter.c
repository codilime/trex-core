...
#define txa_dev_adapter_create_ext(t) txa_evdev(t)->dev_ops->eth_tx_adapter_create
...
static int txa_service_adapter_create_ext(uint8_t id, struct rte_eventdev *dev,
                                          rte_event_eth_tx_adapter_conf_cb conf_cb, void *conf_arg);
static int txa_service_queue_del(uint8_t id, const struct rte_eth_dev *dev, int32_t tx_queue_id);

static int txa_adapter_exist(uint8_t id) { return txa_dev_id_array[id] != TXA_INVALID_DEV_ID; }

static inline int txa_valid_id(uint8_t id) { return id < RTE_EVENT_ETH_TX_ADAPTER_MAX_INSTANCE; }

static void *txa_memzone_array_get(const char *name, unsigned int elt_size, int nb_elems) {
...
static int txa_dev_id_array_init(void) {
    if (txa_dev_id_array == NULL) {
        ...
        txa_dev_id_array =
            txa_memzone_array_get("txa_adapter_array", sizeof(int), RTE_EVENT_ETH_TX_ADAPTER_MAX_INSTANCE);
        if (txa_dev_id_array == NULL)
            return -ENOMEM;
...
static inline struct txa_service_data *txa_service_id_to_data(uint8_t id) { return txa_service_data_array[id]; }

static inline struct txa_service_queue_info *txa_service_queue(struct txa_service_data *txa, uint16_t port_id,
...
static int txa_service_conf_cb(uint8_t __rte_unused id, uint8_t dev_id, struct rte_event_eth_tx_adapter_conf *conf,
                               void *arg) {
    int ret;
...
static int txa_service_ethdev_alloc(struct txa_service_data *txa) {
    ...
    txa_ethdev = rte_zmalloc_socket(txa->mem_name, dev_count * sizeof(*txa_ethdev), 0, txa->socket_id);
    if (txa_ethdev == NULL) {
...
static int txa_service_queue_array_alloc(struct txa_service_data *txa, uint16_t port_id) {
    struct txa_service_queue_info *tqi;
    uint16_t nb_queue;
    int ret;
    ...
    tqi = rte_zmalloc_socket(txa->mem_name, nb_queue * sizeof(struct txa_service_queue_info), 0, txa->socket_id);
    if (tqi == NULL)
        return -ENOMEM;
...
static void txa_service_queue_array_free(struct txa_service_data *txa, uint16_t port_id) {
    struct txa_service_ethdev *txa_ethdev;
    struct txa_service_queue_info *tqi;
...
static struct rte_eth_dev_tx_buffer *txa_service_tx_buf_alloc(struct txa_service_data *txa,
                                                              const struct rte_eth_dev *dev) {
    struct rte_eth_dev_tx_buffer *tb;
    uint16_t port_id;

    port_id = dev->data->port_id;
    tb = rte_zmalloc_socket(txa->mem_name, RTE_ETH_TX_BUFFER_SIZE(TXA_BATCH_SIZE), 0, rte_eth_dev_socket_id(port_id));
    if (tb == NULL)
        RTE_EDEV_LOG_ERR("Failed to allocate memory for tx buffer");
    return tb;
}

static int txa_service_is_queue_added(struct txa_service_data *txa, const struct rte_eth_dev *dev,
                                      uint16_t tx_queue_id) {
    struct txa_service_queue_info *tqi;
...
static void txa_service_buffer_retry(struct rte_mbuf **pkts, uint16_t unsent, void *userdata) {
    struct txa_retry *tr;
...
static void txa_service_tx(struct txa_service_data *txa, struct rte_event *ev, uint32_t n) {
    uint32_t i;
    uint16_t nb_tx;
    struct rte_event_eth_tx_adapter_stats *stats;
...
static int txa_service_adapter_create(uint8_t id, struct rte_eventdev *dev, struct rte_event_port_conf *port_conf) {
    struct txa_service_data *txa;
...
static int txa_service_adapter_create_ext(uint8_t id, struct rte_eventdev *dev,
    ...
    txa = rte_zmalloc_socket(mem_name, sizeof(*txa), RTE_CACHE_LINE_SIZE, socket_id);
    if (txa == NULL) {
...
static int txa_service_queue_add(uint8_t id, __rte_unused struct rte_eventdev *dev, const struct rte_eth_dev *eth_dev,
                                 int32_t tx_queue_id) {
    struct txa_service_data *txa;
    struct txa_service_ethdev *tdi;
    struct txa_service_queue_info *tqi;
...
static int txa_service_stats_get(uint8_t id, struct rte_event_eth_tx_adapter_stats *stats) {
    struct txa_service_data *txa;
...
int rte_event_eth_tx_adapter_create(uint8_t id, uint8_t dev_id, struct rte_event_port_conf *port_conf) {
    struct rte_eventdev *dev;
    int ret;
...
int rte_event_eth_tx_adapter_create_ext(uint8_t id, uint8_t dev_id, rte_event_eth_tx_adapter_conf_cb conf_cb,
                                        void *conf_arg) {
    struct rte_eventdev *dev;
    int ret;
...
int rte_event_eth_tx_adapter_free(uint8_t id) {
    ...
    ret = txa_dev_adapter_free(id) ? txa_dev_adapter_free(id)(id, txa_evdev(id)) : 0;
...
int rte_event_eth_tx_adapter_queue_add(uint8_t id, uint16_t eth_dev_id, int32_t queue) {
    ...
    if (caps & RTE_EVENT_ETH_TX_ADAPTER_CAP_INTERNAL_PORT)
        ret = txa_dev_queue_add(id) ? txa_dev_queue_add(id)(id, txa_evdev(id), eth_dev, queue) : 0;
    else
...
int rte_event_eth_tx_adapter_queue_del(uint8_t id, uint16_t eth_dev_id, int32_t queue) {
    ...
    if (caps & RTE_EVENT_ETH_TX_ADAPTER_CAP_INTERNAL_PORT)
        ret = txa_dev_queue_del(id) ? txa_dev_queue_del(id)(id, txa_evdev(id), eth_dev, queue) : 0;
    else
...
int rte_event_eth_tx_adapter_stats_get(uint8_t id, struct rte_event_eth_tx_adapter_stats *stats) {
    ...
    *stats = (struct rte_event_eth_tx_adapter_stats){0};
    ...
    ret = txa_dev_stats_get(id) ? txa_dev_stats_get(id)(id, txa_evdev(id), stats) : 0;
...
int rte_event_eth_tx_adapter_stats_reset(uint8_t id) {
    ...
    ret = txa_dev_stats_reset(id) ? txa_dev_stats_reset(id)(id, txa_evdev(id)) : 0;
    if (ret == 0)
