...
int rte_event_ring_init(struct rte_event_ring *r, const char *name, unsigned int count, unsigned int flags) {
...
/* create the ring */
struct rte_event_ring *rte_event_ring_create(const char *name, unsigned int count, int socket_id, unsigned int flags) {
    ...
    ring_list = RTE_TAILQ_CAST(rte_event_ring_tailq.head, rte_event_ring_list);
...
void rte_event_ring_free(struct rte_event_ring *r) {
     ...
     */
    if (r->r.memzone == NULL) {
        RTE_LOG(ERR, RING, "Cannot free ring (not created with rte_event_ring_create()");
        return;
    }
