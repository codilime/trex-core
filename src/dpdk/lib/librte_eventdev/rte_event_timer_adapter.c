...
#define EVTIM_BUF_LOG_DBG(...) EVTIM_LOG(DEBUG, evtim_buffer_logtype, __VA_ARGS__)
...
static int default_port_conf_cb(uint16_t id, uint8_t event_dev_id, uint8_t *event_port_id, void *conf_arg) {
...
struct rte_event_timer_adapter *rte_event_timer_adapter_create_ext(const struct rte_event_timer_adapter_conf *conf,
    ...
    mz = rte_memzone_reserve(mz_name, sizeof(struct rte_event_timer_adapter_data), conf->socket_id, 0);
    if (mz == NULL)
    ...
    ret = dev->dev_ops->timer_adapter_caps_get(dev, adapter->data->conf.flags, &adapter->data->caps, &adapter->ops);
    if (ret < 0) {
        rte_errno = ret;
        goto free_memzone;
    }
    ...
    if (!(adapter->data->caps & RTE_EVENT_TIMER_ADAPTER_CAP_INTERNAL_PORT)) {
        ...
        ret = conf_cb(adapter->data->id, adapter->data->event_dev_id, &adapter->data->event_port_id, conf_arg);
        if (ret < 0) {
            rte_errno = ret;
            goto free_memzone;
        }
...
int rte_event_timer_adapter_start(const struct rte_event_timer_adapter *adapter) {
    ...
    if (adapter->data->started) {
        EVTIM_LOG_ERR("event timer adapter %" PRIu8 " already started", adapter->data->id);
        return -EALREADY;
    }
...
int rte_event_timer_adapter_stop(const struct rte_event_timer_adapter *adapter) {
    ...
    if (adapter->data->started == 0) {
        EVTIM_LOG_ERR("event timer adapter %" PRIu8 " already stopped", adapter->data->id);
        return 0;
    }
...
static inline bool event_buffer_full(struct event_buffer *bufp) { return (bufp->head - bufp->tail) == EVENT_BUFFER_SZ; }
...
static int event_buffer_add(struct event_buffer *bufp, struct rte_event *eventp) {
    uint16_t head_idx;
...
static void event_buffer_flush(struct event_buffer *bufp, uint8_t dev_id, uint8_t port_id, uint16_t *nb_events_flushed,
    ...
    *nb_events_inv = 0;
    *nb_events_flushed = rte_event_enqueue_burst(dev_id, port_id, &events[tail_idx], n);
...
static void sw_event_timer_cb(struct rte_timer *tim, void *arg) {
    int ret;
    ...
    if (ret < 0) {
         ...
         * next iteration.
         */
        rte_timer_reset_sync(tim, 0, SINGLE, rte_lcore_id(), sw_event_timer_cb, evtim);
    ...
    if (event_buffer_batch_ready(&sw_data->buffer)) {
        event_buffer_flush(&sw_data->buffer, adapter->data->event_dev_id, adapter->data->event_port_id, &nb_evs_flushed,
                           &nb_evs_invalid);
...
static __rte_always_inline uint64_t get_timeout_cycles(struct rte_event_timer *evtim,
                                                       struct rte_event_timer_adapter *adapter) {
    uint64_t timeout_ns;
    struct rte_event_timer_adapter_sw_data *sw_data;
 ...
 */
static inline bool adapter_did_tick(struct rte_event_timer_adapter *adapter) {
    ...
    cycles_per_adapter_tick = sw_data->timer_tick_ns * (rte_get_timer_hz() / NSECPERSEC);
...
static __rte_always_inline int check_timeout(struct rte_event_timer *evtim,
 ...
 * sched type
 */
static __rte_always_inline int check_destination_event_queue(struct rte_event_timer *evtim,
                                                             const struct rte_event_timer_adapter *adapter) {
    int ret;
    uint32_t sched_type;

    ret = rte_event_queue_attr_get(adapter->data->event_dev_id, evtim->ev.queue_id, RTE_EVENT_QUEUE_ATTR_SCHEDULE_TYPE,
                                   &sched_type);

    if ((ret < 0 && ret != -EOVERFLOW) || evtim->ev.sched_type != sched_type)
        return -1;
...
#define NB_OBJS 32
static int sw_event_timer_adapter_service_func(void *arg) {
    int i, num_msgs;
    ...
    while (rte_atomic16_read(&sw_data->message_producer_count) > 0 || !rte_ring_empty(sw_data->msg_ring)) {

        num_msgs = rte_ring_dequeue_burst(sw_data->msg_ring, (void **)msgs, NB_OBJS, NULL);
        ...
        for (i = 0; i < num_msgs; i++) {
            ...
            case MSG_TYPE_ARM:
                ...
                tim = &msg->tim;
                rte_timer_init(tim);
                cycles = get_timeout_cycles(evtim, adapter);
                ret = rte_timer_reset(tim, cycles, SINGLE, rte_lcore_id(), sw_event_timer_cb, evtim);
                RTE_ASSERT(ret == 0);

                evtim->impl_opaque[0] = (uintptr_t)tim;
                evtim->impl_opaque[1] = (uintptr_t)adapter;

                TAILQ_INSERT_TAIL(&sw_data->msgs_tailq_head, msg, msgs);
                break;
 ...
 */
static int compute_msg_mempool_cache_size(uint64_t nb_requested, uint64_t nb_actual) {
    int i;
    int size;
    int cache_size = 0;
    ...
    for (i = 0;; i++) {
        ...
        if (RTE_MAX_LCORE * size < (int)(nb_actual - nb_requested) && size < RTE_MEMPOOL_CACHE_MAX_SIZE &&
            size <= nb_actual / 1.5)
            cache_size = size;
        else
            break;
...
static int sw_event_timer_adapter_init(struct rte_event_timer_adapter *adapter) {
    int ret;
    struct rte_event_timer_adapter_sw_data *sw_data;
    uint64_t nb_timers;
    unsigned int flags;
    ...
    snprintf(priv_data_name, RTE_RING_NAMESIZE, "sw_evtim_adap_priv_%" PRIu8, adapter->data->id);
    adapter->data->adapter_priv = rte_zmalloc_socket(priv_data_name, sizeof(struct rte_event_timer_adapter_sw_data),
                                                     RTE_CACHE_LINE_SIZE, adapter->data->socket_id);
    ...
    snprintf(msg_ring_name, RTE_RING_NAMESIZE, "sw_evtim_adap_msg_ring_%" PRIu8, adapter->data->id);
    flags =
        adapter->data->conf.flags & RTE_EVENT_TIMER_ADAPTER_F_SP_PUT ? RING_F_SP_ENQ | RING_F_SC_DEQ : RING_F_SC_DEQ;
    sw_data->msg_ring = rte_ring_create(msg_ring_name, nb_timers, adapter->data->socket_id, flags);
    ...
    snprintf(pool_name, RTE_RING_NAMESIZE, "sw_evtim_adap_msg_pool_%" PRIu8, adapter->data->id);
     ...
     */
    flags = adapter->data->conf.flags & RTE_EVENT_TIMER_ADAPTER_F_SP_PUT ? MEMPOOL_F_SC_GET : 0;

    /* The usable size of a ring is count - 1, so subtract one here to
     * make the counts agree.
     */
    int pool_size = nb_timers - 1;
    int cache_size = compute_msg_mempool_cache_size(adapter->data->conf.nb_timers, nb_timers);
    sw_data->msg_pool = rte_mempool_create(pool_name, pool_size, sizeof(struct msg), cache_size, 0, NULL, NULL, NULL,
                                           NULL, adapter->data->socket_id, flags);
    ...
    snprintf(service.name, RTE_SERVICE_NAME_MAX, "sw_evimer_adap_svc_%" PRIu8, adapter->data->id);
    ...
    if (ret < 0) {
        EVTIM_LOG_ERR("failed to register service %s with id %" PRIu32 ": err = %d", service.name, sw_data->service_id,
                      ret);
    ...
    EVTIM_LOG_DBG("registered service %s with id %" PRIu32, service.name, sw_data->service_id);
...
static int sw_event_timer_adapter_uninit(struct rte_event_timer_adapter *adapter) {
    int ret;
    struct msg *m1, *m2;
    struct rte_event_timer_adapter_sw_data *sw_data = adapter->data->adapter_priv;
...
static int sw_event_timer_adapter_start(const struct rte_event_timer_adapter *adapter) {
    ...
    if (mapped_count == 1)
        return rte_service_component_runstate_set(sw_data->service_id, 1);
...
static void sw_event_timer_adapter_get_info(const struct rte_event_timer_adapter *adapter,
                                            struct rte_event_timer_adapter_info *adapter_info) {
    struct rte_event_timer_adapter_sw_data *sw_data;
    sw_data = adapter->data->adapter_priv;
...
static int sw_event_timer_adapter_stats_get(const struct rte_event_timer_adapter *adapter,
                                            struct rte_event_timer_adapter_stats *stats) {
    struct rte_event_timer_adapter_sw_data *sw_data;
    sw_data = adapter->data->adapter_priv;
    *stats = sw_data->stats;
...
static int sw_event_timer_adapter_stats_reset(const struct rte_event_timer_adapter *adapter) {
    struct rte_event_timer_adapter_sw_data *sw_data;
    sw_data = adapter->data->adapter_priv;
...
static __rte_always_inline uint16_t __sw_event_timer_arm_burst(const struct rte_event_timer_adapter *adapter,
    ...
    for (i = 0; i < nb_evtims; i++) {
        ...
        if (evtims[i]->state == RTE_EVENT_TIMER_ARMED) {
            rte_errno = EALREADY;
            break;
        } else if (!(evtims[i]->state == RTE_EVENT_TIMER_NOT_ARMED || evtims[i]->state == RTE_EVENT_TIMER_CANCELED)) {
            rte_errno = EINVAL;
            break;
        }
    ...
    if (i < nb_evtims)
        rte_mempool_put_bulk(sw_data->msg_pool, (void **)&msgs[i], nb_evtims - i);

    return i;
}

static uint16_t sw_event_timer_arm_burst(const struct rte_event_timer_adapter *adapter, struct rte_event_timer **evtims,
                                         uint16_t nb_evtims) {
    return __sw_event_timer_arm_burst(adapter, evtims, nb_evtims);
}

static uint16_t sw_event_timer_cancel_burst(const struct rte_event_timer_adapter *adapter,
                                            struct rte_event_timer **evtims, uint16_t nb_evtims) {
    uint16_t i;
    int ret;
    struct rte_event_timer_adapter_sw_data *sw_data;
...
static uint16_t sw_event_timer_arm_tmo_tick_burst(const struct rte_event_timer_adapter *adapter,
                                                  struct rte_event_timer **evtims, uint64_t timeout_ticks,
                                                  uint16_t nb_evtims) {
    int i;
