...
int rte_event_dev_get_dev_id(const char *name) {
    ...
    for (i = 0; i < eventdev_globals.nb_devs; i++) {
        cmp = (strncmp(rte_event_devices[i].data->name, name, RTE_EVENTDEV_NAME_MAX_LEN) == 0) ||
              (rte_event_devices[i].dev
                   ? (strncmp(rte_event_devices[i].dev->driver->name, name, RTE_EVENTDEV_NAME_MAX_LEN) == 0)
                   : 0);
...
int rte_event_timer_adapter_caps_get(uint8_t dev_id, uint32_t *caps) {
    ...
    *caps = 0;
    ...
    return dev->dev_ops->timer_adapter_caps_get ? (*dev->dev_ops->timer_adapter_caps_get)(dev, 0, caps, &ops) : 0;
}
...
int rte_event_crypto_adapter_caps_get(uint8_t dev_id, uint8_t cdev_id, uint32_t *caps) {
    ...
    *caps = 0;
    ...
    return dev->dev_ops->crypto_adapter_caps_get ? (*dev->dev_ops->crypto_adapter_caps_get)(dev, cdev, caps) : -ENOTSUP;
}
...
int rte_event_eth_tx_adapter_caps_get(uint8_t dev_id, uint16_t eth_port_id, uint32_t *caps) {
    ...
    *caps = 0;
    ...
    return dev->dev_ops->eth_tx_adapter_caps_get ? (*dev->dev_ops->eth_tx_adapter_caps_get)(dev, eth_dev, caps) : 0;
}

static inline int rte_event_dev_queue_config(struct rte_eventdev *dev, uint8_t nb_queues) {
    uint8_t old_nb_queues = dev->data->nb_queues;
    struct rte_event_queue_conf *queues_cfg;
    unsigned int i;

    RTE_EDEV_LOG_DEBUG("Setup %d queues on device %u", nb_queues, dev->data->dev_id);

    /* First time configuration */
    if (dev->data->queues_cfg == NULL && nb_queues != 0) {
        /* Allocate memory to store queue configuration */
        dev->data->queues_cfg =
            rte_zmalloc_socket("eventdev->data->queues_cfg", sizeof(dev->data->queues_cfg[0]) * nb_queues,
        ...
        queues_cfg = rte_realloc(queues_cfg, sizeof(queues_cfg[0]) * nb_queues, RTE_CACHE_LINE_SIZE);
        if (queues_cfg == NULL) {
...
static inline int rte_event_dev_port_config(struct rte_eventdev *dev, uint8_t nb_ports) {
    ...
    RTE_EDEV_LOG_DEBUG("Setup %d ports on device %u", nb_ports, dev->data->dev_id);

    /* First time configuration */
    if (dev->data->ports == NULL && nb_ports != 0) {
        dev->data->ports = rte_zmalloc_socket("eventdev->data->ports", sizeof(dev->data->ports[0]) * nb_ports,
                                              RTE_CACHE_LINE_SIZE, dev->data->socket_id);
        ...
        dev->data->ports_cfg = rte_zmalloc_socket("eventdev->ports_cfg", sizeof(dev->data->ports_cfg[0]) * nb_ports,
                                                  RTE_CACHE_LINE_SIZE, dev->data->socket_id);
        ...
        dev->data->links_map = rte_zmalloc_socket(
            "eventdev->links_map", sizeof(dev->data->links_map[0]) * nb_ports * RTE_EVENT_MAX_QUEUES_PER_DEV,
            RTE_CACHE_LINE_SIZE, dev->data->socket_id);
        ...
        for (i = 0; i < nb_ports * RTE_EVENT_MAX_QUEUES_PER_DEV; i++)
            dev->data->links_map[i] = EVENT_QUEUE_SERVICE_PRIORITY_INVALID;
        ...
        ports = rte_realloc(ports, sizeof(ports[0]) * nb_ports, RTE_CACHE_LINE_SIZE);
        if (ports == NULL) {
        ...
        ports_cfg = rte_realloc(ports_cfg, sizeof(ports_cfg[0]) * nb_ports, RTE_CACHE_LINE_SIZE);
        if (ports_cfg == NULL) {
        ...
        links_map = rte_realloc(links_map, sizeof(dev->data->links_map[0]) * nb_ports * RTE_EVENT_MAX_QUEUES_PER_DEV,
                                RTE_CACHE_LINE_SIZE);
        if (links_map == NULL) {
            dev->data->nb_ports = 0;
        ...
        if (nb_ports > old_nb_ports) {
            ...
            unsigned int old_links_map_end = old_nb_ports * RTE_EVENT_MAX_QUEUES_PER_DEV;
...
int rte_event_dev_configure(uint8_t dev_id, const struct rte_event_dev_config *dev_conf) {
    ...
    if (dev->data->dev_started) {
        RTE_EDEV_LOG_ERR("device %d must be stopped to allow configuration", dev_id);
        return -EBUSY;
    }
    ...
    if (!(dev_conf->event_dev_cfg & RTE_EVENT_DEV_CFG_PER_DEQUEUE_TIMEOUT)) {
        if (dev_conf->dequeue_timeout_ns && (dev_conf->dequeue_timeout_ns < info.min_dequeue_timeout_ns ||
            ...
            RTE_EDEV_LOG_ERR("dev%d invalid dequeue_timeout_ns=%d"
                             ...
                             dev_id, dev_conf->dequeue_timeout_ns, info.min_dequeue_timeout_ns,
    ...
    if (dev_conf->nb_events_limit > info.max_num_events) {
        RTE_EDEV_LOG_ERR("dev%d nb_events_limit=%d > max_num_events=%d", dev_id, dev_conf->nb_events_limit,
                         info.max_num_events);
        return -EINVAL;
    }
    ...
    if (dev_conf->nb_event_queues > info.max_event_queues) {
        RTE_EDEV_LOG_ERR("%d nb_event_queues=%d > max_event_queues=%d", dev_id, dev_conf->nb_event_queues,
    ...
    if (dev_conf->nb_event_ports > info.max_event_ports) {
        RTE_EDEV_LOG_ERR("id%d nb_event_ports=%d > max_event_ports= %d", dev_id, dev_conf->nb_event_ports,
    ...
    if (dev_conf->nb_event_queue_flows > info.max_event_queue_flows) {
        RTE_EDEV_LOG_ERR("dev%d nb_flows=%x > max_flows=%x", dev_id, dev_conf->nb_event_queue_flows,
    ...
    if ((info.event_dev_cap & RTE_EVENT_DEV_CAP_BURST_MODE) &&
        ...
        RTE_EDEV_LOG_ERR("dev%d nb_dq_depth=%d > max_dq_depth=%d", dev_id, dev_conf->nb_event_port_dequeue_depth,
    ...
    if ((info.event_dev_cap & RTE_EVENT_DEV_CAP_BURST_MODE) &&
        ...
        RTE_EDEV_LOG_ERR("dev%d nb_enq_depth=%d > max_enq_depth=%d", dev_id, dev_conf->nb_event_port_enqueue_depth,
    ...
    if (diag != 0) {
        RTE_EDEV_LOG_ERR("dev%d rte_event_dev_queue_config = %d", dev_id, diag);
        return diag;
    }
    ...
    if (diag != 0) {
        ...
        RTE_EDEV_LOG_ERR("dev%d rte_event_dev_port_config = %d", dev_id, diag);
        return diag;
    }
...
static inline int is_valid_queue(struct rte_eventdev *dev, uint8_t queue_id) {
    if (queue_id < dev->data->nb_queues && queue_id < RTE_EVENT_MAX_QUEUES_PER_DEV)
        return 1;
    else
        return 0;
}
...
static inline int is_valid_atomic_queue_conf(const struct rte_event_queue_conf *queue_conf) {
    if (queue_conf && !(queue_conf->event_queue_cfg & RTE_EVENT_QUEUE_CFG_SINGLE_LINK) &&
...
static inline int is_valid_ordered_queue_conf(const struct rte_event_queue_conf *queue_conf) {
    if (queue_conf && !(queue_conf->event_queue_cfg & RTE_EVENT_QUEUE_CFG_SINGLE_LINK) &&
        ((queue_conf->event_queue_cfg & RTE_EVENT_QUEUE_CFG_ALL_TYPES) ||
...
int rte_event_queue_setup(uint8_t dev_id, uint8_t queue_id, const struct rte_event_queue_conf *queue_conf) {
    ...
    if (is_valid_atomic_queue_conf(queue_conf)) {
        if (queue_conf->nb_atomic_flows == 0 ||
            queue_conf->nb_atomic_flows > dev->data->dev_conf.nb_event_queue_flows) {
            RTE_EDEV_LOG_ERR("dev%d queue%d Invalid nb_atomic_flows=%d max_flows=%d", dev_id, queue_id,
                             queue_conf->nb_atomic_flows, dev->data->dev_conf.nb_event_queue_flows);
            return -EINVAL;
        }
    }
    ...
    if (is_valid_ordered_queue_conf(queue_conf)) {
        if (queue_conf->nb_atomic_order_sequences == 0 ||
            queue_conf->nb_atomic_order_sequences > dev->data->dev_conf.nb_event_queue_flows) {
            RTE_EDEV_LOG_ERR("dev%d queue%d Invalid nb_atomic_order_seq=%d max_flows=%d", dev_id, queue_id,
                             queue_conf->nb_atomic_order_sequences, dev->data->dev_conf.nb_event_queue_flows);
            return -EINVAL;
        }
    }
...
int rte_event_port_setup(uint8_t dev_id, uint8_t port_id, const struct rte_event_port_conf *port_conf) {
    ...
    if ((port_conf && !port_conf->new_event_threshold) ||
        (port_conf && port_conf->new_event_threshold > dev->data->dev_conf.nb_events_limit)) {
        RTE_EDEV_LOG_ERR("dev%d port%d Invalid event_threshold=%d nb_events_limit=%d", dev_id, port_id,
    ...
    if ((port_conf && !port_conf->dequeue_depth) ||
        (port_conf && port_conf->dequeue_depth > dev->data->dev_conf.nb_event_port_dequeue_depth)) {
        RTE_EDEV_LOG_ERR("dev%d port%d Invalid dequeue depth=%d max_dequeue_depth=%d", dev_id, port_id,
                         port_conf->dequeue_depth, dev->data->dev_conf.nb_event_port_dequeue_depth);
        return -EINVAL;
    }

    /* Check enqueue_depth limit */
    if ((port_conf && !port_conf->enqueue_depth) ||
        (port_conf && port_conf->enqueue_depth > dev->data->dev_conf.nb_event_port_enqueue_depth)) {
        RTE_EDEV_LOG_ERR("dev%d port%d Invalid enqueue depth=%d max_enqueue_depth=%d", dev_id, port_id,
                         port_conf->enqueue_depth, dev->data->dev_conf.nb_event_port_enqueue_depth);
        return -EINVAL;
    }

    if (port_conf && port_conf->disable_implicit_release &&
        !(dev->data->event_dev_cap & RTE_EVENT_DEV_CAP_IMPLICIT_RELEASE_DISABLE)) {
        RTE_EDEV_LOG_ERR("dev%d port%d Implicit release disable not supported", dev_id, port_id);
        return -EINVAL;
    }
...
int rte_event_port_link(uint8_t dev_id, uint8_t port_id, const uint8_t queues[], const uint8_t priorities[],
                        uint16_t nb_links) {
    struct rte_eventdev *dev;
    ...
    diag = (*dev->dev_ops->port_link)(dev, dev->data->ports[port_id], queues, priorities, nb_links);
    if (diag < 0)
        return diag;
...
static int xstats_get_count(uint8_t dev_id, enum rte_event_dev_xstats_mode mode, uint8_t queue_port_id) {
...
int rte_event_dev_xstats_names_get(uint8_t dev_id, enum rte_event_dev_xstats_mode mode, uint8_t queue_port_id,
                                   struct rte_event_dev_xstats_name *xstats_names, unsigned int *ids,
                                   unsigned int size) {
    RTE_EVENTDEV_VALID_DEVID_OR_ERR_RET(dev_id, -ENODEV);
    const int cnt_expected_entries = xstats_get_count(dev_id, mode, queue_port_id);
    ...
    if (dev->dev_ops->xstats_get_names != NULL)
        return (*dev->dev_ops->xstats_get_names)(dev, mode, queue_port_id, xstats_names, ids, size);

    return -ENOTSUP;
}

/* retrieve eventdev extended statistics */
int rte_event_dev_xstats_get(uint8_t dev_id, enum rte_event_dev_xstats_mode mode, uint8_t queue_port_id,
    ...
    if (dev->dev_ops->xstats_get != NULL)
        return (*dev->dev_ops->xstats_get)(dev, mode, queue_port_id, ids, values, n);
    return -ENOTSUP;
}
...
int rte_event_dev_xstats_reset(uint8_t dev_id, enum rte_event_dev_xstats_mode mode, int16_t queue_port_id,
...
static inline int rte_eventdev_data_alloc(uint8_t dev_id, struct rte_eventdev_data **data, int socket_id) {
    ...
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
        mz = rte_memzone_reserve(mz_name, sizeof(struct rte_eventdev_data), socket_id, 0);
    } else
...
static inline uint8_t rte_eventdev_find_free_device_index(void) {
    uint8_t dev_id;
...
static uint16_t rte_event_tx_adapter_enqueue(__rte_unused void *port, __rte_unused struct rte_event ev[],
...
struct rte_eventdev *rte_event_pmd_allocate(const char *name, int socket_id) {
    ...
    if (eventdev->data == NULL) {
        ...
        int retval = rte_eventdev_data_alloc(dev_id, &eventdev_data, socket_id);
...
int rte_event_pmd_release(struct rte_eventdev *eventdev) {
    ...
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
        ...
        /* Generate memzone name */
        ret = snprintf(mz_name, sizeof(mz_name), "rte_eventdev_data_%u", eventdev->data->dev_id);
