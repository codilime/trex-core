...
static inline void hdr_segment_init(struct rte_mbuf *hdr_segment, struct rte_mbuf *pkt, uint16_t pkt_hdr_offset) {
    /* Copy MBUF metadata */
    ...
    rte_memcpy(rte_pktmbuf_mtod(hdr_segment, char *), rte_pktmbuf_mtod(pkt, char *), pkt_hdr_offset);
}
...
int gso_do_segment(struct rte_mbuf *pkt, uint16_t pkt_hdr_offset, uint16_t pyld_unit_size,
                   struct rte_mempool *direct_pool, struct rte_mempool *indirect_pool, struct rte_mbuf **pkts_out,
