...
static void update_ipv4_tcp_headers(struct rte_mbuf *pkt, uint8_t ipid_delta, struct rte_mbuf **segs,
                                    uint16_t nb_segs) {
    ...
    ipv4_hdr = (struct ipv4_hdr *)(rte_pktmbuf_mtod(pkt, char *) + l3_offset);
...
int gso_tcp4_segment(struct rte_mbuf *pkt, uint16_t gso_size, uint8_t ipid_delta, struct rte_mempool *direct_pool,
