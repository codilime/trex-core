...
static void update_tunnel_ipv4_tcp_headers(struct rte_mbuf *pkt, uint8_t ipid_delta, struct rte_mbuf **segs,
                                           uint16_t nb_segs) {
    ...
    /* Outer IPv4 header. */
    ipv4_hdr = (struct ipv4_hdr *)(rte_pktmbuf_mtod(pkt, char *) + outer_ipv4_offset);
    outer_id = rte_be_to_cpu_16(ipv4_hdr->packet_id);

    /* Inner IPv4 header. */
    ipv4_hdr = (struct ipv4_hdr *)(rte_pktmbuf_mtod(pkt, char *) + inner_ipv4_offset);
...
int gso_tunnel_tcp4_segment(struct rte_mbuf *pkt, uint16_t gso_size, uint8_t ipid_delta,
    ...
    inner_ipv4_hdr = (struct ipv4_hdr *)(rte_pktmbuf_mtod(pkt, char *) + hdr_offset);
    /*
