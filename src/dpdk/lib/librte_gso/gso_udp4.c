...
static inline void update_ipv4_udp_headers(struct rte_mbuf *pkt, struct rte_mbuf **segs, uint16_t nb_segs) {
     ...
     */
    for (i = 0; i < nb_segs; i++) {
        ipv4_hdr = rte_pktmbuf_mtod_offset(segs[i], struct ipv4_hdr *, l2_hdrlen);
...
int gso_udp4_segment(struct rte_mbuf *pkt, uint16_t gso_size, struct rte_mempool *direct_pool,
                     struct rte_mempool *indirect_pool, struct rte_mbuf **pkts_out, uint16_t nb_pkts_out) {
    struct ipv4_hdr *ipv4_hdr;
    ...
    ipv4_hdr = rte_pktmbuf_mtod_offset(pkt, struct ipv4_hdr *, pkt->l2_len);
    ...
    /* Segment the payload */
    ret = gso_do_segment(pkt, hdr_offset, pyld_unit_size, direct_pool, indirect_pool, pkts_out, nb_pkts_out);
    if (ret > 1)
