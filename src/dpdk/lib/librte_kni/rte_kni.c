...
#define KNI_FIFO_SIZE (KNI_FIFO_COUNT_MAX * sizeof(void *) + sizeof(struct rte_kni_fifo))
...
static void *va2pa(struct rte_mbuf *m) {
    return (void *)((unsigned long)m - ((unsigned long)m->buf_addr - (unsigned long)m->buf_iova));
}

static void obj_free(struct rte_mempool *mp __rte_unused, void *opaque, void *obj, unsigned obj_idx __rte_unused) {
...
static void kni_free_fifo_phy(struct rte_mempool *mp, struct rte_kni_fifo *fifo) {
    void *mbuf_phys;
    int ret;
...
static int kni_config_mac_address(uint16_t port_id, uint8_t mac_addr[]) {
    ...
    ret = rte_eth_dev_default_mac_addr_set(port_id, (struct ether_addr *)mac_addr);
    if (ret < 0)
...
int rte_kni_handle_request(struct rte_kni *kni) {
    ...
    case RTE_KNI_REQ_CHANGE_MAC_ADDR: /* Change MAC Address */
        if (kni->ops.config_mac_address)
            req->result = kni->ops.config_mac_address(kni->ops.port_id, req->mac_addr);
    ...
    case RTE_KNI_REQ_CHANGE_PROMISC: /* Change PROMISCUOUS MODE */
        if (kni->ops.config_promiscusity)
            req->result = kni->ops.config_promiscusity(kni->ops.port_id, req->promiscusity);
        else if (kni->ops.port_id != UINT16_MAX)
            req->result = kni_config_promiscusity(kni->ops.port_id, req->promiscusity);
        break;
    default:
...
static void kni_allocate_mbufs(struct rte_kni *kni) {
    ...
    RTE_BUILD_BUG_ON(offsetof(struct rte_mbuf, pool) != offsetof(struct rte_kni_mbuf, pool));
    RTE_BUILD_BUG_ON(offsetof(struct rte_mbuf, buf_addr) != offsetof(struct rte_kni_mbuf, buf_addr));
    RTE_BUILD_BUG_ON(offsetof(struct rte_mbuf, next) != offsetof(struct rte_kni_mbuf, next));
    RTE_BUILD_BUG_ON(offsetof(struct rte_mbuf, data_off) != offsetof(struct rte_kni_mbuf, data_off));
    RTE_BUILD_BUG_ON(offsetof(struct rte_mbuf, data_len) != offsetof(struct rte_kni_mbuf, data_len));
    RTE_BUILD_BUG_ON(offsetof(struct rte_mbuf, pkt_len) != offsetof(struct rte_kni_mbuf, pkt_len));
    RTE_BUILD_BUG_ON(offsetof(struct rte_mbuf, ol_flags) != offsetof(struct rte_kni_mbuf, ol_flags));
    ...
    allocq_free = (kni->alloc_q->read - kni->alloc_q->write - 1) & (MAX_MBUF_BURST_NUM - 1);
...
static enum kni_ops_status kni_check_request_register(struct rte_kni_ops *ops) {
    ...
    if ((ops->change_mtu == NULL) && (ops->config_network_if == NULL) && (ops->config_mac_address == NULL) &&
...
int __rte_experimental rte_kni_update_link(struct rte_kni *kni, unsigned int linkup) {
    ...
    snprintf(path, sizeof(path), "/sys/devices/virtual/net/%s/carrier", kni->name);
