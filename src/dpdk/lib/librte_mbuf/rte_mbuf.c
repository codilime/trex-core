...
void rte_pktmbuf_pool_init(struct rte_mempool *mp, void *opaque_arg) {
    ...
    RTE_ASSERT(mp->elt_size >=
               sizeof(struct rte_mbuf) + user_mbp_priv->mbuf_data_room_size + user_mbp_priv->mbuf_priv_size);
 ...
 */
void rte_pktmbuf_init(struct rte_mempool *mp, __attribute__((unused)) void *opaque_arg, void *_m,
...
struct rte_mempool *rte_pktmbuf_pool_create_by_ops(const char *name, unsigned int n, unsigned int cache_size,
    ...
    mp = rte_mempool_create_empty(name, n, elt_size, cache_size, sizeof(struct rte_pktmbuf_pool_private), socket_id, 0);
    if (mp == NULL)
        return NULL;
...
struct rte_mempool *rte_pktmbuf_pool_create(const char *name, unsigned int n, unsigned int cache_size,
                                            uint16_t priv_size, uint16_t data_room_size, int socket_id) {
    return rte_pktmbuf_pool_create_by_ops(name, n, cache_size, priv_size, data_room_size, socket_id, NULL);
}
...
void rte_pktmbuf_dump(FILE *f, const struct rte_mbuf *m, unsigned dump_len) {
    ...
    fprintf(f,
            "  pkt_len=%" PRIu32 ", ol_flags=%" PRIx64 ", nb_segs=%u, "
            "in_port=%u\n",
...
const void *__rte_pktmbuf_read(const struct rte_mbuf *m, uint32_t off, uint32_t len, void *buf) {
    ...
    while (len > 0) {
        ...
        if (copy_len > len)
            copy_len = len;
        rte_memcpy((char *)buf + buf_off, rte_pktmbuf_mtod_offset(seg, char *, off), copy_len);
        off = 0;
        buf_off += copy_len;
        len -= copy_len;
        seg = seg->next;
...
int rte_get_rx_ol_flag_list(uint64_t mask, char *buf, size_t buflen) {
    const struct flag_mask rx_flags[] = {
        ...
        {PKT_RX_L4_CKSUM_UNKNOWN, PKT_RX_L4_CKSUM_MASK, "PKT_RX_L4_CKSUM_UNKNOWN"},
        ...
        {PKT_RX_IP_CKSUM_UNKNOWN, PKT_RX_IP_CKSUM_MASK, "PKT_RX_IP_CKSUM_UNKNOWN"},
        ...
        {PKT_RX_OUTER_L4_CKSUM_GOOD, PKT_RX_OUTER_L4_CKSUM_MASK, NULL},
        {PKT_RX_OUTER_L4_CKSUM_INVALID, PKT_RX_OUTER_L4_CKSUM_MASK, NULL},
        {PKT_RX_OUTER_L4_CKSUM_UNKNOWN, PKT_RX_OUTER_L4_CKSUM_MASK, "PKT_RX_OUTER_L4_CKSUM_UNKNOWN"},
    };
    const char *name;
    unsigned int i;
    int ret;
