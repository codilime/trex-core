...
int rte_mbuf_set_platform_mempool_ops(const char *ops_name) {
    ...
    if (mz == NULL) {
        mz = rte_memzone_reserve("mbuf_platform_pool_ops", RTE_MEMPOOL_OPS_NAMESIZE, SOCKET_ID_ANY, 0);
        if (mz == NULL)
            return -rte_errno;
...
int rte_mbuf_set_user_mempool_ops(const char *ops_name) {
    ...
    if (mz == NULL) {
        mz = rte_memzone_reserve("mbuf_user_pool_ops", RTE_MEMPOOL_OPS_NAMESIZE, SOCKET_ID_ANY, 0);
        if (mz == NULL)
            return -rte_errno;
    }
