...
static void mempool_add_elem(struct rte_mempool *mp, __rte_unused void *opaque, void *obj, rte_iova_t iova) {
...
uint32_t rte_mempool_calc_obj_size(uint32_t elt_size, uint32_t flags, struct rte_mempool_objsz *sz) {
    ...
    if ((flags & MEMPOOL_F_NO_CACHE_ALIGN) == 0) {
        ...
        sz->trailer_size += ((RTE_MEMPOOL_ALIGN - (sz->total_size & RTE_MEMPOOL_ALIGN_MASK)) & RTE_MEMPOOL_ALIGN_MASK);
    }
    ...
    if ((flags & MEMPOOL_F_NO_SPREAD) == 0) {
        unsigned new_size;
        new_size = optimize_object_size(sz->header_size + sz->elt_size + sz->trailer_size);
...
static void rte_mempool_memchunk_mz_free(__rte_unused struct rte_mempool_memhdr *memhdr, void *opaque) {
...
int rte_mempool_populate_iova(struct rte_mempool *mp, char *vaddr, rte_iova_t iova, size_t len,
    ...
    i = rte_mempool_ops_populate(mp, mp->size - mp->populated_size, (char *)vaddr + off,
                                 (iova == RTE_BAD_IOVA) ? RTE_BAD_IOVA : (iova + off), len - off, mempool_add_elem,
                                 NULL);
...
int rte_mempool_populate_virt(struct rte_mempool *mp, char *addr, size_t len, size_t pg_sz,
    ...
    if (mp->flags & MEMPOOL_F_NO_IOVA_CONTIG)
        return rte_mempool_populate_iova(mp, addr, RTE_BAD_IOVA, len, free_cb, opaque);

    for (off = 0; off + pg_sz <= len && mp->populated_size < mp->size; off += phys_len) {
        ...
        ret = rte_mempool_populate_iova(mp, addr + off, iova, phys_len, free_cb, opaque);
        if (ret < 0)
            goto fail;
...
int rte_mempool_populate_default(struct rte_mempool *mp) {
    ...
    no_pageshift = no_contig || (!external && rte_eal_iova_mode() == RTE_IOVA_VA);
    try_contig = !no_contig && !no_pageshift && (rte_eal_has_hugepages() || external);
    ...
    for (mz_id = 0, n = mp->size; n > 0; mz_id++, n -= ret) {
        ...
        if (try_contig || no_pageshift)
            mem_size = rte_mempool_ops_calc_mem_size(mp, n, 0, &min_chunk_size, &align);
        else
            mem_size = rte_mempool_ops_calc_mem_size(mp, n, pg_shift, &min_chunk_size, &align);
        ...
        ret = snprintf(mz_name, sizeof(mz_name), RTE_MEMPOOL_MZ_FORMAT "_%d", mp->name, mz_id);
        ...
        mz = rte_memzone_reserve_aligned(mz_name, mem_size, mp->socket_id, flags, align);
        ...
        if (min_chunk_size < (size_t)mem_size && mz == NULL) {
             ...
             * have
             */
            mz = rte_memzone_reserve_aligned(mz_name, 0, mp->socket_id, flags, RTE_MAX(pg_sz, align));
        }
        if (mz == NULL) {
            ret = -rte_errno;
            goto fail;
        ...
        if (no_pageshift || try_contig)
            ret = rte_mempool_populate_iova(mp, mz->addr, iova, mz->len, rte_mempool_memchunk_mz_free,
                                            (void *)(uintptr_t)mz);
        else
            ret = rte_mempool_populate_virt(mp, mz->addr, RTE_ALIGN_FLOOR(mz->len, pg_sz), pg_sz,
                                            rte_mempool_memchunk_mz_free, (void *)(uintptr_t)mz);
        if (ret < 0) {
            rte_memzone_free(mz);
            goto fail;
        }
...
static ssize_t get_anon_size(const struct rte_mempool *mp) {
    ...
    pg_sz = getpagesize();
    pg_shift = rte_bsf32(pg_sz);
    size = rte_mempool_ops_calc_mem_size(mp, mp->size, pg_shift, &min_chunk_size, &align);

    return size;
}

/* unmap a memory zone mapped by rte_mempool_populate_anon() */
static void rte_mempool_memchunk_anon_free(struct rte_mempool_memhdr *memhdr, void *opaque) {
    ssize_t size;
...
int rte_mempool_populate_anon(struct rte_mempool *mp) {
    ...
    ret = rte_mempool_populate_virt(mp, addr, size, getpagesize(), rte_mempool_memchunk_anon_free, addr);
    if (ret == 0)
        goto fail;
...
struct rte_mempool_cache *rte_mempool_cache_create(uint32_t size, int socket_id) {
    ...
    cache = rte_zmalloc_socket("MEMPOOL_CACHE", sizeof(*cache), RTE_CACHE_LINE_SIZE, socket_id);
    if (cache == NULL) {
 ...
 * mempool.
 */
void rte_mempool_cache_free(struct rte_mempool_cache *cache) { rte_free(cache); }

/* create an empty mempool */
struct rte_mempool *rte_mempool_create_empty(const char *name, unsigned n, unsigned elt_size, unsigned cache_size,
                                             unsigned private_data_size, int socket_id, unsigned flags) {
    char mz_name[RTE_MEMZONE_NAMESIZE];
    ...
    /* asked cache too big */
    if (cache_size > RTE_MEMPOOL_CACHE_MAX_SIZE || CALC_CACHE_FLUSHTHRESH(cache_size) > n) {
        rte_errno = EINVAL;
        return NULL;
    }
...
/* create the mempool */
struct rte_mempool *rte_mempool_create(const char *name, unsigned n, unsigned elt_size, unsigned cache_size,
...
unsigned int rte_mempool_in_use_count(const struct rte_mempool *mp) { return mp->size - rte_mempool_avail_count(mp); }
...
void rte_mempool_check_cookies(const struct rte_mempool *mp, void *const *obj_table_const, unsigned n, int free) {
#ifdef RTE_LIBRTE_MEMPOOL_DEBUG
    struct rte_mempool_objhdr *hdr;
    ...
    while (n--) {
        ...
        if (free == 0) {
            ...
            if (cookie != RTE_MEMPOOL_HEADER_COOKIE1 && cookie != RTE_MEMPOOL_HEADER_COOKIE2) {
                RTE_LOG(CRIT, MEMPOOL, "obj=%p, mempool=%p, cookie=%" PRIx64 "\n", obj, (const void *)mp, cookie);
...
void rte_mempool_contig_blocks_check_cookies(const struct rte_mempool *mp, void *const *first_obj_table_const,
...
#ifdef RTE_LIBRTE_MEMPOOL_DEBUG
static void mempool_obj_audit(struct rte_mempool *mp, __rte_unused void *opaque, void *obj, __rte_unused unsigned idx) {
...
void rte_mempool_walk(void (*func)(struct rte_mempool *, void *), void *arg) {
    ...
    TAILQ_FOREACH_SAFE(te, mempool_list, next, tmp_te) { (*func)((struct rte_mempool *)te->data, arg); }
