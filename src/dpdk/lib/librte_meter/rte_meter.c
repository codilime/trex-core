...
static void rte_meter_get_tb_params(uint64_t hz, uint64_t rate, uint64_t *tb_period, uint64_t *tb_bytes_per_period) {
    double period;
...
int rte_meter_srtcm_profile_config(struct rte_meter_srtcm_profile *p, struct rte_meter_srtcm_params *params) {
    ...
    if ((p == NULL) || (params == NULL) || (params->cir == 0) || ((params->cbs == 0) && (params->ebs == 0)))
        return -EINVAL;
...
int rte_meter_trtcm_profile_config(struct rte_meter_trtcm_profile *p, struct rte_meter_trtcm_params *params) {
    ...
    /* Check input parameters */
    if ((p == NULL) || (params == NULL) || (params->cir == 0) || (params->pir == 0) || (params->pir < params->cir) ||
...
int __rte_experimental rte_meter_trtcm_rfc4115_profile_config(struct rte_meter_trtcm_rfc4115_profile *p,
    ...
    p->ebs = params->ebs;
    rte_meter_get_tb_params(hz, params->cir, &p->cir_period, &p->cir_bytes_per_period);
    rte_meter_get_tb_params(hz, params->eir, &p->eir_period, &p->eir_bytes_per_period);

    return 0;
}

int __rte_experimental rte_meter_trtcm_rfc4115_config(struct rte_meter_trtcm_rfc4115 *m,
