...
static uint32_t ptype_l3_ip(uint8_t ipv_ihl) {
    static const uint32_t ptype_l3_ip_proto_map[256] = {
        [0x45] = RTE_PTYPE_L3_IPV4,     [0x46] = RTE_PTYPE_L3_IPV4_EXT, [0x47] = RTE_PTYPE_L3_IPV4_EXT,
        [0x48] = RTE_PTYPE_L3_IPV4_EXT, [0x49] = RTE_PTYPE_L3_IPV4_EXT, [0x4A] = RTE_PTYPE_L3_IPV4_EXT,
        [0x4B] = RTE_PTYPE_L3_IPV4_EXT, [0x4C] = RTE_PTYPE_L3_IPV4_EXT, [0x4D] = RTE_PTYPE_L3_IPV4_EXT,
        [0x4E] = RTE_PTYPE_L3_IPV4_EXT, [0x4F] = RTE_PTYPE_L3_IPV4_EXT,
    };
...
static uint32_t ptype_inner_l3_ip6(uint8_t ip6_proto) {
    static const uint32_t ptype_inner_ip6_ext_proto_map[256] = {
        ...
        [IPPROTO_FRAGMENT] = RTE_PTYPE_INNER_L3_IPV6_EXT - RTE_PTYPE_INNER_L3_IPV6,
...
static uint32_t ptype_inner_l3_ip(uint8_t ipv_ihl) {
    static const uint32_t ptype_inner_l3_ip_proto_map[256] = {
        [0x45] = RTE_PTYPE_INNER_L3_IPV4,     [0x46] = RTE_PTYPE_INNER_L3_IPV4_EXT,
        [0x47] = RTE_PTYPE_INNER_L3_IPV4_EXT, [0x48] = RTE_PTYPE_INNER_L3_IPV4_EXT,
        [0x49] = RTE_PTYPE_INNER_L3_IPV4_EXT, [0x4A] = RTE_PTYPE_INNER_L3_IPV4_EXT,
        [0x4B] = RTE_PTYPE_INNER_L3_IPV4_EXT, [0x4C] = RTE_PTYPE_INNER_L3_IPV4_EXT,
        [0x4D] = RTE_PTYPE_INNER_L3_IPV4_EXT, [0x4E] = RTE_PTYPE_INNER_L3_IPV4_EXT,
...
static uint32_t ptype_tunnel(uint16_t *proto, const struct rte_mbuf *m, uint32_t *off) {
    switch (*proto) {
    case IPPROTO_GRE: {
...
static uint8_t ip4_hlen(const struct ipv4_hdr *hdr) { return (hdr->version_ihl & 0xf) * 4; }

/* parse ipv6 extended headers, update offset and return next proto */
int __rte_experimental rte_net_skip_ip6_ext(uint16_t proto, const struct rte_mbuf *m, uint32_t *off, int *frag) {
    struct ext_hdr {
        uint8_t next_hdr;
        uint8_t len;
    };
...
uint32_t rte_net_get_ptype(const struct rte_mbuf *m, struct rte_net_hdr_lens *hdr_lens, uint32_t layers) {
    ...
    if (proto == rte_cpu_to_be_16(ETHER_TYPE_VLAN)) {
        ...
        proto = vh->eth_proto;
    } else if ((proto == rte_cpu_to_be_16(ETHER_TYPE_MPLS)) || (proto == rte_cpu_to_be_16(ETHER_TYPE_MPLSM))) {
        unsigned int i;
...
l3:
    ...
    if (proto == rte_cpu_to_be_16(ETHER_TYPE_IPv4)) {
        ...
        if (ip4h->fragment_offset & rte_cpu_to_be_16(IPV4_HDR_OFFSET_MASK | IPV4_HDR_MF_FLAG)) {
