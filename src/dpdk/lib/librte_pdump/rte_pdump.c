...
static struct pdump_rxtx_cbs {
    ...
    void *filter;
} rx_cbs[RTE_MAX_ETHPORTS][RTE_MAX_QUEUES_PER_PORT], tx_cbs[RTE_MAX_ETHPORTS][RTE_MAX_QUEUES_PER_PORT];

static inline int pdump_pktmbuf_copy_data(struct rte_mbuf *seg, const struct rte_mbuf *m) {
...
static inline struct rte_mbuf *pdump_pktmbuf_copy(struct rte_mbuf *m, struct rte_mempool *mp) {
...
static inline void pdump_copy(struct rte_mbuf **pkts, uint16_t nb_pkts, void *user_params) {
    unsigned i;
    int ring_enq;
    uint16_t d_pkts = 0;
...
static uint16_t pdump_rx(uint16_t port __rte_unused, uint16_t qidx __rte_unused, struct rte_mbuf **pkts,
                         uint16_t nb_pkts, uint16_t max_pkts __rte_unused, void *user_params) {
    pdump_copy(pkts, nb_pkts, user_params);
    return nb_pkts;
}

static uint16_t pdump_tx(uint16_t port __rte_unused, uint16_t qidx __rte_unused, struct rte_mbuf **pkts,
                         uint16_t nb_pkts, void *user_params) {
    pdump_copy(pkts, nb_pkts, user_params);
    return nb_pkts;
}

static int pdump_register_rx_callbacks(uint16_t end_q, uint16_t port, uint16_t queue, struct rte_ring *ring,
                                       struct rte_mempool *mp, uint16_t operation) {
    uint16_t qid;
    ...
    for (; qid < end_q; qid++) {
        ...
        if (cbs && operation == ENABLE) {
            ...
            if (cbs->cb == NULL) {
                RTE_LOG(ERR, PDUMP, "failed to add rx callback, errno=%d\n", rte_errno);
                return rte_errno;
            }
        }
        if (cbs && operation == DISABLE) {
            ...
            if (ret < 0) {
                RTE_LOG(ERR, PDUMP, "failed to remove rx callback, errno=%d\n", -ret);
                return ret;
            }
            cbs->cb = NULL;
        }
...
static int pdump_register_tx_callbacks(uint16_t end_q, uint16_t port, uint16_t queue, struct rte_ring *ring,
                                       struct rte_mempool *mp, uint16_t operation) {
    ...
    for (; qid < end_q; qid++) {
        ...
        if (cbs && operation == ENABLE) {
            ...
            if (cbs->cb == NULL) {
                RTE_LOG(ERR, PDUMP, "failed to add tx callback, errno=%d\n", rte_errno);
                return rte_errno;
            }
        }
        if (cbs && operation == DISABLE) {
            ...
            if (ret < 0) {
                RTE_LOG(ERR, PDUMP, "failed to remove tx callback, errno=%d\n", -ret);
                return ret;
            }
            cbs->cb = NULL;
        }
...
static int set_pdump_rxtx_cbs(const struct pdump_request *p) {
    ...
    operation = p->op;
    if (operation == ENABLE) {
        ret = rte_eth_dev_get_port_by_name(p->data.en_v1.device, &port);
        if (ret < 0) {
            RTE_LOG(ERR, PDUMP, "failed to get port id for device id=%s\n", p->data.en_v1.device);
            return -EINVAL;
        }
        ...
        ret = rte_eth_dev_get_port_by_name(p->data.dis_v1.device, &port);
        if (ret < 0) {
    ...
    if (flags & RTE_PDUMP_FLAG_RX) {
        ...
        ret = pdump_register_rx_callbacks(end_q, port, queue, ring, mp, operation);
        if (ret < 0)
            return ret;
    }
    ...
    if (flags & RTE_PDUMP_FLAG_TX) {
        ...
        ret = pdump_register_tx_callbacks(end_q, port, queue, ring, mp, operation);
        if (ret < 0)
            return ret;
    }
...
int rte_pdump_init(void) { return rte_mp_action_register(PDUMP_MP, pdump_server); }
...
static int pdump_validate_ring_mp(struct rte_ring *ring, struct rte_mempool *mp) {
...
static int pdump_validate_flags(uint32_t flags) {
    if (flags != RTE_PDUMP_FLAG_RX && flags != RTE_PDUMP_FLAG_TX && flags != RTE_PDUMP_FLAG_RXTX) {
...
static int pdump_validate_port(uint16_t port, char *name) {
    ...
    if (ret < 0) {
        RTE_LOG(ERR, PDUMP, "port id to name mapping failed for port id=%u, %s:%d\n", port, __func__, __LINE__);
        rte_errno = EINVAL;
        return -1;
    }
...
static int pdump_prepare_client_request(char *device, uint16_t queue, uint32_t flags, uint16_t operation,
    ...
    if ((operation & ENABLE) != 0) {
        strlcpy(req->data.en_v1.device, device, sizeof(req->data.en_v1.device));
        req->data.en_v1.queue = queue;
        req->data.en_v1.ring = ring;
        req->data.en_v1.mp = mp;
        req->data.en_v1.filter = filter;
    } else {
        strlcpy(req->data.dis_v1.device, device, sizeof(req->data.dis_v1.device));
...
int rte_pdump_enable(uint16_t port, uint16_t queue, uint32_t flags, struct rte_ring *ring, struct rte_mempool *mp,
                     void *filter) {
    ...
    ret = pdump_prepare_client_request(name, queue, flags, ENABLE, ring, mp, filter);

    return ret;
}

int rte_pdump_enable_by_deviceid(char *device_id, uint16_t queue, uint32_t flags, struct rte_ring *ring,
    ...
    ret = pdump_prepare_client_request(device_id, queue, flags, ENABLE, ring, mp, filter);
...
int rte_pdump_disable(uint16_t port, uint16_t queue, uint32_t flags) {
    ...
    ret = pdump_prepare_client_request(name, queue, flags, DISABLE, NULL, NULL, NULL);
...
int rte_pdump_disable_by_deviceid(char *device_id, uint16_t queue, uint32_t flags) {
    ...
    ret = pdump_prepare_client_request(device_id, queue, flags, DISABLE, NULL, NULL, NULL);
