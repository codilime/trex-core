...
#define RTE_PIPELINE_STATS_AH_DROP_WRITE(p, mask) ({ (p)->n_pkts_ah_drop = __builtin_popcountll(mask); })
...
#define RTE_PIPELINE_STATS_TABLE_DROP0(p) ({ (p)->pkts_drop_mask = (p)->action_mask0[RTE_PIPELINE_ACTION_DROP]; })
...
static inline uint32_t rte_mask_get_next(uint64_t mask, uint32_t pos) {
    uint64_t mask_rot = (mask << ((63 - pos) & 0x3F)) | (mask >> ((pos + 1) & 0x3F));
...
struct rte_pipeline *rte_pipeline_create(struct rte_pipeline_params *params) {
    ...
    p = rte_zmalloc_socket("PIPELINE", sizeof(struct rte_pipeline), RTE_CACHE_LINE_SIZE, params->socket_id);
 ...
 *
 */
static int rte_table_check_params(struct rte_pipeline *p, struct rte_pipeline_table_params *params,
                                  uint32_t *table_id) {
    if (p == NULL) {
...
int rte_pipeline_table_create(struct rte_pipeline *p, struct rte_pipeline_table_params *params, uint32_t *table_id) {
    ...
    entry_size = sizeof(struct rte_pipeline_table_entry) + params->action_data_size;
...
int rte_pipeline_table_default_entry_add(struct rte_pipeline *p, uint32_t table_id,
                                         struct rte_pipeline_table_entry *default_entry,
    ...
    if ((default_entry->action == RTE_PIPELINE_ACTION_TABLE) && table->table_next_id_valid &&
    ...
    if ((default_entry->action == RTE_PIPELINE_ACTION_TABLE) && (table->table_next_id_valid == 0)) {
...
int rte_pipeline_table_default_entry_delete(struct rte_pipeline *p, uint32_t table_id,
...
int rte_pipeline_table_entry_add(struct rte_pipeline *p, uint32_t table_id, void *key,
                                 struct rte_pipeline_table_entry *entry, int *key_found,
    ...
    if ((entry->action == RTE_PIPELINE_ACTION_TABLE) && table->table_next_id_valid &&
    ...
    /* Add entry */
    if ((entry->action == RTE_PIPELINE_ACTION_TABLE) && (table->table_next_id_valid == 0)) {
...
int rte_pipeline_table_entry_add_bulk(struct rte_pipeline *p, uint32_t table_id, void **keys,
    ...
    for (i = 0; i < n_keys; i++) {
        if ((entries[i]->action == RTE_PIPELINE_ACTION_TABLE) && table->table_next_id_valid &&
    ...
    /* Add entry */
    for (i = 0; i < n_keys; i++) {
        if ((entries[i]->action == RTE_PIPELINE_ACTION_TABLE) && (table->table_next_id_valid == 0)) {
            table->table_next_id = entries[i]->table_id;
            table->table_next_id_valid = 1;
        }
    }

    return (table->ops.f_add_bulk)(table->h_table, keys, (void **)entries, n_keys, key_found, (void **)entries_ptr);
}

int rte_pipeline_table_entry_delete_bulk(struct rte_pipeline *p, uint32_t table_id, void **keys, uint32_t n_keys,
    ...
    return (table->ops.f_delete_bulk)(table->h_table, keys, n_keys, key_found, (void **)entries);
}
 ...
 *
 */
static int rte_pipeline_port_in_check_params(struct rte_pipeline *p, struct rte_pipeline_port_in_params *params,
                                             uint32_t *port_id) {
    if (p == NULL) {
        RTE_LOG(ERR, PIPELINE, "%s: pipeline parameter NULL\n", __func__);
        return -EINVAL;
    ...
    /* burst_size */
    if ((params->burst_size == 0) || (params->burst_size > RTE_PORT_IN_BURST_SIZE_MAX)) {
...
static int rte_pipeline_port_out_check_params(struct rte_pipeline *p, struct rte_pipeline_port_out_params *params,
                                              uint32_t *port_id) {
    if (p == NULL) {
        RTE_LOG(ERR, PIPELINE, "%s: pipeline parameter NULL\n", __func__);
        return -EINVAL;
...
static inline void rte_pipeline_compute_masks(struct rte_pipeline *p, uint64_t pkts_mask) {
...
static inline void rte_pipeline_action_handler_port_bulk(struct rte_pipeline *p, uint64_t pkts_mask, uint32_t port_id) {
...
static inline void rte_pipeline_action_handler_port(struct rte_pipeline *p, uint64_t pkts_mask) {
    p->pkts_mask = pkts_mask;
...
static inline void rte_pipeline_action_handler_port_meta(struct rte_pipeline *p, uint64_t pkts_mask) {
    p->pkts_mask = pkts_mask;
...
static inline void rte_pipeline_action_handler_drop(struct rte_pipeline *p, uint64_t pkts_mask) {
    if ((pkts_mask & (pkts_mask + 1)) == 0) {
        uint64_t n_pkts = __builtin_popcountll(pkts_mask);
        uint32_t i;
...
int rte_pipeline_run(struct rte_pipeline *p) {
    ...
    for (table_id = port_in->table_id; p->pkts_mask != 0;) {
        ...
        /* Lookup */
        table = &p->tables[table_id];
        table->ops.f_lookup(table->h_table, p->pkts, p->pkts_mask, &lookup_hit_mask, (void **)p->entries);
        ...
        if (lookup_miss_mask != 0) {
            ...
            if (table->f_action_miss != NULL) {
                table->f_action_miss(p, p->pkts, lookup_miss_mask, default_entry, table->arg_ah);

                RTE_PIPELINE_STATS_AH_DROP_READ(p, table->n_pkts_dropped_by_lkp_miss_ah);
            }

            /* Table reserved actions */
            if ((default_entry->action == RTE_PIPELINE_ACTION_PORT) && (p->pkts_mask != 0))
                rte_pipeline_action_handler_port_bulk(p, p->pkts_mask, default_entry->port_id);
            else {
        ...
        if (lookup_hit_mask != 0) {
            ...
            if (table->f_action_hit != NULL) {
                table->f_action_hit(p, p->pkts, lookup_hit_mask, p->entries, table->arg_ah);
            ...
            p->action_mask0[RTE_PIPELINE_ACTION_DROP] |= p->action_mask1[RTE_PIPELINE_ACTION_DROP];
            p->action_mask0[RTE_PIPELINE_ACTION_PORT] |= p->action_mask1[RTE_PIPELINE_ACTION_PORT];
            p->action_mask0[RTE_PIPELINE_ACTION_PORT_META] |= p->action_mask1[RTE_PIPELINE_ACTION_PORT_META];
            p->action_mask0[RTE_PIPELINE_ACTION_TABLE] |= p->action_mask1[RTE_PIPELINE_ACTION_TABLE];
    ...
    rte_pipeline_action_handler_port(p, p->action_mask0[RTE_PIPELINE_ACTION_PORT]);

    /* Table reserved action PORT META */
    rte_pipeline_action_handler_port_meta(p, p->action_mask0[RTE_PIPELINE_ACTION_PORT_META]);

    /* Table reserved action DROP */
    rte_pipeline_action_handler_drop(p, p->action_mask0[RTE_PIPELINE_ACTION_DROP]);
...
int rte_pipeline_port_in_stats_read(struct rte_pipeline *p, uint32_t port_id, struct rte_pipeline_port_in_stats *stats,
                                    int clear) {
    struct rte_port_in *port;
    int retval;
...
int rte_pipeline_table_stats_read(struct rte_pipeline *p, uint32_t table_id, struct rte_pipeline_table_stats *stats,
                                  int clear) {
    struct rte_table *table;
    int retval;
