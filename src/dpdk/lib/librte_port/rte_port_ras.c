...
#define RTE_PORT_RING_WRITER_RAS_STATS_PKTS_IN_ADD(port, val) port->stats.n_pkts_in += val
#define RTE_PORT_RING_WRITER_RAS_STATS_PKTS_DROP_ADD(port, val) port->stats.n_pkts_drop += val
...
typedef void (*ras_op)(struct rte_port_ring_writer_ras *p, struct rte_mbuf *pkt);

static void process_ipv4(struct rte_port_ring_writer_ras *p, struct rte_mbuf *pkt);
static void process_ipv6(struct rte_port_ring_writer_ras *p, struct rte_mbuf *pkt);
...
static void *rte_port_ring_writer_ras_create(void *params, int socket_id, int is_ipv4) {
    ...
    if ((conf->tx_burst_sz == 0) || (conf->tx_burst_sz > RTE_PORT_IN_BURST_SIZE_MAX)) {
    ...
    port->frag_tbl = rte_ip_frag_table_create(RTE_PORT_RAS_N_BUCKETS, RTE_PORT_RAS_N_ENTRIES_PER_BUCKET,
...
static void process_ipv6(struct rte_port_ring_writer_ras *p, struct rte_mbuf *pkt) {
    ...
    if ((frag_data & RTE_IPV6_FRAG_USED_MASK) == 0)
        ...
        /* Process this fragment */
        mo = rte_ipv6_frag_reassemble_packet(tbl, dr, pkt, rte_rdtsc(), pkt_hdr, frag_hdr);
        if (mo != NULL)
            p->tx_buf[p->tx_buf_count++] = mo;
...
static int rte_port_ring_writer_ras_tx_bulk(void *port, struct rte_mbuf **pkts, uint64_t pkts_mask) {
    struct rte_port_ring_writer_ras *p = port;
...
static int rte_port_ras_writer_stats_read(void *port, struct rte_port_out_stats *stats, int clear) {
    struct rte_port_ring_writer_ras *p = port;
