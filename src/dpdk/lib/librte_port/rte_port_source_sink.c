...
#define RTE_PORT_SOURCE_STATS_PKTS_IN_ADD(port, val) port->stats.n_pkts_in += val
#define RTE_PORT_SOURCE_STATS_PKTS_DROP_ADD(port, val) port->stats.n_pkts_drop += val
...
static int pcap_source_load(struct rte_port_source *port, const char *file_name, uint32_t n_bytes_per_pkt,
                            int socket_id) {
    uint32_t n_pkts = 0;
    uint32_t i;
    ...
    const uint8_t *pkt;
    uint8_t *buff = NULL;
    uint32_t pktmbuf_maxlen = (uint32_t)(rte_pktmbuf_data_room_size(port->mempool) - RTE_PKTMBUF_HEADROOM);
    ...
    port->pkt_len = rte_zmalloc_socket("PCAP", (sizeof(*port->pkt_len) * n_pkts), 0, socket_id);
    ...
    port->pkts = rte_zmalloc_socket("PCAP", (sizeof(*port->pkts) * n_pkts), 0, socket_id);
    if (port->pkts == NULL) {
        RTE_LOG(ERR, PORT, "No enough memory\n");
        goto error_exit;
    }
...
#define PCAP_SOURCE_LOAD(port, file_name, n_bytes, socket_id) pcap_source_load(port, file_name, n_bytes, socket_id)
...
static void *rte_port_source_create(void *params, int socket_id) {
    ...
    if (p->file_name) {
        int status = PCAP_SOURCE_LOAD(port, p->file_name, p->n_bytes_per_pkt, socket_id);
...
static int rte_port_source_rx(void *port, struct rte_mbuf **pkts, uint32_t n_pkts) {
    struct rte_port_source *p = port;
    uint32_t i;
...
static int rte_port_source_stats_read(void *port, struct rte_port_in_stats *stats, int clear) {
    struct rte_port_source *p = port;
...
#define RTE_PORT_SINK_STATS_PKTS_IN_ADD(port, val) (port->stats.n_pkts_in += val)
#define RTE_PORT_SINK_STATS_PKTS_DROP_ADD(port, val) (port->stats.n_pkts_drop += val)
...
static int pcap_sink_open(struct rte_port_sink *port, const char *file_name, uint32_t max_n_pkts) {
    pcap_t *tx_pcap;
...
static void pcap_sink_write_pkt(struct rte_port_sink *port, struct rte_mbuf *mbuf) {
    ...
    if (mbuf->nb_segs > 1) {
        ...
        for (jumbo_mbuf = mbuf; jumbo_mbuf != NULL; jumbo_mbuf = jumbo_mbuf->next) {
            rte_memcpy(&jumbo_pkt_buf[pkt_index], rte_pktmbuf_mtod(jumbo_mbuf, uint8_t *), jumbo_mbuf->data_len);
...
#define PCAP_SINK_OPEN(port, file_name, max_n_pkts) pcap_sink_open(port, file_name, max_n_pkts)
...
static int rte_port_sink_tx_bulk(void *port, struct rte_mbuf **pkts, uint64_t pkts_mask) {
    struct rte_port_sink *p = port;
...
static int rte_port_sink_stats_read(void *port, struct rte_port_out_stats *stats, int clear) {
    struct rte_port_sink *p = port;
