...
static inline int timer_data_valid(uint32_t id) { return !!(rte_timer_data_arr[id].internal_flags & FL_ALLOCATED); }
...
reserve:
    rte_errno = 0;
    mz = rte_memzone_reserve_aligned(mz_name, data_arr_size, SOCKET_ID_ANY, 0, RTE_CACHE_LINE_SIZE);
    if (mz == NULL) {
        if (rte_errno == EEXIST) {
...
MAP_STATIC_SYMBOL(int rte_timer_subsystem_init(void), rte_timer_subsystem_init_v1905);
 ...
 * status of the timer
 */
static int timer_set_config_state(struct rte_timer *tim, union rte_timer_status *ret_prev_status,
    ...
    while (success == 0) {
        ...
        if (prev_status.state == RTE_TIMER_RUNNING &&
            (prev_status.owner != (uint16_t)lcore_id || tim != priv_timer[lcore_id].running_tim))
            return -1;
        ...
        success = rte_atomic32_cmpset(&tim->status.u32, prev_status.u32, status.u32);
    }
 ...
 * are <= that time value.
 */
static void timer_get_prev_entries(uint64_t time_val, unsigned tim_lcore, struct rte_timer **prev,
                                   struct priv_timer *priv_timer) {
 ...
 * all skiplist levels.
 */
static void timer_get_prev_entries_for_node(struct rte_timer *tim, unsigned tim_lcore, struct rte_timer **prev,
                                            struct priv_timer *priv_timer) {
    int i;
    ...
    for (i = priv_timer[tim_lcore].curr_skiplist_depth - 1; i >= 0; i--) {
        while (prev[i]->sl_next[i] != NULL && prev[i]->sl_next[i] != tim && prev[i]->sl_next[i]->expire <= tim->expire)
 ...
 */
static void timer_add(struct rte_timer *tim, unsigned int tim_lcore, struct priv_timer *priv_timer) {
    unsigned lvl;
    ...
    const unsigned tim_level = timer_get_skiplist_level(priv_timer[tim_lcore].curr_skiplist_depth);
 ...
 */
static void timer_del(struct rte_timer *tim, union rte_timer_status prev_status, int local_is_locked,
                      struct priv_timer *priv_timer) {
    unsigned lcore_id = rte_lcore_id();
    ...
    if (tim == priv_timer[prev_owner].pending_head.sl_next[0])
        priv_timer[prev_owner].pending_head.expire = ((tim->sl_next[0] == NULL) ? 0 : tim->sl_next[0]->expire);
...
static int __rte_timer_reset(struct rte_timer *tim, uint64_t expire, uint64_t period, unsigned tim_lcore,
                             rte_timer_cb_t fct, void *arg, int local_is_locked, struct rte_timer_data *timer_data) {
    union rte_timer_status prev_status, status;
    int ret;
    unsigned lcore_id = rte_lcore_id();
...
int rte_timer_reset_v20(struct rte_timer *tim, uint64_t ticks, enum rte_timer_type type, unsigned int tim_lcore,
                        rte_timer_cb_t fct, void *arg) {
    uint64_t cur_time = rte_get_timer_cycles();
    uint64_t period;

    if (unlikely((tim_lcore != (unsigned)LCORE_ID_ANY) &&
                 !(rte_lcore_is_enabled(tim_lcore) || rte_lcore_has_role(tim_lcore, ROLE_SERVICE))))
        return -1;
    ...
    return __rte_timer_reset(tim, cur_time + ticks, period, tim_lcore, fct, arg, 0, &default_timer_data);
}
VERSION_SYMBOL(rte_timer_reset, _v20, 2.0);

int rte_timer_reset_v1905(struct rte_timer *tim, uint64_t ticks, enum rte_timer_type type, unsigned int tim_lcore,
                          rte_timer_cb_t fct, void *arg) {
    return rte_timer_alt_reset(default_data_id, tim, ticks, type, tim_lcore, fct, arg);
}
MAP_STATIC_SYMBOL(int rte_timer_reset(struct rte_timer *tim, uint64_t ticks, enum rte_timer_type type,
                                      unsigned int tim_lcore, rte_timer_cb_t fct, void *arg),
                  rte_timer_reset_v1905);
BIND_DEFAULT_SYMBOL(rte_timer_reset, _v1905, 19.05);

int __rte_experimental rte_timer_alt_reset(uint32_t timer_data_id, struct rte_timer *tim, uint64_t ticks,
                                           enum rte_timer_type type, unsigned int tim_lcore, rte_timer_cb_t fct,
                                           void *arg) {
    uint64_t cur_time = rte_get_timer_cycles();
    uint64_t period;
    struct rte_timer_data *timer_data;
    ...
    return __rte_timer_reset(tim, cur_time + ticks, period, tim_lcore, fct, arg, 0, timer_data);
}

/* loop until rte_timer_reset() succeed */
void rte_timer_reset_sync(struct rte_timer *tim, uint64_t ticks, enum rte_timer_type type, unsigned tim_lcore,
                          rte_timer_cb_t fct, void *arg) {
    while (rte_timer_reset(tim, ticks, type, tim_lcore, fct, arg) != 0)
        rte_pause();
}

static int __rte_timer_stop(struct rte_timer *tim, int local_is_locked, struct rte_timer_data *timer_data) {
    union rte_timer_status prev_status, status;
    unsigned lcore_id = rte_lcore_id();
    int ret;
    struct priv_timer *priv_timer = timer_data->priv_timer;
...
int rte_timer_stop_v20(struct rte_timer *tim) { return __rte_timer_stop(tim, 0, &default_timer_data); }
VERSION_SYMBOL(rte_timer_stop, _v20, 2.0);

int rte_timer_stop_v1905(struct rte_timer *tim) { return rte_timer_alt_stop(default_data_id, tim); }
MAP_STATIC_SYMBOL(int rte_timer_stop(struct rte_timer *tim), rte_timer_stop_v1905);
BIND_DEFAULT_SYMBOL(rte_timer_stop, _v1905, 19.05);

int __rte_experimental rte_timer_alt_stop(uint32_t timer_data_id, struct rte_timer *tim) {
    struct rte_timer_data *timer_data;
...
int rte_timer_pending(struct rte_timer *tim) { return tim->status.state == RTE_TIMER_PENDING; }
...
static void __rte_timer_manage(struct rte_timer_data *timer_data) {
    ...
    priv_timer[lcore_id].pending_head.expire = (priv_timer[lcore_id].pending_head.sl_next[0] == NULL)
                                                   ? 0
    ...
    for (tim = run_first_tim; tim != NULL; tim = next_tim) {
        ...
        if (tim->period == 0) {
            ...
            rte_wmb();
            tim->status.u32 = status.u32;
            __rte_timer_reset(tim, tim->expire + tim->period, tim->period, lcore_id, tim->f, tim->arg, 1, timer_data);
            rte_spinlock_unlock(&priv_timer[lcore_id].list_lock);
        }
    }
...
void rte_timer_manage_v20(void) { __rte_timer_manage(&default_timer_data); }
...
int __rte_experimental rte_timer_alt_manage(uint32_t timer_data_id, unsigned int *poll_lcores, int nb_poll_lcores,
    ...
    for (i = 0; i < nb_poll_lcores; i++) {
        ...
        /* if nothing to do just unlock and return */
        if (privp->pending_head.sl_next[0] == NULL || privp->pending_head.sl_next[0]->expire > cur_time) {
        ...
        privp->pending_head.expire =
            (privp->pending_head.sl_next[0] == NULL) ? 0 : privp->pending_head.sl_next[0]->expire;
    ...
    while (1) {
        ...
        if (tim->period == 0) {
            ...
            rte_wmb();
            tim->status.u32 = status.u32;
            __rte_timer_reset(tim, tim->expire + tim->period, tim->period, this_lcore, tim->f, tim->arg, 1, data);
...
int __rte_experimental rte_timer_stop_all(uint32_t timer_data_id, unsigned int *walk_lcores, int nb_walk_lcores,
    ...
    for (i = 0; i < nb_walk_lcores; i++) {
        ...
        for (tim = priv_timer->pending_head.sl_next[0]; tim != NULL; tim = next_tim) {
            next_tim = tim->sl_next[0];
...
static void __rte_timer_dump_stats(struct rte_timer_data *timer_data __rte_unused, FILE *f) {
#ifdef RTE_LIBRTE_TIMER_DEBUG
...
void rte_timer_dump_stats_v20(FILE *f) { __rte_timer_dump_stats(&default_timer_data, f); }
VERSION_SYMBOL(rte_timer_dump_stats, _v20, 2.0);

int rte_timer_dump_stats_v1905(FILE *f) { return rte_timer_alt_dump_stats(default_data_id, f); }
MAP_STATIC_SYMBOL(int rte_timer_dump_stats(FILE *f), rte_timer_dump_stats_v1905);
BIND_DEFAULT_SYMBOL(rte_timer_dump_stats, _v1905, 19.05);

int __rte_experimental rte_timer_alt_dump_stats(uint32_t timer_data_id __rte_unused, FILE *f) {
    struct rte_timer_data *timer_data;
