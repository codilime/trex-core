...
static struct rte_flow *filter_tos_flow_to_rq(uint8_t port_id, uint8_t rx_q, bool ipv6, struct rte_flow_error *error) {
...
static struct rte_flow *filter_drop_all(uint8_t port_id, struct rte_flow_error *error) {
    struct rte_flow_attr attr;
    struct rte_flow_item pattern[MAX_PATTERN_NUM];
    struct rte_flow_action action[MAX_PATTERN_NUM];
    struct rte_flow *flow = NULL;
...
static struct rte_flow *filter_pass_all_to_rx(uint8_t port_id, uint8_t rx_q, struct rte_flow_error *error) {
    struct rte_flow_attr attr;
    struct rte_flow_item pattern[MAX_PATTERN_NUM];
    struct rte_flow_action action[MAX_PATTERN_NUM];
    struct rte_flow *flow = NULL;
...
inline void check_dpdk_filter_result(struct rte_flow *flow, struct rte_flow_error *err) {
    if (flow == NULL) {
...
inline void check_dpdk_filter_result(int res, struct rte_flow_error *err) {
    if (res != 0) {
...
void CDpdkFilterPort::set_tos_filter(bool enable) {
    struct rte_flow_error error;
    if (enable) {
        m_rx_ipv4_tos = filter_tos_flow_to_rq(m_repid, m_rx_q, false, &error);
        check_dpdk_filter_result(m_rx_ipv4_tos, &error);
        m_rx_ipv6_tos = filter_tos_flow_to_rq(m_repid, m_rx_q, true, &error);
...
void CDpdkFilterPort::_set_mode(dpdk_filter_mode_t mode, bool enable) {
    switch (mode) {
    case mfDISABLE:
        break;
    case mfPASS_ALL_RX:
...
CDpdkFilterPort::~CDpdkFilterPort() { set_mode(mfDISABLE); }
