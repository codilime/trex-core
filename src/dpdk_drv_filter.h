...
class CDpdkFilterPort {
  public:
    ...
    void set_rx_q(uint8_t rx_q) { m_rx_q = rx_q; }

    void set_port_id(repid_t repid) { m_repid = repid; }

    /* in case of TCP we don't want to drop*/
    void set_drop_all_mode(bool enable) { m_drop_all = enable; }
  ...
  private:
    ...
    void _set_mode(dpdk_filter_mode_t mode, bool enable);
