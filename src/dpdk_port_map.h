...
class CTRexPortMapper {
  public:
    ...
    uint8_t get_tv_nports() { return (m_max_trex_vports); }
    void set_tv_nports(uint8_t vports) { m_max_trex_vports = vports; }

    uint8_t get_re_nports() { return (m_max_rte_eth_ports); }

    void set_re_nport(uint8_t reports) { m_max_rte_eth_ports = reports; }
...
class CTVPort {
  public:
    CTVPort(tvpid_t tvport) { m_tvport = tvport; }
    ...
    tvpid_t tvpid(void) { return (m_tvport); }

    bool is_dummy(void) { return (get_repid() == DPDK_MAP_IVALID_REPID); }
...
class CREPort {
  public:
    CREPort(repid_t repid) { m_repid = repid; }
    ...
    repid_t repid(void) { return (m_repid); }
