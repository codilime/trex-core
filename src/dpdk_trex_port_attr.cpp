...
DpdkTRexPortAttr::DpdkTRexPortAttr(uint8_t tvpid, uint8_t repid, bool is_virtual, bool fc_change_allowed,
...
void DpdkTRexPortAttr::update_description() {
    ...
    if (intf_info_st.description.find("82599ES") !=
        string::npos) { // works for 82599EB etc. DPDK does not distinguish them
        flag_is_link_change_supported = false;
    }
    if (intf_info_st.description.find("82545EM") !=
        string::npos) { // in virtual E1000, DPDK claims fc is supported, but it's not
...
void DpdkTRexPortAttr::reset_xstats() { rte_eth_xstats_reset(m_repid); }
...
void DpdkTRexPortAttr::dump_link(FILE *fd) {
    ...
    if (m_link.link_status) {
        fprintf(fd, " link : Link Up - speed %u Mbps - %s\n", (unsigned)get_link_speed(),
                (m_link.link_duplex == ETH_LINK_FULL_DUPLEX) ? ("full-duplex") : ("half-duplex\n"));
    } else {
...
void DpdkTRexPortAttr::get_hw_src_mac(struct ether_addr *mac_addr) { rte_eth_macaddr_get(m_repid, mac_addr); }
