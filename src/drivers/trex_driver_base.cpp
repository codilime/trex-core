...
port_cfg_t::port_cfg_t() {
    ...
    m_port_conf.rxmode.offloads = DEV_RX_OFFLOAD_JUMBO_FRAME | DEV_RX_OFFLOAD_SCATTER;

    tx_offloads.common_best_effort = DEV_TX_OFFLOAD_IPV4_CKSUM | DEV_TX_OFFLOAD_UDP_CKSUM | DEV_TX_OFFLOAD_TCP_CKSUM;

    tx_offloads.common_required = DEV_TX_OFFLOAD_MULTI_SEGS;

    tx_offloads.astf_best_effort = DEV_TX_OFFLOAD_TCP_TSO | DEV_TX_OFFLOAD_UDP_TSO;
}
...
void port_cfg_t::update_global_config_fdir(void) { get_ex_drv()->update_global_config_fdir(this); }

void CTRexExtendedDriverDb::register_driver(std::string name, create_object_t func) {
...
void CTRexExtendedDriverBase::wait_after_link_up() { wait_x_sec(CGlobalInfo::m_options.m_wait_before_traffic); }
...
bool CTRexExtendedDriverBase::get_extended_stats_fixed(CPhyEthIF *_if, CPhyEthIFStats *stats, int fix_i, int fix_o) {
    ...
    stats->f_ipackets += 0;
    stats->f_ibytes += 0;
    stats->ierrors += stats1.imissed + stats1.ierrors + stats1.rx_nombuf - prev_stats->imissed - prev_stats->ierrors -
                      prev_stats->rx_nombuf;
...
CTRexExtendedDriverBase *get_ex_drv() { return (CTRexExtendedDriverDb::Ins()->get_drv()); }
...
int CTRexExtendedDriverDummySelector::add_del_rx_flow_stat_rule(CPhyEthIF *_if, enum rte_filter_op op, uint16_t l3,
                                                                uint8_t l4, uint8_t ipv6_next_h, uint16_t id) {
    if (_if->is_dummy()) {
        return 0;
    } else {
...
bool CTRexExtendedDriverDummySelector::get_extended_stats_fixed(CPhyEthIF *_if, CPhyEthIFStats *stats, int fix_i,
                                                                int fix_o) {
    if (_if->is_dummy()) {
        return 0;
    } else {
...
int CTRexExtendedDriverDummySelector::get_rx_stats(CPhyEthIF *_if, uint32_t *pkts, uint32_t *prev_pkts, uint32_t *bytes,
