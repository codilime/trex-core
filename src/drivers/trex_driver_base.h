...
class CTRexExtendedDriverBase {
  ...
  public:
    ...
    virtual int add_del_rx_flow_stat_rule(CPhyEthIF *_if, enum rte_filter_op op, uint16_t l3, uint8_t l4,
                                          uint8_t ipv6_next_h, uint16_t id) {
        return 0;
    }

    bool hardware_support_mac_change() { return ((m_cap & TREX_DRV_CAP_MAC_ADDR_CHG) != 0); }

    bool drop_packets_incase_of_linkdown() { return ((m_cap & TREX_DRV_CAP_DROP_PKTS_IF_LNK_DOWN) != 0); }
    ...
    virtual bool sleep_after_arp_needed() { return (false); }
    ...
    uint32_t get_capabilities(void) { return m_cap; }

    /* return true in case the driver can get 2K buffers and can make then one big 9K buffer
       e.g. mlx5 and napatech can't do that
    */
    virtual bool is_support_for_rx_scatter_gather() { return (true); }

    virtual bool is_override_dpdk_params(CTrexDpdkParamsOverride &dpdk_p) { return (false); }
...
class CTRexExtendedDriverDummySelector : public CTRexExtendedDriverBase {
  public:
    ...
    int get_min_sample_rate(void) { return m_real_drv->get_min_sample_rate(); }
    void update_configuration(port_cfg_t *cfg) { m_real_drv->update_configuration(cfg); }
    void update_global_config_fdir(port_cfg_t *cfg) { m_real_drv->update_global_config_fdir(cfg); }
    int configure_rx_filter_rules(CPhyEthIF *_if);
    int add_del_rx_flow_stat_rule(CPhyEthIF *_if, enum rte_filter_op op, uint16_t l3, uint8_t l4, uint8_t ipv6_next_h,
                                  uint16_t id);
    ...
    int wait_for_stable_link() { return m_real_drv->wait_for_stable_link(); }
    bool sleep_after_arp_needed() { return m_real_drv->sleep_after_arp_needed(); }
    void wait_after_link_up() { m_real_drv->wait_after_link_up(); }
    bool hw_rx_stat_supported() { return m_real_drv->hw_rx_stat_supported(); }
    ...
    CFlowStatParser *get_flow_stat_parser() { return m_real_drv->get_flow_stat_parser(); }

    bool is_support_for_rx_scatter_gather() { return m_real_drv->is_support_for_rx_scatter_gather(); }
...
class CTRexExtendedDriverDb {
  public:
    const std::string &get_driver_name() { return m_driver_name; }
