...
static uint16_t all_eth_types[] = {
    0x0800, 0x0806, 0x0842, 0x22F3, 0x22EA, 0x6003, 0x8035, 0x809B, 0x80F3, 0x8100, 0x8137, 0x8204, 0x86DD,
    0x8808, 0x8809, 0x8819, 0x8847, 0x8848, 0x8863, 0x8864, 0x886D, 0x8870, 0x887B, 0x888E, 0x8892, 0x889A,
    0x88A2, 0x88A4, 0x88A8, 0x88AB, 0x88B8, 0x88B9, 0x88BA, 0x88CC, 0x88CD, 0x88DC, 0x88E1, 0x88E3, 0x88E5,
    0x88E7, 0x88F7, 0x88FB, 0x8902, 0x8906, 0x8914, 0x8915, 0x891D, 0x892F, 0x9000, 0x9100,
};
...
int CTRexExtendedDriverBase40G::get_min_sample_rate(void) { return (RX_CHECK_MIX_SAMPLE_RATE); }

void CTRexExtendedDriverBase40G::clear_extended_stats(CPhyEthIF *_if) { rte_eth_stats_reset(_if->get_repid()); }
...
// ttl is used in statefull mode, and ip_id in stateless. We configure the driver registers so that only one of them
// applies. So, the rule will apply if packet has either the correct ttl or IP ID, depending if we are in statfull or
// stateless.
...
int CTRexExtendedDriverBase40G::configure_rx_filter_rules_statefull(CPhyEthIF *_if) {
    ...
    for (i = 0; i < 10; i++) {
        ...
        add_del_rules(RTE_ETH_FILTER_ADD, repid, RTE_ETH_FLOW_NONFRAG_IPV6_UDP, ttl, 0, RX_CHECK_V6_OPT_TYPE,
                      MAIN_DPDK_RX_Q, 0);
        add_del_rules(RTE_ETH_FILTER_ADD, repid, RTE_ETH_FLOW_NONFRAG_IPV6_TCP, ttl, 0, RX_CHECK_V6_OPT_TYPE,
                      MAIN_DPDK_RX_Q, 0);
        add_del_rules(RTE_ETH_FILTER_ADD, repid, RTE_ETH_FLOW_NONFRAG_IPV6_OTHER, ttl, 0, RX_CHECK_V6_OPT_TYPE,
                      MAIN_DPDK_RX_Q, 0);
        /* Rules for latency measurement packets */
        add_del_rules(RTE_ETH_FILTER_ADD, repid, RTE_ETH_FLOW_NONFRAG_IPV4_OTHER, ttl, 0, IPPROTO_ICMP, MAIN_DPDK_RX_Q,
                      0);
...
int CTRexExtendedDriverBase40G::configure_rx_filter_rules(CPhyEthIF *_if) {
    ...
    if (get_is_stateless()) {
        ...
        add_del_rules(RTE_ETH_FILTER_ADD, repid, RTE_ETH_FLOW_NONFRAG_IPV4_UDP, 0, FLOW_STAT_PAYLOAD_IP_ID, 0,
                      MAIN_DPDK_RX_Q, FDIR_PAYLOAD_RULES_HW_ID);
        add_del_rules(RTE_ETH_FILTER_ADD, repid, RTE_ETH_FLOW_NONFRAG_IPV4_TCP, 0, FLOW_STAT_PAYLOAD_IP_ID, 0,
                      MAIN_DPDK_RX_Q, FDIR_PAYLOAD_RULES_HW_ID);
        add_del_rules(RTE_ETH_FILTER_ADD, repid, RTE_ETH_FLOW_NONFRAG_IPV4_OTHER, 0, FLOW_STAT_PAYLOAD_IP_ID,
                      IPPROTO_ICMP, MAIN_DPDK_RX_Q, FDIR_PAYLOAD_RULES_HW_ID);

        add_del_rules(RTE_ETH_FILTER_ADD, repid, RTE_ETH_FLOW_FRAG_IPV4, 0, FLOW_STAT_PAYLOAD_IP_ID, 0, MAIN_DPDK_RX_Q,
                      FDIR_PAYLOAD_RULES_HW_ID);

        add_del_rules(RTE_ETH_FILTER_ADD, repid, RTE_ETH_FLOW_NONFRAG_IPV6_UDP, 0, FLOW_STAT_PAYLOAD_IP_ID, 0,
                      MAIN_DPDK_RX_Q, FDIR_PAYLOAD_RULES_HW_ID);
        add_del_rules(RTE_ETH_FILTER_ADD, repid, RTE_ETH_FLOW_NONFRAG_IPV6_TCP, 0, FLOW_STAT_PAYLOAD_IP_ID, 0,
                      MAIN_DPDK_RX_Q, FDIR_PAYLOAD_RULES_HW_ID);
        add_del_rules(RTE_ETH_FILTER_ADD, repid, RTE_ETH_FLOW_NONFRAG_IPV6_OTHER, 0, FLOW_STAT_PAYLOAD_IP_ID, 0,
                      MAIN_DPDK_RX_Q, FDIR_PAYLOAD_RULES_HW_ID);

        add_del_rules(RTE_ETH_FILTER_ADD, repid, RTE_ETH_FLOW_FRAG_IPV6, 0, FLOW_STAT_PAYLOAD_IP_ID, 0, MAIN_DPDK_RX_Q,
                      FDIR_PAYLOAD_RULES_HW_ID);
...
int CTRexExtendedDriverBase40G::get_rx_stats(CPhyEthIF *_if, uint32_t *pkts, uint32_t *prev_pkts, uint32_t *bytes,
    ...
    for (int i = min; i <= max; i++) {
        if (unlikely(hw_stats[i - min] > CGlobalInfo::m_options.get_x710_fdir_reset_threshold())) {
            ...
            add_del_rules(RTE_ETH_FILTER_ADD, repid, fdir_hw_id_rule_params[hw_id].rule_type, 0, IP_ID_RESERVE_BASE + i,
                          fdir_hw_id_rule_params[hw_id].l4_proto, MAIN_DPDK_DROP_Q, FDIR_TEMP_HW_ID);
            rte_eth_fdir_stats_reset(repid, &counter, hw_id, 1);
            add_del_rules(RTE_ETH_FILTER_ADD, repid, fdir_hw_id_rule_params[hw_id].rule_type, 0, IP_ID_RESERVE_BASE + i,
                          fdir_hw_id_rule_params[hw_id].l4_proto, MAIN_DPDK_DROP_Q, hw_id);
...
int CTRexExtendedDriverBase40G::verify_fw_ver(tvpid_t tvpid) {
    ...
    if (ret == 0) {
        ...
        if ((((version >> 12) & 0xf) < 5) ||
            ((((version >> 12) & 0xf) == 5) && ((version >> 4 & 0xff) == 0) && ((version & 0xf) < 4))) {
...
void CTRexExtendedDriverBase40G::get_rx_stat_capabilities(uint16_t &flags, uint16_t &num_counters,
                                                          uint16_t &base_ip_id) {
...
bool CTRexExtendedDriverBase40G::hw_rx_stat_supported() {
    if (CGlobalInfo::m_options.preview.get_disable_hw_flow_stat() || (!get_dpdk_mode()->is_hardware_filter_needed())) {
        return false;
    } else {
        return true;
    }
