...
class CTRexExtendedDriverBase40G : public CTRexExtendedDriverBase {
  public:
    ...
    static CTRexExtendedDriverBase *create() { return (new CTRexExtendedDriverBase40G()); }
    ...
    virtual int add_del_rx_flow_stat_rule(CPhyEthIF *_if, enum rte_filter_op op, uint16_t l3_proto, uint8_t l4_proto,
  ...
  private:
    virtual void add_del_rules(enum rte_filter_op op, repid_t repid, uint16_t type, uint8_t ttl, uint16_t ip_id,
                               uint8_t l4_proto, int queue, uint16_t stat_idx);
