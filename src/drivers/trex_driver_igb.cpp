...
CTRexExtendedDriverBase1G::CTRexExtendedDriverBase1G() { m_cap = tdCAP_ALL_NO_RSS | TREX_DRV_CAP_MAC_ADDR_CHG; }
...
int CTRexExtendedDriverBase1G::get_min_sample_rate(void) { return (RX_CHECK_MIX_SAMPLE_RATE_1G); }

// in 1G we need to wait if links became ready to soon
void CTRexExtendedDriverBase1G::wait_after_link_up() { wait_x_sec(6 + CGlobalInfo::m_options.m_wait_before_traffic); }
...
int CTRexExtendedDriverBase1G::configure_rx_filter_rules_statefull(CPhyEthIF *_if) {
    ...
    _if->pci_reg_write(E1000_TTQF(0), protocol | 0x00008100 /* enable */
    ...
    */
    ...
    uint16_t ff_rules_v4[6] = {
        (uint16_t)(0xFF06 - v4_hops), (uint16_t)(0xFE11 - v4_hops), (uint16_t)(0xFF11 - v4_hops),
        (uint16_t)(0xFE06 - v4_hops), (uint16_t)(0xFF01 - v4_hops), (uint16_t)(0xFE01 - v4_hops),
    };
...
bool CTRexExtendedDriverBase1G::get_extended_stats(CPhyEthIF *_if, CPhyEthIFStats *stats) {
    ...
    stats->ierrors +=
        (_if->pci_reg_read(E1000_RNBC) + _if->pci_reg_read(E1000_CRCERRS) + _if->pci_reg_read(E1000_ALGNERRC) +
         _if->pci_reg_read(E1000_SYMERRS) + _if->pci_reg_read(E1000_RXERRC) +

         _if->pci_reg_read(E1000_ROC) + _if->pci_reg_read(E1000_RUC) + _if->pci_reg_read(E1000_RJC) +

         _if->pci_reg_read(E1000_XONRXC) + _if->pci_reg_read(E1000_XONTXC) + _if->pci_reg_read(E1000_XOFFRXC) +
         _if->pci_reg_read(E1000_XOFFTXC) + _if->pci_reg_read(E1000_FCRUC));
...
void CTRexExtendedDriverBase1G::get_rx_stat_capabilities(uint16_t &flags, uint16_t &num_counters,
                                                         uint16_t &base_ip_id) {
    flags =
