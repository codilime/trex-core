...
CTRexExtendedDriverBase10G::CTRexExtendedDriverBase10G() { m_cap = tdCAP_ALL | TREX_DRV_CAP_MAC_ADDR_CHG; }

TRexPortAttr *CTRexExtendedDriverBase10G::create_port_attr(tvpid_t tvpid, repid_t repid) {
    return new DpdkTRexPortAttr(tvpid, repid, false, true, true, false, true);
}

int CTRexExtendedDriverBase10G::get_min_sample_rate(void) { return (RX_CHECK_MIX_SAMPLE_RATE); }

void CTRexExtendedDriverBase10G::clear_extended_stats(CPhyEthIF *_if) { _if->pci_reg_read(IXGBE_RXNFGPC); }
...
int CTRexExtendedDriverBase10G::configure_rx_filter_rules_statefull(CPhyEthIF *_if) {
    ...
    _if->pci_reg_write(IXGBE_FTQF(0), IXGBE_FTQF_PROTOCOL_SCTP |
...
bool CTRexExtendedDriverBase10G::get_extended_stats(CPhyEthIF *_if, CPhyEthIFStats *stats) {
    ...
    stats->ierrors +=
        (_if->pci_reg_read(IXGBE_RLEC) + _if->pci_reg_read(IXGBE_ERRBC) + _if->pci_reg_read(IXGBE_CRCERRS) +
         _if->pci_reg_read(IXGBE_ILLERRC) + _if->pci_reg_read(IXGBE_ROC) + _if->pci_reg_read(IXGBE_RUC) + t);
...
CFlowStatParser *CTRexExtendedDriverBase10G::get_flow_stat_parser() {
    CFlowStatParser *parser =
        new CFlowStatParser((CGlobalInfo::m_options.preview.get_vlan_mode() != CPreviewMode::VLAN_MODE_NONE)
...
void CTRexExtendedDriverBase10G::get_rx_stat_capabilities(uint16_t &flags, uint16_t &num_counters,
                                                          uint16_t &base_ip_id) {
    flags =
