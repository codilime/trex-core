...
std::string &get_mlx5_so_string(void) { return CTRexExtendedDriverBaseMlnx5G::mlx5_so_str; }
...
int CTRexExtendedDriverBaseMlnx5G::get_min_sample_rate(void) { return (RX_CHECK_MIX_SAMPLE_RATE); }
...
int CTRexExtendedDriverBaseMlnx5G::get_rx_stats(CPhyEthIF *_if, uint32_t *pkts, uint32_t *prev_pkts, uint32_t *bytes,
...
bool CTRexExtendedDriverBaseMlnx5G::get_extended_stats(CPhyEthIF *_if, CPhyEthIFStats *stats) {
    ...
    uint32_t opackets = xstats[tx_port_unicast_packets].value + xstats[tx_port_multicast_packets].value +
                        xstats[tx_port_broadcast_packets].value;
    uint32_t ipackets = xstats[rx_port_unicast_packets].value + xstats[rx_port_multicast_packets].value +
                        xstats[rx_port_broadcast_packets].value;
    uint64_t obytes = xstats[tx_port_unicast_bytes].value + xstats[tx_port_multicast_bytes].value +
                      xstats[tx_port_broadcast_bytes].value;
    uint64_t ibytes = xstats[rx_port_unicast_bytes].value + xstats[rx_port_multicast_bytes].value +
    ...
    uint64_t ierrors = xstats[rx_wqe_err].value + xstats[rx_crc_errors_phy].value +
                       xstats[rx_in_range_len_errors_phy].value + xstats[rx_symbol_err_phy].value +
    ...
    if (!xstats_struct->init) {
        ...
        stats->obytes += obytes - prev_stats->obytes + packet_diff * 4; // add FCS

        packet_diff = ipackets - (uint32_t)prev_stats->ipackets;
        stats->ipackets += packet_diff;
        stats->ibytes += ibytes - prev_stats->ibytes + packet_diff * 4; // add FCS

        stats->ierrors +=
            imissed - prev_stats->imissed + rx_nombuf - prev_stats->rx_nombuf + ierrors - prev_stats->ierrors;
...
void CTRexExtendedDriverBaseMlnx5G::get_rx_stat_capabilities(uint16_t &flags, uint16_t &num_counters,
                                                             uint16_t &base_ip_id) {
    flags =
        TrexPlatformApi::IF_STAT_IPV4_ID | TrexPlatformApi::IF_STAT_RX_BYTES_COUNT | TrexPlatformApi::IF_STAT_PAYLOAD;
