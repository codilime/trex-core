...
CTRexExtendedDriverBaseVIC::CTRexExtendedDriverBaseVIC() { m_cap = tdCAP_ALL | TREX_DRV_CAP_MAC_ADDR_CHG; }
...
int CTRexExtendedDriverBaseVIC::get_min_sample_rate(void) { return (RX_CHECK_MIX_SAMPLE_RATE); }
...
void CTRexExtendedDriverBaseVIC::add_del_rules(enum rte_filter_op op, repid_t repid, uint16_t type, uint16_t id,
...
int CTRexExtendedDriverBaseVIC::verify_fw_ver(tvpid_t tvpid) {
    ...
    printf(
        "Warning: In order to fully utilize the VIC NIC, firmware should be upgraded to support advanced filtering \n");
...
int CTRexExtendedDriverBaseVIC::get_rx_stats(CPhyEthIF *_if, uint32_t *pkts, uint32_t *prev_pkts, uint32_t *bytes,
...
void CTRexExtendedDriverBaseVIC::get_rx_stat_capabilities(uint16_t &flags, uint16_t &num_counters,
                                                          uint16_t &base_ip_id) {
    flags =
