...
class CTrexGlobalIoMode {
  public:
    CTrexGlobalIoMode() { m_g_disable_first = false; }
    ...
    */
    ...
    enum PerPortCountersMode { ppDISABLE = 0, ppTABLE = 1, ppSTANDARD = 2, ppLAST = 3 };
    typedef uint8_t PerPortCountersMode_t;

    enum AllPortCountersMode { apDISABLE = 0, apENABLE = 1, apLAST = 2 };
    typedef uint8_t AllPortCountersMode_t;

    enum LatecnyMode { lDISABLE = 0, lENABLE = 1, lENABLE_Extended = 2, lLAST = 3 };
    typedef uint8_t LatecnyMode_t;

    enum RxCheckMode { rcDISABLE = 0, rcENABLE = 1, rcENABLE_Extended = 2, rcLAST = 3 };
    typedef uint8_t RxCheckMode_t;

    enum NatMode { natDISABLE = 0, natENABLE = 1, natLAST = 2 };
