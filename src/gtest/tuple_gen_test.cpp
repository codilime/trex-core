...
class CClientInfoUT {
  public:
    CClientInfoUT(CClientInfo *a) { obj = a; }

    uint16_t m_head_port_get() { return obj->m_head_port; }

    void m_head_port_set(uint16_t head) { obj->m_head_port = head; }

    bool is_port_legal(uint16_t port) { return obj->is_port_legal(port); }
    bool is_port_free(uint16_t port) { return obj->is_port_available(port); }
    void m_bitmap_port_set_bit(uint16_t port) { obj->m_bitmap_port[port] = 1; }
    void m_bitmap_port_reset_bit(uint16_t port) { obj->m_bitmap_port[port] = 0; }
    uint8_t m_bitmap_port_get_bit(uint16_t port) { return obj->m_bitmap_port[port]; }
    void get_next_free_port_by_bit() { obj->get_next_free_port_by_bit(); }
...
TEST(tuple_gen, clientPoolL) {
    CClientPool gen;
    gen.Create(cdSEQ_DIST, 0x10000001, 0x10000f01, 64000, g_dummy, 0, 0);
    CTupleBase result;
    uint32_t result_src;
...
TEST(tuple_gen, clientPool) {
    CClientPool gen;
    gen.Create(cdSEQ_DIST, 0x10000001, 0x10000021, 64000000, g_dummy, 0, 0);
    CTupleBase result;
    uint32_t result_src;
    uint16_t result_port;
...
TEST(tuple_gen, serverPool) {
    CServerPool gen;
    gen.Create(cdSEQ_DIST, 0x30000001, 0x30000ff1, 6400000);
    CTupleBase result;
    ...
    gen.Create(cdSEQ_DIST, 0x30000001, 0x30000003, 64000000);
...
TEST(tuple_gen, servePoolSim) {
    ...
    gen.Create(cdSEQ_DIST, 0x30000001, 0x40000001, 640000);
    CTupleBase result;
    uint32_t result_dest;
...
TEST(tuple_gen, GenerateTuple2) {
    CClientPool c_gen;
    CClientPool c_gen_2;
    c_gen.Create(cdSEQ_DIST, 0x10000001, 0x1000000f, 64000 * 4, g_dummy, 0, 0);
    CServerPool s_gen;
    CServerPool s_gen_2;
    s_gen.Create(cdSEQ_DIST, 0x30000001, 0x30000ff1, 640000);
    CTupleBase result;
    ...
    c_gen.Delete();
    //    EXPECT_EQ((size_t)0, gen.m_clients.size());
    c_gen.Create(cdSEQ_DIST, 0x10000001, 0x1000000f, 64000 * 400, g_dummy, 0, 0);
    s_gen.Create(cdSEQ_DIST, 0x30000001, 0x30000001, 640000);
    for (int i = 0; i < 200; i++) {
        s_gen.GenerateTuple(result);
        c_gen.GenerateTuple(result);
        //    gen.Dump(stdout);
...
int test_gen_astf_rss(uint16_t rss_thread_id, uint16_t rss_thread_max, uint8_t reta_mask, int cnt) {
    CTupleGeneratorSmart gen;
    gen.Create(1, 1);
    gen.set_astf_rss_mode(rss_thread_id, rss_thread_max, reta_mask);
    gen.add_client_pool(cdSEQ_DIST, 0x10000001, 0x10000001, 64000, g_dummy, 0, 0);
    gen.add_server_pool(cdSEQ_DIST, 0x30000001, 0x400000ff, 64000, false);
    CTupleTemplateGeneratorSmart template_1;
    template_1.Create(&gen, 0, 0);
        ...
        if (result.getClientPort() != 0) {
            ...
            if (calc_thread_id != rss_thread_id) {
                printf(" port:%x reverse_port_mask: %x  thread_id: %x original_thread_id: %d \n",
                       result.getClientPort(), (rss_reverse_bits_port(result.getClientPort()) & reta_mask),
...
TEST(tuple_gen, astf_mode3) {
    ...
    for (tcores = 2; tcores < 7; tcores++) {
        for (cid = 0; cid < tcores; cid++) {
            test_gen_astf_rss(cid, tcores, reta_mask, 40000);
        }
    }
}
