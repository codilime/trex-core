...
void CHTimerObj::Dump(FILE *fd) { fprintf(fd, "m_tick_left        :%lu \n", (ulong)m_ticks_left); }
...
void CHTimerOneWheel::dump_link_list(uint32_t bucket_index, void *userdata, htw_on_tick_cb_t cb, FILE *fd) {
...
RC_HTW_t CHTimerWheel::timer_start_rest(CHTimerObj *tmr, htw_ticks_t ticks) {
    int i;
...
RC_HTW_t CHTimerWheel::Create(uint32_t wheel_size, uint32_t num_wheels) {
    RC_HTW_t res;
...
uint16_t CNATimerWheel::on_tick_level_count(int level, void *userdata, htw_on_tick_cb_t cb, uint16_t min_events,
                                            uint32_t &left) {
...
RC_HTW_t CNATimerWheel::timer_start_rest(CHTimerObj *tmr, htw_ticks_t ticks) {
...
void CNATimerWheel::set_level1_cnt_div() { m_cnt_div = (1 << m_wheel_level1_shift); }

RC_HTW_t CNATimerWheel::Create(uint32_t wheel_size, uint8_t level1_div) {
    RC_HTW_t res;
    int i;
    for (i = 0; i < HNA_TIMER_LEVELS; i++) {
        res = m_timer_w[i].Create(wheel_size);
