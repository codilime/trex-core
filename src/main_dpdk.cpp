...
static inline int get_min_sample_rate(void) { return (get_ex_drv()->get_min_sample_rate()); }
...
static int COLD_FUNC usage() {
    ...
    printf(" --astf                     : Enable advanced stateful mode. profile should be in py format and not YAML "
           "format \n");
    printf(" --astf-server-only         : Only server  side ports (1,3..) are enabled with ASTF service. Traffic won't "
           "be transmitted on clients ports. \n");
    printf(" --astf-client-mask         : Enable only specific client side ports with ASTF service. \n");
    printf("                              For example, with 4 ports setup. 0x1 means that only port 0 will be enabled. "
           "ports 2 won't be enabled. \n");
    printf("                              Can't be used with --astf-server-only. \n");
    printf("\n");
    printf(" --stl                      : Starts in stateless mode. must be provided along with '-i' for interactive "
           "mode \n");
    printf(" --active-flows             : An experimental switch to scale up or down the number of active flows.  \n");
    printf("                              It is not accurate due to the quantization of flow scheduler and in some "
           "case does not work. \n");
    printf("                              Example --active-flows 500000 wil set the ballpark of the active flow to be "
           "~0.5M \n");
    printf(" --allow-coredump           : Allow creation of core dump \n");
    printf(" --arp-refresh-period       : Period in seconds between sending of gratuitous ARP for our addresses. Value "
           "of 0 means 'never send' \n");
    printf(" -c <num>                   : Number of hardware threads to allocate for each port pair. Overrides the 'c' "
           "argument from config file \n");
    printf(" --cfg <file>               : Use file as TRex config file instead of the default /etc/trex_cfg.yaml \n");
    printf(" --checksum-offload         : Deprecated,enable by default. Enable IP, TCP and UDP tx checksum offloading, "
           "using DPDK. This requires all used interfaces to support this  \n");
    printf(" --checksum-offload-disable : Disable IP, TCP and UDP tx checksum offloading, using DPDK. This requires "
           "all used interfaces to support this  \n");
    ...
    printf(" --close-at-end             : Call rte_eth_dev_stop and close at exit. Calling these functions caused link "
           "down issues in older versions, \n");
    printf("                               so we do not call them by default for now. Leaving this as option in case "
           "someone thinks it is helpful for him \n");
    ...
    printf(" -e                         : Like -p but src/dst IP will be chosen according to the port (i.e. on client "
           "port send all packets with client src and server dest, and vice versa on server port) \n");
    printf(" --flip                     : Each flow will be sent both from client to server and server to client. This "
           "can achieve better port utilization when flow traffic is asymmetric \n");
    printf(
        " --hdrh                     : Report latency using high dynamic range histograms (http://hdrhistogram.org)\n");
    printf(" --hops <hops>              : If rx check is enabled, the hop number can be assigned. See manual for "
           "details \n");
    ...
    printf(" -l <rate>                  : In parallel to the test, run latency check, sending packets at rate/sec from "
           "each interface \n");
    ...
    printf("      2              Send ICMP requests from client side, and response from server side (for working with "
           "firewall) \n");
    ...
    printf(" --learn-mode [1-3]         : Used for working in NAT environments. Dynamically learn the NAT translation "
           "done by the DUT \n");
    printf("      1    In case of TCP flow, use TCP ACK in first SYN to pass NAT translation information. Initial SYN "
           "packet must be first packet in the TCP flow \n");
    printf("           In case of UDP stream, NAT translation information will pass in IP ID field of first packet in "
           "flow. This means that this field is changed by TRex\n");
    printf("      2    Add special IP option to pass NAT translation information to first packet of each flow. Will "
           "not work on certain firewalls if they drop packets with IP options \n");
    printf("      3    Like 1, but without support for sequence number randomization in server->client direction. "
           "Performance (flow/second) better than 1 \n");
    printf(" --learn-verify             : Test the NAT translation mechanism. Should be used when there is no NAT in "
           "the setup \n");
    printf(" --limit-ports              : Limit number of ports used. Must be even number (TRex always uses port "
           "pairs) \n");
    ...
    printf(" --nc                       : If set, will not wait for all flows to be closed, before terminating - see "
           "manual for more information \n");
    printf(" --no-flow-control-change   : By default TRex disables flow-control. If this option is given, it does not "
           "touch it \n");
    printf(" --no-hw-flow-stat          : Relevant only for Intel x710 stateless mode. Do not use HW counters for flow "
           "stats\n");
    printf("                            : Enabling this will support lower traffic rate, but will also report RX byte "
           "count statistics. See manual for more details\n");
    ...
    printf(" -p                         : Send all flow packets from the same interface (choosed randomly between "
           "client ad server ports) without changing their src/dst IP \n");
    printf(" -pm                        : Platform factor. If you have splitter in the setup, you can multiply the "
           "total results by this factor \n");
    printf("                              e.g --pm 2.0 will multiply all the results bps in this factor \n");
    printf(" --prefix <nam>             : For running multi TRex instances on the same machine. Each instance should "
           "have different name \n");
    ...
    printf(" --rx-check  <rate>         : Enable rx check. TRex will sample flows at 1/rate and check order, latency "
           "and more \n");
    ...
    printf(" --send-debug-pkt <proto>   : Do not run traffic generator. Just send debug packet and dump receive queues "
           "\n");
    printf("                              Supported protocols are 1 for icmp, 2 for UDP, 3 for TCP, 4 for ARP, 5 for "
           "9K UDP \n");
    printf(" --sleeps                   : Use sleeps instead of busy wait in scheduler (less accurate, more power "
           "saving)\n");
    printf(" --software                 : Do not configure any hardware rules. In this mode we use 1 core, and one RX "
           "queue and one TX queue per port\n");
    printf(" --unbind-unused-ports      : Automatically unbind all unused bound ports in same NIC instead of exiting "
           "with error (i40e only)\n");
    ...
    printf("                              When configuring flow stat and latency per stream rules, assume all streams "
           "uses VLAN \n");
    printf(" -w  <num>                  : Wait num seconds between init of interfaces and sending traffic, default is "
           "1 \n");
    printf(" --latency-measurement <md> : Define what method (<md>) is be used to provide timestamps used in latency "
           "calculation. Overrides the 'latency_measurement'\n");
    printf("                              argument from config file. Supported method are 1 for nanoseconds "
           "(clock_gettime), 0 (default) for standard system ticks (RDTSC)\n");
    printf(" --timesync-method <method> : Enable time synchronisation with given method. Overrides the "
           "'timesync_method' argument from config file\n");
    printf("                              Supported method is 1 for PTP. Default is 0 for no synchronisation\n");
    printf(" --timesync-period <num>    : Define how often (in seconds) will time synchronisation take place. "
           "Overrides the 'timesync_period' argument from config file\n");
 ...
 *
 */
static void check_exclusive(const OptHash &args_set, const std::initializer_list<int> &opts,
...
struct ParsingOptException : public std::exception {
    ...
    ParsingOptException(CSimpleOpt &args) : m_err_code(args.LastError()), m_opt_text(args.OptionText()) {}
};

static OptHash args_first_pass(int argc, char *argv[], CParserOption *po) {
...
COLD_FUNC static int parse_options(int argc, char *argv[], bool first_time) {
    ...
    while (args.Next()) {
        if (args.LastError() == SO_SUCCESS) {
            ...
            case OPT_LEARN_MODE:
                ...
                if (!po->is_valid_opt_val(tmp_data, CParserOption::LEARN_MODE_DISABLED, CParserOption::LEARN_MODE_MAX,
                                          "--learn-mode")) {
                    exit(-1);
                }
            ...
            case OPT_LEARN_VERIFY:
                // must configure learn_mode for learn verify to work. If different learn mode will be given later, it
                // will be set instead.
            ...
            case OPT_LATENCY_MEASUREMENT_METHOD:
                sscanf(args.OptionArg(), "%d", &tmp_data);
                if (!po->is_valid_opt_val(tmp_data, CParserOption::LATENCY_METHOD_TICKS,
                                          CParserOption::LATENCY_METHOD_NANOS, "--latency-measurement")) {
                    exit(-1);
                }
            ...
            case OPT_TIME_SYNC_METHOD:
                sscanf(args.OptionArg(), "%d", &tmp_data);
                if (!po->is_valid_opt_val(tmp_data, CParserOption::TIMESYNC_NONE, CParserOption::TIMESYNC_PTP,
                                          "--timesync-method")) {
                    exit(-1);
                }
    ...
    if (CGlobalInfo::is_learn_mode() && po->preview.get_ipv6_mode_enable()) {
        parse_err("--learn mode is not supported with --ipv6, beacuse there is no such thing as NAT66 (ipv6 to ipv6 "
                  "translation) \n"
    ...
    if (po->preview.get_is_rx_check_enable() || po->is_latency_enabled() || CGlobalInfo::is_learn_mode() ||
        (CGlobalInfo::m_options.m_arp_ref_per != 0) || (!get_dpdk_mode()->is_hardware_filter_needed())) {
        po->set_rx_enabled();
    }
...
COLD_FUNC void CPhyEthIF::dump_stats_extended(FILE *fd) {
    ...
    cnt_name_t reg[] = {
        ...
        MY_REG(IXGBE_GPRC),       MY_REG(IXGBE_GORCL),       MY_REG(IXGBE_GORCH),

        MY_REG(IXGBE_RXNFGPC),    MY_REG(IXGBE_RXNFGBCL),    MY_REG(IXGBE_RXNFGBCH),    MY_REG(IXGBE_RXDGPC),
        MY_REG(IXGBE_RXDGBCL),    MY_REG(IXGBE_RXDGBCH),     MY_REG(IXGBE_RXDDGPC),     MY_REG(IXGBE_RXDDGBCL),
        MY_REG(IXGBE_RXDDGBCH),   MY_REG(IXGBE_RXLPBKGPC),   MY_REG(IXGBE_RXLPBKGBCL),  MY_REG(IXGBE_RXLPBKGBCH),
        MY_REG(IXGBE_RXDLPBKGPC), MY_REG(IXGBE_RXDLPBKGBCL), MY_REG(IXGBE_RXDLPBKGBCH), MY_REG(IXGBE_TXDGPC),
        MY_REG(IXGBE_TXDGBCL),    MY_REG(IXGBE_TXDGBCH),     MY_REG(IXGBE_FDIRUSTAT),   MY_REG(IXGBE_FDIRFSTAT),
        MY_REG(IXGBE_FDIRMATCH),  MY_REG(IXGBE_FDIRMISS)
...
COLD_FUNC void CPhyEthIF::configure(uint16_t nb_rx_queue, uint16_t nb_tx_queue, const struct rte_eth_conf *eth_conf) {
    int ret;
    ret = rte_eth_dev_configure(m_repid, nb_rx_queue, nb_tx_queue, eth_conf);
    ...
    if (CGlobalInfo::m_options.preview.getChecksumOffloadEnable()) {
        ...
        if ((m_dev_info->tx_offload_capa & DEV_TX_OFFLOAD_UDP_CKSUM) == 0) {
            rte_exit(EXIT_FAILURE,
                     "Device does not support UDP checksum offload: "
                     "port=%u\n",
                     m_repid);
        }
        if ((m_dev_info->tx_offload_capa & DEV_TX_OFFLOAD_TCP_CKSUM) == 0) {
            rte_exit(EXIT_FAILURE,
                     "Device does not support TCP checksum offload: "
                     "port=%u\n",
                     m_repid);
        }
    }
...
COLD_FUNC void CPhyEthIF::rx_queue_setup(uint16_t rx_queue_id, uint16_t nb_rx_desc, unsigned int socket_id,
                                         const struct rte_eth_rxconf *rx_conf, struct rte_mempool *mb_pool) {

    int ret = rte_eth_rx_queue_setup(m_repid, rx_queue_id, nb_rx_desc, socket_id, rx_conf, mb_pool);
    if (ret < 0)
        rte_exit(EXIT_FAILURE,
...
COLD_FUNC void CPhyEthIF::tx_queue_setup(uint16_t tx_queue_id, uint16_t nb_tx_desc, unsigned int socket_id,
                                         const struct rte_eth_txconf *tx_conf) {

    int ret = rte_eth_tx_queue_setup(m_repid, tx_queue_id, nb_tx_desc, socket_id, tx_conf);
    if (ret < 0)
        rte_exit(EXIT_FAILURE,
...
COLD_FUNC void CPhyEthIF::start() {
    ...
    if ((m_dev_info->tx_offload_capa & DEV_OFFLOAD_CAPA) != DEV_OFFLOAD_CAPA) {
        m_dev_tx_offload_needed =
            DEV_TX_OFFLOAD_VLAN_INSERT; /* make everyting by software, deriver do not report the right capability e.g.
                                           vxnet3 does not support TCP/UDP  */
    } else {
...
int CPhyEthIF::dump_fdir_global_stats(FILE *fd) { return get_ex_drv()->dump_fdir_global_stats(this, fd); }
...
class CCoreEthIF : public CVirtualIF {
  public:
    enum { INVALID_Q_ID = 255 };

  public:
    CCoreEthIF() { m_mbuf_cache = 0; }

    bool Create(uint8_t core_id, uint8_t tx_client_queue_id, CPhyEthIF *tx_client_port, uint8_t tx_server_queue_id,
                CPhyEthIF *tx_server_port, uint8_t tx_q_id_lat);
    void Delete();

    virtual int open_file(std::string file_name) { return (0); }

    virtual int close_file(void) { return (flush_tx_queue()); }
    __attribute__((noinline)) int send_node_flow_stat(rte_mbuf *m, CGenNodeStateless *node_sl, CCorePerPort *lp_port,
    ...
    socket_id_t get_socket_id() { return (CGlobalInfo::m_socket.port_to_socket(m_ports[0].m_port->get_tvpid())); }

    const CCorePerPort *get_ports() { return m_ports; }

  protected:
    int send_burst(CCorePerPort *lp_port, uint16_t len, CVirtualIFPerSideStats *lp_stats);
    int send_pkt(CCorePerPort *lp_port, rte_mbuf_t *m, CVirtualIFPerSideStats *lp_stats);
    int send_pkt_lat(CCorePerPort *lp_port, rte_mbuf_t *m, CVirtualIFPerSideStats *lp_stats);
...
class CCoreEthIFStateless : public CCoreEthIF {
  public:
    ...
    virtual uint16_t rx_burst(pkt_dir_t dir, struct rte_mbuf **rx_pkts, uint16_t nb_pkts);
    /**
     * fast path version
     */
    virtual int send_node(CGenNode *node);
  ...
  protected:
    ...
    inline int send_node_packet(CGenNodeStateless *node_sl, rte_mbuf_t *m, CCorePerPort *lp_port,
  ...
  public:
    void set_rx_queue_id(uint16_t client_qid, uint16_t server_qid) {
...
class CCoreEthIFTcp : public CCoreEthIF {
  public:
    ...
    uint16_t rx_burst(pkt_dir_t dir, struct rte_mbuf **rx_pkts, uint16_t nb_pkts);
...
COLD_FUNC bool CCoreEthIF::Create(uint8_t core_id, uint8_t tx_client_queue_id, CPhyEthIF *tx_client_port,
                                  uint8_t tx_server_queue_id, CPhyEthIF *tx_server_port, uint8_t tx_q_id_lat) {
...
COLD_FUNC void CCoreEthIF::DumpIfCfg(FILE *fd) {
    fprintf(fd, " %d   %6u %6u  %6u  %6u %6u  \n", m_core_id, m_ports[CLIENT_SIDE].m_port->get_tvpid(),
            m_ports[CLIENT_SIDE].m_tx_queue_id, m_ports[SERVER_SIDE].m_port->get_tvpid(),
            m_ports[SERVER_SIDE].m_tx_queue_id, m_ports[SERVER_SIDE].m_tx_queue_id_lat);
}
...
HOT_FUNC int CCoreEthIF::send_burst(CCorePerPort *lp_port, uint16_t len, CVirtualIFPerSideStats *lp_stats) {
    ...
    if (likely(CGlobalInfo::m_options.m_is_queuefull_retry)) {
        while (unlikely(ret < len)) {
            ...
            uint16_t ret1 = lp_port->m_port->tx_burst(lp_port->m_tx_queue_id, &lp_port->m_table[ret], len - ret);
            ret += ret1;
        }
    } else {
...
int HOT_FUNC CCoreEthIF::send_pkt(CCorePerPort *lp_port, rte_mbuf_t *m, CVirtualIFPerSideStats *lp_stats) {
...
HOT_FUNC void CCoreEthIF::send_one_pkt(pkt_dir_t dir, rte_mbuf_t *m) {
    CCorePerPort *lp_port = &m_ports[dir];
    CVirtualIFPerSideStats *lp_stats = &m_stats[dir];
...
HOT_FUNC uint16_t CCoreEthIFTcp::rx_burst(pkt_dir_t dir, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
...
HOT_FUNC inline rte_mbuf_t *CCoreEthIFStateless::generate_node_pkt(CGenNodeStateless *node_sl) {
...
HOT_FUNC inline int CCoreEthIFStateless::send_node_packet(CGenNodeStateless *node_sl, rte_mbuf_t *m,
                                                          CCorePerPort *lp_port, CVirtualIFPerSideStats *lp_stats) {
...
HOT_FUNC uint16_t CCoreEthIFStateless::rx_burst(pkt_dir_t dir, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    uint16_t res = m_ports[dir].m_port->rx_burst(m_rx_queue_id[dir], rx_pkts, nb_pkts);
    return (res);
}

HOT_FUNC int CCoreEthIFStateless::send_node(CGenNode *node) { return send_node_common<false>(node); }

HOT_FUNC int CCoreEthIFStateless::send_node_service_mode(CGenNode *node) { return send_node_common<true>(node); }
 ...
 *
 */
template <bool SERVICE_MODE> HOT_FUNC int CCoreEthIFStateless::send_node_common(CGenNode *node) {
 ...
 *
 */
rte_mbuf_t *CCoreEthIFStateless::generate_slow_path_node_pkt(CGenNodeStateless *node_sl) {
...
HOT_FUNC bool CCoreEthIF::redirect_to_rx_core(pkt_dir_t dir, rte_mbuf_t *m) {
    bool sent = false;
...
HOT_FUNC int CCoreEthIF::send_node(CGenNode *node) {
    ...
    if (unlikely(CGlobalInfo::m_options.preview.get_vlan_mode() != CPreviewMode::VLAN_MODE_NONE)) {
        ...
        if (CGlobalInfo::m_options.preview.get_vlan_mode() == CPreviewMode::VLAN_MODE_LOAD_BALANCE) {
        ...
        } else if (CGlobalInfo::m_options.preview.get_vlan_mode() == CPreviewMode::VLAN_MODE_NORMAL) {
            CCorePerPort *lp_port = &m_ports[dir];
            uint8_t port_id = lp_port->m_port->get_tvpid();
...
pkt_dir_t CCoreEthIF::port_id_to_dir(uint8_t port_id) {
 ...
 * apply HW VLAN
 */
void CPortLatencyHWBase::apply_hw_vlan(rte_mbuf_t *m, uint8_t port_id) {
...
class CLatencyHWPort : public CPortLatencyHWBase {
  public:
    void Create(CPhyEthIF *p, uint8_t tx_queue, uint8_t rx_queue) {
        m_port = p;
...
class CLatencyVmPort : public CPortLatencyHWBase {
  public:
    void Create(uint8_t port_index, CNodeRing *ring, CLatencyManager *mgr, CPhyEthIF *p, bool disable_rx_read) {
    ...
    virtual int tx(rte_mbuf_t *m) { return tx_common(m, false, true); }

    virtual int tx_raw(rte_mbuf_t *m) { return tx_common(m, false, false); }

    virtual int tx_latency(rte_mbuf_t *m) { return tx_common(m, true, true); }
...
class CGlobalStats {
  public:
    enum DumpFormat { dmpSTANDARD, dmpTABLE };
...
COLD_FUNC bool CGlobalStats::is_dump_nat() { return (CGlobalInfo::is_learn_mode() && (get_is_tcp_mode() == 0)); }

COLD_FUNC void CGlobalStats::port_stats_to_json(Json::Value &output, uint8_t port_id) {
...
COLD_FUNC void CGlobalStats::DumpAllPorts(FILE *fd) {
    ...
    fprintf(fd, " Active-flows    : %8llu  Clients : %8llu   Socket-util : %3.4f %%    \n",
            (unsigned long long)m_active_flows, (unsigned long long)m_total_clients, m_socket_util);
    fprintf(fd, " Open-flows      : %8llu  Servers : %8llu   Socket : %8llu Socket/Clients :  %.1f \n",
            (unsigned long long)m_open_flows, (unsigned long long)m_total_servers, (unsigned long long)m_active_sockets,
...
class CGlobalTRex {
  ...
  public:
    ...
    void abort_gracefully(const std::string &on_stdout, const std::string &on_publisher) __attribute__((__noreturn__));
    ...
    bool is_marked_for_shutdown() const { return (m_mark_for_shutdown != SHUTDOWN_NONE); }
  ...
  public:
    ...
    */
    int get_base_num_cores() { return (m_max_ports >> 1); }
  ...
  public:
    ...
    void dump_stats(FILE *fd, CGlobalStats::DumpFormat format);
  ...
  public:
    ...
    uint32_t m_max_cores; /* current number of cores , include master and latency  ==> ( master)1+c*(m_max_ports>>1)+1(
                             latency )  */
    ...
    if (CGlobalInfo::m_options.preview.get_is_client_cfg_enable()) {
        ...
        for (int port_id = 0; port_id < m_max_ports; port_id++) {
            ...
            if (!memcmp(CGlobalInfo::m_options.m_mac_addr[port_id].u.m_mac.dest, empty_mac, ETHER_ADDR_LEN)) {
                ...
                // if port is connected in loopback, no need to send gratuitous ARP. It will only confuse our ingress
                // counters.
                if (need_grat_arp[port_id] && (!pretest.is_loopback(port_id))) {
                    COneIPv4Info ipv4(CGlobalInfo::m_options.m_ip_cfg[port_id].get_ip(),
                                      CGlobalInfo::m_options.m_ip_cfg[port_id].get_vlan(),
                                      CGlobalInfo::m_options.m_mac_addr[port_id].u.m_mac.src, port_id);
                    m_mg.add_grat_arp_src(ipv4);
                }
            }
        }
 ...
 */
COLD_FUNC void CGlobalTRex::abort_gracefully(const std::string &on_stdout, const std::string &on_publisher) {
    ...
    if (!is_all_links_are_up()) {
        ...
        if (!is_all_links_are_up()) { // disable start with link down for now
            ...
            if (get_is_interactive()) {
                ...
                printf(" WARNING : there is no link on one of the ports, driver support auto drop in case of link down "
                       "- continue\n");
            } else {
...
COLD_FUNC void CGlobalTRex::init_vif_cores() {
    int port_offset = 0;
    uint8_t lat_q_id;
    ...
    for (int i = 0; i < get_cores_tx(); i++) {
        ...
        m_cores_vif[j]->Create(j, queue_id, m_ports[port_offset],  /* 0,2*/
                               queue_id, m_ports[port_offset + 1], /*1,3*/
                               lat_q_id);
        port_offset += 2;
...
COLD_FUNC bool CGlobalTRex::Create() {
    ...
    CGlobalInfo::init_pools(m_max_ports * dpdk_p.get_total_rx_desc(), dpdk_p.rx_mbuf_type, use_hugepages);
...
COLD_FUNC TrexSTXCfg CGlobalTRex::get_stx_cfg() {

    TrexSTXCfg cfg;

    /* control plane config */
    cfg.m_rpc_req_resp_cfg.create(TrexRpcServerConfig::RPC_PROT_TCP, global_platform_cfg_info.m_zmq_rpc_port,
                                  &m_cp_lock, CGlobalInfo::m_options.rpc_logfile_name);
    ...
    /* RX core */
    cfg.m_rx_cfg.create(get_cores_tx(), ports);
...
static bool is_val_not_in_range_dpdk_limits(struct rte_eth_desc_lim *lim, uint16_t val, uint16_t &new_val) {
    if (lim->nb_max < val) {
        new_val = lim->nb_max;
        return (true);
    }
...
COLD_FUNC void dump_dpdk_devices(void) {
    printf(" DPDK devices %d : %d \n", rte_eth_dev_count(), rte_eth_dev_count_total());
    printf("-----\n");
    char name[100];
    int j;
...
COLD_FUNC int CGlobalTRex::device_prob_init(void) {
    ...
    if (m_max_ports == 0)
        rte_exit(EXIT_FAILURE,
                 "Error: Could not find supported ethernet ports. You are probably trying to use unsupported NIC \n");
    ...
    if (m_max_ports % 2 != 0) {
        rte_exit(EXIT_FAILURE,
                 " Number of ports in config file is %d. It should be even. Please use --limit-ports, or change "
                 "'port_limit:' in the config file\n",
                 m_max_ports);
    }
    ...
    if (ps->get_expected_ports() > TREX_MAX_PORTS) {
        rte_exit(EXIT_FAILURE,
                 " Maximum number of ports supported is %d. You are trying to use %d. Please use --limit-ports, or "
                 "change 'port_limit:' in the config file\n",
                 TREX_MAX_PORTS, ps->get_expected_ports());
    }

    if (ps->get_expected_ports() > m_max_ports) {
        rte_exit(EXIT_FAILURE,
                 " There are %d ports available. You are trying to use %d. Please use --limit-ports, or change "
                 "'port_limit:' in the config file\n",
    ...
    if (ps->get_number_of_dp_cores_needed() > BP_MAX_CORES) {
        rte_exit(EXIT_FAILURE,
                 " Your configuration require  %d DP cores but the maximum supported is %d - try to reduce `-c value ` "
                 "or 'c:' in the config file\n",
                 (int)ps->get_number_of_dp_cores_needed(), (int)BP_MAX_CORES);
    }
    ...
    if (dev_info.max_tx_queues < tx_queues) {
        printf("ERROR: driver maximum tx queues is (%d) required (%d) reduce number of cores to support it \n",
               (int)dev_info.max_tx_queues, (int)tx_queues);
        exit(1);
    }

    uint16_t rx_queues = get_dpdk_mode()->dp_rx_queues();
    if ((dev_info.max_rx_queues < rx_queues) || (dp_cores < rx_queues)) {
        printf("ERROR: driver maximum rx queues is (%d), number of cores (%d) and requested rx queues (%d) is higher, "
               "reduce the number of dp cores \n",
               (int)dev_info.max_tx_queues, (int)dp_cores, (int)rx_queues);
        exit(1);
    }
    ...
    if (is_valid_dpdk_limits(&dev_info.tx_desc_lim) && is_valid_dpdk_limits(&dev_info.rx_desc_lim)) {
        /* driver support min/max descriptors*/
        CTrexDpdkParams dpdk_p;
        CPlatformYamlInfo *cg = &global_platform_cfg_info;

        get_dpdk_drv_params(dpdk_p);
        if (is_val_not_in_range_dpdk_limits(&dev_info.tx_desc_lim, dpdk_p.tx_desc_num, cg->m_tx_desc)) {
            printf(" WARNING tx_desc_num was reduced from %d to %d \n", (int)dpdk_p.tx_desc_num, (int)cg->m_tx_desc);
        }
        if (is_val_not_in_range_dpdk_limits(&dev_info.rx_desc_lim, dpdk_p.rx_desc_num_data_q, cg->m_rx_desc)) {
            printf(" WARNING rx_desc_num was reduced from %d to %d \n", (int)dpdk_p.rx_desc_num_data_q,
                   (int)cg->m_rx_desc);
        }
        if (is_val_not_in_range_dpdk_limits(&dev_info.rx_desc_lim, dpdk_p.rx_desc_num_dp_q, cg->m_rx_desc)) {
...
COLD_FUNC int CGlobalTRex::queues_prob_init() {
    ...
    if ((m_max_ports >> 1) > get_cores_tx()) {
        rte_exit(EXIT_FAILURE,
                 "You don't have enough physical cores for this configuration dual_ports:%lu physical_cores:%lu "
                 "dp_cores:%lu check lscpu \n",
                 (ulong)(m_max_ports >> 1), (ulong)m_max_cores, (ulong)get_cores_tx());
    }
    ...
    */
    ...
    if (m_max_queues_per_port > BP_MAX_CORES) {
        rte_exit(EXIT_FAILURE, "Error: Number of TX queues exceeds %d. Try running with lower -c <val> \n",
                 BP_MAX_CORES);
    }
...
void CGlobalTRex::get_stats(CGlobalStats &stats) {
    ...
    for (i = 0; i < m_max_ports; i++) {
        ...
        // payload rules
        for (int flow = MAX_FLOW_STATS; flow <= MAX_FLOW_STATS + CFlowStatRuleMgr::instance()->get_max_hw_id_payload();
             flow++) {
            stats.m_port[i].m_tx_per_flow[flow].clear();
        }
    ...
    for (i = 0; i < get_cores_tx(); i++) {
        ...
        stats.m_total_queue_drop +=
            lpt->m_node_gen.m_v_if->get_stats()[0].m_tx_drop + lpt->m_node_gen.m_v_if->get_stats()[1].m_tx_drop;
        ...
        // IP ID rules
        for (int flow = 0; flow <= CFlowStatRuleMgr::instance()->get_max_hw_id(); flow++) {
            stats.m_port[port0].m_tx_per_flow[flow] += lpt->m_node_gen.m_v_if->get_stats()[0].m_tx_per_flow[flow];
            stats.m_port[port0 + 1].m_tx_per_flow[flow] += lpt->m_node_gen.m_v_if->get_stats()[1].m_tx_per_flow[flow];
        }
        // payload rules
        for (int flow = MAX_FLOW_STATS; flow <= MAX_FLOW_STATS + CFlowStatRuleMgr::instance()->get_max_hw_id_payload();
             flow++) {
...
COLD_FUNC float CGlobalTRex::get_cpu_util_per_interface(uint8_t port_id) {
...
COLD_FUNC void CGlobalTRex::publish_async_data(bool sync_now, bool baseline) {
    std::string json;
...
COLD_FUNC void CGlobalTRex::publish_async_barrier(uint32_t key) { m_zmq_publisher.publish_barrier(key); }
...
COLD_FUNC void CGlobalTRex::show_panel() {
...
COLD_FUNC int CGlobalTRex::run_in_core(virtual_thread_id_t virt_core_id) {
    ...
    if (lp->getSingleCore() && (virt_core_id == 2) && (lp->getCores() == 1)) {
...
COLD_FUNC int CGlobalTRex::stop_master() {
    ...
    for (i = 0; i < get_cores_tx(); i++) {
        ...
        total_tx_rx_check +=
            erf_vif->get_stats()[CLIENT_SIDE].m_tx_rx_check_pkt + erf_vif->get_stats()[SERVER_SIDE].m_tx_rx_check_pkt;
    }
...
COLD_FUNC int CGlobalTRex::start_master_astf_batch() {
    ...
    if (CGlobalInfo::m_options.preview.get_is_client_cfg_enable()) {
        ...
        m_mg.set_ip(lat.get_c_ip(), lat.get_s_ip(), lat.get_mask(), m_fl.m_client_config_info);
    } else {
        m_mg.set_ip(lat.get_c_ip(), lat.get_s_ip(), lat.get_mask());
    }
...
COLD_FUNC int CGlobalTRex::start_master_statefull() {
    ...
    if (CGlobalInfo::m_options.preview.get_is_client_cfg_enable()) {
        ...
        m_mg.set_ip(tg->m_client_pool[0].get_ip_start(), tg->m_server_pool[0].get_ip_start(),
                    tg->m_client_pool[0].getDualMask(), m_fl.m_client_config_info);
    } else {
...
COLD_FUNC void CPhyEthIF::configure_rss_astf(bool is_client, uint16_t numer_of_queues, uint16_t skip_queue) {
...
COLD_FUNC void CPhyEthIF::configure_rss() {
    trex_dpdk_rx_distro_mode_t rss_mode = get_dpdk_mode()->get_rx_distro_mode();

    if (rss_mode == ddRX_DIST_ASTF_HARDWARE_RSS) {
        CTrexDpdkParams dpdk_p;
        get_dpdk_drv_params(dpdk_p);

        configure_rss_astf(false, dpdk_p.get_total_rx_queues(), MAIN_DPDK_RX_Q);
    }
}
...
COLD_FUNC void CPhyEthIF::conf_multi_rx() {
    ...
    if (dev_info->flow_type_rss_offloads) {
        lp_rss->rss_hf = (dev_info->flow_type_rss_offloads & (ETH_RSS_NONFRAG_IPV4_TCP | ETH_RSS_NONFRAG_IPV4_UDP |
...
COLD_FUNC void CPhyEthIF::conf_hardware_astf_rss() {
    ...
    if (!rte_eth_dev_filter_supported(m_repid, RTE_ETH_FILTER_HASH)) {
        ...
        if (rte_eth_dev_filter_ctrl(m_repid, RTE_ETH_FILTER_HASH, RTE_ETH_FILTER_SET, &info) < 0) {
    ...
    struct rte_eth_rss_conf *lp_rss = &g_trex.m_port_cfg.m_port_conf.rx_adv_conf.rss_conf;
    lp_rss->rss_hf =
        ETH_RSS_NONFRAG_IPV4_TCP | ETH_RSS_NONFRAG_IPV4_UDP | ETH_RSS_NONFRAG_IPV6_TCP | ETH_RSS_NONFRAG_IPV6_UDP;
...
COLD_FUNC void CPhyEthIF::_conf_queues(uint16_t tx_qs, uint32_t tx_descs, uint16_t rx_qs, rx_que_desc_t &rx_qs_descs,
                                       uint16_t rx_qs_drop_qid, trex_dpdk_rx_distro_mode_t rss_mode,
                                       bool in_astf_mode) {
    ...
    if (CGlobalInfo::m_options.preview.getTsoOffloadDisable()) {
        tx_offloads &= ~(DEV_TX_OFFLOAD_TCP_TSO | DEV_TX_OFFLOAD_UDP_TSO);
    }
    ...
    for (uint16_t qid = 0; qid < tx_qs; qid++) {
        tx_queue_setup(qid, tx_descs, socket_id, &g_trex.m_port_cfg.m_tx_conf);
    }
    ...
    for (uint16_t qid = 0; qid < rx_qs; qid++) {
        ...
        rx_queue_setup(qid, rx_qs_descs[qid], socket_id, &cfg.m_rx_conf, get_rx_mem_pool(socket_id));
    }
    if (rx_qs_drop_qid) {
...
COLD_FUNC void CPhyEthIF::conf_queues(void) {
    ...
    if (get_dpdk_mode()->is_one_tx_rx_queue()) {
        ...
        for (i = 0; i < rx_qs; i++) {
            uint16_t desc;
            if (is_drop_q && (i == MAIN_DPDK_DROP_Q)) {
    ...
    _conf_queues(tx_qs, tx_descs, rx_qs, rx_qs_descs, rx_qs_drop_qid, rss_mode, in_astf_mode);
}
...
COLD_FUNC bool CPhyEthIF::Create(tvpid_t tvpid, repid_t repid) {
    m_tvpid = tvpid;
    m_repid = repid;
    ...
    if (!m_is_dummy) {
        ...
        if (!memcmp(CGlobalInfo::m_options.m_mac_addr[m_tvpid].u.m_mac.src, empty_mac, ETHER_ADDR_LEN)) {
            rte_eth_macaddr_get(m_repid, (struct ether_addr *)&CGlobalInfo::m_options.m_mac_addr[m_tvpid].u.m_mac.src);
        }
    }
...
const std::vector<std::pair<uint8_t, uint8_t>> &CPhyEthIF::get_core_list() {
...
COLD_FUNC int CPhyEthIF::get_flow_stats(rx_per_flow_t *rx_stats, tx_per_flow_t *tx_stats, int min, int max,
                                        bool reset) {
    ...
    if (hw_rx_stat_supported) {
        if (get_ex_drv()->get_rx_stats(this, diff_pkts, m_stats.m_fdir_prev_pkts, diff_bytes, m_stats.m_fdir_prev_bytes,
                                       min, max) < 0) {
            return -1;
        }
    } else {
        get_stateless_obj()->get_stats()->get_rx_stats(get_tvpid(), rx_stats, min, max, reset,
                                                       TrexPlatformApi::IF_STAT_IPV4_ID, get_core_list());
    }
...
COLD_FUNC int CPhyEthIF::get_flow_stats_payload(rx_per_flow_t *rx_stats, tx_per_flow_t *tx_stats, int min, int max,
                                                bool reset) {
    get_stateless_obj()->get_stats()->get_rx_stats(get_tvpid(), rx_stats, min, max, reset,
                                                   TrexPlatformApi::IF_STAT_PAYLOAD, get_core_list());
...
COLD_FUNC TrexSTX *get_stx() { return g_trex.m_stx; }

/**
 * handles an abort
 *
 */
COLD_FUNC void abort_gracefully(const std::string &on_stdout, const std::string &on_publisher) {
...
COLD_FUNC const char *get_exe_name() { return g_exe_name; }
...
COLD_FUNC int update_global_info_from_platform_file() {
    ...
    if (cg->m_latency_measurement.length()) {
        if ((cg->m_latency_measurement.compare("NANOSECONDS") == 0) || (cg->m_latency_measurement.compare("1") == 0)) {
            g_opts->m_latency_measurement = CParserOption::LATENCY_METHOD_NANOS;
        }
    }

    if (cg->m_timesync_method.length()) {
        if ((cg->m_timesync_method.compare("PTP") == 0) || (cg->m_timesync_method.compare("1") == 0)) {
...
COLD_FUNC void update_memory_cfg() {
    ...
    CGlobalInfo::m_memory_cfg.set_number_of_dp_cors(CGlobalInfo::m_options.get_number_of_dp_cores_needed());
...
COLD_FUNC int core_mask_sanity(uint32_t wanted_core_mask) {
    ...
    if (wanted_core_mask != (wanted_core_mask & calc_core_mask)) {
        printf(
            "Serious error: Something is wrong with the hardware. Wanted core mask is %x. Existing core mask is %x\n",
...
COLD_FUNC int update_dpdk_args(void) {
    ...
    g_dpdk_args_num = 0;
#define SET_ARGS(val)                                                                                                  \
    { g_dpdk_args[g_dpdk_args_num++] = (char *)(val); }
...
COLD_FUNC int main_test(int argc, char *argv[]) {
    ...
    if (get_is_tcp_mode()) {
        ...
        if (po->preview.get_is_rx_check_enable()) {
            printf("ERROR advanced stateful does not require --rx-check mode, it is done by default, please remove "
                   "this switch\n");
            return (-1);
        }
    ...
    if (CGlobalInfo::m_options.m_debug_pkt_proto != 0) {
        CTrexDpdkParams dpdk_p;
        get_dpdk_drv_params(dpdk_p);
        CTrexDebug debug = CTrexDebug(g_trex.m_ports[0], g_trex.m_max_ports, dpdk_p.get_total_rx_queues());
        int ret;
    ...
    if (CGlobalInfo::m_options.is_latency_enabled() && (CGlobalInfo::m_options.m_latency_prev > 0)) {
        uint32_t pkts = CGlobalInfo::m_options.m_latency_prev * CGlobalInfo::m_options.m_latency_rate;
...
COLD_FUNC uint32_t TrexDpdkPlatformApi::get_port_count() const { return g_trex.m_max_ports; }

COLD_FUNC uint8_t TrexDpdkPlatformApi::get_dp_core_count() const {
    return CGlobalInfo::m_options.get_number_of_dp_cores_needed();
}

COLD_FUNC void TrexDpdkPlatformApi::port_id_to_cores(uint8_t port_id,
                                                     std::vector<std::pair<uint8_t, uint8_t>> &cores_id_list) const {
...
COLD_FUNC void TrexDpdkPlatformApi::publish_async_data_now(uint32_t key, bool baseline) const {
...
COLD_FUNC int TrexDpdkPlatformApi::get_flow_stats(uint8_t port_id, void *rx_stats, void *tx_stats, int min, int max,
                                                  bool reset, TrexPlatformApi::driver_stat_cap_e type) const {
    ...
    if (type == TrexPlatformApi::IF_STAT_PAYLOAD) {
        ...
        return g_trex.m_ports[port_id]->get_flow_stats((rx_per_flow_t *)rx_stats, (tx_per_flow_t *)tx_stats, min, max,
                                                       reset);
    }
}
...
COLD_FUNC int TrexDpdkPlatformApi::get_rfc2544_info(void *rfc2544_info, int min, int max, bool reset,
    ...
    return get_stateless_obj()->get_stats()->get_rfc2544_info((rfc2544_info_t *)rfc2544_info, min, max, reset,
                                                              period_switch);
}
...
COLD_FUNC bool TrexDpdkPlatformApi::hw_rx_stat_supported() const { return get_ex_drv()->hw_rx_stat_supported(); }
...
COLD_FUNC bool DpdkTRexPortAttr::update_link_status_nowait() {
    ...
    if (new_link.link_speed != m_link.link_speed || new_link.link_duplex != m_link.link_duplex ||
        new_link.link_autoneg != m_link.link_autoneg || new_link.link_status != m_link.link_status) {
        changed = true;
 ...
 */
void TrexDpdkPlatformApi::mark_for_shutdown() const { g_trex.mark_for_shutdown(CGlobalTRex::SHUTDOWN_RPC_REQ); }

CSyncBarrier *TrexDpdkPlatformApi::get_sync_barrier(void) const { return g_trex.m_sync_barrier; }

CFlowGenList *TrexDpdkPlatformApi::get_fl() const { return &g_trex.m_fl; }
