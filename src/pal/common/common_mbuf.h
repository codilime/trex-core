...
static inline rte_mbuf_t *utl_rte_pktmbuf_chain_to_indirect(rte_mbuf_t *base, rte_mbuf_t *indirect, rte_mbuf_t *last) {
...
inline uint8_t *utl_rte_pktmbuf_get_last_bytes(const rte_mbuf_t *m, uint16_t num_bytes, uint8_t *buf) {
    ...
    if (last->data_len >= num_bytes) {
        ...
        bcopy(rte_pktmbuf_mtod(before_last, uint8_t *) + before_last->data_len - (num_bytes - last->data_len), buf,
              num_bytes - last->data_len);
