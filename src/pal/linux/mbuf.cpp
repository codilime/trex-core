...
rte_mempool_t *utl_rte_mempool_create_non_pkt(const char *name, unsigned n, unsigned elt_size, unsigned cache_size,
                                              int socket_id, bool share, bool is_hugepages) {
...
rte_mempool_t *utl_rte_mempool_create(const char *name, unsigned n, unsigned elt_size, unsigned cache_size,
                                      int socket_id, bool is_hugepages) {
...
uint16_t rte_mbuf_refcnt_update(rte_mbuf_t *m, int16_t value) { return (_rte_mbuf_refcnt_update(m, value)); }
...
int rte_pktmbuf_is_contiguous(const struct rte_mbuf *m) { return (m->nb_segs == 1); }

static void rte_pktmbuf_hexdump(const void *buf, unsigned int len) {
...
void _rte_mempool_sp_put(struct rte_mempool *mp, void *obj) { free(obj); }
...
void rte_pktmbuf_dump(const struct rte_mbuf *m, unsigned dump_len) {
    ...
    printf("  pkt_len=%u,  nb_segs=%u, "
           "in_port=%u\n",
           m->pkt_len, (unsigned)m->nb_segs, (unsigned)m->in_port);
    nb_segs = m->nb_segs;
...
uint64_t rte_rand(void) { return (rand()); }
...
static inline int rte_raw_cksum_mbuf(const struct rte_mbuf *m, uint32_t off, uint32_t len, uint16_t *cksum) {
...
static inline uint16_t rte_ipv4_cksum(const struct ipv4_hdr *ipv4_hdr) {
    uint16_t cksum;
...
uint16_t rte_ipv4_phdr_cksum(const struct ipv4_hdr *ipv4_hdr, uint64_t ol_flags) {
    ...
    if (ol_flags & PKT_TX_TCP_SEG) {
        ...
        psd_hdr.len = PAL_NTOHS((uint16_t)(PAL_NTOHS(ipv4_hdr->total_length) - rte_ipv4_header_len(ipv4_hdr)));
    }
...
static void tx_offload_csum(struct rte_mbuf *m, uint64_t tx_offload) {
    ...
    if ((m->ol_flags & PKT_TX_IPV4) && (m->ol_flags & PKT_TX_IP_CKSUM) && !(tx_offload & DEV_TX_OFFLOAD_IPV4_CKSUM)) {
...
void hw_checksum_sim(struct rte_mbuf *m) {
    ...
    if (((m->ol_flags & PKT_TX_L4_MASK) == PKT_TX_TCP_CKSUM) || ((m->ol_flags & PKT_TX_L4_MASK) == PKT_TX_UDP_CKSUM)) {
        tx_offload_csum(m, 0);
    }
}
...
void test_pkt_mbuf() {
    ...
    rte_mempool_t *mp1 = utl_rte_mempool_create("big-const", CONST_NB_MBUF, CONST_MBUF_SIZE, 32);
