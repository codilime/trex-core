...
static inline void rte_mbuf_set_as_core_local(struct rte_mbuf *m) { m->m_core_locality = RTE_MBUF_CORE_LOCALITY_LOCAL; }

static inline void rte_mbuf_set_as_core_const(struct rte_mbuf *m) { m->m_core_locality = RTE_MBUF_CORE_LOCALITY_CONST; }

static inline void rte_mbuf_set_as_core_multi(struct rte_mbuf *m) { m->m_core_locality = RTE_MBUF_CORE_LOCALITY_MULTI; }

#else
static inline void rte_mbuf_set_as_core_local(struct rte_mbuf *m) {}

static inline void rte_mbuf_set_as_core_const(struct rte_mbuf *m) {}

static inline void rte_mbuf_set_as_core_multi(struct rte_mbuf *m) {}
...
inline unsigned rte_mempool_count(rte_mempool_t *mp) { return (10); }
...
inline int rte_mempool_get(struct rte_mempool *mp, void **obj_p) { return (_rte_mempool_sc_get(mp, obj_p)); }

inline void rte_mempool_put(struct rte_mempool *mp, void *obj) { _rte_mempool_sp_put(mp, obj); }

static inline void *rte_memcpy(void *dst, const void *src, size_t n) { return (memcpy(dst, src, n)); }

static inline uint16_t rte_mbuf_refcnt_read(const struct rte_mbuf *m) { return m->refcnt_reserved; }

extern "C" void rte_exit(int exit_code, const char *format, ...);

static inline unsigned rte_lcore_to_socket_id(unsigned lcore_id) { return (0); }
