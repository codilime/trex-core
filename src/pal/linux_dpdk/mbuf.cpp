...
rte_mempool_t *utl_rte_mempool_create(const char *name, unsigned n, unsigned elt_size, unsigned cache_size,
    ...
    rte_mempool_t *res = rte_mempool_create(buffer, n, elt_size, cache_size, sizeof(struct rte_pktmbuf_pool_private),
                                            rte_pktmbuf_pool_init, NULL, rte_pktmbuf_init, NULL, socket_id, flags);
    if (res == NULL) {
...
rte_mempool_t *utl_rte_mempool_create_non_pkt(const char *name, unsigned n, unsigned elt_size, unsigned cache_size,
    ...
    rte_mempool_t *res =
        rte_mempool_create(buffer, n, elt_size, cache_size, 0, NULL, NULL, NULL, NULL, socket_id, flags);
    if (res == NULL) {
        print_alloc_err(is_hugepages);
