...
class JsonRpcMethod : public TrexJsonRpcV2ParsedObject {
  public:
    JsonRpcMethod(const Json::Value &msg_id, TrexRpcCommand *cmd, const Json::Value &params)
        : TrexJsonRpcV2ParsedObject(msg_id), m_cmd(cmd), m_params(params) {}
...
class JsonRpcError : public TrexJsonRpcV2ParsedObject {
  public:
    JsonRpcError(const Json::Value &msg_id, int code, const std::string &msg, bool force = false)
        : TrexJsonRpcV2ParsedObject(msg_id, force), m_code(code), m_msg(msg) {}
...
void TrexJsonRpcV2Parser::generate_common_error(Json::Value &json, const std::string &specific_err) {
    JsonRpcError err(Json::Value::null, JSONRPC_V2_ERR_INTERNAL_ERROR, specific_err, true);

    err.execute(json);
}

void TrexJsonRpcV2Parser::generate_common_error(std::string &response, const std::string &specific_err) {
