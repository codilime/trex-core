...
bool CRxCheckFlowTableMap::Create(int max_size) { return (true); }

void CRxCheckFlowTableMap::Delete() { remove_all(); }
...
uint64_t CRxCheckFlowTableMap::count() { return (m_map.size()); }
...
void RxCheckManager::tw_handle() { m_tw.try_handle_events(m_cur_time); }
...
void RxCheckManager::update_template_err(uint8_t template_id) { get_template(template_id)->inc_error_counter(); }
...
void RxCheckManager::DumpTemplate(FILE *fd, bool verbose) {
    ...
    for (i = 0; i < MAX_TEMPLATES_STATS; i++) {
        ...
        if (verbose || (lp->get_error_counter() > 0)) {
            ...
            fprintf(fd, "[id:%2d val:%8llu,rx:%8llu], ", i, (unsigned long long)lp->get_error_counter(),
...
void RxCheckManager::DumpTemplateFull(FILE *fd) {
    ...
    for (i = 0; i < MAX_TEMPLATES_STATS; i++) {
        ...
        fprintf(fd, " template_id_%2d , errors:%8llu,  jitter: %llu  rx : %llu \n", i,
                (unsigned long long)lp->get_error_counter(), (unsigned long long)lp->get_jitter_usec(),
                (unsigned long long)lp->get_rx_counter());
    }
}
...
void RxCheckManager::DumpShort(FILE *fd) {
    ...
    fprintf(fd, "------------------------------------------------------------------------------------------------------"
                "------\n");
    fprintf(fd, "rx check:  avg/max/jitter latency, %8.0f  ,%8.0f, %8d ", m_hist.get_average_latency(),
            m_hist.get_max_latency(), getTemplateMaxJitter());
    fprintf(fd, "     | ");
    ...
    fprintf(fd, " active flows: %8llu, fif: %8llu,  drop: %8llu, errors: %8llu \n",
            (unsigned long long)m_stats.m_active, (unsigned long long)m_stats.m_fif,
            (unsigned long long)m_stats.m_err_drop, (unsigned long long)m_stats.get_total_err());
    fprintf(fd, "------------------------------------------------------------------------------------------------------"
                "------\n");
}
