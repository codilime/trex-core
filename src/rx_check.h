...
typedef enum { CLIENT_SIDE = 0, SERVER_SIDE = 1, CS_NUM = 2, CS_INVALID = 255 } pkt_dir_enum_t;
...
class CRxCheckFlowPerDir {
  ...
  public:
    ...
    bool is_fif_seen() { return ((m_flags & 2) == 2 ? true : false); }

    void set_init_not_from_first_packet() { m_flags |= 1; }
    bool is_init_not_from_first_pkt() { return ((m_flags & 2) == 2 ? true : false); }
};
...
class CRxCheckFlow {
  ...
  public:
    uint16_t get_total_pkt_seen(void) { return (m_dir[0].m_pkts + m_dir[1].m_pkts); }
    uint16_t get_total_pkt_expected(void) { return (m_dir[0].m_flow_size + m_dir[1].m_flow_size); }
    ...
    void set_both_dir() { m_flags |= 2; }
    bool is_both_dir() { return ((m_flags & 2) == 2 ? true : false); }

    void set_aged_correctly() { m_flags |= 4; }
    bool is_aged_correctly() { return ((m_flags & 4) == 4 ? true : false); }
};
...
class CRxCheckFlowTableStats {
  public:
    ...
    uint64_t get_total_err(void) {
        return (m_err_drop + m_err_aged + m_err_no_magic + m_err_wrong_pkt_id + m_err_fif_seen_twice +
                m_err_open_with_no_fif_pkt + m_err_oo_dup + m_err_oo_early + m_err_oo_late + m_err_flow_length_changed);
    }
...
class uint64_tHashEnv {
  public:
    static uint32_t Hash(uint64_t x) { return ((x >> 40) ^ (x & 0xffffffff)); }
};

template <class T> class CMyFSA {
  public:
    bool Create(uint32_t size, bool supportGetNext, bool ctorRequired) { return (true); }

    void Delete() {}
    void Reset() {}

    T *GetNewItem() { return (new T()); }

    void ReturnItem(T *obj) { delete obj; }
};
...
class CPerTxthreadTemplateInfo {
  ...
  public:
    CPerTxthreadTemplateInfo() { Clear(); }
    void Clear() { memset(m_template_info, 0, sizeof(m_template_info)); }
    void Dump(FILE *fd);
...
class CPerTemplateInfo {
  public:
    ...
    void calc(double dtime) { m_jitter.calc(dtime); }

    uint32_t get_jitter_usec() { return ((uint32_t)(m_jitter.get_jitter() * 1000000.0)); }

    void inc_error_counter(void) { m_errors++; }

    uint64_t get_error_counter() { return (m_errors); }

    void inc_rx_counter(void) { m_rx_pkts++; }

    uint64_t get_rx_counter() { return (m_rx_pkts); }
...
class RxCheckManager {
  ...
  public:
    ...
    uint64_t getTotalRx(void) { return (m_stats.m_total_rx); }
