...
#define RX_CHECK_MAGIC (((RX_CHECK_V6_SF_OPT_LEN & 0xff) << 8) | RX_CHECK_V6_SF_OPT_TYPE)
...
struct CRx_check_header {
  ...
  public:
    bool is_fif_dir(void) { return (m_pkt_id == 0 ? true : false); }
    bool is_eof_dir(void) { return ((m_pkt_id == m_flow_size - 1) ? true : false); }

    void set_dir(int dir) { btSetMaskBit8(m_flags, 0, 0, dir ? 1 : 0); }

    int get_dir(void) { return (btGetMaskBit8(m_flags, 0, 0) ? 1 : 0); }
    void clean() { memset(this, 0, sizeof(CRx_check_header)); }

    /* need to mark if we expect to see both sides of the flow, this is know offline  */
    void set_both_dir(int both) { btSetMaskBit8(m_flags, 1, 1, both ? 1 : 0); }

    int get_both_dir(void) { return (btGetMaskBit8(m_flags, 1, 1) ? 1 : 0); }
