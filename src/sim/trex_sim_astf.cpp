...
void CMbufQueue::add_head(rte_mbuf_t *m) { m_que.push_back(m); }

bool CMbufQueue::is_empty() { return (m_que.size() == 0 ? true : false); }
...
class CCoreEthIFTcpSim : public CErfIFStl {
  ...
  private:
    void write_pkt_to_file(uint8_t dir, struct rte_mbuf *m, double time);
...
void CCoreEthIFTcpSim::write_pkt_to_file(uint8_t dir, struct rte_mbuf *m, double time) {
...
void CCoreEthIFTcpSim::set_rx_burst_time(double time) { m_rx_time = time; }

uint16_t CCoreEthIFTcpSim::rx_burst(pkt_dir_t dir, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
...
static void dump_tcp_counters(CTcpPerThreadCtx *c_ctx, CTcpPerThreadCtx *s_ctx, bool dump_json) {
...
static int load_list_of_cap_files(CParserOption *op, asrtf_args_t *args) {
    /* set TCP mode */
    ...
    dump_tcp_counters(lpt->m_c_tcp, lpt->m_s_tcp, args->dump_json);
