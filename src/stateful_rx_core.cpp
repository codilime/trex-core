...
const uint8_t sctp_pkt[] = {
    ...
    0x45, 0x03, 0x00, 0x30, 0x00, 0x00, 0x40, 0x00, 0xff, 0x84, 0xbd, 0x04, 0x9b, 0xe6, 0x18, 0x9b, // sIP
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0xa0, 0x00, // seq
    0x00, 0x00, 0x00, 0x00,
...
const uint8_t icmp_pkt[] = {
    ...
    0x45, 0x03, 0x00, 0x30, 0x00, 0x00, 0x40, 0x00, 0xff, 0x01, 0xbd, 0x04, 0x9b, 0xe6, 0x18, 0x9b, // SIP
    0xcb, 0xff, 0xfc, 0xc2,                                                                         // DIP

    0x08, 0x00, 0x01, 0x02, // checksum
    0xaa, 0xbb,             // id
...
rte_mbuf_t *CLatencyPktInfo::generate_pkt(int port_id, uint32_t extern_ip, uint32_t extern_dest_ip) {
...
void CLatencyPktInfo::set_ip(uint32_t src, uint32_t dst, uint32_t dual_port_mask) {
    m_client_ip.v4 = src;
    m_server_ip.v4 = dst;
...
void CLatencyPktInfo::set_ip(uint32_t src, uint32_t dst, uint32_t dual_port_mask, uint8_t port_cnt,
...
bool CCPortLatency::Create(uint8_t id, uint16_t payload_offset, uint16_t l4_offset, uint16_t pkt_size,
                           CCPortLatency *rx_port, CLatencyPktMode *pkt_mode, CNatRxManager *nat_manager) {
    m_epoc = 0;
    m_pkt_mode = pkt_mode;
    assert(m_pkt_mode);
...
void CCPortLatency::Delete() { m_hist.Delete(); }
...
void CCPortLatency::dump_json(std::string &json) {
    ...
    json += get_field("error", (float)(m_unsup_prot + m_no_magic + m_no_id + m_seq_error + m_length_error +
                                       m_no_ipv4_option + m_l3_cs_err + m_l4_cs_err + m_tx_pkt_err));
...
void CCPortLatency::DumpShort(FILE *fd) {
    ...
    fprintf(fd, "%8lu,%8lu,%10lu,%5lu,", m_tx_pkt_ok, m_pkt_ok, m_rx_check,
            m_unsup_prot + m_no_magic + m_no_id + m_seq_error + m_length_error + m_no_ipv4_option + m_tx_pkt_err +
                m_l3_cs_err + m_l4_cs_err);

    fprintf(fd, "   %8.0f  ,%8.0f,%8d ", m_hist.get_average_latency(), m_hist.get_max_latency(), get_jitter_usec());
    fprintf(fd, "     | ");
...
bool CCPortLatency::do_learn(uint32_t external_ip, uint32_t external_dest_ip) {
    m_nat_learn = true;
    m_nat_can_send = true;
...
bool CCPortLatency::check_packet(rte_mbuf_t *m, CRx_check_header *&rx_p) {
    ...
    bool is_lateancy_pkt =
        c_l_pkt_mode->IsLatencyPkt(parser.m_ipv4) & IsLatencyPkt(parser.m_l4 + c_l_pkt_mode->l4_header_len(), m_epoc);
    ...
    if (!is_lateancy_pkt) {
        ...
        while (opt_len != 0) {
            ...
            case RX_CHECK_V4_OPT_TYPE:
                ...
                if ((!get_is_rx_check_mode()) || (opt_len < RX_CHECK_LEN)) {
                    m_seq_error++;
                    return (false);
                }
                m_rx_check++;
...
bool CLatencyManager::Create(CLatencyManagerCfg *cfg) {
    ...
    for (int i = 0; i < m_max_ports; i++) {
        ...
        lp->m_port.Create(i, m_pkt_gen.get_payload_offset(), m_pkt_gen.get_l4_offset(), m_pkt_gen.get_pkt_size(), lpo,
                          c_l_pkt_mode, &m_nat_check_manager);
    ...
    for (auto &i : m_port_ids) {
        if (m_port_mask & (1 << i)) {
            ...
            if (lp->m_port.can_send_packet(i % 2)) {
                rte_mbuf_t *m = m_pkt_gen.generate_pkt(i, lp->m_port.external_nat_ip(), lp->m_port.external_dest_ip());
...
double CLatencyManager::grat_arp_timeout() { return (double)CGlobalInfo::m_options.m_arp_ref_per / m_arp_info.size(); }

void CLatencyManager::add_grat_arp_src(COneIPv4Info &ip) { m_arp_info.insert(ip); }
...
void CLatencyManager::handle_rx_pkt(CLatencyManagerPerPort *lp, rte_mbuf_t *m) {
...
void CLatencyManager::tickle() { m_monitor.tickle(); }
...
void CLatencyManager::stop() { m_do_stop = true; }

bool CLatencyManager::is_active() { return (m_is_active); }
...
void CLatencyManager::dump_nat_flow_table(FILE *fd) { m_nat_check_manager.Dump(fd); }
...
void CLatencyManager::update_cpu_util() { m_cpu_cp_u.Update(); }

double CLatencyManager::get_cpu_util() { return m_cpu_cp_u.GetVal(); }
...
void CLatencyManager::DumpRxCheckVerification(FILE *fd, uint64_t total_tx_rx_check) {
    if (!get_is_rx_check_mode()) {
...
void CLatencyPktModeSCTP::send_debug_print(uint8_t *pkt) {
    printf("Sending latency SCTP packet\n");
