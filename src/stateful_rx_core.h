...
class CLatencyPktInfo {
  public:
     ...
     * regular set IP
     */
    void set_ip(uint32_t src, uint32_t dst, uint32_t dual_port_mask);

    /**
     * set IP with client clustering configuration
     */
    void set_ip(uint32_t src, uint32_t dst, uint32_t dual_port_mask, uint8_t port_cnt, ClientCfgDB &db);

    rte_mbuf_t *generate_pkt(int port_id, uint32_t extern_ip = 0, uint32_t extern_dest_ip = 0);

    CGenNode *getNode() { return (&m_dummy_node); }

    uint16_t get_payload_offset(void) { return (m_pkt_indication.getFastPayloadOffset()); }

    uint16_t get_l4_offset(void) { return (m_pkt_indication.getFastTcpOffset()); }

    uint16_t get_pkt_size(void) { return (m_packet->pkt_len); }
...
struct latency_header {
    ...
    uint8_t get_id() { return (magic & 0xff); }
};
...
class CCPortLatency {
  public:
    bool Create(uint8_t id, uint16_t offset, uint16_t l4_offset, uint16_t pkt_size, CCPortLatency *rx_port,
                CLatencyPktMode *pkt_mode, CNatRxManager *nat_manager);
    void Delete();
    void reset();
    void reset_seq();
    void set_epoc(uint8_t epoc) { m_epoc = epoc; }

    void set_enable_none_latency_processing(bool enable) { m_handle_none_latency = enable; }
    ...
    uint32_t external_nat_ip() { return (m_nat_external_ip); }

    uint32_t external_dest_ip() { return (m_nat_external_dest_ip); }

    void update_packet(rte_mbuf_t *m, int port_id);
    bool do_learn(uint32_t external_ip, uint32_t external_dest_ip);
    ...
    uint32_t get_jitter_usec(void) { return ((uint32_t)(m_jitter.get_jitter() * 1000000.0)); }
    ...
    bool is_any_err() {
        if ((m_dummy_port_in_pair || (m_tx_pkt_ok == m_rx_port->m_pkt_ok)) &&
            ...
            ((m_unsup_prot + m_no_magic + m_no_id + m_seq_error + m_l3_cs_err + m_l4_cs_err + m_length_error +
              m_no_ipv4_option + m_tx_pkt_err) == 0)) {
            return (false);
        }
        return (true);
    }
...
class CLatencyManager : public TrexRxCore {
  public:
    ...
    void set_ip(uint32_t client_ip, uint32_t server_ip, uint32_t mask_dual_port) {

        m_pkt_gen.set_ip(client_ip, server_ip, mask_dual_port);
    }

    void set_ip(uint32_t client_ip, uint32_t server_ip, uint32_t mask_dual_port, ClientCfgDB &client_cfg_db) {
    ...
    uint16_t get_latency_header_offset() { return (m_pkt_gen.get_payload_offset()); }
    void update();
    ...
    void set_mask(uint32_t mask) { m_port_mask = mask; }
  ...
  private:
    ...
    void handle_pkt_msg(uint8_t thread_id, CGenNodeLatencyPktInfo *msg);
