...
void CSTTCpPerTGIDPerDir::update_counters(bool is_sum, uint16_t tg_id) {
    ...
    if ((lpt_udp->udps_connects + lpt_udp->udps_accepts) > lpt_udp->udps_closed) {
        udp_active_flows = lpt_udp->udps_connects + lpt_udp->udps_accepts - lpt_udp->udps_closed;
    }
    ...
    if (lpt->tcps_connects > lpt->tcps_closed) {
        m_est_flows = lpt->tcps_connects - lpt->tcps_closed;
    } else {
        m_est_flows = 0;
    }
    ...
    uint64_t total_tx_bytes =
        lpt->tcps_sndbyte_ok + lpt->tcps_sndrexmitbyte + lpt->tcps_sndprobe + lpt_udp->udps_sndbyte;
    m_tx_bw_l7_r = m_tx_bw_l7.add(lpt->tcps_rcvackbyte + lpt_udp->udps_sndbyte) *
                   _1Mb_DOUBLE; /* better to add the acked tx bytes than tcps_sndbyte */
...
static void create_sc(CGTblClmCounters *clm, std::string name, std::string help, uint64_t *c, bool keep_zero,
                      bool error, bool is_abs = false) {
    CGSimpleBase *lp;
...
static void create_sc_32(CGTblClmCounters *clm, std::string name, std::string help, uint32_t *c, bool keep_zero,
                         bool error) {
    CGSimpleBase *lp;
...
static void create_sc_d(CGTblClmCounters *clm, std::string name, std::string help, double *c, bool keep_zero,
                        std::string units, bool error, bool is_abs = false) {
    CGSimpleBase *lp;
...
static void create_bar(CGTblClmCounters *clm, std::string name) {
    CGSimpleBase *lp;
    lp = new CGSimpleBar();
    lp->set_name(name);
    lp->set_abs(true);
...
#define TCP_S_ADD_CNT(f, help)                                                                                         \
    { create_sc(&m_clm, #f, help, &m_tcp.m_sts.f, false, false); }
// for errors
#define TCP_S_ADD_CNT_E(f, help)                                                                                       \
    { create_sc(&m_clm, #f, help, &m_tcp.m_sts.f, false, true); }

#define UDP_S_ADD_CNT(f, help)                                                                                         \
    { create_sc(&m_clm, #f, help, &m_udp.m_sts.f, false, false); }
#define UDP_S_ADD_CNT_E(f, help)                                                                                       \
    { create_sc(&m_clm, #f, help, &m_udp.m_sts.f, false, true); }

#define FT_S_ADD_CNT(f, help)                                                                                          \
    { create_sc_32(&m_clm, #f, help, &m_ft.m_sts.m_##f, false, false); }
#define FT_S_ADD_CNT_Ex(s, f, help)                                                                                    \
    { create_sc_32(&m_clm, s, help, &m_ft.m_sts.m_##f, false, false); }
#define FT_S_ADD_CNT_SZ(f, help)                                                                                       \
    { create_sc_32(&m_clm, #f, help, &m_ft.m_sts.m_##f, true, false); }
#define FT_S_ADD_CNT_E(f, help)                                                                                        \
    { create_sc_32(&m_clm, #f, help, &m_ft.m_sts.m_##f, false, true); }
#define FT_S_ADD_CNT_OK(f, help)                                                                                       \
    { create_sc_32(&m_clm, #f, help, &m_ft.m_sts.m_##f, false, false); }

#define FT_S_ADD_CNT_Ex_E(s, f, help)                                                                                  \
    { create_sc_32(&m_clm, s, help, &m_ft.m_sts.m_##f, false, true); }
#define FT_S_ADD_CNT_SZ_E(f, help)                                                                                     \
    { create_sc_32(&m_clm, #f, help, &m_ft.m_sts.m_##f, true, true); }

#define CMN_S_ADD_CNT(f, help, z)                                                                                      \
    { create_sc(&m_clm, #f, help, &f, z, false, true); }
#define CMN_S_ADD_CNT_d(f, help, z, u)                                                                                 \
    { create_sc_d(&m_clm, #f, help, &f, z, u, false, true); }
...
void CSTTCpPerTGIDPerDir::create_clm_counters() {
    ...
    TCP_S_ADD_CNT(tcps_testdrops,
                  "connections dropped by user at timeout (no-close flag --nc)"); // due to test timeout --nc
...
void CSTTCp::Add(tcp_dir_t dir, CTcpPerThreadCtx *ctx) { m_sts[dir].m_tcp_ctx.push_back(ctx); }
...
void CSTTCp::DumpTable() { m_dtbl.dump_table(stdout, false, true); }
...
void CSTTCp::UpdateTGNames(const std::vector<std::string> &tg_names) { m_tg_names = tg_names; }
...
void CSTTCp::DumpTGStats(Json::Value &result, const std::vector<uint16_t> &tg_ids) {
    for (auto &tg_id : tg_ids) {
        ...
        m_dtbl_per_tg_id[tg_id]->dump_values("counters per TGID: " + std::to_string(tg_id), false,
                                             result[std::to_string(tg_id)]);
    }
}
