...
void CRxAstfCore::handle_astf_latency_pkt(const rte_mbuf_t *m, uint8_t port_id) {
    /* nothing to do */
    if (!m_latency_active)
        return;
...
void CRxAstfCore::create_latency_context() {
    ...
    for (auto &iter_pair : m_rx_port_mngr_map) {
        ...
        lp->m_port.Create(port_id, m_pkt_gen.get_payload_offset(), m_pkt_gen.get_l4_offset(), m_pkt_gen.get_pkt_size(),
                          lpo, m_l_pkt_mode, 0);
...
void CRxAstfCore::start_latency(const lat_start_params_t &args) {
    ...
    m_pkt_gen.set_ip(args.client_ip.v4, args.server_ip.v4, args.dual_ip);
...
void CRxAstfCore::stop_latency() { m_latency_active = false; }
...
void CRxAstfCore::handle_rx_pkt(CLatencyManagerPerPort *lp, rte_mbuf_t *m) {
