  ...
  public:
    ...
    T wait_for_reply(int timeout_ms = 500, int backoff_ms = 1) {
        ...
        while (is_pending()) {
            guard -= backoff_ms;
            if (guard < 0) {
                throw TrexException("internal error: failed to get response from remote core for more than '" +
...
class TrexCpToDpMsgBase {
  public:
    ...
    void set_quit(bool enable) { m_quit_scheduler = enable; }

    bool is_quit() { return (m_quit_scheduler); }
...
class TrexDpPortEventMsg : public TrexDpToCpMsgBase {
  public:
    ...
    uint8_t get_profile_id() { return m_profile_id; }
...
class TrexRxCaptureStart : public TrexRxCapture {
  public:
    TrexRxCaptureStart(const CaptureFilter &filter, uint64_t limit, TrexPktBuffer::mode_e mode,
                       MsgReply<TrexCaptureRCStart> &reply)
        : m_reply(reply) {
...
class TrexRxCaptureFetch : public TrexRxCapture {
  public:
    TrexRxCaptureFetch(capture_id_t capture_id, uint32_t pkt_limit, MsgReply<TrexCaptureRCFetch> &reply)
        : m_reply(reply) {
        m_capture_id = capture_id;
...
class TrexRxStartCapwapProxy : public TrexCpToRxMsgBase {
  public:
    TrexRxStartCapwapProxy(uint8_t port_id, uint8_t pair_port_id, bool is_wireless_side, const Json::Value &capwap_map,
                           uint32_t wlc_ip, MsgReply<std::string> &reply)
        : m_reply(reply) {
...
class TrexRxStopCapwapProxy : public TrexCpToRxMsgBase {
  public:
    TrexRxStopCapwapProxy(uint8_t port_id, MsgReply<std::string> &reply) : m_reply(reply) { m_port_id = port_id; }
...
class TrexRxStartQueue : public TrexCpToRxMsgBase {
  public:
    TrexRxStartQueue(uint8_t port_id, uint64_t size, MsgReply<bool> &reply) : m_reply(reply) {
...
class TrexRxStopQueue : public TrexCpToRxMsgBase {
  public:
    TrexRxStopQueue(uint8_t port_id) { m_port_id = port_id; }
...
class TrexRxGetStackCaps : public TrexCpToRxMsgBase {
  public:
    TrexRxGetStackCaps(uint8_t port_id, MsgReply<uint16_t> &reply) : m_reply(reply) { m_port_id = port_id; }
...
class TrexRxSetL3Mode : public TrexCpToRxMsgBase {
  public:
    TrexRxSetL3Mode(uint8_t port_id, const std::string &src_ipv4, const std::string &dst_ipv4,
...
class TrexRxIsRunningTasks : public TrexCpToRxMsgBase {
  public:
    TrexRxIsRunningTasks(uint8_t port_id, MsgReply<bool> &reply) : m_reply(reply) { m_port_id = port_id; }
...
class TrexRxGetTasksResultsEx : public TrexCpToRxMsgBase {
  public:
    TrexRxGetTasksResultsEx(uint8_t port_id, uint64_t ticket_id, stack_result_t &results,
                            MsgReply<TrexStackResultsRC> &reply)
        : m_results(results), m_reply(reply) {
        m_port_id = port_id;
        m_ticket_id = ticket_id;
    }
...
class TrexRxCancelCfgTasks : public TrexCpToRxMsgBase {
  public:
    TrexRxCancelCfgTasks(uint8_t port_id) { m_port_id = port_id; }
    virtual bool handle(CRxCore *rx_core);
...
class TrexRxSetVLAN : public TrexCpToRxMsgBase {
  public:
    TrexRxSetVLAN(uint8_t port_id, const vlan_list_t &vlan_list) : m_vlan_list(vlan_list) { m_port_id = port_id; }
...
class TrexRxInvalidateDstMac : public TrexCpToRxMsgBase {
  public:
    TrexRxInvalidateDstMac(uint8_t port_id) { m_port_id = port_id; }
...
class TrexRxIsDstMacValid : public TrexCpToRxMsgBase {
  public:
    TrexRxIsDstMacValid(uint8_t port_id, MsgReply<bool> &reply) : m_reply(reply) { m_port_id = port_id; }
...
class TrexRxStartCapturePort : public TrexCpToRxMsgBase {
  public:
    TrexRxStartCapturePort(uint8_t port_id, const std::string &filter, const std::string &endpoint, std::string &err,
                           MsgReply<bool> &reply)
