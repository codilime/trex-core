 ...
 * @param force
 */
void TrexPort::acquire(const std::string &user, uint32_t session_id, bool force) {
...
void TrexPort::start_rx_queue(uint64_t size) {
...
void TrexPort::stop_rx_queue() {
    TrexCpToRxMsgBase *msg = new TrexRxStopQueue(m_port_id);
    send_message_to_rx(msg);
}

void TrexPort::start_capwap_proxy(uint8_t pair_port_id, bool is_wireless_side, const Json::Value &capwap_map,
                                  uint32_t wlc_ip) {
    static MsgReply<std::string> reply;
    reply.reset();

    TrexRxStartCapwapProxy *msg =
        new TrexRxStartCapwapProxy(m_port_id, pair_port_id, is_wireless_side, capwap_map, wlc_ip, reply);
...
void TrexPort::stop_capwap_proxy() {
    static MsgReply<std::string> reply;
    reply.reset();
...
const TrexPktBuffer *TrexPort::get_rx_queue_pkts() {
...
bool TrexPort::is_active() const {
    return ((m_port_state == PORT_STATE_TX) || (m_port_state == PORT_STATE_PAUSE) ||
            (m_port_state == PORT_STATE_PCAP_TX));
}

bool TrexPort::verify_state(int state, const char *cmd_name, bool should_throw) const {
...
void TrexPort::change_state(port_state_e new_state) { m_port_state = new_state; }

/**
 * core is considered active if it has a pending for async stop
 *
 */
bool TrexPort::is_core_active(int core_id) {
 ...
 *
 */
std::string TrexPort::get_state_as_string() const {
...
void TrexPort::encode_stats(Json::Value &port) { get_platform_api().port_stats_to_json(port, m_port_id); }

void TrexPort::send_message_to_all_dp(TrexCpToDpMsgBase *msg, bool send_to_active_only) {
...
uint8_t TrexPort::get_active_cores_count(void) {
...
void TrexPort::send_message_to_dp(uint8_t core_id, TrexCpToDpMsgBase *msg) {
...
void TrexPort::send_message_to_rx(TrexCpToRxMsgBase *msg) {
...
uint64_t TrexPort::get_port_speed_bps() const {
...
bool TrexPort::has_fast_stack(void) { return get_stack_caps() & CStackBase::FAST_OPS; }

std::string &TrexPort::get_stack_name(void) { return CGlobalInfo::m_options.m_stack_type; }
 ...
 */
void TrexPort::set_capture_port_bpf_filter(const std::string &filter) {
