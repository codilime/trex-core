...
CRpcDispatch::~CRpcDispatch() { m_map.clear(); }
...
void CRpcTunnelBatch::update_cmd_count(uint32_t total_commands, uint32_t err_commands) {}

trex_rpc_cmd_rc_e CRpcTunnelBatch::_run(const Json::Value &params, Json::Value &result) {
    assert(0);
    return (TREX_RPC_CMD_OK);
}

void CRpcTunnelBatch::register_func(const string &json_name, rpc_method_cb_t cb) {
    m_rpc_dispatch.register_func(json_name, cb);
}

trex_rpc_cmd_rc_e CRpcTunnelBatch::run_batch(const Json::Value &commands, Json::Value &results) {
