...
class ServiceModeWrapper : public CVirtualIF {
  public:
    ServiceModeWrapper() { m_wrapped = nullptr; }

    void set_wrapped_object(CVirtualIF *wrapped) { m_wrapped = wrapped; }

    CVirtualIF *get_wrapped_object() const { return m_wrapped; }

    virtual int close_file(void) { return m_wrapped->close_file(); }

    virtual int flush_tx_queue(void) { return m_wrapped->flush_tx_queue(); }

    virtual int open_file(std::string file_name) { return m_wrapped->open_file(file_name); }

    /* move to service mode */
    virtual int send_node(CGenNode *node) { return m_wrapped->send_node_service_mode(node); }
    ...
    virtual pkt_dir_t port_id_to_dir(uint8_t port_id) { return m_wrapped->port_id_to_dir(port_id); }

    virtual void send_one_pkt(pkt_dir_t dir, rte_mbuf_t *m) { m_wrapped->send_one_pkt(dir, m); }

    virtual void set_review_mode(CPreviewMode *preview_mode) { m_wrapped->set_review_mode(preview_mode); }

    virtual CVirtualIFPerSideStats *get_stats() { return m_wrapped->get_stats(); }
    ...
    bool redirect_to_rx_core(pkt_dir_t dir, rte_mbuf_t *m) { return m_wrapped->redirect_to_rx_core(dir, m); }
...
void CGenNodeStateless::cache_mbuf_array_copy(CGenNodeCacheMbuf *obj, uint16_t size) {
...
void CGenNodeStateless::cache_mbuf_array_set(uint16_t index, rte_mbuf_t *m) {
    CGenNodeCacheMbuf *p = (CGenNodeCacheMbuf *)m_cache_mbuf;
    p->m_array[index] = m;
}
...
rte_mbuf_t *CGenNodeStateless::alloc_flow_stat_mbuf(rte_mbuf_t *m, struct flow_stat_payload_header *&fsp_head,
    ...
    if (is_const) {
        ...
        if (rte_pktmbuf_is_contiguous(m)) {
            ...
            //     Changing to:
            //     (original) r/w -> (new) indirect (direct is original read_only, after trimming last bytes) -> (new)
            //     latency info
            //  In case of field engine with random packet size, we already have r/w->indirect(direct read only with
            //  packet data).
...
HOT_FUNC rte_mbuf_t *CGenNodeStateless::alloc_node_with_vm() {
    ...
    runner.run((uint32_t *)m_vm_flow_var, m_vm_program_size, m_vm_program, m_vm_flow_var, (uint8_t *)p);
...
bool PerPortProfile::resume_traffic(uint8_t port_id) {
    ...
    assert((m_state == PerPortProfile::ppSTATE_TRANSMITTING) || (m_state == PerPortProfile::ppSTATE_PAUSE));
...
bool TrexStatelessDpPerPort::push_pcap(uint8_t port_id, const std::string &pcap_filename, double ipg_usec,
                                       double min_ipg_sec, double speedup, uint32_t count, bool is_dual) {
    ...
    bool rc = pcap_node->create(port_id, dir, socket_id, mac_addr, slave_mac_addr, pcap_filename, ipg_usec, min_ipg_sec,
                                speedup, count, is_dual);
    if (!rc) {
...
bool PerPortProfile::stop_traffic(uint8_t port_id, bool stop_on_id, int event_id) {
...
bool TrexStatelessDpPerPort::stop_traffic(uint8_t port_id, uint32_t profile_id, bool stop_on_id, int event_id) {
...
bool TrexStatelessDpCore::set_stateless_next_node(CGenNodeStateless *cur_node, CGenNodeStateless *next_node) {
 ...
 */
void TrexStatelessDpCore::start_scheduler() {
...
void TrexStatelessDpCore::_rx_handle_packet(int dir, rte_mbuf_t *m, bool is_idle, bool &drop) {
...
void TrexStatelessDpCore::set_need_to_rx(bool enable) { m_need_to_rx = enable; }
...
bool TrexStatelessDpCore::is_hot_state() { return is_feature_set(LATENCY) || is_feature_set(CAPTURE); }
void TrexStatelessDpCore::rx_handle_packet(int dir, rte_mbuf_t *m, bool is_idle, tvpid_t port_id) {
    bool drop;
...
/* add per port exit */
void TrexStatelessDpCore::add_port_duration(double duration, uint8_t port_id, uint32_t profile_id, int event_id) {
    if (duration > 0.0) {
...
void TrexStatelessDpCore::update_mac_addr(TrexStream *stream, CGenNodeStateless *node, pkt_dir_t dir, char *raw_pkt) {
    ...
    /* take from cfg_file */
    if ((ov_src == false) && (ov_dst == TrexStream::stCFG_FILE)) {
...
void TrexStatelessDpCore::replay_vm_into_cache(TrexStream *stream, CGenNodeStateless *node) {
...
void TrexStatelessDpCore::add_stream(PerPortProfile *profile, uint32_t profile_id, TrexStream *stream,
                                     TrexStreamsCompiledObj *comp, double start_at_ts) {
    ...
    if (stream->m_rx_check.m_enabled) {
        ...
        if (hw_id < MAX_FLOW_STATS + MAX_FLOW_STATS_PAYLOAD) {
            ...
            if ((TrexPlatformApi::driver_stat_cap_e)stream->m_rx_check.m_rule_type ==
...
void TrexStatelessDpCore::start_traffic(TrexStreamsCompiledObj *obj, uint32_t profile_id, double duration, int event_id,
                                        double start_at_ts) {
    ...
    double schd_offset = get_dpdk_mode()->dp_rx_queues() ? SCHD_OFFSET_DTIME_RX_ENABLED : SCHD_OFFSET_DTIME;
...
void TrexStatelessDpCore::push_pcap(uint8_t port_id, int event_id, const std::string &pcap_filename, double ipg_usec,
                                    double m_min_ipg_sec, double speedup, uint32_t count, double duration,
                                    bool is_dual) {
    ...
    if (!rc) {
        ...
        TrexDpToCpMsgBase *event_msg = new TrexDpPortEventMsg(m_core->m_thread_id, port_id, 0, event_id, false);
...
void TrexStatelessDpCore::stop_traffic(uint8_t port_id, uint32_t profile_id, bool stop_on_id, int event_id) {
...
void TrexStatelessDpCore::clear_fs_latency_stats(uint8_t dir) { m_ports[dir].m_fs_latency.reset_stats(); }

void TrexStatelessDpCore::clear_fs_latency_stats_partial(uint8_t dir, int min, int max,
 ...
 * PCAP node
 */
bool CGenNodePCAP::create(uint8_t port_id, pkt_dir_t dir, socket_id_t socket_id, const uint8_t *mac_addr,
                          const uint8_t *slave_mac_addr, const std::string &pcap_filename, double ipg_usec,
                          double min_ipg_sec, double speedup, uint32_t count, bool is_dual) {
    std::stringstream ss;
