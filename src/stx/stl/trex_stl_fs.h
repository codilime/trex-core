...
class rfc2544_info_t_ {
  ...
  public:
    rfc2544_info_t_() { clear(); }

    inline void get_latency_json(Json::Value &json) const { json = m_latency; }

    inline void set_latency_json(Json::Value json) { m_latency = json; }
    ...
    inline uint64_t get_seq_err_cnt() { return m_seq_err; }

    inline uint64_t get_ooo_cnt() { return m_out_of_order; }

    inline uint64_t get_dup_cnt() { return m_dup; }

    inline uint64_t get_seq_err_ev_big() { return m_seq_err_ev_big; }

    inline uint64_t get_seq_err_ev_low() { return m_seq_err_ev_low; }

    inline double get_jitter() const { return m_jitter; }

    inline void set_jitter(double jitter) { m_jitter = jitter; }

    uint32_t get_jitter_usec() { return (uint32_t)(m_jitter * 1000000.0); }

    inline void set_last_max(dsec_t val) { m_last_max_latency = val; }

    inline dsec_t get_last_max() { return m_last_max_latency; }

    inline uint32_t get_last_max_usec() { return (uint32_t)(m_last_max_latency * 1000000.0); }
    ...
    inline bool operator!=(const rfc2544_info_t_ &t_in) {
        if ((m_jitter != t_in.m_jitter) || (m_seq_err != t_in.m_seq_err) || (m_out_of_order != t_in.m_out_of_order) ||
            (m_dup != t_in.m_dup) || (m_seq_err_ev_big != t_in.m_seq_err_ev_big) ||
            (m_seq_err_ev_low != t_in.m_seq_err_ev_low))
            return true;
        return false;
    }
    ...
    friend std::ostream &operator<<(std::ostream &os, const rfc2544_info_t_ &t) {
        os << "jitter:" << t.m_jitter << " errors(seq:" << t.m_seq_err << " out of order:" << t.m_out_of_order
           << " dup:" << t.m_dup << ")";
        return os;
    }
...
class tx_per_flow_t_ {
  public:
    ...
    inline uint64_t get_bytes() const { return m_bytes; }
    inline uint64_t get_pkts() const { return m_pkts; }
    inline void set_bytes(uint64_t bytes) {
        m_bytes = bytes;
        ;
    }
    inline void set_pkts(uint64_t pkts) { m_pkts = pkts; }
    inline void add_bytes(uint64_t bytes) {
        m_bytes += bytes;
        ;
    }
    inline void add_pkts(uint64_t pkts) { m_pkts += pkts; }
    inline void clear() {
        m_bytes = 0;
        m_pkts = 0;
    }
...
class tx_per_flow_with_rate_t_ : public tx_per_flow_t_ {
  public:
    tx_per_flow_with_rate_t_() { clear(); }
    void clear() {
        tx_per_flow_t_::clear();
        m_p_rate = 0;
        m_b_rate = 0;
...
class CFlowStatUserIdInfo {
  public:
    ...
    int stop_started_stream() {
        m_trans_ref_count--;
        return m_trans_ref_count;
    }
  ...
  private:
    void update_vals(const rx_per_flow_t val, tx_per_flow_with_rate_t &to_update, bool is_last, hr_time_t time,
                     bool update_rate);
...
class CFlowStatUserIdInfoPayload : public CFlowStatUserIdInfo {
  public:
    CFlowStatUserIdInfoPayload(uint16_t l3_proto, uint8_t l4_proto, uint8_t ipv6_next_h)
        ...
        m_rfc2544_support = true;
        clear();
    }
    virtual void add_stream(uint8_t proto);

    void clear() { m_rfc2544_info.clear(); }
    inline void get_latency_json(Json::Value &json) const { json = m_rfc2544_info.m_latency; }

    inline void set_latency_json(const Json::Value &json) { m_rfc2544_info.m_latency = json; }

    inline double get_jitter() const { return m_rfc2544_info.get_jitter(); }

    inline void set_jitter(double jitter) { m_rfc2544_info.set_jitter(jitter); }

    uint32_t get_jitter_usec() { return m_rfc2544_info.get_jitter_usec(); }

    inline void set_seq_err_cnt(uint64_t cnt) { m_rfc2544_info.m_seq_err = cnt; }

    inline uint64_t get_seq_err_cnt() const { return m_rfc2544_info.m_seq_err; }

    inline void set_ooo_cnt(uint64_t cnt) { m_rfc2544_info.m_out_of_order = cnt; }

    inline uint64_t get_ooo_cnt() const { return m_rfc2544_info.m_out_of_order; }

    inline void set_dup_cnt(uint64_t cnt) { m_rfc2544_info.m_dup = cnt; }

    inline uint64_t get_dup_cnt() const { return m_rfc2544_info.m_dup; }

    inline void set_seq_err_big_cnt(uint64_t cnt) { m_rfc2544_info.m_seq_err_ev_big = cnt; }

    inline uint64_t get_seq_err_big_cnt() const { return m_rfc2544_info.m_seq_err_ev_big; }

    inline void set_seq_err_low_cnt(uint64_t cnt) { m_rfc2544_info.m_seq_err_ev_low = cnt; }

    inline uint64_t get_seq_err_low_cnt() const { return m_rfc2544_info.m_seq_err_ev_low; }
...
class CFlowStatRuleMgr {
  ...
  private:
    ...
    void update_counters(bool update_rate, uint16_t min_f, uint16_t max_f, uint16_t min_l, uint16_t max_l,
                         bool is_last);
