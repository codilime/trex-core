...
class TrexStatelessDpStart : public TrexCpToDpMsgBase {
  public:
    TrexStatelessDpStart(uint8_t port_id, uint32_t profile_id, int event_id, TrexStreamsCompiledObj *obj,
                         double duration, double start_at_ts = 0);
    TrexStatelessDpStart(uint8_t port_id, int event_id, TrexStreamsCompiledObj *obj, double duration,
                         double start_at_ts = 0)
        : TrexStatelessDpStart(port_id, 0, event_id, obj, duration, start_at_ts) {}
...
class TrexStatelessDpPauseStreams : public TrexCpToDpMsgBase {
  public:
    TrexStatelessDpPauseStreams(uint8_t port_id, uint32_t profile_id, stream_ids_t &stream_ids)
        : m_port_id(port_id), m_profile_id(profile_id), m_stream_ids(stream_ids) {}

    TrexStatelessDpPauseStreams(uint8_t port_id, stream_ids_t &stream_ids)
        : TrexStatelessDpPauseStreams(port_id, 0, stream_ids) {}
...
class TrexStatelessDpResumeStreams : public TrexCpToDpMsgBase {
  public:
    TrexStatelessDpResumeStreams(uint8_t port_id, uint32_t profile_id, stream_ids_t &stream_ids)
        : m_port_id(port_id), m_profile_id(profile_id), m_stream_ids(stream_ids) {}

    TrexStatelessDpResumeStreams(uint8_t port_id, stream_ids_t &stream_ids)
        : TrexStatelessDpResumeStreams(port_id, 0, stream_ids) {}
...
class TrexStatelessDpStop : public TrexCpToDpMsgBase {
  public:
    ...
    CFlowGenListPerThread *get_core_ptr() { return (m_core); }

    void set_event_id(int event_id) { m_event_id = event_id; }

    void set_wait_for_event_id(bool wait) { m_stop_only_for_event_id = wait; }

    virtual void on_node_remove();

    bool get_is_stop_by_event_id() { return (m_stop_only_for_event_id); }
...
class TrexStatelessDpUpdateStreams : public TrexCpToDpMsgBase {
  public:
    ...
    TrexStatelessDpUpdateStreams(uint8_t port_id, stream_ipgs_map_t &ipg_per_stream)
        : TrexStatelessDpUpdateStreams(port_id, 0, ipg_per_stream) {}
...
class TrexStatelessDpPushPCAP : public TrexCpToDpMsgBase {
  public:
    TrexStatelessDpPushPCAP(uint8_t port_id, int event_id, const std::string &pcap_filename, double ipg_usec,
                            double min_ipg_sec, double speedup, uint32_t count, double duration, bool is_dual)
