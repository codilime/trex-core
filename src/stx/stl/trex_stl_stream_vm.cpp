...
void StreamVmInstructionFixChecksumIpv4::Dump(FILE *fd) { fprintf(fd, " fix_check_sum , %lu \n", (ulong)m_pkt_offset); }
...
void StreamVmInstructionFlowMan::sanity_check_valid_opt(uint32_t ins_id, StreamVm *lp) {
    uint8_t valid[] = {FLOW_VAR_OP_INC, FLOW_VAR_OP_DEC, FLOW_VAR_OP_RANDOM};
    int i;
    for (i = 0; i < sizeof(valid) / sizeof(valid[0]); i++) {
        if (valid[i] == m_op) {
            return;
...
void StreamVmInstructionFlowRandLimit::Dump(FILE *fd) {
    ...
    fprintf(fd, " (%lu:%lu:%lu) (min:%lu,max:%lu), is_split_needed:%d, has_previous %d ", m_limit, (ulong)m_size_bytes,
            (ulong)m_seed, m_min_value, m_max_value, m_is_split_needed, m_has_previous);
...
void StreamVmInstructionWriteMaskToPkt::Dump(FILE *fd) {
    fprintf(fd, " flow_var:%s, offset:%lu, cast_size:%lu, mask:0x%lx, shift:%ld, add:%ld, is_big:%lu \n",
            m_flow_var_name.c_str(), (ulong)m_pkt_offset, (ulong)m_pkt_cast_size, (ulong)m_mask, (long)m_shift,
            (long)m_add_value, (ulong)(m_is_big_endian ? 1 : 0));
}
...
void StreamVmInstructionFlowMan::Dump(FILE *fd) {
    ...
    fprintf(fd, " (%lu:%lu:%lu:%lu:), is_split_needed:%d, has_previous:%d ", m_init_value, m_min_value, m_max_value,
            m_step, m_is_split_needed, m_has_previous);
    fprintf(fd, "next_var: %s\n", m_next_var_name.c_str());
}

void StreamVmInstructionWriteToPkt::Dump(FILE *fd) {
    fprintf(fd, " write_pkt , %s ,%lu, add, %ld, big, %lu \n", m_flow_var_name.c_str(), (ulong)m_pkt_offset,
            (long)m_add_value, (ulong)(m_is_big_endian ? 1 : 0));
}
...
void StreamVmInstructionFlowClient::Dump(FILE *fd) {
    ...
    // m_port_min,m_port_max,(ulong)m_limit_num_flows,m_flags);
}
...
StreamDPVmInstructions *StreamVm::get_dp_instruction_buffer() { return &m_instructions; }

const std::vector<StreamVmInstruction *> &StreamVm::get_instruction_list() { return m_inst_list; }

void StreamVm::var_clear_table() { m_flow_var_offset.clear(); }
...
void StreamVm::build_flow_var_table() {
    ...
    for (auto inst : m_inst_list) {
        if (inst->get_instruction_type() == StreamVmInstruction::itFLOW_MAN) {
            ...
            if (var_lookup(ins_man->m_var_name, var) == true) {
                std::stringstream ss;
                ss << "instruction id '" << ins_id << "' flow variable name " << ins_man->m_var_name
                   << " already exists";
                err(ss.str());
            } else {
        ...
        if (inst->get_instruction_type() == StreamVmInstruction::itFLOW_CLIENT) {
            ...
            if (var_lookup(ins_man->m_var_name + ".ip", var) == true) {
                std::stringstream ss;
                ss << "instruction id '" << ins_id << "' client variable name " << ins_man->m_var_name
                   << " already exists";
                err(ss.str());
            }
        ...
        if (m_cur_var_offset > StreamVm::svMAX_FLOW_VAR) {
            std::stringstream ss;
            ss << "too many flow variables current size is :" << m_cur_var_offset << " maximum support is "
               << StreamVm::svMAX_FLOW_VAR;
            err(ss.str());
        }
        ins_id++;
    }
    ...
    for (auto inst : m_inst_list) {
        ...
        if (inst->get_instruction_type() == StreamVmInstruction::itPKT_SIZE_CHANGE) {
            ...
            if (var_lookup(lpPkt->m_flow_var_name, var) == false) {
                ...
                std::stringstream ss;
                ss << "instruction id '" << ins_id << "' packet size with no valid flow variable name '"
                   << lpPkt->m_flow_var_name << "'";
                err(ss.str());
            }

            if (var.m_size_bytes != 2) {
                std::stringstream ss;
                ss << "instruction id '" << ins_id
                   << "' packet size change should point to a flow variable with size 2  ";
                err(ss.str());
            }
            ...
            if (var.m_ins.m_ins_flowv->get_instruction_type() != StreamVmInstruction::itFLOW_MAN) {
                std::stringstream ss;
                ss << "instruction id '" << ins_id
                   << "' packet size change should point to a simple flow variable type (Random/Client) types are not "
                      "supported ";
                err(ss.str());
            }
            ...
            if (var.m_ins.m_ins_flowv->m_value_list.empty()) {
                ...
                if (var.m_ins.m_ins_flowv->m_min_value >= var.m_ins.m_ins_flowv->m_max_value) {
                    std::stringstream ss;
                    ss << "instruction id '" << ins_id << "' min packet size " << var.m_ins.m_ins_flowv->m_min_value
                       << " is bigger or eq to max packet size " << var.m_ins.m_ins_flowv->m_max_value;
                    err(ss.str());
                }
                ...
                if (var.m_ins.m_ins_flowv->m_op == StreamVmInstructionFlowMan::FLOW_VAR_OP_RANDOM) {
                    ...
                    m_pkt_len_data.m_expected_pkt_len =
                        var.m_ins.m_ins_flowv->m_min_value + g_fastrand_util.calc_fastrand_avg(range);
                } else {
                    m_pkt_len_data.m_expected_pkt_len =
                        (var.m_ins.m_ins_flowv->m_min_value + var.m_ins.m_ins_flowv->m_max_value) / 2.0;
                }
                ...
                for (int i = 0; i < (int)var.m_ins.m_ins_flowv->m_value_list.size(); i++) {
                    if (var.m_ins.m_ins_flowv->m_value_list[i] > m_pkt_size ||
                        ...
                        std::stringstream ss;
                        ss << "instruction id '" << ins_id << "' packet size in value_list "
                           << var.m_ins.m_ins_flowv->m_value_list[i] << " is bigger than packet size " << m_pkt_size
                           << " or smaller than 60";
                        err(ss.str());
                    }
                ...
                /* expected packet size calculation */
                m_pkt_len_data.m_expected_pkt_len = std::accumulate(var.m_ins.m_ins_flowv->m_value_list.begin(),
                                                                    var.m_ins.m_ins_flowv->m_value_list.end(), 0.0) /
                                                    var.m_ins.m_ins_flowv->m_value_list.size();
            }
        }
    }
    ...
    for (int i = 0; i < m_inst_list.size(); i++) {
        ...
        if (i == m_inst_list.size() - 1) {
            ...
            if (next_inst_type != StreamVmInstruction::itFLOW_MAN &&
                next_inst_type != StreamVmInstruction::itFLOW_RAND_LIMIT) {
...
void StreamVm::clean_max_field_cnt() { m_max_field_update = 0; }
    ...
    for (int i = m_inst_list.size() - 1; i >= 0; i--) {
        ...
        if (ins_type == StreamVmInstruction::itFIX_HW_CS) {
            ...
            if (m_pkt) {
                ...
                if (ipv4->getVersion() == 4) {
                    ...
                    if (is_ip) {
                        ...
                        if (ipv4->getSize() != l3_len) {
                            std::stringstream ss;
                            ss << "instruction id '" << ins_id << "' fix hw command IPv4 header size is not valid  "
                               << ipv4->getSize();
                            err(ss.str());
                        }
                        if (!((ipv4->getNextProtocol() == IPHeader::Protocol::TCP) ||
                            ...
                            std::stringstream ss;
                            ss << "instruction id '" << ins_id << "' fix hw command L4 should be TCP or UDP  "
                               << ipv4->getSize();
                            err(ss.str());
                        }
                    }
                ...
                if (packet_is_ipv4) {
                    ...
                    if (lpFix->m_l4_type == StreamVmInstructionFixHwChecksum::L4_TYPE_TCP) {
                        ...
                        if (lpFix->m_l4_type == StreamVmInstructionFixHwChecksum::L4_TYPE_UDP) {
                            ...
                            std::stringstream ss;
                            ss << "instruction id '" << ins_id
                               << "' fix hw command offsets should be TCP or UDP for IPv6 ";
                            err(ss.str());
                        }
                    }
                }
                ...
                if ((total_l4_offset + l4_header_size) > m_pkt_size) {
                    ...
                    std::stringstream ss;
                    ss << "instruction id '" << ins_id << "' fix hw command offsets  "
                       << (total_l4_offset + l4_header_size) << "  is too high relative to packet size  " << m_pkt_size;
                    err(ss.str());
                }
        ...
        if (ins_type == StreamVmInstruction::itFIX_IPV4_CS) {
            ...
            if ((lpFix->m_pkt_offset + IPV4_HDR_LEN) > m_pkt_size) {
                ...
                std::stringstream ss;
                ss << "instruction id '" << ins_id << "' fix ipv4 command offset  " << lpFix->m_pkt_offset
                   << "  is too high relative to packet size  " << m_pkt_size;
                err(ss.str());
            }
            ...
            if ((lpFix->m_pkt_offset + offset_next_layer) > m_pkt_size) {
                ...
                std::stringstream ss;
                ss << "instruction id '" << ins_id << "' fix ipv4 command offset  " << lpFix->m_pkt_offset << "plus "
                   << offset_next_layer << " is too high relative to packet size  " << m_pkt_size;
                err(ss.str());
            }
        ...
        if (ins_type == StreamVmInstruction::itFLOW_MAN) {
            ...
            if (lpMan->m_size_bytes == 1) {
                ...
                size_t st_size = sizeof(StreamDPOpFlowVar8Step) + sizeof(uint8_t) * lpMan->m_value_list.size();
            ...
            if (lpMan->m_size_bytes == 2) {
                ...
                size_t st_size = sizeof(StreamDPOpFlowVar16Step) + sizeof(uint16_t) * lpMan->m_value_list.size();
            ...
            if (lpMan->m_size_bytes == 4) {
                ...
                size_t st_size = sizeof(StreamDPOpFlowVar32Step) + sizeof(uint32_t) * lpMan->m_value_list.size();
            ...
            if (lpMan->m_size_bytes == 8) {
                ...
                size_t st_size = sizeof(StreamDPOpFlowVar64Step) + sizeof(uint64_t) * lpMan->m_value_list.size();
        ...
        if (ins_type == StreamVmInstruction::itPKT_WR) {
            ...
            if (var_lookup(lpPkt->m_flow_var_name, var) == false) {
                ...
                std::stringstream ss;
                ss << "instruction id '" << ins_id << "' packet write with no valid flow variable name '"
                   << lpPkt->m_flow_var_name << "'";
                err(ss.str());
            }
            ...
            if (lpPkt->m_pkt_offset + var.m_size_bytes > m_pkt_size) {
                std::stringstream ss;
                ss << "instruction id '" << ins_id << "' packet write with packet_offset   "
                   << lpPkt->m_pkt_offset + var.m_size_bytes << "   bigger than packet size   " << m_pkt_size;
                err(ss.str());
            }
        ...
        if (ins_type == StreamVmInstruction::itPKT_WR_MASK) {
            ...
            if (lpPkt->m_pkt_offset + lpPkt->m_pkt_cast_size > m_pkt_size) {
                std::stringstream ss;
                ss << "instruction id '" << ins_id << "' packet write with packet_offset   "
                   << (lpPkt->m_pkt_offset + lpPkt->m_pkt_cast_size) << "   bigger than packet size   " << m_pkt_size;
                err(ss.str());
            }
 ...
 * @param other
 */
void StreamVm::clone(StreamVm &other) const {
...
void StreamVm::compile(uint16_t pkt_len) {
    ...
    if (get_max_packet_update_offset() > svMAX_PACKET_OFFSET_CHANGE) {
        std::stringstream ss;
        ss << "maximum offset is" << get_max_packet_update_offset() << " bigger than maximum "
           << svMAX_PACKET_OFFSET_CHANGE;
        err(ss.str());
    }
 ...
 *
 */
StreamVmInstructionVar *StreamVm::lookup_var_by_name(const std::string &var_name) {
...
void StreamDPVmInstructions::clear() { m_inst_list.clear(); }
...
uint8_t *StreamDPVmInstructions::get_program() { return (&m_inst_list[0]); }

uint32_t StreamDPVmInstructions::get_program_size() { return (m_inst_list.size()); }
...
void StreamDPOpFlowVar8Step::dump(FILE *fd, std::string opt) {
    fprintf(fd, " %10s  op:%lu, of:%lu, (%lu-%lu-%lu), ls:%lu \n", opt.c_str(), (ulong)m_op, (ulong)m_flow_offset,
            (ulong)m_min_val, (ulong)m_max_val, (ulong)m_step, (ulong)m_list_size);
}
...
void StreamDPOpPktWr8::dump(FILE *fd, std::string opt) {
    fprintf(fd, " %10s  op:%lu, flags:%lu, pkt_of:%lu,  f_of:%lu \n", opt.c_str(), (ulong)m_op, (ulong)m_flags,
            (ulong)m_pkt_offset, (ulong)m_offset);
}
...
void StreamDPOpPktWrMask::dump(FILE *fd, std::string opt) {
    fprintf(fd, " %10s  op:%lu, flags:%lu, var_of:%lu , (%ld-%lu-%lu-%lu-%lu) \n", opt.c_str(), (ulong)m_op,
            (ulong)m_flags, (ulong)m_var_offset, (long)m_shift, (ulong)m_pkt_offset, (ulong)m_mask,
            (ulong)m_pkt_cast_size, (ulong)m_flowv_cast_size);
}
...
void StreamDPOpClientsLimit::dump(FILE *fd, std::string opt) {
    fprintf(fd, " %10s  op:%lu, flow_offset: %lu (%x-%x) (%x-%x) flow_limit :%lu flags:%x \n", opt.c_str(), (ulong)m_op,
            (ulong)m_flow_offset, m_min_ip, m_max_ip, m_min_port, m_max_port, (ulong)m_limit_flows, m_flags);
}

void StreamDPOpClientsUnLimit::dump(FILE *fd, std::string opt) {
    fprintf(fd, " %10s  op:%lu, flow_offset: %lu (%x-%x) flags:%x \n", opt.c_str(), (ulong)m_op, (ulong)m_flow_offset,
            m_min_ip, m_max_ip, m_flags);
}
...
void StreamDPOpFlowRandLimit8::dump(FILE *fd, std::string opt) {
    fprintf(fd, " %10s, flow_offset: %lu  limit :%lu seed:%x (%x-%x) \n", opt.c_str(), (ulong)m_flow_offset,
            (ulong)m_limit, m_seed, m_min_val, m_max_val);
}
...
void StreamDPOpPktWrMask::wr(uint8_t *flow_var_base, uint8_t *pkt_base) {
    uint32_t val = 0;
...
void StreamDPVmInstructionsRunner::slow_commands(uint8_t op_code, uint8_t *flow_var, /* flow var */
