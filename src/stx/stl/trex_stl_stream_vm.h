...
inline uint64_t utl_split_limit(uint64_t limit, uint64_t phase, uint64_t step_mul) {
 ...
 */
static inline uint64_t inc_mod(uint64_t a, uint64_t b, uint64_t c, uint64_t step) {
...
static inline uint64_t dec_mod(uint64_t a, uint64_t b, uint64_t c, uint64_t step) {
 ...
 * step - how many to advance / go back
 */
static inline uint64_t inc_mod_list(uint64_t size, uint64_t index, uint64_t step) {
    int next;
    next = (int)(index + step);
    if (next >= size)
        next = next - size;
...
static inline uint64_t dec_mod_list(uint64_t size, uint64_t index, uint64_t step) {
    int next;
    next = (int)(index - step);
    if (next < 0)
        next = size + index - step;
    return next;
}
 ...
 *
 */
static inline uint64_t inc_mod_of(uint64_t a, uint64_t b, uint64_t c, uint64_t step, bool &of) {
    /* check if we have enough left for simple inc */
    uint64_t left = b - c;
    if (step <= left) {
        of = false;
...
static inline uint64_t dec_mod_of(uint64_t a, uint64_t b, uint64_t c, uint64_t step, bool &of) {
    /* check if we have enough left for simple dec */
    uint64_t left = c - a;
    if (step <= left) {
        of = false;
...
inline void fast_srand(uint32_t &g_seed, int seed) { g_seed = seed; }
...
static inline void vm_srand(uint32_t *per_thread_seed, uint64_t seedval) { fast_srand(*per_thread_seed, seedval); }

static inline uint32_t vm_rand16(uint32_t *per_thread_seed) { return (fastrand(*per_thread_seed)); }
...
struct StreamDPOpFlowVar8Step {
  ...
  public:
    ...
    inline uint16_t get_sizeof_list() { return sizeof(uint8_t) * m_list_size; }
...
struct StreamDPOpFlowVar16Step {
  ...
  public:
    ...
    inline uint16_t get_sizeof_list() { return sizeof(uint16_t) * m_list_size; }
...
struct StreamDPOpFlowVar32Step {
  ...
  public:
    ...
    inline uint16_t get_sizeof_list() { return sizeof(uint32_t) * m_list_size; }
...
struct StreamDPOpFlowVar64Step {
  ...
  public:
    ...
    inline uint16_t get_sizeof_list() { return sizeof(uint64_t) * m_list_size; }
...
struct StreamDPOpPktWrBase {
    enum { PKT_WR_IS_BIG = 1 }; /* for flags */
...
struct StreamDPOpPktWrMask {
    ...
    enum { MASK_PKT_WR_IS_BIG = 1 }; /* for flags */
    ...
    int32_t m_add_value;
    bool is_big() {
        return ((m_flags & StreamDPOpPktWrMask::MASK_PKT_WR_IS_BIG) == StreamDPOpPktWrMask::MASK_PKT_WR_IS_BIG ? true
                                                                                                               : false);
    }
  ...
  public:
    ...
    void wr(uint8_t *flow_var_base, uint8_t *pkt_base);
...
struct StreamDPOpClientsUnLimit {
    enum __MIN_PORT { CLIENT_UNLIMITED_MIN_PORT = 1025 };
...
class StreamDPVmInstructionsRunner {
  public:
    ...
    void slow_commands(uint8_t op_code, uint8_t *flow_var, /* flow var */
                       uint8_t *pkt, uint8_t *&p);

    inline void HOT_FUNC run(uint32_t *per_thread_random, uint32_t program_size, uint8_t *program, /* program */
                             uint8_t *flow_var,                                                    /* flow var */
                             uint8_t *pkt);                                                        /* pkt */

    inline uint16_t get_new_pkt_size() { return (m_new_pkt_size); }

    inline void set_mbuf(rte_mbuf_t *mbuf) { m_m = mbuf; }
    inline rte_mbuf_t *get_mbuf() { return (m_m); }
...
inline HOT_FUNC void StreamDPVmInstructionsRunner::run(uint32_t *per_thread_random, uint32_t program_size,
...
class StreamVmInstruction {
  public:
    ...
    virtual bool need_split() const { return false; }
    ...
    /* nothing to init */
    virtual uint8_t set_bss_init_value(uint8_t *p) { return (0); }
};
...
class StreamVmInstructionFixChecksumIpv4 : public StreamVmInstruction {
  public:
    ...
    virtual instruction_type_t get_instruction_type() const { return (StreamVmInstruction::itFIX_IPV4_CS); }

    virtual void Dump(FILE *fd);

    virtual StreamVmInstruction *clone() { return new StreamVmInstructionFixChecksumIpv4(m_pkt_offset); }
...
class StreamVmInstructionFixHwChecksum : public StreamVmInstruction {
  public:
    ...
    StreamVmInstructionFixHwChecksum(uint16_t l2_len, uint16_t l3_len, uint8_t l4_type) {
        m_l2_len = l2_len;
        m_l3_len = l3_len;
        m_l4_type = l4_type;
    }

    virtual instruction_type_t get_instruction_type() const { return (StreamVmInstruction::itFIX_HW_CS); }

    virtual void Dump(FILE *fd);

    virtual StreamVmInstruction *clone() { return new StreamVmInstructionFixHwChecksum(m_l2_len, m_l3_len, m_l4_type); }
...
class StreamVmInstructionFlowMan : public StreamVmInstructionVar {
  ...
  public:
    virtual instruction_type_t get_instruction_type() const { return (StreamVmInstruction::itFLOW_MAN); }
    ...
    virtual uint64_t get_range() const { return (m_max_value - m_min_value + 1); }

    virtual void set_has_previous(bool has_previous) override { m_has_previous = has_previous; }
     ...
     */
    enum flow_var_op_e { FLOW_VAR_OP_INC, FLOW_VAR_OP_DEC, FLOW_VAR_OP_RANDOM };

    StreamVmInstructionFlowMan(const std::string &var_name, uint8_t size, flow_var_op_e op, uint64_t init_value,
                               uint64_t min_value, uint64_t max_value, uint64_t step = 1, bool is_split_needed = true,
                               const std::string &next_var_name = "", bool has_previous = false)
        : StreamVmInstructionVar(var_name), m_next_var_name(next_var_name) {
    ...
    StreamVmInstructionFlowMan(const std::string &var_name, uint8_t size, flow_var_op_e op,
                               std::vector<uint64_t> value_list, uint64_t step = 1, bool is_split_needed = true,
                               const std::string &next_var_name = "", bool has_previous = false)
        : StreamVmInstructionVar(var_name), m_next_var_name(next_var_name) {
    ...
    virtual StreamVmInstruction *clone() {
        if (m_value_list.empty()) {
            return new StreamVmInstructionFlowMan(m_var_name, m_size_bytes, m_op, m_init_value, m_min_value,
                                                  m_max_value, m_step, m_is_split_needed, m_next_var_name,
                                                  m_has_previous);
        } else {
            return new StreamVmInstructionFlowMan(m_var_name, m_size_bytes, m_op, m_value_list, m_step,
                                                  m_is_split_needed, m_next_var_name, m_has_previous);
        }
    }
...
class StreamVmInstructionFlowRandLimit : public StreamVmInstructionVar {
  ...
  public:
    virtual instruction_type_t get_instruction_type() const { return (StreamVmInstruction::itFLOW_RAND_LIMIT); }

    virtual bool need_split() const { return m_is_split_needed; }

    virtual void set_has_previous(bool has_previous) override { m_has_previous = has_previous; }

    StreamVmInstructionFlowRandLimit(const std::string &var_name, uint8_t size, uint64_t limit, uint64_t min_value,
                                     uint64_t max_value, uint64_t seed, bool is_split_needed = true,
                                     const std::string &next_var_name = "", bool has_previous = false)
        : StreamVmInstructionVar(var_name), m_next_var_name(next_var_name) {
    ...
    virtual StreamVmInstruction *clone() {
        return new StreamVmInstructionFlowRandLimit(m_var_name, m_size_bytes, m_limit, m_min_value, m_max_value, m_seed,
                                                    m_is_split_needed, m_next_var_name, m_has_previous);
    }
...
class StreamVmInstructionWriteMaskToPkt : public StreamVmInstruction {
  public:
    StreamVmInstructionWriteMaskToPkt(const std::string &flow_var_name, uint16_t pkt_offset,
                                      uint8_t pkt_cast_size,    /* valid 1,2,4 */
                                      uint32_t mask, int shift, /* positive is shift left, negetive shift right */
                                      int32_t add_value = 0, bool is_big_endian = true)
        : m_flow_var_name(flow_var_name), m_pkt_offset(pkt_offset), m_pkt_cast_size(pkt_cast_size), m_mask(mask),
          m_shift(shift), m_add_value(add_value), m_is_big_endian(is_big_endian) {}

    virtual instruction_type_t get_instruction_type() const { return (StreamVmInstruction::itPKT_WR_MASK); }

    virtual void Dump(FILE *fd);

    virtual StreamVmInstruction *clone() {
        return new StreamVmInstructionWriteMaskToPkt(m_flow_var_name, m_pkt_offset, m_pkt_cast_size, m_mask, m_shift,
                                                     m_add_value, m_is_big_endian);
    }
...
class StreamVmInstructionFlowClient : public StreamVmInstructionVar {
  ...
  public:
    ...
    virtual instruction_type_t get_instruction_type() const { return (StreamVmInstruction::itFLOW_CLIENT); }

    virtual bool need_split() const { return true; }

    StreamVmInstructionFlowClient(const std::string &var_name, uint32_t client_min_value, uint32_t client_max_value,
                                  uint16_t port_min, uint16_t port_max,
                                  uint32_t limit_num_flows, /* zero means don't limit */
                                  uint16_t flags)
        : StreamVmInstructionVar(var_name), m_ip("ip", 4, StreamVmInstructionFlowMan::FLOW_VAR_OP_INC, client_min_value,
                                                 client_min_value, client_max_value, 1),
    ...
    StreamVmInstructionFlowClient(const StreamVmInstructionFlowClient &other)
        : StreamVmInstructionVar(other.m_var_name), m_ip(other.m_ip), m_port(other.m_port) {
    ...
    static uint8_t get_flow_var_size() { return (4 + 2 + 4); }

    uint32_t get_ip_range() const { return m_ip.get_range(); }

    uint16_t get_port_range() const { return m_port.get_range(); }

    virtual uint64_t get_range() const { return get_ip_range(); }
    ...
    virtual StreamVmInstruction *clone() { return new StreamVmInstructionFlowClient(*this); }
  ...
  protected:
    ...
    void peek_next(uint32_t &next_ip, uint16_t &next_port, int skip = 1) { peek(next_ip, next_port, skip, true); }
...
class StreamVmInstructionWriteToPkt : public StreamVmInstruction {
  public:
    StreamVmInstructionWriteToPkt(const std::string &flow_var_name, uint16_t pkt_offset, int32_t add_value = 0,
                                  bool is_big_endian = true)
        :

          m_flow_var_name(flow_var_name), m_pkt_offset(pkt_offset), m_add_value(add_value),
          m_is_big_endian(is_big_endian) {}

    virtual instruction_type_t get_instruction_type() const { return (StreamVmInstruction::itPKT_WR); }

    virtual void Dump(FILE *fd);

    virtual StreamVmInstruction *clone() {
        return new StreamVmInstructionWriteToPkt(m_flow_var_name, m_pkt_offset, m_add_value, m_is_big_endian);
    }
...
class StreamVmInstructionChangePktSize : public StreamVmInstruction {
  public:
    ...
    virtual instruction_type_t get_instruction_type() const { return (StreamVmInstruction::itPKT_SIZE_CHANGE); }

    virtual void Dump(FILE *fd);

    virtual StreamVmInstruction *clone() { return new StreamVmInstructionChangePktSize(m_flow_var_name); }
...
class StreamVmDp {
  public:
    ...
    StreamVmDp(uint8_t *bss, uint16_t bss_size, uint8_t *prog, uint16_t prog_size, uint16_t max_pkt_offset,
               uint16_t prefix_size, bool a_is_pkt_size_var, bool a_is_random_seed) {
    ...
    StreamVmDp *clone() const {
        StreamVmDp *lp = new StreamVmDp(m_bss_ptr, m_bss_size, m_program_ptr, m_program_size, m_max_pkt_offset_change,
                                        m_prefix_size, m_is_pkt_size_var, m_is_random_seed);
        assert(lp);
        return (lp);
    }
    ...
    uint16_t get_bss_size() { return (m_bss_size); }

    uint8_t *get_bss() { return (m_bss_ptr); }

    uint8_t *get_program() { return (m_program_ptr); }

    uint16_t get_program_size() { return (m_program_size); }

    uint16_t get_max_packet_update_offset() { return (m_max_pkt_offset_change); }

    uint16_t get_prefix_size() { return m_prefix_size; }

    void set_prefix_size(uint16_t prefix_size) { m_prefix_size = prefix_size; }

    void set_pkt_size_is_var(bool pkt_size_var) { m_is_pkt_size_var = pkt_size_var; }
    bool is_pkt_size_var() { return (m_is_pkt_size_var); }
    bool is_random_seed() { return (m_is_random_seed); }
...
class TrexStreamPktLenData {
  public:
    TrexStreamPktLenData() { m_expected_pkt_len = 0; }
...
class StreamVm {
  ...
  public:
    ...
    StreamVmDp *generate_dp_object() {
        ...
        StreamVmDp *lp = new StreamVmDp(get_bss_ptr(), get_bss_size(), get_dp_instruction_buffer()->get_program(),
                                        get_dp_instruction_buffer()->get_program_size(), get_max_packet_update_offset(),
                                        get_prefix_size(), is_var_pkt_size(), m_is_random_var);
        assert(lp);
        return (lp);
    }
    ...
    bool is_vm_empty() const { return (m_inst_list.size() == 0); }

    /**
     * return true if the VM is splitable
     * for multicore
     */
    bool need_split() const { return m_is_split_needed; }
    ...
    uint16_t get_bss_size() { return (m_cur_var_offset); }

    uint8_t *get_bss_ptr() { return (m_bss); }

    uint16_t get_max_packet_update_offset() { return (m_max_field_update); }

    uint16_t get_prefix_size() { return m_prefix_size; }

    bool is_var_pkt_size() { return (m_is_change_pkt_size); }
    ...
    void set_pkt(uint8_t *pkt) { m_pkt = pkt; }
