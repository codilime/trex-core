...
class ClientCfgDirBase {
  ...
  public:
    ClientCfgDirBase() { m_bitfield = 0; }

    virtual void dump(FILE *fd) const;

    bool can_be_used() { return !(m_bitfield & CAN_NOT_USE); }

    bool has_src_mac_addr() const { return (m_bitfield & HAS_SRC_MAC); }
    ...
    bool has_dst_mac_addr() const { return (m_bitfield & HAS_DST_MAC); }
    bool has_vlan() const { return (m_bitfield & HAS_VLAN); }
...
class ClientCfgDirExt : public ClientCfgDirBase {
  ...
  public:
    ...
    bool has_next_hop() const { return (m_bitfield & HAS_NEXT_HOP); }

    bool has_src_ip() const { return (m_bitfield & HAS_SRC_IP); }

    bool has_src_ipv6() const { return (m_bitfield & HAS_SRC_IPV6); }
    ...
    virtual MacAddress get_resolved_mac(uint16_t index) const { return m_resolved_macs[index]; }
};
...
class ClientCfgEntry {
  ...
  public:
    ...
    bool contains(uint32_t ip) const { return ((ip >= m_ip_start) && (ip <= m_ip_end)); }
  ...
  private:
    ...
    void set_cfg(const ClientCfgExt &cfg) { m_cfg = cfg; }
};
...
class ClientCfgDB {
  ...
  public:
     ...
     *
     */
    bool is_empty() { return m_groups.size() == 0; }
