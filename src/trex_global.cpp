...
bool CRteMemPool::dump_one(FILE *fd, const char *name, rte_mempool_t *pool) {
    ...
    fprintf(fd, " %-30s  : %u out of %u (%.2f %%) free %s \n", name, rte_mempool_count(pool), pool->size, p,
            (p < 5.0) ? "<-- need to enlarge" : "");
...
void CRteMemPool::dump(FILE *fd) {
    ...
    if (!ok) {
        fprintf(fd,
                "In order to enlarge the amount of allocated mbufs, need to add section like this to config file:\n");
        fprintf(fd, "memory:\n");
...
void CGlobalInfo::init_pools(uint32_t rx_buffers, uint32_t rx_pool, bool is_hugepages) {
    ...
    for (int sock = 0; sock < MAX_SOCKETS_SUPPORTED; sock++) {
        if (lpSocket->is_sockets_enable((socket_id_t)sock)) {
            ...
            for (int j = 0; j < sizeof(pools) / sizeof(pools[0]); j++) {
                *pools[j].pool_p = utl_rte_mempool_create(pools[j].pool_name, lp->m_mbuf[pools[j].pool_type],
    ...
    m_mem_pool[0].m_mbuf_global_nodes = utl_rte_mempool_create_non_pkt(
        "global-nodes", lp->m_mbuf[MBUF_GLOBAL_FLOWS], sizeof(CGenNode), 128, SOCKET_ID_ANY, true, is_hugepages);
...
void CPreviewMode::set_vlan_mode_verify(uint8_t mode) {
    ...
    if ((CGlobalInfo::m_options.preview.get_vlan_mode() != CPreviewMode::VLAN_MODE_NONE) &&
        ...
        fprintf(stderr, "Error: You are not allowed to specify vlan both in platform config file (--cfg) and traffic "
                        "config file (-f)\n");
...
bool CParserOption::is_valid_opt_val(int val, int min, int max, const std::string &opt_name) {
    if (val < min || val > max) {
        std::cerr << "Value " << val << " for option " << opt_name << " is out of range. Should be (" << min << "-"
                  << max << ")." << std::endl;
        return false;
    }
    ...
    if (preview.get_is_client_cfg_enable()) {
        if (preview.get_vlan_mode() == CPreviewMode::VLAN_MODE_LOAD_BALANCE) {
            throw std::runtime_error(
                "--client_cfg_file option can not be combined with specifing VLAN in traffic profile");
        }
