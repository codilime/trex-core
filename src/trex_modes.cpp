...
uint16_t CDpdkMode_ONE_QUE::total_tx_queues() { return (1); }

bool CDpdkMode_ONE_QUE::is_rx_core_read_from_queue() { return (m_mode->is_astf_mode() ? false : true); }

bool CDpdkMode_ONE_QUE::is_dp_latency_tx_queue() { return (false); }

uint16_t CDpdkMode_MULTI_QUE::total_tx_queues() { return (CGlobalInfo::m_options.preview.getCores()); }

uint16_t CDpdkMode_MULTI_QUE::dp_rx_queues() { return (CGlobalInfo::m_options.preview.getCores()); }

bool CDpdkMode_DROP_QUE_FILTER::is_dp_latency_tx_queue() {
    return ((m_mode->get_opt_mode() == OP_MODE_STL) ? true : false);
}

uint16_t CDpdkMode_DROP_QUE_FILTER::total_tx_queues() { return (CGlobalInfo::m_options.preview.getCores() + 2); }

uint16_t CDpdkMode_RSS_DROP_QUE_FILTER::total_tx_queues() { return (CGlobalInfo::m_options.preview.getCores() + 2); }
...
uint16_t CDpdkMode_RSS_DROP_QUE_FILTER::dp_rx_queues() { return (CGlobalInfo::m_options.preview.getCores()); }
...
void CDpdkMode::set_dpdk_mode(trex_dpdk_mode_t dpdk_mode) {
    ...
    create_mode_cb_t cbs[] = {CDpdkMode_ONE_QUE::create, CDpdkMode_MULTI_QUE::create, CDpdkMode_DROP_QUE_FILTER::create,
...
bool CDpdkMode::is_interactive() {
    if ((m_ttm_mode == OP_MODE_STF) || (m_ttm_mode == OP_MODE_ASTF_BATCH)) {
        return (false);
    }
    return (true);
}
...
bool CDpdkMode::is_astf_mode() {
    if ((m_ttm_mode == OP_MODE_ASTF) || (m_ttm_mode == OP_MODE_ASTF_BATCH)) {
        return (true);
    }
    return (false);
}
...
void CDpdkMode::get_dpdk_drv_params(CTrexDpdkParams &p) {
    ...
    if (mode->is_hardware_filter_needed() || mode->is_one_tx_rx_queue()) {
        p.rx_data_q_num = 1;
    } else {
        p.rx_data_q_num = 0;
    }
...
trex_driver_cap_t CDpdkMode::get_cap_for_mode(trex_dpdk_mode_t dpdk_mode, bool one_core) {
    ...
    const trex_driver_cap_t m[tmDPDK_MODES] = {
        tdCAP_ONE_QUE, tdCAP_MULTI_QUE, tdCAP_DROP_QUE_FILTER,
        one_core ? tdCAP_DROP_QUE_FILTER
                 : tdCAP_RSS_DROP_QUE_FILTER}; /* In case of one core, only filter cap is needed*/

    const trex_driver_cap_t sw[tmDPDK_MODES] = {tdCAP_ONE_QUE, tdCAP_MULTI_QUE, tdNONE, tdNONE};
...
std::string get_str_mode(trex_dpdk_mode_t que_mode) {
    ...
    std::string str[] = {"ONE_QUE", "MULTI_QUE", "DROP_QUE_FILTER", "RSS_DROP_QUE_FILTER"};

    return (str[que_mode]);
}

std::string get_str_trex_traffic_mode(trex_traffic_mode_t ttm_mode) {

    std::string str[] = {"INVALID", "STF", "STL", "ASTF", "STF_BATCH", "DUMP_INTERFACES"};
...
int CDpdkMode::choose_mode(uint32_t cap) {
    ...
    for (i = 0; i < modes.size(); i++) {
        ...
        if ((lp->m_one_core == one_core) && (lp->m_mode == m_ttm_mode)) {
