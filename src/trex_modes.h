...
class CTrexDpdkParams {
  ...
  public:
    uint16_t get_total_rx_queues() { return (rx_data_q_num + rx_drop_q_num + rx_dp_q_num); }

    uint32_t get_total_rx_desc() {
        return ((rx_data_q_num * rx_desc_num_data_q) + (rx_dp_q_num * rx_desc_num_dp_q) +
...
typedef enum {
    ddRX_DIST_NONE = 0, /* no need to distribute as there is one rx queue */
...
typedef enum {
    tmONE_QUE = 0, /* driver support only one TX and one RX queue */
...
class CDpdkModeBase {
  ...
  public:
    /* accurate latency **/
    bool is_accurate_latency() { return (is_hardware_filter_needed()); }

    void set_info(CDpdkMode *mode) { m_mode = mode; }
...
class CDpdkMode_ONE_QUE : public CDpdkModeBase {
  ...
  public:
    static CDpdkModeBase *create() { return (new CDpdkMode_ONE_QUE()); }

    virtual bool is_drop_rx_queue_needed() { return (false); }

    virtual bool is_hardware_filter_needed() { return (false); }

    virtual trex_dpdk_rx_distro_mode_t get_rx_distro_mode() { return (ddRX_DIST_NONE); }

    virtual uint8_t get_dp_rx_queues(uint8_t core_index) { return (0); }

    virtual bool is_one_tx_rx_queue() { return (true); }
...
class CDpdkMode_MULTI_QUE : public CDpdkModeBase {
  ...
  public:
    static CDpdkModeBase *create() { return (new CDpdkMode_MULTI_QUE()); }

    virtual bool is_drop_rx_queue_needed() { return (false); }

    virtual bool is_hardware_filter_needed() { return (false); }

    virtual trex_dpdk_rx_distro_mode_t get_rx_distro_mode() { return (ddRX_DIST_BEST_EFFORT); }

    virtual uint8_t get_dp_rx_queues(uint8_t core_index) { return (core_index); }

    virtual bool is_one_tx_rx_queue() { return (false); }

    virtual bool is_rx_core_read_from_queue() { return (false); }

    virtual bool is_dp_latency_tx_queue() { return (false); }
...
class CDpdkMode_DROP_QUE_FILTER : public CDpdkModeBase {
  ...
  public:
    static CDpdkModeBase *create() { return (new CDpdkMode_DROP_QUE_FILTER()); }

    virtual bool is_drop_rx_queue_needed() { return (true); }

    virtual bool is_hardware_filter_needed() { return (true); }
    ...
    virtual bool is_rx_core_read_from_queue() { return (true); }

    virtual bool is_dp_latency_tx_queue();

    virtual uint16_t dp_rx_queues() { return (0); }
    virtual uint16_t total_tx_queues();
};
...
class CDpdkMode_RSS_DROP_QUE_FILTER : public CDpdkModeBase {
  ...
  public:
    static CDpdkModeBase *create() { return (new CDpdkMode_RSS_DROP_QUE_FILTER()); }
...
class CDpdkMode {
  ...
  public:
    ...
    void force_software_mode(bool sw_mode) { m_force_sw_mode = sw_mode; }
    bool get_force_sw_mode() { return (m_force_sw_mode); }

    void set_opt_mode(trex_traffic_mode_t mode) { m_ttm_mode = mode; }
    trex_traffic_mode_t get_opt_mode() { return (m_ttm_mode); }
    bool is_interactive();
    bool is_astf_mode();
