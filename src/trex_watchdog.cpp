...
__attribute__((noinline)) std::string Backtrace(int skip = 1) {
    void *callstack[128];
...
__attribute__((noinline)) static void _callstack_signal_handler(int signr, siginfo_t *info, void *secret) {
    std::stringstream ss;

    double now = now_sec();

    ss << "WATCHDOG: task '" << global_monitor->get_name() << "' has not responded for more than "
       << global_monitor->get_interval(now) << " seconds - timeout is " << global_monitor->get_timeout_sec()
       << " seconds";
...
extern void __assert_fail(const char *__assertion, const char *__file, unsigned int __line,
    ...
    std::string cause = "assert: " + std::string(__file) + ":" + std::to_string(__line) + " " +
                        std::string(__function) + " Assertion '" + std::string(__assertion) + "' failed.";
    ...
    std::string publish_cause = "assert: " + std::string(__file) + ":" + std::to_string(__line) + " Assertion '" +
                                std::string(__assertion) + "' failed.";
