...
static bool _enough_ips(uint32_t total_ip, double active_flows, bool rss_astf_mode) {
    if (rss_astf_mode) {
...
void CServerPool::Create(IP_DIST_t dist_value, uint32_t min_ip, uint32_t max_ip, double active_flows) {
...
void CClientPool::Create(IP_DIST_t dist_value, uint32_t min_ip, uint32_t max_ip, double active_flows,
...
*/
static uint16_t generate_rand_sport(uint32_t client_index, uint16_t threadid, uint16_t rss_thread_id,
                                    uint16_t rss_thread_max, uint8_t reta_mask, bool rss_astf_mode) {
 ...
 * @param is_long_range
 */
void CClientPool::allocate_simple_clients(uint32_t min_ip, uint32_t total_ip, bool is_long_range) {
...
void CClientPool::configure_client(uint32_t indx) {
    ...
    uint16_t port =
        generate_rand_sport(indx, m_thread_id, m_rss_thread_id, m_rss_thread_max, m_reta_mask, m_rss_astf_mode);
 ...
 * @param is_long_range
 */
void CClientPool::allocate_configured_clients(uint32_t min_ip, uint32_t total_ip, bool is_long_range,
...
bool CTupleGeneratorSmart::add_client_pool(IP_DIST_t client_dist, uint32_t min_client, uint32_t max_client,
                                           double active_flows, ClientCfgDB &client_info, uint16_t tcp_aging,
                                           uint16_t udp_aging) {
    ...
    pool->Create(client_dist, min_client, max_client, active_flows, client_info, tcp_aging, udp_aging);
    m_client_pool.push_back(pool);
    return (true);
}

bool CTupleGeneratorSmart::add_server_pool(IP_DIST_t server_dist, uint32_t min_server, uint32_t max_server,
...
bool CTupleGeneratorSmart::Create(uint32_t _id, uint32_t thread_id)
            ...
            if ((ip_start >= pool_port_start) && (ip_start <= pool_port_end)) {
                if ((ip_end >= pool_port_start) && (ip_end <= pool_port_end)) {
                    ...
                    fprintf(stderr, "Error for range %s - %s. Start is inside range %s - %s, but end is outside\n",
                            ip_to_str(ip_start).c_str(), ip_to_str(ip_end).c_str(), ip_to_str(pool_port_start).c_str(),
                            ip_to_str(pool_port_end).c_str());
                    port = UINT8_MAX;
                    return false;
                }
            }
...
#define UTL_YAML_READ(type, field, target)                                                                             \
    if (node.FindValue(#field)) {                                                                                      \
        utl_yaml_read_##type(node, #field, target);                                                                    \
    } else {                                                                                                           \
        printf("generator definition mising " #field "\n");                                                            \
    }

#define UTL_YAML_READ_NO_MSG(type, field, target)                                                                      \
    if (node.FindValue(#field)) {                                                                                      \
        utl_yaml_read_##type(node, #field, target);                                                                    \
    }
...
void split_ips_v2(uint32_t total_threads, uint32_t rss_thread_id, uint32_t rss_max_threads, uint32_t max_dual_ports,
    ...
    portion.m_ip_start =
        poolinfo.get_ip_start() + (rss_thread_id + rss_max_threads * dual_port_id) * chunks + dual_if_mask;
    portion.m_ip_end = portion.m_ip_start + chunks - 1;
}
