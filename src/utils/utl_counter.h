...
typedef enum { scINFO = 0x12, scWARNING = 0x13, scERROR = 0x14 } counter_info_t_;
...
class CGSimpleBase {
  public:
    ...
    void set_abs(bool is_abs) { m_is_abs = is_abs; }
    ...
    virtual std::string get_val_as_str() { return (""); }

    virtual void get_val(Json::Value &obj) {}

    virtual void dump_val(FILE *fd) {}

    void set_info_level(counter_info_t info) { m_info = info; }

    counter_info_t get_info_level() { return (m_info); }
    ...
    virtual void set_help(std::string name) { m_help = name; }

    std::string get_help() { return (m_help); }

    bool is_skip_zero() { return ((get_dump_zero() == false) && is_zero_val()); }
    virtual bool is_zero_val() { return (false); }

    void set_dump_zero(bool zero) { m_dump_zero = zero; }

    bool get_dump_zero() { return (m_dump_zero); }

    void set_units(std::string name) { m_units = name; }

    std::string get_units() { return (m_units); }

    virtual std::string dump_as_json(bool last);

    virtual std::string dump_as_json_desc(bool last) { return (add_json(m_name, m_help, last)); }

    virtual Json::Value get_json_desc(void);

    virtual bool is_real(void) { return true; }
...
class CGSimpleBar : public CGSimpleBase {
  public:
    virtual void dump_val(FILE *fd) { fprintf(fd, " %15s ", "---"); }
    virtual bool is_real(void) { return false; }
};

class CGSimpleRefCnt32 : public CGSimpleBase {
  public:
    CGSimpleRefCnt32(uint32_t *p) { m_p = p; }

    virtual bool is_zero_val() { return (*m_p == 0); }

    virtual void dump_val(FILE *fd) { fprintf(fd, " %15lu ", (ulong)*m_p); }

    virtual std::string dump_as_json(bool last) { return (add_json(m_name, *m_p, last)); }

    virtual std::string get_val_as_str() { return (std::to_string(*m_p)); }

    virtual void get_val(Json::Value &obj) { obj[std::to_string(m_id)] = *m_p; }
...
class CGSimpleRefCnt64 : public CGSimpleBase {
  public:
    CGSimpleRefCnt64(uint64_t *p) { m_p = p; }

    virtual bool is_zero_val() { return (*m_p == 0); }

    virtual void dump_val(FILE *fd) { fprintf(fd, " %15lu ", *m_p); }
...
class CGSimpleRefCntDouble : public CGSimpleBase {
  public:
    CGSimpleRefCntDouble(double *p, std::string units) {
        m_p = p;
        m_units = units;
    }

    virtual bool is_zero_val() { return (*m_p == 0.0); }
...
class CGTblClmCounters {
  public:
    ...
    void set_free_objects_own(bool enable) { m_free_objects = enable; }
    ...
    void set_name(std::string name) { m_name = name; }
    std::string get_name() { return (m_name.c_str()); }
    uint32_t get_size() { return (m_counters.size()); }
    CGSimpleBase *get_cnt(int index) { return (m_counters[index]); }
...
class CTblGCounters {
  public:
    ...
    void dump_as_json(std::string name, std::string &json);
    ...
    void set_epoch(uint64_t &epoch) { m_epoch = epoch; }
