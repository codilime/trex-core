...
class CIpVlan {
  ...
  public:
    ...
    void dump(FILE *fd) const { fprintf(fd, "ip:%s, vlan:%d\n", ip_to_str(m_ip).c_str(), m_vlan); }
...
class COneIPInfo {
  ...
  public:
    virtual ~COneIPInfo() {}
    virtual void get_mac(uint8_t *mac) const { m_mac.copyToArray(mac); }
    virtual void set_mac(uint8_t *mac) { m_mac.set(mac); }
    uint16_t get_vlan() const { return m_vlan; }
    uint16_t get_port() const { return m_port; }
    void set_port(uint8_t port) { m_port = port; }
    virtual void dump(FILE *fd) const { dump(fd, ""); }
  ...
  protected:
    ...
    virtual const void get_ip_str(char str[100]) const { snprintf(str, 4, "Bad"); }
...
class COneIPv4Info : public COneIPInfo {
  ...
  public:
    COneIPv4Info(uint32_t ip, uint16_t vlan, MacAddress mac) : COneIPInfo(vlan, mac) { m_ip = ip; }
    COneIPv4Info(uint32_t ip, uint16_t vlan) : COneIPv4Info(ip, vlan, MacAddress()) {}
    COneIPv4Info(uint32_t ip, uint16_t vlan, MacAddress mac, uint8_t port) : COneIPInfo(vlan, mac, port) { m_ip = ip; }
    ~COneIPv4Info(){};
    uint32_t get_ip() const { return m_ip; }
  ...
  private:
    virtual const void get_ip_str(char str[100]) const { ip_to_str(m_ip, str); };
    uint32_t m_ip;
};
...
class COneIPv6Info : public COneIPInfo {
  ...
  private:
    virtual const void get_ip_str(char str[100]) { ipv6_to_str((ipaddr_t *)m_ip, str); }
    uint16_t m_ip[8];
};
...
class CManyIPInfo {
  public:
    CManyIPInfo() { m_iter_initiated = false; }
