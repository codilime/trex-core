...
*/
struct rte_mbuf *utl_rte_pktmbuf_cpy(char *p, struct rte_mbuf *mi, uint16_t cp_size, uint16_t &off);

/* deep copy mi to a new mbuf allocated from mp pool, free the input mi at the end */
struct rte_mbuf *utl_rte_pktmbuf_deepcopy(struct rte_mempool *mp, struct rte_mbuf *mi);
...
*/
int utl_rte_pktmbuf_deepcmp(struct rte_mbuf *ma, struct rte_mbuf *mb);

void utl_rte_pktmbuf_fill(rte_mbuf_t *m, uint16_t b_size, uint8_t &start_cnt, int to_rand);

void utl_rte_pktmbuf_dump_k12(FILE *fp, rte_mbuf_t *m);

char *utl_rte_pktmbuf_mem_fill(uint32_t size, int to_rand);

/* convert contiguous buffer to chanin of mbuf in the size of pool  */
struct rte_mbuf *utl_rte_pktmbuf_mem_to_pkt(char *buf, uint32_t size, uint16_t mp_blk_size, struct rte_mempool *mp);

__attribute__((noinline)) int _utl_rte_pktmbuf_trim_ex(struct rte_mbuf *m, uint16_t len);

inline int utl_rte_pktmbuf_trim_ex(struct rte_mbuf *m, uint16_t len) {
    if (m->nb_segs == 1) {
