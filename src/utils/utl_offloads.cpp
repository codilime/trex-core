...
bool _check_offloads(uint64_t supported_offloads, uint64_t requested_offloads, const offload_names_t &offload_names,
                     const string &type) {
    bool success = true;
...
void check_offloads(const struct rte_eth_dev_info *dev_info, const struct rte_eth_conf *m_port_conf) {
    ...
    success &= _check_offloads(dev_info->flow_type_rss_offloads, m_port_conf->rx_adv_conf.rss_conf.rss_hf,
                               offload_names, "RSS");
