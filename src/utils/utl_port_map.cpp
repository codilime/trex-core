...
void CPciPortCfgDesc::dump(FILE *fd) { fprintf(fd, " %s:%d \n", (char *)m_pci.c_str(), (int)m_id); }

int norm_pci_str(std::string pci_n, std::string &out, std::string &err) {
#define BUF_MAX 200
    char buf[BUF_MAX];
    const char *p = pci_n.c_str();

    rte_pci_addr addr;
    if (eal_parse_pci_BDF(p, &addr) != 0 && eal_parse_pci_DomBDF(p, &addr) != 0) {
        snprintf(buf, BUF_MAX, "ERROR not valid PCI addr %s", p);
        err = std::string(buf);
        return (0);
    }
    rte_pci_device_name(&addr, buf, BUF_MAX);
    out = std::string(buf);
    return (0);
}
...
int CPciPortCfgDesc::parse(std::string &err) {
    ...
    if (!isdigit(p[1])) {
        err = "ERROR last char should be int, should have the format dd:dd.d/[d] for example 03:00.0/1 you have: " +
              m_name;
        return (-1);
    }
...
int CPciPorts::set_cfg_input(dpdk_input_args_t &vec, std::string &err) {
...
int CPciPorts::get_map_args(dpdk_input_args_t &dpdk_scan, dpdk_map_args_t &port_map, std::string &err) {
    /* remember the scan */
...
int CPciPorts::find(CPciPortCfgDesc *lp, uint8_t &index) {
    int i;
...
int CPciPorts::dump_vec(FILE *fd, std::string vec_name, dpdk_input_args_t &vec) {
...
int CPciPorts::dump_vec_int(FILE *fd, std::string vec_name, dpdk_map_args_t &vec) {
    fprintf(fd, " %20s : [ ", vec_name.c_str());
    int i;
    int len = vec.size();
    for (i = 0; i < len; i++) {
